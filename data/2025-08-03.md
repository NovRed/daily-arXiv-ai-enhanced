...
Motivation: Existing benchmarks focus on problem-solving, not assessing student solutions. This work aims to fill that gap by evaluating VLMs on grading and mistake identification.
...
Conclusion: The benchmark highlights new research opportunities in AI-assisted assessment, with code available on GitHub.
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in urban-scale scene reconstruction, such as appearance inconsistencies and computational efficiency, while maintaining high visual quality.
...
Result: Effective reconstruction of urban scenes, outperforming prior methods in efficiency and quality.

Conclusion: The framework successfully balances speed, robustness, and visual fidelity for urban-scale scene reconstruction.
...
Conclusion: ScanDiff advances gaze prediction research by better capturing the complexity of human visual behavior, with publicly available source code.
...
Result: SRResNet notably improved classification accuracy, offering computational efficiency and recovering diagnostic value in degraded scans.
...
<details>
  <summary>Details</summary>
Motivation: NeRF models are computationally expensive, limiting their use in resource-constrained scenarios. SNNs offer energy efficiency, prompting the need for a hybrid solution.

Method: Proposes PATA, a dynamic time-step training strategy for spike-based NeRF, optimizing rendering quality vs. time step length. Built on Instant-NGP architecture.

Result: PATA reduces inference time steps by 64% and power by 61.55% while preserving rendering fidelity.

Conclusion: PATA effectively balances quality and efficiency, making NeRF models more viable for edge computing.
...
<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles need fast, accurate navigation in complex environments. Current methods lack efficiency and safety, requiring recurrent memory and abrupt control changes.

Method: NovaDrive processes multiple inputs (images, maps, depth, waypoints) in a single branch with a two-stage cross-attention block. It fine-tunes a vision-language backbone and uses a smoothness loss to avoid abrupt changes.
...
Conclusion: NovaDrive enhances safety and efficiency, with potential for broader embodied-AI applications. Its design reduces fuel/battery usage and simplifies updates.
...
Method: MObI uses a diffusion model for multimodal object inpainting in autonomous driving, while AnydoorMed applies a similar approach for medical imaging, focusing on anomaly inpainting.

Result: Both methods achieve high realism and controllability, enabling accurate spatial positioning and semantic consistency in synthetic data generation.

Conclusion: The methods demonstrate the adaptability of foundation models for reference-guided inpainting across diverse modalities, advancing synthetic data generation for safety-critical applications.
...
Method: Uses a lightweight goal-centered cross-attention layer to fuse vision, map, and waypoint data, processed by a fine-tuned LLaMA-3.2 11B model.

Result: Achieves 95% success and 0.80 SPL on MD-NEX benchmark, outperforming PhysNav-DG by 15% with fewer collisions.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods lack scalability for novel classes. Combining LLMs and VLMs offers potential but is underutilized in classification.

Method: Proposes E-FineR, a training-free framework that refines LLM-generated class names for better fine-grained recognition.

Result: Achieves state-of-the-art performance in fine-grained, zero-shot, and few-shot classification without training.
...
<details>
  <summary>Details</summary>
Motivation: Existing OV-3DIS methods use vision-language models (VLMs) but treat concepts separately. The authors aim to unify these complementary ideas for better performance.

Method: A two-stage approach: 1) 3D proposal generation via tracking-based aggregation and iterative refinement, and 2) instance classification using Alpha-CLIP (mask-enhanced CLIP) and a novel SMS score for similarity normalization.

Result: Achieves state-of-the-art performance on ScanNet200 and S3DIS, outperforming even closed-vocabulary methods in AP and AR metrics.

Conclusion: The proposed framework successfully integrates and refines existing concepts, demonstrating superior performance in OV-3DIS.
...
<details>
  <summary>Details</summary>
Motivation: Prior approaches suffer from identity leakage and struggle with subtle/extreme expressions. X-NeMo aims to overcome these challenges.
...
Result: Outperforms state-of-the-art methods, producing expressive animations with high identity resemblance.
...
<details>
  <summary>Details</summary>
Motivation: Prior methods require light calibration or intermediate cues like normal maps, limiting flexibility and accuracy.

Method: Uses neural implicit fields for geometry and reflectance, with shadow-aware volume rendering and two networks: one for signed distance/reflectance latent codes, another for reflectance estimation.

Result: Outperforms normal-guided methods in shape/lighting accuracy, handles view-unaligned images, and works with complex geometry/reflectance.

Conclusion: The method is robust, flexible, and achieves superior results without requiring intermediate steps or calibration.
...
Method: Uses Resnet-18 for classification and a cascade detector for detection, integrated with a MatLab App Designer interface.

Result: Reliable solution for fruit classification and detection, balancing accuracy and efficiency.

Conclusion: The system offers potential applications in agricultural quality control.
...
Method: Sequence-level contrastive learning aligns four modalities (text, audio, video, motion) in a joint embedding space, incorporating audio for the first time.

Result: Achieves 10.16% improvement in R@10 for text-to-motion and 25.43% in R@1 for video-to-motion retrieval on HumanML3D.

Conclusion: The 4-modal framework enhances motion retrieval performance, highlighting the potential of multi-modal approaches.
...
<details>
  <summary>Details</summary>
Motivation: There's a shortage of suitable datasets for segmenting flooded areas in satellite imagery, despite existing benchmarks.

Method: Collected satellite imagery from the 2019 Midwestern USA floods, processed uniformly, and tested state-of-the-art semantic segmentation models. An ablation study on window sizes was also conducted.

Result: Models achieved modest performance, highlighting the need for future improvements in multimodal and temporal learning.
...
<details>
  <summary>Details</summary>
Motivation: To deceive MLLMs with adversarial images while minimizing distortion and resisting defenses like low-pass filtering.

Method: AGD introduces adversarial-guided noise in reverse diffusion, embedding full-spectrum adversarial signals.
...
Conclusion: The method effectively classifies agglomerated instances, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods struggle with multi-scale feature extraction for small damage and high computational demands, limiting real-time deployment.

Method: Introduces BMS-SPPF for better multi-scale feature extraction and hierarchical channel compression to reduce parameters and GFLOPs.

Result: Achieves 67.6% mAP50, outperforming YOLOv8n by 2.11%, with significant improvement in small-target detection (16.8% for D40) and reduced model size (2.0 MB).
...
<details>
  <summary>Details</summary>
Motivation: Existing Semi-FSCIL methods assume unlabeled data is limited to novel classes, which is impractical. The paper aims to align the framework with real-world scenarios.
...
Conclusion: The GSemi-FSCIL framework and ALDC strategy effectively address the challenge of biased distributions in generalized semi-supervised learning.
...
Result: RL-QR improves NDCG@3 by 11% for multi-modal and 9% for lexical retrievers, but not for semantic/hybrid retrievers.
...
<details>
  <summary>Details</summary>
Motivation: Mapping CN pathways is challenging due to their unique anatomical structures and the skull base environment. This work aims to automate and improve the process.

Method: Multi-stage fiber clustering is applied to ~1,000,000 streamlines from 50 HCP subjects, generating a CN atlas. The atlas is validated against expert annotations on multiple datasets.

Result: The atlas successfully identifies 8 fiber bundles for 5 CN pairs, showing high spatial correspondence with manual annotations across datasets.

Conclusion: The CN atlas enhances automated mapping of CN pathways, improving analysis and understanding of brain structures.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenges in object tracking, such as occlusions and appearance variations, by reviewing and comparing existing paradigms.

Method: Qualitative and quantitative analysis of core design principles, innovations, and limitations of each tracking approach, with a novel categorization and unified comparison.

Result: A detailed review highlighting transformer-based trackers' strengths in spatio-temporal modeling and fast-paced advancements.

Conclusion: The study offers insights into the evolution of tracking methods, emphasizing the growing impact of transformer-based approaches.
...
Conclusion: SATL effectively bridges the gap in structure modeling, improving time series forecasting performance.
...
Result: Experiments on COCO and VG-200 show significant performance improvements in limited-label scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion transformers rely on pre-trained VAEs, introducing errors and artifacts. Researchers aim to simplify the process and improve efficiency.

Method: Proposes PixelNerd, a patch-wise decoding model using neural field representation, eliminating the need for VAEs or cascade pipelines.
...
Conclusion: PixelNerd offers an efficient, single-stage alternative to VAE-based methods, delivering high-quality results without complex pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Manual 3D scan interpretation is labor-intensive, and commercial AI tools are costly, limiting adoption in low-resource settings.

Method: SAM2 is used with a single bounding box annotation, propagated across 3D volume using three slice-wise tracking strategies.

Result: Center-outward propagation yields the most accurate segmentations, with SAM2 performing well under minimal supervision.

Conclusion: General-purpose models like SAM2 can enable affordable 3D medical image analysis in resource-constrained settings.
...
<details>
  <summary>Details</summary>
Motivation: Current transformer-based 3D reconstruction methods face scalability issues due to high computational costs from full attention across multiple input views.

Method: iLRM uses iterative refinement with decoupled scene representation, two-stage attention, and high-resolution information injection.

Result: iLRM outperforms existing methods in quality and speed on datasets like RE10K and DL3DV, with superior scalability.

Conclusion: iLRM offers an efficient, scalable solution for high-fidelity 3D reconstruction, leveraging more input views effectively.
...
<details>
  <summary>Details</summary>
Motivation: To unify reconstruction, generation, and editing in CLIP without degrading its comprehension performance, avoiding the need for additional decoders or quantization.
...
Result: Scores 0.87 and 0.53 on GenEval and WISE benchmarks for text-to-image generation, and 3.62 on ImgEdit Benchmark, surpassing state-of-the-art models.

Conclusion: UniLIP successfully expands CLIP's capabilities, maintaining comprehension while excelling in generation and editing tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLM-based retrieval methods suffer from candidate prior bias, favoring candidates with higher priors over relevance.

Method: Proposes BLiM, leveraging bidirectional likelihood estimation, and CPN for score calibration to mitigate bias.

Result: BLiM with CPN outperforms SOTA by 6.4 R@1, reducing bias and enhancing relevance.

Conclusion: BLiM and CPN effectively address prior bias, with CPN showing broader applicability in multi-modal tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current metrics fail to detect critical structural errors in document layout analysis, necessitating a more comprehensive evaluation method.

Method: Proposes LED with eight error types and three tasks: error existence detection, type classification, and element-wise classification. Uses LED-Dataset, a synthetic dataset with injected structural errors.

Result: LED effectively differentiates structural understanding in models, revealing biases and trade-offs unnoticed by traditional metrics.
...
Result: FreeFine achieves superior image fidelity and edit precision, especially for demanding transformations.

Conclusion: The decoupled pipeline with FreeFine is a robust solution for geometric image editing, validated by GeoBench.
...
<details>
  <summary>Details</summary>
Motivation: Addresses challenges in road perception (e.g., complex geometries, occlusions) by leveraging structured priors often overlooked in existing approaches.

Method: Uses instance-aware attention, shape-prior features, a data-driven shape template space, and a diffusion-based framework for prediction.

Result: Significantly improves perception accuracy on large-scale datasets, especially in challenging conditions.

Conclusion: PriorFusion enhances road element predictions, making them more accurate, regular, and coherent.
...
Method: Controlled computer vision experiments using visual cues to analyze model merging in continual learning.

Result: Merging preserves or enhances shared knowledge while degrading unshared task-specific knowledge; incremental training merging outperforms parallel training.

Conclusion: Linear merging is effective for shared knowledge in continual learning, with incremental training being the preferred approach.
...
<details>
  <summary>Details</summary>
Motivation: To understand how transformer-based diffusion models internally encode content and style in artworks without explicit training guidance.

Method: Uses cross-attention heatmaps to attribute image pixels to specific prompt tokens, isolating regions influenced by content or style.
...
Conclusion: The study provides insights into how generative models represent artistic concepts unsupervised, with tools and data shared for further exploration.
...
Method: Ablation study on hyperparameters (learning rate, batch size, etc.) for seven models trained on ImageNet-1K, evaluated for accuracy, speed, and deployment metrics.
...
Conclusion: Provides practical guidance for building efficient real-time image processing models, with RepVGG-A2 as a standout option.
...
<details>
  <summary>Details</summary>
Motivation: Current visual token pruning methods perform poorly in autonomous driving scenarios. Human drivers focus on relevant foreground areas, suggesting that retaining such information is crucial for decision-making.

Method: Proposes FastDriveVLA with ReconPruner, a plug-and-play visual token pruner using MAE-style pixel reconstruction and adversarial foreground-background reconstruction. Uses the nuScenes-FG dataset for training.

Result: Achieves state-of-the-art results on the nuScenes closed-loop planning benchmark across different pruning ratios.

Conclusion: FastDriveVLA effectively reduces computational costs by focusing on foreground information, enhancing autonomous driving performance.
...
Method: Proposes FASTopoWM, a fast-slow framework with latent world models, enabling parallel supervision of queries and state propagation from past observations.
...
Conclusion: FASTopoWM effectively enhances temporal perception and reasoning in autonomous driving, setting a new benchmark for lane topology understanding.
...
<details>
  <summary>Details</summary>
Motivation: Overcome challenges in traffic sign recognition, such as long-tail data distribution and small, multi-scale targets, to improve autonomous driving safety.

Method: Combines open-vocabulary detection (NanoVerse YOLO with RepVL-PAN and SPD-Conv) and cross-modal learning (TSR-MCL) for robust feature extraction and classification.

Result: Achieves 78.4% mAP on TT100K dataset, with 91.8% accuracy and 88.9% recall, outperforming mainstream methods.

Conclusion: The proposed framework effectively mitigates data imbalance and scale variation issues, demonstrating superior performance in complex scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Road surface reconstruction is critical for autonomous driving but is challenged by occlusions, visual clutter, and appearance degradation due to lighting/weather changes.
...
Conclusion: The framework effectively addresses challenges in road surface reconstruction, offering superior performance under dynamic and cluttered urban scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Real-world conditions like low-resolution imagery degrade face detection performance, necessitating a study on resolution's impact.

Method: Evaluated YOLOv11, YOLOv12, and MTCNN across multiple resolutions (160x160 to 640x640) using precision, recall, mAP50, mAP50-95, and inference time metrics.

Result: YOLOv11 performs best in accuracy at higher resolutions, YOLOv12 has better recall, and MTCNN is slower in real-time inference.

Conclusion: The study offers insights for choosing resolution-aware face detection models based on operational needs.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of comprehensive studies on AGTH quality and distortions, the paper aims to provide a large-scale dataset and evaluation framework.

Method: The study uses THQA-10K dataset with 10,457 AGTHs, subjective ratings from volunteers, and proposes an objective method based on first frame, Y-T slice, and tone-lip consistency.

Result: The proposed objective method achieves SOTA performance in AGTH quality assessment, and the dataset exposes distortions and evaluates talker performance.

Conclusion: The work advances AGTH quality assessment with a large dataset and effective objective method, released publicly for further research.
...
<details>
  <summary>Details</summary>
Motivation: To analyze the evolution of key design patterns in computer vision, focusing on advancements in architectures, generative models, and self-supervised learning techniques.

Method: Examines six influential papers: ResNet, ViT, GANs, LDMs, DINO, and MAE, detailing their methodologies and contributions.

Result: ResNet enabled deeper networks, ViT introduced attention-based models, GANs and LDMs advanced generative tasks, and DINO/MAE improved self-supervised learning.
...
<details>
  <summary>Details</summary>
Motivation: Large vision-language models (LVLMs) face limitations due to high computational costs, and existing NLP layer pruning methods are ineffective for them due to modality differences.

Method: The authors propose Short-LVLM, a framework that addresses non-essential vision-language tokens and inter-layer feature gaps to enable effective pruning.
...
Conclusion: The proposed Short-LVLM framework effectively compresses LVLMs while maintaining performance, offering practical advantages.
...
<details>
  <summary>Details</summary>
Motivation: Existing feature matching methods rely on Transformers, which are computationally expensive due to quadratic complexity. Mamba offers linear complexity, motivating a hybrid approach for efficiency and performance.

Method: VMatcher integrates Mamba's Selective State-Space Model (SSM) with Transformer's attention, proposing hierarchical architectures for robust and efficient feature matching.

Result: VMatcher sets new benchmarks efficiently, demonstrating robustness and practicality for real-time applications.

Conclusion: The hybrid Mamba-Transformer approach in VMatcher balances performance and efficiency, making it suitable for real-time feature matching tasks.
...
Method: Proposes a hierarchical emotional understanding chain with expert queries, fused with emotional representations for diffusion-based image generation, incorporating feedback loops and data filtering.
...
<details>
  <summary>Details</summary>
Motivation: Current CLIP-based UDA methods struggle with noisy samples and unstable alignment, especially in multi-source scenarios, leading to suboptimal feature learning.

Method: MP^2A progressively trains on high-confidence target samples first, then gradually includes harder samples to mitigate noise and improve alignment.

Result: MP^2A achieves state-of-the-art performance on ImageCLEF, Office-Home, and DomainNet benchmarks.

Conclusion: The progressive alignment strategy effectively mitigates confirmation bias and enhances domain-invariant feature learning.
...
<details>
  <summary>Details</summary>
Motivation: To address 3DGS's sensitivity to initialization, limited spatial awareness, and weak inter-Gaussian correlations by leveraging NeRF's strengths.
...
Conclusion: NeRF and 3DGS are complementary, suggesting hybrid approaches for efficient 3D scene representation.
...
Method: AGA uses bidirectional grouping with sparse similarity matrices, adaptive threshold gating, and instance-aware alignment loss for fine-grained image-text matching.

Result: AGA outperforms on retrieval and classification tasks in fine-tuning and zero-shot settings.

Conclusion: AGA effectively addresses limitations of current methods, offering improved performance without external negatives.
...
Result: Achieves state-of-the-art performance with up to 18.10% improvement in OOD detection, reducing computational cost.
...
Method: The dual-teacher framework is enhanced by replacing MLP projection and prediction heads with KANs, leveraging spline-based activations for precise nonlinear modeling.
...
Conclusion: KANs effectively disentangle complex style manifolds, outperforming traditional MLP projections in art style classification.
...
<details>
  <summary>Details</summary>
Motivation: The rapid growth of hyperspectral data archives necessitates efficient storage, yet the individual and joint effects of spectral and spatial compression in learning-based HSI compression remain underexplored.

Method: HyCASS comprises six modules (spectral/spatial encoders/decoders, CR adapter encoder/decoder) using convolutional layers and transformer blocks to capture redundancies.

Result: Experiments on two HSI datasets show HyCASS outperforms existing models, providing guidelines for balancing spectral and spatial compression across CRs.

Conclusion: HyCASS effectively addresses adjustable HSI compression, offering insights into balancing spectral and spatial compression for varying CRs and spatial resolutions.
...
<details>
  <summary>Details</summary>
Motivation: To leverage ML for accurate ailment prediction in chest X-rays without explicit programming, addressing complex data relationships.
...
Conclusion: DenseNet-121 outperforms baseline CNN in focusing on essential X-ray features, enhancing prediction accuracy.
...
TL;DR: The paper introduces CST Anti-UAV, a thermal infrared dataset for tracking tiny UAVs in complex scenes, addressing limitations of existing datasets. It evaluates 20 SOT methods, revealing challenges in tracking performance.
...
Method: The authors create CST Anti-UAV, a dataset with 220 video sequences and 240k bounding box annotations, including manual frame-level attributes. They evaluate 20 SOT methods on this dataset.

Result: State-of-the-art methods achieve only 35.92% accuracy on CST Anti-UAV, compared to 67.69% on Anti-UAV410, highlighting the challenge of tracking tiny UAVs in complex environments.

Conclusion: CST Anti-UAV exposes limitations of current benchmarks and advances UAV tracking research. Its release is expected to spur development of more robust SOT methods and anti-UAV systems.
...
TL;DR: 3D-R1 is a foundation model enhancing 3D VLMs' reasoning by using synthetic data (Scene-30K), RLHF training, and dynamic view selection, achieving 10% better performance.


<details>
  <summary>Details</summary>
Motivation: Current 3D VLMs lack robust reasoning due to poor spatial data and static viewpoints.

Method: Constructs Scene-30K dataset, uses RLHF (GRPO) with three rewards, and dynamic view selection.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the short battery life and data sparsity challenges in compact sensor edge devices by integrating AI for real-time decision-making.

Method: Developed a multi-task neural network with restricted parameters, using the Galar dataset and Viterbi decoding for time-series analysis.

Result: Achieved 93.63% accuracy in localization and 87.48% in anomaly detection with only 1 million parameters.

Conclusion: The model outperforms single-task baselines, advancing AI-based approaches for video capsule endoscopy.
...
<details>
  <summary>Details</summary>
Motivation: Handling large and irregular point clouds efficiently is challenging in deep neural networks.

Method: FastPoint predicts distance trends during farthest point sampling to avoid exhaustive pairwise distance computations.

Result: Achieves 2.55x end-to-end speedup on NVIDIA RTX 3090 GPU while maintaining accuracy.
...
TL;DR: Proposes Stable-Sim2Real, a two-stage depth diffusion model for realistic 3D data simulation, improving performance in real-world 3D tasks.


<details>
  <summary>Details</summary>
Motivation: Bridging the gap between synthetic and real-captured 3D data for better real-world visual tasks, addressing limitations of current methods.

Method: Uses a two-stage depth diffusion model: first stage finetunes Stable-Diffusion for residual generation, second stage refines with adjusted diffusion loss and a 3D discriminator.

Result: Enhances performance in real-world 3D tasks and achieves high similarity between simulated and real data.

Conclusion: Stable-Sim2Real effectively improves 3D data simulation, demonstrating practical benefits for real-world applications.
...
TL;DR: A vision-based pipeline using RGB-D sensing and deep learning for non-destructive, real-time mass estimation of strawberries, addressing occlusion and pose challenges.


<details>
  <summary>Details</summary>
Motivation: Accurate mass estimation of strawberries is difficult due to occlusions and pose variations, necessitating a robust solution for automated harvesting and yield monitoring.
...
Result: Achieved mean errors of 8.11% (isolated) and 10.47% (occluded). CycleGAN outperformed LaMa in occlusion recovery (PAR: 0.978 vs. 1.112; IoU: 92.3% vs. 47.7%).

Conclusion: The proposed method effectively addresses occlusion and pose challenges, offering a practical solution for automated strawberry mass estimation.
...
TL;DR: The paper proposes Hy-CycleAlign, a hyperbolic space-based image registration method for cross-modal alignment, outperforming existing Euclidean space methods.
...
Method: Hy-CycleAlign uses a dual-path cyclic framework and a Hyperbolic Hierarchy Contrastive Alignment (H$^{2}$CA) module to align images in hyperbolic space.

Result: The method significantly improves alignment and fusion quality on multi-modal images.

Conclusion: Hyperbolic space enables more effective cross-modal image registration than Euclidean space.
...
TL;DR: The paper explores how minimal sufficient pixel sets can reveal differences in how image classification models focus on images, showing variations across architectures and misclassifications.


<details>
  <summary>Details</summary>
Motivation: To understand the decision-making process of vision models, as statistical accuracy alone doesn't explain how models work.

Method: Proposes using minimal sufficient pixel sets to analyze model 'concentration'—key pixels for classification—comparing their size, position, and overlap across architectures.

Result: Found that architectures like ConvNext and EVA differ in concentration; misclassified images require larger pixel sets than correct ones.

Conclusion: Minimal sufficient pixel sets provide insights into model behavior, highlighting architectural differences and misclassification patterns.
...
TL;DR: ART introduces an Adaptive Relation Tuning framework using instruction tuning to improve VRD by focusing on informative relations and maintaining generalizability.


<details>
  <summary>Details</summary>
Motivation: VRD models struggle with generalization and novel relations; instruction tuning offers a better solution than handcrafted prompts.

Method: Converts VRD datasets into instruction tuning format, uses adaptive sampling, and focuses on relation classification with given subject-object boxes.

Result: Strong improvement over baselines, ability to infer unseen relations, and practical application in scene segmentation.

Conclusion: ART effectively adapts VLMs for VRD, outperforming existing methods and enabling new capabilities like handling unseen relations.
...
Method: Lifts open-set 2D detection to 3D using a 3D bounding box head, joint training for 2D/3D tasks, geometry-conditioned queries, and canonical image space for cross-dataset training.

Result: Achieves state-of-the-art results on Omni3D (closed-set) and open-set benchmarks (Omni3D to Argoverse 2, ScanNet).

Conclusion: 3D-MOOD effectively addresses open-set challenges, generalizing across diverse scenes and improving performance through novel design choices.
...
TL;DR: The paper introduces Gaussian Splatting Feature Fields (GSFFs) for accurate and privacy-preserving visual localization, combining 3DGS geometry with implicit feature fields. It achieves state-of-the-art performance on real-world datasets.


<details>
  <summary>Details</summary>
Motivation: To improve visual localization accuracy while preserving privacy by leveraging dense geometric information and differentiable rasterization from 3DGS.

Method: Proposes GSFFs, aligning 3D scale-aware feature fields and 2D feature encoders in a contrastive framework, with 3D structure-informed clustering for regularization and segmentation. Pose refinement aligns query features or segmentations with rendered GSFFs.
...
Conclusion: GSFFs effectively combine explicit geometry and implicit features for robust, privacy-aware visual localization.
...
TL;DR: The paper introduces BeyondGloss, a gloss-free SLT framework using VideoLLMs to bridge visual-linguistic gaps by generating fine-grained hand motion descriptions and aligning them with video features. It achieves state-of-the-art results on benchmarks.


<details>
  <summary>Details</summary>
Motivation: SLT is challenging due to the modality gap between visual and linguistic information and the need to capture subtle hand variations.

Method: Proposes BeyondGloss, leveraging VideoLLMs for spatio-temporal reasoning, generating detailed hand motion descriptions, and using contrastive alignment and loss to enhance hand-centric dynamics and reduce modality gaps.

Result: Achieves state-of-the-art performance on Phoenix14T and CSL-Daily benchmarks.

Conclusion: BeyondGloss effectively addresses SLT challenges, demonstrating superior performance and potential for future applications.
...
TL;DR: MamV2XCalib is a V2X-based infrastructure camera calibration method using vehicle-side LiDAR, eliminating manual intervention and reference objects. It introduces a targetless LiDAR-camera calibration technique and achieves robust performance in V2X scenarios.


<details>
  <summary>Details</summary>
Motivation: Traditional manual calibration methods for roadside cameras are inefficient and disruptive. A scalable, automated solution leveraging V2X technology is needed.

Method: Uses vehicle-side LiDAR data and roadside images, combining multi-scale features and a 4D correlation volume. Temporal modeling and Mamba address data defects and viewpoint differences.

Result: Outperforms previous LiDAR-camera methods in V2X scenarios, achieving stable calibration with fewer parameters. Validated on V2X-Seq and TUMTraf-V2X datasets.

Conclusion: MamV2XCalib provides an effective, automated solution for large-scale infrastructure camera calibration, enhancing V2X system scalability.
...
<details>
  <summary>Details</summary>
Motivation: The challenge is inferring unseen appearance and geometric details from a single view while maintaining 3D consistency, as previous methods using 2D diffusion models produce sparse, inconsistent views.
...
Conclusion: MoGA effectively combines generative and diffusion models for high-fidelity 3D avatar reconstruction from single-view images.
...
Method: Slices 3D voxel features to retain vertical geometry, uses directional attention for efficient feature extraction, and compensates for BEV height loss.

Result: Achieves 39.3% mIoU and 27.7 FPS on Occ3D-nuScenes, with 14.8 FPS on edge devices, showing real-time applicability.

Conclusion: The proposed method effectively balances accuracy and speed, making it suitable for real-time autonomous driving systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing VCOD methods rely on spatial appearance features, which are limited due to high similarity between foreground and background. Frequency features and the Mamba model offer potential improvements.

Method: Vcamba combines spatial and frequency features using modules like RFVSS (spatial), AFE (frequency), SLMP (spatial motion), FLMP (frequency motion), and SFMF (fusion).

Result: Vcamba outperforms state-of-the-art methods on 6 metrics across 2 datasets with lower computation cost.

Conclusion: The integration of spatial and frequency features in Vcamba enhances VCOD performance, demonstrating its superiority.
...
TL;DR: The MIDI-B Challenge standardized benchmarking for DICOM image de-identification tools, achieving high accuracy (97.91%-99.93%) with diverse methods.
...
Result: Ten teams achieved high accuracy (97.91%-99.93%) using varied tools like OCR and language models.

Conclusion: MIDI-B successfully benchmarked deID tools, highlighting effective methods and lessons for future research.
...
TL;DR: Incorporating a consistency heuristic into point-matching improves robustness in medical image matching, outperforming state-of-the-art methods and enabling efficient landmark localization.
...
Method: Extends the point-matching algorithm with a consistency heuristic, validated on CT and MRI datasets, including the Deep Lesion Tracking dataset.
...
Conclusion: The method provides high-precision navigation between medical images, offering a practical alternative to machine learning-based approaches.
...
TL;DR: DivControl is a decomposable pretraining framework for unified controllable image generation, enabling efficient adaptation and zero-shot generalization to novel conditions with reduced training costs.
...
Method: DivControl factorizes ControlNet via SVD into disentangled components (learngenes and tailors), uses a dynamic gate for knowledge diversion, and introduces a representation alignment loss for improved fidelity.
...
<details>
  <summary>Details</summary>
Motivation: Current SOTA struggles with small objects and inefficiently uses costly annotations.
...
Conclusion: EMAT is efficient and effective, with new evaluation settings for practical use.
...
TL;DR: A Forward-Forward (FF) based gradient approximation-free training framework for Spiking Neural Networks (SNNs) is proposed, eliminating gradient approximation and reducing computational complexity. It includes a class-aware complexity adaptation mechanism for efficient resource allocation, achieving high test accuracies on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Training SNNs is challenging due to non-differentiability and computational inefficiency of gradient approximation methods. Existing approaches sacrifice accuracy and struggle with deployment on edge devices.

Method: Proposes a Forward-Forward (FF) based training framework that treats spiking activations as black-box modules, avoiding gradient approximation. Introduces a class-aware complexity adaptation mechanism for dynamic loss optimization.
...
Conclusion: The proposed framework offers an efficient, gradient-free training method for SNNs, enhancing accuracy and computational efficiency, making it suitable for edge devices.
...
TL;DR: A framework called Adaptively Distilled ControlNet improves medical image segmentation by using dual-model distillation for better lesion-mask alignment and privacy-preserving generation.
...
Method: The framework uses a teacher model (conditioned on mask-image pairs) to regularize a mask-only student model via noise alignment and adaptive regularization. Only the student model is used during sampling.
...
Conclusion: The proposed framework effectively enhances segmentation performance and privacy in medical imaging.
...
Method: The authors propose OmniTraj, a Transformer-based model pre-trained on heterogeneous data, explicitly conditioned on temporal metadata like frame rates.
...
Conclusion: Explicit temporal conditioning is key for robust zero-shot transfer, and OmniTraj demonstrates its effectiveness across diverse datasets.
...
<details>
  <summary>Details</summary>
Motivation: Addressing challenges in hyperspectral medical imaging like data limitations and hardware variations by leveraging user interaction and spectral fusion.

Method: Combines RGB foundation models with spectral analysis, using user clicks to guide segmentation and spectral similarity. Features a spectral fusion strategy independent of band count/resolution.

Result: Achieved 81.0%-93.4% DICE scores with 1-5 clicks on neurosurgical and porcine datasets, excelling in few-shot/zero-shot learning.

Conclusion: SAMSA offers a flexible, efficient solution for hyperspectral medical imaging, adaptable to diverse spectral datasets.
...
TL;DR: I2V-GS synthesizes autonomous driving data from infrastructure views using Gaussian Splatting, adaptive depth warp, and diffusion models, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current driving data collection is costly and inefficient; synthesizing data from real-world images offers a viable solution.

Method: Uses Gaussian Splatting for view transformation, adaptive depth warp for dense training views, and diffusion models with cross-view optimization for reliability. Introduces RoadSight dataset.

Result: I2V-GS outperforms StreetGaussian by 45.7%, 34.2%, and 14.9% in NTA-Iou, NTL-Iou, and FID metrics.

Conclusion: I2V-GS effectively generates high-quality autonomous driving datasets from infrastructure views, addressing data scarcity.
...
TL;DR: FlowGaussian-VR improves 3D video reconstruction by using a velocity field and adaptive densification, addressing issues in complex motion and scale variations.


<details>
  <summary>Details</summary>
Motivation: Current deformation networks overfit in complex motion and scale variations, and static densification fails for dynamic content.

Method: Proposes FlowGaussian-VR with velocity field rendering (VFR) and flow-assisted adaptive densification (FAD).

Result: Achieves over 2.5 dB PSNR gain, reduces blur, and regularizes Gaussian trajectories.
...
Conclusion: The proposed method improves explainability and provides risk estimation for pixel attribution, aiding safer tumor resections.
...
TL;DR: The paper proposes a data-driven method for deep functional maps, replacing axiomatic regularization with a learned generative model for improved accuracy and generality in non-rigid shape matching.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on axiomatic models for regularization, limiting accuracy and applicability. This work aims to replace these with data-driven approaches.
...
Result: The learned model outperforms axiomatic approaches in zero-shot non-rigid shape matching.

Conclusion: Data-driven regularization can fully replace axiomatic strategies, offering better results and category-agnostic applicability.
...
TL;DR: The paper introduces RAGNet, a large-scale affordance segmentation benchmark, and AffordanceNet, a framework for open-world robotic grasping. It addresses the lack of reasoning-based data and demonstrates strong generalization.
...
Method: Built RAGNet with 273k images, 180 categories, and 26k reasoning instructions. Proposed AffordanceNet, combining a VLM and grasping network.

Result: AffordanceNet shows powerful open-world generalization in benchmarks and real-robot tasks.

Conclusion: The work advances affordance-based grasping with scalable data and a robust framework.
...
TL;DR: DIAS improves Object-Centric Learning (OCL) by reducing slot redundancy and enabling self-distillation, achieving state-of-the-art performance in tasks like object discovery and recognition.


<details>
  <summary>Details</summary>
Motivation: Existing OCL methods suffer from redundant slots competing with informative ones and lack supervision based on internal information, leading to poor object segmentation.

Method: DIAS introduces slot re-initialization to reduce redundancy and self-distillation by aligning attention maps from initial and final iterations.

Result: DIAS outperforms existing methods in object discovery, recognition, visual prediction, and reasoning.

Conclusion: DIAS addresses key limitations in OCL, offering improved performance and versatility for visual tasks.
...
TL;DR: The paper introduces SeqAffordSplat, a benchmark for sequential 3D affordance reasoning, and SeqSplatNet, an end-to-end framework for mapping instructions to 3D affordance masks using LLMs and geometric pre-training.


<details>
  <summary>Details</summary>
Motivation: Current 3DGS methods are limited to single-object, single-step interactions, failing to address long-horizon, multi-object tasks in real-world applications.

Method: Proposes SeqSplatNet, which uses a large language model to autoregressively generate text with segmentation tokens, guiding a conditional decoder for 3D mask production. Includes geometric pre-training and 2D VFM feature fusion.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D human models (e.g., SMPL-X) lack physical interaction capabilities, leading to problems like interpenetration and unrealistic dynamics.

Method: Proposes a 'half-physics' mechanism to transform kinematic motion into physics simulation, maintaining kinematic control while enabling plausible interactions.

Result: The method avoids penetration and unrealistic dynamics, operates in real time, and generalizes to any body shape/motion without training.

Conclusion: The approach successfully integrates physical interactions into kinematic models, preserving motion fidelity and eliminating prior limitations.
...
Conclusion: The study clarifies grounding model construction and benefits other perception tasks, with Phi-Ground demonstrating significant improvements.
...
TL;DR: The paper proposes a method for dynamic scene reconstruction from sparse-view videos, addressing limitations of dense multi-view setups by aligning monocular reconstructions for consistency.


<details>
  <summary>Details</summary>
Motivation: Dense multi-view setups are expensive and impractical for diverse scenes. The goal is to reconstruct dynamic human behaviors from sparse-view cameras.
...
Result: Outperforms prior methods, especially in novel view rendering, as shown on PanopticStudio and Ego-Exo4D datasets.

Conclusion: The method effectively reconstructs dynamic scenes from sparse views, offering a practical alternative to dense setups.
...
TL;DR: CBMs struggle with concept reliability under distribution shifts. SUB, a new benchmark with synthetic images, is introduced to evaluate CBMs, using a novel Tied Diffusion Guidance method for precise control.


<details>
  <summary>Details</summary>
Motivation: To address the unreliability of CBMs in identifying correct concepts under distribution shifts, ensuring robustness in interpretable models.

Method: Introduces SUB, a synthetic image benchmark based on CUB, using Tied Diffusion Guidance (TDG) for precise image generation.

Result: SUB enables rigorous evaluation of CBMs, highlighting their limitations and aiding in developing more robust methods.

Conclusion: SUB and TDG provide tools to improve the reliability of CBMs, advancing interpretable AI in critical fields like medicine.
...
<details>
  <summary>Details</summary>
Motivation: Overcoming challenges in 4D diffusion modeling due to costly data construction and high-dimensional representations of 3D shape, appearance, and motion.

Method: Introduces a Direct 4DMesh-to-GS Variation Field VAE to encode Gaussian Splats and their variations, compressing animations into a latent space. Uses a temporal-aware Diffusion Transformer conditioned on input videos.
...
Conclusion: Paves the way for high-quality animated 3D content generation from single video inputs.
...
Method: Introduces a general framework using multi-objective optimization to balance explanation effectiveness and conciseness, unifying existing methods. Also suggests improved evaluation protocols.

Result: Empirically supports the framework and refined evaluation metrics like Mean Reciprocal Rank and Hits@k.
...
TL;DR: The paper explores Data Readiness for AI (DRAI) principles in leadership-scale scientific datasets for foundation models, proposing a two-dimensional readiness framework for HPC environments.


<details>
  <summary>Details</summary>
Motivation: To address challenges in preprocessing scientific data for scalable AI training, particularly for transformer-based generative models.
...
Result: Identifies common preprocessing patterns and domain-specific constraints, forming a maturity matrix for scientific data readiness.

Conclusion: The framework guides infrastructure development for standardized, cross-domain support of scalable and reproducible AI in science.
...
<details>
  <summary>Details</summary>
Motivation: To address the trade-off between improving logical accuracy and reducing social biases in MLLMs.

Method: Benchmarks three bias-mitigation strategies (SFT, KD, RL) and tests varying proportions of debias and reasoning samples.

Result: A 1:4 mix with RL reduces biases by 10% while retaining 88% of reasoning accuracy.

Conclusion: The study provides actionable insights for balancing fairness and capability in MLLMs.
...
<details>
  <summary>Details</summary>
Motivation: Inspired by Moravec's paradox, the research aims to quantify the gap between human and machine auditory capabilities and identify AI shortcomings.
...
Result: AI models failed catastrophically, with the best achieving only 6.9% accuracy compared to humans' 52%.
...
TL;DR: The paper introduces 'argumentative coherence' in judgmental forecasting, showing its practical value in improving accuracy for both human and LLM-based forecasts, despite user misalignment with the concept.


<details>
  <summary>Details</summary>
Motivation: To study the properties of forecasts from an argumentative perspective and advocate for the property of argumentative coherence.

Method: Formally define argumentative coherence and evaluate its impact on human and LLM-based forecasters, along with crowd-sourced user experiments.
...
Method: The study leverages database setting results and explores the complexity of WSMS computation for various query classes and ontology languages, including first-order-rewritable queries and DL-Lite dialects.

Result: WSMS computation has polynomial data complexity for first-order-rewritable queries but is intractable for ontology languages encoding reachability. Tractable cases are identified for DL-Lite dialects with restricted conjunctive queries.

Conclusion: The work delineates the tractability frontier for WSMS computation, providing insights into feasible and intractable scenarios in ontology-mediated query answering.
...
TL;DR: The paper introduces a divide-and-conquer approach using partial MILP calls and a novel ReLU scoring method (SAS) to improve efficiency in verifying neural networks.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency of previous methods in selecting important ReLU variables for neural network verification.

Method: Proposes SAS for ReLU scoring and adapts BaB-SR/BaB-FSB as global scoring functions, combining them with Hybrid MILP for verification.

Result: SAS reduces binary variables by 6x while maintaining accuracy, and Hybrid MILP cuts undecided instances by 40% with reasonable runtime.

Conclusion: The approach significantly improves efficiency and accuracy in neural network verification, even for large models.
...
Result: Identifies limitations and gaps in current AI Scientist systems, outlining the path toward achieving human-level scientific discovery.

Conclusion: The survey aims to clarify the state of AI Scientists, their limitations, and the ultimate goals for advancing scientific AI.
...
TL;DR: The paper argues against fully autonomous AI (level 3) due to risks, advocating for human oversight, and provides arguments, counterarguments, and evidence.


<details>
  <summary>Details</summary>
Motivation: To highlight the dangers of fully autonomous AI and emphasize the need for human oversight, especially with the potential rise of artificial superintelligence (ASI).

Method: The paper discusses theories of autonomy, AI, and agents, presents 12 arguments and 6 counterarguments with rebuttals, and includes 15 pieces of evidence on AI risks.

Result: The paper concludes that fully autonomous AI is too risky and human oversight is essential to mitigate potential dangers.

Conclusion: AI should not be fully autonomous; responsible human oversight is crucial to prevent risks, especially as ASI becomes a near-future possibility.
...
TL;DR: The paper introduces a benchmark for evaluating data science agents powered by LLMs, assessing models like Claude-4.0-Sonnet, Gemini-2.5-Flash, and OpenAI-o4-Mini across various tasks and prompting methods. It highlights performance disparities and practical deployment factors.


<details>
  <summary>Details</summary>
Motivation: Systematic benchmarks for data science agents are lacking despite their rapid adoption, prompting the need for a real-world evaluation framework.

Method: The study evaluates three LLMs using three approaches (zero-shot, multi-step, and SmolAgent) across eight data science task categories, testing sensitivity to prompting issues and temperature parameters.

Result: Performance disparities among models and methodologies are revealed, with insights into factors affecting deployment.

Conclusion: The benchmark provides a foundation for future research on robust data science agents.
...
<details>
  <summary>Details</summary>
Motivation: To meet the demand for individualized railway services by leveraging LLMs for personalized and accurate service recommendations.

Method: Proposes the QTAO prompting framework for verbal reasoning and task-oriented actions, and introduces the CRFD-25 dataset with an LLM-based zero-shot recommender for railway catering.

Result: LLM4Rail provides accurate and personalized railway services, including food recommendations aligned with the CRFD-25 dataset.

Conclusion: LLM4Rail effectively integrates LLMs with railway services, offering tailored solutions through innovative frameworks and datasets.
...
TL;DR: A dual-agent LLM architecture for translating natural language queries into SQL for ERP systems, improving reliability.
...
Result: The agent effectively translates queries into executable SQL using open-weight LLMs.

Conclusion: The dual-agent approach improves reliability in query generation for ERP systems.
...
TL;DR: Self-Foveate is an LLM-driven method for synthesizing diverse and challenging instructions from unsupervised text, improving upon conventional human-reliant or limited automated approaches.


<details>
  <summary>Details</summary>
Motivation: Current methods for training instruction-following LLMs rely heavily on human annotation or lack diversity and difficulty in synthesized instructions.

Method: Self-Foveate uses a 'Micro-Scatter-Macro' multi-level foveation approach to guide LLMs in extracting fine-grained information from unsupervised text.

Result: Experiments show Self-Foveate enhances instruction diversity and difficulty, outperforming existing methods.

Conclusion: Self-Foveate is effective and superior for instruction synthesis, with publicly released data and code.
...
TL;DR: Advanced reasoning models like OpenAI's o-series and DeepSeek-R show significant improvements in causal discovery, especially with structured in-context pipelines, outperforming conventional methods.


<details>
  <summary>Details</summary>
Motivation: To explore whether state-of-the-art reasoning models can robustly perform causal discovery, addressing overfitting and poor performance under data perturbations.

Method: Uses the Corr2Cause benchmark and introduces a modular in-context pipeline inspired by Tree-of-Thoughts and Chain-of-Thoughts methodologies.

Result: Achieves nearly three-fold improvements over conventional baselines, with qualitative and quantitative gains.

Conclusion: Advanced reasoning models, combined with structured frameworks, offer a substantial leap in causal discovery, providing a generalizable blueprint for diverse domains.
...
TL;DR: The paper introduces causal explanations for image classifiers, combining formal rigor with practical black-box applicability, and demonstrates their efficiency and versatility.


<details>
  <summary>Details</summary>
Motivation: Existing explanation methods for image classifiers lack formal rigor or rely on impractical assumptions, creating a need for a robust yet practical approach.

Method: The authors propose causal explanations, proving their formal properties and introducing contrastive and confidence-aware variants. They implement these definitions for black-box models.

Result: Experiments show varying patterns of sufficiency, contrastiveness, and completeness across models, with efficient computation (6s per image on ResNet50).

Conclusion: Causal explanations offer a rigorous, practical, and efficient solution for explaining image classifier outputs without requiring model internals or specific properties.
...
TL;DR: DICE introduces a dynamic, theoretically grounded method for selecting in-context demonstrations to improve LLM agent performance, addressing spurious dependencies and ensuring generalization.
...
Conclusion: Principled, context-aware demo selection is crucial for robust and efficient LLM agents, with DICE providing a framework-agnostic solution.
...
TL;DR: Proposes an autonomous trust orchestration method using semantic chain-of-trust and agentic AI for efficient trust evaluations in collaborative systems.
...
Method: Uses agentic AI and hypergraphs to autonomously evaluate trust during device idle periods, analyze task-resource alignment, and manage collaborators hierarchically.

Result: Achieves resource-efficient trust evaluation and enables efficient coordination in large-scale systems.

Conclusion: The method balances overhead and trust accuracy, enhancing collaborative task execution.
...
TL;DR: The paper introduces a strategy-guided agent-assisted memory recall method, MemoCue, which transforms queries into cue-rich ones using a Recall Router framework and hierarchical recall tree, outperforming LLM-based methods by 17.74%.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of conventional memory modules in human-computer interaction by leveraging proactive memory activation through effective cues.

Method: Proposes a Recall Router framework with a 5W Recall Map for scenario classification and a hierarchical recall tree combined with Monte Carlo Tree Search for strategy optimization. Fine-tunes LLMs to develop MemoCue.

Result: MemoCue outperforms LLM-based methods by 17.74% in recall inspiration and shows advantages in human evaluations.

Conclusion: The strategy-guided approach significantly enhances memory recall performance, demonstrating practical applicability in human-computer interaction.
...
Conclusion: RAR offers a scalable and effective solution for personalized question recommendation, with publicly available code.
...
TL;DR: TextQuests is a benchmark for evaluating AI agents' intrinsic long-context reasoning in exploratory environments, using text-based adventures to test autonomous problem-solving.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack assessment of agents' autonomous, long-horizon reasoning in exploratory settings.

Method: TextQuests uses Infocom interactive fiction games to evaluate agents' trial-and-error learning and sustained problem-solving without external tools.

Result: The benchmark provides a framework to assess AI agents' self-contained problem-solving in long-context scenarios.

Conclusion: TextQuests aims to advance AI agents' robust intrinsic reasoning by simulating real-world exploratory challenges.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with theorem proving due to unclear supervision in natural language. Formal languages like Lean offer clear verification, enabling effective reinforcement learning.

Method: Seed-Prover iteratively refines proofs using Lean feedback, proved lemmas, and self-summarization. Three test-time inference strategies enhance reasoning. Seed-Geometry supplements Lean for geometry tasks.

Result: Seed-Prover solves 78.1% of formalized IMO problems, saturates MiniF2F, and exceeds 50% on PutnamBench. Seed-Geometry outperforms prior formal geometry engines. Together, they solve 5/6 IMO 2025 problems.

Conclusion: The work advances automated mathematical reasoning by combining formal verification with long chain-of-thought reasoning, demonstrating significant improvements over prior methods.
...
<details>
  <summary>Details</summary>
Motivation: To improve LLM training by generating high-quality synthetic data through structured reasoning and planning, surpassing current datasets.
...
Result: Outperforms datasets like s1k and OpenMathReasoning in reasoning tasks (MATH500, AMC23, etc.) and human/standard prompts in instruction-following tasks (AlpacaEval 2.0, Arena-Hard).
...
TL;DR: SimuRA introduces a generalized agentic reasoning architecture using world-model-based planning to overcome autoregressive LLM limitations, achieving significant success in web browsing tasks.


<details>
  <summary>Details</summary>
Motivation: Current AI agents rely on one-task-one-agent approaches, lacking scalability and generality, while humans reason by simulating outcomes. SimuRA aims to bridge this gap.

Method: SimuRA uses a world model implemented with LLMs for flexible planning via simulation, tested on web browsing tasks.

Result: SimuRA improved flight search success from 0% to 32.2%, with world-model-based planning outperforming autoregressive methods by up to 124%.

Conclusion: SimuRA demonstrates the potential of world-model-based reasoning for general AI agents, with plans to expand its capabilities across environments.
...
TL;DR: The paper evaluates two LLMs (Mistral 7B and Mixtral 8x7B) for improving accommodation data consistency on a booking platform. Mixtral outperforms Mistral in quality but at higher computational costs.


<details>
  <summary>Details</summary>
Motivation: Inconsistent third-party data in property booking platforms frustrates users and harms market share, prompting the need for better solutions.

Method: Two LLMs (Mistral 7B fine-tuned with QLoRA and Mixtral 8x7B with refined prompts) were tested for generating consistent descriptions and minimizing hallucinations.
...
Conclusion: The study highlights trade-offs between model quality and resource efficiency, aiding practical LLM deployment in production for reliable data enhancement.
...
<details>
  <summary>Details</summary>
Motivation: Current systems like China's 95598 hotline have slow response times and lack domain expertise. LLMs like GPT-4o lack the required empathy and specialization for power marketing.

Method: ElectriQ introduces a dialogue dataset, four evaluation metrics, and a knowledge augmentation method to enhance LLMs.

Result: Fine-tuned smaller models (e.g., LLama3-8B) outperformed GPT-4o in professionalism and user-friendliness.

Conclusion: ElectriQ provides a foundation for developing specialized LLMs for power marketing, demonstrating the potential of smaller, fine-tuned models.
...
TL;DR: A hierarchical framework combining fine-tuned language models and semi-supervised ensemble learning detects and classifies illicit marketplace content across diverse platforms, outperforming baselines with high accuracy.


<details>
  <summary>Details</summary>
Motivation: Illicit trade on concealed internet platforms is challenging to detect due to limited labeled data, evolving language, and structural heterogeneity.
...
Result: Achieves high accuracy (0.96489), F1-score (0.93467), and TMCC (0.95388), outperforming models like BERT and DarkBERT.
...
Method: A hybrid approach uses ML models to predict the number of LCSH labels and post-edit LLM outputs to align with actual LCSH terms.

Result: The hybrid framework produces more controlled and vocabulary-aligned subject term predictions.
...
TL;DR: The paper proposes a novel KG integration method focusing on context matching, combining label and triple matching techniques to improve entity-matching accuracy, validated on diverse datasets.


<details>
  <summary>Details</summary>
Motivation: Current KG integration methods overlook context matching, which is crucial for handling diverse and complex real-world KGs.
...
Result: The approach outperforms leading systems in the OAEI competition and supervised methods, achieving high accuracy. A new dataset is introduced for comprehensive evaluation.

Conclusion: The proposed method effectively addresses the gap in context matching for KG integration, demonstrating robust performance across varied scenarios.
...
TL;DR: The paper rigorously defines and analyzes hallucination in LLMs, distinguishing intrinsic and extrinsic types. It introduces a hallucination risk metric, derives bounds using learning theory, surveys detection and mitigation strategies, and proposes a unified workflow and evaluation protocols.


<details>
  <summary>Details</summary>
Motivation: Hallucination in LLMs undermines their reliability, necessitating formal definitions, theoretical analysis, and practical solutions to detect and mitigate it.

Method: The paper uses learning-theoretic frameworks (PAC-Bayes and Rademacher complexity) to derive bounds on hallucination risk. It surveys detection (e.g., uncertainty estimation) and mitigation (e.g., retrieval-augmented generation) strategies, proposing a unified workflow.

Result: The work provides theoretical bounds on hallucination risk, a comprehensive survey of detection and mitigation techniques, and a proposed workflow for integration.

Conclusion: The paper establishes a foundation for addressing hallucination in LLMs, offering theoretical insights and practical guidelines to improve model reliability.
...
<details>
  <summary>Details</summary>
Motivation: Standard RAG struggles with temporal coherence in queries, limiting its ability to track entities over time.
...
Result: The approach improves answer accuracy by 13% to 27% over standard RAG, validated on the ADQAB benchmark.

Conclusion: The work provides a pathway for RAG systems to handle complex, time-sensitive queries effectively.
...
TL;DR: The paper examines feature universality in Gemma-2 language models (2B and 9B parameters) to determine if internal concepts converge despite scale differences. Using Sparse Autoencoders (SAEs) and alignment techniques, it finds middle layers show strong feature overlap, while early and late layers differ. Multi-token analysis reveals similar semantic subspaces, supporting universality as a basis for cross-model interpretability.


<details>
  <summary>Details</summary>
Motivation: To explore whether language models of vastly different scales (2B vs. 9B parameters) develop comparable internal representations, which could validate universality as a principle for interpretability.

Method: Employed Sparse Autoencoders (SAEs) to extract features from residual-stream activations of Gemma-2 models. Features were aligned using activation correlation and compared with SVCCA and RSA. Extended analysis to multi-token subspaces.

Result: Middle layers exhibited strong feature overlap, while early and late layers showed less similarity. Multi-token subspaces with semantic similarity interacted comparably across models.

Conclusion: Large language models, regardless of size, develop broadly similar and interpretable features, supporting universality as a foundation for cross-model interpretability.
...
TL;DR: The paper evaluates methods for predicting serious adverse event (SAE) outcomes in clinical trials using trial registration data, achieving 77.6% AUC for classification and 18.6% RMSE for regression.


<details>
  <summary>Details</summary>
Motivation: To improve clinical trial design by predicting SAE outcomes early, reducing unnecessary risks to participants and avoiding trial terminations.

Method: Used transfer learning with pretrained language models (ClinicalT5, BioBERT) for feature extraction, combined with downstream models. A sliding window method was developed for handling long trial texts.

Result: Best model (ClinicalT5+Transformer+MLP) achieved 77.6% AUC for classification and 18.6% RMSE for regression. Sliding window method improved performance.

Conclusion: ClinicalTrials.gov data is underutilized; predicting trial outcomes early can enhance trial design and detect safety discrepancies.
...
TL;DR: This paper presents a structured taxonomy and analysis of discrete tokenization methods for LLMs, categorizing 8 VQ variants, discussing their principles, challenges, and applications, and identifying future research directions.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of LLMs necessitates effective discrete tokenization methods, yet there's no comprehensive survey on VQ techniques in LLM-based systems.

Method: The paper categorizes 8 VQ variants, analyzes their principles, training dynamics, and integration challenges with LLMs, and reviews applications in classical, single-modality, and multimodal systems.

Result: The survey highlights key challenges like codebook collapse and unstable gradients, and discusses emerging directions like dynamic quantization and unified frameworks.

Conclusion: This work bridges traditional VQ and modern LLM applications, serving as a foundational reference for efficient multimodal systems.
...
<details>
  <summary>Details</summary>
Motivation: Address the issues of high cost, hallucinations, and wasted resources in language models by ensuring correctness through a multi-stage validation process.

Method: Propose the LMC algorithm, where a language model's response is validated against candidate answers, and incorrect responses are passed to slower, more predictive models iteratively.
...
TL;DR: The paper examines if social media sentiment predicts stock movements, focusing on GameStop and AMC. It finds weak correlation between sentiment and prices, but stronger signals from comment volume and search trends.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of online sentiment, especially from platforms like Reddit, on stock market movements, inspired by events like the GameStop short squeeze.

Method: Analyzes sentiment using two existing methods and a new ChatGPT-annotated RoBERTa model, focusing on informal language and emojis. Uses correlation and causality metrics.
...
Method: Systematic evaluation of prompt translation strategies (pre-translation vs. cross-lingual prompting) for classification tasks with RAG-enhanced LLMs.
...
<details>
  <summary>Details</summary>
Motivation: To understand the role of language background in peer feedback experiences for international students in online CS courses.
...
TL;DR: Proposes a Hierarchical Memory (H-MEM) architecture for LLM Agents to enhance memory organization and retrieval, outperforming baselines in long-term dialogue tasks.
...
Method: Introduces H-MEM, a multi-level memory architecture with positional indexing and layer-by-layer retrieval for efficient access.

Result: Outperforms five baseline methods on the LoCoMo dataset, showing effectiveness in long-term dialogue scenarios.

Conclusion: H-MEM improves memory organization and retrieval, enhancing LLM Agents' reasoning and contextual coherence.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fail to capture the complete document context for relation extraction, as they focus only on sentences where entities appear, ignoring shifts in relationships across contexts.

Method: The proposed approach represents entities as standalone segments, independent of their positions, to leverage global relationships and multi-sentence reasoning.
...
TL;DR: The paper introduces a fine-grained benchmark, Placeholder-RAG-Benchmark, to evaluate LLM-specific capabilities in RAG systems, focusing on filtering, combination, and reasoning. It highlights LLM limitations in error resilience and context faithfulness.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks lack granular evaluation of LLM-specific capabilities in RAG systems, focusing instead on overall performance.

Method: The authors propose a multi-level benchmark with placeholder-based decoupling to assess LLM contributions separately from external knowledge.
...
TL;DR: The study investigates the faithfulness of Chain-of-Thought (CoT) prompting in Large Language Models (LLMs) by analyzing feature-level causality, revealing its effectiveness in larger models like Pythia-2.8B but not in smaller ones like Pythia-70M.


<details>
  <summary>Details</summary>
Motivation: To determine if CoT-generated thoughts truly reflect the internal reasoning process of LLMs and to assess the faithfulness of CoT prompting.

Method: Combined sparse autoencoders with activation patching to extract monosemantic features from Pythia-70M and Pythia-2.8B while solving GSM8K math problems under CoT and plain (noCoT) prompting.

Result: Swapping CoT-reasoning features into noCoT runs significantly improved answer log-probabilities in Pythia-2.8B but not in Pythia-70M. CoT also increased activation sparsity and feature interpretability in the larger model.
...
TL;DR: EH-Benchmark evaluates hallucinations in Medical Large Language Models (MLLMs) for ophthalmic diagnosis, proposing a multi-agent framework to improve accuracy.


<details>
  <summary>Details</summary>
Motivation: MLLMs face accuracy issues due to hallucinations from limited ophthalmic knowledge, visual reasoning gaps, and lack of multimodal data. Existing benchmarks fail to address these.

Method: Introduces EH-Benchmark, categorizing hallucinations into Visual Understanding and Logical Composition. Proposes a three-phase framework: Knowledge-Level Retrieval, Task-Level Case Studies, Result-Level Validation.
...
Conclusion: EH-Benchmark and the proposed framework effectively address MLLM hallucinations in ophthalmic diagnosis, enhancing model performance.
...
TL;DR: The paper introduces a method to create synthetic PII-revealing text data for privacy research, using LLMs to mimic Reddit posts, ensuring reproducibility, unlinkability, and indistinguishability.


<details>
  <summary>Details</summary>
Motivation: Lack of open-source labeled datasets hampers research on PII privacy risks in social media. Synthetic data can enable safer, reproducible studies.

Method: Developed a taxonomy of 19 PII categories, used LLMs (Llama2-7B, Llama3-8B, zephyr-7b-beta) with sequential prompting to generate synthetic Reddit-like posts.

Result: Created a synthetic dataset evaluated for reproducibility, unlinkability, and indistinguishability from original data.

Conclusion: The synthetic dataset and methodology support reproducible research into PII privacy risks, with released resources for public use.
...
TL;DR: ACC-RAG dynamically adjusts compression rates for RAG, improving efficiency without losing accuracy, outperforming fixed-rate methods.
...
Method: Proposes ACC-RAG, combining a hierarchical compressor and context selector to adaptively compress based on input complexity.

Result: Outperforms fixed-rate methods, achieving up to 4x faster inference while maintaining or improving accuracy.

Conclusion: ACC-RAG optimizes RAG efficiency by dynamically adjusting compression, balancing speed and accuracy.
...
Result: Achieves 26% annualized return and Sharpe ratio of 1.2, outperforming equal-weighted and S&P 500 benchmarks.
...
TL;DR: The paper reviews neural-symbolic systems integrating VLMs with external symbolic systems to enhance reasoning, memory, and interpretability in visual-language tasks.


<details>
  <summary>Details</summary>
Motivation: Current visual-language models lack interpretability, require retraining for updates, and struggle with logical reasoning, prompting the need for neural-symbolic solutions.

Method: The study systematically reviews techniques for improving visual-language understanding by combining pre-trained VLMs with external symbolic systems.

Result: Neural-symbolic systems offer interpretable outputs, reduce retraining needs, and improve reasoning and memory capabilities.

Conclusion: Integrating VLMs with symbolic systems presents a practical approach to overcoming limitations of standalone visual-language models.
...
### [142] [OAEI-LLM-T: A TBox Benchmark Dataset for Understanding Large Language Model Hallucinations in Ontology Matching](https://arxiv.org/abs/2503.21813)
*Zhangcheng Qiang,Kerry Taylor,Weiqing Wang,Jing Jiang*

Main category: cs.CL

TL;DR: A new benchmark dataset, OAEI-LLM-T, is introduced to address hallucinations in LLM-based ontology matching (OM) tasks, derived from OAEI datasets. It categorizes OM-specific hallucinations and aids in LLM leaderboard construction and fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Hallucinations in LLMs pose challenges for ontology matching tasks, necessitating a dedicated benchmark to evaluate and mitigate these issues.

Method: The dataset OAEI-LLM-T is created from seven TBox datasets in OAEI, capturing hallucinations from ten LLMs in OM tasks, categorized into two main and six sub-categories.

Result: The dataset enables the construction of an LLM leaderboard for OM tasks and supports fine-tuning of LLMs for improved performance.

Conclusion: OAEI-LLM-T provides a valuable resource for addressing hallucinations in LLM-based OM systems, facilitating evaluation and enhancement of LLM performance.
...
### [143] [Deep Learning Approaches for Multimodal Intent Recognition: A Survey](https://arxiv.org/abs/2507.22934)
...
TL;DR: The paper surveys deep learning methods for intent recognition, highlighting the shift from unimodal to multimodal techniques, Transformer-based advancements, and future research directions.


<details>
  <summary>Details</summary>
Motivation: To address the growing need for natural human-computer interaction by exploring intent recognition beyond text, incorporating multimodal data.

Method: Reviews deep learning approaches, including Transformer-based models, and covers datasets, methodologies, and applications in multimodal intent recognition.

Result: Identifies breakthroughs in the field, particularly through multimodal and Transformer-based techniques, and outlines current challenges.

Conclusion: Provides insights into the latest developments in MIR and suggests directions for future research to advance the field.
...
TL;DR: The paper explores Knowledge Graph construction for operational intelligence in aviation, evaluating NLP and LLM tools for trusted applications, highlighting performance limitations and recommending improvements.


<details>
  <summary>Details</summary>
Motivation: The challenge of balancing data confidentiality and integration in operational intelligence, coupled with NLP tool limitations in specialized domains like aviation, drives the need for trusted Knowledge Graph solutions.

Method: The study breaks down Knowledge Extraction into components (NER, Coreference Resolution, etc.), evaluates 16 NLP tools and LLMs, and tests them on an FAA-derived dataset in a confidential setting.
...
Conclusion: The paper recommends enhancing trust in NLP/LLM tools and provides an open-source dataset for further evaluation in trusted operational intelligence applications.
...
TL;DR: The paper compares five LLMs (GPT, Claude, Perplexity, Gemini, DeepSeek) on FinNLP tasks using 10-K filings, evaluating performance via human annotation, automated metrics, and diagnostics. GPT performs best, while Gemini and DeepSeek show more variability.
...
Method: Evaluates five LLMs using domain-specific prompts and three methodologies: human annotation, automated metrics (ROUGE, Cosine Similarity, Jaccard), and model behavior diagnostics.

Result: GPT delivers the most coherent and contextually relevant answers, followed by Claude and Perplexity. Gemini and DeepSeek exhibit more variability and less agreement. Performance varies by company and prompt.

Conclusion: GPT is the top performer in FinNLP tasks, but model outputs are sensitive to prompts and source material, highlighting the need for careful evaluation in financial applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing AIOps models are limited to specific tasks, and combining multiple models can enhance performance, as seen in ensemble learning and LLM training.
...
Conclusion: CoE-Ops effectively addresses AIOps challenges, demonstrating superior performance in task routing and problem resolution.
...
TL;DR: The paper proposes enhancing text-based QA systems by incorporating graph representations of flowcharts from VLMs, improving retrieval for image-based questions in telecom documents.


<details>
  <summary>Details</summary>
Motivation: Text-based RAG systems struggle with QA tasks involving figures like flowcharts, necessitating a solution that integrates visual data.

Method: The approach involves processing technical documents, classifying images, generating graph representations using VLMs, and integrating them with text embeddings for retrieval.

Result: Graph representations from fine-tuned VLMs show lower edit distance to ground truth, and the hybrid retrieval system performs well with text-based embeddings.

Conclusion: The method effectively bridges the gap between text and image-based QA, reduces reliance on VLMs during inference, and offers cost benefits for deployment.
...
TL;DR: PARROT is a large, multilingual, open-access dataset of fictional radiology reports for NLP testing, validated through a human vs. AI differentiation study.


<details>
  <summary>Details</summary>
Motivation: To create a privacy-unconstrained, multicentric dataset for testing NLP applications in radiology across languages and regions.

Method: Radiologists contributed fictional reports with metadata; a study assessed human vs. AI report differentiation.

Result: The dataset includes 2,658 reports in 13 languages; participants struggled to distinguish human from AI reports (53.9% accuracy).

Conclusion: PARROT facilitates NLP development in radiology globally, overcoming privacy and linguistic barriers.
...
Method: Integrates a fact-checking classifier, GRPO reinforcement learning, and mechanistic interpretability.

Result: Improves factual robustness by up to 49.90% while maintaining performance on benchmarks like Math-500 and GPQA.

Conclusion: Provides insights for future training methods to enhance factual robustness through activation-guided optimization.
...
TL;DR: SigBERT is a temporal survival analysis framework that processes EHR data by extracting sentence embeddings and using signature features to improve survival model performance.


<details>
  <summary>Details</summary>
Motivation: Existing survival analysis methods struggle with the complexity of textual EHR data, especially in sequential form.

Method: SigBERT extracts sentence embeddings from timestamped reports, applies signature extraction for temporal dynamics, and integrates features into a LASSO-penalized Cox model.

Result: Achieved a C-index score of 0.75 on an oncology dataset, demonstrating improved risk estimation.

Conclusion: SigBERT effectively leverages sequential medical data for enhanced survival analysis.
...
<details>
  <summary>Details</summary>
Motivation: To enhance the efficiency of validating code-based algorithms in large claims databases, reducing the time and resources typically required for manual chart reviews.
...
Result: NLP reduced review time by 40%, and adaptive sampling could have prevented 77% of chart reviews without significant loss of precision.

Conclusion: The approach enables more routine validation of algorithms, improving the reliability of findings from database studies.
...
Conclusion: The framework offers a neutral theory of arbitrariness, applicable to human systems and AI explainability, highlighting its functional necessity.
...
<details>
  <summary>Details</summary>
Motivation: There's limited research on SDMs' practical effectiveness in emulating human conversations, especially compared to extensively benchmarked text-based LLMs. Spoken dialogue's complexity (ambiguity, context-dependency) adds unique challenges.

Method: A benchmark dataset of 1,079 instances in English and Chinese is presented, along with an LLM-based evaluation method aligned with human judgment.

Result: The dataset and evaluation method enable comprehensive exploration of SDMs' performance in handling spoken dialogue challenges.

Conclusion: The work aims to advance SDM development by providing tools to assess and improve their comprehension and emulation of human conversations.
...
TL;DR: The paper investigates whether LLMs can perform Natural Language Inference (NLI) on mathematical texts (Math NLI). It evaluates human- and LLM-generated hypotheses, finding mixed results: LLMs sometimes match human performance but struggle with mathematical language.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' capability in handling NLI tasks specifically for mathematical texts, a domain where language and logic intertwine.
...
Result: LLMs can approximate human performance in some settings (via majority voting) but often struggle with mathematical language and basic inferences.

Conclusion: LLMs show promise in Math NLI but have limitations. The provided corpus aims to support future research in this niche area.
...
TL;DR: The paper explores using In-Context Learning (ICL) with Large Language Models (LLMs) for Frame Semantic Parsing (FSP) without fine-tuning, achieving competitive results on Frame Identification (FI) and Frame Semantic Role Labeling (FSRL).


<details>
  <summary>Details</summary>
Motivation: To investigate whether ICL with LLMs can effectively perform FSP tasks without fine-tuning, leveraging the FrameNet database.

Method: Automatically generates task-specific prompts for FI and FSRL using frame definitions and annotated examples from FrameNet, then evaluates six LLMs on violent event-related frames.

Result: Achieves F1 scores of 94.3% for FI and 77.4% for FSRL, demonstrating competitive performance.
...
Method: CARoPE dynamically generates head-specific frequency patterns using token embeddings, integrating input-dependent phase shifts into RoPE.
...
<details>
  <summary>Details</summary>
Motivation: Prior models focus on local edits, lacking global coherence. SMART-Editor addresses this gap for structured (posters, websites) and unstructured (natural images) domains.

Method: Uses Reward-Refine (inference-time refinement) and RewardDPO (training-time preference optimization) to ensure global coherence. Introduces SMARTEdit-Bench for evaluation.

Result: Outperforms baselines like InstructPix2Pix and HIVE, with RewardDPO achieving 15% gains in structured settings and Reward-Refine excelling on natural images.

Conclusion: Reward-guided planning in SMART-Editor produces semantically consistent and visually aligned edits, validated by automatic and human evaluations.
...
Method: Introduces a component-based retrieval architecture that decomposes database schemas and metadata into discrete semantic units, separately indexed for targeted retrieval.

Result: The method maintains high recall and accuracy, outperforming baselines on massive databases with varying structures and metadata.
...
TL;DR: The paper investigates LLMs' handling of ambiguous Chinese text, revealing their fragility and overconfidence in interpretation, and calls for improved methods.


<details>
  <summary>Details</summary>
Motivation: To assess LLM trustworthiness by examining their behavior with ambiguous narrative text, focusing on Chinese ambiguity.
...
Result: LLMs struggle with ambiguity: cannot reliably distinguish ambiguous text, overconfidently interpret single meanings, and overthink possible interpretations.

Conclusion: Current LLMs have limitations in handling ambiguity, impacting real-world applications. Improved approaches for uncertainty in language understanding are needed.
...
TL;DR: ISO-Bench evaluates multimodal models' ability to infer causal dependencies between visual and text steps, revealing poor performance (best F1: 0.57) compared to humans (0.98).
...
Result: Models perform poorly (best F1: 0.57), with chain-of-thought reasoning offering slight improvement (0.62 F1), far behind humans (0.98 F1).
...
TL;DR: The paper studies implicit user feedback from interaction logs (WildChat and LMSYS) to improve language models (LMs), finding that feedback content (not just polarity) enhances performance on short questions (MTBench) but not complex ones (WildBench). Feedback usefulness depends on prompt quality.


<details>
  <summary>Details</summary>
Motivation: To improve LMs continuously without disruptive direct feedback by analyzing implicit feedback from user-LM interactions.

Method: Analyzed user feedback in WildChat and LMSYS datasets, studying its occurrence and harvesting learning signals.

Result: Feedback content improves performance on short questions (MTBench) but not complex ones (WildBench). Feedback usefulness is tied to prompt quality.

Conclusion: Implicit user feedback has potential but limitations, with effectiveness depending on feedback content and prompt quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing ensemble methods for LLMs often use simplistic approaches like voting, ignoring context-dependent model reliability.
...
Conclusion: Internal representations in LLMs provide valuable confidence signals, enhancing ensemble learning without extra computation or parameter changes.
...
<details>
  <summary>Details</summary>
Motivation: The need for scalable, hardware-optimized AI solutions drives demand for automated GPU kernel development, especially for complex deep learning workloads.

Method: GEAK uses LLMs and Reflexion-style feedback to generate Triton-based GPU kernels, evaluated on benchmarks for correctness and performance.

Result: GEAK achieves 63% correctness and 2.59X speedup over baselines, demonstrating superior performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for few-shot ICL focus on Positive samples, ignoring the potential of Negative samples to improve example selection and mitigate biases.

Method: Construct Positive and Negative sample corpora using Zero-Shot-Cot, then use semantic similarity to select examples from both for a query, further refining Positive examples based on Negative ones.

Result: The approach outperforms methods using only Positive samples, showing Negative samples aid in better Positive example selection and improved ICL performance.

Conclusion: Leveraging Negative samples enhances few-shot ICL by refining Positive example selection, demonstrating their untapped potential in contextual learning.
...
TL;DR: MTMs improve topic modeling by using interpretable features from sparse autoencoders, enabling deeper themes and controllable text generation.


<details>
  <summary>Details</summary>
Motivation: Traditional topic models struggle with semantic abstraction and expressive topic articulation due to reliance on bag-of-words or word lists.
...
Result: MTMs match or outperform baselines in coherence, are preferred in evaluations, and allow effective steering of LLM outputs.

Conclusion: MTMs offer a more expressive and controllable approach to topic modeling compared to traditional methods.
...
Method: The framework uses few-shot tabular prompts and finetunes TableGPT2 with qLoRA for binary classification of AD or cognitively normal cases.

Result: TAP-GPT outperforms advanced general-purpose LLMs and a tabular foundation model in AD prediction.

Conclusion: This is the first LLM application for tabular biomarker prediction, opening avenues for LLM-driven biomedical frameworks.
...
TL;DR: The paper explores pragmatic reasoning in LLMs for mental health, introducing the P-ReMe dataset and tasks for implicature and presupposition. It benchmarks four models, finding Mistral and Qwen perform well, and proposes StiPRompts to study stigma, with Claude-3.5-haiku handling it best.


<details>
  <summary>Details</summary>
Motivation: To address the lack of exploration in reasoning aspects for explainability and dialogue discourse in mental health chatbots.

Method: Introduces the P-ReMe dataset, defines pragmatic phenomena (implicature, presupposition), formulates tasks, and benchmarks four LLMs. Also proposes StiPRompts to study stigma with GPT-4o mini, Deepseek-chat, and Claude-3.5-haiku.

Result: Mistral and Qwen show strong reasoning capabilities. Claude-3.5-haiku handles mental health stigma more responsibly than GPT-4o mini and Deepseek-chat.

Conclusion: The study advances pragmatic reasoning in LLMs for mental health, highlighting model performance and stigma handling, with implications for responsible AI development.
...
TL;DR: The paper investigates challenges in Bengali NLP, evaluates 10 LLMs on translated datasets, and identifies performance gaps, tokenization issues, and promising architectures like DeepSeek.
...
Result: Performance gaps exist for Bengali vs. English, especially for smaller models. Tokenization efficiency inversely affects accuracy. DeepSeek showed robustness.
...
TL;DR: The paper identifies 'Super Experts' (SEs) in MoE LLMs, which are crucial for model performance. Pruning SEs significantly degrades performance, especially in tasks like mathematical reasoning. SEs exhibit unique activation patterns and are model-specific.
...
Method: Analyzes activation patterns, prunes SEs to assess their impact, and explores their role in attention mechanisms.

Result: SEs are rare but crucial, with extreme activation outliers. Pruning SEs causes performance decline, especially in mathematical reasoning, and disrupts attention mechanisms.

Conclusion: SEs are vital for MoE LLMs, and their pruning harms performance. The findings deepen understanding of expert roles in such models.
...
TL;DR: GPT-4o-mini implicitly moderates sensitive content, reducing derogatory and taboo language without explicit instructions, and outperforms traditional methods in zero-shot sensitivity classification.


<details>
  <summary>Details</summary>
Motivation: To explore whether LLMs implicitly sanitize language without explicit training, focusing on GPT-4o-mini's behavior in paraphrasing sensitive content.
...
Result: GPT-4o-mini systematically moderates content toward less sensitive classes, significantly reducing derogatory and taboo language, and shows strong zero-shot classification capabilities.

Conclusion: LLMs like GPT-4o-mini implicitly moderate content effectively, suggesting potential for automated moderation without explicit training.
...
TL;DR: MusT-RAG enhances LLMs for music question answering using a specialized vector database (MusWikiDB) and RAG, outperforming traditional fine-tuning.
...
Method: Proposes MusT-RAG, combining MusWikiDB (music-specialized vector database) with RAG for retrieval and fine-tuning.

Result: MusT-RAG outperforms fine-tuning, improving performance on in- and out-of-domain MQA tasks. MusWikiDB is more effective than general Wikipedia.

Conclusion: MusT-RAG effectively adapts LLMs for music tasks, demonstrating superior performance and efficiency.
...
Method: TeQoDO uses an LLM to build ontologies from scratch by leveraging its SQL capabilities and dialogue theory in prompts, without supervision.

Result: TeQoDO outperforms transfer learning, scales to larger datasets, and performs competitively in dialogue state tracking.

Conclusion: TeQoDO advances LLM explainability by enabling autonomous ontology construction, with potential for broader applications.
...
TL;DR: The paper introduces MPCC, a benchmark for evaluating multimodal planning in MLLMs, addressing gaps in assessing real-world planning and handling complex constraints.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks lack the ability to assess real-world multimodal planning and handle complex constraints, limiting progress in MLLM applications.

Method: MPCC evaluates MLLMs on three real-world tasks (Flight, Calendar, Meeting Planning) with graded constraints (EASY, MEDIUM, HARD).

Result: Experiments show poor performance: closed-source models achieve 21.3% feasible plans, open-source models average below 11%. MLLMs struggle with constraint complexity.

Conclusion: MPCC formalizes multimodal constraints, provides a rigorous evaluation framework, and underscores the need for better constraint-aware reasoning in MLLMs.
...
TL;DR: Causal2Vec enhances decoder-only LLMs for embedding tasks without altering their architecture, using a lightweight BERT-style model and optimized pooling, achieving SOTA performance with reduced computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing methods for embedding tasks with decoder-only LLMs either compromise semantic extraction or increase computational overhead by relying on extra input text.

Method: Causal2Vec prepends a Contextual token (pre-encoded by a lightweight BERT-style model) to the LLM's input and concatenates the last hidden states of Contextual and EOS tokens for the final embedding.

Result: Achieves state-of-the-art performance on MTEB, reduces sequence length by 85%, and cuts inference time by 82% compared to leading methods.

Conclusion: Causal2Vec effectively leverages decoder-only LLMs for embeddings without significant architectural changes or computational costs.
...
<details>
  <summary>Details</summary>
Motivation: Address concerns about data privacy, security, and equitable access in cloud-based AI translation tools by investigating local deployment options.

Method: Evaluates three open-source models on CPU-based platforms, comparing them to commercial cloud-based chatbots, focusing on functional performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-document summarization struggle with single-relational graphs and predefined cluster numbers, limiting their ability to model rich relationships and reduce redundancy adaptively.

Method: The framework constructs a multi-relational graph for semantic and discourse relations, applies structural entropy minimization for clustering, and uses position-aware compression for summarization.

Result: Experiments on benchmark datasets show MRGSEM-Sum outperforms unsupervised methods and matches some supervised models, with human evaluations confirming high-quality summaries.

Conclusion: MRGSEM-Sum effectively addresses limitations of existing methods, producing high-quality summaries with adaptive clustering and multi-relational modeling.
...
TL;DR: An enhanced Dense Passage Retrieval (DPR) framework for Arabic NLP, featuring a novel Attentive Relevance Scoring (ARS) to improve semantic relevance and retrieval performance.
...
Conclusion: The framework addresses Arabic NLP challenges effectively, with code publicly available for further use.
...
TL;DR: The paper introduces proactive critical thinking in AI, where models actively seek missing information to resolve queries, and presents benchmarks (GSM-MC, GSM-MCE) to evaluate this. Reinforcement learning (RL) significantly improves performance.


<details>
  <summary>Details</summary>
Motivation: Prior work focused on passive critical thinking (rejecting flawed queries). This work aims to enhance AI's ability to collaborate with users by proactively addressing incomplete or misleading queries.

Method: Introduces GSM-MC and GSM-MCE benchmarks for mathematical reasoning under incomplete/misleading conditions. Uses RL to improve model performance.

Result: RL boosts Qwen3-1.7B's accuracy from 0.15% to 73.98% on GSM-MC. Larger models struggle with proactive critical thinking.

Conclusion: Proactive critical thinking improves AI-user collaboration. RL is effective in enhancing this capability, especially for smaller models.
...
Result: LLMs showed moderate performance (57.2% avg), with a 13.3% drop in high-risk scenarios. Medical LLMs outperformed general-purpose models.
...
TL;DR: Med-R³ is a medical retrieval-augmented reasoning framework using progressive reinforcement learning to jointly optimize retrieval and reasoning, outperforming existing models like GPT-4o-mini and Qwen2.5-14B.
...
Conclusion: The framework effectively addresses the limitations of isolated optimization and supervised fine-tuning, demonstrating superior performance in medical reasoning tasks.
...
Method: T-Detect replaces Gaussian normalization with a heavy-tailed discrepancy score based on the Student's t-distribution, leveraging leptokurtosis in adversarial texts.

Result: T-Detect improves AUROC by up to 3.9% and achieves state-of-the-art performance (AUROC 0.926) on the RAID benchmark.

Conclusion: T-Detect provides a robust, theoretically-grounded solution for detecting adversarial machine-generated texts.
...
TL;DR: The paper focuses on detecting and masking hate speech in Arabic text using deep learning and transformers, achieving high accuracy in detection but moderate results in text cleaning.
...
Method: Used deep learning models and transformers for hate speech detection, and treated text cleaning as a machine translation task to mask offensive words.

Result: Achieved 92% Macro F1 score and 95% accuracy in detection; text cleaning scored 0.3 BLEU with 1-gram.

Conclusion: The methods are effective for hate speech detection but text cleaning needs improvement, though results are competitive with existing systems.
...
<details>
  <summary>Details</summary>
Motivation: To improve KG completeness and reasoning by making complex logical rules understandable through natural language explanations.

Method: Extracts rules using AMIE 3.5.1 from datasets FB15k-237, FB-CVT-REV, and FB+CVT-REV; tests prompting strategies like zero-/few-shot and chain-of-thought reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Standard QA benchmarks are indirect and may overestimate differences in LLMs' problem-solving abilities. A more holistic and scalable evaluation method is needed.

Method: Proposes a cascaded question disclosure approach, revealing partial question information stagewise to elicit generalized reasoning in LLMs.

Result: The framework improves model comparisons and intermediate reasoning traces, narrowing performance gaps seen in standard QA evaluations.

Conclusion: The cascaded approach offers a more accurate and scalable evaluation of LLMs' problem-solving capabilities, challenging the validity of standard QA benchmarks.
...
TL;DR: NeuroAR, a generative autoregressive transformer model, synthesizes brain aging trajectories from MRI scans, outperforming state-of-the-art models like LDMs and GANs in fidelity and realism.
...
Method: NeuroAR uses autoregressive transformers to estimate future brain scans from token embeddings, incorporating previous scans and age data via cross-attention.

Result: NeuroAR outperforms LDMs and GANs in image fidelity and realism, validated by a pre-trained age predictor.

Conclusion: NeuroAR effectively models subject-specific brain aging with high fidelity, offering superior performance over existing methods.
...
TL;DR: A keystroke-based framework detects LLM-assisted cheating in Korean, outperforming humans in distinguishing bona fide, paraphrased, and transcribed ChatGPT responses.
...
Method: Dataset of 69 participants performing writing tasks under three conditions (bona fide, paraphrasing, transcribing ChatGPT) across six cognitive processes. Temporal and rhythmic features are extracted and evaluated using classifiers.
...
<details>
  <summary>Details</summary>
Motivation: MLPs have limitations like poor interpretability and difficulty capturing complex features, motivating the adoption of KANs.

Method: The review examines KANs from three perspectives: data-driven learning, physics-informed modeling, and deep operator learning, comparing them to MLPs.

Result: KANs outperform MLPs in accuracy, convergence, and spectral representation, showing better handling of complex dynamics.

Conclusion: Future research should address computational efficiency, theoretical guarantees, and scalability of KANs.
...
<details>
  <summary>Details</summary>
Motivation: Traditional single-hazard methods fail to capture complex interactions among concurrent climatic events, necessitating intelligent, adaptable early warning systems.

Method: Combines sequential deep learning models (e.g., BiLSTM) with XAI techniques (TimeSHAP) to predict and explain multiple severe events.
...
TL;DR: FedCVD++ is an enhanced federated learning framework for predicting coronary heart disease risk, integrating parametric and non-parametric models while addressing FL challenges like communication overhead and class imbalance.


<details>
  <summary>Details</summary>
Motivation: The urgent need for privacy-preserving predictive systems in cardiovascular disease (CVD) due to its high global mortality rate.

Method: Proposes tree-subset sampling, XGBoost-based feature extraction, and federated SMOTE synchronization to improve FL efficiency and performance.

Result: Achieves state-of-the-art results (F1 = 0.80 for XGBoost, 0.81 for Random Forest) and reduces bandwidth consumption by 3.2X.

Conclusion: FedCVD++ is the first practical integration of non-parametric models into federated healthcare, offering privacy-preserving and scalable solutions.
...
TL;DR: GSM-UTCI, a deep learning framework, predicts hyperlocal UTCI with high accuracy, enabling rapid urban heat mitigation planning.


<details>
  <summary>Details</summary>
Motivation: Cities need scalable tools to assess and mitigate outdoor heat stress due to climate change and urbanization, as traditional models are computationally expensive.

Method: GSM-UTCI uses multimodal deep learning, combining surface morphology, land cover, and meteorological data with a FiLM architecture, trained on SOLWEIG-derived UTCI maps.

Result: Achieves R2 of 0.9151 and MAE of 0.41°C, reducing inference time to under five minutes for city-wide analysis. Demonstrates cooling effects of landscape transformations in Philadelphia.

Conclusion: GSM-UTCI is a scalable, fine-grained tool for urban climate adaptation, aiding in evaluating greening strategies efficiently.
...
<details>
  <summary>Details</summary>
Motivation: To highlight the ontological error in interpreting LLM performance on human tests as evidence of human-like traits, due to lack of validation and theoretical justification.

Method: Critically analyzes the misuse of human psychological and educational tests for AI evaluation, emphasizing issues like validity, data contamination, and cultural bias.

Result: Demonstrates that current practices mischaracterize AI capabilities and lack empirical support.

Conclusion: Advocates for developing principled, AI-specific evaluation frameworks to accurately assess AI systems.
...
<details>
  <summary>Details</summary>
Motivation: To investigate the unexplored potential of inverse mappings in task-specific AI models and assess their broader utility.
...
Conclusion: KLLM effectively combines K-Means quantization and outlier detection to enhance LLM inference efficiency without significant accuracy loss.
...
TL;DR: The paper integrates actor behavior analysis with Granger causality to uncover temporal and causal dynamics in process mining, revealing how individual actor behaviors directly impact process performance.
...
Result: Identifies a small set of influential lags showing actor behavior directly impacts process performance, particularly throughput time.

Conclusion: Actor-centric, time series-based methods offer nuanced insights into how individual behaviors drive process outcomes, enhancing process efficiency understanding.
...
Method: Analyzed 10,288 ICU patients from MIMIC-IV, defined kidney injury by KDIGO criteria, selected features via SelectKBest and Random Forest, and tested six algorithms with cross-validation. Interpretability was assessed using SHAP, ALE, and Bayesian analysis.

Result: CatBoost performed best (AUROC 0.818), with key predictors like phosphate and bilirubin. SHAP and ALE provided insights into risk factors and dose-response patterns.
...
<details>
  <summary>Details</summary>
Motivation: To address privacy concerns in thresholding bandit problems by ensuring local differential privacy while identifying high-reward arms.
...
Result: The proposed algorithms provide strong privacy guarantees and match derived lower bounds up to poly-logarithmic factors.
...
<details>
  <summary>Details</summary>
Motivation: Accurate fracture prediction is critical for safety and reliability, but existing methods are fragmented, data-hungry, or computationally expensive.

Method: A transformer-based architecture combines multimodal inputs (structured/unstructured meshes, text embeddings) to adapt flexibly across simulations.

Result: The model generalizes well, requiring minimal data for fine-tuning, and works on unseen materials like titanium and concrete.
...
TL;DR: This study evaluates the performance and energy usage of edge devices (RPi, INCS, NJn, GCU) for AI inferences, focusing on trade-offs between model accuracy, inference time, power, and memory.


<details>
  <summary>Details</summary>
Motivation: The lack of comprehensive studies on performance and energy usage of edge devices for AI inferences motivates this research to aid informed device and model selection.
...
Result: Findings highlight the impact of hardware and framework optimization, along with model parameter tuning, on balancing performance and resource usage.

Conclusion: The study provides insights for practical edge AI deployments by addressing performance-energy trade-offs in device and model selection.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep generative models for graphs often ignore edge weights or fail to model their joint distribution with topology, limiting their applicability to weighted graphs.

Method: BiGG-E extends BiGG to learn a joint distribution over weighted graphs, leveraging sparsity for efficiency.

Result: BiGG-E outperforms benchmarks in capturing weighted graph distributions while remaining scalable.

Conclusion: BiGG-E effectively models weighted graphs, addressing limitations of prior methods.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of missing data in federated learning due to stragglers and user opt-out, which introduces bias and degrades model performance.
...
Conclusion: FLOSS successfully addresses the issue of missing data in federated learning, enhancing model robustness in real-world scenarios.
...
TL;DR: The paper explores how training conditions and input features affect the robustness and generalization of spoken keyword classifiers under OOD conditions, finding noise-aware training improves robustness.
...
Method: Benchmarked neural architectures across evaluation sets, using Fairness (F) and Robustness (R) metrics to quantify noise impact.
...
TL;DR: The paper explores observational multiplicity in probabilistic classification, where multiple models perform similarly but may conflict in predictions. It introduces regret as a measure of arbitrariness and proposes a method to estimate it, highlighting its impact on safety and interpretability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of arbitrariness in probabilistic classification tasks, where competing models can produce conflicting predictions, undermining interpretability and safety.
...
Result: Results show higher regret for certain groups in datasets, and the method demonstrates practical applications in improving safety through abstention and targeted data collection.

Conclusion: The study concludes that estimating regret can enhance safety and interpretability in probabilistic classification tasks, offering actionable insights for real-world applications.
...
TL;DR: Proposes an AI paradigm for solving differential equations (DEs) with a novel data generation method and a scale-dilation operator (SDO) AI solver, addressing high-frequency approximation challenges and achieving superior accuracy.
...
Method: Introduces a first-principles data generation methodology and a reversible SDO AI solver leveraging Fourier transforms and attention-based Transformers to handle multiscale solutions.
...
Conclusion: The proposed AI paradigm makes DE-solving AI truly usable in broad fields by overcoming data scarcity and high-frequency approximation issues.
...
<details>
  <summary>Details</summary>
Motivation: Urban heatwaves, droughts, and land degradation require accurate spatio-temporal LST data, but satellite trade-offs limit resolution.

Method: FuseTen combines Sentinel-2, Landsat 8, and Terra MODIS data using a generative architecture with attention, normalization, and PatchGAN discriminator.

Result: FuseTen improves quantitative metrics by 32.06% and visual fidelity by 31.42% over baselines.

Conclusion: FuseTen is the first non-linear method to achieve daily LST at 10m resolution, addressing critical climate challenges.
...
<details>
  <summary>Details</summary>
Motivation: Practitioners need LLMs to balance inference-time budget, factual authenticity, and reasoning capacity, but no model optimizes all three.

Method: Formal proof of the trade-off and introduction of the BAR Theorem framework.

Result: A principled framework (BAR Theorem) for LLM-application design.

Conclusion: The BAR Theorem provides a structured approach to navigating trade-offs in LLM design.
...
TL;DR: NaN-propagation detects sparsity in black-box functions by exploiting NaN contamination, eliminating false negatives in gradient calculations for faster optimization.


<details>
  <summary>Details</summary>
Motivation: Existing finite-difference methods for sparsity detection suffer from false negatives due to coincidental zero gradients, leading to corrupted gradient calculations.

Method: Systematically contaminates inputs with NaN to trace dependencies, reconstructing conservative sparsity patterns. Uses IEEE 754 compliance for cross-language compatibility.

Result: Achieves a 1.52x speedup in an aerospace wing weight model, detecting missed dependencies.

Conclusion: NaN-propagation improves gradient-based optimization by eliminating false negatives, leveraging IEEE 754 standards for broad applicability.
...
TL;DR: DocsRay is a training-free system for understanding multimodal documents, combining pseudo-TOC generation and hierarchical RAG to improve efficiency and accuracy.
...
Method: Integrates pseudo-TOC generation, zero-shot multimodal analysis, and hierarchical retrieval to process diverse document elements without training.

Result: Reduced query latency by 45% (3.89s to 2.12s) and achieved 64.7% accuracy on MMLongBench-Doc, surpassing prior methods.

Conclusion: DocsRay effectively processes multimodal documents efficiently and accurately without specialized training.
...
TL;DR: A method using a generator-agnostic observer model detects AI hallucinations via a linear probe on its residual stream, outperforming baselines and showing robust performance across model sizes.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of contextual hallucinations in AI-generated text by identifying a low-dimensional, internal signal for detection and mitigation.

Method: A linear probe on the residual stream of an observer model isolates a transferable direction separating hallucinated from faithful text, with localization via gradient-times-activation.

Result: Outperforms baselines by 5-27 points, robust across model sizes (2B to 27B), and causal manipulation of the direction steers hallucination rates.

Conclusion: The study provides evidence of internal hallucination tracking linked to specific MLP sub-circuits, with actionable insights for detection and mitigation.
...
Method: Proposes IAU, an algorithm that reframes unlearning as an incremental learning problem, utilizing gradient optimization for efficiency.
...
Conclusion: The IAU algorithm provides a practical solution for machine unlearning by drawing on incremental learning principles, validated by empirical results.
...
TL;DR: DynaSwarm is a dynamic multi-agent system framework that optimizes collaboration graph structures using reinforcement learning and adaptive graph selection, outperforming static and single-agent baselines.


<details>
  <summary>Details</summary>
Motivation: Current multi-agent systems rely on static collaboration graphs, limiting adaptability and performance. DynaSwarm addresses this by introducing dynamic, sample-specific structures.

Method: DynaSwarm uses an actor-critic reinforcement learning (A2C) mechanism for stable graph optimization and a dynamic graph selector for adaptive structure choice via LLM fine-tuning.
...
Conclusion: Sample-aware structural flexibility is crucial for LLM-based multi-agent systems, as demonstrated by DynaSwarm's success.
...
TL;DR: The paper introduces a framework to analyze privacy leakage dynamics in deep learning, focusing on how and when membership information is encoded during training. It reveals correlations between sample learning difficulty and privacy risk.
...
Result: Samples highly vulnerable in the final model show early privacy risk determination, with a strong correlation between intrinsic learning difficulty and privacy risk.

Conclusion: The findings enhance understanding of privacy risk emergence during training, supporting the development of proactive, privacy-aware training strategies.
...
TL;DR: A neural network layer API for sequence modeling, enabling both layer-by-layer and step-by-step execution with stateful design for correctness and streaming.


<details>
  <summary>Details</summary>
Motivation: To simplify the creation of sequence models that work seamlessly in both training and autoregressive sampling, while ensuring correctness and reducing bugs.
...
Conclusion: SequenceLayers provides a robust, flexible framework for building production-scale sequence models, with implementations available in JAX and TensorFlow 2.
...
<details>
  <summary>Details</summary>
Motivation: Existing NBP methods focus on future purchases but overlook detecting forgotten items and lack transparency in recommendations.

Method: Two interpretable-by-design algorithms are proposed to identify forgotten items with clear explanations.

Result: Experiments on a real-world dataset show a 10-15% improvement over state-of-the-art NBP baselines.

Conclusion: The study successfully addresses the gap in forgotten item prediction with interpretable solutions, enhancing recommendation transparency.
...
<details>
  <summary>Details</summary>
Motivation: Conventional RL approaches for LRMs rely on sparse outcome-only rewards, leading to inefficient optimization. The work aims to enhance training by leveraging process-based rewards.
...
TL;DR: CostCert is a scalable and precise defender for certified recovery in deep learning, outperforming PatchGuard by avoiding pairwise comparisons and combinatorial explosion.
...
Conclusion: CostCert offers a more efficient and accurate solution for certified recovery against adversarial patch attacks.
...
<details>
  <summary>Details</summary>
Motivation: Bike-sharing systems face imbalanced inventory due to varying user demands, requiring dynamic pricing to manage costs and efficiency.

Method: A differentiable agent-based simulation is developed to design dynamic pricing, tested on small (25 stations) and large (289 stations) scales.

Result: The method reduces loss by 73-78% and speeds convergence by over 100x, achieving balanced inventory without manual relocation.

Conclusion: Optimal dynamic pricing can balance bike-sharing inventory efficiently, with minimized discount costs when initial conditions are set appropriately.
...
TL;DR: The paper extends model-based drift explanations to causal explanations to improve actionability in addressing concept drift in systems like industrial manufacturing.


<details>
  <summary>Details</summary>
Motivation: Understanding and explaining concept drift is vital for preventing model failures and real-world errors, necessitating actionable explanations.
...
Result: Evaluation on use cases shows the framework's practical usefulness in enabling targeted interventions.

Conclusion: Causal explanations enhance the actionability of drift explanations, aiding in targeted interventions.
...
TL;DR: PLARE introduces a novel offline RL method using vision-language models (VLMs) to replace manual reward design, achieving competitive performance in robotic tasks.


<details>
  <summary>Details</summary>
Motivation: Offline RL avoids costly online interactions but relies on reward-labeled data, which is expensive to design. PLARE aims to bypass this bottleneck by using VLMs for preference-based guidance.

Method: PLARE queries VLMs for preference labels on visual trajectory segments based on language task descriptions, training policies directly with supervised contrastive preference learning.
...
TL;DR: A ML-based system detects honey adulteration using mineral element profiles, with preprocessing and classification phases. Random forest outperforms other models with 98.37% accuracy.


<details>
  <summary>Details</summary>
Motivation: To develop an accurate method for detecting honey adulteration using mineral element profiles, addressing the need for reliable authentication techniques.

Method: The system involves preprocessing (missing-value treatment, normalization) and classification using logistic regression, decision tree, and random forest models.

Result: Mineral element content effectively discriminates adulterated honey. Random forest achieves the highest accuracy (98.37%).

Conclusion: The proposed ML system, especially random forest, is effective for honey adulteration detection using mineral profiles.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of learning solution operators for time-dependent PDEs efficiently while capturing long-range spatiotemporal dynamics.

Method: Introduces a factorization of spatial and temporal dimensions using structured state-space models, enabling parameter efficiency and flexible modeling.

Result: Outperforms alternatives on PDE benchmarks, achieves competitive performance with fewer parameters, and improves under partial observability.

Conclusion: ST-SSM offers efficient, generalizable PDE modeling with strong theoretical foundations.
...
TL;DR: Coflex is a novel HW-NAS framework using Sparse Gaussian Process and multi-objective Bayesian optimization to efficiently co-optimize neural network performance and hardware energy efficiency.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of extensive search space and high computational cost in HW-NAS for practical adoption.
...
Conclusion: Coflex enables scalable, efficient HW-NAS with high predictive accuracy and reduced overhead.
...
Method: Analyzed 554 T2DM patients over 16.8 years, identified key features, and trained ML models (best: EST). SHAP was used for interpretability.

Result: EST model achieved C-statistic 0.776 and high AUC values (0.86-0.826) for 5-16.8-year predictions.
...
TL;DR: The paper examines when structural uncertainty in causal relationships (e.g., $X \rightarrow Y$ vs. $X \leftarrow Y$) justifies using Bayesian model averaging, identifying key conditions for its benefit.


<details>
  <summary>Details</summary>
Motivation: Practitioners often overlook structural uncertainty in causal inference, which can impact decision-making. This paper aims to address when and how to account for it.

Method: The study analyzes bivariate causal relationships, using Bayesian model averaging over competing structures. It establishes conditions for its effectiveness and proves optimality under regularity conditions.

Result: Model averaging is beneficial under moderate-to-high structural uncertainty, significant effect differences between structures, and sensitive loss functions. Simulations show causal discovery methods can quantify this uncertainty.

Conclusion: The framework addresses a gap in robust causal inference by tackling structural uncertainty, complementing existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Static ensemble aggregation rules in off-policy RL are inflexible and discard valuable information, limiting adaptability to task needs or training phases.
...
Conclusion: DEA provides a data-driven, adaptive approach to ensemble aggregation, enhancing performance in off-policy RL tasks.
...
Method: Proves a Verifier Trade-off Theorem and applies it to analyze complexity class separations and problems like string periodicity.

Result: Shows that reducing verifier time requires longer certificates, forming a hierarchy based on certificate complexity.

Conclusion: The theorem provides insights into complexity class separations and the P vs. NP problem, linking it to sub-linear certificates.
...
Result: With fixed clipping, the method converges faster to a neighborhood of the optimal solution, balancing convergence speed and DP noise.

Conclusion: The work bridges the gap between gradient clipping and DP, offering a refined trade-off for optimization under heavy-tailed noise.
...
TL;DR: A novel CL method, Experience Blending, uses synthetic boundary data and stored key samples to mitigate catastrophic forgetting, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Address limitations of experience replay in CL by improving decision boundary stability with synthetic data.

Method: Proposes Experience Blending: (1) DP noise for synthetic boundary data, (2) joint training with stored samples.

Result: Outperforms 9 CL baselines with accuracy improvements of 10%, 6%, and 13% on CIFAR-10, CIFAR-100, and Tiny ImageNet.
...
TL;DR: The paper discusses the importance of transparency in AI for high-stakes decisions, sharing practical insights and strategies for integrating interpretability into AI systems.
...
Result: Provides actionable strategies and guidance for organizations at different AI maturity levels.
...
TL;DR: The paper benchmarks PEFT methods on convolutional architectures for edge devices, finding varied efficiency and performance compared to LLMs.


<details>
  <summary>Details</summary>
Motivation: To explore the underexplored application of PEFT methods on smaller models like CNNs for edge devices, addressing computational cost and efficiency.

Method: Evaluates LoRA, DoRA, and GaLore on convolutional architectures, using PyTorch profilers to compare performance and computational costs with traditional fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: To address the need for fast processing of attention matrices, particularly in applications like fast attention computation in large language models (LLMs).

Method: Develops algorithms for matrix-vector products under the assumption that the sum of matrix entries scales linearly with n, not quadratically. Validates this assumption experimentally.

Result: Achieves the first subquadratic-time algorithm for unrestricted vectors under the linear scaling assumption.

Conclusion: The proposed method efficiently computes matrix-vector products for Gaussian Kernel matrices, with practical validation in real-world settings.
...
Method: Train SNNs with Q-learning, quantize to 8-bit, deploy on SpiNNaker2. Compare latency, power, and energy to GPU.

Result: 32x energy reduction on SpiNNaker2, latency on par with GPU, improved in some tasks.

Conclusion: SpiNNaker2 is viable for real-time neuromorphic control, offering scalable, low-energy deep Q-learning.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of selecting minimal yet informative feature subsets in high-dimensional settings, capturing inter-feature dependencies often overlooked by greedy methods.

Method: Uses simulated annealing for global search over feature subsets, guided by Fisher discriminant ratio as a proxy for model quality.

Result: Selects compact feature subsets with high predictive accuracy, outperforming greedy approaches.

Conclusion: SA-FDR offers a flexible, effective solution for interpretable models in high-dimensional scenarios where sparsity and performance are key.
...
<details>
  <summary>Details</summary>
Motivation: Existing GraphRAG methods struggle with multi-hop reasoning due to rigid heuristics and underutilized LLM potential.

Method: Uses process-constrained RL (modified GRPO), PRA and CAF rewards, phase-dependent training, and hybrid graph-textual retrieval.

Result: Outperforms state-of-the-art GraphRAG methods on in/out-of-domain datasets, with flexible integration into existing methods.

Conclusion: GraphRAG-R1 significantly improves LLM reasoning for complex problems and is adaptable to various retrieval methods.
...
TL;DR: The paper introduces EB-gMCR, a deep learning-based method for signal unmixing, which automatically identifies the smallest set of components to reconstruct data accurately, outperforming classical matrix factorization approaches.


<details>
  <summary>Details</summary>
Motivation: Classical MCR methods require user-specified component counts and struggle with scalability and reliability for large datasets or high component counts.

Method: The study reformulates MCR as a generative process (gMCR) and uses an energy-based deep learning solver (EB-gMCR) to automatically select active components from a large candidate pool.

Result: EB-gMCR achieved high accuracy (R^2 >= 0.98) in noisy synthetic datasets and recovered component counts within 5% of ground truth.
...
TL;DR: Proposes a novel method combining feudal HRL and message-passing for decentralized MARL, addressing partial observability and non-stationarity.
...
Method: Uses feudal HRL and hierarchical graph structure for planning; lower-level agents receive goals and exchange messages, with reward-assignment based on upper-level advantage.

Result: Outperforms state-of-the-art on benchmarks.

Conclusion: The method effectively combines hierarchical policies and message-passing for scalable MARL.
...
TL;DR: L-GTA is a transformer-based generative model for time series data augmentation, improving reliability and predictive accuracy.


<details>
  <summary>Details</summary>
Motivation: Data augmentation is crucial for time series tasks like forecasting and classification, but existing methods lack reliability and controllability.
...
Conclusion: L-GTA outperforms direct transformation methods, offering significant improvements in time series augmentation.
...
Result: Identifies the expressive properties of softmax attention and its components, explaining its superior accuracy.

Conclusion: Softmax attention's RNN-like structure and component interactions make it more expressive than linear attention.
...
<details>
  <summary>Details</summary>
Motivation: FL in medical settings is vulnerable to attacks and data heterogeneity, requiring robust defenses for privacy and performance.

Method: OptiGradTrust evaluates gradients via a six-dimensional fingerprint and uses a hybrid RL-attention module; FedBN-Prox combines Federated Batch Normalization with proximal regularization.

Result: Outperforms state-of-the-art defenses, achieving +1.6 percentage points over FLGuard under non-IID conditions and robustness against diverse attacks.
...
<details>
  <summary>Details</summary>
Motivation: To explore the underexplored role of feature attribution methods like SHAP in guiding model optimization, aiming to improve both predictive performance and interpretability.

Method: Uses entropy-based penalties to encourage sparse, concentrated feature attributions and stability across samples, applicable to regression and classification tasks. Initially tested with TreeSHAP on tree-based models.

Result: Improves generalization performance on benchmark datasets while ensuring robust and interpretable feature attributions.
...
TL;DR: TweakLLM is a lightweight LLM-based routing architecture that dynamically adapts cached responses to improve efficiency and relevance in high-volume LLM deployments.
...
Conclusion: TweakLLM offers a scalable, resource-efficient caching solution for high-volume LLM deployments without compromising user experience.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion policies are slow due to iterative sampling, limiting responsiveness.

Method: FPMD leverages flow matching models for 1-step sampling, with two variants: flow policy and MeanFlow policy.

Result: FPMD matches diffusion policy performance with hundreds of times fewer function evaluations.
...
<details>
  <summary>Details</summary>
Motivation: Current microbiome data imputation methods struggle with sparsity, noise, and ignoring microbial interdependencies and metadata, limiting downstream applications like biomarker discovery.

Method: DepMicroDiff integrates diffusion-based generative modeling with a Dependency-Aware Transformer (DAT), uses VAE-based pretraining on cancer datasets, and conditions on LLM-encoded patient metadata.

Result: DepMicroDiff achieves superior performance (Pearson correlation up to 0.712, cosine similarity up to 0.812) and lower RMSE/MAE across multiple cancer types.

Conclusion: DepMicroDiff is a robust and generalizable solution for microbiome data imputation, addressing key limitations of existing methods.
...
TL;DR: The paper explores using anomalous samples in training for binary anomaly classification, proposing a multi-score method and validating it with augmentation techniques.


<details>
  <summary>Details</summary>
Motivation: Anomalous data is often scarce, but even small amounts can be impactful. The study questions the reliance on normal samples and investigates the utility of anomalous ones.

Method: Proposes a multi-score anomaly detection method combining Zero-Shot and memory-based techniques, and introduces augmentation-based validation.

Result: Demonstrates effectiveness on industrial datasets, comparing the utility of anomalous vs. normal samples.

Conclusion: Anomalous samples can significantly improve anomaly detection, with augmentation-based validation optimizing score aggregation.
...
<details>
  <summary>Details</summary>
Motivation: Existing AL strategies overlook internal annotator factors (e.g., mood, fatigue), leading to inefficiencies. This study aims to address this gap by integrating these factors to minimize misclassifications.

Method: The study introduces an RS that ranks annotators based on past accuracy, mood, and fatigue levels, simulating realistic annotator behavior. It predicts performance to select the best annotators for queries.

Result: The approach reduces annotation errors and model uncertainty, with improved accuracy and F1-scores, though the latter improvements were less significant.

Conclusion: The study highlights the importance of human cognitive factors in AL and demonstrates the effectiveness of the proposed RS in enhancing annotation efficiency and model performance.
...
TL;DR: SmartCourse is an AI-driven advising system for CS undergraduates, integrating transcript and plan data for personalized recommendations, outperforming context-omitted methods.


<details>
  <summary>Details</summary>
Motivation: Address limitations of traditional advising tools by leveraging student-specific context for better course recommendations.

Method: Combines CLI and Gradio GUI, manages accounts/courses/grading, and uses a locally hosted LLM for personalized advice based on transcripts and major plans.

Result: Full context (transcript + plan) yields more relevant recommendations than context-omitted modes, validated by custom metrics.

Conclusion: Transcript-aware AI significantly improves academic advising, as demonstrated by SmartCourse.
...
TL;DR: ELMES is an open-source framework for evaluating LLMs in education, addressing gaps in pedagogical assessment with modular, flexible design and hybrid evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks lack focus on pedagogical capabilities, and evaluation metrics for educational LLM applications are inconsistent or missing.

Method: ELMES uses a modular architecture for dynamic, multi-agent dialogues and a hybrid evaluation engine with LLM-as-a-Judge methodology.

Result: Benchmarking reveals distinct capability distributions among LLMs in educational scenarios, highlighting context-specific strengths and weaknesses.

Conclusion: ELMES reduces barriers for LLM adoption in education, offering an accessible, adaptable framework for pedagogical evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Manual association of AI failure reports limits scalability and delays pattern identification, necessitating an automated solution.

Method: The task is formalized as a ranking problem using embedding cosine similarity, comparing traditional lexical methods, cross-encoders, and transformer-based models.
...
### [254] [On LLM-Assisted Generation of Smart Contracts from Business Processes](https://arxiv.org/abs/2507.23087)
*Fabian Stiehle,Hans Weytjens,Ingo Weber*

Main category: cs.SE

TL;DR: The paper explores using large language models (LLMs) to generate smart contract code from business process descriptions, addressing limitations of traditional methods. It introduces an automated evaluation framework and tests LLMs on larger datasets, finding their performance insufficient for reliable smart contract development.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of traditional rule-based code generation and explore LLMs for generating smart contract code from business process descriptions.
...
Result: LLM performance is insufficient for perfect reliability in smart contract development, highlighting gaps in enforcing critical execution properties.
...
### [255] [SWE-Debate: Competitive Multi-Agent Debate for Software Issue Resolution](https://arxiv.org/abs/2507.23348)
*Han Li,Yuling Shi,Shaoxin Lin,Xiaodong Gu,Heng Lian,Xin Wang,Yantao Jia,Tao Huang,Qianxiang Wang*

Main category: cs.SE

TL;DR: SWE-Debate is a multi-agent debate framework for issue resolution in software engineering, outperforming existing methods by leveraging diverse reasoning paths and collaborative convergence.


<details>
  <summary>Details</summary>
Motivation: Existing agent-based issue resolution methods often get stuck in local solutions and miss broader issue patterns. SWE-Debate aims to address this by encouraging diverse reasoning.

Method: SWE-Debate creates multiple fault propagation traces, organizes a three-round debate among specialized agents, and integrates the consolidated fix plan into an MCTS-based code modification agent.

Result: Experiments on SWE-bench show SWE-Debate achieves state-of-the-art results, outperforming baselines significantly.

Conclusion: SWE-Debate effectively improves issue resolution by combining competitive multi-agent debate with structured collaboration, setting a new benchmark in the field.
...
### [256] [SWE-Exp: Experience-Driven Software Issue Resolution](https://arxiv.org/abs/2507.23361)
*Silin Chen,Shaoxin Lin,Xiaodong Gu,Yuling Shi,Heng Lian,Longfei Yun,Dong Chen,Weiguo Sun,Lin Cao,Qianxiang Wang*

Main category: cs.SE

TL;DR: SWE-Exp enhances LLM agents for software issue resolution by leveraging past experiences, improving efficiency and success rates.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents lack memory, leading to redundant exploration and missed opportunities for reuse of successful repair methods.

Method: Introduces a multi-faceted experience bank capturing successful and failed repair attempts, enabling continuous learning.
...
### [257] [AutoBridge: Automating Smart Device Integration with Centralized Platform](https://arxiv.org/abs/2507.23178)
*Siyuan Liu,Zhice Yang,Huangxun Chen*

Main category: cs.SE

TL;DR: AutoBridge automates IoT integration code generation, reducing human effort. It uses a divide-and-conquer strategy and multi-stage debugging, achieving high success rates and function coverage.


<details>
  <summary>Details</summary>
Motivation: Reducing the human expertise and effort required for programming complex IoT integration code.

Method: Divide-and-conquer: generates device control logic and synthesizes platform-compliant code, with multi-stage debugging for correctness.

Result: Achieves 93.87% success rate and 94.87% function coverage; reaches 100% with minimal user feedback. Outperforms experts by 50-80% in accuracy.

Conclusion: AutoBridge effectively automates IoT integration, significantly reducing human effort while maintaining high accuracy.
...
### [258] [XABPs: Towards eXplainable Autonomous Business Processes](https://arxiv.org/abs/2507.23269)
...
<details>
  <summary>Details</summary>
Motivation: ABPs improve efficiency but raise issues like trust and accountability, necessitating explainable systems (XABPs).

Method: The paper outlines a systematic approach to XABPs, characterizing their forms and structuring explainability.

Result: Identifies key BPM research challenges for implementing XABPs.

Conclusion: XABPs are essential for mitigating concerns in ABPs, requiring focused research and design efforts.
...
### [259] [Quality Evaluation of COBOL to Java Code Transformation](https://arxiv.org/abs/2507.23356)
...
<details>
  <summary>Details</summary>
Motivation: Address challenges like model opacity and translation quality complexity in LLM-based translators.
...
### [260] [Trae Agent: An LLM-based Agent for Software Engineering with Test-time Scaling](https://arxiv.org/abs/2507.23370)
...
TL;DR: Trae Agent is an agent-based ensemble reasoning approach for repository-level software issue resolution, outperforming existing methods with a 10.22% average improvement in Pass@1.


<details>
  <summary>Details</summary>
Motivation: Existing prompting-based methods for LLM-based issue resolution struggle with large ensemble spaces and lack repository-level understanding, limiting effectiveness.

Method: Trae Agent uses modular agents for generation, pruning, and selection to address large ensemble spaces and repository-level understanding.

Result: Trae Agent achieves a 75.20% Pass@1 score, outperforming four state-of-the-art techniques by 10.22% on average.

Conclusion: Trae Agent is released as open-source, demonstrating superior performance in repository-level issue resolution.
...
### [261] [Noise-Coded Illumination for Forensic and Photometric Video Analysis](https://arxiv.org/abs/2507.23002)
*Peter F. Michael,Zekun Hao,Serge Belongie,Abe Davis*

Main category: cs.GR

TL;DR: The paper proposes a method to combat video manipulation by embedding subtle, noise-like modulations into scene illumination, creating an asymmetry favoring verification.


<details>
  <summary>Details</summary>
Motivation: The rise of sophisticated video manipulation tools has made fake videos harder to detect, necessitating new verification methods.

Method: The approach involves coding subtle illumination modulations to create a temporal watermark, encoding an unmanipulated scene image.

Result: This method makes creating plausible fake videos more difficult, even if adversaries are aware of the technique.

Conclusion: The technique is promising for high-stakes settings where illumination can be controlled but cameras cannot.
...
### [262] [XSpecMesh: Quality-Preserving Auto-Regressive Mesh Generation Acceleration via Multi-Head Speculative Decoding](https://arxiv.org/abs/2507.23777)
*Dian Chen,Yansong Qu,Xinyang Li,Ming Li,Shengchuan Zhang*

Main category: cs.GR

TL;DR: XSpecMesh accelerates auto-regressive mesh generation by using speculative decoding and verification, achieving 1.7x speedup without quality loss.


<details>
  <summary>Details</summary>
Motivation: Auto-regressive models for mesh generation are slow due to sequential token predictions. XSpecMesh aims to reduce latency while preserving quality.
...
Result: Achieves 1.7x speedup in inference without compromising mesh quality.

Conclusion: XSpecMesh effectively balances speed and quality in auto-regressive mesh generation.
...
### [263] [Exploring Dynamic Parameters for Vietnamese Gender-Independent ASR](https://arxiv.org/abs/2507.22964)
...
Method: Characterized acoustic transitions in SSCFs using polar parameters, combined with MFCCs, and used SSCF0 as a pseudo-feature for F0 to robustly describe tonal information.

Result: Significant reduction in word error rates and greater gender independence compared to baseline MFCCs.

Conclusion: The proposed method effectively enhances ASR performance by leveraging dynamic speech characteristics and tonal information.
...
### [264] [MECAT: A Multi-Experts Constructed Benchmark for Fine-Grained Audio Understanding Tasks](https://arxiv.org/abs/2507.23511)
*Yadong Niu,Tianzi Wang,Heinrich Dinkel,Xingwei Sun,Jiahao Zhou,Gang Li,Jizhong Liu,Xunying Liu,Junbo Zhang,Jian Luan*

Main category: eess.AS

TL;DR: The paper introduces MECAT, a benchmark for fine-grained audio understanding, and a novel metric (DATE) to evaluate detailed model outputs, addressing limitations in current benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks fail to distinguish between generic and detailed audio model outputs, limiting nuanced comprehension.
...
Result: The benchmark and metric reveal insights into state-of-the-art audio models' capabilities and limitations.

Conclusion: MECAT and DATE address gaps in audio understanding benchmarks, enhancing evaluation of detailed model outputs.
...
### [265] [DNN-based Methods of Jointly Sensing Number and Directions of Targets via a Green Massive H2AD MIMO Receiver](https://arxiv.org/abs/2507.22906)
*Bin Deng,Jiatong Bai,Feilong Zhao,Zuming Xie,Maolin Li,Yan Wang,Feng Shu*

Main category: eess.SP

TL;DR: A two-stage sensing framework for H2AD MIMO addresses multi-emitter sensing challenges, using EDC, DNN, and 1D-CNN for target number estimation and OMC-DOA for DOA estimation, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To solve the open problem of intelligently sensing multi-emitters in H2AD MIMO, addressing high energy, cost, and complexity issues of fully-digital MIMO.

Method: Proposes a two-stage framework: three target number sensing methods (EDC, DNN, 1D-CNN) and OMC-DOA for DOA estimation, with CRLB as a benchmark.

Result: 100% target sensing at moderate-high SNRs; 1D-CNN excels at low SNR. OMC-DOA outperforms existing DOA methods in multi-source scenarios.

Conclusion: The framework effectively addresses H2AD MIMO's sensing challenges, offering high accuracy and low complexity.
...
### [266] [On the Complexity of Finding Stationary Points in Nonconvex Simple Bilevel Optimization](https://arxiv.org/abs/2507.23155)
...
### [267] [Extended Factorization Machine Annealing for Rapid Discovery of Transparent Conducting Materials](https://arxiv.org/abs/2507.23160)
*Daisuke Makino,Tatsuya Goto,Yoshinori Suga*

Main category: cond-mat.mtrl-sci

TL;DR: The paper introduces an extended FMA framework combining Factorization Machines and annealing to efficiently search for optimal compositions and crystal structures in the (Al$_x$Ga$_y$In$_z$)$_2$O$_3$ system, outperforming Bayesian optimization and genetic algorithms.


<details>
  <summary>Details</summary>
Motivation: The need for cost-effective and high-performance transparent conducting materials (TCMs) for next-generation devices like solar cells and displays drives this research.

Method: The method includes binarization of continuous variables, Hopfield network utilization, adaptive random flips for global search, and bit-string local search for fine-tuning.

Result: The method demonstrated faster and more accurate searches than Bayesian optimization and genetic algorithms, and successfully handled multi-objective optimization for material design.

Conclusion: The framework shows promise for larger, complex material design problems, advancing materials informatics.
...
### [268] [Simulation-based inference for Precision Neutrino Physics through Neural Monte Carlo tuning](https://arxiv.org/abs/2507.23297)
...
TL;DR: The paper proposes neural likelihood estimation for modeling detector energy response in neutrino experiments, using conditional normalizing flows and a transformer-based regressor, validated with JUNO as a case study.


<details>
  <summary>Details</summary>
Motivation: Accurate modeling of detector energy response is critical for next-generation neutrino experiments, but computational challenges arise due to the lack of analytical likelihoods.

Method: Two neural density estimators (conditional normalizing flows and a transformer-based regressor) are developed to model likelihoods of calibration data, integrated with Bayesian nested sampling for parameter inference.

Result: The framework achieves uncertainties limited by statistics with near-zero systematic biases, offering flexibility for unbinned or binned likelihood analysis.

Conclusion: The approach provides a template for similar applications in neutrino and broader particle physics experiments.
...
TL;DR: A federated learning framework with quantum-enhanced LSTM and privacy techniques improves fraud detection by 5% and security by 4-8%.


<details>
  <summary>Details</summary>
Motivation: Addressing the rise in digital fraud and limitations of traditional detection methods.

Method: Combines quantum-enhanced LSTM with privacy-preserving techniques (FedRansel) to detect fraud and defend against attacks.

Result: 5% performance boost in fraud detection; 4-8% reduction in model degradation and inference accuracy.

Conclusion: The framework enhances fraud detection accuracy and secures financial data effectively.
...
### [270] [Typing Tensor Calculus in 2-Categories (I)](https://arxiv.org/abs/1908.01212)
*Fatimah Rita Ahmadi*
...
### [271] [Beyond Rigid AI: Towards Natural Human-Machine Symbiosis for Interoperative Surgical Assistance](https://arxiv.org/abs/2507.23088)
...
TL;DR: The paper introduces a Perception Agent for natural human-machine interaction in surgery, using LLMs, SAM, and tracking models to segment known and unseen elements flexibly.


<details>
  <summary>Details</summary>
Motivation: Current AI-driven surgical solutions are rigid and lack natural interaction, requiring extensive pre-training and manual prompting.

Method: The Perception Agent integrates speech, LLMs, SAM, and tracking models with a memory repository and novel segmentation mechanisms.
...
Conclusion: The Perception Agent advances human-machine symbiosis in surgery by enabling intuitive, real-time assistance.
...
### [272] [Learning to Prune Branches in Modern Tree-Fruit Orchards](https://arxiv.org/abs/2507.23015)
*Abhinav Jain,Cindy Grimm,Stefan Lee*

Main category: cs.RO

TL;DR: A closed-loop visuomotor controller for robotic pruning in orchards is presented, using optical flow images and achieving 30% success rate in real-world tests.


<details>
  <summary>Details</summary>
Motivation: Dormant tree pruning is labor-intensive, and robotic solutions can improve efficiency in modern orchards.

Method: The controller uses optical flow images from a wrist-mounted camera, trained in a novel orchard simulation, to guide the cutter to specified cut points.

Result: The system achieved a 30% success rate in real-world deployment, half the performance of an oracle planner.

Conclusion: The approach shows promise for robotic pruning, though further improvements are needed to match human or oracle performance.
...
### [273] [Multi-Waypoint Path Planning and Motion Control for Non-holonomic Mobile Robots in Agricultural Applications](https://arxiv.org/abs/2507.23350)
*Mahmoud Ghorab,Matthias Lorenzen*

Main category: cs.RO

TL;DR: An integrated navigation framework for agricultural robots combines DTSP for global path planning and NMPC for local control, reducing path length by 16% and improving efficiency.


<details>
  <summary>Details</summary>
Motivation: Autonomous robots are needed for tasks like weed control in unstructured agricultural environments, requiring efficient, curvature-constrained navigation.

Method: The framework uses DTSP for global path planning (minimizing travel distance with curvature constraints) and NMPC for local control and trajectory optimization.

Result: Simulations showed smoother, shorter paths (16% reduction) and accurate waypoint tracking with NMPC.

Conclusion: The framework is effective for autonomous navigation in agriculture, balancing efficiency and constraint adherence.
...
### [274] [H-RDT: Human Manipulation Enhanced Bimanual Robotic Manipulation](https://arxiv.org/abs/2507.23523)
*Hongzhe Bi,Lingxuan Wu,Tianwei Lin,Hengkai Tan,Zhizhong Su,Hang Su,Jun Zhu*

Main category: cs.RO

TL;DR: H-RDT leverages human manipulation data to enhance robot learning, outperforming existing methods by 13.9% (simulation) and 40.5% (real-world).


<details>
  <summary>Details</summary>
Motivation: Scarcity of large-scale robot demonstration data and challenges in cross-embodiment training motivate the use of human data as behavioral priors.

Method: Two-stage training: pre-training on human egocentric videos with 3D hand poses, then fine-tuning on robot data using modular encoders/decoders and a diffusion transformer.

Result: H-RDT achieves significant improvements over baselines (13.9% in simulation, 40.5% in real-world) and outperforms Pi0 and RDT.

Conclusion: Human manipulation data is a powerful foundation for bimanual robotic policy learning, validated by H-RDT's success.
...
### [275] [GSFusion:Globally Optimized LiDAR-Inertial-Visual Mapping for Gaussian Splatting](https://arxiv.org/abs/2507.23273)
*Jaeseok Park,Chanoh Park,Minsu Kim,Soohwan Kim*

Main category: cs.RO

TL;DR: GSFusion integrates LiDAR, inertial, and visual data for high-quality 3D mapping, overcoming limitations of traditional 3DGS methods with efficient initialization and optimization.


<details>
  <summary>Details</summary>
Motivation: Traditional 3DGS methods face issues like high computational load, poor performance in low-texture/lighting, and short ranges. LiDAR offers robustness but introduces alignment and optimization challenges.

Method: GSFusion uses a surfel-to-surfel constraint for global pose-graph optimization, pixel-aware Gaussian initialization for sparse data, and a bounded sigmoid constraint to control Gaussian growth.
...
Conclusion: GSFusion effectively addresses LiDAR-3DGS integration challenges, offering superior performance in photorealistic mapping.
...
### [276] [villa-X: Enhancing Latent Action Modeling in Vision-Language-Action Models](https://arxiv.org/abs/2507.23682)
...
TL;DR: The paper introduces villa-X, a Visual-Language-Latent-Action (ViLLA) framework that enhances latent action modeling for generalizable robot manipulation policies, achieving superior performance in simulated and real-world environments.


<details>
  <summary>Details</summary>
Motivation: To improve the learning and incorporation of latent actions in Visual-Language-Action (VLA) models for better generalization in robot manipulation tasks.

Method: The villa-X framework advances latent action modeling by refining how latent actions are learned and integrated into VLA pre-training.

Result: villa-X outperforms existing methods in simulated environments (SIMPLER, LIBERO) and real-world setups (gripper and dexterous hand manipulation).

Conclusion: The ViLLA paradigm, exemplified by villa-X, shows significant promise and lays a strong foundation for future research in robot manipulation.
...
### [277] [A Unified Perception-Language-Action Framework for Adaptive Autonomous Driving](https://arxiv.org/abs/2507.23540)
*Yi Zhang,Erik Leo Haß,Kuo-Yi Chao,Nenad Petrovic,Yinglei Song,Chengdong Wu,Alois Knoll*

Main category: cs.RO

TL;DR: A unified Perception-Language-Action (PLA) framework integrates multi-sensor fusion and LLM-augmented reasoning to improve adaptability and interpretability in autonomous driving.
...
Method: Proposes a PLA framework combining multi-sensor fusion (cameras, LiDAR, radar) with a GPT-4.1-powered reasoning core for context-aware decision-making.
...
Conclusion: Language-augmented cognitive frameworks enhance safety, interpretability, and scalability in autonomous driving.
...
### [278] [Can LLM-Reasoning Models Replace Classical Planning? A Benchmark Study](https://arxiv.org/abs/2507.23589)
*Kai Goebel,Patrik Zips*

Main category: cs.RO

TL;DR: The paper evaluates state-of-the-art language models for robotic task planning, comparing their performance with classical planners. While effective for simpler tasks, they struggle with complex scenarios requiring precision and constraint compliance.
...
Method: Systematic evaluation of LLMs using Planning Domain Definition Language (PDDL) prompts, comparing their performance with the Fast Downward planner across various benchmarks. Success rates and plan executability were measured.

Result: LLMs perform well on simpler tasks but struggle with complex scenarios needing precise resource management, state tracking, and constraint compliance.

Conclusion: The study highlights challenges in applying LLMs to robotic planning and suggests integrating them with classical planners for improved reliability and scalability.
...
### [279] [User Experience Estimation in Human-Robot Interaction Via Multi-Instance Learning of Multimodal Social Signals](https://arxiv.org/abs/2507.23544)
...
<details>
  <summary>Details</summary>
Motivation: Accurate UX assessment in HRI is essential for adaptive robot behaviors, but existing methods often overlook multi-faceted UX aspects.
...
Result: The model outperforms human evaluators in UX estimation by leveraging temporal dynamics.

Conclusion: The proposed method offers a more comprehensive UX representation, enhancing adaptability in social robots.
...
### [280] [Scalable Multi-Task Reinforcement Learning for Generalizable Spatial Intelligence in Visuomotor Agents](https://arxiv.org/abs/2507.23698)
...
<details>
  <summary>Details</summary>
Motivation: Overcoming RL models' tendency to overfit specific tasks or environments, hindering generalizable behaviors.

Method: Cross-view goal specification for multi-task RL representation, automated task synthesis in Minecraft, and a distributed RL framework.
...
Conclusion: RL training in 3D simulated environments like Minecraft holds immense potential for advancing visuomotor agents' spatial reasoning.
...
### [281] [Distributed AI Agents for Cognitive Underwater Robot Autonomy](https://arxiv.org/abs/2507.23735)
*Markus Buchholz,Ignacio Carlucho,Michele Grimaldi,Yvan R. Petillot*

Main category: cs.RO

TL;DR: UROSA is a decentralized AI architecture for underwater robots, using ROS 2 and LLMs to enhance autonomy, adaptability, and reliability in unpredictable environments.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of robust cognitive autonomy in robots navigating complex, unpredictable underwater environments.

Method: UROSA decentralizes cognition into specialized AI agents for perception, reasoning, planning, and decision-making, leveraging ROS 2, vector databases, reinforcement learning, and dynamic node generation.

Result: Demonstrated adaptability and reliability in simulations and real-world missions, outperforming rule-based systems in handling uncertainties and novel objectives.

Conclusion: UROSA advances underwater autonomy and provides a scalable, versatile framework for broader robotics applications.
...
### [282] [Evaluating LLMs for Visualization Generation and Understanding](https://arxiv.org/abs/2507.22890)
...
<details>
  <summary>Details</summary>
Motivation: To explore the potential of LLMs in automating visualization tasks and understanding their capabilities and limitations.

Method: Evaluated popular LLMs by testing their ability to generate visualization code from prompts and answer questions about common visualizations.

Result: LLMs succeeded in simpler tasks but faltered with complex visualizations and detailed questions.

Conclusion: The findings highlight LLMs' current limitations and potential for improving both LLMs and visualization systems.
...
### [283] [Invisible Architectures of Thought: Toward a New Science of AI as Cognitive Infrastructure](https://arxiv.org/abs/2507.22893)
*Giuseppe Riva*

Main category: cs.HC

TL;DR: The paper introduces Cognitive Infrastructure Studies (CIS) to analyze how AI systems reshape human cognition pre-consciously, focusing on their role as 'cognitive infrastructures' that influence knowledge and action in digital societies.


<details>
  <summary>Details</summary>
Motivation: Current research overlooks AI's pre-conscious impact on human cognition, creating a blind spot in understanding distributed cognition. CIS aims to fill this gap by reconceptualizing AI as cognitive infrastructures.

Method: CIS employs narrative scenarios at individual, collective, and societal scales and introduces 'infrastructure breakdown methodologies' to study AI's invisible influence by withdrawing AI preprocessing after habituation.

Result: The framework highlights how AI automates relevance judgment, shifts epistemic agency, and reshapes cognition, public reasoning, and social epistemologies across scales.

Conclusion: CIS bridges gaps in cognitive science, digital sociology, and computational approaches, offering interdisciplinary methods to study AI's pervasive but invisible influence on cognition.
...
### [284] [iLearnRobot: An Interactive Learning-Based Multi-Modal Robot with Continuous Improvement](https://arxiv.org/abs/2507.22896)
*Kohou Wang,ZhaoXiang Liu,Lin Bai,Kun Fan,Xiang Liu,Huan Hu,Kai Wang,Shiguo Lian*

Main category: cs.HC

TL;DR: An interactive learning-based robot system using a Multi-modal Large Language Model (MLLM) improves adaptability by learning from natural dialogues with non-expert users, avoiding repeated mistakes through dual-modality retrieval and clarifying intent with a chain of questions.


<details>
  <summary>Details</summary>
Motivation: Robots often face novel scenarios post-deployment, necessitating systems that can adapt and improve performance without expert intervention.

Method: The system leverages MLLM for interactive learning, uses a chain of questions to clarify user intent, and employs dual-modality retrieval to learn from interactions.

Result: Experiments show quantitative and qualitative improvements in adaptability and performance.

Conclusion: The proposed system enhances robot adaptability in diverse environments through interactive learning, outperforming current MLLM-based approaches.
...
### [285] [RecUserSim: A Realistic and Diverse User Simulator for Evaluating Conversational Recommender Systems](https://arxiv.org/abs/2507.22897)
*Luyu Chen,Quanyu Dai,Zeyu Zhang,Xueyang Feng,Mingyu Zhang,Pengcheng Tang,Xu Chen,Yue Zhu,Zhenhua Dong*

Main category: cs.HC

TL;DR: RecUserSim is an LLM-based user simulator for conversational recommender systems, enhancing realism, diversity, and explicit ratings for evaluation.


<details>
  <summary>Details</summary>
Motivation: Evaluating CRS is challenging due to lack of realistic and diverse user simulators. Existing LLM-based simulators struggle with individual user emulation and explicit ratings.

Method: RecUserSim uses modules for user personas, interaction memory, bounded rationality-inspired decision-making, and response refinement.

Result: RecUserSim produces diverse, controllable, and realistic dialogues, with consistent ratings across LLMs.

Conclusion: RecUserSim effectively addresses gaps in CRS evaluation, offering improved realism and quantitative metrics.
...
### [286] [Tool or Trouble? Exploring Student Attitudes Toward AI Coding Assistants](https://arxiv.org/abs/2507.22900)
...
<details>
  <summary>Details</summary>
Motivation: To understand how AI tools impact novice programmers' learning and confidence in a programming course.

Method: Two-part exam: one with AI support, one without. Collected Likert-scale and open-ended responses from 20 students.

Result: AI tools boosted confidence initially but caused difficulties in unaided tasks, indicating overreliance and knowledge gaps.
...
### [287] [Toward the Autonomous AI Doctor: Quantitative Benchmarking of an Autonomous Agentic AI Versus Board-Certified Clinicians in a Real World Setting](https://arxiv.org/abs/2507.22902)
*Hashim Hayat,Maksim Kudrautsau,Evgeniy Makarov,Vlad Melnichenko,Tim Tsykunou,Piotr Varaksin,Matt Pavelle,Adam Z. Oskowitz*

Main category: cs.HC

TL;DR: A multi-agent LLM-based AI system, Doctronic, matched human clinicians in 81% of diagnoses and 99.2% of treatment plans in urgent-care telehealth, with no clinical hallucinations. AI outperformed humans in 36.1% of discordant cases.


<details>
  <summary>Details</summary>
Motivation: Address the global healthcare practitioner shortage and reduce administrative burden by evaluating an autonomous AI doctor in real-world clinical practice.

Method: Retrospective comparison of Doctronic and clinicians across 500 urgent-care telehealth cases, assessing diagnostic concordance, treatment consistency, and safety via blinded LLM adjudication and expert review.

Result: 81% diagnostic match, 99.2% treatment alignment, no hallucinations. AI was superior in 36.1% of discordant cases, humans in 9.3%.

Conclusion: Autonomous AI doctors can match or exceed human clinicians, offering a viable solution to healthcare workforce shortages.
...
### [288] [SketchMind: A Multi-Agent Cognitive Framework for Assessing Student-Drawn Scientific Sketches](https://arxiv.org/abs/2507.22904)
*Ehsan Latif,Zirak Khan,Xiaoming Zhai*

Main category: cs.HC

TL;DR: SketchMind is a multi-agent framework for evaluating student-drawn scientific sketches, offering personalized feedback and outperforming baseline models in accuracy and human evaluations.


<details>
  <summary>Details</summary>
Motivation: Existing AI solutions for sketch evaluation lack interpretability, pedagogical alignment, and adaptability across cognitive levels.

Method: SketchMind uses modular agents for rubric parsing, sketch perception, cognitive alignment, and iterative feedback, integrating SRG for enhanced performance.

Result: SketchMind achieves 77.1% average accuracy (+21.4% over baselines) and human evaluators rate its feedback significantly higher (4.1/5 vs. 2.3/5).

Conclusion: SketchMind demonstrates potential to support conceptual growth through guided revision, with plans to release code and dataset for reproducibility.
...
### [289] [Hybrid EEG--Driven Brain--Computer Interface: A Large Language Model Framework for Personalized Language Rehabilitation](https://arxiv.org/abs/2507.22892)
*Ismail Hossain,Mridul Banik*

Main category: cs.HC

TL;DR: A hybrid EEG-BCI and LLM framework for adaptive AAC systems, enhancing language rehabilitation for users with neurological impairments.


<details>
  <summary>Details</summary>
Motivation: Existing AAC systems lack real-time adaptation to users' cognitive and linguistic needs, especially in neurological conditions like aphasia or ALS.

Method: Combines EEG-based BCIs for neural intent detection with LLMs for contextual language generation, enabling real-time personalization and task adjustment.
...
### [290] [Voice-guided Orchestrated Intelligence for Clinical Evaluation (VOICE): A Voice AI Agent System for Prehospital Stroke Assessment](https://arxiv.org/abs/2507.22898)
...
TL;DR: An AI system for stroke evaluation via voice interaction and smartphone video was tested, showing promising accuracy but requiring human oversight due to some errors.


<details>
  <summary>Details</summary>
Motivation: Addressing inconsistent and often inaccurate stroke recognition by first responders, which causes treatment delays.

Method: Developed a voice-driven AI system for stroke evaluation, tested by non-medical volunteers on simulated cases, measuring accuracy, time, user confidence, and expert review.

Result: AI identified 84% of stroke signs, 75% of LVOs, and 86% of actual strokes, but had some false positives. Expert review improved diagnosis accuracy but highlighted AI errors.

Conclusion: While current limitations require human oversight, advancements in AI suggest future potential for highly accurate, expert-level assessments in emergency care.
...
### [291] [Automated Label Placement on Maps via Large Language Models](https://arxiv.org/abs/2507.22952)
*Harry Shomer,Jiejun Xu*

Main category: cs.HC

TL;DR: The paper introduces a new method for automatic label placement (ALP) using LLMs, backed by the MAPLE dataset, showing LLMs can achieve expert-level accuracy with structured prompts.


<details>
  <summary>Details</summary>
Motivation: Manual label placement is time-consuming and lacks scalability; existing automated systems fail to adapt to cartographic conventions or context.

Method: Formulates ALP as a data editing problem, uses LLMs with retrieval-augmented generation (RAG) to integrate labeling guidelines, and evaluates four LLMs on the MAPLE dataset.

Result: LLMs, guided by structured prompts and domain-specific retrieval, perform accurate spatial edits aligning with expert standards.

Conclusion: The work presents a scalable AI-assisted framework for map finishing and highlights the potential of foundation models in structured data editing.
...
### [292] [Accessibility Scout: Personalized Accessibility Scans of Built Environments](https://arxiv.org/abs/2507.23190)
...
Method: Accessibility Scout employs LLMs to analyze photos of built environments, tailoring scans to users' mobility levels and preferences through Human-AI collaboration.
...
### [293] [Automated Feedback on Student-Generated UML and ER Diagrams Using Large Language Models](https://arxiv.org/abs/2507.23470)
...
<details>
  <summary>Details</summary>
Motivation: Traditional teaching methods struggle with scalable, personalized feedback for UML and ER diagrams, which require abstract thinking and contextual understanding.

Method: DUET uses a multi-stage LLM pipeline to compare student-submitted and reference diagrams, generating reflective feedback and analytical insights.

Result: Evaluation with educators and teaching assistants highlighted strengths like accessibility and scalability, but noted limitations such as reliability and potential misuse.

Conclusion: DUET shows promise for integrating LLMs into modeling education, with potential for future classroom use and empirical evaluation.
...
### [294] [Digital literacy interventions can boost humans in discerning deepfakes](https://arxiv.org/abs/2507.23492)
*Dominique Geissler,Claire Robertson,Stefan Feuerriegel*

Main category: cs.HC

TL;DR: The paper evaluates five digital literacy interventions to improve deepfake detection, showing a 13% boost in discernment while maintaining trust in real images.
...
Method: Tested five interventions (textual guidance, visual demonstrations, gamified exercise, implicit learning, and AI explanations) with 1,200 U.S. participants.

Result: Interventions improved deepfake detection by up to 13% without reducing trust in real images.
...
### [295] ["I made this (sort of)": Negotiating authorship, confronting fraudulence, and exploring new musical spaces with prompt-based AI music generation](https://arxiv.org/abs/2507.23365)
*Bob L. T. Sturm*
...
<details>
  <summary>Details</summary>
Motivation: To investigate the creative process and authorship in AI-generated music, questioning human identity and AI's impact on musical expression.

Method: Created two albums using prompt-based AI music platforms, then used an LLM to interview the author about the experience.

Result: Raised questions about authorship, human identity in AI-generated music, and the limitations of AI in producing "unpolished" music.

Conclusion: Reflects on the role of AI in music creation and the potential of LLM-mediated self-reflection as a method for deeper analysis.
...
### [296] [An Information Bottleneck Asset Pricing Model](https://arxiv.org/abs/2507.23218)
*Che Sun*

Main category: cs.CE

TL;DR: Proposes an information bottleneck asset pricing model to filter noise and retain critical financial data for better DNN performance.


<details>
  <summary>Details</summary>
Motivation: Addresses over-fitting in DNNs for financial asset pricing by eliminating redundant information.

Method: Uses mutual information constraints to compress data, reducing noise while preserving key pricing signals.

Result: Improves asset pricing accuracy by filtering noise and retaining essential nonlinear relationships.

Conclusion: The model enhances DNN performance in financial data by balancing information compression and noise reduction.
...
### [297] [Adjoint-Based Aerodynamic Shape Optimization with a Manifold Constraint Learned by Diffusion Models](https://arxiv.org/abs/2507.23443)
*Long Chen,Emre Oezkaya,Jan Rottmayer,Nicolas R. Gauger,Zebang Shen,Yinyu Ye*

Main category: cs.CE

TL;DR: An adjoint-based aerodynamic shape optimization framework integrates a diffusion model to learn viable shapes, using adjoint gradients for optimization, achieving superior performance.


<details>
  <summary>Details</summary>
Motivation: To enhance aerodynamic shape optimization by integrating AI-generated priors with adjoint methods, eliminating ad hoc tuning and improving robustness.

Method: Uses a diffusion model trained on existing designs to constrain the optimization problem, computes adjoint gradients via automatic differentiation, and applies them in transonic RANS airfoil design.

Result: Eliminates parameter tuning, maintains robustness, and outperforms conventional methods in aerodynamic performance.
...
### [298] [Rethink Domain Generalization in Heterogeneous Sequence MRI Segmentation](https://arxiv.org/abs/2507.23110)
...
TL;DR: PancreasDG is a multi-center 3D MRI dataset for pancreas segmentation, addressing domain generalization challenges in medical imaging, including cross-center and cross-sequence variations. It outperforms existing methods with a semi-supervised approach.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks overlook variability in MR sequences and under-represent the pancreas, despite its clinical importance. PancreasDG aims to address this gap.

Method: The dataset includes 563 MRI scans from six institutions, with pixel-accurate masks. A semi-supervised approach leverages anatomical invariances.
...
### [299] [Towards High-Resolution Alignment and Super-Resolution of Multi-Sensor Satellite Imagery](https://arxiv.org/abs/2507.23150)
...
<details>
  <summary>Details</summary>
Motivation: Addressing challenges in data fusion due to differing spatial resolutions and spectral/temporal characteristics of satellite sensors.

Method: Developed a preliminary framework using HLS10 as reference to align and harmonize HLS30 imagery.

Result: Quantitative and qualitative evaluations show improved super-resolved Landsat imagery.

Conclusion: The study demonstrates feasibility and highlights key considerations for future advancements in heterogeneous satellite image super-resolution.
...
### [300] [LesionGen: A Concept-Guided Diffusion Model for Dermatology Image Synthesis](https://arxiv.org/abs/2507.23001)
*Jamil Fayyad,Nourhan Bayasi,Ziyang Yu,Homayoun Najjaran*

Main category: eess.IV

TL;DR: LesionGen, a T2I-DPM framework, synthesizes realistic skin lesion images using structured dermatological captions, achieving classification accuracy comparable to real images.


<details>
  <summary>Details</summary>
Motivation: Limited datasets for skin disease classification due to privacy, cost, and demographic gaps; T2I-DPMs underutilized in dermatology.

Method: Fine-tune pretrained diffusion model on expert-annotated and pseudo-generated dermatological captions to generate diverse, realistic images.

Result: Synthetic dataset achieves comparable classification accuracy to real images, with improved worst-case subgroup performance.

Conclusion: LesionGen demonstrates the potential of T2I-DPMs in dermatology for data augmentation and addressing dataset limitations.
...
### [301] [MRpro - open PyTorch-based MR reconstruction and processing package](https://arxiv.org/abs/2507.23129)
...
TL;DR: MRpro is an open-source PyTorch-based framework for MR image reconstruction, offering unified data structures, composable operators, and deep learning tools to enhance reproducibility and collaboration.


<details>
  <summary>Details</summary>
Motivation: To provide a versatile, open-source tool for MR image reconstruction that supports diverse applications and fosters collaborative research.

Method: MRpro integrates unified data structures, a library of operators and algorithms, and deep learning components like data consistency layers and backbone networks.

Result: Demonstrated effectiveness in various applications, including Cartesian, radial, and spiral acquisitions, motion correction, and quantitative MR.

Conclusion: MRpro is an extensible, reproducible framework that supports collaborative development and advances MR imaging research.
...
### [302] [Learning Arbitrary-Scale RAW Image Downscaling with Wavelet-based Recurrent Reconstruction](https://arxiv.org/abs/2507.23219)
*Yang Ren,Hai Jiang,Wei Li,Menglong Yang,Heng Zhang,Zehua Sheng,Qingsheng Ye,Shuaicheng Liu*

Main category: eess.IV

TL;DR: A wavelet-based framework for RAW image downscaling preserves details and avoids artifacts, outperforming existing methods.
...
Method: Uses wavelet transformation with Low-Frequency Arbitrary-Scale Downscaling Module (LASDM) and High-Frequency Prediction Module (HFPM), plus an energy-maximization loss.

Result: Outperforms state-of-the-art methods quantitatively and visually.

Conclusion: The proposed framework effectively downscales RAW images while preserving structural and textural integrity.
...
### [303] [EMedNeXt: An Enhanced Brain Tumor Segmentation Framework for Sub-Saharan Africa using MedNeXt V2 with Deep Supervision](https://arxiv.org/abs/2507.23256)
...
TL;DR: EMedNeXt, an enhanced brain tumor segmentation framework, addresses challenges in sub-Saharan Africa by improving segmentation accuracy and robustness despite resource constraints and lower-quality MRI scans.


<details>
  <summary>Details</summary>
Motivation: Manual MRI segmentation for gliomas is time-consuming and requires expert radiologists, which is infeasible in under-resourced regions like sub-Saharan Africa.

Method: EMedNeXt enhances MedNeXt V2 with deep supervision, optimized post-processing, a larger ROI, an improved nnU-Net v2-based architecture, and robust model ensembling.

Result: Achieved an average LesionWise DSC of 0.897 and NSD of 0.541 (0.5 mm tolerance) and 0.84 (1.0 mm tolerance) on validation.

Conclusion: EMedNeXt offers a promising solution for robust brain tumor segmentation in resource-limited settings.
...
### [304] [Pixel Embedding Method for Tubular Neurite Segmentation](https://arxiv.org/abs/2507.23359)
...
<details>
  <summary>Details</summary>
Motivation: Handling large-scale neuroimaging data requires efficient neuron annotation and analysis, but existing methods struggle with occlusions and complex neuronal branch structures.

Method: Proposes a deep network for pixel-level embedding vectors, an end-to-end pipeline for mapping images to neuron structure trees (SWC format), and a novel topological assessment metric.
...
Conclusion: The framework effectively improves segmentation accuracy and introduces a better evaluation metric for neuronal topology reconstruction.
...
### [305] [Smart Video Capsule Endoscopy: Raw Image-Based Localization for Enhanced GI Tract Investigation](https://arxiv.org/abs/2507.23398)
*Oliver Bause,Julia Werner,Paul Palomero Bernardo,Oliver Bringmann*

Main category: eess.IV

TL;DR: A lightweight CNN with 63,000 parameters achieves 93.06% accuracy for organ classification on Bayer images, saving 89.9% energy in Video Capsule Endoscopy.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks are unsuitable for low-power sensor edge devices due to large model sizes and high computational demands. Avoiding RGB conversion on Bayer images can save energy.

Method: Uses a CNN with 63,000 parameters and Viterbi decoding for time-series analysis, implemented on a PULPissimo System-on-Chip with a RISC-V core and hardware accelerator.

Result: Achieves 93.06% accuracy on Bayer images, consuming only 5.31 µJ per image, saving 89.9% energy compared to traditional methods.

Conclusion: The approach enables energy-efficient AI-based image classification for resource-constrained devices, particularly beneficial for medical applications like Video Capsule Endoscopy.
...
### [306] [JPEG Processing Neural Operator for Backward-Compatible Coding](https://arxiv.org/abs/2507.23521)
*Woo Kyoung Han,Yongjun Lee,Byeonghun Lee,Sang Hyun Park,Sunghoon Im,Kyong Hwan Jin*

Main category: eess.IV

TL;DR: JPNeO is a backward-compatible JPEG algorithm using neural operators to improve chroma preservation and reconstruction fidelity, with reduced memory and parameters.
...
Method: Incorporates neural operators in encoding and decoding stages to enhance image quality.

Result: Improved chroma preservation, reconstruction fidelity, and reduced memory usage.
...
### [307] [Towards Field-Ready AI-based Malaria Diagnosis: A Continual Learning Approach](https://arxiv.org/abs/2507.23648)
...
TL;DR: The paper explores continual learning (CL) to improve malaria CAD models' robustness against domain shifts, using YOLO-based object detection and evaluating four CL strategies.
...
Method: Investigates CL in a domain-incremental learning scenario, testing rehearsal and regularization-based methods on a multi-site clinical dataset.

Result: CL, especially rehearsal-based methods, significantly enhances model performance across domains.

Conclusion: Continual learning can aid in developing deployable CAD tools for malaria diagnosis.
...
### [308] [Topology Optimization in Medical Image Segmentation with Fast Euler Characteristic](https://arxiv.org/abs/2507.23763)
...
<details>
  <summary>Details</summary>
Motivation: Conventional metrics like Dice score often fail to ensure clinically acceptable topological accuracy, such as continuous boundaries or closed surfaces. Existing topology-aware methods using persistent homology are computationally expensive for high-dimensional data.

Method: Proposes a fast χ computation for 2D/3D data, uses χ error as a topological metric, generates topological violation maps, and refines segmentation with a correction network.
...
Conclusion: The method efficiently addresses topological constraints in medical image segmentation, offering a practical alternative to persistent homology-based approaches.
...
### [309] [From Propagator to Oscillator: The Dual Role of Symmetric Differential Equations in Neural Systems](https://arxiv.org/abs/2507.22916)
*Kun Jiang*

Main category: cs.NE

TL;DR: The study explores a neuron model's dual functional behaviors—signal propagation and generation—through parameter analysis, introducing a new metric for state monitoring.
...
Method: Systematic parameter space exploration and mathematical analysis, introducing the 'on-road energy' metric for simulations.

Result: The model exhibits two behaviors: stable signal propagation and self-excited oscillations. Transitions between modes are controllable.

Conclusion: The findings align with biological neurons' dual functions, offering theoretical and practical insights for neuromorphic engineering.
...
### [310] [A survey of multi-agent geosimulation methodologies: from ABM to LLM](https://arxiv.org/abs/2507.23694)
*Virginia Padilla,Jacinto Dávila*

Main category: cs.MA

TL;DR: The paper examines agent-based approaches in multi-agent systems, proposing a framework for geosimulation platforms and validating the integration of LLMs as agent components.


<details>
  <summary>Details</summary>
Motivation: To formalize and validate a framework for geosimulation platforms by integrating agent-based principles and LLMs.

Method: Comprehensive examination of agent-based approaches over two decades, leading to a formalized architecture for agent activities.

Result: LLMs can effectively serve as agent components when aligned with structured architectures for perception, memory, planning, and action.

Conclusion: The proposed framework provides a robust foundation for next-generation geosimulation systems.
...
### [311] [A Smoothing Newton Method for Rank-one Matrix Recovery](https://arxiv.org/abs/2507.23017)
...
<details>
  <summary>Details</summary>
Motivation: The instability of BWGD and the lack of theoretical guarantees for higher rank matrix recovery motivate the development of a stable method with rigorous convergence guarantees.

Method: The authors reveal BWGD's connection to Newton's method and propose a smoothing framework to regularize the nonsmooth, nonconvex objective, ensuring stability and superlinear convergence.

Result: Experiments on synthetic data confirm the method's superior stability and fast convergence.

Conclusion: The proposed smoothing framework resolves BWGD's instability, providing a stable method with superlinear convergence guarantees for phase retrieval.
...
### [312] [Optimal Transport Learning: Balancing Value Optimization and Fairness in Individualized Treatment Rules](https://arxiv.org/abs/2507.23349)
*Wenhai Cui,Xiaoting Ji,Wen Su,Xiaodong Yan,Xingqiu Zhao*

Main category: stat.ML

TL;DR: The paper proposes a method to balance fairness and optimization in individualized treatment rules (ITRs) using optimal transport theory, introducing an improved trade-off ITR with theoretical guarantees and empirical validation.


<details>
  <summary>Details</summary>
Motivation: ITRs can lead to unfair outcomes when influenced by sensitive attributes like race or gender, necessitating a method to ensure fairness without sacrificing value.

Method: A flexible approach based on optimal transport theory transforms optimal ITRs into fair ones, with an improved trade-off ITR balancing fairness and value. A smoothed fairness constraint estimates the adjustable parameter.

Result: The method ensures demographic parity, provides a theoretical upper bound on value loss, and is validated through simulations and a real-world dataset.

Conclusion: The proposed approach effectively balances fairness and optimization in ITRs, offering practical and theoretical benefits.
...
### [313] [DICOM De-Identification via Hybrid AI and Rule-Based Framework for Scalable, Uncertainty-Aware Redaction](https://arxiv.org/abs/2507.23736)
*Kyle Naddeo,Nikolas Koutsoubis,Rahul Krish,Ghulam Rasool,Nidhal Bouaynaya,Tony OSullivan,Raj Krish*

Main category: stat.ML

TL;DR: A hybrid de-identification framework combining rule-based and AI-driven techniques for secure PHI/PII removal from medical imaging data, ensuring compliance with regulatory standards.


<details>
  <summary>Details</summary>
Motivation: The need for ethical and secure sharing of medical imaging data, hindered by PHI/PII in DICOM files, drives the development of a robust de-identification solution.

Method: A two-tiered rule-based system and LLM fine-tuned for NER handle metadata, while an uncertainty-aware Faster R-CNN with OCR and NER processes pixel data. Uncertainty quantification enhances reliability.

Result: The framework achieves robust performance across benchmark datasets and meets DICOM, HIPAA, and TCIA compliance metrics.

Conclusion: The solution enables secure, ethical, and trustworthy release of medical imaging data for research by combining automation, uncertainty quantification, and quality assurance.
...
### [314] [Scaled Beta Models and Feature Dilution for Dynamic Ticket Pricing](https://arxiv.org/abs/2507.23767)
*Jonathan R. Landers*
...
### [315] [Formal Bayesian Transfer Learning via the Total Risk Prior](https://arxiv.org/abs/2507.23768)
*Nathan Wycoff,Ali Arab,Lisa O. Singh*

Main category: stat.ML

TL;DR: The paper proposes a Bayesian transfer learning method to improve statistical procedures in data-limited scenarios by jointly modeling source and target datasets, outperforming frequentist baselines.


<details>
  <summary>Details</summary>
Motivation: Existing transfer learning methods struggle with limited and misaligned source datasets, lacking Bayesian uncertainty quantification.

Method: The approach uses a risk minimizer conditional on source parameters to construct a joint prior, enabling Bayesian uncertainty quantification and model averaging via Gibbs sampling.

Result: The method shows superior predictive performance, especially with limited source data, and connects to minimax-frequentist techniques as an approximate MAP approach.

Conclusion: The Bayesian framework enhances transfer learning by addressing limitations of existing methods and improving predictive accuracy in data-scarce scenarios.
...
### [316] [Holistic Evaluations of Topic Models](https://arxiv.org/abs/2507.23364)
*Thomas Compton*

Main category: cs.IR

TL;DR: The paper evaluates topic models, particularly BERTopic, to address their 'black box' nature and optimize parameters for better interpretation and responsible use.


<details>
  <summary>Details</summary>
Motivation: Topic models are widely used but risk being treated as black boxes, leading to uncritical acceptance of their outputs. The study aims to improve understanding and responsible usage.

Method: The study analyzes 1140 BERTopic model runs to identify trade-offs in parameter optimization.

Result: Findings highlight key trade-offs in optimizing topic model parameters, impacting their accuracy and interpretability.

Conclusion: The study underscores the need for careful parameter tuning and critical interpretation to ensure responsible use of topic models.
...
### [317] [Are Recommenders Self-Aware? Label-Free Recommendation Performance Estimation via Model Uncertainty](https://arxiv.org/abs/2507.23208)
*Jiayu Li,Ziyi Ye,Guohao Jian,Zhiqiang Guo,Weizhi Ma,Qingyao Ai,Min Zhang*

Main category: cs.IR

TL;DR: The paper explores whether a recommender model can be self-aware by quantifying its uncertainty, proposing the LiDu method to measure this and validate its effectiveness.


<details>
  <summary>Details</summary>
Motivation: To enable more informed understanding and decision-making in recommender systems by assessing their self-awareness without relying on labels.

Method: Proposes LiDu, a probability-based method to measure uncertainty in recommender systems by analyzing ranking list distributions.

Result: LiDu shows higher correlation with recommendation performance than other label-free estimators and provides insights into model states during training and inference.

Conclusion: The study links recommendation uncertainty to performance, advancing towards transparent and self-evaluating recommender systems.
...
### [318] [Not Just What, But When: Integrating Irregular Intervals to LLM for Sequential Recommendation](https://arxiv.org/abs/2507.23209)
*Wei-Wei Du,Takuma Udagawa,Kei Tateno*

Main category: cs.IR

TL;DR: IntervalLLM integrates dynamic time intervals into sequential recommendation tasks, improving performance by 4.4% and addressing cold-start scenarios from an interval perspective.


<details>
  <summary>Details</summary>
Motivation: Existing methods overlook dynamic time intervals, which are crucial for user profiling and recommendation accuracy.

Method: Proposes IntervalLLM with interval-infused attention to jointly consider items and intervals, and introduces interval-based cold-start evaluation.

Result: Achieves 4.4% average improvement and best performance in warm/cold scenarios, with interval-based cold challenges showing significant performance drops.

Conclusion: Highlights the importance of interval integration in sequential recommendation and the need for further research on interval-based cold challenges.
...
### [319] [KLAN: Kuaishou Landing-page Adaptive Navigator](https://arxiv.org/abs/2507.23459)
*Fan Li,Chang Meng,Jiaqi Fu,Shuchang Liu,Jiashuo Zhang,Tianke Zhang,Xueliang Wang,Xiaoqiang Feng*

Main category: cs.IR

TL;DR: The paper introduces Personalized Landing Page Modeling (PLPM) and KLAN, a hierarchical framework for optimizing landing page selection to enhance user engagement and satisfaction.


<details>
  <summary>Details</summary>
Motivation: Most research focuses on in-page interactions (Stage II), neglecting page navigation (Stage I). The paper aims to address this gap by optimizing landing page selection.
...
### [320] [AutoIndexer: A Reinforcement Learning-Enhanced Index Advisor Towards Scaling Workloads](https://arxiv.org/abs/2507.23084)
...
<details>
  <summary>Details</summary>
Motivation: Index selection is critical for database performance, but existing RL-based methods struggle with scaling workloads due to large action spaces and trial-and-error inefficiencies.

Method: AutoIndexer combines workload compression, query optimization, and specialized RL models to reduce search complexity while maintaining index quality.

Result: It reduces query execution time by up to 95%, outperforms RL-based advisors by ~20% in cost savings, and cuts tuning time by over 50%.
...
### [321] [Counterfactual Evaluation for Blind Attack Detection in LLM-based Evaluation Systems](https://arxiv.org/abs/2507.23453)
...
<details>
  <summary>Details</summary>
Motivation: To address vulnerabilities in LLM-based evaluation systems, particularly against blind attacks where answers deceive evaluators independently of the truth.
...
Result: SE+CFE significantly improves attack detection compared to standard evaluation, with minimal performance trade-offs.
...
### [322] [LLM-Based Identification of Infostealer Infection Vectors from Screenshots: The Case of Aurora](https://arxiv.org/abs/2507.23611)
...
<details>
  <summary>Details</summary>
Motivation: Manual analysis of infostealer logs is impractical due to volume (29M logs in 2024). Current research lacks reactive analysis of infection artifacts like screenshots.

Method: Leverages LLMs to process screenshots, identifying infection vectors (URLs, files, themes) and correlating data to track campaigns.

Result: Extracted 337 URLs and 246 files from 1000 screenshots, revealing 3 distinct malware campaigns and distribution tactics.

Conclusion: LLM-driven analysis of screenshots offers a scalable, reactive approach to enhance threat intelligence and enable early intervention.
...
Method: CommLLM, a two-step framework using GPT-4o and prompt-based reasoning, fuses LLM outputs with graph structure. Evaluated on six datasets using NMI, ARI, VOI, and cluster purity.
...
Conclusion: LLMs show promise for graph-based tasks, but success depends on tailoring interactions to graph structure.
