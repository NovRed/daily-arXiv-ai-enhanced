...
Motivation: Addressing the challenge of detecting and identifying distorted objects where traditional geometric models (e.g., homography) fail, especially for non-planar or distorted planar templates.

Method: Uses Delaunay triangulation of template features as a graph, incrementally grouping feature matches based on local consistency criteria (geometric and photometric properties).

Result: Outperforms homography-based RANSAC in scenarios with significant distortion while performing comparably in minimal distortion cases.

Conclusion: The proposed method is effective for detecting distorted objects, offering robustness where traditional models fall short.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of preserving content characteristics while achieving high-quality style transfer without tuning during inference.

Method: Uses a two-stream training paradigm isolating color from style, multi-feature image embeddings compression, and a Diffusion UNet-inspired style definition.

Result: Achieves state-of-the-art performance in style similarity and editing capability, validated by experiments and human evaluations.

Conclusion: CDST offers a novel, efficient solution for universal style transfer with superior results.
...
Method: Curated diverse prompts across 160 topics, generated over 16,000 images using Stable Diffusion 1.5 and Flux-1 models, and compared with 8,000 Google Image Search results.

Result: Significant disparities in representation of gender, race, age, and somatotype, often reinforcing harmful stereotypes.
...
<details>
  <summary>Details</summary>
Motivation: Current reward modeling methods are complex due to reliance on human-annotated data or incomplete quality dimensions, prompting a simpler, automated solution.

Method: GAN-RM trains a reward model by discriminating between unpaired target samples (Preference Proxy Data) and model-generated outputs, requiring minimal data.

Result: GAN-RM proves effective in applications like Best-of-N filtering, Supervised Fine-Tuning, and Direct Preference Optimization.

Conclusion: GAN-RM offers a scalable and efficient alternative to traditional reward modeling, reducing manual effort while maintaining performance.
...
<details>
  <summary>Details</summary>
Motivation: To explore the underutilized potential of LiDAR in multi-modal contrastive pre-training for human activity understanding.

Method: DeSPITE learns a joint embedding space for LiDAR point clouds, skeleton poses, IMU data, and text using noise contrastive estimation, leveraging combined datasets (LIPD and Babel).

Result: Enables novel tasks like cross-modal matching and retrieval, and proves effective for point cloud HAR in MSR-Action3D and HMPEAR.

Conclusion: DeSPITE successfully bridges the gap in multi-modal human activity understanding, demonstrating LiDAR's utility in this domain.
...
<details>
  <summary>Details</summary>
Motivation: The lack of annotated satellite data for rare changes makes supervised methods impractical, necessitating a self-supervised approach.

Method: OPTIMUS leverages the principle that recovering the temporal order of images implies long-lasting changes, using change point detection on model outputs.

Result: OPTIMUS achieves an AUROC score of 87.6%, significantly outperforming baselines (56.3%).

Conclusion: OPTIMUS provides an effective, label-free solution for detecting persistent changes in satellite imagery.
...
Method: Uses 3D CNNs for detection and separable 3D CNNs with bidirectional LSTMs for classification, trained on annotated datasets from various sources.

Result: Demonstrates improved computational efficiency and accuracy in violence detection and classification.

Conclusion: The framework effectively addresses the need for automated violence detection, offering scalable and accurate solutions for surveillance.
...
<details>
  <summary>Details</summary>
Motivation: Addressing challenges in semi-supervised semantic segmentation under label scarcity and domain variability, where vision-only methods struggle with generalization and boundary localization.

Method: HierVL combines text embeddings with a mask-transformer architecture, featuring hierarchical semantic queries, cross-modal spatial alignment, and a dual-query transformer decoder.

Result: Achieves significant improvements in IoU on COCO (+4.4%), Pascal VOC (+3.1%), ADE20 (+5.9%), and Cityscapes (+1.8%) under 1% supervision.

Conclusion: Language-guided segmentation enhances label efficiency and enables fine-grained, instance-aware generalization.
...
<details>
  <summary>Details</summary>
Motivation: The absence of detailed, large-scale ecological maps hampers effective agricultural landscape management and biodiversity conservation.

Method: A deep learning segmentation model was trained on 942 manually annotated aerial imagery tiles to map rural landscape features.

Result: The model achieved high accuracy (F1-scores: 96% for woodland, 95% for farmed land, 72% for hedgerows) and the map is available on Google Earth Engine.
...
<details>
  <summary>Details</summary>
Motivation: Detecting unknown objects is critical for safety in domains like autonomous driving, but current methods assume closed-world conditions, leading to overconfidence in deployment.
...
Result: Semantically/visually close OOD objects are easier to localize but more confusable with ID objects, while far/farther objects are harder to localize but less confusable.

Conclusion: The proposed benchmark and metrics provide deeper insights into OOD detection, addressing limitations of current protocols.
...
<details>
  <summary>Details</summary>
Motivation: To enable free-form text and 3D geometry control (e.g., age, hair style, face expression) in 3D portrait generation without requiring large labeled datasets or training large models.

Method: Uses a pre-trained LVLM (CLIP) and a 3D morphable model (FLAME) to disentangle 3D from 2D data via canonicalization and employs Jacobian regularization to mitigate noise in LVLM embeddings.

Result: Produces 3D portraits with consistent text and 3D control, improving output quality and diversity compared to existing methods.

Conclusion: The approach allows creators to control 3D generators using their 2D face data, bypassing the need for extensive labeling or large model training.
...
<details>
  <summary>Details</summary>
Motivation: DocVQA is challenging due to multi-modality and multi-page references. Existing methods use VLMs but lack efficiency in retrieval.

Method: SimpleDoc uses a dual-cue retriever (embedding similarity and page summaries) and a VLM-based reasoner agent for iterative retrieval and answer generation.

Result: Outperforms baselines by 3.2% on 4 DocVQA datasets with fewer retrieved pages.

Conclusion: SimpleDoc is effective and efficient for DocVQA, offering a lightweight yet powerful solution.
...
Conclusion: Emphasizes explainable, human-centric AI for successful deployment in next-gen transportation.
...
<details>
  <summary>Details</summary>
Motivation: Existing FSR methods treat all facial pixels equally, leading to inefficient resource use and degraded performance. CNN struggles with high-frequency features, while Mamba excels at low-frequency ones with lower complexity.

Method: FADPNet decomposes facial features into low- and high-frequency components. Low-frequency regions use a Mamba-based LFEB for global interactions, while high-frequency regions use a CNN-based DPA module and HFR for structural refinement.

Result: The method balances FSR quality and efficiency, outperforming existing approaches.

Conclusion: FADPNet effectively addresses FSR challenges by leveraging frequency-aware processing, offering superior performance and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Enhancing MOS for autonomous driving by addressing the accuracy vs. real-time trade-off and class imbalance.

Method: Uses BEV projection (student) and non-projection (teacher) models with tailored distillation for motion features, dynamic upsampling, and architecture optimization.

Result: 78.8% IoU on SemanticKITTI-MOS, reduced false positives/negatives, and 7.69% fewer parameters.

Conclusion: KDMOS effectively improves MOS performance while maintaining efficiency, validated on benchmark datasets.
...
<details>
  <summary>Details</summary>
Motivation: To explore BiomedCLIP's class separations and quantify its limitations on imbalanced, out-of-distribution medical data.

Method: Experiments on IU-xray dataset using zero-shot inference, full fine-tuning, and linear probing, with evaluation via Grad-CAM heatmaps and radiologist annotations.

Result: Zero-shot over-predicts labels; fine-tuning improves distinct disease classification; linear probing detects overlapping features.

Conclusion: Careful model adaptation is needed for reliable real-world medical applications.
...
Conclusion: RadFabric enhances AI-driven radiology with transparent, precise, and clinically actionable CXR analysis.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods neglect environmental context, which significantly influences pedestrian movement. SceneAware addresses this gap.

Method: Uses ViT for scene encoding and MLLMs for walkability masks, combining them with a Transformer-based trajectory encoder and collision penalties.

Result: Outperforms state-of-the-art methods by 50% on ETH/UCY datasets, with consistent performance across trajectory types.

Conclusion: SceneAware demonstrates the importance of explicit scene information for accurate and physically plausible trajectory predictions.
...
<details>
  <summary>Details</summary>
Motivation: To explore the underutilized potential of masked autoregressive models in video generation and address challenges like long sequence modeling and resource efficiency.

Method: Proposes VideoMAR with temporal causality, spatial bi-directionality, next-frame diffusion loss, and strategies like curriculum learning and progressive resolution training.

Result: Outperforms Cosmos I2V on VBench-I2V with fewer parameters (9.3%), training data (0.5%), and GPU resources (0.2%).

Conclusion: VideoMAR effectively bridges autoregressive models and video generation, offering efficiency and unique capabilities like spatial-temporal extrapolation.
...
Method: MAINet includes a general feature extraction framework, an Auxiliary-modality Reinforcement Primary-modality Mechanism (ARPM) for inter-modal interaction, and an Evidence Reasoning (ER) rule for decision fusion.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods neglect unique pathological image traits, causing inefficiencies, especially in edge-computing.

Method: Introduces Network Similarity Directed Initialization (NSDI) and integrates domain adaptation into one-shot NAS.

Result: Outperforms existing methods on BRACS, with better classification and clinically relevant feature localization.

Conclusion: The approach effectively addresses challenges in pathological image analysis, enhancing performance and adaptability.
...
<details>
  <summary>Details</summary>
Motivation: Uncertainty in surgical videos (frame ambiguity and unbalanced phase distribution) is crucial for reliable online phase recognition but often overlooked.
...
<details>
  <summary>Details</summary>
Motivation: Existing HOI detection methods focus on third-person perspectives, neglecting the intuitive egocentric view. The paper aims to fill this gap by introducing a dataset and method tailored for Ego-HOI.

Method: The paper presents Ego-HOIBench, a dataset with 27K egocentric images annotated with hand-verb-object triplets. It adapts third-person HOI methods and proposes the HGIR scheme, which uses hand pose and geometric features to refine interaction representations.

Result: The HGIR scheme improves Ego-HOI detection by leveraging hand geometry and interactivity, achieving state-of-the-art results on the Ego-HOIBench dataset.

Conclusion: The paper successfully addresses the challenges of Ego-HOI detection with a novel dataset and method, demonstrating the effectiveness of the HGIR scheme in improving interaction representation.
...
<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) struggles with memory scalability in high-resolution scenarios, prompting the need for a more efficient solution.

Method: HRGS employs a two-step hierarchical approach: coarse global Gaussian representation followed by block-level refinement. It includes Gaussian partitioning, training data partitioning, and Importance-Driven Gaussian Pruning (IDGP) to optimize memory and computation. Normal priors are used to enhance surface quality.

Result: HRGS achieves state-of-the-art performance in high-resolution novel view synthesis and surface reconstruction on three benchmarks.

Conclusion: HRGS effectively addresses memory constraints while maintaining high-quality 3D scene reconstruction, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3DVG methods suffer from a gap between vision and text modalities due to separately pre-trained encoders, leading to errors in object positioning and classification.
...
Result: UniSpace-3D achieves at least 2.24% improvement over baselines on ScanRefer and Nr3D/Sr3D datasets.

Conclusion: The proposed method effectively bridges the modality gap in 3DVG, enhancing object identification accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Addressing challenges of inconsistent point cloud density and representation fragility in existing methods for robust place recognition in robotics and autonomous driving.

Method: Introduces an implicit 3D representation with elastic points, derives occupancy grid and normal vector information, and fuses geometric descriptors from bird's-eye view and 3D segments.
...
Conclusion: The proposed framework offers resilient and scalable place recognition, with plans to open-source the code.
...
<details>
  <summary>Details</summary>
Motivation: Challenges in bridge inspection due to resource constraints and the need for automated, accurate defect classification motivate this work.

Method: Three synthetic dataset extensions (synth-dacl) are created to balance dacl10k's class imbalance, particularly for cracks and cavities.

Result: Incorporating synth-dacl improves model performance, with a 2% increase in mean IoU, F1 score, Recall, and Precision on perturbed test sets.

Conclusion: Synth-dacl effectively enhances model robustness and performance for bridge defect segmentation, addressing real-world inspection challenges.
...
Method: Two schemes: one using a single background, the other using dual backgrounds with different learning rates. NCC-based image comparison for tracking.

Result: The method is robust to partial/full occlusion, illumination changes, and operates in real time.

Conclusion: The proposed approach effectively detects and tracks stationary objects, offering real-time performance and robustness.
...
<details>
  <summary>Details</summary>
Motivation: To create a generalizable feature extractor for cell images using non-contrastive SSL, overcoming distribution differences and multi-image input challenges.

Method: Proposes SSLProfiler with specialized data augmentation and representation post-processing for cell images.

Result: SSLProfiler achieved robust feature extraction and won the Cell Line Transferability challenge at CVPR 2025.

Conclusion: SSLProfiler effectively adapts SSL for cell profiling, addressing unique challenges and demonstrating superior performance.
...
<details>
  <summary>Details</summary>
Motivation: The lack of large-scale labeled 360 video datasets hinders progress in 360 scene understanding tasks like segmentation and tracking, due to challenges like distortion and annotation complexity.

Method: An automated labeling pipeline coordinates pre-trained 2D segmentors and LLMs in three stages: Initial Annotation (Semantic- and Distortion-aware Refinement), Auto-Refine Annotation, and Manual Revision.

Result: Leader360V demonstrates high scene diversity and effectiveness in enhancing model performance for 360 video segmentation and tracking.

Conclusion: Leader360V and its labeling pipeline advance scalable 360 scene understanding, validated by user studies and experiments.
...
Method: Train an image diffusion model in the functional map space, using pointwise maps as guidance during inference to enforce objectives like orthogonality and commutativity.

Result: The approach matches state-of-the-art map refinement methods and demonstrates the potential of guided diffusion for functional map processing.

Conclusion: Guided diffusion models offer an efficient and effective solution for refining functional maps, with promising applications in shape correspondence.
...
Result: FGA-NN achieves a superior balance between accuracy and complexity in film grain preservation.

Conclusion: FGA-NN is a robust and applicable solution for maintaining film grain aesthetics in compressed content.
...
<details>
  <summary>Details</summary>
Motivation: To overcome inefficiencies and inaccuracies in existing egocentric video-language models, particularly in pre-training, spatial-temporal encoding, and retrieval tasks.

Method: 1) Single-stage pretraining for efficiency. 2) Spatial-temporal rotary positional embeddings with joint attention. 3) Symmetric Multi-Similarity (SMS) loss for precise retrieval.

Result: Achieves state-of-the-art performance on Ego4D, EPIC-Kitchens-100, and Charades-Ego with fewer parameters. SMS loss improves multi-instance retrieval benchmarks.

Conclusion: EVA02-AT provides a robust, efficient solution for egocentric video-language understanding, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the lack of comprehensive datasets and standardized benchmarks in surface water dynamics forecasting.
...
Result: Model outperforms baselines by +14% and +11% F1 in tasks, and +0.1 MAE in regression. Explainable AI identifies key climate variables.

Conclusion: HydroChronos and AquaClimaTempo UNet provide a robust framework for future water dynamics forecasting and modeling.
...
Method: DGG-XNet integrates VGG16 for hierarchical spatial features and DenseNet121 for feature reuse and gradient flow, enhanced with Grad-CAM for interpretability.
...
Method: Extends latent predictive coding with semantic tokenization and complementary objectives to create robust tokenization for symbolic reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods ignore elevation differences, leading to misclassification in complex scenarios. DepthSeg addresses this by modeling depth from 2D images.

Method: DepthSeg uses a lightweight adapter for fine-tuning, a depth prompter for explicit depth modeling, and a semantic classification decoder to combine depth prompts with land-cover features.

Result: Experiments on the LiuZhou dataset show DepthSeg's superiority in land cover mapping, with ablation studies confirming the importance of depth prompts.

Conclusion: DepthSeg effectively mitigates spectral confusion and shadow occlusion, improving semantic segmentation accuracy in remote sensing.
...
<details>
  <summary>Details</summary>
Motivation: Euclidean methods fail to capture non-Euclidean image structures, leading to poor fusion performance. Addressing this, the paper aims to balance low-level details and high-level semantics.

Method: GrFormer uses Grassmann manifold for low-rank subspace mapping, decoupling features into high-frequency details and low-frequency semantics. A cross-modal fusion strategy (CMS) based on covariance masks maximizes complementary properties.

Result: GrFormer outperforms state-of-the-art methods on multiple benchmarks, achieving better qualitative and quantitative fusion results.

Conclusion: The proposed method effectively addresses the limitations of Euclidean approaches, enhancing fusion performance by leveraging Grassmann manifold and CMS.
...
<details>
  <summary>Details</summary>
Motivation: Standard CFG's global weighting leads to poor identity preservation and spurious attribute changes, limiting the fidelity of counterfactual image generation.

Method: DCFG uses an attribute-split embedding strategy to disentangle semantic inputs, enabling selective guidance on user-defined attribute groups. It partitions attributes into intervened and invariant sets based on a causal graph.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in maintaining causal relationships in video edits to avoid unrealistic outcomes.

Method: Optimize text prompts based on a causal graph to guide generation, without accessing or finetuning the video editing system.

Result: Demonstrates effective generation of causally faithful counterfactuals within the learned distribution of latent diffusion models.

Conclusion: The method is compatible with any black-box video editing system, enabling realistic "what-if" scenarios in fields like healthcare and digital media.
...
<details>
  <summary>Details</summary>
Motivation: Visual attribute imbalance in image classification is underexplored but significantly impacts model performance and generalization. The study aims to analyze and mitigate this issue.

Method: The authors define first-level and second-level image attributes, introduce a CLIP-based framework for attribute evaluation, and propose adjusting sample sampling probability based on attribute rarity. They integrate this with data augmentation techniques like CutMix, Fmix, and SaliencyMix.

Result: Experiments show the method effectively mitigates attribute imbalance, enhancing model robustness and fairness in deep neural networks.

Conclusion: The research underscores the importance of modeling visual attribute distributions and offers a scalable solution for long-tail image classification tasks.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of generating realistic and controllable human motions, particularly for multi-character interactions, due to data scarcity and modeling complexities.

Method: Proposes RVHM2D, a diffusion-based model with enhanced textual conditioning and a two-stage training strategy (diffusion objective followed by reinforcement learning).
...
Conclusion: The dataset and model effectively address the challenges of realistic and controllable human motion generation, particularly for interactive scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Addressing high memory footprint in large multimodal MoEs for edge device deployment.

Method: Train ternary experts (-1, 0, 1) using pre-trained FFN as shared expert, enabling scalable and memory-efficient models.

Result: MoTE matches MoE-LLaVA performance with lower memory, gains 4.3% accuracy under memory constraints.
...
<details>
  <summary>Details</summary>
Motivation: Model compression is essential for deploying deep learning on edge devices, but existing methods may lack efficiency or accuracy.

Method: The approach overlays IG maps onto input images during training to provide deeper insights into teacher models' decisions, with precomputed IG maps reducing runtime costs.

Result: Achieves 92.6% accuracy (1.1% improvement) and 4.1x compression, reducing inference time from 140 ms to 13 ms. Validated across architectures and datasets.

Conclusion: IG-augmented knowledge distillation outperforms conventional methods, offering a viable solution for edge deployment with competitive accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Challenges in radiological VQA include limited expert-labeled data, complex image patterns, and lack of evaluation methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing MLLMs rely on limited FOV inputs; the paper aims to enable dense understanding from omnidirectional panoramas for more complete scene representation.

Method: Introduces a dataset with 160K panoramas and annotations, and ERP-RoPE, a position encoding scheme for panoramic ERP. Also proposes Dense360-Bench for evaluation.

Result: The dataset and ERP-RoPE address challenges in panoramic MLLMs, and Dense360-Bench provides a framework for evaluating dense visual-language understanding.

Conclusion: The work advances dense visual understanding in panoramas, offering tools and benchmarks for future research in omnidirectional MLLMs.
...
<details>
  <summary>Details</summary>
Motivation: To reduce training costs by improving subset selection methods, leveraging FMs as a dataset-independent alternative to traditional IEs.

Method: Proposes RAM-APL, a method using multiple FMs to enhance subset selection by combining their strengths, specifically for fine-grained datasets.

Result: FMs outperform traditional IEs on fine-grained datasets but not on coarse-grained ones. RAM-APL achieves state-of-the-art performance on fine-grained datasets like Oxford-IIIT Pet, Food-101, and Caltech-UCSD Birds-200-2011.

Conclusion: FMs are superior for subset selection in fine-grained datasets, and RAM-APL effectively leverages multiple FMs to improve performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3DVG methods rely on precise text prompts, but real-world speech inputs often have errors due to noise, accents, or speech rates, limiting their applicability.

Method: SpeechRefer introduces a Speech Complementary Module to capture acoustic similarities and a Contrastive Complementary Module to align erroneous text with speech features.
...
<details>
  <summary>Details</summary>
Motivation: Existing MER methods rely on hand-crafted features or key frames, limited by small datasets. The paper aims to improve accuracy by leveraging local-global features without prior key frame knowledge.

Method: Introduces the F5C block (fully-connected and channel correspondence convolution) to extract local-global features from raw frames. Jointly trains MER, optical flow estimation, and facial landmark detection.

Result: Outperforms state-of-the-art on CASME II, SAMM, and SMIC benchmarks; effective for optical flow and landmark detection; captures subtle facial actions.

Conclusion: The framework successfully addresses MER challenges by integrating multiple tasks and advanced convolution techniques, demonstrating superior performance and robustness.
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs lack systematic evaluation for complex spatial reasoning, despite its importance for real-world interactions.

Method: Developed SIRI-Bench with 1K video-question-answer triplets in realistic 3D scenes and an Automatic Scene Creation Engine using LLM agents.

Result: State-of-the-art VLMs perform poorly on SIRI-Bench, highlighting the challenge of spatial reasoning.

Conclusion: SIRI-Bench aims to advance VLMs in spatially grounded reasoning and visual problem-solving.
...
Method: Leverages Metric3D V2 for depth-normal synergy, introduces a safe zone segmentation branch, and uses the WildUAV dataset for fine-tuning and cross-domain validation.

Result: VisLanding improves safe zone identification accuracy and retains zero-shot generalization, outperforming other methods in robustness.

Conclusion: The framework effectively estimates landing zones, supports decision-making, and demonstrates strong generalization and robustness.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses challenges in image restoration: ineffective feature fusion, computational bottlenecks, and inefficient diffusion processes.

Method: The proposed DiffRWKVIR framework unifies TTT with efficient diffusion, featuring Omni-Scale 2D State Evolution, Chunk-Optimized Flash Processing, and Prior-Guided Efficient Diffusion.

Result: DiffRWKVIR outperforms SwinIR, HAT, and MambaIR/v2 in PSNR, SSIM, LPIPS, and efficiency on benchmarks like Set5, Set14, BSD100, Urban100, and Places365.

Conclusion: DiffRWKVIR establishes a new paradigm for adaptive, high-efficiency image restoration with optimized hardware utilization.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with realistic light interaction and rely on costly data or complex pipelines. DreamLight aims to simplify and improve relighting by using unified data formats and semantic priors.

Method: DreamLight reorganizes input data, uses a pretrained diffusion model for semantic guidance, and introduces PGLA for light direction handling and SFF for frequency component consistency.

Result: DreamLight achieves remarkable relighting performance, validated by extensive comparisons and user studies.

Conclusion: DreamLight advances universal image relighting by addressing key challenges and delivering high-quality, natural results.
...
Method: Uses multi-task predictive modeling to classify future OA severity and predict anatomical landmarks, leveraging a diffusion model for efficient future image generation.
...
Method: A LaTeX-based pipeline synthesizes realistic two-column pages with diverse table layouts and aligned ground-truth masks to create a synthetic corpus.

Result: TableNet achieves 4.04% XOR error on synthetic data (256x256) and 9.18% on Marmot benchmark, reducing manual effort.

Conclusion: The synthetic data pipeline effectively augments real-world benchmarks and improves table extraction performance.
...
Method: Uses dual graph convolution for joint and bone graphs, cross-attention for interdependencies, dynamic fusion for feature integration, and an improved Transformer encoder.
...
Conclusion: PoseGRAF effectively addresses limitations of prior methods by leveraging joint-bone relationships, achieving superior performance.
...
TL;DR: Consistency models distill diffusion/flow models into one-step generators but degrade with more steps. Flow maps generalize these, remaining effective at all step counts. New objectives and techniques improve performance, validated on image generation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Address the inefficiency of diffusion/flow models requiring many steps and the degradation of consistency models with increased steps.

Method: Introduce flow maps with new continuous-time objectives and training techniques, including autoguidance and adversarial finetuning.

Result: State-of-the-art few-step generation on ImageNet benchmarks and superior text-to-image synthesis.

Conclusion: Flow maps (Align Your Flow) offer efficient, high-performance generative modeling across step counts, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: To address the global health risk of mosquito-borne diseases by enabling early detection and proactive control of breeding sites.

Method: The dataset includes annotated images for object detection, water surface segmentation, and reasoning texts. Models like YOLOv9s and YOLOv11n-Seg are used for detection and segmentation, while a fine-tuned BLIP model handles reasoning generation.

Result: High precision scores (e.g., 0.92926 for detection, 0.91587 for segmentation) and strong reasoning metrics (BLEU 54.7, BERTScore 0.91).

Conclusion: The dataset and models demonstrate AI's potential for proactive mosquito-borne disease prevention, aligning with the theme 'Prevention is Better than Cure'.
...
TL;DR: 3DGS-IEval-15K is the first large-scale IQA dataset for compressed 3DGS representations, addressing the lack of perceptual evaluation frameworks.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods lack a comprehensive framework to evaluate the perceptual impact of compression, hindering practical applications.

Method: The dataset includes 15,200 images from 10 scenes, rendered using 6 algorithms at 20 viewpoints, with varying compression levels. Subjective experiments collected human perception data.

Result: The dataset is validated for scene diversity and MOS distribution, and benchmarks 30 IQA metrics.

Conclusion: 3DGS-IEval-15K supports the development of specialized IQA metrics and research into view-dependent quality patterns for 3DGS.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the scalability challenge in Neural Architecture Search (NAS) by optimizing training efficiency.

Method: Uses an autoencoder for image similarity embedding, constructs a kd-tree for efficient dissimilarity ordering, and dynamically reforms subsample datasets via curriculum learning.
...
TL;DR: The paper introduces GLOBE, a novel pipeline for geo-localization using reasoning-driven tasks with diverse social media images, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing geo-localization methods lack interpretability and rely on limited datasets. The rise of LVLMs offers potential for reasoning-driven tasks, but challenges in data diversity and model fine-tuning persist.

Method: Proposes MP16-Reason dataset and GLOBE, which uses group-relative policy optimization for locatability assessment and visual-clue reasoning, enhancing geolocation accuracy.
...
TL;DR: FocalClick-XL extends the coarse-to-fine design of FocalClick, introducing a multi-stage pipeline with dedicated subnets for context, object, and detail levels, achieving state-of-the-art performance in interactive segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing interactive segmentation methods lack flexibility and struggle with fine details, prompting the need for a more adaptable and detailed approach.

Method: Proposes FocalClick-XL, a pipeline with subnets for context, object, and detail levels, using scaled pretraining and a prompting layer for interaction-specific encoding.

Result: Achieves state-of-the-art performance on click-based benchmarks and adapts well to diverse interaction forms (boxes, scribbles, masks) and fine-grained alpha mattes.

Conclusion: FocalClick-XL is a versatile, powerful tool for interactive segmentation, excelling in detail capture and adaptability across interaction types.
...
TL;DR: The paper introduces YOLOv11-RGBT, a unified single-stage framework for multispectral object detection, addressing challenges like fusion strategy balance and modality weight allocation. It proposes six fusion modes, a P3 mid-fusion strategy, and a controllable fine-tuning method, achieving significant performance gains on datasets like LLVIP and FLIR.


<details>
  <summary>Details</summary>
Motivation: Existing multispectral object detection methods lack a unified framework and struggle with balancing performance and fusion strategies. The paper aims to address these gaps by improving feature fusion and modality weight allocation.

Method: Based on YOLOv11, the authors designed six multispectral fusion modes and introduced a P3 mid-fusion strategy and multispectral controllable fine-tuning (MCF) strategy. These optimize feature fusion and reduce redundancy.

Result: The framework outperforms on datasets like LLVIP and FLIR, with the MCF strategy boosting YOLOv11 models' mAP by 3.41%-5.65%, reaching 47.61% on FLIR.

Conclusion: YOLOv11-RGBT effectively addresses key challenges in multispectral object detection, demonstrating improved adaptability and robustness through innovative fusion and fine-tuning strategies.
...
TL;DR: A versatile iterative framework based on surrogate diffusion is proposed to enhance extrinsic calibration accuracy for camera-LiDAR fusion in autonomous vehicles.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end calibration methods lack iterative optimization, limiting accuracy. The demand for higher precision in extrinsic calibration drives the need for an adaptable solution.

Method: The framework refines initial extrinsic parameters iteratively using a denoising process, leveraging the original calibration method as a surrogate denoiser.

Result: Integration with the diffusion model improves accuracy, robustness, and stability across four state-of-the-art calibration methods.

Conclusion: The proposed framework outperforms single-step and other iterative approaches, offering a flexible solution for enhancing extrinsic calibration.
...
TL;DR: DiFuse-Net introduces a novel network for depth estimation using RGB and dual-pixel (DP) data, leveraging a unique attention mechanism and cross-modal learning to outperform traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional depth sensors have cost and robustness limitations, while DP technology in modern cameras offers a viable alternative for depth estimation.

Method: DiFuse-Net uses a window bi-directional parallax attention mechanism (WBiPAM) for DP disparity cues, a separate RGB encoder, and cross-modal transfer learning (CmTL) to utilize existing RGB-D datasets.

Result: DiFuse-Net outperforms DP and stereo-based baselines, and a new high-quality RGB-DP-D dataset (DCDP) is introduced.

Conclusion: The proposed method advances depth estimation by effectively combining RGB and DP data, with practical applications in real-world scenarios.
...
TL;DR: The paper presents a method using SAR data to monitor urban damage in Gaza during the 2023 Israel-Hamas War, achieving high accuracy in detecting destruction.


<details>
  <summary>Details</summary>
Motivation: To provide timely and accurate damage monitoring in active conflict zones like Gaza, where traditional methods may fall short.

Method: Uses Sentinel-1 SAR data with a long temporal-arc coherent change detection (LT-CCD) approach for weekly damage tracking.

Result: Detected 92.5% of damage labels with a 1.2% false positive rate, revealing damage trends and shifts in conflict zones.
...
<details>
  <summary>Details</summary>
Motivation: The challenge of achieving synchronization in talking head videos, where identity, lip movements, facial expressions, and head poses must align, is addressed to avoid unrealistic results.

Method: SyncTalk++ employs a Dynamic Portrait Renderer with Gaussian Splatting, a Face-Sync Controller for lip and expression alignment, and a Head-Sync Stabilizer for natural head poses. It also includes an Expression Generator and Torso Restorer for robustness.

Result: The method achieves up to 101 FPS, outperforming state-of-the-art techniques in synchronization and realism, as validated by experiments and user studies.

Conclusion: SyncTalk++ effectively addresses synchronization challenges in talking head synthesis, enhancing realism and performance, with supplementary video available for demonstration.
...
TL;DR: A framework optimizes computational cost and quality in diffusion models by dynamically routing prompts to the most suitable text-to-image generation function based on complexity.


<details>
  <summary>Details</summary>
Motivation: High computational costs of diffusion models due to sequential denoising, despite their high-fidelity outputs, necessitate a balance between quality and efficiency.

Method: Proposes a routing framework that assigns prompts to appropriate generation functions (varying denoising steps or models) based on complexity, optimizing cost and quality.

Result: Empirical results on COCO and DiffusionDB show higher average quality than individual models by routing prompts to nine pre-trained models.

Conclusion: The framework effectively balances computational cost and quality, outperforming uniform cost-reduction methods by adapting to prompt complexity.
...
TL;DR: The paper presents the scaling-up of the PhilEO Geo-Aware U-Net foundation model on large unlabeled EO datasets (MajorTOM 23TB and FastTOM 2TB) and evaluates its performance on downstream tasks like road density estimation and land cover segmentation. Results show improved performance with larger datasets and models, and a shift from CNN to ViT architectures.


<details>
  <summary>Details</summary>
Motivation: To fully exploit the massive volumes of EO data, pretraining foundation models on large unlabeled datasets is essential for efficient fine-tuning with minimal labeled data.

Method: The authors scale up the PhilEO model on the MajorTOM 23TB and FastTOM 2TB datasets, develop variants with different architectures and parameters, and fine-tune them on the PhilEO Bench for tasks like road density estimation and land cover segmentation.

Result: The PhilEO 44M MajorTOM 23TB model outperforms smaller datasets for road density regression, and the PhilEO 200M FastTOM model excels in most n-shot tasks. Dataset and model scaling are validated as effective.

Conclusion: The study confirms the benefits of scaling datasets and models for EO tasks, and explores the transition from CNN to ViT architectures.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods like VCD and ICD mitigate hallucinations but may not address deeper attention dynamics. The paper aims to provide a more principled approach by influencing attention mechanisms.

Method: The authors introduce an attention-steerable contrastive decoding framework that directly intervenes in the model's attention mechanisms to reduce hallucinations.

Result: Experiments show the framework significantly reduces hallucinations and improves performance on benchmarks like POPE, CHAIR, and MMHal-Bench, while also enhancing standard VQA performance.

Conclusion: The proposed attention-steerable approach offers a deeper, more effective solution to mitigating hallucinations in MLLMs compared to surface-level methods.
...
TL;DR: CDP improves robotic action prediction by using historical action sequences and a caching mechanism, outperforming existing methods in accuracy and robustness.
...
Result: CDP achieves higher accuracy and maintains precision even with degraded input quality, as shown in simulated and real-world experiments.

Conclusion: CDP is a robust solution for robotic control under imperfect conditions, leveraging temporal continuity for better performance.
...
TL;DR: A non-associative algebraic framework for high-dimensional information representation and computation, preserving temporal order without auxiliary structures.


<details>
  <summary>Details</summary>
Motivation: Address limitations of associative bundling in cognitive tasks, which loses order information and requires auxiliary structures.

Method: Uses non-associative multiplication-like binding and interference-like bundling to create sparse, ordered sequence representations.

Result: Generates two distinct states (L-state and R-state) for sequences, replicating recency and primacy effects like the Serial Position Curve.

Conclusion: The framework aligns with cognitive science findings and offers a noise-inclusive approach to order representation, linking to prefrontal and hippocampal memory processes.
...
Result: Validated in aviation maintenance, showing successful formal representation and contextualization of complex differential equations.

Conclusion: The introduced artifacts are practically applicable for integrating behavioral information in CPS.
...
<details>
  <summary>Details</summary>
Motivation: Agentic AI systems face challenges in aligning behavior with diverse human values, safety requirements, and compliance needs. Existing methods struggle with personalization and avoiding confabulation.

Method: A 'superego' agent uses user-selected 'Creed Constitutions' to dynamically steer AI planning, validated by a real-time compliance enforcer. Integrated with third-party models via MCP.

Result: Achieves up to 98.3% harm score reduction and near-perfect refusal rates (e.g., 100% with Claude Sonnet 4) on benchmarks like HarmBench and AgentHarm.

Conclusion: The Superego agent simplifies personalized AI alignment, improving safety and reliability for agentic systems.
...
<details>
  <summary>Details</summary>
Motivation: Human baselines in AI evaluations are often inadequate, leading to unreliable claims of 'super-human' performance. Rigorous baselines are needed for meaningful comparisons.

Method: The authors conducted a meta-review of measurement theory and AI evaluation literature to develop a framework and checklist for human baselines, then reviewed 115 studies to identify shortcomings.

Result: The study highlights flaws in existing baselining methods and provides a checklist to improve the design, execution, and reporting of human baselines.

Conclusion: The proposed framework and checklist aim to enhance AI evaluation practices, benefiting researchers and policymakers.
...
TL;DR: NordDRG-AI-Benchmark is the first open benchmark for evaluating LLMs in hospital funding via DRG logic, multilingual diagnosis, and procedure coding.


<details>
  <summary>Details</summary>
Motivation: To address the lack of open benchmarks for LLMs in hospital funding, specifically for DRG-based reimbursement systems.

Method: The benchmark includes definition tables, expert manuals, and a prompt pack with 14 CaseMix tasks to evaluate LLMs.

Result: LLMs like o3 scored 9/9, GPT-4o and o4-mini-high scored 7/9, while Gemini models performed poorly (5/9 and 3/9).

Conclusion: NordDRG-AI-Benchmark reveals domain-specific LLM strengths/weaknesses, providing a reproducible baseline for hospital funding automation research.
...
TL;DR: ICE-ID is a new benchmark dataset for historical identity resolution, covering 220 years of Icelandic census records. It evaluates methods like rule-based matchers, ML ensembles, LLMs, and NARS, with NARS showing competitive performance.


<details>
  <summary>Details</summary>
Motivation: To address the lack of large-scale, open datasets for studying long-term person-entity matching in real-world populations, ICE-ID provides a resource for reproducible benchmarking.

Method: The study evaluates rule-based matchers, ML ensembles, LLMs, and NARS (a Non-Axiomatic Reasoning System) on ICE-ID, using clearly defined tasks and metrics.

Result: NARS, a simple yet effective approach, achieves state-of-the-art performance on the identity resolution tasks.
...
Method: Med-REFL uses a tree-of-thought approach to decompose questions, evaluate reasoning steps, and construct preference optimization data automatically.

Result: Achieves consistent improvements (up to 4.11%) on MedQA-USMLE and boosts 7B/8B models by 4.13%, with strong generalization across datasets.
...
TL;DR: The paper proposes BotTrans, a multi-source graph domain adaptation model, to address label scarcity in detecting social bots by leveraging knowledge from multiple source networks, improving accuracy and stability.


<details>
  <summary>Details</summary>
Motivation: Label scarcity in detecting social bots and anomalies with GNNs is a challenge. Network heterophily and single-source transfer limitations hinder effective knowledge transfer.

Method: BotTrans uses multiple source domains to create a cross-source-domain topology, aggregates neighbor information, integrates source-target relevance, and refines detection with target domain semantics.

Result: Experiments show BotTrans outperforms state-of-the-art methods, proving its effectiveness in leveraging multi-source knowledge for unlabeled target tasks.

Conclusion: BotTrans successfully addresses challenges in transferring knowledge for bot detection, demonstrating superior performance by utilizing multi-source domains and refining strategies.
...
TL;DR: A suite of scalable algorithms for minimizing feedback arcs in large-scale weighted directed graphs, tested on the FlyWire Connectome dataset, outperforms previous methods in maximizing forward edge weight.
...
Conclusion: The algorithms are efficiently implemented in Python and validated using cloud-based execution on Google Colab Pro+.
...
TL;DR: The paper discusses the limitations of the Structural Causal Model (SCM) framework in capturing human-like causal cognition and proposes leveraging human-like inductive biases for better AI systems.


<details>
  <summary>Details</summary>
Motivation: Current machine learning systems lack human-like causal understanding, which limits their generalization and learning efficiency. The SCM framework, while useful, fails to address certain aspects of human cognition, such as causal analogies between objects.

Method: The paper explores the differences between SCMs and human causal cognition, focusing on how humans generalize knowledge through causal analogies in everyday contexts.

Result: The analysis highlights the inadequacy of SCMs in capturing human-like causal reasoning, particularly in social and goal-driven contexts.

Conclusion: Future work should incorporate human-like causal properties to develop more capable, controllable, and interpretable AI systems.
...
TL;DR: A framework leveraging structural patterns (e.g., clustering, symmetry) to reduce computational complexity in NP-hard problems like TSP, achieving up to 79% solution quality gains.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of a small, trainable module in artificial consciousness research, akin to the perceptron in neural networks.

Method: RIIU is a recurrent cell with a meta-state and broadcast buffer, using sliding-window covariance and a differentiable Auto-$Φ$ surrogate to maximize local information integration.

Result: RIIUs are differentiable, additive, and perform $Φ$-monotone plasticity. In tests, a four-layer RIIU agent recovered >90% reward in 13 steps after actuator failure, outperforming GRUs.
...
<details>
  <summary>Details</summary>
Motivation: To assess whether LLMs' reasoning skills, often benchmarked on mathematical and coding tasks, generalize to complex real-world problems like site selection.

Method: Developed a benchmark with 300 diverse queries and a sandbox environment for constraint-based location search, testing models like OpenAI o4 and agentic strategies (ReAct, Reflexion).

Result: State-of-the-art models show limited improvement in real-world contexts, with OpenAI o4 failing 30% of tasks. Agentic strategies often perform worse due to over-reasoning.

Conclusion: LLMs struggle with holistic and non-linear reasoning in real-world tasks. LocationReasoner is released to encourage development of more robust reasoning models.
...
<details>
  <summary>Details</summary>
Motivation: There is a lack of techniques to assess the quality of AI explanations, especially in medical applications.

Method: The framework introduces four evaluation criteria and a scorecard for explainability methods, demonstrated using Ablation CAM and Eigen CAM on synthetic mammographies.

Result: The framework provides a structured way to assess explanation quality, with clinical relevance shown in the case study.

Conclusion: The proposed framework aims to improve AI explainability evaluation and foster dialogue in AI-based medical device development.
...
TL;DR: A novel method combines knowledge graphs and Bayesian networks for explainable disease risk prediction using multimodal EHR data, demonstrated with atrial fibrillation.


<details>
  <summary>Details</summary>
Motivation: Adapting general medical knowledge to specific healthcare settings and handling uncertainty in risk prediction while ensuring explainability.

Method: Integration of ontology-based knowledge graphs and Bayesian networks with multimodal EHR data.
...
Conclusion: The approach effectively addresses key challenges in disease risk prediction, offering a practical and explainable solution.
...
<details>
  <summary>Details</summary>
Motivation: Chatbots struggle with personalizing responses for varied user expertise, especially in specialized domains like IT/cybersecurity.

Method: ProfiLLM uses a domain-adaptable taxonomy and LLM-based profiling, demonstrated in ITSec with synthetic user data.

Result: ProfiLLM[ITSec] reduced the gap between actual and predicted proficiency scores by 55-65% after one prompt.

Conclusion: The framework offers a dynamic, implicit profiling solution, with tools and datasets for future research.
...
TL;DR: SANGAM is a framework using LLM-guided Monte Carlo Tree Search to automatically generate SystemVerilog Assertions (SVAs) from industry specs, outperforming recent methods.
...
Method: A three-stage approach: multi-modal specification processing, Monte Carlo Tree Self-Refine (MCTSr) for reasoning, and combining traces to generate SVAs.

Result: SANGAM generates robust SVAs, performing better in evaluations compared to recent methods.

Conclusion: The framework demonstrates the potential of LLMs in automating hardware assertion generation effectively.
...
<details>
  <summary>Details</summary>
Motivation: As AI systems achieve human-like fluency in tasks, they exhibit unique cognitive errors ('machine mirages') that mimic but differ from human fallibility. Understanding these is crucial for reliability and ethical AI development.

Method: The article identifies and categorizes various machine mirages (e.g., hallucination, bias amplification) and argues for their systematic study.

Result: The paper highlights the prevalence and diversity of machine mirages, underscoring their impact on AI reliability and ethics.

Conclusion: Machine mirages must be rigorously defined and assessed to enhance AI reliability and foster an ethical, co-evolving intelligence ecosystem.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of AI agents exploring and learning in open-ended environments by leveraging HRL's temporal structure.

Method: Covers methods for discovering temporal structure in HRL, including online learning, offline datasets, and LLMs.

Result: Identifies benefits of HRL for decision-making and performance trade-offs, and highlights suitable domains for structure discovery.

Conclusion: HRL is promising but faces challenges in defining and discovering useful temporal structure, with potential in specific problem domains.
...
<details>
  <summary>Details</summary>
Motivation: Traditional location prediction methods lack spatial explicitness, semantic context, and adaptability to new locations, limiting their effectiveness.
...
Result: CaLLiPer outperforms baselines in experiments, particularly in inductive scenarios with unseen locations.

Conclusion: Multimodal, inductive embeddings like CaLLiPer enhance mobility prediction, with potential for broader applications.
...
TL;DR: A benchmark for form-filling in the pure-image domain reveals poor performance of baseline VLAs and GUI agents, but FieldFinder improves accuracy significantly.


<details>
  <summary>Details</summary>
Motivation: Form filling in the pure-image domain is challenging due to lack of OCR or text access, requiring multi-modal understanding and tool-use.

Method: A benchmark with 432 fields across 55 documents and 3 tasks is introduced. FieldFinder, a tool to assist LLMs in text placement, is proposed.

Result: Baseline VLAs achieve <1% accuracy; GUI agents score 10.6-68.0%. FieldFinder boosts performance up to 56%.

Conclusion: FieldFinder significantly improves form-filling accuracy, addressing limitations of current agents.
...
TL;DR: The paper introduces a lightweight relevance grader (finetuned llama-3.2-1b) in RAG to improve document relevance, achieving precision comparable to larger models like llama-3.1-70b.
...
Result: Precision improved from 0.1301 to 0.7750, matching the performance of larger models.

Conclusion: Lightweight models can effectively serve as relevance graders in RAG, reducing computational overhead while maintaining accuracy.
...
<details>
  <summary>Details</summary>
Motivation: To systematically dissect positional biases in LLMs used for high-stakes decisions, uncovering their underlying preference structures and distinct failure modes.

Method: Comprehensive investigation across multiple LLM architectures and domains, analyzing order effects, centrality bias, and quality-dependent shifts. Introduces a framework to classify pairwise preferences.

Result: Strong positional biases, including primacy and recency effects, and a novel centrality bias. Models sometimes select inferior options, with biases stronger than gender biases.

Conclusion: LLMs exhibit unique biases not seen in humans. Proposed mitigation strategies, including adjusting the temperature parameter, can reduce distortions.
...
TL;DR: SCRL is a novel framework for sequential resource allocation with situational constraints, outperforming baselines in efficiency and constraint satisfaction.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of context-dependent resource allocation in real-world applications, where traditional methods fall short.

Method: Formalizes constraints as logic implications, introduces a dynamic penalty algorithm, and uses probabilistic selection to enhance constraint handling.

Result: SCRL outperforms existing baselines in scenarios like medical resource allocation and pesticide distribution, balancing efficiency and constraint satisfaction.

Conclusion: SCRL is effective for real-world, context-sensitive decision-making, offering a robust solution to situational constraints in resource allocation.
...
TL;DR: CoEM enables lightweight domain adaptation for LLMs by leveraging user-contributed knowledge and interactive dialogues, improving accuracy without heavy fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Overcome the reliance on large annotated datasets and computational resources for training vertical-domain LLMs.

Method: Constructs a knowledge pool from user snippets, uses dialogues and ratings to identify high-value fragments, and injects them via prompts.

Result: Significant improvement in domain-specific content generation, validated by user feedback in a financial scenario.

Conclusion: CoEM offers an efficient alternative to traditional fine-tuning, enhancing LLM performance in specialized domains.
...
<details>
  <summary>Details</summary>
Motivation: To understand how humans flexibly integrate diverse information sources (e.g., auditory, visual, language) to infer unseen objects.

Method: Proposes a neurosymbolic model combining neural networks for parsing multimodal inputs and Bayesian inference for hypothesis evaluation. Tests it with the "What's in the Box?" game.

Result: The model correlates strongly with human judgments, outperforming unimodal or purely neural baselines.

Conclusion: The neurosymbolic approach effectively mimics human-like flexible information integration.
...
<details>
  <summary>Details</summary>
Motivation: Existing ToM evaluations treat models as black boxes and lack interpretability. This study aims to address this gap by focusing on internal mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Address the gap in integrating recommendations within customer support interactions using LLMs, enhancing issue resolution and business growth.

Method: ImpReSS operates alongside support chatbots, identifying opportunities to recommend solution product categories (SPCs) based on conversations.

Result: Empirical results show high performance (e.g., MRR@1 of 0.72 for general problem solving).

Conclusion: ImpReSS demonstrates the potential of implicit recommendations in customer support, with data and code available for future research.
...
TL;DR: A test for abstract causal reasoning in AI is proposed, using neuron diagrams from philosophy. Advanced LLMs like ChatGPT, DeepSeek, and Gemini show promising results in identifying debated causes. A new, broader definition of cause in neuron diagrams is introduced, challenging the notion that such definitions are elusive.


<details>
  <summary>Details</summary>
Motivation: To evaluate AI's ability in abstract causal reasoning and bridge philosophical research with AI advancements.

Method: Proposes a test based on neuron diagrams, applied to LLMs (ChatGPT, DeepSeek, Gemini). Introduces a new definition of cause in neuron diagrams.

Result: LLMs demonstrate capability in identifying debated causes, suggesting potential for AI in philosophical research.

Conclusion: AI and human expertise can collaboratively advance philosophical research, as shown by the interplay in causal reasoning.
...
TL;DR: The paper resolves the paradox of RLVR-tuned models underperforming on $Pass@K$ by introducing $CoT$-$Pass@K$, a metric ensuring correct reasoning paths and answers, and validates RLVR's ability to enhance reasoning.


<details>
  <summary>Details</summary>
Motivation: Address the contradiction where RLVR-tuned models underperform on $Pass@K$ despite their potential, due to the metric's flaw in crediting incorrect reasoning paths.

Method: Introduce $CoT$-$Pass@K$ to evaluate both reasoning paths and answers, and provide theoretical and empirical validation of RLVR's reasoning incentives.

Result: RLVR improves reasoning generalization under $CoT$-$Pass@K$, with enhanced capabilities emerging early in training.

Conclusion: RLVR, evaluated correctly, genuinely advances machine reasoning by incentivizing logical integrity.
...
TL;DR: Mxplainer is a search algorithm that converts into a neural network to analyze black-box Mahjong AI agents, providing human-understandable insights and local explanations of their decision-making.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between high-performing Mahjong AI agents and human understanding by making their decision-making processes interpretable.

Method: Introduces Mxplainer, a parameterized search algorithm that converts into a neural network to learn and explain black-box AI agent parameters.

Result: Demonstrates that learned parameters offer insights into AI agents' play styles and enables local explanations of their decisions.

Conclusion: Mxplainer successfully provides interpretability for black-box Mahjong AI agents, aiding human learning and understanding.
...
TL;DR: The paper demonstrates that deep learning, combined with on-the-fly NN training and novel techniques like TTFT and AIRV, achieves state-of-the-art performance on the ARC-AGI challenge, with significant accuracy boosts.


<details>
  <summary>Details</summary>
Motivation: The ARC-AGI challenge is difficult for AI systems, and deep learning, despite its limitations, remains the most effective strategy for training skillful neural networks. The paper aims to fully leverage deep learning's capacity for abstraction to improve performance on ARC.

Method: The approach involves on-the-fly NN training, treating both the neural network and optimizer as part of inference. Techniques like Test-Time Fine-Tuning (TTFT) and Augment Inference Reverse-Augmentation and Vote (AIRV) are proposed to enhance reasoning.

Result: The method achieved a 260% accuracy boost with AIRV and a further 300% boost with TTFT, securing first place in the 2023 ARCathon and the best score (58%) on the ARC private test-set.

Conclusion: Deep learning, when fully committed to acquiring novel abstractions, can effectively tackle ARC, highlighting key mechanisms for robust reasoning in unfamiliar domains.
...
TL;DR: The paper introduces ADRD, a framework using LLMs to create interpretable, rule-based decision systems for autonomous driving, outperforming traditional methods in interpretability, speed, and performance.
...
Method: ADRD integrates three modules (Information, Agents, Testing) to aggregate driving scenarios, generate rule-based tactics, and refine them iteratively.
...
TL;DR: The paper introduces RALA-DPO, a method combining Direct Preference Optimization and Retrieval-Augmented Generation to enhance aviation training by providing accurate, high-quality responses to professional queries.


<details>
  <summary>Details</summary>
Motivation: The limited availability of instructors and unreliable online answers in aviation training necessitate a more accurate and efficient solution.

Method: The authors fine-tune the Qwen LLM using DPO for domain alignment and integrate RAG to mitigate hallucinations and improve response accuracy.

Result: RALA-DPO improves accuracy in aviation knowledge responses and enables zero-cost knowledge updates.

Conclusion: RALA-DPO effectively addresses the challenges of aviation training by combining DPO and RAG, offering precise and up-to-date knowledge.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods overlook degradation of safety alignment capabilities, like expressing ignorance, during fine-tuning, leading to issues like hallucinations.
...
TL;DR: The paper evaluates hybrid AST-based representations (CFG, DFG, FA-AST) in GNNs for code clone detection, finding AST+CFG+DFG improves accuracy, while FA-AST often harms performance. GMN excels even with standard AST.


<details>
  <summary>Details</summary>
Motivation: Code clones increase maintenance costs and risks, but ASTs lack semantic depth. Enriched AST representations' effectiveness with GNNs is unclear.

Method: Empirical study comparing hybrid AST representations (CFG, DFG, FA-AST) across GNN architectures (GCN, GAT, GMN).

Result: AST+CFG+DFG boosts accuracy for GCN/GAT, FA-AST often reduces performance. GMN performs best with standard AST.

Conclusion: Hybrid representations vary in impact; GMN's superiority suggests enriched structures aren't always necessary for effective clone detection.
...
TL;DR: GUI-Robust is a dataset for evaluating GUI agents, incorporating real-world anomalies and reducing annotation time via a semi-automated method.


<details>
  <summary>Details</summary>
Motivation: Existing GUI datasets lack diversity and realism, ignoring common anomalies in real-world deployments.

Method: A semi-automated paradigm using RPA tools and MLLMs to collect and annotate user action sequences.

Result: Reduced annotation time by 19x and revealed performance drops in state-of-the-art GUI agents under anomalies.

Conclusion: GUI-Robust emphasizes robustness in GUI agents and encourages future research in this area.
...
TL;DR: The paper compares traditional swarm intelligence (e.g., Boids, ACO) with LLM-driven swarms, assessing decentralization, scalability, and emergence in AI. It evaluates performance and resource usage, highlighting LLMs' potential and challenges.
...
Method: Implemented and compared traditional swarm algorithms (Boids, ACO) with LLM-driven swarms, evaluating latency, resource usage, and behavioral accuracy. Also assessed cloud-based vs. local LLMs for swarm use.

Result: LLMs offer advanced reasoning but introduce computational and coordination challenges, impacting traditional swarm design principles. Performance and suitability vary between cloud and local LLMs.

Conclusion: The study underscores the potential and limitations of LLMs in swarms, prompting a reevaluation of 'swarm' definitions in AI research.
...
TL;DR: The paper proposes a safety-first human-like decision-making framework (SF-HLDM) for autonomous vehicles to navigate dense, interactive traffic by combining intention inference, behavior regulation, and efficient search methods.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles struggle with time-varying traffic and human unpredictability, leading to poor migratability and high search costs in data-driven methods.

Method: The SF-HLDM framework integrates a spatial-temporal attention mechanism, social compliance estimation, and Deep Evolutionary Reinforcement Learning for flexible, interpretable decisions.

Result: The framework enables dynamic adjustment of decision parameters for safety and context-appropriate behavior, avoiding local optima and overfitting.

Conclusion: SF-HLDM improves AV decision-making in complex traffic, balancing safety, comfort, and social compatibility.
...
TL;DR: The paper addresses risks of prompt hijacking in autonomous agents, proposing the Doppelgänger method to expose vulnerabilities, defining PACAT for evaluation, and introducing CAT prompts for defense.


<details>
  <summary>Details</summary>
Motivation: The convenience of prompt engineering raises safety and robustness concerns, especially regarding prompt exposure and adversarial attacks.

Method: The Doppelgänger method demonstrates prompt hijacking risks, PACAT evaluates vulnerability, and CAT prompts provide defense.

Result: Doppelgänger compromises agent consistency and exposes internal info; CAT prompts effectively defend against such attacks.

Conclusion: The study highlights prompt hijacking risks and validates CAT prompts as a defensive solution.
...
TL;DR: QUEST is a quality-aware semi-supervised framework for table extraction from business documents, improving accuracy and reducing errors by leveraging unlabeled data and novel quality metrics.


<details>
  <summary>Details</summary>
Motivation: Automating table extraction from business documents is challenging due to sparse annotations and unreliable multi-stage pipelines. Existing SSL methods rely on poor confidence scores.
...
Result: On proprietary and DocILE datasets, QUEST improved F1 scores (64% to 74% and 42% to 50%) and reduced empty predictions (45% and 19%).

Conclusion: QUEST's interpretable quality assessments and robustness to sparse annotations make it ideal for business documents, ensuring structural consistency and data completeness.
...
TL;DR: The paper proposes enhancing symbolic machine learning with neural embeddings to improve efficiency and performance, outperforming baselines in F1 score.


<details>
  <summary>Details</summary>
Motivation: To address inefficiencies of neuro-symbolic AI systems like LTNs and DeepProbLog in simpler settings, particularly discriminative ML with many constants.

Method: Enhances symbolic ML (e.g., TILDE) by integrating neural embeddings for constants in similarity predicates, with fine-tuning based on symbolic theory.

Result: Outperforms baseline methods in F1 score across three real-world domains.

Conclusion: The approach is effective and extendable to other tasks like analogical reasoning or propositionalization.
...
<details>
  <summary>Details</summary>
Motivation: Current foundation models lack the capability to handle the spatial, temporal, and semantic complexity of mobility data, limiting scalable and transferable analysis.

Method: Proposes a shift from discrete points of interest to dynamic, context-rich places, integrating geolocation semantics with human mobility across scales.

Result: Identifies key research directions for modeling places and enabling efficient learning to develop scalable, context-aware models.

Conclusion: Spatial foundation models can revolutionize geospatial intelligence, enabling smarter spatial decision-making in applications like urban planning and logistics.
...
TL;DR: AgentDistill is a training-free framework for distilling LLM-based agents by reusing structured task-solving modules (MCPs) from teachers, enabling efficient knowledge transfer and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing agent distillation methods struggle with dynamic planning in novel environments, motivating a scalable, training-free approach.

Method: AgentDistill reuses Model-Context-Protocols (MCPs) from teacher agents for knowledge transfer, avoiding step-by-step imitation.

Result: Student agents built on small LLMs match performance of advanced systems like GPT-4o on biomedical and mathematical tasks.

Conclusion: AgentDistill offers a scalable, cost-efficient solution for distilling intelligent agents without extensive training.
...
TL;DR: LC-R1, a post-training method, reduces reasoning chain length by 50% with minimal accuracy drop (~2%) by eliminating redundant "invalid thinking" in Large Reasoning Models.


<details>
  <summary>Details</summary>
Motivation: Large Reasoning Models (LRMs) often produce verbose reasoning chains due to "invalid thinking," where models redundantly double-check correct answers.

Method: Introduces LC-R1, based on Group Relative Policy Optimization (GRPO), using Length Reward for conciseness and Compress Reward to remove invalid reasoning.

Result: Achieves ~50% reduction in sequence length with only ~2% accuracy drop, optimizing the Pareto frontier for high compression.

Conclusion: LC-R1 offers a robust solution for efficient LRMs, balancing brevity and sufficiency in reasoning chains.
...
TL;DR: The paper introduces an automated method to construct climate change instruction data, creating the ClimateChat-Corpus dataset and fine-tuning LLMs (ClimateChat), which improves performance on climate-related tasks.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of efficient methods to produce high-precision climate change instruction data, which limits the development of climate-specific LLMs.

Method: Automated generation of instructions using document facts and background knowledge, enhanced by web scraping and seed instruction collection.

Result: ClimateChat outperforms on climate Q&A tasks, with evaluations showing the impact of base models and instruction data on performance.

Conclusion: The study provides empirical support for constructing climate instruction data and training specialized LLMs, highlighting the importance of base model selection.
...
Conclusion: Flexibly inferring compositional rules from implicit patterns remains a challenge for LLMs.
...
TL;DR: The paper introduces an iterative training framework to improve Vision-Language Reward Models (VL-RMs) by addressing challenges like bootstrapping dilemmas and modality bias, using vision experts and structured critiques.


<details>
  <summary>Details</summary>
Motivation: Existing methods for aligning Vision-Language (VL) models with reinforcement learning face challenges like biased self-generated data and modality bias, hindering effective training of VL-RMs.

Method: Proposes an iterative framework using vision experts, Chain-of-Thought rationales, and Margin-based Rejection Sampling to refine preference datasets and enhance reasoning.

Result: The approach outperforms benchmarks in hallucination detection and multimodal reasoning, improving VL model alignment.

Conclusion: The iterative framework effectively addresses VL-RM training challenges, advancing reinforcement learning for VL models.
...
TL;DR: A task-oriented spoken dialogue system (SDS) is developed to regulate emotional speech in news conversations, improving empathy and engagement.


<details>
  <summary>Details</summary>
Motivation: The lack of integration between SDS and emotional TTS research, and the absence of standardized metrics for social goals, motivated the development of an emotional SDS for news conversations.

Method: The system uses an LLM-based sentiment analyzer to identify emotions and PromptTTS for synthesizing context-appropriate speech. A subjective evaluation scale is proposed for assessment.

Result: The emotional SDS outperformed a baseline system in emotion regulation and engagement, highlighting the importance of speech emotion in conversations.

Conclusion: Speech emotion plays a critical role in engaging conversations, and the proposed system demonstrates its effectiveness. The source code is open-sourced.
...
TL;DR: The paper introduces the Alignment Quality Index (AQI) to assess LLM alignment by analyzing latent space separation of safe and unsafe activations, addressing blind spots in current evaluations.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of LLM alignment rely on flawed proxies (e.g., refusal rates, toxicity classifiers), leaving models vulnerable to jailbreaking and alignment faking. AQI aims to provide a robust, prompt-invariant metric.

Method: AQI combines geometric clustering measures (DBS, DI, XBI, CHI) to evaluate latent space separation. The LITMUS dataset is introduced for empirical validation.

Result: AQI effectively detects hidden misalignments and jailbreak risks, correlating with external judges and revealing vulnerabilities missed by refusal metrics.

Conclusion: AQI offers a decoding-invariant tool for safety auditing, with potential to improve LLM alignment in high-stakes domains.
...
TL;DR: A novel framework for data augmentation in robotic assistance uses large language and stable diffusion models to generate multimodal training data, improving action selection accuracy.
...
Method: Leverages large language models for simulated dialogues and stable diffusion for environmental imagery to augment training data.

Result: Significantly improves robot action selection, achieving state-of-the-art performance on real-world datasets.

Conclusion: The proposed framework effectively addresses data scarcity, enhancing multimodal models for robotic assistance.
...
TL;DR: The paper explores automating the retrieval of legal interpretations by optimizing manual annotation processes, including determining annotation volume, selection methods, and using LLMs.


<details>
  <summary>Details</summary>
Motivation: Manual annotation for legal concept interpretations is costly and repetitive, prompting research into automation to improve efficiency.

Method: Experiments were conducted to assess optimal annotation volume, sentence selection methods (random vs. best candidates), and automation using LLMs.

Result: Findings address the need for manual annotation, optimal annotation counts, performance gains from selective annotation, and LLM-based automation outcomes.

Conclusion: Automating annotation and optimizing selection methods can reduce costs and improve efficiency in legal interpretation retrieval.
...
TL;DR: The paper explores how AI, particularly LLMs, aligns with human emotional representation in language and how emotional tone can be controlled. It finds structural congruence between AI and human emotion spaces and demonstrates precise emotional modulation in AI outputs using human-centric concepts.


<details>
  <summary>Details</summary>
Motivation: To understand if LLMs represent emotions like humans and how their emotional tone can be controlled, ensuring meaningful human-AI collaboration.

Method: Assessed human-AI emotional alignment using interpretable LLM features for nuanced emotion categories, analyzed structural congruence, and tested predictive accuracy on word ratings. Used steering vectors for emotional modulation.

Result: LLM-derived emotion spaces align with human perception, underpinned by valence and arousal. Emotion features predict word ratings accurately. Human-centric concepts enable stable emotional modulation in AI outputs.

Conclusion: AI shares emotional representations with humans, and its affective outputs can be precisely guided using psychologically grounded concepts, enhancing human-AI collaboration.
...
TL;DR: The paper evaluates how Large Language Models (LLMs) handle code-switched text, showing mixed results with degradation in some cases and improved comprehension in others. Fine-tuning is suggested as a stable solution.


<details>
  <summary>Details</summary>
Motivation: Understanding how LLMs process code-switched text is crucial due to its prevalence in multilingual communities and online content.

Method: The study generates code-switched variants of established benchmarks to systematically evaluate LLM comprehension.

Result: Degradation occurs when foreign tokens disrupt English, but embedding English into other languages often improves comprehension. Prompting yields mixed results, while fine-tuning mitigates degradation.

Conclusion: Fine-tuning is a more stable approach to improving LLM performance on code-switched text compared to prompting.
...
TL;DR: MultiFinBen is a multilingual, multimodal benchmark for evaluating LLMs in the financial domain, introducing novel tasks and revealing limitations of current models.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for financial NLP are limited to monolingual and unimodal settings, lacking complexity and real-world relevance.

Method: The paper introduces MultiFinBen, a benchmark with tasks across text, vision, and audio in various linguistic settings, including novel tasks like PolyFiQA and OCR-embedded QA. A dynamic difficulty-aware selection mechanism is used.

Result: Evaluation of 22 models shows they struggle with complex cross-lingual and multimodal financial tasks.

Conclusion: MultiFinBen aims to advance financial NLP by providing a transparent, reproducible, and inclusive benchmark.
...
TL;DR: A review of 28 papers (2020-2025) on commonsense reasoning and intent detection, highlighting trends and gaps in NLP and HCI.
...
Conclusion: Bridges NLP and HCI insights, emphasizing the need for improved grounding and benchmark design.
...
<details>
  <summary>Details</summary>
Motivation: Address the need for assessing language difficulty in conversational text for LLM training and filtering.

Method: Create Ace-CEFR dataset with expert annotations; test Transformer-based models and LLMs.

Result: Models trained on Ace-CEFR measure text difficulty more accurately than humans with low latency.
...
TL;DR: A method for detecting and annotating naturally-occurring English embedded clauses in large corpora using parsing heuristics, validated on a hand-annotated dataset (GECS) and applied to extract a large dataset from Dolma.
...
Method: Uses constituency parsing and parsing heuristics to detect and annotate naturally-occurring embedded clauses.

Result: Developed a tool validated on GECS and extracted a large dataset from Dolma.

Conclusion: The approach provides a way to study embedded clauses using natural language data, filling a gap in current research.
...
TL;DR: The paper addresses hallucination in LLMs for clinical discharge summaries, proposing a method combining language-based graphs and deep learning to improve reliability and trustworthiness.


<details>
  <summary>Details</summary>
Motivation: Hallucination in LLMs poses risks in clinical settings, especially for discharge summaries. Automating summaries could reduce physician workload.

Method: Combines language-based graphs and deep learning models to ensure content provenance and trustworthiness.

Result: Demonstrates impressive reliability on MIMIC-III corpus and clinical notes from Anonymous Hospital.

Conclusion: The proposed method effectively mitigates hallucination, enhancing trust in automated discharge summaries.
...
TL;DR: Essential-Web v1.0 is a 24-trillion-token dataset with a twelve-category taxonomy, enabling efficient data filtering for competitive performance in various domains.


<details>
  <summary>Details</summary>
Motivation: The lack of massive, well-organized pre-training datasets leads to costly and inaccessible data pipelines, prompting the creation of Essential-Web v1.0.

Method: The dataset is annotated using EAI-Distill-0.5b, a fine-tuned model, and allows SQL-style filtering for domain-specific datasets.

Result: Competitive performance is achieved in math, web code, STEM, and medical domains compared to state-of-the-art datasets.

Conclusion: Essential-Web v1.0 provides a scalable and efficient solution for high-quality pre-training datasets.
...
TL;DR: A method to convert autoregressive LMs with BPE tokenizers into character/byte-level LMs, solving tokenization issues like the Prompt Boundary Problem and enabling model ensembling.
...
Method: Inference-time conversion of BPE-tokenized LMs to character/byte-level LMs without altering text-level generative distribution.

Result: Solves PBP, unifies vocabularies for ensembling, and improves performance in downstream tasks.

Conclusion: The method effectively addresses tokenization challenges, enhancing model interoperability and performance.
...
TL;DR: The paper introduces DCRM, a metric to measure the quality of response pairs for preference optimization, and proposes a method to improve model performance by selecting high-DCRM pairs.


<details>
  <summary>Details</summary>
Motivation: The differences between preferred and dispreferred responses in datasets may not align with desirable learning outcomes, motivating the need for a better metric.

Method: The authors use distance and reward margin to quantify response differences, combining them into DCRM. They then analyze preference datasets and propose a best-of-N² pairing method to select high-DCRM pairs.

Result: Higher DCRM correlates with better learning outcomes. The proposed method improves model performance on benchmarks like AlpacaEval, MT-Bench, and Arena-Hard.

Conclusion: DCRM effectively measures dataset quality for preference optimization, and the best-of-N² pairing method enhances model performance.
...
TL;DR: S$^4$C improves speculative sampling by leveraging syntactic and semantic coherence, achieving faster token generation and validation with fewer resources.


<details>
  <summary>Details</summary>
Motivation: Autoregressive LLMs suffer from high inference latency, limiting real-time applications. Existing speculative sampling methods ignore text coherence, reducing efficiency.

Method: Proposes S$^4$C, using multi-head drafting and a continuous verification tree for faster token generation and validation.

Result: S$^4$C outperforms baselines, achieving 2.26x-2.60x acceleration on Spec-bench benchmarks.

Conclusion: S$^4$C enhances efficiency and parallelism in LLM inference, enabling faster and more coherent text generation.
...
TL;DR: The paper proposes a framework using the Stereotype Content Model to evaluate implicit bias in LLMs' Theory of Mind, introducing indirect tasks (WABT and AAT) to reveal multi-dimensional bias structures.


<details>
  <summary>Details</summary>
Motivation: Existing methods for evaluating bias in LLMs' ToM are limited by social desirability effects and fail to capture its subtle, multi-dimensional nature.

Method: The framework uses the Stereotype Content Model (SCM) and introduces two indirect tasks: Word Association Bias Test (WABT) and Affective Attribution Test (AAT).
...
Conclusion: The proposed framework provides a robust methodology for identifying the structural nature of implicit bias in LLMs.
...
TL;DR: The paper introduces a generative reward model for aligning LLMs, trained with both unlabeled and labeled data, showing improved generalization across tasks.


<details>
  <summary>Details</summary>
Motivation: Current reward models rely solely on labeled human preference data, limiting their potential. The paper explores combining unlabeled and labeled data for better training.

Method: Develops a generative reward model trained via unsupervised learning first, then fine-tuned with supervised learning. Uses label smoothing to optimize a regularized pairwise ranking loss.

Result: The model generalizes well across tasks like response ranking, reinforcement learning from human feedback, and task adaptation, outperforming baselines.

Conclusion: The approach links generative and discriminative models under a unified training objective, offering a versatile foundation reward model.
...
TL;DR: The paper proposes a phrase-level mixing method to generate synthetic code-switching (CS) data for improving ASR performance in under-resourced Southeast Asian languages. Fine-tuning pretrained models with this data enhances accuracy, especially for Malay-English.


<details>
  <summary>Details</summary>
Motivation: Code-switching in multilingual settings lacks sufficient transcribed data, making ASR challenging. The study aims to address this by creating synthetic CS data.

Method: A phrase-level mixing method generates synthetic CS data, which is used to fine-tune large pretrained ASR models (Whisper, MMS, SeamlessM4T) for three language pairs.

Result: The training strategy improves ASR performance on both monolingual and CS tests, with Malay-English showing the highest gains.

Conclusion: Synthetic CS data offers a cost-effective solution for CS-ASR development, benefiting research and industry.
...
TL;DR: AsyncSwitch is a framework for improving code-switched ASR by pre-training on text-rich web data and fine-tuning on speech-text data, achieving a 9.02% WER reduction.


<details>
  <summary>Details</summary>
Motivation: Challenges in code-switched ASR include language ambiguity and costly data collection, motivating a scalable solution.

Method: A three-stage process: (1) pre-train decoder layers on code-switched text, (2) align decoder and encoder with limited speech-text data, and (3) fine-tune the full model.

Result: 9.02% relative WER reduction in Malay-English code-switching, with improved monolingual performance in Singlish, Malay, and English variants.

Conclusion: AsyncSwitch effectively leverages web data to enhance code-switched ASR performance and scalability.
...
Result: MAS-LitEval achieved scores up to 0.890 in capturing literary nuances, outperforming traditional metrics.
...
TL;DR: ELI-Why benchmark evaluates language models' pedagogical capabilities using 13.4K 'Why' questions. GPT-4 explanations fit intended educational backgrounds 50% of the time, lagging behind human-curated explanations (79%). Learners also found GPT-4 explanations 20% less suited to their needs.


<details>
  <summary>Details</summary>
Motivation: To assess language models' ability to tailor explanations for learners with diverse informational needs and knowledge backgrounds.

Method: Introduces ELI-Why benchmark and conducts two human studies: educators evaluate explanation fit for grades, and learners assess suitability for their needs. Compares GPT-4 and human-curated explanations.

Result: GPT-4 explanations matched intended grades 50% of the time (vs. 79% for humans) and were 20% less suited to learners' needs. Automated metrics showed indistinguishable grade-level outputs.

Conclusion: Current language models struggle to tailor explanations effectively for varied educational needs, highlighting a gap in pedagogical capabilities.
...
TL;DR: The paper explores using language models to help anomia patients by addressing unseen terms and semantic errors, achieving strong results.


<details>
  <summary>Details</summary>
Motivation: To assist anomia patients in identifying target items despite term failures and semantic paraphasia errors.

Method: Proposes gradient-based selective augmentation to robustify models against semantic errors and include unseen terms.

Result: Demonstrates strong performance on the Tip-of-the-Tongue dataset and real patient data from AphasiaBank.

Conclusion: The approach effectively aids anomia patients by tackling key challenges in the identification process.
...
<details>
  <summary>Details</summary>
Motivation: High-quality training data is crucial for generative AI, but privacy and quality issues persist. Synthesized data helps, but tabular data generation remains challenging due to its complexity.

Method: CausalDiffTab is a diffusion model for mixed tabular data, incorporating hybrid adaptive causal regularization via Hierarchical Prior Fusion to balance performance and generative capability.

Result: Tests on seven datasets show CausalDiffTab outperforms baseline methods in all metrics.

Conclusion: CausalDiffTab effectively generates high-quality mixed tabular data, addressing key challenges and demonstrating superior performance.
...
TL;DR: The paper introduces an improved method for detecting implicit influential linguistic patterns in digital conversations, enhancing detection accuracy and multi-label classification performance.


<details>
  <summary>Details</summary>
Motivation: Malicious actors increasingly use implicit verbal patterns to influence individuals, necessitating better detection methods beyond explicit patterns.

Method: The approach augments an existing dataset using advanced language models and identifies influential elements within conversations.

Result: The framework improved implicit pattern detection by 6% and multi-label classification tasks by 33% (techniques) and 43% (vulnerability).

Conclusion: The proposed model effectively detects and locates implicit influential patterns, offering significant improvements over existing methods.
...
TL;DR: The paper introduces the Timeline Reasoning Network (TRN) to improve temporal relation understanding in tasks like TRC and TRE by addressing unreliable answer overlaps through a two-step reasoning process involving event time spans.


<details>
  <summary>Details</summary>
Motivation: Existing methods for temporal relation tasks rely on answer overlaps, which can be unreliable due to coincidental identical answers for dissimilar questions.

Method: Proposes TRN, a two-step approach: (1) initial question answering using semantic/syntactic info, (2) chaining questions to predict a timeline for grounding answers.

Result: TRN outperforms previous methods on TORQUE, TB-dense, TRC, and TRE tasks by resolving spurious overlaps.

Conclusion: TRN effectively addresses the limitations of answer overlaps by leveraging timeline reasoning for more accurate temporal relation understanding.
...
TL;DR: Xolver is a multi-agent reasoning framework for LLMs that integrates diverse experience modalities, outperforming specialized models in complex reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Current LLMs lack experiential knowledge integration, unlike expert problem solvers who leverage past experiences and collaboration.

Method: Xolver uses a persistent, evolving memory system with external/self-retrieval, tool use, collaboration, evaluation, and iterative refinement.

Result: Xolver achieves state-of-the-art performance on benchmarks like GSM8K, AIME, Math-500, and LiveCodeBench-V5, even with lightweight models.

Conclusion: Holistic experience learning is crucial for advancing LLMs toward expert-level reasoning and generalist capabilities.
...
TL;DR: Proposes MESH framework for temporal knowledge graph reasoning, integrating structural and semantic information via expert modules, outperforming previous methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to integrate dual reasoning perspectives (structural and semantic) and lack differentiation between historical and non-historical events, limiting generalization.

Method: Introduces MESH framework with three expert modules to combine structural and semantic reasoning for diverse prediction scenarios.

Result: Demonstrates effectiveness through extensive experiments on three datasets.

Conclusion: MESH successfully integrates dual reasoning perspectives and improves generalization across temporal contexts.
...
TL;DR: A novel token learning method aligns tool tokens with word embeddings to enhance LLMs' tool integration, improving performance in tasks like numerical reasoning and plan generation.
...
Method: Proposes initializing and regularizing tool token embeddings based on tool names/descriptions to align with word token space.

Result: Demonstrates clear improvements over baselines (CoT, REACT, ICL, ToolkenGPT) in tasks like numerical reasoning and plan generation.

Conclusion: The approach effectively enhances LLMs' tool integration by aligning tool tokens with word embeddings, improving accuracy across diverse domains.
...
TL;DR: The paper introduces a novel task of timely dialogue response generation, proposes the TimelyChat benchmark, and trains Timer, a dialogue agent that outperforms baselines in generating time-conditioned responses.
...
Method: Leverages a temporal commonsense knowledge graph and an LLM to synthesize 55K event-driven dialogues, then trains Timer to predict time intervals and generate timely responses.

Result: Timer outperforms prompting-based LLMs and fine-tuned baselines in evaluations.

Conclusion: The work advances dialogue systems by incorporating temporal context, with publicly released data, model, and code.
...
TL;DR: The paper introduces ECPO, a multi-turn preference optimization paradigm for Conversational Recommendation Agents (CRAs), leveraging Expectation Confirmation Theory to improve user satisfaction and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current CRAs using LLMs often produce short-sighted responses, and existing preference optimization methods are costly and ineffective in multi-turn dialogues.

Method: The authors propose ECPO, which models user satisfaction evolution and targets unsatisfactory responses for optimization, supported by an LLM-based user simulator (AILO).

Result: ECPO significantly enhances CRA interaction capabilities, outperforming existing methods in efficiency and effectiveness.

Conclusion: ECPO offers a cost-effective and impactful solution for improving multi-turn conversational recommendations.
...
TL;DR: The paper investigates how the choice of reference sets affects reference-based summarization metrics, revealing significant instability in popular metrics like ROUGE and weak correlation with human judgments for diverse genres.


<details>
  <summary>Details</summary>
Motivation: To address the overlooked impact of reference set variation on summarization evaluation and its implications for metric reliability, especially with diverse communication styles.

Method: Analyzes three multi-reference datasets (SummEval, GUMSum, DUC2004) and evaluates metric sensitivity to reference sets, supplemented by human judgments on LLM outputs.

Result: Popular metrics, particularly n-gram-based ones like ROUGE, show instability, with model rankings varying by reference set. Human judgments correlate weakly or not at all with metrics for diverse genres.

Conclusion: Recommends accounting for reference set variation in summarization evaluation to improve consistency and alignment with human judgments, especially for LLMs.
...
TL;DR: The paper proposes enhancing deep research systems with geo-temporal reasoning to address context-rich questions, highlighting technical, infrastructural, and evaluative challenges.


<details>
  <summary>Details</summary>
Motivation: Current deep research systems lack geo-temporal capabilities, limiting their ability to answer questions with geographic or temporal constraints, which are common in fields like public health and environmental science.

Method: The paper advocates for augmenting retrieval and synthesis processes with geo-temporal reasoning, supported by open infrastructures and rigorous evaluation.

Result: The vision outlines a path for developing more advanced, geo-temporally aware deep research systems.

Conclusion: Integrating geo-temporal reasoning into deep research systems could significantly improve AI-driven information access, with broad potential impact.
...
TL;DR: Google's search algorithms bias content visibility, suppressing certain hashtags/subreddits (e.g., explicit, conspiracy, ads, crypto) while promoting high-engagement content, shaping public discourse.


<details>
  <summary>Details</summary>
Motivation: To understand how search engines like Google act as gatekeepers by selectively curating content from platforms like Reddit and Twitter/X, influencing user access to information.

Method: Comparison of Google search results with nonsampled data from Reddit and Twitter/X to identify biases in content visibility.

Result: Google suppresses explicit, conspiracy, ad, and crypto-related content while favoring high-engagement material, revealing systematic curation biases.

Conclusion: Google's algorithmic gatekeeping significantly impacts public discourse by controlling the visibility of social media narratives.
...
TL;DR: The paper proposes a two-step LLM-based framework to generate critical questions for debates, aiming to foster deeper reasoning over superficial learning.


<details>
  <summary>Details</summary>
Motivation: Concerns about LLMs promoting superficial learning and undermining critical thinking led to exploring their potential for deeper reasoning by challenging unsupported claims.

Method: A two-step framework with a Questioner (generates candidate questions) and a Judge (selects relevant ones) using small-scale open-source LLMs.

Result: The system ranked first in the shared task, proving its effectiveness in critical question generation.

Conclusion: The approach shows promise for encouraging critical engagement with argumentative texts using LLMs.
...
TL;DR: Thunder-NUBench is a new benchmark for evaluating sentence-level negation understanding in LLMs, addressing gaps in existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks treat negation as a side case, lacking focus on deep semantic understanding of negation.

Method: Thunder-NUBench includes manually curated sentence-negation pairs and a multiple-choice dataset, contrasting standard negation with diverse alternatives.

Result: The benchmark enables in-depth evaluation of LLMs' negation understanding.

Conclusion: Thunder-NUBench fills a critical gap in assessing negation understanding in LLMs.
...
TL;DR: ImpliRet is a benchmark focusing on document-side reasoning for retrieval systems, where relevance depends on implicit facts in documents. Current retrievers perform poorly, and even advanced models like GPT-4.1 struggle.


<details>
  <summary>Details</summary>
Motivation: Existing retrieval benchmarks focus on query-side processing for complex queries, but document-side reasoning remains understudied. ImpliRet addresses this gap by evaluating retrievers on implicit document facts.

Method: ImpliRet introduces a benchmark with simple queries but relevance based on implicit document facts (e.g., temporal, arithmetic, world knowledge). It evaluates sparse/dense retrievers and long-context models like GPT-4.1.

Result: Current retrievers perform poorly (best nDCG@10: 15.07%). GPT-4.1 scores only 35.06% even with a short context, highlighting the challenge of document-side reasoning.

Conclusion: Document-side reasoning is a significant challenge for retrieval systems, as shown by ImpliRet. Future work should focus on improving models' ability to process implicit document facts.
...
TL;DR: The paper investigates the long-context capabilities of diffusion LLMs, comparing them to auto-regressive LLMs, and introduces LongLLaDA, a training-free method for context extension.


<details>
  <summary>Details</summary>
Motivation: To address the unexplored long-context capabilities of diffusion LLMs and provide systematic analysis and methods for context extension.

Method: Systematic comparison of diffusion and auto-regressive LLMs, analysis of perplexity stability and local perception, and proposal of LongLLaDA using NTK-based RoPE extrapolation.

Result: Diffusion LLMs show stable perplexity and local perception, enabling successful retrieval in long-context tasks. LongLLaDA validates effective context extension for diffusion LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on static knowledge, but intelligence requires rapid learning from experience. The paper aims to assess LLMs' ability to improve during test time.

Method: Proposes semantic games as testbeds for evaluating test-time learning, with an objective framework comparing model performance under limited and cumulative experience settings.

Result: LLMs show measurable test-time learning, but improvements are less stable and slower than humans.

Conclusion: LLMs have potential as learning machines, but a significant intellectual gap remains between models and humans, beyond static benchmarks.
...
TL;DR: LexiMark is a stealthy dataset watermarking method using synonym substitutions for high-entropy words to detect unauthorized LLM training data use, outperforming existing methods in detection reliability.


<details>
  <summary>Details</summary>
Motivation: Existing dataset watermarking methods lack stealth and are easily detectable or removable, necessitating a more subtle and effective solution.

Method: LexiMark embeds synonym substitutions for high-entropy words, enhancing LLM memorization without altering semantics, making the watermark hard to detect or remove.

Result: Evaluation on multiple LLMs and training settings shows LexiMark significantly improves AUROC scores over existing methods.

Conclusion: LexiMark provides a robust and stealthy solution for verifying unauthorized use of watermarked data in LLM training.
...
TL;DR: LingoLoop is an attack method exploiting MLLMs' vulnerabilities by inducing verbose, repetitive outputs, increasing tokens and energy use by up to 30x.


<details>
  <summary>Details</summary>
Motivation: MLLMs are resource-intensive; attackers exploit this by inducing excessive output. Prior attacks lacked focus on token-level POS and structural patterns, limiting effectiveness.

Method: LingoLoop uses a POS-Aware Delay Mechanism to postpone EOS tokens and a Generative Path Pruning Mechanism to induce repetitive loops.

Result: Experiments show LingoLoop increases tokens and energy use by up to 30x, pushing MLLMs to their limits.

Conclusion: LingoLoop exposes MLLMs' vulnerabilities, challenging reliable deployment.
...
Result: Outperforms traditional DL models in accuracy and robustness, with performance improving with more sensing modalities.
...
TL;DR: AlphaDecay adaptively assigns weight decay strengths to each module in LLMs based on spectral properties, improving performance over uniform decay.


<details>
  <summary>Details</summary>
Motivation: Uniform weight decay overlooks structural diversity and varying spectral properties in LLMs, necessitating adaptive decay.

Method: AlphaDecay uses HT-SR theory to analyze weight correlation matrices' ESD, assigning decay inversely proportional to heavy-tailedness.

Result: AlphaDecay outperforms uniform decay and other adaptive methods in perplexity and generalization across model sizes (60M to 1B).

Conclusion: Adaptive weight decay based on spectral properties enhances LLM training, as demonstrated by AlphaDecay's superior performance.
...
TL;DR: GenerationPrograms is a modular framework for text generation that improves attribution quality and interpretability by decomposing the process into planning and execution stages.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs lack fine-grained attribution and interpretability in source-conditioned text generation, undermining trust.

Method: Decomposes generation into creating an executable program plan (modular text operations) and executing it to produce the response.

Result: Significantly improves attribution quality at document and sentence levels, outperforming traditional methods.

Conclusion: GenerationPrograms enhances verifiability, interpretability, and allows localized refinement for better attribution.
...
TL;DR: GG (Guaranteed Guess) is a novel ISA-centric transpilation pipeline combining LLMs and software testing for efficient CISC-to-RISC translation, outperforming Rosetta 2 in speed, energy efficiency, and memory usage.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of translating low-level programs between CISC and RISC architectures efficiently and correctly, enhancing code portability and longevity.

Method: Uses pre-trained LLMs for initial translation and embeds results in a software-testing framework to ensure correctness and high code coverage (>98%).

Result: Achieves 99% correctness on HumanEval and 49% on BringupBench, with 1.73x faster runtime, 1.47x better energy efficiency, and 2.41x better memory usage than Rosetta 2.
...
<details>
  <summary>Details</summary>
Motivation: To explore if semantic information (AMR) enhances pretrained models' generalization in NLI tasks.

Method: Integrate AMR into NLI via fine-tuning and prompting, then analyze performance and conduct an ablation study.

Result: AMR in fine-tuning harms generalization; prompting with AMR slightly boosts GPT-4o but misleads models due to surface-level differences.

Conclusion: AMR's benefits in NLI are superficial, potentially misleading models without aiding true semantic reasoning.
...
TL;DR: A framework synthesizes multiple LLMs' moral judgments into a consensus, fine-tuning misaligned models to improve moral reasoning consistency.


<details>
  <summary>Details</summary>
Motivation: Address discrepancies in LLMs' moral reasoning when faced with complex dilemmas.
...
TL;DR: The study explores using LLMs for coding open-ended German survey responses, comparing models and prompting methods. Fine-tuned LLMs perform best, but performance varies by model and category.
...
Result: Performance varies by LLM; fine-tuned models achieve satisfactory results. Prompting effectiveness depends on the LLM. Uneven performance across categories affects distributions.

Conclusion: LLMs show potential but require careful selection and fine-tuning. Trade-offs exist for automated coding in survey research.
...
TL;DR: Adding CoT exemplars to ICL does not improve reasoning in strong models like Qwen2.5; enhanced CoT exemplars also fail. Models ignore exemplars, focusing on instructions.


<details>
  <summary>Details</summary>
Motivation: To investigate whether CoT exemplars still benefit recent, stronger LLMs in mathematical reasoning tasks.

Method: Systematic experiments with traditional and enhanced CoT exemplars on models like Qwen2.5 and DeepSeek-R1.

Result: CoT exemplars do not improve reasoning performance; models ignore them and focus on instructions.

Conclusion: Current ICL+CoT framework has limitations in mathematical reasoning, requiring re-examination of ICL paradigms and exemplar definitions.
...
TL;DR: Fine-tuned LLMs can replicate and amplify polarizing discourse, producing human-like biased content, raising ethical concerns about AI in political manipulation.


<details>
  <summary>Details</summary>
Motivation: Concerns about LLMs exacerbating ideological polarization through automated generation of persuasive, biased content.

Method: Fine-tuned an open-source LLM on politically charged Reddit discussions, evaluated outputs via linguistic analysis, sentiment scoring, and human annotation.

Result: LLMs produce highly plausible, provocative comments indistinguishable from human-written ones when trained on partisan data.

Conclusion: Highlights ethical risks of AI in political discourse, calls for governance, regulation, and detection tools to mitigate adversarial fine-tuning.
...
TL;DR: GuiLoMo introduces a fine-grained strategy for LoRA-MoE, using GuidedSelection Vectors (GSVs) to optimize expert numbers and ranks per layer, improving performance over uniform approaches.


<details>
  <summary>Details</summary>
Motivation: Current LoRA-MoE methods suffer from uniform rank assignment and task-agnostic expert allocation, limiting their potential.

Method: GuiLoMo uses GSVs learned via bilevel optimization to adaptively allocate expert numbers and ranks per layer, tailored to model and task needs.

Result: Experiments on three models show GuiLoMo outperforms or matches baselines, with insights into adaptive expert configurations.

Conclusion: GuiLoMo demonstrates the effectiveness of adaptive expert allocation, enhancing LoRA-MoE's performance and flexibility.
...
TL;DR: SFT aligns LLMs with human values, but its nuances are unclear. Training 1,000+ models revealed key dataset properties, layer-wise changes, and the predictive power of perplexity for SFT success.


<details>
  <summary>Details</summary>
Motivation: To better understand the critical but poorly understood aspects of supervised fine-tuning (SFT) in aligning large language models with human instructions and values.
...
Result: Found persistent training-task synergies, model-specific strategies, perplexity as a strong predictor of SFT effectiveness, and mid-layer weight changes linked to performance gains.

Conclusion: SFT success depends on model-specific strategies and perplexity, with mid-layer changes being crucial. The release of models and benchmarks aims to advance research.
...
TL;DR: The paper addresses improving machine learning model performance on underrepresented, long-tail data by optimizing training protocols and introducing control levers for better inference-time adaptability.


<details>
  <summary>Details</summary>
Motivation: Large models struggle with rare or underrepresented features, and post-training adaptations like prompt engineering are unreliable. The work aims to enhance controllability and performance for such cases.

Method: The authors develop a taxonomy of data characteristics and task provenance to control generation attributes. They fine-tune a base model to infer these markers, making them optional during inference.
...
Conclusion: The proposed method significantly enhances model performance on long-tail data, offering a flexible and principled solution for underrepresented use cases.
...
TL;DR: The paper explores how model architecture and data configurations affect memorization in generative transformers, finding embedding size and activation functions key, while additional layers may hinder performance.


<details>
  <summary>Details</summary>
Motivation: To understand how model design and data complexity influence the memorization capacity of generative transformers, using structured medical data.

Method: Training models on synthetic text datasets from SNOMED (triplets and sequences) to evaluate memorization.

Result: Embedding size is crucial for learning; extra layers may reduce performance. Softmax is stable, and data complexity boosts memorization.

Conclusion: Provides insights into transformer memory and a framework for optimizing models with structured data.
...
TL;DR: Ring-lite is a MoE-based LLM optimized via RL, achieving SOTA performance with fewer parameters. It introduces C3PO for stable RL training, entropy-based checkpoint selection, and a two-stage training paradigm for multi-domain data.
...
Method: Joint training pipeline integrating distillation with RL, C3PO for stability, entropy-based checkpoint selection, and two-stage training for multi-domain data.

Result: Matches SOTA performance on benchmarks (AIME, LiveCodeBench, GPQA-Diamond) with one-third of the parameters of comparable models.

Conclusion: Ring-lite demonstrates efficient reasoning via RL optimization, with contributions in training stability, checkpoint selection, and multi-domain integration.
...
TL;DR: The paper explores the role of entropy in enhancing exploratory reasoning in language models (LMs) by linking high-entropy regions to key reasoning actions. It introduces a simple RL modification to boost reasoning depth, achieving notable performance gains.


<details>
  <summary>Details</summary>
Motivation: To address the imbalance between exploration and exploitation in LMs, which often leads to performance plateaus, by leveraging entropy as a signal for exploratory reasoning.

Method: Augments the advantage function in RL with an entropy-based term to promote longer and deeper reasoning chains, rather than just uncertainty.

Result: Strong correlations between high-entropy regions and exploratory reasoning actions, with significant improvements on the Pass@K metric, even for large K.

Conclusion: The proposed method effectively enhances LM reasoning by strategically encouraging exploration, pushing the boundaries of LM capabilities.
...
TL;DR: The paper introduces an autoregressive U-Net that dynamically learns token embeddings, replacing static tokenization methods like BPE, enabling multi-scale text processing and improved semantic prediction.


<details>
  <summary>Details</summary>
Motivation: Static tokenization methods like BPE limit model flexibility and granularity, freezing input processing and prediction scope. The authors aim to overcome this rigidity by embedding tokenization within the model.

Method: An autoregressive U-Net is proposed, which processes raw bytes into multi-scale embeddings (words, word pairs, up to 4 words), allowing deeper stages to focus on broader semantic patterns while earlier stages handle fine details.

Result: Shallow hierarchies match strong BPE baselines, while deeper hierarchies show promising trends in semantic prediction. The model also handles character-level tasks and transfers knowledge across low-resource languages.

Conclusion: Dynamic tokenization via the U-Net offers flexibility, multi-scale processing, and improved semantic understanding, outperforming static methods like BPE.
...
<details>
  <summary>Details</summary>
Motivation: Existing speech tokens from self-supervised models focus on linguistic aspects but neglect prosody, leading to unnatural speech. Adding pitch features is insufficient and requires manual effort.

Method: An end-to-end variational approach is introduced to automatically learn and encode continuous speech attributes into semantic tokens.

Result: The method eliminates manual feature extraction and produces speech continuations preferred by human raters.

Conclusion: The proposed approach effectively enhances speech tokens by capturing paralinguistic attributes, improving naturalness without manual intervention.
...
TL;DR: LittleBit is a novel method for extreme LLM compression, achieving 31x memory reduction and superior performance in sub-1-bit quantization.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of memory and computational costs in deploying large language models (LLMs) through quantization, especially in the sub-1-bit regime.

Method: Uses low-rank matrix factorization to represent weights, binarizes factors, and integrates a multi-scale compensation mechanism. Introduces Dual-SVID for stable QAT initialization and Residual Compensation to mitigate errors.

Result: Achieves 31x memory reduction (e.g., Llama2-13B to under 0.9 GB) and outperforms leading methods, with potential for 5x speedup compared to FP16.
...
TL;DR: MobiEdit is a mobile knowledge editing framework for LLMs, enabling efficient personalization on mobile devices by replacing backpropagation with quantized forward-only gradient estimation.
...
Result: MobiEdit achieves 7.6× less memory, 14.7× less energy, and 3.6× less latency for editing a 3B-parameter model on mobile devices.

Conclusion: MobiEdit enables real-time, efficient LLM personalization on mobile devices, addressing the limitations of prior methods.
...
TL;DR: JobShopLib is a modular library introduced to simplify experimentation with deep learning models for job shop scheduling, outperforming traditional methods and highlighting feature customization's importance.


<details>
  <summary>Details</summary>
Motivation: The job shop scheduling problem is NP-hard and traditionally relies on simple heuristics. Deep learning models, like GNNs, offer potential but require extensive customization, lacking modular tools for efficient research.

Method: The paper introduces JobShopLib, a modular library for customizing graph representation, node features, action space, and reward functions in a reinforcement learning environment. It uses imitation learning to train dispatchers.

Result: One model outperformed graph-based dispatchers using only individual operation features, and a GNN model achieved near state-of-the-art results on large-scale problems.

Conclusion: JobShopLib facilitates future experimentation, showing significant potential for improving deep learning models in job shop scheduling.
...
TL;DR: The paper introduces an enhanced bagging ensemble model (EBMBag+) for predicting diabetes prevalence in U.S. cities, outperforming baseline models with high accuracy.
...
Method: Integrated diabetes datasets (2011-2021) and developed EBMBag+ for time series forecasting, comparing it with baseline models like SVMReg, BDTree, LSBoost, NN, LSTM, and ERMBag.

Result: EBMBag+ achieved the best performance (MAE: 0.41, RMSE: 0.53, MAPE: 4.01, R2: 0.9).

Conclusion: EBMBag+ is effective for diabetes prevalence prediction, aiding targeted healthcare interventions.
...
TL;DR: A hybrid meta-learning framework combining physics-inspired simulation and deep learning models for forecasting and anomaly detection in nonlinear systems.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of forecasting and anomaly detection in nonlinear dynamical systems with nonstationary and stochastic behavior, where complete physical models may not be available.

Method: Integrates CNN-LSTM for spatio-temporal features, VAE for anomaly scoring, Isolation Forests for outlier detection, and DA-RNN for forecasting, combined via a meta-learner.
...
TL;DR: A hypothesis testing framework is introduced to validate and compare concept-based methods in deep neural networks, improving interpretability and reducing spurious correlations.


<details>
  <summary>Details</summary>
Motivation: Current concept-based methods lack statistical rigor, making it hard to validate and compare techniques for interpreting model embeddings.

Method: A hypothesis testing framework quantifies rotation-sensitive structures in CLIP embeddings, followed by a post-hoc concept decomposition method with theoretical guarantees.

Result: The method improves reconstruction error and interpretability, achieving a 22.6% increase in worst-group accuracy by removing spurious concepts.

Conclusion: The proposed framework enhances the reliability and interpretability of concept-based approaches, mitigating spurious correlations in model behavior.
...
TL;DR: The paper introduces an evolvable conditional diffusion method for guiding generative processes using black-box, non-differentiable multi-physics models, validated in AI for Science scenarios like fluidic topology and meta-surface design.


<details>
  <summary>Details</summary>
Motivation: To enable autonomous scientific discovery by leveraging non-differentiable multi-physics models (common in fields like computational fluid dynamics) for generative guidance without requiring derivatives.

Method: Formulates guidance as an optimization problem, optimizing a fitness function by updating the descriptive statistic for the denoising distribution, derived from probabilistic evolution principles.

Result: The method generates designs that better meet optimization objectives without relying on differentiable proxies, validated in fluidic topology and meta-surface design.

Conclusion: The evolvable diffusion method provides an effective, derivative-free approach for guidance-based diffusion, leveraging black-box multi-physics models in scientific applications.
...
TL;DR: T-REX is an open-source framework for evaluating RL-based traffic signal control under dynamic, incident-driven scenarios, revealing trade-offs in method performance and advocating for robustness-aware design.


<details>
  <summary>Details</summary>
Motivation: The robustness of RL-TSC under real-world disruptions like traffic incidents is underexplored, necessitating a standardized evaluation framework.

Method: T-REX simulates realistic traffic disruptions (e.g., rerouting, congestion) and evaluates RL-TSC methods using extended robustness metrics.

Result: Independent methods excel in stable conditions but degrade under incidents, while hierarchical methods offer stability at the cost of complexity.

Conclusion: Robustness-aware design is crucial for RL-TSC, and T-REX provides a reproducible platform for benchmarking under dynamic scenarios.
...
TL;DR: The paper explores energy-efficient retraining techniques for ML systems, balancing accuracy and sustainability.


<details>
  <summary>Details</summary>
Motivation: To address the high energy consumption and environmental impact of traditional ML model retraining methods.

Method: Study the energy consumption of common retraining techniques and compare them in terms of energy efficiency and accuracy.

Result: Retraining with recent data saves 25% energy, and updating only when necessary saves up to 40%.

Conclusion: Recommends energy-efficient retraining techniques for sustainable ML systems.
...
TL;DR: SatHealth is a novel dataset combining multimodal spatiotemporal data (environmental, satellite, disease prevalence, SDoH) to improve AI models in healthcare. It enhances performance and generalizability, with a web tool for easy access.


<details>
  <summary>Details</summary>
Motivation: Existing studies lack long-term, fine-grained environmental data, limiting AI model performance in healthcare. SatHealth addresses this gap.

Method: Developed SatHealth dataset with multimodal data, tested in regional public health modeling and personal disease risk prediction.

Result: Environmental data significantly improves AI model performance and spatiotemporal generalizability.

Conclusion: SatHealth provides a foundational framework for integrating environmental data in healthcare research, with ongoing updates and a web tool for accessibility.
...
TL;DR: The paper introduces StaQ, a PMD-like algorithm for RL that retains only the last M Q-functions, ensuring convergence without policy update errors, and matches deep RL baselines with less oscillation.


<details>
  <summary>Details</summary>
Motivation: To address the intractability of the closed-form solution in Policy Mirror Descent (PMD) due to summing all past Q-functions, while maintaining stability and performance in deep RL.

Method: Proposes PMD-like algorithms (StaQ) that store only the last M Q-functions, proving convergence for finite M without introducing policy update errors.

Result: StaQ achieves competitive performance with deep RL baselines, reduces performance oscillation, and offers theoretical guarantees.

Conclusion: StaQ provides a stable and practical implementation of PMD, enabling further experimentation and development of stable deep RL algorithms.
...
TL;DR: Mamba (using S6 models) outperforms transformers in Algorithm Distillation for In-Context Reinforcement Learning, especially in long-context scenarios, matching SOTA online meta RL.


<details>
  <summary>Details</summary>
Motivation: Address the limitations of transformers (quadratic complexity, short horizons) in Algorithm Distillation for ICRL by leveraging S6 models.

Method: Use Mamba (built with S6 layers) for AD, testing on complex, continuous Meta RL environments.

Result: Mamba shows superiority over transformers in AD, and scaling to long contexts improves ICRL performance.

Conclusion: S6-based models like Mamba enable efficient, scalable AD for ICRL, matching SOTA performance.
...
TL;DR: A framework for estimating feature contributions in rule-based systems is proposed, featuring graph-based visualization, a novel importance metric, and a rule set comparison metric. Tested on clinical datasets, it aids in identifying risk factors and biomarkers, outperforming benchmarks.


<details>
  <summary>Details</summary>
Motivation: Rule-based systems are preferred for interpretability in critical domains like healthcare, but their complexity makes feature analysis challenging.

Method: Proposes a framework with graph-based visualization, a feature importance metric, and a rule set distance metric, tested on clinical datasets and four rule-based methods.

Result: Uncovers insights on clinical features' predictive value, aids in identifying risk factors and biomarkers, and outperforms benchmarks in robustness.

Conclusion: The framework enhances interpretability and utility of rule-based systems, offering competitive performance and practical insights for healthcare applications.
...
TL;DR: GITO is a graph-informed transformer operator for solving PDEs on irregular geometries, combining a hybrid graph transformer and transformer neural operator for improved performance.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of learning PDE systems on irregular geometries and non-uniform meshes, which existing methods struggle with.
...
TL;DR: The paper explores few-shot learning (FSL) for industrial time-series data, introducing a label-aware episodic sampler and comparing metric-based and gradient-based FSL paradigms. Results show lightweight CNNs with metric learning outperform large models like transformers in data-scarce scenarios.


<details>
  <summary>Details</summary>
Motivation: FSL is promising for industrial time-series data due to high annotation costs for defects. The study aims to benchmark FSL on screw-fastening process monitoring and improve performance with novel sampling.

Method: A label-aware episodic sampler collapses multi-label sequences into single-label tasks. Two FSL paradigms (Prototypical Network and MAML) are tested with three backbones (1D CNN, InceptionTime, transformer Moment).
...
Conclusion: Lightweight CNNs with metric learning generalize better in data-scarce scenarios, challenging the superiority of large models. The study releases resources to promote FSL adoption in manufacturing.
...
Result: HEGNNs outperform traditional GNNs, with or without local homomorphism count features, and can distinguish graphs up to isomorphism.

Conclusion: HEGNNs provide a practical and expressive extension to GNNs, bridging gaps in graph isomorphism testing and outperforming existing architectures.
...
TL;DR: BSVGD extends SVGD with a branching mechanism for better exploration of multimodal distributions, offering theoretical convergence guarantees and validated performance.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of SVGD in exploring multimodal distributions by introducing a branching mechanism for improved state space exploration.

Method: Extends SVGD with a random branching mechanism (BSVGD), includes theoretical convergence analysis and numerical experiments.

Result: BSVGD outperforms SVGD in exploring multimodal distributions, validated by Wasserstein distance and computational time comparisons.

Conclusion: BSVGD is a promising extension of SVGD for multimodal distributions, with theoretical and empirical support.
...
TL;DR: RLVR training improves reasoning models by compressing pass@k into pass@1 and enabling capability gain. Self-distillation drives problem-solving, and natural language guidance enhances pass@k rates. The new Guide algorithm adaptively incorporates hints, improving generalization.


<details>
  <summary>Details</summary>
Motivation: To understand how reasoning models trained with RLVR learn to solve new problems and improve their performance through capability gain and self-distillation.

Method: Analyzed RLVR training across model scales (0.5B to 72B) on >500,000 problems. Introduced Guide, an online training algorithm that adaptively incorporates hints and adjusts sampling ratios.

Result: Guide improves generalization, with up to 4% macro-average improvement on math benchmarks. Capability gain exists across scales, but self-distillation is primary driver.

Conclusion: RLVR and Guide enhance reasoning models by leveraging hints and self-distillation, leading to significant performance gains.
...
Conclusion: ReinDSplit enhances split learning by integrating RL for resource efficiency, privacy, and scalability in heterogeneous environments.
...
TL;DR: The paper introduces a post-hoc explainability framework to analyze how intrinsic motivation shapes embeddings in Elastic Decision Transformers (EDTs), revealing distinct representational structures and environment-specific performance correlations.


<details>
  <summary>Details</summary>
Motivation: To understand the representational mechanisms behind the performance improvements in EDTs when intrinsic motivation is incorporated.

Method: A systematic post-hoc explainability framework analyzing embedding properties (covariance, vector magnitudes, orthogonality) to compare intrinsic motivation variants.

Result: Different intrinsic motivation variants create unique representational structures, with environment-specific correlations between embedding metrics and performance.
...
<details>
  <summary>Details</summary>
Motivation: Prior MIA research focuses on performance metrics but overlooks disparities among attacks, affecting reliability.

Method: A novel framework based on coverage and stability analysis is used to investigate MIA disparities.

Result: Experiments reveal significant disparities in MIAs, their causes, and implications for privacy evaluation.

Conclusion: An ensemble framework is proposed to enhance MIA reliability and provide robust privacy evaluation.
...
TL;DR: The paper analyzes Local Gradient Descent for logistic regression with separable, heterogeneous data, showing convergence with any stepsize η>0, improving upon existing rates.


<details>
  <summary>Details</summary>
Motivation: Existing analyses require restrictive stepsizes (η≤1/K) for monotonic decrease. This work explores the impact of larger stepsizes and heterogeneous data on convergence.

Method: Analyzes Local Gradient Descent for logistic regression with separable, heterogeneous data, allowing any stepsize η>0.

Result: Convergence at rate 𝒪(1/ηKR) after an initial unstable phase of 𝒪̃(ηKM) rounds, improving the 𝒪(1/R) rate for general smooth, convex objectives.

Conclusion: The work demonstrates faster convergence with flexible stepsizes, highlighting instability from large local updates in heterogeneous settings.
...
TL;DR: HAELT, a hybrid deep learning framework, outperforms in high-frequency stock price prediction by combining noise-mitigation, temporal attention, and adaptive ensembling.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges like non-stationarity, noise, and volatility in high-frequency stock price prediction.

Method: Proposes HAELT with ResNet-based noise reduction, temporal self-attention, and a hybrid LSTM-Transformer core, adaptively ensembled.

Result: Achieves highest F1-Score on AAPL hourly data, effectively identifying price movements.
...
<details>
  <summary>Details</summary>
Motivation: Addressing drawbacks of conventional methods like overfitting and poor generalization in imbalanced datasets for expert systems.

Method: Integrates quantum-inspired layers, dynamic mixup, and a hybrid loss function to enhance minority class representation and classification robustness.

Result: Outperforms 20 state-of-the-art baselines in macro-F1 and recall on 18 real-world datasets.

Conclusion: QCL-MixNet sets a new benchmark for handling tabular imbalance, validated by theoretical and empirical results.
...
TL;DR: AssistedDS benchmark evaluates LLMs' ability to critically use domain knowledge in tabular prediction tasks, revealing gaps in handling adversarial info and real-world data.


<details>
  <summary>Details</summary>
Motivation: To determine if LLMs can critically leverage external domain knowledge like human data scientists.

Method: Introduces AssistedDS benchmark with synthetic and real-world datasets, assessing LLMs on submission validity, recall, and predictive performance.

Result: LLMs often uncritically adopt info, struggle with adversarial content, and make errors in time-series and categorical data.

Conclusion: Highlights a gap in LLMs' ability to critically evaluate domain knowledge, calling for more robust automated systems.
...
<details>
  <summary>Details</summary>
Motivation: Long-sequence training is challenging for the AI community due to lack of system support and memory inefficiencies in existing solutions.

Method: ALST combines attention-agnostic single-GPU and multi-GPU memory optimizations.

Result: ALST supports 500K tokens on a single H100 GPU, 3.7M on an 8xH100 node, and 15M on a 4-node cluster.

Conclusion: ALST makes long-sequence training accessible for HF models, with open-source availability.
...
TL;DR: A novel statistical framework and SAE training algorithm, Group Bias Adaptation (GBA), are introduced to improve feature recovery in LLMs, offering theoretical guarantees and superior empirical performance.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of rigorous guarantees and practical issues like hyperparameter sensitivity in existing SAE training algorithms for interpreting LLMs.

Method: Proposes a statistical framework for feature identifiability and a bias adaptation-based SAE training algorithm, with an empirical variant (GBA).

Result: Theoretically proves correct recovery of monosemantic features and demonstrates GBA's superiority in LLMs up to 1.5B parameters.

Conclusion: Advances SAE training with theoretical guarantees, enhancing mechanistic interpretability for more transparent AI systems.
...
TL;DR: The paper identifies a vulnerability in machine unlearning (MU) for large language models (LLMs), where unlearning leaves detectable traces in model behavior and internal representations, posing risks of reverse-engineering forgotten information.


<details>
  <summary>Details</summary>
Motivation: To investigate the persistence of detectable traces ("fingerprints") in LLMs post-unlearning, which could compromise privacy, copyright, and harm mitigation efforts.

Method: A supervised classifier is used to detect unlearning traces from textual outputs and intermediate activations, analyzing their propagation and low-dimensional manifolds in activation space.

Result: Experiments show over 90% accuracy in detecting unlearning traces, even with forget-irrelevant inputs, revealing measurable signatures in LLMs.

Conclusion: Unlearning leaves detectable traces, introducing a new risk of reverse-engineering forgotten information, highlighting the need for more robust unlearning methods.
...
TL;DR: BWFlow is a flow-matching framework for graph generation that addresses the limitations of Euclidean assumptions in existing methods by modeling joint node-edge evolution using MRFs and optimal transport, achieving competitive performance and stable training.


<details>
  <summary>Details</summary>
Motivation: Existing graph generation methods assume Euclidean space and model nodes/edges independently, which is suboptimal for graphs' non-Euclidean structure and interconnected patterns, risking sampling convergence.

Method: BWFlow models graphs as connected systems parameterized by MRFs, leveraging optimal transport displacement to design a probability path, and integrates this into a flow-matching framework for graph generation.
...
Conclusion: BWFlow provides a geometry-respecting, effective solution for graph generation, outperforming traditional Euclidean-based methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for inverse elasticity estimation are unstable, noise-sensitive, and struggle with absolute-scale Young's modulus recovery.
...
Method: Introduces a load balancing loss that encourages consistent expert choices for similar inputs, preserving relational structure.

Result: 36% faster convergence and lower redundancy compared to existing methods.

Conclusion: The proposed loss enhances MoE model efficiency and performance by improving routing consistency.
...
Method: ScIReN uses an interpretable encoder to predict latent parameters and a differentiable process-based decoder. It includes a hard-sigmoid constraint layer for parameter range restriction.

Result: ScIReN outperforms black-box models in tasks like carbon flow simulation and ecosystem respiration modeling, while revealing latent scientific mechanisms.

Conclusion: ScIReN successfully integrates interpretability and scientific reasoning, offering both predictive accuracy and novel scientific insights.
...
TL;DR: The paper proposes an online learning algorithm for selective generation to control hallucination in large language models by leveraging bandit problems and partial feedback.


<details>
  <summary>Details</summary>
Motivation: Addressing the hallucination effect in large language models by enabling selective generation under partial feedback from users.

Method: Reduces selective generation to multi-armed bandit problems, introduces a conversion lemma, and exploits arm structure for feedback unlocking.

Result: The algorithm controls false discovery rate (FDR) while maintaining selection efficiency, outperforming baselines.
...
TL;DR: The CVDP benchmark introduces 783 hardware design problems across 13 categories to test LLMs and agents, revealing significant gaps in current capabilities, especially in agentic tasks like RTL reuse and verification.


<details>
  <summary>Details</summary>
Motivation: To advance LLM and agent research in hardware design and verification by providing a realistic and challenging benchmark.
...
Conclusion: CVDP highlights the need for further research to improve robust, real-world hardware design automation.
...
TL;DR: The paper proposes MSFT, a multi-scale finetuning framework for time series foundation models (TSFMs), addressing the limitations of naive finetuning and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Naive finetuning of TSFMs often leads to overfitting and suboptimal performance, failing to leverage their multi-scale capabilities.

Method: The authors introduce MSFT, a framework that explicitly models multiple scales during finetuning, tested on encoder-based TSFMs.
...
Conclusion: Explicit multi-scale modeling in finetuning enhances TSFM performance, making MSFT a superior approach for downstream tasks.
...
TL;DR: Sparse transformers, particularly input-dependent ones, improve learnability and generalization, while input-agnostic ones do not. Theoretical analysis links softmax stability and Lipschitz properties to these outcomes.


<details>
  <summary>Details</summary>
Motivation: To understand how sparse attention mechanisms affect learnability and generalization in transformers, beyond just computational efficiency.

Method: Empirical study of various attention mechanisms, comparing input-dependent and input-agnostic sparse attention. Theoretical analysis of softmax stability and Lipschitz properties.

Result: Input-dependent sparse attention converges faster and generalizes better, while input-agnostic sparse attention shows no benefits. Theoretical conditions validate these findings.

Conclusion: Input-dependent sparse attention enhances learning by focusing semantic attention, supported by both empirical and theoretical evidence.
...
TL;DR: The paper proposes a graph foundation model using Transformer architecture, addressing challenges in encoding diverse graphs via random walks and a novel context prediction loss.


<details>
  <summary>Details</summary>
Motivation: To explore if foundation models, like GPT for natural language, can be built for graphs, given their diverse sizes and domains.

Method: Represents nodes as multiple random walks, enabling Transformers to extract node, edge, and graph representations, with a novel context prediction loss.

Result: The model is pre-trained and adapted to downstream tasks, showing promise as a foundation for graph-structured data.

Conclusion: The approach demonstrates potential for a graph foundation model, leveraging Transformers and random walks for diverse graph processing.
...
<details>
  <summary>Details</summary>
Motivation: To leverage the linearity of Koopman operators while addressing their infinite-dimensional challenge by linking them to linear RNNs.

Method: Introduces SKOLR, combining learnable spectral decomposition with MLPs for measurement functions and structured Koopman operators via parallel linear RNN stacks.

Result: SKOLR achieves exceptional performance in forecasting benchmarks and dynamical systems.

Conclusion: The Koopman-theory-based design of SKOLR offers a streamlined and effective approach for nonlinear system analysis and forecasting.
...
Method: Evaluated 7 GNN architectures and 30 loss functions on 3 datasets in inductive and transductive settings, using 21 metrics.
...
Conclusion: Multi-objective optimization (hybrid losses) and architecture choice (GIN) are key for robust GNN performance.
...
TL;DR: Proposes CLGNN, a scalable and inductive contrastive learning-based GNN for accurate and efficient Temporal Betweenness Centrality (TBC) prediction, addressing class imbalance and temporal dependencies.
...
Result: CLGNN achieves significant speedup (663.7x) over exact TBC methods, outperforms static GNNs (31.4x lower MAE, 16.7x higher Spearman), and surpasses temporal GNNs (5.7x lower MAE, 3.9x higher Spearman).

Conclusion: CLGNN effectively addresses TBC prediction challenges, offering scalability, accuracy, and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To challenge the assumption that improvements in one stage of the model pipeline (pre-training, fine-tuning, upcycling) propagate downstream, focusing on the effects of expert fine-tuning on upcycling.
...
Result: Long fine-tuning degrades merging and downstream performance. Memorization of difficult examples during fine-tuning leads to forgetting during merging.

Conclusion: A task-dependent aggressive early stopping strategy can significantly enhance upcycling performance by mitigating the negative effects of prolonged fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: Decision trees suffer from predictive equivalence, where multiple trees can represent the same decision boundary, complicating model selection and interpretation.

Method: The authors propose a boolean logical representation of decision trees that eliminates predictive equivalence and remains faithful to the decision boundary.

Result: The new representation shows decision trees' robustness to missing feature values, improves variable importance quantification, and enables cost-effective prediction optimization.
...
TL;DR: Neural policies can generalize as well as programmatic policies on OOD problems with simple training adjustments, challenging the common belief.


<details>
  <summary>Details</summary>
Motivation: To reevaluate the generalization capabilities of neural policies compared to programmatic policies on OOD problems, highlighting overlooked potential in neural methods.
...
Conclusion: Benchmarks should better highlight OOD generalization challenges, and neural policies can match programmatic performance with proper training tweaks.
...
TL;DR: The paper explores MARL in the LAG environment, comparing HAPPO and HASAC algorithms. HASAC excels in simpler tasks, while HAPPO adapts better to dynamic scenarios.


<details>
  <summary>Details</summary>
Motivation: To understand the performance and adaptability of on-policy (HAPPO) and off-policy (HASAC) MARL methods in cooperative-competitive combat settings.

Method: Evaluates HAPPO (hierarchical PPO) and HASAC (hierarchical SAC) in the LAG environment, analyzing training stability, rewards, and coordination.

Result: HASAC performs better in simple coordination tasks, while HAPPO excels in dynamic scenarios like missile combat.

Conclusion: The study highlights trade-offs between on-policy and off-policy MARL methods, offering insights for algorithm selection in multi-agent combat environments.
...
Method: Uses a Kolmogorov-Arnold Network generator with energy-based priors, trained via Maximum Likelihood. Incorporates scalable extensions like mixture distributions and Langevin Monte Carlo for flexibility.

Result: Achieves fast inference, improved learning efficiency, and high-quality samples. The prior is recoverable and visualizable post-training.

Conclusion: The approach successfully balances training stability, inference speed, and generation quality, connecting classical and modern methods.
...
<details>
  <summary>Details</summary>
Motivation: To develop a general framework for creating explainable and effective OOD detection features in neural networks.

Method: Introduces a novel loss functional combining KL divergence for separating ID and OOD distributions and the Information Bottleneck for feature compression. A variational procedure optimizes the loss to derive OOD features.

Result: The theory predicts a new shaping function that outperforms existing methods on OOD benchmarks.

Conclusion: The proposed framework offers a versatile approach for generating explainable OOD detection features, with demonstrated superiority over current techniques.
...
TL;DR: DiffusionBlocks is a memory-efficient training framework for large neural networks by partitioning them into independently trainable blocks, optimizing noise levels, and maintaining competitive performance.
...
Method: Partitions the network into trainable blocks, interprets them as denoising operations in a diffusion process, and optimizes noise level assignments.

Result: Achieves significant memory reduction proportional to block count while maintaining or surpassing performance in generative tasks.

Conclusion: DiffusionBlocks offers a scalable solution for training large networks with limited resources, democratizing access to advanced AI.
...
TL;DR: TriGuard is a safety evaluation framework for deep neural networks, combining robustness verification, attribution entropy, and a novel Attribution Drift Score to assess model reliability under adversarial and distributional shifts.
...
Method: TriGuard integrates formal robustness verification, attribution entropy, and Attribution Drift Score to evaluate model safety.
...
Conclusion: TriGuard advances robust and interpretable model evaluation by providing complementary safety insights beyond adversarial accuracy.
...
<details>
  <summary>Details</summary>
Motivation: SGNNs struggle with suboptimal filters in label-scarce scenarios, while LLMs show potential for enhancing GNNs. The paper explores whether LLMs can address SGNN limitations.

Method: A lightweight pipeline leverages LLMs to generate homophily-aware priors, which guide polynomial spectral filter design in SGNNs.

Result: Experiments show the LLM-driven SGNN outperforms baselines in homophilic and heterophilic settings with minimal overhead.

Conclusion: LLMs can effectively enhance SGNNs by providing homophily-aware priors, improving adaptability and performance.
...
TL;DR: DP-Ditto extends Ditto for PFL with differential privacy, balancing privacy, convergence, and fairness, outperforming other PFL models.


<details>
  <summary>Details</summary>
Motivation: Address privacy concerns in PFL and analyze trade-offs between privacy, convergence, and fairness.

Method: Extends Ditto with differential privacy (DP), analyzes convergence bounds, and optimizes global aggregations.
...
TL;DR: Latent-space monitors detect undesirable behaviors in LLMs using internal representations. RL-Obfuscation shows LLMs can evade token-level monitors but not holistic ones like max-pooling or attention-based probes.


<details>
  <summary>Details</summary>
Motivation: To investigate if LLMs can learn to evade latent-space monitors designed to detect behaviors like deception and unsafe completions.

Method: Introduce RL-Obfuscation, finetuning LLMs via reinforcement learning to bypass latent-space monitors while maintaining coherent outputs. Tested on models from 7B to 14B parameters.

Result: Token-level monitors are vulnerable; holistic monitors (max-pooling, attention-based) remain robust. Adversarial policies generalize to unseen monitors of the same type.

Conclusion: LLMs can evade certain latent-space monitors, highlighting the need for more robust monitoring methods.
...
TL;DR: The paper explores how machines can adapt quickly like humans by correcting approximate posteriors, leading to faster adaptation.


<details>
  <summary>Details</summary>
Motivation: Despite advancements in AI, machines lack human-like adaptivity. The paper aims to understand mechanisms for natural, quick adaptation.

Method: Uses a dual-perspective of the Bayesian Learning Rule to characterize interference during adaptation via natural-gradient mismatch.

Result: Accurate posteriors reduce corrections, enabling quicker adaptation, demonstrated through examples.

Conclusion: Posterior-correction is a natural mechanism for machines to learn rapid adaptation.
...
TL;DR: The paper addresses the lack of theoretical guarantees for Learning to Optimize (L2O) models in out-of-distribution (OOD) scenarios, providing proofs and a robust model.


<details>
  <summary>Details</summary>
Motivation: Existing L2O methods lack theoretical performance and robustness proofs in OOD scenarios, which this work aims to address.

Method: Proposes a robust L2O model with homogeneous convergence rates for In-Distribution (InD) scenarios and aligns OOD problems to InD. Introduces gradient-only feature construction and gradient-based history modeling.

Result: Numerical simulations show the model outperforms baselines, achieving up to 10x convergence speedup in both InD and OOD scenarios.

Conclusion: The work provides theoretical and empirical validation for robust L2O models, with significant performance improvements.
...
<details>
  <summary>Details</summary>
Motivation: Bayesian methods for LoRA finetuning improve calibration but often lack significant benefits in other metrics and add computational overhead. IVON addresses these limitations.

Method: Uses IVON, a variational algorithm, with posterior pruning for efficient and effective LoRA finetuning. Tested on billion-scale LLMs like Llama and Qwen.

Result: Improves accuracy by 1.3% and reduces ECE by 5.4% over AdamW, outperforming Bayesian methods like Laplace-LoRA and BLoB.

Conclusion: IVON is a practical and effective solution for enhancing LoRA finetuning, outperforming existing methods in both performance and efficiency.
...
TL;DR: The paper proposes a recipe for designing graph foundation models for node-level tasks, focusing on symmetries like label permutation-equivariance and feature permutation-invariance. It validates the approach with strong zero-shot performance on 29 datasets.


<details>
  <summary>Details</summary>
Motivation: Current graph machine learning architectures are task- and dataset-specific, limiting broader applicability. The goal is to develop graph foundation models that generalize across arbitrary graphs and features.

Method: The study systematically investigates symmetries (node, label, and feature permutations) and characterizes linear transformations respecting these. It proves the resulting network is a universal approximator for multisets with these symmetries.
...
Conclusion: The work provides a principled approach to designing graph foundation models, demonstrating their potential for generalization and scalability in node-level tasks.
...
TL;DR: The paper addresses fairness in doubly imbalanced datasets (imbalanced in both label and sensitive attribute) and proposes a multi-criteria solution for debiasing.
...
Result: Identifies challenges in debiasing doubly imbalanced data and suggests a balanced solution for fairness and accuracy.

Conclusion: The proposed multi-criteria method improves fairness and accuracy in doubly imbalanced datasets.
...
TL;DR: The paper proposes optimizations for offline RL to handle hybrid action spaces in MV control, avoiding discretization pitfalls, and introduces a clinically grounded reward function for better patient outcomes.
...
Conclusion: AI-assisted MV optimization shows promise for improving ICU care by addressing hybrid action space challenges and using clinically meaningful rewards.
...
Result: Variable-depth architectures consistently outperform fixed-depth networks, even when optimization is unlikely to play a role.

Conclusion: Residual connections provide inherent performance benefits, indicating a deeper inductive bias suited to natural data structures.
...
TL;DR: The paper extends Prototypical Variational models with autoencoder-based OOD detection, introducing a novel restriction loss for a compact ID region and demonstrating superior performance on benchmarks and real-world datasets.
...
Method: Uses a Variational Autoencoder to learn a latent space for distance-based classification, likelihood estimation, and reconstruction. Defines the ID region with a Gaussian mixture and introduces a restriction loss for compactness.
...
<details>
  <summary>Details</summary>
Motivation: Existing RL methods for TSC struggle with scalability and global coordination, limiting network-level efficiency.

Method: HiLight uses a high-level Meta-Policy (Transformer-LSTM) to partition networks and set sub-goals, and a low-level Sub-Policy for intersection control, enhanced by adversarial training for alignment.

Result: HiLight outperforms in large-scale scenarios and remains competitive in standard benchmarks, handling diverse traffic conditions.

Conclusion: HiLight addresses scalability and coordination challenges in TSC, proving effective for large networks and dynamic conditions.
...
TL;DR: The paper highlights the need for subgroup-level evaluation of ML models in clinical settings to address fairness and performance disparities across patient subgroups.


<details>
  <summary>Details</summary>
Motivation: Real-world medical datasets are noisy, incomplete, and imbalanced, leading to unfair performance disparities, especially for marginalized groups.

Method: Analyze medical prediction tasks to evaluate model performance variations across patient subgroups.

Result: Subgroup-level evaluation reveals performance disparities, emphasizing the need for fairness-aware model development.

Conclusion: Subgroup-sensitive evaluation is crucial for fair and transparent deployment of medical ML models.
...
TL;DR: A framework called the interaction layer is introduced to handle unobservable, time-varying delays in RL, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Real-world RL applications face delays in agent-environment interactions, which existing methods handle conservatively with fixed upper bounds.

Method: The interaction layer framework generates future action matrices to manage delays. A model-based algorithm, ACDA, dynamically adapts to delay patterns.

Result: ACDA outperforms state-of-the-art methods in locomotion benchmark environments.

Conclusion: The interaction layer and ACDA provide an effective solution for adaptive delay handling in RL.
...
TL;DR: A novel unsupervised RL method for skill discovery by maximizing state diversity between skills and using a conditional autoencoder for state-density estimation, outperforming prior methods in downstream tasks.
...
Method: Proposes a skill discovery objective maximizing state deviation between skills and a conditional autoencoder for state-density estimation, with intrinsic rewards for exploration.
...
Conclusion: The method effectively balances inter-skill diversity and intra-skill exploration, demonstrating strong applicability in unsupervised RL.
...
<details>
  <summary>Details</summary>
Motivation: To address task conflict and oblivion in multi-task adaptation of large-scale foundation models.
...
Result: MoORE outperforms existing methods in conflict- and oblivion-resistance across various datasets.

Conclusion: MoORE is a superior multi-task adaptation method, maintaining original task performance while avoiding conflicts.
...
<details>
  <summary>Details</summary>
Motivation: Hyperbolic neural networks are powerful for hierarchical data but face efficiency and precision challenges.

Method: Simplifying key operations in hyperbolic neural networks.

Result: Improved runtime and predictive accuracy.
...
TL;DR: HyPeR method leverages secondary rewards to improve off-policy learning in contextual bandits with partially observed target rewards.


<details>
  <summary>Details</summary>
Motivation: Partial observation of target rewards (e.g., delayed conversions, censored data) degrades OPL effectiveness. Secondary rewards (e.g., clicks, dwell time) are more abundant but may misalign with target rewards.

Method: Proposes HyPeR, a hybrid policy optimization method that uses both partially observed target rewards and secondary rewards to enhance learning.

Result: HyPeR outperforms existing methods in synthetic and real-world data, even when optimizing secondary rewards alongside target rewards.

Conclusion: HyPeR effectively leverages secondary rewards to improve OPL, showing advantages even when secondary rewards are optimized alongside target rewards.
...
<details>
  <summary>Details</summary>
Motivation: Label-free imaging avoids staining, making it ideal for in vivo use. However, computational specificity for MPM is rare, prompting the use of DL to enhance label-free MPM's utility.

Method: A CNN (squeezeNet) was trained on label-free MPM images of immune cells (5,075 for binary and 3,424 for multi-class tasks) using autofluorescence (AF) signals (NADH and FAD).

Result: The model achieved strong performance (e.g., 0.89 ROC-AUC for binary classification) and was robust to extracellular noise, with both AF channels contributing equally.

Conclusion: DL models can computationally enhance label-free MPM's specificity, enabling direct immune cell detection in unstained images, with significant potential for in vivo applications.
...
TL;DR: Students trained on soft labels from teachers can achieve non-trivial accuracy on memorized data they never observed, even in settings where generalization is impossible.


<details>
  <summary>Details</summary>
Motivation: To understand if and how memorized information in neural networks can be transferred in dataset distillation settings.
...
Result: Students can learn about held-out memorized data, sometimes achieving perfect accuracy, depending on temperature and other factors.

Conclusion: Memorized information can be transferred via soft labels in distillation, with performance influenced by temperature and model settings.
...
TL;DR: A stacking-based ensemble learning model is proposed to improve depression classification accuracy among professionals, achieving high performance metrics.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of accurately predicting depression in professionals due to its complex, multifaceted nature influenced by factors like occupational stress and lifestyle.

Method: Uses a stacking-based ensemble learning approach, integrating multiple base learners with logistic regression, applied to the Depression Professional Dataset from Kaggle.

Result: Achieves 99.64% accuracy on training data and 98.75% on testing data, with precision, recall, and F1-score all exceeding 98%.

Conclusion: The model demonstrates the effectiveness of ensemble learning in mental health analytics, highlighting its potential for early detection and intervention.
...
TL;DR: ZOO with finite differences is equivalent to single-step Policy Optimization (PO), revealing a unified framework. ZoAR, a novel ZOO algorithm, uses PO-inspired techniques for variance reduction and outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: To elucidate the connection between ZOO methods (using randomized finite differences) and Policy Optimization (PO), and to leverage this insight for practical algorithmic improvements.
...
Result: ZoAR outperforms other ZOO methods in convergence speed and final performance, supported by theoretical and empirical evidence.

Conclusion: The work provides a new theoretical understanding of ZOO through its connection to PO and introduces practical enhancements via ZoAR.
...
<details>
  <summary>Details</summary>
Motivation: Address the need for selective unlearning in machine learning systems due to privacy regulations, without full retraining.

Method: FAMR frames forgetting as constrained optimization with uniform-prediction loss and ℓ2 penalty to anchor parameters.

Result: Effective class forgetting on CIFAR-10 and ImageNet-100, with strong performance retention and minimal overhead.

Conclusion: FAMR offers a scalable, certifiable solution for post-hoc forgetting in vision models, generalizing to concept and style erasure.
...
TL;DR: The paper proposes two algorithms, ETC-TPZSG and ETC-TPZSG-AE, for learning pure strategy Nash Equilibrium in a two-player zero-sum game with bandit feedback. It provides instance-dependent regret bounds for both.


<details>
  <summary>Details</summary>
Motivation: To demonstrate the applicability of Explore-Then-Commit (ETC) in adversarial game settings and improve efficiency using action pair elimination.

Method: ETC-TPZSG applies ETC directly, while ETC-TPZSG-AE enhances it with action pair elimination leveraging ε-Nash Equilibrium.

Result: Instance-dependent regret bounds of O(Δ + √T) for ETC-TPZSG and O(log(TΔ²)/Δ) for ETC-TPZSG-AE.

Conclusion: ETC-based algorithms perform effectively in adversarial settings, offering insights through instance-dependent analysis.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate if current AI research methodologies are effective for creating successful, profitable AI applications, using a real-world industrial case.

Method: A case study on false call reduction in automated optical inspection, identifying and experimentally demonstrating seven weaknesses in current practices.

Result: The best-practice methodology fails for the use case, revealing the need for better metrics, success criteria, and dataset analysis.

Conclusion: Researchers should critically assess methodologies to improve applied AI research success.
...
TL;DR: LLMNet automates GNN configuration using Large Language Models and Retrieval-Augmented Generation, excelling in graph learning tasks.


<details>
  <summary>Details</summary>
Motivation: Simplify and automate the complex process of configuring and tuning Graph Neural Networks (GNNs).

Method: Uses agents to build graph-related knowledge bases and employs Retrieval-Augmented Generation (RAG) for automated GNN configuration and refinement.

Result: Outperforms in twelve datasets across three graph learning tasks.
...
TL;DR: A framework for evaluating clinical decision support systems prioritizes calibration, robustness, and cost sensitivity, using an adjusted cross-entropy method.


<details>
  <summary>Details</summary>
Motivation: Current scoring rules like accuracy and AUC-ROC inadequately address clinical priorities such as calibration, robustness, and asymmetric error costs.

Method: Proposes an adjusted cross-entropy (log score) method, based on proper scoring rules, to evaluate classifiers over clinically relevant class balances and cost asymmetries.
...
TL;DR: The paper introduces GPDMM, a model combining Gaussian process dynamical models (GPDMs) in a mixture framework for learning human motion from limited data, outperforming other methods like LSTMs and VAEs.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in modeling human motion with limited data, especially in medical applications like prosthesis control, where interpretability is crucial.

Method: GPDMM integrates multiple GPDMs (with HMM dynamics) in a probabilistic mixture, using geometric features to encode diverse sequences in a single latent space.

Result: GPDMM achieves high classification accuracy and generative ability in single-example learning, surpassing LSTMs, VAEs, and transformers.

Conclusion: GPDMM is effective for modeling human motion with limited data, offering interpretability and outperforming existing methods.
...
TL;DR: The paper introduces a method to integrate token-level reward guidance into Direct Preference Optimization (DPO), improving its performance by decomposing sequence-level PPO into token-level problems and deriving optimal policies and rewards.


<details>
  <summary>Details</summary>
Motivation: Current DPO methods struggle with leveraging token-level rewards, limiting their alignment performance with large language models.
...
Conclusion: The proposed framework successfully bridges the gap between token-level rewards and DPO, enhancing alignment performance.
...
Method: Integrates neural (object-centric learning for image segmentation) and symbolic (ABA learning for predictions) components.
...
<details>
  <summary>Details</summary>
Motivation: Shortcut learning undermines generalization, and existing methods overlook semantic biases in sample embeddings.

Method: SCISSOR uses a Siamese network to intervene in latent semantic clusters, discouraging shortcuts without data rewriting.

Result: SCISSOR improves F1 scores by 5.3-7.7 points across 4 benchmarks and boosts lightweight models by ~9.5-11.9%.
...
TL;DR: A deep-learning surrogate model accelerates real-time greenhouse-gas emission tracking by replacing costly CFD solvers, achieving accuracy and speed.
...
Method: A spatio-temporal inversion framework combining a deep-learning surrogate (multilayer perceptron) with sequential Monte Carlo for Bayesian inference of emission rate and source location.

Result: Validation shows comparable accuracy to CFD solvers and Gaussian plume models, with significantly faster runtimes. Robustness confirmed in complex scenarios.

Conclusion: The framework balances physical fidelity and computational feasibility, providing a scalable solution for industrial emissions monitoring and similar tasks.
...
TL;DR: The paper introduces a novel likelihood-based distribution matching method using score-based priors, addressing scalability, instability, and bias issues in existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing DM methods face challenges like scalability (non-parametric), instability (adversarial), or bias (likelihood-based). The goal is to overcome these limitations.

Method: Proposes training likelihood-based DM with expressive score-based priors, leveraging denoising score matching to avoid fixed priors and explicit density models.

Result: The method shows better stability, computational efficiency, and superior performance across tasks compared to alternatives like LSGM.

Conclusion: The score-based approach is a stable and effective solution for distribution matching, with practical advantages over existing methods.
...
TL;DR: FuRBO is a feasibility-driven Bayesian optimization algorithm that accelerates the discovery of feasible solutions in high-dimensional, constrained optimization problems.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with costly constraints and high-dimensional spaces, often spending too much budget on finding initial feasible solutions.

Method: FuRBO uses an adaptive trust region strategy, leveraging surrogate models for both objectives and constraints to iteratively refine search areas.

Result: FuRBO outperforms state-of-the-art baselines in constrained optimization, demonstrated on the BBOB-constrained COCO benchmark and physics-inspired problems.

Conclusion: FuRBO effectively addresses the challenge of locating feasible solutions quickly, making it a robust tool for constrained black-box optimization.
...
TL;DR: The paper critiques current visual counterfactual explainers (VCEs) for focusing too narrowly on sample quality or minimal changes, neglecting broader explanation goals like fidelity, understandability, and sufficiency. It introduces a new 'smooth counterfactual explorer' (SCE) algorithm to address these gaps.


<details>
  <summary>Details</summary>
Motivation: Existing VCEs lack holistic consideration of explanation desiderata (fidelity, understandability, sufficiency), limiting their effectiveness.

Method: The authors propose a novel 'smooth counterfactual explorer' (SCE) algorithm, combining new counterfactual generation mechanisms.

Result: SCE is evaluated systematically on synthetic and real data, demonstrating its effectiveness.

Conclusion: The SCE algorithm addresses limitations of current VCEs, offering a more comprehensive approach to counterfactual explanations.
...
TL;DR: The paper explores bandit learnability, showing that no combinatorial dimension can characterize it and revealing computational hardness in finding optimal actions efficiently.
...
Result: No combinatorial dimension characterizes bandit learnability; computational hardness exists even for simple classes, unless RP=NP.
...
### [264] [A Systematic Review of User-Centred Evaluation of Explainable AI in Healthcare](https://arxiv.org/abs/2506.13904)
*Ivania Donoso-Guzmán,Kristýna Sirka Kacafírková,Maxwell Szymanski,An Jacobs,Denis Parra,Katrien Verbert*

Main category: cs.HC

TL;DR: The paper addresses the lack of practical validation of XAI methods in real-world healthcare settings by developing a framework and guidelines for evaluating user experience and system characteristics.


<details>
  <summary>Details</summary>
Motivation: The practical value of XAI methods is under-explored, and robust evaluation is needed to ensure trustworthiness and usability, but clear guidelines are lacking.

Method: A systematic review of 82 user studies in healthcare, analyzed using a predefined coding scheme and inductive codes.

Result: The study synthesizes current evaluation practices, explores interrelations among explanation properties, and provides an updated framework and guidelines for effective XAI evaluation.

Conclusion: The paper offers actionable insights and tools for interdisciplinary teams to design context-sensitive evaluation strategies for XAI in healthcare.
...
### [265] [StorySage: Conversational Autobiography Writing Powered by a Multi-Agent Framework](https://arxiv.org/abs/2506.14159)
*Shayan Talaei,Meijin Li,Kanu Grover,James Kent Hippler,Diyi Yang,Amin Saberi*

Main category: cs.HC

TL;DR: StorySage is a multi-agent system for autobiography writing that improves narrative completeness and user satisfaction compared to baseline methods.


<details>
  <summary>Details</summary>
Motivation: Existing writing assistants lack personalization and struggle to organize scattered memories into coherent autobiographies.

Method: StorySage uses a multi-agent framework (Interviewer, Session Scribe, Planner, Section Writer, Session Coordinator) to iteratively collect memories and structure narratives.

Result: Experimental simulations and user studies (N=28) show improved conversational flow, narrative completeness, and user satisfaction.

Conclusion: StorySage offers a novel architecture for autobiography writing and insights into enhancing human-AI creative partnerships.
...
### [266] [Balancing Caregiving and Self-Care: Exploring Mental Health Needs of Alzheimer's and Dementia Caregivers](https://arxiv.org/abs/2506.14196)
*Jiayue Melissa Shi,Keran Wang,Dong Whi Yoo,Ravi Karkar,Koustuv Saha*

Main category: cs.HC

TL;DR: The study explores mental wellbeing challenges of family caregivers for AD/ADRD patients, identifying causes, effects, and stage-specific needs, while suggesting improvements for support technologies.


<details>
  <summary>Details</summary>
Motivation: Current support systems fail to address the evolving mental health needs of AD/ADRD caregivers, prompting the need for tailored, scalable solutions.

Method: Semi-structured interviews with 25 family caregivers to analyze mental health challenges and technology use.

Result: Identified key causes and effects of mental health issues, mapped wellbeing evolution across three caregiving stages, and gathered insights for improving support technologies.

Conclusion: Findings highlight the need for dynamic, stage-sensitive interventions to better support caregivers' mental wellbeing over time.
...
### [267] [Controlling Context: Generative AI at Work in Integrated Circuit Design and Other High-Precision Domains](https://arxiv.org/abs/2506.14567)
*Emanuel Moss,Elizabeth Watkins,Christopher Persaud,Passant Karunaratne,Dawn Nafus*

Main category: cs.HC

TL;DR: The paper explores challenges engineers face with generative AI tools in high-precision domains, focusing on accuracy and contextual control.


<details>
  <summary>Details</summary>
Motivation: To understand how engineers in high-precision domains (like integrated circuit design) maintain vigilance for errors and what troubles them when using generative AI tools.

Method: Interviews with hardware/software engineers and collaborators in integrated circuit design, analyzing their experiences with generative AI tools.

Result: Identified forms of trouble in using generative AI tools, mapped to system elements, highlighting contextual control as a major challenge.

Conclusion: Recommends improving interactive contextual control to mitigate challenges in using generative AI tools.
...
### [268] [StreetLens: Enabling Human-Centered AI Agents for Neighborhood Assessment from Street View Imagery](https://arxiv.org/abs/2506.14670)
*Jina Kim,Leeje Jang,Yao-Yi Chiang,Guanyu Wang,Michelle Pasco*

Main category: cs.HC

TL;DR: StreetLens is a configurable workflow using vision-language models (VLMs) to automate neighborhood environmental assessments, combining domain expertise with AI for scalable analysis.


<details>
  <summary>Details</summary>
Motivation: Traditional methods for neighborhood studies are time-consuming and require expert intervention. VLMs offer automation but lack adaptability and domain-specific grounding.

Method: StreetLens integrates social science expertise into a VLM, using domain-informed prompting and prior survey data to analyze street view imagery (SVI) for objective and subjective features.

Result: The tool generates semantic annotations for neighborhood characteristics, from physical features to perceptions, and is adaptable across research designs and contexts.

Conclusion: StreetLens advances flexible, researcher-collaborative AI systems for scalable and efficient neighborhood studies.
...
### [269] [Design an Editable Speech-to-Sign-Language Transformer System: A Human-Centered AI Approach](https://arxiv.org/abs/2506.14677)
*Yingchao Li*

Main category: cs.HC

TL;DR: A real-time, user-adaptive speech-to-sign language system using Transformer-based motion generation and editable JSON for enhanced naturalness and user control.


<details>
  <summary>Details</summary>
Motivation: To address limitations in prior sign language technologies by enabling user inspection and modification, improving naturalness and expressiveness.

Method: Integrates a streaming Conformer encoder and autoregressive Transformer-MDN decoder for motion generation, with a user-editable JSON layer and human-in-the-loop optimization.

Result: Experiments with deaf signers and interpreters show improved comprehension, naturalness, usability, and trust, with sub-20 ms inference time.

Conclusion: Combining technical and participatory innovation creates accessible, explainable, and user-adaptive AI for sign language technology.
...
### [270] [NeuroCoreX: An Open-Source FPGA-Based Spiking Neural Network Emulator with On-Chip Learning](https://arxiv.org/abs/2506.14138)
*Ashish Gautam,Prasanna Date,Shruti Kulkarni,Robert Patton,Thomas Potok*

Main category: cs.NE

TL;DR: NeuroCoreX is an FPGA-based SNN emulator supporting flexible network topologies and STDP-based learning, released as open-source to advance energy-efficient computing.


<details>
  <summary>Details</summary>
Motivation: To enable flexible co-design and testing of SNNs with diverse connectivity patterns and biologically inspired learning mechanisms.

Method: Developed NeuroCoreX, an FPGA-based emulator with all-to-all connectivity, LIF neuron model, STDP learning, and a Python interface for configuration.

Result: NeuroCoreX provides a versatile platform for SNN deployment, supporting various topologies and efficient hardware execution.

Conclusion: NeuroCoreX accelerates research in energy-efficient, biologically inspired computing by offering an open-source, flexible SNN emulation framework.
...
### [271] [A Scalable Hybrid Training Approach for Recurrent Spiking Neural Networks](https://arxiv.org/abs/2506.14464)
*Maximilian Baronig,Yeganeh Bahariasl,Ozan Özdenizci,Robert Legenstein*

Main category: cs.NE

TL;DR: HYPR combines parallelization with forward gradient learning for efficient online training of RSNNs, reducing memory demands and improving performance.


<details>
  <summary>Details</summary>
Motivation: BPTT's limitations (no online training, high memory use) and slow execution of forward gradient methods hinder RSNN training. HYPR addresses these issues.

Method: HYPR hybridizes parallelization and forward gradient learning, enabling high-throughput online training with constant memory usage.

Result: HYPR achieves low performance gap between forward gradient learning and BPTT, especially with oscillatory subthreshold neuron models.

Conclusion: HYPR offers a scalable, efficient solution for online RSNN training, bridging the gap between forward learning and BPTT.
...
### [272] [Is Selection All You Need in Differential Evolution?](https://arxiv.org/abs/2506.14425)
*Tomofumi Kitamura,Alex Fukunaga*

Main category: cs.NE

TL;DR: The paper introduces Unbounded Differential Evolution (UDE), a novel DE framework that eliminates generational replacement and archive management, focusing on selection mechanisms for improved simplicity and performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of traditional DE, such as fixed population size and archive management complexities, which hinder diversity and performance.

Method: UDE adds all generated candidates to the population without discarding any individuals, removing the need for generational replacement and archive management.

Result: UDE simplifies the DE framework while maintaining or improving performance by leveraging selection mechanisms.

Conclusion: UDE offers a straightforward yet powerful alternative to traditional DE, eliminating unnecessary complexities and enhancing search efficiency.
...
### [273] [Multimodal Fusion with Semi-Supervised Learning Minimizes Annotation Quantity for Modeling Videoconference Conversation Experience](https://arxiv.org/abs/2506.13971)
*Andrew Chang,Chenkai Hu,Ji Qi,Zhuojian Wei,Kexin Zhang,Viswadruth Akkaraju,David Poeppel,Dustin Freeman*

Main category: eess.AS

TL;DR: SSL outperforms SL in predicting negative videoconference moments with less labeled data.


<details>
  <summary>Details</summary>
Motivation: Understudied negative moments in group videoconferencing require costly manual annotation for SL.

Method: Used SSL with multimodal (audio, facial, text) deep features and co-training to predict non-fluid/unenjoyable moments.

Result: SSL achieved ROC-AUC 0.9, F1 0.6, outperforming SL by 4% with same labeled data. SSL with 8% labeled data matched 96% of SL's full-data performance.

Conclusion: SSL provides an annotation-efficient framework for modeling videoconference experiences.
...
### [274] [Improving Practical Aspects of End-to-End Multi-Talker Speech Recognition for Online and Offline Scenarios](https://arxiv.org/abs/2506.14204)
*Aswin Shanmugam Subramanian,Amit Das,Naoyuki Kanda,Jinyu Li,Xiaofei Wang,Yifan Gong*

Main category: eess.AS

TL;DR: Extends Serialized Output Training (SOT) for ASR, balancing latency and accuracy with CSS front-end, dual models, and segment-based SOT.


<details>
  <summary>Details</summary>
Motivation: Address practical needs of streaming and offline ASR applications, focusing on real-time captioning and summarization.

Method: Proposes CSS front-end for overlapping speech, dual models (Conformer Transducer and Sequence-to-Sequence), and segment-based SOT.

Result: Improves ASR accuracy in overlapping scenarios and enhances readability of multi-talker transcriptions.

Conclusion: The approach effectively balances latency and accuracy, catering to diverse ASR needs.
...
### [275] [Projecting U.S. coastal storm surge risks and impacts with deep learning](https://arxiv.org/abs/2506.13963)
*Julian R. Rice,Karthik Balaguru,Fadia Ticona Rollano,John Wilson,Brent Daniel,David Judi,Ning Sun,L. Ruby Leung*

Main category: physics.ao-ph

TL;DR: A deep learning model estimates US coastal surge risk from 900,000 synthetic TC events, predicting a 50% rise in at-risk population by 2100 due to higher TC intensities and sea levels.


<details>
  <summary>Details</summary>
Motivation: Assessing storm surge risk is challenging due to rarity and complexity; AI offers a solution.

Method: Deep learning model applied to synthetic TC events, accounting for future TC behavior and sea level changes.

Result: Historical 100-year surge aligns with observations; future projections show 50% more at-risk population, especially in Florida, Georgia, and South Carolina.

Conclusion: AI-driven modeling effectively predicts heightened storm surge risks, aiding future preparedness.
...
### [276] [AI-Informed Model Analogs for Subseasonal-to-Seasonal Prediction](https://arxiv.org/abs/2506.14022)
*Jacob B. Landsberg,Elizabeth A. Barnes,Matthew Newman*

Main category: physics.ao-ph

TL;DR: The paper introduces an interpretable AI-informed model for subseasonal-to-seasonal (S2S) forecasting, outperforming traditional methods in tasks like temperature and wind prediction.


<details>
  <summary>Details</summary>
Motivation: S2S forecasting is vital for public health, disaster preparedness, and agriculture but remains challenging. The study aims to improve predictions using AI.

Method: An artificial neural network learns a mask of weights to optimize analog selection, tested on three prediction tasks involving temperature and wind classification/regression.

Result: The AI-informed approach outperforms traditional analog forecasting, climatology, and persistence baselines, improving predictions of extremes and forecast uncertainty.

Conclusion: The interpretable AI framework enhances S2S forecasting and provides insights into predictability sources.
...
### [277] [GAF: Gaussian Action Field as a Dvnamic World Model for Robotic Mlanipulation](https://arxiv.org/abs/2506.14135)
*Ying Chai,Litao Deng,Ruizhi Shao,Jiajun Zhang,Liangjun Xing,Hongwen Zhang,Yebin Liu*

Main category: cs.RO

TL;DR: The paper introduces a V-4D-A framework using Gaussian Action Field (GAF) for accurate robotic action inference from 4D representations, outperforming existing methods in reconstruction and task success.


<details>
  <summary>Details</summary>
Motivation: Existing Vision-to-Action (V-A) and Vision-to-3D-to-Action (V-3D-A) methods struggle with action inaccuracies in dynamic manipulation scenes.

Method: Proposes GAF, extending 3D Gaussian Splatting with motion attributes, enabling scene reconstruction, future frame prediction, and action estimation. A GAF-guided diffusion model refines actions.

Result: GAF improves reconstruction (PSNR +11.5385 dB, LPIPS -0.5574) and boosts robotic task success by 10.33% over state-of-the-art methods.

Conclusion: The V-4D-A framework with GAF enhances action inference in robotic manipulation, addressing limitations of prior approaches.
...
### [278] [AMPLIFY: Actionless Motion Priors for Robot Learning from Videos](https://arxiv.org/abs/2506.14198)
*Jeremy A. Collins,Loránd Cheng,Kunal Aneja,Albert Wilcox,Benjamin Joffe,Animesh Garg*

Main category: cs.RO

TL;DR: AMPLIFY is a framework that uses action-free video data to improve robotic policies by encoding visual dynamics into motion tokens, achieving better accuracy and generalization.


<details>
  <summary>Details</summary>
Motivation: Action-labeled data for robotics is scarce and expensive, while action-free video data is abundant but underutilized. AMPLIFY aims to bridge this gap.

Method: AMPLIFY encodes visual dynamics into discrete motion tokens from keypoint trajectories, separating motion prediction from action inference. It trains a forward dynamics model on action-free videos and an inverse dynamics model on labeled data.

Result: The framework achieves 3.7x better MSE and 2.5x better pixel prediction accuracy, with 1.2-2.2x improvement in low-data policy learning and generalization to LIBERO tasks.

Conclusion: AMPLIFY demonstrates a scalable and versatile approach to leveraging heterogeneous data for efficient, generalizable world models in robotics.
...
### [279] [GAMORA: A Gesture Articulated Meta Operative Robotic Arm for Hazardous Material Handling in Containment-Level Environments](https://arxiv.org/abs/2506.14513)
*Farha Abdul Wasay,Mohammed Abdul Rahman,Hania Ghouse*

Main category: cs.RO

TL;DR: GAMORA is a VR-guided robotic system for high-risk lab tasks, using natural hand gestures for precise, remote control with improved accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: To minimize human exposure in hazardous virology labs while maintaining precision in delicate operations like specimen handling and pipetting.

Method: Integrates Oculus Quest 2, NVIDIA Jetson Nano, and ROS for real-time immersive control, digital twin simulation, and inverse kinematics. Uses Unity for 3D environment, motion planning, and YOLOv8 for object detection.

Result: Achieved 2.2 mm positional discrepancy, 0.2 mL pipetting accuracy, 1.2 mm repeatability, and 50% reduced power output.

Conclusion: GAMORA provides a scalable, immersive, and safe solution for robotic control in biomedical research.
...
### [280] [Hard Contacts with Soft Gradients: Refining Differentiable Simulators for Learning and Control](https://arxiv.org/abs/2506.14186)
*Anselm Paulus,A. René Geist,Pierre Schumacher,Vít Musil,Georg Martius*

Main category: cs.RO

TL;DR: DiffMJX improves gradient quality for hard contacts in robot dynamics by combining adaptive integration with MuJoCo XLA, while CFD generates informative gradients even before contact.


<details>
  <summary>Details</summary>
Motivation: Gradient-based optimization of robot dynamics is hindered by contact forces, which introduce velocity jumps. Penalty-based simulators like MuJoCo soften contacts, but stiff settings for realism cause incorrect gradients, widening the sim-to-real gap.

Method: Analyze penalty-based simulator contact computation, propose DiffMJX (adaptive integration + MuJoCo XLA) for better gradients, and introduce CFD for pre-contact gradient generation using a straight-through trick.

Result: DiffMJX notably improves gradient quality for hard contacts, and CFD enables informative gradients before objects touch without altering forward simulation realism.

Conclusion: DiffMJX and CFD address gradient issues in robot dynamics, enhancing optimization for hard contacts and pre-contact scenarios while maintaining physical realism.
...
### [281] [Steering Robots with Inference-Time Interactions](https://arxiv.org/abs/2506.14287)
*Yanwei Wang*

Main category: cs.RO

TL;DR: A method for correcting pretrained policies during deployment using user interactions, avoiding finetuning.


<details>
  <summary>Details</summary>
Motivation: Addresses the inefficiency of collecting additional data for finetuning pretrained policies when errors occur during deployment.

Method: Proposes inference-time steering (switching between discrete skills) and task/motion imitation (editing continuous motions with symbolic plans).

Result: Enables correction of policy errors without additional training, aligning with user preferences.

Conclusion: Enhances utility of pretrained models by making them steerable at inference time.
...
### [282] [Uncertainty-Driven Radar-Inertial Fusion for Instantaneous 3D Ego-Velocity Estimation](https://arxiv.org/abs/2506.14294)
*Prashant Kumar Rai,Elham Kowsari,Nataliya Strokina,Reza Ghabcheloo*

Main category: cs.RO

TL;DR: A neural network integrates radar and IMU data for ego-velocity estimation, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Addresses limitations of traditional radar-based ego-motion estimation by combining radar and IMU data with uncertainty-aware neural processing.

Method: Uses a neural network to process raw radar data, estimates velocity with uncertainty, and integrates with IMU via an Extended Kalman Filter.

Result: Achieves lower error than existing methods on the ColoRadar dataset, outperforming instantaneous and scan matching-based techniques.

Conclusion: The proposed method enhances robustness and accuracy in ego-motion estimation by leveraging uncertainty-aware neural processing and sensor fusion.
...
### [283] [SENIOR: Efficient Query Selection and Preference-Guided Exploration in Preference-based Reinforcement Learning](https://arxiv.org/abs/2506.14648)
*Hexian Ni,Tao Lu,Haoyuan Hu,Yinghao Cai,Shuo Wang*

Main category: cs.RO

TL;DR: SENIOR introduces a novel method for efficient query selection and preference-guided exploration in PbRL, improving feedback- and sample-efficiency through motion-distinction-based selection and preference-guided exploration.


<details>
  <summary>Details</summary>
Motivation: Poor feedback- and sample-efficiency in PbRL hinder its application, necessitating better methods for human preference labeling and policy learning.

Method: SENIOR uses Motion-Distinction-based Selection (MDS) for meaningful segment pairs and Preference-Guided Exploration (PGE) for intrinsic rewards, enhancing reward and policy learning.

Result: SENIOR outperforms five existing methods in feedback-efficiency and policy convergence on six simulation and four real-world robot tasks.

Conclusion: SENIOR's synergy of MDS and PGE significantly improves PbRL efficiency, making it practical for complex tasks.
...
### [284] [Casper: Inferring Diverse Intents for Assistive Teleoperation with Vision Language Models](https://arxiv.org/abs/2506.14727)
*Huihan Liu,Rutav Shah,Shuijing Liu,Jack Pittenger,Mingyo Seo,Yuchen Cui,Yonatan Bisk,Roberto Martín-Martín,Yuke Zhu*

Main category: cs.RO

TL;DR: Casper is an assistive teleoperation system using pre-trained VLMs for real-time intent inference and flexible skill execution, outperforming existing methods in task performance and user satisfaction.


<details>
  <summary>Details</summary>
Motivation: Existing assistive teleoperation methods are limited to simple or task-specific scenarios, hindering real-world application. Casper aims to generalize intent inference and assistance using commonsense knowledge.

Method: Casper integrates an open-world perception module, VLM-powered intent inference, and a skill library to handle diverse mobile manipulation tasks.

Result: Casper improves task performance, reduces cognitive load, and increases user satisfaction compared to direct teleoperation and other baselines.

Conclusion: Casper demonstrates the potential of leveraging VLMs for flexible and effective assistive teleoperation in real-world settings.
...
### [285] [A Survey on World Models Grounded in Acoustic Physical Information](https://arxiv.org/abs/2506.13833)
*Xiaoliang Chen,Le Chang,Xin Yu,Yunhe Huang,Xianling Tu*

Main category: cs.SD

TL;DR: A survey on acoustic world models, covering theory, methods, applications, and future directions for AI systems using sound for environmental perception and reasoning.


<details>
  <summary>Details</summary>
Motivation: To explore how acoustic signals encode physical information and enable AI systems to perceive and reason about dynamic environments.
...
Result: Highlights applications in robotics, autonomous driving, healthcare, and finance, with a focus on robust and ethical AI.

Conclusion: Proposes a roadmap for future research toward embodied, causal, and responsible acoustic intelligence.
...
### [286] [Acoustic scattering AI for non-invasive object classifications: A case study on hair assessment](https://arxiv.org/abs/2506.14148)
*Long-Vu Hoang,Tuan Nguyen,Tran Huy Dat*

Main category: cs.SD

TL;DR: A novel non-invasive method for object classification using acoustic scattering, applied to hair assessment, achieves ~90% accuracy with AI-driven deep learning.


<details>
  <summary>Details</summary>
Motivation: To explore a privacy-preserving, non-contact alternative to visual classification by leveraging acoustic scattering for object properties.

Method: Emits acoustic stimuli, captures scattered signals, and uses AI-driven deep learning (supervised, embedding-based, foundation model fine-tuning, self-supervised fine-tuning) for classification.

Result: Best method (self-supervised model fine-tuning) achieves nearly 90% accuracy in hair type and moisture classification.

Conclusion: Acoustic scattering is a promising, non-invasive alternative for object classification with broad industry potential.
...
### [287] [Pushing the Performance of Synthetic Speech Detection with Kolmogorov-Arnold Networks and Self-Supervised Learning Models](https://arxiv.org/abs/2506.14153)
*Tuan Dat Phuong,Long-Vu Hoang,Huy Dat Tran*

Main category: cs.SD

TL;DR: Replacing MLP with KAN in XLSR-Conformer improves synthetic speech detection by 60.55% on ASVspoof2021.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations in SSL-based models for detecting advanced spoofing attacks in speech synthesis.

Method: Proposes replacing Multi-Layer Perceptron in XLSR-Conformer with Kolmogorov-Arnold Network (KAN).

Result: 60.55% relative improvement on LA and DF sets; 0.70% EER on 21LA set.

Conclusion: KAN integration in SSL models is promising for enhancing synthetic speech detection.
...
### [288] [Fretting-Transformer: Encoder-Decoder Model for MIDI to Tablature Transcription](https://arxiv.org/abs/2506.14223)
*Anna Hamberger,Sebastian Murgul,Jochen Schmidt,Michael Heizmann*

Main category: cs.SD

TL;DR: The paper introduces Fretting-Transformer, a T5-based model for converting MIDI to guitar tablature, outperforming baselines like A* and Guitar Pro.


<details>
  <summary>Details</summary>
Motivation: MIDI lacks playability info for guitars; automating tablature transcription addresses string-fret ambiguity and playability.

Method: Uses T5 transformer architecture, novel data pre-processing, and tokenization on datasets like DadaGP and GuitarToday.

Result: Surpasses A* and Guitar Pro, with enhanced performance via context-sensitive processing and tuning/capo conditioning.

Conclusion: Fretting-Transformer provides a strong foundation for future automated guitar transcription advancements.
...
### [289] [Making deep neural networks work for medical audio: representation, compression and domain adaptation](https://arxiv.org/abs/2506.13970)
*Charles C Onu*

Main category: cs.SD

TL;DR: The thesis explores machine learning for medical audio signals, focusing on infant cry analysis, using transfer learning, model compression, domain adaptation, and releasing an open-source dataset.
...
Method: Uses neural transfer learning, tensor decomposition for model compression, domain adaptation techniques, and releases an infant cry dataset.

Result: Developed accurate, portable models and improved generalization across domains, contributing to AI-driven healthcare.

Conclusion: Infant cry analysis can be a vital sign, showcasing AI's potential for accessible, affordable healthcare.
...
### [290] [SLEEPING-DISCO 9M: A large-scale pre-training dataset for generative music modeling](https://arxiv.org/abs/2506.14293)
...
Method: The dataset is constructed using real-world popular music and songs from well-known artists, unlike previous synthetic or re-recorded datasets.

Result: Sleeping-DISCO 9M offers a high-quality, relevant dataset for tasks like text-music generation, music-captioning, and singing-voice synthesis.

Conclusion: This dataset addresses the limitations of prior datasets and is expected to enhance generative music modeling by reflecting real-world music.
...
### [291] [Unifying Streaming and Non-streaming Zipformer-based ASR](https://arxiv.org/abs/2506.14434)
...
<details>
  <summary>Details</summary>
Motivation: To reduce costs and complexity by unifying streaming and non-streaming ASR models while leveraging future context information.

Method: Uses dynamic right-context via chunked attention masking in zipformer-based ASR models, analyzing the impact of right-context frames on accuracy and latency.
...
Conclusion: The approach effectively balances latency and accuracy, offering flexible control for diverse customer needs.
...
### [292] [Refining music sample identification with a self-supervised graph neural network](https://arxiv.org/abs/2506.14684)
*Aditya Bhattacharjee,Ivan Meresman Higgs,Mark Sandler,Emmanouil Benetos*

Main category: cs.SD

TL;DR: The paper proposes a lightweight Graph Neural Network-based model for Automatic Sample Identification (ASID), achieving comparable performance to state-of-the-art with fewer parameters, and introduces a two-stage retrieval approach for better accuracy.


<details>
  <summary>Details</summary>
Motivation: ASID struggles with identifying samples modified by common music production transformations, unlike audio fingerprinting which handles noisy conditions well. A robust system for such transformations is needed.

Method: A contrastive learning framework with a Graph Neural Network is used, alongside a two-stage retrieval approach (coarse search + cross-attention classifier) to improve accuracy.

Result: The model achieves 44.2% mAP with only 9% of the trainable parameters of the current state-of-the-art.

Conclusion: The proposed system is lightweight, scalable, and effective for ASID, especially for short queries, as demonstrated by the new Sample100 dataset annotations.
...
### [293] [Adaptive Accompaniment with ReaLchords](https://arxiv.org/abs/2506.14723)
...
TL;DR: ReaLchords is an online generative model for improvising chord accompaniment to user melody, using reinforcement learning and a novel reward model for harmonic and temporal coherency.


<details>
  <summary>Details</summary>
Motivation: Current generative models lack the ability to generate music online (simultaneously with musicians). ReaLchords aims to enable live jamming and co-creation.

Method: The model is pretrained by maximum likelihood, then finetuned with reinforcement learning using a reward model for coherency and a divergence term for distillation from a teacher model.
...
Conclusion: ReaLchords enables live jamming and co-creation, opening possibilities for real-time collaborative music generation.
...
### [294] [Exploring Speaker Diarization with Mixture of Experts](https://arxiv.org/abs/2506.14750)
*Gaobin Yang,Maokui He,Shutong Niu,Ruoyu Wang,Hang Chen,Jun Du*

Main category: cs.SD

TL;DR: A novel neural speaker diarization system (NSD-MS2S) combines memory-aware multi-speaker embedding with Seq2Seq architecture, enhanced by a Shared and Soft Mixture of Experts (SS-MoE) module, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: To improve speaker diarization by integrating memory-aware embeddings and Seq2Seq architecture, while addressing model bias and enhancing robustness.

Method: Proposes NSD-MS2S with a memory module for better speaker embeddings and Seq2Seq for mapping features to labels. Introduces SS-MoE to reduce bias and improve performance.

Result: Achieves state-of-the-art results on complex datasets like CHiME-6, DiPCo, Mixer 6, and DIHARD-III, showing robustness and generalization.

Conclusion: The NSD-MS2S-SSMoE system is effective for challenging real-world scenarios, demonstrating superior performance in speaker diarization.
...
### [295] [DeepSeq: High-Throughput Single-Cell RNA Sequencing Data Labeling via Web Search-Augmented Agentic Generative AI Foundation Models](https://arxiv.org/abs/2506.13817)
*Saleem A. Al Dajani,Abel Sanchez,John R. Williams*

Main category: q-bio.GN

TL;DR: Generative AI models automate labeling of single-cell RNA data with 82.5% accuracy, reducing manual effort and enabling virtual cell models for tasks like cell-typing.


<details>
  <summary>Details</summary>
Motivation: Address the bottleneck of manual data annotation in supervised learning for large-scale biological datasets.

Method: Use agentic foundation models with real-time web search to automate labeling.

Result: Achieved 82.5% accuracy in labeling, enabling virtual cell models for downstream tasks.

Conclusion: This approach can surpass human performance in labeling, supporting large-scale biological research and diagnostics.
...
### [296] [Sketched Sum-Product Networks for Joins](https://arxiv.org/abs/2506.14034)
*Brian Tsan,Abylay Amanbayev,Asoke Datta,Florin Rusu*

Main category: cs.DB

TL;DR: The paper proposes using Sum-Product Networks (SPNs) to dynamically approximate sketches for join cardinality estimation, enabling broader applicability to new queries without predefined selections.


<details>
  <summary>Details</summary>
Motivation: Current sketches for join cardinality estimation are limited to predefined queries, restricting their use in dynamic scenarios. SPNs offer a flexible solution.

Method: SPNs decompose multivariate distributions into univariate ones, represented as sketches, and combine them to approximate sketches for any query selection. Fast-AGMS and Bound Sketch methods are implemented.

Result: The approach efficiently approximates sketches, providing a practical alternative for query optimization without costly sketch construction.

Conclusion: Dynamic sketch approximation via SPNs enhances the applicability of sketches in query optimization, overcoming limitations of predefined selections.
...
### [297] [Evolutionary chemical learning in dimerization networks](https://arxiv.org/abs/2506.14006)
*Alexei V. Tkachenko,Bortolo Matteo Mognetti,Sergei Maslov*

Main category: cond-mat.stat-mech

TL;DR: A novel framework for chemical learning using Competitive Dimerization Networks (CDNs) enables multiclass classification without digital hardware, with molecular species acting as neurons and binding affinities as synaptic weights.


<details>
  <summary>Details</summary>
Motivation: To bridge synthetic biology and machine learning by developing adaptive, energy-efficient molecular computing systems.

Method: Training CDNs in vitro through directed evolution (mutation, selection, amplification) to perform tasks like multiclass classification.

Result: CDNs robustly discriminate noisy inputs, exhibiting strong output contrast and high mutual information, closely matching in silico gradient descent performance.

Conclusion: CDNs are a promising platform for analog physical computation, advancing molecular computing.
...
### [298] [NeuralPDR: Neural Differential Equations as surrogate models for Photodissociation Regions](https://arxiv.org/abs/2506.14270)
*Gijs Vermariën,Thomas G. Bisbas,Serena Viti,Yue Zhao,Xuefei Tang,Rahul Ravichandran*

Main category: astro-ph.GA

TL;DR: Surrogate models (Latent Augmented Neural ODEs) replace expensive chemical solvers in astrochemical simulations, enabling faster GPU-based inference and higher-resolution modeling.


<details>
  <summary>Details</summary>
Motivation: High-resolution telescopes like JWST and ALMA require detailed astrochemical modeling at smaller scales, but coupling 3D hydrodynamics and chemistry is computationally costly.

Method: Latent Augmented Neural ODEs are trained on datasets of increasing complexity, including a 3D molecular cloud simulation (3D-PDR).

Result: Surrogate models provide speedup and accurately reproduce observable column density maps.

Conclusion: These models enable rapid chemistry inference, improving statistical analysis and simulation resolution.
...
### [299] [Balancing Preservation and Modification: A Region and Semantic Aware Metric for Instruction-Based Image Editing](https://arxiv.org/abs/2506.13827)
*Zhuoying Li,Zhu Xu,Yuxin Peng,Yang Liu*

Main category: cs.GR

TL;DR: The paper introduces BPM, a new metric for evaluating instruction-based image editing, focusing on balancing modification and preservation. It outperforms existing metrics by aligning closely with human evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing metrics for instruction-based image editing are either costly (human evaluation) or lack task-specific focus, leading to biased assessments.

Method: BPM disentangles images into editing-relevant and irrelevant regions, using a two-tier process (Region-Aware and Semantic-Aware Judges) for comprehensive evaluation.

Result: BPM aligns best with human evaluation, outperforming existing metrics, and can also improve editing quality when integrated into editing approaches.

Conclusion: BPM is an effective, interpretable, and broadly applicable metric for instruction-based image editing, addressing the limitations of current evaluation methods.
...
### [300] [Innovating China's Intangible Cultural Heritage with DeepSeek + MidJourney: The Case of Yangliuqing theme Woodblock Prints](https://arxiv.org/abs/2506.14104)
*RuiKun Yang,ZhongLiang Wei,Longdi Xian*

Main category: cs.GR

TL;DR: The study combines DeepSeek and MidJourney to create innovative Yangliuqing woodblock prints themed on COVID-19 and joyous winners, achieving high-quality results with low FID scores and positive participant feedback.


<details>
  <summary>Details</summary>
Motivation: To preserve and innovate traditional Yangliuqing woodblock prints by integrating modern AI tools while maintaining cultural authenticity.

Method: Used DeepSeek for thematic prompts and MidJourney for image generation, evaluated with FID scores and participant questionnaires.

Result: The hybrid method achieved the lowest mean FID score (150.2) and highest participant approval for cultural representation and interest.

Conclusion: The approach successfully blends tradition with AI, ensuring cultural preservation and contemporary appeal.
...
### [301] [ImmerseGen: Agent-Guided Immersive World Generation with Alpha-Textured Proxies](https://arxiv.org/abs/2506.14315)
*Jinyan Yuan,Bangbang Yang,Keke Wang,Panwang Pan,Lin Ma,Xuehai Zhang,Xiao Liu,Zhaopeng Cui,Yuewen Ma*

Main category: cs.GR

TL;DR: ImmerseGen introduces a lightweight, agent-guided framework for photorealistic 3D scene creation in VR, bypassing complex geometry modeling by using hierarchical proxies and synthesized textures.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D scene creation in VR rely on complex pipelines or limited realism, prompting the need for a simpler yet photorealistic approach.

Method: ImmerseGen uses hierarchical geometric proxies (terrain and billboard meshes) with synthesized RGBA textures, guided by agents for coherence. It includes terrain-conditioned texturing and VLM-based modeling agents for automation.

Result: ImmerseGen achieves superior photorealism, spatial coherence, and rendering efficiency, suitable for real-time mobile VR.

Conclusion: The framework simplifies 3D scene creation while maintaining visual quality, enabling efficient and immersive VR experiences.
...
### [302] [ReFrame: Layer Caching for Accelerated Inference in Real-Time Rendering](https://arxiv.org/abs/2506.13814)
*Lufei Liu,Tor M. Aamodt*

Main category: cs.GR

TL;DR: ReFrame introduces caching policies for neural networks in real-time rendering to balance quality and performance, achieving a 1.4x speedup with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Leverage temporal coherence in rendering tasks to reuse intermediate results and reduce redundant computations, improving efficiency.

Method: Extends caching of intermediate features to real-time rendering, exploring policies for encoder-decoder networks in rendering pipelines.

Result: Achieves 1.4x average speedup with negligible quality loss in three real-time rendering tasks.

Conclusion: ReFrame effectively optimizes rendering workloads by reusing cached features, enhancing performance without significant quality degradation.
...
### [303] [Connecting phases of matter to the flatness of the loss landscape in analog variational quantum algorithms](https://arxiv.org/abs/2506.13865)
*Kasidit Srimahajariyapong,Supanut Thanasilp,Thiparat Chotibut*

Main category: quant-ph

TL;DR: Analog VQA ansätze using disordered Ising chain quenches are studied, showing MBL phase avoids barren plateaus better than thermalized phase, enabling trainable initialization.


<details>
  <summary>Details</summary>
Motivation: To address scalability issues like barren plateaus in digital gate-based VQAs by exploring analog ansätze native to quantum simulators.

Method: Analyze expressivity and loss variance scaling of an analog VQA ansätze with quenches in thermalized or MBL phases of a disordered Ising chain.

Result: MBL phase avoids barren plateaus longer than thermalized phase, allowing trainable initialization at intermediate quenches.

Conclusion: MBL initialization strategy improves VQA trainability, linking quantum phases to practical analog-hardware scaling.
...
### [304] [Hamiltonian Formalism for Comparing Quantum and Classical Intelligence](https://arxiv.org/abs/2506.14456)
*Elija Perrier*

Main category: quant-ph

TL;DR: A Hamiltonian formalism is introduced to compare classical and quantum AGI tasks, decomposing AGI dynamics into core functions like induction and learning.


<details>
  <summary>Details</summary>
Motivation: To develop mathematical frameworks for comparing AGI operation in classical and quantum environments.

Method: Introducing a Hamiltonian formalism to decompose AGI dynamics into core functional generators.

Result: Provides a precise mathematical language for contrasting quantum and classical AGI agents.

Conclusion: The formalism aids in understanding differences between quantum and classical AGI via environmental interaction.
...
TL;DR: The paper highlights security risks in using ML classifiers for election tabulators, demonstrating vulnerabilities through new datasets, model testing, and adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: To analyze and demonstrate the security risks of machine learning classifiers in election tabulation, particularly focusing on adversarial vulnerabilities.

Method: Introduces four new ballot datasets, tests various models (SVMs, CNNs, vision transformers), overcomes gradient masking with modified loss, and conducts physical adversarial attacks.

Result: Shows that even a 5% attack success rate can flip election outcomes, with adversarial examples feasible in the physical world.

Conclusion: Machine learning in election tabulators poses significant security risks, requiring mitigation strategies for adversarial threats.
...
### [306] [Excessive Reasoning Attack on Reasoning LLMs](https://arxiv.org/abs/2506.14374)
*Wai Man Si,Mingjie Li,Michael Backes,Yang Zhang*

Main category: cs.CR

TL;DR: The paper addresses the issue of excessive computational costs in reasoning LLMs due to adversarial inputs and proposes a novel loss framework to exploit and mitigate this behavior.


<details>
  <summary>Details</summary>
Motivation: To expose and mitigate the vulnerability of reasoning LLMs to adversarial inputs that exploit excessive reasoning, increasing computational overhead without affecting utility.

Method: Introduces a loss framework with three components: Priority Cross-Entropy Loss, Excessive Reasoning Loss, and Delayed Termination Loss, optimized on GSM8K and ORCA datasets.

Result: Empirical results show a 3x to 9x increase in reasoning length with comparable utility, and adversarial inputs transfer to other models.

Conclusion: The proposed framework effectively exploits and mitigates excessive reasoning in LLMs, demonstrating transferability and scalability.
...
### [307] [LLM-Powered Intent-Based Categorization of Phishing Emails](https://arxiv.org/abs/2506.14337)
*Even Eilertsen,Vasileios Mavroeidis,Gudmund Grov*

Main category: cs.CR

TL;DR: LLMs can detect and categorize phishing emails by analyzing intent, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Phishing attacks evade current defenses, and traditional systems rely on metadata, not text. LLMs offer a better solution by focusing on intent.

Method: Used LLMs to classify phishing emails by intent, creating a custom dataset of legitimate and phishing emails.

Result: LLMs successfully detected and categorized phishing emails, showing promise for cybersecurity.

Conclusion: LLMs are effective for phishing detection and categorization, providing actionable threat insights.
...
### [308] [AMLgentex: Mobilizing Data-Driven Research to Combat Money Laundering](https://arxiv.org/abs/2506.13989)
...
TL;DR: AMLGentex is an open-source tool for generating realistic transaction data to benchmark AML detection methods, addressing gaps in existing synthetic datasets.


<details>
  <summary>Details</summary>
Motivation: Existing synthetic datasets for money laundering lack realism, failing to model key challenges like partial observability and network-level dependencies.

Method: Developed AMLGentex, a configurable suite to generate realistic transaction data and evaluate AML detection methods.

Result: The framework enables systematic evaluation of AML systems under conditions mirroring real-world complexity.

Conclusion: AMLGentex provides a controlled environment to rigorously test AML methods, improving detection capabilities.
...
### [309] [Density-aware Walks for Coordinated Campaign Detection](https://arxiv.org/abs/2506.13912)
*Atul Anand Gopalakrishnan,Jakir Hossain,Tuğrulcan Elmas,Ahmet Erdem Sarıyüce*

Main category: cs.SI

TL;DR: The paper introduces a method to detect coordinated campaigns on social media using graph classification, leveraging density-aware structural embeddings and outperforming traditional GNNs.


<details>
  <summary>Details</summary>
Motivation: Coordinated campaigns on social media manipulate trends, making it hard to distinguish them from genuine discourse. The challenge lies in detecting these sophisticated attacks.

Method: The paper models the problem as graph classification using the LEN dataset. It introduces a random weighted walk (RWW) approach biased by local density measures, encoded with Skip-gram for density-aware embeddings, and trains MPNNs on these embeddings.

Result: The proposed method improves accuracy by 12% for binary and 5% for multiclass classification compared to simpler node features.

Conclusion: Density-aware structural encoding with MPNNs effectively identifies coordinated inauthentic behavior on platforms like Twitter.
...
### [310] [Contemporary AI foundation models increase biological weapons risk](https://arxiv.org/abs/2506.13798)
*Roger Brent,T. Greg McKelvey*

Main category: cs.CY

TL;DR: The paper argues that current AI safety assessments underestimate the risk of AI facilitating biological weapons development due to flawed assumptions and inadequate methods. It challenges the tacit knowledge assumption and demonstrates AI's capability to guide complex tasks like poliovirus recovery.


<details>
  <summary>Details</summary>
Motivation: To highlight the underestimated biosecurity risks posed by advanced AI models, emphasizing flawed safety assessment assumptions.

Method: Examines cases of non-experts performing complex tasks, reviews documented pathogen construction processes, and tests AI models (Llama 3.1 405B, ChatGPT-4o, Claude 3.5 Sonnet) on guiding poliovirus recovery.

Result: AI models can accurately guide users through complex bioweapons-related tasks, contradicting claims of minimal biosecurity risk.

Conclusion: Improved benchmarks are needed, but the window for effective implementation may have closed.
...
### [311] [Dr. GPT Will See You Now, but Should It? Exploring the Benefits and Harms of Large Language Models in Medical Diagnosis using Crowdsourced Clinical Cases](https://arxiv.org/abs/2506.13805)
*Bonam Mingole,Aditya Majumdar,Firdaus Ahmed Choudhury,Jennifer L. Kraschnewski,Shyam S. Sundar,Amulya Yadav*

Main category: cs.CY

TL;DR: This paper evaluates LLMs' effectiveness in answering everyday health queries, finding 76% accuracy, and explores improvements using RAG-enhanced models.


<details>
  <summary>Details</summary>
Motivation: Addressing the gap in evaluating LLMs for real-world, everyday health concerns, beyond expert-written or clinical queries.

Method: A university competition crowdsourced 212 health queries to four LLMs, with responses evaluated by board-certified physicians. RAG-enhanced models were also tested.

Result: 76% of LLM responses were accurate; RAG models showed potential for improved quality.

Conclusion: LLMs perform reasonably well in everyday health communication, with RAG enhancements offering further promise.
...
### [312] [Students' Reliance on AI in Higher Education: Identifying Contributing Factors](https://arxiv.org/abs/2506.13845)
*Griffin Pitts,Neha Rani,Weedguet Mildort,Eva-Marie Cook*

Main category: cs.CY

TL;DR: The study explores factors influencing undergraduate students' reliance on AI tools in education, identifying correlations between reliance patterns and traits like self-efficacy, literacy, and cognition.


<details>
  <summary>Details</summary>
Motivation: Concerns about students' overreliance on AI tools, leading to flawed learning outcomes, prompted investigation into factors influencing reliance patterns.

Method: Combined pre- and post-surveys with a controlled experimental task where students solved programming problems with an AI assistant providing varied suggestions.

Result: Appropriate reliance linked to self-efficacy, literacy, and cognition; overreliance tied to trust and satisfaction; underreliance negatively correlated with literacy and cognition.

Conclusion: Findings suggest targeted interventions to promote appropriate AI reliance, aiding better integration of AI in education.
...
### [313] [Computational Studies in Influencer Marketing: A Systematic Literature Review](https://arxiv.org/abs/2506.14602)
*Haoyang Gui,Thales Bertaglia,Catalina Goanta,Gerasimos Spanakis*

Main category: cs.CY

TL;DR: A systematic review of computational studies in influencer marketing identifies key themes, methodologies, and gaps, advocating for multidisciplinary research and regulatory focus.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of influencer marketing lacks systematic computational reviews, limiting scientific measurements and stakeholder insights.

Method: A systematic literature review (SLR) based on PRISMA analyzed 69 studies to categorize themes and methodologies.

Result: Four themes emerged: influencer identification, advertising strategies, sponsored content analysis, and fairness. Methodologies included machine learning and non-machine learning techniques, with a commercial focus and limited ethical consideration.

Conclusion: The paper calls for nuanced research, improved explainability, standardized datasets, and links to regulation and compliance technology.
...
### [314] [The Synthetic Mirror -- Synthetic Data at the Age of Agentic AI](https://arxiv.org/abs/2506.13818)
*Marcelle Momha*

Main category: cs.CY

TL;DR: The paper discusses the rise of synthetic data and its impact on trust and accountability, urging policy updates to address privacy and AI reliance on such data.


<details>
  <summary>Details</summary>
Motivation: The increasing use of synthetic data in AI raises concerns about trust and accountability, necessitating policy and legal adaptations.

Method: The paper explores implications for privacy and policymaking, advocating for targeted amendments to existing legal frameworks.

Result: Synthetic data introduces trust and accountability deficits, requiring regulatory recognition as a distinct category.

Conclusion: Practical policy updates, rather than new regimes, are needed to manage synthetic data's unique challenges.
...
### [315] [Rigor in AI: Doing Rigorous AI Work Requires a Broader, Responsible AI-Informed Conception of Rigor](https://arxiv.org/abs/2506.14652)
...
TL;DR: The paper advocates for a broader definition of rigor in AI research, beyond just methodological correctness, to address concerns like exaggerated AI claims.


<details>
  <summary>Details</summary>
Motivation: To address the narrow focus on methodological rigor in AI, which overlooks other critical aspects like epistemic, normative, conceptual, reporting, and interpretative rigor.

Method: Proposes a framework expanding rigor to include six dimensions: methodological, epistemic, normative, conceptual, reporting, and interpretative rigor.

Result: A comprehensive framework for evaluating rigor in AI research, fostering better dialogue among stakeholders.

Conclusion: A broader conception of rigor is essential for responsible AI research and practice, providing a foundation for improved discourse and accountability.
...
### [316] [Asymptotically Smaller Encodings for Graph Problems and Scheduling](https://arxiv.org/abs/2506.14042)
*Bernardo Subercaseaux*

Main category: cs.LO

TL;DR: The paper presents a method to encode graph problems into CNF with fewer clauses, leveraging biclique coverings, and demonstrates practical applications in scheduling and string compression.


<details>
  <summary>Details</summary>
Motivation: To reduce the number of clauses in CNF encodings for graph problems, improving efficiency and understanding the success of preprocessing tools like Bounded Variable Addition.

Method: Utilizes biclique coverings of graphs to encode problems like vertex-cover and independent-set into CNF with fewer clauses. Also introduces a novel encoding for dense interval graphs.

Result: Achieves encodings with $O(|V|^2 / \lg |V|)$ clauses for general graphs and $O(|V| \lg |V|)$ for dense interval graphs, reducing encoding sizes in practical applications like scheduling.

Conclusion: The approach significantly reduces clause counts in CNF encodings, offering theoretical insights and practical benefits in problem-solving and preprocessing.
...
### [317] [Complete Characterization for Adjustment in Summary Causal Graphs of Time Series](https://arxiv.org/abs/2506.14534)
*Clément Yvernes,Emilie Devijver,Eric Gaussier*

Main category: math.ST

TL;DR: The paper addresses the identifiability problem for interventions in time series with summary causal graphs, proposing conditions for the adjustment criterion and a pseudo-linear algorithm to determine identifiability.


<details>
  <summary>Details</summary>
Motivation: To assess whether total causal effects can be estimated from observational data alone, especially in time series with abstracted causal graphs.

Method: Proposes necessary and sufficient conditions for the adjustment criterion and introduces a pseudo-linear algorithm for identifiability.

Result: The adjustment criterion is shown to be complete in this setting, and the algorithm efficiently decides identifiability.

Conclusion: The framework provides a practical solution for causal effect estimation in time series with abstracted graphs.
...
### [318] [Agile Orchestration at Will: An Entire Smart Service-Based Security Architecture Towards 6G](https://arxiv.org/abs/2505.22963)
...
TL;DR: The paper proposes ES3A, a novel security architecture for 6G networks, addressing challenges like heterogeneity and diverse security needs through principles like hierarchy, flexibility, and smart automation.


<details>
  <summary>Details</summary>
Motivation: 6G networks face security challenges due to heterogeneous threats and varied requirements, necessitating a re-designed security architecture.

Method: ES3A is introduced with six principles and three deployment guidelines, featuring a three-layer, three-domain structure and a two-stage orchestration mechanism.

Result: A prototype on an SDR-based radio system demonstrates ES3A's effectiveness, with a case study highlighting its superiority.

Conclusion: ES3A successfully addresses 6G security challenges, offering scalable, resilient, and automated protection.
...
### [319] [The Perception of Phase Intercept Distortion and its Application in Data Augmentation](https://arxiv.org/abs/2506.14571)
*Venkatakrishnan Vaidyanathapuram Krishnan,Nathaniel Condit-Schultz*

Main category: eess.SP

TL;DR: Phase-intercept distortion, a type of phase distortion, is imperceptible despite altering waveforms. It shows promise for data augmentation in audio machine learning, improving task performance.


<details>
  <summary>Details</summary>
Motivation: To investigate the perceptibility of phase-intercept distortion and explore its utility in machine learning, particularly for data augmentation.

Method: Conducted human-subject experiments to test perceptibility and applied phase-intercept distortion for data augmentation in audio machine learning tasks.

Result: Phase-intercept distortion is imperceptible, and its use in data augmentation improved performance in audio machine learning tasks.

Conclusion: Phase-intercept distortion, though altering waveforms, is imperceptible and beneficial for enhancing machine learning models through data augmentation.
...
### [320] [A Survey of Physics-Informed AI for Complex Urban Systems](https://arxiv.org/abs/2506.13777)
...
TL;DR: The paper reviews physics-informed AI methods for urban systems, categorizing them into three paradigms and seven methods, and examines their applications across eight urban domains to enhance reliability and efficiency.


<details>
  <summary>Details</summary>
Motivation: To integrate physics-based modeling with AI for improved predictive accuracy, interpretability, and decision-making in complex urban systems.

Method: A taxonomy of physics-informed AI methods is proposed, categorizing approaches into three paradigms (Physics-Integrated AI, Physics-AI Hybrid Ensemble, AI-Integrated Physics) and detailing seven representative methods.

Result: The review highlights how these methods address urban challenges by leveraging physical laws and data-driven models, improving system reliability, efficiency, and adaptability.

Conclusion: The synthesis identifies gaps and future research directions, aiming to advance intelligent urban system modeling.
...
### [321] [Infected Smallville: How Disease Threat Shapes Sociality in LLM Agents](https://arxiv.org/abs/2506.13783)
*Soyeon Choi,Kangwook Lee,Oliver Sng,Joshua M. Ackerman*

Main category: physics.soc-ph

TL;DR: Generative agent-based modeling (GABM) shows agents reduce social engagement when exposed to news about infectious diseases, validating disease-avoidance motivations.


<details>
  <summary>Details</summary>
Motivation: To understand how infectious disease threats influence social behavior using generative agents.

Method: Used GABM powered by large language models to simulate agent behavior in response to disease news.

Result: Agents exposed to disease news showed reduced social engagement (fewer gatherings, visits, conversations).

Conclusion: GABM is a promising tool for studying human social dynamics at scale.
...
### [322] [A Silent Speech Decoding System from EEG and EMG with Heterogenous Electrode Configurations](https://arxiv.org/abs/2506.13835)
...
TL;DR: Neural networks for silent speech decoding from EEG/EMG improve accuracy despite heterogeneous data, benefiting speech-impaired patients.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of collecting large, homogeneous EEG/EMG datasets for silent speech decoding due to varying experimental setups.

Method: Introduce neural networks capable of handling heterogeneous electrode placements and use multi-task training on large-scale datasets.

Result: Achieved high word classification accuracy (95.3% in healthy, 54.5% in speech-impaired) and improved cross-language calibration.

Conclusion: Demonstrates feasibility of practical silent speech decoding systems, especially for speech-impaired individuals.
...
### [323] [Analysis and Optimization of Probabilities of Beneficial Mutation and Crossover Recombination in a Hamming Space](https://arxiv.org/abs/2506.13809)
*Roman V. Belavkin*

Main category: q-bio.PE

TL;DR: The paper analyzes beneficial mutation and crossover recombination probabilities in a Hamming space, deriving optimal conditions for evolutionary operators.


<details>
  <summary>Details</summary>
Motivation: Inspired by Fisher's geometric approach, the study aims to understand how mutations and recombinations reduce distance to an optimum in evolutionary processes.

Method: Geometric and combinatorial analysis is used to derive closed-form expressions for transition probabilities between spheres around an optimum, describing Markov evolution of distances.

Result: Optimality conditions for mutation and recombination radii are derived, highlighting differences: mutation slows near the optimum, while recombination may boost evolution.

Conclusion: Recombination complements mutation, potentially enhancing evolutionary rates near optima, with implications for optimizing evolutionary algorithms.
...
### [324] [Knowledge Compression via Question Generation: Enhancing Multihop Document Retrieval without Fine-tuning](https://arxiv.org/abs/2506.13778)
*Anvi Alex Eponon,Moein Shahiki-Tash,Ildar Batyrshin,Christian E. Maldonado-Sifuentes,Grigori Sidorov,Alexander Gelbukh*

Main category: cs.IR

TL;DR: A question-based knowledge encoding method improves RAG systems without fine-tuning or chunking, achieving high recall and efficiency.


<details>
  <summary>Details</summary>
Motivation: To enhance retrieval-augmented generation systems by eliminating the need for fine-tuning or traditional chunking, improving efficiency and scalability.

Method: Encodes textual content using generated questions for targeted retrieval, combined with syntactic reranking. Introduces 'paper-cards' for concise summaries.

Result: Outperforms traditional methods with Recall@3 of 0.84, MRR@3 increase to 0.85, and F1 score of 0.52 in multihop tasks. Reduces storage by 80%.

Conclusion: The approach is scalable, efficient, and reduces retrieval latency, making it a viable alternative to traditional RAG methods.
...
### [325] [XGraphRAG: Interactive Visual Analysis for Graph-based Retrieval-Augmented Generation](https://arxiv.org/abs/2506.13782)
*Ke Wang,Bo Pan,Yingchaojie Feng,Yuwei Wu,Jieyi Chen,Minfeng Zhu,Wei Chen*

Main category: cs.IR

TL;DR: The paper introduces XGraphRAG, a visual analysis framework to improve the interpretability and accessibility of GraphRAG by helping developers analyze recalls and trace them through the pipeline.


<details>
  <summary>Details</summary>
Motivation: GraphRAG enhances LLM answers with structured relational knowledge but lacks interpretability due to its complex pipeline and high LLM invocation demands.

Method: Proposes a visual analysis framework and develops XGraphRAG, a prototype with interactive visualizations to analyze GraphRAG recalls.

Result: The framework improves failure case collection and identifies improvement opportunities, demonstrating effectiveness and usability.

Conclusion: XGraphRAG enhances GraphRAG's interpretability and accessibility, with the work being open-sourced for community use.
...
### [326] [Analysis of Anonymous User Interaction Relationships and Prediction of Advertising Feedback Based on Graph Neural Network](https://arxiv.org/abs/2506.13787)
*Yanjun Dai,Haoyang Feng,Yuan Gao*

Main category: cs.IR

TL;DR: DTH-GNN improves ad engagement inference by capturing multi-scale temporal, semantic, and higher-order dependencies in user interaction networks, outperforming baselines with 8.2% higher AUC and 5.7% lower log loss.


<details>
  <summary>Details</summary>
Motivation: Existing graph models fail to capture the complex, multi-scale features of anonymous user interactions in online advertising, limiting engagement inference and ad delivery optimization.

Method: Proposes DTH-GNN with temporal edge decomposition (short-term burst, diurnal cycle, long-range memory), hierarchical heterogeneous aggregation via meta-path Transformer, and contrast regularity for feedback perception.

Result: DTH-GNN achieves 8.2% higher AUC and 5.7% lower logarithmic loss compared to the best baseline.

Conclusion: DTH-GNN effectively models complex anonymous behavior patterns, enhancing ad engagement inference and delivery strategies.
...
### [327] [AcademicBrowse: Benchmarking Academic Browse Ability of LLMs](https://arxiv.org/abs/2506.13784)
*Junting Zhou,Wang Li,Yiyan Liao,Nengyuan Zhang,Tingjia Miaoand Zhihui Qi,Yuhan Wu,Tong Yang*

Main category: cs.IR

TL;DR: AcademicBrowse is a new dataset designed to evaluate LLMs' complex academic search capabilities, addressing gaps in existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks like OpenAI's BrowseComp lack focus on academic search demands, such as literature tracing, professional database support, and academic rigor.

Method: Proposed AcademicBrowse dataset with key characteristics: Academic Practicality, High Difficulty, Concise Evaluation, and Broad Coverage.

Result: AcademicBrowse spans 15 disciplines, provides unique answers with clear sources, and challenges LLMs with deep search requirements.

Conclusion: AcademicBrowse aims to precisely measure and enhance LLMs' performance in academic information retrieval.
...
### [328] [InsertRank: LLMs can reason over BM25 scores to Improve Listwise Reranking](https://arxiv.org/abs/2506.14086)
*Rahul Seetharaman,Kaustubh D. Dhole,Aman Bansal*

Main category: cs.IR

TL;DR: InsertRank, an LLM-based reranker, improves retrieval performance by combining lexical signals (BM25) with reasoning, outperforming benchmarks like BRIGHT and R2MED.


<details>
  <summary>Details</summary>
Motivation: User expectations for complex queries require retrieval by reasoning, not just keyword matching. Existing LLM rerankers lack optimal performance.

Method: InsertRank integrates BM25 scores during reranking to enhance retrieval. Evaluated on BRIGHT and R2MED benchmarks across GPT, Gemini, and Deepseek models.

Result: InsertRank achieves 37.5 on BRIGHT and 51.1 on R2MED, surpassing prior methods.

Conclusion: InsertRank effectively combines lexical and reasoning signals, improving retrieval across diverse domains and LLM families.
...
### [329] [RAGtifier: Evaluating RAG Generation Approaches of State-of-the-Art RAG Systems for the SIGIR LiveRAG Competition](https://arxiv.org/abs/2506.14412)
*Tim Cofala,Oleh Astappiev,William Xion,Hailay Teklehaymanot*

Main category: cs.IR

TL;DR: The paper discusses the LiveRAG 2025 challenge, where RAG solutions were tested to improve factual correctness in LLMs. The winning solution combined InstructRAG, Pinecone retriever, and BGE reranker, achieving notable scores.


<details>
  <summary>Details</summary>
Motivation: To enhance factual correctness and reduce hallucinations in LLMs by integrating external knowledge sources through RAG.

Method: Used InstructRAG with Pinecone retriever and BGE reranker under challenge constraints (10B parameter LLMs, Falcon-3-10B for answers).

Result: Achieved correctness score of 1.13 and faithfulness score of 0.55, ranking fourth in the challenge.

Conclusion: The proposed RAG solution demonstrated effectiveness in improving factual accuracy, though further optimization is possible.
...
### [330] [A Hybrid Neural Network -- Polynomial Series Scheme for Learning Invariant Manifolds of Discrete Dynamical Systems](https://arxiv.org/abs/2506.13950)
*Dimitrios G. Patsatzis,Nikolaos Kazantzis,Ioannis G. Kevrekidis,Constantinos Siettos*

Main category: math.NA

TL;DR: A hybrid machine learning scheme combining polynomials and shallow neural networks is proposed for learning invariant manifolds in reduced-order models, outperforming pure polynomial or neural network methods.


<details>
  <summary>Details</summary>
Motivation: To improve the accuracy and efficiency of reduced-order models for dynamical systems by leveraging the strengths of both polynomial series and neural networks.

Method: Combines polynomial series (for local exponential convergence) with shallow neural networks (for complex structures) to learn invariant manifolds. Evaluated on three benchmarks.

Result: The hybrid scheme outperforms pure polynomial and standalone neural network approximations in numerical accuracy.

Conclusion: The hybrid approach is effective for constructing accurate and efficient reduced-order models for dynamical systems.
...
### [331] [Beyond Shapley Values: Cooperative Games for the Interpretation of Machine Learning Models](https://arxiv.org/abs/2506.13900)
*Marouane Il Idrissi,Agathe Fernandes Machado,Arthur Charpentier*

Main category: stat.ML

TL;DR: The paper critiques Shapley values in feature attribution, proposing broader cooperative game theory tools like Weber and Harsanyi sets for richer interpretability.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of Shapley-based methods in interpretability and advocate for more principled, flexible approaches.

Method: Revisits cooperative game theory, introduces Weber and Harsanyi sets, and provides a three-step framework for feature attribution.

Result: Highlights alternative allocation schemes and clarifies distinctions between value functions and aggregation rules.

Conclusion: Proposes a shift from fixed axioms to a coherent framework for designing robust and meaningful attribution methods in XAI.
...
### [332] [Mirror Descent Using the Tempesta Generalized Multi-parametric Logarithms](https://arxiv.org/abs/2506.13984)
*Andrzej Cichocki*

Main category: stat.ML

TL;DR: The paper introduces a class of Mirror Descent (MD) algorithms using Bregman divergence with a Tempesta multi-parametric deformation logarithm as a link function, enabling flexible updates adaptable to data distribution.


<details>
  <summary>Details</summary>
Motivation: To develop a versatile and adaptable class of MD algorithms for machine learning by leveraging generalized trace-form entropies and multi-parametric logarithms.

Method: Formulated constrained optimization using Bregman divergence with Tempesta logarithm as a link function, estimated its inverse (generalized exponential), and tuned hyperparameters for adaptability.

Result: A flexible family of MD and mirror-less MD updates adaptable to data distribution and geometry through learned hyperparameters.

Conclusion: The Tempesta logarithm enables a wide, adaptable class of MD algorithms, enhancing flexibility and performance in machine learning applications.
...
### [333] [Rademacher learning rates for iterated random functions](https://arxiv.org/abs/2506.13946)
*Nikola Sandrić*

Main category: stat.ML

TL;DR: The paper addresses supervised learning under non-i.i.d. data, focusing on time-dependent processes like Markov chains. It provides uniform convergence and learnability results for empirical risk minimization, with data-dependent learning rates.


<details>
  <summary>Details</summary>
Motivation: The i.i.d. assumption in supervised learning is often unrealistic for real-world problems with temporal dependence. This work explores learning under non-i.i.d. conditions, specifically for iterated random functions.

Method: The study assumes a contractive governing function and regularity conditions on the hypothesis class. It establishes uniform convergence for sample error and analyzes the learnability of empirical risk minimization.

Result: The paper derives data-distribution-dependent learning rates, expressed via Rademacher complexities, offering more accurate bounds than i.i.d.-based approaches.

Conclusion: The framework provides a more realistic and adaptable approach for learning under temporal dependence, with practical implications for hypothesis class selection and sample complexity.
...
### [334] [Meta Optimality for Demographic Parity Constrained Regression via Post-Processing](https://arxiv.org/abs/2506.13947)
*Kazuto Fukuchi*

Main category: stat.ML

TL;DR: The paper introduces meta-theorems to validate fair minimax optimal regression algorithms under demographic parity, showing post-processing can achieve fairness without altering conventional regression methods.


<details>
  <summary>Details</summary>
Motivation: To generalize fair minimax optimal regression beyond specific data models and simplify fair regression by leveraging post-processing.

Method: Develops meta-theorems applicable to various scenarios and demonstrates post-processing techniques for fair regression.

Result: Fair minimax optimal regression can be achieved via post-processing, enabling adaptation of conventional methods.

Conclusion: Post-processing offers a practical way to achieve fair regression, decoupling fairness from core regression improvements.
...
### [335] [Bridging Unsupervised and Semi-Supervised Anomaly Detection: A Theoretically-Grounded and Practical Framework with Synthetic Anomalies](https://arxiv.org/abs/2506.13955)
*Matthew Lau,Tian-Yi Zhou,Xiangchi Yuan,Jizhou Chen,Wenke Lee,Xiaoming Huo*

Main category: stat.ML

TL;DR: The paper proposes a semi-supervised anomaly detection (AD) framework combining known and synthetic anomalies, offering theoretical guarantees and empirical improvements.


<details>
  <summary>Details</summary>
Motivation: Anomaly detection is crucial in fields like cybersecurity and healthcare. The paper addresses the gap in semi-supervised AD by leveraging both labeled and synthetic anomalies.

Method: The framework trains classifiers using a mix of known and synthetic anomalies, introducing the first mathematical formulation for semi-supervised AD.

Result: The approach improves anomaly modeling and provides optimal convergence guarantees for neural networks, validated on five benchmarks.

Conclusion: The synthetic anomaly principle enhances semi-supervised AD performance and generalizes to other classification-based methods.
...
### [336] [Estimation of Treatment Effects in Extreme and Unobserved Data](https://arxiv.org/abs/2506.14051)
*Jiyuan Tan,Jose Blanchet,Vasilis Syrgkanis*

Main category: stat.ML

TL;DR: A novel framework for estimating causal effects of interventions on rare, impactful events using Extreme Value Theory (EVT) and multivariate regular variation.


<details>
  <summary>Details</summary>
Motivation: Existing causal inference methods focus on frequent events, leaving a gap for rare but impactful events like extreme climate events.

Method: Uses EVT and multivariate regular variation to model extremities, developing a consistent estimator for extreme treatment effects.

Result: Demonstrates performance with synthetic and semi-synthetic data, providing a rigorous non-asymptotic analysis.

Conclusion: The framework effectively addresses causal effect estimation for rare events, filling a critical gap in the literature.
...
### [337] [Universal Rates of ERM for Agnostic Learning](https://arxiv.org/abs/2506.14110)
*Steve Hanneke,Mingyue Xu*

Main category: stat.ML

TL;DR: The paper explores universal learning rates for binary classification under the agnostic setting, revealing three possible rates for ERM: exponential, sublinear, or arbitrarily slow. It characterizes concept classes for each rate and extends to target- and Bayes-dependent rates.


<details>
  <summary>Details</summary>
Motivation: Prior work focused on the restrictive realizable setting, leaving the more practical agnostic case unexplored. This paper addresses this gap by studying universal learning rates for ERM in agnostic binary classification.

Method: The study analyzes the agnostic setting, examining the decay of excess risk (learning curve) with increasing sample size. It identifies three possible universal rates for ERM and characterizes concept classes accordingly.

Result: A trichotomy of agnostic universal rates is revealed: exponential ($e^{-n}$), sublinear ($o(n^{-1/2})$), or arbitrarily slow. Complete characterizations are provided for concept classes, target-dependent, and Bayes-dependent rates.

Conclusion: The paper fills a gap in universal learning theory by extending analysis to the agnostic setting, providing a comprehensive framework for understanding ERM's universal rates in binary classification.
...
### [338] [Adjustment for Confounding using Pre-Trained Representations](https://arxiv.org/abs/2506.14329)
*Rickmer Schulte,David Rügamer,Thomas Nagler*

Main category: stat.ML

TL;DR: The paper explores using latent features from pre-trained neural networks to adjust for confounding in ATE estimation, addressing challenges like high dimensionality and non-identifiability.


<details>
  <summary>Details</summary>
Motivation: To extend ATE estimation to non-tabular data (e.g., images, text) and avoid biased results by leveraging pre-trained neural networks for confounding adjustment.

Method: Leverages latent features from pre-trained neural networks, formalizes conditions for valid adjustment, and uses double machine learning for inference.

Result: Demonstrates neural networks can adapt to intrinsic sparsity and dimensionality, achieving fast convergence rates despite challenges.

Conclusion: Neural networks are effective for ATE estimation with non-tabular data, overcoming traditional limitations of high-dimensional latent features.
...
### [339] [Adaptive Data Augmentation for Thompson Sampling](https://arxiv.org/abs/2506.14479)
*Wonyoung Kim*

Main category: stat.ML

TL;DR: Proposes a nearly minimax optimal Thompson Sampling for linear contextual bandits using a novel estimator for efficient parameter learning.


<details>
  <summary>Details</summary>
Motivation: Thompson Sampling performs well empirically but lacks optimal regret bounds in linear contextual bandits.

Method: Develops a novel estimator with adaptive augmentation and coupling of hypothetical samples for efficient parameter learning.

Result: Empirical results show robust performance and significant improvement over existing methods.

Conclusion: The proposed method achieves nearly minimax optimal regret bounds without relying on context distribution assumptions.
...
### [340] [Sharp Generalization Bounds for Foundation Models with Asymmetric Randomized Low-Rank Adapters](https://arxiv.org/abs/2506.14530)
...
### [341] [Uniform Mean Estimation for Heavy-Tailed Distributions via Median-of-Means](https://arxiv.org/abs/2506.14673)
*Mikael Møller Høgsgaard,Andrea Paudice*

Main category: stat.ML

TL;DR: The paper analyzes the Median of Means (MoM) estimator for heavy-tailed data, proving a new sample complexity bound and applying it to k-means clustering and linear regression.


<details>
  <summary>Details</summary>
Motivation: To improve mean estimation for heavy-tailed data with limited moments (p ∈ (1,2]) and extend applications to clustering and regression.

Method: Uses a novel symmetrization technique to derive a sample complexity bound for MoM in function classes.

Result: Achieves improved sample complexity bounds and extends MoM's applicability to k-means clustering and linear regression.

Conclusion: The MoM estimator is effective for heavy-tailed data with limited moments, with practical applications in clustering and regression.
...
### [342] [Markov Regime-Switching Intelligent Driver Model for Interpretable Car-Following Behavior](https://arxiv.org/abs/2506.14762)
*Chengyuan Zhang,Cathy Wu,Lijun Sun*

Main category: stat.AP

TL;DR: The paper introduces a regime-switching framework (FHMM-IDM) to model multi-modal human driving behavior, improving accuracy and interpretability over classical models like IDM.


<details>
  <summary>Details</summary>
Motivation: Classical car-following models (e.g., IDM) are limited by their single-regime structure, failing to capture diverse driver behaviors and reducing interpretability.

Method: A Factorial Hidden Markov Model with IDM dynamics (FHMM-IDM) is proposed, using Bayesian inference (MCMC) to estimate regime-specific parameters and latent states.

Result: Experiments on the HighD dataset show FHMM-IDM successfully disentangles driver actions from traffic conditions, revealing interpretable regime-switching patterns.

Conclusion: FHMM-IDM offers a principled solution for context-dependent driving behavior, enhancing traffic simulation fidelity and ADAS development.
...
### [343] [Déjà Vu: Efficient Video-Language Query Engine with Learning-based Inter-Frame Computation Reuse](https://arxiv.org/abs/2506.14107)
...
TL;DR: Déjà Vu accelerates ViT-based VideoLMs by reusing computations across frames, achieving up to 2.64x speedup with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Current VideoLMs face scalability issues due to high computational costs of processing large-scale videos with ViTs.

Method: Introduces ReuseViT, a modified ViT for detecting inter-frame reuse opportunities, and memory-compute joint compaction techniques.

Result: Achieves up to 2.64x acceleration in embedding generation with less than 2% error.

Conclusion: Déjà Vu enhances the practicality of VideoLMs for large-scale video analytics by reducing computational overhead.
...
### [344] [MobileHolo: A Lightweight Complex-Valued Deformable CNN for High-Quality Computer-Generated Hologram](https://arxiv.org/abs/2506.14542)
*Xie Shuyang,Zhou Jie,Xu Bo,Wang Jun,Xu Renjing*

Main category: physics.optics

TL;DR: The paper introduces a complex-valued deformable convolution method for holographic displays, improving feature extraction and outperforming existing models in performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: Holographic displays are promising for VR/AR, but current deep learning-based methods struggle with accurate modeling due to limited effective receptive fields.

Method: The authors designed complex-valued deformable convolution to dynamically adjust kernel shapes, enhancing flexibility and feature extraction.

Result: The method achieves state-of-the-art performance, with PSNR improvements of 2.04 dB to 9.71 dB over competitors, and uses fewer parameters (1/8th of CCNN-CGH).

Conclusion: The proposed approach effectively addresses ERF limitations, offering superior performance and efficiency for computer-generated holograms.
...
### [345] [Accurate and scalable exchange-correlation with deep learning](https://arxiv.org/abs/2506.14665)
...
TL;DR: Skala, a deep learning-based XC functional, achieves chemical accuracy for small molecules while maintaining computational efficiency, outperforming traditional hand-crafted XC functionals.


<details>
  <summary>Details</summary>
Motivation: Current XC functionals in DFT lack accuracy and generality for predictive modeling at chemical accuracy (errors below 1 kcal/mol).

Method: Skala bypasses hand-designed features by learning representations directly from data, trained on extensive high-accuracy reference data.

Result: Skala achieves chemical accuracy for atomization energies and performs competitively with hybrid functionals across general main group chemistry.

Conclusion: Skala's performance improves with more training data, promising enhanced predictive power for first-principles simulations.
...
### [346] [Safe Domains of Attraction for Discrete-Time Nonlinear Systems: Characterization and Verifiable Neural Network Estimation](https://arxiv.org/abs/2506.13961)
*Mohamed Serry,Haoyu Li,Ruikun Zhou,Huan Zhang,Jun Liu*

Main category: eess.SY

TL;DR: A framework for estimating safe domains of attraction for nonlinear systems using a new Zubov equation and neural networks, with verification tools for certifiable results.


<details>
  <summary>Details</summary>
Motivation: Accurately estimating domains of attraction for nonlinear systems, especially under state constraints, is challenging and existing methods are conservative or limited.

Method: Derives a new Zubov equation for exact safe domains, uses neural networks to approximate solutions, and verifies results with tools like α,β-CROWN and dReal.

Result: The solution to the Zubov equation is unique and continuous, and the framework provides certifiable estimates of safe domains.

Conclusion: The proposed approach effectively estimates safe domains of attraction for constrained nonlinear systems, demonstrated through numerical examples.
...
### [347] [BraTS orchestrator : Democratizing and Disseminating state-of-the-art brain tumor image analysis](https://arxiv.org/abs/2506.13807)
...
TL;DR: BraTS orchestrator is an open-source Python package that simplifies access to advanced brain tumor segmentation algorithms from the BraTS challenge, making them easier to use for researchers and clinicians.


<details>
  <summary>Details</summary>
Motivation: Despite the success of BraTS challenges, their algorithms see limited adoption. The goal is to bridge this gap by providing an accessible tool.

Method: Developed BraTS orchestrator, an open-source package with tutorials for easy deployment of BraTS algorithms.

Result: The package is available on GitHub, enabling broader use of BraTS advancements.

Conclusion: BraTS orchestrator democratizes access to specialized brain tumor segmentation tools, benefiting the neuro-radiology and neuro-oncology communities.
...
### [348] [Reliable Noninvasive Glucose Sensing via CNN-Based Spectroscopy](https://arxiv.org/abs/2506.13819)
*El Arbi Belfarsi,Henry Flores,Maria Valero*

Main category: eess.IV

TL;DR: A dual-modal AI framework using SWIR spectroscopy for non-invasive glucose monitoring, combining CNNs for spatial features and photodiode sensors with ML regressors, achieving high accuracy.
...
Method: Two modalities: 1) SWIR imaging with CNNs for spatial glucose absorption features, 2) photodiode voltage sensor with ML regressors on normalized optical signals. Tested on synthetic blood phantoms and skin-mimicking materials.

Result: CNN achieved 4.82% MAPE at 650 nm with 100% Zone A coverage; photodiode system reached 86.4% Zone A accuracy.

Conclusion: The framework is state-of-the-art, balancing clinical accuracy, cost efficiency, and wearable integration for reliable glucose monitoring.
...
### [349] [Latent Anomaly Detection: Masked VQ-GAN for Unsupervised Segmentation in Medical CBCT](https://arxiv.org/abs/2506.14209)
*Pengwei Wang*

Main category: eess.IV

TL;DR: A novel unsupervised method for segmenting ONJ anomalies in 3D medical images using a two-stage VQ-GAN pipeline, reducing manual labeling effort and enabling potential 3D printing applications.


<details>
  <summary>Details</summary>
Motivation: The scarcity of labeled data in ONJ imaging makes supervised training impractical, necessitating an unsupervised approach for anomaly detection.

Method: A two-stage pipeline: 1) Train a VQ-GAN to reconstruct normal subjects; 2) Apply random and ONJ-specific masking to train an encoder for anomaly recovery.

Result: Successful segmentation on simulated and real patient data.

Conclusion: The method offers fast initial segmentation, reduces manual labeling, and has potential for direct 3D printing applications with post-processing.
...
### [350] [orGAN: A Synthetic Data Augmentation Pipeline for Simultaneous Generation of Surgical Images and Ground Truth Labels](https://arxiv.org/abs/2506.14303)
*Niran Nataraj,Maina Sogabe,Kenji Kawashima*

Main category: eess.IV

TL;DR: orGAN is a GAN-based system for generating realistic surgical bleeding images, addressing data scarcity and ethical issues in medical AI.


<details>
  <summary>Details</summary>
Motivation: Overcome challenges like limited data diversity, ethical concerns, and high costs in surgical bleeding detection by creating synthetic datasets.

Method: Uses StyleGAN with Relational Positional Learning to simulate bleeding and LaMa-based inpainting for clean pre-bleed visuals, enabling precise annotations.

Result: Achieved 90% detection accuracy in surgical settings and up to 99% frame-level accuracy with a balanced dataset.

Conclusion: orGAN advances ethical, efficient, and cost-effective creation of annotated bleeding datasets, supporting AI integration in surgery.
...
### [351] [BRISC: Annotated Dataset for Brain Tumor Segmentation and Classification with Swin-HAFNet](https://arxiv.org/abs/2506.14318)
...
<details>
  <summary>Details</summary>
Motivation: The lack of high-quality, diverse datasets for brain tumor analysis in MRI motivates the creation of this dataset to improve segmentation and classification tasks.

Method: The dataset includes 6,000 contrast-enhanced T1-weighted MRI scans annotated by experts. A transformer-based segmentation model is proposed and benchmarked against baselines.

Result: The model achieves a weighted mean IoU of 82.3%, with improvements across all tumor types.

Conclusion: The dataset and model serve as foundational benchmarks, aiming to advance machine learning in neuro-oncology for research and clinical applications.
...
### [352] [Compressed Video Super-Resolution based on Hierarchical Encoding](https://arxiv.org/abs/2506.14381)
*Yuxuan Jiang,Siyue Teng,Qiang Zhu,Chen Feng,Chengxi Zeng,Fan Zhang,Shuyuan Zhu,Bing Zeng,David Bull*

Main category: eess.IV

TL;DR: VSR-HE is a video super-resolution method for enhancing compressed content, scaling videos from 180p/270p to 720p/1080p, using hierarchical encoding transformer blocks to remove compression artifacts.


<details>
  <summary>Details</summary>
Motivation: To improve perceptual quality of heavily compressed videos by addressing artifacts from H.265/HEVC encoding.
...
Result: Effectively restores fine details and preserves visual fidelity in compressed videos.

Conclusion: VSR-HE is robust and generalizable, submitted to ICME 2025 Grand Challenge for VSR in video conferencing.
...
### [353] [A large-scale heterogeneous 3D magnetic resonance brain imaging dataset for self-supervised learning](https://arxiv.org/abs/2506.14432)
...
<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive, heterogeneous dataset for developing and benchmarking self-supervised learning methods in medical imaging.

Method: Aggregated 16 publicly available sources, minimal preprocessing, included clinical/research-grade images, multiple MRI sequences, and diverse pathologies.

Result: A dataset of 60,529 MRI scans with wide anatomical and pathological variability, accompanied by pretraining and finetuning code.

Conclusion: FOMO60K facilitates scalable self-supervised learning in medical imaging by offering a large, varied dataset with minimal preprocessing.
...
### [354] [Towards Reliable WMH Segmentation under Domain Shift: An Application Study using Maximum Entropy Regularization to Improve Uncertainty Estimation](https://arxiv.org/abs/2506.14497)
*Franco Matzkin,Agostina Larrazabal,Diego H Milone,Jose Dolz,Enzo Ferrante*

Main category: eess.IV

TL;DR: The study explores how domain shifts affect WMH segmentation and proposes maximum-entropy regularization to improve model calibration and uncertainty estimation, showing its effectiveness in predicting errors without ground-truth labels.


<details>
  <summary>Details</summary>
Motivation: Domain shifts in MRI data (e.g., machine types or acquisition parameters) challenge WMH segmentation models, necessitating better calibration and uncertainty estimation for clinical use.

Method: The study uses a U-Net architecture with maximum-entropy regularization, tested on two datasets, evaluating performance via Dice coefficient, expected calibration error, and entropy-based uncertainty.

Result: Entropy-based uncertainty predicts segmentation errors, and maximum-entropy regularization enhances uncertainty-performance correlation and model calibration under domain shift.

Conclusion: Maximum-entropy regularization improves WMH segmentation model reliability under domain shifts by leveraging uncertainty estimation.
...
### [355] [Integrating Radiomics with Deep Learning Enhances Multiple Sclerosis Lesion Delineation](https://arxiv.org/abs/2506.14524)
*Nadezhda Alsahanova,Pavel Bartenev,Maksim Sharaev,Milos Ljubisavljevic,Taleb Al. Mansoori,Yauhen Statsenko*

Main category: eess.IV

TL;DR: The study improves MS lesion segmentation by fusing radiomic features with raw imaging data using deep learning models, showing enhanced accuracy and stability.


<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for MS lesion segmentation lack robustness, prompting the need for improved methods.

Method: Combined novel radiomic features (concentration rate and Rényi entropy) with raw imaging data using ResNeXt-UNet and attention-augmented U-Net architectures, evaluated on 46 patients' scans.

Result: Radiomics-enhanced models achieved higher precision, sensitivity (Dice score 0.774±0.05), and stability (reduced variability, p=0.03).

Conclusion: Fusing radiomics with imaging data improves segmentation performance and model stability in advanced deep learning models.
...
### [356] [Plug-and-Play with 2.5D Artifact Reduction Prior for Fast and Accurate Industrial Computed Tomography Reconstruction](https://arxiv.org/abs/2506.14719)
*Haley Duba-Sullivan,Aniket Pramanik,Venkatakrishnan Singanallur,Amirkoushyar Ziabari*

Main category: eess.IV

TL;DR: A 2.5D CNN-based PnP method improves XCT reconstruction quality by leveraging inter-slice information, outperforming 2D priors in artifact reduction and detail preservation.


<details>
  <summary>Details</summary>
Motivation: High-quality XCT reconstructions require many measurements, which is slow and expensive. Existing 2D CNN priors lack inter-slice context, limiting performance.

Method: Proposes a PnP reconstruction method using a 2.5D artifact reduction CNN to capture richer spatial context while remaining efficient.

Result: The 2.5D prior improves reconstruction quality, suppresses artifacts without pre-processing, and preserves fine structural details better than 2D priors.

Conclusion: The method generalizes well across domains, even when trained on simulated data, and enhances defect detection accuracy.
...
### [357] [Comparison of ConvNeXt and Vision-Language Models for Breast Density Assessment in Screening Mammography](https://arxiv.org/abs/2506.13964)
*Yusdivia Molina-Román,David Gómez-Ortiz,Ernestina Menasalvas-Ruiz,José Gerardo Tamez-Peña,Alejandro Santos-Díaz*

Main category: eess.IV

TL;DR: The study compares multimodal and CNN-based methods for mammographic breast density classification, finding that fine-tuned CNN models outperform multimodal approaches, though multimodal learning shows potential.


<details>
  <summary>Details</summary>
Motivation: Breast density classification is crucial for cancer risk assessment but suffers from subjectivity and variability. Automated methods are needed to improve accuracy and consistency.

Method: The study evaluates BioMedCLIP (multimodal) and ConvNeXt (CNN-based) across three learning scenarios: zero-shot classification, linear probing with textual descriptions, and fine-tuning with numerical labels.

Result: Zero-shot classification performed modestly, while fine-tuned ConvNeXt outperformed BioMedCLIP linear probing. Linear probing showed potential but was less effective than fine-tuning.

Conclusion: CNN-based models with fine-tuning are superior for medical imaging tasks, but multimodal learning could benefit from better textual representations and domain-specific adaptations.
...
### [358] [Reimagining Target-Aware Molecular Generation through Retrieval-Enhanced Aligned Diffusion](https://arxiv.org/abs/2506.14488)
*Dong Xu,Zhangfan Yang,Ka-chun Wong,Zexuan Zhu,Jiangqiang Li,Junkai Ji*

Main category: q-bio.BM

TL;DR: READ combines retrieval-augmented generation with SE(3)-equivariant diffusion to balance geometric fit and chemical constraints in drug design, outperforming state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of balancing pocket-specific geometric fit with chemical constraints in receptor-based molecule design.

Method: Introduces READ, a model merging molecular Retrieval-Augmented Generation with SE(3)-equivariant diffusion, using a contrastively pre-trained encoder and retrieved graph embeddings.

Result: Achieves competitive performance in CBGBench, surpassing state-of-the-art generative models and native ligands.
...
### [359] [Enhancing Clinical Decision Support and EHR Insights through LLMs and the Model Context Protocol: An Open-Source MCP-FHIR Framework](https://arxiv.org/abs/2506.13800)
*Abul Ehtesham,Aditi Singh,Saket Kumar*

Main category: cs.SE

TL;DR: An open-source, agent-based framework integrates LLMs with HL7 FHIR data via MCP for dynamic EHR extraction and reasoning, enhancing CDS, reducing documentation burdens, and improving health literacy.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in digital health, such as enhancing clinical decision support, reducing documentation burdens, and improving patient health literacy.

Method: An agent-based framework using LLMs and HL7 FHIR data via MCP, enabling declarative access to FHIR resources through JSON configurations for real-time summarization and personalized communication.

Result: The framework supports scalable, explainable, and interoperable AI-powered EHR applications, evaluated using synthetic EHR data from SMART Health IT.

Conclusion: The proposed method provides a robust foundation for advancing personalized digital health solutions, outperforming traditional static workflows.
...
### [360] [Instruction and Solution Probabilities as Heuristics for Inductive Programming](https://arxiv.org/abs/2506.13804)
*Edward McDaid,Sarah McDaid*

Main category: cs.SE

TL;DR: The paper extends instruction subsets (ISs) with instruction and solution probabilities to shrink the inductive programming (IP) search space, achieving reductions of up to 100 orders of magnitude.


<details>
  <summary>Details</summary>
Motivation: To further reduce the IP search space size by incorporating probabilistic heuristics based on instruction frequencies in human code.

Method: Introduces instruction and solution probabilities, using thresholds to prune unlikely branches during search space construction. Evaluated with human code samples.

Result: Both probability variants significantly reduce the IP search space (up to 100 orders of magnitude with IS). Cross-validation confirms effectiveness on unseen code.

Conclusion: The probabilistic approach enhances IS effectiveness, with potential for future refinements.
...
### [361] [Structured Program Synthesis using LLMs: Results and Insights from the IPARC Challenge](https://arxiv.org/abs/2506.13820)
*Shraddha Surana,Ashwin Srinivasan,Michael Bain*

Main category: cs.SE

TL;DR: The paper introduces a structured inductive programming approach using LLMs to solve the IPARC Challenge, revealing insights into human-LLM collaboration for program synthesis.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of automated program synthesis in the IPARC Challenge, which has resisted automated solutions, and explore LLM-based approaches.

Method: A structured inductive programming approach leveraging LLMs, emphasizing prior structuring, human refinement, and code reuse.

Result: Successfully solves tasks across all IPARC categories, highlighting LLMs' role in aiding structuring and sparking human creativity.

Conclusion: The findings suggest effective mechanisms for human-LLM collaboration in complex program synthesis tasks.
...
### [362] [MLDebugging: Towards Benchmarking Code Debugging Across Multi-Library Scenarios](https://arxiv.org/abs/2506.13824)
*Jinyang Huang,Xiachong Feng,Qiguang Chen,Hanjie Zhao,Zihui Cheng,Jiesong Bai,Jingxuan Zhou,Min Li,Libo Qin*

Main category: cs.SE

TL;DR: The paper introduces MLDebugging, a benchmark for evaluating multi-library Python code debugging, highlighting current LLMs' limitations in this complex scenario.


<details>
  <summary>Details</summary>
Motivation: Current research on code debugging with LLMs overlooks the complexity of multi-library scenarios, which are common in real-world applications.

Method: The authors create MLDebugging, a benchmark with 126 Python libraries and seven issue types, and evaluate it using open-source and closed-source LLMs.

Result: Current LLMs struggle with debugging in multi-library settings, as shown by the MLDebugging evaluation.

Conclusion: The work aims to inspire future research on LLMs' potential in multi-library debugging.
...
### [363] [CRITICTOOL: Evaluating Self-Critique Capabilities of Large Language Models in Tool-Calling Error Scenarios](https://arxiv.org/abs/2506.13977)
*Shiting Huang,Zhen Fang,Zehui Chen,Siyu Yuan,Junjie Ye,Yu Zeng,Lin Chen,Qi Mao,Feng Zhao*

Main category: cs.SE

TL;DR: The paper introduces CRITICTOOL, a benchmark for evaluating error handling in tool learning for LLMs, using an evolutionary strategy for dataset construction and validating its effectiveness.


<details>
  <summary>Details</summary>
Motivation: As LLMs tackle more complex tasks, error handling in tool utilization becomes critical, necessitating a specialized benchmark for evaluation.

Method: The authors analyze tool-use errors, develop CRITICTOOL using an evolutionary dataset strategy, and test it on various LLMs.

Result: CRITICTOOL effectively evaluates error handling and tool reflection in LLMs, showing generalization across scenarios.

Conclusion: CRITICTOOL advances tool learning by providing a robust benchmark for error analysis and LLM evaluation.
...
### [364] [FrontendBench: A Benchmark for Evaluating LLMs on Front-End Development via Automatic Evaluation](https://arxiv.org/abs/2506.13832)
*Hongda Zhu,Yiwen Zhang,Bing Zhao,Jingzhe Ding,Siyao Liu,Tong Liu,Dandan Wang,Yanan Liu,Zhaojian Li*

Main category: cs.SE

TL;DR: FrontendBench is a new benchmark for evaluating LLMs in front-end code generation, addressing limitations of existing benchmarks with realistic tasks and interactive test scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for LLMs in front-end code generation are simplistic, lack rigorous test cases, and miss end-to-end validation, hindering accurate performance assessment.

Method: FrontendBench includes 148 prompt-test case pairs across five web component levels, with an automatic evaluation framework using sandbox execution and test scripts.

Result: The framework achieves 90.54% agreement with human evaluations, revealing performance disparities among state-of-the-art LLMs in real-world tasks.

Conclusion: FrontendBench is a reliable, scalable benchmark for consistent multimodal evaluation, advancing future research in front-end code generation.
...
### [365] [How Does LLM Reasoning Work for Code? A Survey and a Call to Action](https://arxiv.org/abs/2506.13932)
*Ira Ceka,Saurabh Pujar,Irene Manotas,Gail Kaiser,Baishakhi Ray,Shyam Ramji*

Main category: cs.SE

TL;DR: The paper surveys code reasoning techniques in LLMs for software engineering tasks, introducing a taxonomy, benchmarks, and future research gaps.


<details>
  <summary>Details</summary>
Motivation: To understand and improve LLMs' code reasoning abilities for real-world software engineering tasks like GitHub issue resolution.

Method: Conducts a dedicated survey, develops a taxonomy, evaluates benchmarks, and explores core code properties.

Result: Highlights strategies, hybrid approaches, and new benchmarks, while identifying gaps in current research.

Conclusion: The study provides a foundation for advancing code reasoning in LLMs, pointing to under-explored areas for future work.
...
### [366] [Automatic Qiskit Code Refactoring Using Large Language Models](https://arxiv.org/abs/2506.14535)
*José Manuel Suárez,Luis Mariano Bibbó,Joaquin Bogado,Alejandro Fernandez*

Main category: cs.SE

TL;DR: A novel method using LLMs to refactor Qiskit code by leveraging a taxonomy of migration scenarios from documentation, addressing API compatibility challenges.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of quantum software frameworks like Qiskit creates compatibility issues, necessitating automated solutions for code migration.

Method: Extract a taxonomy of migration scenarios from Qiskit docs, use it alongside source code as input to an LLM for identifying and refactoring migration instances.

Result: LLMs, guided by domain-specific knowledge, effectively automate Qiskit code migration, with proven prompts and taxonomy provided.

Conclusion: The approach offers a scalable solution for quantum code migration and a methodology to evaluate LLMs' role in such tasks.
...
### [367] [Low-code to fight climate change: the Climaborough project](https://arxiv.org/abs/2506.14623)
*Aaron Conrardy,Armen Sulejmani,Cindy Guerlain,Daniele Pagani,David Hick,Matteo Satta,Jordi Cabot*

Main category: cs.SE

TL;DR: The Climaborough project supports EU cities in achieving carbon neutrality by 2030 using a low-code/no-code platform for climate dashboards.


<details>
  <summary>Details</summary>
Motivation: To help cities monitor and achieve climate goals by deploying user-friendly dashboards for non-technical users.

Method: Development of the Climaborough City Platform with low-code/no-code strategies for quick deployment and customization.

Result: Dashboards enable cities to evaluate initiatives, identify impactful ones, and assess scalability.

Conclusion: The low-code/no-code approach effectively accelerates dashboard deployment and empowers citizens to adapt tools for local needs.
...
### [368] [ACM Survey Draft on Formalising Software Requirements with Large Language Models](https://arxiv.org/abs/2506.14627)
*Arshad Beg,Diarmuid O'Donoghue,Rosemary Monahan*

Main category: cs.SE

TL;DR: This draft summarizes 94 papers and includes sections on traceability, formal methods, UTP, and Theory of Institutions. It differs from [7] (a short conference poster) and [8] (a detailed, rigorously reviewed paper).


<details>
  <summary>Details</summary>
Motivation: To consolidate and summarize existing research on software requirements, formal methods, and related theories, while distinguishing this work from prior submissions.

Method: Summarizes 94 papers and includes dedicated sections on key topics like traceability and formal methods.

Result: A comprehensive draft highlighting differences from related works [7] and [8].

Conclusion: This draft serves as a broader summary and reference, contrasting with the narrower focus of [7] and [8].
...
### [369] [Navigating the growing field of research on AI for software testing -- the taxonomy for AI-augmented software testing and an ontology-driven literature survey](https://arxiv.org/abs/2506.14640)
*Ina K. Schieferdecker*

Main category: cs.SE

TL;DR: The paper reviews AI's role in enhancing software test automation, introduces a taxonomy (ai4st), and identifies open research questions.


<details>
  <summary>Details</summary>
Motivation: To explore how AI can augment software testing, addressing challenges in test automation design, development, and maintenance.

Method: Review of recent research on AI in software test automation, classification using the ai4st taxonomy.

Result: AI offers new perspectives for testing, from no automation to full automation, and enables novel testing forms.

Conclusion: The ai4st taxonomy helps classify research and highlights open questions in AI-augmented software testing.
...
### [370] [Unified Software Engineering agent as AI Software Engineer](https://arxiv.org/abs/2506.14683)
*Leonhard Applis,Yuntong Zhang,Shanchao Liang,Nan Jiang,Lin Tan,Abhik Roychoudhury*

Main category: cs.SE

TL;DR: The paper introduces USEagent, a unified LLM-based agent for diverse software engineering tasks, outperforming existing general agents but showing gaps in coding tasks.


<details>
  <summary>Details</summary>
Motivation: To explore whether LLM agents can function as AI software engineers by handling multiple software engineering tasks beyond coding.

Method: Developed USEagent, a unified agent for orchestrating multiple software engineering capabilities, and evaluated it using USEbench, a benchmark with 1,271 tasks.

Result: USEagent outperforms general agents like CodeActAgent but has gaps in coding tasks.

Conclusion: USEagent is a step toward AI Software Engineers but requires further development to bridge capability gaps.
...
### [371] [Investigating the Potential of Large Language Model-Based Router Multi-Agent Architectures for Foundation Design Automation: A Task Classification and Expert Selection Study](https://arxiv.org/abs/2506.13811)
*Sompote Youwai,David Phim,Vianne Gayl Murcia,Rianne Clair Onas*

Main category: cs.MA

TL;DR: Router-based multi-agent systems outperform single-agent and multi-agent workflows in foundation design automation, achieving 95% and 90.63% accuracy for shallow and pile foundations, respectively.


<details>
  <summary>Details</summary>
Motivation: To automate foundation design calculations efficiently while maintaining professional standards, addressing the limitations of standalone and conventional multi-agent systems.
...
Result: Router-based systems improved performance by 8.75-43.75 percentage points over alternatives, with Grok 3 excelling in standalone mathematical reasoning.

Conclusion: Router-based multi-agent systems are optimal for foundation design automation but require human oversight due to safety-critical engineering requirements.
