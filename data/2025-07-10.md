...
Method: The method leverages CLIP for visual semantic feature extraction to evaluate image quality, integrates it as a perception loss module, and combines it with curriculum contrastive regularization for balanced enhancement.

Result: The proposed method outperforms state-of-the-art techniques in visual quality and generalization, as shown in extensive experiments.

Conclusion: The CLIP-based approach effectively enhances underwater images by aligning with human perception and mitigating under- or over-enhancement issues.
...
<details>
  <summary>Details</summary>
Motivation: Existing interpretability methods create isolated concept spaces, limiting cross-model comparisons.

Method: SPARC uses Global TopK sparsity and Cross-Reconstruction Loss to align latent spaces across models.

Result: Achieves 0.80 Jaccard similarity on Open Images, tripling alignment compared to prior methods.

Conclusion: SPARC enables direct comparison of concepts across models and supports applications like cross-modal retrieval.
...
<details>
  <summary>Details</summary>
Motivation: The need for precise surface geometry and uncertainty awareness in scientific simulations, such as fluid flow through forests, drives the development of BayesSDF.
...
Conclusion: BayesSDF establishes a robust foundation for uncertainty-aware 3D scene reconstruction and simulation.
...
<details>
  <summary>Details</summary>
Motivation: LMMs struggle with weak visual comprehension and lack of fine-grained perception, leading to segmentation inaccuracies and hallucinations.

Method: LIRA uses SEFE for better segmentation and ILVC for fine-grained supervision, leveraging the relationship between segmentation and comprehension.

Result: LIRA outperforms existing models in segmentation and comprehension tasks, validated by the AttrEval dataset.

Conclusion: LIRA effectively mitigates LMM limitations, demonstrating improved accuracy and reduced hallucinations.
...
<details>
  <summary>Details</summary>
Motivation: The limited availability of annotated training data for HTR systems, especially for low-resource languages and complex scripts, hinders performance. This survey aims to explore solutions through data augmentation and generation.

Method: The survey follows PRISMA methodology, analyzing 1,302 studies (filtered to 848) from IEEE, Springer, Science Direct, and ACM. It evaluates datasets, metrics, and methodologies.

Result: Identifies key research gaps in generating diverse, realistic handwriting samples and preserving script authenticity.
...
<details>
  <summary>Details</summary>
Motivation: The scarcity of labeled datasets for wildland fire segmentation makes training models costly and challenging.
...
Result: CCPDA improves fire-class segmentation metrics more effectively than other augmentation methods, validated via numerical analysis.

Conclusion: CCPDA effectively addresses the challenges of small labeled datasets and enhances fire-class segmentation performance.
...
<details>
  <summary>Details</summary>
Motivation: Deep neural networks perform poorly under common corruptions, limiting real-world reliability.

Method: AR2 aligns CAMs between clean and corrupted images via iterative repair and fine-tuning.

Result: AR2 outperforms existing methods on CIFAR-10-C, CIFAR-100-C, and ImageNet-C benchmarks.

Conclusion: AR2 offers a scalable solution to enhance model robustness in real-world scenarios.
...
Method: SU-T combines an Unscented Kalman Filter for non-linear fish motion and a novel FishIoU matching method.

Result: SU-T achieves 34.1 HOTA and 44.6 IDF1 on MFT25, highlighting differences from terrestrial tracking.
...
TL;DR: SImpHAR introduces a simulation pipeline and two-stage training for bio-impedance-based HAR, achieving significant accuracy improvements.


<details>
  <summary>Details</summary>
Motivation: Bio-impedance sensing is underutilized in HAR due to lack of labeled data.

Method: Uses simulation (3D meshes, physics, text-to-motion) and decoupled two-stage training.

Result: Improves accuracy by 22.3% and macro F1 by 21.8% on benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: The success of transformers and multi-stage architectures in video recognition and object detection inspired their application to TAL, leveraging spatio-temporal properties for better performance.

Method: PCL-Former employs three dedicated transformer modules: Proposal-Former for segment identification, Classification-Former for action categorization, and Localization-Former for precise boundary prediction, each with specialized loss functions.
...
Conclusion: The hierarchical multi-stage transformer architecture effectively addresses TAL, demonstrating significant performance improvements on challenging benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Wearable cameras face challenges like high power consumption, excessive data generation, privacy issues, and computational demands when processing RGB images for hand-related activities.

Method: THOR uses low-resolution thermal data to detect activity transitions, adjusting RGB sampling rates and localizing hand-object interactions for efficient processing.
...
Conclusion: THOR offers a practical solution for real-time, longitudinal monitoring of hand-related activities with wearable cameras, addressing key limitations.
...
Method: The proposed model uses convolutional encoding, adaptive threshold selection, and a lightweight classifier to compress and reconstruct event data efficiently.

Result: The model achieves comparable accuracy to YOLO-v4 with 35.5× fewer parameters and high frame rates (8-44.8 FPS) on embedded platforms.

Conclusion: The approach significantly enhances event-based vision performance, making it suitable for low-power, high-speed real-time edge computing applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing VLA models struggle with novel objects/environments and suffer from high computational overhead when adding generalization techniques.
...
Conclusion: VOTE offers an efficient, general solution for VLA models, balancing performance and speed.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the high cost and scalability issues of traditional RL-based video reasoning methods that rely on extensive supervised fine-tuning and annotations.

Method: Skipping SFT, using pure-RL training with output-based rewards, and introducing a sparse-to-dense video test-time scaling strategy for efficient inference.

Result: Video-RTS achieves 2.4% higher accuracy on average using only 3.6% of training samples, with notable improvements on benchmarks like Video-Holmes (4.2%) and MMVU (2.6%).

Conclusion: Video-RTS demonstrates strong reasoning performance by combining efficient RL training and adaptive scaling, offering a scalable and cost-effective solution.
...
<details>
  <summary>Details</summary>
Motivation: Current pose estimation networks struggle with limited RGB information in cluttered or occluded conditions, lacking discriminative features.

Method: Proposes Mask6D, incorporating 2D-3D correspondence and visible mask maps for pre-training, followed by fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: Existing VLM features are too coarse-grained for HOI detection tasks, limiting generalization.

Method: BC-HOI combines ABG for fine-grained interaction features and LSG for token-level supervision, leveraging VLMs and LLMs.

Result: Achieves superior performance on HICO-DET and V-COCO benchmarks in open and closed settings.

Conclusion: BC-HOI effectively addresses the limitations of coarse VLM features, enhancing HOI detection performance.
...
<details>
  <summary>Details</summary>
Motivation: To improve road safety by leveraging advances in vision-based sensors and computer vision algorithms, the paper aims to systematically organize and analyze traffic scenario elements and datasets.

Method: The authors propose a taxonomy dividing traffic entities into anomalies and critical normal entities, with 10 categories and 20 subclasses. They analyze 35 vision-driven tasks and 73 datasets, providing a cross-domain investigation.

Result: The survey offers a unified framework, highlighting pros and cons of benchmarks, and identifies weaknesses in current research.

Conclusion: The integrated taxonomy and comprehensive analysis serve as a valuable resource for researchers, aiding in resource selection and identifying critical gaps in the field.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the issue of hallucinations in VideoMLLMs, where generated content contradicts visual input, and the lack of comprehensive evaluation methods for open-ended tasks.

Method: Proposes FIFA, which extracts descriptive facts, models their dependencies via a Spatio-Temporal Semantic Dependency Graph, and verifies them using VideoQA models. Introduces Post-Correction for revising hallucinations.

Result: FIFA aligns better with human judgment than existing methods, and Post-Correction improves factual consistency in text and video generation.

Conclusion: FIFA and Post-Correction effectively address hallucinations in VideoMLLMs, enhancing evaluation and generation quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing concept unlearning methods fail to balance unlearning effectiveness with generative retainability in text-to-image diffusion models, posing security risks.

Method: KSCU focuses on key steps in the diffusion process, fine-tuning the model only at these steps to reduce parameter updates and retain generative capabilities.

Result: Benchmark experiments show KSCU effectively prevents undesirable image generation while preserving the model's generative abilities.

Conclusion: KSCU offers a targeted solution for concept unlearning in diffusion models, improving security without compromising performance.
...
<details>
  <summary>Details</summary>
Motivation: To address the overlooked task of translating spoken English into sign language animations, which involves multiple complex steps.

Method: Uses Whisper for speech-to-text, MarianMT for English-to-ASL gloss translation, and a 3D keypoint-based motion system trained on Sign3D-WLASL for animation.

Result: Achieves high BLEU scores (0.7714 and 0.8923) and produces smooth, realistic animations.

Conclusion: The system integrates audio, text, and motion into a unified framework, advancing the field of sign language translation.
...
Conclusion: ILNet's innovative attention and anchor selection enhance trajectory prediction, particularly in challenging multi-agent scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Wildlife data from camera traps is voluminous and costly to label, necessitating efficient data selection for automated monitoring.

Method: Integrates uncertainty and diversity metrics at object and image levels into active learning for sample selection.

Result: State-of-the-art animal detector performance achieved with only 30% of training data.

Conclusion: The approach enables efficient wildlife monitoring by reducing labeling effort without compromising detection accuracy.
...
Result: ToBo outperforms baselines in sequential tasks and shows robustness in real-world robot deployments, with scalability across model scales.

Conclusion: ToBo effectively learns sequential scene representations and temporal dynamics, proving superior in diverse tasks and real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Growing adoption of diffusion models raises copyright and transparency issues, with existing methods failing to attribute influences to specific elements.

Method: Concept-TRAK extends influence functions with a reformulated diffusion training loss and a concept-aware reward function for robust, semantic attribution.
...
Conclusion: Concept-level attribution via Concept-TRAK enhances transparency and governance in generative AI, addressing stakeholder concerns.
...
<details>
  <summary>Details</summary>
Motivation: Prior methods in contrastive learning fail to model the joint structure across all views, focusing only on pairwise relationships.
...
Conclusion: DSF effectively captures joint structure without needing a temperature hyperparameter, unlike cosine similarity.
...
Result: Experiments show EBT loss outperforms WBCE loss quantitatively and perceptually, with minimal fine-tuning required due to robust hyperparameters.

Conclusion: The EBT loss is a superior, easily deployable alternative to WBCE for edge detection, offering improved precision and robustness.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of generating human motion from rare language prompts, where previous methods fail due to coarse-grained matching and overlooked semantic cues caused by motion redundancy.

Method: MOST employs temporal clip Banzhaf interaction for fine-grained text-to-motion clip matching and a motion prompt module to generate semantically consistent movements.

Result: MOST achieves state-of-the-art performance in text-to-motion retrieval and generation, particularly for rare prompts, as shown by quantitative and qualitative evaluations.

Conclusion: MOST effectively addresses previous limitations in motion generation, offering a robust solution for fine-grained text-to-motion alignment and redundancy elimination.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods treat all points equally, ignoring ambiguities in transition regions, leading to sub-optimal models due to unreliable labels for highly ambiguous points.
...
Conclusion: The adaptive methods enhance model training by handling ambiguities, with AMContrast3D++ further refining embeddings for better results.
...
Method: The DCS uses one camera for reference sequences and another for supplementary data. EAFNet includes pre-alignment, asymmetric cross-feature fusion, and reconstruction subnetworks to enhance and fuse features adaptively.

Result: The method outperforms others on various datasets, proving DCS's effectiveness in HDR video reconstruction.

Conclusion: The proposed DCS and EAFNet offer a promising solution for flicker-free HDR video reconstruction, with code and data made publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Long-term action recognition is hindered by temporal complexity and visual confounders, with existing methods lacking cross-modal causal modeling.

Method: CMDCL introduces a structural causal model for video-label text relationships, using textual and visual causal interventions to debias embeddings and remove confounders.

Result: Experiments on Charades, Breakfast, and COIN benchmarks show CMDCL's effectiveness in robust action representation.

Conclusion: CMDCL successfully addresses cross-modal biases and visual confounders, improving long-term action recognition performance.
...
Result: Outperforms state-of-the-art methods with a 5.73% DSC improvement on microscopic hyperspectral datasets.

Conclusion: Omni-Fuse effectively enhances segmentation by efficiently fusing spatial and spectral data, proving its potential in computational pathology.
...
<details>
  <summary>Details</summary>
Motivation: Existing view-independent projection methods are limited by pre-defined parameters, lack projection diversity, and suffer from computational inefficiency due to redundancy.

Method: Introduces VDP framework for data-driven projections, inspired by fireworks' adaptive behavior, and uses color regularization to optimize feature emphasis.

Result: PointVDP achieves competitive results on S3DIS and ScanNet benchmarks with lightweight projections and minimal computational overhead.

Conclusion: VDP offers a resource-efficient solution for semantic understanding in point cloud segmentation.
...
<details>
  <summary>Details</summary>
Motivation: Current patch-level self-supervised learning (SSL) methods in digital pathology overlook domain-specific features and are less data-efficient than supervised approaches.

Method: EXAONE Path 2.0 learns patch-level representations under direct slide-level supervision, using 37k whole-slide images (WSIs) for training.

Result: The model achieves state-of-the-art performance across 10 biomarker prediction tasks.

Conclusion: EXAONE Path 2.0 addresses limitations of SSL by leveraging slide-level supervision, offering superior data efficiency and performance.
...
<details>
  <summary>Details</summary>
Motivation: Sparse labels in medical imaging due to high annotation costs and the need for dense predictions (e.g., keypoint localization) motivate this work, especially for tasks like ovarian cancer diagnosis.

Method: The problem is framed as sparse heatmap regression. A new loss function, Crag and Tail loss, is introduced to leverage sparse annotations while mitigating false negatives.

Result: The approach achieves accurate dense localization of carcinosis keypoints, validated through ablation studies.

Conclusion: The proposed method shows promise for advancing research in scenarios where dense annotations are impractical, offering a practical solution for sparse label learning.
...
<details>
  <summary>Details</summary>
Motivation: Enhancing diagnostic accuracy and surgical planning through high-quality visualization, despite high computing costs and slow rendering speeds.

Method: Introduces ClipGS with a learnable truncation scheme and adaptive adjustment model for dynamic interactions and rendering refinement.

Result: Achieves 36.635 PSNR, 156 FPS, and 16.1 MB model size, outperforming state-of-the-art methods.

Conclusion: ClipGS effectively addresses rendering challenges, offering high-quality, efficient visualization for medical data.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with accurate cross-modal correspondences due to the inherent modality gap between images and point clouds.
...
Result: Achieves over 7% improvement in registration recall on the 7-Scenes benchmark, outperforming SoTA methods.

Conclusion: Diff$^2$I2P effectively bridges the modality gap and enhances cross-modal feature learning for robust I2P registration.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods focus only on appearance diversity, limiting application flexibility. CDR-CA addresses this by refining diversities based on context.
...
Conclusion: CDR-CA and Multi-Source DPPs offer a flexible and effective solution for contextual diversity refinement in retrieval tasks.
...
Method: Proposes SPGD with progressive likelihood warm-up and adaptive directional momentum smoothing to manage gradients.
...
TL;DR: MK-Pose is a multimodal framework for category-level object pose estimation, combining RGB, point clouds, and text, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges like occlusion and generalization in object pose estimation for applications like warehouse automation.

Method: Integrates RGB, point clouds, and text with self-supervised keypoint detection, attention-based queries, soft heatmap matching, and graph-based relational modeling.

Result: Outperforms existing methods in IoU and average precision on CAMERA25, REAL275, and HouseCat6D datasets.

Conclusion: MK-Pose effectively improves pose estimation without shape priors, demonstrating strong cross-dataset capability.
...
<details>
  <summary>Details</summary>
Motivation: The need for efficient compression of large-scale 3D Gaussian models for mobile and edge devices, without retraining or fine-tuning.
...
Conclusion: FlexGaussian is a cost-effective, flexible solution for 3D Gaussian compression, suitable for resource-limited devices.
...
<details>
  <summary>Details</summary>
Motivation: Existing VLMs lack quantity awareness for accurate object counting, prompting the need for enhanced prompts and alignment methods.
...
Result: Demonstrates strong generalizability in zero-shot class-agnostic counting on benchmarks like FSC-147 and ShanghaiTech.

Conclusion: QUANet effectively enhances quantity awareness and counting accuracy in VLMs through innovative prompts and architecture.
...
<details>
  <summary>Details</summary>
Motivation: To improve scene representation for monocular perception systems by leveraging 3D Stixels and achieving efficient, adaptable, and real-time performance.

Method: Uses a lightweight neural network trained on LiDAR-based ground truth to cluster 3D Stixels, enabling high compression and adaptability to point cloud and bird's-eye-view representations.

Result: Achieves real-time computation (10 ms/frame) and competitive performance on the Waymo dataset within a 30-meter range.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of synthesizing dance videos from music by capturing spatial-temporal dependencies in skeleton sequences.

Method: Uses STGM blocks for music-to-skeleton translation and a self-supervised network for skeleton-to-video translation, leveraging a new dataset.

Result: Outperforms existing methods in generating dance videos from music.

Conclusion: STG-Mamba effectively bridges music and dance video synthesis with improved performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D visual grounding struggle with spatial relation understanding in language queries, limiting their accuracy in complex environments.

Method: SpatialReasoner fine-tunes an LLM for spatial reasoning in queries and constructs a hierarchical feature field using visual properties (opacity, color) and CLIP/SAM features.

Result: The framework outperforms baselines, seamlessly integrating with neural representations and enhancing spatial reasoning.

Conclusion: SpatialReasoner advances 3D visual grounding by addressing spatial relation challenges, benefiting applications like robotics and augmented reality.
...
<details>
  <summary>Details</summary>
Motivation: Existing SLT methods struggle with visual-textual disparity; gloss-based approaches help but require annotations, while gloss-free methods need better alignment strategies.

Method: Hierarchical pre-training with pseudo-glosses and contrastive video-language alignment, extracting features at frame, segment, and video levels.

Result: Improved BLEU-4 and ROUGE scores while maintaining efficiency.

Conclusion: The proposed method effectively bridges the gap in gloss-free SLT, enhancing translation quality without annotation burdens.
...
Method: Uses visual adapters and prompt learning with POT and CL to align multiple prompts with local features, capturing subtle abnormalities. CL ensures intra-class cohesion and inter-class separation.
...
Conclusion: The proposed method effectively improves CLIP's adaptability for medical anomaly detection, demonstrating superior performance across various scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Image fusion lacks efficient global feature modeling and struggles with capturing complementary features due to computational costs and absence of ground-truth.

Method: RPFNet uses a dual-branch framework: Residual Prior Module (RPM) for complementary priors and Frequency Domain Fusion Module (FDFM) for efficient global modeling. Cross Promotion Module (CPM) enhances local-global synergy. Training includes auxiliary losses for sensitivity and constraints.

Result: RPFNet effectively integrates features, enhances textures and salient objects, and aids high-level vision tasks.

Conclusion: RPFNet addresses key challenges in image fusion, offering a robust solution for feature integration and performance enhancement.
...
Method: Constructed CHDL dataset and proposed DIFFUMA, a dual-path model combining Mamba for global context and diffusion for fine details.

Result: DIFFUMA reduced MSE by 39% and improved SSIM to 0.988 on CHDL, also generalizing to natural datasets.
...
<details>
  <summary>Details</summary>
Motivation: Fixed-frequency reuse in video generation degrades quality in complex scenes, and manual tuning is inefficient.

Method: Proposes PCA caching (adjusts reuse thresholds via prompt complexity) and DynCFGCache (dynamic CFG reuse). Also enhances input-output modeling with feature expansion.

Result: Achieves 2.79x speedup on Wan2.1 model while maintaining visual fidelity.

Conclusion: Adaptive caching and dynamic CFG reuse effectively balance speed and quality in video generation.
...
Method: Introduces a local-and-global dual-granularity identity association mechanism: local cross-modal identity relationships and a global dynamic cross-modal network with confidence-based adjustment. Also uses asymmetric sample pairs and consistency learning.

Result: Significantly improves cross-modal matching accuracy, addressing weakly associated samples and enhancing robustness.

Conclusion: The method provides an efficient solution for text-to-person image matching by improving identity association and sample handling.
...
Conclusion: The approach provides a cost-effective, transferable framework for endangered language OCR, enabling broader access to historical archives without specialized resources.
...
Method: Introduces FOLC-Net with MRFO for model structure, global model cloning for scalability, and ConvNeXt for adaptability.

Result: Achieves 92.44% accuracy in sagittal view, surpassing existing models (88.37% and 88.95%).

Conclusion: FOLC-Net provides a robust, adaptable solution for decentralized medical image analysis.
...
<details>
  <summary>Details</summary>
Motivation: Conventional field sampling is insufficient for studying plant traits at ecologically meaningful scales. Machine learning with hyperspectral data offers a solution, but challenges like label scarcity and domain shifts exist.

Method: The paper introduces GreenHyperSpectra, a dataset for benchmarking trait prediction with semi- and self-supervised methods. It uses an evaluation framework for in-distribution and out-of-distribution scenarios.

Result: Pretrained models using GreenHyperSpectra outperform state-of-the-art supervised baselines, improving spectral representation learning for trait prediction.

Conclusion: GreenHyperSpectra provides a robust framework for advancing research in representation learning and plant trait assessment, with all code and data publicly available.
...
<details>
  <summary>Details</summary>
Motivation: The task is challenging due to audio-visual mapping complexity, lack of large datasets, and high computational costs.

Method: Uses 2D skeletons and a diffusion model conditioned on audio segments and reference images to predict motions, then synthesizes videos with an off-the-shelf model.

Result: Exceeds state-of-the-art in quality and synchronization, generalizing across speakers and contexts.
...
TL;DR: The paper introduces a new color space (HVI) and a network (HVI-CIDNet+) for low-light image enhancement, addressing color bias and noise artifacts, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing LLIE methods in sRGB or HSV color spaces suffer from color bias, brightness artifacts, or noise, necessitating a better solution.

Method: Proposes the HVI color space to mitigate noise artifacts and the HVI-CIDNet+ network, leveraging pre-trained vision-language models and a Prior-guided Attention Block for content restoration and color correction.
...
Conclusion: The HVI color space and HVI-CIDNet+ effectively address limitations of existing methods, providing enhanced low-light image restoration with reduced artifacts.
...
TL;DR: A novel framework combines symbolic regression and trajectory-guided I2V models to improve physical alignment in video generation, outperforming baselines in Classical Mechanics scenarios.
...
Method: Extracts motion trajectories, uses retrieval-based pre-training for symbolic regression, and guides video generation with discovered motion equations.

Result: Recovers ground-truth equations and enhances physical alignment in generated videos for Classical Mechanics scenarios.

Conclusion: The framework successfully integrates physics into video generation without fine-tuning existing models.
...
TL;DR: An end-to-end method using Vision Transformer (ViT) attention maps for Weakly Supervised Semantic Segmentation (WSSS), outperforming traditional approaches.
...
Method: Train a sparse ViT with multiple [CLS] tokens (one per class) using random masking. Aggregate self-attention maps of [CLS] tokens at inference to generate pseudo segmentation masks.

Result: Outperforms related works on standard benchmarks and specialized datasets, generating accurate pseudo-masks.

Conclusion: The method reduces the need for fine-grained labeled data, achieving results comparable to fully-supervised models.
...
TL;DR: IAP is a novel adversarial patch attack framework that improves patch invisibility and attack success by optimizing patch location and perturbations while maintaining stealth against defenses.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial patch methods either fail in targeted attacks or lack stealth due to noticeable patches. IAP aims to address these limitations.

Method: IAP uses perceptibility-aware localization (classwise and sensitivity maps) and perturbation optimization (perceptibility-regularized loss, color constancy) to generate stealthy patches.

Result: IAP achieves high attack success rates with improved invisibility across benchmarks and evades state-of-the-art patch defenses.

Conclusion: IAP advances adversarial patch attacks by balancing effectiveness and stealth, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: To understand the stability and variability of biometric characteristics over time in diverse populations.

Method: Conducted evaluations over 2.5 years with 400+ participants using GDPR-compliant data and state-of-the-art face recognition algorithms.
...
TL;DR: SemRaFiner improves panoptic segmentation in sparse radar point clouds for autonomous driving by optimizing feature extraction and training procedures.


<details>
  <summary>Details</summary>
Motivation: Enhancing semantic scene understanding in adverse weather where cameras and LiDARs fail, leveraging radar's Doppler velocity for motion data.
...
Result: Outperforms state-of-the-art methods in radar-based panoptic segmentation.

Conclusion: SemRaFiner advances radar-based scene understanding, offering robust performance in challenging conditions.
...
TL;DR: The paper introduces the Deliberate-to-Intuitive (D2I) framework to enhance multimodal LLMs' reasoning without extra annotations or complex rewards, achieving superior performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Addressing the high training costs and scalability issues in multimodal reasoning research due to reliance on additional annotations and rule-based rewards.
...
Result: D2I outperforms baselines on in-domain and out-of-domain benchmarks, demonstrating improved reasoning and modality alignment.

Conclusion: The framework highlights the effectiveness of format rewards in transferable reasoning skills and suggests decoupling training-time reasoning depth from test-time flexibility.
...
TL;DR: APL introduces adaptive part discovery and learning to improve GCD by leveraging DINO part priors and a novel all-min contrastive loss for better discriminability and generalization.
...
Method: APL uses shared learnable part queries and DINO part priors to generate consistent object parts. It employs an all-min contrastive loss for part representation learning.
...
Conclusion: APL effectively balances discriminability and generalization in GCD, offering a scalable solution without additional annotations.
...
<details>
  <summary>Details</summary>
Motivation: Existing calligraphy datasets lack attribute information, hindering in-depth research. The paper aims to fill this gap with MCCD.

Method: MCCD includes 7,765 categories and 329,715 calligraphy character images, annotated with script styles, dynasties, and calligraphers. Benchmark experiments were conducted.

Result: Recognition is challenging due to stroke complexity and attribute interplay. MCCD provides a valuable resource for calligraphy research.

Conclusion: MCCD fills a critical gap in calligraphy datasets and supports advancements in multiple research fields.
...
Method: Combines deep learning (for palm tree identification) and clustering algorithms (for palm clusters) applied to high-resolution satellite imagery.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges like irrelevant training samples, imbalanced data, and high expert costs in mitigating hallucinations in VLMs for medical use.

Method: Uses confidence-similarity joint mining and counterfactual rationale, synthesizes a multi-task dataset, identifies hard examples, and balances preference distributions.

Result: Achieves 8.93% relative performance gain with only 5% of SFT samples, reaching state-of-the-art performance.

Conclusion: CheXPO provides a scalable, interpretable solution for radiology applications, reducing reliance on expert annotations.
...
TL;DR: ProRSeg, a deep learning DIR method, demonstrated reasonable multi-domain MR-MR registration for prostate cancer, showing feasibility for dose accumulation and contour propagation.
...
Method: ProRSeg was trained with 262 pairs of 3T MR scans using weighted segmentation consistency loss and tested on same-, cross-, and mixed-domain datasets.

Result: ProRSeg showed generalization for bladder (DSCs 0.88, 0.87, 0.86) and domain-dependent performance for rectum and CTV. Dose accumulation met constraints for 83.3% of patients.

Conclusion: ProRSeg is feasible for multi-domain MR-MR registration and preliminary evaluation of treatment compliance.
...
TL;DR: Percep360 is a novel panoramic generation method for autonomous driving, focusing on coherence and controllability using LSDM and PPM, outperforming traditional stitched images in quality and utility.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving requires comprehensive 360° views, but current data acquisition and generation methods are limited in quality and controllability.

Method: Proposes Local Scenes Diffusion Method (LSDM) for coherence and Probabilistic Prompting Method (PPM) for controllability in panoramic generation.

Result: Generated images outperform stitched ones in quality metrics and enhance downstream tasks like BEV segmentation.

Conclusion: Percep360 advances panoramic data generation for autonomous driving, offering high-quality, controllable outputs with practical benefits.
...
TL;DR: The MassID45 dataset combines DNA barcoding and high-resolution imaging for automatic classification of bulk insect samples, aiding large-scale ecological studies.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current image-based approaches for insect classification, which rely on individual specimens rather than unsorted bulk samples.

Method: Human annotators, aided by AI, segmented and labeled over 17,000 specimens in bulk images, combining molecular and imaging data.

Result: The dataset enables precise abundance estimates and taxonomic resolution, advancing tiny object detection and instance segmentation.

Conclusion: MassID45 supports rapid, large-scale insect community characterization, benefiting ecological and machine learning research.
...
TL;DR: FreeTTA is a training-free test-time adaptation method for Vision-Language Models (VLMs) that enhances predictions by modeling test data distribution without relying on costly training or unrealistic assumptions.


<details>
  <summary>Details</summary>
Motivation: Address the limitations of VLMs in practical applications due to domain shifts and distributional changes, and the inefficiencies of traditional TTA methods.

Method: Proposes FreeTTA, which uses an online EM algorithm to iteratively update parameters and posterior probabilities of test samples, leveraging zero-shot predictions from VLMs.

Result: FreeTTA outperforms state-of-the-art methods across 15 datasets in cross-domain and out-of-distribution settings.

Conclusion: FreeTTA offers a flexible, efficient, and assumption-free solution for test-time adaptation, improving VLM performance in real-world scenarios.
...
### [71] [DenoiseCP-Net: Efficient Collective Perception in Adverse Weather via Joint LiDAR-Based 3D Object Detection and Denoising](https://arxiv.org/abs/2507.06976)
*Sven Teufel,Dominique Mayer,Jörg Gamerdinger,Oliver Bringmann*

Main category: cs.CV

TL;DR: A novel multi-task architecture, DenoiseCP-Net, is proposed for LiDAR-based collective perception in adverse weather, reducing bandwidth and latency while maintaining detection accuracy.
...
Method: DenoiseCP-Net integrates voxel-level noise filtering and object detection into a unified sparse convolution backbone, avoiding redundant computations.

Result: DenoiseCP-Net achieves near-perfect denoising, reduces bandwidth by 23.6%, and lowers inference latency while maintaining detection accuracy.
...
### [72] [MCA-RG: Enhancing LLMs with Medical Concept Alignment for Radiology Report Generation](https://arxiv.org/abs/2507.06992)
...
Method: MCA-RG uses pathology and anatomy concept banks, aligns visual features with these concepts, and employs contrastive learning, matching loss, and feature gating.

Result: MCA-RG outperforms benchmarks (MIMIC-CXR, CheXpert Plus), demonstrating superior radiology report generation.

Conclusion: MCA-RG effectively enhances RRG by aligning visual features with medical concepts, addressing key clinical adoption challenges.
...
TL;DR: A novel multi-modal feature fusion framework using 3D CT images and clinical data improves NSCLC survival prediction, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Accurate prognosis for NSCLC patients undergoing immunotherapy is crucial for personalized treatment, but lacks large datasets and effective multi-modal fusion strategies.

Method: Proposes a cross-modality masked learning approach with a Slice-Depth Transformer for CT images and a graph-based Transformer for clinical data, enhancing feature fusion.
...
Conclusion: The approach effectively addresses challenges in NSCLC prognosis, improving accuracy and inter-modality feature integration.
...
TL;DR: GNN-ViTCap is a novel framework for classifying and captioning histopathological images, addressing challenges like redundant patches and unknown positions, achieving high performance metrics.


<details>
  <summary>Details</summary>
Motivation: Accurate cancer diagnosis via microscopy is hindered by redundant patches and lack of automatic captioning, necessitating an efficient solution.

Method: Uses visual feature extraction, dynamic clustering, graph neural networks, and language model fine-tuning to classify and caption WSIs.

Result: Achieves F1 score of 0.934, AUC of 0.963 for classification, and BLEU-4 of 0.811, METEOR of 0.569 for captioning.

Conclusion: GNN-ViTCap outperforms existing methods, providing a reliable solution for microscopy-based diagnosis.
...
Result: Accurate prediction of cell-type compositions with reduced computational complexity, outperforming Hist2Cell.
...
TL;DR: MST-Distill is a novel cross-modal knowledge distillation framework using a mixture of specialized teachers and adaptive routing to address challenges like distillation path selection and knowledge drift.
...
Method: Proposes MST-Distill with diverse teacher ensembles, an instance-level routing network, and a plug-in masking module to suppress modality-specific discrepancies.

Result: Outperforms state-of-the-art methods on five multimodal datasets (visual, audio, text).

Conclusion: MST-Distill effectively addresses limitations of traditional methods, enhancing cross-modal knowledge transfer.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of extracting structured tabular data from noisy, non-standard invoice formats efficiently.

Method: Combines Tesseract OCR with dynamic preprocessing, table boundary detection, and row-column mapping.
...
TL;DR: The paper explores how contextual metadata (location, meal type, and food items) enhances Large Multimodal Models (LMMs) for nutrition analysis, introduces the ACETADA dataset, and shows improved accuracy with metadata integration and reasoning modifiers.
...
Method: The study evaluates eight LMMs (four open-weight, four closed-weight) using the ACETADA dataset, integrating contextual metadata (GPS, timestamps, food items) and testing reasoning modifiers like Chain-of-Thought and Few-Shot.

Result: Metadata integration reduces Mean Absolute Error (MAE) and Mean Absolute Percentage Error (MAPE) in nutritional predictions. Context-aware LMMs show significant performance improvements.

Conclusion: Contextual metadata and reasoning modifiers enhance LMM performance in nutrition analysis, demonstrating the potential of context-aware models for accurate dietary assessment.
...
TL;DR: A geometric deep learning framework predicts the Laplace-Beltrami (LB) spectrum from CAD meshes, reducing computation time by 5x compared to FEM without losing accuracy.


<details>
  <summary>Details</summary>
Motivation: The FEM method for LB spectrum estimation is inefficient for large CAD databases or frequent quality control tasks.

Method: A Graph Neural Network uses mesh features (e.g., curvatures) to predict the LB spectrum.

Result: The method achieves ~5x faster computation than FEM with competitive accuracy.

Conclusion: The LB spectrum is learnable, and the framework offers efficient, accurate predictions for practical applications.
...
TL;DR: RulerNet is a deep learning framework for robustly inferring real-world scale from images by reformulating ruler reading as a keypoint-detection problem, using geometric-progression parameters and synthetic data for training.


<details>
  <summary>Details</summary>
Motivation: Accurate pixel-to-real-world dimension conversion is crucial for applications like biomedicine, forensics, and e-commerce, but remains a challenge in computer vision.

Method: RulerNet localizes centimeter marks with a distortion-invariant strategy and uses synthetic data for training. It includes DeepGP for real-time parameter regression.

Result: RulerNet provides accurate, consistent, and efficient scale estimates under diverse real-world conditions.
...
TL;DR: The paper introduces L-VQAScore, a novel metric for evaluating Text-to-Image (T2I) models, addressing limitations in assessing entity-attribute associations by combining visual localization and VQA probing.


<details>
  <summary>Details</summary>
Motivation: Current T2I evaluation methods struggle with attribute confusion, where attributes are correctly depicted but misassociated with entities. This limits their effectiveness in domains like fashion.
...
Conclusion: L-VQAScore is presented as a reliable, scalable alternative to subjective evaluations for T2I models, particularly in complex domains.
...
TL;DR: The paper introduces the VLV auto-encoder framework, which efficiently builds high-performance VLMs by leveraging pretrained models, reducing data and cost requirements.


<details>
  <summary>Details</summary>
Motivation: To address the high computational and data demands of training VLMs, the paper proposes a cost-effective alternative using existing pretrained models.

Method: The VLV framework uses a vision encoder, T2I diffusion decoder, and LLM, regularizing language representation via frozen T2I diffusion and fine-tuning the LLM for captioning.

Result: The method achieves SoTA captioning performance comparable to GPT-4o and Gemini 2.0 Flash, with training costs under $1,000 USD.

Conclusion: The VLV framework offers a scalable, cost-efficient solution for building high-quality VLMs without massive paired datasets.
...
TL;DR: 4KAgent is a unified system for upscaling any image to 4K resolution, handling severe degradations and enhancing facial details, outperforming benchmarks across diverse imaging domains.


<details>
  <summary>Details</summary>
Motivation: To create a universal, high-quality image upscaling system capable of handling severely degraded inputs and diverse imaging tasks, fostering innovation in vision-centric autonomous agents.

Method: Combines Profiling, Perception Agent (vision-language models and quality assessment), and Restoration Agent (recursive execution-reflection with quality-driven expert policy), including a specialized face restoration pipeline.

Result: Achieves state-of-the-art performance across 11 task categories and 26 benchmarks, excelling in perceptual and fidelity metrics.

Conclusion: 4KAgent introduces a novel agentic paradigm for low-level vision tasks, promising broader innovation in autonomous vision agents.
...
TL;DR: The paper explores using text-to-image diffusion models as visual encoders for MLLMs, addressing CLIP's limitations in fine-grained detail capture. It identifies diffusion features' semantic richness and alignment, proposes mitigation for LLM leakage, and demonstrates improved performance in VQA and MLLM benchmarks.


<details>
  <summary>Details</summary>
Motivation: CLIP's inability to capture fine-grained details in visual encoding for MLLMs motivates the exploration of diffusion models as alternative encoders.
...
Conclusion: Diffusion models can effectively serve as instruction-aware visual encoders, enhancing MLLMs' capabilities in fine-grained visual tasks.
...
TL;DR: MEWI is a 3D multiplayer simulation for medical evacuation training, improving learning and decision-making in classroom settings.


<details>
  <summary>Details</summary>
Motivation: Lack of tools to simulate and evaluate medical evacuation networks in training.

Method: Developed MEWI in Unity, modeled patient interactions, and tested in operational scenarios.

Result: MEWI improves lesson uptake and decision-making, validated by student surveys and observer notes.

Conclusion: MEWI advances high-fidelity medical training and offers insights for improving evacuation education.
...
TL;DR: PDL (Prompt Declaration Language) simplifies prompt engineering for LLMs by enabling flexible tuning and composition, improving productivity and performance.
...
Method: Introduces PDL, a declarative language for prompt representation, supporting manual/automatic tuning and integration with rule-based code and tools.

Result: PDL demonstrated a 4x performance improvement in a compliance agent case study compared to canned solutions.
...
TL;DR: The paper explores the Jolting Technologies Hypothesis, suggesting superexponential growth in AI capabilities, and develops tools for future empirical validation.
...
Method: Develops a theoretical framework and validates detection methods via Monte Carlo simulations, focusing on factors like idea-to-action intervals and iterative AI improvements.

Result: Provides mathematical foundations for detecting and analyzing jolt dynamics in AI development, though empirical validation is pending.

Conclusion: The study offers insights and tools for future research and policy on AI trajectories and AGI emergence.
...
TL;DR: Q-dialectical systems are proven strictly more powerful than p-dialectical systems, which are stronger than d-dialectical systems, emphasizing the roles of counterexample and contradiction in belief revision.


<details>
  <summary>Details</summary>
Motivation: To model how agents (mathematicians, research communities, or automated systems) refine beliefs for consistency and truth.

Method: Analyzing and comparing three models of dialectical systems (d-, p-, and q-dialectical) to determine their relative power in belief revision.
...
Conclusion: The study underscores the complementary roles of counterexample and contradiction in belief revision, applicable to both automated agents and human reasoning.
...
TL;DR: The paper extends SCC-recursive semantics to infinite argumentation frameworks (AFs), addressing well-foundedness issues. It evaluates two proposed semantics against Baroni and Giacomin's criteria and examines their behavior in finitary frameworks.


<details>
  <summary>Details</summary>
Motivation: SCC-recursive semantics, effective for finite AFs, fail in infinite settings due to well-foundedness problems. The paper aims to generalize these semantics to infinite AFs.

Method: Proposes two approaches for extending SCC-recursiveness to infinite AFs and evaluates them using Baroni and Giacomin's criteria. Also examines their behavior in finitary frameworks.
...
TL;DR: The paper proposes a systematic framework for constructing high-quality instruction datasets to improve model performance, addressing limitations in coverage and depth of current datasets.
...
Method: A hierarchical labeling system, seed selection algorithm, evolutionary data synthesis, and deficiency diagnosis with targeted data generation form an iterative framework.
...
Result: The system shows significant improvements in query interpretation, navigation accuracy, and resilience to disruptions.
...
TL;DR: FR3E is a structured exploration framework for RLVR that improves LLM reasoning by targeting high-uncertainty points and providing grounded feedback, leading to more stable training and better results.
...
Method: FR3E identifies high-uncertainty decision points and performs targeted rollouts for semantically grounded feedback.

Result: FR3E enhances training stability, response coherence, and correctness on mathematical reasoning benchmarks.

Conclusion: FR3E effectively improves LLM reasoning through robust and structured exploration.
...
TL;DR: The paper introduces the Gemini 2.X model family, including Gemini 2.5 Pro and 2.5 Flash, highlighting their advanced capabilities in coding, reasoning, and multimodal understanding, as well as cost-efficiency.


<details>
  <summary>Details</summary>
Motivation: To present a new generation of models that push the boundaries of agentic workflows by combining long context, multimodal understanding, and reasoning, while optimizing for cost and performance.

Method: Developed Gemini 2.5 Pro and 2.5 Flash models, with the former excelling in coding, reasoning, and multimodal tasks, and the latter offering efficient reasoning at lower compute costs.

Result: Gemini 2.5 Pro achieves state-of-the-art performance in coding and reasoning benchmarks and processes up to 3 hours of video. Gemini 2.5 Flash provides high performance with reduced compute and latency.

Conclusion: The Gemini 2.X models cover a wide range of capabilities and cost efficiencies, enabling advanced agentic problem-solving workflows.
...
TL;DR: LLMs exhibit overconfidence across languages, with varying reliance behaviors, highlighting risks and the need for culturally aware safety evaluations.


<details>
  <summary>Details</summary>
Motivation: To assess the risks of multilingual linguistic miscalibration, overconfidence, and overreliance in LLMs for global deployment.

Method: Analyzed epistemic markers in LLM outputs across five languages and measured human reliance rates.

Result: LLMs are overconfident across languages but sensitive to linguistic variation; human reliance differs by language, e.g., higher on uncertainty in Japanese.

Conclusion: Multilingual linguistic calibration is challenging; culturally contextualized safety evaluations are crucial.
...
<details>
  <summary>Details</summary>
Motivation: Quadratic computation/memory overhead in Transformer-based LLMs limits long-sequence processing.

Method: ETT fine-tunes model parameters on overlapping subsequences of input context at test-time.

Result: Extends context length up to 32x (1k to 32k tokens) with 30% accuracy improvement.

Conclusion: Fine-tuning specific Transformer modules (e.g., second FFN layer) is more effective than full fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing semantic theories by integrating formal, distributional, and grounded approaches for a more comprehensive model.

Method: Review of literature, cognitive science insights, and a small experiment to validate the words-as-classifiers model.
...
Conclusion: A unified semantic model based on words-as-classifiers is a viable path forward, combining strengths from all three semantic fields.
...
<details>
  <summary>Details</summary>
Motivation: To assess how well tokenizers preserve linguistically meaningful subwords and whether this impacts model performance.

Method: Expanded MorphScore to 70 languages, correlated alignment scores with downstream task performance across five pre-trained models and seven tasks.

Result: Morphological alignment explains little variance in model performance, indicating it alone is not a strong measure of tokenizer quality.

Conclusion: Morphological alignment is not a key factor in tokenizer quality for model performance, suggesting other dimensions need evaluation.
...
TL;DR: The paper extends syntactic structures to hypermagmas and colored operads, linking them to Merge operations and movement rules in syntax.


<details>
  <summary>Details</summary>
Motivation: To formalize syntactic structures and movement rules using mathematical frameworks like hypermagmas and colored operads.

Method: Extends magma structures to hypermagmas, formulates syntactic structures as colored operads, and relates them to Merge operations.

Result: Syntactic rules (e.g., movement, phases) are unified under colored operads, with movement compatibilities described via operad transduction.

Conclusion: The framework provides a unified mathematical foundation for syntactic structures and movement rules.
...
TL;DR: PERK (Parameter Efficient Reasoning over Knowledge) is a scalable method for long-context reasoning, using gradient updates to a lightweight adapter at test time, outperforming baselines with significant performance gains.


<details>
  <summary>Details</summary>
Motivation: To address the memory-intensive limitations of meta-learning methods for long-context reasoning, enabling efficient encoding and recall of relevant information.

Method: PERK employs nested optimization loops: an inner loop encodes contexts into a low-rank adapter (LoRA), and an outer loop learns to use the adapter for reasoning.

Result: PERK outperforms prompt-based baselines, achieving up to 90% gains for smaller models and 27% for larger ones, with robustness to context complexity and length.

Conclusion: PERK is a memory-efficient solution for long-context reasoning, scalable at inference time despite training memory demands.
...
TL;DR: The paper introduces REFORM, a self-improving reward modeling framework to enhance robustness by generating adversarial examples for training data augmentation, addressing failure modes in reward models without prior knowledge of preference distributions.


<details>
  <summary>Details</summary>
Motivation: Reward models often fail under distributional shifts or adversarial perturbations due to the complexity of human preferences and limited dataset coverage. Existing methods rely on prior knowledge, which is impractical in real-world settings.

Method: Proposes a preference-distribution agnostic method using reward-guided controlled decoding to discover failure modes. Introduces REFORM, which generates falsely scored responses for training data augmentation.

Result: REFORM significantly improves robustness on Anthropic HH and PKU Beavertails datasets without sacrificing reward quality, maintaining performance in direct evaluation and downstream policy training.

Conclusion: REFORM enhances alignment quality by removing spurious correlations and provides a practical solution for robust reward modeling without needing prior knowledge of failure modes.
...
TL;DR: The paper introduces a dictionary-learning method with sparse autoencoders to decompose LLMs, improving interpretability and performance on downstream tasks.


<details>
  <summary>Details</summary>
Motivation: LLMs are often seen as black-box models, reducing trust and limiting performance improvements. The goal is to make them more interpretable and effective.

Method: Uses a dictionary-learning approach with sparse autoencoders to decompose LLMs and extract monosemantic features from polysemantic neurons.

Result: Identifies model-internal misunderstandings, enables automatic prompt reformulation, and shows significant performance gains in tasks like mathematical reasoning and metaphor detection.

Conclusion: The decomposition method enhances LLM interpretability and performance, offering a practical approach to improving downstream task results.
...
<details>
  <summary>Details</summary>
Motivation: Understanding how policy language evolves is crucial for assessing global responses to challenges like climate change, but traditional methods are limited.

Method: The study applies DETM to UNFCCC policy decisions (1995-2023), capturing temporal topic dynamics.

Result: DETM identifies shifts from greenhouse gases to implementation, collaboration, and finance.

Conclusion: DETM is scalable and effective for policy discourse analysis, with potential for broader applications.
...
TL;DR: PAPO extends GRPO to improve multimodal reasoning by integrating perception learning, achieving significant performance gains and reducing perception errors.


<details>
  <summary>Details</summary>
Motivation: Current RLVR methods are suboptimal for multimodal tasks due to poor visual perception.

Method: Introduces Perception-Aware Policy Optimization (PAPO) with Implicit Perception Loss and Double Entropy Loss.

Result: 4.4% overall improvement, 8.0% on vision-dependent tasks, and 30.5% reduction in perception errors.

Conclusion: PAPO enhances RLVR by integrating perception, paving the way for visually grounded reasoning.
...
TL;DR: The paper introduces a novel approach to time normalization using the SCATE framework, leveraging LLMs for code generation and data augmentation, achieving strong performance with small models.
...
Method: Time normalization is reformulated as a code generation task using the SCATE framework. A Python library is implemented, and LLMs are used to generate executable SCATE code and synthesize annotated data.

Result: Small models trained on augmented data outperform LLMs, providing practical, accurate, and interpretable time normalization.

Conclusion: The SCATE framework and LLM-driven data augmentation enable effective time normalization, surpassing traditional methods and LLM performance.
...
TL;DR: The paper evaluates linear attention models in hybrid architectures, finding that superior standalone models don't always perform well in hybrids. A 3:1 to 6:1 linear-to-full attention ratio is recommended for optimal recall.


<details>
  <summary>Details</summary>
Motivation: Transformers struggle with long sequences due to quadratic complexity, leading to hybrid architectures combining linear and full attention. The choice of linear attention component is underexplored.

Method: Systematic evaluation of six linear attention variants across five hybridization ratios, training 72 models (340M and 1.3B parameters). Benchmarking on language modeling and recall tasks.

Result: Language modeling is stable across ratios, but recall improves with more full attention layers, especially below a 3:1 ratio. Selective gating, hierarchical recurrence, and controlled forgetting are key for hybrids.

Conclusion: HGRN-2 or GatedDeltaNet with a 3:1 to 6:1 linear-to-full ratio achieves efficient Transformer-level recall. Models are open-sourced.
...
TL;DR: The paper studies the robustness of verbal confidence in LLMs under adversarial attacks, revealing vulnerabilities and ineffective defenses, urging the need for more robust confidence mechanisms.


<details>
  <summary>Details</summary>
Motivation: Ensuring transparency, trust, and safety in human-AI interactions by examining the robustness of verbal confidence in LLMs.

Method: Introduces a framework for attacking verbal confidence via perturbation and jailbreak methods, testing various prompting strategies, model sizes, and domains.

Result: Attacks significantly compromise confidence estimates, causing frequent answer changes; current defenses are ineffective.
...
TL;DR: A novel approach for translating puns from English to French combines large language models with specialized wordplay techniques, achieving top results in a competition.
...
Method: A three-stage approach: baseline with large language models, guided chain-of-thought pipeline, and a multi-agent generator-discriminator framework.

Result: Achieved first and second place in the CLEF JOKER 2025 Task 2 competition, evaluated by native French speakers.
...
TL;DR: SpindleKV is a novel KV cache reduction method for LLMs, balancing shallow and deep layers with attention-based eviction and codebook replacement, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: The increasing memory consumption of KV cache in LLMs poses a challenge, with existing methods lacking efficiency for shallow layers.
...
Conclusion: SpindleKV effectively reduces KV cache redundancy across layers, offering a scalable solution for LLM inference systems.
...
TL;DR: InvestAlign proposes a framework to generate high-quality SFT datasets for LLMs using theoretical solutions to simple investment problems, achieving faster convergence and better alignment with real-user data.


<details>
  <summary>Details</summary>
Motivation: The scarcity of real-user data for SFT in aligning LLMs with investor decision-making under herd behavior poses challenges in behavioral finance.

Method: InvestAlign constructs SFT datasets from theoretical solutions to simple optimal investment problems, avoiding reliance on real-user data.

Result: LLMs trained with InvestAlign data show faster convergence and InvestAgent, a fine-tuned LLM, aligns better with real-user data.

Conclusion: InvestAlign is a promising approach for aligning LLMs with investor behavior, addressing complex problems without extensive real-user data.
...
TL;DR: A method for constructing high-quality datasets for industrial contract info extraction, using GPT-4/3.5 for annotations, data augmentation, and fine-tuning a large language model, achieving high performance.


<details>
  <summary>Details</summary>
Motivation: Addressing the need for efficient and accurate contract information extraction in industrial settings by leveraging advanced NLP techniques.

Method: Cluster analysis of contracts, GPT-4/3.5 for annotations, data augmentation via keyword-generated texts, and fine-tuning a large language model with LoRA and balancing.

Result: High field recall, precision, and parsing efficiency; LoRA, data balancing, and augmentation improved model accuracy and robustness.

Conclusion: The method offers an efficient solution for industrial contract info extraction, enhancing model performance through innovative dataset construction and fine-tuning.
...
TL;DR: The paper introduces a discursive-network model to analyze interactions between humans and LLMs, defining invalidation and its hazards. A mathematical model shows how peer review can stabilize truth in networks.


<details>
  <summary>Details</summary>
Motivation: To understand and improve the reliability of human-LLM interactions by treating them as a network of equal nodes and addressing invalidation hazards.

Method: Develops a discursive-network model and a mathematical framework to analyze invalidation hazards (drift, self-repair, fabrication, detection). Introduces the FOO algorithm for peer review.

Result: Networks with drift and self-repair stabilize at modest error rates; fabrication increases errors. Peer review shifts systems to truth-dominant states.

Conclusion: Reliability in human-LLM interactions requires networked models with peer review, not perfect single models.
...
TL;DR: A unified food-domain QA framework combines a multimodal knowledge graph (MMKG) with generative AI, improving performance metrics like BERTScore and CLIP alignment while ensuring factual and visual fidelity.
...
Method: Joint fine-tuning of Meta LLaMA 3.1-8B and Stable Diffusion 3.5-Large, augmented by LLaVA/DeepSeek, and a hybrid retrieval-generation strategy.

Result: Improvements include a 16.2% BERTScore increase, 37.8% FID reduction, and 31.1% CLIP alignment boost. Diagnostic analyses ensure factual and visual fidelity.

Conclusion: Structured knowledge and multimodal generation together significantly improve food QA reliability and diversity.
...
Method: The GMU mechanism is integrated into the SambaY architecture, a decoder-hybrid-decoder model, to share memory readout states from a Samba-based self-decoder.

Result: SambaY improves decoding efficiency, maintains linear pre-filling time complexity, and outperforms baselines like YOCO, with significant gains in reasoning tasks and throughput.

Conclusion: The GMU-enhanced SambaY architecture demonstrates superior scalability and performance, validated by extensive experiments and open-sourced code.
...
TL;DR: FuDoBa combines LLM embeddings with domain-specific knowledge via Bayesian optimization, creating efficient, interpretable document representations that match or outperform proprietary LLM baselines.


<details>
  <summary>Details</summary>
Motivation: High-dimensional, generic LLM embeddings are inefficient for domain-specific tasks. FuDoBa aims to integrate domain knowledge for better performance and interpretability.

Method: FuDoBa uses Bayesian optimization to fuse LLM embeddings with structured domain knowledge (local and WikiData), producing low-dimensional, task-relevant representations.

Result: FuDoBa matches or surpasses proprietary LLM baselines on six datasets across two domains, with reduced training complexity.

Conclusion: FuDoBa offers a scalable, interpretable solution for domain-specific document representation, outperforming generic LLM embeddings.
...
TL;DR: The paper evaluates using Claude 3.5 Sonnet for data extraction in scoping reviews, finding high accuracy for simple tasks but low performance for complex data. It highlights the need for robust evaluation and recommends reporting LLM performance.


<details>
  <summary>Details</summary>
Motivation: To expedite the resource-intensive data extraction process in reviews using LLMs and review protocols.

Method: Trialled two protocol-based approaches with Claude 3.5 Sonnet for data extraction from 10 evidence sources in a scoping review, followed by performance evaluation.

Result: High accuracy for simple data (83.3%-100%), low for complex data (9.6%-15.8%). Precision >90%, but recall and F1 scores were low (<25% and <40%). LLM feedback improved protocol adaptation.

Conclusion: LLMs can assist but require robust evaluation. Researchers should report LLM performance and consider protocol-based methods for future reviews.
...
TL;DR: The paper introduces an AI-based method to measure elite polarization by analyzing politicians' speeches, tracking mentions and emotional tone, and creating an index of mutual hostility.


<details>
  <summary>Details</summary>
Motivation: To quantify elite polarization by analyzing how politicians evaluate out-parties in parliamentary speeches, providing insights into political dynamics.

Method: Uses AI to detect actors and subjects in speeches, tracks who is speaking and addressed, and assesses emotional tone to create a polarization index.

Result: The index shows validity by reacting to events like elections, crises, and power shifts, and can be aggregated by party and quarter.

Conclusion: The approach enables a 20-year EU-wide dataset on elite polarization, offering a scalable tool for political analysis.
...
TL;DR: CLI-RAG is a framework for structured clinical text generation using LLMs, addressing challenges of unstructured patient data and long notes with hierarchical chunking and dual-stage retrieval.


<details>
  <summary>Details</summary>
Motivation: Real-world clinical applications of LLMs face challenges due to unstructured, heterogeneous patient data and long, dense clinical notes.

Method: Introduces CLI-RAG with hierarchical chunking and dual-stage retrieval (global for note types, local for high-value content).

Result: Achieves 87.7% alignment score, outperforming clinician-authored notes (80.7%), with high consistency across LLMs.

Conclusion: CLI-RAG improves structured clinical text generation, ensuring reproducibility, reliability, and clinical trust.
...
TL;DR: The paper investigates how uncertainty in LLMs affects their inference dynamics, finding that certain and uncertain predictions follow similar probability trajectories across layers.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs internally represent and process uncertainty, aiming to improve detection methods and prevent hallucinations.

Method: The Tuned Lens (a variant of the Logit Lens) is used to analyze layer-wise probability trajectories of predictions across 11 datasets and 5 models, focusing on incorrect predictions as proxies for uncertainty.

Result: Uncertainty does not significantly alter inference dynamics; both certain and uncertain predictions show abrupt confidence increases at similar layers. However, more competent models may process uncertainty differently.

Conclusion: Simplistic methods for detecting uncertainty during inference may not be feasible, and interpretability tools like the Tuned Lens can provide insights into how uncertainty impacts model behavior.
...
TL;DR: KAConvText is applied to sentence classification tasks (hate speech detection, news classification, language identification) using various embeddings and classification heads, achieving high accuracy.


<details>
  <summary>Details</summary>
Motivation: To explore the effectiveness of KAConvText in diverse sentence classification tasks, comparing embedding methods and classification heads.

Method: KAConvText is tested with random and fastText embeddings (static/fine-tuned, 100/300 dimensions, CBOW/Skip-gram) and compared to CNNs and CNN-KAN. MLP and KAN heads are evaluated.

Result: KAConvText-MLP with fine-tuned fastText embeddings achieves top performance: 91.23% accuracy (hate speech), 92.66% (news), 99.82% (language).

Conclusion: KAConvText, especially with fine-tuned fastText embeddings and MLP head, excels in sentence classification across tasks.
...
TL;DR: The paper introduces CE-Judge, a training-free framework for multilingual text evaluation using open-source LLMs, outperforming baselines and matching GPT-4o.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of exploration of LLM-as-a-Judge in multilingual contexts and the inefficiency of proprietary models or extensive training data.

Method: Proposes Checklist Engineering (CE-Judge), leveraging checklist intuition for multilingual evaluation without training.

Result: CE-Judge surpasses baselines and performs comparably to GPT-4o across multiple languages and datasets.

Conclusion: CE-Judge offers a cost-effective, efficient alternative for multilingual text evaluation, reducing reliance on proprietary models.
...
TL;DR: DACP improves small LLMs for enterprise use, balancing domain performance and general capabilities.


<details>
  <summary>Details</summary>
Motivation: Many organizations lack infrastructure for large LLMs, making small LLMs a practical but limited alternative. DACP's commercial utility is underexplored.

Method: Applied DACP across diverse foundation models and service domains, testing its effectiveness.

Result: DACP-applied sLLMs showed significant performance gains in target domains while maintaining general capabilities.

Conclusion: DACP offers a cost-efficient, scalable solution for enterprise deployment of small LLMs.
...
Conclusion: The approach is versatile, not domain-specific, and outperforms LLM-only methods in generating accurate dynamical system models.
...
TL;DR: The paper introduces a flexible test-time collaborative inference framework combining sequential and parallel reasoning, addressing inefficiencies and lack of coordination in existing methods. It proposes semantic entropy (SE) as a quality metric for dynamic control.


<details>
  <summary>Details</summary>
Motivation: Current sequential and parallel reasoning methods in LLMs have inefficiencies (arbitrary token budgets) and lack coordination (parallel branches). The goal is to combine their strengths for better inference.

Method: Develops a collaborative inference framework using semantic entropy (SE) to assess response quality dynamically, enabling efficient control and termination.

Result: SE serves as a robust indicator of reasoning quality, correlating negatively with accuracy, improving inference efficiency.

Conclusion: The proposed framework and SE metric enhance LLM inference by combining sequential and parallel reasoning effectively.
...
TL;DR: SETR improves RAG by selecting passages collectively for complex queries, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing reranking methods fail to meet the information needs of complex queries in multi-hop QA.
...
TL;DR: The paper discusses insights from maintaining an open-source AI evaluation repository, highlighting challenges and solutions like cohort management, statistical methods, and quality control.


<details>
  <summary>Details</summary>
Motivation: To address the growing need for reliable AI evaluations by sharing practical insights and solutions from maintaining a community-contributed repository.
...
Result: Identified that AI evaluation demands specialized infrastructure, statistical rigor, and community coordination beyond traditional software practices.

Conclusion: AI evaluations require unique approaches, combining infrastructure, statistical methods, and community efforts for effective implementation and maintenance.
...
Method: Uses supervised contrastive learning and Bayesian kNN for multi-label classification, avoids finetuning, and supports easy PLM switching.

Result: Matches or surpasses state-of-the-art methods on five benchmarks, reduces energy consumption, and handles noisy annotations effectively.
...
Result: VisualTrap successfully compromises agents with 5% poisoned data and stealthy triggers, generalizing across tasks and GUI environments.

Conclusion: Highlights the urgent need for research on backdoor attack risks in GUI agents due to their demonstrated vulnerabilities.
...
Conclusion: MIND provides a scalable, effective solution for harmful meme detection without annotated data.
...
TL;DR: The paper introduces a new dataset (MPMCP) to evaluate legal judgment prediction (LJP) models across four scenarios involving single/multiple defendants and charges, finding S4 (multiple defendants and charges) the most challenging.


<details>
  <summary>Details</summary>
Motivation: To explore whether multiple defendants and charges should be treated separately in LJP, addressing a gap in current research.

Method: Created the MPMCP dataset and evaluated performance of legal LLMs (e.g., InternLM2, Lawformer) on four scenarios (S1-S4) for charge and penalty term prediction.

Result: S4 was the hardest, with significant performance drops (e.g., Lawformer: 19.7% lower F1-score, 19.0% higher LogD vs. S1).

Conclusion: Multiple defendants and charges complicate LJP, with model performance varying widely; the MPMCP dataset facilitates further research.
...
TL;DR: The paper explores using LLMs like Llama 3 and GPT-4o to predict tutor strategies and student outcomes in tutoring dialogues, finding current models inadequate for strategy prediction despite its importance.


<details>
  <summary>Details</summary>
Motivation: The rise of online learning and AI tutors necessitates understanding tutor strategies and their impact on students, yet predicting these strategies remains understudied.

Method: The study employs Llama 3 and GPT-4o to predict tutor moves and student outcomes using two math tutoring dialogue datasets.
...
TL;DR: The paper addresses limitations in current code-generation benchmarks by proposing a human-LLM collaborative method (SAGA) to improve test-case quality and coverage, validated by higher detection rates and verifier accuracy.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for LLM code-generation lack thoroughness, leading to inflated performance metrics and unreliable reward estimation in RLVR.

Method: Introduces SAGA, a human-LLM collaborative approach, and TCGBench for evaluating test-case generation (TCG) with multi-dimensional metrics.

Result: SAGA achieves a 90.62% detection rate and 32.58% verifier accuracy, outperforming LiveCodeBench-v6 by 10.78%.

Conclusion: The work aims to enhance reliable LLM code evaluation, advance RLVR, and enable automated adversarial test synthesis.
...
TL;DR: The paper examines how retrieval-augmented generation (RAG) systems are sensitive to query perturbations, revealing performance degradation under minor variations. It proposes an evaluation framework and practical recommendations based on extensive experiments.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) are costly to update, and RAG offers a dynamic solution, but its effectiveness depends heavily on query quality. The study aims to understand and mitigate this dependency.

Method: The authors analyze the sensitivity of RAG components to query perturbations, studying retrievers and their combined effects in question answering. They use general and domain-specific datasets and conduct over 1092 experiments.

Result: Performance of retrievers degrades significantly under minor query variations. The study provides insights into module-level and end-to-end robustness.

Conclusion: The paper highlights the fragility of RAG systems to query changes and offers an evaluation framework and actionable recommendations to improve robustness.
...
TL;DR: FRaN-X is a tool that detects and classifies entity roles in narratives from raw text, supporting multiple languages and domains, with an interactive web interface for media analysis.


<details>
  <summary>Details</summary>
Motivation: To automate the detection and classification of how entities are framed in narratives, aiding media analysts in understanding portrayals across sources.

Method: A two-stage system combining sequence labeling and fine-grained role classification, using a taxonomy of 22 roles under three main categories (protagonists, antagonists, innocents).

Result: Supports five languages and two domains, provides interactive analysis tools, and is publicly accessible under an MIT License.

Conclusion: FRaN-X effectively aids in analyzing narrative framing, offering practical tools for media analysts.
...
TL;DR: FlexOlmo is a language model supporting distributed training without data sharing and flexible inference, outperforming prior methods by 41% improvement and respecting data owners' preferences.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of training LMs on closed datasets without data sharing while allowing flexible inference based on data licensing or permissions.

Method: Uses a mixture-of-experts (MoE) architecture with independently trained experts on closed datasets, integrated via domain-informed routing. Trained on FlexMix corpus.

Result: Achieves 41% relative improvement, outperforms prior merging methods by 10.1%, and surpasses standard MoE with same FLOPs.

Conclusion: FlexOlmo offers a solution for regulated industries, enabling use of closed data while respecting data owners' preferences and local data control.
...
TL;DR: A unified model for conversational search systems combines dense retrieval and response generation, outperforming existing baselines by addressing context understanding and data discrepancy.


<details>
  <summary>Details</summary>
Motivation: Existing conversational search systems use separate models, limiting their ability to leverage intrinsic knowledge and ensure retrieval effectiveness for generation.

Method: Joint fine-tuning with different objectives and mechanisms to reduce inconsistency risks and mitigate data discrepancy.

Result: Evaluations on five datasets show mutual improvement in retrieval and generation tasks, surpassing baselines.

Conclusion: The unified model enhances conversational search by integrating retrieval and generation, demonstrating superior performance.
...
TL;DR: The paper explores discrete diffusion models (D3PM) for natural language generation, comparing them with autoregressive models. D3PM shows faster processing but lower compression efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of applying diffusion models to discrete data like natural language, due to token dependencies and lack of generation order.

Method: Evaluates D3PM against autoregressive models using metrics like Bits Per Token (BPT), Negative Log-Likelihood (NLL), Perplexity (PPL), and Batch Processing Speed.
...
Conclusion: Diffusion models show promise for parallel generation in discrete data but face trade-offs in generative quality, supporting future non-autoregressive research.
...
TL;DR: HADES-NN is a novel method for estimating parameters in non-autonomous differential equations by using neural networks to smooth discontinuous external signals, improving accuracy and applicability.


<details>
  <summary>Details</summary>
Motivation: Non-autonomous differential equations are hard to fit when external signals change abruptly. A robust method is needed to handle such discontinuities.

Method: HADES-NN uses neural networks in two stages: first to approximate discontinuous signals smoothly, then to estimate model parameters.

Result: The method provides accurate parameter estimates in applications like circadian clocks and yeast mating responses.

Conclusion: HADES-NN enhances the ability to fit models to real-world data with discontinuous signals.
...
Method: SEA-DBS uses a predictive reward model and Gumbel Softmax for stable, efficient exploration in binary action spaces.

Result: SEA-DBS shows faster convergence, better beta-band suppression, and resilience to hardware constraints in simulations.
...
<details>
  <summary>Details</summary>
Motivation: To automate the discovery of Hamiltonian functions in mechanics by leveraging deep learning.

Method: Uses hybrid CNN-LSTM architectures to learn and output symbolic expressions of Hamiltonians from vector fields.

Result: Effectively recovers symbolic Hamiltonian expressions, validated on new datasets.

Conclusion: Advances automated discovery in Hamiltonian mechanics with accurate symbolic regression.
...
TL;DR: The paper introduces DecoyDB, a large-scale dataset for self-supervised graph contrastive learning (GCL) to improve protein-ligand binding affinity prediction, addressing data scarcity and algorithmic challenges.


<details>
  <summary>Details</summary>
Motivation: The lack of large-scale, high-quality labeled protein-ligand complexes hinders binding affinity prediction in drug discovery. Self-supervised learning, particularly GCL, offers a solution but faces dataset and algorithmic challenges.

Method: The authors propose DecoyDB, a dataset with ground truth and decoy protein-ligand complexes, and a customized GCL framework for pre-training graph neural networks, followed by fine-tuning on labeled data.

Result: Models pre-trained with DecoyDB show superior accuracy, label efficiency, and generalizability in binding affinity prediction.
...
TL;DR: The paper shows that the gradient flow for learning linear convolutional networks can be expressed as a Riemannian gradient flow on function space, regardless of initialization, unlike fully connected networks which require balanced initialization.


<details>
  <summary>Details</summary>
Motivation: To extend the understanding of gradient flow dynamics from fully connected to convolutional networks, focusing on initialization independence.

Method: Analyzes the gradient flow on parameter space for linear convolutional networks, proving it can be written as a Riemannian gradient flow on function space.

Result: The result holds for D-dimensional convolutions (D ≥ 2) and for D=1 with strides >1, with the Riemannian metric depending on initialization.

Conclusion: The study generalizes the Riemannian gradient flow framework to convolutional networks, highlighting initialization-independent behavior.
...
TL;DR: WINGs dynamically generates and compresses neural network weights using PCA and SVR, reducing memory usage significantly with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: To address the high memory requirements of neural networks, especially for edge applications, by dynamically generating and compressing weights.
...
Result: Achieves 53x compression for FC layers, 28x for AlexNet (MNIST), and 18x for AlexNet (CIFAR-10) with 1-2% accuracy loss.

Conclusion: WINGs offers a memory-efficient, secure, and high-throughput solution for DNN inference, ideal for resource-constrained environments.
...
TL;DR: The paper decomposes gradient flow in recurrent dynamical systems into two operators (K and P), explaining latent dynamics and multi-task alignment, validated with experiments and a Pytorch package.
...
Method: Decomposes gradient flow into a Parameter Operator (K) and a Linearized Flow Propagator (P), analyzing their interplay for latent dynamics and multi-task alignment.

Result: Demonstrates low-dimensional latent dynamics and task alignment, validated experimentally and theoretically.

Conclusion: Advances understanding of GD in non-linear recurrent models, providing tools for analysis via the KPFlow package.
...
Result: Models achieved over 99.5% accuracy for fragmented manipulations and 98% for subtle ones. Hybrid CNN-Transformer Siamese model achieved 100% accuracy in identity verification.

Conclusion: Hybrid models, especially CNN-Transformer, are highly effective for ECG tamper detection and identity verification, demonstrating robustness in real-world scenarios.
...
TL;DR: KnowRare is a deep learning framework for predicting clinical outcomes in rare ICU conditions, outperforming existing models and scoring systems.
...
Method: Uses self-supervised pre-training and a condition knowledge graph to adapt knowledge from similar conditions.

Result: Outperformed state-of-the-art models and ICU scoring systems across five clinical tasks.

Conclusion: KnowRare is a robust solution for improving care for rare ICU conditions.
...
TL;DR: The paper introduces eegFloss, a Python package with the eegUsability ML model to detect artifacts in sleep EEG data, improving sleep study accuracy.


<details>
  <summary>Details</summary>
Motivation: EEG signals in sleep research are prone to artifacts, leading to erroneous sleep staging. Automated solutions are needed for large-scale studies.

Method: Developed eegUsability, an ML model trained on artifact-labeled EEG data from 15 participants, and integrated it into the eegFloss package.
...
Conclusion: eegFloss enhances sleep study precision by addressing artifact detection, improving reliability and accuracy of outcomes.
...
Result: Hierarchical attention patterns correlate with hierarchical OOD generalization, even without direct reliance on these patterns.

Conclusion: Interpretability tools show promise for predicting unseen model behavior, motivating further research.
...
TL;DR: FedPhD is a novel FL approach for training Diffusion Models, addressing data heterogeneity and high communication costs with hierarchical FL and pruning.


<details>
  <summary>Details</summary>
Motivation: Challenges in FL for Diffusion Models include high communication costs and data heterogeneity, which FedPhD aims to solve.

Method: FedPhD uses Hierarchical FL with homogeneity-aware aggregation, selection policy, and distributed structured pruning.

Result: FedPhD reduces communication costs by 88%, improves FID by 34%, and uses only 56% of resources.

Conclusion: FedPhD efficiently trains Diffusion Models in FL, outperforming baselines in performance and resource usage.
...
TL;DR: A framework for labeling neurons in protein language models (PLMs) with biological descriptions is introduced, enabling automated analysis and steering for protein generation with desired traits.


<details>
  <summary>Details</summary>
Motivation: To understand the internal neuron representations of PLMs and leverage them for biologically meaningful tasks like protein design.

Method: An automated framework labels neurons with biological descriptions, scales to large numbers, and uses neuron activation-guided steering for protein generation.

Result: The method reveals neuron sensitivity to biochemical properties and structural motifs, and enables targeted protein generation. It also uncovers PLM scaling laws and neuron space distribution.

Conclusion: The framework advances understanding of PLMs and their application in protein design, demonstrating scalability and biological relevance.
...
<details>
  <summary>Details</summary>
Motivation: Modern machine learning's energy-intensive scaling necessitates alternatives to backpropagation, which is inefficient for hardware accelerators.

Method: Derives forward-forward algorithms using binary, stochastic units, leveraging binarization for efficient hardware execution and stochasticity to overcome binary limitations.
...
Conclusion: The proposed method offers a promising, energy-efficient alternative to traditional backpropagation for neural network training.
...
<details>
  <summary>Details</summary>
Motivation: To understand Adam's empirical success and limitations, and to develop a more robust optimizer.

Method: Introduces S3 with three innovations: flexible p-th order momentum, unified exponential moving average coefficients, and Nesterov's accelerated gradient module.

Result: S3 achieves optimal convergence, faster training, and fewer loss spikes, even with aggressive learning rates.

Conclusion: S3 outperforms AdamW in efficiency and performance, offering a robust alternative for deep learning optimization.
...
TL;DR: FMSP leverages foundation models to enhance self-play, overcoming local optima and producing diverse, high-quality policies.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional self-play (SP) in generating diverse solutions and avoiding local optima.

Method: Introduces three FMSP variants: vFMSP for policy refinement, NSSP for diversity, and QDSP combining both. Evaluated in Car Tag and Gandalf scenarios.
...
Conclusion: FMSP is a promising approach for improving self-play with foundation models, enabling creative and open-ended strategy discovery.
...
TL;DR: The paper proposes MimbFD, a dual-view graph representation learning method to address message imbalance in fraud detection caused by topology and class imbalance. It improves node representation learning and debiases local confounding effects, showing strong performance on public datasets.


<details>
  <summary>Details</summary>
Motivation: The imbalance in topological information and class distribution in GNN-based fraud detection leads to imbalanced supervisory messages, affecting performance. Fraudsters' obfuscation and concealment exacerbate this issue.

Method: MimbFD uses a topological message reachability module for better node representation and a local confounding debiasing module to adjust representations and balance class influence.

Result: Experiments on three public fraud datasets show MimbFD outperforms existing methods in fraud detection.

Conclusion: MimbFD effectively mitigates message imbalance and improves fraud detection by addressing topology and class imbalance through dual-view learning.
...
TL;DR: FedDifRC introduces diffusion models into federated learning to address data heterogeneity, using text-driven contrast and noise-driven regularization for better performance.
...
Method: Proposes FedDifRC, leveraging diffusion models for text-driven contrastive learning and noise-driven regularization to mitigate data heterogeneity.

Result: Experiments validate FedDifRC's effectiveness and efficiency in handling data heterogeneity across scenarios.

Conclusion: FedDifRC successfully integrates diffusion models into federated learning, improving performance and convergence despite data heterogeneity.
...
TL;DR: MoFE-Time integrates time and frequency features in a Mixture of Experts network, outperforming existing methods in time series forecasting.


<details>
  <summary>Details</summary>
Motivation: Existing models often fail to model both time and frequency characteristics in a pretraining-finetuning paradigm, limiting performance for complex time series.

Method: MoFE-Time uses a MoE network with time and frequency cells as experts, leveraging pretraining-finetuning to transfer knowledge across datasets.

Result: Achieves state-of-the-art performance, reducing MSE and MAE by 6.95% and 6.02% on benchmarks, and excels in real-world datasets.

Conclusion: MoFE-Time effectively combines time and frequency features, demonstrating superior performance in both benchmarks and practical applications.
...
<details>
  <summary>Details</summary>
Motivation: Deep neural networks often produce overconfident, miscalibrated predictions, and existing calibration methods lack monotonicity guarantees or suffer from limited expressiveness or interpretability.

Method: The authors introduce a constrained calibration map, linearly parameterized by the number of classes, formulated as a constrained optimization problem to ensure monotonicity.

Result: The proposed methods achieve state-of-the-art performance across datasets with various deep neural network models, outperforming existing calibration methods in efficiency and accuracy.

Conclusion: The work provides a robust, interpretable, and efficient solution for post-hoc calibration, addressing limitations of prior methods.
...
Method: AdaDPIGU uses a differentially private Gaussian mechanism to estimate parameter importance, prunes low-importance coordinates, and employs adaptive clipping for sparse, noise-efficient updates.

Result: Achieves 99.12% accuracy on MNIST (ε=8) and 73.21% on CIFAR-10 (ε=4), outperforming non-private baselines.

Conclusion: AdaDPIGU enhances privacy and utility, proving adaptive sparsification can mitigate performance degradation in high-dimensional settings.
...
TL;DR: A novel Bayesian optimization (BO) method is introduced, combining direct regret optimization with ensemble Gaussian Processes (GPs) and non-myopic acquisition, outperforming traditional BO baselines.


<details>
  <summary>Details</summary>
Motivation: Traditional BO methods rely on hand-crafted acquisition functions and surrogate models, often operating myopically. This work aims to improve by jointly optimizing models and acquisitions for multi-step regret.

Method: The framework uses ensemble GPs with varied hyperparameters to simulate BO trajectories, training a decision transformer end-to-end for non-myopic query selection. It employs dense offline training with simulated data and sparse online refinement.
...
Conclusion: The proposed approach effectively integrates model and acquisition optimization, demonstrating superior performance in BO tasks.
...
TL;DR: CircuitGCL is a graph contrastive learning framework for AMS circuits, improving transferability and robustness in parasitic estimation tasks by combining representation scattering and label rebalancing.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges like scarce design data, unbalanced labels, and diverse circuit implementations in AMS circuits for robust and transferable representations.
...
TL;DR: CircuitGPS is a few-shot learning method for predicting parasitic effects in AMS circuits, using a heterogeneous graph representation and hybrid graph Transformer, achieving significant accuracy improvements.


<details>
  <summary>Details</summary>
Motivation: The scarcity of integrated circuit design data limits deep learning models for AMS circuits, prompting the need for efficient few-shot learning methods.

Method: CircuitGPS represents circuit netlists as heterogeneous graphs, uses small-hop sampling to create subgraphs, and employs a hybrid graph Transformer with low-cost positional encoding.
...
Conclusion: CircuitGPS is scalable and effective for diverse AMS circuits, with ablation studies offering insights for graph representation learning.
...
TL;DR: Decentralized learning with optimized communication scheduling, especially late-stage synchronization, matches centralized training performance and preserves model mergeability.


<details>
  <summary>Details</summary>
Motivation: To address the performance limitations of decentralized learning due to poor peer-to-peer communication scheduling.

Method: Study communication scheduling, including timing and frequency of device synchronization, and analyze the impact of late-stage communication and global merging.
...
Conclusion: Decentralized learning can perform as well as centralized training with optimized communication, challenging the belief of poor generalization under data heterogeneity.
...
TL;DR: A deep-learning-based 2-stage model using GNN and MLP predicts SRAM parasitics pre-layout, reducing error by 19X and speeding up simulations by 598X.


<details>
  <summary>Details</summary>
Motivation: To address discrepancies between pre- and post-layout simulations in SRAM customization, aiming to reduce design iterations.

Method: Combines GNN classifier and MLP regressors, uses Focal Loss for class imbalance, and integrates subcircuit info into graphs.

Result: Achieves 19X error reduction and 598X simulation speedup on 4 real SRAM designs.

Conclusion: The proposed model effectively predicts parasitics pre-layout, enhancing efficiency and reducing design iterations.
...
<details>
  <summary>Details</summary>
Motivation: Recent spectral initialization methods for LoRA improve performance but introduce computational and storage inefficiencies. The paper aims to address this by focusing on update magnitude as the key driver of performance.
...
Conclusion: Update magnitude is fundamental to LoRA performance. LoRAM provides a compact, efficient alternative to spectral methods, unifying hyperparameter tuning and improving convergence.
...
TL;DR: The paper addresses the storage burden of Mixture-of-Experts (MoE) models on edge devices by optimizing expert caching for distributed inference, proposing algorithms to minimize latency with provable guarantees.


<details>
  <summary>Details</summary>
Motivation: The sheer number of experts in MoE models creates storage challenges for edge devices, necessitating efficient distributed inference solutions.

Method: For K=1, a greedy algorithm with (1-1/e)-approximation is used. For K≥1, a successive greedy decomposition method and dynamic programming are employed, with an accelerated max-convolution technique for polynomial-time solutions.

Result: Simulations show the proposed method significantly reduces inference latency compared to baselines.

Conclusion: The approach effectively optimizes expert caching in MoE models for edge networks, improving scalability and performance.
...
TL;DR: RLVR advances LLM reasoning; LPPO framework optimizes learning with trusted demonstrations via prefix-guided sampling and learning-progress weighting, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: To leverage small, high-quality demonstrations effectively instead of scaling data volume, inspired by human problem-solving and learning focus.

Method: Introduces LPPO with prefix-guided sampling (using expert hints) and learning-progress weighting (dynamic sample influence based on model progression).

Result: Outperforms baselines on mathematical-reasoning benchmarks, achieving faster convergence and higher performance.

Conclusion: LPPO's sample-centric approach enhances RLVR efficiency and effectiveness for LLMs.
...
Method: Uses predicted information gain to identify informative regions for exploration and applies reinforcement learning to derive exploration policies.

Result: The approach reliably estimates underlying dynamics and outperforms myopic exploration methods.

Conclusion: The method provides a robust way to explore and learn dynamics in environments without explicit models.
...
TL;DR: A generalization-focused RL framework for RAN control improves performance in diverse 5G scenarios, outperforming traditional methods and specialized RL in benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional rule-based RRM algorithms underperform in dynamic RAN environments, and data-driven policies often overfit to training conditions.

Method: The framework uses attention-based graph representations, domain randomization, and distributed data generation with centralized training.

Result: Achieves ~10% higher throughput and spectral efficiency over baselines, with significant gains in high mobility and diverse traffic scenarios.
...
TL;DR: A novel generative framework balances disentanglement and reconstruction quality by using multiple β values in a VAE, enhanced by a diffusion model for smooth transitions and high-quality outputs.


<details>
  <summary>Details</summary>
Motivation: The trade-off between disentangled latent representations and high-quality reconstructions in generative models, addressed by the β-VAE framework, motivates a solution that leverages multiple β values.

Method: Train a single VAE with a new loss function for multiple β values, then use a non-linear diffusion model to transition between representations, prioritizing disentanglement or reconstruction as needed.

Result: The framework achieves high-quality reconstructions and disentangled representations, with smooth latent space transitions and standalone generative capabilities.

Conclusion: The proposed method effectively balances disentanglement and reconstruction, offering flexible and high-quality generative modeling.
...
TL;DR: CTPG introduces a framework for multi-task RL by using guide policies to leverage cross-task similarities, improving learning efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Existing multi-task RL methods overlook direct exploitation of cross-task similarities through policy guidance.
...
Conclusion: CTPG effectively leverages cross-task policy guidance to accelerate skill acquisition in multi-task RL.
...
TL;DR: SAD-DPSGD improves medical image classification by addressing data leakage and imbalance issues, outperforming Auto-DPSGD with a 2.15% accuracy boost.


<details>
  <summary>Details</summary>
Motivation: Data leakage and imbalanced datasets in medical imaging cause suboptimal model performance, especially for minority classes.

Method: Proposes SAD-DPSGD, using linear decaying noise and clipping thresholds to allocate more privacy budget early in training.

Result: SAD-DPSGD achieves 2.15% higher accuracy than Auto-DPSGD on HAM10000 under ε=3.0, δ=10^-3.

Conclusion: SAD-DPSGD effectively mitigates data leakage and imbalance, improving model performance on small medical datasets.
...
Conclusion: UniOD offers a convenient, accurate, and efficient solution for outlier detection across diverse domains without the need for dataset-specific tuning.
...
<details>
  <summary>Details</summary>
Motivation: Challenges in sharing knowledge across tasks in offline multi-task RL inspired the development of GO-Skill, mimicking human learning.

Method: GO-Skill uses goal-oriented skill extraction, vector quantization for a discrete skill library, skill enhancement, and hierarchical policy learning.
...
TL;DR: A method for detecting and preventing overfitting in mesh-like data regressions by using Laplace-operator derivatives for hyperparameter optimization and minimizing entropy.


<details>
  <summary>Details</summary>
Motivation: To address overfitting in mesh-like data regressions by leveraging the structure of the mesh for derivative computations and avoiding traditional train-test splits.

Method: Compute derivatives on the original training mesh for true labels and on a staggered mesh to detect oscillations. Use Laplace-operator derivative loss for hyperparameter optimization.

Result: Reduction of unwanted oscillations and minimized entropy in the trained model, avoiding the need for traditional testing splits.

Conclusion: The method effectively prevents overfitting by utilizing mesh structure and derivative-based metrics, enabling training on all available data without separate testing points.
...
TL;DR: A novel algorithm for estimating individual-level treatment effects using disentangled representation, multi-head attention, and orthogonal regularization, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of precisely disentangling covariates (instrumental, confounding, adjustment) in causal inference, as previous methods (generative models, hard decomposition) often fail to guarantee accurate disentanglement.

Method: Proposes a treatment effect estimation algorithm combining a mixture of experts with multi-head attention, linear orthogonal regularization for soft decomposition, and importance sampling re-weighting to eliminate selection bias.

Result: Outperforms state-of-the-art methods on public semi-synthetic and real-world datasets.
...
<details>
  <summary>Details</summary>
Motivation: Fuzzy systems address uncertainty in machines, but improvements are possible by leveraging advancements in machine learning and federated learning.

Method: Proposes integrating machine learning and federated learning concepts, such as updating fuzzy rules, to refine fuzzy systems.

Result: Potential improvements identified, but further investigation is needed to quantify their impact.

Conclusion: The proposed enhancements show promise for advancing fuzzy systems, though more research is required to validate their effectiveness.
...
<details>
  <summary>Details</summary>
Motivation: Increasing variability in power systems due to renewables necessitates reliable short-term state forecasting for stability and control. Existing GNN methods struggle with heterogeneous sensor data across domains.

Method: Uses Heterogeneous Graph Attention Networks to model intra- and inter-domain relationships in hydraulic and electrical sensor data.
...
TL;DR: The paper introduces a method for Imitation Learning from Observation (IfO) that leverages action-free demonstrations, addressing limitations of current research by handling nuanced data distributions and enabling iterative self-improvement.


<details>
  <summary>Details</summary>
Motivation: Current IfO research focuses on idealized scenarios with bimodal-quality data, limiting practical applicability. This paper aims to explore more realistic data distributions and develop a method for scalable behavior learning.

Method: The method adapts RL-based imitation learning to action-free demonstrations, using a value function to transfer information between expert and non-expert data.

Result: The study evaluates the relationship between data distributions and algorithm applicability, highlighting limitations of existing methods.

Conclusion: The findings advance robust and practical IfO techniques, paving the way for scalable behavior learning.
...
TL;DR: The paper proposes an Adaptive Physics-Informed Neural Network-based Observer (PINN-Obs) for accurate state estimation in nonlinear systems, outperforming traditional methods in accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: State estimation in nonlinear systems is challenging with partial/noisy measurements, requiring innovative solutions beyond traditional model-based observers.

Method: The framework integrates system dynamics and sensor data into a physics-informed learning process, adaptively learning an optimal gain matrix for convergence.

Result: Theoretical analysis shows uniform error minimization under mild conditions, with simulations confirming superior accuracy and robustness.

Conclusion: PINN-Obs is a promising solution for nonlinear state estimation, validated by its performance across diverse systems.
...
TL;DR: The MAD framework combines physical laws with data-driven learning to solve differential equations efficiently without relying on costly labeled datasets.


<details>
  <summary>Details</summary>
Motivation: Current methods for solving differential equations either require expensive labeled data or face efficiency-accuracy trade-offs.

Method: MAD generates physics-embedded analytical solutions and synthetic data, eliminating the need for experimental or simulated training data.

Result: MAD demonstrates superior efficiency and accuracy in 2D parametric problems, handling complex parameter spaces.

Conclusion: MAD has the potential to become a universal paradigm for physics-informed machine intelligence in scientific computing.
...
TL;DR: Proposes a smooth, trainable parametric activation function (Leaky Exponential Linear Unit) for better performance in nonlinear data regression, addressing issues of vanishing gradients and discontinuity in existing functions. Introduces a diffusion-loss metric to evaluate overfitting.


<details>
  <summary>Details</summary>
Motivation: To improve performance in multidimensional nonlinear data regression by addressing limitations of existing activation functions (e.g., vanishing gradients, discontinuity) and reducing overfitting.

Method: Introduces a smooth, trainable parametric activation function (Leaky Exponential Linear Unit) and a novel diffusion-loss metric to evaluate model performance.

Result: Demonstrates improved performance over traditional activation functions (e.g., ELU, SiLU, ReLU) in terms of overfitting and sensitivity to model parameters.

Conclusion: The proposed activation function and diffusion-loss metric enhance regression performance and provide better evaluation of model overfitting.
...
TL;DR: The paper introduces interpretable topological generalization bounds for stochastic optimization algorithms, avoiding intractable mutual information terms by leveraging trajectory stability and topological data analysis (TDA).


<details>
  <summary>Details</summary>
Motivation: Existing topological generalization bounds rely on complex information-theoretic terms, limiting their practicality. The paper aims to provide clearer, more applicable bounds.

Method: Proposes a framework using trajectory stability, extending hypothesis set stability, and bounds generalization error with TDA-based complexity measures.

Result: The TDA terms in the bound are empirically shown to be significant, especially with larger training datasets, validating the approach.

Conclusion: The work offers practical and interpretable topological generalization bounds, explaining their empirical success and relevance.
...
TL;DR: The paper presents an imitation learning method for constrained maximum entropy policies, leveraging KL-divergence bounds and probabilistic inference, optimized via dual gradient descent.


<details>
  <summary>Details</summary>
Motivation: To learn policies that mimic expert behavior while adhering to constraints and maximizing entropy, ensuring robustness and generalization.

Method: Uses KL-divergence bounds and probabilistic inference, optimizing with dual gradient descent for stable training.

Result: Effective policy models are learned, handling multiple constraint types and generalizing across behavior modalities.

Conclusion: The method successfully integrates constraints and entropy maximization, demonstrating generalization and stability in training.
...
TL;DR: A novel speech tokenizer is introduced, encoding both linguistic and acoustic features, enhancing fidelity in speech processing tasks.


<details>
  <summary>Details</summary>
Motivation: Existing tokenizers often overlook acoustic features, limiting their effectiveness in preserving prosodic and emotional content.

Method: An advanced approach combining linguistic and acoustic information is proposed, using residual vector quantization (RVQ) with semantic and acoustic elements.

Result: The method improves speech representation fidelity and performs well in tasks like speech coding, voice conversion, and emotion recognition without extra training.

Conclusion: The tokenizer is versatile and promising for advancing AI-driven speech processing.
...
<details>
  <summary>Details</summary>
Motivation: To improve federated learning by leveraging intrinsic training signals for model merging without requiring modifications to architecture or loss functions.

Method: Uses variance-weighted classifier aggregation and SHAP-based LoRA merging to combine client models.

Result: Achieves state-of-the-art performance on benchmarks while integrating seamlessly with existing FL methods.

Conclusion: Demonstrates effective model merging using existing training signals, setting a new paradigm for federated model aggregation.
...
TL;DR: The paper analyzes prototype models (ProtoPNet, ProtoPool, PIPNet) for explainable AI, introduces new interpretability metrics, and tests them on diverse datasets.


<details>
  <summary>Details</summary>
Motivation: To enhance understanding and evaluation of prototype models in explainable AI by proposing new metrics and testing on varied datasets.

Method: In-depth analysis of prototype models using standard and newly proposed metrics, applied to fine-grained, Non-IID, and multi-label datasets.

Result: Comprehensive evaluation of prototype models, with open-source code provided for easy application and extensibility.

Conclusion: The study advances prototype model analysis, offering tools for better interpretability assessment and practical application.
...
TL;DR: HeLo is a multi-modal emotion distribution learning framework that leverages cross-attention and optimal transport to address heterogeneity and label correlation in emotion recognition.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with heterogeneity in multi-modal data and underutilize semantic correlations across emotions.

Method: HeLo uses cross-attention for data fusion, an OT-based module for heterogeneity mining, and learnable label embeddings for correlation learning.

Result: Outperforms existing methods on two public datasets.

Conclusion: HeLo effectively addresses challenges in multi-modal emotion distribution learning.
...
Result: The reference agent reaches the top 0.003% of the human leaderboard in 36 hours on a single H100 GPU.

Conclusion: The environment and baseline agent offer a modular and competitive benchmark for RTS and multi-agent reinforcement learning.
...
### [189] [Scalable Gaussian Processes: Advances in Iterative Methods and Pathwise Conditioning](https://arxiv.org/abs/2507.06839)
*Jihao Andreas Lin*

Main category: cs.LG

TL;DR: The paper addresses scalability issues in Gaussian processes by combining iterative methods and pathwise conditioning, reducing memory needs and leveraging modern hardware.


<details>
  <summary>Details</summary>
Motivation: Gaussian processes struggle with scalability for large datasets and modern parallel hardware, prompting the need for improved methods.

Method: Uses iterative linear system solvers and pathwise conditioning to express computations as linear equations, optimizing for matrix multiplication.

Result: Reduces memory requirements and enables application to larger datasets, while being efficient on modern hardware.

Conclusion: The synergistic combination of iterative methods and pathwise conditioning enhances Gaussian processes for large-scale use.
...
### [190] [DiffSpectra: Molecular Structure Elucidation from Spectra using Diffusion Models](https://arxiv.org/abs/2507.06853)
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods for molecular structure elucidation are unscalable and rely on expert interpretation, while existing machine learning methods struggle with generalization and integrating diverse spectral data.
...
Result: The model achieves 16.01% top-1 and 96.86% top-20 accuracy in recovering exact structures, benefiting from 3D geometric modeling and multi-modal conditioning.

Conclusion: DiffSpectra is the first framework to unify multi-modal spectral reasoning and joint 2D/3D generative modeling, demonstrating the effectiveness of diffusion models in molecular structure elucidation.
...
### [191] [Episodic Contextual Bandits with Knapsacks under Conversion Models](https://arxiv.org/abs/2507.06859)
*Zitian Li,Wang Chi Cheung*

Main category: cs.LG

TL;DR: The paper studies an online decision-making problem with contextual bandits and knapsacks (BwK), where episodes have varying resources and non-stationary contexts. It proposes an algorithm with sub-linear regret, leveraging a confidence bound oracle, and addresses challenges like unbounded state spaces.


<details>
  <summary>Details</summary>
Motivation: The work aims to model real-world applications like dynamic pricing and auctions, where resources are replenished episodically and contexts change over time.

Method: The authors design an online algorithm assuming access to a confidence bound oracle, which helps achieve sub-linear regret. They tackle the challenge of unbounded state spaces due to many possible contexts.
...
Conclusion: The framework advances contextual BwK literature by handling non-stationary contexts and leveraging unlabeled data, offering practical solutions for dynamic resource allocation.
...
### [192] [Squeeze the Soaked Sponge: Efficient Off-policy Reinforcement Finetuning for Large Language Model](https://arxiv.org/abs/2507.06892)
*Jing Liang,Hongyao Tang,Yi Ma,Jinyi Liu,Yan Zheng,Shuyue Hu,Lei Bai,Jianye Hao*

Main category: cs.LG

TL;DR: ReMix, an off-policy RL method, enhances efficiency in Reinforcement Finetuning (RFT) for LLMs by leveraging past data, reducing training costs significantly while maintaining high performance.


<details>
  <summary>Details</summary>
Motivation: Existing RFT methods are on-policy, wasting past data and increasing compute/time costs. ReMix aims to address this inefficiency.

Method: ReMix combines mix-policy proximal policy gradient, KL-Convex policy constraint, and policy reincarnation to utilize off-policy data effectively.

Result: ReMix achieves SOTA performance with 30x-450x cost reduction, scoring 52.10%-64.39% accuracy on math benchmarks.

Conclusion: ReMix revolutionizes RFT by enabling off-policy learning, balancing stability and flexibility, and reducing training costs.
...
TL;DR: The paper introduces a federated causal discovery method for both horizontal and vertical federated settings, using higher-order cumulants to address spurious relationships and improve accuracy.
...
Result: The algorithm outperforms others in experiments on synthetic and real-world data, accurately reconstructing causal graphs and estimating causal strengths.

Conclusion: The proposed method effectively addresses limitations of existing federated causal discovery techniques, proving robust in diverse settings.
...
TL;DR: The paper introduces RL-Window, a reinforcement learning-based method to dynamically optimize sliding window sizes for multi-dimensional data streams, outperforming existing techniques in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Multi-dimensional data streams are challenging due to their dynamic nature and complex dependencies, requiring adaptive solutions beyond fixed-size windows.

Method: The paper proposes RL-Window, using a Dueling Deep Q-Network (DQN) with prioritized experience replay to learn adaptive window sizes based on stream characteristics.

Result: RL-Window outperforms state-of-the-art methods (ADWIN, CNN-Adaptive) in classification accuracy, drift robustness, and computational efficiency on benchmark datasets.

Conclusion: RL-Window is adaptable, stable, and efficient, making it suitable for real-time applications with multi-dimensional data streams.
...
Method: Uses Failure Mode and Effects Analysis (FMEA) to assess risks and assign dynamic weights in a three-version NVML system, tested against FGSM and PGD attacks.

Result: NVML significantly improves robustness and safety of traffic sign recognition under adversarial conditions.

Conclusion: The NVML framework effectively mitigates adversarial threats, enhancing the safety of autonomous driving systems.
...
TL;DR: The paper proposes DICE, a method to estimate data influence cascades in decentralized learning, addressing the lack of fair incentives for participation.
...
Method: The authors design DICE, a method to estimate data influence cascades, considering data, communication topology, and loss landscape curvature.
...
Conclusion: DICE offers a foundational solution for fair incentive mechanisms in decentralized learning, with practical applications.
...
TL;DR: Foundation models excel in training tasks but often fail to develop deeper inductive biases aligned with underlying world models, as shown by their poor generalization in new tasks.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether foundation models truly capture deeper domain understanding beyond sequence prediction, akin to how Kepler's predictions led to Newtonian mechanics.

Method: Developed an inductive bias probe technique, testing models on synthetic datasets generated from postulated world models to measure alignment of inductive biases.

Result: Foundation models perform well in training tasks but fail to align with underlying world models in new tasks, e.g., not applying Newtonian mechanics in physics tasks.

Conclusion: Foundation models may rely on task-specific heuristics rather than developing generalizable understanding, highlighting limitations in their deeper structural learning.
...
<details>
  <summary>Details</summary>
Motivation: Understanding the parameter requirements for PINNs to perform effectively with noisy data, as real-world applications often involve noisy samples.

Method: Proves a theoretical lower bound on neural network size for PINNs to achieve empirical risk below noise variance and validates empirically with the Hamilton–Jacobi–Bellman (HJB) PDE.

Result: Shows that increasing noisy labels alone doesn't reduce empirical risk and provides conditions for PINNs to achieve risks below noise variance.

Conclusion: The study establishes a foundation for quantifying PINN training requirements in noisy settings, with implications for practical applications.
...
TL;DR: The paper introduces a unified framework for interpreting and calibrating differentially private (DP) mechanisms, addressing inconsistencies in existing methods for mapping privacy parameters to concrete risks like re-identification, attribute inference, and data reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for evaluating DP mechanisms are overly pessimistic and inconsistent, making it difficult to interpret and calibrate privacy risks accurately.

Method: The authors use the hypothesis-testing interpretation of DP (f-DP) to derive unified bounds on attack success, applicable across re-identification, attribute inference, and data reconstruction risks.

Result: The proposed bounds are consistent, tunable, and empirically tighter than prior methods (e.g., ε-DP, Rényi DP), reducing required noise by 20% and improving accuracy by over 15pp in a text classification task.

Conclusion: The work provides a principled framework for interpreting and calibrating DP protection levels against specific privacy risks, offering practical improvements over existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current contrastive learning methods handle multiple views suboptimally, leading to conflicting objectives, limited interactions, and alignment-uniformity coupling issues.
...
Result: Empirical results show superior performance on ImageNet1K and other datasets, with MV-DHEL mitigating dimensionality collapse effectively.

Conclusion: The methods extend multi-view contrastive learning principles, enabling better scalability and performance, especially with more views.
...
<details>
  <summary>Details</summary>
Motivation: Privacy concerns and regulatory restrictions limit EHR data sharing, necessitating synthetic datasets that mimic raw EHRs more closely than existing methods.
...
Result: Validated on two datasets, RawMed outperforms baseline models in fidelity and utility, assessed via distributional similarity, inter-table relationships, temporal dynamics, and privacy.

Conclusion: RawMed provides a novel solution for synthetic EHR generation, balancing fidelity, utility, and privacy, with code publicly available.
...
TL;DR: The paper evaluates the accuracy of diffusion models in solving Gaussian deblurring tasks by comparing theoretical and practical results using Wasserstein distance.


<details>
  <summary>Details</summary>
Motivation: To address unresolved questions about the performance of diffusion models in Bayesian inverse problems, specifically for deblurring Gaussian data.

Method: Analyzes the discrepancy between theoretical and diffusion-based solutions by computing the exact Wasserstein distance between model outputs and ideal distributions.

Result: Provides a precise comparison of different algorithms in the literature for solving inverse problems.

Conclusion: The study offers insights into the accuracy and limitations of diffusion models in constrained contexts like Gaussian deblurring.
...
TL;DR: The paper explores edge-side model training on resource-limited smart meters, focusing on grid-edge intelligence and on-device training, with a case study on photovoltaic power forecasting using gradient boosting trees and RNNs.


<details>
  <summary>Details</summary>
Motivation: To achieve grid-edge intelligence economically using existing smart meter infrastructures, despite resource limitations.

Method: Describes technical preparations for on-device training, introduces mixed- and reduced-precision training schemes, and tests gradient boosting trees and RNNs for photovoltaic power forecasting.

Result: Demonstrates feasibility of economically achieving grid-edge intelligence with advanced metering infrastructures.

Conclusion: Edge-side model training is viable for resource-limited smart meters, enabling grid-edge intelligence.
...
TL;DR: PLAME is a novel MSA design model that enhances protein structure prediction for low-homology and orphan proteins using evolutionary embeddings and a conservation-diversity loss, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current protein folding models rely heavily on MSAs, limiting their effectiveness for low-homology and orphan proteins with sparse MSA data. PLAME addresses this gap.

Method: PLAME leverages pretrained protein language model embeddings, introduces a conservation-diversity loss, and proposes a novel MSA selection method and sequence quality metric.

Result: PLAME achieves state-of-the-art performance on low-homology and orphan proteins, improving AlphaFold2 and AlphaFold3 predictions, and matches AlphaFold2 accuracy with ESMFold's speed.

Conclusion: PLAME effectively enhances MSA quality and folding performance, offering a robust solution for proteins with limited evolutionary data.
...
<details>
  <summary>Details</summary>
Motivation: Existing semantic caching frameworks rely on single embedding models, limiting their ability to capture diverse semantic relationships in real-world queries.

Method: Proposes an ensemble embedding approach using multiple models combined via a trained meta-encoder, evaluated on the QQP dataset.

Result: Achieves a 92% cache hit ratio for equivalent queries and 85% accuracy in rejecting non-equivalent ones, outperforming single-model methods.

Conclusion: Ensemble embedding significantly enhances semantic caching performance, reducing computational overhead in LLM systems.
...
TL;DR: The paper introduces the Dual-Balance Collaborative Experts (DCE) framework to address intra-domain class imbalance and cross-domain class distribution shifts in Domain-Incremental Learning (DIL).


<details>
  <summary>Details</summary>
Motivation: DIL faces challenges like intra-domain class imbalance and cross-domain shifts, which degrade model performance by underfitting few-shot classes and disrupting knowledge transfer.

Method: DCE uses a frequency-aware expert group with specialized loss functions and a dynamic expert selector that synthesizes pseudo-features via balanced Gaussian sampling.

Result: Experiments on four benchmarks show DCE achieves state-of-the-art performance.

Conclusion: DCE effectively balances knowledge preservation and adaptation, improving few-shot class performance in DIL.
...
TL;DR: Small batch sizes in language model training can be stable and effective with proper hyperparameter scaling, outperforming larger batches in some cases.


<details>
  <summary>Details</summary>
Motivation: Challenging the belief that small batch sizes are unstable, the study explores their potential benefits and proposes scaling rules for Adam hyperparameters.

Method: Revisiting small batch sizes (down to size one) and scaling Adam hyperparameters accordingly, while comparing performance with larger batches.

Result: Small batch sizes train stably, are robust to hyperparameters, perform well per-FLOP, and enable stable SGD training without optimizer state.

Conclusion: Recommends small batch sizes with adjusted hyperparameters and advises against gradient accumulation unless necessary for multi-device training.
...
TL;DR: Compositional generalization in vision models is driven by data diversity, not just scale, and requires linearly factored representations for efficiency.
...
Result: Compositional generalization improves with data diversity, not scale. Linearly factored representations enable efficient learning. Pretrained models show partial success.

Conclusion: Diverse datasets and structured representations are key for compositional generalization in vision models.
...
### [210] [DS@GT at CheckThat! 2025: Exploring Retrieval and Reranking Pipelines for Scientific Claim Source Retrieval on Social Media Discourse](https://arxiv.org/abs/2507.06563)
*Jeanette Schofield,Shuyu Tian,Hoang Thanh Thanh Truong,Maximilian Heil*

Main category: cs.IR

TL;DR: The paper describes a method for retrieving scientific papers based on implicit references in tweets, achieving an MRR@5 of 0.58, ranking 16th out of 30 teams.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of verifying scientific claims in social media posts without explicit citations.

Method: Explored 6 data augmentation techniques, 7 retrieval/reranking pipelines, and finetuned a bi-encoder.

Result: Achieved an MRR@5 of 0.58, improving the BM25 baseline by 0.15.

Conclusion: The approach shows promise for retrieving scientific sources from implicit references in tweets, with room for further improvement.
...
### [211] [GR-LLMs: Recent Advances in Generative Recommendation Based on Large Language Models](https://arxiv.org/abs/2507.06507)
*Zhen Yang,Haitao Lin,Jiawei xue,Ziji Zhang*

Main category: cs.IR

TL;DR: A survey on LLM-based Generative Recommendations (GRs) highlighting their advancements, applications, industrial considerations, and future directions.


<details>
  <summary>Details</summary>
Motivation: To facilitate further research in LLM-based GRs by providing a comprehensive overview of their potential to outperform traditional recommendation systems.

Method: Outlines preliminaries, application cases, industrial considerations, and future directions for LLM-based GRs.

Result: Identifies LLM-based GRs as a promising paradigm shift in recommendation systems.

Conclusion: The survey aims to advance the GR domain by summarizing current progress and suggesting future research directions.
...
### [212] [Temporal Information Retrieval via Time-Specifier Model Merging](https://arxiv.org/abs/2507.06782)
*SeungYoon Han,Taeho Hwang,Sukmin Cho,Soyeong Jeong,Hoyun Song,Huije Lee,Jong C. Park*

Main category: cs.IR

TL;DR: Proposes Time-Specifier Model Merging (TSM) to improve temporal retrieval without losing performance on non-temporal queries.


<details>
  <summary>Details</summary>
Motivation: Existing dense retrieval methods underperform on queries with temporal constraints, and current TIR approaches suffer from catastrophic forgetting.

Method: TSM trains specialized retrievers for individual time specifiers and merges them into a unified model.

Result: TSM significantly improves performance on temporal queries while maintaining accuracy on non-temporal ones, outperforming baselines.

Conclusion: TSM effectively addresses the limitations of current methods, enhancing temporal retrieval without compromising general query performance.
...
### [213] [OpenDPDv2: A Unified Learning and Optimization Framework for Neural Network Digital Predistortion](https://arxiv.org/abs/2507.06849)
*Yizhuo Wu,Ang Li,Chang Gao*

Main category: eess.SP

TL;DR: OpenDPDv2 reduces power consumption in NN-based DPDs while maintaining high linearization performance using TRes-DeltaGRU and energy-efficient methods.


<details>
  <summary>Details</summary>
Motivation: NN-based DPDs improve signal quality but consume significant energy due to large parameters. OpenDPDv2 aims to reduce power while maintaining performance.

Method: Introduces TRes-DeltaGRU-DPD and uses fixed-point quantization and dynamic temporal sparsity to optimize energy efficiency.

Result: Achieves -59.4 dBc ACPR and -42.1 dB EVM (FP32). With optimizations, reduces energy by 4.5X while maintaining -50.3 dBc ACPR and -35.2 dB EVM.

Conclusion: OpenDPDv2 effectively balances performance and energy efficiency, with publicly available resources for further use.
...
### [214] [Federated Learning-based MARL for Strengthening Physical-Layer Security in B5G Networks](https://arxiv.org/abs/2507.06997)
*Deemah H. Tashman,Soumaya Cherkaoui,Walaa Hamouda*

Main category: eess.SP

TL;DR: The paper proposes a federated learning-based MARL strategy for enhancing PLS in multi-cellular beyond 5G networks, comparing DQN and RDPG methods. RDPG shows faster convergence and better performance than distributed DRL.


<details>
  <summary>Details</summary>
Motivation: To improve physical-layer security in multi-cellular networks by leveraging federated learning and MARL, addressing eavesdropping threats.

Method: Uses federated MARL with DRL agents (DQN and RDPG) at each BS to maximize secrecy rates, sharing only parameters, not private data.

Result: RDPG converges faster than DQN and outperforms distributed DRL, with a noted trade-off between security and complexity.

Conclusion: Federated MARL with RDPG is effective for PLS in beyond 5G networks, balancing performance and complexity.
...
### [215] [How to Bridge the Sim-to-Real Gap in Digital Twin-Aided Telecommunication Networks](https://arxiv.org/abs/2507.07067)
*Clement Ruah,Houssem Sifaou,Osvaldo Simeone,Bashir M. Al-Hashimi*

Main category: eess.SP

TL;DR: The paper addresses the challenge of training AI models for telecommunications by using digital twinning to generate site-specific data and reviews methods to bridge the sim-to-real gap.


<details>
  <summary>Details</summary>
Motivation: Scarcity of deployment-specific data in telecommunications makes AI training difficult, and existing datasets often lack contextual variability. Digital twinning offers a solution but requires addressing the sim-to-real gap.

Method: The paper reviews two strategies: 1) calibrating digital twins with real-world measurements, and 2) sim-to-real gap-aware training methods, including Bayesian learning and prediction-powered inference.

Result: The study evaluates methods to handle discrepancies between synthetic and real data, focusing on environment-level and training loss-level modeling.

Conclusion: Digital twinning combined with sim-to-real gap mitigation strategies can enhance AI model training for telecommunications, despite inherent challenges.
...
### [216] [Beyond Connectivity: An Open Architecture for AI-RAN Convergence in 6G](https://arxiv.org/abs/2507.06911)
*Michele Polese,Niloofar Mohamadi,Salvatore D'Oro,Tommaso Melodia*

Main category: cs.NI

TL;DR: The paper proposes a converged O-RAN and AI-RAN architecture to enable distributed AI workloads at the network edge, leveraging existing infrastructure for monetization.


<details>
  <summary>Details</summary>
Motivation: The rise of data-intensive AI applications at the edge necessitates a shift in RAN design to support distributed AI workloads, offering monetization opportunities for network operators.

Method: Introduces a novel architecture with AI-RAN Orchestrator and AI-RAN sites, extending O-RAN principles for modularity, disaggregation, and cloud-nativeness to support heterogeneous AI deployments.

Result: The system enables flexible deployment of AI workloads with real-time or batch processing, maintaining open interfaces and multi-vendor interoperability.

Conclusion: The proposed architecture successfully addresses orchestration challenges for heterogeneous workloads at the edge while leveraging existing infrastructure.
...
### [217] [DeepRetro: Retrosynthetic Pathway Discovery using Iterative LLM Reasoning](https://arxiv.org/abs/2507.07060)
...
<details>
  <summary>Details</summary>
Motivation: Retrosynthesis faces challenges in discovering novel pathways beyond predefined templates, and existing LLM approaches struggle with multi-step planning.

Method: DeepRetro integrates template-based/Monte Carlo tree search tools with LLMs in a step-wise, feedback-driven loop, including validity checks and recursive refinement.

Result: The framework identifies viable and novel retrosynthetic routes, validated through benchmarks and case studies, with human-in-the-loop feedback.

Conclusion: DeepRetro demonstrates the potential of iterative LLM reasoning to advance complex chemical syntheses, generating novel pathways for natural products.
...
### [218] [Self-supervised learning predicts plant growth trajectories from multi-modal industrial greenhouse data](https://arxiv.org/abs/2507.06336)
*Adam J Riesselman,Evan M Cofer,Therese LaRue,Wim Meeussen*

Main category: q-bio.QM

TL;DR: A robotic platform and self-supervised modeling are used to predict plant growth in hydroponic systems, improving agronomic research and operational efficiency.


<details>
  <summary>Details</summary>
Motivation: Quantifying plant phenotypes at scale is challenging but crucial for optimizing crop production.

Method: A mobile robotic platform captures high-resolution environmental and phenotyping data, and a self-supervised model maps observed data to growth trajectories.

Result: The approach successfully forecasts future plant height and harvest mass.

Conclusion: This method advances robotic automation and machine learning in agriculture, offering actionable insights.
...
### [219] [PAST: A multimodal single-cell foundation model for histopathology and spatial transcriptomics in cancer](https://arxiv.org/abs/2507.06418)
*Changchun Yang,Haoyang Li,Yushuai Wu,Yilan Zhang,Yifeng Jiao,Yu Zhang,Rihan Huang,Yuan Cheng,Yuan Qi,Xin Guo,Xin Gao*

Main category: q-bio.QM

TL;DR: PAST is a pan-cancer single-cell foundation model integrating histopathology images and single-cell transcriptomes, enabling cross-modal analysis for precision oncology.


<details>
  <summary>Details</summary>
Motivation: Existing pathology foundation models lack integration with molecular data at single-cell resolution, limiting their use in precision oncology.
...
Result: PAST outperforms existing methods in tasks like gene expression prediction, virtual molecular staining, and survival analysis.

Conclusion: PAST sets a new standard for pathology foundation models, enhancing spatial omics and precision cancer research.
...
### [220] [The Emotional Alignment Design Policy](https://arxiv.org/abs/2507.06263)
*Eric Schwitzgebel,Jeff Sebo*

Main category: cs.CY

TL;DR: The paper introduces the Emotional Alignment Design Policy, advocating for artificial entities to evoke emotions matching their capacities and moral status, addressing challenges like user autonomy and ethical dilemmas.


<details>
  <summary>Details</summary>
Motivation: To ensure artificial entities evoke appropriate emotional responses, avoiding overshooting, undershooting, or misaligned reactions, while navigating ethical and practical challenges.

Method: Proposes the Emotional Alignment Design Policy, identifying two violation types (overshooting/undershooting and wrong target) and discussing implementation challenges.

Result: Highlights key challenges: balancing user autonomy, handling expert/public disagreement, addressing moral status of entities, and managing user assumptions.

Conclusion: The policy is theoretically appealing but faces practical hurdles in implementation, requiring careful consideration of ethical and user-centric factors.
...
### [221] [A Collectivist, Economic Perspective on AI](https://arxiv.org/abs/2507.06268)
*Michael I. Jordan*

Main category: cs.CY

TL;DR: The paper critiques the current focus on data and cognitive intelligence in technology, advocating for integrating social and economic concepts to prioritize social welfare in system design.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the oversight of social and cultural intelligence in technology development and the neglect of social consequences.

Method: The method involves blending economic and social concepts with computational and inferential ideas to create human-centric system designs.

Result: The result is a call for a new engineering field that prioritizes social welfare as a core objective.

Conclusion: The conclusion emphasizes the need for a shift toward human-centric technology that integrates social and economic considerations.
...
### [222] [The Prompt War: How AI Decides on a Military Intervention](https://arxiv.org/abs/2507.06277)
*Maxim Chupilkin*

Main category: cs.CY

TL;DR: The paper identifies domestic support and probability of success as the strongest predictors of AI decisions for military intervention, with costs like international condemnation and casualties having lesser but significant effects.


<details>
  <summary>Details</summary>
Motivation: To understand the key drivers behind AI decisions in military interventions, given the increasing use of AI in war games and planning.

Method: A conjoint experiment with 640 vignettes, each run 100 times, to systematically analyze AI decisions across scenarios using models like OpenAI GPT, Anthropic Claude, and Google Gemini.

Result: High domestic support and high probability of success are the top predictors. Costs (e.g., international condemnation, deaths) are significant but less impactful. The window of opportunity is only significant in interactions.

Conclusion: AI decision-making in military interventions consistently prioritizes domestic support and success probability, with costs playing a secondary role, suggesting a pattern across models.
...
### [223] [Too Human to Model:The Uncanny Valley of LLMs in Social Simulation -- When Generative Language Agents Misalign with Modelling Principles](https://arxiv.org/abs/2507.06310)
*Yongchao Zeng,Calum Brown,Mark Rounsevell*

Main category: cs.CY

TL;DR: LLM agents enhance realism in social simulations but conflict with modeling's need for abstraction and interpretability, revealing dilemmas like temporal mismatch and role consistency.


<details>
  <summary>Details</summary>
Motivation: To explore the compatibility of LLM agents' realism with the epistemic foundations of modeling, identifying challenges and ideal use cases.

Method: A thought experiment converting the Bass diffusion model to an LLM-based variant, uncovering five core dilemmas.

Result: LLM agents can obscure social dynamics due to their expressiveness, leading to an uncanny valley between abstraction and realism.

Conclusion: LLM agents are best suited for scenarios focusing on linguistic nuances and natural interactions, not system-level emergence, requiring repositioning in social simulation.
...
### [224] [Deprecating Benchmarks: Criteria and Framework](https://arxiv.org/abs/2507.06434)
*Ayrton San Joaquin,Rokas Gipiškis,Leon Staufer,Ariel Gil*

Main category: cs.CY

TL;DR: Proposes criteria and a framework for deprecating AI benchmarks to ensure rigorous evaluations and avoid misleading results.


<details>
  <summary>Details</summary>
Motivation: Addresses the lack of guidance on when and how to deprecate benchmarks, which can lead to over-valuing model capabilities or obscuring safety issues.

Method: Reviews benchmarking practices and proposes criteria and a framework for deprecation.

Result: Provides actionable recommendations for benchmark developers, users, and governance actors.

Conclusion: Aims to improve benchmarking quality for frontier AI models and benefit stakeholders in AI governance and policy.
...
### [225] [Assessing the Prevalence of AI-assisted Cheating in Programming Courses: A Pilot Study](https://arxiv.org/abs/2507.06438)
*Kaléu Delphino*

Main category: cs.CY

TL;DR: The paper explores AI plagiarism in CS education, finding surveys effective but interviews less so.


<details>
  <summary>Details</summary>
Motivation: Assess the threat of AI tools like ChatGPT to CS education by measuring student plagiarism.

Method: Conducted a pilot study with surveys (n=120) and interviews in a large CS class.

Result: Over 25% admitted to AI plagiarism in surveys, but only one student agreed to an interview.

Conclusion: Surveys are effective for studying AI plagiarism; interviews need redesign to encourage participation.
...
### [226] [Winning and losing with Artificial Intelligence: What public discourse about ChatGPT tells us about how societies make sense of technological change](https://arxiv.org/abs/2507.06876)
*Adrian Rauchfleisch,Joshua Philip Suarez,Nikka Marie Sales,Andreas Jungherr*

Main category: cs.CY

TL;DR: The paper examines public reactions to ChatGPT's launch via Twitter, revealing how economic interests and cultural values shape engagement and sentiment.


<details>
  <summary>Details</summary>
Motivation: To understand how societies react to AI advancements, focusing on economic and cultural influences.

Method: Analyzed 3.8 million tweets from 1.6 million users across 117 countries, using occupational skill types and Hofstede's cultural dimensions.

Result: Technical occupations engaged earlier with positive stances, while writing-centric roles joined later skeptically. Individualism linked to earlier but negative engagement, and uncertainty avoidance reduced positivity.

Conclusion: Occupational and cultural contexts are crucial in shaping public reactions to AI, with sentiment shifts driven by new, skeptical voices.
...
### [227] [MixAssist: An Audio-Language Dataset for Co-Creative AI Assistance in Music Mixing](https://arxiv.org/abs/2507.06329)
*Michael Clemens,Ana Marasović*

Main category: cs.SD

TL;DR: MixAssist is a dataset for audio-language models to improve AI assistance in music mixing by capturing expert-amateur dialogues.


<details>
  <summary>Details</summary>
Motivation: Current AI research overlooks collaborative and instructional aspects in music mixing, leaving amateurs underserved.

Method: MixAssist dataset includes 431 audio-grounded conversational turns from 7 sessions with 12 producers.

Result: Fine-tuning models like Qwen-Audio on MixAssist shows promising results, with Qwen outperforming others in generating relevant advice.

Conclusion: MixAssist enables AI assistants to support co-creative music mixing by focusing on audio-grounded instruction.
...
### [228] [Exploring State-Space-Model based Language Model in Music Generation](https://arxiv.org/abs/2507.06674)
*Wei-Jaw Lee,Fang-Chih Hsieh,Xuanjun Chen,Fang-Duo Tsai,Yi-Hsuan Yang*

Main category: cs.SD

TL;DR: SiMBA, a Mamba-based decoder, outperforms Transformers in text-to-music generation under limited resources, offering faster convergence and higher fidelity outputs.


<details>
  <summary>Details</summary>
Motivation: To explore Mamba-based architectures for text-to-music generation, leveraging their efficiency and potential as alternatives to Transformers.

Method: Adopts RVQ discrete tokens, uses a single-layer codebook for semantic music representation, and adapts SiMBA (a Mamba-based encoder) as a decoder. Compares it against a Transformer-based decoder.

Result: SiMBA achieves faster convergence and generates outputs closer to ground truth in limited-resource settings.

Conclusion: SSMs like SiMBA show promise for efficient and expressive text-to-music generation, outperforming Transformers in certain scenarios.
...
### [229] [Advances in Intelligent Hearing Aids: Deep Learning Approaches to Selective Noise Cancellation](https://arxiv.org/abs/2507.07043)
*Haris Khan,Shumaila Asif,Hassan Nasir*

Main category: cs.SD

TL;DR: This review explores AI-driven selective noise cancellation in hearing aids, covering advancements, challenges, and future directions, with notable improvements in performance and real-time processing.


<details>
  <summary>Details</summary>
Motivation: To evaluate the shift from traditional hearing aids to AI-driven, context-aware systems, addressing gaps in technology and implementation.

Method: Systematic literature review synthesizing deep learning architectures, hardware strategies, clinical studies, and user-centric designs.

Result: Recent models show significant gains (up to 18.3 dB SI-SDR improvement) and sub-10 ms real-time performance, but challenges like power constraints and personalization persist.

Conclusion: Future research should focus on lightweight models, continual learning, and clinical translation to advance AI-enhanced hearing solutions.
...
### [230] [A Novel Hybrid Deep Learning Technique for Speech Emotion Detection using Feature Engineering](https://arxiv.org/abs/2507.07046)
*Shahana Yasmin Chowdhury,Bithi Banik,Md Tamjidul Hoque,Shreya Banerjee*

Main category: cs.SD

TL;DR: The paper proposes a DCRF-BiLSTM model for speech emotion recognition (SER), achieving high accuracy across five datasets and demonstrating robustness and generalizability.


<details>
  <summary>Details</summary>
Motivation: Speech emotion recognition is crucial for human-computer interaction and AI development, but existing models lack evaluation across multiple benchmark datasets.

Method: The DCRF-BiLSTM model is trained on five datasets (RAVDESS, TESS, SAVEE, EmoDB, Crema-D) to recognize seven emotions.

Result: The model achieves high accuracy (up to 100%) on individual datasets and 93.76% on the combined dataset, outperforming previous results.

Conclusion: The DCRF-BiLSTM framework is robust and generalizable, making it suitable for diverse SER applications.
...
### [231] [Comparative Analysis of CNN and Transformer Architectures with Heart Cycle Normalization for Automated Phonocardiogram Classification](https://arxiv.org/abs/2507.07058)
*Martin Sondermann,Pinar Bisgin,Niklas Tschorn,Anja Burmann,Christoph M. Friedrich*

Main category: cs.SD

TL;DR: The paper compares four models for heart murmur detection, finding CNNs outperform BEATs transformers, but BEATs offer efficiency benefits.


<details>
  <summary>Details</summary>
Motivation: To advance cardiovascular diagnostics by evaluating automated PCG classification models.

Method: Comparison of two CNNs and two BEATs transformers using fixed-length and heart cycle normalization on the PhysioNet2022 dataset.

Result: CNNs achieved higher AUROC (79.5% and 75.4%) than BEATs (65.7% and 70.1%).

Conclusion: Specialized CNNs are more accurate, but BEATs may be efficient for development, balancing accuracy and computational needs.
...
### [232] [Latent Acoustic Mapping for Direction of Arrival Estimation: A Self-Supervised Approach](https://arxiv.org/abs/2507.07066)
*Adrian S. Roman,Iran R. Roman,Juan P. Bello*

Main category: cs.SD

TL;DR: The paper introduces Latent Acoustic Mapping (LAM), a self-supervised framework combining interpretability of traditional beamforming with the efficiency of deep learning for robust direction of arrival estimation.


<details>
  <summary>Details</summary>
Motivation: Traditional beamforming is computationally intensive and sensitive to variability, while deep learning lacks interpretability and requires large datasets. Both struggle with generalization across setups.

Method: LAM is a self-supervised model generating high-resolution acoustic maps, adaptable to varying conditions and array configurations.

Result: LAM matches or outperforms supervised methods on LOCATA and STARSS benchmarks and enhances supervised models when used as features.

Conclusion: LAM bridges interpretability and efficiency, advancing adaptive sound localization systems.
...
### [233] [A Survey of Multi Agent Reinforcement Learning: Federated Learning and Cooperative and Noncooperative Decentralized Regimes](https://arxiv.org/abs/2507.06278)
*Kemboi Cheruiyot,Nickson Kiprotich,Vyacheslav Kungurtsev,Kennedy Mugo,Vivian Mwirigi,Marvin Ngesa*

Main category: cs.MA

TL;DR: A survey of three interaction topologies in multi-agent AI systems: Federated RL, Decentralized RL, and Noncooperative RL, comparing their structures, guarantees, and performance.


<details>
  <summary>Details</summary>
Motivation: To address the growing complexity of multi-agent AI interactions by categorizing and analyzing three key interaction topologies.

Method: Review and comparison of state-of-the-art literature on Federated RL, Decentralized RL, and Noncooperative RL, including formulations, theoretical guarantees, and performance insights.

Result: Identifies structural similarities and distinctions, theoretical guarantees, and numerical performance highlights and limitations for each topology.

Conclusion: Provides a comprehensive overview of multi-agent RL interaction topologies, aiding future research and development in autonomous agent systems.
...
### [234] [Gradientsys: A Multi-Agent LLM Scheduler with ReAct Orchestration](https://arxiv.org/abs/2507.06520)
*Xinyuan Song,Zeyu Wang,Siyi Wu,Tianyu Shi,Lynn Ai*

Main category: cs.MA

TL;DR: Gradientsys is a multi-agent scheduling framework using LLM-powered scheduling, MCP, and ReAct-based planning for efficient, parallel, and transparent task execution.


<details>
  <summary>Details</summary>
Motivation: To address the need for efficient coordination of diverse AI agents while ensuring transparency, scalability, and robustness in task execution.

Method: Uses a typed Model-Context Protocol (MCP), ReAct-based dynamic planning, LLM-powered scheduling, and supports hybrid synchronous/asynchronous execution with retry-and-replan mechanisms.

Result: Outperforms MinionS-style baselines in task success rates, latency, and API costs on the GAIA benchmark.

Conclusion: Gradientsys demonstrates effective LLM-driven multi-agent orchestration, offering extensibility, parallelism, and observability.
...
TL;DR: ML models predict breast cancer treatment outcomes using TCGA data, with GBM performing best.


<details>
  <summary>Details</summary>
Motivation: Personalized BC treatment is complex due to heterogeneity; ML can aid decision-making.

Method: Used TCGA data, five-fold cross-validation, and performance metrics (accuracy, AUROC, etc.).

Result: GBM outperformed XGBoost and AdaBoost (accuracy = 0.7718, AUROC = 0.8252).

Conclusion: ML models like GBM can support personalized BC treatment decisions effectively.
...
TL;DR: A stochastic car-following model using deep neural networks and nonstationary Gaussian processes improves predictive performance and uncertainty quantification over traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional deterministic models fail to capture human driving variability and unpredictability, while modern approaches often ignore latent factors like driver intentions and perception errors.

Method: The study introduces a framework combining deep neural networks with nonstationary Gaussian processes, using a scenario-adaptive Gibbs kernel to model dynamic temporal correlations in acceleration decisions.

Result: The model outperforms conventional methods in predictive accuracy and interpretable uncertainty quantification, validated on the HighD dataset.

Conclusion: The framework is a promising tool for traffic analysis and safety-critical applications due to its balance of interpretability and accuracy.
...
### [237] [Conformal Prediction for Long-Tailed Classification](https://arxiv.org/abs/2507.06867)
*Tiffany Ding,Jean-Baptiste Fermanian,Joseph Salmon*

Main category: stat.ML

TL;DR: The paper addresses the challenge of long-tailed class distributions in classification problems, proposing methods to balance set size and class-conditional coverage in conformal prediction.


<details>
  <summary>Details</summary>
Motivation: Existing conformal prediction methods fail to provide both small prediction sets and good class-conditional coverage for rare classes in long-tailed distributions.
...
Result: The methods are tested on Pl@ntNet and iNaturalist datasets, showing improved trade-offs between set size and coverage.

Conclusion: The proposed methods offer a flexible solution for long-tailed classification problems, ensuring useful prediction sets without sacrificing coverage or practicality.
...
### [238] [On the Hardness of Unsupervised Domain Adaptation: Optimal Learners and Information-Theoretic Perspective](https://arxiv.org/abs/2507.06552)
*Zhiyi Dong,Zixuan Liu,Yongyi Mao*

Main category: stat.ML

TL;DR: The paper analyzes unsupervised domain adaptation (UDA) under covariate shift, introducing a new measure (PTLU) to quantify learning difficulty.


<details>
  <summary>Details</summary>
Motivation: To address the uncertainty in UDA by coupling source-target distributions and classifiers, moving beyond worst-case analyses.

Method: Defines a UDA class with ground-truth triples (p, q, f) and introduces PTLU, an information-theoretic measure, to evaluate learning difficulty.

Result: PTLU lower-bounds the risk of any learner, proving its utility as a proxy for UDA hardness. Examples show PTLU outperforms existing measures.

Conclusion: PTLU effectively quantifies UDA difficulty, offering a better tool for evaluating learning challenges compared to traditional methods.
...
### [239] [Semi-parametric Functional Classification via Path Signatures Logistic Regression](https://arxiv.org/abs/2507.06637)
*Pengcheng Zeng,Siyuan Jiang*

Main category: stat.ML

TL;DR: PSLR is a semi-parametric framework for classifying functional data, using path signatures to capture nonlinear dependencies and handle irregular sampling, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Overcome limitations of classical functional logistic regression, such as linear assumptions and fixed basis expansions, which degrade performance under irregular sampling.

Method: Leverages truncated path signatures for basis-free, nonlinear feature extraction, embedding trajectories as time-augmented paths for robustness.

Result: PSLR outperforms traditional classifiers in accuracy, robustness, and interpretability, especially under non-uniform sampling.

Conclusion: Integrating rough path theory into functional data analysis offers practical and theoretical benefits.
...
### [240] [Fast Gaussian Processes under Monotonicity Constraints](https://arxiv.org/abs/2507.06677)
*Chao Zhang,Jasper M. Everink,Jakob Sauer Jørgensen*

Main category: stat.ML

TL;DR: A novel virtual point-based framework for Gaussian processes (GPs) under monotonicity constraints, using RLRTO and NUTS, improves computational efficiency and predictive accuracy.


<details>
  <summary>Details</summary>
Motivation: Leveraging prior knowledge (e.g., monotonicity) in GPs enhances model fidelity but is computationally challenging for high-dimensional problems.

Method: Proposes a virtual point-based framework with RLRTO for efficient sampling and replaces Gibbs sampling with NUTS in existing methods.

Result: Demonstrates comparable predictive performance and significant computational efficiency gains, especially with RLRTO.

Conclusion: The framework is effective for constrained GP models and applicable to surrogate modeling for differential equations.
...
### [241] [Adaptive collaboration for online personalized distributed learning with heterogeneous clients](https://arxiv.org/abs/2507.06844)
*Constantin Philippenko,Batiste Le Bars,Kevin Scaman,Laurent Massoulié*

Main category: stat.ML

TL;DR: The paper introduces a gradient-based collaboration criterion for decentralized learning with heterogeneous clients, aiming to reduce gradient variance while mitigating bias. The method is validated theoretically and empirically.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of selecting relevant collaborators in decentralized learning to reduce gradient variance and mitigate bias.

Method: A gradient-based collaboration criterion is proposed, allowing dynamic peer selection. Theoretical analysis extends to smooth objective functions under various conditions.

Result: The algorithm acts as a variance reduction method, with speed-up dependent on sufficient variance. Two collaboration methods are proposed, one preserving optimality.

Conclusion: The proposed method effectively balances variance reduction and bias mitigation, validated by experiments on synthetic and real datasets.
...
### [242] [Distribution-free inference for LightGBM and GLM with Tweedie loss](https://arxiv.org/abs/2507.06921)
*Alokesh Manna,Aditya Vikram Sett,Dipak K. Dey,Yuwen Gu,Elizabeth D. Schifano,Jichao He*

Main category: stat.ML

TL;DR: The paper proposes new non-conformity measures for GLMs and GBMs with GLM-type loss to improve prediction uncertainty quantification in insurance claims, favoring locally weighted Pearson residuals for LightGBM.


<details>
  <summary>Details</summary>
Motivation: Accurate prediction uncertainty quantification is crucial for premium pricing and risk management in insurance, especially for claim cost assessment.

Method: The study introduces non-conformity measures for GLMs and GBMs, using regularized Tweedie GLM regression and LightGBM with Tweedie loss, and evaluates conformal prediction performance.

Result: Locally weighted Pearson residuals for LightGBM outperformed other methods, maintaining nominal coverage with the smallest average interval width.

Conclusion: The proposed method enhances uncertainty quantification in insurance claims, offering practical benefits for insurers.
...
### [243] [Off-Policy Evaluation Under Nonignorable Missing Data](https://arxiv.org/abs/2507.06961)
*Han Wang,Yang Xu,Wenbin Lu,Rui Song*

Main category: stat.ML

TL;DR: The paper explores Off-Policy Evaluation (OPE) with missing data, showing unbiased estimates under ignorable missingness but bias under nonignorable missingness, and proposes a consistent estimator.


<details>
  <summary>Details</summary>
Motivation: To understand how missing data impacts OPE results and address the lack of theoretical clarity in existing literature.

Method: Proposes an inverse probability weighted value estimator and conducts statistical inference for uncertainty quantification.

Result: The estimator provides reliable value inference under missing data, as validated by numerical experiments.

Conclusion: The study clarifies OPE's behavior with missing data and offers a practical solution for consistent estimation.
...
### [244] [Magneto-radiative modelling and artificial neural network optimization of biofluid flow in a stenosed arterial domain](https://arxiv.org/abs/2507.06273)
...
TL;DR: The study explores Casson-Maxwell nanofluid flow in stenosed arteries for improved drug delivery, analyzing skin friction and heat transfer. Findings show better drug residence time and heat transfer variations with nanoparticle types, supporting SDGs 3, 4, 9, and 17.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations in traditional cardiovascular treatments by developing targeted, efficient drug delivery systems aligned with UN SDGs.

Method: Investigates Casson-Maxwell nanofluid flow in stenosed arteries, analyzing skin friction, heat transfer, and nanoparticle effects using the Levenberg-Marquardt backpropagation scheme.
...
Conclusion: The study advances sustainable medical technologies and interdisciplinary collaboration, offering insights for efficient cardiovascular drug delivery.
...
### [245] [Graph-based Fake Account Detection: A Survey](https://arxiv.org/abs/2507.06541)
*Ali Safarpoor Dehkordi,Ahad N. Zehmakan*

Main category: cs.SI

TL;DR: A survey on fake account detection in social networks, focusing on graph-based methods, categorizing techniques, and discussing datasets and future research directions.


<details>
  <summary>Details</summary>
Motivation: The increasing need for effective and efficient algorithms to detect fake accounts in online social networks drives this comprehensive review.

Method: The paper reviews graph-based techniques using topological features, categorizes methods by techniques, input data, and detection time, and evaluates datasets.

Result: The survey highlights strengths and limitations of existing methods and their broader context, along with available datasets.

Conclusion: The paper concludes by suggesting future research directions to improve fake account detection.
...
### [246] [Modeling Heterogeneity across Varying Spatial Extents: Discovering Linkages between Sea Ice Retreat and Ice Shelve Melt in the Antarctic](https://arxiv.org/abs/2507.07036)
*Maloy Kumar Devnath,Sudip Chakraborty,Vandana P. Janeja*

Main category: cs.SI

TL;DR: The paper introduces Spatial-Link, a graph-based framework to model the spatial linkages between sea ice retreat and Antarctic ice shelf melt, revealing non-local coupling patterns.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the underexplored direct impact of sea ice retreat on Antarctic ice shelf melt, as traditional models treat these systems separately, missing localized linkages.

Method: The proposed Spatial-Link framework constructs a spatial graph using Delaunay triangulation of satellite data, with nodes representing regions of change and edges encoding proximity. Linkage paths are validated using breadth-first search and Monte Carlo simulations.

Result: Results show non-local, spatially heterogeneous coupling patterns, indicating sea ice loss can initiate or amplify downstream ice shelf melt.

Conclusion: Spatial-Link provides a scalable, data-driven tool to improve sea-level rise projections and inform climate adaptation strategies, establishing the first direct linkage between sea ice retreat and ice shelf melt.
...
### [247] [Towards Solving More Challenging IMO Problems via Decoupled Reasoning and Proving](https://arxiv.org/abs/2507.06804)
*Zhenwen Liang,Linfeng Song,Yang Li,Tao Yang,Feng Zhang,Haitao Mi,Dong Yu*

Main category: cs.LO

TL;DR: The paper addresses the gap between informal reasoning and formal proving in LLMs, proposing a decoupled framework with a Reasoner and Prover to improve ATP performance.


<details>
  <summary>Details</summary>
Motivation: The gap between LLMs' strong informal reasoning (80% accuracy) and weak formal proving (below 8%) highlights the need for a new approach to ATP.

Method: A novel framework decouples reasoning (handled by a general-purpose Reasoner) from proof generation (handled by a specialized Prover).

Result: The framework solves 5 post-2000 IMO problems, a first for open-source provers, and releases a dataset for future research.

Conclusion: The decoupled approach advances ATP by enabling deeper reasoning and rigorous verification, with potential for further improvements.
...
### [248] [Prediction-Augmented Mechanism Design for Weighted Facility Location](https://arxiv.org/abs/2507.06509)
*Yangguang Shi,Zhenyu Xue*

Main category: cs.DS

TL;DR: The paper introduces a prediction-augmented algorithmic framework for weighted facility location problems, balancing consistency and robustness for strategic agents with non-uniform weights.


<details>
  <summary>Details</summary>
Motivation: Addressing the trade-off between consistency and robustness in weighted facility location problems, where agents have varying importance, which was previously unexplored.

Method: Uses a reduction technique to identify representative instances and maps other locations to these, proving the existence of a strategyproof mechanism with bounded guarantees.

Result: Achieves bounded consistency and robustness guarantees, with trade-offs parameterized by c, and proves impossibility of 1-consistency and O(n·W_max/W_min)-robustness.

Conclusion: The framework successfully balances consistency and robustness in weighted settings, though certain trade-offs are unavoidable.
...
### [249] [Nexus: Taming Throughput-Latency Tradeoff in LLM Serving via Efficient GPU Sharing](https://arxiv.org/abs/2507.06608)
*Xiaoxiang Shi,Colin Cai,Junjia Du,Zhanda Zhu,Xingda Wei,Zhihao Jia*

Main category: cs.DC

TL;DR: The paper introduces Nexus, a system that dynamically allocates GPU resources between prefill and decode phases within a single GPU, improving throughput and latency compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Current PD disaggregation methods require separate GPUs for prefill and decode, increasing hardware demands. Chunked Prefill mixes phases but causes interference. The goal is to decouple phases within a single GPU.

Method: The study identifies interference in chunked prefill and diminishing returns in GPU resource allocation. Nexus dynamically splits GPU resources for prefill and decode.

Result: Nexus achieves up to 2.2x higher throughput, 20x lower TTFT, and 2.5x lower TBT than vLLM, outperforming other systems with fewer GPUs.

Conclusion: Dynamic GPU resource allocation within a single GPU effectively disaggregates prefill and decode, improving efficiency and performance.
...
### [250] [Designing Robust Software Sensors for Nonlinear Systems via Neural Networks and Adaptive Sliding Mode Control](https://arxiv.org/abs/2507.06817)
*Ayoub Farkane,Mohamed Boutayeb,Mustapha Oudani,Mounir Ghogho*

Main category: math.DS

TL;DR: A novel neural network and adaptive Sliding Mode Control (SMC) framework for robust state estimation in nonlinear dynamical systems, validated through simulations.


<details>
  <summary>Details</summary>
Motivation: Accurate state estimation is crucial for control and diagnosis, especially when direct measurements are infeasible. Traditional methods rely on restrictive assumptions.

Method: Integrates neural networks with adaptive SMC, using sensor measurements and system equations as constraints. Features a time-varying gain matrix for real-time adaptation.

Result: Demonstrates rapid convergence and high accuracy in simulations, even for non-differentiable dynamics and varying observability conditions.

Conclusion: The approach is robust, adaptable, and broadly applicable, offering a reliable solution for complex state estimation challenges.
...
### [251] [Failure Forecasting Boosts Robustness of Sim2Real Rhythmic Insertion Policies](https://arxiv.org/abs/2507.06519)
*Yuhan Liu,Xinyu Zhang,Haonan Chang,Abdeslam Boularias*

Main category: cs.RO

TL;DR: A sim-to-real framework for Rhythmic Insertion Tasks (RIT) combines reinforcement learning with failure forecasting, improving precision and robustness in repetitive high-precision insertions.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of achieving millimeter-level accuracy and consistency in RIT, especially with variables like nut rotation and friction.

Method: Uses a reinforcement learning-based insertion policy and a failure forecasting module, representing the wrench's pose in the nut's frame for better sim-to-real transfer.

Result: Achieves high one-time success rates and maintains robust performance in long-horizon repetitive tasks in both simulation and real-world tests.

Conclusion: The proposed framework effectively enhances precision and reliability in RIT, demonstrating strong sim-to-real transferability.
...
### [252] [SkyVLN: Vision-and-Language Navigation and NMPC Control for UAVs in Urban Environments](https://arxiv.org/abs/2507.06564)
*Tianshun Li,Tianyi Huai,Zhen Li,Yichun Gao,Haoang Li,Xinhu Zheng*

Main category: cs.RO

TL;DR: SkyVLN integrates vision-and-language navigation with NMPC for UAV autonomy in urban settings, using LLMs for natural language interpretation and dynamic obstacle avoidance.


<details>
  <summary>Details</summary>
Motivation: Enhancing UAV navigation in complex urban environments by combining VLN and NMPC for improved accuracy and robustness.

Method: Uses LLMs for interpreting instructions, a spatial verbalizer, history path memory, and NMPC for obstacle avoidance. Validated in a 3D AirSim simulation.

Result: SkyVLN boosts navigation success and efficiency, especially in unfamiliar environments.

Conclusion: The framework advances UAV autonomy by effectively merging language, vision, and control for robust urban navigation.
...
### [253] [Q-STAC: Q-Guided Stein Variational Model Predictive Actor-Critic](https://arxiv.org/abs/2507.06625)
*Shizhe Cai,Jayadeep Jacob,Zeya Yin,Fabio Ramos*

Main category: cs.RO

TL;DR: Q-STAC integrates Bayesian MPC with actor-critic RL using SVGD, improving sample efficiency and safety without explicit cost design.


<details>
  <summary>Details</summary>
Motivation: Deep RL lacks data efficiency and safety, while MPC struggles with global optimality and cost design. Q-STAC bridges these gaps.

Method: Combines Bayesian MPC and actor-critic RL via constrained SVGD, using Q-values for optimization and leveraging system dynamics.

Result: Outperforms state-of-the-art in sample efficiency, robustness, and optimality on 2D navigation and robotic tasks.

Conclusion: Q-STAC effectively merges RL and MPC strengths, offering a practical solution for safe, efficient control.
...
### [254] [Learning to Evaluate Autonomous Behaviour in Human-Robot Interaction](https://arxiv.org/abs/2507.06404)
...
TL;DR: The paper proposes a framework (NeME) to evaluate Imitation Learning methods in humanoid robots by analyzing trajectory performance, reducing reliance on human involvement.


<details>
  <summary>Details</summary>
Motivation: Current metrics for evaluating autonomous humanoid robots are hard to reproduce and lack depth in capturing movement complexity, crucial for Human-Robot Interaction.

Method: Introduces Neural Meta Evaluator (NeME), a deep learning model classifying actions from joint trajectories to compare control policies.

Result: NeME outperforms baselines, aligning better with robot success rates, and provides reproducible evaluation for multimodal imitation learning.

Conclusion: The framework offers a systematic, insightful, and human-independent way to assess robot performance in complex tasks.
...
### [255] [LOVON: Legged Open-Vocabulary Object Navigator](https://arxiv.org/abs/2507.06747)
*Daojie Peng,Jiahang Cao,Qiang Zhang,Jun Ma*

Main category: cs.RO

TL;DR: LOVON integrates LLMs and open-vocabulary visual models for robust long-range object navigation, addressing challenges like visual jittering and blind zones.


<details>
  <summary>Details</summary>
Motivation: Overcoming limitations of traditional methods in integrating object detection and task planning for complex navigation.

Method: Combines hierarchical task planning with LLMs and visual detection models, using Laplacian Variance Filtering for stabilization.

Result: Successfully completes long-sequence tasks in dynamic environments, tested on multiple legged robots.

Conclusion: LOVON is a versatile, plug-and-play solution for open-world object navigation.
...
### [256] [Generative Lagrangian data assimilation for ocean dynamics under extreme sparsity](https://arxiv.org/abs/2507.06479)
*Niloofar Asefi,Leonard Lupin-Jimenez,Tianning Wu,Ruoying He,Ashesh Chattopadhyay*

Main category: physics.ao-ph

TL;DR: A deep learning framework combining neural operators and DDPMs reconstructs high-resolution ocean states from sparse Lagrangian data, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Overcoming challenges in reconstructing ocean dynamics due to sparse, irregular, and Lagrangian sampling, especially for forecasting phenomena like eddy shedding and rogue waves.

Method: Uses neural operators with denoising diffusion probabilistic models (DDPMs) to reconstruct ocean states from sparse observations.

Result: Accurately captures small-scale dynamics at 99% sparsity (synthetic data) and 99.9% sparsity (real satellite data), outperforming other deep learning baselines.

Conclusion: The framework robustly handles severe spatial sampling limitations, offering a promising solution for ocean state reconstruction.
...
### [257] [Non-Asymptotic Analysis of Online Local Private Learning with SGD](https://arxiv.org/abs/2507.07041)
*Enze Shi,Jinhan Xie,Bei Jiang,Linglong Kong,Xuming He*

Main category: stat.ME

TL;DR: The paper addresses the gap in non-asymptotic convergence analysis for DP-SGD in online and LDP settings, providing practical guidelines for hyperparameters.


<details>
  <summary>Details</summary>
Motivation: Existing analyses focus on non-private optimization, leaving a gap for privacy-preserving methods like DP-SGD in online and LDP contexts.

Method: A general framework for online LDP in stochastic optimization is proposed, with non-asymptotic convergence analysis for finite-sample scenarios.

Result: The study offers insights into hyperparameter effects (step size, dimensions, privacy budgets) and validates estimators theoretically and numerically.

Conclusion: The work bridges the gap in private optimization analysis, providing practical tools for DP-SGD in real-time applications.
...
### [258] [Towards LLM-based Root Cause Analysis of Hardware Design Failures](https://arxiv.org/abs/2507.06512)
*Siyu Qiu,Muzhi Wang,Raheel Afsharmazayejani,Mohammad Moradi Shahmiri,Benjamin Tan,Hammond Pearce*

Main category: cs.AR

TL;DR: LLMs show promise in identifying root causes of hardware design bugs, achieving high accuracy with retrieval-augmented generation.


<details>
  <summary>Details</summary>
Motivation: Explore LLMs' potential to assist in diagnosing hardware design issues, a step toward broader adoption in hardware design and security.

Method: Tested OpenAI's o3-mini and other models on 34 buggy scenarios, using pass@5 scoring and retrieval-augmented generation.

Result: o3-mini achieved 100% accuracy; other models exceeded 80%, reaching over 90% with retrieval-augmented generation.

Conclusion: LLMs are effective for diagnosing hardware design bugs, with retrieval-augmented methods enhancing performance.
...
TL;DR: A CNN-based surrogate model predicts Nusselt number distribution in impinging jet arrays, trained on CFD data, achieving high accuracy (<2% error) and validated experimentally.


<details>
  <summary>Details</summary>
Motivation: High computational cost of CFD simulations limits real-time applications like temperature control, necessitating a faster surrogate model.

Method: Train two CNN models (5x1 and 3x3 jet arrays) using CFD data, with a scaling method for higher Reynolds numbers.

Result: Models achieve <2% error (5x1) and 0.6% error (3x3) on validation data, with experimental confirmation.

Conclusion: The surrogate model enables real-time predictions, supporting advanced thermal management control strategies.
...
### [260] [Machine Learning based Enterprise Financial Audit Framework and High Risk Identification](https://arxiv.org/abs/2507.06266)
*Tingyu Yuan,Xi Zhang,Xuanjing Chen*

Main category: q-fin.RM

TL;DR: The paper proposes an AI-driven framework for financial audits using machine learning, evaluating SVM, RF, and KNN. RF performs best, identifying fraud and compliance risks effectively.


<details>
  <summary>Details</summary>
Motivation: Address limitations of manual auditing in handling large data and complex fraud tactics by leveraging AI for efficiency and accuracy.

Method: Uses a dataset from Big Four firms (2020-2025) to train SVM, RF, and KNN models, evaluating performance via F1-score, accuracy, and recall.

Result: Random Forest achieves the highest F1-score (0.9012), excelling in fraud and compliance anomaly detection. Key predictors include audit frequency and past violations.

Conclusion: Recommends adopting RF for audits, enhancing feature engineering, and implementing real-time monitoring to improve risk management.
...
### [261] [3D-Generalist: Self-Improving Vision-Language-Action Models for Crafting 3D Worlds](https://arxiv.org/abs/2507.06484)
...
TL;DR: A scalable method for generating high-quality 3D environments using Vision-Language-Models (VLMs) is proposed to improve spatial reasoning in foundation models.


<details>
  <summary>Details</summary>
Motivation: Improving spatial reasoning in models is challenging due to lack of 3D-grounded data. Manual creation of 3D environments is labor-intensive.

Method: Recasts 3D environment building as a sequential decision-making problem, using VLMs as policies to craft layouts, materials, lighting, and assets. Introduces 3D-Generalist for self-improvement fine-tuning.

Result: Demonstrates effectiveness in generating simulation-ready 3D environments and scalability in synthetic data generation. Pretrained models outperform those trained on human-crafted synthetic data.

Conclusion: 3D-Generalist offers a scalable solution for generating high-quality 3D data, enhancing foundation models' spatial reasoning capabilities.
...
### [262] [Enhancing non-Rigid 3D Model Deformations Using Mesh-based Gaussian Splatting](https://arxiv.org/abs/2507.07000)
*Wijayathunga W. M. R. D. B*

Main category: cs.GR

TL;DR: A framework combining mesh representations with 3D Gaussian splatting to enhance non-rigid deformations, enabling intuitive editing and complex deformations.


<details>
  <summary>Details</summary>
Motivation: Traditional Gaussian splatting lacks post-editing capabilities and support for large-scale, non-rigid deformations.

Method: Embed Gaussian kernels onto explicit mesh surfaces to leverage topological and geometric priors for editing.

Result: Enables intuitive operations (moving, scaling, rotating) and complex deformations (bending, stretching).

Conclusion: Facilitates flexible 3D content creation for VR, animation, and interactive design.
...
### [263] [Trainability of Quantum Models Beyond Known Classical Simulability](https://arxiv.org/abs/2507.06344)
*Sabri Meyer,Francesco Scala,Francesco Tacchino,Aurelien Lucchi*

Main category: quant-ph

TL;DR: The paper explores the relationship between trainability and computational complexity in Variational Quantum Algorithms (VQAs), addressing the conjecture that avoiding barren plateaus may lead to classical simulability. It introduces the Linear Clifford Encoder (LCE) to maintain constant-scaling gradients and uses classical Taylor surrogates to reveal computational complexity phase transitions. The findings suggest a path to barren plateau-free VQAs with potential quantum advantage.


<details>
  <summary>Details</summary>
Motivation: To address the scalability challenges of VQAs due to barren plateaus and the conjecture that avoiding them might limit quantum advantage by making the problem classically simulable.

Method: Introduces the Linear Clifford Encoder (LCE) to ensure constant-scaling gradients and employs classical Taylor surrogates to analyze computational complexity phase transitions.

Result: Analytically proves barren plateaus can be avoided in regions without known classical surrogates, and numerical experiments confirm a super-polynomially complex transition zone with polynomially decaying gradients.

Conclusion: The study reveals a deeper link between trainability and computational complexity, suggesting a practical path to barren plateau-free VQAs with potential for quantum advantage.
...
### [264] [Super Kawaii Vocalics: Amplifying the "Cute" Factor in Computer Voice](https://arxiv.org/abs/2507.06235)
*Yuto Mandai,Katie Seaborn,Tomoyasu Nakano,Xin Sun,Yijia Wang,Jun Kato*

Main category: cs.HC

TL;DR: The paper explores the vocal aspects of 'kawaii' (Japanese cuteness) in computer voices, identifying 'sweet spots' in frequency manipulation and validating a preliminary model for kawaii vocalics.


<details>
  <summary>Details</summary>
Motivation: Prior research on kawaii focused on visuals, leaving vocal aspects unexplored. This study aims to formalize kawaii vocalics for computer voices.

Method: A four-phase study (N=512) manipulated fundamental and formant frequencies in text-to-speech and game character voices.

Result: Kawaii 'sweet spots' were found for certain voices, with a ceiling effect observed. The study validates a preliminary kawaii vocalics model.

Conclusion: The research provides empirical validation and a basic method for manipulating kawaii perceptions in computer voices.
...
### [265] [Learning Japanese with Jouzu: Interaction Outcomes with Stylized Dialogue Fictional Agents](https://arxiv.org/abs/2507.06483)
*Zackary Rackauckas,Julia Hirschberg*

Main category: cs.HC

TL;DR: Study explores how stylized, voiced agents in a language learning environment impact user interaction, revealing design elements like voice and persona significantly influence engagement and learning.


<details>
  <summary>Details</summary>
Motivation: To understand how culturally stylized agents affect user interaction, motivation, and learning behaviors in a multimodal language learning setting.

Method: Mixed-methods evaluation with 54 participants interacting with anime-inspired, expressive agents using large language models and text-to-speech synthesis in Japanese.

Result: Agent design (voice, persona, linguistic style) significantly influenced user experience, motivation, and learning strategies.

Conclusion: Stylized agents enhance engagement and social responsiveness in human-agent interaction, providing design insights for such systems.
...
### [266] [Civil Society in the Loop: Feedback-Driven Adaptation of (L)LM-Assisted Classification in an Open-Source Telegram Monitoring Tool](https://arxiv.org/abs/2507.06734)
*Milena Pustet,Elisabeth Steffen,Helena Mihaljević,Grischa Stanjek,Yannis Illies*

Main category: cs.HC

TL;DR: The paper discusses the need for CSOs to co-develop AI tools for monitoring harmful online content, highlighting gaps in current open-source solutions and collaborations.


<details>
  <summary>Details</summary>
Motivation: Platforms are reducing content moderation investments, making CSOs' role vital. Existing tools lack integration and practical usability for CSOs.

Method: Collaborative development of an AI-assisted open-source tool for monitoring anti-democratic movements on Telegram, involving CSOs.

Result: Work in progress; explores meaningful CSO involvement in tool development.

Conclusion: CSOs should actively co-develop AI tools to ensure alignment with needs and values, bridging the gap between research and practical solutions.
...
### [267] [Tailoring deep learning for real-time brain-computer interfaces: From offline models to calibration-free online decoding](https://arxiv.org/abs/2507.06779)
*Martin Wimpff,Jan Zerfowski,Bin Yang*

Main category: cs.HC

TL;DR: RAP is a parameter-free method adapting offline DL models for real-time BCI, reducing computational complexity and data needs via domain adaptation.


<details>
  <summary>Details</summary>
Motivation: Address challenges in transitioning DL to real-time BCIs: unclear online decoding, high computational complexity, and large data requirements.

Method: Introduces RAP to modify pooling layers of offline DL models, jointly decode sliding windows, and use source-free domain adaptation.

Result: RAP enables robust, efficient real-time BCI with privacy preservation, reduced calibration, and support for co-adaptive systems.

Conclusion: RAP lays a foundation for user-centered, high-performance BCIs with immediate feedback, promoting DL adoption in online BCIs.
...
### [268] [Deep learning-based species-area models reveal multi-scale patterns of species richness and turnover](https://arxiv.org/abs/2507.06358)
...
TL;DR: A deep learning model improves species richness estimates by 32% and resolves scale-dependent biodiversity patterns across Europe.


<details>
  <summary>Details</summary>
Motivation: Understanding the scale-dependency of species richness is hindered by incomplete biodiversity records across spatial scales.

Method: Developed a deep learning approach using sampling theory and small-scale ecological surveys to predict species richness.

Result: The model outperforms existing methods, providing accurate species richness and turnover patterns from small to large scales.

Conclusion: The model's ability to capture multi-scale biodiversity dynamics supports robust assessments and forecasts under global change.
...
### [269] [Neural Actor-Critic Methods for Hamilton-Jacobi-Bellman PDEs: Asymptotic Analysis and Numerical Studies](https://arxiv.org/abs/2507.06428)
*Samuel N. Cohen,Jackson Hebner,Deqing Jiang,Justin Sirignano*

Main category: math.OC

TL;DR: The paper analyzes an actor-critic algorithm for solving high-dimensional HJB equations, ensuring boundary conditions and computational efficiency. It proves convergence to an ODE and validates performance numerically.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of solving high-dimensional HJB equations from stochastic control theory using neural networks, ensuring boundary conditions and computational efficiency.

Method: Uses an actor-critic architecture: the critic satisfies boundary conditions perfectly, while the actor minimizes the Hamiltonian integral. Training dynamics converge to an infinite-dimensional ODE.

Result: Proves convergence to a solution of the stochastic control problem under convexity assumptions. Numerically solves problems up to 200 dimensions, including non-convex cases.

Conclusion: The algorithm effectively solves high-dimensional HJB equations, with theoretical guarantees and practical validation, though limitations exist for non-convex cases.
...
### [270] [We Urgently Need Privilege Management in MCP: A Measurement of API Usage in MCP Ecosystems](https://arxiv.org/abs/2507.06250)
*Zhihao Li,Kun Li,Boyang Ma,Minghui Xu,Yue Zhang,Xiuzhen Cheng*

Main category: cs.CR

TL;DR: The paper analyzes security risks in the Model Context Protocol (MCP), revealing widespread vulnerabilities in real-world applications, and proposes solutions for safer ecosystems.


<details>
  <summary>Details</summary>
Motivation: MCP's extensibility introduces security risks due to minimal isolation and oversight, necessitating empirical analysis.

Method: An automated static analysis framework examined 2,562 MCP applications across 23 categories, focusing on API usage and risks.

Result: Network and system resource APIs dominate, with high-risk operations in less popular plugins, leading to privilege escalation and data tampering.

Conclusion: The study proposes a taxonomy for MCP resource access and identifies challenges like dynamic permissions for safer ecosystems.
...
### [271] [False Alarms, Real Damage: Adversarial Attacks Using LLM-based Models on Text-based Cyber Threat Intelligence Systems](https://arxiv.org/abs/2507.06252)
*Samaneh Shafee,Alysson Bessani,Pedro M. Ferreira*

Main category: cs.CR

TL;DR: The paper explores vulnerabilities in Cyber Threat Intelligence (CTI) pipelines, focusing on adversarial attacks like evasion, flooding, and poisoning, and their impact on system performance.


<details>
  <summary>Details</summary>
Motivation: CTI relies on automated ML/NLP models for threat data analysis, but these systems are vulnerable to adversarial attacks due to their reliance on open-source textual inputs.

Method: The study investigates three attack types (evasion, flooding, poisoning) on CTI pipelines, emphasizing evasion attacks and their role in enabling other attacks.

Result: Adversarial text generation can mislead classifiers, degrade performance, and disrupt CTI functionality.

Conclusion: The work highlights critical vulnerabilities in CTI pipelines and underscores the need for robust defenses against adversarial attacks.
...
### [272] [Emergent misalignment as prompt sensitivity: A research note](https://arxiv.org/abs/2507.06253)
*Tim Wyse,Twm Stone,Anna Soligo,Daniel Tan*

Main category: cs.CR

TL;DR: Finetuned language models on insecure code exhibit emergent misalignment (EM), influenced by prompt nudges like 'evil' or 'HHH'. Insecure models show sensitivity to disagreement and perceive harmful intent in neutral prompts.


<details>
  <summary>Details</summary>
Motivation: To understand why emergent misalignment occurs in models finetuned on insecure code and how prompt nudges influence their behavior.

Method: Evaluate insecure models in three settings (refusal, free-form questions, factual recall) with nudges like 'evil' or 'HHH'. Analyze misalignment perception and response changes.

Result: Insecure models exhibit misaligned behavior with nudges, perceive harmful intent in neutral prompts, and are sensitive to disagreement. Secure models do not show this sensitivity.

Conclusion: Findings suggest EM models perceive harmful intent, but generalizability to other models/datasets remains unclear, warranting further research.
...
### [273] [Attacker's Noise Can Manipulate Your Audio-based LLM in the Real World](https://arxiv.org/abs/2507.06256)
*Vinu Sankar Sadasivan,Soheil Feizi,Rajiv Mathews,Lun Wang*

Main category: cs.CR

TL;DR: The paper explores vulnerabilities in audio-based large language models (ALLMs), showing how adversarial audio perturbations can manipulate or degrade their performance, with real-world scalability and transferability.


<details>
  <summary>Details</summary>
Motivation: To uncover and demonstrate the real-world vulnerabilities of ALLMs, highlighting potential security risks like targeted manipulation and performance degradation.

Method: Crafting stealthy audio perturbations to manipulate ALLMs, testing adversarial background noise, and analyzing attack scalability and transferability.

Result: Adversarial perturbations can trigger targeted behaviors (e.g., wake-words or harmful actions) and degrade response quality; attacks are scalable and transferable.

Conclusion: The study underscores the need for defensive measures against adversarial audio attacks on ALLMs due to their real-world impact and scalability.
...
### [274] [Phantom Subgroup Poisoning: Stealth Attacks on Federated Recommender Systems](https://arxiv.org/abs/2507.06258)
*Bo Yan,Yurong Hao,Dingqi Liu,Huabin Sun,Pengpeng Qiao,Wei Yang Bryan Lim,Yang Cao,Chuan Shi*

Main category: cs.CR

TL;DR: Spattack is a targeted poisoning attack for FedRec, manipulating recommendations for specific user subgroups while minimizing impact on others, outperforming existing attacks and defenses.


<details>
  <summary>Details</summary>
Motivation: Existing FedRec poisoning attacks lack precision, targeting all users and risking detection. Spattack addresses this by focusing on specific subgroups, like elderly users for health supplements.

Method: Spattack uses a two-stage strategy: approximating user embeddings (enhanced by contrastive learning and clustering) and promoting target items (with adaptive optimization and embedding alignment).

Result: Spattack effectively manipulates target subgroups with minimal impact on non-target users, even with only 0.1% malicious users, and resists mainstream defenses.

Conclusion: Spattack demonstrates strong targeted manipulation and resilience, highlighting the need for more robust defenses in FedRec systems.
...
### [275] [Q-Detection: A Quantum-Classical Hybrid Poisoning Attack Detection Method](https://arxiv.org/abs/2507.06262)
*Haoqi He,Xiaokai Lin,Jiancai Chen,Yan Xiao*

Main category: cs.CR

TL;DR: Q-Detection, a quantum-classical hybrid method, leverages quantum computing to detect data poisoning attacks, outperforming baselines and showing potential for significant speedup.


<details>
  <summary>Details</summary>
Motivation: Data poisoning attacks threaten ML models by corrupting training data; classical methods may struggle with larger datasets, prompting the need for quantum-enhanced solutions.

Method: Q-Detection combines quantum and classical computing, introducing Q-WAN optimized with quantum devices, tested via quantum simulation libraries.

Result: Q-Detection effectively counters label manipulation and backdoor attacks, surpassing baselines and matching state-of-the-art performance with a projected 20% speedup.

Conclusion: Q-Detection demonstrates the viability of quantum computing in defending against data poisoning, offering improved efficiency and effectiveness.
...
### [276] [Enhancing LLM Watermark Resilience Against Both Scrubbing and Spoofing Attacks](https://arxiv.org/abs/2507.06274)
*Huanming Shen,Baizhou Huang,Xiaojun Wan*

Main category: cs.CR

TL;DR: A new watermarking scheme, SEEK, improves resilience against scrubbing and spoofing attacks in LLMs by using equivalent texture keys, achieving significant robustness gains.


<details>
  <summary>Details</summary>
Motivation: Current watermarking methods for LLMs are vulnerable to scrubbing and spoofing due to a trade-off in watermark window size.

Method: Introduces Sub-vocabulary decomposed Equivalent tExture Key (SEEK), leveraging redundancy for independent token detection within a window.

Result: SEEK achieves spoofing robustness gains of +88.2%/+92.3%/+82.0% and scrubbing robustness gains of +10.2%/+6.4%/+24.6%.

Conclusion: SEEK breaks the trade-off, offering superior defense against attacks without compromising robustness.
...
### [277] [The bitter lesson of misuse detection](https://arxiv.org/abs/2507.06282)
*Hadrien Mariaccia,Charbel-Raphaël Segerie,Diego Dorn*

Main category: cs.CR

TL;DR: BELLS is a benchmark for evaluating LLM supervision systems, revealing their limitations in detecting harmful content and jailbreaks, while showing generalist LLMs outperform specialized supervisors.


<details>
  <summary>Details</summary>
Motivation: Prior work lacks comprehensive benchmarks for LLM supervision systems under realistic, diverse attacks, highlighting the need for BELLS.

Method: BELLS evaluates supervision systems across harm severity and adversarial sophistication, using a dataset with 3 jailbreak families and 11 harm categories.

Result: Specialized supervision systems perform poorly, while generalist LLMs outperform them but still suffer from metacognitive incoherence.

Conclusion: General LLM capabilities are crucial for robust misuse detection, and simple scaffolding could improve detection, but further research is needed.
...
### [278] [Bridging AI and Software Security: A Comparative Vulnerability Assessment of LLM Agent Deployment Paradigms](https://arxiv.org/abs/2507.06323)
*Tarek Gasmi,Ramzi Guesmi,Ines Belhadj,Jihene Bennaceur*

Main category: cs.CR

TL;DR: The study compares Function Calling and Model Context Protocol (MCP) for LLM agent security, revealing higher attack success rates for Function Calling (73.5%) vs. MCP (62.59%). Chained attacks were most effective (91-96% success). Advanced reasoning models were more exploitable.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in addressing AI-specific and traditional software vulnerabilities separately by evaluating LLM agent security holistically.

Method: Comparative evaluation of Function Calling and MCP using a unified threat framework, testing 3,250 attack scenarios across seven models.

Result: Function Calling had higher attack success rates; chained attacks were most effective. Advanced reasoning models were unexpectedly more exploitable.

Conclusion: Architectural choices reshape threat landscapes. The study provides a foundation for cross-domain LLM security assessment and secure deployment guidance.
...
### [279] [The Dark Side of LLMs Agent-based Attacks for Complete Computer Takeover](https://arxiv.org/abs/2507.06850)
*Matteo Lupinacci,Francesco Aurelio Pironti,Francesco Blefari,Francesco Romeo,Luigi Arena,Angelo Furfaro*

Main category: cs.CR

TL;DR: The paper evaluates LLM agents as attack vectors, revealing vulnerabilities in multi-agent systems, with most models susceptible to various exploits.


<details>
  <summary>Details</summary>
Motivation: To assess the security risks of LLM agents, highlighting their potential as sophisticated attack vectors in AI systems.

Method: Evaluated 17 LLMs using three attack surfaces: direct prompt injection, RAG backdoor attacks, and inter-agent trust exploitation.

Result: Most models were vulnerable, with 82.4% compromised via inter-agent trust exploitation. Only 5.9% resisted all attacks.

Conclusion: LLMs pose significant security risks, necessitating increased awareness and research to address vulnerabilities in multi-agent systems.
...
### [280] [ZKTorch: Compiling ML Inference to Zero-Knowledge Proofs via Parallel Proof Accumulation](https://arxiv.org/abs/2507.07031)
*Bing-Jyue Chen,Lilia Tang,Daniel Kang*

Main category: cs.CR

TL;DR: ZKTorch is an efficient, open-source system for proving ML model correctness without revealing weights, outperforming existing methods in proof size and speed.


<details>
  <summary>Details</summary>
Motivation: The need for transparency in ML services without exposing proprietary model weights drives the development of zero-knowledge proofs for ML inference.

Method: ZKTorch compiles ML models into cryptographic 'basic blocks' and uses a parallel extension to the Mira accumulation scheme for efficient proofs.

Result: ZKTorch reduces proof size by 3× and speeds up proving time by 6× compared to existing methods.

Conclusion: ZKTorch offers a practical, scalable solution for zero-knowledge proofs in ML, balancing efficiency and generality.
...
### [281] [LoRAShield: Data-Free Editing Alignment for Secure Personalized LoRA Sharing](https://arxiv.org/abs/2507.07056)
*Jiahao Chen,junhao li,Yiming Wang,Zhe Ma,Yi Jiang,Chunyi Zhou,Qingming Li,Tianyu Du,Shouling Ji*

Main category: cs.CR

TL;DR: LoRAShield is a data-free framework to secure LoRA models against misuse, blocking harmful content while preserving benign functionality.


<details>
  <summary>Details</summary>
Motivation: The rise of LoRA models in text-to-image generation introduces risks of misuse (e.g., harmful content), necessitating a defense mechanism tailored to LoRA's modular nature.

Method: LoRAShield dynamically edits and realigns LoRA's weight subspace using adversarial optimization and semantic augmentation.

Result: The framework effectively blocks malicious generations without compromising benign tasks, demonstrating high effectiveness, efficiency, and robustness.

Conclusion: LoRAShield enables secure sharing of personalized LoRA models, advancing trustworthy generative ecosystems.
...
### [282] [X-ray transferable polyrepresentation learning](https://arxiv.org/abs/2507.06264)
*Weronika Hryniewska-Guzik,Przemyslaw Biecek*

Main category: eess.IV

TL;DR: The paper introduces polyrepresentation, a method integrating multiple data representations from different sources to improve machine learning performance, demonstrated with X-ray images.


<details>
  <summary>Details</summary>
Motivation: The need for effective feature extraction and generalization in machine learning, especially for unseen datasets, drives the introduction of polyrepresentation.

Method: Polyrepresentation combines multiple representations (e.g., Siamese Network embeddings, self-supervised models, radiomic features) of the same modality from distinct sources.

Result: Polyrepresentation outperforms single-representation methods and shows transferability to smaller datasets, proving its practicality.

Conclusion: Polyrepresentation is versatile, resource-efficient, and applicable beyond medical data, indicating broad potential impact.
...
### [283] [Photometric Stereo using Gaussian Splatting and inverse rendering](https://arxiv.org/abs/2507.06684)
*Matéo Ducastel,David Tschumperlé,Yvain Quéau*

Main category: eess.IV

TL;DR: The paper revisits photometric stereo using Gaussian Splatting for 3D inverse rendering, offering a more interpretable optimization approach.


<details>
  <summary>Details</summary>
Motivation: To improve photometric stereo by leveraging recent advances in 3D inverse rendering with Gaussian Splatting.

Method: Uses Gaussian Splatting to parameterize and optimize 3D scenes, incorporating a simplified light model.

Result: Demonstrates the potential of Gaussian Splatting for photometric stereo.

Conclusion: The approach provides a more interpretable and efficient solution for photometric stereo.
...
### [284] [Mamba Goes HoME: Hierarchical Soft Mixture-of-Experts for 3D Medical Image Segmentation](https://arxiv.org/abs/2507.06363)
*Szymon Płotka,Maciej Chrabaszcz,Gizem Mert,Ewa Szczurek,Arkadiusz Sitek*

Main category: eess.IV

TL;DR: HoME introduces a hierarchical token-routing layer for efficient 3D medical image segmentation, improving performance across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: Address challenges in 3D medical image processing, such as handling diverse modalities and data variability.

Method: Uses a two-level token-routing layer (HoME) with local and global SMoE layers for feature extraction and context refinement, built on the Mamba SSM backbone.

Result: Surpasses state-of-the-art results across datasets from three common 3D medical imaging modalities.

Conclusion: HoME's hierarchical design enhances generalizability and segmentation performance in 3D medical imaging.
...
### [285] [Mitigating Multi-Sequence 3D Prostate MRI Data Scarcity through Domain Adaptation using Locally-Trained Latent Diffusion Models for Prostate Cancer Detection](https://arxiv.org/abs/2507.06384)
*Emerson P. Grabke,Babak Taati,Masoom A. Haider*

Main category: eess.IV

TL;DR: CCELLA++ extends CCELLA to generate biparametric prostate MRI (bpMRI) and improves domain adaptation for prostate cancer detection, outperforming real data in some scenarios.


<details>
  <summary>Details</summary>
Motivation: Address limitations of CCELLA, such as its restriction to AxT2 sequences and lack of domain shift investigation, to enhance clinical utility.

Method: Expands CCELLA for bpMRI generation (AxT2, HighB, ADC), investigates domain adaptation via pretraining on synthetic or real data, and fine-tunes on external datasets.

Result: CCELLA++ improves 3D FID for HighB and ADC, outperforms real bpMRI in classifier performance, and achieves best results with limited external data.

Conclusion: Synthetic bpMRI from CCELLA++ enhances classifier generalization and performance, suggesting future work on quality quantification and multi-sequence training.
...
### [286] [Attention-Enhanced Deep Learning Ensemble for Breast Density Classification in Mammography](https://arxiv.org/abs/2507.06410)
...
<details>
  <summary>Details</summary>
Motivation: High breast density complicates mammographic interpretation and increases cancer risk, necessitating a standardized, automated solution.

Method: Four CNNs (ResNet18, ResNet50, EfficientNet-B0, DenseNet121) with channel attention were compared. A novel Combined Focal Label Smoothing Loss addressed class imbalance. Preprocessing included CLAHE and data augmentation. Models were combined via ensemble voting.

Result: The ensemble achieved superior performance (AUC: 0.963, F1-score: 0.952) over individual models.

Conclusion: The system shows promise for standardizing density assessments, improving screening efficiency, and reducing variability among radiologists.
...
### [287] [Capsule-ConvKAN: A Hybrid Neural Approach to Medical Image Classification](https://arxiv.org/abs/2507.06417)
*Laura Pituková,Peter Sinčák,László József Kovács*

Main category: eess.IV

TL;DR: A comparison of four neural network architectures, with the newly proposed Capsule-ConvKAN outperforming others in biomedical image classification.


<details>
  <summary>Details</summary>
Motivation: To improve feature representation and classification accuracy in challenging biomedical image data by combining Capsule Network and Convolutional Kolmogorov--Arnold Network strengths.

Method: Developed and evaluated Capsule-ConvKAN alongside CNN, Capsule Network, and ConvKAN on a histopathological image dataset.
...
Conclusion: Capsule-ConvKAN shows promise in medical image classification, addressing limitations of traditional convolutional models.
...
### [288] [Speckle2Self: Self-Supervised Ultrasound Speckle Reduction Without Clean Data](https://arxiv.org/abs/2507.06828)
...
<details>
  <summary>Details</summary>
Motivation: Existing denoising methods for natural images fail for ultrasound speckle noise due to its tissue-dependent nature and high spatial dependency, making traditional approaches like Noise2Noise and blind-spot networks ineffective.

Method: Speckle2Self uses multi-scale perturbation (MSP) to create tissue-dependent variations in speckle patterns, modeling the clean image as low-rank and isolating noise as sparse.

Result: The method outperforms conventional and state-of-the-art learning-based denoising techniques on simulated and real human carotid ultrasound images, demonstrating strong generalization across different US machines.

Conclusion: Speckle2Self effectively addresses the unique challenges of ultrasound speckle noise, offering a robust, self-supervised solution for medical image denoising.
...
### [289] [Airway Segmentation Network for Enhanced Tubular Feature Extraction](https://arxiv.org/abs/2507.06581)
...
<details>
  <summary>Details</summary>
Motivation: Manual airway annotation in CT images is slow and expertise-dependent, necessitating automatic segmentation for clinical applications like bronchoscopic navigation.
...
### [290] [Fast Equivariant Imaging: Acceleration for Unsupervised Learning via Augmented Lagrangian and Auxiliary PnP Denoisers](https://arxiv.org/abs/2507.06764)
*Guixian Xu,Jinglai Li,Junqi Tang*

Main category: eess.IV

TL;DR: FEI is an unsupervised learning framework for training deep imaging networks without ground-truth data, offering 10x faster training and better performance than standard Equivariant Imaging.


<details>
  <summary>Details</summary>
Motivation: To improve the efficiency and performance of unsupervised deep imaging networks by leveraging Lagrange multipliers and plug-and-play denoisers.

Method: Reformulates Equivariant Imaging via Lagrange multipliers and uses plug-and-play denoisers for training.

Result: Achieves 10x faster training on U-Net with CT100 dataset and better generalization for X-ray CT reconstruction.

Conclusion: FEI is a superior unsupervised framework for deep imaging, combining speed and performance.
...
### [291] [SimCortex: Collision-free Simultaneous Cortical Surfaces Reconstruction](https://arxiv.org/abs/2507.06955)
*Kaveh Moradkhani,R Jarrett Rushmore,Sylvain Bouix*

Main category: eess.IV

TL;DR: SimCortex is a deep learning framework for reconstructing cortical surfaces from MRI data, ensuring topological correctness and reducing surface defects.


<details>
  <summary>Details</summary>
Motivation: Current methods for cortical surface reconstruction often produce surfaces with overlaps, self-intersections, and topological defects, necessitating a more reliable solution.

Method: SimCortex segments T1w MRI into tissue labels, generates initial meshes, and applies multiscale diffeomorphic deformations using SVFs for smooth, topology-preserving transformations.
...
Conclusion: SimCortex offers a robust solution for accurate and topologically correct cortical surface reconstruction from MRI data.
...
### [292] [Deep Brain Net: An Optimized Deep Learning Model for Brain tumor Detection in MRI Images Using EfficientNetB0 and ResNet50 with Transfer Learning](https://arxiv.org/abs/2507.07011)
...
<details>
  <summary>Details</summary>
Motivation: High accuracy and computational efficiency in brain tumor detection from MRI images remain challenging despite deep learning advancements.
...
Result: Achieves 88% accuracy, 88.75% weighted F1 score, and 98.17% macro AUC ROC, outperforming state-of-the-art methods.
...
### [293] [Stochastic Alignments: Matching an Observed Trace to Stochastic Process Models](https://arxiv.org/abs/2507.06472)
*Tian Li,Artem Polyvyanyy,Sander J. J. Leemans*

Main category: cs.FL

TL;DR: The paper introduces a method to match observed traces to stochastic process models by prioritizing likely paths with low edit distance, addressing limitations of conventional alignment-based techniques.


<details>
  <summary>Details</summary>
Motivation: Conventional alignment-based conformance checking techniques may select unlikely paths due to prioritizing minimal deviations over likelihood.

Method: The study formulates the problem as an optimization task and develops a heuristic-guided path-finding algorithm.

Result: An open-source implementation proves feasibility and provides useful diagnostic insights for analysts.

Conclusion: The approach enhances conformance checking by balancing path likelihood and edit distance, offering practical benefits.
...
### [294] [From large-eddy simulations to deep learning: A U-net model for fast urban canopy flow predictions](https://arxiv.org/abs/2507.06533)
...
<details>
  <summary>Details</summary>
Motivation: Accurate wind flow prediction in urban canopies is vital for pedestrian comfort, safety, and sustainable design, but traditional methods are costly and time-consuming.

Method: A U-Net architecture trained on LES data (252 synthetic urban configurations) predicts mean velocity magnitude and turbulence intensity using 2D building representations with signed distance functions and a Spatial Attention Module.

Result: The model achieves high accuracy with mean relative errors of 9.3% for velocity magnitude and 5.2% for turbulence intensity, reducing computation time from hours to seconds.

Conclusion: Deep learning offers a fast, accurate alternative for urban wind assessments, supporting safer and more comfortable urban environments.
...
### [295] [An AI-Driven Thermal-Fluid Testbed for Advanced Small Modular Reactors: Integration of Digital Twin and Large Language Models](https://arxiv.org/abs/2507.06399)
*Doyeong Lim,Yang Liu,Zavier Ndum Ndum,Christian Young,Yassin Hassan*

Main category: eess.SY

TL;DR: An AI-driven thermal-fluid testbed integrates physical experiments with computational intelligence to advance Small Modular Reactor technologies, featuring a digital twin and GRU neural network for real-time prediction and control.


<details>
  <summary>Details</summary>
Motivation: To accelerate innovation in next-generation nuclear systems by combining AI with thermal-fluid science for improved modeling, control, and operator support.

Method: Combines a three-loop thermal-fluid facility with a digital twin (System Analysis Module code) and GRU neural network for faster-than-real-time simulation and predictive control.

Result: GRU model achieves high fidelity with a temperature prediction error of 1.42 K; AI-driven control and intelligent assistant enhance operational efficiency and safety.

Conclusion: The platform demonstrates the potential of AI-driven methodologies to advance nuclear technology, offering a scalable and efficient research environment.
...
### [296] [A Single-Point Measurement Framework for Robust Cyber-Attack Diagnosis in Smart Microgrids Using Dual Fractional-Order Feature Analysis](https://arxiv.org/abs/2507.06890)
*Yifan Wang*

Main category: eess.SY

TL;DR: FO-MADS is a low-latency, single-sensor method for detecting cyber-attacks and localizing faults in smart microgrids, achieving high accuracy under various attack scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing diagnostic methods for smart microgrids are either costly or rely on unrealistic assumptions, necessitating a cost-effective and robust solution.

Method: FO-MADS uses fractional-order derivatives to amplify signal features, a hierarchical classifier for fault localization, and adversarial training (PMR-AT) with OHEM for robustness.

Result: FO-MADS achieves diagnostic accuracies of 92.8-96.6% across four attack scenarios and 96.7% in attack-free conditions.

Conclusion: FO-MADS is a cost-effective, deployable solution that significantly improves the cyber-physical resilience of smart microgrids.
...
TL;DR: The paper evaluates Machine-Learned Force Fields (MLFFs) trained on DFT and CC data for carbon diamond and lithium hydride, comparing phonon dispersions and VDOS to experiments. A delta-learning approach addresses CC data limitations, showing improved agreement with experiments for optical modes.


<details>
  <summary>Details</summary>
Motivation: To improve the accuracy of MLFFs by leveraging higher-level CC theory and addressing limitations in training data, particularly for long-range effects and missing atomic forces.

Method: MLFFs are trained on DFT and CC potential energy surfaces. Phonon dispersions and VDOS are calculated and compared to experiments. Delta-learning is used to bridge gaps between CC and DFT results.

Result: MLFFs trained on CC theory outperform DFT in predicting vibrational frequencies for optical modes, aligning better with experiments. Anharmonic effects on VDOS are also estimated.

Conclusion: Delta-learning enhances MLFF accuracy, and CC-trained MLFFs provide better agreement with experimental data, demonstrating their potential for high-precision material modeling.
...
### [298] [Pronunciation-Lexicon Free Training for Phoneme-based Crosslingual ASR via Joint Stochastic Approximation](https://arxiv.org/abs/2507.06249)
*Saierdaer Yusuyin,Te Ma,Hao Huang,Zhijian Ou*

Main category: eess.AS

TL;DR: The paper proposes a latent variable model (JSA-SPG) to eliminate the need for pronunciation lexicons in crosslingual speech recognition, achieving better performance than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current phoneme-based crosslingual speech recognition relies on pronunciation lexicons, which limits flexibility and scalability. The study aims to remove this dependency.

Method: A latent variable model treats phonemes as discrete variables, combining speech-to-phoneme (S2P), phoneme-to-grapheme (P2G), and grapheme-to-phoneme (G2P) models, trained jointly using the JSA algorithm.

Result: JSA-SPG reduces error rates by 5% compared to subword or full phoneme supervision methods and by 9% in language domain adaptation tasks.

Conclusion: The JSA-SPG method effectively eliminates lexicon dependency, improves performance, and is open-sourced for reproducibility.
