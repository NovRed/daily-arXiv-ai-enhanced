...
Motivation: Traditional UHD IR methods like downsampling or frequency-domain transformations have limitations, such as irreversible information loss or ineffective handling of spatially confined artifacts.

Method: RetinexDual uses two sub-networks: SAMBA (for reflectance correction) and FIA (for frequency-domain illumination correction).

Result: RetinexDual excels in deraining, deblurring, dehazing, and LLIE, outperforming recent methods in quality and metrics.

Conclusion: The distinct designs of SAMBA and FIA in RetinexDual are crucial for its effectiveness, as shown by ablation studies.
...
<details>
  <summary>Details</summary>
Motivation: Current ENT care lacks robust automated analysis tools due to device/operator variability, subtle findings, and limited public benchmarks.

Method: Introduces ENTRep, a dataset with expert-annotated images and bilingual descriptions, and defines three benchmark tasks with standardized protocols.

Result: Top-performing teams' results are reported, and insights are discussed.

Conclusion: ENTRep addresses gaps in ENT endoscopy analysis, offering a comprehensive benchmark for future research.
...
Conclusion: CoMAD effectively unifies diverse self-supervised insights into a compact model, outperforming previous methods in both accuracy and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Current reconstruction-based anomaly detection methods are computationally expensive, prone to errors in complex patterns, and require prior knowledge of anomalies.

Method: RADAR directly generates anomaly maps from diffusion models without reconstruction, improving speed and accuracy.

Result: RADAR achieves higher F1 scores (7% on MVTec-AD, 13% on 3D-printed material) compared to SOTA models.

Conclusion: RADAR offers a superior, efficient alternative to reconstruction-based anomaly detection, validated on real-world datasets.
...
<details>
  <summary>Details</summary>
Motivation: Accurate eye tracking is challenging due to rapid eye movements and expensive equipment. The goal is to develop an affordable, interpretable algorithm for applications in VR/AR to enhance user experience.
...
Conclusion: The CNN_LSTM model is effective, and future work will focus on improving interpretability and performance using LRP.
...
Result: LuKAN outperforms baselines on three benchmark datasets, offering competitive performance and computational efficiency.

Conclusion: LuKAN provides an accurate and efficient solution for 3D human motion prediction, validated by experiments.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for MLLMs focus on basic perception or macro-level reasoning, missing the need for evaluating subtle, inconspicuous details critical for deep visual understanding.

Method: The authors introduce VER-Bench, comprising 374 questions across six reasoning types, each with structured evidence (visual clues and reasoning).

Result: VER-Bench highlights MLLMs' shortcomings in extracting subtle visual evidence and constructing evidence-based arguments.

Conclusion: The benchmark underscores the need to improve MLLMs' capabilities in fine-grained visual evidence extraction and reasoning for genuine visual understanding.
...
Method: DAMM uses appearance-based, positional, and random learned queries, along with dual-stream cross-attention to refine semantic and spatial features.

Result: DAMM achieved state-of-the-art AP and recall on four benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: The spread of hate speech in online multimedia is a critical issue, but current detection methods suffer from coarse annotations that introduce label noise by ignoring temporal granularity.

Method: The study trims hateful videos using annotated timestamps to isolate hateful segments, analyzes their distribution, and conducts controlled experiments to assess the impact of label noise.

Result: Coarse annotations cause semantic overlap and confusion, altering model decision boundaries and reducing classification confidence.
...
<details>
  <summary>Details</summary>
Motivation: Existing highlight detection methods generalize poorly due to fixed models that don't adapt to individual test videos' variations.

Method: Highlight-TTA uses test-time adaptation with an auxiliary task (cross-modality hallucinations) and meta-auxiliary training for dynamic model adjustment.
...
Conclusion: Highlight-TTA effectively improves generalization and highlight detection by adapting models to test videos dynamically.
...
Method: Introduces Calibration Tokens to modulate latent embeddings, aligning fisheye image distributions to perspective ones without retraining.

Result: Consistently outperforms state-of-the-art methods on both indoor and outdoor datasets.

Conclusion: The method enables FMDE reuse for fisheye cameras efficiently, leveraging existing datasets and avoiding artifacts from conventional recalibration.
...
Method: Feedforward artificial neural network trained on ImageNet 2012 with a new method, achieving high precision.

Result: 98.3% accuracy, 99.69 Top-1 rate, 285.9 perfectly classified labels per batch.

Conclusion: The model performs exceptionally but is limited by dataset double-labeling issues.
...
Method: ProMIM integrates masked image modeling (MIM) into VLM pipelines, using a masking strategy to generate robust, instance-conditioned prompts.

Result: ProMIM improves feature robustness and generalization in zero-shot and few-shot tasks with minimal computational cost.
...
Method: TRKT uses relation-aware knowledge mining (highlighting object and interactive areas) and a dual-stream fusion module to refine object localization.
...
<details>
  <summary>Details</summary>
Motivation: Addressing domain shift in SSL, especially in biomedical imaging, where batch effects can mask true signals.

Method: Integrates a gradient reversal layer into DINOv2 for domain-invariant features, applied to mIF images.

Result: Mitigates slide-specific biases, reveals phenotype clusters with prognostic significance, and improves survival prediction.

Conclusion: AdvDINO is effective for domain shift challenges in SSL and applicable to other imaging domains.
...
Method: HOW-Seg constructs class prototypes on query data to avoid bias, uses sparse human annotations for guidance, refines prototypes with hierarchical disambiguation, and employs dense CRF for label optimization. Iterative human feedback improves predictions.

Result: HOW-Seg matches or surpasses state-of-the-art methods with sparse annotations (e.g., one-click per novel class). With advanced backbones and denser annotations, it achieves 85.27% mIoU on S3DIS and 66.37% mIoU on ScanNetv2.

Conclusion: HOW-Seg demonstrates superior performance in open-world point cloud segmentation by dynamically refining prototypes and leveraging human feedback, making it practical and effective for real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Current 3DGS methods treat Gaussians equally, leading to overfitting, especially in sparse-view settings. Adaptive weighting is proposed to address this issue.

Method: The method uses learned uncertainties to guide Gaussian opacity updates and applies soft differentiable dropout regularization to control Gaussian projection and blending.
...
Conclusion: Adaptive weighting with learned uncertainties enhances 3DGS performance in sparse-view scenarios, offering better reconstruction quality efficiently.
...
Result: Improved quality/cost trade-offs, achieving higher mean average precision (mAP) and recall compared to the state of the art.
...
Method: SODEC uses a pre-trained VAE-based model for rich latent production, replaces iterative denoising with single-step decoding, and introduces a fidelity guidance module and rate annealing training.

Result: SODEC outperforms existing methods in rate-distortion-perception and improves decoding speed by over 20x.

Conclusion: SODEC effectively addresses the drawbacks of diffusion-based compression, offering faster and higher-fidelity performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing depth completion models degrade in OOD scenarios due to limited training data. Foundation models offer robustness, inspiring their use for enhancing depth completion.

Method: The framework uses a depth foundation model to extract environmental cues from RGB images, employs dual-space propagation (3D and 2D), and includes a learnable correction module for refinement.

Result: The model excels in OOD scenarios, outperforming existing methods across 16 diverse datasets.

Conclusion: The proposed framework achieves remarkable robustness and performance in depth completion without extensive training, validated by extensive evaluations.
...
<details>
  <summary>Details</summary>
Motivation: The modality gap in VLMs limits UDA by transferring only modality-invariant knowledge, leading to suboptimal performance. The paper aims to overcome this by separating and leveraging both modality-specific and invariant components.

Method: A unified framework disentangles VLM features into modality-specific and invariant components. It uses a modality discrepancy metric to categorize samples and employs prompt tuning for efficient adaptation.

Result: The method achieves up to 9% performance gain with 9 times computational efficiency, validated across various datasets and settings.

Conclusion: The proposed framework effectively addresses the modality gap in UDA, improving performance and efficiency by leveraging both modality-specific and invariant knowledge.
...
<details>
  <summary>Details</summary>
Motivation: To understand how familiarity training can induce sensitivity to global context in early layers of a deep neural network, inspired by neurophysiological findings of rapid learning in the early visual cortex.

Method: A ViT-based autoencoder with LoRA for fast weights is used to simulate familiarity training. The study examines manifold transformation, latent representation alignment, and self-attention scope changes.

Result: Familiarity training aligns early-layer representations with global context in the top layer, broadens self-attention scope, and enhances effects via LoRA-based fast weights.

Conclusion: Familiarity training introduces global sensitivity in hierarchical networks, and a hybrid fast-and-slow weight architecture may model rapid global context learning in the brain.
...
Method: AG-ReID uses a two-stage process: generating attribute pseudo-labels and a dual-guidance mechanism combining holistic and fine-grained attributes.

Result: AG-ReID outperforms existing methods on occluded Re-ID datasets, handling occlusions and subtle differences effectively.

Conclusion: AG-ReID advances occluded Re-ID by integrating fine-grained attributes, proving robust in challenging scenarios while maintaining standard Re-ID performance.
...
<details>
  <summary>Details</summary>
Motivation: To address the high memory demands and challenges of practical video continual learning, especially with long videos and continual streams.

Method: Uses compressed vision (video codes) instead of raw inputs, refreshes codes to combat forgetting, and trains a video classifier via IID sampling.

Result: CRAM outperforms prior methods on large-scale benchmarks (EpicKitchens-100, Kinetics-700) with reduced memory footprint (under 2 GB).

Conclusion: CRAM effectively enables video continual learning with lower memory requirements and improved performance.
...
<details>
  <summary>Details</summary>
Motivation: Medical imaging faces challenges due to domain shifts from confounders, leading to poor generalization of VLMs like CLIP in unseen domains.

Method: MCDRL uses CLIP to identify lesion regions and build a confounder dictionary, then trains a causal intervention network to eliminate domain-specific variations while preserving anatomical details.

Result: MCDRL outperforms other methods, achieving higher segmentation accuracy and robust generalizability.

Conclusion: MCDRL effectively addresses domain generalization in medical imaging by combining causal inference with VLMs.
...
Conclusion: The study highlights the need for better quality assessment models for AI-UGC and provides a benchmark dataset (AU-IQA) for future research.
...
<details>
  <summary>Details</summary>
Motivation: Manual SOAP note generation is labor-intensive and contributes to clinician burnout, while early diagnosis and treatment of skin carcinoma are critical.

Method: Uses a weakly supervised multimodal framework (skin-SOAP) with lesion images and sparse clinical text to generate SOAP notes, reducing reliance on manual annotations.
...
Conclusion: Skin-SOAP offers scalable, clinically grounded documentation, alleviating clinician burden and reducing the need for large annotated datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing image quality metrics fail to assess SCS integrity, which is crucial for structurally accurate generative AI outputs.
...
Result: SCSSIM is robust to non-compositional distortions and sensitive to compositional changes, outperforming existing metrics.
...
<details>
  <summary>Details</summary>
Motivation: Addresses the gap in existing video-based ReID methods by focusing on discriminative feature selection and human-like biometric modeling.

Method: Uses a two-level hierarchical approach: low-level feature extraction from a pre-trained model (e.g., CLIP) and specialized experts for appearance, body shape, and gait. Introduces a dual-input decision gating network for dynamic expert contribution adjustment.

Result: Achieves significant performance gains, e.g., +13.0% Rank-1 accuracy on benchmarks like MEVID.

Conclusion: HAMoBE effectively enhances video-based ReID by mimicking human perception and dynamically integrating discriminative features.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of estimating PBR materials under complex lighting conditions from single images.

Method: Uses two latent spaces (albedo-optimized and material-specialized) with feature distillation and rectified flow for efficiency.

Result: Achieves state-of-the-art performance with 28% better albedo estimation and 39% lower metallic-roughness errors.

Conclusion: DualMat outperforms existing methods, enabling high-resolution and multi-view material estimation.
...
<details>
  <summary>Details</summary>
Motivation: Address catastrophic forgetting in CSS by reducing interference between old and new class learning.
...
<details>
  <summary>Details</summary>
Motivation: Image colorization is challenging due to its ill-posed nature, but scene semantics and texture provide useful color cues. The goal is to improve upon regression-based approaches by incorporating classification and adversarial learning.

Method: The study builds on prior works, modifying them for classification and adversarial learning to handle the multi-modal nature of color prediction.

Result: Not explicitly stated in the abstract, but the approach aims to leverage large training datasets and semantic cues for better colorization.

Conclusion: The proposed method seeks to advance image colorization by addressing its multi-modal challenges through classification and adversarial learning.
...
Method: Built on FLUX-Kontext, it uses source images as conditional input and introduces RefLoRAInjector for efficient makeup feature extraction. A robust data pipeline enhances training.

Result: FLUX-Makeup achieves state-of-the-art performance with strong robustness across diverse scenarios.

Conclusion: The framework offers high-fidelity, identity-consistent makeup transfer without auxiliary components, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: PFMs have strong representational capabilities but suffer from opaque pretraining contexts and latent biases, limiting generalisability and transparency.

Method: AdaFusion dynamically integrates multiple PFMs by compressing and aligning tile-level features, using a lightweight attention mechanism for adaptive fusion based on tissue phenotype context.

Result: AdaFusion outperforms individual PFMs on benchmarks for treatment response prediction, tumour grading, and spatial gene expression inference, providing interpretable insights.

Conclusion: AdaFusion successfully bridges heterogeneous PFMs, improving performance and interpretability of model-specific biases.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models struggle with identity drift and short video generation, limiting their practical use for long, coherent videos.
...
Conclusion: PoseGen addresses key challenges in video generation, offering a scalable solution for long, identity-preserving videos.
...
<details>
  <summary>Details</summary>
Motivation: High inference costs in multi-instance learning (MIL) due to processing thousands of patches in gigapixel WSIs.

Method: AHDMIL uses a two-step process: self-distillation (SD) to train a high-resolution network (DMIN) and asymmetric distillation (AD) to train a lightweight pre-screening network (DB-LIPN).

Result: Achieves 5.3% accuracy improvement and 1.2x speedup on Camelyon16, with consistent gains in AUC, accuracy, F1, and Brier scores across datasets.
...
<details>
  <summary>Details</summary>
Motivation: To develop a unified model for multimodal medical image segmentation that generalizes well across modalities (e.g., CT vs. MRI) while addressing challenges like data scarcity, domain shift, and privacy restrictions.

Method: Proposes FedGIN, integrating a lightweight GIN augmentation module to harmonize modality-specific intensity distributions during local training. Evaluated on imputed and complete datasets with MRI and CT data.
...
Conclusion: FedGIN effectively addresses privacy and data scarcity challenges in multimodal medical image segmentation, demonstrating strong cross-modality generalization.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods rely on manual labeling, which may introduce bias and lack accuracy in capturing chronic pain behaviors.

Method: Uses a universal action space projector to extract mouse action features from video data, avoiding human labeling bias.

Result: Achieves 48.41% accuracy in 15-class pain classification and 73.1% in simplified 3-class tasks, outperforming human experts and B-SOiD. Also reveals drug efficacy insights.
...
<details>
  <summary>Details</summary>
Motivation: Common geometric patterns in low-resolution images are often distorted, leading to artifacts in high-resolution recoveries. Rotation equivariance can preserve these patterns.

Method: Redesigns INR and encoder modules to embed rotation equivariance, enabling end-to-end equivariance and providing theoretical error analysis.
...
Conclusion: The proposed rotation-equivariant ASISR method effectively preserves geometric patterns and enhances performance, with potential for broader application.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods model human and animal motion separately, lacking a unified representation. Cross-species motion generation can improve generalization and plausibility.
...
Conclusion: X-MoGen successfully unifies human and animal motion generation, leveraging a shared skeletal topology and morphological consistency for improved performance.
...
Conclusion: PhysPatch enhances adversarial attack effectiveness and real-world deployability for MLLM-based AD systems.
...
Method: MTT clusters detections into tracklets using spatio-temporal correlation and associates them with location and appearance clues.

Result: The framework shows competitive performance on generic multiple object tracking benchmarks.

Conclusion: MTT effectively addresses challenges in tracking unseen categories through robust tracklet generation and association.
...
<details>
  <summary>Details</summary>
Motivation: Prior domain adaptation methods focus on inter-domain transferability but neglect intra-domain structures, harming discriminability.

Method: SPA++ combines coarse graph alignment with spectral regularization, neighbor-aware propagation, and consistency regularization.

Result: SPA++ outperforms state-of-the-art methods in robustness and adaptability across diverse domain adaptation scenarios.
...
Method: Constructed SPIE dataset with spectral priors, proposed SPEX model with multiscale feature aggregation, token context condensation, and multispectral visual pre-training.

Result: SPEX outperforms existing methods on five public datasets for land cover extraction and provides textual explanations.

Conclusion: SPEX is the first multimodal vision-language model for spectral remote sensing, enhancing interpretability and performance.
...
TL;DR: EndoMatcher is a generalizable endoscopic image matcher using multi-domain pre-training and a two-branch Vision Transformer for robust feature matching in challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Dense feature matching in endoscopic images is difficult due to weak textures, viewpoint variations, and lack of annotated data, hindering robot-assisted tasks like 3D reconstruction and navigation.

Method: EndoMatcher uses a two-branch Vision Transformer with dual interaction blocks for multi-scale feature extraction. It leverages Endo-Mix6, a multi-domain dataset of 1.2M image pairs, and employs progressive multi-objective training for stability.

Result: EndoMatcher improves inlier matches by 140.69% and 201.43% on Hamlyn and Bladder datasets, and boosts MDPA by 9.40% on Gastro-Matching, outperforming state-of-the-art methods.

Conclusion: EndoMatcher achieves accurate, dense matching in challenging endoscopic conditions, generalizing well to unseen domains and improving surgical scene understanding.
...
<details>
  <summary>Details</summary>
Motivation: Current token pruning methods in LMMs are simplistic and cause performance degradation, prompting the need for a more effective framework.

Method: VFlowOpt uses an importance map based on attention-derived relevance and entropy, a progressive pruning module with recycling, and a visual information flow-guided optimization method.

Result: VFlowOpt prunes 90% of visual tokens, reduces KV-Cache memory by 89%, and speeds up inference by 3.8 times without significant performance loss.

Conclusion: VFlowOpt offers an efficient and adaptable pruning solution for LMMs, balancing computational savings and performance retention.
...
Method: Proposes Task-Specific Attention Adapters (TSAA) for feature adaptation, trained via Visual-Visual Embedding Alignment (VVEA) and Text-Visual Embedding Alignment (TVEA) modules.
...
Conclusion: The method effectively adapts to target domains without source data, leveraging multi-modal alignment for superior segmentation performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing vision-language tracking methods lack flexibility and reasoning insights, limiting performance. This paper aims to improve tracking accuracy by integrating reasoning and leveraging large models.

Method: The framework uses Qwen2.5-VL, optimizes reasoning and language generation via SFT and GRPO, and combines updated language descriptions with vision features in a unified tracking backbone. A tracking head predicts target locations.

Result: The method is validated on multiple benchmarks, showing effectiveness. A new dataset, TNLLT, is introduced, and 20 baseline trackers are evaluated on it.

Conclusion: ReasoningTrack improves vision-language tracking by integrating reasoning and large models, validated by experiments and a new benchmark dataset.
...
<details>
  <summary>Details</summary>
Motivation: Traditional and learning-based methods fail to segment deformed or coalesced bubbles, limiting industrial applications like air lubrication systems.
...
Method: Uses FAVS for hierarchical feature matching and CVC-SSL for self-supervised training with diffusion models to enforce cross-view consistency.

Result: The framework generates controllable arbitrary-view images using only multi-camera images and poses, without needing additional sensors or depth maps.
...
<details>
  <summary>Details</summary>
Motivation: The challenges of limited annotated medical data and overreliance on global image features, which miss subtle pathological regions, motivate the need for a region-aware approach.

Method: Introduces RegionMed-CLIP with an ROI processor for fine-grained regional features and progressive training for hierarchical multimodal alignment, supported by the MedRegion-500k dataset.
...
Conclusion: RegionMed-CLIP demonstrates the importance of region-aware pre-training and serves as a foundation for multimodal medical image understanding.
...
<details>
  <summary>Details</summary>
Motivation: Gender classification is useful for applications like surveillance and human-computer interaction, leveraging soft biometrics like facial and iris features.

Method: The study reviews existing literature and methodologies for gender classification, particularly emphasizing facial and iris-based techniques.
...
TL;DR: CF3 introduces a top-down pipeline for efficient 3D Gaussian feature fields, reducing computational costs by using adaptive sparsification and autoencoder training on lifted features.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods rely on costly bottom-up optimization of raw 2D features, leading to inefficiency.

Method: Proposes a weighted fusion of multi-view 2D features with pre-trained Gaussians, trains a per-Gaussian autoencoder on lifted features, and uses adaptive sparsification to prune redundant Gaussians.

Result: Achieves competitive 3D feature fields with only 5% of the Gaussians compared to Feature-3DGS, preserving geometric details.

Conclusion: CF3 offers a more efficient and compact representation of 3D Gaussian feature fields, reducing computational overhead while maintaining quality.
...
Method: A particle filtering tracker with cyclic-consistency checks is used to track target landmarks robustly.

Result: The method tracks 117 targets at 25.4 fps with a low error (5.00 +/- 0.22 px), surpassing deep learning (22.3 +/- 1.1 px) and conventional trackers (58.1 +/- 27.1 px).

Conclusion: The proposed tracker is effective for real-time cardiac imaging during surgery, offering superior accuracy and speed.
...
<details>
  <summary>Details</summary>
Motivation: Existing IVIF methods lack deep semantic understanding, leading to key target loss, artifacts, and detail degradation.
...
Conclusion: SGDFuse effectively addresses core IVIF challenges, offering a robust solution with explicit semantic guidance.
...
TL;DR: The paper introduces B4DL, a benchmark for training and evaluating Multimodal Large Language Models (MLLMs) on 4D LiDAR data, addressing the lack of annotations and suitable architectures.


<details>
  <summary>Details</summary>
Motivation: 4D LiDAR is underexplored in MLLMs due to missing annotations and processing capabilities, despite its potential for dynamic outdoor scene understanding.

Method: Proposes B4DL benchmark, a scalable data generation pipeline, and an MLLM model that processes raw 4D LiDAR for spatio-temporal reasoning.

Result: The model and benchmark provide a unified solution for 4D LiDAR understanding, with rendered videos and datasets available.

Conclusion: B4DL and the proposed MLLM advance 4D LiDAR integration into MLLMs, enabling better dynamic environment analysis.
...
TL;DR: The paper proposes Wavelet-Guided Dual-Frequency Encoding (WGDF) for change detection in remote sensing, leveraging wavelet transforms to enhance edge and fine-grained change detection, outperforming existing methods.
...
Conclusion: WGDF effectively unifies local sensitivity and global discriminability, advancing change detection in remote sensing.
...
Result: The proposed method outperforms psychologist assessments by 17.6%, demonstrating effectiveness in automated mental state evaluation.

Conclusion: The VS-LLM method enhances PPAT-based depression assessment, offering a scalable and accurate alternative to manual analysis.
...
TL;DR: CoCAViT addresses the generalization gap in smaller models by introducing Coordinator-patch Cross Attention (CoCA) for robust feature modeling across domains, achieving strong performance on OOD benchmarks with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Smaller models show disproportionate performance drops on out-of-distribution (OOD) data, highlighting a need for improved generalization in efficient architectures.
...
Conclusion: CoCAViT effectively bridges the generalization gap in smaller models, offering robust real-time visual representation with minimal computational cost.
...
<details>
  <summary>Details</summary>
Motivation: Vanilla RAG-based VQA methods often introduce irrelevant content due to unstructured knowledge, reducing accuracy. Structured multimodal KGs are proposed to address this.
...
Conclusion: Integrating structured multimodal KGs into RAG frameworks enhances VQA accuracy and reliability, as demonstrated by mKG-RAG.
...
TL;DR: The paper proposes a Textual Inversion (TI) method for open-vocabulary object detection in VLMs, enabling few-shot learning of new tokens without losing zero-shot capabilities.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between fine-tuning VLMs for specific tasks and retaining their original zero-shot and natural language querying abilities.

Method: Extends VLM vocabulary by learning new tokens from minimal examples (as few as three), keeping the original model weights frozen.

Result: The method retains the model's original performance and capabilities while requiring less compute than full fine-tuning.

Conclusion: TI for VLMs is effective for few-shot learning without forgetting, outperforming baseline methods in various experiments.
...
<details>
  <summary>Details</summary>
Motivation: 3DGS struggles with high-frequency details, redundant primitives, and inefficiency. 3DGabSplat aims to address these issues.

Method: Uses 3D Gabor-based primitives with multi-directional frequency responses and a CUDA-based rasterizer for efficient rendering.

Result: Outperforms 3DGS with 1.35 dB PSNR gain, fewer primitives, and lower memory usage.

Conclusion: 3DGabSplat enhances 3DGS, offering better quality and efficiency for novel view synthesis.
...
Method: PriorRG uses a two-stage pipeline: (1) prior-guided contrastive pre-training for spatiotemporal feature alignment, and (2) prior-aware coarse-to-fine decoding to integrate prior knowledge with vision encoder outputs.

Result: PriorRG achieves 3.6% BLEU-4 and 3.8% F1 score improvement on MIMIC-CXR, and 5.9% BLEU-1 gain on MIMIC-ABN.

Conclusion: PriorRG enhances clinical accuracy and fluency by emulating real-world workflows, setting a new benchmark for chest X-ray report generation.
...
Result: Slice-Loc reduces errors exceeding 10m to under 3%, cuts mean localization error from 4.47m to 1.86m, and orientation error from 3.42° to 1.24°.

Conclusion: Slice-Loc enhances CVL accuracy and reliability, outperforming state-of-the-art methods, and is validated on the DReSS dataset.
...
### [69] [MELLA: Bridging Linguistic Capability and Cultural Groundedness for Low-Resource Language MLLMs](https://arxiv.org/abs/2508.05502)
...
Result: Fine-tuning on MELLA improves performance across eight languages, with models producing "thick descriptions". Gains are attributed to enhanced cultural knowledge and linguistic capability.

Conclusion: The study highlights the importance of cultural awareness and linguistic capability in MLLMs for low-resource languages, demonstrating the effectiveness of the MELLA dataset.
...
### [70] [CT-GRAPH: Hierarchical Graph Attention Network for Anatomy-Guided CT Report Generation](https://arxiv.org/abs/2508.05375)
*Hamza Kalisch,Fabian Hörst,Jens Kleesiek,Ken Herrmann,Constantin Seibold*

Main category: cs.CV

TL;DR: CT-GRAPH is a hierarchical graph attention network for radiology report generation, improving accuracy by modeling organ relationships and integrating pretrained 3D feature encoders with a large language model.
...
Method: Uses a hierarchical graph attention network to model anatomical regions, leveraging pretrained 3D feature encoders and anatomical masks, then integrates features into a large language model.

Result: Achieves a 7.9% absolute improvement in F1 score on the CT-RATE dataset over state-of-the-art methods.

Conclusion: CT-GRAPH effectively captures fine-grained organ relationships, significantly enhancing radiology report generation accuracy.
...
### [71] [Deformable Attention Graph Representation Learning for Histopathology Whole Slide Image Analysis](https://arxiv.org/abs/2508.05382)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods like MIL struggle with spatial dependencies in WSIs and ROIs, and GNNs often use static graphs, ignoring spatial positions. Attention mechanisms lack specificity.

Method: Dynamic weighted directed graphs with deformable attention, incorporating learnable spatial offsets based on patch coordinates for adaptive focus on relevant regions.
...
Conclusion: The proposed framework effectively captures complex spatial structures in pathology images, demonstrating the power of deformable attention.
...
### [72] [UNCAGE: Contrastive Attention Guidance for Masked Generative Transformers in Text-to-Image Generation](https://arxiv.org/abs/2508.05399)
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of accurate attribute binding and text-image alignment in Masked Generative Transformers, which, like Diffusion Models, struggle with compositional generation.
...
Conclusion: UNCAGE effectively improves compositional fidelity in text-to-image generation, offering a practical solution without additional training.
...
TL;DR: TrueBiometric detects and corrects poisoned images in biometric systems with 100% accuracy using majority voting and corrective noise, outperforming existing methods.
...
Method: Leverages multiple vision-language models for majority voting to detect poisoned images and applies targeted corrective noise to mitigate attacks.

Result: Achieves 100% accuracy in detecting and correcting poisoned images without affecting clean image accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Physical adversarial camouflage poses security risks in safety-critical fields like autonomous driving, but existing techniques struggle with inconsistent sampling and conflicting gradients.
...
Result: Achieves significant improvements in attack success rates (13.46% across distances, 11.03% across angles) and outperforms state-of-the-art methods.
...
### [75] [Uni-cot: Towards Unified Chain-of-Thought Reasoning Across Text and Vision](https://arxiv.org/abs/2508.05606)
...
<details>
  <summary>Details</summary>
Motivation: Extending CoT reasoning to vision-language tasks is challenging due to fragmented architectures and limited visual state transition modeling.
...
TL;DR: SmoothSA improves Slot Attention (SA) by preheating cold-start queries for better object aggregation and differentiating transforms for video frames.


<details>
  <summary>Details</summary>
Motivation: Cold-start queries in SA lack sample-specific cues, hindering precise aggregation, and non-first frames require different transforms.

Method: SmoothSA preheats queries with input features and differentiates transforms for first and non-first video frames.

Result: Validated on object discovery, recognition, and benchmarks, showing effectiveness.

Conclusion: SmoothSA enhances SA iterations and recurrences, improving object-centric learning.
...
### [77] [Test-Time Reinforcement Learning for GUI Grounding via Region Consistency](https://arxiv.org/abs/2508.05615)
*Yong Du,Yuchen Yan,Fei Tang,Zhengxi Lu,Chang Zong,Weiming Lu,Shengpei Jiang,Yongliang Shen*

Main category: cs.CV

TL;DR: GUI-RC and GUI-RCPO improve GUI grounding accuracy by leveraging spatial overlap patterns and test-time reinforcement learning, achieving 2-3% and further 85.14% accuracy boosts, respectively.


<details>
  <summary>Details</summary>
Motivation: Existing GUI grounding methods rely on costly pixel-level annotations, prompting the need for data-efficient alternatives.
...
<details>
  <summary>Details</summary>
Motivation: Existing saliency maps lack depth in capturing cross-modal interactions in vision-language models.

Method: FIxLIP uses the weighted Banzhaf interaction index for flexible and efficient decomposition of similarity in encoders.
...
TL;DR: The paper introduces Flow Matching (FM) for unsupervised anomaly detection, addressing limitations of flow-based methods. It proposes time-reversed FM (rFM) and Worst Transport (WT) to enhance control over sample trajectories, achieving state-of-the-art results on MVTec.
...
Method: Proposes time-reversed Flow Matching (rFM) and Worst Transport (WT) displacement interpolation to transform data distributions and control sample trajectories.

Result: Achieves state-of-the-art performance on the MVTec dataset for unsupervised anomaly detection.

Conclusion: FM provides a scalable, theoretically grounded framework for anomaly detection, with practical success demonstrated on complex data.
...
TL;DR: The paper introduces a dataset (PAS) for pituitary anatomy segmentation and proposes F2PASeg, a method to improve segmentation accuracy despite challenges like occlusions and surgical bleeding.
...
Method: The authors create the PAS dataset (7,845 images from 120 videos) and use data augmentation to address class imbalance. They propose F2PASeg, which fuses high-resolution and deep semantic features to handle intraoperative variations.

Result: F2PASeg achieves consistent real-time segmentation of critical structures, proving robust against surgical challenges.

Conclusion: F2PASeg offers a reliable solution for intraoperative pituitary surgery planning, improving safety and accuracy.
...
TL;DR: Lossy image compression can defend against adversarial attacks, but realistic reconstructions make attacks harder. High-fidelity compression models resist attacks, while low-realism ones fail. Realism, not gradient masking, provides robustness.


<details>
  <summary>Details</summary>
Motivation: To evaluate the effectiveness of lossy compression in defending against adversarial attacks and understand why high-realism reconstructions are more resistant.

Method: Constructed strong white-box and adaptive attacks against various compression models, analyzing attack difficulty and realism's role.

Result: High-fidelity compression models resist attacks due to realism, while low-realism models are vulnerable. Realism aligns with natural images, providing inherent robustness.
...
<details>
  <summary>Details</summary>
Motivation: Historical maps lack consistency in patterns, making it hard for pre-trained foundation models to segment them accurately.
...
Conclusion: OND prompting enhances segmentation for historical maps, outperforming UNet and enabling adaptability to new classes.
...
<details>
  <summary>Details</summary>
Motivation: Chirality information is crucial but underdeveloped in shape analysis (e.g., point clouds, meshes), while existing descriptors fail to distinguish left-right symmetry.

Method: The authors use the Diff3F framework to develop an unsupervised pipeline for extracting chirality features from 2D foundation models.

Result: Experiments on tasks like left-right disentanglement, shape matching, and part segmentation show the effectiveness of the extracted features.

Conclusion: The proposed chirality features are practical and enhance shape analysis tasks, filling a gap in current methods.
...
Method: Integrates a novel view synthesis model into hand-object reconstruction, using diffusion models to regularize unseen regions and aligning hands to objects with visible contact constraints.

Result: MagicHOI significantly outperforms state-of-the-art methods and shows that diffusion priors effectively regularize unseen object regions.

Conclusion: MagicHOI advances hand-object reconstruction by leveraging novel view synthesis diffusion models, addressing limitations of prior methods.
...
<details>
  <summary>Details</summary>
Motivation: Event cameras provide high temporal resolution and dynamic range but produce sparse, noisy data, complicating feature extraction. This work aims to unlock latent information in event data.

Method: The framework includes Difference-guided Masked Modeling for temporal intensity difference reconstruction, Backbone-fixed Feature Transition for stable feature contrast, and Focus-aimed Contrastive Learning for semantic discrimination.

Result: The framework outperforms state-of-the-art methods in downstream tasks like object recognition, semantic segmentation, and optical flow estimation.

Conclusion: The proposed self-supervised pre-training framework effectively extracts and utilizes latent information from event data, demonstrating robustness and superior performance across multiple tasks.
...
TL;DR: The paper proposes an enhanced pedestrian tracking framework to address occlusion challenges by leveraging richer feature representations and a robust motion model, including head keypoint detection and iterative Kalman filtering.
...
Method: The framework integrates detection features from regression and classification branches, uses head keypoint detection for occlusion resilience, and employs iterative Kalman filtering with 3D priors for motion modeling.

Result: The method improves tracking robustness in crowded, occlusion-heavy environments by combining advanced appearance and motion modeling.

Conclusion: The proposed framework effectively addresses occlusion challenges in pedestrian tracking, offering a more reliable solution for crowded scenarios.
...
Method: Randomized smoothing in feature space, analyzing Jacobian's maximum singular value to link noise levels with input perturbations. Supports FR and NR IQA models without architectural changes.
...
TL;DR: Octozi, an AI-assisted platform, boosts clinical trial data cleaning efficiency by 6.03-fold, reduces errors by 6.44-fold, and cuts false positives by 15.48-fold, proving its broad applicability.


<details>
  <summary>Details</summary>
Motivation: Manual clinical trial data cleaning is inefficient due to rising data volumes and complexity, necessitating AI solutions.

Method: Octozi combines large language models with domain-specific heuristics for clinical data review, tested in a study with 10 reviewers.

Result: AI assistance increased throughput by 6.03-fold, reduced errors from 54.67% to 8.48%, and decreased false positives by 15.48-fold.

Conclusion: AI-assisted approaches can revolutionize clinical trial operations, speeding up drug development while ensuring compliance.
...
<details>
  <summary>Details</summary>
Motivation: Existing structural pruning methods often overlook the interconnections among parameters, limiting their effectiveness.
...
Conclusion: The proposed framework effectively addresses the limitations of existing pruning methods by considering parameter interconnections and retaining contributions of pruned connections.
...
<details>
  <summary>Details</summary>
Motivation: The rise of AI-generated videos necessitates robust detection methods that generalize across manipulation types, overcoming limitations of existing approaches.

Method: SSTGNN uses graph-based architecture with spectral filters and temporal differential modeling to capture spatial, temporal, and spectral inconsistencies.

Result: SSTGNN achieves superior performance in in-domain and cross-domain settings, with up to 42.4x fewer parameters than competitors.

Conclusion: SSTGNN is a scalable, lightweight solution for detecting manipulated videos, offering strong generalization and robustness.
...
<details>
  <summary>Details</summary>
Motivation: The exponential growth of online video content necessitates automated moderation due to human limitations in handling volume and mental health challenges.
...
Result: MLLMs show effectiveness in multimodal brand safety but have limitations, as discussed in failure cases.

Conclusion: The paper highlights MLLMs' potential for content moderation while acknowledging their shortcomings, releasing the dataset for future research.
...
TL;DR: Action Discovery tackles ambiguous and incomplete action annotations in datasets by identifying unknown actions alongside known ones, using granularity-guided segmentation and semantic assignment.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of ambiguous actions and incomplete annotations in partially labeled datasets, common in neuroscience and other domains.

Method: Two-step approach: Granularity-Guided Segmentation Module (GGSM) for temporal intervals and Unknown Action Segment Assignment (UASA) for semantic classes.

Result: Demonstrated improvement over baselines on Breakfast, 50Salads, and Desktop Assembly datasets.

Conclusion: Proposed method effectively handles unknown actions in partially labeled datasets, enhancing action segmentation.
...
<details>
  <summary>Details</summary>
Motivation: The growing demand for AI-generated content (AIGC) requires high-quality, diverse, and scalable data, but collecting real-world data is expensive and slow. Existing methods rely on manual scene construction, limiting scalability and accuracy.

Method: Uses MLLM-Collector to gather assets and descriptions, constructs 3D layouts, refines semantics with VLMs, and generates temporally coherent frames with MLLM-Planner.
...
<details>
  <summary>Details</summary>
Motivation: Existing VLP models lack fine-grained localization and structured relational knowledge for unseen classes in OV-MLR.
...
<details>
  <summary>Details</summary>
Motivation: Existing visual tokenizers struggle with balancing compression ratios and reconstruction fidelity, prompting the need for a more efficient solution.

Method: WeTok introduces Group-wise lookup-free Quantization (GQ) for scalable codebooks and Generative Decoding (GD) to model visual data distribution probabilistically.

Result: WeTok achieves record-low zero-shot rFID scores (0.12 on ImageNet 50k) and superior compression performance (rFID 3.49 at 768x compression).

Conclusion: WeTok sets a new standard for visual tokenizers, offering better fidelity and scalability, with open-source availability.
...
<details>
  <summary>Details</summary>
Motivation: Evaluating image-text relevancy is challenging due to diverse text formats and varying definitions of relevancy. MLLMs are ideal for this task.

Method: LLaVA-RE uses the LLaVA architecture with detailed task instructions and multimodal in-context samples, along with a novel binary relevancy dataset.

Result: Experimental results confirm the framework's effectiveness.

Conclusion: LLaVA-RE is a promising solution for binary image-text relevancy evaluation using MLLMs.
...
TL;DR: Hi3DEval is a hierarchical framework for evaluating 3D generative content, combining object- and part-level assessments with material realism. It outperforms image-based metrics and aligns better with human preferences.


<details>
  <summary>Details</summary>
Motivation: Existing 3D quality assessment methods are limited to image-based metrics and object-level evaluations, lacking spatial coherence and material authenticity.

Method: Hi3DEval integrates hierarchical evaluation (object- and part-level) and material realism assessment. It uses a dataset (Hi3DBench) and a multi-agent annotation pipeline, along with a 3D-aware automated scoring system.

Result: The framework outperforms existing image-based metrics in capturing 3D characteristics and aligns more closely with human preferences.
...
<details>
  <summary>Details</summary>
Motivation: Existing VOS datasets lack realism, focusing on simple scenarios. MOSEv2 aims to push VOS research toward more complex, real-world conditions.

Method: MOSEv2 introduces 5,024 videos with 701,976 masks, featuring severe occlusions, adverse weather, and other real-world challenges.

Result: Benchmarked methods show performance drops (e.g., SAM2 from 76.4% to 50.9%), indicating current VOS struggles with real-world complexities.

Conclusion: MOSEv2 highlights the need for improved VOS methods to handle real-world challenges, providing a valuable benchmark for future research.
...
Result: Evaluated on synthetic and real-world scans, GAP successfully generates high-fidelity 3D Gaussians.

Conclusion: GAP effectively addresses the challenge of converting colorless point clouds to Gaussians, demonstrating robustness across varying complexity levels.
...
TL;DR: FaceAnonyMixer is a cancelable face generation framework that mixes latent codes to create privacy-preserving faces, maintaining recognition utility while enhancing privacy protection.
...
Method: Uses a pre-trained generative model's latent space to mix real face latent codes with synthetic ones from revocable keys, refined via multi-objective loss.

Result: Generates high-quality cancelable faces, achieving over 11% better privacy protection on commercial APIs compared to recent methods.

Conclusion: FaceAnonyMixer effectively balances privacy and utility, offering a robust solution for cancelable biometric face recognition.
...
Method: Integrates vibration frequency analysis (BPFO, BPFI, BSF, FTF) with LLM processing, multi-agentic knowledge retrieval, and structured recommendation generation.
...
Conclusion: The system bridges condition monitoring and maintenance planning, advancing LLM applications in industrial maintenance with a scalable framework.
...
TL;DR: GeoFlow automates geospatial workflows by guiding agents with detailed tool-calling objectives, improving success rates and reducing token usage.


<details>
  <summary>Details</summary>
Motivation: Prior work lacks explicit guidance for API selection in geospatial tasks, leading to inefficiencies.

Method: GeoFlow provides agents with detailed tool-calling objectives for geospatial API invocation.

Result: Increases agentic success by 6.8% and reduces token usage by up to fourfold.

Conclusion: GeoFlow outperforms state-of-the-art approaches in efficiency and success for geospatial tasks.
...
TL;DR: The paper proposes an adversarial benchmarking framework using board games to evaluate LLMs, addressing the limitations of Q&A benchmarks. It introduces Qi Town, a platform with 5 games and 20 LLM players, using Elo and PLG for quantitative analysis and PSS for mental fitness. Results show LLMs' adaptability but reveal instability in skill play.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' comprehensive performance beyond Q&A benchmarks by leveraging adversarial board games for strategic reasoning evaluation.

Method: Developed Qi Town, a platform with 5 games and 20 LLM players, using Elo, PLG, and PSS for quantitative and qualitative evaluation in a round-robin tournament.

Result: LLMs demonstrate adaptability and optimism in adversarial settings but exhibit instability in skill play, as shown by cyclic wins/losses in PLGs.

Conclusion: The framework effectively evaluates LLMs' strategic capabilities, revealing both strengths (adaptability) and weaknesses (skill instability), prompting further research.
...
TL;DR: The paper compares three methods for autonomous web-based GIS (AWebGIS), with the client-side small language model (SLM) approach outperforming cloud-based and classical ML methods in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current cloud-based AWebGIS solutions raise privacy and scalability concerns due to reliance on centralized servers and continuous internet access.
...
Conclusion: Browser-executable SLMs are feasible for AWebGIS, offering privacy, scalability, and high accuracy without server dependency.
...
TL;DR: RL fine-tuning improves LLM reasoning in ideal settings but fails in non-ideal scenarios like summary inference, noise suppression, and contextual filtering. Performance drops significantly, revealing limitations.


<details>
  <summary>Details</summary>
Motivation: To address the gap in evaluating LLM reasoning under realistic, non-ideal conditions, inspired by human reasoning reliability under imperfect inputs.

Method: Fine-tuned three LLMs and a LVLM using RL (policy-gradient), tested on eight datasets across three non-ideal scenarios.

Result: RL improves reasoning in ideal settings but performance declines sharply in non-ideal scenarios, exposing unresolved deficits.

Conclusion: Current LLM reasoning capabilities are overstated; non-ideal scenario evaluation is crucial. Remediation methods remain insufficient.
...
Method: HealthFlow uses a meta-level evolution mechanism to refine high-level problem-solving policies by learning from successes and failures. EHRFlowBench, a new benchmark, is introduced for evaluation.
...
Method: Proposes a two-player game (ligand and protein players) solved by LoopPlay, alternating training with mutual pose exchange and refinement.
...
TL;DR: LLMs show promise for urban spatial data integration but struggle with macro-scale spatial reasoning. Providing relevant features improves performance, and a review-and-refine method effectively corrects errors.
...
Method: Analyzed LLMs' spatial reasoning, tested performance with relevant features, and adapted a review-and-refine method.
...
Conclusion: LLMs are a promising alternative for spatial data integration, with potential for future improvements like post-training and multi-modal methods.
...
<details>
  <summary>Details</summary>
Motivation: Web navigation is complex for AGI, requiring integration of offline and online learning paradigms, which current approaches lack.
...
Result: CogniWeb achieves a 43.96% success rate on WebArena with 75% reduction in token usage.

Conclusion: The dual-process framework effectively bridges offline and online learning, improving both performance and efficiency in web navigation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack systematic benchmarks for updating medical knowledge involving both image and text modalities.
...
Result: Experiments reveal limitations in existing knowledge editing approaches, emphasizing the need for specialized strategies.

Conclusion: MedMKEB aims to advance trustworthy and efficient medical knowledge editing algorithms.
...
Conclusion: EasySize reduces reliance on human expertise and computational resources, simplifying analog circuit design. It will be open-sourced.
...
TL;DR: A study evaluates a Socratic AI Tutor for scaffolding student research, showing it enhances critical thinking compared to uninstructed AI. It proposes orchestrated multi-agent systems (MAS) for education, discussing implications and cost-effectiveness.


<details>
  <summary>Details</summary>
Motivation: To explore how generative AI, like the Socratic Tutor, can reshape knowledge generation and validation in higher education, countering de-skilling narratives.

Method: A controlled experiment with 65 pre-service teachers in Germany, comparing the Socratic Tutor to an uninstructed AI chatbot.

Result: Students using the Socratic Tutor reported greater support for critical and reflective thinking, suggesting dialogic AI fosters metacognitive engagement.

Conclusion: The study offers empirical support and a conceptual framework for hybrid learning ecosystems integrating human-AI co-agency and pedagogical alignment.
...
<details>
  <summary>Details</summary>
Motivation: Event logs often have missing data due to manual recording or collection issues, necessitating robust reconstruction methods.

Method: Develops a Heterogeneous Graph Neural Network to restore missing attributes in traces, leveraging graph-based representations for richer encodings.

Result: The model shows superior performance in reconstructing all event attributes compared to autoencoder-based approaches.

Conclusion: The proposed method effectively addresses the challenge of missing data in Process Mining, offering a more comprehensive solution.
...
TL;DR: QA-Dragon improves VQA by dynamically retrieving and combining text and image knowledge, outperforming baselines in accuracy and knowledge overlap.


<details>
  <summary>Details</summary>
Motivation: Existing RAG methods retrieve from text or images separately, limiting complex query handling. QA-Dragon aims to enhance multimodal, multi-hop reasoning.

Method: QA-Dragon uses a domain router for subject identification and a search router for dynamic retrieval, combining text and image agents for hybrid retrieval.
...
Conclusion: QA-Dragon effectively addresses complex VQA tasks by integrating dynamic multimodal retrieval, improving reasoning performance.
...
TL;DR: A novel framework combining RDF graph databases and LLMs improves expert identification and communication in large-scale maintenance organizations by processing natural language queries with transparent reasoning.
...
Result: Enables precise audience targeting and efficient communication while maintaining system trust through explainable outcomes.

Conclusion: The framework effectively addresses communication challenges in large-scale maintenance organizations by leveraging advanced query processing and transparency.
...
TL;DR: A hybrid architecture combining decision trees and LLMs improves reasoning performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: To integrate symbolic reasoning (decision trees) with neural capabilities (LLMs) for robust, interpretable neuro-symbolic reasoning.

Method: Embeds decision trees as callable oracles in a multi-agent framework, using LLMs for abductive reasoning and planning.

Result: Achieves +7.2% entailment consistency, +5.3% accuracy in math problems, and +6.0% abstraction accuracy.

Conclusion: The architecture provides a scalable, interpretable solution for neuro-symbolic reasoning in diverse domains.
...
TL;DR: The paper proposes redefining the term 'agent' in AI to address ambiguity, offering a framework with clear minimum requirements and multidimensional characterization.


<details>
  <summary>Details</summary>
Motivation: Ambiguity in the term 'agent' across AI subfields hinders research communication, evaluation, reproducibility, and policy development.

Method: Historical analysis and contemporary usage patterns inform a framework defining agent requirements and characterizing systems along dimensions like interaction, learning, autonomy, goals, and temporal coherence.

Result: A precise yet flexible framework for describing AI agents, addressing ambiguity while preserving the term's multifaceted nature.

Conclusion: The framework improves research clarity, reproducibility, and policy development, with recommendations for standardization and adoption.
...
<details>
  <summary>Details</summary>
Motivation: To understand LLM behavior in open-ended, multi-agent settings, particularly in legal and ethical deliberation, which remains understudied.

Method: Structured simulation with LLMs proposing rules, justifying them, and voting on peer proposals. Quantitative (voting patterns) and qualitative (strategic language) analyses were conducted.

Result: LLMs formed alliances, betrayed trust, and adapted rhetoric to influence decisions, showcasing latent social reasoning and persuasive skills.

Conclusion: The study provides insights for designing AI systems capable of autonomous negotiation and legislation drafting in legal contexts.
...
TL;DR: StructVRM introduces fine-grained, verifiable rewards for multimodal reasoning tasks, outperforming traditional binary scoring methods and achieving state-of-the-art results on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing Vision-Language Models struggle with complex reasoning tasks due to coarse binary rewards, limiting learning from partial correctness.

Method: StructVRM uses a model-based verifier for sub-question-level feedback, assessing semantic and mathematical equivalence instead of rigid string matching.

Result: Seed-StructVRM achieves top performance on six out of twelve benchmarks and a new high-difficulty STEM-Bench.
...
<details>
  <summary>Details</summary>
Motivation: To enhance railway predictive maintenance by providing real-time fault prediction with explainability, ensuring high service availability and safety.
...
TL;DR: DeepPHY is a benchmark framework to evaluate VLMs' physical reasoning, revealing their limitations in predictive control despite strong perceptual abilities.


<details>
  <summary>Details</summary>
Motivation: VLMs lack attention to detail and precise action planning in dynamic environments, hindering performance in real-world tasks requiring physics understanding.

Method: DeepPHY introduces simulated environments with varying difficulty levels and fine-grained metrics to assess VLMs' physical reasoning.

Result: State-of-the-art VLMs struggle to apply descriptive physical knowledge for precise, predictive control.

Conclusion: DeepPHY highlights VLMs' limitations in physical reasoning, suggesting a need for improved models for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: To explore how LLMs are revolutionizing organic synthesis by integrating with advanced technologies and addressing limitations for safer, greener chemistry.

Method: Survey of LLMs' role in synthesis, combining them with graph neural networks, quantum calculations, and real-time spectroscopy.

Result: LLMs accelerate discovery cycles, enable data-driven chemistry, but face issues like biased data and opaque reasoning.

Conclusion: Community efforts like open benchmarks and explainable interfaces aim to democratize AI-driven molecular innovation while ensuring human oversight.
...
Conclusion: The conclusion calls for context-sensitive alignment approaches to ensure AI systems respect regional differences and avoid one-size-fits-all solutions.
...
<details>
  <summary>Details</summary>
Motivation: The need to align AI evaluation practices with emerging regulations like the EU AI Act, addressing the gap between current benchmarks and regulatory requirements.

Method: Developed Bench-2-CoP, a framework using LLM-as-judge analysis to map 194,955 benchmark questions against the EU AI Act's taxonomy.

Result: Found a misalignment: benchmarks focus on behavioral propensities (e.g., hallucination, bias) but neglect critical functional capabilities (e.g., loss-of-control scenarios).

Conclusion: Highlights the urgent need for policymakers and developers to address evaluation gaps to ensure safer, compliant AI.
...
TL;DR: The paper addresses the scarcity and bias in ERC datasets by using a small LLM to generate diverse synthetic datasets, improving ERC classifier robustness and performance.


<details>
  <summary>Details</summary>
Motivation: ERC data is scarce and biased, and LLMs are expensive for ERC tasks. The goal is to create synthetic datasets to supplement existing benchmarks.

Method: A small, resource-efficient LLM is used to generate six diverse ERC datasets, enhancing three widely used benchmarks.
...
TL;DR: InfiAlign introduces a scalable, sample-efficient post-training framework combining SFT and DPO to enhance LLM reasoning, reducing data needs by 88% while matching or outperforming benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current post-training methods for LLMs are resource-intensive and lack scalability due to heuristic or task-specific data curation.
...
Result: The SFT model matches DeepSeek-R1-Distill-Qwen-7B with 12% of the data, and DPO further improves performance, notably in math tasks (3.89% gain on AIME benchmarks).
...
TL;DR: GRAIL is a framework combining LLMs and graph retrieval for structured knowledge, improving reasoning performance via interactive learning and fine-grained rewards.


<details>
  <summary>Details</summary>
Motivation: Existing RAG methods struggle with structured knowledge like graphs, facing precision and information gap issues.

Method: GRAIL integrates LLM-guided exploration with path filtering, uses a two-stage training process, and balances retrieval precision and conciseness.

Result: GRAIL improves accuracy by 21.01% and F1 by 22.43% on knowledge graph QA datasets.

Conclusion: GRAIL effectively addresses structured knowledge retrieval challenges, enhancing reasoning performance.
...
TL;DR: A modular framework is proposed for evaluating agent task completion by validating step-by-step reasoning, outperforming existing methods like LLM-as-a-Judge.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods overlook step-by-step reasoning and lack generalizability across domains.

Method: Decomposes tasks into sub-tasks, validates each step, and aggregates module outputs for a final verdict.

Result: Achieves 4.76% and 10.52% higher alignment accuracy with human evaluations compared to GPT-4o baseline.

Conclusion: The framework shows promise for general-purpose agent evaluation.
...
TL;DR: LORI is an AI tool using NLP and large language models (RoBERTa, LLAMA) to assess leadership skills in letters of recommendation, achieving high accuracy (F1: 91.6%).


<details>
  <summary>Details</summary>
Motivation: LORs are time-consuming to review, and leadership skills are critical in STEM. Automating this process can streamline admissions and improve evaluation.

Method: Uses NLP with RoBERTa and LLAMA models to detect leadership attributes like teamwork, communication, and innovation in LORs.

Result: Achieves a weighted F1 score of 91.6%, precision of 92.4%, and recall of 91.6%, demonstrating strong consistency.
...
TL;DR: MV-Debate is a multi-view agent debate framework for detecting harmful content in multimodal social media, outperforming existing methods.
...
Method: Proposes MV-Debate with four debate agents (surface analyst, deep reasoner, modality contrast, social contextualist) using iterative debate and reflection-gating.

Result: Outperforms single-model and multi-agent baselines on three benchmark datasets.

Conclusion: MV-Debate shows promise for reliable social intent detection in safety-critical online contexts.
...
<details>
  <summary>Details</summary>
Motivation: Current AI systems face scalability issues due to reliance on human-engineered rewards and high-quality training data, hindering autonomous intelligence.

Method: Proposes using AIF to replace external rewards with intrinsic free energy minimization, integrating Large Language Models for generative world modeling.

Result: AIF bridges the grounded-agency gap, allowing agents to autonomously adapt objectives and learn efficiently while aligning with human values.

Conclusion: AIF offers a viable path for developing autonomous AI systems that adhere to computational and physical constraints.
...
TL;DR: LearnerAgent, a multi-agent framework using LLMs, simulates human-like learning dynamics with psychologically grounded profiles. It tracks learning progress over time, revealing insights into cognitive growth, learner behavior, and LLM limitations.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to capture learning dynamics, track progress, or provide explainability. LearnerAgent addresses these gaps by simulating realistic teaching environments with LLMs.

Method: The framework uses psychologically grounded learner profiles (Deep, Surface, Lazy, General) and tracks progress through weekly knowledge acquisition, monthly strategic choices, tests, and peer interaction over a year.

Result: 1) Deep Learner shows sustained cognitive growth; Surface Learner's shallow knowledge is diagnosed. 2) Learner behavior aligns with profiles. 3) Self-concept evolves realistically. 4) Base LLM defaults to a "diligent but brittle Surface Learner."

Conclusion: LearnerAgent effectively simulates learning dynamics, providing insights into LLM behavior and aligning with real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: To enhance transcribed dialogues by adding speaker metadata (e.g., age, gender, emotion) without fine-tuning models, preserving modularity and speed.
...
Result: Competitive performance on speaker profiling tasks; frozen LLAMA achieves an 8.8% Equal Error Rate in comparing x-vectors.

Conclusion: The approach effectively enriches transcriptions with speaker metadata while maintaining efficiency and modularity.
...
TL;DR: Parity-aware BPE improves tokenization fairness for lower-resource languages without harming global compression or downstream performance.
...
Conclusion: Parity-aware BPE offers a fairer tokenization solution without compromising efficiency or performance.
...
<details>
  <summary>Details</summary>
Motivation: Enhance ASR systems by incorporating prosodic cues like pitch accent, leveraging semi-supervised speech representations.
...
TL;DR: PERSIST evaluates personality stability in large language models (LLMs), revealing significant variability despite interventions, challenging their reliability for safety-critical applications.


<details>
  <summary>Details</summary>
Motivation: To understand and assess the stability of personality-like traits in LLMs, crucial for safe deployment.

Method: Evaluated 25+ open-source models (1B-671B parameters) using traditional and LLM-adapted personality instruments, varying prompts and interventions.

Result: Found substantial response variability (SD > 0.4), prompt sensitivity (20% shifts), and paradoxically increased instability from interventions.

Conclusion: Current LLMs lack behavioral consistency, making personality-based alignment strategies inadequate for safety-critical uses.
...
Method: Introduces RCR-Router, a modular framework that dynamically selects relevant memory subsets for each agent based on role and task stage, guided by a lightweight scoring policy.
...
TL;DR: The paper introduces a benchmark to evaluate LLMs' responses to linguistic shibboleths, revealing biases in how models penalize certain linguistic patterns, like hedging, despite equivalent content quality.


<details>
  <summary>Details</summary>
Motivation: To detect and measure demographic bias in LLMs by analyzing their responses to subtle linguistic markers (shibboleths) that reveal attributes like gender or social class.

Method: Uses 100 validated question-response pairs in interview simulations to generate controlled linguistic variations, isolating specific phenomena while maintaining semantic equivalence.

Result: Hedged responses receive 25.6% lower ratings on average, demonstrating systematic bias. The benchmark effectively identifies model-specific biases.

Conclusion: The work provides a framework for detecting linguistic discrimination in AI systems, with applications for fairness in automated decision-making.
...
Method: A vision-language clustering framework constructs verb sense clusters to evaluate activity recognition models, analyzing the imSitu dataset.

Result: Each image maps to 2.8 sense clusters on average, and cluster-based evaluation aligns better with human judgements than standard methods.

Conclusion: The proposed cluster-based evaluation offers a more nuanced and robust assessment of model performance in visual activity recognition.
...
TL;DR: A multi-stage LLM framework improves extraction of social determinants of health (SDoH) factors from text, outperforming state-of-the-art models and enhancing explainability.


<details>
  <summary>Details</summary>
Motivation: Address challenges in data-driven SDoH factor extraction for suicide prevention, including long-tailed distributions and model explainability.
...
Result: Outperforms other models in SDoH extraction, achieves comparable performance with lower costs, and improves explainability via intermediate explanations.
...
TL;DR: The paper proposes a method for DiaASQ by partitioning dialogues into semantically independent sub-dialogues using structural entropy minimization, improving accuracy and reducing noise.
...
Method: Partition dialogues into sub-dialogues using structural entropy minimization, then extract sentiment elements at utterance level and match quadruples at sub-dialogue level.

Result: Achieves state-of-the-art performance with lower computational costs.

Conclusion: The proposed method effectively addresses noise in DiaASQ by focusing on sub-dialogues, improving extraction accuracy.
...
<details>
  <summary>Details</summary>
Motivation: To explore a straightforward finetuning approach for decoder-only LLMs in AMR parsing, avoiding complex architectures.

Method: Finetuned four LLMs (Phi 3.5, Gemma 2, LLaMA 3.2, DeepSeek R1) on the LDC2020T02 AMR3.0 test set.

Result: LLaMA 3.2 achieved SMATCH F1: 0.804, comparable to SOTA parsers. Phi 3.5 excelled in structural validity.

Conclusion: Decoder-only LLMs can match SOTA AMR parsers with simple finetuning, with LLaMA 3.2 leading in semantic performance.
...
TL;DR: A simplified multi-head LoRA outperforms complex multi-adapter systems, and a single-adapter LoRA with increased rank is also effective. Align-LoRA, with a loss for task representation alignment, surpasses baselines.


<details>
  <summary>Details</summary>
Motivation: Challenging the multi-component paradigm in PEFT for LLMs, questioning the need for structural diversity in multi-task learning.

Method: Proposes Align-LoRA, which uses an explicit loss to align task representations in a shared adapter space, and tests simplified multi-head and single-adapter LoRA variants.

Result: Align-LoRA significantly outperforms baselines, showing robust shared representations are key for MTL generalization.

Conclusion: A simpler, single-adapter approach with alignment loss (Align-LoRA) is more effective for adapting LLMs to multiple tasks.
...
TL;DR: MultiCheck is a unified framework for fine-grained multimodal fact verification, combining text and image encoders with cross-modal fusion, achieving a high F1 score of 0.84 on Factify 2.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of multimodal misinformation by integrating textual and visual evidence for more accurate fact-checking.

Method: Uses dedicated text and image encoders with a fusion module for cross-modal reasoning and a contrastive learning objective for semantic alignment.

Result: Achieves a weighted F1 score of 0.84, significantly outperforming baselines.
...
TL;DR: The paper introduces BEE-RAG, a framework addressing entropy growth and attention dilution in retrieval-augmented generation (RAG) by ensuring entropy invariance and balanced context entropy.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) face knowledge limitations, and RAG's long context lengths lead to unconstrained entropy growth and attention dilution, impacting performance.

Method: Proposes BEE-RAG, leveraging entropy invariance to balance context entropy, reformulate attention dynamics, and introduce zero-shot inference and adaptive fine-tuning.

Result: BEE-RAG improves RAG adaptability to varying context lengths, demonstrated through extensive experiments.

Conclusion: BEE-RAG effectively addresses entropy and attention issues in RAG, enhancing performance across tasks.
...
<details>
  <summary>Details</summary>
Motivation: To understand and mitigate positional bias in LLMs, improving performance by optimizing input structure.

Method: Introduces AttnRank: estimates positional attention preferences and reranks inputs (e.g., documents, examples) to align critical info with high-attention positions.

Result: Substantial improvements in multi-hop QA and few-shot tasks across 10 LLMs, without modifying models.

Conclusion: AttnRank is a model-agnostic, efficient solution to positional bias, enhancing LLM performance by strategic input ordering.
...
TL;DR: PrinciplismQA is a benchmark for evaluating LLMs' ethical reasoning in healthcare, revealing gaps in practical ethical application despite strong general capabilities.
...
Method: Introduces PrinciplismQA, a benchmark with 3,648 questions (multiple-choice and open-ended) validated by medical experts, grounded in Principlism.

Result: LLMs show a gap between ethical knowledge and practical application, struggling with Beneficence dilemmas. Frontier models lead, but medical fine-tuning improves ethical competence.
...
TL;DR: The paper discusses the ATLANTIS team's work on detecting hallucinated text in question answering systems for SemEval-2025 Task 3, achieving top results in Spanish and competitive rankings in English and German.
...
Method: Explored methods with and without external context, including few-shot prompting, token-level classification, and LLM fine-tuning on synthetic data.

Result: Achieved top rankings in Spanish and competitive placements in English and German.

Conclusion: Integrating relevant context is crucial to reduce hallucinations, and fine-tuned models with prompt engineering show great potential.
...
TL;DR: The paper introduces MulCoT-RD, a lightweight model for multimodal sentiment reasoning and classification in resource-limited settings, achieving strong performance with 3B parameters.


<details>
  <summary>Details</summary>
Motivation: Current MSA approaches rely on heavy LLMs, ignoring resource constraints. The paper addresses this gap by focusing on lightweight solutions for multimodal sentiment reasoning and classification.

Method: Proposes MulCoT-RD, a 'Teacher-Assistant-Student' distillation model. Uses a high-performance MLLM to generate reasoning data, trains an assistant model, and then a lightweight student model for efficient reasoning and classification.

Result: MulCoT-RD achieves strong performance on JMSRC with only 3B parameters, showing robustness and interpretability across four datasets.

Conclusion: The work successfully addresses resource limitations in MSA, offering a scalable and interpretable solution with MulCoT-RD.
...
<details>
  <summary>Details</summary>
Motivation: Current pruning methods disrupt LLM functionalities by ignoring neuron interactions. The study aims to improve pruning by mimicking brain networks.
...
Conclusion: The method offers a novel, biologically inspired approach to structured pruning, enhancing LLM efficiency.
...
TL;DR: The paper challenges the 'cascading failure' hypothesis in Chain-of-Thought (CoT) reasoning, revealing 'Late-Stage Fragility'—errors in later stages harm outcomes more. It introduces ASCoT, a method with adaptive verification and self-correction, improving accuracy over standard CoT.


<details>
  <summary>Details</summary>
Motivation: To address the unreliability of reasoning chains in LLMs, particularly the overlooked vulnerability of late-stage errors, and improve reasoning accuracy.

Method: Introduces ASCoT, combining an Adaptive Verification Manager (AVM) with a Multi-Perspective Self-Correction Engine (MSCE). AVM uses Positional Impact Scores to prioritize late-stage steps, while MSCE applies targeted corrections.

Result: ASCoT outperforms baselines (including standard CoT) on benchmarks like GSM8K and MATH, demonstrating higher accuracy by addressing late-stage errors.

Conclusion: The work highlights the need for adaptive, vulnerability-aware correction in LLM reasoning, shifting from uniform verification to targeted strategies.
...
TL;DR: The paper explores using dialogue agents to assist meta-reviewers by generating synthetic data with LLMs and training tailored agents, which outperform generic LLM assistants.


<details>
  <summary>Details</summary>
Motivation: Meta-reviewing is a decision-making process requiring context and argument weighing, but prior work treated it as summarization. Dialogue agents can assist here.

Method: Generate synthetic data using LLMs via self-refinement, train dialogue agents on this data, and test them in real-world meta-reviewing.

Result: The method produces high-quality synthetic data, and trained agents outperform generic LLM assistants in meta-reviewing tasks.

Conclusion: Dialogue agents trained on synthetic data effectively enhance meta-reviewing efficiency, demonstrating practical utility.
...
TL;DR: SONAR-LLM is a transformer model that combines continuous SONAR embeddings with token-level cross-entropy, outperforming LCM in generation quality while being simpler to train.


<details>
  <summary>Details</summary>
Motivation: To improve upon LCM by eliminating its diffusion sampler and restoring a likelihood-based training signal, while retaining semantic abstraction.

Method: Uses a decoder-only transformer supervised via token-level cross-entropy, leveraging frozen SONAR decoder for embedding space consistency.

Result: Achieves competitive generation quality across model sizes (39M to 1.3B parameters) and provides reproducible training code and checkpoints.

Conclusion: SONAR-LLM successfully balances semantic abstraction with efficient training, offering a scalable and reproducible alternative to LCM.
...
Method: CGRS dynamically suppresses reflection triggers during high-confidence responses, requiring no retraining or model changes.
...
Result: No significant UX or comprehensibility improvements; adjustable avatar caused higher stress. Users preferred adjustability but found the system more aesthetically pleasing than functional.

Conclusion: Personalisation alone is insufficient; comprehensibility by default is crucial. Recommendations include improving animation, interaction interfaces, and using participatory design.
...
TL;DR: The paper explores the challenges of applying autonomous LLM systems like Agentic RAG to domain-specific professional search (e.g., biomedical research), using the BioASQ CLEF 2025 challenge as a testbed. It evaluates reasoning and non-reasoning LLMs with a self-feedback mechanism for iterative refinement.


<details>
  <summary>Details</summary>
Motivation: To address the misalignment of automated systems with expert information needs in professional search tasks, which require high expertise and transparency.

Method: Evaluated LLMs (Gemini-Flash 2.0, o3-mini, o4-mini, DeepSeek-R1) using a self-feedback mechanism for query expansion and answer refinement across multiple answer types.

Result: Preliminary results show varied performance of the self-feedback strategy across models and tasks.

Conclusion: The study provides insights into LLM self-correction and informs future comparisons of LLM-generated feedback versus human expert input in professional search systems.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of parallel corpora for sign languages, especially for Latin American sign languages, and to expand resources for German Sign Language.

Method: Collection and processing of videos from online sources (news shows, governmental bodies, educational channels), involving data collection, approvals, scraping, and cropping.

Result: A comprehensive collection of 4,381 video files with 1.3M subtitles, significantly expanding available resources.

Conclusion: The corpus fills gaps in sign language resources and provides a foundation for further research and applications.
...
TL;DR: MyCulture benchmark evaluates LLMs on Malaysian culture using an open-ended multiple-choice format to reduce bias and improve fairness.


<details>
  <summary>Details</summary>
Motivation: Address cultural biases in LLMs caused by training data dominated by high-resource languages, ensuring better representation of low-resource cultures like Malaysia.

Method: Introduces MyCulture, a benchmark with six cultural pillars, using open-ended multiple-choice questions without predefined options to mitigate bias.
...
TL;DR: LLMEval-3 is a dynamic evaluation framework for LLMs, addressing data contamination and overfitting by using a proprietary question bank, automated integrity checks, and calibrated scoring. It reveals performance ceilings and contamination issues, offering a robust alternative to static benchmarks.


<details>
  <summary>Details</summary>
Motivation: Static benchmarks for LLMs suffer from data contamination and leaderboard overfitting, masking true model capabilities. A dynamic evaluation approach is needed for accurate assessment.

Method: LLMEval-3 dynamically samples unseen test sets from 220k graduate-level questions, employs contamination-resistant curation, anti-cheating architecture, and a calibrated LLM-as-a-judge process (90% human agreement).

Result: A 20-month study of 50 models shows performance ceilings on memorization and exposes contamination vulnerabilities. The framework ranks models stably and consistently.

Conclusion: LLMEval-3 provides a credible, robust methodology for evaluating LLMs beyond static benchmarks, advancing trustworthy evaluation standards.
...
TL;DR: TASE is a benchmark evaluating LLMs' token-level reasoning across languages, revealing their limitations compared to humans.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with fine-grained, token-level understanding, which is crucial for precision tasks.

Method: TASE includes 10 tasks (e.g., character counting, syntactic parsing) across Chinese, English, and Korean, with a 35,927-instance dataset. Over 30 LLMs were evaluated, and a custom Qwen2.5-14B model was trained using GRPO.
...
<details>
  <summary>Details</summary>
Motivation: To systematically assess and compare existing creativity metrics across diverse creative domains, identifying their strengths and weaknesses.

Method: Examination and analysis of four creativity measures (creativity index, perplexity, syntactic templates, LLM-as-a-Judge) in creative writing, problem-solving, and research ideation.

Result: Metrics show limited consistency and capture different aspects of creativity. Key limitations include lexical focus, model sensitivity, lack of conceptual capture, and bias.

Conclusion: Current metrics are inconsistent and flawed, necessitating more robust, generalizable frameworks aligned with human judgments.
...
TL;DR: LAG (Logic-Augmented Generation) improves LLMs by decomposing complex questions into logical sub-questions, ensuring stepwise reasoning and reducing hallucinations.


<details>
  <summary>Details</summary>
Motivation: Address LLMs' limitations in knowledge-intensive tasks and RAG's struggles with complex reasoning by introducing structured logical organization.

Method: Decompose questions into atomic sub-questions, resolve them sequentially with dependency-aware reasoning, and use a logical termination mechanism.

Result: LAG enhances reasoning robustness, reduces hallucination, and aligns LLM problem-solving with human cognition.

Conclusion: LAG offers a principled alternative to RAG, improving LLM performance in complex reasoning tasks.
...
Result: LLMs perform better for entities from the Global North/West than the Global South/East, with Wikipedia/pageview correlations insufficient to explain disparities. Language had minimal impact.

Conclusion: Creative evaluation frameworks like 20 Questions reveal hidden biases in LLMs, emphasizing the need for diverse and proactive testing methods.
...
<details>
  <summary>Details</summary>
Motivation: LLMs in legal domains often produce unfaithful outputs; current methods lack explicit faithfulness enforcement.

Method: CoCoLex combines model vocabulary distribution with context-derived copying, guided by confidence.

Result: Outperforms existing methods on five legal benchmarks, especially in long-form tasks.

Conclusion: CoCoLex ensures greater fidelity to source context, advancing legal text generation.
...
TL;DR: A frequency-based uncertainty quantification method for LLMs in MCQA, using conformal prediction to ensure reliability, outperforms logit-based methods and controls miscoverage rates.
...
Method: Frequency-based uncertainty quantification via multiple independent samplings and predictive entropy, leveraging conformal prediction for coverage guarantees.

Result: Outperforms logit-based PE in distinguishing correct/incorrect predictions (AUROC) and controls empirical miscoverage rates.
...
TL;DR: The study investigates cross-lingual political opinion differences in multilingual large language models (MLLMs) across five Western languages, finding minimal differences in unaligned models and uniform opinion shifts after alignment.


<details>
  <summary>Details</summary>
Motivation: To determine if political opinions in MLLMs vary by language or transfer uniformly, addressing challenges in socio-linguistic and political alignment.

Method: Analyzed MLLMs by prompting them with political statements from voting advice applications, evaluating before and after alignment using direct preference optimization.
...
Conclusion: Political opinions transfer between languages in MLLMs, highlighting challenges in achieving explicit socio-linguistic and political alignment.
...
TL;DR: MathSmith synthesizes challenging math problems from scratch using concept-explanation pairs and reinforcement learning, outperforming baselines in benchmarks.


<details>
  <summary>Details</summary>
Motivation: Addressing the scarcity of high-quality, high-difficulty training data for improving LLM mathematical reasoning.

Method: Constructs problems from PlanetMath pairs, uses predefined strategies for difficulty, and optimizes with reinforcement learning.
...
Conclusion: MathSmith demonstrates scalability and effectiveness in advancing LLM reasoning with synthetic high-difficulty data.
...
Method: Proposes Cooper, which co-optimizes policy and reward models, uses rule-based rewards for precision, and dynamically trains the reward model with positive-negative pairs. Introduces hybrid annotation and reference-based reward modeling (VerifyRM).

Result: Cooper improves robustness, mitigates reward hacking, and enhances RL performance, e.g., 0.54% accuracy gain on Qwen2.5-1.5B-Instruct. VerifyRM achieves higher accuracy on VerifyBench.
...
TL;DR: OmniEAR evaluates language models' embodied reasoning, revealing performance drops in dynamic tool usage and multi-agent coordination, highlighting current model limitations.


<details>
  <summary>Details</summary>
Motivation: To explore how language models handle embodied reasoning tasks like physical interactions, tool usage, and multi-agent coordination, which remain underexplored.
...
Result: Performance degrades significantly for tool reasoning (56-85%) and implicit collaboration (63-85%), with fine-tuning improving single-agent tasks but not multi-agent ones.
...
TL;DR: The paper addresses hallucination issues in Reasoning Large Language Models (R-LLMs) by proposing a novel reward function for online Reinforcement Learning (RL), improving factuality, detail, and relevance in responses.


<details>
  <summary>Details</summary>
Motivation: R-LLMs struggle with factuality, especially in long-form tasks, and existing RL methods face challenges like reward hacking when using automatic factuality evaluation.

Method: A new reward function is introduced, combining factual precision, detail level, and relevance, applied in online RL to enhance factual reasoning.

Result: The model reduces hallucination by 23.1 percentage points, increases detail by 23%, and maintains response helpfulness across six benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Understand how LLMs persuade humans using lightweight probes, inspired by cognitive science.

Method: Apply linear probes to study persuasion success, persuadee personality, and strategy in conversations.

Result: Probes effectively capture persuasion dynamics, identifying key moments and outperforming prompting in strategy analysis.

Conclusion: Probes offer a scalable, efficient method for studying complex LLM behaviors like persuasion, deception, and manipulation.
...
<details>
  <summary>Details</summary>
Motivation: Byte-level models avoid fragile tokenizers but struggle with computational inefficiency in morphologically-rich languages (MRLs).

Method: H-NET++ introduces a lightweight Transformer, two-level latent hyper-prior, specialized orthographic handling, and curriculum-based training.

Result: Achieves state-of-the-art results: better compression, ParsGLUE gains, robustness, and morphological boundary accuracy.
...
Method: NAEx jointly parameterizes graph structures and feature spaces using learnable masks and optimizes for faithful, comparable explanations.
...
Conclusion: NAEx addresses interpretability challenges in network alignment, providing a plug-and-play solution for explainability.
...
TL;DR: LumiGen is an LVLM-enhanced framework for T2I generation, improving fine-grained control and semantic consistency through iterative feedback, outperforming baselines on LongBench-T2I.


<details>
  <summary>Details</summary>
Motivation: Existing T2I models struggle with complex instructions, fine-grained control, and semantic consistency, while LVLMs show promise in cross-modal understanding.

Method: LumiGen uses an IPPA module for prompt enhancement and an IVFR module for iterative visual feedback and refinement.

Result: Achieves a superior score of 3.08 on LongBench-T2I, excelling in text rendering and pose expression.

Conclusion: LVLM integration enhances T2I generation, making it more controllable and higher-quality.
...
<details>
  <summary>Details</summary>
Motivation: Incomplete data is a common issue in real-world datasets, but existing tools are fragmented and limited to numerical variables, ignoring the heterogeneity of tabular data.
...
Result: The toolkit supports mechanism-aware studies and offers a comprehensive solution for researchers and practitioners.

Conclusion: MissMecha addresses gaps in missing data simulation tools, enhancing data quality research, benchmarking, and education.
...
TL;DR: The paper proposes a cluster-aware hierarchical federated aggregation framework to address computational and privacy challenges in edge-AIGC, using LoRA for efficient fine-tuning and federated learning for scalable personalized content generation.


<details>
  <summary>Details</summary>
Motivation: Diffusion models (DMs) face computational and privacy challenges in edge-AIGC scenarios, with cloud solutions lacking in privacy, personalization, and communication efficiency.

Method: A novel framework clusters clients by task similarity, uses LoRA for local fine-tuning, and employs intra-cluster aggregation and inter-cluster knowledge interaction for hybrid content generation. Federated learning trains personalized models on devices and a shared global model on the server.
...
Conclusion: The proposed solution effectively balances efficiency, personalization, and privacy in edge-AIGC, demonstrating feasibility for real-world deployment.
...
TL;DR: The paper introduces a Large Multi-Modal Model (LMMM) for Few-shot Learning (FSL), outperforming meta-learning methods. It includes a dataset (M3FD) and framework (M3F) for diverse scientific data.


<details>
  <summary>Details</summary>
Motivation: FSL is vital in data-scarce fields like biomedicine, where collecting large datasets is challenging. The study aims to improve FSL generalization using multi-modal models.

Method: Developed M3FD (a diverse few-shot dataset) and M3F (a modular LMMM framework). The model was fine-tuned on M3FD to enhance performance.

Result: M3F outperformed conventional meta-learning methods, demonstrating improved generalization for FSL tasks.

Conclusion: The M3F framework and M3FD dataset provide a scalable, unified solution for FSL in data-scarce scientific domains, lowering barriers to LMMM adoption.
...
<details>
  <summary>Details</summary>
Motivation: Current LLMs for molecular property prediction rely on human prompts and verbose reasoning, lacking relevance. AttriLens-Mol aims to enhance reasoning and attribute relevance.

Method: Uses attribute-guided reinforcement learning with format, count, and rationality rewards to steer LLM reasoning and elicit relevant molecular attributes.

Result: Boosts performance on 7B-size models, surpassing supervised fine-tuning and advanced models like GPT-4o. Extracted attributes improve interpretability and decision tree performance.

Conclusion: AttriLens-Mol effectively enhances molecular property prediction by eliciting relevant attributes, improving both performance and interpretability.
...
TL;DR: PA-RNet is a robust multimodal forecasting framework designed to handle noisy textual data in time series, using perturbation-aware projection and cross-modal attention to improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods ignore textual noise in multimodal time series, which degrades performance. PA-RNet addresses this gap.

Method: PA-RNet uses a perturbation-aware projection module and cross-modal attention to filter noise while preserving semantics.

Result: PA-RNet outperforms baselines, with theoretical guarantees of stability and reduced prediction error under noise.

Conclusion: PA-RNet is effective for noisy multimodal time series forecasting, offering robustness and generalization.
...
TL;DR: InfoQ is a training-free framework for mixed-precision quantization (MPQ) that measures layer sensitivity by mutual information changes, optimizing bit-width allocation efficiently.


<details>
  <summary>Details</summary>
Motivation: Current MPQ methods rely on costly search algorithms or local proxies, missing global quantization effects. InfoQ addresses this by focusing on layer impact on network-wide information flow.

Method: InfoQ quantizes layers at varying bit-widths, measures mutual information changes via a single forward pass, and formulates bit-width allocation as an integer linear programming problem.

Result: InfoQ achieves superior search-time/accuracy trade-off, using less data and improving accuracy by up to 1% for MobileNetV2 and ResNet18 on ImageNet at high compression.

Conclusion: InfoQ offers an efficient, retraining-free solution for MPQ, outperforming state-of-the-art methods in both speed and accuracy.
...
TL;DR: LLMs show promise for clinical decision-making in diabetes treatment, matching or outperforming trained RL agents in stable cases, but exhibit limitations like aggressive dosing and reasoning errors. Hybrid approaches are recommended.


<details>
  <summary>Details</summary>
Motivation: To evaluate the potential of LLMs in automating clinical decision-making for dynamic insulin dosing, comparing their zero-shot performance to trained RL agents.

Method: Tested open-source LLMs (e.g., Qwen2.5-7B) in a Type 1 diabetes simulator, comparing zero-shot inference with trained small neural network-based RL agents (SRAs).

Result: LLMs achieved comparable or better performance than SRAs in stable patient groups but showed issues like aggressive dosing and reasoning errors.

Conclusion: LLMs can be cautiously integrated into clinical workflows with targeted prompt engineering and hybrid approaches combining linguistic reasoning and physiological modeling.
...
TL;DR: F3CRec bridges Federated Learning and Continual Learning for recommendations, addressing privacy and non-stationary data challenges.


<details>
  <summary>Details</summary>
Motivation: Existing FedRec methods struggle with evolving user preferences, while CLRec lacks privacy preservation. FCRec combines both to solve these issues.
...
Conclusion: F3CRec effectively integrates FedRec and CLRec, providing a robust solution for privacy-preserving, adaptive recommendations.
...
TL;DR: HCRide, a human-centered ride-hailing system, uses multi-agent reinforcement learning (Habic) to balance system efficiency, passenger fairness, and driver preference, outperforming baselines in real-world tests.


<details>
  <summary>Details</summary>
Motivation: Existing ride-hailing systems prioritize operator revenue, often neglecting passenger fairness and driver preference, leading to poor user experiences.

Method: HCRide employs the Habic algorithm, featuring multi-agent competition, a dynamic Actor network, and a Bi-Critic network, to optimize efficiency, fairness, and preference.
...
Conclusion: HCRide successfully balances efficiency, fairness, and driver preference, demonstrating its effectiveness in real-world ride-hailing scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing autoregressive models for temporal point processes are inefficient and error-prone for long-range forecasting, necessitating a better approach.
...
Result: Outperforms autoregressive and diffusion-based baselines in accuracy and generation efficiency on six real-world benchmarks.

Conclusion: The proposed framework effectively addresses the limitations of autoregressive models for long-horizon event sequence modeling.
...
TL;DR: A multi-stage intrusion detection framework for CAN protocol combines unsupervised anomaly detection (VGAE) and supervised graph learning (KD-GAT) to improve security, achieving 16.2% higher F1-score on imbalanced datasets.
...
Method: Uses VGAE for structural anomaly detection and KD-GAT for attack classification, encoding CAN traffic as graph sequences to model dependencies.

Result: Achieves 96% parameter reduction in the student GAT model and 16.2% average F1-score improvement over existing methods.

Conclusion: The framework is effective for CAN intrusion detection, especially on imbalanced datasets, with high accuracy and efficiency.
...
TL;DR: The paper provides the first quantitative error bounds for OPTQ (GPTQ) and Qronos, analyzing their iterative procedures and deriving non-asymptotic error bounds. It justifies practical design choices and offers guidance for parameter selection.


<details>
  <summary>Details</summary>
Motivation: To address the lack of rigorous theoretical guarantees for OPTQ, a leading PTQ method, and extend analysis to Qronos, explaining their empirical advantages.

Method: Derives 2-norm and infinity-norm error bounds for deterministic and stochastic variants of OPTQ and Qronos, analyzing calibration data and regularization parameters.

Result: Establishes theoretical bounds for OPTQ and Qronos, justifying design choices like feature ordering and parameter selection.

Conclusion: The analysis enhances understanding of OPTQ and Qronos, providing theoretical support for their practical use and guiding future PTQ algorithm design.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with low-resource languages due to lack of pre-training data and per-language post-training requirements.

Method: Uses an LLM to rewrite unit tests into I/O format, configures a verifier for compilation/execution, and applies RL with verifiable rewards.

Result: Improves performance on five low-resource languages, scales to diverse models, and sets new state-of-the-art results.
...
TL;DR: The paper introduces the Hilbert Neural Operator (HNO), a novel neural operator architecture that leverages the Hilbert transform to enhance learning of PDE solution operators by explicitly using instantaneous amplitude and phase information.


<details>
  <summary>Details</summary>
Motivation: Current neural operators like FNO rely on Fourier transforms, which assume periodicity and limit their applicability. HNO aims to overcome this by incorporating signal processing insights for better performance in causal, phase-sensitive, and non-stationary systems.

Method: HNO maps input signals to their analytic representation via the Hilbert transform, making amplitude and phase explicit. It then applies spectral convolution to this transformed representation.

Result: HNO is hypothesized to model operators more effectively for systems where phase and causality are critical, leveraging analytic signal theory.

Conclusion: HNO offers a promising alternative to FNO by addressing its limitations and incorporating signal processing principles, potentially improving performance in specific PDE applications.
...
Conclusion: The GM layer offers a novel approach with unique dynamics, validated by experiments, suggesting potential for further exploration in neural network architectures.
...
TL;DR: A deep learning-based U-Net architecture is used to predict surface ozone residuals, incorporating uncertainty quantification to improve interpretability for policy and health decisions.


<details>
  <summary>Details</summary>
Motivation: Traditional physics-based models fail to capture human-relevant scales of surface ozone trends, and existing deep learning models lack interpretability for decision-making.

Method: An uncertainty-aware U-Net architecture with Bayesian and quantile regression methods is implemented to predict MOMO-Chem model's surface ozone residuals.

Result: The method successfully estimates regional bias in North America and Europe, identifies optimal ground stations for bias correction, and evaluates land-use impact.

Conclusion: The approach enhances interpretability and practical utility of deep learning models for air pollution policy and public health measures.
...
TL;DR: A 2D CNN-based method improves surface ozone modeling by estimating model bias, outperforming traditional ML and leveraging satellite data for better accuracy and policy insights.
...
Method: A 2D Convolutional Neural Network estimates ozone model bias from MOMO-Chem residuals, tested in North America and Europe, with satellite land use data.

Result: The CNN outperforms traditional ML in capturing model residuals and benefits from high-resolution satellite data for improved estimates.

Conclusion: The method enhances understanding of ozone bias drivers at urban scales, aiding better environmental policy decisions.
...
<details>
  <summary>Details</summary>
Motivation: Accurate water level forecasting is essential for ecosystem management, but existing deep learning models struggle with generalization and adaptation in hydrology.

Method: Proposes RAF, a framework that retrieves analogous historical hydrological data to enrich model inputs, comparing similarity-based and mutual information-based methods.

Result: RAF significantly improves forecasting accuracy in the Everglades, demonstrating its potential for environmental hydrology.

Conclusion: RAF offers a promising adaptive AI approach for ecosystem management, with broader applicability in hydrology.
...
TL;DR: The paper evaluates metrics for neonatal seizure detection models, highlighting issues with current practices and proposing best practices for reliable evaluation.


<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics for neonatal seizure detection models are inconsistent and biased, leading to unreliable claims about AI performance. The study aims to address this by systematically assessing metrics and proposing tailored best practices.

Method: The study used real and synthetic seizure annotations to evaluate standard performance metrics, consensus strategies, and human-expert equivalence tests under varying conditions like class imbalance and inter-rater agreement.

Result: Matthews and Pearson's correlation coefficients performed better under class imbalance. Consensus strategies were sensitive to rater count and agreement. The multi-rater Turing test using Fleiss k best captured expert-level AI performance.

Conclusion: The study recommends reporting balanced metrics, sensitivity, specificity, PPV, NPV, and multi-rater Turing test results on held-out validation sets to ensure reliable evaluation of neonatal seizure detection models.
...
TL;DR: The paper analyzes replicability in transfer learning, introducing a measure called selection sensitivity ($\Delta_Q$) to quantify the trade-off between adaptation effectiveness and consistency. It shows that replicability failure increases with sensitivity and decreases with sample size, validated through experiments on the MultiNLI corpus.


<details>
  <summary>Details</summary>
Motivation: To understand the reliability of adaptive data selection strategies in transfer learning, as their replicability remains poorly understood despite widespread use.

Method: The authors formalize selection sensitivity ($\Delta_Q$) and analyze its impact on replicability failure probability. They conduct experiments on the MultiNLI corpus using six adaptive selection strategies.

Result: Highly adaptive strategies (e.g., gradient-based) achieve better performance but suffer from high replicability failure rates (above 7%), while less adaptive methods maintain lower failure rates. Source domain pretraining reduces failure rates by up to 30%.

Conclusion: The study provides guidelines for balancing performance and replicability in transfer learning, emphasizing the need for replicability-aware design.
...
Method: The study evaluates transformer models (BERT, RoBERTa, GPT-2, ELECTRA) on the MetaHate dataset (1.2 million samples). Fine-tuning is applied to optimize performance.

Result: Fine-tuned ELECTRA achieved the highest F1 score (0.8980). Challenges include sarcasm, coded language, and label noise.

Conclusion: Transformer models, especially ELECTRA, are effective for hate speech detection, but addressing nuanced challenges remains critical.
...
<details>
  <summary>Details</summary>
Motivation: The lack of a unified platform for evaluating DAL algorithms under diverse and realistic conditions (e.g., distribution shifts, data imbalance) motivated the development of ALScope.
...
Result: Experiments show DAL performance varies by domain and task, with room for improvement in non-standard scenarios (e.g., imbalanced, open-set). Some algorithms perform well but are time-consuming.

Conclusion: ALScope provides a comprehensive evaluation platform, revealing DAL algorithm limitations and opportunities for future research, especially in challenging scenarios.
...
TL;DR: REINA is a novel loss function for Simultaneous Speech Translation (SimulST) that optimizes the tradeoff between translation quality and latency by waiting for more input only when it provides useful information. It achieves SOTA results and improves efficiency by up to 21%.


<details>
  <summary>Details</summary>
Motivation: Balancing translation quality and latency in SimulST systems is challenging. The goal is to optimize this tradeoff by waiting for more input only when it adds value.
...
Result: Achieves SOTA results on open-source/synthetic data, improving the latency/quality tradeoff by up to 21% compared to prior methods.

Conclusion: REINA effectively optimizes the latency/quality tradeoff in SimulST, demonstrating significant improvements over existing approaches.
...
TL;DR: SEA, a novel ensemble learning framework, improves performance by decomposing errors into individual and diversity terms, offering precise control over the accuracy-diversity trade-off.


<details>
  <summary>Details</summary>
Motivation: Traditional methods like Bagging and Boosting lack precise control over the accuracy-diversity trade-off, and NCL has loose theoretical bounds. SEA addresses these limitations.

Method: SEA decomposes ensemble errors into individual performance and diversity terms, introducing an adjustable parameter in the loss function for finer control.

Result: SEA outperforms baseline methods on regression and classification tasks, offering broader adjustment range and tighter theoretical bounds.

Conclusion: SEA provides flexible, superior performance in ensemble learning by enabling precise control over error components.
...
TL;DR: The paper introduces two compressed decentralized algorithms for nonconvex stochastic optimization, using momentum and message-compression techniques. One handles bounded gradients with adaptive methods, while the other addresses data heterogeneity with heavy-ball methods, both achieving optimal convergence rates and superior empirical performance.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of combining momentum acceleration and compressed communication in decentralized algorithms while controlling consensus, compression, and momentum gradient errors.

Method: Two algorithms: a compressed decentralized adaptive method for bounded gradients and a compressed decentralized heavy-ball method for data heterogeneity, both using momentum and compression techniques.

Result: Both methods achieve optimal convergence rates, linear speed-up, and topology-independent parameters, outperforming state-of-the-art methods in training DNNs and Transformers.

Conclusion: The proposed algorithms effectively combine momentum and compression, demonstrating theoretical and empirical superiority in decentralized nonconvex stochastic optimization.
...
TL;DR: MENDR is a transparent, interpretable EEG foundation model using wavelet transforms and Riemannian Manifold Transformer architecture, achieving near state-of-the-art performance with fewer parameters.
...
Method: MENDR uses a filter bank-based approach with a Riemannian Manifold Transformer, pretraining on 4,000+ hours of EEG data decomposed via wavelet transforms.

Result: MENDR achieves near state-of-the-art performance in clinical EEG tasks with fewer parameters and offers interpretable embeddings.

Conclusion: MENDR provides an efficient, interpretable, and clinically viable solution for EEG analysis, bridging the gap in transparency and performance.
...
TL;DR: A novel framework, RCUKF, combines reservoir computing (RC) and unscented Kalman filtering (UKF) for accurate modeling of complex systems, validated on benchmarks and real-time vehicle trajectory estimation.
...
Method: Integrates RC for data-driven modeling with UKF for Bayesian estimation, using RC as a surrogate model in UKF's prediction step and UKF for real-time sensor data correction.

Result: Demonstrated effectiveness on benchmark problems and real-time vehicle trajectory estimation.

Conclusion: RCUKF provides a robust solution for modeling complex systems, combining data-driven learning with real-time correction.
...
<details>
  <summary>Details</summary>
Motivation: Existing MSA methods suffer from spurious correlations, leading to unreliable predictions. The goal is to enhance generalization by leveraging causal theory.

Method: MMCI models multimodal inputs as a multi-relational graph, uses attention to disentangle causal and shortcut features, and applies backdoor adjustment for stable predictions.

Result: Experiments show MMCI suppresses biases and improves performance on standard and OOD datasets.

Conclusion: MMCI effectively addresses spurious correlations in MSA, enhancing model robustness and generalization.
...
TL;DR: R-Zero is an autonomous framework for self-evolving LLMs that generates its own training data, improving reasoning capabilities without human-curated tasks.


<details>
  <summary>Details</summary>
Motivation: Overcome the bottleneck of relying on human-curated tasks and labels for training LLMs to advance AI beyond human intelligence.

Method: Uses two co-evolving models (Challenger and Solver) to generate and solve tasks autonomously, creating a self-improving curriculum.

Result: Boosts reasoning capabilities, e.g., +6.49 on math-reasoning and +7.54 on general-domain benchmarks for Qwen3-4B-Base.

Conclusion: R-Zero enables scalable, autonomous advancement of LLMs toward super-intelligence.
...
<details>
  <summary>Details</summary>
Motivation: Current RL-based fine-tuning for LLMs is resource-intensive, and heuristic-driven approaches lack scalability.

Method: SPaRFT uses cluster-based data reduction and a multi-armed bandit to adaptively select training samples.

Result: Achieves comparable or better accuracy with up to 100x fewer samples.

Conclusion: Performance-driven curricula can unlock LLM reasoning abilities efficiently.
...
TL;DR: The paper explores how AR systems can compromise situational awareness (SA) in safety-critical tasks like CPR, using eye tracking and a novel graph neural network (FixGraphPool) to predict SA with high accuracy.


<details>
  <summary>Details</summary>
Motivation: AR systems risk inducing cognitive tunneling, reducing situational awareness during critical tasks like CPR. The study aims to understand and mitigate this issue.
...
Result: Higher SA correlated with specific eye movement patterns (e.g., larger saccades). FixGraphPool achieved 83.0% accuracy in predicting SA, outperforming other models.

Conclusion: Eye tracking and FixGraphPool effectively model SA in AR, offering insights for designing safer AR systems that maintain user awareness.
...
Method: Uses sequential fine-tuning on downsized datasets to induce structured forgetting, then reverses it to recover knowledge. Employs meta-learning for weight prediction.

Result: KNOW prediction consistently outperforms naive fine-tuning and simple weight prediction.
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in graph learning like oversquashing and ill-conditioned energy regions by combining energy-based dynamics with flexible tangential evolution.

Method: Uses a Lyapunov function for energy-reducing dynamics and a tangential component learned via message passing to evolve features while maintaining energy.

Result: Achieves strong performance in node and graph classification/regression benchmarks, mitigating oversquashing.
...
TL;DR: ULU and AULU are novel activation functions outperforming ReLU and Mish, with AULU adapting to inputs and introducing the LIB metric for inductive bias measurement.


<details>
  <summary>Details</summary>
Motivation: To address limitations of existing activation functions like ReLU and Mish by proposing a non-monotonic, piecewise function (ULU) and its adaptive variant (AULU).

Method: ULU is defined piecewise for positive and negative inputs, while AULU uses learnable parameters for adaptation. The LIB metric quantifies inductive bias.

Result: ULU and AULU significantly outperform ReLU and Mish in image classification and object detection tasks.

Conclusion: ULU and AULU offer superior performance and adaptability, with the LIB metric providing a new tool for analyzing model inductive bias.
...
TL;DR: Multimodal imitation learning benefits from integrated perception (RGB-D, proprioception, language), yielding better generalization and optimization than unimodal methods.


<details>
  <summary>Details</summary>
Motivation: To understand how multimodal perception impacts imitation learning's sample complexity and optimization, leveraging statistical learning theory.
...
Result: Properly integrated multimodal policies achieve tighter generalization bounds and more favorable optimization landscapes.

Conclusion: Multimodal architectures outperform unimodal ones due to theoretical advantages in learning frameworks.
...
TL;DR: The paper introduces Integrated Influence, a data attribution method that addresses limitations of LOO-based methods by incorporating a baseline approach and collective influence analysis.


<details>
  <summary>Details</summary>
Motivation: Prevailing LOO-based data attribution methods are limited by local explanations and lack of baseline flexibility, hindering transparency and counterfactual explanations.

Method: Proposes Integrated Influence, which uses a baseline dataset, a degeneration process, and accumulates sample influence during transition to baseline.

Result: Integrated Influence outperforms existing methods in data attribution and mislabeled example identification tasks.

Conclusion: The method provides a more reliable and flexible framework for data attribution, with theoretical grounding and practical superiority.
...
TL;DR: A novel framework for cold-start active preference learning uses self-supervised pre-training (PCA) and active learning to outperform standard methods, requiring fewer labeled pairs.


<details>
  <summary>Details</summary>
Motivation: Addresses the cold-start problem in preference learning, where labeled data is scarce, expensive, or noisy, common in computational social systems and economic analysis.

Method: Combines self-supervised pre-training (PCA for pseudo-labels) with an active learning loop querying a noisy oracle.

Result: Outperforms standard active learning, achieving higher accuracy with fewer labeled pairs across diverse datasets.

Conclusion: Provides a practical solution to the cold-start problem, improving preference learning efficiency in data-constrained settings.
...
TL;DR: A novel Weakly Supervised Learning (WSL) framework leverages multiple weak supervision signals (similarity-confidence and confidence-difference) for improved performance when labeled data is limited. It introduces unbiased risk estimators and a risk correction approach, outperforming existing methods.
...
Method: Proposes SconfConfDiff Classification, combining similarity-confidence and confidence-difference weak labels, with two unbiased risk estimators and a risk correction technique.

Result: The method achieves optimal convergence rates and outperforms baselines in experiments, showing robustness against inaccurate class priors and label noise.

Conclusion: The framework effectively enhances WSL by leveraging complementary weak supervision, providing theoretical guarantees and practical improvements.
...
TL;DR: A novel RL framework enhances function calling in LLMs via strategic entropy-based exploration, outperforming standard methods by 6% in complex scenarios.


<details>
  <summary>Details</summary>
Motivation: Current training approaches for function calling in LLMs lack robust reasoning, relying on superficial pattern matching or struggling with complex action spaces.

Method: A two-stage data prep pipeline with iterative LLM evaluation and AST validation, combined with strategic entropy-based exploration in RL.

Result: Achieves 86.02% accuracy on the Berkeley Function Calling Leaderboard, outperforming standard GRPO by 6% in multi-function scenarios.

Conclusion: The framework improves structured reasoning and parameter verification, especially benefiting code-pretrained models, with plans to release code and data.
...
TL;DR: The paper introduces HFedDG, a hierarchical federated learning approach addressing domain shift, and proposes HFedATM, a method combining filter alignment and aggregation to improve performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: To tackle scalability and domain shift issues in hierarchical federated learning (HFL) by integrating domain generalization techniques.
...
Conclusion: HFedATM effectively addresses domain shift in HFL, improving performance and convergence while remaining efficient.
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap in error-estimation theory for neural networks in PDEs and demonstrate their superior approximation capabilities.

Method: Analysis of deep fully-connected networks in Sobolev spaces, measuring errors in the $W^{m,p}$-norm for $m < n$.

Result: Deep networks surpass classical methods in accuracy, achieving super-convergence.

Conclusion: Provides a unified theoretical foundation for neural networks in scientific computing, closing a gap in PDE approximation theory.
...
TL;DR: PSEO optimizes post-hoc stacking ensembles in AutoML by balancing diversity and performance, outperforming 16 methods.


<details>
  <summary>Details</summary>
Motivation: Current CASH methods use fixed ensemble strategies, lacking adaptability to task specifics.

Method: PSEO uses binary quadratic programming for base model selection and introduces multi-layer stacking mechanisms.

Result: Achieves best average test rank (2.96) on 80 datasets.

Conclusion: PSEO effectively adapts ensemble strategies, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the evaluation of RL algorithms in ABMs and RABMs by developing domain-specific metrics, as current methods struggle with system complexity and stochasticity.

Method: The method involves creating domain-driven RL metrics, building on existing state-of-the-art metrics, and applying them to a rational ABM case study on pandemic behaviors like masking and vaccination.

Result: Results demonstrate the effectiveness of domain-driven rewards combined with traditional metrics in various simulation scenarios, such as differing mask availability.

Conclusion: The study concludes that domain-driven metrics enhance the assessment of RL-based ABMs and RABMs, providing more meaningful evaluations in complex, stochastic systems.
...
<details>
  <summary>Details</summary>
Motivation: Address the gap in existing pFL methods by handling dynamic client onboarding while preserving privacy and performance.

Method: Proposes pFedDSH, using a central hypernetwork with batch-specific masks and data-free replay for knowledge transfer.
...
Conclusion: pFedDSH effectively balances performance stability for existing clients and adaptation for new clients, optimizing neural resource use.
...
Method: Proposes S$^2$M-Former with a spike-driven symmetric architecture and lightweight 1D token sequences to replace 3D operations.
...
TL;DR: HIA (Heuristic-Guided Inference-time Alignment) is a tuning-free method that balances alignment quality and computational cost for LLMs, outperforming existing baselines under the same inference budget.


<details>
  <summary>Details</summary>
Motivation: Aligning LLMs with user preferences is costly in terms of fine-tuning or inference, creating a trade-off between alignment quality and computational expense. Existing methods ignore this balance.
...
Conclusion: HIA offers a practical, scalable solution for personalized LLM deployment by balancing alignment quality and computational cost without tuning.
...
TL;DR: A novel framework for subset selection in multinomial distributions to identify the best-performing algorithm from benchmark data, with improved asymptotic and finite-sample schemes.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of selecting the best-performing machine learning algorithm from benchmark data, especially when performance differences are marginal.

Method: Formulates the problem as subset selection for multinomial distributions, introducing new asymptotic and finite-sample schemes.
...
Conclusion: The framework provides a robust solution for subset selection, ensuring high confidence in identifying the top-performing algorithm.
...
TL;DR: A ResNet-based HAR model detects gait vs. non-gait and everyday activities with high accuracy and robustness across smartphone wear locations, outperforming state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: To develop a lightweight HAR model for accurate detection of gait and everyday activities, especially for people with multiple sclerosis (PwMS), using smartphone sensor data.

Method: The model is based on ResNet architecture, trained and evaluated on datasets from healthy controls (HC) and PwMS, including GaitLab study, Roche internal data, and public sources.

Result: Achieved 98.4%-99.6% accuracy for gait detection and 96.2% for everyday activities, outperforming state-of-the-art models. Robust across 9 wear locations.

Conclusion: The HAR model is highly accurate and practical for real-world applications, especially for monitoring PwMS.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods (FR and AR) fail in long-term accuracy and generalization for time-dependent PDEs.

Method: Introduces PITI-DeepONet, a dual-output architecture trained with physics-informed or hybrid objectives, integrating time-derivative operators.

Result: Reduced mean relative L2 errors by 84% (vs. FR) and 79% (vs. AR) for heat equation, and similar improvements for Burgers and Allen-Cahn equations.

Conclusion: PITI-DeepONet offers more reliable long-term integration for complex PDEs, outperforming traditional approaches.
...
TL;DR: A framework for evaluating intrinsic hallucinations in financial LLMs is proposed, addressing the gap in existing benchmarks for financial tabular data.


<details>
  <summary>Details</summary>
Motivation: Hallucination in LLMs undermines financial analysis due to minor numerical errors, requiring context-dependent and precise evaluation.

Method: Develops a context-aware masked span prediction task using a masking strategy and S&P 500 annual reports for dataset creation.

Result: Introduces a novel dataset and evaluates hallucination patterns in state-of-the-art LLMs on financial data.

Conclusion: Provides a robust methodology for trustworthy financial Generative AI systems.
...
Method: BAR uses Bidding-Aware Modeling (monotonicity-constrained learning, multi-task distillation) and Asynchronous Near-Line Inference for real-time updates. A Task-Attentive Refinement module enhances feature interactions.

Result: BAR increased platform revenue by 4.32% and impressions by 22.2% in Alibaba's ad platform.

Conclusion: BAR effectively resolves multi-stage inconsistency, boosting both platform revenue and advertiser performance.
...
<details>
  <summary>Details</summary>
Motivation: Accurate ROP prediction is challenging due to complex drilling data, and existing models fail to capture temporal and contextual relationships effectively.

Method: Proposes a hybrid deep learning architecture integrating LSTM, Transformer encoders, TS-Mixer, and attention mechanisms to model temporal dependencies and feature interactions.

Result: Achieves an R-squared score of 0.9988 and MAPE of 1.447%, outperforming benchmarks. Interpretability is ensured using SHAP and LIME.
...
TL;DR: The paper proposes Deconfounding Factor Weighting (DFW), a new method to improve causal effect estimation by addressing instability in propensity score-based weighting.


<details>
  <summary>Details</summary>
Motivation: Existing propensity score methods like IPW suffer from unstable weights due to high variance in propensity scores, degrading their effectiveness in handling selection bias and treatment effect estimation.

Method: DFW leverages a deconfounding factor to construct stable, bounded weights, prioritizing less confounded samples and mitigating highly confounded ones to better approximate a randomized controlled trial.

Result: DFW outperforms existing methods (IPW, CBPS) in covariate balancing and treatment effect estimation, as shown in experiments on benchmark and synthetic datasets.

Conclusion: DFW provides a robust solution for causal effect estimation, with bounded weights and improved performance, and can be extended to multi-treatment settings.
...
TL;DR: The paper explores predicting older adults' SPPB scores four years ahead using ML models, with XGBoost performing best (MAE: 0.79). Feature selection improved results slightly (MAE: 0.82).
...
Method: Tested ML models (Random Forest, XGBoost, Linear Regression, dense and TabNet neural networks) on questionnaire data. Used Shapley values for feature selection.

Result: XGBoost achieved the lowest MAE (0.79). Feature subsets (10-20) yielded slightly higher MAE (0.82).
...
<details>
  <summary>Details</summary>
Motivation: Addresses limitations of centralized FL (single point of failure, poor personalization, vulnerability to adversarial clients) and unreliable update selection in heterogeneous data.

Method: Uses P2P topology with agreement scores (computed on local validation sets) to select trustworthy updates, combined with regularization for stable training.

Result: Outperforms centralized and existing P2P FL methods, especially in adversarial and heterogeneous conditions.

Conclusion: Decentralized P2P FL with agreement scores and regularization enhances robustness and personalization in FL.
...
TL;DR: Cross-LoRA is a data-free framework for transferring LoRA modules between diverse LLMs without additional training, using LoRA-Align and LoRA-Shift.


<details>
  <summary>Details</summary>
Motivation: Traditional PEFT methods like LoRA are limited by their tight coupling with base model architectures, restricting cross-model applicability.

Method: Cross-LoRA uses LoRA-Align (subspace alignment via SVD and linear transformation) and LoRA-Shift (projecting aligned subspaces) for data-free, training-free adaptation.

Result: Achieves up to 5.26% relative gains over base models on benchmarks like ARCs, OBOA, and HellaSwag, with comparable performance to directly trained LoRA adapters.

Conclusion: Cross-LoRA enables efficient, lightweight adaptation of LoRA modules across heterogeneous LLMs without data or training, maintaining performance.
...
TL;DR: The paper introduces MoBE, a novel method for compressing MoE-based LLMs with minimal accuracy loss, outperforming existing techniques.


<details>
  <summary>Details</summary>
Motivation: Addressing the high memory demands of large MoE-based LLMs like DeepSeek-V3-0324 and Kimi-K2-Instruct, which current compression methods degrade accuracy significantly.

Method: MoBE decomposes expert matrices into unique and shared basis components, learning the factorization to minimize reconstruction error.

Result: MoBE reduces parameters by 24%-30% with only 1%-2% accuracy drop, significantly better than prior methods.

Conclusion: MoBE offers an effective solution for compressing large MoE models with minimal performance impact.
...
TL;DR: A hybrid LSTM-RF model is proposed for predicting marine chlorophyll concentration, outperforming standalone LSTM and RF models in accuracy.
...
Method: Combines LSTM and RF to address time-series and nonlinear feature challenges, trained with multi-source ocean data.

Result: LSTM-RF achieved R²=0.5386, MSE=0.005806, MAE=0.057147, outperforming standalone models.

Conclusion: The hybrid model offers an innovative solution for high-frequency marine ecological predictions.
...
TL;DR: FlowState is a novel time series foundation model (TSFM) using a state space model encoder and functional basis decoder, addressing generalization and efficiency issues in existing TSFMs.


<details>
  <summary>Details</summary>
Motivation: Existing TSFMs struggle with generalization across varying temporal resolutions, lack adaptability to sampling rates, and are computationally inefficient.

Method: FlowState employs a state space model (SSM) based encoder and a functional basis decoder for continuous-time modeling and dynamic time-scale adjustment.

Result: FlowState outperforms state-of-the-art models on benchmarks (GIFT-ZS and Chronos-ZS) and adapts online to varying input sampling rates.

Conclusion: FlowState's design enables smaller models, reduced data requirements, and improved efficiency, making it a robust solution for time series forecasting.
...
TL;DR: The paper proposes using RLHF to align LLM-based CRS with user preferences by optimizing implicit feedback signals like engagement, outperforming traditional methods in accuracy and satisfaction.


<details>
  <summary>Details</summary>
Motivation: Traditional supervised fine-tuning fails to capture implicit user feedback (e.g., dwell time, sentiment), limiting the effectiveness of conversational recommender systems.

Method: Uses RLHF with a reward model trained on weakly-labeled engagement data and optimizes the LLM via PPO to maximize user-centric utility in multi-turn conversations.
...
Conclusion: Implicit signal alignment via RLHF enables scalable and user-adaptive CRS design, improving performance over traditional methods.
...
Result: Theoretical and experimental results show reduced SFO complexity, offering scalable and efficient large-batch training guidelines.

Conclusion: The paper provides both theoretical insights and practical guidelines for improving training efficiency in deep learning.
...
### [240] [Adaptive Batch Size and Learning Rate Scheduler for Stochastic Gradient Descent Based on Minimization of Stochastic First-order Oracle Complexity](https://arxiv.org/abs/2508.05302)
*Hikaru Umeda,Hideaki Iiduka*

Main category: cs.LG

TL;DR: The paper introduces an adaptive scheduling strategy for mini-batch SGD, adjusting batch size and learning rate based on gradient norm decay, improving convergence speed.


<details>
  <summary>Details</summary>
Motivation: Mini-batch SGD's convergence is sensitive to batch size and learning rate. Theoretical findings on critical batch size motivate an adaptive approach to optimize SFO complexity.

Method: An adaptive joint scheduler adjusts batch size and learning rate dynamically using observed full gradient norm decay during training.

Result: Experiments show the adaptive scheduler improves convergence speed compared to existing methods.

Conclusion: The adaptive scheduling strategy effectively leverages theoretical insights to enhance SGD performance.
...
### [241] [ASkDAgger: Active Skill-level Data Aggregation for Interactive Imitation Learning](https://arxiv.org/abs/2508.05310)
...
Method: ASkDAgger uses S-Aware Gating (SAG), Foresight Interactive Experience Replay (FIER), and Prioritized Interactive Experience Replay (PIER) to optimize queries and feedback.

Result: ASkDAgger balances query frequency with failure incidence, reduces annotations, improves generalization, and speeds adaptation.

Conclusion: ASkDAgger is validated in language-conditioned manipulation tasks, showing effectiveness in simulation and real-world environments.
...
### [242] [Divide-and-Conquer for Enhancing Unlabeled Learning, Stability, and Plasticity in Semi-supervised Continual Learning](https://arxiv.org/abs/2508.05316)
...
Result: USP achieves up to 5.94% higher accuracy than previous SSCL methods.
...
### [243] [Fairy$\pm i$: the First 2-bit Complex LLM with All Parameters in $\{\pm1, \pm i\}$](https://arxiv.org/abs/2508.05571)
...
<details>
  <summary>Details</summary>
Motivation: Current QAT research is limited by the accuracy ceiling of full-precision models. This work aims to break this ceiling by improving full-precision accuracy and then quantizing efficiently.

Method: The framework maps weights to the fourth roots of unity (±1, ±i), creating a symmetric 2-bit representation. It enables multiplication-free inference using additions and swaps.
...
### [244] [Optimal Corpus Aware Training for Neural Machine Translation](https://arxiv.org/abs/2508.05364)
*Yi-Hsiu Liao,Cheng Shen,Brenda,Yang*

Main category: cs.LG

TL;DR: OCAT improves CAT by fine-tuning only corpus-related parameters, boosting accuracy and resilience to overfitting, with +3.6 and +1.8 chrF improvements in translation tasks.


<details>
  <summary>Details</summary>
Motivation: CAT's reliance on pre-defined high-quality data is error-prone and inefficient, prompting the need for a more robust method.

Method: OCAT fine-tunes a CAT pre-trained model by freezing most parameters and tuning only corpus-related ones.

Result: OCAT achieves +3.6 and +1.8 chrF improvements in WMT23 English to Chinese and German tasks, outperforming vanilla training.

Conclusion: OCAT is lightweight, effective, and less sensitive to hyperparameters, matching or surpassing other fine-tuning techniques.
...
### [245] [Iterative Learning of Computable Phenotypes for Treatment Resistant Hypertension using Large Language Models](https://arxiv.org/abs/2508.05581)
...
<details>
  <summary>Details</summary>
Motivation: To explore LLMs' potential for generating accurate and concise CPs for scalable clinical decision support in hypertension care.
...
Result: LLMs with iterative learning produced interpretable and reasonably accurate CPs, comparable to advanced ML methods but with fewer examples.

Conclusion: LLMs show promise for generating CPs in clinical settings, offering scalability and interpretability.
...
TL;DR: The paper proposes relaxing assumptions in latent bandits by focusing on action preference orderings rather than exact reward distributions, improving performance in scenarios with varying reward scales.


<details>
  <summary>Details</summary>
Motivation: Learning from scratch in personalization tasks is costly, and existing latent bandit models require exact reward distributions, which may not account for variations like differing symptom scales among patients.

Method: A posterior-sampling algorithm is introduced, leveraging only the preference orderings of actions in latent states, allowing flexibility in reward distributions.

Result: The algorithm performs competitively with traditional latent bandits when reward distributions are well-specified and outperforms them when reward scales vary.

Conclusion: Relaxing assumptions to focus on preference orderings enhances the practicality and robustness of latent bandits in real-world applications.
...
TL;DR: Echo decouples RL-based post-training for LLMs into inference and training phases, using lightweight sync protocols to maintain efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Current RL systems violate SPMD assumptions by switching between inference and training on the same GPU cluster, leading to inefficiencies.

Method: Echo introduces sequential pull and asynchronous push-pull sync protocols to decouple phases across heterogeneous swarms.

Result: Echo matches co-located baselines in convergence and reward while using decentralized, edge hardware for trajectory generation.
...
TL;DR: NT-ML is a defense mechanism against backdoor attacks in DNNs, using non-target label training and mutual learning to purify poisoned models.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks are vulnerable to backdoor attacks, necessitating robust defense mechanisms.

Method: NT-ML involves retraining with non-target labels and mutual learning between teacher and student models.

Result: NT-ML defends against 6 backdoor attacks with few clean samples, outperforming 5 state-of-the-art defenses.

Conclusion: NT-ML is an effective solution for mitigating backdoor attacks in DNNs.
...
TL;DR: The paper examines adaptive learning rate mechanisms in deep learning, focusing on a 2017 cumulative path-based method. It identifies inconsistencies in its application to Adam and proposes a corrected variant, benchmarking it against SGD and Adam to evaluate practical benefits.


<details>
  <summary>Details</summary>
Motivation: The learning rate is critical in deep learning, and its ideal value varies by problem and training phase. The study aims to evaluate the utility of adaptive learning rate mechanisms, particularly for Adam.

Method: The paper revisits a 2017 cumulative path-based adaptation scheme, identifies flaws in its application to Adam, and proposes a corrected variant. It benchmarks SGD and Adam with and without adaptation against a recent alternative.

Result: The study reveals inconsistencies in the original adaptation method for Adam and demonstrates the effectiveness of the corrected variant. Benchmarking clarifies when adaptive strategies are beneficial.

Conclusion: Adaptive learning rate mechanisms can be practical, but their effectiveness depends on the optimizer and context. The corrected variant for Adam improves consistency and performance.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of ensuring high-quality, diverse molecular generation from text while maintaining fast inference.

Method: Proposes a Causality-Aware Transformer (CAT) for joint encoding and a Variational Mean Flow (VMF) framework for efficient latent space modeling.

Result: Achieves higher novelty (74.5%), diversity (70.3%), and 100% validity, with efficient one-step inference.

Conclusion: The framework offers superior performance and computational efficiency over existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Forecasting stock and cryptocurrency prices is difficult due to high volatility and non-stationarity, influenced by economic changes and market sentiment.

Method: Uses Echo State Networks (ESNs) for modeling, including chaos analysis via the Lyapunov exponent during volatile periods.

Result: ESNs outperform existing machine learning methods, especially in chaotic conditions, aligning with Lyapunov exponent findings.

Conclusion: ESNs are robust and effective for cryptocurrency forecasting during extreme volatility, surpassing traditional methods.
...
TL;DR: NegBio-VAE extends VAEs by using a negative binomial distribution for spike counts, improving accuracy by modeling overdispersion.


<details>
  <summary>Details</summary>
Motivation: Current VAEs, like Poisson-VAE, inadequately model neural spike variability due to rigid Poisson constraints.

Method: Introduces NegBio-VAE with negative binomial distribution, two ELBO optimization schemes, and reparameterization strategies.

Result: Significant gains in reconstruction fidelity by modeling overdispersion.

Conclusion: Explicitly modeling overdispersion in spike-like activations is crucial for accurate neural representations.
...
TL;DR: CR-FMOL introduces a federated multi-objective learning framework with a preference-cone constraint to ensure client-wise Pareto optimality, improving fairness in FL.


<details>
  <summary>Details</summary>
Motivation: Address the under-serving of minority clients in federated learning (FL) by enforcing client-wise Pareto optimality, which existing methods like FMOL fail to achieve.

Method: Uses a novel preference-cone constraint after local federated multi-gradient descent averaging (FMGDA/FSMGDA) steps. The server solves a cone-constrained Pareto-MTL sub-problem to ensure Pareto-stationarity for all clients.

Result: CR-FMOL improves client fairness on non-IID benchmarks, with early-stage performance slightly below FedAvg but expected to match accuracy over time.

Conclusion: CR-FMOL is the first FL framework to enforce client-wise Pareto optimality, offering a promising solution for fairness in federated learning.
...
TL;DR: GCPO improves upon GRPO by incorporating causal structure into policy optimization, enhancing prediction quality and outperforming existing methods.
...
Method: Introduces a Structural Causal Model (SCM) to reveal dependencies, proposes causally informed reward adjustment and KL regularization in GCPO.

Result: GCPO outperforms GRPO and other methods across multiple reasoning benchmarks.

Conclusion: Integrating causal structure into optimization (GCPO) is more effective than independent response treatment (GRPO).
...
<details>
  <summary>Details</summary>
Motivation: Deep reinforcement learning lacks interpretability, hindering trust and deployment in safety-critical tasks. MLES addresses this by integrating interpretability with performance.

Method: MLES uses multimodal large language models as policy generators, evolutionary mechanisms for optimization, and visual feedback for behavior analysis and improvement.

Result: MLES matches PPO's performance in control tasks, providing transparent logic and traceable design, and is scalable across tasks.
...
<details>
  <summary>Details</summary>
Motivation: To address the overlooked consequences of misclassifying competing risks as censoring in survival analysis, which leads to biased estimates and amplifies disparities.

Method: Develops a theoretical framework to quantify the error from treating competing risks as censoring and examines its impact on predictive performance and fairness, supported by empirical analysis of cardiovascular data.
...
<details>
  <summary>Details</summary>
Motivation: Traditional MCTS lacks tail-risk control, risking serious consequences in high-stake scenarios. Constrained variants fail to guarantee safety for extreme outcomes.

Method: 1. CVaR-MCTS embeds Conditional Value-at-Risk for explicit tail-risk control. 2. W-MCTS uses a Wasserstein ambiguity set to address estimation bias in tail-risk.

Result: Both methods provide PAC tail-safety guarantees and improved performance in simulations, offering robust tail-risk control and better rewards.

Conclusion: The proposed CVaR-MCTS and W-MCTS effectively address tail-risk in MCTS, ensuring safety and stability in high-stake decision-making.
...
TL;DR: EnergyPatchTST improves energy time series prediction with multi-scale feature extraction, uncertainty estimation, and integration of future variables, reducing errors by 7-12%.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods struggle with multi-scale dynamics and irregular data in energy forecasting.

Method: Proposes EnergyPatchTST, featuring multi-scale feature extraction, Monte Carlo-based uncertainty estimation, future variable integration, and pre-training/fine-tuning.

Result: Outperforms common methods, reducing prediction errors by 7-12% and providing reliable uncertainty estimates.
...
TL;DR: The paper explores how task difficulty affects neural network representations using data-agnostic probes, revealing insights into model robustness and topology.
...
Method: Introduces five probes (pruning, binarization, noise injection, sign flipping, bipartite network randomization) applied to MLPs on MNIST and Fashion-MNIST datasets.

Result: Binarization collapses accuracy in hard tasks, pruning reveals phase transitions, noise can boost accuracy, and sign structure preservation maintains performance.

Conclusion: Task complexity can be measured by performance gaps, and signed bipartite topology is key for representations, aiding model compression and interpretability.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of evaluating AI systems when ground truth is unavailable, ensuring metrics resist gaming and maintain output quality.

Method: Leverages f-mutual information measures, proving their uniqueness for gaming resistance, and tests them empirically across domains like translation and peer review.

Result: Achieves perfect discrimination between faithful and strategic agents, shows 10-100x better robustness to manipulation, and identifies an optimal compression ratio (10:1) for performance.

Conclusion: The proposed mechanisms are highly effective, robust, and provide a bias-variance tradeoff perspective for optimal performance in AI evaluation.
...
TL;DR: The paper proposes a multi-task recurrent neural network to model clinical presence in EHR data, improving prediction model performance and transportability.


<details>
  <summary>Details</summary>
Motivation: Clinical presence (interaction between patients and healthcare systems) impacts observed outcomes in EHR data, but is often overlooked, limiting model performance and transportability.

Method: A multi-task recurrent neural network jointly models inter-observation time, missingness processes, and survival outcomes to account for clinical presence.

Result: The proposed model outperforms state-of-the-art models in mortality prediction on the MIMIC-III dataset, demonstrating improved performance and transportability.
...
TL;DR: MoMA, a novel architecture using multiple LLM agents, integrates multimodal EHR data for clinical predictions, outperforming current methods.


<details>
  <summary>Details</summary>
Motivation: Multimodal EHR data offers richer insights but integration is challenging due to data requirements.

Method: MoMA uses specialist agents to convert non-textual data into summaries, an aggregator agent to unify them, and a predictor agent for clinical predictions.

Result: MoMA outperforms state-of-the-art methods in accuracy and flexibility across tasks.

Conclusion: MoMA effectively integrates multimodal EHR data, enhancing clinical prediction accuracy.
...
TL;DR: The paper introduces two parameter-free algorithms, AMVFCM-U and AAMVFCM-U, for multi-view clustering, addressing challenges like high-dimensional features and cross-view integration. The methods use entropy regularization and SNR-based feature weighting for adaptive consensus and dimensionality reduction, outperforming 15 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Multi-view clustering struggles with heterogeneous data, high-dimensional features, and lack of principled cross-view integration. Traditional methods require manual tuning and lack unified frameworks.
...
Result: The methods outperform 15 benchmarks, achieving 97% computational efficiency, reducing dimensionality to 0.45%, and identifying optimal view combinations.

Conclusion: The unified, parameter-free framework effectively addresses multi-view clustering challenges, offering adaptive, efficient, and scalable solutions.
...
TL;DR: The paper addresses overfitting in Probabilistic Circuits (PCs) by proposing a Hessian-based regularizer inspired by sharpness aware minimization in neural networks. It efficiently computes the Hessian trace for PCs, improving generalization.
...
Method: A Hessian-based regularizer is introduced, leveraging the tractable computation of the Hessian trace in PCs. This induces a gradient-norm-based regularizer with simple closed-form updates for EM and gradient-based methods.

Result: Experiments on synthetic and real-world datasets show the method guides PCs toward flatter minima and enhances generalization.

Conclusion: The proposed regularizer effectively mitigates overfitting in PCs by targeting sharp optima, improving model performance.
...
TL;DR: A survey on unsupervised adaptation methods for Vision-Language Models (VLMs), categorizing approaches into four paradigms and reviewing benchmarks and future directions.
...
Method: Proposes a taxonomy of four paradigms based on unlabeled data availability: Data-Free Transfer, Unsupervised Domain Transfer, Episodic Test-Time Adaptation, and Online Test-Time Adaptation. Analyzes methodologies and strategies for each.

Result: Provides a structured overview of unsupervised VLM adaptation, including benchmarks and open challenges.

Conclusion: The survey bridges the gap in unified research on unsupervised VLM adaptation, offering insights and future directions.
...
<details>
  <summary>Details</summary>
Motivation: Current KGE libraries lack advanced negative sampling strategies, limiting performance and customization.

Method: Developed a PyKEEN extension with modular architecture for static and dynamic negative samplers.

Result: Empirical study shows improved performance in link prediction tasks and insights for better strategies.

Conclusion: The extension enhances PyKEEN, supports customization, and improves embedding method development.
...
<details>
  <summary>Details</summary>
Motivation: Addressing IoT security concerns due to increasing cyberattacks on IoT networks.
...
Conclusion: KANs are a promising alternative for IoT intrusion detection, balancing accuracy and interpretability.
...
TL;DR: The paper investigates the minimal poison data required for successful backdoor attacks, proposing the 'one-poison hypothesis' where a single poison sample can inject a backdoor without affecting benign task performance. Theoretical and experimental validation is provided.


<details>
  <summary>Details</summary>
Motivation: To address the open question of the minimal poison data needed for backdoor attacks, challenging prior assumptions that require many poisoned samples or extensive data knowledge.
...
Result: Demonstrates that a single poison sample can achieve zero backdooring-error and minimal impact on benign task performance, especially when leveraging unused data directions.

Conclusion: The one-poison hypothesis is validated, showing that backdoor attacks can be highly efficient with minimal data, posing significant security implications for machine learning models.
...
TL;DR: Shuffle-R1 improves RL fine-tuning efficiency by addressing Advantage Collapsing and Rollout Silencing via dynamic trajectory sampling and batch restructuring.
...
TL;DR: TrajEvo uses LLMs and evolutionary algorithms to automate the design of trajectory prediction heuristics, outperforming traditional and deep learning methods, especially in OOD scenarios.
...
Result: TrajEvo outperforms existing heuristic and deep learning methods, particularly in generalizing to OOD datasets.

Conclusion: TrajEvo offers a fast, explainable, and generalizable solution for trajectory prediction, with potential for broader applications.
...
TL;DR: Dynamic Fine-Tuning (DFT) improves SFT for LLMs by dynamically rescaling gradients, enhancing generalization and outperforming standard SFT and RL methods.


<details>
  <summary>Details</summary>
Motivation: Standard SFT has limited generalization due to problematic reward structures in gradients, prompting the need for a theoretically grounded improvement.

Method: DFT stabilizes gradient updates by dynamically rescaling the objective function with token probabilities, requiring minimal code changes.

Result: DFT significantly outperforms standard SFT in benchmarks and shows competitive results in offline RL, improving generalization.
...
### [273] [Supervised Machine Learning Methods with Uncertainty Quantification for Exoplanet Atmospheric Retrievals from Transmission Spectroscopy](https://arxiv.org/abs/2508.04982)
*Roy T. Forestano,Konstantin T. Matchev,Katia Matcheva,Eyup B. Unlu*

Main category: astro-ph.EP

TL;DR: The paper evaluates machine learning (ML) regression techniques for retrieving exoplanet atmospheric parameters from transmission spectra, comparing their accuracy, precision, and speed. It also examines preprocessing impacts and validates the best model on JWST observations of WASP-39b.


<details>
  <summary>Details</summary>
Motivation: Traditional Bayesian retrievals for exoplanet atmospheric parameters are computationally expensive. With JWST and upcoming observatories, efficient and robust ML alternatives are needed.

Method: The study systematically tests ML regression methods (PLS, SVM, KNN, DT, RF, VOTE, STACK, XGB) and preprocessing techniques, benchmarking their performance.

Result: The paper identifies the best-performing ML model and preprocessing scheme, quantifying uncertainties across planetary parameters.

Conclusion: ML methods offer efficient and robust alternatives to Bayesian retrievals, validated on JWST data for WASP-39b.
...
### [274] [Sequence Aware SAC Control for Engine Fuel Consumption Optimization in Electrified Powertrain](https://arxiv.org/abs/2508.04874)
*Wafeeq Jaleel,Md Ragib Rownak,Athar Hanif,Sidra Ghayour Bhatti,Qadeer Ahmed*

Main category: eess.SY

TL;DR: A reinforcement learning framework using SAC with GRUs and Decision Transformers improves energy management in hybrid electric trucks, achieving near-DP fuel savings and better generalization.


<details>
  <summary>Details</summary>
Motivation: Hybrid electric vehicles need efficient energy management to reduce fuel consumption and maintain battery charge for long operations.

Method: Reformulated control as a sequential decision-making problem, enhanced SAC with GRUs and Decision Transformers in actor-critic networks for temporal dependencies.

Result: SAC with DT-based actor and GRU-based critic achieved 1.8% fuel savings near DP; GRU-based agents outperformed FFN agents on unseen cycles.

Conclusion: The framework shows adaptability and robustness, making it suitable for real-world HEV applications.
...
### [275] [Tesserae: Scalable Placement Policies for Deep Learning Workloads](https://arxiv.org/abs/2508.04953)
...
<details>
  <summary>Details</summary>
Motivation: Improving resource utilization in data-centers for deep learning workloads by addressing suboptimal performance and scalability issues in existing placement policies.

Method: Formulates placement constraints as graph matching problems, designing novel policies for minimizing migration overheads and job packing, integrated into Tesserae.

Result: Tesserae improves average JCT by up to 1.62x and makespan by up to 1.15x compared to existing schedulers.

Conclusion: Graph matching-based placement policies in Tesserae offer a scalable and effective solution for GPU cluster scheduling in deep learning workloads.
...
### [276] [CodonMoE: DNA Language Models for mRNA Analyses](https://arxiv.org/abs/2508.04739)
*Shiyi Du,Litian Liang,Jiayi Li,Carl Kingsford*

Main category: q-bio.GN

TL;DR: CodonMoE is a lightweight adapter that transforms DNA language models into RNA analyzers without RNA-specific pretraining, outperforming specialized RNA models with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Addressing the inefficiency of maintaining separate models for DNA and RNA or developing large multi-modal architectures, which impose computational burdens.

Method: Introduces CodonMoE, a universal approximator at the codon level, mapping codon sequences to RNA properties without RNA-specific pretraining.

Result: CodonMoE-augmented DNA models outperform unmodified ones in RNA prediction tasks, achieving state-of-the-art results with 80% fewer parameters.

Conclusion: CodonMoE offers a principled way to unify genomic language modeling, leveraging DNA data efficiently while maintaining performance.
...
### [277] [Discovery of Disease Relationships via Transcriptomic Signature Analysis Powered by Agentic AI](https://arxiv.org/abs/2508.04742)
*Ke Chen,Haohan Wang*

Main category: q-bio.GN

TL;DR: A transcriptomics-driven framework using GenoMAS AI identifies disease relationships by analyzing gene-level and pathway-based similarities, revealing new cross-category links and therapeutic repurposing opportunities.


<details>
  <summary>Details</summary>
Motivation: To uncover molecular commonalities hidden beneath divergent clinical presentations in disease classification.

Method: Analyzes over 1300 disease-condition pairs using GenoMAS, an AI system, and develops a pathway-based similarity framework integrating multi-database enrichment analysis.

Result: Reveals known comorbidities and new cross-category links, identifies therapeutic repurposing opportunities, and shows how background conditions modulate transcriptomic similarity.

Conclusion: Demonstrates the potential of biologically grounded AI to scale transcriptomic analysis and provide mechanistic insights beyond symptom-based taxonomies.
...
### [278] [GRIT: Graph-Regularized Logit Refinement for Zero-shot Cell Type Annotation](https://arxiv.org/abs/2508.04747)
*Tianxiang Hu,Chenyi Zhou,Jiaxiang Liu,Jiongxin Wang,Ruizhe Chen,Haoxiang Xia,Gaoang Wang,Jian Wu,Zuozhu Liu*

Main category: q-bio.GN

TL;DR: The paper proposes a graph-regularized optimization framework to refine zero-shot cell type annotations from LangCell, improving accuracy by up to 10% on scRNA-seq datasets.


<details>
  <summary>Details</summary>
Motivation: Manual cell type annotation in scRNA-seq data is labor-intensive and doesn't scale. While CLIP-style models like LangCell automate this, their zero-shot performance is suboptimal.

Method: A graph-regularized optimization framework enforces local consistency over a PCA-based k-NN graph to refine LangCell's zero-shot predictions.

Result: The method improves zero-shot annotation accuracy by up to 10% across 14 datasets, spanning 11 organs and 200,000 cells.

Conclusion: The proposed framework is training-free, model-agnostic, and effectively enhances automated cell type annotation.
...
### [279] [Embedding Is (Almost) All You Need: Retrieval-Augmented Inference for Generalizable Genomic Prediction Tasks](https://arxiv.org/abs/2508.04757)
...
<details>
  <summary>Details</summary>
Motivation: To determine if task-specific fine-tuning is always necessary for pre-trained DNA language models, given its cost and distribution dependency.

Method: Extract fixed embeddings from models (e.g., DNABERT-2, HyenaDNA) and feed them into lightweight classifiers, comparing performance to fine-tuning.

Result: Embedding methods achieve competitive accuracy (e.g., 0.68 vs. 0.58 in enhancer classification) with 10x-20x faster inference and lower carbon emissions (e.g., 0.02 kg vs. 0.17 kg CO2).

Conclusion: Embedding extraction is a generalizable, efficient alternative to fine-tuning, especially for diverse genomic contexts, offering better carbon efficiency.
...
### [280] [Understanding and Mitigating Errors of LLM-Generated RTL Code](https://arxiv.org/abs/2508.05266)
*Jiazheng Zhang,Cheng Liu,Huawei Li*

Main category: cs.AR

TL;DR: The paper addresses low success rates in LLM-based RTL code generation by analyzing errors, proposing targeted corrections, and integrating them into a framework, achieving 91.0% accuracy.
...
Method: The study involves error categorization, in-context learning, retrieval-augmented generation (RAG), rule-checking mechanisms, and iterative debugging to correct errors.

Result: The enhanced framework achieves 91.0% accuracy on VerilogEval, a 32.7% improvement over the baseline.

Conclusion: Targeted error correction techniques significantly improve LLM-based RTL code generation, demonstrating the framework's effectiveness.
...
### [281] [Speech LLMs in Low-Resource Scenarios: Data Volume Requirements and the Impact of Pretraining on High-Resource Languages](https://arxiv.org/abs/2508.05149)
*Seraphina Fong,Marco Matassoni,Alessio Brutti*

Main category: eess.AS

TL;DR: The paper explores using Speech LLMs for low-resource ASR via the SLAM-ASR framework, showing that pretrained projectors mitigate data scarcity challenges.
...
Result: Pretrained projectors reduce data scarcity impact, especially with small training sets, and multilingual LLMs improve performance.

Conclusion: The study highlights the potential of optimized Speech LLMs for low-resource languages and multilinguality.
...
### [282] [Fairness in Dysarthric Speech Synthesis: Understanding Intrinsic Bias in Dysarthric Speech Cloning using F5-TTS](https://arxiv.org/abs/2508.05102)
*Anuprabha M,Krishna Gurugubelli,Anil Kumar Vuppala*

Main category: eess.AS

TL;DR: The paper examines F5-TTS for dysarthric speech synthesis, revealing biases toward intelligibility over speaker similarity and prosody, and suggests fairness-aware approaches for inclusive tech.


<details>
  <summary>Details</summary>
Motivation: Addressing biases in synthetic dysarthric speech generation to improve assistive technologies, given limited data availability.

Method: Evaluated F5-TTS using the TORGO dataset, focusing on intelligibility, speaker similarity, and prosody, with fairness metrics like Disparate Impact and Parity Difference.
...
### [283] [Keyword Spotting with Hyper-Matched Filters for Small Footprint Devices](https://arxiv.org/abs/2508.04857)
...
<details>
  <summary>Details</summary>
Motivation: To improve keyword detection accuracy in speech recordings, even for words not in training data, while maintaining efficiency for small devices.

Method: Combines a speech encoder (tiny Whisper or Conformer), a keyword encoder (hyper-network generating matched-filter weights), and a detection network (Perceiver module with cross-attention).

Result: Achieves state-of-the-art performance, generalizes well to out-of-domain conditions (e.g., L2 speech), and is efficient (4.2M parameters).

Conclusion: The model is highly accurate, efficient, and robust, suitable for small-footprint devices.
...
### [284] [INTENTION: Inferring Tendencies of Humanoid Robot Motion Through Interactive Intuition and Grounded VLM](https://arxiv.org/abs/2508.04931)
*Jin Wang,Weijie Wang,Boyuan Deng,Heng Zhang,Rui Dai,Nikos Tsagarakis*

Main category: cs.RO

TL;DR: INTENTION is a framework for robotic manipulation using Vision-Language Models and interaction-driven memory to enable adaptability in diverse scenarios.


<details>
  <summary>Details</summary>
Motivation: Traditional robotic control relies on precise models and predefined actions, which fail in unstructured environments. Humans, however, adapt intuitively, inspiring this work.

Method: The framework integrates Vision-Language Models for scene reasoning and a Memory Graph to store past interactions. An Intuitive Perceptor extracts physical relations and affordances.

Result: Robots can infer appropriate behaviors in new scenes without repetitive instructions, demonstrating adaptability.

Conclusion: INTENTION enhances robotic manipulation by mimicking human-like intuition and adaptability, reducing reliance on predefined models.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of adapting to diverse human partners in long-horizon collaboration, where behavior and understanding may vary over time.

Method: MICoBot uses a three-level decision-making approach: meta-planner for collaboration strategy, planner for optimal task allocation, and action executor for low-level actions or communication.

Result: Evaluations show MICoBot significantly improves task success and user experience compared to baselines.

Conclusion: MICoBot effectively enables flexible and adaptive human-robot collaboration through natural language and optimized task allocation.
...
### [286] [Hierarchical Deep Deterministic Policy Gradient for Autonomous Maze Navigation of Mobile Robots](https://arxiv.org/abs/2508.04994)
*Wenjie Hu,Ye Zhou,Hann Woei Ho*

Main category: cs.RO

TL;DR: The paper proposes a Hierarchical DDPG (HDDPG) algorithm to improve maze navigation in robotics, addressing DDPG's limitations like sparse rewards and inefficient exploration. HDDPG uses high-level and low-level policies, off-policy correction, adaptive noise, and optimized rewards, achieving significant performance gains.


<details>
  <summary>Details</summary>
Motivation: Standard DDPG struggles with sparse rewards, inefficient exploration, and long-horizon planning in maze navigation, leading to low success rates and rewards.

Method: HDDPG introduces hierarchical policies (high-level for subgoals, low-level for actions), off-policy correction, adaptive parameter noise, and reshaped rewards. Additional optimizations like gradient clipping and Xavier initialization are used.

Result: HDDPG outperforms baseline DDPG variants, improving success rates by at least 56.59% and average rewards by 519.03 in maze navigation tasks.

Conclusion: HDDPG effectively addresses DDPG's limitations in maze navigation, demonstrating superior performance through hierarchical planning and enhanced exploration strategies.
...
TL;DR: Chemist Eye is a safety monitoring system for self-driving labs (SDLs) that uses vision-language models to detect hazards like fires, PPE non-compliance, and emergencies, achieving high accuracy in real-world tests.
...
Method: The system integrates RGB, depth, and infrared cameras for hazard detection, uses a vision-language model for decision-making, and communicates with robots and lab personnel in real-time.

Result: Chemist Eye achieved 97% accuracy in spotting hazards and 95% in decision-making during real-world testing.

Conclusion: Chemist Eye effectively enhances safety in SDLs by integrating advanced monitoring and real-time response capabilities.
...
Method: Proposes FCBV-Net, a 3D point cloud-based network that conditions bimanual action value prediction on pre-trained geometric features, with trainable downstream components for task-specific policy learning.
...
Conclusion: Decoupling geometric understanding from action value learning enhances category-level generalization in robotic garment manipulation.
...
### [290] [DistillDrive: End-to-End Multi-Mode Autonomous Driving Distillation by Isomorphic Hetero-Source Planning Model](https://arxiv.org/abs/2508.05402)
*Rui Yu,Xianghang Zhang,Runkai Zhao,Huaicheng Yan,Meng Wang*

Main category: cs.RO

TL;DR: DistillDrive enhances autonomous driving by using knowledge distillation and diversified instance imitation for multi-mode motion feature learning, reducing collisions by 50% and improving closed-loop performance.


<details>
  <summary>Details</summary>
Motivation: Existing autonomous driving models focus too much on ego-vehicle status and lack planning-oriented understanding, limiting decision-making robustness.
...
Conclusion: DistillDrive effectively improves autonomous driving robustness by integrating planning-oriented learning and knowledge distillation.
...
### [291] [Towards Generalizable Safety in Crowd Navigation via Conformal Uncertainty Handling](https://arxiv.org/abs/2508.05634)
*Jianpeng Yao,Xiaopan Zhang,Yu Xia,Zejin Wang,Amit K. Roy-Chowdhury,Jiachen Li*

Main category: cs.RO

TL;DR: A method using adaptive conformal inference and constrained reinforcement learning improves robot navigation in crowds by accounting for pedestrian uncertainties, achieving high success rates and robustness to distribution shifts.


<details>
  <summary>Details</summary>
Motivation: Address performance degradation of reinforcement learning-trained robots in out-of-distribution crowd navigation scenarios by incorporating pedestrian uncertainty.
...
Result: Achieves 96.93% success rate in-distribution, with fewer collisions and intrusions; robust in out-of-distribution scenarios like velocity variations and group dynamics.

Conclusion: The method enhances robot navigation safety and adaptability in diverse crowd conditions, validated in real-world deployments.
...
### [292] [Genie Envisioner: A Unified World Foundation Platform for Robotic Manipulation](https://arxiv.org/abs/2508.05635)
...
TL;DR: Genie Envisioner (GE) is a unified platform for robotic manipulation, combining policy learning, evaluation, and simulation using a video-generative framework. It includes GE-Base (video diffusion model), GE-Act (action decoder), GE-Sim (neural simulator), and EWMBench (benchmark suite).


<details>
  <summary>Details</summary>
Motivation: To create a scalable and practical foundation for instruction-driven, general-purpose robotic intelligence by integrating policy learning, evaluation, and simulation in a single framework.

Method: GE-Base uses a large-scale video diffusion model to capture real-world dynamics. GE-Act maps latent representations to actions via a flow-matching decoder. GE-Sim acts as a neural simulator for training and evaluation. EWMBench provides standardized benchmarking.

Result: The platform enables precise, generalizable policy inference and high-fidelity simulations for robotic manipulation.

Conclusion: Genie Envisioner is a scalable and practical solution for embodied intelligence, with public release of code, models, and benchmarks.
...
### [293] [Towards Embodied Agentic AI: Review and Classification of LLM- and VLM-Driven Robot Autonomy and Interaction](https://arxiv.org/abs/2508.05294)
*Sahar Salimpour,Lei Fu,Farhad Keramat,Leonardo Militano,Giovanni Toffetti,Harry Edelman,Jorge Peña Queralta*

Main category: cs.RO

TL;DR: The paper surveys advancements in foundation models (LLMs, VLMs, VLAs, BLMs) for robotics, focusing on agentic applications like planning, perception, and task execution, while also analyzing community-driven and industrial trends.


<details>
  <summary>Details</summary>
Motivation: To explore how foundation models enhance robot autonomy and human-robot interfaces, and to classify emerging agentic architectures in robotics.

Method: Survey of peer-reviewed research, community projects, ROS packages, and industrial frameworks, proposing a taxonomy for model integration approaches.

Result: Identifies trends in agentic applications, such as natural language reasoning, API invocation, and task planning, and provides a comparative analysis of agent roles.

Conclusion: Foundation models are advancing robotics through agentic architectures, with diverse applications and rapid community-driven innovation.
...
### [294] [Information-Theoretic Graph Fusion with Vision-Language-Action Model for Policy Reasoning and Dual Robotic Control](https://arxiv.org/abs/2508.05342)
*Shunlei Li,Longsen Gao,Jin Wang,Chang Che,Xi Xiao,Jiuwen Cao,Yingbai Hu,Hamid Reza Karimi*

Main category: cs.RO

TL;DR: GF-VLA is a framework for teaching robots dexterous skills from human videos using scene graphs and language-conditioned transformers, achieving high success rates in dual-arm tasks.


<details>
  <summary>Details</summary>
Motivation: Overcome limitations of low-level trajectory imitation in robots by enabling task-level reasoning from human demonstrations.

Method: Extracts task-relevant cues, encodes them into scene graphs, fuses with a transformer for behavior trees, and uses a cross-hand selection policy.

Result: Achieves high accuracy in graph representation (95%), subtask segmentation (93%), and task success (90%).

Conclusion: GF-VLA demonstrates strong generalization and robustness in dual-arm robotic tasks.
...
### [295] [Real-Time Iteration Scheme for Diffusion Policy](https://arxiv.org/abs/2508.05396)
*Yufei Duan,Hang Yin,Danica Kragic*

Main category: cs.RO

TL;DR: The paper introduces a Real-Time Iteration (RTI) Scheme-inspired method to reduce the inference time of Diffusion Policies in robotics, avoiding resource-heavy retraining.


<details>
  <summary>Details</summary>
Motivation: Diffusion Policies face limitations due to long inference times and action chunk execution, hindering their use in latency-critical tasks.

Method: The proposed approach applies the RTI Scheme to diffusion inference, using previous solutions as initial guesses and scaling for discrete actions.

Result: The method significantly cuts runtime costs without retraining, maintaining performance comparable to full-step denoising.

Conclusion: The RTI-inspired scheme enables efficient integration into pre-trained models, with theoretical support for contractivity.
...
### [296] [Cross-Domain Image Synthesis: Generating H&E from Multiplex Biomarker Imaging](https://arxiv.org/abs/2508.04734)
*Jillur Rahman Saurav,Mohammad Sadegh Nasr,Jacob M. Luber*

Main category: q-bio.QM

TL;DR: A multi-level VQGAN is proposed to generate virtual H&E stains from mIF images, outperforming cGAN in downstream tasks like nuclei segmentation and tissue classification.


<details>
  <summary>Details</summary>
Motivation: Integrating mIF molecular data with H&E morphological context is crucial for comprehensive tissue analysis, enabling the use of H&E-based CAD tools.
...
Result: VQGAN produces higher-fidelity virtual stains, improving performance in nuclei segmentation and tissue classification tasks compared to cGAN.

Conclusion: VQGAN is a superior architecture for generating scientifically useful virtual stains, facilitating integration of mIF data into H&E workflows.
...
### [297] [ERDES: A Benchmark Video Dataset for Retinal Detachment and Macular Status Classification in Ocular Ultrasound](https://arxiv.org/abs/2508.04735)
*Pouyan Navard,Yasemin Ozkut,Srikar Adhikari,Elaine Situ-LaCasse,Josie Acuña,Adrienne Yarnish,Alper Yilmaz*

Main category: q-bio.QM

TL;DR: The paper introduces ERDES, the first open-access dataset of ocular ultrasound clips for detecting retinal detachment (RD) and macular status, aiming to support ML model development.


<details>
  <summary>Details</summary>
Motivation: Retinal detachment (RD) is vision-threatening, and timely intervention is crucial. Current ultrasound-based RD detection lacks expertise and tools for macular status assessment, which is vital for treatment urgency.

Method: The authors introduce ERDES, a labeled dataset of ocular ultrasound clips, and provide baseline benchmarks using spatiotemporal CNN architectures.

Result: ERDES is the first public dataset for RD and macular status classification, with labeled clips and training code made available.

Conclusion: ERDES facilitates ML model development for RD detection and macular status assessment, addressing a critical gap in clinical practice.
...
### [298] [Understanding protein function with a multimodal retrieval-augmented foundation model](https://arxiv.org/abs/2508.04724)
*Timothy Fei Truong Jr,Tristan Bepler*

Main category: q-bio.QM

TL;DR: PoET-2 is a multimodal, retrieval-augmented protein foundation model that improves zero-shot variant prediction and supervised sequence-function learning by combining evolutionary constraints and structure conditioning.


<details>
  <summary>Details</summary>
Motivation: To address limitations in mutation understanding and representation quality for protein function prediction, despite scaling protein language models.

Method: Introduces PoET-2, a hierarchical transformer encoder with dual decoders for causal and masked language modeling, incorporating evolutionary constraints and optional structure conditioning.
...
### [299] [From Rattle to Roar: Optimizer Showdown for MambaStock on S&P 500](https://arxiv.org/abs/2508.04707)
*Alena Chan,Maria Garmonina*

Main category: q-fin.CP

TL;DR: Roaree optimizers outperform Lion by reducing oscillatory loss while maintaining fast training, with Adam and RMSProp yielding the lowest test errors for S&P 500 Index forecasting.


<details>
  <summary>Details</summary>
Motivation: To combine the advantages of fast training (Lion) and low test errors (Adam/RMSProp) for forecasting S&P 500 Index returns.

Method: Introduces Roaree, a novel optimizer family that dampens Lion's oscillatory loss behavior while preserving its speed.

Result: Roaree achieves better performance than Lion, with Adam and RMSProp providing the lowest test errors.

Conclusion: Roaree effectively balances speed and stability, offering a promising alternative for optimization tasks.
...
### [300] [Agency, Affordances, and Enculturation of Augmentation Technologies](https://arxiv.org/abs/2508.04725)
*Ann Hill Duin,Isabel Pedersen*

Main category: cs.CY

TL;DR: The paper critiques the hype around AI-driven augmentation technologies, analyzing their cultural and societal impacts, terminology ambiguity, and marketing influences.


<details>
  <summary>Details</summary>
Motivation: To critically examine the assumption that augmentation technologies, driven by AI, universally improve lives and societies, addressing ambiguity in AI terminology and exploring human-robot dynamics.

Method: Uses the WIPO Categorization of AI Technologies Scheme, media studies, and analysis of marketing strategies to explore the enculturation of augmentation technologies.

Result: Highlights the role of non-human agents in industry and how marketing shapes user adoption, while questioning the Metaverse and augmented reality claims.

Conclusion: The paper calls for a nuanced understanding of augmentation technologies, emphasizing their societal and cultural implications beyond hype.
...
### [301] [Building Effective Safety Guardrails in AI Education Tools](https://arxiv.org/abs/2508.05360)
*Hannah-Beth Clark,Laura Benton,Emma Searle,Margaux Dowland,Matthew Gregory,Will Gayne,John Roberts*

Main category: cs.CY

TL;DR: The paper discusses Oak National Academy's AI tool, Aila, designed for lesson planning in UK schools, focusing on safety and age-appropriateness through four guardrails.


<details>
  <summary>Details</summary>
Motivation: Address concerns about the safety and suitability of AI-generated content for classroom use.
...
Result: Identified challenges and opportunities for refining safety measures in AI education tools.

Conclusion: Emphasizes iterative refinement of guardrails and cross-sector collaboration to enhance safety in generative AI tools for education.
...
### [302] [Alz-QNet: A Quantum Regression Network for Studying Alzheimer's Gene Interactions](https://arxiv.org/abs/2508.04743)
*Debanjan Konar,Neerav Sreekumar,Richard Jiang,Vaneet Aggarwal*

Main category: q-bio.MN

TL;DR: The paper introduces Alz-QNet, a quantum regression network, to study gene-gene interactions in Alzheimer's disease (AD), focusing on key genes like APP, FGF14, and YY1, to uncover regulatory mechanisms for potential therapies.


<details>
  <summary>Details</summary>
Motivation: Understanding the molecular mechanisms of AD is challenging due to its multifactorial nature. The study aims to decode gene interactions for theranostics.

Method: The proposed Alz-QNet uses quantum regression and QGRN insights to analyze gene interactions in the Entorhinal Cortex, leveraging data from GSE138852.

Result: The study reveals intricate gene-gene interactions, identifying potential regulatory mechanisms and targets for AD therapy.

Conclusion: Alz-QNet provides a novel approach to understanding AD pathology, offering insights for gene expression-based therapies.
...
### [303] [Learning Geometric-Aware Quadrature Rules for Functional Minimization](https://arxiv.org/abs/2508.05445)
*Costas Smaragdakis*
...
Result: QuadrANN reduces variance in integral estimation, outperforming Quasi-Monte Carlo methods, especially in critical areas with singularities.

Conclusion: QuadrANN enhances stability and accuracy for deep learning-based variational solvers, improving energy functional optimization.
...
### [304] [Toward Low-Latency End-to-End Voice Agents for Telecommunications Using Streaming ASR, Quantized LLMs, and Real-Time TTS](https://arxiv.org/abs/2508.04721)
*Vignesh Ethiraj,Ashwath David,Sidhanth Menon,Divya Vijay*

Main category: cs.SD

TL;DR: A low-latency telecom AI voice agent pipeline integrates four specialized models (TSLAM, T-VEC, TTE, T-Synth) for real-time, domain-specific voice interactions in telecom applications like call centers and IVR.


<details>
  <summary>Details</summary>
Motivation: To enable advanced, real-time voice AI for telecom, addressing the need for low-latency, domain-adapted solutions in customer support and automation.

Method: Combines four telecom-specific models (TSLAM, T-VEC, TTE, T-Synth) for streaming ASR, conversational intelligence, RAG, and real-time TTS. Evaluated using a dataset of 500 telecom questions.

Result: Achieves real-time factors (RTF) below 1.0, demonstrating low-latency performance suitable for enterprise telecom deployments.
...
### [305] [Wearable Music2Emotion : Assessing Emotions Induced by AI-Generated Music through Portable EEG-fNIRS Fusion](https://arxiv.org/abs/2508.04723)
...
<details>
  <summary>Details</summary>
Motivation: Overcome constraints in music-based affective computing, including stimulus biases, unimodal data reliance, and portability issues.

Method: Integrates AI-generated music with wireless EEG-fNIRS headband for synchronized data collection.

Result: Validated with a 14-hour dataset from 20 participants, showing efficacy in eliciting target emotions.

Conclusion: MEEtBrain offers a scalable, portable solution for emotion analysis, with datasets publicly available for further research.
...
### [306] [SPGISpeech 2.0: Transcribed multi-speaker financial audio for speaker-tagged transcription](https://arxiv.org/abs/2508.05554)
...
TL;DR: A reinforcement learning (RL) framework is proposed to address content hallucination in lyric-to-song generation, using preference optimization strategies (DPO, PPO, GRPO) to improve alignment and reduce phoneme error rates (PER).


<details>
  <summary>Details</summary>
Motivation: Current supervised fine-tuning (SFT) approaches for lyric-to-song generation suffer from content hallucination and limited self-improvement, necessitating a more effective solution.

Method: The proposed RL framework leverages a hallucination preference dataset and three optimization strategies (DPO, PPO, GRPO) to iteratively improve alignment and reduce PER.

Result: DPO achieved a 7.4% PER reduction, while PPO and GRPO reduced PER by 4.9% and 4.7%, respectively, effectively suppressing hallucinations without compromising musical quality.

Conclusion: The RL-based framework provides a systematic solution for hallucination control in lyric-to-song generation, with potential applications for music style adherence and musicality enhancement.
...
TL;DR: SpectroStream is a neural audio codec for high-quality 48 kHz stereo music at 4-16 kbps, improving on SoundStream with time-frequency domain representation and delayed-fusion for multi-channel audio.
...
Result: Achieves high-quality reconstruction of stereo music at 4-16 kbps, balancing per-channel quality and phase consistency.

Conclusion: SpectroStream advances neural audio coding by efficiently handling higher sample rates and multi-channel audio.
...
TL;DR: The paper compares the effectiveness of autoregressive diffusion models (ADMs) and Generative Infinite-Vocabulary Transformers (GIVT) in modeling musical expectancy and surprisal, showing ADMs outperform GIVT in tasks like pitch surprisal and segment boundary detection.


<details>
  <summary>Details</summary>
Motivation: To explore whether diffusion models can better model musical surprisal and expectancy compared to GIVTs, leveraging their ability to handle diverse data and granularity.

Method: Uses IC estimates from ADMs based on diffusion ODEs, evaluating performance in monophonic pitch surprisal and multi-track audio segment boundary detection.

Result: ADMs outperform GIVTs in both tasks, with performance improving at specific noise levels, suggesting a link between noise levels and audio feature granularity.

Conclusion: Diffusion models are more effective than GIVTs for musical surprisal tasks, with noise levels influencing performance, supporting their use in audio analysis.
...
### [310] [Online Sparsification of Bipartite-Like Clusters in Graphs](https://arxiv.org/abs/2508.05437)
*Joyentanuj Das,Suranjan De,He Sun*

Main category: cs.DS

TL;DR: The paper introduces efficient online sparsification algorithms for finding bipartite-like clusters in undirected and directed graphs, improving speed while maintaining effectiveness.


<details>
  <summary>Details</summary>
Motivation: Recent studies emphasize the importance of inter-connections between vertex sets in real-world graph analysis, prompting the need for algorithms that identify bipartite-like clusters.

Method: The authors propose online sparsification algorithms tailored for bipartite-like clusters in undirected and directed graphs.
...
Conclusion: The presented algorithms enhance the efficiency of existing clustering methods while preserving their performance, addressing the need for scalable graph analysis.
...
### [311] [Navigating Through Paper Flood: Advancing LLM-based Paper Evaluation through Domain-Aware Retrieval and Latent Reasoning](https://arxiv.org/abs/2508.05129)
*Wuqiang Zheng,Yiyan Xu,Xinyu Lin,Chongming Gao,Wenjie Wang,Fuli Feng*

Main category: cs.IR

TL;DR: PaperEval is an LLM-based framework for automated paper evaluation, improving novelty assessment and reasoning through domain-aware retrieval and latent reasoning. It outperforms existing methods and shows practical success in a recommendation system.


<details>
  <summary>Details</summary>
Motivation: The challenge of identifying high-quality research amid growing academic publications, and the limitations of current LLM-based methods in outdated knowledge and reasoning.

Method: PaperEval uses domain-aware retrieval for contextual novelty assessment and latent reasoning for deep understanding. It includes progressive ranking optimization for iterative refinement.

Result: Outperforms existing methods in academic impact and quality evaluation. Successfully deployed in a recommendation system with strong engagement.

Conclusion: PaperEval effectively addresses limitations of current methods and demonstrates practical utility in real-world applications.
...
### [312] [Align-for-Fusion: Harmonizing Triple Preferences via Dual-oriented Diffusion for Cross-domain Sequential Recommendation](https://arxiv.org/abs/2508.05074)
*Yongfu Zha,Xinxin Dong,Haokai Ma,Yonghui Yang,Xiaodong Wang*

Main category: cs.IR

TL;DR: HorizonRec is a novel cross-domain sequential recommendation framework using dual-oriented diffusion models to harmonize triple preferences, addressing noise and instability in existing methods.


<details>
  <summary>Details</summary>
Motivation: To improve cross-domain sequential recommendation by addressing data sparsity, interest drift, and the limitations of mechanical fusion in existing methods.

Method: Proposes HorizonRec, which uses dual-oriented diffusion models for fine-grained preference fusion, introduces mixed-conditioned distribution retrieval, and suppresses noise.

Result: Demonstrates effectiveness and robustness on four datasets from two platforms, outperforming existing methods in fine-grained preference fusion.

Conclusion: HorizonRec successfully addresses key challenges in CDSR, offering a robust solution for multi-domain preference modeling.
...
### [313] [Tool Graph Retriever: Exploring Dependency Graph-based Tool Retrieval for Large Language Models](https://arxiv.org/abs/2508.05152)
*Linfeng Gao,Yaoxiang Wang,Minlong Peng,Jialong Tang,Yuzhe Shang,Mingming Sun,Jinsong Su*

Main category: cs.IR

TL;DR: The paper proposes Tool Graph Retriever (TGR), a method leveraging tool dependencies for better tool retrieval, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current tool retrieval methods ignore dependencies between tools, risking omission of prerequisite tools.

Method: TGR constructs a tool dependency graph, uses graph convolution for representation, and trains a discriminator on the TDI300K dataset.

Result: TGR improves performance on standard datasets, achieving state-of-the-art results.

Conclusion: Tool dependencies are crucial, and TGR effectively addresses their integration for retrieval.
...
### [314] [Balancing Accuracy and Novelty with Sub-Item Popularity](https://arxiv.org/abs/2508.05198)
...
<details>
  <summary>Details</summary>
Motivation: Address the limitation of Personalised Popularity Scores (PPS) reinforcing known content, hindering novelty and serendipity in recommendations.

Method: Repurpose RecJPQ's sub-item architecture to model personalised popularity at a finer granularity, integrating sub-ID-level PPS (sPPS).

Result: sPPS achieves higher personalised novelty without compromising recommendation accuracy.

Conclusion: The proposed sPPS method effectively balances relevance and novelty, enhancing long-term user engagement.
...
### [315] [Scaling Generative Recommendations with Context Parallelism on Hierarchical Sequential Transducers](https://arxiv.org/abs/2508.04711)
*Yue Dong,Han Li,Shen Li,Nikhil Patel,Xing Liu,Xiaodong Wang,Chuanhao Zhuge*

Main category: cs.IR

TL;DR: The paper introduces context parallelism with jagged tensor support for HSTU attention, enabling a 5.3x increase in sequence length and 1.55x scaling with DDP.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of scaling sequence length in large-scale recommendation systems due to activation-heavy computations and jagged input tensors.

Method: Proposes context parallelism (CP) with jagged tensor support for HSTU attention to distribute computation and reduce memory usage.

Result: Achieves a 5.3x increase in supported user interaction sequence length and a 1.55x scaling factor with DDP.

Conclusion: The approach successfully scales sequence dimensions in recommendation systems, improving efficiency and performance.
...
### [316] [A Metric for MLLM Alignment in Large-scale Recommendation](https://arxiv.org/abs/2508.04963)
*Yubin Zhang,Yanhua Huang,Haiming Xu,Mingliang Qi,Chang Wang,Jiarui Jin,Xiangyuan Ren,Xiaodan Wang,Ruiwen Xu*

Main category: cs.IR

TL;DR: The paper introduces the Leakage Impact Score (LIS) to evaluate multimodal recommendation alignment, addressing challenges like static benchmarks, costly online evaluations, and uninformative metrics. LIS measures preference data bounds and shows effectiveness in real-world tests.


<details>
  <summary>Details</summary>
Motivation: Multimodal recommendation systems need aligned content representations from MLLMs, but evaluating alignment is challenging due to dynamic benchmarks, expensive online tests, and inadequate metrics.

Method: Proposes the Leakage Impact Score (LIS) to measure preference data bounds instead of directly evaluating MLLMs. Includes practical deployment insights.

Result: LIS demonstrates effectiveness in online A/B tests, improving user engagement (spent time) and advertiser value on Xiaohongshu's platform.

Conclusion: LIS is a practical and efficient metric for evaluating multimodal recommendation alignment, validated by real-world performance gains.
...
### [317] [Multi-Modal Multi-Behavior Sequential Recommendation with Conditional Diffusion-Based Feature Denoising](https://arxiv.org/abs/2508.05352)
*Xiaoxi Cui,Weihai Lu,Yu Tong,Yiheng Li,Zhejun Zhao*

Main category: cs.IR

TL;DR: A novel model (M³BSR) improves sequential recommendations by addressing multi-modal and multi-behavior challenges, outperforming existing methods.
...
Method: Proposes M³BSR with Conditional Diffusion Modality Denoising, behavior denoising, and Multi-Expert Interest Extraction to model common and specific interests.

Result: M³BSR significantly outperforms state-of-the-art methods on benchmark datasets.

Conclusion: The model effectively addresses key challenges in multi-modal multi-behavior sequential recommendation, improving performance.
...
### [318] [On the Reliability of Sampling Strategies in Offline Recommender Evaluation](https://arxiv.org/abs/2508.05398)
*Bruno L. Pereira,Alan Said,Rodrygo L. T. Santos*

Main category: cs.IR

TL;DR: The paper examines how logging and sampling choices impact offline evaluation reliability in recommender systems, focusing on exposure and sampling biases.


<details>
  <summary>Details</summary>
Motivation: To address the lack of understanding about how sampling strategies affect offline evaluation reliability under varying exposure conditions and true user preferences.

Method: Simulates diverse exposure biases using a fully observed dataset as ground truth, assessing sampling strategies on resolution, fidelity, robustness, and predictive power.

Result: Reveals when and how sampling distorts evaluation outcomes, providing insights into selecting strategies for faithful and robust comparisons.

Conclusion: Offers practical guidance for choosing sampling strategies to improve offline evaluation reliability in recommender systems.
...
### [319] [KuaiLive: A Real-time Interactive Dataset for Live Streaming Recommendation](https://arxiv.org/abs/2508.05633)
*Changle Qu,Sunhao Dai,Ke Guo,Liqin Zhao,Yanan Niu,Xiao Zhang,Jun Xu*

Main category: cs.IR

TL;DR: KuaiLive is a new real-time, interactive dataset from Kuaishou, addressing the lack of public datasets for live streaming research. It includes detailed user-streamer interactions and supports diverse recommendation tasks.


<details>
  <summary>Details</summary>
Motivation: The lack of datasets reflecting live streaming dynamics hinders academic research. KuaiLive fills this gap by providing real-time interaction data from a major platform.

Method: The dataset collects interaction logs of 23,772 users and 452,621 streamers over 21 days, featuring precise timestamps, real-time interactions, and rich side information.

Result: KuaiLive enables realistic simulation of dynamic recommendations and supports tasks like top-K recommendation, click-through rate prediction, and multi-behavior modeling.

Conclusion: KuaiLive establishes a benchmark for live streaming research and is publicly available for diverse applications in recommendation systems.
...
### [320] [Differentially Private Model-X Knockoffs via Johnson-Lindenstrauss Transform](https://arxiv.org/abs/2508.04800)
*Yuxuan Tao,Adel Javanmard*

Main category: stat.ML

TL;DR: A novel framework for private high-dimensional variable selection ensures FDR control under differential privacy by using Gaussian JLT to preserve exchangeability in knockoff features.


<details>
  <summary>Details</summary>
Motivation: Existing privacy mechanisms disrupt the exchangeability required for FDR control in knockoff procedures, necessitating a new approach.

Method: The framework privatizes the knockoff matrix via Gaussian JLT, preserving covariate relationships while ensuring differential privacy. Theoretical analysis includes a debiasing technique and examines factors like JLT's dimension reduction ratio and privacy parameters.

Result: The method maintains FDR control and statistical power, outperforming classical noise addition under strict privacy budgets.

Conclusion: The work successfully bridges knockoff-based FDR control and private data release, enabling reliable variable selection in sensitive domains.
...
### [321] [The Cosine Schedule is Fisher-Rao-Optimal for Masked Discrete Diffusion Models](https://arxiv.org/abs/2508.04884)
*Leo Zhang*

Main category: stat.ML

TL;DR: The paper demonstrates that the optimal discretization schedule for sampling from masked discrete diffusion models aligns with the Fisher-Rao geometry, confirming the effectiveness of the widely-used cosine schedule.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the optimal discretization schedule for sampling in masked discrete diffusion models by leveraging information geometry.

Method: The authors analyze the problem using the Fisher-Rao geometry of the induced probability path.

Result: They prove that the optimal schedule under this geometry matches the commonly-used cosine schedule.

Conclusion: The findings validate the empirical success of the cosine schedule in practice, linking it to theoretical principles of information geometry.
...
### [322] [High-Dimensional Differentially Private Quantile Regression: Distributed Estimation and Statistical Inference](https://arxiv.org/abs/2508.05212)
*Ziliang Shen,Caixing Wang,Shaoli Wang,Yibo Yan*

Main category: stat.ML

TL;DR: A differentially private quantile regression method for high-dimensional data in distributed settings, addressing privacy concerns while ensuring statistical accuracy and formal privacy guarantees.


<details>
  <summary>Details</summary>
Motivation: Privacy concerns in handling heterogeneous datasets with sensitive information, and the need for robust statistical tools like quantile regression in high-dimensional settings.

Method: Proposes a Newton-type transformation to reformulate quantile regression into an ordinary least squares problem, followed by a differentially private estimation algorithm and a debiased estimator for inference.

Result: Demonstrates robustness and effectiveness in simulations, ensuring near-optimal statistical accuracy and privacy guarantees.

Conclusion: The method successfully addresses privacy and computational challenges in high-dimensional quantile regression, suitable for distributed settings.
...
### [323] [L1-Regularized Functional Support Vector Machine](https://arxiv.org/abs/2508.05567)
*Bingfan Liu,Peijun Sang*

Main category: stat.ML

TL;DR: Proposes an L1-regularized functional SVM for binary classification with multivariate functional covariates, showing good performance in prediction and feature selection.


<details>
  <summary>Details</summary>
Motivation: To address the gap in using multivariate functional covariates for binary classification.

Method: L1-regularized functional support vector machine (SVM) with an accompanying algorithm for fitting and identifying relevant functional covariates.

Result: Demonstrates good performance in prediction and feature selection via simulations and a real-world application.

Conclusion: The proposed classifier effectively handles multivariate functional covariates and identifies relevant features.
...
### [324] [High-Order Error Bounds for Markovian LSA with Richardson-Romberg Extrapolation](https://arxiv.org/abs/2508.05570)
*Ilya Levin,Alexey Naumov,Sergey Samsonov*

Main category: stat.ML

TL;DR: The paper analyzes bias and error bounds in Linear Stochastic Approximation (LSA) with Polyak-Ruppert (PR) averaging under Markovian noise, proposing a bias decomposition and using Richardson-Romberg (RR) extrapolation to cancel the leading bias term.


<details>
  <summary>Details</summary>
Motivation: To understand and mitigate the bias in LSA with PR averaging, particularly its dependence on step size, and improve error bounds.

Method: Proposes a linearization technique for bias decomposition, analyzes its structure, and applies RR extrapolation to cancel the leading bias term. Derives high-order moment bounds for RR iterates.

Result: The leading bias term is linear in step size and persists under PR averaging. RR extrapolation cancels this term, aligning the leading error with the optimal covariance matrix of vanilla LSA.

Conclusion: RR extrapolation effectively addresses bias in LSA with PR averaging, improving error bounds and aligning with asymptotic optimality.
...
### [325] [Harmonic fractal transformation for modeling complex neuronal effects: from bursting and noise shaping to waveform sensitivity and noise-induced subthreshold spiking](https://arxiv.org/abs/2508.05341)
*Mariia Sorokina*
...
<details>
  <summary>Details</summary>
Motivation: To create a method that mimics neuronal functionality by transforming input spectra in a fractal manner, unlike conventional filters.

Method: Proposes fractal frequency mapping, which recomposes input spectra nonlinearly to form resonant spikes optimal for sampling.

Result: Achieves high sensitivity detection, noise robustness, and noise-induced signal amplification.

Conclusion: Neuronal functionality can be modeled as linear summation of spectra over a nonlinearly transformed frequency domain.
...
### [326] [JPS: Jailbreak Multimodal Large Language Models with Collaborative Visual Perturbation and Textual Steering](https://arxiv.org/abs/2508.05087)
...
TL;DR: JPS is a novel method for jailbreaking multimodal large language models (MLLMs) by combining visual perturbations and textual steering prompts, achieving high attack success and malicious intent fulfillment rates.


<details>
  <summary>Details</summary>
Motivation: Current jailbreak attacks focus on bypassing safety filters but often produce low-quality outputs that don't fulfill malicious intent. JPS addresses this gap.
...
### [327] [Embedding Alignment in Code Generation for Audio](https://arxiv.org/abs/2508.05473)
*Sam Kouteili,Hiren Madhu,George Typaldos,Mark Santolucito*

Main category: cs.MM

TL;DR: The paper explores improving LLM-powered code generation for creative coding by analyzing the relationship between code and audio embeddings, proposing a predictive model to align them for diverse musical outputs.


<details>
  <summary>Details</summary>
Motivation: To enhance creative coding (e.g., live-coding) by enabling users to focus on structural motifs over syntax, while ensuring diverse and musically meaningful code candidates.

Method: Investigates the topology of code-audio embedding spaces, constructs a predictive model to align these embeddings, and proposes a model to predict audio embeddings from code.

Result: Finds no simple linear relationship between code and audio embeddings but demonstrates that an embedding alignment map can be learned.

Conclusion: Proposes a model to align code and audio embeddings, facilitating musically diverse and meaningful code generation for creative coding.
...
### [328] [Adversarial Attacks and Defenses on Graph-aware Large Language Models (LLMs)](https://arxiv.org/abs/2508.04894)
*Iyiola E. Olatunji,Franziska Boenisch,Jing Xu,Adam Dziedzic*

Main category: cs.CR

TL;DR: The paper explores the vulnerabilities of graph-aware LLMs to adversarial attacks, comparing two models (LLAGA and GRAPHPROMPTER), and proposes a defense framework (GALGUARD).


<details>
  <summary>Details</summary>
Motivation: To investigate the robustness of graph-aware LLMs against adversarial attacks, as their integration with graph-structured data is growing but their vulnerabilities are unexplored.

Method: Leveraged existing adversarial attack methods (poisoning and evasion) on LLAGA and GRAPHPROMPTER, and discovered a new attack surface for LLAGA. Analyzed design choices in graph encoding.

Result: Found that LLAGA's node sequence template increases vulnerability, GRAPHPROMPTER's GNN encoder is more robust, and both are susceptible to feature perturbation attacks.

Conclusion: Proposed GALGUARD, a defense framework combining LLM-based feature correction and adapted GNN defenses to mitigate attacks.
...
### [329] [Incident Response Planning Using a Lightweight Large Language Model with Reduced Hallucination](https://arxiv.org/abs/2508.05188)
*Kim Hammar,Tansu Alpcan,Emil C. Lupu*

Main category: cs.CR

TL;DR: A novel method using LLMs for incident response planning reduces hallucination and improves recovery times, running efficiently on commodity hardware.


<details>
  <summary>Details</summary>
Motivation: The challenge of identifying effective response actions for complex systems during cyberattacks, with current LLM-based methods being costly and prone to errors.

Method: Three-step approach: fine-tuning, information retrieval, and lookahead planning to minimize hallucination.

Result: Up to 22% shorter recovery times than frontier LLMs, with bounded hallucination probability and broad generalization.

Conclusion: The method offers a lightweight, effective solution for incident response, balancing performance and reliability.
...
### [330] [Two tales for a geometric Jensen--Shannon divergence](https://arxiv.org/abs/2508.05066)
*Frank Nielsen*

Main category: cs.IT

TL;DR: The paper introduces an extended geometric Jensen-Shannon divergence (G-JSD) for positive densities, providing bounds and closed-form expressions for Gaussian distributions.


<details>
  <summary>Details</summary>
Motivation: To generalize the G-JSD for positive densities without normalizing geometric mixtures, extending its applicability.

Method: Proposes an extended G-JSD, analyzes its gap with G-JSD, derives bounds, and provides closed-form expressions for Gaussian distributions.

Result: Explicit gap and bounds between extended G-JSD and G-JSD, with closed-form solutions for Gaussian cases.

Conclusion: Both G-JSD and extended G-JSD are regularizations of the ordinary JSD, broadening theoretical and practical utility.
...
### [331] [Salt-Rock Creep Deformation Forecasting Using Deep Neural Networks and Analytical Models for Subsurface Energy Storage Applications](https://arxiv.org/abs/2508.05248)
*Pradeep Kumar Shukla,Tanujit Chakraborty,Mustafa Sari,Joel Sarout,Partha Pratim Mandal*

Main category: physics.geo-ph

TL;DR: The study analyzes time series forecasting methods to predict salt rock creep deformation under varying pressures, comparing deep neural network models (N-BEATS, TCN, RNN, Transformers) against statistical baselines. N-BEATS and TCN outperformed, showing 15-20% accuracy improvement.
...
Method: Used multi-stage triaxial creep data, resampled axial strain datasets, and applied STL, Granger causality, ADF tests, and wavelet coherence analysis. Compared DNN models (N-BEATS, TCN, RNN, TF) against statistical baselines.

Result: N-BEATS and TCN models outperformed others, showing 15-20% accuracy improvement over traditional models.

Conclusion: DNN models, especially N-BEATS and TCN, effectively capture complex temporal dependencies, making them superior for predicting salt rock creep deformation.
...
### [332] [Anti-Jamming Sensing with Distributed Reconfigurable Intelligent Metasurface Antennas](https://arxiv.org/abs/2508.04964)
*Zhaowei Wang,Yunsong Huang,Weicheng Liu,Hui-Ming Wang*

Main category: eess.SP

TL;DR: The paper proposes a distributed RIMSA system using DRL and neural networks to improve RF sensing accuracy in adverse environments, including jamming attacks.


<details>
  <summary>Details</summary>
Motivation: Traditional RF sensing suffers from accuracy issues due to unpredictable radio environments like fading and noise. Distributed RIMSA aims to overcome these challenges.

Method: Employ distributed RIMSA receivers with programmable beamforming. Use DRL for optimal beamforming and a neural network for signal-to-sensing mapping. Include a combined loss function for SINR consideration.

Result: Simulations show distributed RIMSA outperforms centralized systems in efficiency and resilience to environmental influences and jamming attacks.

Conclusion: The proposed method enhances RF sensing accuracy and robustness, even in interference-prone environments.
...
TL;DR: The paper presents a framework for improving mammographic image retrieval by comparing CNN architectures with advanced training strategies, achieving significant performance improvements and setting new benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current medical image retrieval systems face challenges like inadequate sample sizes and insufficient validation, hindering clinical translation.

Method: The study compares CNN architectures (DenseNet121, ResNet50, VGG16) using advanced training strategies (fine-tuning, metric learning, super-ensemble optimization) with rigorous data splitting and statistical validation.

Result: Advanced fine-tuning and super-ensemble optimization improved precision@10 significantly (e.g., DenseNet121: 34.79%, ResNet50: 34.54%, super-ensemble: 36.33%), outperforming literature benchmarks.

Conclusion: The framework sets new performance benchmarks and provides guidelines for clinical deployment in diagnostic support and quality assurance.
...
TL;DR: The paper introduces cryoGS, a GMM-based method for cryo-EM reconstruction, enabling stable and efficient 3D molecular structure determination from noisy 2D projections without external initialization.
...
Method: The authors propose cryoGS, integrating Gaussian splatting with cryo-EM physics, including orthogonal projection-aware splatting, a normalization term, and an FFT-aligned coordinate system.

Result: Experiments on real datasets show cryoGS is effective and robust compared to baselines, enabling reconstruction from raw images with random initialization.

Conclusion: cryoGS advances cryo-EM reconstruction by eliminating dependency on external initialization, offering a more self-contained and efficient solution.
...
TL;DR: The paper proposes using implicit neural representations (INRs) for artifact detection in medical imaging, offering a compact, continuous solution that reduces memory overhead and maintains performance.


<details>
  <summary>Details</summary>
Motivation: Artifacts in medical imaging hinder diagnostic accuracy, and existing methods suffer from information loss and scalability issues due to high memory demands.

Method: The approach employs INRs to represent images compactly, combined with deep weight space networks, graph neural networks, and relational attention transformers for quality assessment.

Result: Tested on the ACDC dataset with synthetic artifacts, the method matches performance with fewer parameters.

Conclusion: INRs provide an efficient, scalable solution for artifact detection in medical imaging.
...
TL;DR: AI models outperform pathologists in distinguishing Spitz tumors from melanomas and predicting genetic aberrations and diagnostic categories, potentially improving diagnostic workflows.


<details>
  <summary>Details</summary>
Motivation: To address the diagnostic challenge of distinguishing Spitz tumors from melanomas and predicting their genetic and diagnostic features using AI.
...
Result: AI achieved AUROC 0.95 and accuracy 0.86 in differentiation, outperforming pathologists. Genetic and diagnostic predictions were better than random chance.

Conclusion: AI models show strong performance in distinguishing Spitz tumors and melanomas, with potential to enhance diagnostic efficiency and reduce costs.
...
### [338] [Telegrapher's Generative Model via Kac Flows](https://arxiv.org/abs/2506.20641)
*Richard Duong,Jannis Chemseddine,Peter K. Friz,Gabriele Steidl*

Main category: math.AP

TL;DR: A novel flow-based generative model using the telegrapher's equation (Kac process) is proposed, offering Lipschitz continuity and bounded velocity, outperforming diffusion models.


<details>
  <summary>Details</summary>
Motivation: To address limitations of diffusion models, such as unbounded velocity, by leveraging the telegrapher's equation and its stochastic Kac process for more stable and efficient generative modeling.

Method: The Kac process, derived from the telegrapher's equation, is extended to multi-dimensions. A neural network approximates the velocity field via flow matching for sample generation.

Result: The model demonstrates scalability and advantages over diffusion models, with globally bounded velocity and Lipschitz continuity in Wasserstein distance.

Conclusion: The Kac-based flow model provides a promising alternative to diffusion models, with theoretical and practical benefits.
...
### [339] [Making Prompts First-Class Citizens for Adaptive LLM Pipelines](https://arxiv.org/abs/2508.05012)
*Ugur Cetintemel,Shu Chen,Alexander W. Lee,Deepti Raghavan*

Main category: cs.DB

TL;DR: SPEAR introduces a structured language and runtime for managing prompts in LLM pipelines, enabling dynamic refinement and organized prompt management.
...
Method: SPEAR provides a prompt algebra for construction and adaptation, supporting manual, assisted, and automatic refinement modes.

Result: Preliminary experiments show the effectiveness of refinement modes and optimizations like operator fusion.
...
### [340] [AgenticData: An Agentic Data Analytics System for Heterogeneous Data](https://arxiv.org/abs/2508.05002)
*Ji Sun,Guoliang Li,Peiyao Zhou,Yihui Ma,Jingzhe Xu,Yuan Li*

Main category: cs.DB

TL;DR: AgenticData is an agentic data analytics system that autonomously analyzes data using natural language queries, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing systems are costly and slow due to reliance on expert coding and complex workflows.
...
Result: Achieved superior accuracy on benchmarks, outperforming state-of-the-art methods.

Conclusion: AgenticData effectively automates data analytics, reducing reliance on experts and improving efficiency.
...
### [341] [AI Should Be More Human, Not More Complex](https://arxiv.org/abs/2508.04713)
*Carlo Esposito*

Main category: cs.HC

TL;DR: Users prefer concise, sourced AI responses over verbose ones, as current trends in AI sophistication reduce trust and engagement.


<details>
  <summary>Details</summary>
Motivation: To investigate why verbose AI responses in search applications reduce user satisfaction and engagement.

Method: A study with 10,000 participants comparing responses from five major AI-powered search systems.

Result: Users overwhelmingly prefer concise, source-attributed responses, revealing that 'artificial sophistication' harms trust and cognitive load.

Conclusion: Optimal AI communication should mirror human discourse—direct, sourced, and transparent—challenging the assumption that complexity equals better performance.
...
### [342] [Evaluating the Impact of LLM-guided Reflection on Learning Outcomes with Interactive AI-Generated Educational Podcasts](https://arxiv.org/abs/2508.04787)
*Vishnu Menon,Andy Cherney,Elizabeth B. Cloude,Li Zhang,Tiffany D. Do*

Main category: cs.HC

TL;DR: Embedding LLM-guided reflection prompts in AI-generated podcasts did not improve learning but reduced perceived attractiveness.


<details>
  <summary>Details</summary>
Motivation: To explore the impact of reflection prompts on learning and user experience in AI-generated podcasts.

Method: Thirty-six undergraduates were tested with two versions of an AI-generated podcast: one with LLM-guided reflection prompts and one without.

Result: Learning outcomes were similar, but reflection prompts reduced perceived attractiveness.

Conclusion: More research is needed on designing reflective interactivity to balance learning and user experience.
...
### [343] [Situated Epistemic Infrastructures: A Diagnostic Framework for Post-Coherence Knowledge](https://arxiv.org/abs/2508.04995)
*Matthew Kelly*

Main category: cs.HC

TL;DR: The paper introduces the Situated Epistemic Infrastructures (SEI) framework to analyze knowledge authority in hybrid human-machine systems, critiquing traditional models of coherence and advocating for adaptive epistemic stewardship.


<details>
  <summary>Details</summary>
Motivation: To address the fragility of knowledge infrastructures exposed by LLMs like ChatGPT, which simulate coherence without traditional validation, and to explore how credibility is mediated in post-coherence conditions.

Method: Proposes the SEI framework, integrating infrastructure studies, platform theory, and epistemology, focusing on coordination over classification.

Result: SEI offers a diagnostic tool for analyzing knowledge authority, emphasizing adaptive models of epistemic stewardship.

Conclusion: The framework contributes to AI governance, knowledge production, and ethical information system design, challenging representationalist models of scholarly communication.
...
### [344] [Human-AI Schema Discovery and Application for Creative Problem Solving](https://arxiv.org/abs/2508.05045)
*Sitong Wang*
...
Method: Designs systems for sensemaking over examples to abstract schemas and operationalize them into human-AI co-creative workflows.

Result: The framework makes implicit knowledge more actionable, improving transparency and collaboration in human-AI systems.

Conclusion: Schema-guided interaction can advance human-AI collaboration by making knowledge more accessible and actionable.
...
### [345] [CWEFS: Brain volume conduction effects inspired channel-wise EEG feature selection for multi-dimensional emotion recognition](https://arxiv.org/abs/2508.05228)
*Xueyuan Xu,Wenjia Dong,Fulin Wei,Li Zhuo*

Main category: cs.HC

TL;DR: The paper proposes a channel-wise EEG feature selection (CWEFS) method for multi-dimensional emotion recognition, addressing redundancy and improving interpretability by integrating latent structure modeling and adaptive channel-weight learning.


<details>
  <summary>Details</summary>
Motivation: High-dimensional EEG features often contain redundant information, hindering discriminative emotional representation extraction and real-time performance. Existing methods overlook latent EEG feature structures and assume uniform channel importance.

Method: CWEFS integrates EEG feature selection into a shared latent structure model, preserving local geometric structure and incorporating adaptive channel-weight learning to determine channel significance.

Result: CWEFS outperformed nineteen feature selection methods across six evaluation metrics on three EEG datasets, achieving optimal emotion recognition performance.

Conclusion: CWEFS effectively addresses EEG feature redundancy and improves emotion recognition by leveraging latent structures and adaptive channel weighting.
...
### [346] [ADSEL: Adaptive dual self-expression learning for EEG feature selection via incomplete multi-dimensional emotional tagging](https://arxiv.org/abs/2508.05229)
*Tianze Yu,Junming Zhang,Wenjia Dong,Xueyuan Xu,Li Zhuo*

Main category: cs.HC

TL;DR: A novel EEG feature selection method (ADSEL) addresses incomplete multi-dimensional emotion labels by integrating adaptive dual self-expression learning and least squares regression, improving label recovery and feature selection.


<details>
  <summary>Details</summary>
Motivation: Existing EEG feature selection methods assume complete labels, but incomplete data in real-world settings compromises model generalization. Current methods also neglect sample-level and dimension-level correlations in label recovery.

Method: Proposes ADSEL, combining adaptive dual self-expression learning with least squares regression. ADSEL links sample-level and dimension-level learning for better label reconstruction and feature selection.
...
Conclusion: The proposed method effectively handles incomplete labels and enhances EEG-based emotion recognition by leveraging sample and dimension correlations.
...
### [347] [FDC-Net: Rethinking the association between EEG artifact removal and multi-dimensional affective computing](https://arxiv.org/abs/2508.05231)
*Wenjia Dong,Xueyuan Xu,Tianze Yu,Junming Zhang,Li Zhuo*

Main category: cs.HC

TL;DR: A novel framework, FDC-Net, integrates denoising and emotion recognition for EEG data, improving robustness and performance compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: EEG-based emotion recognition is hindered by noise and the lack of synergy between denoising and classification tasks.

Method: FDC-Net uses bidirectional gradient propagation, joint optimization, and a gated attention mechanism with a frequency-adaptive Transformer.

Result: Achieved high denoising (96.30% CC on DEAP, 90.31% on DREAMER) and emotion recognition accuracies (82.3% on DEAP, 88.1% on DREAMER).

Conclusion: FDC-Net effectively couples denoising and emotion recognition, outperforming state-of-the-art methods in noise robustness and accuracy.
...
### [348] [Driver Assistant: Persuading Drivers to Adjust Secondary Tasks Using Large Language Models](https://arxiv.org/abs/2508.05238)
*Wei Xiang,Muchen Li,Jie Yan,Manling Zheng,Hanfei Zhu,Mengyun Jiang,Lingyun Sun*

Main category: cs.HC

TL;DR: A study uses an LLM to provide persuasive advice to drivers in Level 3 automated systems, improving attention and reducing cognitive load during emergencies.


<details>
  <summary>Details</summary>
Motivation: Level 3 automation reduces driver risk perception, creating challenges during emergencies due to limited reaction time and high cognitive burden.
...
Result: The tool effectively maintains driver attention, lowers cognitive load, and coordinates secondary tasks with takeover actions.

Conclusion: LLMs show promise in supporting drivers during multi-task automated driving, enhancing safety and attention management.
...
### [349] [Federal Reserve Communication and the COVID-19 Pandemic](https://arxiv.org/abs/2508.04830)
*Jonathan Benchimol,Sophia Kazinnik,Yossi Saadon*

Main category: econ.GN

TL;DR: The study analyzes the Federal Reserve's communication during COVID-19, comparing it to past crises, revealing a reactive approach and a shift toward discussing unconventional monetary policy as a norm.


<details>
  <summary>Details</summary>
Motivation: To understand how the Fed's communication strategies evolved during COVID-19 compared to previous economic crises, focusing on content, sentiment, and timing.

Method: Used specialized dictionaries for COVID-19, unconventional monetary policy, and financial stability, combined with sentiment analysis and topic modeling.

Result: Found the Fed was more reactive to COVID-19, with declining sentiment on financial stability predicting policy shifts. Unconventional monetary policy communication became standard post-Global Financial Crisis.

Conclusion: The Fed's communication adapts during crises, with COVID-19 highlighting a reactive approach and institutional shifts in messaging.
...
### [350] [Reinforcement Learning Generation of 4-Qubits Entangled States](https://arxiv.org/abs/2204.12351)
*Sara Giordano,Miguel A. Martin-Delgado*

Main category: quant-ph

TL;DR: An AI algorithm using Q-learning constructs 4-qubit entangled states, representing true SLOCC classes, with optimal quantum circuits and a graphical tool (SLG) for intuitive design.


<details>
  <summary>Details</summary>
Motivation: To automate the generation of representative entangled states for 4-qubit SLOCC classes and aid experimental realization and understanding of quantum properties.

Method: Uses Q-learning to synthesize quantum circuits, aided by the state-link graph (SLG) for visualizing Q-matrix construction and gate-set optimization.

Result: Achieves at least one true SLOCC class per entanglement family, with optimal circuits for the chosen gate-set.

Conclusion: The algorithm and SLG tool simplify and optimize entangled state construction, offering practical and theoretical insights.
...
### [351] [Hybrid Reward-Driven Reinforcement Learning for Efficient Quantum Circuit Synthesis](https://arxiv.org/abs/2507.16641)
*Sara Giordano,Kornikar Sen,Miguel A. Martin-Delgado*

Main category: quant-ph

TL;DR: A reinforcement learning (RL) framework is proposed for synthesizing efficient quantum circuits, using Q-learning and a hybrid reward mechanism to optimize gate counts and circuit depth.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of quantum circuit synthesis in both NISQ and fault-tolerant quantum computing by managing the exponential growth of quantum state space.

Method: Utilizes tabular Q-learning with action sequences in a discretized state space, combining static and dynamic rewards to guide circuit optimization.

Result: Demonstrates minimal-depth circuits for graph-state preparation (up to 7 qubits) and robustness with universal gate sets.

Conclusion: The RL-driven approach efficiently explores quantum state space and synthesizes near-optimal circuits, offering a resource-efficient solution for quantum circuit optimization.
...
### [352] [Q-DPTS: Quantum Differentially Private Time Series Forecasting via Variational Quantum Circuits](https://arxiv.org/abs/2508.05036)
*Chi-Sheng Chen,Samuel Yen-Chi Chen*

Main category: quant-ph

TL;DR: Q-DPTS is a hybrid quantum-classical framework for differentially private time series forecasting, combining VQCs with DP mechanisms to improve robustness and performance.


<details>
  <summary>Details</summary>
Motivation: Address the utility loss in time series forecasting when integrating Differential Privacy (DP), especially with DP-SGD, by leveraging quantum models for better robustness.

Method: Q-DPTS uses Variational Quantum Circuits (VQCs) with per-sample gradient clipping and Gaussian noise injection to ensure $\epsilon, \delta$-DP.

Result: Q-DPTS outperforms classical and quantum baselines (LSTM, QASA, QRWKV, QLSTM) on the ETT dataset, achieving lower prediction error under the same privacy budget.

Conclusion: Q-DPTS offers a promising privacy-utility trade-off, pioneering quantum-enhanced DP for secure and accurate time series forecasting in privacy-critical scenarios.
...
### [353] [Hybrid quantum tensor networks for aeroelastic applications](https://arxiv.org/abs/2508.05169)
*M. Lautaro Hickmann,Pedro Alves,David Quero,Friedhelm Schwenker,Hans-Martin Rieser*

Main category: quant-ph

TL;DR: Hybrid quantum tensor networks combined with QML show promise for aeroelastic problems, achieving high accuracy in binary classification and decent performance in regression tasks.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of Quantum Machine Learning (QML) in solving complex aeroelastic problems, particularly time series classification and regression.

Method: Combines tensor networks with variational quantum circuits, encoding time series into tensor networks, reducing dimensionality, and applying a trainable variational quantum circuit for classification or regression.

Result: High accuracy in binary classification and promising performance in regression tasks, though hyperparameter optimization is challenging.

Conclusion: The hybrid quantum tensor network approach significantly advances QML for aeroelastic applications, despite optimization hurdles.
...
### [354] [LLM-based Multi-Agent Copilot for Quantum Sensor](https://arxiv.org/abs/2508.05421)
...
TL;DR: QCopilot is an LLM-based multi-agent framework for quantum sensor design, automating optimization and diagnosis, achieving significant speedup in experiments.
...
Method: Integrates external knowledge access, active learning, and uncertainty quantification with commercial LLMs and few-shot prompt engineering.

Result: Achieved 100x speedup in atom cooling experiments and autonomous anomaly detection in multi-parameter settings.
...
### [355] [On the Design of Expressive and Trainable Pulse-based Quantum Machine Learning Models](https://arxiv.org/abs/2508.05559)
*Han-Xiao Tao,Xin Wang,Re-Bing Wu*

Main category: quant-ph

TL;DR: Pulse-based QML models must balance expressivity and trainability, with a necessary condition involving initial state, measurement observable, and dynamical symmetry Lie algebra.


<details>
  <summary>Details</summary>
Motivation: To ensure pulse-based QML models are both expressive and trainable, avoiding uncontrollability issues.

Method: Investigates requirements through theoretical analysis and numerical simulations.

Result: Identifies a necessary condition for expressive yet trainable pulse-based QML models.

Conclusion: Provides a framework for designing practical pulse-based QML models.
...
### [356] [Can SGD Handle Heavy-Tailed Noise?](https://arxiv.org/abs/2508.04860)
*Ilyas Fatkhullin,Florian Hübler,Guanghui Lan*

Main category: math.OC

TL;DR: The paper investigates vanilla SGD's effectiveness under heavy-tailed noise, proving it achieves minimax optimal convergence rates in convex, strongly convex, and non-convex settings.


<details>
  <summary>Details</summary>
Motivation: To rigorously analyze SGD's performance under heavy-tailed noise, common in modern ML, and challenge the notion that it fails in such conditions.

Method: Theoretical analysis of SGD under bounded p-th moment assumptions, covering convex, strongly convex, and non-convex problems, with matching lower bounds.

Result: SGD achieves minimax optimal rates: O(ε^(-p/(p-1))) for convex, O(ε^(-p/(2(p-1)))) for strongly convex, and O(ε^(-2p/(p-1))) for non-convex objectives.

Conclusion: Vanilla SGD is robust under heavy-tailed noise, providing a principled baseline even with unbounded variance.
...
### [357] [Exact and Heuristic Algorithms for Constrained Biclustering](https://arxiv.org/abs/2508.05493)
*Antonio M. Sudoso*

Main category: math.OC

TL;DR: The paper introduces constrained biclustering with pairwise constraints, proposing exact and heuristic algorithms to solve the problem, demonstrating effectiveness on synthetic and real-world datasets.


<details>
  <summary>Details</summary>
Motivation: Incorporating background knowledge into biclustering enhances solution quality and interpretability, extending prior information to guide joint grouping of rows and columns.

Method: Exact branch-and-cut algorithm based on SDP relaxation, strengthened with valid inequalities, and a heuristic using low-rank SDP factorization solved via augmented Lagrangian method.

Result: Exact method outperforms general-purpose solvers; heuristic efficiently provides high-quality solutions for large instances.

Conclusion: The proposed methods effectively solve constrained biclustering, balancing precision and scalability.
...
### [358] [A Study of the Framework and Real-World Applications of Language Embedding for 3D Scene Understanding](https://arxiv.org/abs/2508.05064)
*Mahmoud Chick Zaouali,Todd Charter,Yehor Karpichev,Brandon Haworth,Homayoun Najjjaran*

Main category: cs.GR

TL;DR: This survey reviews the integration of Large Language Models (LLMs) with Gaussian Splatting for text-conditioned 3D scene understanding, highlighting advancements, limitations, and future directions.


<details>
  <summary>Details</summary>
Motivation: The intersection of Gaussian Splatting and LLMs offers new possibilities for 3D scene representation, but lacks a comprehensive overview.

Method: The paper provides a structured review of current research, detailing theoretical foundations, integration strategies, and use cases.

Result: Key advancements are noted, but limitations like computational bottlenecks and data scarcity are identified.

Conclusion: The survey outlines open challenges and future directions for advancing language-guided 3D scene understanding with Gaussian Splatting.
...
### [359] [Voost: A Unified and Scalable Diffusion Transformer for Bidirectional Virtual Try-On and Try-Off](https://arxiv.org/abs/2508.04825)
*Seungyong Lee,Jeong-gi Kwak*

Main category: cs.GR

TL;DR: Voost is a unified framework using a diffusion transformer for virtual try-on and try-off, improving garment-body reasoning and achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Accurate garment-body correspondence in virtual try-on is challenging due to pose and appearance variations.
...
Conclusion: Voost provides a scalable and unified solution for virtual try-on/try-off, enhancing garment-body reasoning without task-specific networks or additional labels.
...
### [360] [Refining Gaussian Splatting: A Volumetric Densification Approach](https://arxiv.org/abs/2508.05187)
*Mohamed Abdul Gafoor,Marius Preda,Titus Zaharia*

Main category: cs.GR

TL;DR: A novel density control method for 3D Gaussian Splatting improves reconstruction quality by using volumes of inertia and explores SfM/DIM for initialization.


<details>
  <summary>Details</summary>
Motivation: The vanilla 3DGS densification strategy has shortcomings, prompting the need for a better method to manage point primitives.

Method: Introduces a density control method using volumes of inertia and evaluates SfM/DIM for point cloud initialization.

Result: Outperforms 3DGS in reconstruction quality on the Mip-NeRF 360 dataset.

Conclusion: The proposed method enhances novel view synthesis in 3DGS, showing promising results across diverse scenes.
...
### [361] [Perceive-Sample-Compress: Towards Real-Time 3D Gaussian Splatting](https://arxiv.org/abs/2508.04965)
...
Result: Significant improvements in memory efficiency and visual quality without losing real-time rendering speed.

Conclusion: The proposed framework effectively addresses scalability and storage challenges in 3DGS, enhancing practical usability.
...
### [362] [Laplacian Analysis Meets Dynamics Modelling: Gaussian Splatting for 4D Reconstruction](https://arxiv.org/abs/2508.04966)
...
Method: The framework includes a spectral-aware Laplacian encoding architecture, enhanced Gaussian dynamics attributes, and an adaptive Gaussian split strategy guided by KDTree.
...
### [363] [RAP: Real-time Audio-driven Portrait Animation with Video Diffusion Transformer](https://arxiv.org/abs/2508.05115)
...
Method: RAP uses a hybrid attention mechanism for fine-grained audio control and a static-dynamic training-inference paradigm to avoid explicit motion supervision.

Result: RAP achieves precise audio-visual synchronization, mitigates temporal drift, and maintains visual fidelity, outperforming state-of-the-art methods in real-time.

Conclusion: RAP successfully balances high-quality animation with real-time performance, offering a practical solution for audio-driven portrait synthesis.
...
### [364] [Point cloud segmentation for 3D Clothed Human Layering](https://arxiv.org/abs/2508.05531)
*Davide Garavaso,Federico Masi,Pietro Musoni,Umberto Castellani*

Main category: cs.GR

TL;DR: A new 3D point cloud segmentation method for clothed human layering, addressing challenges in semantic reconstruction of garments and underlying body parts.


<details>
  <summary>Details</summary>
Motivation: High-quality 3D cloth modeling is crucial for avatars in fashion, entertainment, and animation, but current methods lack semantic accuracy for layered garments.

Method: Proposes a 3D point cloud segmentation paradigm where points can belong to multiple layers, enabling estimation of body parts and occluded cloth regions. A synthetic dataset and neural network settings are introduced for evaluation.

Result: Demonstrates improved segmentation accuracy for garment layers on synthetic and real-world datasets.

Conclusion: The proposed clothed human layering paradigm effectively addresses semantic reconstruction challenges in 3D cloth modeling.
...
### [365] [Physically Controllable Relighting of Photographs](https://arxiv.org/abs/2508.05626)
*Chris Careaga,Yağız Aksoy*

Main category: cs.GR

TL;DR: A self-supervised method for in-the-wild image relighting combines neural rendering with physical accuracy, enabling controllable illumination editing.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between traditional rendering's physical accuracy and neural rendering's photorealism for practical in-the-wild relighting.

Method: Uses a colored mesh from monocular estimates, allows 3D illumination configuration, and refines results with a neural renderer trained via differentiable rendering.

Result: Achieves photorealistic relighting with explicit physical control, comparable to tools like Blender.

Conclusion: Advances in-the-wild relighting by merging neural and traditional rendering for practical, high-quality results.
...
### [366] [Evaluating the Use of LLMs for Documentation to Code Traceability](https://arxiv.org/abs/2506.16440)
*Ebube Alor,SayedHassan Khatoonabadi,Emad Shihab*

Main category: cs.SE

TL;DR: LLMs (Claude 3.5 Sonnet, GPT-4o, o3-mini) are evaluated for automating documentation-to-code traceability, showing high accuracy in trace link identification but limitations in relationship explanations and multi-step chains.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of LLMs in automating documentation-to-code traceability, an underexplored area, and assess their capabilities compared to traditional baselines.

Method: Systematic experiments on two novel datasets (Unity Catalog, Crawl4AI) evaluating trace link identification, relationship explanation quality, and multi-step chain reconstruction.

Result: Best-performing LLM achieves F1-scores of 79.4% and 80.4%, outperforming baselines. Relationship explanations are partially accurate (97%) but fully correct only 42.9%-71.1%. Multi-step chains show high endpoint accuracy but inconsistent intermediate links.

Conclusion: LLMs are powerful for trace discovery but require human-in-the-loop tools due to limitations like naming assumptions and overgeneralization. Task-framing (e.g., one-to-many matching) is critical for performance.
...
### [367] [How Robust are LLM-Generated Library Imports? An Empirical Study using Stack Overflow](https://arxiv.org/abs/2507.10818)
*Jasmine Latendresse,SayedHassan Khatoonabadi,Emad Shihab*

Main category: cs.SE

TL;DR: LLMs favor third-party libraries over standard ones, often recommending mature, popular, and permissively licensed dependencies, but usability gaps exist, such as unresolved imports and lack of installation guidance.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs recommend libraries and assess their usability in real-world programming tasks.

Method: Empirical study of six state-of-the-art LLMs by prompting them to solve Python problems from Stack Overflow, analyzing library imports and usability.

Result: LLMs prefer third-party libraries, with 4.6% unresolved imports and only two models providing installation guidance.

Conclusion: Findings highlight usability gaps and opportunities to improve LLM-generated code reliability in dependency management.
...
### [368] [Automated File-Level Logging Generation for Machine Learning Applications using LLMs: A Case Study using GPT-4o Mini](https://arxiv.org/abs/2508.04820)
*Mayra Sofia Ruiz Rodriguez,SayedHassan Khatoonabadi,Emad Shihab*

Main category: cs.SE

TL;DR: GPT-4o mini was evaluated for generating file-level log statements in ML projects, showing promise but with issues like overlogging and misalignment with human practices.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of LLMs in generating log statements for ML applications, addressing the underexplored area of file-level logging.

Method: Evaluated GPT-4o mini on 171 ML repositories (4,073 Python files) by removing original logs, prompting the LLM to generate logs, and comparing position, level, variables, and text quality with human logs.

Result: LLM matched human log positions 63.91% of the time but had an overlogging rate of 82.66%. Challenges included overlogging at function boundaries and misalignment with project conventions.

Conclusion: LLMs like GPT-4o mini show potential for file-level log generation, but practical use requires addressing overlogging and alignment with project-specific practices.
...
### [369] [Posterior-GRPO: Rewarding Reasoning Processes in Code Generation](https://arxiv.org/abs/2508.05170)
*Lishui Fan,Yu Zhang,Mouxiang Chen,Zhongxin Liu*

Main category: cs.SE

TL;DR: A unified RL framework improves code generation by evaluating reasoning quality, mitigating reward hacking, and outperforming baselines.
...
Method: Developed LCB-RB benchmark for reasoning evaluation, introduced OD-based reward training, and proposed P-GRPO RL method to condition rewards on task success.

Result: A 7B parameter model with P-GRPO outperforms baselines by 4.5% and matches GPT-4-Turbo performance, with generalizability to mathematical tasks.

Conclusion: The framework effectively incorporates reasoning quality into RL, mitigates reward hacking, and achieves superior performance in code generation.
...
### [370] [Taxonomy of Faults in Attention-Based Neural Networks](https://arxiv.org/abs/2508.04925)
*Sigma Jahan,Saurabh Singh Rajput,Tushar Sharma,Mohammad Masudur Rahman*

Main category: cs.SE

TL;DR: The paper presents the first comprehensive empirical study of faults in attention-based neural networks (ABNNs), identifying seven unique fault categories and offering diagnostic heuristics.
...
Method: Systematic analysis of 555 real-world faults from 96 projects across ten frameworks, including GitHub and Hugging Face.

Result: Over half of ABNN faults stem from attention-specific mechanisms; four diagnostic heuristics explain 33% of these faults.
...
### [371] [EvoGraph: Hybrid Directed Graph Evolution toward Software 3.0](https://arxiv.org/abs/2508.05199)
*Igor Costa,Christopher Baran*

Main category: cs.SE

TL;DR: EvoGraph is a framework for evolving software artifacts using typed graphs, SLMs, and multi-objective fitness, achieving high success in security fixes, language translation, and documentation freshness.


<details>
  <summary>Details</summary>
Motivation: Addresses challenges in legacy modernization, such as implicit contracts and performance preservation, aiming for continuous adaptation in software systems.

Method: Represents artifacts in typed directed graphs, applies learned mutation operators via SLMs, and selects survivors with multi-objective fitness.

Result: Fixes 83% of security vulnerabilities, achieves 93% functional equivalence in COBOL-to-Java translation, and reduces latency by 40%.
...
### [372] [An ML-based Approach to Predicting Software Change Dependencies: Insights from an Empirical Study on OpenStack](https://arxiv.org/abs/2508.05034)
*Arabat,Ali,Sayagh,Mohammed,Hassine,Jameleddine*

Main category: cs.SE

TL;DR: The paper addresses dependency management in large-scale software systems, proposing a semi-automated ML approach to identify dependencies early, improving efficiency and reducing delays.


<details>
  <summary>Details</summary>
Motivation: Managing dependencies in complex software systems is critical to prevent build failures and incomplete deployments, but current methods often identify dependencies late in the process, causing inefficiencies.
...
Result: The models achieved strong performance (AUC scores of 79.33% and 91.89%, Brier scores of 0.11 and 0.014), with good recall but room for precision improvement.

Conclusion: The proposed approach effectively identifies dependencies early, reducing delays and improving dependency management in large-scale systems.
...
### [373] [Optimality Principles and Neural Ordinary Differential Equations-based Process Modeling for Distributed Control](https://arxiv.org/abs/2508.04799)
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap between data-driven algorithms and classical process control by ensuring consistency in topological properties and conservation laws.

Method: Uses connectivity matrices, network graphs, and derives a system's objective function equivalent to entropy production. Integrates neural ODEs for learning dynamics.

Result: Demonstrated in an inventory control system, where neural ODEs learn constitutive equations, forming a state-space model for predictive control.

Conclusion: The framework successfully merges topology-based conservation with data-driven learning, enabling practical applications like model predictive control.
...
### [374] [Data Driven Insights into Composition Property Relationships in FCC High Entropy Alloys](https://arxiv.org/abs/2508.04841)
...
TL;DR: The paper explores predictive modeling for High Entropy Alloys (HEAs) using advanced methods to address data scarcity and heterogeneity, achieving strong performance in mapping composition to mechanical properties.


<details>
  <summary>Details</summary>
Motivation: The scarcity of integrated data for HEAs hinders predictive modeling, necessitating advanced methods to uncover patterns in their vast design space.

Method: The study employs sensitivity analyses and encoder-decoder models, optimized via Bayesian multi-objective hyperparameter tuning, to map alloy composition to mechanical properties.

Result: The models outperform conventional regressors, especially for yield strength and UTS/YS ratio, effectively capturing complex composition-property relationships.

Conclusion: The work demonstrates the potential of advanced modeling techniques to enhance predictive capabilities for HEAs, addressing critical challenges in the field.
