...
Motivation: Existing masked autoencoder approaches for skeleton-based action recognition use raw joint coordinates or simple variants, causing computational redundancy and limited semantic representation.

Method: Collaborative learning framework with lightweight target generation network that produces diversified supervision signals across spatial-temporal hierarchies, incorporating constrained optimization to ensure feature diversity and prevent model collapse.
...
Conclusion: High-level feature prediction framework provides both computational efficiency and superior representation quality for skeleton-based action recognition.
...
Method: Two-stage framework: 1) Adaptive Bias Integrated Learning (ABIL) trains encoders for each attribute and integrates them with main backbone to recognize biases, 2) Gradient Suppression Fine Tuning prunes bias directions from backbone's gradients. Also introduces Scaled Bias Amplification (SBA) metric to measure bias.

Result: Validated on FB CMNIST, CelebA, and COCO datasets. Boosted worst group accuracy, halved multi-attribute bias amplification, and set new low in SBA even with increasing bias complexity and distribution shifts.
...
<details>
  <summary>Details</summary>
Motivation: Enable real-time clinical measurements from echocardiography by addressing nnU-Net's limitations of large size and slow speed for cardiac segmentation.

Method: Conducted ablation study to identify most effective nnU-Net components, then developed lightweight U-Net with simplified augmentations and reduced parameters.
...
Result: Significant improvements over original treeX algorithm: F1-score gains of +0.11 to +0.49 for ground-based data, and successful segmentation of ULS data (F1=0.58) where original failed. Achieves accuracy comparable to recent open-source deep learning methods.

Conclusion: Provides resource-efficient alternative to deep learning for tree segmentation, particularly useful when data characteristics align with method requirements, and for semi-automatic label generation for deep learning models.
...
Method: Dual-supervision paradigm with complementary object-level and frame-level reconstruction tasks within a dual-encoder architecture, enforcing geometric consistency to develop spatial reasoning capabilities.
...
Method: Introduces a unified end-to-end quantization strategy across both neural network models and transmitted message representations to simultaneously reduce computational load and transmission bandwidth while operating under low-bit constraints.

Result: QuantV2X achieves accuracy comparable to full-precision systems, reduces system-level latency by 3.2x, improves mAP30 by +9.5 over full-precision baselines, and enables larger models to fit within strict memory budgets.

Conclusion: QuantV2X demonstrates the viability of fully quantized multi-agent intermediate fusion systems for real-world deployment in V2X cooperative perception, offering significant efficiency gains without sacrificing performance.
...
<details>
  <summary>Details</summary>
Motivation: Graph matching networks for floor plan comparison rely on costly intermediate cross-graph node interactions, making them slow during inference time. There's a need for more efficient methods that maintain accuracy while improving speed.
...
Conclusion: The proposed LayoutGKN approach provides an efficient alternative to traditional graph matching networks for floor plan comparison, achieving similar or better performance with substantially faster inference times.
...
Conclusion: CLIP-SVD provides an effective parameter-efficient adaptation technique that enhances performance while preserving generalization ability and model interpretability through natural language-based analysis.
...
Conclusion: Integrating domain knowledge through decoupled attention provides an effective approach for edge-deployed precision agriculture AI, enabling efficient and accurate plant disease diagnosis on resource-constrained devices.
...
<details>
  <summary>Details</summary>
Motivation: Underwater camouflaged object detection is crucial for marine ecology but faces challenges from optical distortions, water turbidity, and complex marine organism traits, making accurate identification difficult.
...
Conclusion: The proposed SLENet framework effectively addresses underwater camouflaged object detection challenges and establishes a strong benchmark for future research in this domain.
...
Method: A simple training strategy that incorporates temporal inductive bias from continuous video frames into diffusion training, requiring no architectural modifications and seamlessly integrating with standard pipelines.

Result: Empirical evaluation shows 2x faster convergence, lower FID scores on both training and validation distributions, improved generative diversity, and reduced gradient variance contributing to faster optimization.

Conclusion: Leveraging temporal coherence in video data provides significant benefits for diffusion model training, improving convergence speed, distribution coverage, and model generalization without requiring architectural changes.
...
Conclusion: MedVista3D effectively addresses key challenges in 3D radiology analysis through multi-scale semantic-enriched pretraining, demonstrating strong performance across multiple medical vision-language tasks with good transfer capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based prompt learning methods show limited effectiveness on fine-grained datasets, as they struggle to capture subtle visual discrepancies between similar classes.

Method: Two modules: 1) Attribute disentanglement using Brownian Bridge Diffusion Model to separate shared and class-specific attributes; 2) Granule learning that integrates attributes under causal inference strategies to construct visual granules for more discriminative text prompts.

Result: Extensive experiments on 15 datasets show CaPL significantly outperforms state-of-the-art prompt learning methods, particularly excelling on fine-grained datasets.

Conclusion: Visual granulation through causal inference enables more effective text prompt learning for CLIP, especially for fine-grained recognition tasks where subtle visual differences matter.
...
Method: Presents 'event-lucky insight' revealing correlation between turbulence distortions and event streams, then proposes EGTM framework that extracts pixel-level reliable turbulence-free guidance from turbulent events for temporal lucky fusion. Also builds the first real-world event-driven TM dataset.

Result: Significantly surpasses existing SOTA TM method by 710 times in model size, 214 times in inference latency, and 224 times in model complexity, while achieving state-of-the-art restoration quality (+0.94 PSNR and +0.08 SSIM) on real-world dataset.

Conclusion: Demonstrates the great efficiency merit of introducing event modality into turbulence mitigation tasks, with event cameras providing microsecond-level temporal resolution and efficient sparse imaging to address computational bottlenecks.
...
<details>
  <summary>Details</summary>
Motivation: Existing RGB-Event detection methods process all regions uniformly, including low-information areas, leading to high computational costs and suboptimal performance. Fixed token sparsification methods fail to adapt to varying sample complexity.

Method: Proposes FocusMamba with Event-Guided Multimodal Sparsification (EGMS) to identify and discard low-information regions using event camera data, and Cross-Modality Focus Fusion (CMFF) to effectively integrate complementary features from both modalities.
...
Conclusion: FocusMamba achieves better balance between accuracy and efficiency through adaptive sparsification and effective fusion, demonstrating state-of-the-art performance in RGB-Event detection tasks.
...
<details>
  <summary>Details</summary>
Motivation: Food recognition needs methods for unseen categories, but existing approaches face challenges with background distractions, role confusion between dishes, and semantic bias in attributes.
...
Method: In-depth survey analyzing over 200 papers across vision, text, and audio modalities, detailing the five-phase generation process: input, motion planning, motion video generation, refinement, and output.

Result: Provides thorough overview of latest developments and technological trends, highlights milestone works, and discusses LLM potential in enhancing human motion video generation.
...
<details>
  <summary>Details</summary>
Motivation: Object function guides correspondence establishment as functional parts share shape/appearance similarities, enabling better cross-category matching for tasks like shape reconstruction and robot manipulation.
...
Result: The approach outperforms baseline solutions using off-the-shelf self-supervised image representations and grounded vision language models on curated synthetic and real evaluation datasets.

Conclusion: Functional correspondence based on object function provides effective guidance for dense matching across different categories, with vision-language models enabling weakly-supervised learning of functional parts.
...
Method: Proposes a dual attention mechanism: Memory Attn-Adapter refines category embeddings using support examples, and Local-Global Attn-Adapter enriches image embeddings by integrating local and global features for dynamic adaptation.
...
Conclusion: Attn-Adapter provides an effective online few-shot learning solution that enhances CLIP's adaptability without retraining, addressing overfitting risks and computational inefficiency of traditional fine-tuning approaches.
...
<details>
  <summary>Details</summary>
Motivation: Standard n-gram metrics fail to capture semantic correctness for long captions, while RS metrics have computational issues and LLM-based metrics are too expensive for iterative use during model development.
...
Result: SPECS matches the performance of open-source LLM-based metrics in correlation to human judgments while being far more efficient computationally.

Conclusion: SPECS provides a practical and efficient alternative for iterative checkpoint evaluation during image captioning model development, balancing performance with computational efficiency.
...
Method: Built a latent diffusion transformer architecture pretrained on 960,000 chest X-ray and report pairs, enabling text-, mask-, and bounding box-guided synthesis of radiographs.

Result: Achieved accurate radiograph synthesis validated by expert evaluations and quantitative metrics. Demonstrated utility for data augmentation and supervised pretraining, improving disease classification, detection, and segmentation with less training data. Enhanced model fairness through diverse patient cohort creation.

Conclusion: Generative foundation models like ChexGen can play a transformative role in building more accurate, data-efficient, and equitable medical AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Multiview video enables immersive 3D scene reconstruction but poses major storage and transmission challenges due to massive data volume. Existing deep learning-based video coding focuses mainly on single-view or stereo videos, leaving general multiview scenarios underexplored.

Method: Proposes feature-based inter-view motion vector prediction using decoded independent-view motion features, inter-view motion entropy model for motion priors, disparity-free inter-view context prediction from independent-view content features, and inter-view contextual entropy model for context priors.

Result: Outperforms the reference software of traditional MV-HEVC standard by a large margin, establishing a strong baseline for future research in learned multiview video coding.

Conclusion: The proposed LMVC framework effectively leverages inter-view correlations to enhance compression efficiency while maintaining important practical requirements like random access and backward compatibility, representing significant advancement in multiview video coding.
...
<details>
  <summary>Details</summary>
Motivation: Pneumothorax can be life-threatening if undetected, and small cases are often subtle on chest X-rays, requiring automated detection to support radiologists.
...
Result: Achieved IoU of 0.7008 and Dice score of 0.8241 on independent PTX-498 test dataset.

Conclusion: The model can accurately localize pneumothoraces and provide valuable support to radiologists in clinical diagnosis.
...
<details>
  <summary>Details</summary>
Motivation: Existing OOD detection methods lack understanding of OOD images and suffer from false negative labels that degrade near-OOD performance, requiring a more accurate negative space construction.

Method: Leverage MLLMs to identify OOD samples and generate expressive negative sentences for far-OOD detection, and create visually similar negative labels for near-OOD scenarios using reasoning capabilities, with an adaptive weighted score to balance both approaches.

Result: Significantly reduces FPR95 by 4.2% on ImageNet benchmark, establishing new state-of-the-art performance while being training-free and zero-shot for high scalability.

Conclusion: ANTS effectively addresses OOD detection challenges by creating adaptive negative textual spaces through MLLM understanding and reasoning, achieving superior performance without task-specific prior knowledge.
...
Method: Proposes MMChange with three key modules: Image Feature Refinement (IFR) to highlight key regions and suppress noise, vision language model for semantic text descriptions, Textual Difference Enhancement (TDE) to capture semantic shifts, and Image-Text Feature Fusion (ITFF) for cross-modal integration.
...
<details>
  <summary>Details</summary>
Motivation: To address WSI classification challenges by incorporating spatial relationships between instances and achieving efficient full instance correlations without complex Transformer architectures or custom CUDA kernels.
...
Conclusion: SAC-MIL provides an efficient, easy-to-deploy alternative to Transformer-based methods for WSI classification, handling spatial relationships and length extrapolation effectively while maintaining high performance.
...
<details>
  <summary>Details</summary>
Motivation: Prompt design for Large Multimodal Models in Multiple-Choice Question Answering remains poorly understood, with minor variations causing significant accuracy deviations, posing challenges for transparent and fair model evaluation.

Method: Introduce Promptception framework with 61 prompt types across 15 categories and 6 supercategories to evaluate prompt sensitivity. Test 10 LMMs (including GPT-4o and Gemini 1.5 Pro) across 3 MCQA benchmarks: MMStar, MMMU-Pro, MVBench.

Result: Proprietary models exhibit greater sensitivity to prompt phrasing, reflecting tighter alignment with instruction semantics. Open-source models are steadier but struggle with nuanced and complex phrasing.

Conclusion: Propose Prompting Principles tailored to proprietary and open-source LMMs to enable more robust and fair model evaluation, addressing the significant impact of prompt variations on LMM performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D semantic occupancy methods overlook height-axis information and use uniform channel attention across all height layers, limiting their ability to capture semantic variations along the vertical dimension.

Method: Proposes SliceSemOcc with vertical slice-based multimodal processing, extracting voxel features using global and local vertical slices, a global-local fusion module, and SEAttention3D module that preserves height-wise resolution with dynamic channel attention weights per height layer.

Result: Extensive experiments on nuScenes-SurroundOcc and nuScenes-OpenOccupancy datasets show significant mean IoU improvements, with particularly pronounced gains on small-object categories.

Conclusion: The proposed SliceSemOcc framework effectively addresses height-axis information processing limitations and demonstrates superior performance in 3D semantic occupancy prediction, especially for small objects.
...
Method: Proposed a novel detection method for spurious correlations in vision transformers, conducting large-scale experiments on ImageNet using both supervised and self-supervised trained models.

Result: Method successfully identified spurious correlations; training methodology significantly impacts model reliance on spurious signals; certain ImageNet classes contain easily detectable spurious patterns; provided exhaustive list of problematic images.

Conclusion: Spurious correlations are a critical issue requiring detection and mitigation; call for caution in using identified problematic images; real-world medical case study demonstrates practical relevance of the findings.
...
Method: Uses a Counting Network to estimate bag-level majority labels by counting instances per class, and a Majority Proportion Enhancement Module that removes minority instances to increase majority class proportion.
...
Conclusion: LML is a valuable problem formulation with practical applications, and the proposed method effectively solves it by leveraging majority class proportions and instance counting.
...
Method: Uses Phytium FT-2000/4 processors with MLU220 accelerator cards in multi-card parallel architecture, combined with lightweight YOLOv5s detection network and DeepSORT tracking algorithm for closed-loop detection-tracking-feedback control.

Result: Achieves stable single-frame processing delay of 50-100ms at 1920*1080 resolution with multi-scale target recognition accuracy over 98.5%, demonstrating both low latency and high precision.

Conclusion: Provides an innovative solution for UAV monitoring applications using domestic chips, successfully addressing real-time response challenges in video surveillance with high performance computing architecture.
...
<details>
  <summary>Details</summary>
Motivation: Previous re-ranking methods rely on single-view images which suffer from view bias, pose variation, viewpoint changes, and occlusions. Multi-view features can help reduce these issues and improve re-identification accuracy.

Method: K-nearest Weighted Fusion (KWF) method that aggregates neighbors' features in an unsupervised manner to generate multi-view features. The approach explores different weight selection strategies during feature aggregation and doesn't require model fine-tuning or extra annotations.

Result: Significant improvements in Rank@1 and mAP metrics: 9.8% improvement on MSMT17 and 22.0% improvement on Occluded-DukeMTMC. The method also demonstrates substantial computational efficiency enhancements compared to other re-ranking approaches.

Conclusion: The proposed KWF-based re-ranking method effectively addresses view bias issues in person re-identification by generating multi-view features from neighbors, achieving state-of-the-art performance with improved computational efficiency on challenging datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing AVVP methods either treat noisy segment-level labels as reliable supervision or spread indiscriminate attention across frames, causing initial errors to be amplified during training.
...
Conclusion: The proposed method effectively integrates strengths of previous research directions to locate and purify semantic cues while enabling precise semantic information dissemination, addressing error amplification issues in weakly supervised AVVP.
...
<details>
  <summary>Details</summary>
Motivation: Advanced Driver Assistance Systems require rapid processing and real-time response for safety in real-world environments, necessitating efficient perception models that can handle multiple tasks simultaneously with low computational demands.
...
Result: On BDD100k dataset, TriLiteNet_base achieved: 85.6% recall for vehicle detection, 92.4% mIoU for drivable area segmentation, and 82.3% accuracy for lane line segmentation. Both configurations showed low latency and reasonable power consumption on embedded devices.

Conclusion: TriLiteNet offers a practical and deployable solution for real-world autonomous driving applications by balancing performance, computational efficiency, and scalability, making it suitable for embedded systems with limited resources.
...
<details>
  <summary>Details</summary>
Motivation: Event cameras offer low latency, high dynamic range, and motion robustness, making them ideal for pedestrian safety applications. However, there's a lack of comprehensive datasets for event-based pedestrian intention analysis, especially under varied weather conditions.

Method: Created dataset with two sources: (1) synthetic event streams from CARLA simulator with controlled approach-cross scenes under varied weather/lighting, and (2) real-world JAAD dash-cam videos converted to event streams using v2e tool. Includes paired RGB frames, DVS event frames (33ms accumulations), and frame-level crossing labels.

Result: Provides baseline spiking neural networks (SNNs) using SpikingJelly demonstrating dataset usability. Reveals a sim-to-real gap, highlighting the need for domain adaptation and multimodal fusion approaches.

Conclusion: DVS-PedX aims to accelerate research in event-based pedestrian safety, intention prediction, and neuromorphic perception by providing a comprehensive dataset that bridges synthetic and real-world event data for pedestrian crossing analysis.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with character consistency and dialogue rendering, leading to artifacts and disjointed storytelling in multi-character story visualization.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models struggle with complex multi-element prompts and limited stylistic diversity, requiring a more sophisticated approach to handle spatial relationships and diverse styles.

Method: Two-component framework: 1) Position-Style-Aware module using fine-tuned LLM for prompt decomposition, 2) Multi-Expert Diffusion module with dynamic expert routing and attention-based gating for cross-region generation using specialized models.

Result: MEPG significantly outperforms baseline models with the same backbone in both image quality and style diversity.

Conclusion: The proposed framework successfully addresses limitations of current text-to-image models by integrating LLM-based planning with specialized expert modules, providing strong extensibility and real-time editing capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing deepfake detectors perform poorly (61-69% accuracy) on real-world benchmarks compared to commercial solutions (82% accuracy), highlighting the need for more accessible and effective open-source detectors.

Method: Revisited Ojha et al.'s approach of adapting standard pretrained vision backbones for deepfake detection, but with improved hyperparameter tuning rather than complex architectural changes.

Result: Achieved 81% accuracy on Deepfake-Eval-2024 benchmark, an 18% improvement over previously reported baseline results, making it competitive with leading commercial detectors.

Conclusion: Simple vision backbone approaches with proper hyperparameter tuning can achieve state-of-the-art deepfake detection performance, offering practical deployment options with good accuracy-computational cost tradeoffs.
...
<details>
  <summary>Details</summary>
Motivation: Traditional image annotation tools require extensive manual input, limiting scalability for large datasets. The labor-intensive nature of annotation from simple classification to complex tasks like object detection and segmentation needs automation.

Method: Integrates foundation models including CLIP with pre-trained detectors (Ultralytics models) for common classes and zero-shot models (Grounding DINO) for custom labels. Uses low-confidence thresholding for initial annotations, interactive refinement tools, on-the-fly segmentation with Segment Anything accelerated via WebGPU, and employs clustering and IoU-graph for redundant detection suppression.

Result: Demonstrates up to 90% reduction in manual effort while maintaining high annotation accuracy. Initial predictions on COCO-type classes are mostly correct, and the tool supports multiple export formats (YOLO, COCO, Pascal VOC, CSV) with offline operation after model caching.

Conclusion: VisioFirm provides an effective hybrid approach that significantly reduces human annotation effort through AI-assisted automation while ensuring high-quality annotations, making it a scalable solution for large dataset labeling tasks.
...
Method: Leverages text-to-image generative model to disentangle object features from domain styles, then uses progressive mutual neighbor alignment to reliably align disentangled features across domains.

Result: Achieves state-of-the-art performance across three benchmark datasets spanning 13 domains.

Conclusion: Feature disentanglement combined with progressive cross-domain alignment effectively addresses the domain gap problem in unsupervised cross-domain image retrieval.
...
Method: Integrates Monte Carlo quality estimation with IoU-based region quality evaluation to jointly optimize accuracy and diversity in preference data construction, enabling adaptive reasoning steps based on task complexity.
...
<details>
  <summary>Details</summary>
Motivation: Existing segmentation networks are optimized for ground-perspective photos and don't handle remote sensing challenges like extreme scale variation, foreground-background imbalance, and large image sizes in overhead imagery.
...
Method: A 3D diffusion model that uses textual prompts from plasma p-tau217 measurements and anatomical constraints from structural MRI to synthesize realistic 3D tau PET images.

Result: The approach successfully generates clinically meaningful 3D tau PET images across various disease stages using ADNI data, demonstrating realistic synthesis capabilities.
...
Method: Integrates explicit volume prior at image scale and Threshold Dynamics spatial regularization into backbone segmentation network. Uses regression network to estimate target region volumes for unlabeled images, enforced by image-scale Wasserstein distance constraint. Also employs dataset-scale Wasserstein distance loss based on weak implicit volume prior to match volume distributions between labeled and unlabeled datasets.
...
Conclusion: The proposed framework effectively leverages both spatial regularization and volume prior information at multiple scales, significantly improving semi-supervised medical image segmentation performance across diverse medical imaging datasets.
...
<details>
  <summary>Details</summary>
Motivation: Prior methods require dense multi-view observations and ground-truth camera poses, which limits practical applications. This work aims to operate with minimal views and no camera supervision.

Method: 1) Independent reconstruction of each articulation using sparse-view 3D reconstruction, 2) Learning deformation field for dense correspondences across poses, 3) Progressive disentanglement to separate static from moving parts, 4) Joint optimization with self-supervised loss enforcing cross-view and cross-pose consistency.

Result: Produces accurate and detailed articulated object representations under significantly weaker input assumptions than existing approaches, as demonstrated on standard benchmarks and real-world examples.

Conclusion: The method successfully learns articulated object representations from sparse, unposed images without camera supervision, overcoming limitations of previous approaches that required dense views and pose information.
...
Method: Leverages synthetic images generated by text-to-image models as reliable reference points to identify and correct mislabeled samples, rather than using them directly for training.

Result: Improves classification accuracy by 30% on CIFAR-10, 11% on CIFAR-100 under 70% semantic noise, and 24% on ImageNet-100 under real-world noise conditions.
...
TL;DR: A DINO-based model for odd-one-out anomaly detection that reduces parameters by 33% and training time by 3x while maintaining competitive performance, with insights on MLLM limitations in visual reasoning.
...
Result: The model maintains competitive performance while achieving substantial efficiency gains. Experimental evaluation also revealed limitations of Multimodal Large Language Models in structured visual reasoning tasks.

Conclusion: The proposed efficient DINO-based approach successfully addresses the computational challenges of odd-one-out anomaly detection while providing insights into current MLLM capabilities for visual reasoning tasks.
...
Conclusion: GeoArena provides a more accurate, privacy-conscious, and human-centered benchmarking approach for evaluating LVLMs on worldwide image geolocalization.
...
TL;DR: FE2E framework adapts image editing models (not text-to-image generators) for dense geometry estimation, achieving superior performance through structural priors, consistent velocity training, and joint depth-normal estimation.


<details>
  <summary>Details</summary>
Motivation: Dense prediction is inherently an image-to-image task, suggesting image editing models may be better foundation models than text-to-image generative models for fine-tuning in geometry estimation tasks.

Method: FE2E adapts Diffusion Transformer (DiT) editing model with consistent velocity training objective, logarithmic quantization for precision, and leverages global attention for joint depth and normal estimation in single forward pass.
...
Conclusion: Image editing models possess superior structural priors for dense geometry tasks compared to generative models, and the FE2E framework demonstrates their potential through innovative adaptation techniques.
...
TL;DR: MICACL is a novel multi-instance learning framework for dynamic facial expression recognition that addresses long-tailed distributions and spatio-temporal complexity through graph-enhanced instance interaction, weighted aggregation, and multiscale contrastive learning.
...
Method: Proposes MICACL framework with: 1) Graph-Enhanced Instance Interaction Module (GEIIM) using adaptive adjacency matrices and multiscale convolutions, 2) Weighted Instance Aggregation Network (WIAN) for dynamic importance weighting, and 3) Multiscale Category-aware Contrastive Learning (MCCL) to balance major/minor category training.
...
Conclusion: MICACL effectively addresses long-tailed distribution challenges and spatio-temporal complexity in DFER, achieving significant performance improvements through integrated instance interaction, weighted aggregation, and balanced contrastive learning.
...
Method: Uses monocular SLAM to estimate camera trajectories and reconstruct environment layout. Identifies key viewpoints through camera pose clustering, selects representative frames, and fuses them into panoramic images using multi-frame stitching techniques.

Result: The system generates spatially coherent panoramic images that summarize incident scenes, enabling rapid understanding of complex environments.

Conclusion: This approach provides efficient visual summaries that support quick interpretation of incident scenes, facilitating better decision-making and incident review for first responders.
...
<details>
  <summary>Details</summary>
Motivation: Text-based person anomaly search is critical for public safety but faces challenges in fine-grained cross-modal alignment and anomaly recognition with sparse real-world samples. LMMs show potential but are hindered by domain gaps and lack of efficient adaptation strategies.
...
TL;DR: ASE-MLLM is a novel framework that integrates aesthetic saliency into multimodal large language models for improved aesthetic image captioning, outperforming existing methods and achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: Existing aesthetic image captioning methods using MLLMs don't specifically adapt models to focus on aesthetic content and primarily rely on fine-tuning without explicit aesthetic saliency integration.

Method: Proposed ASE-MLLM framework with Image Aesthetic Saliency Module (IASM) to extract aesthetic features and IAS-ViT encoder that fuses aesthetic saliency with original image features via cross-attention mechanism.
...
Conclusion: ASE-MLLM is the first framework to successfully integrate image aesthetic saliency into MLLMs specifically for aesthetic image captioning tasks, demonstrating superior performance.
...
TL;DR: A novel 3D style transfer pipeline that uses diffusion priors to generate stylized key views with cross-view attention and instance-level consistency, achieving superior results compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing 3D style transfer methods struggle with extracting high-level style semantics and maintaining structural clarity, making it difficult to distinguish different objects within 3D scenes.
...
Result: The method significantly outperforms state-of-the-art approaches across various scenes (forward-facing to 360-degree environments), producing more structured, coherent, and artistically enriched stylizations.

Conclusion: The proposed pipeline effectively integrates 2D diffusion model knowledge into 3D style transfer, addressing limitations of existing methods through cross-view style alignment and instance-level consistency techniques.
...
TL;DR: PtyINR is a self-supervised neural framework that simultaneously reconstructs both object and probe in X-ray ptychography without requiring pre-characterized probes, achieving superior quality and robustness in low-signal conditions.


<details>
  <summary>Details</summary>
Motivation: X-ray ptychography's nanoscale resolution is limited by inaccurate image reconstruction when the illuminating probe is unknown, especially under low-signal conditions in low-dose and high-speed experiments.
...
Result: Generated RMS dataset with 35k image-text pairs and demonstrated effectiveness through extensive experiments on various tasks, confirming scalability of the image-oriented approach.
...
Conclusion: The method significantly outperforms existing CM distillation approaches and enables the TRELLIS framework to achieve superior performance in few-step 3D generation.
...
TL;DR: Durian is a zero-shot method for portrait animation with facial attribute transfer using dual reference networks and diffusion models, achieving state-of-the-art performance without explicit triplet supervision.


<details>
  <summary>Details</summary>
Motivation: To enable high-fidelity and spatially consistent facial attribute transfer from reference images to target portraits in portrait animation videos without requiring paired training data.
...
<details>
  <summary>Details</summary>
Motivation: Collimation reduces radiation dose by restricting exposure to ROI, but detecting collimator shadows is challenging when edges are obscured by scattered radiation. Prior knowledge that collimation forms polygonal shadows enables geometry-constrained solutions.

Method: Differentiable Hough transform-based network that detects collimation borders and extracts ROI center information. Combines both tasks during inference to generate refined, line-constrained segmentation masks.
...
Conclusion: The method effectively handles collimation shadow detection despite obscured edges and is not fundamentally limited by the number of edges, though typically involves at most four shadow borders in this application.
...
TL;DR: UCF-UM introduces a cyclic evaluation framework to measure semantic drift in unified visual language models when alternating between image-to-text and text-to-image generation, revealing significant variations in cross-modal consistency that single-pass metrics miss.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations for unified visual language models assess image-to-text and text-to-image capabilities in isolation, failing to reveal whether models maintain semantic consistency when cycling between modalities or if they can render concepts they understand.

Method: UCF-UM uses a cyclic evaluation protocol that alternates I2T and T2I over multiple generations, with three metrics: Mean Cumulative Drift (embedding-based semantic loss), Semantic Drift Rate (decay rate), and Multi-Generation GenEval (object-level compliance). A new benchmark ND400 is created from NoCaps and DOCCI to assess generalization beyond COCO.
...
<details>
  <summary>Details</summary>
Motivation: Growing demand for spatial intelligence in applications like VR, autonomous driving, and robotics requires addressing the unique challenges of omnidirectional images (ODIs) that differ significantly from perspective images in geometric projection, spatial distribution, and boundary continuity.

Method: Systematic review of panoramic imaging pipeline and projection methods, analysis of three main domain adaptation challenges (geometric distortions, non-uniform sampling, boundary continuity), and classification of 20+ tasks into four categories with cross-method and cross-task analysis.

Result: Comprehensive survey covering 300+ research papers, identifying representative strategies for addressing panoramic-specific challenges, and categorizing panoramic vision into four major areas: visual quality enhancement/assessment, visual understanding, multimodal understanding, and visual generation.

Conclusion: The survey provides insights into panoramic vision technologies, discusses open challenges in data, models, and applications, and offers forward-looking perspectives to advance the development of spatial intelligence through omnidirectional imaging.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models lack flexibility for applying fine or coarse edits while maintaining visual and narrative consistency across multiple frames in story visualization.
...
Result: The framework addresses the challenge of maintaining consistency while allowing both fine and coarse edits in story visualization.
...
TL;DR: VFR is a novel video generation model for virtual try-on that produces arbitrarily long videos through segment-by-segment generation using prefix video conditions and anchor videos for smoothness and consistency.
...
Method: Auto-regressive segment-by-segment generation process using prefix video conditions for local smoothness between segments and anchor videos (360-degree wholebody capture) for global temporal consistency across segments.

Result: Successfully generates minute-scale virtual try-on videos with both local smoothness and global temporal consistency under various motion scenarios.

Conclusion: VFR represents a pioneering approach in long virtual try-on video generation, effectively addressing the dual challenges of segment smoothness and temporal consistency through its innovative framework.
...
TL;DR: The paper proposes PG-Agent, a GUI agent framework that transforms sequential operation episodes into page graphs to capture complex transition relationships, uses RAG to retrieve GUI perception guidelines, and employs a multi-agent framework with task decomposition for better generalization to unseen scenarios.
...
Conclusion: The proposed PG-Agent framework successfully addresses the limitations of existing GUI agents by leveraging page graphs and RAG technology, enabling better generalization to unseen GUI scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of computing probability bounds in causal models where exogenous variables are not fully specified, leading to partial identifiability issues that prevent precise probability computation.

Method: Develops a column generation approach that constructs a sequence of auxiliary linear integer programs to compute probability bounds, exploiting input probabilities over endogenous variables and showing polynomial cardinality representation for exogenous variables.

Result: The column generation technique proves superior to existing methods in experiments, providing efficient computation of tight probability bounds for scenarios with single interventions.

Conclusion: The proposed algorithm effectively simplifies multilinear programming problems in quasi-Markovian causal models, offering practical improvements for computing probability bounds when dealing with partially identifiable queries and unspecified exogenous variable distributions.
...
TL;DR: Proposes Diffusion-AC, a novel autonomous conflict resolution framework that uses diffusion probabilistic models to overcome unimodal bias in DRL policies, enabling multimodal decision-making for safer air traffic management.
...
Method: Integrates diffusion probabilistic models into CD&R, modeling policy as a reverse denoising process guided by a value function to generate multimodal action distributions. Uses Density-Progressive Safety Curriculum for stable training from sparse to high-density traffic environments.

Result: Significantly outperforms state-of-the-art DRL benchmarks, achieving 94.1% success rate in high-density scenarios and reducing Near Mid-Air Collisions by 59% compared to the next-best baseline.

Conclusion: Diffusion-AC's multimodal decision-making capability provides flexible alternative maneuvers, significantly enhancing safety margins in air traffic conflict resolution and overcoming limitations of traditional unimodal approaches.
...
TL;DR: Training LLM agents to dynamically decide when to plan during test-time improves efficiency and performance on long-horizon tasks compared to always-planning or never-planning approaches.


<details>
  <summary>Details</summary>
Motivation: Existing methods like ReAct require LLMs to plan before every action, which is computationally expensive and degrades performance on long-horizon tasks, while never planning limits performance.

Method: A two-stage training pipeline: (1) supervised fine-tuning on synthetic data to prime models for dynamic planning, and (2) reinforcement learning to refine this capability in long-horizon environments.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of AI reasoning adaptation to novel problems with limited data and distribution shifts, building on the Abstraction and Reasoning Corpus framework.

Method: Creates reasoning tasks sampled from structural causal models, providing principled data augmentations including observational, interventional, and counterfactual feedback as few-shot demonstrations for in-context learning.

Result: The paper demonstrates CausalARC's utility across four evaluation settings: abstract reasoning with test-time training, counterfactual reasoning with in-context learning, program synthesis, and causal discovery with logical reasoning.

Conclusion: CausalARC provides a formal framework for evaluating AI reasoning capabilities in causal understanding and adaptation to novel problem settings under data constraints and distribution shifts.
...
<details>
  <summary>Details</summary>
Motivation: LLMs lack robust mental representations for logical reasoning and struggle with human-like comprehension, needing grounding in embodied cognitive structures.

Method: Combine LLMs with declarative spatial reasoning using Answer Set Programming to formalize image schemas (sensorimotor-derived cognitive patterns) as executable programs.

Result: LLMs can be guided to interpret scenarios through embodied cognitive structures, formalized as executable programs that support effective logical reasoning with enhanced interpretability.

Conclusion: The system establishes a computational foundation for incorporating more complex dynamic representations, demonstrating that grounding in spatial primitives improves LLM reasoning capabilities.
...
TL;DR: RL enhances LLM reasoning through emergent hierarchical structure with strategic planning and procedural execution phases. HICRA algorithm focuses optimization on high-impact planning tokens, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: To understand the underlying mechanisms behind RL's success in improving LLM reasoning abilities and address inefficiencies in current RL algorithms that apply optimization pressure indiscriminately.

Method: Analysis of RL dynamics reveals two-phase learning process, leading to development of HIerarchy-Aware Credit Assignment (HICRA) algorithm that concentrates optimization on planning tokens and uses semantic entropy for strategic exploration measurement.

Result: HICRA significantly outperforms strong baselines like GRPO, demonstrating that focusing on strategic bottlenecks is key to unlocking advanced reasoning capabilities in LLMs.

Conclusion: RL success in LLM reasoning stems from emergent hierarchical structure, and targeted optimization on high-level planning tokens through approaches like HICRA is more effective than agnostic optimization across all tokens.
...
TL;DR: Equal-length segmentation outperforms complex time series segmentation methods for SHAP-based time series explanations, with segment count being more important than segmentation method. A novel length-weighted normalization further improves attribution quality.


<details>
  <summary>Details</summary>
Motivation: SHAP's computational complexity limits its practicality for long time series. While feature aggregation via segmentation reduces computation time, the optimal segmentation strategy remains unknown.

Method: Investigated 8 time series segmentation algorithms using two XAI evaluation methodologies (InterpretTime and AUC Difference) on multivariate and univariate time series data.

Result: Number of segments has greater impact than specific segmentation method. Equal-length segmentation consistently outperforms most custom algorithms. Novel length-weighted normalization improves attribution quality.
...
<details>
  <summary>Details</summary>
Motivation: Current automated red-teaming approaches don't consider how human identities and backgrounds shape red-teaming strategies, limiting the diversity of risks uncovered in AI safety testing.

Method: Developed PersonaTeaming with persona-based prompt mutation using both expert and regular user personas, plus a dynamic persona-generating algorithm and new mutation distance metrics.
...
Conclusion: Persona mutation significantly enhances automated red-teaming effectiveness, revealing opportunities for complementarity between automated and human red-teaming approaches.
...
<details>
  <summary>Details</summary>
Motivation: To systematically characterize LLM personality across training stages, validate self-reported traits against behavior, and test interventions like persona injection.
...
Conclusion: Surface-level trait expression differs from behavioral consistency, challenging assumptions about LLM personality and highlighting the need for deeper evaluation in alignment and interpretability.
...
TL;DR: LLMs show significant internal inconsistencies in behavior across different experimental settings, failing to maintain consistency between their revealed internal states and dialogue behaviors, which limits their ability to substitute for real human participants in research.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether synthetic agents (LLMs) can serve as valid substitutes for real human participants in research by examining their internal consistency across different experimental settings.

Method: Developed a study to (a) reveal the agent's internal state and (b) examine agent behavior in basic dialogue settings, testing behavioral hypotheses to assess consistency between internal state and conversation behavior.

Result: Found significant internal inconsistencies in LLMs across model families and sizes. While agents may generate responses matching human counterparts, they fail to maintain internal consistency.

Conclusion: LLMs' internal inconsistency represents a critical gap in their capability to accurately substitute for real participants in human-subject research, despite their ability to generate human-like responses.
...
<details>
  <summary>Details</summary>
Motivation: Conventional LLMs often fail in highly specialized or unexpected scenarios in offshore wind maintenance, where accuracy and safety are paramount.

Method: RAGuard uses parallel queries to two indices (knowledge and safety) with separate retrieval budgets, plus SafetyClamp extension that fetches larger candidate pool with hard-clamping exact slot guarantees to safety.
...
Result: Successfully deployed in JD.com's real-world scenario, effectively reducing labor and improving accuracy, stock availability, and other key supply chain metrics.
...
Method: Introduced Meta-Policy Deliberation Framework (MPDF) with decentralized policy over meta-cognitive actions. Developed SoftRankPO algorithm using rank-based advantage shaping with smooth normal quantiles to stabilize reinforcement learning in this setting.
...
Conclusion: Presents a paradigm shift from designing fixed protocols to learning dynamic, deliberative strategies for multi-agent LLM systems, enabling adaptive meta-cognitive policies that improve complex reasoning performance.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate whether LLMs can align with domain experts to inform social policymaking for homelessness alleviation, a global challenge affecting over 150 million people.

Method: Developed a benchmark with decision scenarios across four geographies, grounded in Capability Approach framework, and created an automated pipeline connecting policies to agent-based models for social impact simulation.
...
Conclusion: With proper safeguards and expert collaboration, LLMs can provide valuable scalable policy insights for complex social challenges like homelessness.
...
<details>
  <summary>Details</summary>
Motivation: OMOP CDM mapping of source medical terms to standard concepts is resource-intensive and error-prone, and LLMs tend to hallucinate making them unsuitable for clinical use without training/validation.

Method: Developed a system based on Model Context Protocol (MCP) framework that allows LLMs to interact with external resources and tools for zero-training, explainable mapping with real-time vocabulary lookups.

Result: The system significantly improves efficiency and accuracy with minimal effort, providing structured reasoning outputs suitable for immediate use in both exploratory and production environments.

Conclusion: The MCP-based system enables reliable, explainable medical term mapping without training requirements, making it suitable for clinical deployment while preventing LLM hallucinations.
...
Method: Multidimensional framework using multimodal social media data: semantic segmentation for visual focus areas, color clustering for aesthetic preferences, hybrid sentiment analysis (rule-based + multi-task BERT) for reviews across four dimensions.
...
Conclusion: Provides integrated, data-driven approach to decode tourist perception, contributing to informed decision-making in tourism, heritage conservation, and public space design.
...
<details>
  <summary>Details</summary>
Motivation: Address reliability concerns in Generative AI models (hallucinations, semantic drift, biases) and overcome limitations of current subjective human evaluation methods that lack scalability and transparency.
...
TL;DR: E&E is a hybrid exploration strategy that combines local novelty search with goal-directed expeditions using vision-language models to discover diverse patterns in continuous cellular automata.


<details>
  <summary>Details</summary>
Motivation: Traditional novelty search methods plateau when local novelty is exhausted and fail to reach distant unexplored regions in high-dimensional behavioral spaces.

Method: Alternates between local novelty-driven expansions and goal-directed expeditions using a Vision-Language Model to generate linguistic goals describing hypothetical patterns.
...
Conclusion: Agentic conversational paradigm provides a lightweight, accessible alternative to traditional app interfaces, enabling more efficient marketplace management.
...
TL;DR: DeepMedix-R1 is a medical foundation model for chest X-ray interpretation that provides transparent reasoning with local region grounding, achieving superior performance in report generation and visual question answering compared to existing models.


<details>
  <summary>Details</summary>
Motivation: Current medical foundation models operate as black-box systems without transparent reasoning processes, lacking interpretability which hinders clinical deployment. There's a need for models that provide both answers and explainable reasoning tied to specific image regions.
...
Result: Significant improvements: 14.54% and 31.32% better report generation than LLaVA-Rad and MedGemma; 57.75% and 23.06% better visual question answering than MedGemma and CheXagent. Expert review showed strong preference (0.7416 vs 0.2584) over Qwen2.5-VL-7B for interpretability and clinical plausibility.

Conclusion: DeepMedix-R1 advances medical foundation models toward holistic, transparent, and clinically actionable modeling for CXR interpretation by providing both answers and explainable reasoning tied to local image regions.
...
TL;DR: A new best-first heuristic search algorithm that explicitly treats control parameters as decision points rather than constraints, using delayed partial expansion to efficiently handle infinite decision spaces in automated planning.
...
Result: The algorithm proves a notion of completeness in the limit under certain conditions and demonstrates competitive performance compared to existing approaches for planning problems with control parameters.
...
TL;DR: WorMI framework combines LLMs with domain-specific world models for test-time composition, enabling embodied agents to adapt to novel domains without retraining through prototype-based retrieval and compound attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of enabling embodied AI agents to robustly adapt to novel domains without requiring extensive data collection or retraining, improving cross-domain adaptability and data efficiency.

Method: World model implanting framework with prototype-based world model retrieval using trajectory-based abstract representation matching, and world-wise compound attention to integrate knowledge and align representations.
...
TL;DR: MPR is a hybrid framework that consolidates LLM-generated reflections into structured Meta-Policy Memory, improving agent performance without weight updates through soft memory-guided decoding and hard rule checks.


<details>
  <summary>Details</summary>
Motivation: LLM agents show repeated failures, inefficient exploration, and limited cross-task adaptability. Existing reflective strategies produce ephemeral task-specific traces, while RL alternatives require substantial compute and parameter updates.

Method: MPR creates predicate-like Meta-Policy Memory from LLM reflections, applying it through soft memory-guided decoding and hard rule admissibility checks (HAC) to enforce constraints and improve decision-making.
...
Conclusion: MPR externalizes reusable corrective knowledge without model updates, enforces domain constraints, and retains language-based reflection adaptability, showing promise for scalable agent improvement.
...
TL;DR: AutoPBO is an LLM-powered framework that automatically enhances PBO local search solvers, achieving significant performance improvements over previous approaches while maintaining competitiveness with state-of-the-art solvers.


<details>
  <summary>Details</summary>
Motivation: Local search solvers for Pseudo-Boolean Optimization require significant expert effort and manual tuning. LLMs have shown potential in automating algorithm design but haven't been applied to optimizing PBO solvers.
...
Conclusion: AutoPBO offers a promising approach to automating local search solver design, showing that LLMs can effectively optimize PBO solvers without extensive manual tuning.
...
TL;DR: CoT-Space reframes LLM reasoning as continuous optimization in semantic space, explaining optimal Chain-of-Thought length through underfitting-overfitting tradeoff.


<details>
  <summary>Details</summary>
Motivation: Traditional token-level RL frameworks don't align with multi-step reasoning processes like Chain-of-Thought, creating a theoretical gap.

Method: Introduces CoT-Space framework that transforms reasoning from discrete token prediction to continuous optimization in semantic space, analyzed from noise and risk perspectives.

Result: Demonstrates convergence to optimal CoT length as natural consequence of underfitting-overfitting tradeoff, with experimental validation.
...
<details>
  <summary>Details</summary>
Motivation: To harness human-like flexible representation capabilities (drawing diagrams, changing representations, creative analogies) and endow machines with similar power to improve human-machine compatibility.
...
Result: Created a functional implementation (Oruga) that can execute representation transformations through structure transfer, demonstrated with brief examples.

Conclusion: Oruga successfully implements key aspects of RST and provides a foundation for machines to perform flexible representation transformations similar to human cognitive processes.
...
Method: The study compares four formal languages across three datasets and seven different LLMs to evaluate how formal language choice affects both syntactic and semantic reasoning capabilities.

Result: The research demonstrates that the choice of formal language has substantial effects on reasoning performance, with varying impacts across different LLM architectures and capabilities.

Conclusion: The intermediate language challenge - selecting appropriate formal languages - is a critical factor in neurosymbolic reasoning that significantly influences both syntactic and semantic performance across diverse LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Emergency situations require fast flight path re-calculation for airliners, where conventional solvers may be too slow for real-time route optimization.

Method: Train RL agent to pre-compute near-optimal paths using location and atmospheric data, then use these as constraints to reduce search space for underlying path planning solver.

Result: Empirical results with Airbus performance models show fuel consumption nearly identical to unconstrained solver (within 1% deviation) while computation speed improves by up to 50%.

Conclusion: Combining RL with search-based planners effectively accelerates emergency route optimization without significant compromise on fuel efficiency, though global optimality is not guaranteed.
...
<details>
  <summary>Details</summary>
Motivation: Most computer poker research focuses on win rates while overlooking bluffing behavior, which is essential in human poker play. The study aims to examine whether popular algorithms DQN and CFR exhibit bluffing.
...
Result: Both DQN and CFR exhibited bluffing behavior but in different ways with different bluffing rates. However, the percentage of successful bluffs (where opponent folds) was roughly the same for both algorithms.

Conclusion: Bluffing appears to be an essential aspect of poker gameplay rather than being algorithm-specific. Future work should explore different bluffing styles and examine bluffing in full poker games.
...
TL;DR: AI may surpass human capabilities but cannot match human emotional experience and ethical understanding due to lacking a biological central nervous system, making humans uniquely qualified for universal leadership.


<details>
  <summary>Details</summary>
Motivation: To challenge the assumption that AI superiority in cognitive tasks would make it a rightful successor to human leadership by highlighting the irreplaceable role of biological emotional experience in ethical development.

Method: Philosophical analysis comparing human biological systems (central nervous system) with AI capabilities, focusing on emotional experience and ethical understanding as differentiators.

Result: Identifies that while AI may exceed human cognitive abilities, the inability to replicate biological emotional experience through a CNS prevents AI from developing true ethical understanding required for leadership.

Conclusion: Human biological systems (DNA-based) provide the necessary foundation for ethical leadership that AI (silicon-based) cannot replicate, ensuring humans remain best qualified to lead despite AI's technical superiority.
...
TL;DR: A domain-specific language for representing recipes as directed action graphs that captures cooking processes, transfers, environments, concurrency, and compositional structure to enable precise modeling and future automation of culinary workflows.


<details>
  <summary>Details</summary>
Motivation: Formalizing cooking procedures is challenging due to their inherent complexity and ambiguity. Current recipe representations lack the precision needed for machine understanding and automation in both home and professional kitchen settings.
...
Result: Initial manual evaluation on a full English breakfast recipe demonstrated the DSL's expressiveness and suitability for automated recipe analysis and execution. The approach enables structured machine understanding and precise interpretation of cooking processes.

Conclusion: This work represents initial steps towards an action-centric ontology for cooking using temporal graphs, enabling scalable automation of culinary processes in both home and professional settings through structured machine understanding.
...
TL;DR: Analysis of Markov logic networks' asymptotic behavior as domain size increases, focusing on three specific cases: quantifier-free MLNs with unary relations, triangle/clique-minimizing MLNs, and degree-constrained MLNs. Shows varied limit behaviors and incomparability with lifted Bayesian networks.
...
Result: Different MLN types exhibit distinct limit behaviors - weights may or may not influence asymptotic behavior. Quantifier-free MLNs and lifted Bayesian networks are asymptotically incomparable. MLN distributions concentrate probability mass differently than uniform distributions on large domains.

Conclusion: MLNs demonstrate varied asymptotic behaviors depending on constraint types, showing that soft constraints significantly impact limit distributions. The formalism captures complex probabilistic patterns that differ fundamentally from uniform distributions and other probabilistic graphical models.
...
<details>
  <summary>Details</summary>
Motivation: To provide game developers with a systematic approach to evaluate and prioritize quality aspects when co-creating game narratives with generative AI.
...
Result: Develops a methodology that can inform game developers on prioritizing quality aspects in AI-generated game narratives.

Conclusion: The proposed structured evaluation approach helps developers understand the impact of different narrative quality dimensions on player satisfaction when using generative AI for game narrative creation.
...
TL;DR: This paper analyzes AlphaZero deployment in changed test environments and proposes simple modifications to improve performance with low planning budgets.


<details>
  <summary>Details</summary>
Motivation: AlphaZero assumes static environments from training to test, limiting its applicability when test environments change. The authors want to address this constraint.

Method: The authors combine simple modifications to the standard AlphaZero framework to handle environment changes at test time.

Result: The proposed modifications significantly boost performance even with limited planning budgets available in changed environments.

Conclusion: Simple framework modifications can effectively extend AlphaZero's applicability to dynamic test environments while maintaining performance with constrained planning resources.
...
TL;DR: MBTI-in-Thoughts framework uses MBTI personality conditioning via prompt engineering to enhance LLM agent behavior without fine-tuning, showing consistent personality-driven performance across tasks and enabling structured multi-agent interactions.


<details>
  <summary>Details</summary>
Motivation: To bridge psychological theory and LLM behavior design by creating psychologically grounded AI agents that exhibit consistent, interpretable personality-driven behaviors across diverse tasks.

Method: Priming LLM agents with distinct MBTI personality archetypes through prompt engineering, controlling behavior along cognition and affect axes, and using the official 16Personalities test for automated trait verification.
...
Conclusion: The framework establishes a foundation for psychologically enhanced AI agents without fine-tuning, generalizes to other psychological frameworks (Big Five, HEXACO, Enneagram), and enables structured multi-agent communication with improved interaction quality.
...
TL;DR: The paper proposes concept-level memory for LLMs, where reusable abstractions from reasoning traces are stored and retrieved to enable test-time continual learning without weight updates, achieving 7.5% relative gain on ARC-AGI benchmark.
...
Method: Develop concept-level memory with strategies for abstracting takeaways from solution rollouts and retrieving relevant entries for new queries. Concepts are modular natural language abstractions that enable test-time continual learning without weight updates.

Result: 7.5% relative gain over strong no-memory baseline on ARC-AGI benchmark, with performance scaling with inference compute. Abstract concepts outperformed other memory designs at all tested compute scales.

Conclusion: Concept-level memory enables effective test-time continual learning, with dynamic memory updates outperforming fixed memory settings. The approach demonstrates self-improvement through solving more problems and abstracting patterns to memory.
...
TL;DR: Large language model adaptation strategies for dementia detection from speech, showing that proper demonstration selection, reasoning augmentation, and fine-tuning methods can achieve performance matching commercial systems.
...
Method: Evaluated nine text-only and three multimodal audio-text models on DementiaBank speech corpus using various adaptations: in-context learning with different demonstration policies, reasoning-augmented prompting, parameter-efficient fine-tuning, and multimodal integration.
...
TL;DR: RBA framework uses self-synthesis data from teacher LLM and reinforcement learning to improve speech-based LLMs' instruction-following capabilities, outperforming traditional methods and achieving SOTA results on benchmarks.
...
Method: Reinforced Behavior Alignment (RBA) framework that employs self-synthesis methodology to generate alignment data from teacher LLM, then uses reinforcement learning to align SpeechLMs with teacher behavior.
...
Conclusion: RBA framework successfully bridges performance gap for SpeechLMs using self-generated data and reinforcement learning, demonstrating strong generalization across speech tasks.
...
<details>
  <summary>Details</summary>
Motivation: To address cryptocurrency news analysis challenges, particularly large language model hallucinations, by creating a structured multilevel analytical approach that combines graph and text representations for comprehensive insights.

Method: Uses a fine-tuned Mistral 7B large language model with 4-bit quantization via PEFT/LoRA approach and retrieval-augmented generation (RAG). Performs hierarchical analysis: Level 1 generates graph/text summaries with sentiment scores and JSON representations; higher levels consolidate these into comprehensive reports.

Result: The approach successfully eliminates LLM hallucination problems by representing cryptocurrency news as knowledge graphs. The model conducts both informative qualitative and quantitative analytics, providing important insights into cryptocurrency news.

Conclusion: Fine-tuned Mistral 7B LLM with RAG is effective for multilevel cryptocurrency news analysis, offering complementary graph and text views that prevent hallucinations and deliver comprehensive analytical reports.
...
TL;DR: ProLiFIC is a comprehensive event log of Italian lawmaking process (1987-2022) created using LLMs from unstructured data, addressing dataset limitations for process mining in legal systems.


<details>
  <summary>Details</summary>
Motivation: Process mining's application to legal systems is limited by poor dataset accessibility and quality, requiring better data resources for effective analysis.

Method: Created ProLiFIC event log from unstructured Normattiva portal data using large language models (LLMs) to structure the Italian lawmaking process data.

Result: Developed a comprehensive benchmark dataset covering 35 years of Italian legislative procedures, enabling preliminary process mining analyses.
...
TL;DR: A multi-modal framework for earnings calls that creates hierarchical discourse tree embeddings with text, audio, and video emotional signals, using a two-stage transformer architecture for semantic representation.


<details>
  <summary>Details</summary>
Motivation: Existing financial sentiment analysis systems use flat models that fail to capture the layered discourse structure of earnings calls, which blend scripted commentary with unscripted dialogue.

Method: Two-stage transformer architecture: first encodes multi-modal content and discourse metadata at node level using contrastive learning, second synthesizes global embedding for entire conference. Nodes include monologues or Q&A pairs enriched with emotional signals from text/audio/video and structured metadata.

Result: Embeddings form stable, semantically meaningful representations that reflect affective tone, structural logic, and thematic alignment. The system generalizes to other high-stakes unscripted domains like tele-medicine, education, and political discourse.

Conclusion: Provides robust and explainable multi-modal discourse representation with practical utility for financial forecasting and discourse evaluation, offering a generalizable method for high-stakes communication domains.
...
<details>
  <summary>Details</summary>
Motivation: Suicide is a leading cause of death among adolescents, but many cases go undetected due to lack of mental health contact. Social media provides real-time insights into youth struggles.

Method: Transformer-based model that sequentially processes user's posts and engagement content to predict future suicidal ideation/behavior posts without using self-disclosure as input.
...
TL;DR: A scalable method for real-time detection of entity-level hallucinations in long-form LLM outputs using web-search-annotated datasets and linear classifiers, achieving superior performance over existing methods.
...
Method: Developed an annotation methodology using web search to label model responses with grounded entity-level hallucinations, then trained efficient linear probe classifiers on this dataset to detect fabricated entities in real-time.

Result: Classifiers consistently outperformed baselines across four model families, achieving AUC 0.90 vs 0.71 for Llama-3.3-70B, and showed generalization to mathematical reasoning tasks despite being trained only on entity-level labels.

Conclusion: The approach provides a promising scalable solution for real-world hallucination detection, with datasets publicly released to facilitate reuse and cross-model application.
...
TL;DR: Developed UDIB method using Deterministic Information Bottleneck to improve semantic divergence detection in LLMs by creating more informative topic clusters from prompt-response embeddings.
...
Method: Transformed DIB method into practical UDIB algorithm by replacing intractable KL divergence with efficient upper bound, creating entropy-regularized K-means that favors informative clusters for joint prompt-response embedding clustering.
...
TL;DR: Multi-modal question generation system that automatically creates diverse question types from various content formats to address the lack of practice materials for educational resources.


<details>
  <summary>Details</summary>
Motivation: Address the gap in educational practice materials despite abundant available resources by providing tailored questions for different content formats.
...
Conclusion: The system successfully addresses the challenge of creating practice materials for diverse educational resources through automated multi-modal question generation.
...
TL;DR: AR² framework uses adversarial reinforcement learning to enhance LLM abstraction skills by training models to extract computational kernels from complex narrative problems
...
Method: AR² employs a teacher model to transform kernel problems into narrative-rich descriptions while a student coding model learns to extract the underlying computational logic from these complex narratives

Result: Experimental results show AR² substantially improves student model accuracy on previously unseen, challenging programming tasks

Conclusion: Abstraction is a key skill for enhancing LLM generalization, and explicit training for abstraction through adversarial reinforcement learning significantly improves performance on complex programming problems
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with factual consistency due to parametric memory limitations. RAG methods treat knowledge as unstructured text, limiting compositional reasoning and factual inconsistency identification.
...
Result: Consistent improvements in factual accuracy, answer precision, and interpretability over baseline prompting and static KG-augmented methods on three factual QA benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Understanding scientific evolution requires structured cross-document reasoning over thematically related research, going beyond single-paper summaries to capture how ideas develop across multiple papers.

Method: Agent-based framework with three coordinated agents: Plan Agent for task decomposition, Mmap-Agent for motivation-method mind maps, and Lchart-Agent for experimental line charts. Handles temporal alignment of methods and standardization of heterogeneous experimental tables.

Result: Outperforms strong baselines like GPT-4o in semantic alignment, structural consistency, and visual fidelity despite using 7B-scale agents. Includes ResearchPulse-Bench benchmark dataset.

Conclusion: The framework successfully addresses multi-document scientific inference challenges and provides a structured approach to analyzing research development chains across related papers.
...
Method: Developed NoteBar tool that leverages persona information and efficient language models, and created a novel dataset of 3,173 notes with 8,494 annotated concepts across 16 MBTI personas for evaluation.

Result: NoteBar demonstrates practical deployment in a cost-effective manner, enabling interactive use without heavy infrastructure requirements.
...
Result: Qwen achieved highest precision (0.54) but Sprinklr-Edge-OCR delivered best overall F1 score (0.46), processing images 35x faster (0.17s per image) at less than 0.01x the cost ($0.006 per 1000 images) compared to LVLMs.
...
Method: Introduced a curated dataset distinguishing justified vs unjustified self-preference, and constructed steering vectors using Contrastive Activation Addition (CAA) and an optimization-based approach to mitigate bias at inference time.
...
Conclusion: Steering vectors show promise but have limits as safeguards for LLM-as-judges, motivating the need for more robust interventions as self-preference appears to span multiple or nonlinear directions.
...
TL;DR: Analysis of SNOMED CT concept co-occurrence patterns and embedding-based semantic similarity in clinical notes using MIMIC-IV data, showing complementary value for documentation improvement and clinical relationship discovery.
...
Method: Used MIMIC-IV database with Normalized Pointwise Mutual Information (NPMI) and pretrained embeddings (ClinicalBERT, BioBERT) to examine concept relationships, temporal evolution, and cross-specialty patterns.

Result: Weak correlation between co-occurrence and semantic similarity, but embeddings captured clinically meaningful associations. Embedding suggestions frequently matched later-documented concepts. Clustering revealed coherent clinical themes mapping to patient phenotypes and care patterns. Co-occurrence patterns linked to outcomes like mortality and readmission.
...
TL;DR: MLSD uses metric learning with triplet loss for cross-target/cross-domain stance detection, improving performance across multiple models and datasets.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of stance detection across different domains and targets by capturing semantic similarities and enhancing domain adaptation.

Method: Utilizes metric learning with triplet loss to construct a discriminative embedding space that captures semantic similarities between stance targets.
...
Conclusion: MLSD effectively enables cross-target and cross-domain stance detection by learning useful examples from new target domains through metric learning.
...
TL;DR: SiLVERScore is a novel embedding-based metric that directly evaluates sign language generation in a joint semantic space, overcoming limitations of back-translation methods and achieving near-perfect discrimination on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Current sign language evaluation uses back-translation which introduces ambiguity, fails to capture multimodal aspects (facial expressions, spatial grammar, prosody), and makes it hard to distinguish errors from generation vs translation systems.

Method: Proposes SiLVERScore - a semantically-aware embedding-based evaluation metric that assesses sign language generation in a joint embedding space, capturing semantic meaning directly rather than through text conversion.

Result: On PHOENIX-14T and CSL-Daily datasets, SiLVERScore achieves ROC AUC = 0.99 and overlap < 7%, substantially outperforming traditional metrics with near-perfect discrimination between correct and random pairs.

Conclusion: SiLVERScore provides a robust, semantically-aware evaluation framework that addresses the limitations of existing metrics and demonstrates strong performance across different sign language datasets.
...
TL;DR: A new four-metric suite evaluates vision language models in interactive grounding contexts, revealing that current VLMs diverge from human patterns despite task success, with GPT4o-mini performing closest to humans.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks evaluate VLMs in single-turn or question answering settings, but grounding is an interactive process where people develop shared understanding through ongoing communication, requiring better evaluation methods.

Method: Introduced a four-metric suite (grounding efficiency, content alignment, lexical adaptation, and human-likeness) and deployed it on 150 self-play sessions of interactive referential games between three proprietary VLMs, comparing them with human dyads.
...
Conclusion: The metric suite and findings provide a framework for future research on VLM grounding, highlighting the need for better evaluation of interactive reasoning capabilities in vision language models.
...
TL;DR: Align-then-Slide framework for evaluating document-level machine translation by aligning source-target sentences and using multi-granularity sliding window evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods assume sentence-by-sentence alignment, but LLMs produce whole-document outputs that challenge this assumption, requiring new evaluation approaches for document-level MT.

Method: Two-stage framework: 1) Align stage automatically infers sentence-level correspondences and rebuilds target to match source structure; 2) n-Chunk Sliding Evaluate stage calculates averaged metric scores under different chunk sizes for multi-granularity assessment.

Result: Achieves Pearson correlation of 0.929 with expert MQM rankings on WMT benchmark, aligns closely with human judgments on real-world test set, and enables effective CPO training and GRPO reward modeling.

Conclusion: The framework provides an accurate, robust, and actionable evaluation tool for document-level machine translation systems, validated through strong correlation with human judgments and practical applications in training.
...
<details>
  <summary>Details</summary>
Motivation: Traditional audio deepfake detection works at sentence level, while partial detection requires frame-level positioning. Semantic information from audio, particularly named entities, remains underexplored for this task.
...
Conclusion: Integrating named entity knowledge through attention mechanisms significantly improves partial audio deepfake detection performance.
...
TL;DR: LLMs fail to understand "Drivelology" - syntactically coherent but pragmatically paradoxical language - despite excelling at other NLP tasks, revealing limitations in pragmatic understanding.


<details>
  <summary>Details</summary>
Motivation: To investigate whether current large language models can comprehend complex linguistic phenomena where surface-level coherence masks deeper pragmatic paradoxes and rhetorical subversion.
...
Conclusion: Statistical fluency in LLMs does not imply true cognitive comprehension, revealing a significant representational gap in pragmatic understanding that challenges current NLP capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Despite advancements in Long-CoT reasoning improving LLM performance, there's limited understanding of how CoT-based reasoning affects language model trustworthiness across key dimensions.

Method: Comprehensive survey of recent work on reasoning models and CoT techniques, analyzing studies chronologically with detailed methodology and findings analysis across five trustworthiness dimensions.
...
TL;DR: Probing-based safety detection methods in LLMs fail because they learn superficial patterns (instructional formats and trigger words) rather than semantic harmfulness, leading to poor out-of-distribution performance and creating a false sense of security.
...
Result: Confirmed that probes learn superficial patterns (instructional patterns and trigger words) rather than semantic harmfulness, demonstrating comparable performance to simple n-gram methods and poor generalization to out-of-distribution data.

Conclusion: Current probing-based approaches create a false sense of security; there is a need to redesign both models and evaluation protocols for responsible safety research in LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based mobile agents have three main issues: heavy reliance on LLM comprehension leading to errors, lack of external environment interaction causing task termination, and absence of memory capabilities requiring task reconstruction for each instruction.

Method: Proposed MobileRAG framework with three components: InterRAG, LocalRAG, and MemRAG, leveraging Retrieval-Augmented Generation to quickly identify user queries and accomplish complex mobile tasks. Also introduced MobileRAG-Eval benchmark for comprehensive evaluation.

Result: Extensive experiments on MobileRAG-Eval show MobileRAG can handle real-world mobile tasks effectively, achieving 10.3% improvement over state-of-the-art methods with fewer operational steps.

Conclusion: MobileRAG successfully addresses the limitations of current mobile agents through RAG enhancement, demonstrating superior performance in handling complex, real-world mobile tasks that require external knowledge assistance.
...
TL;DR: Matrix of Thought (MoT) enhances LLM reasoning for complex QA through multi-dimensional thinking and fact-correction, achieving superior performance with 14.4% of baseline reasoning time.
...
Method: Proposes Matrix of Thought (MoT) with column-cell communication for horizontal/vertical exploration, plus fact-correction mechanism using knowledge units from KG triples and raw text to enhance reasoning.

Result: Outperforms SOTA methods on four datasets in F1 and EM scores, with reasoning time only 14.4% of baseline methods, demonstrating efficiency and accuracy.

Conclusion: MoT provides an efficient and accurate framework for complex QA by enabling multi-strategy deep thinking and knowledge enhancement, significantly improving LLM reasoning capabilities.
...
Method: Built multi-label emotion dataset with 7,662 entries (line-level and work-level) from 483 poems by 5 Korean poets, annotated with 44 emotion categories. Used sequential fine-tuning of Korean language model - first on general corpora then on KPoEM dataset.

Result: Fine-tuned model significantly outperformed previous models with 0.60 F1-micro score compared to 0.34 from general corpus models. Model demonstrated enhanced ability to identify culturally specific emotional expressions while preserving core sentiments.
...
<details>
  <summary>Details</summary>
Motivation: Existing supervised fine-tuning methods for RAG scenarios cause catastrophic forgetting where models lose previously acquired knowledge and general capabilities, requiring access to general instruction data or facing distribution preservation limitations.
...
Result: Extensive experiments show SelfAug achieves superior balance between downstream learning and general capability retention, with empirical analysis revealing direct correlation between distribution shifts and catastrophic forgetting severity.

Conclusion: SelfAug advances understanding of catastrophic forgetting in RAG contexts and provides a practical solution applicable across diverse fine-tuning scenarios, with code publicly available.
...
TL;DR: SPFT-SQL enhances self-play fine-tuning for Text-to-SQL by adding verification-based iterative fine-tuning and error-driven loss to improve SQL generation accuracy.
...
Method: Introduces verification-based iterative fine-tuning before self-play to create high-quality data, and error-driven loss during self-play to focus on opponent's incorrect outputs.

Result: Outperforms state-of-the-art methods on six LLMs and five benchmarks, demonstrating improved SQL generation accuracy.

Conclusion: SPFT-SQL effectively addresses SPIN's limitations in Text-to-SQL by combining iterative data synthesis and error-driven learning for better performance.
...
Conclusion: VoxRole addresses critical gaps in speech-based RPCA evaluation by providing the first comprehensive benchmark that incorporates paralinguistic features and standardized assessment of persona consistency across multi-turn dialogues.
...
Result: LLMs exhibit limitations in generating accurate conclusions, with factual fabrication being the most common failure mode. Models are unreliable for standalone fact-checking.

Conclusion: While LLMs alone are unreliable for fact-checking, they show considerable potential to augment human performance when used as assistive tools in collaborative scenarios.
...
TL;DR: This paper addresses the lack of NLP resources for low-resource languages like Ladin by creating synthetic datasets through translation and rigorous filtering, improving machine translation performance.


<details>
  <summary>Details</summary>
Motivation: Large Language Models struggle with extremely low-resource languages like indigenous languages due to lack of labeled data, and there's limited availability of high-quality NLP datasets for these languages.

Method: Using a small set of parallel Ladin-Italian sentence pairs, the authors create synthetic datasets for sentiment analysis and MCQA by translating monolingual Italian data, applying rigorous filtering and back-translation procedures to ensure quality.
...
Conclusion: The paper provides the first publicly available sentiment analysis and MCQA datasets for Ladin, establishing foundational resources to support broader NLP research and downstream applications for this underrepresented language.
...
TL;DR: Llama-3-Motif is a 102B parameter language model built on Llama 3 architecture, specifically enhanced for Korean language while maintaining strong English performance, achieving GPT-4 comparable results on Korean benchmarks.


<details>
  <summary>Details</summary>
Motivation: To develop a specialized language model that significantly improves Korean language capabilities while preserving strong English performance, addressing the need for high-quality Korean language AI models.

Method: Built on Llama 3 architecture using advanced training techniques including LlamaPro and Masked Structure Growth. Trained efficiently on MoAI platform across hyperscale GPU clusters with a carefully curated balanced dataset of Korean and English data.

Result: Demonstrates decent performance on Korean-specific benchmarks, outperforms existing models, and achieves results comparable to GPT-4.

Conclusion: Llama-3-Motif successfully enhances Korean language capabilities while maintaining English performance, representing an effective approach to developing specialized multilingual language models.
...
TL;DR: RTQA is a novel framework for temporal knowledge graph question answering that uses recursive decomposition and LLM-based solving without training, achieving state-of-the-art performance on complex temporal queries.
...
Conclusion: RTQA effectively addresses limitations of current TKGQA methods by enhancing reasoning over temporal knowledge graphs without requiring training, demonstrating superior performance on complex temporal queries.
...
TL;DR: LLMs show significant performance drops when benchmark questions are paraphrased, challenging the reliability of current evaluation methods and highlighting the need for robustness-aware benchmarks.


<details>
  <summary>Details</summary>
Motivation: To assess whether benchmark-based evaluations reliably measure LLM capabilities in real-world scenarios with linguistic variability, and to test model robustness to paraphrased inputs.

Method: Systematically generated various paraphrases of all questions across six common benchmarks and measured effectiveness variations of 34 state-of-the-art LLMs of different sizes.

Result: LLM rankings remained relatively stable but absolute effectiveness scores declined significantly with paraphrased inputs, showing models struggle with linguistic variability.

Conclusion: Current benchmark evaluations may not fully capture real-world robustness, necessitating the development of robustness-aware benchmarks that better reflect practical deployment scenarios.
...
Result: Larger and more capable models show increased cross-lingual consistency. Models exhibit higher consistency within themselves across languages than agreement with other models prompted in the same language.
...
TL;DR: Proposes synthetic sheet music problems for evaluating and training AI models, showing improved reasoning and music composition capabilities.
...
Method: Developed a data synthesis framework generating verifiable sheet music questions in textual and visual formats, creating SSMR-Bench benchmark and training data for reinforcement learning with verifiable rewards.

Result: Models show improved sheet music reasoning, with Qwen3-8B-Base surpassing GPT-4 on MusicTheoryBench and achieving comparable reasoning performance. Also improved math problem solving and facilitated music composition.

Conclusion: First to propose music theory-based sheet music problem synthesis, demonstrating effectiveness for advancing model reasoning and enabling new AI-assisted music creation possibilities.
...
<details>
  <summary>Details</summary>
Motivation: The COVID-19 pandemic accelerated digital technology adoption in education, particularly e-learning systems and chatbots. While AI and NLP advancements have made chatbots popular in various domains, there's a need to assess the state of Arabic educational chatbots specifically.

Method: Conducted a comprehensive survey of existing Arabic chatbots in education, analyzing characteristics such as adopted approaches, language variety, and performance metrics used.

Result: Found that despite the success of chatbots in languages like English, only a few educational Arabic chatbots utilize modern techniques like BERT and GPT, revealing significant research gaps.

Conclusion: Identified the need for more advanced Arabic educational chatbots and discussed future research directions to bridge the gap with other language implementations.
...
TL;DR: This paper presents a system for detecting covert narratives in news articles using fine-tuned BERT for multi-label classification and GPT-4o with ReACT framework for generating evidence-based explanations, enhanced by auxiliary knowledge to improve accuracy.
...
Method: Fine-tune BERT model with recall-oriented approach for narrative detection, then use GPT-4o pipeline with ReACT framework (Reasoning + Acting) and semantic retrieval-based few-shot prompting for explanation generation, incorporating structured taxonomy table as auxiliary knowledge.

Result: Integration of auxiliary knowledge in prompts improves both classification accuracy and justification reliability, demonstrating effective detection of narratives and generation of grounded explanations.
...
TL;DR: Study investigates constructing stable personalized lexical profiles for conversational agents using minimal spoken data, finding optimal profiles with 5 adjectives/conjunctions and 10 adverbs/nouns/pronouns/verbs each from 10 minutes of transcribed speech.


<details>
  <summary>Details</summary>
Motivation: Lexical alignment contributes to successful human communication but remains underexplored in conversational agents despite recent LLM advancements. Need foundational research on personalized lexical profiles as basis for alignment.
...
Result: Smaller, more compact profiles created from 10 min of transcribed speech (5 items for adjectives/conjunctions, 10 items for adverbs/nouns/pronouns/verbs each) offered best balance of performance and data efficiency.
...
TL;DR: MultiWikiQA is a new multilingual reading comprehension dataset covering 306 languages, with Wikipedia-based contexts, LLM-generated questions, and human-verified quality.
...
Method: Used Wikipedia articles as context data, generated questions using a large language model (LLM), and conducted crowdsourced human evaluation across 30 languages to verify question fluency and quality.

Result: The dataset contains high-quality questions (human-evaluated as fluent), benchmark evaluation of 6 different language models showed the dataset is sufficiently challenging with significant performance discrepancies across languages.

Conclusion: MultiWikiQA provides a valuable multilingual reading comprehension resource that reveals substantial performance gaps across languages, serving as a challenging benchmark for evaluating language models' cross-lingual capabilities.
...
<details>
  <summary>Details</summary>
Motivation: There's a growing need for scalable psychological counseling with privacy-compliant data, but high-quality training data for fine-tuning LLMs remains scarce.

Method: A multi-agent framework where specialized LLM agents handle different psychological techniques and sub-tasks, coordinated to generate counseling sessions. Also introduces a unified evaluation framework with nine expert assessment aspects.

Result: MAGneT significantly outperforms baselines, improving general counseling skills by 3.2% and CBT-specific skills by 4.3% on CTRS. Experts prefer MAGneT sessions in 77.2% of cases. Fine-tuning on MAGneT data yields 6.3-7.3% improvements over baseline-generated data.

Conclusion: MAGneT provides a superior approach for generating synthetic psychological counseling data that captures real counseling structure and nuance, enabling better LLM fine-tuning for scalable mental health applications.
...
Method: Combines explicit text augmentation using LLMs with five generation strategies and implicit feature-space augmentation with five novel perturbation techniques on structural fused embeddings.
...
Method: Proposed Inverse IFEval benchmark with 8 challenge types including Question Correction and Counterfactual Answering. Created 1012 high-quality Chinese/English questions across 23 domains using human-in-the-loop pipeline, evaluated with optimized LLM-as-a-Judge framework.
...
TL;DR: LLMs in healthcare risk providing outdated medical advice due to static training data. Researchers created two QA datasets (MedRevQA and MedChangeQA) showing all tested LLMs rely on obsolete knowledge, highlighting the need for more current medical AI systems.
...
Method: Created two novel question-answering datasets from systematic reviews: MedRevQA (16,501 QA pairs for general biomedical knowledge) and MedChangeQA (512 QA pairs where medical consensus has changed over time). Evaluated eight prominent LLMs on these datasets and analyzed influence of obsolete pre-training data and training strategies.

Result: Evaluation revealed consistent reliance on outdated knowledge across all eight tested LLMs, demonstrating their inability to keep up with evolving medical consensus and updated research findings.

Conclusion: The study highlights the critical problem of LLMs' outdated medical knowledge and provides groundwork for developing mitigation strategies to create more current and reliable medical AI systems that can adapt to evolving medical knowledge.
...
TL;DR: PARCO improves contextual ASR by integrating phoneme-aware encoding, contrastive entity disambiguation, and hierarchical filtering to better handle homophones and multi-token entities.


<details>
  <summary>Details</summary>
Motivation: ASR systems struggle with domain-specific named entities and homophones, with contextual ASR having limited entity diversity and treating entities as independent tokens, leading to incomplete multi-token biasing.
...
Conclusion: PARCO effectively addresses homophone recognition and multi-token entity challenges in contextual ASR through integrated phonetic and contrastive learning approaches.
...
TL;DR: LLM gender bias evaluations are highly sensitive to prompt design - minor changes can substantially alter or even reverse bias outcomes, raising questions about ecological validity of current benchmarks.
...
Method: Tested models under different prompt conditions that make testing context and gender-focused content salient, using four task formats with both token-probability and discrete-choice metrics to assess prompt sensitivity.
...
<details>
  <summary>Details</summary>
Motivation: Most prior work on temporal reasoning in LMs has focused solely on the Gregorian calendar, ignoring culturally significant non-Gregorian systems like Japanese, Hijri, and Hebrew calendars that are actively used.
...
Result: Some models can perform calendar conversions, but even Japanese-centric models struggle with Japanese-calendar arithmetic and maintaining consistency across different calendar systems.

Conclusion: Current LMs have significant limitations in handling culture-specific calendar systems, highlighting the need for developing models with better non-Gregorian calendar understanding capabilities.
...
<details>
  <summary>Details</summary>
Motivation: To develop optimizers that leverage the natural Riemannian metric structure of neural network loss landscapes when embedded in higher-dimensional space, providing geometric insights for improved training.

Method: Developed new optimizers using the induced Riemannian metric from loss landscape embeddings, compared against SGD, Adam, AdamW, and Muon across various tasks and architectures.

Result: Highly effective in low-dimensional examples, provides slight improvements over state-of-the-art methods, automatically decreases learning rate in high-curvature regions, and enables natural decoupled weight decay.

Conclusion: The Riemannian geometry approach yields theoretically desirable optimizers with computational complexity comparable to Adam, offering automatic curvature adaptation and can be used to modify existing preconditioning methods.
...
Method: Reciprocal application: PCC provides predictability for robust NMF rank estimation, while NMF enables approximate PCC implementation through independent mixture modeling and clustering.

Result: PCC-based rank estimation is stable against noise, resolves NMF nonidentifiability, and produces stable features. NMF enables effective clustering by common causes and data denoising.

Conclusion: The synergy between NMF and PCC creates a powerful framework for robust dimensionality reduction, stable feature extraction, clustering, and denoising applications.
...
TL;DR: FedDeCAB is a semi-decentralized client selection method for federated learning that handles time-series data availability issues by using probabilistic rankings and neighbor-assisted parameter sharing when clients go offline.


<details>
  <summary>Details</summary>
Motivation: Federated learning in IoT scenarios faces challenges with data heterogeneity, limited energy budgets, and client availability issues, particularly with time-series data where clients may frequently disconnect or rejoin.

Method: Proposed FedDeCAB method uses probabilistic rankings for client selection and allows disconnected clients to obtain partial model parameters from nearest neighbors for joint optimization, reducing communication overhead.

Result: Experiments on real-world taxi and vessel trajectory datasets show FedDeCAB effectively handles highly heterogeneous data distribution, limited communication budgets, and dynamic client offline/rejoining scenarios.

Conclusion: FedDeCAB improves performance of offline models and reduces communication overhead in federated learning with time-series data, making it suitable for practical IoT applications with dynamic client availability.
...
TL;DR: Deep RL framework for autonomous microgrid management using transformer forecasting and PPO agent to optimize renewable energy dispatch.


<details>
  <summary>Details</summary>
Motivation: Develop autonomous microgrid management for remote communities to minimize costs and maximize renewable energy utilization, advancing toward zero-carbon energy systems.
...
Result: Significant improvements in energy efficiency and operational resilience compared to traditional rule-based methods.

Conclusion: The framework successfully advances smart-grid technologies and provides an open-source simulation platform for microgrid environments, contributing to sustainable energy solutions.
...
TL;DR: SharedRep-RLHF improves fairness in RLHF by learning shared traits across groups instead of separate reward models, addressing limitations of MaxMin-RLHF with minority groups.
...
Conclusion: Learning shared traits across groups is more effective than separate group models for fair RLHF, especially when dealing with minority groups.
...
TL;DR: A machine learning-powered SCM-FSCM model combining economic theories with algorithms like random forests, LSTM, XGBoost, and reinforcement learning to improve supply chain efficiency, reduce financing costs, and optimize inventory management.


<details>
  <summary>Details</summary>
Motivation: To address efficiency loss, financing constraints, and risk transmission in supply chains by integrating economic theories with machine learning technology for a collaborative SCM-FSCM approach.

Method: Combines Transaction Cost and Information Asymmetry theories with machine learning algorithms. Uses random forests for multi-dimensional data processing, LSTM for demand forecasting, clustering/regression for benefit allocation, reinforcement learning for inventory optimization, and XGBoost for credit assessment.
...
TL;DR: Empirical analysis of gradient variance and standard deviation evolution during training, leading to a hyperparameter-free gradient normalization method that aligns with natural gradient scaling and improves optimization stability.


<details>
  <summary>Details</summary>
Motivation: To understand how gradient dynamics affect neural network stability and generalization, and to bridge the gap between theoretical expectations and empirical gradient behaviors.

Method: Proposed a hyperparameter-free gradient normalization method that aligns gradient scaling with their natural evolution during training, preventing unintended amplification while maintaining convergence guarantees.

Result: Experiments on CIFAR-100 with ResNet-20, ResNet-56, and VGG-16-BN show maintained or improved test accuracy under strong generalization, with stabilized optimization.

Conclusion: Direct tracking of gradient dynamics is crucial for optimization research, and the proposed method effectively stabilizes training while preserving performance, providing insights for future optimization techniques.
...
TL;DR: Comprehensive review of multi-agent reinforcement learning (MARL) applications in modern games, covering achievements like AlphaStar and OpenAI Five, analyzing challenges, and proposing future research directions.


<details>
  <summary>Details</summary>
Motivation: With MARL demonstrating superhuman performance in games and growing impact in the field, there's a need for a comprehensive review to examine its applications across various game genres and address critical challenges.

Method: Thorough examination of MARL's application from turn-based two-agent games to real-time multi-agent video games across popular genres (Sports, FPS, RTS, MOBA), analysis of challenges, and successful implementations in specific games.

Result: Provides insights into MARL in video game AI systems, proposes a novel method to estimate game complexity, and highlights successful MARL implementations in various games including Rocket League, Minecraft, Quake III Arena, StarCraft II, Dota 2, and Honor of Kings.

Conclusion: The paper offers a comprehensive review of MARL applications in gaming, identifies critical challenges, and suggests future research directions to advance MARL and inspire innovation in game development.
...
TL;DR: Proposes hierarchical federated foundation models (HF-FMs) that address modality and task heterogeneity in fog/edge networks by aligning multi-modal multi-task foundation models with hierarchical infrastructure and enabling D2D communications.


<details>
  <summary>Details</summary>
Motivation: The rise of multi-modal multi-task foundation models (M3T FMs) like GPT-4 creates a need for federated approaches that can handle heterogeneity in collected modalities and executed tasks across distributed fog/edge networks.
...
Result: The paper prototypes HF-FMs in a wireless network setting and releases open-source code to demonstrate their potential and foster exploration in this emerging field.

Conclusion: HF-FMs represent a novel paradigm that addresses two overlooked heterogeneity dimensions in distributed learning, offering unique capabilities for multi-modal multi-task federated learning in hierarchical network infrastructures.
...
Result: EmbedOR extends consistency results for tSNE to broader datasets, reduces fragmentation of continuous high-density regions, and provides better geometry preservation in both synthetic and real data experiments.

Conclusion: EmbedOR offers superior visualization capabilities and geometry preservation compared to existing SNE methods, and its distance metric can be used to annotate existing visualizations for deeper geometric insights.
...
TL;DR: Online Testing Problem (OTP) with correlated costly tests requires optimal subset selection. Missing data from partial testing creates bias, leading to Ω(T^{2/3}) minimax regret vs Θ(√T) in standard MDPs. Explore-Then-Commit algorithm achieves Õ(T^{2/3}) regret, while variant with missingness-independent rewards achieves Õ(√T) regret.


<details>
  <summary>Details</summary>
Motivation: Real-world testing scenarios often involve correlated and costly tests where conducting all tests is impractical. Need to develop optimal sequential testing policies that balance information gain against testing costs under unknown outcome distributions.

Method: Formulate as Online Testing Problem (OTP) with missing data. Prove Ω(T^{2/3}) minimax regret lower bound. Develop Explore-Then-Commit algorithm for discrete/Gaussian distributions. Study variant with missingness-independent rewards using iterative-elimination.

Result: Established fundamental difficulty: Ω(T^{2/3}) regret lower bound due to missing data bias. Matched by algorithm achieving Õ(T^{2/3}) regret. Variant with different reward structure achieves Õ(√T) regret, breaking the lower bound. Numerical results confirm theoretical findings.

Conclusion: Missing data fundamentally increases exploration-exploitation difficulty in sequential testing. Reward structure significantly impacts achievable regret rates. Provides framework for designing efficient testing policies under partial information and guides understanding of exploration under missing data.
...
Method: The paper provides a perspective on X-Learning architecture, analyzing connections to graph theory and Markov chains to explore design possibilities.

Result: The work sheds light on intuitive but non-trivial connections between X-Learning, graph theory, and Markov chains, establishing a foundation for this new architecture.

Conclusion: The paper presents X-Learning as a promising distributed learning approach and provides open research directions to stimulate further investigation in this area.
...
Method: Proposed differentiable approximation of range-partition entropy, designed EntropyNet neural module to restructure data into low-entropy forms, and applied entropy regularization directly to Transformer attention mechanisms.

Result: Achieved up to 4.1x runtime speedups with <0.2% error in geometry tasks, and 6% higher accuracy at 80% sparsity compared to L1 baselines in deep learning applications.
...
<details>
  <summary>Details</summary>
Motivation: To address the gap in understanding representational properties of neural operators despite their growing importance in scientific computing, and to test the Platonic Representation Hypothesis that suggests neural networks converge to similar representations across architectures.
...
Conclusion: The framework successfully enables mechanistic interpretability of large neural operators and demonstrates that lifting and operator techniques provide significant advantages for representation recovery and cross-resolution robustness in neural operator models.
...
<details>
  <summary>Details</summary>
Motivation: Satellite ML applications suffer from sparse, clustered labeled data despite global coverage, but previous research focused on model architectures rather than data optimization for large-scale monitoring.

Method: Novel problem formulation for spatial training data optimization with heterogeneous costs and budget constraints, plus new methods tested across three continents and four tasks.

Result: Experiments show substantial performance gains from optimized sampling strategies, with delineated settings where optimization is particularly effective.

Conclusion: The framework generalizes across SatML domains and provides immediate practical value for augmenting agricultural surveys in Togo, addressing a critical gap in satellite ML research.
...
<details>
  <summary>Details</summary>
Motivation: To develop an efficient method for approximating real-valued functions on smooth manifolds that addresses computational challenges of high-dimensional data while maintaining accuracy.
...
Conclusion: The proposed methodology provides an effective and scalable approach for function approximation on manifolds, successfully addressing high-dimensional computational challenges while delivering superior performance compared to existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Overcome limitations of hand-crafted training environments by developing environments that scale in complexity alongside learning agents, enabling truly generalizable and robust embodied intelligence.

Method: Goal-conditioned generative world model where an Attacker agent synthesizes challenging world states to exploit Defender weaknesses, while Defender team learns cooperative policies to overcome these generated scenarios.
...
TL;DR: This paper introduces Policy-Aware Matrix Completion (PAMC), showing that low-rank structure in reward matrices enables efficient sparse-reward RL with polynomial sample complexity instead of exponential.
...
Method: Developed Policy-Aware Matrix Completion (PAMC) framework that connects matrix completion theory with reinforcement learning via policy-dependent sampling analysis, including reward-free representation learning and distribution-free confidence sets.

Result: PAMC achieves 1.6-2.1x sample efficiency improvement over strong baselines with only 20% computational overhead, finding exploitable structure in over half of 100 systematically sampled domains.

Conclusion: Low-rank reward structure enables polynomial sample complexity for sparse-reward RL, establishing structural reward learning as a promising paradigm for safety-critical, sample-expensive applications.
...
<details>
  <summary>Details</summary>
Motivation: Reliable connectivity in mmWave and sub-THz networks depends on surface reflections, as high-frequency signals are vulnerable to blockage. Surface roughness determines scattering behavior (specular vs diffuse), which is crucial for network performance.

Method: Collected dataset of 78,000+ points from 15 indoor materials, partitioned into 3cm patches. Extracted patch-level features (elevation angle, log-scaled intensity, max-to-mean ratio) and trained Random Forest, XGBoost, and neural network classifiers.

Result: Ensemble tree-based models (Random Forest, XGBoost) provided the best trade-off between accuracy and robustness, confirming LiDAR-derived features effectively capture roughness-induced scattering effects.

Conclusion: The framework enables generation of scatter-aware environment maps and digital twins, supporting adaptive beam management, blockage recovery, and environment-aware connectivity in next-generation networks.
...
TL;DR: Neural network-based models achieve 92% accuracy in classifying traffic accident severity using autoencoders for dimensionality reduction and dense networks.


<details>
  <summary>Details</summary>
Motivation: To leverage machine learning advances to study traffic accident data and mitigate risk through improved accident severity classification.

Method: Analyzed feature colinearity, used autoencoders for unsupervised dimensionality reduction, and implemented dense neural networks on traffic accident data.

Result: Achieved cross-validated accuracy of up to 92% in classifying accident severity using the proposed deep neural network approach.

Conclusion: Deep neural networks with autoencoder-based dimensionality reduction are effective for traffic accident severity classification, demonstrating high predictive power on imbalanced data.
...
<details>
  <summary>Details</summary>
Motivation: Accurate prediction of V2I communication link quality is essential for smooth handovers and reliable low-latency communication, but MLLMs lack 3D spatial understanding despite having access to vehicle sensor data.

Method: Proposes a plug-and-play BEV injection connector that constructs bird's-eye view representations from neighboring vehicle sensing data and fuses it with ego vehicle input. Uses CARLA simulator and MATLAB-based ray tracing to generate multimodal data (RGB, LiDAR, GPS, wireless signals) and extract ground-truth responses.
...
Conclusion: The proposed BEV injection approach effectively addresses MLLMs' spatial understanding limitations and demonstrates robust performance improvements in V2I link prediction, particularly in adverse environmental conditions.
...
TL;DR: Proposes deep latent variable MFG model for inverse reinforcement learning that handles heterogeneous agent objectives without prior context knowledge


<details>
  <summary>Details</summary>
Motivation: Existing IRL methods in mean field games assume agent homogeneity, limiting their ability to handle real-world demonstrations with heterogeneous and unknown objectives

Method: Deep latent variable MFG model with associated IRL method that can infer rewards from different but structurally similar tasks without prior context knowledge or model modification

Result: Superior performance over state-of-the-art IRL methods in MFGs demonstrated on simulated scenarios and real-world spatial taxi-ride pricing problem

Conclusion: The proposed approach effectively addresses the limitation of agent homogeneity assumption and provides practical framework for inferring reward functions from expert demonstrations with heterogeneous objectives
...
Method: Develops a metric that evaluates DAs based on their capacity to maximize contextual mutual information while ensuring predictions align with ground truth labels, enabling automatic DA selection.

Result: Extensive evaluations show the method significantly improves state-of-the-art QAT and KD performance across various models and datasets.

Conclusion: The proposed DA selection metric effectively enhances quantization-aware knowledge distillation, demonstrating the importance of input transformation optimization in low-bit model training.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods overlook lead-lag dependencies at multiple grouping scales and fail to capture hierarchical lead-lag effects in complex systems.
...
Result: Achieved 95.67% precision, sensitivity, and specificity, F1-score of 0.9565, AUC of 0.9748, MCC of 0.9134, and Cohen's kappa of 0.9131
...
Result: TCPNet consistently outperforms state-of-the-art geometric graph neural networks across four protein representation learning tasks. Shows particular strength in fold classification tasks that require understanding of secondary structure arrangements.

Conclusion: Hierarchical topological features are crucial for effective protein analysis, and the Topotein framework successfully captures multi-scale structural patterns through topological deep learning.
...
TL;DR: Analysis of online learning mistake-bound model with arithmetic operation caps, solving problems from previous works and extending to operation-constrained settings.


<details>
  <summary>Details</summary>
Motivation: To understand the computational limits of online learning algorithms by investigating the minimum arithmetic operations required per round to learn arbitrary function families with finite mistakes.

Method: Proving general bounds on arithmetic operations per round needed for mistake-bounded learning, solving agnostic learning problems with bandit feedback, and extending results to operation-capped settings.

Result: Established fundamental bounds on computational requirements for online learning, resolved open problems from previous research, and provided extensions to operation-constrained environments.

Conclusion: The paper provides important theoretical foundations for understanding the computational complexity of online learning algorithms, particularly in resource-constrained settings with operation caps.
...
Result: The approach improves trustworthiness of AI-based fault detection systems by ensuring stable performance under uncertainty through formal verification.

Conclusion: Formal verification using Marabou enables development of highly reliable AI-based satellite fault detection systems that can operate with the required dependability levels.
...
TL;DR: Framework for aligning ML models with clinical knowledge through constraints, showing improved alignment without performance loss.
...
Method: Developed a reproducible framework incorporating clinical knowledge (collected via survey) as constraints into ML models, tested on prostate cancer outcome prediction with analysis across degrees of underspecification, and conducted randomized experiments with clinicians to examine feedback-driven alignment.

Result: Successfully aligned ML models with clinical knowledge without compromising performance, and found that larger differences between constrained and unconstrained models lead to more apparent differences in clinical interpretation.

Conclusion: Clinical knowledge can be effectively incorporated into ML models through constraints, enabling better alignment with medical expertise while maintaining model performance, with potential applications in improving clinical adoption of AI systems.
...
TL;DR: FedQuad is a novel federated learning method that addresses data heterogeneity by optimizing intra-class variance and inter-class variance across clients through metric learning, showing superior performance on CIFAR datasets.


<details>
  <summary>Details</summary>
Motivation: Federated learning faces challenges with data heterogeneity, especially with limited and imbalanced datasets, which negatively impacts global model generalization during model aggregation.

Method: Proposes FedQuad that explicitly minimizes distance between similar pairs and maximizes distance between negative pairs to disentangle client data in shared feature space, optimizing intra-class and inter-class variances.

Result: Demonstrates superior performance on CIFAR-10 and CIFAR-100 datasets under various data distributions with many clients compared to existing approaches.

Conclusion: Metric learning-based strategies effectively address representational learning challenges in federated settings by improving feature disentanglement and reducing negative impacts of model aggregation.
...
TL;DR: SP-CCI is a new framework that uses synthetic counterfactual data to create tighter prediction intervals while maintaining coverage guarantees, addressing the over-conservatism of existing conformal counterfactual inference methods.
...
TL;DR: This paper addresses fairness concerns in algorithmic recourse systems, providing theoretical analysis of unfairness and introducing a new social burden-based fairness framework with practical algorithm MISOB that reduces social burden across groups while maintaining classifier accuracy.


<details>
  <summary>Details</summary>
Motivation: Emerging legislation mandates that classifiers must provide actionable recourse for negative decisions, but existing fairness guarantees in the recourse process itself are inadequate, requiring a holistic theoretical framework to address unfairness in algorithmic recourse.

Method: The authors provide theoretical characterization of unfairness in algorithmic recourse, formally link fairness guarantees between recourse and classification, and introduce a novel social burden-based fairness framework with a practical algorithm called MISOB that works under real-world conditions.

Result: Empirical evaluation on real-world datasets demonstrates that MISOB successfully reduces social burden across all demographic groups without compromising the overall accuracy of the classifier.

Conclusion: The proposed social burden framework and MISOB algorithm provide an effective approach to ensuring fairness in algorithmic recourse systems, addressing limitations of traditional equal cost paradigms while maintaining practical applicability in real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: To improve machine learning classification performance through synthetic data generation, particularly for tabular data, without the need for additional LLM training.
...
TL;DR: AFA is a novel attention mechanism that models input sequences as linear SDEs, using closed-form solutions to compute attention weights as maximum likelihood solutions with robust precision reweightings.


<details>
  <summary>Details</summary>
Motivation: To develop a more theoretically grounded attention mechanism that incorporates learnable dynamics modeling rather than direct query-key comparisons, providing better uncertainty propagation and robustness.

Method: Models input sequence as discrete observations of linear stochastic differential equations, uses closed-form solution to differential Lyapunov equation for efficient uncertainty propagation, derives attention weights as maximum likelihood solutions with precision reweightings.

Result: Developed AFA mechanism that can recover standard dot-product attention as a special case, maintains same computational complexity, and provides simplified variant with constrained eigenvalues.

Conclusion: AFA provides a theoretically sound alternative to standard attention with learnable dynamics modeling, robust uncertainty handling, and equivalent computational efficiency while encompassing standard attention as a limiting case.
...
<details>
  <summary>Details</summary>
Motivation: Self-supervised speech models have shown strong performance in speech processing, but their effectiveness on non-speech bioacoustic data remains underexplored, creating an opportunity to study their transfer learning capabilities for animal sound detection and classification.

Method: Analyzed transfer learning capabilities using linear probing on time-averaged representations, extended the approach with downstream architectures to account for time-wise information, and studied the effects of frequency range and noise on performance.

Result: Models generated rich latent representations of animal sounds across taxa, with results competitive with fine-tuned bioacoustic pre-trained models. Demonstrated the impact of noise-robust pre-training setups on performance.
...
TL;DR: Time series forecasting models are vulnerable to membership inference attacks, with user-level attacks achieving perfect detection. New attacks (multivariate LiRA adaptation and Deep Time Series attack) outperform existing methods, showing increased vulnerability with longer prediction horizons and smaller training sets.
...
Method: Developed two new attacks: (1) adaptation of multivariate LiRA to time-series forecasting, and (2) novel Deep Time Series (DTS) end-to-end learning approach. Benchmarked against adapted classification attacks on TUH-EEG and ELD datasets using LSTM and N-HiTS architectures under record- and user-level threat models.

Result: Forecasting models are vulnerable to attacks, with user-level attacks often achieving perfect detection. Proposed methods achieved strongest performance in several settings. Vulnerability increases with longer prediction horizons and smaller training populations.

Conclusion: Establishes new baselines for privacy risk assessment in time series forecasting, showing similar vulnerability trends to large language models and highlighting significant privacy concerns in time series applications.
...
TL;DR: This paper provides a critical analysis and extension of Cai and Wang's (2020) work on over-smoothing in GNNs, showing exponential decrease of Dirichlet energy under mild spectral conditions with Leaky-ReLU and spectral polynomial filters, with experimental validation.


<details>
  <summary>Details</summary>
Motivation: To address and extend the understanding of over-smoothing phenomena in Graph Neural Networks (GNNs) through Dirichlet energy analysis, building upon previous work by Cai and Wang.
...
Result: Demonstrates that Dirichlet energy of node embeddings decreases exponentially with depth under specified conditions, and identifies practical scenarios where Dirichlet energy increases.

Conclusion: Provides theoretical insights into over-smoothing mechanisms and suggests practical approaches (edge deletion and weight amplification) to mitigate over-smoothing in GNNs.
...
Conclusion: SBD provides a simple, flexible acceleration paradigm that requires no architectural changes, maintains KV-caching compatibility, and can be implemented by fine-tuning existing models.
...
TL;DR: ZooCast is a framework that intelligently combines multiple Time Series Foundation Models (TSFMs) by creating a model zoo that dynamically selects the best model for each forecasting task using unified embeddings.
...
Conclusion: The framework successfully leverages complementary TSFM capabilities through dynamic model selection, achieving improved accuracy while maintaining efficiency and supporting progressive model integration.
...
TL;DR: This paper introduces precision and recall metrics to evaluate the relationship phase of dimensionality reduction, helping explain missing expected cluster structures and guide hyperparameter tuning.
...
Method: Leverages a framework dividing DR into relationship and mapping phases, and introduces two supervised metrics (precision and recall) to evaluate how well modeled relationships align with expected cluster structures based on labels.

Result: The approach is illustrated using t-SNE and UMAP, validated through various usage scenarios, and shown to guide hyperparameter tuning, uncover projection artifacts, and determine if expected structures are captured.

Conclusion: The introduced metrics make the DR process faster and more reliable by providing quantitative evaluation of the relationship phase and explaining missing expected structures.
...
<details>
  <summary>Details</summary>
Motivation: To theoretically analyze and compare the long-range dependency modeling capabilities of state-space models (SSM) and transformers, as current benchmarking lacks mathematical foundation for systematic improvement.
...
Result: SSM's long-range dependency decays exponentially with sequence length (similar to RNN), while attention mechanism in transformers is more flexible without exponential decay constraint.

Conclusion: Transformers theoretically perform better at long-range dependency with sufficient resources, but new SSM formulation combines attention's flexibility with SSM's computational efficiency while maintaining stability.
...
Method: Developed a unified analytical framework connecting layer-wise noise injection strategies with optimization objectives and privacy budget allocations. Proposed SNR-Consistent noise allocation strategy that ensures inter-layer signal-to-noise ratio consistency and efficient privacy budget utilization.

Result: Extensive experiments in centralized and federated learning settings show the proposed method consistently outperforms existing allocation strategies, achieving better privacy-utility tradeoffs.
...
<details>
  <summary>Details</summary>
Motivation: Heart failure research faces limitations due to privacy regulations and institutional barriers that restrict access to large, shareable patient datasets. Synthetic data generation offers a solution to overcome these challenges while preserving patient confidentiality.

Method: Generated synthetic HF datasets from 12,552 unique patients using five deep learning models: tabular variational autoencoder (TVAE), normalizing flow, ADSGAN, SurvivalGAN, and tabular denoising diffusion probabilistic models (TabDDPM). Evaluated through statistical similarity metrics, survival prediction using machine learning, and privacy assessments.
...
TL;DR: RL fine-tuning preserves prior knowledge better than SFT despite similar task performance, with KL-divergence determining forgetting. On-policy RL favors KL-minimal solutions while SFT can diverge arbitrarily.


<details>
  <summary>Details</summary>
Motivation: To understand why reinforcement learning fine-tuning preserves model capabilities better than supervised fine-tuning when adapting to new tasks, despite similar performance outcomes.

Method: Theoretical analysis of distributional shift measured by KL-divergence between fine-tuned and base policies, validated through experiments with large language models and robotic foundation models.

Result: RL preserves prior knowledge significantly better than SFT, with on-policy RL implicitly biased towards KL-minimal solutions. The degree of forgetting is determined by KL-divergence from the base model.

Conclusion: On-policy RL updates lead to smaller KL changes, establishing "RL's Razor" principle: RL prefers solutions closest in KL to the original model among all ways to solve a new task.
...
TL;DR: A novel method for efficiently finding optimal privacy-accuracy trade-offs in differential privacy using multi-objective optimization with improved preference learning through hypothetical trade-off curve selection.


<details>
  <summary>Details</summary>
Motivation: Standard multi-objective optimization approaches for differential privacy are inefficient because they fail to leverage the unique structure where Pareto optimal points can be generated directly by maximizing accuracy for fixed privacy levels.

Method: Theoretical derivation of the trade-off shape to model Pareto front directly, replacing pairwise comparisons with more informative interaction where users select preferred trade-offs from hypothetical curves.

Result: Experiments on differentially private logistic regression and deep transfer learning across six datasets show convergence to optimal privacy-accuracy trade-off with significantly less computational cost and user interaction than baselines.

Conclusion: The proposed method effectively leverages the structural properties of differential privacy trade-offs to achieve more efficient optimization and preference learning compared to standard approaches.
...
TL;DR: Machine learning fails in real-world applications due to causal and statistical structures, with two overlooked problems: no fair lunch and subgroup separability that current fair representation methods don't solve.


<details>
  <summary>Details</summary>
Motivation: Machine learning methods often fail in high-stakes real-world applications, particularly across socially sensitive contexts, limiting adoption in critical areas like medical diagnosis where they could provide significant benefits if safely deployed.

Method: The paper introduces causal and statistical structures that cause ML failures in image analysis, identifies two overlooked problems (no fair lunch and subgroup separability), analyzes why current fair representation learning methods fail to address them, and proposes potential solutions.

Result: The analysis reveals fundamental limitations in current fair ML approaches for image analysis, specifically highlighting that existing fair representation learning methods are inadequate for solving the identified no fair lunch and subgroup separability problems.

Conclusion: The paper calls for new approaches in fair machine learning that can properly address the identified causal and statistical challenges, proposing potential paths forward to enable safer deployment of ML methods in sensitive real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Real-world decision-making problems can be encoded as causal multi-armed bandits at different abstraction levels, but existing methods don't effectively exploit the information and computational advantages across these levels.

Method: AT-UCB uses causal abstraction theory to explore within cheap-to-simulate coarse-grained CMAB instances first, then applies traditional UCB on a restricted set of potentially optimal actions in the target CMAB.

Result: The algorithm achieves significant reductions in cumulative regret compared to classical UCB, demonstrated both theoretically through novel regret bounds and empirically using epidemiological simulators with varying resolution.

Conclusion: AT-UCB provides an effective methodology for exploiting shared information between CMAB instances at different abstraction levels, offering computational efficiency and improved performance in complex decision-making scenarios.
...
TL;DR: Proposes a data-driven behavioral model to analyze energy usage patterns on US Army installations for climate resilience planning and assessing impact of disruptions.
...
Method: Develops behavioral models that analyze, predict, and cluster multimodal energy usage data from non-residential buildings, using similarly structured open access data for methodology demonstration.

Result: Creates behavior profiles of energy usage that serve as baseline assessments for disruption impacts and benchmarks for future resilience measures.

Conclusion: The proposed data-driven approach provides a foundation for understanding energy usage patterns and evaluating resilience strategies for Army installations facing climate-related threats.
...
TL;DR: Proposes SST-iTransformer for parking availability prediction, using clustering and multi-source traffic data with self-supervised learning and dual-branch attention, achieving state-of-the-art results.
...
Method: Uses K-means clustering to create parking cluster zones, integrates multi-source traffic data, and develops SST-iTransformer with self-supervised learning and dual-branch attention mechanism (Series Attention for temporal dependencies, Channel Attention for cross-variate interactions).

Result: Outperforms baseline models (Informer, Autoformer, Crossformer, iTransformer) with lowest MSE and competitive MAE. Ride-hailing data provides largest performance gains, followed by taxi data, while fixed-route transit contributes marginally.

Conclusion: SST-iTransformer effectively models spatio-temporal dependencies and leverages multi-source data for superior parking prediction, with spatial correlations being crucial for performance.
...
Method: Proposes active learning strategies based on bias-variance tradeoff to reduce bias between experimental rounds. Introduces cobias-covariance relationship for leveraging historical data quadratically and eigendecomposition strategy for batching.

Result: The difference-based method using cobias-covariance relationship with quadratic estimator in batched settings outperforms canonical methods like BALD and Least Confidence.

Conclusion: The proposed framework effectively handles heteroskedastic aleatoric uncertainty in experimental settings and provides superior performance through novel bias-reduction strategies and batching mechanisms.
...
TL;DR: PagedEviction is a fine-grained KV cache pruning strategy that improves memory efficiency in LLM inference by using block-wise eviction tailored for paged memory layouts, achieving better memory usage and accuracy than baselines.
...
TL;DR: TiM introduces continuous-time transition models that enable flexible sampling with arbitrary steps, achieving SOTA performance with fewer parameters and monotonic quality improvement as step count increases.


<details>
  <summary>Details</summary>
Motivation: Address the fundamental dilemma between diffusion model fidelity (high computational cost) and few-step alternatives (quality ceiling) by developing a unified approach that bridges both paradigms.

Method: Proposes an exact continuous-time dynamics equation that defines state transitions across any finite time interval, creating Transition Models (TiM) that adapt to arbitrary-step transitions from single leaps to fine-grained refinement.
...
Conclusion: TiM provides a novel generative paradigm that resolves the step-quality conflict, offering both efficiency and high fidelity with flexible sampling strategies.
...
TL;DR: IPA improves LoRA by using feature-aware projections instead of random initialization, achieving better performance with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: LoRA's random down-projection discards useful information and becomes a bottleneck, as it changes little during training while up-projection does most adaptation work.

Method: IPA framework preserves information in reduced hidden space using feature-aware projections, approximating top principal components for efficient pretraining with minimal inference overhead.

Result: IPA consistently outperforms LoRA and DoRA across benchmarks: +1.5 points on commonsense reasoning, +2.3 points on VTAB-1k, matches full LoRA performance with half the trainable parameters when projection is frozen.

Conclusion: Feature-aware projections significantly improve parameter-efficient fine-tuning by preserving important information, making IPA a superior alternative to random initialization methods like LoRA.
...
Result: HCL achieves superior performance in both clustering and structure learning tasks, and successfully recovers biologically meaningful mechanisms in real-world single-cell perturbation data.
...
TL;DR: Unified framework connecting RL and SFT approaches through a single optimization process with a Unified Policy Gradient Estimator, leading to Hybrid Post-Training (HPT) algorithm that outperforms baselines on reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between online (model-generated) and offline (human demonstrations) training data approaches, showing they are not contradictory but instances of a unified optimization process.

Method: Derived Unified Policy Gradient Estimator with four interchangeable components, then developed Hybrid Post-Training (HPT) algorithm that dynamically selects training signals for effective exploitation and stable exploration.

Result: HPT consistently surpasses strong baselines across six mathematical reasoning benchmarks and two out-of-distribution suites, working effectively across models of varying scales and families.

Conclusion: The unified theoretical framework successfully connects different post-training approaches, and HPT demonstrates practical effectiveness in balancing demonstration exploitation and exploration stability.
...
TL;DR: This paper reframes Echo State Networks (ESNs) as state-space models, providing theoretical foundations and new analysis methods for reservoir computing systems.


<details>
  <summary>Details</summary>
Motivation: ESNs are typically designed using heuristics rather than first principles. The authors aim to provide a unified systems-theoretic framework that connects reservoir computing with classical identification and modern kernelized state-space models.

Method: The authors: 1) Show echo-state property as input-to-state stability for contractive nonlinear SSMs; 2) Develop small-signal linearizations and lifted/Koopman random-feature expansions; 3) Cast teacher forcing as state estimation and propose Kalman/EKF-assisted readout learning with EM for hyperparameter optimization.

Result: The paper provides verifiable stability conditions, frequency-domain characterizations of memory spectra, and clarifies when ESNs emulate structured SSM kernels. It also enables transfer-function and convolutional-kernel analyses.

Conclusion: Recasting ESNs as state-space models provides a principled theoretical foundation, enabling better analysis, design, and optimization of reservoir computing systems through established systems theory and control principles.
...
Method: Analyzes how beneficial design choices in tabular DL (numerical feature embeddings, retrieval-augmented models, advanced ensembling) implicitly handle data uncertainty.

Result: Provides a unifying understanding of recent performance improvements and develops more effective numerical feature embeddings based on data-uncertainty insights.

Conclusion: The data-uncertainty perspective paves the way for foundational understanding of modern tabular methods, enables concrete advancements, and outlines future research directions.
...
TL;DR: Delta Activations method represents finetuned LLMs as vector embeddings by measuring activation shifts from base models, enabling effective clustering and analysis of model relationships.
...
Conclusion: Delta Activations provide an effective way to structure and understand the landscape of finetuned models, facilitating better reuse of publicly available LLMs.
...
Result: The proposed models demonstrated efficacy in learning interpretable human decision models for kidney allocation tasks, matching or surpassing accuracy of prior pairwise decision-making models.

Conclusion: The structured processing approach provides more cognitively faithful models that better represent underlying human decision-making processes compared to standard elicitation methods.
...
TL;DR: ChronoGraph is a new dataset for multivariate time series forecasting and anomaly detection in microservice systems, featuring real-world service metrics, dependency graphs, and incident annotations.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack the combination of multivariate time series, explicit dependency graphs, and real incident labels needed for realistic evaluation of forecasting and anomaly detection in microservice environments.

Method: Built from real production microservices, with nodes representing services emitting performance metrics (CPU, memory, network) and directed edges encoding service dependencies, plus expert-annotated incident windows.

Result: Provides baseline results for forecasting models, pretrained time-series foundation models, and standard anomaly detectors on this new benchmark dataset.

Conclusion: ChronoGraph offers a realistic benchmark for studying structure-aware forecasting and incident-aware evaluation in microservice systems, addressing limitations of existing datasets.
...
### [224] [Simplicity Lies in the Eye of the Beholder: A Strategic Perspective on Controllers in Reactive Synthesis](https://arxiv.org/abs/2509.04129)
*Mickael Randour*
...
<details>
  <summary>Details</summary>
Motivation: Simple strategies with limited memory are believed to produce better controllers that are easier to understand, implement, and maintain, but the actual complexity requirements need systematic analysis.

Method: Game-theoretic approach to controller synthesis, examining the interaction between system and environment as a game, and analyzing strategy complexity through recent research findings on memory usage and randomness.

Result: Recent results reveal insights about memory and randomness requirements in various synthesis contexts, showing that traditional notions of strategy complexity may need expansion.

Conclusion: The complexity of strategies in controller synthesis involves more than just traditional measures, requiring consideration of memory, randomness, and potentially other factors beyond conventional complexity metrics.
...
### [225] [Finetuning AI Foundation Models to Develop Subgrid-Scale Parameterizations: A Case Study on Atmospheric Gravity Waves](https://arxiv.org/abs/2509.03816)
...
<details>
  <summary>Details</summary>
Motivation: Subgrid-scale closures for unresolved atmospheric-oceanic processes like gravity waves are a major source of uncertainty in climate models, and foundation models remain largely unexplored for climate research applications.

Method: Fine-tuning a 2.3B parameter encoder-decoder foundation model (NASA/IBM's Prithvi WxC) to learn gravity wave fluxes from high-resolution atmospheric reanalysis data, comparing against an Attention U-Net baseline.

Result: The foundation model parameterization showed superior predictive performance throughout the atmosphere (Hellinger distance 0.06 vs 0.11 for baseline), even in regions excluded from pre-training.

Conclusion: Foundation models offer versatile reusability for creating observation-driven, physically accurate parameterizations across various earth-system processes, representing a promising new approach for climate modeling.
...
### [226] [RepoDebug: Repository-Level Multi-Task and Multi-Language Debugging Evaluation of Large Language Models](https://arxiv.org/abs/2509.04078)
...
<details>
  <summary>Details</summary>
Motivation: Existing debugging datasets focus on function-level code repair and lack diversity in tasks, languages, and error types, providing an incomplete understanding of LLMs' challenges in realistic repository-level debugging scenarios.

Method: The paper introduces RepoDebug - a multi-task, multi-language repository-level debugging dataset with 22 error subtypes, supporting 8 programming languages and 3 debugging tasks, then evaluates 10 LLMs on this dataset.

Result: Evaluation shows that Claude 3.5 Sonnet, the best-performing model among the 10 tested LLMs, still performs poorly in repository-level debugging, indicating significant challenges remain.

Conclusion: Repository-level debugging presents substantial challenges for current LLMs that are not captured by function-level datasets, highlighting the need for more comprehensive evaluation frameworks like RepoDebug to advance code debugging capabilities.
...
### [227] [An Empirical Study of Vulnerabilities in Python Packages and Their Detection](https://arxiv.org/abs/2509.04260)
*Haowei Quan,Junjie Wang,Xinzhe Li,Terry Yue Zhuo,Xiao Chen,Xiaoning Du*

Main category: cs.SE

TL;DR: PyVul is the first comprehensive benchmark suite with 1,157 verified Python package vulnerabilities, revealing limitations in current detection tools and showing multi-lingual packages are more vulnerable.


<details>
  <summary>Details</summary>
Motivation: Python packages have significant security vulnerabilities that are complex due to language interoperability, and current detection tools' effectiveness is underexplored.

Method: Created PyVul benchmark with 1,157 developer-verified vulnerabilities, using LLM-assisted data cleansing for accuracy. Conducted distribution analysis and evaluated state-of-the-art detectors.
...
Conclusion: PyVul provides the most precise large-scale Python vulnerability benchmark, highlighting the need for improved detection tools to address real-world security issues in Python packages.
...
TL;DR: EWFM enables continuous normalizing flows to model Boltzmann distributions using only energy evaluations, requiring significantly fewer computations than existing methods.


<details>
  <summary>Details</summary>
Motivation: Sampling from complex Boltzmann distributions is computationally challenging, and existing generative approaches either require large datasets of target samples or cannot effectively use advanced architectures like continuous normalizing flows with only energy evaluations.

Method: Energy-Weighted Flow Matching (EWFM) reformulates conditional flow matching via importance sampling, allowing training with samples from arbitrary proposal distributions. Two algorithms: iterative EWFM (iEWFM) for progressive proposal refinement, and annealed EWFM (aEWFM) with temperature annealing for challenging landscapes.

Result: On benchmark systems including 55-particle Lennard-Jones clusters, the algorithms achieve sample quality competitive with state-of-the-art energy-only methods while requiring up to three orders of magnitude fewer energy evaluations.

Conclusion: EWFM provides an efficient framework for modeling Boltzmann distributions using only energy function evaluations, significantly reducing computational costs while maintaining high sample quality.
...
TL;DR: This paper proposes a dimension reduction method for diffusion models that integrates compressed sensing to accelerate both training and inference for synthetic data generation.


<details>
  <summary>Details</summary>
Motivation: To address the computational inefficiency of diffusion models in synthetic data generation by reducing dimensionality through compressed sensing techniques.

Method: Three-step approach: (1) compress data into latent space, (2) train diffusion model in latent space, (3) apply compressed sensing algorithm to generated latent samples for efficient reconstruction.

Result: Proven faster convergence under sparsity assumptions, optimal latent dimension determination, and successful numerical experiments on diverse datasets including images and financial time series.

Conclusion: The integration of compressed sensing with diffusion models provides an effective framework for accelerating synthetic data generation while maintaining performance across various data types.
...
<details>
  <summary>Details</summary>
Motivation: To assess whether nodal covariates are correlated with network structure in applications where networks have node-level features, providing computationally efficient alternatives to existing approaches.

Method: Two methods use linear models relating node covariates to latent variables driving network structure. Two methods apply canonical correlation analysis between node features and network structure without linear modeling assumptions.

Result: Theoretical guarantees provided for low-rank latent space models with high-dimensional covariates. Methods are computationally cheaper and require fewer modeling assumptions than previous approaches.

Conclusion: Four effective methods developed for network dependency testing that perform well on both simulated and real-world data, offering improved computational efficiency and reduced modeling requirements.
...
<details>
  <summary>Details</summary>
Motivation: To develop a generative model that can handle both simulation (sampling from likelihood) and inference (sampling from posterior) in Bayesian inverse problems

Method: Combines two triangular normalizing flows (upper and lower maps) into one invertible mapping, with proposed training loss for direct map training

Result: Developed theoretical properties of the invertible generative model and demonstrated its effectiveness on stylized numerical examples

Conclusion: The approach provides a unified framework for conditional generative modeling that can perform both simulation and inference tasks effectively
...
<details>
  <summary>Details</summary>
Motivation: To address the NP-hard combinatorial optimization problem in stochastic matching where naive approaches incur O(K^N) computational cost per round, making real-world applications infeasible.

Method: Proposed batched algorithms that limit matching update frequency, reducing amortized computational cost to O(1) while maintaining optimal regret performance.

Result: Achieved Õ(√T) regret bound (optimal up to logarithmic factors) with significantly reduced computational complexity of O(1) amortized cost per round.

Conclusion: The batched approach successfully bridges the gap between computational efficiency and regret performance in MNL-based stochastic matching problems, making the framework practical for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: To clarify the intimate connections and equivalences between widely used post-training techniques (RLHF, RLIF, test-time scaling) and reveal intrinsic links between diffusion guidance and test-time scaling methods.
...
Result: The paper demonstrates equivalences between reinforcement learning with human feedback (RLHF), reinforcement learning with internal feedback (RLIF), and soft best-of-N sampling (test-time scaling), while also connecting diffusion guidance with test-time scaling.

Conclusion: The analysis reveals fundamental connections between various post-training alignment techniques and provides a novel resampling approach that eliminates the need for explicit reinforcement learning methods in alignment tasks.
...
### [234] [An exact multiple-time-step variational formulation for the committor and the transition rate](https://arxiv.org/abs/2509.03539)
...
<details>
  <summary>Details</summary>
Motivation: Existing committor estimation methods from trajectory data are biased when using practical lag times, as they only minimize correctly for single time steps, leading to inaccurate transition rate estimates.

Method: Introduced an alternative expression that is minimized by the exact committor at any lag time, derived additional transition rate expressions, and related to variational approaches using mean-squared residuals with error decomposition analysis.

Result: Numerical tests on benchmark systems show the new committor and transition rate estimates are much less sensitive to lag time choice compared to existing methods.

Conclusion: The proposed method provides unbiased committor estimation for any lag time, significantly improving the robustness and accuracy of transition rate calculations from trajectory data.
...
### [235] [ARDO: A Weak Formulation Deep Neural Network Method for Elliptic and Parabolic PDEs Based on Random Differences of Test Functions](https://arxiv.org/abs/2509.03757)
*Wei Cai,Andrew Qing He*

Main category: math.NA

TL;DR: ARDO method: derivative-free deep learning approach for PDEs using weak adversarial formulation with random difference operators on test functions, especially effective for Fokker-Planck type equations.
...
Method: Uses weak adversarial formulation where random difference operators are applied to test functions instead of the solution network, creating a fully derivative-free framework with respect to the neural network solution.

Result: The method is particularly effective for second-order elliptic and parabolic PDEs of Fokker-Planck type, providing a derivative-free alternative to traditional deep learning approaches for PDE solving.

Conclusion: ARDO method offers a novel derivative-free deep learning framework for PDEs that shows strong potential for Fokker-Planck type equations and could be extended to other PDE-related problems.
...
### [236] [Evaluating the Robustness of Retrieval-Augmented Generation to Adversarial Evidence in the Health Domain](https://arxiv.org/abs/2509.03787)
*Shakiba Amirshahi,Amin Bigdeli,Charles L. A. Clarke,Amira Ghenai*

Main category: cs.IR

TL;DR: RAG systems are vulnerable to misinformation from retrieved evidence, especially adversarial content in health domain. Robustness can be maintained when helpful evidence is present alongside harmful content.


<details>
  <summary>Details</summary>
Motivation: RAG systems can reduce LLM hallucinations but introduce vulnerability to misinformation, particularly in high-stakes domains like healthcare where incorrect responses can cause harm.

Method: Controlled experiments with health questions, varying document types (helpful, harmful, adversarial) and question framing (consistent, neutral, inconsistent), measuring alignment between model outputs and ground-truth answers.

Result: Adversarial documents significantly degrade alignment, but robustness is preserved when helpful evidence is also present in the retrieval pool.

Conclusion: Retrieval safeguards are needed for safer RAG systems in high-stakes domains, and the findings provide actionable insights for system design.
...
### [237] [NER Retriever: Zero-Shot Named Entity Retrieval with Type-Aware Embeddings](https://arxiv.org/abs/2509.04011)
*Or Shachar,Uri Katz,Yoav Goldberg,Oren Glickman*

Main category: cs.IR

TL;DR: NER Retriever is a zero-shot framework that uses LLM internal representations for schema-free entity retrieval by aligning entity mentions and type descriptions in semantic space.


<details>
  <summary>Details</summary>
Motivation: Traditional NER requires predefined entity types, but real-world applications need flexible retrieval based on user-defined descriptions without fixed schemas or fine-tuned models.

Method: Uses mid-layer transformer block value vectors from LLMs to embed entities and type descriptions, then trains lightweight contrastive projection network to align compatible types in semantic space.
...
Conclusion: Demonstrates effective use of LLM internal representations for scalable schema-free entity retrieval, with empirical support for representation selection strategies within language models.
...
### [238] [ACT: Automated Constraint Targeting for Multi-Objective Recommender Systems](https://arxiv.org/abs/2509.03661)
...
<details>
  <summary>Details</summary>
Motivation: Recommender systems need to maximize primary objectives while ensuring secondary guardrails meet minimum thresholds, but enforcing these constraints through manual hyperparameter tuning is challenging and doesn't adapt well to system changes.

Method: Automated Constraint Targeting (ACT) framework uses offline pairwise evaluation on unbiased data to find minimal hyperparameter changes, with continuous retraining to adapt to system and user behavior changes.

Result: The framework demonstrates empirical efficacy and has been successfully deployed in a large-scale production environment.

Conclusion: ACT provides an automated solution for maintaining guardrail constraints in recommender systems without requiring manual tuning, adapting continuously to system changes.
...
### [239] [Global-to-Local or Local-to-Global? Enhancing Image Retrieval with Efficient Local Search and Effective Global Re-ranking](https://arxiv.org/abs/2509.04351)
*Dror Aiger,Bingyi Cao,Kaifeng Chen,Andre Araujo*

Main category: cs.IR

TL;DR: Introduces a local-to-global retrieval paradigm that combines efficient local feature search with global feature re-ranking, achieving state-of-the-art results on standard datasets.


<details>
  <summary>Details</summary>
Motivation: Traditional global-to-local image retrieval systems miss partial matches that local features can detect, while emerging efficient local feature search and global re-ranking techniques enable a new paradigm.

Method: Proposes a local-to-global approach where efficient local feature search is followed by global feature re-ranking computed on-the-fly using multidimensional scaling to create embeddings that respect local similarities.

Result: Demonstrates solid retrieval performance and sets new state-of-the-art results on the Revisited Oxford and Paris datasets.

Conclusion: The local-to-global paradigm effectively combines the strengths of both local and global feature approaches, enabling better partial match retrieval while maintaining computational efficiency.
...
### [240] [LLM-based Relevance Assessment for Web-Scale Search Evaluation at Pinterest](https://arxiv.org/abs/2509.03764)
...
Method: Fine-tuned large language models (LLMs) to generate relevance judgments, rigorously validated against human annotations for alignment.

Result: LLMs provide reliable relevance measurements comparable to human judgments, significantly improve evaluation efficiency, reduce Minimum Detectable Effect (MDE), and enable expanded query sets and optimized sampling.

Conclusion: LLM-based automation successfully replaces human annotation for relevance evaluation, enabling scalable, efficient, and high-quality assessment of search experiences while reducing costs and turnaround time.
...
### [241] [Enhancing Technical Documents Retrieval for RAG](https://arxiv.org/abs/2509.04139)
*Songjiang Lai,Tsun-Hin Cheung,Ka-Chun Fung,Kaiwen Xue,Kwan-Ho Lin,Yan-Ming Choi,Vincent Ng,Kin-Man Lam*

Main category: cs.IR

TL;DR: Technical-Embeddings framework enhances semantic retrieval in technical documentation using LLMs, query expansion, and contextual summarization, outperforming baselines on technical datasets.
...
Method: Leverage LLMs for query expansion to capture user intent, apply summary extraction for contextual encoding, and fine-tune bi-encoder BERT model with soft prompting and separate parameters for queries/documents.
...
### [242] [Decoupled Entity Representation Learning for Pinterest Ads Ranking](https://arxiv.org/abs/2509.04337)
...
TL;DR: Novel upstream-downstream framework for user/Pin embeddings using diverse data sources and complex architectures, deployed in Pinterest's ad systems with significant performance gains.


<details>
  <summary>Details</summary>
Motivation: To deliver personalized Pins and ads effectively by constructing high-quality user and item embeddings from diverse data sources.

Method: Upstream models trained on extensive data with varied signals using complex architectures to capture user-Pin relationships. Entity embeddings are learned and refreshed asynchronously, then integrated into downstream tasks like ad retrieval and ranking models.

Result: Achieves notable performance improvements in both offline and online settings across various downstream tasks. Deployed in production ad ranking systems with significant gains in online metrics.

Conclusion: The framework successfully enables effective personalized content delivery through scalable upstream embedding generation and downstream integration, demonstrating substantial real-world impact.
...
### [243] [Combining feature-based approaches with graph neural networks and symbolic regression for synergistic performance and interpretability](https://arxiv.org/abs/2509.03547)
...
Method: Integrates latent representations from diverse pretrained GNN models (MEGNet, ROOST, ORB) with GNN-approximated descriptors and symbolic regression features. Augments feature-based model MODNet on Matbench tasks with an interpretability module using surrogate models and symbolic regression.

Result: Significant error reductions with accuracy increases exceeding 40% for multiple tasks. Performance becomes competitive with and often superior to state-of-the-art end-to-end GNNs.

Conclusion: MatterVial provides a unified framework that advances materials informatics by offering high performance with explainable AI principles, enabling more targeted and autonomous materials discovery.
...
### [244] [Deficiency of equation-finding approach to data-driven modeling of dynamical systems](https://arxiv.org/abs/2509.03769)
...
<details>
  <summary>Details</summary>
Motivation: To investigate how imperfect data (due to disturbances and measurement errors) affects the discovery of governing equations in chaotic systems using sparse optimization methods.

Method: Analyzed sparse-optimization methods for equation discovery, examined sensitivity to measurement procedures, compared generated chaotic attractors, and calculated Koopman spectra to identify eigenvalue differences.

Result: Different equation sets from sparse optimization produce virtually identical chaotic attractors despite being sensitive to measurement procedures. They agree on large eigenvalues but diverge for eigenvalues smaller than equation-dependent thresholds.
...
### [245] [Exoplanetary atmospheres retrieval via a quantum extreme learning machine](https://arxiv.org/abs/2509.03617)
*Marco Vetrano,Tiziano Zingales,G. Massimo Palma,Salvatore Lorenzo*

Main category: quant-ph

TL;DR: Quantum extreme learning machines (QELMs) are used for exoplanetary atmospheric retrieval, offering a fault-tolerant quantum computing approach that reduces computational overhead compared to traditional forward modeling methods.
...
Method: The paper introduces a framework using quantum extreme learning machines (QELMs) that employ quantum systems as black boxes for processing input data, with an intrinsically fault-tolerant strategy suitable for near-term quantum devices like IBM Fez.

Result: The QELM architecture demonstrates fault tolerance and shows potential for quantum computing in astrophysical data analysis, enabling faster and more efficient atmospheric retrieval.

Conclusion: Quantum extreme learning machines present a promising near-term solution for exoplanetary atmospheric analysis, potentially unlocking new computational tools for faster, more efficient, and more accurate modeling of exoplanetary atmospheres.
...
### [246] [A Small Dataset May Go a Long Way: Process Duration Prediction in Clinical Settings](https://arxiv.org/abs/2509.03522)
*Harald Störrle,Anastasia Hort*

Main category: stat.AP

TL;DR: Simple central tendency methods perform as well as complex models for predicting surgery durations, enabling effective optimization with small datasets through expert knowledge integration.


<details>
  <summary>Details</summary>
Motivation: Optimizing operating theater utilization through better surgery scheduling can reduce hospital costs and improve medical outcomes, but previous complex models require large datasets.

Method: Combined factor analysis and regression models using practitioner expertise and theory-guided data analysis to predict perioperative process duration with limited clinical data.

Result: Simple central tendency methods performed on par with or sometimes outperformed more complex methods from literature, showing expert knowledge integration improves data quality and model performance.

Conclusion: Integrating conventional data science with qualitative clinical studies yields better results than previous approaches, enabling effective prediction even with small datasets.
...
### [247] [SAMVAD: A Multi-Agent System for Simulating Judicial Deliberation Dynamics in India](https://arxiv.org/abs/2509.03793)
...
<details>
  <summary>Details</summary>
Motivation: Empirical studies of judicial panels face ethical and practical barriers, requiring a simulation platform to understand judicial deliberation complexities.

Method: Multi-Agent System with Judge, Prosecution Counsel, Defense Counsel, and Adjudicator agents powered by LLMs, integrated with RAG using Indian Penal Code and Constitution knowledge base.

Result: System enables legally sound instructions/arguments with source citations, iterative deliberation rounds, and consensus-based verdicts with enhanced fidelity and transparency.

Conclusion: Provides configurable, explainable MAS platform for exploring legal reasoning and group decision-making in Indian judicial context with verifiable legal grounding.
...
### [248] [Divergence-Kernel method for linear responses and diffusion models](https://arxiv.org/abs/2509.03992)
*Angxiu Ni*
...
<details>
  <summary>Details</summary>
Motivation: To develop a general formula for computing linear responses (parameter-derivatives of distributions) in random dynamical systems that works for multiplicative and parameterized noise without requiring hyperbolicity assumptions.

Method: Derives divergence-kernel formula for linear response, formally passes to continuous-time limit, develops pathwise Monte-Carlo algorithm for computing linear responses.

Result: Successfully derives the general formula that works for various noise types and time periods, and implements a forward-only diffusion generative model.

Conclusion: The proposed divergence-kernel formula and pathwise Monte-Carlo algorithm provide effective tools for computing linear responses in random dynamical systems, with applications in diffusion generative modeling.
...
### [249] [How many patients could we save with LLM priors?](https://arxiv.org/abs/2509.04250)
*Shota Arai,David Selby,Andrew Vargo,Sebastian Vollmer*

Main category: stat.ME

TL;DR: A novel hierarchical Bayesian framework that uses LLM-informed priors to reduce patient numbers needed in clinical trials while maintaining statistical power.


<details>
  <summary>Details</summary>
Motivation: To leverage large language models' encoded clinical knowledge to improve efficiency of clinical trials and enable more robust safety assessment with fewer patients.

Method: Systematically elicits informative priors for hyperparameters in hierarchical Bayesian models using pre-trained LLMs, obtaining parametric priors directly rather than generating synthetic data.

Result: LLM-derived priors consistently improve predictive performance compared to traditional meta-analytical approaches, as demonstrated through temperature sensitivity analysis and cross-validation on real clinical trial data.

Conclusion: This approach enables substantial reductions in required patient numbers for robust safety assessment and has potential to transform drug safety monitoring and regulatory decision making.
...
### [250] [Contextualized Token Discrimination for Speech Search Query Correction](https://arxiv.org/abs/2509.04393)
*Junyu Lu,Di Jiang,Mengze Hong,Victor Junqiu Wei,Qintian Guo,Zhiyang Su*

Main category: cs.SD

TL;DR: A novel Contextualized Token Discrimination (CTD) method for speech query correction using BERT contextualized representations and composition layers to correct ASR errors.
...
Conclusion: CTD effectively addresses speech query correction challenges and provides a strong foundation for audio query correction research with demonstrated superior performance.
...
### [251] [AUDETER: A Large-scale Dataset for Deepfake Audio Detection in Open Worlds](https://arxiv.org/abs/2509.04345)
*Qizhou Wang,Hanxun Huang,Guansong Pang,Sarah Erfani,Christopher Leckie*

Main category: cs.SD

TL;DR: AUDETER is a large-scale deepfake audio dataset with 4,500+ hours of synthetic audio from 11 TTS models and 10 vocoders, designed to address domain shift issues in deepfake detection and improve generalization performance.


<details>
  <summary>Details</summary>
Motivation: Current deepfake detection methods struggle with real-world effectiveness due to domain shift between training and test samples, and existing datasets lack diverse, up-to-date audio samples from both real and deepfake categories.

Method: Created AUDETER dataset with over 4,500 hours of synthetic audio (3 million clips) generated by 11 recent TTS models and 10 vocoders, providing comprehensive coverage of TTS/vocoder patterns for robust model development.

Result: SOTA methods trained on existing datasets show poor generalization to novel deepfake samples and high false positive rates. Methods trained on AUDETER achieved 44.1-51.6% reduction in detection error rate, reaching only 4.17% error on cross-domain samples.

Conclusion: AUDETER enables highly generalized deepfake detection performance and significantly reduces error rates, paving the way for training generalist deepfake audio detectors that work effectively in real-world scenarios.
...
### [252] [Hardware-Aware Data and Instruction Mapping for AI Tasks: Balancing Parallelism, I/O and Memory Tradeoffs](https://arxiv.org/abs/2509.03846)
*Md Rownak Hossain Chowdhury,Mostafizur Rahman*

Main category: cs.AR

TL;DR: A streaming-based mapping framework for deep learning inference that pre-plans computation and communication to reduce host involvement and off-chip memory usage, achieving high utilization and throughput.


<details>
  <summary>Details</summary>
Motivation: To reduce reliance on I/O, off-chip memory, and host control in deep learning inference by leveraging predictable neural network behavior for ahead-of-time planning.

Method: Generates unified stream of instructions and data for hardware execution, uses fine-grained message passing on programmable architecture with techniques like stationary-weight reuse, in-array multicasting, and staged reductions.

Result: Achieves 88-92% utilization on VGG-19, with 97% internal message generation and 89% on-chip transfers. Scales beyond 1 TFLOP/s with traffic reductions up to 100 MB per layer.

Conclusion: The framework effectively enables streaming-based computation through tight coordination of data and instruction flow, demonstrating significant performance improvements for deep learning inference.
...
TL;DR: A new computational framework using physics-constrained neural fields and differentiable rendering enables high-speed 3D reconstruction of protoplanetary disk structures from ALMA observations, revealing previously undetected features.


<details>
  <summary>Details</summary>
Motivation: Protoplanetary disks are crucial for planet formation, but traditional methods cannot capture their complex 3D structure at the unprecedented resolution provided by ALMA observations.

Method: Developed RadJAX - a GPU-accelerated, fully differentiable line radiative transfer solver that integrates physics-constrained neural fields with differentiable rendering, achieving 10,000x speedups over conventional ray tracers.
...
Conclusion: This work establishes a new paradigm for extracting complex disk structures and advancing understanding of protoplanetary evolution through high-dimensional neural reconstructions.
...
### [254] [Gravity Well Echo Chamber Modeling With An LLM-Based Confirmation Bias Model](https://arxiv.org/abs/2509.03832)
...
Method: Extended established gravity well model by adding dynamic confirmation bias variable calculated from user posting history and responses to diverse viewpoints

Result: Improved echo chamber detection on nineteen Reddit communities, revealing community-level markers of information health
...
### [255] [BiND: A Neural Discriminator-Decoder for Accurate Bimanual Trajectory Prediction in Brain-Computer Interfaces](https://arxiv.org/abs/2509.03521)
*Timothee Robert,MohammadAli Shaeri,Mahsa Shoaran*

Main category: q-bio.NC

TL;DR: BiND is a two-stage model that classifies motion type then decodes continuous hand velocities, outperforming 6 state-of-the-art models in bimanual movement decoding from intracortical recordings.


<details>
  <summary>Details</summary>
Motivation: Decoding bimanual hand movements is challenging due to overlapping neural representations and nonlinear interlimb interactions in BCIs.

Method: Two-stage approach: first classifies motion type (unimanual left/right or bimanual), then uses specialized GRU-based decoders with trial-relative time index for continuous 2D velocity prediction.

Result: Achieves mean R² of 0.76 (±0.01) for unimanual and 0.69 (±0.03) for bimanual prediction, surpassing next-best GRU model by 2% in both tasks. Shows 4% better robustness to session variability.

Conclusion: Task-aware discrimination and temporal modeling effectively enhance bimanual decoding performance and robustness in BCIs.
...
### [256] [A software security review on Uganda's Mobile Money Services: Dr. Jim Spire's tweets sentiment analysis](https://arxiv.org/abs/2509.03545)
*Nsengiyumva Wilberforce*

Main category: cs.CY

TL;DR: Analysis of #StopAirtelThefty Twitter campaign reveals public security concerns about Uganda's mobile money systems following a theft incident where a phone thief accessed accounts, withdrew funds, and took loans.
...
Result: Revealed deep-seated public anxiety over mobile money safety, identified specific security gaps experienced by users, and connected these findings to Uganda's mobile money regulatory and operational environment.

Conclusion: Provides crucial insights for mobile money providers and policymakers to address security vulnerabilities and improve the future of secure digital finance in Uganda.
...
### [257] [No Thoughts Just AI: Biased LLM Recommendations Limit Human Agency in Resume Screening](https://arxiv.org/abs/2509.04404)
*Kyra Wilson,Mattea Sim,Anna-Maria Gueorguieva,Aylin Caliskan*

Main category: cs.CY

TL;DR: Study shows people adopt AI's racial biases in hiring decisions, with implicit association tests reducing bias by 13%. Even when users distrust AI, they're still influenced by its biased recommendations.


<details>
  <summary>Details</summary>
Motivation: To investigate how human decision-makers are influenced by AI systems exhibiting racial bias in hiring scenarios, and whether implicit association tests can mitigate this influence.

Method: Conducted resume-screening experiment (N=528) with simulated AI models showing race-based preferences. Used quality-controlled resumes with racial cues through names/affinity groups across 1,526 scenarios. Measured unconscious associations with implicit association tests (IATs).

Result: People select candidates equally without biased AI, but adopt AI's racial preferences up to 90% of the time when AI is biased. IAT completion before screening increases selection of candidates against race-status stereotypes by 13%. Users remain vulnerable to AI bias even when they perceive recommendations as low quality.

Conclusion: Human-AI collaboration in hiring is vulnerable to bias adoption. Organizations need policies acknowledging complex AI-human decision making, education for users, and oversight mechanisms. IATs show promise for bias mitigation.
...
### [258] [Hypothesis Selection: A High Probability Conundrum](https://arxiv.org/abs/2509.03734)
*Anders Aamand,Maryam Aliakbarpour,Justin Y. Chen,Sandeep Silwal*

Main category: cs.DS

TL;DR: Improved time complexity for hypothesis selection from O~(n/(δ³ε³)) to O~(n/(δε²)) while maintaining optimal sample complexity and approximation factor C=3. Also studied three alternative settings: optimal approximation for expected distance, known OPT case with improved runtime, and preprocessing approach for subquadratic runtime.


<details>
  <summary>Details</summary>
Motivation: Despite decades of research, key aspects of hypothesis selection problem remain unresolved, particularly optimal running time for algorithms achieving optimal sample complexity and best approximation factor. Previous state-of-the-art had suboptimal dependence on confidence and error parameters.

Method: Developed improved algorithms for hypothesis selection that reduce time complexity while maintaining optimal performance. Explored three alternative settings: expected distance analysis, known OPT case optimization, and preprocessing approaches.

Result: Achieved time complexity of O~(n/(δε²)), significantly improving over previous O~(n/(δ³ε³)). Settled optimal approximation factor for expected distance case. For known OPT case, obtained C=3 with runtime O~(n/ε²) and optimal sample complexity. With preprocessing, achieved C=3 with subquadratic runtime.

Conclusion: The paper provides significant improvements in time complexity for hypothesis selection while maintaining optimal approximation factors and sample complexity. The results resolve several open questions and offer practical algorithms for different problem variants with better computational efficiency.
...
### [259] [Designing Gaze Analytics for ELA Instruction: A User-Centered Dashboard with Conversational AI Support](https://arxiv.org/abs/2509.03741)
...
TL;DR: Iterative design of gaze-based learning analytics dashboard for ELA using eye-tracking data, featuring LLM-powered conversational agent for natural language interaction with gaze analytics.


<details>
  <summary>Details</summary>
Motivation: Eye-tracking provides valuable insights into student cognition and engagement but remains underutilized in educational technology due to data interpretation challenges and accessibility issues.

Method: User-centered design approach with five iterative studies involving teachers and students, guided by data storytelling principles. Developed gaze analytics dashboard with familiar visualizations, layered explanations, narrative scaffolds, and LLM-powered conversational agent.

Result: Gaze analytics proved approachable and pedagogically valuable when supported by appropriate visualizations and explanations. The conversational agent successfully lowered cognitive barriers to interpreting gaze data through natural language interactions.

Conclusion: Design implications for integrating novel data modalities like eye-tracking in classroom EdTech systems, emphasizing the importance of familiar interfaces and natural language interaction capabilities.
...
### [260] [Unobtrusive In-Situ Measurement of Behavior Change by Deep Metric Similarity Learning of Motion Patterns](https://arxiv.org/abs/2509.04174)
...
Conclusion: The approach provides advantages including in-situ measurement without user input, generalizable motion analysis, user-specific evaluation, and real-time capability for studying avatar-induced behavior changes.
...
### [261] [HumAIne-Chatbot: Real-Time Personalized Conversational AI via Reinforcement Learning](https://arxiv.org/abs/2509.04303)
...
TL;DR: HumAIne-chatbot is an AI conversational agent that personalizes responses using user profiling and reinforcement learning, showing significant improvements in user satisfaction and personalization accuracy.


<details>
  <summary>Details</summary>
Motivation: Current conversational AI systems provide generic interactions that overlook individual user characteristics and lack adaptive dialogue management.

Method: The system uses a novel user profiling framework pre-trained on GPT-generated virtual personas, combined with online reinforcement learning that refines per-user models using implicit signals (typing speed, sentiment, engagement) and explicit feedback (likes/dislikes) to dynamically inform dialogue policy.

Result: Controlled experiments with 50 synthetic personas showed consistent improvements in user satisfaction, personalization accuracy, and task achievement, with statistical analysis confirming significant differences and large effect sizes between personalized and non-personalized conditions.
...
### [262] [LibriQuote: A Speech Dataset of Fictional Character Utterances for Expressive Zero-Shot Speech Synthesis](https://arxiv.org/abs/2509.04072)
*Gaspard Michel,Elena V. Epure,Christophe Cerisara*

Main category: eess.AS

TL;DR: LibriQuote dataset provides 18K hours of English speech (12.7K neutral + 5.3K expressive) from audiobooks with context annotations, enabling expressive TTS fine-tuning and benchmarking.


<details>
  <summary>Details</summary>
Motivation: Existing large TTS datasets lack clear expressive speech proportions, and current expressive corpora are too small for effective fine-tuning of zero-shot TTS systems.

Method: Created LibriQuote dataset from read audiobooks with expressive speech subset containing character quotations, context information, and pseudo-labels from speech verbs/adverbs. Includes challenging test set for benchmarking.

Result: Fine-tuning TTS on LibriQuote significantly improves speech intelligibility. Current systems fail to match ground-truth expressiveness and naturalness despite the dataset's quality.

Conclusion: LibriQuote enables effective expressive TTS development and benchmarking, revealing current system limitations in synthesizing natural expressive speech while preserving speaker timbre.
...
### [263] [Neural Video Compression with In-Loop Contextual Filtering and Out-of-Loop Reconstruction Enhancement](https://arxiv.org/abs/2509.04051)
...
Method: Categorized enhancement filtering into in-loop contextual filtering (refines temporal context and mitigates error propagation) and out-of-loop reconstruction enhancement (refines frame quality). Introduced adaptive coding decision strategy to dynamically determine filtering application during encoding.
...
### [264] [EHVC: Efficient Hierarchical Reference and Quality Structure for Neural Video Coding](https://arxiv.org/abs/2509.04118)
*Junqi Liao,Yaojun Wu,Chaoyi Lin,Zhipin Deng,Li Li,Dong Liu,Xiaoyan Sun*

Main category: eess.IV

TL;DR: EHVC is an efficient hierarchical neural video codec that addresses reference-quality mismatch through hierarchical multi-reference scheme, enhances quality with lookahead strategy, and stabilizes quality structures with layer-wise quality scale training.


<details>
  <summary>Details</summary>
Motivation: Neural video codecs show superior coding efficiency but suffer from misalignment between reference structures and hierarchical quality structures, with room for further optimization of quality structures.

Method: Proposes three innovations: 1) hierarchical multi-reference scheme aligning reference and quality structures, 2) lookahead strategy using encoder-side context from future frames, 3) layer-wise quality scale with random quality training strategy.

Result: EHVC achieves significantly superior performance compared to state-of-the-art neural video codecs.

Conclusion: The proposed EHVC codec successfully addresses reference-quality mismatch and optimizes hierarchical quality structures, demonstrating substantial improvements in neural video coding efficiency.
...
### [265] [Accurate and scalable deep Maxwell solvers using multilevel iterative methods](https://arxiv.org/abs/2509.03622)
*Chenkai Mao,Jonathan A. Fan*

Main category: physics.comp-ph

TL;DR: Neural network surrogates combined with iterative algorithms can accurately solve PDE problems with different scales, resolutions, and boundary conditions, enabling large-scale Maxwell's equation solving and nanophotonic device design.
...
Method: Developed a subdomain neural operator model supporting arbitrary Robin-type boundary conditions, used as a flexible preconditioner in iterative algorithms. Constructed global coarse spaces for multilevel domain decomposition to enable large-scale PDE solving.

Result: Successfully trained a single network to simulate large-scale 2D Maxwell's equations with varying sizes, resolutions, wavelengths, and dielectric media distributions. Demonstrated accurate inverse design of multi-wavelength nanophotonic devices.

Conclusion: This approach presents a promising path for building accurate and scalable multi-physics surrogate solvers for large practical problems, combining neural networks with traditional iterative methods effectively.
...
### [266] [Natural Latents: Latent Variables Stable Across Ontologies](https://arxiv.org/abs/2509.03780)
*John Wentworth,David Lorell*

Main category: math.PR

TL;DR: Two Bayesian agents with different generative models can translate between their latent variables if they satisfy natural latent conditions, which are both necessary and sufficient for guaranteed translatability and robust to approximation errors.


<details>
  <summary>Details</summary>
Motivation: To determine when two agents with different latent variable structures but identical predictive distributions can functionally translate between their respective latent representations.

Method: Theoretical analysis establishing necessary and sufficient conditions (natural latent conditions) under which translation between latent variables of different generative models is guaranteed to be possible.

Result: Natural latent conditions provide both necessary and sufficient guarantees for translatability between latent variables across different generative models, with robustness to approximation errors.

Conclusion: The natural latent conditions represent the most general framework for ensuring functional translation between latent variables of different Bayesian agents that share the same predictive distribution over observables.
...
### [267] [LuxDiT: Lighting Estimation with Video Diffusion Transformer](https://arxiv.org/abs/2509.03680)
...
<details>
  <summary>Details</summary>
Motivation: Learning-based lighting estimation is constrained by scarce ground-truth HDR environment maps, which are expensive to capture and limited in diversity. Existing approaches struggle with indirect visual cues, global context inference, and HDR output recovery.

Method: Fine-tunes a video diffusion transformer on a large synthetic dataset with diverse lighting conditions. Uses low-rank adaptation finetuning strategy with collected HDR panoramas to improve semantic alignment between input and predicted environment maps.

Result: Produces accurate lighting predictions with realistic angular high-frequency details. Outperforms existing state-of-the-art techniques in both quantitative and qualitative evaluations.

Conclusion: LuxDiT effectively learns to infer illumination from indirect visual cues and generalizes well to real-world scenes, providing a superior solution for HDR environment map generation from visual input.
...
### [268] [ContraGS: Codebook-Condensed and Trainable Gaussian Splatting for Fast, Memory-Efficient Reconstruction](https://arxiv.org/abs/2509.03775)
...
<details>
  <summary>Details</summary>
Motivation: 3DGS requires large GPU memory for high-quality rendering due to many 3D Gaussians, causing inefficiencies in memory access and training/rendering speeds.

Method: Uses codebooks to store Gaussian parameters compactly and solves non-differentiable parameter learning via Bayesian inference with MCMC sampling over compressed representations.

Result: Achieves 3.49x memory reduction, 1.36x training acceleration, and 1.88x rendering speedup while maintaining near state-of-art quality.

Conclusion: ContraGS provides an effective framework for training compressed 3DGS models with significant memory and performance benefits without substantial quality loss.
...
### [269] [TensoIS: A Step Towards Feed-Forward Tensorial Inverse Subsurface Scattering for Perlin Distributed Heterogeneous Media](https://arxiv.org/abs/2509.04047)
*Ashish Tiwari,Satyam Bhardwaj,Yash Bachwana,Parag Sarvoday Sahu,T. M. Feroz Ali,Bhargava Chintalapati,Shanmuganathan Raman*

Main category: cs.GR

TL;DR: Proposes TensoIS, a learning-based framework using low-rank tensor components to estimate heterogeneous subsurface scattering parameters from multi-view images, with scattering modeled via Fractal Perlin noise.


<details>
  <summary>Details</summary>
Motivation: Estimating scattering parameters for heterogeneous media is severely under-constrained. Existing approaches either use analysis-by-synthesis or differentiable rendering, but learning-based methods assume homogeneous media. There's no known distribution to explicitly model real-world heterogeneous scattering parameters.
...
### [270] [SMooGPT: Stylized Motion Generation using Large Language Models](https://arxiv.org/abs/2509.04058)
*Lei Zhong,Yi Yang,Changjian Li*

Main category: cs.GR

TL;DR: SMooGPT uses fine-tuned LLMs for stylized motion generation through body-part text reasoning, enabling interpretable control and better generalization to new styles compared to latent space methods.


<details>
  <summary>Details</summary>
Motivation: Existing motion stylization methods suffer from low interpretability, limited control, poor generalization to new styles, and are biased toward walking motions. The authors observed that human motion can be effectively described using natural language in a body-part centric manner, and LLMs can reason about motion compositionally.

Method: Proposes SMooGPT, a fine-tuned LLM that acts as reasoner, composer, and generator using body-part text space as intermediate representation. Uses reasoning-composition-generation approach instead of latent space embeddings.

Result: The method achieves higher interpretability, enables fine-grained motion control, resolves content-style conflicts effectively, and generalizes well to new styles thanks to LLMs' open-vocabulary ability.

Conclusion: SMooGPT demonstrates effectiveness in pure text-driven stylized motion generation, outperforming existing methods through comprehensive experiments, evaluations, and user perceptual studies.
...
### [271] [Hyper Diffusion Avatars: Dynamic Human Avatar Generation using Network Weight Space Diffusion](https://arxiv.org/abs/2509.04145)
*Dongliang Cao,Guoxing Sun,Marc Habermann,Florian Bernard*

Main category: cs.GR

TL;DR: A novel two-stage method that combines person-specific rendering with diffusion modeling to generate high-quality dynamic human avatars with realistic pose-dependent deformations and real-time performance.
...
Method: Two-stage pipeline: 1) Optimize person-specific UNets for each avatar capturing pose-dependent details, 2) Train hyper diffusion model over the optimized network weights to enable weight generation during inference.

Result: Outperforms state-of-the-art human avatar generation methods, achieving high photorealism with realistic deformations using a large-scale cross-identity multi-view video dataset.

Conclusion: Successfully unites person-specific rendering quality with generative modeling generalization, enabling real-time controllable rendering of dynamic human avatars with both high quality and realistic deformations.
...
### [272] [NeuroBreak: Unveil Internal Jailbreak Mechanisms in Large Language Models](https://arxiv.org/abs/2509.03985)
*Chuhan Zhang,Ye Zhang,Bowen Shi,Yuyou Gan,Tianyu Du,Shouling Ji,Dazhan Deng,Yingcai Wu*

Main category: cs.CR

TL;DR: NeuroBreak is a top-down jailbreak analysis system that examines neuron-level safety mechanisms in large language models to identify vulnerabilities and provide insights for developing better defense strategies against jailbreak attacks.


<details>
  <summary>Details</summary>
Motivation: The increasing sophistication of jailbreak attacks that bypass safety alignment in LLMs requires deeper understanding of internal security mechanisms and vulnerabilities, which is challenging due to the models' complexity and large parameter counts.
...
Result: The system provides comprehensive analysis of various jailbreak methods, offers novel insights into generation steps, and enables deeper exploration of security mechanisms through quantitative evaluations and case studies.

Conclusion: NeuroBreak provides mechanistic insights for developing next-generation defense strategies against evolving jailbreak attacks by offering a systematic approach to analyze neuron-level safety mechanisms in LLMs.
...
### [273] [LMAE4Eth: Generalizable and Robust Ethereum Fraud Detection by Exploring Transaction Semantics and Masked Graph Embedding](https://arxiv.org/abs/2509.03939)
*Yifan Jia,Yanbin Wang,Jianguo Sun,Ye Tian,Peng Qian*

Main category: cs.CR

TL;DR: LMAE4Eth is a multi-view learning framework that combines transaction semantics, masked graph embedding, and expert knowledge to improve Ethereum fraud detection, outperforming 21 baselines by over 10% F1-score.


<details>
  <summary>Details</summary>
Motivation: Current Ethereum fraud detection methods fail to capture transaction semantics and face challenges with homogeneous transaction records and scalability issues in graph learning methods.

Method: Proposes TxCLM for linguistic transaction representations, MAGAE with generative self-supervised learning for account detection, and cross-attention fusion to combine both embeddings with layer-neighbor sampling for scalability.

Result: Outperforms 21 baseline approaches by over 10% in F1-score on two out of three datasets evaluated.

Conclusion: LMAE4Eth effectively addresses limitations of current methods by integrating semantic understanding and scalable graph learning for superior Ethereum fraud account detection.
...
### [274] [KubeGuard: LLM-Assisted Kubernetes Hardening via Configuration Files and Runtime Logs Analysis](https://arxiv.org/abs/2509.04191)
...
Method: KubeGuard uses large language models (LLMs) with modular prompt-chaining workflows to analyze manifests and runtime logs. It performs two tasks: Resource Creation (for new resources) and Resource Refinement (for existing manifests) to produce least-privilege configurations.
...
Conclusion: KubeGuard provides a practical framework that translates runtime observability into actionable, least-privilege configuration guidance, helping users enhance Kubernetes cluster security through reviewable recommendations.
...
### [275] [Efficient Virtuoso: A Latent Diffusion Transformer Model for Goal-Conditioned Trajectory Planning](https://arxiv.org/abs/2509.03658)
*Antonio Guillen-Perez*

Main category: cs.RO

TL;DR: Efficient Virtuoso is a conditional latent diffusion model for goal-conditioned trajectory planning that achieves state-of-the-art performance with minADE of 0.25 on Waymo Open Motion Dataset through a novel two-stage normalization pipeline and efficient MLP denoiser.


<details>
  <summary>Details</summary>
Motivation: Achieving high fidelity, computational efficiency, and precise control in generating diverse and plausible future trajectories for autonomous vehicle planning systems remains a significant challenge despite recent generative model advances.

Method: Uses a conditional latent diffusion model with two-stage normalization (geometric aspect ratio scaling + PCA latent space normalization), simple MLP denoiser in low-dimensional latent space, and Transformer-based StateEncoder for scene context fusion.

Result: Achieves state-of-the-art performance on Waymo Open Motion Dataset with minADE of 0.25. Ablation study shows multi-step sparse route representation enables precise tactical execution mirroring human driving behavior.

Conclusion: The method demonstrates that while single endpoint goals resolve strategic ambiguity, richer multi-step sparse routes are essential for high-fidelity tactical execution in autonomous vehicle trajectory planning.
...
### [276] [INGRID: Intelligent Generative Robotic Design Using Large Language Models](https://arxiv.org/abs/2509.03842)
*Guanglu Jia,Ceng Zhang,Gregory S. Chirikjian*

Main category: cs.RO

TL;DR: INGRID is an AI framework that automatically designs parallel robotic mechanisms using screw theory and kinematic synthesis, enabling non-experts to create custom robots and decouple AI advances from hardware constraints.


<details>
  <summary>Details</summary>
Motivation: Current robotic systems are constrained by serial mechanisms, limiting robotic intelligence. There's a need to bridge mechanism theory with machine learning to enable automated design of parallel mechanisms.

Method: Decomposes design into four tasks: constraint analysis, kinematic joint generation, chain construction, and complete mechanism design. Integrates reciprocal screw theory and kinematic synthesis methods.

Result: Successfully generates novel parallel mechanisms with both fixed and variable mobility, including configurations not previously documented. Validated through three case studies for task-specific robot design.

Conclusion: INGRID establishes mechanism intelligence where AI designs robotic hardware, potentially transforming embodied AI development by decoupling intelligence advances from hardware limitations.
...
### [277] [OVGrasp: Open-Vocabulary Grasping Assistance via Multimodal Intent Detection](https://arxiv.org/abs/2509.04324)
...
### [278] [Reactive In-Air Clothing Manipulation with Confidence-Aware Dense Correspondence and Visuotactile Affordance](https://arxiv.org/abs/2509.03889)
*Neha Sunil,Megha Tippur,Arnau Saumell,Edward Adelson,Alberto Rodriguez*

Main category: cs.RO

TL;DR: A dual-arm visuotactile framework for garment manipulation that combines visual correspondence with tactile-supervised grasp affordance to handle crumpled and suspended clothing, using confidence estimates to guide adaptive folding strategies.


<details>
  <summary>Details</summary>
Motivation: Clothing manipulation is challenging due to complex configurations, variable material dynamics, and frequent self-occlusion. Prior systems often flatten garments or assume visibility of key features, limiting their real-world applicability.

Method: Uses confidence-aware dense visual correspondence trained on custom simulated dataset with distributional loss, combined with visuotactile grasp affordance network self-supervised using high-resolution tactile feedback. Features reactive state machine that adapts strategies based on perceptual uncertainty.
...
Conclusion: The framework enables more generalizable and scalable garment manipulation, paving the way for extracting grasp targets from human video demonstrations and other planning modalities.
...
### [279] [DEXOP: A Device for Robotic Transfer of Dexterous Human Manipulation](https://arxiv.org/abs/2509.04441)
...
<details>
  <summary>Details</summary>
Motivation: To overcome limitations of teleoperation for robotic data collection by creating a more natural and efficient method that maximizes transferability of human manipulation skills to robots.
...
Conclusion: DEXOP represents an effective paradigm (perioperation) for advancing robot dexterity by providing superior data collection that maximizes transfer of human manipulation skills to robots.
...
### [280] [Keypoint-based Diffusion for Robotic Motion Planning on the NICOL Robot](https://arxiv.org/abs/2509.04076)
*Lennart Clasmeier,Jan-Gerrit Habekost,Connor Gäde,Philipp Allgeuer,Stefan Wermter*

Main category: cs.RO

TL;DR: A diffusion-based action model for robotic motion planning that achieves 10x faster runtime than numerical methods while maintaining 90% success rate on collision-free solutions.


<details>
  <summary>Details</summary>
Motivation: Numerical planning approaches for robotic motion planning have significant runtime requirements, creating a need for faster alternatives that maintain solution quality.

Method: Uses deep learning with diffusion models trained on dataset generated by numerical planners. Initially used point cloud embeddings but found conditioning challenging. Refined dataset to address biases and improved performance.
...
Conclusion: Diffusion-based deep learning approach provides a viable alternative to traditional numerical planning methods, offering dramatically improved runtime while maintaining high success rates for robotic motion planning tasks.
...
### [281] [Balancing Signal and Variance: Adaptive Offline RL Post-Training for VLA Flow Models](https://arxiv.org/abs/2509.04063)
...
<details>
  <summary>Details</summary>
Motivation: Current VLA models rely solely on imitation learning, lacking deep understanding of data quality distribution properties that RL excels at, leading to unsatisfactory action accuracy on complex tasks.

Method: Theoretical offline RL post-training objective for VLA flow models with adaptive scaling factor in loss function to balance RL signal impact, creating bias-variance trade-off objective.
...
Conclusion: Adaptive Reinforced Flow Matching provides stable and efficient fine-tuning by optimally controlling RL impact on flow loss, significantly improving VLA model performance on complex robotic manipulation tasks.
...
### [282] [Reservoir Predictive Path Integral Control for Unknown Nonlinear Dynamics](https://arxiv.org/abs/2509.03839)
*Daisuke Inoue,Tadayoshi Matsumori,Gouhei Tanaka,Yuji Ito*

Main category: eess.SY

TL;DR: Integration of echo-state networks and model predictive path integral control for fast nonlinear dynamics learning and control, with uncertainty-aware extension that improves performance by up to 60%
...
Method: Combines echo-state networks (ESNs) for fast dynamics learning with model predictive path integral (MPPI) control for parallelized computation without linearization. Extended to uncertainty-aware RPPI (URPPI) that balances exploration and exploitation using ESN uncertainty

Result: URPPI improves control performance significantly, reducing control costs by up to 60% compared to traditional quadratic programming-based MPC methods in experiments with Duffing oscillator and four-tank systems

Conclusion: The proposed reservoir predictive path integral framework enables effective learning and control of nonlinear dynamics with substantial performance improvements over conventional methods
...
### [283] [Sample Efficient Certification of Discrete-Time Control Barrier Functions](https://arxiv.org/abs/2509.03899)
*Sampath Kumar Mulagaleti,Andrea Del Prete*

Main category: eess.SY

TL;DR: A method for efficiently verifying Control Barrier Functions (CBFs) using Lipschitz arguments and sample-efficient certification algorithms to ensure robust constraint satisfaction.


<details>
  <summary>Details</summary>
Motivation: Control Invariant sets are crucial for certifying safety in dynamical systems, but computing CBFs involves complex robust optimization problems that can be intractable. Scenario-based methods simplify computation but require verification of robust constraint satisfaction.
...
Result: The proposed procedure was validated through a numerical example, demonstrating its efficiency in verifying CBF robust constraint satisfaction.
...
### [284] [Sailing Towards Zero-Shot State Estimation using Foundation Models Combined with a UKF](https://arxiv.org/abs/2509.04213)
...
<details>
  <summary>Details</summary>
Motivation: Traditional state estimation requires extensive manual system identification. Foundation models could reduce deployment effort, but current transformer approaches are limited to sensor models seen during training.

Method: Combines transformer-based model of system dynamics with analytically known sensor models using an unscented Kalman filter (UKF) framework.

Result: Achieves competitive accuracy, effort, and robustness trade-off on container ship benchmark compared to classical methods and end-to-end approaches.

Conclusion: FM-UKF enables generalization across varying dynamics without retraining for new sensor configurations, with open-sourced benchmark to support future zero-shot state estimation research.
...
### [285] [Reinforcement Learning for Robust Ageing-Aware Control of Li-ion Battery Systems with Data-Driven Formal Verification](https://arxiv.org/abs/2509.04288)
*Rudi Coppola,Hovsep Touloujian,Pierfrancesco Ombrini,Manuel Mazo Jr*

Main category: eess.SY

TL;DR: A hybrid control strategy combining reinforcement learning and data-driven formal methods for lithium-ion battery charging protocols that balances charging speed with battery aging while providing probabilistic performance guarantees.


<details>
  <summary>Details</summary>
Motivation: Address the fundamental trade-off between fast charging speed and battery aging/capacity loss in lithium-ion batteries, which is a critical challenge in battery management systems.

Method: Counterexample-Guided Inductive Synthesis scheme combining Reinforcement Learning (RL) for individual controller synthesis with data-driven formal methods to create a hybrid switched control structure based on initial battery measurements.

Result: Developed a hybrid system with discrete selection among RL-based controllers that works with continuous battery dynamics, providing probabilistic guarantees on closed-loop battery cell performance.

Conclusion: The approach successfully synthesizes data-driven charging and safety protocols that balance charging speed and battery aging while offering formal performance guarantees through the hybrid control strategy.
...
### [286] [SAFE--MA--RRT: Multi-Agent Motion Planning with Data-Driven Safety Certificates](https://arxiv.org/abs/2509.04413)
...
<details>
  <summary>Details</summary>
Motivation: To enable homogeneous linear multi-agent systems to operate safely in obstacle-filled environments without requiring explicit system models, using only data-driven approaches for motion planning and coordination.
...
Result: The framework produces dynamically feasible trajectories that are provably safe with respect to environmental constraints and inter-agent collisions, using only data and convex optimization without runtime re-solving.

Conclusion: The proposed data-driven approach effectively synthesizes synchronized, safe trajectories for multiple agents under shared dynamics using convex optimization tools, demonstrating practical applicability through simulations.
...
### [287] [COBRA: Multimodal Sensing Deep Learning Framework for Remote Chronic Obesity Management via Wrist-Worn Activity Monitoring](https://arxiv.org/abs/2509.04210)
...
Method: Hybrid D-Net architecture combining U-Net spatial modeling, multi-head self-attention, and BiLSTM temporal processing with spectral-temporal feature extraction on wrist-worn multimodal sensor data.

Result: 96.86% overall accuracy with category-specific F1-scores: 98.55% (Physical Activity), 95.53% (Food Intake), 94.63% (Sedentary), 98.68% (Daily Living), outperforming baselines by 1.18%.

Conclusion: COBRA provides robust, generalizable behavioral monitoring with low demographic variance, enabling scalable deployment for personalized obesity interventions and continuous lifestyle tracking.
...
### [288] [Gromov-Wasserstein and optimal transport: from assignment problems to probabilistic numeric](https://arxiv.org/abs/2509.04089)
...
Method: Extends linear assignment to Gromov-Wasserstein (GW) distances for different metric spaces, proposes GW-MultiInit strategy to avoid local optima, uses entropic Sinkhorn approximations, and develops fused GW variants that integrate structural and feature information.

Result: GW-MultiInit consistently achieves near-optimal solutions on capacitated QAP instances and scales efficiently to large problems where exact methods fail. Parameterized EGW and FGW variants provide flexible accuracy-runtime trade-offs.

Conclusion: The work provides theoretical foundations, computational insights, and practical guidelines for applying optimal transport and GW methods to QAP and real-world matching problems in machine learning and logistics, demonstrating superior scalability and performance over traditional approaches.
...
### [289] [Shuffling Heuristic in Variational Inequalities: Establishing New Convergence Guarantees](https://arxiv.org/abs/2509.04133)
*Daniil Medyakov,Gleb Molodtsov,Grigoriy Evseev,Egor Petrov,Aleksandr Beznosikov*

Main category: math.OC

TL;DR: First theoretical convergence analysis for shuffling methods in variational inequalities, showing faster convergence than independent sampling approaches.


<details>
  <summary>Details</summary>
Motivation: Stochastic methods for variational inequalities typically assume independent data sampling, but shuffling heuristic (permuting dataset before sequential processing) lacks theoretical guarantees despite practical utility.

Method: Provide theoretical convergence estimates and rigorous bounds for shuffling methods in variational inequalities, extending the theoretical framework for this class of algorithms.

Result: Established convergence rates and bounds for shuffling methods, validated through extensive experiments on diverse benchmark variational inequality problems.

Conclusion: Shuffling methods demonstrate faster convergence compared to independent sampling approaches in variational inequalities, with theoretical guarantees now established.
...
### [290] [Predicting Antimicrobial Resistance (AMR) in Campylobacter, a Foodborne Pathogen, and Cost Burden Analysis Using Machine Learning](https://arxiv.org/abs/2509.03551)
*Shubham Mishra,The Anh Han,Bruno Silvester Lopes,Shatha Ghareeb,Zia Ush Shamszaman*

Main category: q-bio.QM

TL;DR: Machine learning analysis of Campylobacter genomic data predicts rising antimicrobial resistance and significant economic burden by 2050, with Random Forest models achieving 74% accuracy in resistance prediction.


<details>
  <summary>Details</summary>
Motivation: Antimicrobial resistance poses serious public health and economic challenges, requiring advanced methods to predict resistance trends and forecast healthcare impacts.

Method: Used machine learning (Random Forest with bootstrap resampling) on genomic and epidemiological data from PubMLST and UK surveillance databases, analyzing 6,683 isolates with whole-genome sequencing data and time-series forecasting models (SARIMA, SIR, Prophet).

Result: Achieved 74% accuracy in predicting AMR phenotypes, identified key resistance determinants (gyrA mutations, tet(O) gene), and forecasted campylobacteriosis cases potentially exceeding 130/100,000 people by 2050 with economic burden surpassing 1.9 billion GBP annually.

Conclusion: The study demonstrates machine learning's effectiveness in predicting AMR trends, revealing sustained high beta-lactam resistance, increasing fluoroquinolone resistance, and the urgent need for intervention to prevent significant economic and public health impacts.
