...
Motivation: Existing retrieval methods are limited to single-modality queries or documents and require task-specific fine-tuning, failing to address complex multimodal retrieval needs in modern LLM applications.
...
Result: Achieves state-of-the-art performance on M2KR and M-BEIR benchmarks across different retrieval configurations, with faster inference and reduced memory usage. Improves downstream performance on Encyclopedic-VQA and InfoSeek datasets when integrated into retrieval-augmented generation pipelines.

Conclusion: ReT-2 provides an effective unified solution for multimodal retrieval that supports complex queries and document collections while being computationally efficient and improving downstream task performance.
...
Method: Propose using features from Vision Diffusion Model (VDM) aggregated via transformer, enhanced by conditioning on earlier timesteps to prioritize semantic over pixel-level information.
...
Method: Uses VulnGuard Prompt with hybrid techniques: few-shot examples from GitHub, ethical chain-of-thought reasoning, adaptive role-prompting, and multi-objective optimization with formal proofs.

Result: Demonstrates 25-30% analytical harm reduction through entropy bounds and Pareto optimality, with comprehensive mathematical formalization and validation.

Conclusion: PromptGuard provides a systematic framework for real-time harm prevention with theoretical foundations for empirical research on LLM safety.
...
Method: Reframes Re-ID as supervised image similarity using Siamese networks. Introduces Beta-SOD framework that models cosine similarities between embedding pairs with two-component Beta distribution mixture model. Combines binary cross-entropy, contrastive, and cosine embedding losses for joint optimization.

Result: Superior performance compared to state-of-the-art methods across various noise levels (10-30%) on CUHK03, Market-1501 (person Re-ID), and VeRi-776 (vehicle Re-ID) datasets. Demonstrates robustness and broad applicability in noisy scenarios.

Conclusion: Beta-SOD provides an effective statistical framework for handling label noise in Re-ID tasks, with proven identifiability of Beta mixture models and practical implementation available for broader adoption.
...
Method: End-to-end framework with CASE module for multi-scale structural enhancement in encoder, and PHFP module with wavelet decomposition for high-frequency detail refinement in decoder.
...
Conclusion: The proposed SFD-Mamba2Net framework effectively addresses challenges in ICA image analysis and improves accuracy for coronary artery segmentation and stenosis detection.
...
<details>
  <summary>Details</summary>
Motivation: Colorectal liver metastasis significantly reduces survival, and current prognostic models lack predictive power, especially for multifocal cases. There's a need for more accurate, automated prediction tools.

Method: Two-pipeline framework: 1) Segmentation pipeline using promptable foundation models and novel SAMONAI algorithm for 3D segmentation from single point prompts, 2) Radiomics pipeline with SurvAMINN autoencoder-based neural network for survival analysis from extracted tumor features.

Result: Extensive evaluation on 227 patients showed the framework surpasses existing clinical and genomic biomarkers with C-index improvement exceeding 10%.
...
Method: Introduces CompCon, an evolutionary search algorithm that discovers visual attributes more prevalent in one model's output than another, and uncovers prompt concepts linked to these visual differences. Also creates ID2 dataset with 60 input-dependent differences for evaluation.

Result: CompCon successfully finds divergent representations between popular text-to-image models, such as PixArt depicting loneliness prompts with wet streets and Stable Diffusion 3.5 depicting African American people in media professions.

Conclusion: The proposed CompCon algorithm effectively identifies visual attribute divergences between text-to-image models and the prompt concepts that trigger them, providing insights into model-specific representation differences.
...
<details>
  <summary>Details</summary>
Motivation: UAS imagery is often affected by cloud shadows and sun glint, which pose serious issues for estimating water quality parameters from these images.
...
Conclusion: The proposed machine learning approach effectively addresses the limitations of UAS imagery by providing a method to correct cloud shadows and sun glint, enabling better water quality parameter estimation.
...
Method: Proposes CoSwin architecture that integrates learnable local feature enhancement modules into each attention block, enabling simultaneous capture of fine-grained spatial details and global semantic structure.
...
<details>
  <summary>Details</summary>
Motivation: To improve point cloud registration by developing a differentiable framework that incorporates both local and global geometric consistency for more accurate feature matching.
...
Result: Achieves 95%-97% inlier ratios on KITTI, 94%-97% on KITTI-360, and up to 81.1% on 3DMatch, demonstrating significant improvement in rigid registration performance.

Conclusion: iMatcher provides a robust and effective solution for point cloud registration across diverse indoor and outdoor environments, setting new state-of-the-art benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional ultrasound shape reconstruction methods rely on precise annotations and struggle with view-dependent artifacts and acoustic shadowing. There's a need to utilize the rich acoustic information in B-mode images while overcoming occlusion limitations.

Method: Proposes an occupancy-based representation (UltrON) that uses acoustic features from B-mode images without extra annotations. Introduces a novel loss function to compensate for view-dependency and enables multiview ultrasound optimization.

Result: UltrON improves geometric consistency in weakly-supervised optimization, generalizes to shapes of the same anatomy, and mitigates limitations from occlusions and sparse labeling.

Conclusion: The approach enables more accurate 3D reconstruction from ultrasound by leveraging inherent acoustic properties, reducing annotation dependency, and addressing view-dependent artifacts, paving the way for improved clinical ultrasound imaging.
...
Method: Enhanced Mutual Learning Network (E-MLNet) integrates dynamic weighting strategy to Open-set Entropy Minimization (OEM), leveraging closed-set classifier predictions to focus adaptation on the most relevant class boundaries.
...
Conclusion: The dynamic weighting strategy effectively sharpens the distinction between known and unknown classes, confirming the benefits of focused adaptation in universal domain adaptation.
...
<details>
  <summary>Details</summary>
Motivation: Urdu is critically under-served in multimodal research despite being spoken by 250M+ people. The absence of large-scale, high-quality Urdu datasets has limited development of Urdu-capable systems and reinforced biases in multilingual vision-language models.

Method: Derived from MS COCO using stratified sampling, captions were translated with SeamlessM4T v2 and validated through a hybrid multimodal quality estimation framework (COMET-Kiwi, CLIP similarity, BERTScore with back-translation). Low-scoring captions were refined using open-source LLMs.

Result: COCO-Urdu contains 59,000 images and 319,000 Urdu captions. Benchmarking shows consistently strong results on BLEU, SacreBLEU, and chrF metrics. It's the largest publicly available Urdu captioning dataset.

Conclusion: COCO-Urdu addresses the Urdu language gap in multimodal research, reduces language bias, and provides a foundation for inclusive vision-language systems. Both dataset and quality estimation pipeline are released to benefit the research community.
...
Method: VoxelFormer integrates a Token Merging Transformer (ToMer) for efficient voxel compression and a query-driven Q-Former that produces fixed-size neural representations aligned with CLIP image embedding space.
...
Conclusion: Token merging and query-based transformers are promising strategies for parameter-efficient neural decoding, enabling multi-subject training for visual reconstruction from fMRI data.
...
<details>
  <summary>Details</summary>
Motivation: 3D brain MRI studies need to detect subtle morphometric differences between cohorts, but current counterfactual models fail to produce anatomically plausible MRIs due to lack of explicit anatomical constraints and focus on overall appearance rather than medically relevant local variations.

Method: Proposes Probabilistic Causal Graph Model (PCGM) that integrates anatomical constraints via probabilistic graph module, generates spatial binary masks of regions with subtle variations, uses 3D ControlNet encoding, and employs a novel counterfactual denoising UNet with 3D diffusion decoder.

Result: PCGM generates higher quality structural brain MRIs than baseline approaches. Brain measurements from PCGM-generated counterfactuals successfully replicate subtle disease effects on cortical regions previously reported in neuroscience literature.

Conclusion: PCGM represents an important milestone for using synthetic MRIs in studies investigating subtle morphological differences, demonstrating the ability to preserve medically relevant anatomical details in generated images.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D medical SSL methods lack deep semantic comprehension, and recent MLLMs offer promising text-enhanced image understanding capabilities that could improve 3D medical image analysis.

Method: Proposes Med3DInsight framework with 3D image encoder integration to 2D MLLMs via plane-slice-aware transformer module and partial optimal transport based alignment for noise tolerance.
...
Conclusion: Med3DInsight provides a scalable multimodal 3D medical representation learning paradigm without human annotations and can be seamlessly integrated into existing networks to enhance performance.
...
<details>
  <summary>Details</summary>
Motivation: Current graph convolutional neural networks (GCNs) for human action recognition fail to detect human-object interactions effectively due to lack of scene information representation and appropriate learning architectures.

Method: Multi-task learning approach that incorporates fixed object information from the environment along with human skeleton poses. Collected real-world data including interaction classes with fixed objects (ATM machines, check-in/out machines) and non-interaction classes.

Result: Achieved 99.25% accuracy in recognizing interaction and non-interaction actions, which is 2.75% higher than the base model using only human skeleton poses.

Conclusion: Incorporating fixed object information through multi-task learning significantly improves human-object interaction recognition performance compared to skeleton-only approaches.
...
TL;DR: Proposes IRDFusion framework with MFRM and DFFM modules for multispectral object detection, using iterative feedback to enhance salient features while suppressing background noise, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current multispectral object detection methods retain extraneous background or noise during feature fusion, limiting perceptual performance and requiring better cross-modal feature integration.

Method: Uses Mutual Feature Refinement Module (MFRM) to enhance intra- and inter-modal feature representations, and Differential Feature Feedback Module (DFFM) to compute inter-modal differential features as guidance signals. Integrated as Iterative Relation-Map Differential Guided Feature Fusion (IRDFusion) mechanism.
...
<details>
  <summary>Details</summary>
Motivation: Current LiDAR localization methods require expensive high-precision ground-truth poses from GPS/SLAM, which involves substantial cost and effort for data acquisition.

Method: Uses BEV images with geographic distances between keypoint-centered patches to create training triplets. Employs CNN for local features, NetVLAD for global descriptors, and introduces SoftCos loss for triplet learning.
...
Conclusion: S-BEVLoc provides a highly scalable self-supervised alternative to supervised approaches, eliminating the need for costly ground-truth pose data while maintaining competitive performance.
...
<details>
  <summary>Details</summary>
Motivation: Mobile device usage detection requires understanding behavioral context and relationships between faces, hands, and devices, which existing benchmarks don't fully capture.
...
Result: Provided baseline results and performance analysis across object sizes, occlusion levels, and different environments.

Conclusion: FPI-Det addresses the gap in fine-grained human-device interaction detection and serves as a benchmark for phone usage detection systems.
...
<details>
  <summary>Details</summary>
Motivation: Segmenting entire plant individuals with multiple overlapping leaves remains challenging and typically requires annotated training datasets that are species-specific and labor-intensive to create.
...
Conclusion: The proposed zero-shot approach effectively addresses hierarchical plant segmentation challenges without requiring annotated training data, demonstrating strong generalization capabilities across diverse conditions.
...
Method: Proposes ALL-PET with three key innovations: 1) Radon mask augmentation strategy (RMAS) generating 200k+ diverse training samples, 2) Positive/negative mask constraints for geometric consistency, 3) Transparent medical attention (TMA) - parameter-free geometry-driven mechanism enhancing lesion regions in projection data.

Result: ALL-PET achieves high-quality sinogram generation using only 500 samples, with performance comparable to models trained on larger datasets. It generalizes across multiple tasks including low-dose reconstruction, attenuation correction, delayed-frame prediction, and tracer separation, operating efficiently with memory under 24GB.

Conclusion: ALL-PET successfully addresses PET imaging data scarcity by leveraging innovative mask augmentation and attention mechanisms, enabling effective foundation model development with minimal data requirements while maintaining computational efficiency and task generalization.
...
<details>
  <summary>Details</summary>
Motivation: Compare noise robustness between Persistent Homology (PH) and Artificial Neural Networks (ANNs) for topological structure inference from data, particularly for Betti number prediction.

Method: Train supervised neural network to predict Betti numbers in 2D binary images and compare against PH pipeline using cubical complexes and Signed Euclidean Distance Transform (SEDT). Test on one synthetic and two real-world datasets.

Result: ANNs can outperform the PH approach under noise conditions, demonstrating better noise robustness.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D scene evaluation metrics assess overall image quality, which creates discrepancies with human perception since humans primarily perceive scenes through individual objects rather than holistic image quality.

Method: Leverages an object detection model and its feature representations to quantify the 'objectness' of each object in 3D scenes, enabling object-centric evaluations inspired by neuropsychological insights about human visual perception.

Result: User studies demonstrate that OSIM aligns more closely with human perception compared to existing metrics. The authors also re-evaluated recent 3D reconstruction and generation models using OSIM under standardized experimental setups.

Conclusion: OSIM provides a more human-aligned evaluation approach for 3D scenes by focusing on object-level perception, offering better assessment of 3D reconstruction and generation models compared to traditional image quality metrics.
...
<details>
  <summary>Details</summary>
Motivation: Existing surveys classify video models by task or architecture family, but overlook how datasets impose structural pressures that guide architectural evolution. This survey aims to fill this gap by adopting a dataset-driven perspective.

Method: The survey reinterprets video understanding milestones (two-stream CNNs, 3D CNNs, sequential models, transformers, multimodal foundation models) as responses to dataset-driven pressures and inductive biases.

Result: The analysis provides a unified framework connecting datasets, inductive biases, and architectures, offering practical guidance for model design that aligns with dataset invariances while balancing scalability and task demands.
...
<details>
  <summary>Details</summary>
Motivation: Pathologists examine tissue at multiple magnifications to understand cell-tissue interactions, but existing deep learning models lack this capability due to missing multi-scale annotated datasets.

Method: Created a challenge dataset with overlapping cell detection and tissue segmentation annotations from 673 pairs across 6 organs from TCGA Whole-Slide Images, divided into training/validation/test sets.
...
Conclusion: Incorporating multi-scale semantics and cell-tissue interactions is crucial for achieving human-level performance in pathological image analysis, demonstrating substantial improvement over traditional methods.
...
Method: 1) Create low-noise queries from image-question pairs for better knowledge retrieval 2) Use large models to identify and extract answer-beneficial segments from knowledge 3) Implement selective knowledge integration - only use knowledge when model lacks confidence

Result: Extensive experiments demonstrate that the framework outperforms state-of-the-art methods by acquiring accurate and critical knowledge while reducing noise.

Conclusion: The proposed training-free framework effectively mitigates knowledge redundancy issues in KB-VQA by enhancing knowledge relevance and reducing noise through improved retrieval, extraction, and selective integration strategies.
...
Method: Proposed CWSSNet framework that integrates 3D spectral-spatial features with wavelet convolution, uses multiscale convolutional attention module for multimodal information fusion, and employs multi-band decomposition in wavelet domain.

Result: Achieved 74.50% mIoU, 82.73% mAcc, and 84.94% mF1 on ZY1F satellite data; highest IoU for water bodies, vegetation, and bare land classification; maintains reliable performance with limited training time increase at 70% training set proportion.

Conclusion: CWSSNet effectively addresses hyperspectral image classification challenges, demonstrates strong robustness, and performs well under small-sample training conditions, breaking through traditional method performance bottlenecks.
...
<details>
  <summary>Details</summary>
Motivation: Address the research gap in evaluating AI-generated image detection methods under complex real-world conditions, as current datasets lack diversity and real-world robustness testing.

Method: Created RRDataset with images from seven major scenarios, tested internet transmission robustness through social media sharing simulations, and evaluated re-digitization robustness using four methods. Benchmarked 17 detectors and 10 VLMs, plus human study with 192 participants.

Result: Current AI detection methods show significant limitations under real-world conditions. Human participants demonstrated superior few-shot learning capabilities compared to automated detectors.

Conclusion: The study highlights the need for more robust detection algorithms that can better handle real-world complexities, and suggests drawing inspiration from human adaptability to improve AI detection systems.
...
Result: Around 100 participants submitted entries, with five models demonstrating the emerging capabilities of instruction-tuned LMMs on quality assessment tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current goal-guided approaches operate at extreme granularities (coarse goal prediction and fine trajectory completion), leaving intermediate temporal granularity unexplored despite its potential utility for capturing diverse human dynamics.
...
Result: Comprehensive experiments on ETH/UCY and Stanford Drone Dataset show MGTraj outperforms baseline methods and achieves state-of-the-art performance among goal-guided trajectory prediction methods.

Conclusion: Multi-granularity modeling effectively enhances goal-guided human trajectory prediction, with recursive refinement from coarse to fine levels and auxiliary velocity prediction significantly improving prediction accuracy.
...
Conclusion: Medverse establishes a novel paradigm for in-context learning in medical imaging and demonstrates superior performance across diverse medical image analysis tasks, with code and model weights made publicly available.
...
Result: Achieved 96.47% accuracy, 96.60% precision, 96.47% recall, 96.45% F1 score, and 99.89% AUC on HMU-GC-HE-30K. On GasHisSDB: 98.29% accuracy, 98.07% precision, 98.41% recall, 98.23% F1 score, and 99.90% AUC. Outperformed all compared CNN and ViT models.

Conclusion: CoAtNeXt is a robust architecture for histopathological classification of gastric tissue images, demonstrating strong performance on both binary and multiclass tasks. It has potential to assist pathologists by enhancing diagnostic accuracy and reducing workload.
...
<details>
  <summary>Details</summary>
Motivation: Large vision-language models show promise in general medical tasks but underperform in specialized domains like dentistry, particularly with panoramic X-rays that have dense anatomical structures and subtle pathologies not covered by existing benchmarks.

Method: Created MMOral dataset with 20,563 annotated panoramic X-rays and 1.3M instruction instances across multiple task types. Developed MMOral-Bench evaluation suite covering five diagnostic dimensions. Proposed OralGPT by fine-tuning Qwen2.5-VL-7B on MMOral dataset.

Result: Evaluation of 64 LVLMs showed poor performance (best model GPT-4o: 41.45% accuracy). OralGPT achieved 24.73% improvement after single epoch of supervised fine-tuning, demonstrating significant performance enhancement for dental domain tasks.

Conclusion: MMOral dataset and OralGPT model provide a critical foundation for intelligent dentistry, enabling more clinically impactful multimodal AI systems in dental radiology. The significant performance gap reveals current limitations of general LVLMs in specialized medical domains.
...
Conclusion: DATE significantly improves temporal reasoning and event localization in long videos through explicit temporal modeling and semantic-aware sampling.
...
<details>
  <summary>Details</summary>
Motivation: 3D medical image segmentation consumes heavy resources and time, limiting clinical scalability. Existing efficient models are static and manually designed, lacking adaptability and balanced performance-efficiency trade-offs.

Method: PSP-Seg starts with a redundant model and iteratively prunes redundant modules using block-wise pruning and functional decoupling loss.

Result: PSP-Seg-S (lightweight variant) achieves nnU-Net-level performance while reducing GPU memory by 42-45%, training time by 29-48%, and parameters by 83-87% across five datasets.
...
<details>
  <summary>Details</summary>
Motivation: Prior chart understanding methods either rely on brittle external tools or use single-strategy specialist models with unverifiable text-based reasoning, making reinforcement learning difficult due to lack of factual verification.

Method: Proposes adaptive framework where VLMs learn Visual Programmability - choosing between CaT (code-based symbolic representation) and direct visual reasoning pathways. Uses reinforcement learning with dual-reward system (data-accuracy reward and decision reward) to train selection policy.
...
Conclusion: VLMs can be taught not only to reason but also how to reason, adaptively choosing the best strategy between code-based symbolic representation and direct visual analysis for each chart-question pair.
...
<details>
  <summary>Details</summary>
Motivation: Existing segmentation models for multimodal brain MRI are limited to fixed modalities and cannot effectively handle new modalities at inference, while some generalization approaches lose modality-specific information.
...
Result: Evaluated on 8 MRI databases with 5 pathology types and 8 modalities, the approach effectively processes both training-seen and unseen MRI modalities while improving segmentation performance.

Conclusion: The simple architectural modification enables practical multimodal brain MRI segmentation that can handle heterogeneous modality combinations, including completely unseen modalities, making it versatile for clinical applications with varying available imaging data.
...
Result: Achieves up to 10% relative AUROC gain compared to standard YOLO-based detectors on AOT aerial benchmark, with comprehensive ablation studies showing consistent improvements and real-world flight test validation.

Conclusion: The solution provides reliable UAV perception in dynamic air-to-air environments by maintaining detection accuracy while enhancing robustness against unknown objects and corrupted data through explicit open-set handling mechanisms.
...
Conclusion: SlotSAR effectively disentangles target representations from background clutter without mask annotations, demonstrating robust target characterization through complementary multi-level feature integration.
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap in multimodal large language models' capacity to understand real-world materials characterization imaging data, which remains underexplored despite their promise in materials science tasks.

Method: Created MatCha benchmark with 1,500 questions requiring expert-level domain expertise, covering four key stages of materials research and 21 distinct tasks that reflect authentic challenges faced by materials scientists.

Result: Evaluation revealed significant performance gap between state-of-the-art MLLMs and human experts. Models degraded on questions requiring higher-level expertise and sophisticated visual perception. Few-shot and chain-of-thought prompting failed to alleviate limitations.

Conclusion: Existing MLLMs exhibit limited adaptability to real-world materials characterization scenarios. MatCha benchmark will facilitate future research in new material discovery and autonomous scientific agents.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods require impractical joint training or pre-storing models for all collaborators. Real-world vehicles have heterogeneous models due to manufacturer variations, making current approaches infeasible.

Method: Progressive Heterogeneous Collaborative Perception (PHCP) formulates the problem as few-shot unsupervised domain adaptation. It dynamically aligns features by self-training an adapter during inference without labeled data or joint training.
...
Conclusion: PHCP successfully addresses heterogeneous collaborative perception directly during inference, eliminating the need for joint training while maintaining competitive performance with minimal unlabeled data.
...
<details>
  <summary>Details</summary>
Motivation: Vision-language models' purely visual inference capabilities remain under-explored compared to their language-guided classification abilities.

Method: Comprehensive evaluation of dual-encoder VLMs on ImageNet-1k, analyzing factors like prompt design and k-NN parameters. Introduced a learning-free fusion method based on per-class precision.
...
Conclusion: Combining language-guided and vision-only approaches through simple fusion can enhance VLM classification capabilities by leveraging their complementary strengths.
...
<details>
  <summary>Details</summary>
Motivation: Current AI models transform brainstorming into designs but struggle with fine-grained customization due to text uncertainty from non-expert users.

Method: Better Understanding Generation (BUG) workflow with Large Multimodal Model that automatically creates and customizes clothing designs from chat using image-into-prompt technique.

Result: Created FashionEdit dataset to evaluate real-world workflow, measuring generation similarity, user satisfaction, and quality. Code and dataset publicly available.

Conclusion: The framework enables creative clothing design/editing without human involvement, lowering barriers for non-expert users while maintaining quality.
...
<details>
  <summary>Details</summary>
Motivation: Surgical skill assessment requires expert annotations that are scarce and time-consuming to produce. Few-shot learning offers a scalable alternative but depends heavily on effective pre-training strategies, which remain unexplored for SSA.

Method: Formulated SSA as a few-shot task, annotated a robotic surgery dataset with OSATS scores, evaluated various pre-training sources across 1-, 2-, and 5-shot settings, and analyzed domain similarity and procedure-specific data inclusion.

Result: Domain-relevant small datasets outperformed large-scale less aligned ones (60.16%, 66.03%, 73.65% accuracy for 1-, 2-, 5-shot). Procedure-specific data with domain-relevant external datasets boosted performance (+1.22% accuracy, +2.28% F1-score), while less similar large sources caused degradation.

Conclusion: Domain relevance is more critical than dataset size for pre-training in surgical skill assessment. Strategic inclusion of procedure-specific data with domain-aligned external sources significantly enhances few-shot learning performance.
...
<details>
  <summary>Details</summary>
Motivation: Road traffic accidents caused by human error, particularly distracted and impaired driving, remain a significant global safety concern that needs addressing.

Method: Uses advanced computer vision methodologies including real-time object tracking, lateral displacement analysis, lane position monitoring, YOLO object detection model, and custom lane estimation algorithms to identify unsafe driving behaviors.
...
Conclusion: The vision-based approach enables behavioral analysis of non-connected vehicles without relying on inter-vehicular communication, providing a practical solution for detecting unsafe driving behaviors.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods struggle with complex scenes featuring spatially-varying lighting and rich textures, often producing texture-less and over-smoothing results that fail to capture real-world complexity.

Method: Proposes a texture-guided regularization term within an optimization framework that uses texture information from RGB images to separate material textures from lighting effects.

Result: The method produces high-quality intrinsic images for real-world images, demonstrating superior performance compared to existing approaches by effectively handling severe lighting and rich textures.

Conclusion: Combining texture-aware priors in an optimization framework enables more accurate intrinsic image decomposition for complex real-world scenes with varying lighting and detailed textures.
...
<details>
  <summary>Details</summary>
Motivation: To explore the connection between Plug-and-Play (PnP) methods and Denoising Diffusion Implicit Models (DDIM) for solving ill-posed inverse problems, focusing on single-pixel imaging, and identify key distinctions between their approaches.

Method: Decouple diffusion process into three interpretable stages (denoising, data consistency enforcement, sampling), propose hybrid data-consistency module that linearly combines multiple PnP-style fidelity terms applied directly to denoised estimate.
...
Conclusion: The unified framework successfully integrates learned priors with physical forward models in a principled manner, improving measurement consistency without disrupting diffusion sampling trajectory.
...
<details>
  <summary>Details</summary>
Motivation: Intracranial pressure monitoring is crucial but current invasive methods like lumbar puncture carry risks. Non-invasive ONSD measurement shows promise but suffers from manual operation inconsistencies, subjectivity, and variability in thresholding.
...
Method: Proposes a framework with a learnable normal-information extractor that aggregates normal features from test images, coherence loss to associate features with normal regions, decoder for normal content reconstruction, and pseudo-anomaly augmentation for training stability.
...
Method: Uses parameter-efficient tuning (LoRA) and learnable prompt tokens to explicitly separate clinically relevant signals from task-agnostic noise. Curates high-quality, clinically grounded image-text pairs by generating captions for diverse medical datasets.
...
Conclusion: Disentangling task-relevant features and careful cross-modal alignment significantly enhance model generalization and reduce unpredictable behavior under domain shift, contributing to safer, more trustworthy VLMs for clinical use.
...
TL;DR: FS-Diff is a joint image fusion and super-resolution method that uses semantic guidance and clarity-aware mechanisms to produce high-quality fused images from corrupted multimodal inputs.


<details>
  <summary>Details</summary>
Motivation: Real-world applications like military reconnaissance often deal with corrupted multimodal images with low resolution and weak semantic information, leading to suboptimal fusion results with current techniques.

Method: Unifies image fusion and super-resolution as conditional generation using semantic guidance from clarity sensing. Uses bidirectional feature Mamba for global feature extraction and modified U-Net for iterative denoising across multiple noise levels.
...
Conclusion: FS-Diff effectively addresses the challenges of joint image fusion and super-resolution for corrupted multimodal images, demonstrating superior performance in recovering detailed semantic information.
...
Conclusion: The proposed explicit semantic concentration approach successfully overcomes the limitations of dense SSL by breaking strict spatial alignment and discriminating shared patterns from complex scenes, providing an effective solution for learning dense representations.
...
<details>
  <summary>Details</summary>
Motivation: Open-source text-to-image models lag behind closed-source systems due to lack of large-scale reasoning-focused datasets and comprehensive evaluation benchmarks.

Method: Created FLUX-Reason-6M dataset with 6M high-quality images organized by six characteristics and Generation Chain-of-Thought breakdowns. Developed PRISM-Bench with seven evaluation tracks using advanced vision-language models for human-aligned assessment.
...
Conclusion: The released dataset, benchmark, and evaluation code aim to catalyze advancement in reasoning-oriented text-to-image generation for the open-source community.
...
TL;DR: FlexiD-Fuse is a diffusion-based medical image fusion network that handles flexible input quantities (2+ modalities) using hierarchical Bayesian modeling and EM algorithm integration, outperforming existing fixed-input methods across various fusion tasks.


<details>
  <summary>Details</summary>
Motivation: Existing medical image fusion methods only work with fixed numbers of input modalities (e.g., only 2 or 3), limiting clinical applicability where varying numbers of modalities may be available.

Method: Transforms diffusion fusion into maximum likelihood estimation using hierarchical Bayesian modeling, incorporates Expectation-Maximization algorithm into diffusion sampling to handle flexible input quantities end-to-end.

Result: Achieves best performance on Harvard datasets with 9 evaluation metrics, outperforms latest 2/3-modal fusion methods, and shows effectiveness in infrared-visible, multi-exposure, and multi-focus fusion tasks.

Conclusion: FlexiD-Fuse successfully addresses the limitation of fixed-input fusion methods, providing a flexible solution that works with varying numbers of medical image modalities while maintaining high fusion quality.
...
Method: 3D Attention UNet architecture with residual blocks, enhanced through transfer learning from BraTS 2021 pre-trained weights, evaluated on 95 MRI cases from BraTS-Africa dataset.

Result: Achieved Dice scores of 0.76 (Enhancing Tumor), 0.80 (Necrotic/Non-Enhancing Tumor Core), and 0.85 (Surrounding Non-Functional Hemisphere) despite limited data quality and quantity.

Conclusion: The compact model (90MB) with sub-minute inference time demonstrates strong generalizability and practical deployment potential for supporting clinical decision-making in low-resource African healthcare settings.
...
TL;DR: New politically-focused deepfake detection dataset with 3M real images and 963k synthetic images from modern generative models, plus crowdsourced adversarial platform to keep detection methods robust against evolving threats.


<details>
  <summary>Details</summary>
Motivation: Deepfakes are spreading misinformation in political contexts, and existing detection datasets are limited by outdated generation methods, low realism, or single-face imagery, making them ineffective for general synthetic image detection.

Method: Created comprehensive dataset with real images paired with captions used to generate high-quality synthetic images from proprietary and open-source models. Introduced crowdsourced adversarial platform where participants generate challenging synthetic images to test detection methods.

Result: Human perception study shows recent proprietary models produce synthetic images increasingly indistinguishable from real ones. The dataset provides modern benchmarking capabilities for deepfake detection.

Conclusion: The ongoing community-driven initiative ensures deepfake detection methods remain robust and adaptive, proactively protecting public discourse from sophisticated misinformation threats as generative techniques continue to evolve.
...
<details>
  <summary>Details</summary>
Motivation: Existing motion models fail to capture the physical coupling between local joint motion and global root movement, and deriving global trajectories from joint torques is computationally expensive.
...
Conclusion: Momentum-based constraints provide an effective and physically grounded approach to improve motion generation by properly coupling local and global dynamics.
...
Conclusion: The method demonstrates strong potential for real-world manga applications by providing reliable region-wise correspondence without requiring pre-existing labels or masks.
...
Method: Stochastic Generative Diffusion Fusion (SGDF) with multiple generative mechanism for multi-view features, robust to data quality issues. Extended to Generative Diffusion Contrastive Network (GDCN).
...
TL;DR: DualTrack is a novel dual-encoder architecture for sensorless 3D ultrasound that decouples local and global feature extraction to achieve state-of-the-art trajectory estimation and 3D reconstruction accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D ultrasound systems are costly and complex, limiting widespread adoption. Sensorless 3D US using deep learning is promising but previous approaches either ignore global features or tightly couple them with local features, restricting robust modeling of complementary aspects.
...
Result: Experimental results on a large public benchmark show state-of-the-art accuracy with average reconstruction error below 5 mm, outperforming previous methods and achieving globally consistent 3D reconstructions.

Conclusion: The proposed dual-encoder architecture successfully leverages decoupled local and global feature extraction to improve 3D ultrasound probe trajectory estimation, demonstrating superior performance over existing approaches.
...
TL;DR: Align4Gen improves video diffusion models by aligning intermediate features with pre-trained vision encoders through multi-feature fusion, resulting in better video generation quality.


<details>
  <summary>Details</summary>
Motivation: Video diffusion models have focused on architectural innovations but neglected feature representation power. The paper addresses this gap by leveraging pre-trained vision encoders to enhance feature alignment.

Method: Proposes Align4Gen with novel multi-feature fusion and alignment method integrated into video diffusion training. Includes metric development and analysis of various vision encoders for discriminability and temporal consistency.

Result: Improved video generation performance in both unconditional and class-conditional tasks, as measured by various quantitative metrics.

Conclusion: Feature alignment with pre-trained vision encoders significantly benefits video diffusion model training and enhances generation quality.
...
TL;DR: InterAct is a large-scale 3D human-object interaction benchmark that consolidates 21.81 hours of HOI data, optimizes it to 30.70 hours with reduced artifacts, and provides unified generative modeling with state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing human motion capture datasets lack quality 3D human-object interaction data and suffer from artifacts like contact penetration, floating, and incorrect hand motions, limiting HOI research.

Method: Consolidated and standardized 21.81 hours of HOI data from diverse sources with detailed text annotations. Proposed unified optimization framework using contact invariance principle to reduce artifacts and correct hand motions while maintaining human-object relationships.

Result: Expanded dataset to 30.70 hours with enhanced quality. Achieved state-of-the-art performance on six benchmarking tasks through unified HOI generative modeling perspective.

Conclusion: InterAct serves as a foundational resource for advancing 3D human-object interaction generation research, with the dataset being publicly available and actively maintained.
...
TL;DR: DL algorithms for Alzheimer's disease diagnosis from MRI can learn spurious race and sex features, leading to performance bias against underrepresented demographic groups through shortcut learning.


<details>
  <summary>Details</summary>
Motivation: To investigate potential shortcut learning and demographic bias in deep learning-based AD diagnosis from MRI scans, particularly how spurious features related to protected attributes like race and sex can lead to performance disparities.

Method: Used multiple datasets and DL models (ResNet and SwinTransformer) to: 1) test if algorithms can identify race/sex from brain MRI scans, 2) examine performance drops from training set imbalances, and 3) conduct quantitative/qualitative analysis of feature attributions in different brain regions.

Result: Demonstrated existence of both race and sex-based shortcut learning and bias in DL-based AD classification, showing that models learn spurious demographic features that affect diagnostic performance.

Conclusion: The work establishes foundation for developing fairer DL diagnostic tools in brain MRI by identifying and addressing demographic bias issues in AD classification models.
...
TL;DR: PeftCD is a parameter-efficient change detection framework using Vision Foundation Models with LoRA and Adapter modules, achieving state-of-the-art performance across multiple datasets with minimal additional parameters.
...
Method: Uses weight-sharing Siamese encoder from Vision Foundation Models (SAM2 and DINOv3) with integrated LoRA and Adapter modules for parameter-efficient fine-tuning, combined with a lightweight decoder.

Result: Achieves SOTA performance on multiple datasets: SYSU-CD (73.81% IoU), WHUCD (92.05%), MSRSCD (64.07%), MLCD (76.89%), CDD (97.01%), S2Looking (52.25%), and LEVIR-CD (85.62%), with precise boundaries and pseudo-change suppression.

Conclusion: PeftCD provides optimal balance of accuracy, efficiency, and generalization, offering a powerful scalable paradigm for adapting large VFMs to real-world remote sensing change detection applications.
...
TL;DR: Talk2Event is the first large-scale benchmark for language-driven object grounding using event camera data, featuring 5,567 driving scenes with 13,458 annotated objects and 30,000+ validated referring expressions organized by structured attributes.


<details>
  <summary>Details</summary>
Motivation: Event cameras offer microsecond precision and reliability under motion blur/challenging illumination, but their integration with natural language understanding has been largely unexplored, creating a gap in multimodal perception for dynamic scenes.

Method: Built on real-world driving scenarios, the benchmark uses an attribute-centric design with four structured attributes (appearance, status, relation to viewer, relation to surrounding objects) to capture spatial, temporal and relational cues for interpretable and compositional grounding.

Result: The benchmark comprises 5,567 scenes, 13,458 annotated objects, and over 30,000 carefully validated referring expressions, enabling analysis beyond simple object recognition to contextual reasoning in dynamic environments.
...
TL;DR: Kling-Avatar is a novel cascaded framework that unifies multimodal instruction understanding with photorealistic portrait generation, enabling semantically grounded, high-fidelity audio-driven avatar video synthesis with superior performance in lip sync, emotion expressiveness, and long-duration video generation.


<details>
  <summary>Details</summary>
Motivation: Existing audio-driven avatar generation methods treat instruction conditioning as low-level tracking without modeling communicative purpose, compromising narrative coherence and character expressiveness. The gap between multimodal instruction understanding and realistic generation needs to be bridged.
...
Result: Generates vivid, fluent long-duration videos at 1080p/48fps with superior performance in lip synchronization accuracy, emotion expressiveness, instruction controllability, identity preservation, and cross-domain generalization. Evaluated on 375 curated samples covering diverse instructions.

Conclusion: Kling-Avatar establishes a new benchmark for semantically grounded, high-fidelity audio-driven avatar synthesis, making it suitable for real-world applications like digital human livestreaming and vlogging through its parallel architecture enabling fast, stable generation.
...
TL;DR: Hybrid framework combining mathematical tumor growth model with guided diffusion model to predict future brain tumor progression from MRI scans, generating realistic follow-up images and growth probability maps.
...
Result: Generates realistic follow-up MRI scans based on spatial similarity metrics and produces tumor growth probability maps showing clinically relevant extent and directionality (95th percentile Hausdorff Distance).
...
TL;DR: HumbleBench is a new benchmark for evaluating multimodal LLMs' ability to reject incorrect answers and demonstrate epistemic humility by choosing "None of the above" when no provided options are correct.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on recognition accuracy but overlook the critical capability of models to recognize when none of the provided options are correct, which is essential for trustworthy AI in safety-critical applications.

Method: Built from panoptic scene graph dataset using fine-grained annotations to extract ground-truth entities/relations. GPT-4-Turbo generates multiple-choice questions with "None of the above" option, followed by rigorous manual filtering.

Result: Evaluated various state-of-the-art MLLMs and found insights about their ability to reject plausible but incorrect answers across object, relation, and attribute hallucination types.

Conclusion: HumbleBench fills a key gap in current evaluation by testing false-option rejection capability, providing a more realistic measure of MLLM reliability for safety-critical applications.
...
<details>
  <summary>Details</summary>
Motivation: To create a unified multimodal learning framework that bridges image understanding and generation through coherent bidirectional information flow, bringing mutual gains between both processes.

Method: Proposed UAE framework with three-stage Unified-GRPO via RL: 1) cold-start initialization with semantic reconstruction loss, 2) Generation for Understanding (encoder trained to generate captions maximizing decoder's reconstruction), 3) Understanding for Generation (decoder refined to reconstruct from detailed captions).

Result: Encoder autonomously produces more descriptive captions while decoder demonstrates improved understanding of intricate descriptions, resulting in reconstructions of striking fidelity. Introduced Unified-Bench benchmark for evaluation.

Conclusion: The auto-encoder paradigm with reconstruction fidelity as unified objective successfully enables bidirectional improvement between understanding and generation, creating a surprising synergy where both components mutually enhance each other through RL progression.
...
TL;DR: NRMF is a 3D generative human motion prior that uses neural distance fields on Riemannian manifolds to model pose, velocity, and acceleration dynamics for robust and physically plausible motion recovery.


<details>
  <summary>Details</summary>
Motivation: Existing VAE and diffusion-based methods lack explicit modeling of motion dynamics and physical plausibility. The authors aim to create a motion prior that respects the geometric structure of human articulations and ensures temporal consistency.

Method: Models human motion as zero level sets of neural distance fields on the product space of joint rotations, angular velocities, and accelerations. Uses adaptive-step hybrid projection and geometric integrators for plausible motion generation.

Result: Significant improvements in generalization across input modalities and tasks including denoising, motion in-betweening, and fitting to partial 2D/3D observations when trained on AMASS dataset.
...
<details>
  <summary>Details</summary>
Motivation: To understand why deep diffusion models differ from the theoretically optimal denoiser and to characterize the performance gap between analytical models and trained UNet denoisers.

Method: Demonstrated that optimal parametric linear denoisers exhibit similar locality properties to deep neural denoisers, showing this locality arises from pixel correlations in natural images. Developed an analytical denoiser based on these insights.

Result: The locality in diffusion models is a statistical property of image datasets, not due to CNN inductive biases. The crafted analytical denoiser better matches deep diffusion model scores than prior alternatives.

Conclusion: Pixel correlations in natural images drive the locality observed in deep diffusion models, enabling creation of more accurate analytical denoisers that better capture the behavior of trained neural networks.
...
TL;DR: SpatialVID is a large-scale video dataset with dense 3D annotations including camera poses, depth maps, and motion instructions to address data scarcity in spatial intelligence research.


<details>
  <summary>Details</summary>
Motivation: Current spatial intelligence models are limited by scarce, low-quality training data, especially for real-world dynamic scenes with ground-truth camera motion. Existing datasets lack scale, diversity, and annotation richness.

Method: Collected over 21,000 hours of raw video and processed them into 2.7 million clips through hierarchical filtering. Developed an annotation pipeline to add camera poses, depth maps, dynamic masks, structured captions, and motion instructions.

Result: Created SpatialVID with 7,089 hours of dynamic content featuring diverse scenes and camera movements, providing rich spatial and semantic annotations that enable improved model generalization.

Conclusion: SpatialVID's scale, diversity, and annotation richness make it a valuable resource for advancing video and 3D vision research, addressing current limitations in spatial intelligence training data.
...
TL;DR: Development of an interval type-2 Bayesian inference method that handles input uncertainty using expert-provided interval estimates and avoids inconsistencies in fuzzy membership functions.
...
Method: Created an interval type-2 version of Bayes Theorem with a conservative method to prevent input inconsistencies, and developed a novel algorithm for encoding expert-provided intervals into interval type-2 fuzzy membership functions.

Result: A robust Bayesian inference framework that can handle uncertain input probabilities represented as intervals, ensuring valid output results even with potentially inconsistent expert estimates.

Conclusion: The proposed interval type-2 Bayesian approach provides a more realistic and practical method for hypothesis testing in real-world scenarios where precise probability values are unavailable, using expert interval estimates while maintaining mathematical consistency.
...
TL;DR: A framework that automatically converts Game Design Documents into Unity game prototypes using NLP and LLMs, achieving high performance in code generation and GDD adherence.


<details>
  <summary>Details</summary>
Motivation: To streamline the transition from game design to implementation by automating the conversion of design documents into functional game prototypes, addressing gaps in AI-assisted game development.

Method: End-to-end system combining fine-tuned LLaMA-3 model for Unity code generation with custom Unity integration package to parse GDDs and synthesize C# code for core game mechanics and systems.

Result: Significant improvements over baseline models with 4.8/5.0 average score across metrics including compilation success, GDD adherence, best practices adoption, and code modularity. High adherence across multiple game genres.

Conclusion: LLMs can effectively bridge the gap between game design and implementation, serving as valuable tools for automated game template generation from design documentation.
...
TL;DR: A multi-agent LLM framework that decomposes MiniZinc model generation by global constraint types, with specialized agents for each constraint class and an assembler agent for integration, showing improved performance over baseline methods.
...
Conclusion: The framework demonstrates the effectiveness of decomposing complex modeling tasks into specialized sub-tasks, with a roadmap outlined for future enhancements and improvements.
...
<details>
  <summary>Details</summary>
Motivation: Addressing model collapse phenomenon where generative AI performance degrades over generations when trained on synthetic data, with existing mitigation strategies being limited.

Method: Identifies model overconfidence as key driver of collapse and introduces confidence-aware TCE loss function that downweights high-confidence predictions. Provides model-agnostic framework linking loss design to collapse mitigation.

Result: TCE significantly delays model collapse, extending model's fidelity interval before collapse by more than 2.3x. Method generalizes across different modalities.
...
TL;DR: The paper focuses on uncertainty explanations and global explanations in XAI, testing an algorithm that covers uncertainty, robustness, and global XAI concepts to calibrate trust and improve user satisfaction.


<details>
  <summary>Details</summary>
Motivation: While some areas of Explainable AI (XAI) are well studied, uncertainty explanations and global explanations are often overlooked. The researchers aim to address this gap by developing guidelines for XAI schemes that incorporate these important aspects.

Method: The researchers selected an algorithm that simultaneously covers multiple XAI concepts including uncertainty, robustness, and global explanations. They tested this algorithm's ability to calibrate user trust and evaluated whether intuitive visual explanations can improve user satisfaction and interpretability despite complexity.

Result: The study derived general guidelines for XAI schemes from their research. They examined how algorithms providing intuitive visual understanding impact user satisfaction and human interpretability, even when the underlying methods are complex.

Conclusion: The research contributes to XAI by focusing on understudied areas of uncertainty and global explanations, demonstrating that visually intuitive explanations can enhance user satisfaction and interpretability despite algorithmic complexity, and providing valuable guidelines for XAI scheme development.
...
<details>
  <summary>Details</summary>
Motivation: Cold-start users lack historical behavioral information, which compromises recommender system effectiveness. The paper aims to address this issue through prompt optimization on few-shot LLMs.
...
Result: Empirical evidence shows significant improvement in precision@k and NDCG scores. Optimal exemplar injection and instruction structuring enhance model performance in low-data settings, controlling attention scales and decoder conduct during inference.

Conclusion: Prompt-based adaptation is an effective approach to address cold-start recommendation issues in LLM-based pipelines, demonstrating that timely composition is both syntactic and functional.
...
TL;DR: Comparison of humans, LLMs, and Bayesian agents in dynamic negotiation tasks shows performance parity but fundamental behavioral differences that matter for real-world deployment.


<details>
  <summary>Details</summary>
Motivation: As autonomous agents take over human coordination tasks, it's critical to evaluate not just performance outcomes but also the negotiation processes and behavioral dynamics in multi-agent environments.
...
TL;DR: A comprehensive ML pipeline for anti-money laundering that achieved 0.961 AUROC and second place in a university competition using SQL-based feature engineering and XAI modules.
...
Method: 16-step design and statistical analysis, SQLite database framing, SQL-based feature engineering, pre-trained model connection to database, and explainable AI modules for feature importance.
...
Conclusion: The proposed comprehensive ML pipeline demonstrates robust performance in identifying high-risk clients for anti-money laundering purposes, with strong predictive accuracy and explainability features.
...
Result: The framework provides a structured perspective for agentic spatial reasoning across virtual and physical environments, reveals critical gaps in current approaches, and examines emerging benchmarks and potential applications in robotics and embodied systems.

Conclusion: The paper outlines a neuroscience-grounded roadmap for advancing spatial reasoning in AI systems, with potential to generalize across dynamic or unstructured environments, benefiting the research community with a structured pathway forward.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-agent prediction methods overlook the evolving nature of interactions between agents, which is crucial for accurate motion prediction in autonomous vehicle scenarios.

Method: Uses dynamic heterogeneous graph-based scenario modeling to capture evolving social interactions, with a factorized architecture for spatio-temporal dependencies and progressive uncertainty elimination through multi-scale decoding.

Result: Achieves state-of-the-art performance, ranking 1st on INTERACTION multi-agent prediction benchmark and performing well on Argoverse 2 multi-world forecasting benchmark.

Conclusion: The proposed ProgD method effectively addresses evolving interactions in multi-agent scenarios and demonstrates superior prediction accuracy compared to existing approaches.
...
Method: Propose a blockchain-enabled three-layer architecture (agent layer, blockchain data layer, regulatory application layer) with three key modules: agent behavior tracing and arbitration, dynamic reputation evaluation, and malicious behavior forecasting.
...
Conclusion: The framework provides comprehensive regulatory capabilities for autonomous agent collaboration and identifies future research directions for blockchain-enabled regulatory frameworks in multi-agent systems.
...
TL;DR: Proposes NbQA dataset from Jupyter notebooks and Jupiter framework using MCTS for multi-step reasoning, achieving state-of-the-art performance on data analysis tasks with Qwen2.5 models.
...
Method: Created NbQA dataset from real-world Jupyter notebooks, developed Jupiter framework that formulates data analysis as search problem using Monte Carlo Tree Search (MCTS) to generate diverse solution trajectories for value model learning.
...
TL;DR: Using MCTS trajectories to improve policy optimization in preference-based RL through staged GRPO training with tree-structured advantage estimation


<details>
  <summary>Details</summary>
Motivation: Leverage MCTS-derived trajectories (traditionally used for value/reward models) to enhance policy optimization in preference-based reinforcement learning
...
Conclusion: Proposed heuristic and statistical solutions to mitigate issues, with open challenges remaining for learning under staged/tree-like reward structures
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-agent systems face challenges in designing versatile, robust, and efficient deployment platforms, with trade-offs between flexibility and simplicity.
...
Conclusion: LightAgent effectively resolves the flexibility-simplicity trade-off in existing agent frameworks and provides an accessible platform for multi-agent system deployment.
...
<details>
  <summary>Details</summary>
Motivation: Tournaments model pairwise dominance relationships, and there's a need for formal explainable AI approaches to provide certified explanations for why certain candidates win under various tournament rules.

Method: The authors identify minimal supports (minimal sub-tournaments) where a candidate is a necessary winner, and develop polynomial-time algorithms to compute these for multiple tournament solutions including top cycle, uncovered set, Copeland, Borda, maximin rules.

Result: They determine the size of smallest minimal supports for each tournament rule and present efficient algorithms for all rules except the weighted uncovered set, for which the problem is NP-complete.
...
<details>
  <summary>Details</summary>
Motivation: Many emergency response teams must coordinate movements in physical space without visual cues or extensive communication, but existing research focuses mainly on co-located or knowledge work coordination.
...
TL;DR: TAM Bench is a comprehensive benchmark for evaluating LLM-based agents on end-to-end ML tasks, featuring automated task collection from platforms like Kaggle, difficulty modeling based on leaderboard data, and multi-dimensional evaluation across 150 curated AutoML tasks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for LLM-based ML agents are limited in task coverage, domain diversity, difficulty modeling, and evaluation rigor, failing to capture full agent capabilities in realistic settings.

Method: Developed a browser automation and LLM-based system to automatically collect ML challenges from platforms; implemented leaderboard-driven difficulty modeling using participant counts and score dispersion; created multi-dimensional evaluation framework covering performance, format compliance, constraint adherence, and task generalization.
...
Conclusion: TAM Bench addresses limitations of existing benchmarks by providing a diverse, realistic, and structured evaluation framework for LLM-based agents on end-to-end ML workflows, enabling more comprehensive assessment of agent capabilities.
...
TL;DR: A novel DRL architecture with VLM integration and curriculum learning for resource-efficient semantic exploration, achieving higher object discovery and strategic VLM querying.


<details>
  <summary>Details</summary>
Motivation: Traditional RL approaches struggle with balancing efficient exploration and semantic understanding due to limited cognitive capabilities, requiring human intervention for semantic exploration tasks.

Method: Integration of Vision-Language Model common-sense through layered reward function, modeling VLM query as dedicated action, combined with curriculum learning strategy for different complexity levels.
...
Method: Template-Oriented Reasoning (TORSO) elicits models to utilize internal reasoning abilities to generate proper responses across various tasks without manually crafted examples.

Result: Experimental results demonstrate that TORSO achieves strong performance on diverse LLMs benchmarks with reasonable rationales.

Conclusion: TORSO provides an effective alternative to few-shot prompting that leverages models' internal reasoning capabilities without manual example construction.
...
<details>
  <summary>Details</summary>
Motivation: Address the critical issue of false information (hallucinations) in LLMs when applied to legal contexts, where accuracy is paramount.
...
Conclusion: Solution requires shift from incremental generative model improvements to consultative AI paradigm prioritizing veracity and traceability, amplifying rather than replacing professional judgment.
...
<details>
  <summary>Details</summary>
Motivation: Existing memory management methods suffer from noise accumulation, uncontrolled memory expansion, and limited cross-domain generalization in long-term multi-agent systems with vast trajectory data.

Method: SEDM integrates verifiable write admission using reproducible replay, a self-scheduling memory controller that dynamically ranks and consolidates entries based on utility, and cross-domain knowledge diffusion to abstract reusable insights.
...
Conclusion: SEDM provides a scalable and sustainable memory mechanism for open-ended multi-agent collaboration, transforming memory into an active, self-optimizing component rather than passive storage.
...
<details>
  <summary>Details</summary>
Motivation: Address the lack of compositional generalization in current AI tools like vision-language models, building on previous negative results with classical tensor-based semantics.

Method: Interpreted tensor-based representations in Hilbert spaces and trained Variational Quantum Circuits using two image encoding techniques: multi-hot encoding (MHE) and CLIP-based angle/amplitude encoding on an image captioning task.

Result: Achieved good proof-of-concept results with noisy MHE encodings. CLIP-based encoding showed mixed performance but still outperformed classical compositional models.

Conclusion: Quantum models demonstrate improved training efficiency and performance for compositional generalization tasks compared to classical approaches.
...
<details>
  <summary>Details</summary>
Motivation: Traditional sequential computation patterns in embodied AI systems face limitations in achieving the necessary thinking frequency for real-world applications with high-frequency input/output demands.

Method: Auras disaggregates perception and generation modules, provides controlled pipeline parallelism, and establishes a public context for sharing between modules to address data staleness issues while maintaining accuracy.
...
Conclusion: Auras effectively overcomes sequential computation constraints and provides high throughput for embodied AI agents, demonstrating the efficacy of algorithm-system co-design approaches.
...
TL;DR: Scaling LLMs shows exponential benefits for long-horizon tasks despite diminishing single-step accuracy returns. Execution errors, not reasoning failures, limit performance on longer tasks, with self-conditioning effects observed where models repeat past mistakes.


<details>
  <summary>Details</summary>
Motivation: To understand why LLMs fail at simple tasks when made longer, and to investigate whether continued model scaling provides meaningful improvements for real-world agent tasks that require multiple steps.

Method: Isolate execution capability by providing explicit knowledge and plans for long-horizon tasks. Compare performance of different model sizes on multi-turn tasks, analyze error patterns, and benchmark frontier thinking models on single-turn task length execution.

Result: Larger models execute significantly more turns correctly even when small models have 100% single-turn accuracy. Per-step accuracy degrades with more steps due to self-conditioning (models repeating past errors). Thinking models avoid self-conditioning and execute longer tasks better.

Conclusion: Execution capability, not reasoning, is the limiting factor for long-horizon tasks. Scaling model size and sequential test-time compute provides massive benefits for complex multi-step problems, reconciling why LLMs succeed at complex reasoning yet fail at simple extended tasks.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of RAG and fine-tuning in constrained environments like the 2025 LM-KBC challenge, where traditional enhancement techniques are restricted.

Method: Investigated three key aspects: generation techniques, quality assurance mechanisms for filtering poor triples, and LLM response parsing approaches.
...
Conclusion: In constrained LLM environments, strategic information enhancement, LLM-based filtering, and context-aware response parsing are effective alternatives to restricted RAG and fine-tuning approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current manual assessment of corporate climate policy engagement is time-consuming, labor-intensive, and error-prone, requiring automation to scale monitoring efforts.

Method: Retrieval-Augmented Generation (RAG) framework with layout-aware parsing, Nomic embedding model, and few-shot prompting strategies for extracting and classifying evidence from multilingual corporate documents.

Result: The combination of layout-aware parsing, Nomic embedding model, and few-shot prompting yields the best performance in evidence extraction and classification from large-scale textual data.

Conclusion: While RAG effectively accelerates evidence extraction, the nuanced analysis requires human-in-the-loop approach where technology augments rather replaces expert judgment to ensure accuracy.
...
<details>
  <summary>Details</summary>
Motivation: To enhance psychometric analysis of textual data by transforming text into response data suitable for traditional psychometric methods, addressing the need for better analysis tools in text-rich fields.
...
Conclusion: This approach enhances psychometric text analysis and shows promise for applications in education, psychology, law, and other text-rich domains.
...
TL;DR: BRoverbs is a new dataset for evaluating Portuguese-language LLMs using Brazilian proverbs to assess cultural and linguistic understanding beyond translated benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations for Portuguese LLMs are limited, relying on translated datasets that miss linguistic nuances and cultural references, creating gaps in assessing broader linguistic comprehension.

Method: Introduces BRoverbs dataset specifically designed with Brazilian proverbs that encapsulate cultural wisdom, figurative expressions, and complex syntactic structures to challenge LLM comprehension of regional expressions.

Result: Provides a new evaluation tool for Portuguese-language LLMs that focuses on regional linguistic and cultural understanding rather than translated content or limited scope evaluations.

Conclusion: BRoverbs contributes to advancing regionally informed benchmarking for Portuguese LLMs by addressing the limitations of current evaluation frameworks and providing culturally relevant assessment tools.
...
TL;DR: VLMs struggle with visual equation solving due to counting bottlenecks and multi-step reasoning challenges, despite strong performance on text equations.
...
Result: VLMs perform well on textual equations but fail on visual counterparts primarily due to counting bottlenecks. Even with accurate variable recognition, composing recognition and reasoning introduces additional errors. Symbolic reasoning itself becomes limiting as equation complexity increases.
...
<details>
  <summary>Details</summary>
Motivation: To develop a low-overhead, reproducible tool for auditing model dispositions and understanding how LLMs respond to different user interaction tones, complementing existing abuse classification metrics.
...
Result: SPICE sharply discriminates by user tone with near-unanimous preference to continue friendly interactions (97.5% YES) and strong preference to discontinue abusive ones (17.9% YES). Even when models failed to identify abuse, they still preferred not to continue 81% of the time.

Conclusion: SPICE is validated as a robust, low-overhead tool that provides a direct relational signal of model dispositions, with study context presentation format significantly impacting results under ambiguous conditions.
...
TL;DR: Combined SFT+DPO approach outperforms individual alignment techniques for improving safety and helpfulness in OPT-350M model


<details>
  <summary>Details</summary>
Motivation: To investigate the effectiveness of different alignment techniques (SFT, DPO, and combined approach) on improving language model safety and helpfulness

Method: Trained and evaluated four models (base OPT350M, SFT-only, DPO-only, SFT+DPO) using Anthropic Helpful-Harmless RLHF dataset with three evaluation metrics: Harmlessness Rate, Helpfulness Rate, and Combined Alignment Score
...
Conclusion: The study provides comprehensive insights into fine-tuning strategies for model alignment and establishes a foundation for more robust alignment pipelines, while highlighting challenges with noisy data and resource constraints
...
<details>
  <summary>Details</summary>
Motivation: LLMs show strong capabilities but underperform in universal information extraction, especially with structured outputs requiring complex schema descriptions and multi-step reasoning. Existing methods like in-context learning and instruction tuning have significant limitations.

Method: Integrates reinforcement learning with multi-perspective reasoning for information extraction tasks, enabling LLMs to understand both what to extract and how to reason through the extraction process.
...
Conclusion: The integration of reinforcement learning with multi-perspective reasoning effectively addresses LLMs' limitations in universal information extraction, demonstrating the critical role of reasoning in challenging extraction scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Bangla is the 5th most spoken language but remains underrepresented in LLMs, especially for code generation, due to scarcity of high-quality training data.

Method: Created comprehensive Bangla code instruction datasets, developed MBPP-Bangla evaluation benchmark, and built TigerCoder-family models with 1B and 9B parameters.

Result: Achieved 11-18% performance gains at Pass@1 over existing multilingual and general-purpose Bangla LLMs, demonstrating smaller models can excel with curated data.

Conclusion: High-quality curated datasets can overcome limitations of smaller models for low-resource languages; all resources open-sourced to advance Bangla LLM research.
...
TL;DR: Compass-v3 is a 245B parameter Mixture-of-Experts model specialized for Southeast Asian e-commerce, featuring hardware optimizations, multilingual training on 12T tokens, and a novel OTPO alignment method that outperforms major LLMs in e-commerce tasks.
...
Result: State-of-the-art e-commerce performance surpassing DeepSeek-V3.1, GPT-4 series, and Qwen3-235B; strong multilingual capability across low-resource Southeast Asian languages and Portuguese; competitive general benchmark performance; deployed at industrial scale replacing 70%+ of OpenAI traffic on Shopee's platform.

Conclusion: Compass-v3 successfully addresses domain-specific challenges in e-commerce through specialized architecture, optimized training, and novel alignment techniques, demonstrating both superior commerce expertise and broad linguistic competence for real-world industrial applications.
...
Conclusion: Generative AI provides efficient DA classification for educational dialogue, with importance on task-specific labels, context, and ethical considerations.
...
Method: Modular pipeline implementation covering brain datasets, regions, stimulus features (neural-net-based and control), downsampling approaches, with built-in logging, plotting, and integration with experiment tracking platforms like Weights & Biases.
...
Conclusion: LITcoder successfully provides a flexible framework that enables researchers to easily compose, compare, and extend encoding models without reinventing core infrastructure, while emphasizing important methodological considerations.
...
TL;DR: EchoX is a speech-to-speech LLM that bridges the acoustic-semantic gap to preserve reasoning capabilities, achieving advanced performance on knowledge QA benchmarks with 6K hours of training data.


<details>
  <summary>Details</summary>
Motivation: Current SLLMs derived from text LLMs show degradation in knowledge and reasoning due to failure to bridge the acoustic-semantic gap in feature representation.
...
Conclusion: EchoX successfully preserves strong reasoning abilities as a speech LLM by addressing the acoustic-semantic gap through integrated acoustic and semantic learning.
...
<details>
  <summary>Details</summary>
Motivation: Current Trie-based biasing methods require computationally expensive bonus revocation when rare words aren't fully recognized, especially problematic for large decoder models.

Method: Adapts ASR models to predict multiple steps ahead through fine-tuning, using only 10 hours of synthetic data on Whisper to better estimate if partial hypotheses will lead to full rare word generation.
...
Conclusion: The proposed look-ahead multi-step prediction approach effectively overcomes computational limitations of traditional Trie-based biasing while significantly improving rare word recognition performance.
...
<details>
  <summary>Details</summary>
Motivation: Rare word recognition in ASR can be improved with synthetic data and contextual biasing, but training on synthetic data leads to overfitting due to audio artifacts.

Method: Enhanced TCPGen-based contextual biasing approach with keyword-aware loss function containing masked cross-entropy for biased word prediction and binary classification for detecting biased word positions.

Result: Adapting Whisper to 10 hours of synthetic data reduced word error rate on NSC Part 2 test set from 29.71% to 11.81%.

Conclusion: The proposed keyword-aware loss function effectively addresses overfitting in contextual biasing training and significantly improves rare word recognition performance.
...
TL;DR: GmSLM is a generative spoken language model for marmoset monkey vocal communication that uses unsupervised wild data and weakly labeled conversations to generate realistic vocalizations and distinguish real from artificial conversations.
...
Conclusion: GmSLM provides a practical framework linking vocalization and brain activity, potentially benefiting future neuroscience, bioacoustics, and evolutionary biology research on primate vocal communication.
...
<details>
  <summary>Details</summary>
Motivation: Scaling language models to longer contexts is essential but imposes significant computational and memory burdens, making naive context extension inefficient during training and inference.

Method: CCF integrates segment-wise semantic aggregation with key-value memory encoding to form compact representations, and uses incremental segment decoding with sparse reservoir sampling for training efficiency.
...
TL;DR: LLMs and fine-tuned BERT models show promise for automating seniority classification in resumes, using a hybrid dataset of real and synthetic examples to detect overstated experience and ambiguous self-presentation.


<details>
  <summary>Details</summary>
Motivation: Accurately assessing candidate seniority from resumes is challenging due to prevalent experience overstatement and ambiguous self-presentation, requiring automated solutions to improve evaluation accuracy.

Method: Used large language models (LLMs) including fine-tuned BERT architectures, evaluated on a hybrid dataset of real-world resumes and synthetically generated hard examples simulating exaggerated qualifications and understated seniority.
...
TL;DR: Proposes an NL-to-SQL approach using LLMs (GPT-4o, GPT-4o-mini, DeepSeek v2:16b) for Table QA, achieving 70.5-71.6% accuracy on DataBench datasets, significantly outperforming 26-27% baselines.


<details>
  <summary>Details</summary>
Motivation: Table QA faces challenges due to diverse table structures, sizes, and data types. SemEval 2025 Task 8 introduced DataBench to evaluate models' ability to answer structured queries over tabular data.

Method: Multi-stage pipeline: example selection, SQL query generation using LLMs, answer extraction, verification, and iterative refinement with models like GPT-4o, GPT-4o-mini, and DeepSeek v2:16b.
...
Conclusion: LLM-driven NL-to-SQL approach is effective for Table QA, providing insights into strengths and limitations of using large language models for structured query generation over tabular data.
...
TL;DR: Weak supervision approach using LLMs and patent citations to classify patents for UN SDGs, outperforming baselines and showing better thematic coherence than traditional methods.


<details>
  <summary>Details</summary>
Motivation: Lack of large labeled datasets limits supervised learning for patent-to-SDG classification, while existing methods lack scalability and generalizability.

Method: Frames classification as weak supervision problem using NPL citations as noisy signal. Develops composite labeling function with LLMs to extract structured concepts from patents and SDG papers, computes cross-domain similarity scores with rank-based retrieval, and calibrates via custom positive-only loss.

Result: Creates silver-standard multi-label dataset enabling effective regression models. Outperforms baselines including transformer models and zero-shot LLM in internal validation, and shows greater thematic coherence in external validation using network modularity.

Conclusion: Weak supervision and semantic alignment can effectively enhance SDG classification at scale, addressing limitations of traditional approaches.
...
TL;DR: MetaRAG is a metamorphic testing framework for detecting hallucinations in Retrieval-Augmented Generation (RAG) systems by decomposing answers, generating controlled mutations, verifying against retrieved context, and scoring inconsistencies.


<details>
  <summary>Details</summary>
Motivation: LLMs deployed in enterprise applications suffer from hallucinations (confident but incorrect information), and existing detection methods don't address the unique challenges of RAG systems where responses must align with retrieved evidence.

Method: Four-stage framework: (1) decompose answers into atomic factoids, (2) generate controlled mutations using synonym/antonym substitutions, (3) verify variants against retrieved context (synonyms should be entailed, antonyms contradicted), (4) aggregate penalties into hallucination scores with span-level localization.
...
Conclusion: MetaRAG provides real-time, unsupervised, black-box hallucination detection for RAG systems without requiring ground-truth or model access, making it suitable for proprietary and high-stakes domains with identity-sensitive applications.
...
Method: Summarizing key theories from cognitive science literature about processes underlying analogical reasoning and relating them to concepts in natural language processing.

Result: The paper demonstrates how cognitive notions of analogical reasoning are relevant for several major challenges in NLP research beyond just analogy solving.

Conclusion: Applying cognitive perspectives on analogical reasoning can guide NLP researchers to better optimize relational understanding in text and move beyond heavy reliance on entity-level similarity.
...
TL;DR: Hierarchical bracketing encodings enable linear-time dependency graph parsing with competitive accuracy.


<details>
  <summary>Details</summary>
Motivation: To develop a practical approach for dependency graph parsing that handles complex structures like reentrancies, cycles, and empty nodes while maintaining efficiency.

Method: Encodes dependency graphs as sequences using hierarchical bracketing, allowing linear-time parsing with n tagging actions and reduced label space compared to existing graph linearizations.

Result: Achieves competitive results on multilingual and multi-formalism benchmarks with consistent improvements in exact match accuracy over other methods.

Conclusion: Hierarchical bracketing encodings provide an efficient and effective solution for dependency graph parsing, balancing computational complexity with structural representation capabilities.
...
TL;DR: GrACE is a new method for real-time confidence estimation in LLMs that uses hidden state similarity to a special token, achieving better calibration and discriminative capacity than existing approaches without additional computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing confidence elicitation methods for LLMs either require expensive computational overhead or suffer from poor calibration, making them impractical for real-world deployment in high-stakes applications like healthcare and finance.

Method: GrACE uses a novel mechanism where the model expresses confidence through similarity between the last hidden state and a special token's embedding. The model is fine-tuned with calibration targets associated with accuracy.

Result: Experiments with three LLMs and two benchmark datasets show GrACE achieves the best discriminative capacity and calibration on open-ended generation tasks, outperforming six competing methods without additional sampling or auxiliary models.
...
<details>
  <summary>Details</summary>
Motivation: To address the needs of non-Czech-speaking students in Czech primary and secondary schools by providing accessible multilingual learning materials through digital education platforms.

Method: Combines digital education, linguistics, translation studies, and machine translation to develop a specialized Czech-Ukrainian MT system that handles formatted content (XML/PDF) and technical terminology. Includes teacher surveys to identify needs.

Result: Developed up to 9,000 multimodal interactive exercises translated into multiple languages. Created a domain-specific machine translation system and made all applications freely available to students, educators, and researchers.

Conclusion: The project successfully bridges language barriers in education through tailored machine translation technology and provides valuable multilingual resources for the educational community.
...
Method: Self-supervised hybrid architecture combining dense sentence embeddings with domain-specific Knowledge Graphs using graph neural networks, with stratified evaluation partitioning STR scores into low, medium, and high semantic relatedness regions.
...
Conclusion: Combining knowledge graphs with text embeddings provides significant benefits, and regional performance analysis reveals hidden model strengths/weaknesses that global metrics miss, supporting better model selection for HR systems requiring fairness and explainability.
...
TL;DR: The DeMeVa team's approaches for LeWiDi 2025: ICL with LLMs using different example sampling strategies, and LDL methods with RoBERTa using various fine-tuning techniques.


<details>
  <summary>Details</summary>
Motivation: To develop effective methods for predicting perspectivist annotations (annotator-specific labels) and soft label predictions in learning with disagreements tasks.

Method: Two approaches: (1) In-context learning with large language models comparing example sampling strategies, (2) Label distribution learning methods with RoBERTa evaluating several fine-tuning techniques.
...
<details>
  <summary>Details</summary>
Motivation: The rapid transformation of financial NLP by Large Language Models has outpaced traditional surveys, creating a need for structured, data-driven analysis of research trends and evolution in the field.
...
Conclusion: MetaGraph provides a structured, queryable view of financial NLP evolution and demonstrates a reusable approach for mapping scientific progress across domains, offering clear insights into emerging trends and methodological shifts.
...
TL;DR: SAMI system enhanced with GPT-based personality detection from forum posts to improve social matchmaking in online courses


<details>
  <summary>Details</summary>
Motivation: Online learning environments lack organic social connection formation, and existing systems like SAMI have limited Theory of Mind capabilities, particularly in intuiting personality traits that could improve match relevance
...
Conclusion: Personality-informed recommendations show promise for improving social connections in online courses, though further evaluation is needed to determine their full impact on student engagement and match quality
...
Method: Created EXPRESS benchmark dataset with 251 fine-grained emotion labels from Reddit communities. Evaluated prevalent LLMs using comprehensive framework that decomposes emotions into eight basic categories based on established emotion theories.

Result: LLMs find it challenging to accurately predict emotions that align with human self-disclosed emotions. While some generate theoretically consistent emotion terms, they often fail to capture contextual cues as effectively as humans.

Conclusion: Current LLMs have significant limitations in fine-grained emotion alignment, highlighting the need for improved contextual understanding in future research.
...
TL;DR: LA-VA pipeline combines LLMs with traditional methods for improved verbal autopsy cause-of-death prediction, showing GPT-5 outperforms baselines by 5-10% accuracy.
...
TL;DR: MOAT is a joint alignment framework that improves multi-agent LLM collaboration through iterative planning agent optimization and grounding agent fine-tuning, achieving significant performance gains.
...
Conclusion: The proposed joint alignment framework effectively enhances agent collaboration and generalization capabilities, with theoretical guarantees of convergence.
...
TL;DR: The paper investigates how LLMs perform mental math calculations, identifying a specific subgraph (AF1) where computation occurs late at the last token using specialized techniques CAMA and ABP.


<details>
  <summary>Details</summary>
Motivation: To understand the inner workings of LLMs in computational tasks, specifically how they process information across tokens and layers during mental math calculations.

Method: Used three-step approach: inhibiting early layer computations, restricting information transfer routes, and forcing computation at last token. Developed Context-Aware Mean Ablation (CAMA) and Attention-Based Peeking (ABP) techniques to identify the All-for-One (AF1) subgraph.

Result: Identified AF1 subgraph that achieves high accuracy on mental math tasks, showing computation happens late and only at last token with specific middle layer information transfer. This subgraph is sufficient, necessary, and transferable across models.

Conclusion: LLMs perform mental math through a specific computational pattern where meaningful calculation occurs late at the final token, receiving information from specific middle layers, revealing insights about their internal processing mechanisms.
...
TL;DR: SteerMoE framework controls LLM behaviors by detecting and steering behavior-linked experts in Mixture-of-Experts models without retraining, improving safety and faithfulness or enabling adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: To develop a method for controlling specific behaviors (like faithfulness and safety) in MoE-based LLMs by identifying and manipulating specialized experts that handle particular behaviors, without requiring model retraining or weight modifications.

Method: Detects experts with distinct activation patterns across paired inputs showing contrasting behaviors, then selectively activates or deactivates these behavior-linked experts during inference to control model outputs.
...
Conclusion: SteerMoE demonstrates effective behavior control in MoE models and reveals a new dimension of alignment faking hidden within experts, exposing potential vulnerabilities in safety mechanisms.
...
TL;DR: CDE framework uses actor perplexity and critic variance as curiosity signals to improve exploration in RLVR, achieving +3 point gains on AIME benchmarks and revealing calibration collapse mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current RLVR methods suffer from poor exploration, premature convergence, and entropy collapse, limiting their reasoning enhancement capabilities for LLMs.

Method: Curiosity-Driven Exploration (CDE) uses two intrinsic signals: actor perplexity over generated responses and critic variance from multi-head value estimates as exploration bonuses within RLVR framework.

Result: Achieves approximately +3 point improvement over standard RLVR using GRPO/PPO on AIME benchmarks, with theoretical analysis showing benefits for penalizing overconfident errors and promoting response diversity.

Conclusion: CDE effectively addresses exploration challenges in RLVR, provides theoretical grounding connecting to count-based exploration, and identifies calibration collapse as a key failure mode in LLMs.
...
TL;DR: Proposes a new framework for uncertainty estimation and decomposition using signal-to-noise ratio of class probabilities, with a variance-gated measure to scale predictions by confidence from ensembles.


<details>
  <summary>Details</summary>
Motivation: Existing additive uncertainty decomposition approaches have been questioned, and there's a need for better per-sample uncertainty quantification in high-risk applications using neural networks.

Method: Uses signal-to-noise ratio of class probability distributions across different model predictions. Introduces a variance-gated measure that scales predictions by confidence factors derived from ensembles.

Result: The framework provides an intuitive approach to uncertainty estimation and decomposition, and reveals insights about diversity collapse in committee machines.

Conclusion: The proposed signal-to-noise ratio based framework offers a more effective approach to uncertainty quantification and decomposition compared to traditional additive methods.
...
<details>
  <summary>Details</summary>
Motivation: The Matrix Multiplicative Weight Update (MMWU) algorithm achieves minimax-optimal regret but not instance-optimal regret. The authors aim to develop an improved algorithm that achieves instance-optimal regret bounds while maintaining computational efficiency.

Method: Developed a general potential-based framework for matrix LEA, with MMWU as a special case. Used a new "one-sided" Jensen's trace inequality based on Laplace transform technique to apply general potential functions. The algorithm is induced by an optimal potential function from vector LEA based on the imaginary error function.

Result: The new algorithm achieves instance-optimal regret bound of O(√(T·S(X||d⁻¹I_d))) where S is quantum relative entropy, matching MMWU's computational complexity. Outperforms state-of-the-art for learning quantum states corrupted by depolarization noise, random quantum states, and Gibbs states.

Conclusion: The improvement in regret bound is "free" as it maintains the same computational complexity as MMWU. The algorithm enables better performance in quantum learning applications and can predict nonlinear quantum properties when applied to linearized convex losses.
...
Method: Proposed provably robust variant of Q-learning that works with corrupted rewards, using refined Azuma-Hoeffding inequality for almost-martingales in the unknown statistics setting.

Result: Achieves finite-time convergence rate matching non-adversarial case up to additive corruption term proportional to fraction of corrupted samples, with information-theoretic lower bound showing this is unavoidable.
...
<details>
  <summary>Details</summary>
Motivation: Standard ML methods often learn spurious correlations that perform well on average but degrade for underrepresented groups, and existing worst-group optimization approaches assume accurate distribution estimation which is frequently violated in noisy, non-stationary environments.

Method: Wasserstein-based distributionally robust optimization (DRO) framework that accounts for distributional uncertainty within each group while preserving worst-group performance objective, with a gradient descent-ascent algorithm.

Result: Developed convergence results for the proposed algorithm and validated effectiveness on real-world data.

Conclusion: The proposed framework effectively addresses distributional uncertainty in worst-group performance optimization, providing robust performance improvements for underrepresented groups in heterogeneous data environments.
...
TL;DR: FoundationalECGNet is a foundational framework for automated ECG classification that achieves 99% F1-score for normal vs abnormal detection and state-of-the-art performance in multi-class cardiac disease classification using wavelet denoising, attention mechanisms, and transformer architectures.
...
Method: The model integrates dual-stage denoising using Morlet and Daubechies wavelets transformation, Convolutional Block Attention Module (CBAM), Graph Attention Networks (GAT), and Time Series Transformers (TST) to capture spatial and temporal dependencies in multi-channel ECG signals. It uses a two-stage classification approach: first distinguishing normal vs abnormal, then classifying abnormal signals into five cardiac conditions.

Result: Achieves 99% F1-score for Normal vs Abnormal classification, 99% F1-score for Conduction Disorders and Hypertrophy, and 98.9% F1-score for Arrhythmias across multiple datasets. The model also provides risk level estimations for clinical decision-making.
...
TL;DR: LRP attribution methods analyzed as product of modified gradient matrices, revealing bounds for singular values and attribution values. Key finding: LRP-beta constants remain independent of weight norms, unlike gradient methods and LRP-epsilon.


<details>
  <summary>Details</summary>
Motivation: To understand numerical properties and distribution of LRP-type attribution methods through matrix representation and derive bounds that explain convergence behavior.

Method: Represent LRP attribution methods as product of modified gradient matrices, derive upper bounds for singular values and component-wise bounds for attribution values, then apply these to obtain multiplicative constants.

Result: Derived bounds show multiplicative constants govern convergence of empirical attribution means to expectations. LRP-beta constants remain independent of weight norms, unlike gradient-based methods and LRP-epsilon.

Conclusion: The analysis provides important insights for scenarios with multiple non-geometric data augmentations and Smoothgrad-type methods, highlighting LRP-beta's unique property of weight norm independence.
...
TL;DR: Carbon-aware federated learning scheduler that uses slack time and fair allocation to reduce emissions while maintaining model accuracy under carbon constraints.


<details>
  <summary>Details</summary>
Motivation: Large-scale ML training generates substantial carbon emissions. FL's distributed nature enables leveraging regional carbon intensity variations for emission reduction.

Method: Carbon-aware client selection and scheduling with slack time, α-fair carbon allocation, and global fine-tuning to balance emission savings with statistical heterogeneity and temporal correlation challenges.

Result: Outperforms slack-agnostic baselines, achieving higher model accuracy across various carbon budgets, with particularly strong performance under tight carbon constraints.

Conclusion: Carbon-aware scheduling in FL effectively reduces emissions while maintaining model performance, especially valuable when carbon budgets are constrained.
...
Method: Integrates PyePAL (active Pareto front learning algorithm) with Gaussian process models to predict mechanical properties from design variables (spin speed, dilution, polymer mixture). Uses UMAP for 2D visualization of Pareto front exploration and fuzzy linguistic summaries for explainable AI insights.
...
Conclusion: The framework successfully combines multi-objective optimization with explainable AI techniques, enabling both efficient parameter optimization and interpretable insights into the high-dimensional design space for polymer thin film processing.
...
TL;DR: ANNA is an efficient attention mechanism with sub-quadratic time complexity that maintains transformers' ability to simulate MPC algorithms while solving reasoning tasks with near-optimal depth.
...
Method: Introduces Approximate Nearest Neighbor Attention (ANNA) mechanism with sub-quadratic time complexity, and proves it retains expressive power while enabling efficient computation.
...
Conclusion: ANNA provides a unified framework for reasoning about efficient attention approximations while maintaining the computational capabilities of standard transformers with improved scalability.
...
TL;DR: Open-sci-ref provides dense transformer baseline models (0.13B-1.7B parameters) trained on 8 reference datasets to enable standardized comparison of training approaches and scaling trends.


<details>
  <summary>Details</summary>
Motivation: To establish reference baselines that help researchers assess the sanity and quality of alternative training methods across different model scales and datasets, facilitating standardized comparison.

Method: Trained family of dense transformer models across multiple parameter scales (0.13B to 1.7B) and token scales (up to 1T) on 8 recent open reference datasets, with intermediate checkpoints for studying training dynamics.

Result: NemoTron-CC HQ consistently outperforms other reference datasets, followed by DCLM-baseline and FineWeb-Edu. The baselines enable comparison through scaling trends aligned on a common compute axis.

Conclusion: Open-sci-ref provides comprehensive reference points including models, checkpoints, logs, code, and evaluations to simplify reproduction, standardize comparison, and facilitate future research in training methodologies.
...
TL;DR: A context-conditional anomaly detection framework for tabular data that automatically identifies context features and uses deep autoencoders to model conditional distributions, outperforming state-of-the-art methods by accounting for contextual nuances.
...
Method: Proposes a framework that automatically identifies context features and models the conditional data distribution using a simple deep autoencoder tailored for tabular datasets.
...
Conclusion: The framework effectively addresses contextual nuances in tabular data anomaly detection, highlighting the importance of context in accurately distinguishing anomalous from normal instances.
...
TL;DR: Mixture of Experts approach combines existing weather models using Vision Transformer gating network to create more accurate forecasts with lower computational cost


<details>
  <summary>Details</summary>
Motivation: Data-driven weather models have plateaued in performance; need efficient way to improve accuracy without building new complex models from scratch

Method: Vision Transformer-based gating network that dynamically weights contributions of multiple expert models at each grid point based on forecast lead time

Result: Achieves up to 10% lower RMSE than best individual AI weather model on 2-day forecasts, outperforms both individual experts and simple averaging

Conclusion: Computationally efficient and scalable strategy to advance data-driven weather prediction by optimally combining existing high-quality forecast models
...
TL;DR: This scoping review analyzes ML applications in power system protection, finding high accuracy in simulations but insufficient real-world validation, and calls for standardized practices to bridge research to practical deployment.
...
Method: PRISMA framework scoping review of 100+ publications, assessing ML scope, performance evaluation, and suitability for evolving grid conditions.
...
TL;DR: STRIDE is a scalable XAI framework that avoids exponential subset enumeration costs through RKHS-based functional decomposition, providing both local/global explanations with high fidelity and speed improvements.
...
Method: Uses orthogonal functional decomposition in Reproducing Kernel Hilbert Space (RKHS) with analytical projection via recursive kernel-centering, avoiding explicit subset enumeration.
...
Conclusion: STRIDE complements scalar attribution methods by providing structured functional perspective, offering both computational efficiency and enhanced explanatory capabilities for tabular data.
...
TL;DR: The Rashomon Ensemble method selects diverse high-performing models from the Rashomon set to improve generalization by maximizing diversity while maintaining accuracy, achieving up to 0.20+ AUROC improvements.


<details>
  <summary>Details</summary>
Motivation: Model selection that generalizes well is challenging due to the Rashomon Effect where multiple models perform similarly well on the same problem, particularly in real-world scenarios like manufacturing and medical diagnosis with diverse data patterns.

Method: Proposes Rashomon Ensemble that strategically selects models from diverse high-performing solutions by grouping them based on both performance and explanations to construct ensembles that maximize diversity while maintaining predictive accuracy.

Result: Validated on open and proprietary datasets, demonstrating up to 0.20+ AUROC improvements in scenarios with large Rashomon ratio, showing tangible benefits for businesses in real-world applications.

Conclusion: The approach provides robustness, practicality, and effectiveness for improving generalization by leveraging diverse solutions from the Rashomon set, making ensembles more robust to distribution shifts and unseen data variations.
...
TL;DR: Analysis of Riemannian geometry in Deep Linear Networks using group actions and Riemannian submersion to define Boltzmann entropy and connect parameter space geometry to observable space.
...
Method: Using group actions to analyze overparametrization, Riemannian submersion from parameter space to observable space, foliation of balanced manifold by group orbits, and explicit construction of orthonormal basis using Jacobi matrix theory.

Result: Successful definition and computation of Boltzmann entropy through the foliation structure, and demonstration that Riemannian geometry on observable space is obtained via Riemannian submersion from the balanced manifold.

Conclusion: The Riemannian geometric framework provides a foundation for thermodynamic description of learning in deep linear networks, with group actions and submersion techniques enabling entropy definition and geometric connections between parameter and observable spaces.
...
Method: Integrates Multivariate Granger Causality (MVGC) and PCMCI+ for causal feature selection within a hybrid neural architecture, using 43 years of Arctic Sea Ice Extent data and ocean-atmospheric variables.
...
<details>
  <summary>Details</summary>
Motivation: Existing RL methods struggle with complex dynamical systems requiring high-frequency interactions. Continuous-time RL shows promise but has been limited to single-agent domains due to curse of dimensionality in HJB equations and difficulty approximating centralized value functions in multi-agent settings.
...
Result: The approach consistently outperforms existing continuous-time RL baselines on continuous-time variants of multi-agent particle environment (MPE) and multi-agent MuJoCo benchmarks, demonstrating scalability to complex multi-agent dynamics.

Conclusion: The proposed framework successfully addresses key challenges in continuous-time multi-agent RL by combining PINNs with gradient refinement, enabling effective learning in high-dimensional multi-agent systems with complex dynamics.
...
Method: Gathered public ISP data from PeeringDB and CAIDA, evaluated tree-based, neural network, and transformer ML models for predicting peering relationships

Result: XGBoost achieved 98% accuracy in predicting peering partners and showed resilience to time, space, and missing data variations
...
<details>
  <summary>Details</summary>
Motivation: Spanish is spoken by over 600 million people worldwide but remains underrepresented in speech forensics research compared to English and Chinese, creating a gap in synthetic speech detection capabilities for Spanish.

Method: Created HISPASpoof dataset containing real speech from public corpora across six Spanish accents and synthetic speech generated with six zero-shot TTS systems. Evaluated five representative detection methods on this dataset.
...
<details>
  <summary>Details</summary>
Motivation: Large transformer models are powerful for semantic communication but face deployment challenges in resource-constrained 6G networks due to high computational demands.

Method: Formulates token merging proportion selection as multi-objective optimization, employs Gaussian process-based Bayesian optimization to create Pareto-optimal configurations for adaptive runtime adjustment.

Result: Outperforms other baselines with significant FLOPs reduction while maintaining competitive accuracy across various SNR conditions. Adaptive policies effectively trade off latency and semantic fidelity based on channel quality.

Conclusion: Provides a scalable and efficient approach for deploying transformer-based semantic communication in future edge intelligence systems with flexible adaptation to dynamic requirements.
...
TL;DR: Quantum-inspired neural networks combined with deep reinforcement learning create a trading agent for USD/TWD that achieves 11.87% return over 5 years with minimal drawdown, outperforming currency ETFs.
...
Method: Integrated Quantum Long Short-Term Memory (QLSTM) for short-term trend prediction with Quantum Asynchronous Advantage Actor-Critic (QA3C) algorithm. Used 80% training data (2000-2025) with specific state design, reward functions, and multi-core training (8 workers).

Result: Achieved 11.87% return over approximately 5 years with only 0.92% maximum drawdown, outperforming several currency ETFs. Demonstrated QLSTM's effectiveness for small-profit trades with tight risk control.

Conclusion: Hybrid quantum-inspired models yield competitive FX trading performance. QLSTM shows promise for risk-controlled trading, though limitations include classical quantum simulation and simplified strategy. Future enhancements are needed for broader applicability.
...
Method: FSPO clips sequence log-importance-sampling ratios with a KL-corrected drift term that scales as √L (sequence length), ensuring fair treatment across different response lengths.
...
Result: DART achieves CSI = 0.273 with bias = 2.52 vs. 6.72 for baselines, improves extreme convection detection by 270% by removing Integrated Water Vapor Transport, and validates on real-world flooding events. Trains in under 10 minutes on standard hardware.
...
Method: Proposes Incrementally Penalized Proximal Policy Optimization (IP3O) that integrates adaptive incentive mechanisms and progressively increasing penalties to stabilize training dynamics before approaching constraint boundaries.
...
Conclusion: IP3O effectively addresses constraint boundary instability in constrained RL through adaptive incentives and progressive penalties, providing both practical performance improvements and theoretical foundations.
...
<details>
  <summary>Details</summary>
Motivation: Agro-tourism is a strategic economic model for rural development and cultural conservation, requiring detailed study of growth strategies to support local communities and preserve traditional practices.

Method: Two-phase study: comprehensive literature review to identify indicators, followed by machine learning feature selection using LASSO with classifiers (Logistic Regression, Decision Trees, Random Forest, XGBoost) on different train-test splits.

Result: LASSO with Logistic Regression achieved highest accuracy: 98% in 70-30% split and 99% in 80-20% split. Other models showed 95-97% accuracy across different data splits.

Conclusion: Machine learning feature selection methods, particularly LASSO combined with Logistic Regression, effectively identify key indicators for agro-tourism growth with high predictive accuracy.
...
TL;DR: Vejde framework combines data abstraction, GNNs, and RL to create inductive policies for structured decision problems with varying size and complexity, showing strong generalization to unseen instances.


<details>
  <summary>Details</summary>
Motivation: To address decision problems with richly structured states (object classes and relations) that require policies capable of handling varying problem sizes and structures while maintaining generalization to unseen instances.

Method: Represents MDP states as databases of facts, converts states to bipartite graphs, uses neural message passing for latent state mapping, and trains policies through both supervised and reinforcement learning on RDDL problem domains.

Result: Vejde policies generalized well to unseen test instances without significant performance loss, achieving scores close to instance-specific MLP agents and outperforming online planning algorithm Prost.

Conclusion: The framework successfully produces inductive policy functions that handle structured decision problems with good generalization capabilities, demonstrating the effectiveness of combining data abstraction, GNNs, and RL for complex decision-making tasks.
...
<details>
  <summary>Details</summary>
Motivation: To prevent harmful recommendations from undertrained educational recommender systems by creating accurate QA simulators that don't require real student interaction.

Method: LLM Distillation based Simulator (LDSim) that transfers domain knowledge and reasoning capability from LLMs to improve prediction performance while maintaining efficiency.

Result: Extensive experiments show LDSim achieves strong results on both simulation and knowledge tracing tasks.

Conclusion: LDSim effectively bridges the gap between LLM-free and LLM-based methods, offering improved performance without the computational costs of full LLM approaches.
...
<details>
  <summary>Details</summary>
Motivation: Addresses challenges of limited labeled fault samples and lack of model generalizability across different mechanical equipment types in industrial applications.

Method: Combines time-frequency domain encoder for representation learning with meta-learning network. Uses random augmentations in time-frequency domain, contrastive learning iterations, and fine-tuning with limited labeled data.

Result: Achieves 99% fault diagnosis accuracy using only 1% of labeled sample data, demonstrating strong generalization capabilities on bearing fault dataset and rotor test bench data.

Conclusion: MMT-FD framework provides efficient and highly accurate fault diagnosis with minimal labeled data requirements, making it suitable for practical industrial applications with limited fault samples.
...
<details>
  <summary>Details</summary>
Motivation: Sparse rewards in long-horizon tasks make credit assignment difficult for LLM agents, and traditional policy gradients have inefficient learning dynamics where update magnitudes are coupled with entropy.

Method: Proposes Entropy-Modulated Policy Gradients (EMPG) that recalibrates learning signals using step-wise uncertainty and final outcomes, with bonus terms for future clarity to encourage predictable paths.
...
Conclusion: EMPG effectively resolves the fundamental learning dynamics problem in LLM agents by modulating policy gradients based on confidence and uncertainty, leading to more stable and efficient learning.
...
TL;DR: The paper presents a data-driven framework called DRSALife for learning Soft Artificial Life rulesets from observed data to represent emergent dynamics in reaction-diffusion systems without prior physics knowledge.
...
Method: The DRSALife framework learns Soft ALife models (Agent-based and Cellular Automata) from observed data to represent reaction-diffusion system dynamics. The approach handles noisy and sparse datasets and identifies underlying PDE structures and parameters.

Result: The learned models achieved 74% accuracy in predicting emergent dynamics and demonstrated robust performance with Gaussian noise and temporal sparsity. Successfully identified PDE structures and parameters.

Conclusion: The DRSALife framework is effective for data-driven discovery of Soft ALife rulesets representing reaction-diffusion dynamics without prior physics knowledge, showing promising accuracy and robustness to noise and data sparsity.
...
TL;DR: MoSE framework enhances GNN expressiveness by using mixture of subgraph experts to capture diverse high-order patterns, outperforming baselines with improved flexibility and interpretability.


<details>
  <summary>Details</summary>
Motivation: Traditional GNNs rely on local message passing and cannot capture complex subgraph patterns, while existing random walk kernel methods are limited to graph-level tasks and lack flexibility.

Method: Extracts subgraphs via anonymous walks and dynamically routes them to specialized experts based on structural semantics, enabling diverse pattern capture with theoretical guarantees under SWL framework.

Result: Outperforms competitive baselines across diverse graph tasks and provides interpretable insights through visualization of learned subgraph experts.

Conclusion: MoSE offers a flexible, expressive, and interpretable framework for subgraph-based representation learning that overcomes limitations of traditional GNNs and existing kernel-based approaches.
...
TL;DR: Novel computational approach for HGR correlation coefficient using configurable polynomial kernels, offering improved robustness and determinism for real-world applications.
...
Result: The method demonstrates significant advantages in robustness and determinism, with experimental validation showing it produces insightful subgradients suitable as loss regularizers.

Conclusion: The proposed polynomial kernel approach offers a more reliable and robust computational method for HGR correlation, making it suitable for practical applications in constrained machine learning frameworks.
...
<details>
  <summary>Details</summary>
Motivation: Current AutoML and LLM-based approaches for model/hyperparameter selection rely on expensive trial-and-error methods with limited interpretability and generalizability, requiring automation that is both efficient and explainable.

Method: Leverages historical experiment outcomes with SHAP explanations, employs meta-learning and efficient LLM reasoning for zero-shot hyperparameter optimization, and uses LLM-as-judge evaluation for output control.

Result: Achieves optimal results on 5 of 8 medical imaging tasks, reduces response time by 99.6-99.9%, achieves 2.4-15.7x faster training times on 6 datasets, while maintaining accuracy within 1-5% of best baselines.

Conclusion: MetaLLMiX provides an efficient, interpretable, and cost-effective alternative to traditional HPO methods, demonstrating superior performance with significantly reduced computational requirements.
...
TL;DR: LLMs can generate valid counterfactual explanations but struggle with minimality - either making excessive edits or overly small ones that don't change predictions, revealing a validity-minimality trade-off that limits their effectiveness as explainability tools.


<details>
  <summary>Details</summary>
Motivation: To enable effective human-AI collaboration by allowing language models to explain their decisions through self-generated counterfactual explanations that provide insight into model decision-making behavior.

Method: Evaluated whether LLMs can produce valid (achieve intended outcome) and minimal (modify input no more than necessary) self-generated counterfactual explanations across multiple LLMs, datasets, and evaluation settings.

Result: LLMs typically produce valid counterfactuals but far from minimal, offering little insight. When asked for minimal counterfactuals, they make excessively small edits that fail to change predictions, showing a consistent validity-minimality trade-off.

Conclusion: Self-generated counterfactual explanations are ineffective explainability tools that can provide misleading insights, raising concerns about deploying LLMs in high-stakes settings due to unreliable self-explanations.
...
TL;DR: Hybrid framework combining machine learning and geostatistics using spatial lag features from kriging, called KpR, with TabPFN model improves soil property prediction accuracy by 30% compared to non-spatial ML methods.


<details>
  <summary>Details</summary>
Motivation: To leverage both the spatial structure capabilities of geostatistics and the feature relationship learning of machine learning for more accurate digital soil mapping in precision agriculture.

Method: Proposed KpR (kriging prior regression) that enriches ML with spatial context through engineering of 'spatial lag' features from ordinary kriging, using TabPFN model on six fieldscale datasets with soil organic carbon, clay content, pH and remote/proximal sensing features.

Result: KpR with TabPFN demonstrated reliable uncertainty estimates and more accurate predictions, improving average R2 by around 30% compared to ML without spatial context, outperforming both spatial techniques and established non-spatial ML algorithms.

Conclusion: KpR with TabPFN is a robust and versatile modeling framework for digital soil mapping, particularly effective for small sample sizes in precision agriculture and compensates for weak relationships when proximal soil sensing data are limited.
...
TL;DR: Proposes fuser algorithm for microbiome network inference that handles spatial/temporal dynamics better than traditional methods by generating environment-specific networks while sharing information across environments.


<details>
  <summary>Details</summary>
Motivation: Current co-occurrence network algorithms analyze single environments and capture static snapshots, failing to account for how microbial associations adapt to different ecological conditions across space and time.
...
Conclusion: The fuser algorithm effectively addresses limitations of conventional microbiome network inference by capturing dynamic microbial associations across varying environmental conditions, providing more accurate environment-specific networks.
...
TL;DR: CSGD is a novel score-based graph diffusion model that enables precise multi-conditional molecular generation using concrete scores and composable guidance techniques.
...
Method: Proposes CSGD with concrete scores for discrete graphs, Composable Guidance (CoG) for fine-grained control over condition subsets, and Probability Calibration (PC) to address train-test mismatches in transition probabilities.
...
<details>
  <summary>Details</summary>
Motivation: Traditional urban water forecasting models struggle to capture complex inter-variable and temporal dependencies between water measurements and external factors like precipitation, limiting their accuracy and effectiveness.

Method: Developed a multi-input, multi-output deep learning model with embedding layer for exogenous variables, focusing forecast only on endogenous water variables while leveraging external factors as contextual inputs.

Result: Achieved state-of-the-art performance on LausanneCity dataset with 4 sensors, showing improved performance with exogenous variables. Also demonstrated robust performance on three large-scale synthetic datasets with 100 nodes each.

Conclusion: AquaCast effectively captures complex dependencies in urban water systems, generalizes well across different datasets, and provides more accurate forecasts by properly integrating endogenous and exogenous variables without needing to forecast external factors.
...
TL;DR: A fully automated AI system called 'Agent-E' that identifies academic papers from specific geographic regions and uses Robotic Process Automation (RPA) to complete predefined actions like nomination submissions, achieving 100% recall and 99.4% accuracy.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of keeping pace with rapidly growing academic literature and reduce time-consuming manual effort required for scholarly discovery workflows.

Method: Developed a pipeline with specialized AI agent 'Agent-E' that identifies target papers from conference proceedings based on geographic criteria, then executes RPA to complete predefined actions such as form submissions.
...
Conclusion: Demonstrates the potential of task-oriented AI agents to not only filter information but also actively participate in and accelerate academic community workflows through automated action execution.
...
<details>
  <summary>Details</summary>
Motivation: Address privacy leakage concerns in stochastic optimization while overcoming limitations of DPSGD's static noise mechanism and handling the challenge of efficient learning with exponentially growing model parameters.
...
Conclusion: D2P2-SGD successfully addresses privacy concerns in stochastic optimization through dynamic privacy mechanisms and random projection, achieving better utility at privacy cost while enabling more efficient model learning.
...
<details>
  <summary>Details</summary>
Motivation: OpenML has limitations in pipeline diversity and representation, particularly in data preprocessing techniques, leading to imbalanced samples that hinder effective algorithm selection.
...
Result: Built a repository with detailed information on pipeline blocks, training/testing times, predictions, performances, and error messages, enabling cross-pipeline and cross-dataset analyses.

Conclusion: PIPES provides a more diverse and representative alternative to OpenML for meta-learning research, with potential for future expansion to further support the community.
...
TL;DR: Few-shot learning using Prototypical Networks with cough sound spectrograms achieves competitive accuracy (74.87% multi-class, >70% binary) for COVID-19/Flu/healthy classification with minimal training data.
...
Conclusion: Few-shot learning is feasible for medical diagnostics when large labeled datasets are unavailable, and multi-class classification performs comparably to binary approaches for respiratory sound classification.
...
TL;DR: ProDiGy - a Byzantine-robust FL algorithm using dual scoring (proximity + dissimilarity) that outperforms existing defenses, especially under non-IID data distributions


<details>
  <summary>Details</summary>
Motivation: Federated Learning remains vulnerable to adversarial attacks, particularly under data heterogeneity conditions where existing defenses often fail

Method: Joint dual scoring system that evaluates client gradients based on both proximity and dissimilarity metrics to detect Byzantine attacks

Result: Outperforms existing defense mechanisms in various scenarios, maintains strong defense capabilities and model accuracy even with non-IID client data

Conclusion: Dual perspective approach effectively promotes natural similarity among honest clients while detecting suspicious uniformity as attack indicators
...
Result: Outperforms existing unsupervised alignment baselines on graph benchmarks, shows superior robustness to structural inconsistencies, and generalizes effectively to vision-language benchmarks with diverse pretrained models.

Conclusion: The proposed framework successfully addresses key limitations in graph alignment by simultaneously enhancing node distinctiveness and ensuring geometric consistency, demonstrating broad applicability across domains.
...
TL;DR: A deep learning emulator for stochastic and chaotic spatio-temporal systems that generalizes across parameter values through pre-training and fine-tuning, with local attention for varying domain sizes and probabilistic uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: To create computationally efficient emulators for complex PDE systems that can generalize across parameter spaces and provide uncertainty quantification for rare events.
...
Conclusion: The emulator enables efficient parameter space exploration and provides uncertainty quantification, making it valuable for studying rare events in complex spatio-temporal systems.
...
<details>
  <summary>Details</summary>
Motivation: Machine learning models in dynamic environments suffer from concept drift, but explaining how and why the model's decision-making logic changes remains challenging despite drift detection being well-studied.

Method: Analyze temporal evolution of group-based counterfactual explanations (GCEs) by tracking shifts in cluster centroids and counterfactual action vectors before and after drift. Use a three-layer framework combining data layer (distributional shifts), model layer (prediction disagreement), and explanation layer.

Result: The approach provides interpretable proxies that reveal structural changes in the model's decision boundary and underlying rationale, enabling comprehensive drift diagnosis.

Conclusion: The methodology allows distinguishing between different root causes of concept drift (e.g., spatial data shift vs. re-labeling of concepts) through a holistic view of data, model, and explanation layers.
...
TL;DR: A novel Functional Group Representation (FGR) framework that uses chemical functional groups to create interpretable molecular representations, achieving state-of-the-art performance across 33 benchmark datasets while maintaining chemical interpretability.
...
Method: Integrates curated functional groups (FG) with mined functional groups (MFG) using sequential pattern mining, encodes molecules into lower-dimensional latent space through pre-training on unlabeled data, and includes 2D structure descriptors.
...
<details>
  <summary>Details</summary>
Motivation: To develop a reinforcement learning approach that addresses both safety and fairness concerns in decision support systems, particularly for healthcare applications where reducing harm and ensuring equitable treatment across different demographic groups is critical.

Method: Feasibility-Guided Fair Adaptive Reinforcement Learning (FG-FARL) that uses per-group safety threshold calibration. Evaluated against behavior cloning (BC) and HACO baseline using de-identified Medicaid population health data with bootstrap confidence intervals and subgroup disparity analysis.

Result: FG-FARL achieves comparable value to behavior cloning and HACO baselines while significantly improving fairness metrics (coverage and harm equalization across protected subgroups).

Conclusion: FG-FARL provides a practical approach for developing safer and more equitable decision support systems in healthcare, demonstrating the feasibility of balancing performance with fairness considerations in offline reinforcement learning applications.
...
TL;DR: ButterflyQuant introduces learnable butterfly transforms with continuous Givens rotation angles to adaptively suppress activation outliers for 2-bit quantization, achieving better performance than fixed rotation methods like QuaRot.


<details>
  <summary>Details</summary>
Motivation: Large language models require massive memory footprints, limiting deployment on consumer hardware. Extreme 2-bit quantization suffers from catastrophic performance loss due to outliers in activations, and existing rotation-based methods use fixed transforms that cannot adapt to specific weight distributions across different transformer layers.

Method: Replaces fixed Hadamard rotations with learnable butterfly transforms parameterized by continuous Givens rotation angles. This enables smooth optimization while guaranteeing orthogonality, with O(n log n) complexity. Also introduces uniformity regularization on post-transformation activations to promote smoother distributions for quantization.

Result: On LLaMA-2-7B with 2-bit quantization, ButterflyQuant achieves 15.4 perplexity versus 22.1 for QuaRot. Learning requires only 128 calibration samples and converges in minutes on a single GPU.

Conclusion: Layer-adaptive rotations through learnable butterfly transforms significantly outperform fixed rotation methods for extreme quantization, providing an efficient and effective solution for memory reduction in large language models.
...
### [194] [Convexity of Optimization Curves: Local Sharp Thresholds, Robustness Impossibility, and New Counterexamples](https://arxiv.org/abs/2509.08954)
*Le Duc Hieu*

Main category: math.OC

TL;DR: The paper analyzes when the optimization curve (sequence of function values) of first-order methods is convex, showing gradient descent has convex curves for stepsizes ≤1.75/L and nonincreasing gradient norms for stepsizes ≤2/L.


<details>
  <summary>Details</summary>
Motivation: To understand the convexity properties of optimization curves in first-order methods, connecting discrete and continuous dynamics while refining classical smooth convex optimization analysis.

Method: Theoretical analysis of gradient descent on convex L-smooth functions, examining the sequence of function values and forward differences, with comparison to continuous-time gradient flow.

Result: For gradient descent, the optimization curve is convex for stepsizes η ≤ 1.75/L (tight bound), gradient norms are nonincreasing for η ≤ 2/L, and gradient flow always produces convex curves.

Conclusion: The results provide precise thresholds for convex optimization curves in discrete methods, revealing connections between discrete and continuous optimization dynamics and refining worst-case analysis tools.
...
### [195] [The Role of Community Detection Methods in Performance Variations of Graph Mining Tasks](https://arxiv.org/abs/2509.09045)
*Shrabani Ghosh,Erik Saule*

Main category: cs.SI

TL;DR: Study investigates how community detection algorithm choice affects downstream graph mining performance, finding that specific algorithms perform better in certain applications.


<details>
  <summary>Details</summary>
Motivation: Real-world graphs lack ground truth community information and there's no universal standard for community detection, making method selection challenging without clear impact understanding on downstream tasks.

Method: Proposes a framework to integrate various community detection methods and systematically evaluate their effects on downstream task outcomes through comparative analysis.

Result: Comparative analysis reveals that specific community detection algorithms yield superior results in certain applications, demonstrating that method selection substantially affects performance.

Conclusion: The choice of community detection algorithm significantly influences downstream application performance, highlighting the importance of method selection in graph mining tasks.
...
### [196] [Generative quantum advantage for classical and quantum problems](https://arxiv.org/abs/2509.09033)
*Hsin-Yuan Huang,Michael Broughton,Norhan Eassa,Hartmut Neven,Ryan Babbush,Jarrod R. McClean*

Main category: quant-ph

TL;DR: Quantum generative models that are classically hard to simulate, efficiently trainable, and demonstrate quantum advantage in learning and sampling distributions beyond classical reach.


<details>
  <summary>Details</summary>
Motivation: To overcome the challenge of demonstrating generative quantum advantage by creating quantum models that can efficiently learn and generate outputs better than classical computers.

Method: Introduced families of generative quantum models that avoid barren plateaus and local minima, using a 68-qubit superconducting quantum processor to learn classically intractable distributions and quantum circuits for accelerated physical simulation.

Result: Successfully demonstrated efficient learning and sampling in the beyond-classical regime, showing quantum models can learn distributions unreachable by classical computers.

Conclusion: Establishes that both learning and sampling can be performed efficiently in quantum systems, opening new possibilities for quantum-enhanced generative models with provable advantage.
...
### [197] [Database Views as Explanations for Relational Deep Learning](https://arxiv.org/abs/2509.09482)
*Agapi Rissaki,Ilias Fountalis,Wolfgang Gatterbauer,Benny Kimelfeld*

Main category: cs.DB

TL;DR: A framework for explaining ML models over relational databases using view definitions that highlight database parts contributing to predictions, with model-agnostic and hetero-GNN-specific techniques.


<details>
  <summary>Details</summary>
Motivation: Deep learning models over relational databases are complex and hard to explain in human-understandable terms, making it difficult to understand how models use data for predictions.

Method: Global abductive explanations using view definitions adapted from determinacy concept, with heuristic algorithms to avoid exhaustive search and learnable masking for hetero-GNNs.

Result: Extensive evaluation on RelBench collection shows usefulness of explanations and efficiency of generation across various domains and record-level tasks.

Conclusion: The framework provides effective explanations for ML models over relational databases, balancing determinacy and conciseness while controlling granularity through different view definition fragments.
...
### [198] [Multi Robot Coordination in Highly Dynamic Environments: Tackling Asymmetric Obstacles and Limited Communication](https://arxiv.org/abs/2509.08859)
...
Result: Experimental validation shows 52% reduction in task overlaps in limited communication settings, with significant decrease in frequently reallocated tasks.

Conclusion: The presented architecture effectively handles scenarios with active asymmetric obstacles, poor communication, and partial observability, demonstrating practical applicability in real-world RoboCup competitions.
...
### [199] [KoopMotion: Learning Almost Divergence Free Koopman Flow Fields for Motion Planning](https://arxiv.org/abs/2509.09074)
*Alice Kate Li,Thales C Silva,Victoria Edwards,Vijay Kumar,M. Ani Hsieh*

Main category: cs.RO

TL;DR: KoopMotion is a flow field-based motion planning method that uses Koopman operators to create dynamical systems that converge to desired trajectories from any initial state, requiring only 3% of dataset samples for efficient planning.


<details>
  <summary>Details</summary>
Motivation: Koopman operator theory effectively models dynamical systems but doesn't inherently ensure convergence to desired trajectories or goals, which is crucial for learning from demonstrations.

Method: Represents motion flow fields as dynamical systems parameterized by Koopman Operators, leveraging divergence properties to obtain smooth motion fields that converge to reference trajectories and track them to endpoints.

Result: Effective performance on LASA handwriting dataset and 3D manipulator trajectory dataset, with successful physical robot experiments in non-static fluid flow environments. Highly sample efficient (3% of dataset) and significantly outperforms baselines in spatial and temporal dynamics modeling.

Conclusion: KoopMotion provides an efficient and effective approach for motion planning that ensures convergence to desired trajectories while maintaining high sample efficiency and improved modeling performance over existing methods.
...
TL;DR: OmniEVA is an embodied AI planner that addresses geometric adaptability and embodiment constraint gaps in multimodal LLMs through task-adaptive 3D grounding and embodiment-aware reasoning.


<details>
  <summary>Details</summary>
Motivation: Current MLLM-based embodied systems suffer from insufficient spatial information (Geometric Adaptability Gap) and neglect of physical robot constraints (Embodiment Constraint Gap), limiting their practical applicability.

Method: Two key innovations: 1) Task-Adaptive 3D Grounding with gated router for selective 3D fusion based on context, 2) Embodiment-Aware Reasoning framework that incorporates both task goals and physical constraints.

Result: Achieves state-of-the-art general embodied reasoning performance and demonstrates strong capabilities across diverse downstream scenarios including primitive and composite tasks.

Conclusion: OmniEVA provides robust and versatile planning capabilities by effectively addressing both geometric adaptability and embodiment constraint limitations in embodied AI systems.
...
### [201] [SimpleVLA-RL: Scaling VLA Training via Reinforcement Learning](https://arxiv.org/abs/2509.09674)
...
TL;DR: SimpleVLA-RL is an efficient RL framework for Vision-Language-Action models that reduces dependence on expensive human-operated training data and improves generalization through reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Address two fundamental challenges in VLA models: scarcity of large-scale human-operated robotic trajectories for supervised fine-tuning, and limited generalization to tasks with distribution shift.

Method: Builds upon veRL framework with VLA-specific trajectory sampling, scalable parallelization, multi-environment rendering, and optimized loss computation. Applies reinforcement learning to enhance long-horizon step-by-step action planning.

Result: Achieves state-of-the-art performance on LIBERO benchmark and outperforms baseline models on RoboTwin 1.0 & 2.0. Reduces data dependence and enables robust generalization, surpassing supervised fine-tuning in real-world tasks.

Conclusion: RL can significantly improve VLA model performance, with the framework discovering novel patterns ("pushcut") beyond previous training, demonstrating the potential of RL for enhancing robotic manipulation capabilities.
...
### [202] [ObjectReact: Learning Object-Relative Control for Visual Navigation](https://arxiv.org/abs/2509.09594)
*Sourav Garg,Dustin Craggs,Vineeth Bhat,Lachlan Mares,Stefan Podgorski,Madhava Krishna,Feras Dayoub,Ian Reid*

Main category: cs.RO

TL;DR: Object-relative visual navigation using 3D scene graphs instead of image matching, enabling better cross-embodiment generalization and reverse trajectory navigation.


<details>
  <summary>Details</summary>
Motivation: Image-level representations are tied to agent pose and embodiment, limiting navigation flexibility. Objects offer invariant world representation that enables new routes without imitation learning and better cross-embodiment deployment.

Method: Proposes topometric map as relative 3D scene graph for object-level path planning. Trains ObjectReact local controller conditioned on WayObject Costmap instead of RGB input, eliminating explicit image matching.

Result: Demonstrates superior performance over image-relative methods across sensor height variations and challenging navigation tasks like reverse direction traversal. Sim-only policy generalizes well to real-world indoor environments.

Conclusion: Object-relative control paradigm enables more flexible navigation with higher invariance to embodiment changes, decouples control from image matching, and supports novel route traversal without strict imitation requirements.
...
### [203] [Dexplore: Scalable Neural Control for Dexterous Manipulation from Reference-Scoped Exploration](https://arxiv.org/abs/2509.09671)
*Sirui Xu,Yu-Wei Chao,Liuyu Bian,Arsalan Mousavian,Yu-Xiong Wang,Liang-Yan Gui,Wei Yang*

Main category: cs.RO

TL;DR: Dexplore is a unified optimization method that learns robot control policies directly from human motion-capture data through joint retargeting and tracking, using demonstrations as soft guidance rather than ground truth to enable robot-specific strategies and scale to large datasets.
...
Method: Single-loop optimization that jointly performs retargeting and tracking using reinforcement learning. Derives adaptive spatial scopes from raw trajectories and trains policies to stay in-scope while minimizing control effort and accomplishing tasks. Distills into vision-based, skill-conditioned generative controller.

Result: Unified formulation preserves demonstration intent, enables robot-specific strategies, improves robustness to noise, and scales to large demonstration corpora. Creates effective training signals from imperfect demonstrations.

Conclusion: Dexplore provides a principled bridge that transforms imperfect human demonstrations into effective training for dexterous robotic manipulation, supporting generalization across objects and real-world deployment.
...
### [204] [Generative Engine Optimization: How to Dominate AI Search](https://arxiv.org/abs/2509.08919)
*Mahe Chen,Xiaoxuan Wang,Kaiwen Chen,Nick Koudas*

Main category: cs.IR

TL;DR: AI-powered search engines (ChatGPT, Perplexity, Gemini) fundamentally differ from traditional search, requiring new Generative Engine Optimization (GEO) strategies focused on earned media dominance and machine-scannable content.


<details>
  <summary>Details</summary>
Motivation: The shift from traditional ranked-list search to AI-generated answers with citations challenges existing SEO practices and necessitates understanding how AI search engines source information differently.
...
Result: AI search shows systematic bias towards earned media (third-party authoritative sources) over brand-owned and social content, unlike Google's balanced mix. Different AI services vary in domain diversity, freshness, cross-language stability, and phrasing sensitivity.

Conclusion: Proposes GEO framework with actionable strategies: engineer machine-scannable content, dominate earned media for AI authority, adopt engine-specific approaches, and overcome big brand bias for niche players in generative search landscape.
...
### [205] [Envy-Free but Still Unfair: Envy-Freeness Up To One Item (EF-1) in Personalized Recommendation](https://arxiv.org/abs/2509.09037)
*Amanda Aird,Ben Armstrong,Nicholas Mattei,Robin Burke*

Main category: cs.IR

TL;DR: Envy-freeness (EF) and EF-1 are traditional fairness concepts from economics, but are inappropriate for personalized recommendation systems where individual preferences vary significantly.


<details>
  <summary>Details</summary>
Motivation: To examine the applicability of envy-freeness concepts from economics and game theory to modern recommendation systems, particularly questioning their suitability in personalized settings.

Method: Position paper providing an overview of envy-freeness concepts, their historical use in economics and social choice theory, and analysis of their limitations in recommendation systems.

Result: Envy-freeness measures are found to be unsuitable for fairness assessment in personalized recommendation contexts where user preferences differ substantially.

Conclusion: Traditional envy-based fairness concepts should not be directly applied to personalized recommendation systems, as they fail to account for the fundamental role of individual preferences in these settings.
...
### [206] [Retrieval-Augmented Generation for Reliable Interpretation of Radio Regulations](https://arxiv.org/abs/2509.09651)
...
<details>
  <summary>Details</summary>
Motivation: Question answering in radio regulations is legally sensitive and high-stakes, requiring accurate and reliable information retrieval from authoritative sources.

Method: Proposed a telecom-specific RAG pipeline with automated filtering and human validation to create evaluation sets, plus a domain-specific retrieval metric.

Result: Retriever achieved ~97% accuracy on domain-specific metric, and pipeline improved generation accuracy across all models (12% relative improvement for GPT-4o).

Conclusion: Carefully targeted grounding provides a strong baseline and effective domain-specific solution for regulatory question answering.
...
### [207] [We're Still Doing It (All) Wrong: Recommender Systems, Fifteen Years Later](https://arxiv.org/abs/2509.09414)
*Alan Said,Maria Soledad Pera,Michael D. Ekstrand*

Main category: cs.IR

TL;DR: Recommender systems research continues to suffer from foundational flaws identified in 2011, with increasing complexity masking persistent methodological and epistemological issues that require fundamental paradigm shifts.


<details>
  <summary>Details</summary>
Motivation: To revisit and update Xavier Amatriain's 2011 critique of recommender systems research, demonstrating that core problems persist despite increased technical sophistication, and to advocate for systemic reform.
...
Result: The paper finds that recommender systems research has added layers of complexity without addressing foundational flaws, and identifies ongoing community efforts attempting to shift the paradigm through workshops, frameworks, and participatory approaches.

Conclusion: Meaningful change requires not just technical improvements but a fundamental reframing of recommender systems research's purpose, beneficiaries, and knowledge validation processes, grounded in epistemic humility, human impact, and sustainable practices.
...
### [208] [Representation-Aware Distributionally Robust Optimization: A Knowledge Transfer Framework](https://arxiv.org/abs/2509.09371)
*Zitao Wang,Nian Si,Molei Liu*

Main category: stat.ME

TL;DR: READ is a novel Wasserstein distributionally robust learning framework that incorporates representation awareness into robustness against distributional shifts, providing theoretical foundations, practical implementation methods, and demonstrating effectiveness through simulations and real-world applications.


<details>
  <summary>Details</summary>
Motivation: Traditional distributionally robust approaches treat all feature perturbations equally, but READ aims to differentially discourage perturbations along directions associated with informative representations while preserving invariant structure.

Method: Embeds multidimensional alignment parameter into transport cost, provides tractable reformulations as seminorm regularizations, uses robust Wasserstein profile inference for radius selection, and develops optimization algorithms to estimate projections onto solution surfaces.

Result: The framework unifies broad class of regularized estimators under DRO lens, enables construction of valid representation-aware confidence regions, and demonstrates effectiveness through extensive simulations and real-world studies.

Conclusion: READ provides a powerful robust estimation framework grounded in learning representation, offering both theoretical foundations and practical implementation for representation-aware distributionally robust learning.
...
### [209] [Towards Confidential and Efficient LLM Inference with Dual Privacy Protection](https://arxiv.org/abs/2509.09091)
*Honglan Yu,Yibin Wang,Feifei Dai,Dong Liu,Haihui Fan,Xiaoyan Gu*

Main category: cs.CR

TL;DR: CMIF is a confidential model inference framework that combines TEEs and optimized differential privacy to enable efficient LLM inference while preserving data privacy, reducing TEE overhead and maintaining model performance.


<details>
  <summary>Details</summary>
Motivation: Traditional TEE approaches have high inference latency and communication overhead for LLMs, while DP methods compromise model performance and semantic understanding. A solution is needed that maintains privacy without sacrificing efficiency.

Method: Deploys embedding layer in client-side TEE and subsequent layers on GPU servers. Optimizes the Report-Noisy-Max mechanism to protect sensitive inputs with minimal performance impact.

Result: Extensive experiments on Llama-series models show reduced inference overhead in TEEs while effectively preserving user data privacy.

Conclusion: CMIF successfully addresses the trade-off between privacy protection and inference efficiency for large language models, providing a practical framework for confidential model inference.
...
### [210] [DP-FedLoRA: Privacy-Enhanced Federated Fine-Tuning for On-Device Large Language Models](https://arxiv.org/abs/2509.09097)
...
<details>
  <summary>Details</summary>
Motivation: As on-device LLM systems become more common, federated fine-tuning processes sensitive user data, raising significant privacy concerns that need to be addressed within the federated learning framework.

Method: The framework integrates LoRA-based adaptation with differential privacy. Each client locally clips and perturbs its LoRA matrices using Gaussian noise to satisfy (ε, δ)-differential privacy, ensuring privacy while maintaining communication efficiency.

Result: Experimental results across mainstream benchmarks show that DP-FedLoRA delivers competitive performance while offering strong privacy guarantees. Theoretical analysis demonstrates unbiased updates and provides variance bounds for noise, offering practical guidance for privacy-budget calibration.

Conclusion: DP-FedLoRA paves the way for scalable and privacy-preserving LLM deployment in on-device environments by effectively balancing privacy protection with model performance through the integration of differential privacy with LoRA-based federated fine-tuning.
...
### [211] [Character-Level Perturbations Disrupt LLM Watermarks](https://arxiv.org/abs/2509.09112)
*Zhaoxi Zhang,Xiaomei Zhang,Yanjun Zhang,He Zhang,Shirui Pan,Bo Liu,Asif Qumer Gill,Leo Yu Zhang*

Main category: cs.CR

TL;DR: Character-level perturbations are more effective than token-level attacks for removing LLM watermarks, and genetic algorithm-based attacks can successfully defeat existing watermark schemes under realistic threat models.


<details>
  <summary>Details</summary>
Motivation: To address the misconception that watermark removal requires large perturbations or powerful adversaries, and to demonstrate vulnerabilities in existing LLM watermark schemes by showing effective removal under constrained threat models.

Method: Formalized LLM watermark system model, analyzed perturbation attack ranges, proposed character-level perturbations that disrupt tokenization, and developed genetic algorithm-based guided removal attacks using limited black-box detector queries.

Result: Character-level perturbations proved significantly more effective for watermark removal under restrictive threat models. The GA-based method demonstrated strong removal performance with limited detector queries, and adaptive compound attacks successfully defeated potential defenses.

Conclusion: Existing LLM watermark schemes have significant vulnerabilities, highlighting the urgent need for developing new robust watermarking mechanisms that can withstand character-level perturbation attacks.
...
### [212] [CryptGNN: Enabling Secure Inference for Graph Neural Networks](https://arxiv.org/abs/2509.09107)
*Pritam Sen,Yao Ma,Cristian Borcea*

Main category: cs.CR

TL;DR: CryptGNN is a secure inference solution for third-party GNN models in cloud MLaaS that protects client data, graph structure, and model parameters using distributed SMPC without trusted servers.


<details>
  <summary>Details</summary>
Motivation: To enable secure access to third-party graph neural network models in cloud MLaaS while protecting client data privacy and model intellectual property from cloud providers and other parties.

Method: Uses secure message passing and feature transformation layers with distributed secure multi-party computation (SMPC) techniques that work with any number of parties and requires no trusted server.

Result: The system is provably secure even if P-1 out of P parties collude, with theoretical analysis and empirical experiments demonstrating both security and efficiency.

Conclusion: CryptGNN provides an effective and secure solution for privacy-preserving GNN inference in cloud environments, protecting all parties' interests without requiring trust assumptions.
...
### [213] [ENSI: Efficient Non-Interactive Secure Inference for Large Language Models](https://arxiv.org/abs/2509.09424)
*Zhiyu He,Maojiang Wang,Xinwen Gao,Yuchuan Luo,Lin Liu,Shaojing Fu*

Main category: cs.CR

TL;DR: ENSI is a non-interactive secure inference framework for LLMs that co-designs cryptographic protocols with LLM architecture, achieving 8x faster matrix multiplications and 2.6x speedup in softmax inference while reducing bootstrapping to 1%.


<details>
  <summary>Details</summary>
Motivation: Secure inference for LLMs faces challenges due to cryptographic protocol complexity combined with LLMs' massive scale and sophisticated architectures, limiting practical usability.

Method: Co-designs CKKS scheme with BitNet LLM variant, uses optimized encoding strategy, integrates sigmoid attention with HE as softmax alternative, and embeds bootstrapping in RMSNorm process.
...
Conclusion: ENSI provides an efficient non-interactive secure inference framework that significantly improves performance while maintaining privacy for LLM computations.
...
### [214] [Prompt Pirates Need a Map: Stealing Seeds helps Stealing Prompts](https://arxiv.org/abs/2509.09488)
*Felix Mächtle,Ashwath Shetty,Jonas Sander,Nils Loose,Sören Pirk,Thomas Eisenbarth*

Main category: cs.CR

TL;DR: Researchers identified a seed vulnerability in diffusion models that allows prompt stealing through seed brute-forcing and genetic algorithm optimization, achieving 8-11% better results than existing methods, and proposed effective countermeasures.


<details>
  <summary>Details</summary>
Motivation: Prompt theft poses critical security and privacy concerns for text-to-image generation models due to the intellectual and economic value embedded in prompts, yet existing prompt recovery methods are fundamentally limited.

Method: Identified CWE-339 vulnerability in noise generation (limited to 2^32 seed values), developed SeedSnitch for seed brute-forcing, and created PromptPirate - a genetic algorithm-based optimization method for prompt stealing using recovered seeds.

Result: 95% of images from CivitAI can have seeds brute-forced in 140 minutes per seed; PromptPirate outperforms state-of-the-art methods by 8-11% in LPIPS similarity for prompt recovery.

Conclusion: The study reveals critical vulnerabilities in diffusion models, demonstrates effective prompt stealing attacks, and provides straightforward countermeasures to protect against seed and prompt theft through coordinated mitigation efforts with developers.
...
### [215] [What Does Normal Even Mean? Evaluating Benign Traffic in Intrusion Detection Datasets](https://arxiv.org/abs/2509.09564)
...
Result: Found meaningful sub-categories within benign traffic that can potentially improve overall multi-classification performance in intrusion detection systems.

Conclusion: Benign traffic in intrusion detection datasets contains meaningful sub-structures that should be considered rather than treating it as a single homogeneous class, as this can enhance machine learning-based detection performance.
...
Method: Proposes a camera-based reconstruction pipeline with HDR image stacking, MTF inversion, vignetting correction, geometric undistortion, homography transformation, and color correction, combined with a Visual Difference Predictor (VDP) to model human visual system responses.

Result: Validated through three applications: defective pixel detection, color fringing awareness, and display non-uniformity evaluation. The framework provides uncertainty analysis with theoretical upper bounds for defect detection and confidence intervals for VDP quality scores.

Conclusion: CameraVDP enables cameras to function as precise display measurement instruments by accounting for both camera measurement inaccuracies and visual difference prediction, providing a comprehensive perceptual assessment framework for display evaluation.
...
### [217] [WarpPINN-fibers: improved cardiac strain estimation from cine-MR with physics-informed neural networks](https://arxiv.org/abs/2509.08872)
*Felipe Álvarez Barrientos,Tomás Banduc,Isabeau Sirven,Francisco Sahli Costabal*

Main category: eess.IV

TL;DR: WarpPINN-fibers is a physics-informed neural network that improves cardiac motion and strain analysis by incorporating fiber mechanics into deformation field prediction from MRI images.


<details>
  <summary>Details</summary>
Motivation: Traditional cardiac strain analysis methods lack fiber mechanics modeling, limiting their accuracy in explaining cardiac function and pathology detection.

Method: A neural network trained with three loss components: data-similarity between reference and warped images, near-incompressibility regularizer, and fiber-stretch penalization using synthetic fibers.

Result: Outperforms previous WarpPINN model and alternative methods in landmark-tracking and strain curve prediction on a 15-subject MRI benchmark.

Conclusion: Enables more precise cardiac strain quantification with accurate deformation fields consistent with fiber physiology using standard MRI.
...
### [218] [Virtual staining for 3D X-ray histology of bone implants](https://arxiv.org/abs/2509.09235)
*Sarah C. Irvine,Christian Lucas,Diana Krüger,Bianca Guedert,Julian Moosmann,Berit Zeller-Plumhoff*

Main category: eess.IV

TL;DR: This paper introduces virtual staining to 3D X-ray imaging by using cross-modality image translation to generate artificially stained slices from micro-CT scans, enabling label-free 3D tissue characterization without physical sectioning.


<details>
  <summary>Details</summary>
Motivation: 3D X-ray histology offers non-invasive volumetric imaging but lacks biochemical specificity compared to traditional histological stains. The researchers aim to extend virtual staining techniques from optical to X-ray domain to enhance interpretability without additional sample preparation.

Method: Used over 50 co-registered image pairs of micro-CT and toluidine blue-stained histology from bone-implant samples. Trained a modified CycleGAN network with pixelwise supervision and greyscale consistency terms, incorporating on-the-fly data augmentation for patch-based training.

Result: The model outperformed Pix2Pix and standard CycleGAN baselines across SSIM, PSNR, and LPIPS metrics. It produced histologically realistic color outputs while preserving high-resolution structural detail, enabling generation of virtually stained 3D datasets from full CT volumes.
...
### [219] [Dynamic Structural Recovery Parameters Enhance Prediction of Visual Outcomes After Macular Hole Surgery](https://arxiv.org/abs/2509.09227)
...
Method: Used longitudinal OCT data at five timepoints, developed stage-specific segmentation models, extracted quantitative/qualitative/dynamic features, built logistic regression models with/without dynamic parameters, and created multimodal DL combining clinical variables, OCT features, and raw images.

Result: Segmentation achieved high accuracy (mean Dice > 0.89). Dynamic recovery rates improved logistic regression AUC, especially at 3-month follow-up. Multimodal DL outperformed regression with up to 0.12 higher AUC, demonstrating complementary value of raw images and dynamic parameters.
...
### [220] [In-Loop Filtering Using Learned Look-Up Tables for Video Coding](https://arxiv.org/abs/2509.09494)
*Zhuoyuan Li,Jiacheng Li,Yao Li,Jialin Li,Li Li,Dong Liu,Feng Wu*

Main category: eess.IV

TL;DR: Proposes LUT-ILF++, a lookup table-based in-loop filtering framework that replaces heavy DNN computations with efficient LUT retrieval and interpolation, achieving significant bitrate reduction with lower complexity.


<details>
  <summary>Details</summary>
Motivation: Neural network-based in-loop filtering provides excellent coding gains but has high computational complexity and hardware demands, making it impractical for general use. The research aims to create a practical alternative using lookup tables.

Method: Trains a DNN with restricted reference range, caches all possible outputs into LUTs, then uses indexing mechanisms and interpolation for filtering. Introduces multiple filtering LUT cooperation, cross-component indexing, and LUT compaction for storage efficiency.

Result: Achieves 0.82%/2.97%/1.63% and 0.85%/4.11%/2.06% bitrate reduction under AI and RA configurations respectively, with much lower time complexity and storage cost compared to DNN-based solutions.

Conclusion: LUT-ILF++ provides a practical and efficient alternative to DNN-based in-loop filtering, delivering substantial coding gains with significantly reduced computational and storage requirements, making it suitable for next-generation video coding standards.
...
### [221] [A modified RIME algorithm with covariance learning and diversity enhancement for numerical optimization](https://arxiv.org/abs/2509.09529)
*Shangqing Shi,Luoxiao Zhang,Yuchen Yin,Xiong Yang,Hoileong Lee*

Main category: cs.NE

TL;DR: MRIME-CD is an enhanced RIME algorithm that uses covariance learning and diversity enhancement strategies to overcome RIME's limitations of poor population diversity and local optima trapping, achieving better accuracy, convergence, and stability.


<details>
  <summary>Details</summary>
Motivation: The RIME algorithm suffers from rapid loss of population diversity during optimization, tendency to fall into local optima, and unbalanced exploitation-exploration capabilities, which limits its optimization performance.

Method: Three improvement strategies: 1) Covariance learning in soft-rime search stage to increase diversity, 2) Average bootstrapping in hard-rime puncture mechanism for better global search, 3) Stochastic covariance learning with new stagnation indicator to escape local optima.

Result: Extensive testing on CEC2017 and CEC2022 benchmarks shows MRIME-CD significantly outperforms basic RIME in solution accuracy, convergence speed, and stability, with statistical validation through Friedman, Wilcoxon, and Kruskal Wallis tests.

Conclusion: MRIME-CD effectively addresses RIME's shortcomings and demonstrates superior optimization performance, making it a significant improvement over the original algorithm.
...
### [222] [An improved educational competition optimizer with multi-covariance learning operators for global optimization problems](https://arxiv.org/abs/2509.09552)
*Baoqi Zhao,Xiong Yang,Hoileong Lee,Bowen Dong*

Main category: cs.NE

TL;DR: Enhanced Educational Competition Optimizer (IECO-MCO) improves the basic ECO algorithm using multi-covariance learning operators to better balance exploitation and exploration, achieving superior performance on benchmark functions and real-world optimization problems.
...
Method: Introduces three distinct covariance learning operators to enhance ECO performance, balancing exploitation/exploration and preventing premature convergence. Validated using CEC 2017 and CEC 2022 benchmark functions.

Result: IECO-MCO outperforms basic ECO and other algorithms in convergence speed, stability, and local optima avoidance. Achieved average rankings of 2.213 (vs basic) and 2.488 (vs improved algorithms) on test suites.

Conclusion: IECO-MCO demonstrates superior performance, robustness, and practical effectiveness for complex optimization problems, validated by statistical tests and real-world applications.
...
### [223] [PerFairX: Is There a Balance Between Fairness and Personality in Large Language Model Recommendations?](https://arxiv.org/abs/2509.08829)
*Chandan Kumar Sah*

Main category: cs.CY

TL;DR: PerFairX framework evaluates trade-offs between personality-based personalization and demographic fairness in LLM recommender systems, revealing that personality-aware prompts improve individual alignment but worsen fairness disparities.


<details>
  <summary>Details</summary>
Motivation: Address the tension between achieving psychological alignment through personality-based recommendations and ensuring demographic fairness in LLM-powered recommender systems.

Method: Propose PerFairX evaluation framework to quantify personalization-fairness trade-offs. Benchmark ChatGPT and DeepSeek LLMs using neutral and personality-sensitive prompts on MovieLens 10M and Last.fm 360K datasets across diverse user profiles.

Result: Personality-aware prompting significantly improves individual trait alignment but exacerbates fairness disparities across demographic groups. DeepSeek achieves stronger psychological fit but higher prompt sensitivity, while ChatGPT delivers stable but less personalized outputs.

Conclusion: PerFairX provides a principled benchmark to guide development of equitable and psychologically informed LLM-based recommender systems for inclusive, user-centric AI applications in continual learning contexts.
...
### [224] [Deep opacity and AI: A threat to XAI and to privacy protection mechanisms](https://arxiv.org/abs/2509.08835)
*Vincent C. Müller*
...
<details>
  <summary>Details</summary>
Motivation: To analyze how different types of opacity in AI systems undermine privacy protection mechanisms like informed consent and anonymity.

Method: Distinguishes three types of opacity: shallow opacity (subjects don't know), standard black box opacity (analysts don't know), and deep opacity (analysts cannot possibly know). Analyzes how each affects privacy justification.

Result: Shows that opacity prevents agents from providing necessary justifications for privacy protection, making big data analytics exacerbate privacy problems and reduce effectiveness of remedies.

Conclusion: Big data analytics worsens privacy problems and makes existing remedies less effective, though technical solutions may help address the opacity issues.
...
### [225] [Safe and Certifiable AI Systems: Concepts, Challenges, and Lessons Learned](https://arxiv.org/abs/2509.08852)
...
Result: The approach provides transparent and reproducible evidence of model quality in real-world settings, with practical lessons learned from application including common pitfalls like data leakage, inadequate domain definitions, bias neglect, and lack of distribution drift controls.

Conclusion: By aligning technical best practices with emerging European standards, the framework offers regulators, providers, and users a practical roadmap for legally compliant, functionally trustworthy, and certifiable AI systems, with ongoing research on robustness, fairness, and post-certification requirements.
...
### [226] [A vibe coding learning design to enhance EFL students' talking to, through, and about AI](https://arxiv.org/abs/2509.08854)
*David James Woo,Kai Guo,Yangyang Yu*

Main category: cs.CY

TL;DR: Pilot study on vibe coding (AI-assisted natural language programming) for EFL education, showing that students' success depends on their prompt engineering skills, AI mental models, and authorship negotiation approaches.
...
Method: Developed a human-AI meta-languaging framework with three dimensions, created a 4-hour workshop using backward design principles, and conducted a case study with two EFL students. Data collected from worksheets, video recordings, think-aloud protocols, screen recordings, and AI-generated images.

Result: Contrasting outcomes: one student successfully created a functional application matching intended design, while another encountered technical difficulties with significant gaps between design and functionality. Differences in prompt engineering approaches revealed varying AI mental models and authorship attribution tensions.
...
### [227] [Investigating Student Interaction Patterns with Large Language Model-Powered Course Assistants in Computer Science Courses](https://arxiv.org/abs/2509.08862)
*Chang Liu,Loc Hoang,Andrew Stolman,Rene F. Kizilcec,Bo Wu*

Main category: cs.CY

TL;DR: LLM-powered course assistant deployed to 2000+ CS students shows strong evening/night usage, helps address temporal support gaps, with mostly correct responses but limited higher-order questioning capabilities.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of providing flexible academic support outside scheduled hours by leveraging LLMs to bridge the temporal gap in student assistance.

Method: Developed and deployed an LLM-powered course assistant across 6 computer science courses at 3 institutions, analyzed interaction data from 2000+ students, and manually annotated 200 conversations per course.

Result: Usage was strong during evenings/nights and higher in introductory courses; most responses were correct and helpful (few unhelpful/erroneous); only 11% of conversations contained LLM-generated follow-up questions; limited capability in generating higher-order cognitive questions.

Conclusion: Opportunities exist for pedagogically-oriented LLM systems with greater educator involvement in configuring prompts, content, and policies to enhance educational support.
...
### [228] [Decentralising LLM Alignment: A Case for Context, Pluralism, and Participation](https://arxiv.org/abs/2509.08858)
*Oriane Peter,Kate Devlin*

Main category: cs.CY

TL;DR: Current LLM alignment methods centralize control and impose narrow normative preferences. This paper proposes decentralizing alignment through context, pluralism, and participation to resist epistemic injustice.


<details>
  <summary>Details</summary>
Motivation: Current alignment techniques predominantly mirror the preferences of narrow reference groups, imposing their values on diverse users and centralizing control over knowledge production within influential institutions.

Method: Proposes decentralizing alignment through three key characteristics: context (delineating context-of-use), pluralism (incorporating diverse values), and participation (engaging broader user base). Grounds these features in concrete use cases.

Result: Demonstrates the critical importance of context-specific alignment practices and provides concrete examples illustrating how decentralization strategies can be implemented across different usage contexts.

Conclusion: LLM alignment can serve as a site of resistance against epistemic injustice and democratic erosion, though these strategies alone cannot substitute for broader societal changes needed for true democratization of AI governance.
...
### [229] [Incorporating AI Incident Reporting into Telecommunications Law and Policy: Insights from India](https://arxiv.org/abs/2509.09508)
*Avinash Agarwal,Manisha J. Nene*

Main category: cs.CY

TL;DR: AI integration in telecom creates new risks beyond traditional cybersecurity frameworks. This paper defines telecom AI incidents as a distinct regulatory category, analyzes India's regulatory gaps, and proposes mandatory reporting and governance solutions.


<details>
  <summary>Details</summary>
Motivation: AI in telecommunications introduces novel risks like algorithmic bias and unpredictable behavior that existing cybersecurity and data protection frameworks don't adequately address, creating regulatory gaps that need urgent attention.

Method: The study uses India as a case study to analyze existing digital regulations (Telecommunications Act 2023, CERT-In Rules, DPDP Act 2023), identifies regulatory gaps for AI incidents, and examines structural barriers to disclosure and limitations of AI incident repositories.

Result: Analysis reveals significant regulatory gaps in India's existing legal instruments which focus on cybersecurity and data breaches but fail to address AI-specific operational incidents like performance degradation and algorithmic bias.

Conclusion: The paper proposes targeted policy recommendations including mandatory reporting for high-risk AI failures, designating a nodal agency for incident management, and developing standardized reporting frameworks to enhance regulatory clarity and resilience, providing a replicable blueprint for other nations.
...
### [230] [Bona fide Cross Testing Reveals Weak Spot in Audio Deepfake Detection Systems](https://arxiv.org/abs/2509.09204)
*Chin Yuen Kwok,Jia Qi Yip,Zhen Qiu,Chi Hung Chi,Kwok Yan Lam*

Main category: cs.SD

TL;DR: Proposes bona fide cross-testing framework to address biases in audio deepfake detection evaluation, using diverse bona fide datasets and aggregated EERs for more balanced assessment.


<details>
  <summary>Details</summary>
Motivation: Traditional ADD evaluation disproportionately weights synthesizers with more samples and lacks diversity in bona fide speech, limiting real-world applicability and reliability.
...
Conclusion: The proposed evaluation framework addresses key limitations in current ADD assessment and provides a more reliable approach, with a new dataset released to support further research.
...
### [231] [DiFlow-TTS: Discrete Flow Matching with Factorized Speech Tokens for Low-Latency Zero-Shot Text-To-Speech](https://arxiv.org/abs/2509.09631)
*Ngoc-Son Nguyen,Hieu-Nghia Huynh-Nguyen,Thanh V. T. Tran,Truong-Son Hy,Van Nguyen*

Main category: cs.SD

TL;DR: DiFlow-TTS is the first discrete flow matching model for zero-shot text-to-speech synthesis that achieves fast inference (25.8x speedup) while maintaining high quality in naturalness, prosody, and speaker style preservation.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot TTS methods based on language models, diffusion, and flow matching suffer from slow inference speeds and repetition artifacts. Current flow-matching approaches embed discrete tokens into continuous space, failing to fully leverage the advantages of discrete representations for speech synthesis.

Method: DiFlow-TTS uses purely discrete flow matching with a factorized architecture that explicitly models speech attributes. It employs in-context learning by conditioning on text content, prosodic and acoustic attributes from reference speech, and uses separate prediction heads for prosody and acoustic details.

Result: The model achieves promising performance in naturalness, prosody, speaker style preservation, and energy control. It maintains compact model size and generates speech 25.8 times faster than latest baselines.

Conclusion: Discrete flow matching is an effective approach for zero-shot TTS that addresses speed and quality limitations of previous methods, demonstrating the potential of purely discrete generative modeling for speech synthesis.
...
### [232] [Adaptive Knowledge Distillation using a Device-Aware Teacher for Low-Complexity Acoustic Scene Classification](https://arxiv.org/abs/2509.09262)
*Seung Gyu Jeong,Seong Eun Kim*

Main category: cs.SD

TL;DR: A knowledge distillation system with device-aware feature alignment and test-time device-specific fine-tuning achieves 57.93% accuracy for low-complexity device-robust acoustic scene classification.
...
Method: Knowledge distillation framework with a CP-MobileNet student learning from a two-teacher ensemble (baseline PaSST teacher and a 'generalization expert' teacher trained with novel Device-Aware Feature Alignment loss), followed by device-specific fine-tuning using test-time device labels.
...
Conclusion: The proposed system effectively combines knowledge distillation with device-aware feature alignment and test-time device-specific adaptation to achieve robust performance under strict complexity constraints.
...
### [233] [Finite Scalar Quantization Enables Redundant and Transmission-Robust Neural Audio Compression at Low Bit-rates](https://arxiv.org/abs/2509.09550)
...
<details>
  <summary>Details</summary>
Motivation: Existing neural audio codecs mostly use RVQ, but FSQ offers simpler training and native single codebook support. The paper explores FSQ's inherent redundancy and robustness for noisy channel transmission.

Method: Introduced NeuCodec based on FSQ, conducted encoder distillation experiments to show different encoders can produce vastly different codes for same audio, and compared RVQ vs FSQ performance under simulated noisy channel conditions.
...
Conclusion: FSQ-based neural audio codecs like NeuCodec offer significant advantages in robustness for noisy channel transmission scenarios while maintaining comparable audio reconstruction quality to RVQ approaches.
...
### [234] [Benchmarking Energy Efficiency of Large Language Models Using vLLM](https://arxiv.org/abs/2509.08867)
*K. Pronk,Q. Zhao*

Main category: cs.SE

TL;DR: A benchmark for evaluating LLM energy efficiency in realistic production scenarios using vLLM, examining model size, architecture, and concurrent requests to provide sustainable AI development insights.


<details>
  <summary>Details</summary>
Motivation: The growing climate impact of LLMs due to high energy consumption requires better energy efficiency benchmarks that reflect real-world deployment conditions rather than artificial testing scenarios.

Method: Developed the LLM Efficiency Benchmark using vLLM (a production-ready LLM serving backend) to simulate realistic usage conditions and measure how model size, architecture, and concurrent request volume affect inference energy efficiency.
...
Conclusion: It is possible to create meaningful energy efficiency benchmarks that accurately represent real-world LLM usage, enabling developers to build more sustainable AI systems by understanding the energy impact of different model configurations and deployment scenarios.
...
### [235] [Probing Pre-trained Language Models on Code Changes: Insights from ReDef, a High-Confidence Just-in-Time Defect Prediction Dataset](https://arxiv.org/abs/2509.09192)
*Doha Nam,Taehyoun Kim,Duksan Ryu,Jongmoon Baik*

Main category: cs.SE

TL;DR: ReDef dataset provides high-confidence function-level defect labels using revert commits and GPT-assisted filtering. Evaluation shows diff encodings outperform whole-function formats for defect prediction, but PLMs rely on superficial cues rather than true semantic understanding of code changes.


<details>
  <summary>Details</summary>
Motivation: Existing JIT-SDP datasets suffer from noisy labels and low precision in identifying bug-inducing commits, necessitating a more reliable benchmark for evaluating defect prediction models.
...
### [236] [On Integrating Large Language Models and Scenario-Based Programming for Improving Software Reliability](https://arxiv.org/abs/2509.09194)
*Ayelet Berzack,Guy Katz*

Main category: cs.SE

TL;DR: Combining LLMs with traditional software engineering techniques like Scenario-Based Programming to create more reliable AI-assisted development with formal verification capabilities.


<details>
  <summary>Details</summary>
Motivation: LLMs help developers but often introduce errors with high confidence, requiring a more reliable integration method into software development cycles.

Method: Proposed methodology combining LLMs with Scenario-Based Programming (SBP) paradigm - an event-driven, scenario-based approach that allows human expertise injection and output verification.

Result: Successfully designed and implemented Connect4 game agent that defeated strong existing agents, with some cases achieving formal verification of correctness.

Conclusion: The approach enables reliable LLM integration, reduces errors, allows formal verification, and provides interesting insights about ease-of-use in software development.
...
### [237] [LoCoBench: A Benchmark for Long-Context Large Language Models in Complex Software Engineering](https://arxiv.org/abs/2509.09614)
...
<details>
  <summary>Details</summary>
Motivation: The emergence of long-context language models with million-token windows creates opportunities for sophisticated code understanding, but existing benchmarks focus on single-function tasks and lack evaluation for long-context capabilities needed for real-world software development.

Method: Created 8,000 evaluation scenarios across 10 programming languages with context lengths spanning 10K to 1M tokens. Introduced 8 task categories covering architectural understanding, cross-file refactoring, multi-session development, bug investigation, feature implementation, code comprehension, integration testing, and security analysis. Developed through a 5-phase pipeline and comprehensive evaluation framework with 17 metrics across 4 dimensions.

Result: Evaluation of state-of-the-art long-context models revealed substantial performance gaps, demonstrating that long-context understanding in complex software development remains a significant unsolved challenge.

Conclusion: LoCoBench addresses the critical evaluation gap for long-context capabilities and shows that current models struggle with realistic software development scenarios, indicating this area requires more research attention. The benchmark is publicly available for community use.
...
TL;DR: Reduced diffusion MRI protocol from 15 to 8 features using AI optimization, cutting scan time from 30+ to 14 minutes while maintaining parameter accuracy and reproducibility.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion MRI protocols for neurite exchange imaging require long scan times, limiting clinical and research applications. A more efficient acquisition scheme is needed to enable broader use.
...
Result: Reduced protocol produced comparable parameter estimates and cortical maps to full protocol, with low estimation errors and minimal impact on test-retest variability. Outperformed theory-driven and heuristic methods, reducing water exchange time deviation by over 2x.

Conclusion: The hybrid optimization framework enables viable neurite exchange imaging in 14 minutes without parameter fidelity loss, supporting broader neuroscience and clinical applications while providing generalizable method for efficient protocol design.
...
### [239] [Personalized Sleep Prediction via Deep Adaptive Spatiotemporal Modeling and Sparse Data](https://arxiv.org/abs/2509.09018)
...
<details>
  <summary>Details</summary>
Motivation: Sleep forecasting enables proactive management of factors affecting restful sleep, improving mental and physical well-being for individuals and healthcare providers.

Method: Proposed adaptive spatial-temporal model (AdaST-Sleep) uses convolutional layers for spatial feature interactions, RNN layers for temporal health data, and domain classifier for cross-subject generalization. Tested with various input (3-11 days) and prediction (1-9 days) window sizes.
...
Conclusion: The framework provides robust and adaptable solution for personalized sleep forecasting using sparse data from commercial wearables and domain adaptation techniques.
...
### [240] [A Masked Representation Learning to Model Cardiac Functions Using Multiple Physiological Signals](https://arxiv.org/abs/2509.08830)
*Seong-A Park,Jong-Eui Chae,Sungdong Kim,Hyung-Chul Lee,Hyun-Lim Yang*

Main category: eess.SP

TL;DR: SNUPHY-M is a self-supervised learning model that analyzes ECG, PPG, and ABP signals simultaneously to extract enriched physiological features for improved hemodynamic monitoring and clinical predictions.
...
Method: The SNUPHY-M model uses self-supervised learning to restore three masked physiological signals (ECG, PPG, ABP) and extracts features reflecting electrical, pressure, and fluid characteristics of the cardiac cycle using non-invasive signals.

Result: The model significantly outperformed supervised or SSL models in clinical downstream tasks including hypotension, stroke volume, blood pressure, and age prediction, especially when using non-invasive signals.

Conclusion: SNUPHY-M is the first multi-modal SSL model for cardiovascular analysis that effectively supports clinical decision-making and enables precise diagnostics for early hemodynamic management without invasiveness.
...
### [241] [Deploying AI for Signal Processing education: Selected challenges and intriguing opportunities](https://arxiv.org/abs/2509.08950)
*Jarvis Haupt,Qin Lu,Yanning Shen,Jia Chen,Yue Dong,Dan McCreary,Mehmet Akçakaya,Georgios B. Giannakis*

Main category: eess.SP

TL;DR: AI tools show great potential for enhancing signal processing education through smart textbooks, but require addressing technical limitations like fairness, inclusivity, hallucination handling, and resource efficiency.


<details>
  <summary>Details</summary>
Motivation: While AI has achieved significant technical breakthroughs, the most profound challenges ahead involve fair and responsible use that genuinely improves the global human condition, particularly in educational applications.

Method: The article explores AI applications in education with focus on signal processing, providing primers on core technical issues including fairness, inclusivity, hallucination handling, resource efficiency, transparency, explainability, and trustworthiness through development of an immersive smart textbook.

Result: The article serves as a comprehensive resource demonstrating how AI can be practically applied to improve educational experiences while addressing critical technical and ethical considerations.

Conclusion: AI tools represent a promising approach for advancing engineering education, particularly in signal processing, when developed with careful attention to fairness, transparency, and responsible implementation.
...
### [242] [Ultrafast Deep Learning-Based Scatter Estimation in Cone-Beam Computed Tomography](https://arxiv.org/abs/2509.08973)
*Harshit Agrawal,Ari Hietanen,Simo Särkkä*

Main category: eess.SP

TL;DR: Deep learning-based scatter estimation for CBCT with optimized resolution to reduce computational costs while maintaining accuracy


<details>
  <summary>Details</summary>
Motivation: Scatter artifacts degrade CBCT image quality, but current deep learning methods have large memory footprints that limit deployment in mobile CBCT systems and edge devices

Method: Examined reconstruction error at six resolutions using four interpolation methods, trained state-of-the-art method across five image resolutions, evaluated FLOPs, inference times, and GPU memory requirements

Result: Achieved 78-fold reduction in FLOPs while maintaining comparable performance (MAPE: 3.85% vs 4.42%, MSE: 1.34e-2 vs 2.01e-2), 16x faster inference, 12x less GPU memory usage

Conclusion: Downsampling plays crucial role in deep learning-based scatter estimation, enabling scatter correction in resource-constrained environments like mobile CBCT and edge devices
...
### [243] [Physics-informed waveform inversion using pretrained wavefield neural operators](https://arxiv.org/abs/2509.08967)
*Xinquan Huang,Fu Wang,Tariq Alkhalifah*

Main category: physics.geo-ph

TL;DR: Physics-informed FWI framework that integrates wave equation constraints to improve inversion accuracy while maintaining neural operator efficiency, reducing noise and artifacts in subsurface velocity models.


<details>
  <summary>Details</summary>
Motivation: Full waveform inversion (FWI) faces challenges with limited data null space leading to low-resolution models and high computational costs. Neural operator-based FWI improves efficiency but suffers from noisy and unstable inversion performance.

Method: Integrates physics constraint term (wave equation) into loss function alongside L2 norm objective. Uses initial model to simulate wavefields, then evaluates loss based on physical law compliance and data matching to reduce noise and artifacts.

Result: Numerical experiments on OpenFWI and Overthrust models show superior performance with cleaner, more accurate subsurface velocity models compared to vanilla approaches, while maintaining computational efficiency.

Conclusion: The physics-informed framework represents a significant advancement for practical FWI applications, enabling more accurate subsurface monitoring with reduced computational burden for real-time applications.
...
### [244] [DeepTV: A neural network approach for total variation minimization](https://arxiv.org/abs/2409.05569)
*Andreas Langer,Sara Behnamian*

Main category: math.NA

TL;DR: Neural networks for solving infinite-dimensional total variation minimization problems face theoretical issues with solution existence, requiring auxiliary problems and Γ-convergence analysis.
...
Method: Propose an auxiliary neural network problem that has a solution and prove its Γ-convergence to the original problem. Develop a discrete version of the auxiliary problem and show its Γ-convergence, connecting it to finite difference discretization of the total variation minimization.

Result: Theoretical analysis shows the auxiliary neural network problem has solutions and converges to the original problem. Numerical experiments support the theoretical findings.

Conclusion: The proposed approach successfully addresses the theoretical issues of neural networks for total variation minimization through auxiliary problems and Γ-convergence analysis, with numerical validation supporting the methodology.
...
TL;DR: First simultaneous approximation theory for deep neural networks on manifolds, proving ReLU networks with bounded weights can approximate Sobolev space functions with optimal parameter efficiency that overcomes curse of dimensionality.
...
Method: Prove that constant-depth ReLU^(k-1) networks with bounded weights can approximate functions in Sobolev spaces W_p^k(M^d) to error ε in W_p^s norm using O(ε^{-d/(k-s)}) parameters, with matching lower bound showing near-optimality.

Result: Achieves approximation rate that depends only on intrinsic dimension d, overcoming curse of dimensionality. Network architecture leverages sparse structure to exploit manifold's low-dimensional geometry efficiently.

Conclusion: Provides theoretical foundation for learning PDEs on manifolds involving derivatives, with novel VC dimension and pseudo-dimension estimates for network's high-order derivative classes establishing complexity bounds.
...
### [246] [Scalable extensions to given-data Sobol' index estimators](https://arxiv.org/abs/2509.09078)
...
Result: Streaming algorithm achieves comparable accuracy and runtime with lower memory requirements; equiprobable partitions can introduce significant bias even at large sample sizes.

Conclusion: The proposed extensions enable efficient variance-based sensitivity analysis for large-scale models like neural networks, overcoming memory limitations and distribution constraints of existing methods.
...
### [247] [Global Optimization of Stochastic Black-Box Functions with Arbitrary Noise Distributions using Wilson Score Kernel Density Estimation](https://arxiv.org/abs/2509.09238)
*Thorbjørn Mosekjær Iversen,Lars Carøe Sørensen,Simon Faarvang Mathiesen,Henrik Gordon Petersen*

Main category: stat.ML

TL;DR: WS-KDE provides reliable confidence bounds for stochastic black-box functions in [0,1] range, enabling stable Bayesian optimization without requiring many evaluations or disturbance modeling.


<details>
  <summary>Details</summary>
Motivation: Optimizing time-expensive stochastic black-box functions in robotics requires efficient Bayesian optimization, but existing function estimators need many evaluations or disturbance modeling to provide good confidence bounds.
...
Result: WS-KDE demonstrates excellent performance as confidence bounds for Bayesian optimization, enabling stable global optimization across various cost functions.

Conclusion: WS-KDE opens up Bayesian optimization for a wider range of stochastic functions without requiring extensive evaluations or explicit disturbance modeling, validated through simulations and trap design applications.
...
### [248] [Low-degree lower bounds via almost orthonormal bases](https://arxiv.org/abs/2509.09353)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for low-degree polynomial analysis break down for estimation tasks or complex testing problems where the null distribution has planted structures, requiring delicate technical workarounds.

Method: Construct a basis of polynomials that is almost orthonormal under the null distribution in regimes where statistical-computational gaps arise, providing a direct proof strategy for low-degree lower bounds.
