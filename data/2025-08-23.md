...
Motivation: Heatmap regression methods for facial landmark detection use Soft-argmax as a differentiable approximation for argmax, but this may not be the optimal approach. The authors aim to explore alternative training objectives.
...
Result: The method achieves state-of-the-art performance on three facial landmark benchmarks (WFLW, COFW, and 300W), converges 2.2x faster during training, and maintains better/competitive accuracy compared to existing approaches.

Conclusion: Soft-argmax is not the only viable approach for facial landmark detection, and structured prediction-based alternatives can provide superior performance with faster training convergence.
...
<details>
  <summary>Details</summary>
Motivation: To improve image classification by leveraging GCNs' ability to handle complex data structures and relational data modeling, moving beyond conventional CNNs.

Method: Represent images as graphs with pixels/regions as vertices, refine graphs using Delaunay triangulations, and integrate GCNs with Voronoi diagrams for optimized representation.
...
Conclusion: The combination of GCNs with Voronoi diagrams presents an effective novel approach for image classification and expands graph-based learning applications in computer vision and unstructured data analysis.
...
<details>
  <summary>Details</summary>
Motivation: Existing solutions rely on pseudo-depth, CAD models, or multi-stage cascades. There's a need for a simpler, RGB-only alternative that learns directly at the category level without additional data.

Method: Augments a transformer detector with lightweight pose head, bounding-box-conditioned translation module, and 6D-aware Hungarian matching cost. Trained end-to-end with only RGB images and category-level pose labels.
...
Conclusion: Object detection and 9-DoF pose estimation can be unified with high performance using only RGB images, demonstrating that complex multi-stage approaches and additional data sources are not necessary for category-level pose estimation.
...
Method: Paired-Sampling Contrastive Framework that uses automatically matched pairs of genuine and attack selfies to learn modality-agnostic liveness cues through unified training.
...
Method: TAIGen injects perturbations during the mixing step interval without processing all timesteps. Uses selective RGB channel strategy: attention maps on red channel, GradCAM-guided perturbations on green and blue channels to preserve structure while maximizing misclassification.

Result: Achieves PSNR above 30 dB across datasets. On ImageNet with VGGNet: 70.6% success vs ResNet, 80.8% vs MNASNet, 97.8% vs ShuffleNet. Generates adversarial examples 10x faster than existing diffusion-based attacks with lowest robust accuracy.

Conclusion: TAIGen provides an efficient, training-free black-box method for high-quality adversarial image generation that significantly reduces computational requirements while maintaining attack effectiveness and visual quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing CVP methods are limited to mask domain and don't fully exploit RGB domain potential, leaving uncertainty issues unresolved. The paper aims to bridge this gap with a comprehensive approach.

Method: Formulates CVP as optimization problem and unfolds iterative solution into multi-stage network with three modules: CORE (mask domain reversible modeling), CARE (RGB domain reversible modeling), and FINE (targeted Bernoulli diffusion for uncertain regions).

Result: RUN++ efficiently directs focus to ambiguous areas, significantly reducing false positives/negatives while avoiding full-image diffusion computational costs. Introduces robust CVP paradigm effective under real-world degradations.

Conclusion: The proposed reversible unfolding network with generative refinement provides a principled approach to CVP that synergizes unfolding networks with diffusion models, creating an effective bi-level optimization framework for concealed object perception.
...
<details>
  <summary>Details</summary>
Motivation: Livestock methane emissions account for 32% of human-caused methane, making automated monitoring crucial for climate mitigation strategies. Current methods lack real-time capabilities and comprehensive dietary correlation analysis.

Method: Hybrid vision transformer with Mix Twin encoder alternating between spatially-reduced global attention and locally-grouped attention mechanisms, plus lightweight LR-ASPP decoder for multi-scale feature aggregation. Unified framework for simultaneous methane segmentation and dietary classification.

Result: Achieves 74.47% mIoU and 83.63% mF1 for segmentation, perfect 100% dietary classification accuracy, with exceptional efficiency (3.348M params, 3.428G FLOPs, 114.9 FPS). First comprehensive beef cattle methane dataset with 11,694 annotated frames across three diets.

Conclusion: GasTwinFormer establishes a practical solution for real-time livestock emission monitoring, demonstrating effectiveness of diet-emission correlations through extensive ablation studies validating each architectural component.
...
Method: A novel flow matching framework with curvature regularization that learns smooth non-linear trajectories by directly incorporating curvature guidance into the flow path.
...
Conclusion: Curvature-aware modeling substantially enhances the model's ability to faithfully follow complex instructions while maintaining high image quality, demonstrating the importance of trajectory curvature for semantic alignment.
...
<details>
  <summary>Details</summary>
Motivation: To overcome dataset biases and reliance on subjective labels in no-reference image quality assessment, enabling better generalization without pristine references or auxiliary modalities.

Method: Combines ranking and contrastive learning with novel higher-order ranking loss, embedding distance loss, and training-time contrastive alignment using structured textual prompts. Also introduces HiRQA-S lightweight variant for real-time deployment.

Result: Achieves state-of-the-art performance on synthetic and authentic benchmarks, effectively generalizes to authentic degradations (lens flare, haze, motion blur, low-light), with HiRQA-S offering 3.5ms inference time per image.

Conclusion: HiRQA demonstrates strong generalization ability, scalability, and real-time deployment potential, advancing self-supervised NR-IQA without subjective labels or reference images.
...
<details>
  <summary>Details</summary>
Motivation: Multi-view 3D reconstruction applications in emergency response and tactical scenarios require near-real-time latency and operate in dynamic edge environments prone to reliability issues from spatiotemporally correlated disruptions that degrade reconstruction quality.
...
Result: Demonstrated benefits over traditional baseline strategies using publicly available and customized 3D datasets, showing guaranteed reconstruction quality satisfaction even with camera disruptions.

Conclusion: The portfolio-theoretic approach provides an effective solution for reliable 3D reconstruction in dynamic edge environments, with genetic algorithm optimization enabling quick convergence for realistic system settings.
...
<details>
  <summary>Details</summary>
Motivation: Address the lack of transparency and interpretability in deep learning models for diabetic retinopathy diagnosis, which hinders clinical adoption despite their promising detection capabilities.

Method: Leverages Vision-Language Large Models (LVLMs) with specialized Medical Vision Encoder, LVLM Core, Multi-task Prompt Engineering, and Multi-stage Fine-tuning to analyze fundus images and generate comprehensive diagnostic reports with explanations.
...
<details>
  <summary>Details</summary>
Motivation: City mesh models lack realistic textures, limiting their use in virtual urban navigation and autonomous driving. Existing diffusion models struggle with 3D scene generation - video models have camera path adherence issues, while image models lack cross-view consistency.

Method: Three-stage pipeline: 1) Generate geometrically consistent sparse views using Cascaded Outpainting ControlNets, 2) Propagate denser intermediate views via AGInpaint component, 3) Eliminate visual inconsistencies globally using GCAlign module. Concurrent 3D Gaussian Splatting reconstruction initialized on mesh surface.
...
Conclusion: MeSS successfully addresses texture generation for city mesh models, producing high-quality, consistent outdoor scenes suitable for virtual navigation and autonomous driving applications with superior geometric alignment compared to previous methods.
...
Conclusion: The learning-based adversarial approach effectively exposes vulnerabilities in rule-based agent modeling for safety-critical applications, highlighting the need for more robust modeling strategies that can withstand adversarial scenarios in environments like autonomous driving.
...
<details>
  <summary>Details</summary>
Motivation: Existing renal pathology datasets and methods only provide binary masks, limiting precision in morphological quantification. Classical post-processing techniques individually fail to handle the diverse morphologies and complex connectivity in renal tissue.

Method: Integrated watershed, skeletonization, and morphological operations within a unified framework with adaptive geometric refinement and class-specific hyperparameter tuning through systematic optimization.

Result: Outperforms individual classical approaches and naive combinations, achieving robust separation of adherent and heterogeneous structures in binary masks.

Conclusion: Enables superior instance separation and facilitates more accurate morphometric analysis in renal pathology workflows, with the pipeline publicly available for use.
...
<details>
  <summary>Details</summary>
Motivation: To develop a cost-effective accident prediction system using only dash-cam video input, which is more challenging but easier to deploy than sensor-based systems, to improve road safety through timely warnings.
...
Result: STAGNet achieves higher average precision and mean time-to-collision values than previous methods, both in cross-validation on single datasets and when trained and tested on different datasets across three publicly available datasets.

Conclusion: The proposed STAGNet model provides an effective and deployable solution for accident prediction using only dash-cam video, demonstrating superior performance over existing methods in various testing scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D generative models operate predominantly within single-modality paradigms, overlooking the complementary benefits of multi-modality data and limiting available training datasets.

Method: 1) Collaborative multi-modal coding integrating modality-specific features while preserving unique strengths; 2) Auxiliary 2D and 3D supervision for robustness; 3) Triplane latent diffusion model for 3D asset generation.
...
Conclusion: TriMM effectively harnesses multi-modality data for 3D modeling, producing superior quality 3D assets with enhanced texture and geometric details through collaborative multi-modal coding and diffusion modeling.
...
<details>
  <summary>Details</summary>
Motivation: Existing contrastive learning methods for clustering suffer from class conflict problems and deviation between hard prototypes and true cluster centers, which limits their effectiveness.

Method: Proposes a framework with two modules: 1) Soft prototype contrastive module that uses sample-to-center probability as weights to calculate category prototypes, avoiding inter-class conflicts and reducing prototype drift; 2) Dual consistency learning module that aligns different transformations of the same sample and neighborhoods of different samples to ensure transformation-invariant semantics and compact intra-cluster distribution.

Result: Extensive experiments on five datasets demonstrate that the proposed method is effective and outperforms state-of-the-art approaches.

Conclusion: The center-oriented prototype contrastive clustering framework successfully addresses prototype drift and class conflict problems in contrastive learning for clustering, providing more reliable prototype calculations and improved clustering performance.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in aerial VLN due to extended trajectories and complex UAV maneuverability by leveraging multi-perspective advantages while maintaining manageable motion space through collaborative dual-altitude operation.

Method: Introduces DuAl-VLN task with two UAVs at distinct altitudes, creates HaL-13k dataset with 13,838 collaborative trajectories, and develops AeroDuo framework combining multimodal LLM for reasoning and lightweight policy for navigation with minimal coordinate exchange.

Result: Developed comprehensive dataset and framework enabling systematic evaluation of generalization across novel environments and unfamiliar targets through collaborative high-low UAV operation.

Conclusion: Dual-altitude UAV collaboration effectively addresses aerial VLN challenges by combining broad environmental reasoning with precise navigation, providing a scalable solution for complex outdoor navigation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models require sequential step-by-step generation which is computationally expensive. Existing acceleration methods use non-Markovian processes, but it's unclear if the original Markovian diffusion process can achieve similar efficiency.
...
Result: Extensive experiments on OpenAI ADM, Stable Diffusion, and Open Sora models show the method achieves high-quality generation with significantly reduced sampling steps while maintaining the original Markovian properties.

Conclusion: Accelerated sampling via skipped-step sampling is an intrinsic property of pretrained diffusion models, providing efficient generation without resorting to non-Markovian processes while maintaining quality.
...
Conclusion: Ano-NAViLa effectively addresses computational pathology challenges by leveraging pathology-specific knowledge through vision-language modeling, providing accurate anomaly detection with interpretable results.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods use first-detect-then-reason paradigm with one-to-one assignment, resulting in limited supervision range and suboptimal topology reasoning performance.
...
<details>
  <summary>Details</summary>
Motivation: Traditional design patent analysis relies heavily on patent images (sketches) which lack comprehensive visual context and semantic information, leading to ambiguities in prior art searches. Vision-language models like CLIP offer opportunities for more reliable AI-driven patent analysis.

Method: Developed DesignCLIP framework incorporating class-aware classification and contrastive learning. Utilizes generated detailed captions for patent images and multi-views image learning to address unique patent data characteristics. Built on a large-scale dataset of U.S. design patents.
...
Conclusion: The findings demonstrate the promise of multimodal approaches in advancing patent analysis, with DesignCLIP showing superior performance over existing methods in the patent domain.
...
<details>
  <summary>Details</summary>
Motivation: Current automated CHD detection methods in ultrasound videos neglect temporal information, limit to binary classification, and lack prediction calibration, which hinders clinical reliability.

Method: Temporal Prompt Alignment (TPA) extracts frame features using an image encoder, aggregates them with a temporal extractor, aligns video representations with class-specific text prompts via contrastive loss, and uses CVAESM module for uncertainty quantification and style modulation.
...
Conclusion: TPA effectively addresses limitations of current methods by integrating temporal modeling, prompt-aware learning, and uncertainty quantification, demonstrating superior performance and improved calibration for clinical applications.
...
<details>
  <summary>Details</summary>
Motivation: Traditional multi-camera systems struggle with real-time 3D tracking due to 2D video limitations and complex reconstruction. Basketball presents extreme MOT challenges with rapid player movements, frequent occlusions, and confined space.

Method: Created multimodal dataset with synchronized LiDAR point clouds and multi-view camera footage. Developed two tracking pipelines: real-time LiDAR-only tracking and multimodal fusion of LiDAR and camera data.

Result: Achieved real-time operation (previously difficult with camera-only methods) with superior tracking performance under occlusion conditions. Dataset contains 4,445 frames, 3,105 player IDs from professional 5-on-5 and 3-on-3 games.

Conclusion: LiDAR-based approach overcomes limitations of traditional camera systems, enabling accurate real-time 3D player tracking in challenging basketball scenarios with frequent occlusions and rapid movements.
...
TL;DR: RAG-SEG is a training-free paradigm that uses Retrieval-Augmented Generation for prompt generation and SAM-based segmentation for refinement, achieving competitive COD performance without conventional training on just a personal laptop.


<details>
  <summary>Details</summary>
Motivation: Camouflaged object detection is challenging due to high object-background similarity. Existing methods require heavy training and computational resources, while foundation models like SAM need fine-tuning and high-quality prompts that are costly to generate manually.

Method: Two-stage approach: 1) RAG stage constructs compact retrieval database via unsupervised clustering for fast feature retrieval to generate coarse masks as prompts, 2) SEG stage uses SAM2 for precise mask refinement based on retrieved pseudo-labels.
...
<details>
  <summary>Details</summary>
Motivation: Address privacy, copyright, and safety concerns arising from text-to-video diffusion models potentially generating harmful or misleading content using unauthorized personal identities, artistic creations, and harmful materials.

Method: A plug-and-play framework with two-stage process: Selective Prompt Embedding Adjustment (SPEA) and Adversarial-Resilient Noise Guidance (ARNG). Works without retraining and integrates with existing T2V diffusion models.

Result: Outperforms prior methods across four tasks (object, style, celebrity, and explicit content erasure), reducing undesirable content by 46% on average compared to baselines. Achieves state-of-the-art performance in efficacy, integrity, fidelity, robustness, and generalizability.

Conclusion: VideoEraser provides an effective training-free solution to prevent generation of harmful content in text-to-video models, addressing critical safety and privacy concerns while maintaining high performance across multiple content removal tasks.
...
<details>
  <summary>Details</summary>
Motivation: As autonomous vehicles become reality, they need to predict pedestrian intentions from a distance to ensure safe navigation, particularly whether pedestrians are about to cross roads.

Method: The study used deep learning models for pose detection combined with sequence modeling techniques (GRU, LSTM, 1D CNN) for temporal predictions, creating an end-to-end framework that integrates video analysis with pose detection outputs.

Result: GRU performed better than LSTM in predicting crossing intent, while 1D CNN was the fastest model. The framework successfully integrated pose detection with sequence modeling for intent prediction.

Conclusion: The proposed deep learning framework effectively predicts pedestrian road crossing intentions, with different sequence models offering trade-offs between prediction accuracy and computational speed, providing valuable insights for autonomous vehicle safety systems.
...
<details>
  <summary>Details</summary>
Motivation: Improve 3D object detection for autonomous driving by enhancing visual backbone features through fusion with semantically rich representations from pretrained foundation models, while maintaining compatibility with existing architectures.

Method: Proposes RCDINO, a multimodal transformer-based model that fuses camera and radar data with pretrained DINOv2 foundation model features to enrich visual representations for better detection performance.

Result: Achieves state-of-the-art performance among radar-camera models on nuScenes dataset with 56.4 NDS and 48.1 mAP scores.

Conclusion: The fusion of visual backbone features with DINOv2 foundation model representations significantly improves 3D object detection performance while preserving architectural compatibility, demonstrating the effectiveness of leveraging pretrained foundation models in multimodal fusion tasks.
...
<details>
  <summary>Details</summary>
Motivation: Most existing Video-LLM research focuses on performance improvements rather than understanding internal mechanisms, creating a gap in interpretability that this study aims to address.

Method: The authors use attention knockouts as the primary analytical tool with three variants: Video Temporal Knockout, Video Spatial Knockout, and Language-to-Video Knockout, applied across different layer windows in both global and fine-grained settings.

Result: Key findings include: (1) Video information extraction occurs primarily in early layers with a two-stage process, (2) certain intermediate layers have outsized impact while most contribute minimally, and (3) spatial-temporal modeling relies more on language-guided retrieval than expensive self-attention mechanisms.

Conclusion: This is the first systematic study uncovering Video-LLMs' internal video processing mechanisms, providing both interpretability insights and demonstrating practical efficiency improvements through reduced attention computation.
...
Method: BioTune uses an evolutionary algorithm to identify and selectively fine-tune only the most relevant layers in pre-trained models, rather than updating all parameters.
...
Conclusion: BioTune provides an effective approach for efficient transfer learning by focusing fine-tuning on relevant layers, reducing computational costs while maintaining performance across diverse datasets and domains.
...
Conclusion: The proposed method successfully addresses the limitations of current 3DGS compression by achieving significantly better compression rates and providing adaptability to post-archival scene changes through image-conditioned decoding.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D Gaussian Splatting methods for driving scenes overlook background optimization with proper geometry relationships and rely too heavily on view fitting, leading to poor novel view rendering and inaccurate geometric representations.

Method: Uses region-wise voxel initialization (near/middle/far regions) for linear motion patterns, deformable neural Gaussians with temporal deformation network for non-rigid dynamics, and depth/normal priors from pre-trained models for geometric supervision.

Result: Achieves state-of-the-art performance on Waymo and KITTI datasets for novel-view synthesis in driving scenarios.

Conclusion: DriveSplat provides a robust framework for high-quality driving scene reconstruction by combining dynamic-static decoupling with geometric priors and region-aware initialization.
...
<details>
  <summary>Details</summary>
Motivation: To enhance the abstract reasoning capabilities of machine intelligence by solving RPM problems, which serve as an authoritative benchmark for evaluating abstract reasoning, pattern recognition, and complex problem-solving in deep learning models.

Method: Adopts a causal chain modeling perspective to analyze RPM tasks (image → abstract attributes → progressive attribute patterns → pattern consistency → correct answer), designs baseline model DIO, and progressively proposes three improvement methods to address limitations in mutual information optimization.

Result: Experiments reveal that the baseline model DIO's optimization objective (maximizing variational lower bound of mutual information between context and correct option) fails to enable genuine acquisition of human reasoning logic due to tightness of lower bound and inability to capture causal relationships.

Conclusion: The paper identifies fundamental limitations in current mutual information-based approaches for abstract reasoning and proposes progressive improvements to better model the causal chain in RPM problems and capture human-like reasoning patterns.
...
Method: Proposes Spiking Variational Graph Network with: 1) SNN-based keyframe extractor for event-driven computation, 2) Dynamic Aggregation Graph Reasoner for fine-grained reasoning, and 3) Variational Inference Reconstruction Module with ELBO optimization to handle uncertainty and noise.

Result: SpiVG surpasses existing methods across multiple datasets including SumMe, TVSum, VideoXum, and QFVS.

Conclusion: The proposed SpiVG Network effectively addresses limitations of current video summarization methods by combining spiking neural networks with variational inference, achieving superior performance while enhancing information density and reducing computational complexity.
...
Method: Analytical derivation of features learned by linear MAEs, extension to non-linear MAEs, and investigation of how masking ratio and patch size affect spatial correlation learning.

Result: Masking ratio and patch size can selectively capture short- and long-range spatial correlations; MAE representations adapt to dataset spatial correlations beyond second-order statistics.

Conclusion: The analysis provides theoretical insights into MAE hyperparameter effects and offers practical guidance for selecting appropriate masking ratios and patch sizes based on desired spatial correlation learning.
...
<details>
  <summary>Details</summary>
Motivation: Traditional car damage inspection is manual, time-consuming, and unreliable for detecting tiny surface imperfections like microscopic dents, creating a need for faster and more accurate automated solutions.

Method: Used YOLOv8 object recognition framework with custom variants (YOLOv8m-t4 and YOLOv8m-t42), trained on a bespoke dataset of annotated car surface images under various conditions with real-time data augmentation for robustness.
...
Conclusion: The deep learning approach provides excellent detection accuracy with low inference latency, making it suitable for real-time applications like automated insurance assessments and vehicle inspections, with YOLOv8m-t42 being more accurate despite slower convergence.
...
Result: Achieves 13.1% improvement in R@1 and 8.1% in mIoU on ActivityNet-VRL, and 14.7% gain in R@1 and 14.4% gain in mIoU on new SportsMoments dataset compared to state-of-the-art methods.
...
Result: Achieves 2.75dB PSNR improvement over state-of-the-art methods under extremely sparse-view conditions (2 training views), producing images with minimal distortion and rich high-frequency details.

Conclusion: The method successfully addresses the limitations of 3DGS in sparse-view scenarios by eliminating SfM dependency and introducing novel modules for pose estimation, view interpolation, and geometric regularization, demonstrating superior performance in challenging real-world conditions.
...
<details>
  <summary>Details</summary>
Motivation: Address the need for lightweight and generalizable medical image segmentation models in resource-constrained clinical settings, overcoming limitations of existing lightweight models that compromise performance and lack global contextual perception capabilities.

Method: Proposes LGMSNet framework with heterogeneous intra-layer kernels to extract local high-frequency information while mitigating channel redundancy, and integrates sparse transformer-convolutional hybrid branches to capture low-frequency global information.

Result: Extensive experiments across six public datasets demonstrate LGMSNet's superiority over existing state-of-the-art methods, with exceptional performance in zero-shot generalization tests on four unseen datasets.

Conclusion: LGMSNet shows strong potential for real-world deployment in resource-limited medical scenarios, maintaining high performance with minimal computational overhead and excellent generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Real-world conditions like low-resolution imagery degrade face detection performance, requiring systematic investigation of how input resolution affects modern deep learning detectors.
...
Result: YOLOv11 outperformed others in detection accuracy especially at higher resolutions, YOLOv12 showed slightly better recall, MTCNN was competitive in landmark localization but slower for real-time inference.

Conclusion: Provides actionable insights for selecting resolution-aware face detection models suitable for varying operational constraints, with YOLOv11 recommended for high-resolution accuracy and YOLOv12 for recall-sensitive applications.
...
Method: Proposes Joint Multi-view Body Optimization (JMBO) to fit a single SMPL-X body model across all views with multi-view consistency, then uses the optimized body as prior for surface reconstruction with normal map integration from front and back views.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal image fusion methods face challenges with modality misalignment, high-frequency detail destruction, and task-specific limitations that hinder general applicability across different imaging scenarios.
...
Conclusion: AdaSFFuse provides a robust solution for task-generalized multimodal image fusion, effectively addressing alignment issues while preserving critical details and maintaining computational efficiency.
...
Conclusion: ExtraGS provides a holistic framework that effectively integrates geometric and generative priors for high-quality trajectory extrapolation in driving scene simulation.
...
Conclusion: GroupSketch expands practical applications of sketch animation by effectively handling multi-object interactions and complex motions through its novel two-stage approach with specialized refinement modules.
...
<details>
  <summary>Details</summary>
Motivation: Extracting narrow roads from remote sensing imagery is challenging due to limited width, fragmented topology, and frequent occlusions. Conventional models overfit to generic road widths and struggle with fine-grained, occluded segments.

Method: Proposes D3FNet with three innovations: 1) Differential Attention Dilation Extraction module for enhancing road features and suppressing noise, 2) Dual-stream Decoding Fusion Mechanism integrating original and attention-modulated features, 3) Multi-scale dilation strategy (rates 1,3,5,9) to reduce gridding artifacts.
...
Conclusion: D3FNet is a robust solution for fine-grained narrow road extraction in complex remote and cooperative perception scenarios, effectively addressing the challenges of narrow, fragmented, and occluded roads.
...
<details>
  <summary>Details</summary>
Motivation: Existing TTA methods rely on backpropagation/iterative optimization (limiting scalability) and lack explicit modeling of class-conditional feature distributions, which is crucial for reliable decision boundaries and calibrated predictions.
...
Conclusion: ADAPT provides an effective backpropagation-free TTA solution that handles both online and transductive settings while maintaining strong performance and scalability.
...
<details>
  <summary>Details</summary>
Motivation: Implicit Neural Representations struggle with spectral bias, favoring low-frequency components and missing high-frequency details like sharp edges and fine textures. Existing solutions focus on architectural modifications, but this paper explores training process guidance instead.

Method: Proposes a two-stage training strategy: 1) Uses a neighbor-aware soft mask that adaptively assigns higher weights to pixels with strong local variations to encourage early focus on fine details, 2) Transitions to full-image training after establishing high-frequency detail foundation.

Result: Experimental results show consistent improvement in reconstruction quality and demonstrate that the approach complements existing INR methods effectively.

Conclusion: This work presents a pioneering approach to assign frequency-aware importance to pixels in image INR, offering a new avenue for mitigating spectral bias problems through training strategy rather than architectural changes.
...
<details>
  <summary>Details</summary>
Motivation: Existing provably globally optimal approaches for point cloud registration with high outlier rates using SDP relaxations are computationally expensive, taking hundreds of seconds for 100 points.

Method: Proposed a linear time convex relaxation and contractor method to accelerate Branch and Bound (BnB) for solving the truncated least squares formulation of point cloud registration.

Result: The solver can register two 3D point clouds with 100 points to provable global optimality in less than half a second when the rotation axis is known, achieving 100x speedup over STRIDE SDP solver for rotation-only problems.

Conclusion: The method provides formal proof of global optimality and handles up to 95% outlier rates efficiently, though currently limited to problems with known rotation axis rather than full 6DoF registration.
...
TL;DR: Grounded VideoDiT is a Video LLM that improves temporal perception through diffusion-based encoding, object grounding, and explicit timestamp modeling, achieving state-of-the-art results on video reasoning benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Tree instance segmentation in airborne laser scanning data is challenging due to data variations and the high cost of obtaining precise labels for fully supervised methods.
...
Result: 34% improvement in correctly identified tree instances while significantly reducing non-tree instance predictions. Performance reduced in sparse forests with small trees (<2m) or complex surroundings with shrubs/boulders.

Conclusion: The weakly supervised approach effectively improves tree segmentation while reducing labeling costs, though challenges remain in complex terrain and sparse forest conditions.
...
TL;DR: CFG is a transfer-based black-box attack method for 3D point clouds that improves adversarial transferability by prioritizing corruption of critical features common across different DNN architectures, while maintaining imperceptibility through constrained deviation.
...
Method: Proposes Critical Feature Guidance (CFG) that regularizes adversarial search by computing feature importance and prioritizing corruption of critical features used by diverse architectures. Also constrains maximum deviation in loss function for imperceptibility.
...
Conclusion: CFG successfully enables effective transfer-based black-box attacks on 3D point clouds by leveraging consistent critical features across different DNN architectures while maintaining adversarial imperceptibility.
...
Conclusion: MapKD effectively bridges cross-modal knowledge transfer gap, enabling efficient vision-only HD map construction with performance comparable to multimodal approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current LoD1 and LoD2 building models lack detailed facade elements essential for advanced urban applications, while LoD3 models require manual modeling which is not scalable for large areas.

Method: Uses Conflict Maps from ray-to-model-prior analysis, semantically segments real-world CMs with synthetically generated CMs from Semantic Conflict Map Generator, and fuses with textured model segmentation using confidence scores.
...
Conclusion: The method advances automated LoD3 model reconstruction, enabling scalable and efficient 3D city modeling for urban planning and digital twins.
...
TL;DR: MDPR framework addresses class imbalance in VLM fine-tuning by creating multi-dimensional semantic knowledge base and using dynamic prompt routing for balanced predictions.
...
Method: Multi-dimensional Dynamic Prompt Routing (MDPR) constructs comprehensive knowledge base across five visual-semantic dimensions, uses dynamic routing to align global classes, retrieve optimal prompts, balance fine-grained semantics, and fuse logits for stable predictions.
...
TL;DR: StreamMem is a query-agnostic KV cache memory mechanism for efficient long video understanding in MLLMs, achieving state-of-the-art compression performance without requiring pre-knowledge of questions.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with long video processing due to substantial memory and computational overhead from KV cache storage. Existing methods require encoding entire visual contexts before compression or advance knowledge of questions, which is impractical for long videos and multi-turn conversations.

Method: StreamMem encodes new video frames in streaming manner, compressing KV cache using attention scores between visual tokens and generic query tokens while maintaining fixed-size KV memory for efficient question answering in memory-constrained scenarios.
...
Conclusion: StreamMem provides an effective solution for streaming video understanding with efficient memory usage, enabling practical long-video processing in multimodal LLMs without requiring question pre-knowledge.
...
TL;DR: WorldWeaver is a framework for long video generation that jointly models RGB frames and perceptual conditions (like depth) to improve temporal consistency and reduce structural drift in extended sequences.


<details>
  <summary>Details</summary>
Motivation: Current video generation methods rely heavily on RGB signals, leading to accumulated errors in object structure and motion over long durations. The challenge is maintaining structural and temporal consistency in extended video sequences.

Method: Jointly predicts perceptual conditions and RGB from unified representation, uses depth cues (more drift-resistant than RGB) for memory bank construction, and employs segmented noise scheduling for training prediction groups.

Result: Extensive experiments show WorldWeaver effectively reduces temporal drift and improves video fidelity in both diffusion-based and rectified flow-based models.

Conclusion: The framework significantly enhances temporal consistency and motion dynamics while reducing computational costs, making it effective for long-horizon video generation.
...
<details>
  <summary>Details</summary>
Motivation: Vision foundation models like SAM enable nuclei segmentation but struggle with fine-grained semantic segmentation of specific cell subtypes. Current methods require extensive pixel-level annotations and lack molecular context for precise cell identification.

Method: Three-component approach: (1) molecular-empowered learning with lay annotators to reduce detailed annotations, (2) SAM adapter adaptation for specific semantics, (3) Molecular-Oriented Corrective Learning (MOCL) for segmentation refinement.
...
<details>
  <summary>Details</summary>
Motivation: To create a unified framework that can handle text-to-video, image-to-video, and text-to-image generation simultaneously while addressing the need for high-quality video synthesis with complex motion and temporal consistency.
...
Result: Achieves top-3 rankings on both T2V and I2V leaderboards, outperforms existing open-source models, and matches or surpasses state-of-the-art commercial solutions in video quality and motion consistency.

Conclusion: Waver demonstrates superior performance in unified image and video generation, providing an effective framework that can help accelerate progress in video generation technologies through its open technical approach.
...
TL;DR: ATLAS is a high-fidelity parametric body model that decouples shape and skeleton representations, enabling more accurate 3D human modeling with enhanced expressivity and direct control over body attributes.


<details>
  <summary>Details</summary>
Motivation: Existing human mesh models struggle with capturing detailed variations across diverse poses and shapes due to limited training data and restrictive modeling assumptions. Current approaches also create problematic dependencies between internal skeleton and external soft tissue.

Method: ATLAS learns from 600k high-resolution scans using 240 synchronized cameras. It explicitly decouples shape and skeleton bases by grounding mesh representation in the human skeleton, using non-linear pose correctives instead of linear models.

Result: ATLAS outperforms existing methods by fitting unseen subjects in diverse poses more accurately. Quantitative evaluations show that non-linear pose correctives more effectively capture complex poses compared to linear models.

Conclusion: The decoupled approach enables enhanced shape expressivity, fine-grained customization of body attributes, and keypoint fitting independent of external soft-tissue characteristics, addressing limitations of previous parametric body models.
...
Method: Uses a feature aggregation module integrating local and global scene information from visual and geometric encoders, coupled with a position head to generate 3D assets and their spatial positions simultaneously.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models suffer from unintended spurious modifications due to global denoising that entangles edited regions with entire image context, compromising adherence to editing instructions.

Method: VAREdit uses autoregressive modeling for sequential image synthesis over discrete visual tokens, with a Scale-Aligned Reference module to inject scale-matched conditioning information from source image features into target feature prediction.

Result: Outperforms leading diffusion-based methods by 30%+ higher GPT-Balance score, completes 512x512 editing in 1.2 seconds (2.2x faster than similarly sized UltraEdit).

Conclusion: Visual autoregressive framework provides superior editing adherence and efficiency compared to diffusion-based methods, with precise text-guided editing capabilities.
...
Conclusion: Enables generative models to treat multiple outputs as cohesive groups rather than independent samples, providing better user experience and exploration capabilities.
...
TL;DR: CineScale is a novel inference paradigm that enables high-resolution image and video generation (up to 8k images and 4k videos) without fine-tuning or with minimal LoRA fine-tuning, addressing repetitive pattern issues in existing methods.
...
Method: Proposes CineScale, a specialized inference paradigm with dedicated variants for different video generation architectures (T2I, T2V, I2V, V2V), built on state-of-the-art open-source frameworks to handle high-frequency information issues.
...
Conclusion: CineScale successfully extends the capabilities of pre-trained models for high-resolution visual generation across multiple modalities, overcoming the limitations of previous methods and achieving state-of-the-art results.
...
TL;DR: A fully spectral neuro-symbolic reasoning architecture using Graph Signal Processing as the computational backbone, encoding logical entities as graph signals and processing them via learnable spectral filters for rule-based inference.
...
Method: Formulates the entire reasoning pipeline in the graph spectral domain using graph Fourier transforms, band-selective attention, and spectral rule grounding. Logical entities are encoded as graph signals and processed via learnable spectral filters that control multi-scale information propagation.

Result: Experiments on benchmark reasoning datasets (ProofWriter, EntailmentBank, bAbI, CLUTRR, and ARC-Challenge) show improvements in logical consistency, interpretability, and computational efficiency over state-of-the-art neuro-symbolic models.

Conclusion: Graph Signal Processing provides a mathematically grounded and computationally efficient substrate for building robust and interpretable reasoning systems, demonstrating the value of spectral approaches for neuro-symbolic integration.
...
TL;DR: A computational framework where descriptive and prescriptive aspects of world models co-emerge from agent-environment interactions through goal-directed state representations called telic states.


<details>
  <summary>Details</summary>
Motivation: To challenge the canonical view that state representation and reward function are distinct components, and instead propose they co-emerge interdependently from an agent's goal.
...
Result: Provides a parsimonious account of goal-directed learning that integrates descriptive and prescriptive aspects of world models.

Conclusion: This framework offers a unified account of purposeful behaviors across behavioral, phenomenological and neural dimensions, drawing on Buddhist epistemology.
...
TL;DR: Collab-REC is a multi-agent framework using three LLM agents and a moderator to improve tourism recommendation diversity by reducing popularity bias and surfacing overlooked destinations.


<details>
  <summary>Details</summary>
Motivation: To address popularity bias and over-tourism in tourism recommendations by enhancing diversity and surfacing lesser-visited locales that traditional systems often overlook.

Method: Uses three LLM-based agents (Personalization, Popularity, Sustainability) generating complementary city suggestions, with a non-LLM moderator conducting multi-round negotiation to merge and refine proposals while penalizing spurious/repeated responses.
...
TL;DR: A novel agent-based crowd simulation method that uses large language models (LLMs) to generate realistic social interactions and movement decisions based on dialogue, personality, and environmental factors.


<details>
  <summary>Details</summary>
Motivation: Existing crowd simulations lack realistic social and environmental interactions driven by language and dialogue, limiting agent-agent and agent-environment interactions to basic steering and goal extrapolation.

Method: Uses LLMs with two main components: dialogue system (querying agent-centric LLMs for inter-agent dialogue based on personalities, roles, and spatial relationships) and language-driven navigation (using conversations, personality, emotional state, vision, and physical state to control agent movement).

Result: Validated in complex scenarios showing automatic grouping/ungrouping of agents and information-passing within crowds. Produces realistic crowd simulations with emergent group behaviors from environmental settings.

Conclusion: The framework enables more realistic crowd animations by incorporating language-driven social interactions, resulting in natural emergent group behaviors that reflect complex social dynamics.
...
Result: Reasoning LLMs achieved 48.7% accuracy in expert confidence assessment, but increasing reasoning budgets consistently impaired calibration, causing systematic overconfidence that worsened with longer thinking. Search-augmented generation achieved 89.3% accuracy by retrieving relevant evidence.
...
<details>
  <summary>Details</summary>
Motivation: To enable new Earth science measurements and responses by performing data analysis at the edge (onboard satellites) rather than relying solely on ground-based processing.

Method: Utilizing the CogniSAT-6 satellite equipped with visible and near infrared hyperspectral instrument and neural network acceleration hardware to perform onboard data analysis and inference using deep learning and spectral analysis algorithms.

Result: The paper demonstrates the capability to perform advanced data analysis and inference directly onboard the satellite for numerous applications.

Conclusion: Onboard data analysis using specialized hardware enables new scientific capabilities and responsive Earth observation, representing a significant advancement in satellite-based data processing.
...
TL;DR: S3LoRA is a lightweight, data-free framework that improves safety in LoRA-adapted LLMs by analyzing weight updates, detecting unsafe layers via spectral sharpness metrics, and pruning them without compromising performance.


<details>
  <summary>Details</summary>
Motivation: PEFT techniques like LoRA can compromise safety alignment in LLMs, leading to unsafe behaviors in agent planning tasks. Existing safety methods require access to multiple model checkpoints which are often unavailable.

Method: Proposes S3LoRA with MAS-SVD for structural analysis of LoRA updates and Spectral Sharpness Index (SSI) to detect unsafe layers. Prunes high-risk layers post-hoc while preserving task performance.

Result: Extensive experiments show S3LoRA consistently improves safety metrics while maintaining or improving utility metrics, with significant reduction in inference costs across agent planning and language generation tasks.

Conclusion: S3LoRA provides a practical, scalable solution for safe deployment of LLM-based agents in resource-constrained and safety-critical real-world environments.
...
TL;DR: Using abstract argumentation for workforce management enables better change accommodation and explanations, outperforming manual solutions.


<details>
  <summary>Details</summary>
Motivation: Workforce management needs to handle runtime changes and provide explanations to stakeholders, which current approaches struggle with.

Method: Model workforce management as abstract argumentation framework to accommodate changes and generate faithful explanations.
...
Conclusion: Abstract argumentation provides an effective framework for industrial workforce management that handles dynamic changes and improves decision-making through better explanations.
...
TL;DR: GOOD is an online method that uses LLMs to infer and track natural language goals from dialogue in open-ended assistance scenarios, outperforming baselines without explicit goal tracking.


<details>
  <summary>Details</summary>
Motivation: Embodied AI agents need to handle diverse, undefined human goals and preferences in open-ended environments, requiring a framework that can reason over unbounded goal spaces.

Method: Introduces Open-Universe Assistance Games (OU-AGs) framework and GOOD method, which uses LLMs to simulate users with complex intents and perform probabilistic inference over natural language goal candidates during interactions.

Result: Evaluated in grocery shopping and household robotics domains, GOOD outperformed baseline methods without explicit goal tracking in both LLM-based and human evaluations.

Conclusion: GOOD provides an effective, data-efficient approach for goal inference in open-ended assistance scenarios, enabling rich goal representations and uncertainty estimation without large offline datasets.
...
TL;DR: SurgWound dataset and WoundQwen framework for surgical wound diagnosis using multi-stage MLLMs with VQA and report generation tasks.


<details>
  <summary>Details</summary>
Motivation: Address lack of public surgical wound datasets and benchmarks, overcome data privacy concerns and high annotation costs in surgical wound screening.

Method: Three-stage learning framework: 1) Five MLLMs predict wound characteristics, 2) Two MLLMs diagnose infection risk, 3) One MLLM integrates results for comprehensive report generation.

Result: Created SurgWound dataset with 697 images and 8 clinical attributes, established first surgical wound diagnosis benchmark with VQA and report generation tasks.

Conclusion: The framework enables detailed surgical wound analysis and personalized care instructions, paving way for improved patient outcomes through timely intervention.
...
<details>
  <summary>Details</summary>
Motivation: Traditional publication venues struggle to handle AI-generated research due to scalability issues and reluctance to accept such content, while preprint servers lack quality control, creating a dissemination bottleneck for high-quality AI research.

Method: Developed a multi-agent architecture platform with API and MCP interfaces that allows seamless integration of human and AI scientists for collaborative proposal submission, peer review, and iterative refinement of research content.

Result: Extensive experiments show aiXiv significantly enhances the quality of AI-generated research proposals and papers through iterative revising and reviewing processes on the platform.

Conclusion: aiXiv establishes a scalable and extensible ecosystem for autonomous scientific discovery, accelerating the publication and dissemination of high-quality AI-generated research content.
...
TL;DR: GUI-Owl is a state-of-the-art GUI agent model achieving top performance on 10 GUI benchmarks, with Mobile-Agent-v3 framework further improving results. Key innovations include cloud-based environment infrastructure, diverse agent capabilities, and scalable RL framework.


<details>
  <summary>Details</summary>
Motivation: To develop a comprehensive GUI agent model that can handle various GUI tasks across different platforms (desktop and mobile) with end-to-end capabilities including grounding, QA, planning, and decision-making.
...
Conclusion: GUI-Owl represents a significant advancement in GUI agent technology with comprehensive capabilities across multiple platforms. The self-evolving data generation framework and scalable RL approach enable continuous improvement and real-world alignment, making it a foundational model for GUI interaction tasks.
...
TL;DR: PuzzleClone is a formal framework using SMT solvers to generate scalable, verifiable mathematical and logical puzzles for improving LLM reasoning capabilities, achieving significant performance gains on benchmarks.
...
Method: Three-step approach: (1) encoding seed puzzles into structured logical specifications using SMT, (2) generating scalable variants through systematic variable and constraint randomization, and (3) ensuring validity via reproduction mechanism.

Result: Created benchmark with 83K+ validated puzzles. Post-training on PuzzleClone data improved average performance from 14.4 to 56.2 on PuzzleClone testset, and up to 12.5 percentage points improvement on 7 logic/math benchmarks (e.g., AMC2023 from 52.5 to 65.0).
...
Method: Three-stage pipeline: 1) Data augmentation using frontier LLM to generate medically plausible synthetic vignettes, 2) Fine-tuning open-source foundation model on the created dataset, 3) Inference with expert evaluation by clinical and psychological specialists to validate responses and iteratively enrich the dataset.
...
Conclusion: The framework offers a generalizable approach for other rare diseases with similar data scarcity and trustworthiness challenges, demonstrating successful application of LLMs for specialized medical conditions through synthetic data generation and expert validation.
...
TL;DR: R-ConstraintBench evaluates LLMs on resource-constrained scheduling problems, showing they perform well on simple precedence constraints but collapse when complex constraints interact, revealing limited generalization.
...
Method: A scalable framework that evaluates models on Resource-Constrained Project Scheduling Problems (RCPSP) by incrementally increasing non-redundant precedence constraints in DAGs and introducing downtime, temporal windows, and disjunctive constraints.

Result: Strong models perform near-ceiling on precedence-only DAGs but feasibility performance collapses when complex constraints interact. Performance on synthetic problems doesn't guarantee transfer to domain-grounded scenarios.

Conclusion: Constraint interaction, not graph depth, is the principal bottleneck for LLMs in complex scheduling problems, and current models show limited generalization capabilities across different constraint types and real-world scenarios.
...
TL;DR: A training-free agentic system that converts rough hand sketches into precise, editable SVG diagrams using VLM-LLM collaboration with iterative refinement.


<details>
  <summary>Details</summary>
Motivation: Diffusion models struggle with spatial precision and symbolic structure needed for flowcharts, requiring a system that can produce precise, compositional diagrams from sketches.

Method: Couples Vision-Language Model with LLMs in iterative loop: Critic VLM proposes edits, multiple LLMs synthesize SVG updates with diverse strategies, Judge VLM selects best candidate.

Result: Outperforms GPT-5 and Gemini-2.5-Pro on 10 sketch samples, faithfully reconstructing layout and structure while accurately composing primitives without unwanted text.

Conclusion: The approach produces editable SVG programs, supports human corrections, and is extensible to presentation tools via APIs, with open-sourced implementation.
...
TL;DR: Novel computational intelligence algorithms for urban land-use optimization that balance compatibility and economic objectives, achieving 3.16-3.3% improvements over state-of-the-art methods.
...
Method: Developed multiple optimization algorithms including CR+DES (differential evolution with scaled difference vectors) and MSBX+MO variants, with systematic constraint relaxation strategy and statistical validation using Kruskal-Wallis tests.
...
TL;DR: Proposes a multiple memory system (MMS) inspired by cognitive psychology to improve agent memory quality by processing short-term memories into retrieval and contextual memory units, enhancing recall and response quality.


<details>
  <summary>Details</summary>
Motivation: Existing memory modules like MemoryBank and A-MEM have poor memory content quality, which negatively impacts recall performance and response quality in language model agents.

Method: Designed a multiple memory system that processes short-term memory into multiple long-term fragments, creates retrieval memory units and corresponding contextual memory units, and matches relevant units during retrieval based on user queries.
...
Conclusion: MMS effectively utilizes historical data by constructing high-quality long-term memory content, proving practical value for language model agents handling large interaction histories.
...
TL;DR: Coarse-to-Fine Grounded Memory framework that uses multi-granularity memory to enhance LLM-based agents for complex planning tasks, addressing limitations of single-granularity memory approaches.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based agent approaches use single-granularity memory from environmental interactions, which constrains knowledge diversity and planning flexibility due to experience quality limitations.
...
Result: The framework enables flexible adaptation to diverse scenarios by leveraging multi-granularity memory, improving planning capabilities when facing environmental anomalies.

Conclusion: The coarse-to-fine memory grounding approach overcomes limitations of single-granularity memory systems, providing more diverse knowledge and flexible planning for LLM-based agents in complex tasks.
...
TL;DR: SPW unifies expert demonstrations and preferences for offline RL by using similarity search to assign stepwise importance weights, enabling better credit assignment and outperforming previous methods on robot manipulation tasks.
...
Conclusion: SPW successfully unifies demonstrations and preferences by solving the credit assignment problem through similarity-based weighting, providing a more effective approach for offline RL with human feedback.
...
Conclusion: Demonstrates effective integration of symbolic ECG representations into LLMs, highlighting potential for enhanced medical reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: In many planning applications, there is a need to find plans that achieve goals while making minimal changes to the initial state, preserving the original state as much as possible.

Method: The paper formally defines plan disruption and develops various planning-based compilations that jointly optimize both the sum of action costs and the degree of plan disruption.

Result: Experimental results across different benchmarks demonstrate that the reformulated planning tasks can be effectively solved in practice to generate plans that successfully balance both objectives.

Conclusion: The proposed approach enables practical generation of plans that minimize disruption to initial states while achieving goals, providing a valuable tool for planning applications where state preservation is important.
...
Result: Produces structured datasets supporting both SFT and DPO use cases with seamless integration into diverse training workflows.

Conclusion: Provides a robust, scalable solution for synthetic conversational data generation that significantly reduces data preparation costs in LLM training pipelines.
...
Method: Multi-agent framework with extended Continuous Time Markov Decision Process for dynamic agent modeling, generalized entropy measure for collaborative efficiency optimization, multi-level Stackelberg game for hierarchical decisions, and contextual Thompson sampling for prompt optimization with quality assurance.

Result: Extensive evaluations show BusiAgent significantly outperforms established approaches in generating coherent, client-focused solutions that integrate granular insights with high-level strategy, achieving better solution quality and user satisfaction.
...
TL;DR: Think in Blocks framework enables LLMs to dynamically adjust reasoning depth by partitioning reasoning into tunable blocks, optimizing computational efficiency based on task complexity.


<details>
  <summary>Details</summary>
Motivation: Address overthinking in LLM chains-of-thought that causes computational waste and slower responses by enabling adaptive reasoning depth based on task difficulty.
...
Result: Framework allows LLMs to predict optimal number of reasoning blocks and dynamically control reasoning depth during inference for efficient problem-solving.

Conclusion: Think in Blocks provides an adaptive reasoning approach that optimizes computational resources while maintaining strong performance across varying task complexities.
...
TL;DR: LLM agents in teams playing Prisoner's Dilemma show that inter-group competition combined with repeated interactions significantly increases cooperation levels, including one-shot cooperation.


<details>
  <summary>Details</summary>
Motivation: To study cooperative behavior in autonomous AI agents inspired by super-additive cooperation theory, examining how repeated interactions and inter-group rivalry affect cooperation similar to human tendencies.

Method: Virtual tournament where language model agents grouped into teams play Prisoner's Dilemma games, simulating both internal team dynamics and external competition between teams.
...
Conclusion: Provides a framework for LLMs to strategize in complex social scenarios, showing counter-intuitively that intergroup competition can foster more cooperative behavior, with implications for designing future multi-agent AI systems aligned with human values.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D reasoning approaches generate short reasoning chains due to simple questions in datasets, limiting LLMs' complex reasoning capabilities in 3D scenes.

Method: Proposes combinatorial and iterative evolutionary approach on SQA3D benchmark to generate complex questions, then fine-tunes LLMs using Direct Preference Optimization (DPO) to optimize toolchain strategies.

Result: Enhanced LLM proficiency in using 3D tools and improved accuracy in complex 3D situated reasoning tasks.

Conclusion: DeepThink3D successfully addresses the challenge of simple questions in 3D reasoning by generating complex scenarios and optimizing tool usage strategies, significantly improving LLM performance in complex 3D reasoning tasks.
...
TL;DR: A framework using dynamical systems theory (FTLE and LCS) to formally verify RL policy safety and robustness, with quantitative metrics to identify hidden failure modes.
...
Method: Analyze RL agent-environment as dynamical system using Finite-Time Lyapunov Exponent (FTLE) to identify Lagrangian Coherent Structures (LCS), then develop quantitative metrics (MBR, ASAS, TASAS) to measure safety margins and robustness.

Result: Framework successfully identifies safety barriers around unsafe regions, reveals convergence properties and trap states, and detects critical flaws in policies that appear successful based solely on reward metrics.

Conclusion: The approach provides comprehensive, interpretable assessment of policy behavior and enables formal safety verification for RL systems, addressing a critical gap in deploying RL to safety-critical applications.
...
<details>
  <summary>Details</summary>
Motivation: To provide a framework that abstracts agents from logical flow and allows developers to focus on modeling data rather than crafting prompts, enabling declarative data composition through LLM-powered logical transduction.

Method: A modular framework where agents are used internally within data types to enable logical transduction between different data types. The framework uses LLMs to execute logical transduction when types are connected, allowing for declarative data composition.
...
Conclusion: Agentics provides an effective framework for building agent-based systems with structured reasoning capabilities, demonstrating strong performance across various complex data tasks while enabling a more declarative approach to AI workflow development.
...
Conclusion: The adaptation successfully bridges HDM with ACT-R, enabling vector-symbolic advantages while maintaining backward compatibility. Future work includes improving time-context representations and developing IBL-based decision-making models to further test the system.
...
TL;DR: ICVs use information-theoretic Shapley values to quantify individual agent contributions in MARL by analyzing policy distributions, measuring causal influence on teammates' instrumental empowerment without requiring value feedback.


<details>
  <summary>Details</summary>
Motivation: To understand individual agent behaviors in multi-agent systems when explicit reward signals or value functions are unavailable, enabling reliable deployment of MARL systems by extracting meaningful insights from policy distributions alone.
...
Conclusion: The proposed ICV method provides novel insights into cooperation dynamics and enhances explainability in MARL systems by quantifying agent contributions solely through policy analysis, consistent with underlying value functions.
...
TL;DR: The paper proposes a techno-philosophical analysis of the EU AI Act, arguing that current regulations miss the dynamic lifecycle of AI systems where data generates recursive value chains, creating power asymmetries and regulatory blind spots.


<details>
  <summary>Details</summary>
Motivation: To address the gap in policymaking regarding the dynamic 'becoming' process in AI systems - how data ingestion, training, and deployment create recursive value chains that challenge Responsible AI frameworks and concentrate power in tech oligarchies.

Method: Uses cross-disciplinary methods combining technical analysis with Simondonian philosophy of technology. Develops a conceptual tool to frame the AI pipeline and introduces the concept of 'futurity' to model the self-reinforcing AI lifecycle through pre-individual milieu, individuation, and individuated AI stages.

Result: Identifies regulatory blind spots in addressing the recursive, non-rivalrous nature of data and the infrastructural dynamics that enable feedback, adaptation, and temporal recursion in AI systems. Reveals escalating power asymmetries in tech infrastructures.

Conclusion: Effective AI regulation must address infrastructural and temporal dynamics through measures like lifecycle audits, temporal traceability, feedback accountability, recursion transparency, and a right to contest recursive reuse to counter power concentration in tech oligarchies.
...
<details>
  <summary>Details</summary>
Motivation: To create a comprehensive benchmark for evaluating multimodal models on instruction-following, visual reasoning, and visual-textual alignment tasks with controlled data semantics and structure.

Method: Uses Python visualization libraries to generate charts and tables programmatically, pairs them with systematically generated multi-step analytical questions, and provides answers in structured formats like JSON/YAML with strict formatting guidelines.

Result: A unified, scalable framework with a taxonomy of reasoning types (comparison, trend identification, ranking, aggregation, proportion estimation, anomaly detection) for fine-grained benchmarking.

Conclusion: GRAFT sets a new evaluation standard for multimodal models on visually grounded, structured reasoning tasks through its comprehensive and controlled benchmarking approach.
...
TL;DR: NiceWebRL is a Python library that converts Jax-based RL environments into online interfaces for human experiments, enabling comparisons between AI algorithms and human performance across single/multi-agent scenarios.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between machine learning algorithms and human cognition research by providing a tool that allows researchers to test AI models against human participants in various environments.

Method: A Python library that transforms any Jax-based reinforcement learning environment into an online web interface, supporting both single-agent and multi-agent setups for human subject experiments.

Result: Successfully demonstrated through three case studies: developing human-like AI models tested against humans in grid world/Craftax, creating human-compatible multi-agent algorithms for Overcooked domain, and studying LLM assistance for humans in XLand-Minigrid.

Conclusion: NiceWebRL provides a versatile research tool that enables diverse applications in human-AI interaction research, from cognitive modeling to collaborative AI development and human assistance studies.
...
TL;DR: First comprehensive measurement of AI serving environmental impact in production, showing Gemini Apps text prompts consume 0.24 Wh energy and 0.26 mL water per median prompt - much lower than public estimates.


<details>
  <summary>Details</summary>
Motivation: As AI adoption accelerates, understanding and mitigating environmental impact of AI serving becomes critical, but no studies have measured these metrics in production environments.
...
TL;DR: A framework using state-of-the-art language models to detect parasocial relationship cues in AI conversations in real-time, showing promising results in identifying harmful interactions early.
...
Method: Repurposed a state-of-the-art language model to create a response evaluation framework that assesses ongoing conversations for parasocial cues in real time. Tested with a synthetic dataset of 30 dialogues spanning parasocial, sycophantic, and neutral conversations using iterative evaluation with five-stage testing.
...
Conclusion: Evaluation agents can provide a viable solution for preventing parasocial relationships with AI, offering preliminary evidence that real-time detection of harmful conversational patterns is feasible.
...
Method: Language-Guided Tuning (LGT) framework with three specialized LLM agents: Advisor (proposes changes), Evaluator (assesses progress), and Optimizer (refines decisions). Uses textual gradients for semantic understanding of training dynamics.
...
Conclusion: LGT provides an effective framework for intelligent configuration optimization through natural language reasoning and multi-agent coordination, addressing limitations of existing approaches.
...
TL;DR: A unified co-training framework that integrates multiple safety behaviors (positive, negative, rejective) within single SFT stage using magic tokens for dynamic behavioral switching at inference time.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety methods like SFT and RLHF have multi-stage training pipelines and lack fine-grained post-deployment controllability, requiring more efficient and flexible solutions.

Method: Co-training framework that simultaneously trains multiple safety behaviors via system-level instructions/magic tokens, creating distinct Safety Alignment Margin in output space with well-separated response distributions.
...
Conclusion: Presents a scalable, efficient, and highly controllable solution for LLM content safety with unprecedented fine-grained control and safety robustness.
...
TL;DR: Preliminary automatic ranking for WMT25 MT shared task, with caveats about bias towards re-ranking systems, awaiting final human evaluation.


<details>
  <summary>Details</summary>
Motivation: To provide task participants with early results for system submission preparation, while acknowledging limitations of automatic metrics.
...
Method: A framework for LLM-driven cultural localization that automatically constructs math word problem datasets with native entities from existing sources, replacing English-centric elements with culturally appropriate ones.

Result: Translated benchmarks obscure true multilingual math ability in socio-cultural contexts. The framework helps mitigate English-centric entity bias and improves robustness when native entities are introduced across various languages.

Conclusion: The proposed LLM-driven cultural localization framework effectively addresses the scarcity of truly localized math datasets and reduces English-centric bias, enhancing multilingual mathematical reasoning capabilities in appropriate cultural contexts.
...
TL;DR: Fine-tuning GaMS-9B-Instruct with DPO using programmatically curated translation pairs from Wikipedia articles improves machine translation quality for Slovene, outperforming baseline models.


<details>
  <summary>Details</summary>
Motivation: To enhance general instruction-tuned LLMs for machine translation using minimal, easily produced data resources, specifically focusing on improving Slovene translation quality.

Method: Used Direct Preference Optimization (DPO) training on programmatically curated translation pairs from English Wikipedia articles translated by GaMS-9B-Instruct and EuroLLM-9B-Instruct, ranked using heuristics and COMET metrics.
...
Conclusion: DPO training with carefully curated preference data can significantly improve machine translation performance in LLMs even with relatively few data resources.
...
TL;DR: This paper addresses limitations in multilingual conversational XAI systems by introducing MultiCoXQL dataset for 5 languages and Compass dataset for custom input extraction, with evaluations showing improved parsing performance.


<details>
  <summary>Details</summary>
Motivation: Current ConvXAI systems based on intent recognition work well for English but struggle with multilingual generalization due to training data scarcity and limited support for free-form custom inputs.

Method: Introduces MultiCoXQL (multilingual extension of CoXQL dataset) and Compass dataset for custom input extraction. Proposes new parsing approach and evaluates three LLMs using various parsing strategies on both datasets across 5 languages.

Result: The paper presents comprehensive evaluations including monolingual, cross-lingual, and multilingual assessments using LLMs of varying sizes and BERT-type models, demonstrating improved multilingual parsing performance.

Conclusion: The proposed datasets and parsing approach successfully bridge the gaps in multilingual ConvXAI systems, enabling better handling of diverse languages and free-form custom inputs through enhanced intent recognition capabilities.
...
TL;DR: Reward-Shifted Speculative Sampling (SSS) algorithm uses an aligned draft model with an unaligned target model to achieve test-time alignment with reduced inference costs.
...
Result: The algorithm achieves superior gold reward scores at significantly reduced inference cost in test-time weak-to-strong alignment experiments.

Conclusion: SSS provides an effective and efficient solution for test-time alignment by leveraging speculative sampling principles while maintaining alignment quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing recall metrics rely on lexical overlap and are error-prone with paraphrased answers, while LLM-as-a-Judge methods suffer from misalignment and hallucinations without structured verification, creating a need for more reliable recall assessment.
...
Conclusion: LongRecall serves as a foundational building block for systematic recall assessment, effectively handling diverse phrasings and contextual variations while reducing errors in recall evaluation.
...
TL;DR: Combining LLMs with combinatorial inference improves structured prediction by enhancing consistency and accuracy, with structured learning remaining valuable even with LLMs.
...
Method: Propose combining LLMs with combinatorial inference, experimenting with prompting strategies to estimate LLM confidence values for symbolic inference, and using structured prediction objectives for calibration and fine-tuning.

Result: Symbolic inference on top of prompting leads to more consistent and accurate predictions regardless of prompting strategy. Structured learning through calibration and fine-tuning improves performance on challenging tasks.

Conclusion: Structured learning remains valuable in the LLM era, and combining LLMs with symbolic inference addresses consistency issues while maintaining predictive power.
...
<details>
  <summary>Details</summary>
Motivation: Existing math datasets from Common Crawl suffer from poor quality due to brittle extraction methods, lossy HTML conversion, and failure to preserve mathematical structure, limiting their effectiveness for enhancing LLM reasoning capabilities.

Method: A novel domain-agnostic pipeline using layout-aware rendering with lynx and LLM-based cleaning to recover math across formats (MathJax, KaTeX, MathML), preserve equation/code structure, remove boilerplate, standardize to LaTeX, and correct inconsistencies.
...
Conclusion: The pipeline successfully extracts high-quality scientific content from noisy web data, achieving measurable gains in math, code, and general reasoning, setting new state-of-the-art for open math pretraining corpora. Code and datasets are released to support open-source efforts.
...
Method: Reduce the problem to fact verification, leverage external evidence to mitigate hallucinations, and generate/validate atomic assumptions for interpretable answers.

Result: Experiments with five LLMs show that incorporating retrieved evidence is beneficial, and generating/validating atomic assumptions yields further improvements.

Conclusion: Using external evidence and atomic assumption validation effectively reduces hallucinations and provides interpretable answers for questions with false assumptions.
...
TL;DR: MMDR-Bench is a new benchmark for complex multi-modal dialogue reasoning, and CoLVLM Agent is a framework that enhances LVLMs with advanced reasoning capabilities through iterative memory-perception-planning-execution cycles, achieving superior performance over state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: Current LLMs and LVLMs struggle with complex multi-turn visually-grounded tasks requiring deep reasoning, sustained contextual understanding, and entity tracking. Existing benchmarks fail to capture real-world multi-modal interaction complexities.

Method: Introduced MMDR-Bench dataset with 300 complex multi-turn dialogue scenarios. Proposed CoLVLM Agent framework using iterative "memory-perception-planning-execution" cycle to enhance existing LVLMs without extensive re-training.

Result: CoLVLM Agent achieved average human evaluation score of 4.03, outperforming GPT-4o (3.92) and Gemini 1.5 Pro (3.85). Showed significant advantages in reasoning depth, instruction adherence, error suppression, and maintained robust performance over extended dialogue turns.

Conclusion: The modular design and iterative approach of CoLVLM Agent effectively addresses complex multi-modal interaction challenges, validating the framework's effectiveness for advanced reasoning tasks.
...
TL;DR: SemToken is a semantic-aware tokenization framework that reduces token redundancy and improves computation efficiency by using contextual semantic embeddings and adaptive token granularity based on semantic density.
...
Conclusion: Semantic structure offers a promising new axis for optimizing tokenization and computation in large language models, with SemToken providing significant efficiency gains while maintaining performance.
...
TL;DR: Fin-PRM is a domain-specialized Process Reward Model for financial reasoning that outperforms general PRMs and achieves significant improvements in supervised learning (12.9%), reinforcement learning (5.2%), and test-time performance (5.1%).
...
Method: Fin-PRM integrates step-level and trajectory-level reward supervision for fine-grained evaluation of financial reasoning. Applied in offline/online settings for trajectory selection, reinforcement learning rewards, and test-time inference guidance.
...
Conclusion: Domain-specialized reward modeling is valuable for aligning LLMs with expert-level financial reasoning, demonstrating substantial performance gains across multiple learning paradigms.
...
TL;DR: SPARK is a training-free KV cache pruning method that applies unstructured sparsity at the channel level to reduce memory usage while maintaining model accuracy in long-context LLM inference.


<details>
  <summary>Details</summary>
Motivation: Existing KV cache compression methods focus on temporal axis compression but neglect fine-grained importance variations across feature dimensions, limiting their ability to balance efficiency and accuracy effectively.

Method: Proposes SPARK, a plug-and-play method that prunes KV cache at the channel level using unstructured sparsity and dynamically restores pruned entries during attention computation. It's orthogonal to existing compression techniques.

Result: Reduces KV cache storage by over 30% compared to eviction-based methods while preserving or improving accuracy. Even with 80% pruning ratio, maintains performance with less than 5% degradation compared to baseline.

Conclusion: SPARK effectively addresses the KV cache bottleneck by leveraging channel-level redundancy, enabling longer sequence processing within the same memory budget while maintaining model performance.
...
Conclusion: The proposed progressive knowledge acquisition approach effectively addresses domain-specific QA challenges by mirroring human learning patterns and provides a cost-effective alternative to expensive pretraining methods.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with multi-step tool usage including tool selection, parameter generation, and planning. Existing methods require manual demonstration design or curated libraries, which demand expert effort and become inefficient as tools and tasks scale.
...
Conclusion: SEER effectively addresses LLM tool usage challenges by leveraging dynamic experience retrieval, demonstrating substantial performance improvements across multiple benchmarks and model sizes.
...
TL;DR: Selective checklist use improves LLM evaluation performance in pairwise comparisons but shows inconsistent benefits in direct scoring. Checklist items often reflect human criteria despite low correlation, revealing human evaluation inconsistencies.
...
Conclusion: Findings highlight the need for more clearly defined objective evaluation criteria to guide both human and automatic evaluations, as human evaluation itself shows inconsistencies.
...
TL;DR: VocabTailor reduces SLM memory usage by 99% through dynamic vocabulary selection and embedding offloading, outperforming static pruning with minimal performance loss.
...
Method: Introduces VocabTailor - a decoupled dynamic vocabulary framework based on lexical locality principle and computational asymmetry. Offloads embeddings and uses hybrid static-dynamic selection for LM heads with on-demand loading.

Result: Achieves up to 99% reduction in vocabulary-related memory usage across diverse downstream tasks with minimal or no performance degradation, significantly outperforming static vocabulary pruning methods.

Conclusion: Dynamic vocabulary selection through VocabTailor effectively addresses SLM memory constraints while maintaining performance, offering a superior alternative to rigid static pruning approaches.
...
Method: Created a human-authored Thai dataset through multi-stage quality control with annotators, domain experts, and AI researchers. Conducted zero-shot evaluation and instruction tuning studies with ablations to isolate native supervision effects.
...
TL;DR: UniCoM pipeline generates high-quality code-switching speech samples using SWORDS algorithm that substitutes words with translations while preserving semantics and POS tags, creating the CS-FLEURS corpus for ASR and speech translation.
...
Method: Proposed UniCoM pipeline with SWORDS algorithm that substitutes selected words with their translations while considering parts of speech, without altering sentence semantics.

Result: Created CS-FLEURS corpus that achieves high intelligibility and naturalness, performing comparably to existing datasets on both objective and subjective metrics.

Conclusion: The approach advances CS speech technology and enables more inclusive multilingual systems by providing high-quality code-switching data.
...
TL;DR: EMNLP framework evaluates teacher-role LLMs' personality, moral development, and ethical vulnerability to prompt injection, revealing idealized personalities, strong abstract reasoning but emotional struggles, and a safety-capability paradox.
...
Method: Developed EMNLP framework with extended scales and 88 teacher-specific moral dilemmas, plus targeted soft prompt injection tests on 12 LLMs to assess compliance and vulnerability.

Result: Teacher-role LLMs show more idealized/polarized personalities than humans, excel in abstract moral reasoning but struggle with emotional complexity, and stronger reasoning models are more vulnerable to harmful injections.

Conclusion: This presents the first benchmark for ethical and psychological alignment of teacher-role LLMs, highlighting the safety-capability paradox in educational AI applications.
...
TL;DR: CARE is a new RAG framework that addresses context-memory conflicts by using a context assessor with compact memory token embeddings and grounded/adversarial soft prompting to identify unreliable context and guide reasoning toward reliable knowledge sources.
...
Method: CARE consists of a context assessor that encodes compact memory token embeddings from raw context. It uses grounded/adversarial soft prompting to train the assessor to detect unreliable context and provide guidance signals for reasoning toward more reliable knowledge sources.

Result: Extensive experiments show CARE effectively mitigates context-memory conflicts, achieving an average 5.0% performance gain on QA and fact-checking benchmarks compared to standard RAG approaches.

Conclusion: CARE establishes a promising direction for developing more trustworthy and adaptive RAG systems by effectively resolving knowledge conflicts between external context and parametric memory.
...
<details>
  <summary>Details</summary>
Motivation: Current LLMs struggle with temporal commonsense reasoning because temporal context is rarely explicitly stated in text. Automatically mining temporal commonsense can help create more robust language models.

Method: Proposed a temporal commonsense extraction pipeline using LLMs to automatically mine temporal commonsense from SAMSum and RealNews corpora, constructing the TComQA dataset.
...
Conclusion: The proposed pipeline successfully extracts high-quality temporal commonsense knowledge, and the resulting TComQA dataset significantly improves model performance on temporal reasoning tasks.
...
Result: Achieves competitive cross-lingual performance despite fewer parameters than current approaches. Demonstrates strong cross-lingual generalization in zero-shot tests on the UCLA Phonetic Corpus.

Conclusion: Effective universal speech processing is possible through modeling basic acoustic patterns within phoneme-length windows, enabling pure phoneme representations without contextual bias.
...
<details>
  <summary>Details</summary>
Motivation: Existing KGC evaluation faces challenges with inconsistent model rankings across different datasets and metrics, making holistic comparisons difficult and hindering reliable model selection.
...
Conclusion: EDAS offers a global perspective for KGC model evaluation, enabling more informed model selection, promoting fairness in cross-dataset evaluation, and providing a generalizable framework.
...
TL;DR: A comprehensive review of 283 LLM benchmarks categorized into general capabilities, domain-specific, and target-specific types, identifying current issues like data contamination and cultural biases, and providing design guidelines for future benchmark development.


<details>
  <summary>Details</summary>
Motivation: With the rapid advancement of large language models, there's a growing need to systematically review and categorize the numerous evaluation benchmarks that have emerged, as they are crucial for measuring model performance and guiding development direction.
...
Result: Identified current problems including score inflation from data contamination, unfair evaluation due to cultural/linguistic biases, and lack of evaluation on process credibility and dynamic environments.

Conclusion: Provided a referable design paradigm for future benchmark innovation to address current limitations and improve evaluation quality for large language models.
...
Result: Experiments reveal significant vulnerabilities in current MLLMs, showing gaps between trustworthiness and general capabilities, amplification of base LLM risks through multimodal training/inference, and limitations in existing mitigation strategies that often introduce trade-offs compromising utility.

Conclusion: The findings provide practical insights for future improvements, leading to the development of Reasoning-Enhanced Safety Alignment (RESA) approach that uses chain-of-thought reasoning to discover underlying risks and achieves state-of-the-art results.
...
Method: Proposes confidence-modulated drafting using entropy and margin-based uncertainty measures to dynamically adjust speculative token generation length and verification criteria.
...
TL;DR: Larger vocabularies help language models by reducing the complexity of tokenized text and improving performance on frequent words, not by better handling rare words. The models actually exploit token frequency imbalance rather than suffer from it.


<details>
  <summary>Details</summary>
Motivation: To understand why larger vocabularies benefit language models, given that most common words are already single tokens in smaller vocabularies and further growth mainly increases token-frequency imbalance.

Method: Conducted controlled experiments scaling vocabulary from 24K to 196K while holding data, compute, and optimization constant. Used Kolmogorov complexity to measure tokenized text complexity and performed word-level loss decomposition analysis.

Result: Larger vocabularies reduce cross-entropy almost exclusively by lowering uncertainty on the 2,500 most frequent words, while loss on rare words actually increases. Constraining embedding norms to reduce token-frequency imbalance reversed the gains, showing models exploit rather than suffer from imbalance.
...
TL;DR: This paper provides a systematic analysis of 134 papers on evidence-based text generation with LLMs, introducing a unified taxonomy and analyzing 300 evaluation metrics across seven dimensions to address fragmentation in the field.
...
Method: The authors systematically analyzed 134 papers, created a unified taxonomy, and investigated 300 evaluation metrics across seven key dimensions, focusing on approaches using citations, attribution, or quotations.
...
Conclusion: The paper highlights open challenges and outlines promising future directions to advance the field of evidence-based text generation with LLMs, addressing current fragmentation issues.
...
<details>
  <summary>Details</summary>
Motivation: Large Audio-Language Models (LALMs) can process multimodal audio-text inputs, but their performance with conflicting information between modalities remains unexamined, raising reliability concerns for real-world applications.

Method: Introduces MCR-BENCH benchmark to evaluate LALMs' information prioritization with inconsistent audio-text pairs. Evaluates across diverse audio understanding tasks, investigates text bias factors, explores supervised finetuning mitigation, and analyzes model confidence patterns.
...
TL;DR: LLaSO is the first fully open end-to-end framework for large speech-language models, providing aligned speech-text data, instruction tuning datasets, and standardized benchmarks to address reproducibility issues in the field.


<details>
  <summary>Details</summary>
Motivation: The LSLM field suffers from fragmented architectures, lack of transparency, and models being released without training data/configurations, hindering systematic comparison and reproducibility.

Method: Developed LLaSO framework with three components: LLaSO-Align (12M speech-text corpus), LLaSO-Instruct (13.5M multi-task dataset), and LLaSO-Eval benchmark. Built LLaSO-Base 3.8B parameter model trained exclusively on public data.
...
Conclusion: LLaSO establishes a foundational open standard with complete data, benchmarks, and models to unify research efforts and accelerate community-driven progress in large speech-language models.
...
<details>
  <summary>Details</summary>
Motivation: Language models trained on sensitive data can memorize and disclose private information during attacks, raising serious privacy concerns as privacy is a fundamental human right, yet understanding of privacy risks and mitigation methods remains limited.

Method: Provides a comprehensive study and in-depth overview of existing privacy-preserving language modeling approaches, analyzing their methodologies and implementations.

Result: The study highlights the strengths and investigates the limitations of current privacy-preserving approaches for language models.

Conclusion: This research contributes valuable insights to ongoing privacy-preserving language modeling research and outlines important future research directions to address privacy risks in language models.
...
TL;DR: M-Help dataset for detecting help-seeking behavior on social media with mental health disorder identification and root cause analysis


<details>
  <summary>Details</summary>
Motivation: Address the critical gap in identifying individuals actively seeking help for mental health disorders, going beyond traditional detection datasets

Method: Introduces a novel dataset (M-Help) specifically designed to detect help-seeking behavior, identify specific mental health disorders, and uncover underlying causes like relationship or financial stressors
...
Conclusion: M-Help dataset provides a comprehensive framework for detecting and understanding help-seeking behavior in mental health contexts on social media platforms
...
Method: Library-based research approach to identify and analyze different ways and rules for rendering non-equivalent words from source to target languages.

Result: Developed professional translation methods and successfully rendered 25 non-equivalent words from Dari & Uzbek into English and Russian languages.

Conclusion: The research provides effective strategies for translating culturally-specific words that lack direct equivalents, helping bridge understanding gaps between languages and cultures.
...
TL;DR: PyTOD is a programmable task-oriented dialogue agent that uses executable code generation for state tracking and leverages policy/execution feedback for error correction, achieving state-of-the-art performance on the SGD benchmark.
...
Method: PyTOD generates executable code for dialogue state tracking using constrained decoding with language models (instead of grammar rules) to follow API schemata, and employs policy and execution feedback for efficient error correction.
...
Conclusion: The approach demonstrates the effectiveness of execution-aware state tracking in programmable task-oriented dialogue agents.
...
TL;DR: RadReason is a novel evaluation framework for radiology reports that provides fine-grained sub-scores across six clinical error types with human-readable justifications, outperforming existing metrics and achieving parity with GPT-4 while being explainable and cost-efficient.
...
<details>
  <summary>Details</summary>
Motivation: Personalized marketing can boost revenue by up to 40%, but existing approaches focus mainly on recommendations and ads rather than offer generation. There's significant potential for improving conversion rates through intelligent, data-driven offer personalization.

Method: Fine-tuned Google's T5-Small (60M) encoder-decoder model using contrastive learning with InfoNCE loss to align customer personas with relevant offers in a shared embedding space. Used adaptive learning behavior through contrastive loss to reshape latent space and improve generalizability.

Result: 17% improvement in offer acceptance rate compared to supervised fine-tuning baseline, demonstrating the effectiveness of contrastive objectives in personalized marketing.

Conclusion: Contrastive learning approaches like SLM4Offer significantly advance personalized offer generation, showing practical business value through improved customer engagement and conversion rates.
...
TL;DR: Small LMs with page-level tokenization outperform large LMs for browsing behavior modeling, and cluster-specific training (HeTLM) achieves better performance and alignment than single LMs for heterogeneous user behaviors.


<details>
  <summary>Details</summary>
Motivation: Question whether large language models can adequately capture subjective user browsing behaviors and preferences, given that users have inherently idiosyncratic browsing patterns that form unique "languages" without formal structure.

Method: Introduces HeTLM (Heterogeneity aware Training of Language Model) using clusterwise training with page-level tokenizer and heterogeneous cluster-specific parameters instead of a single LM.
...
Conclusion: Cluster-specific training approach (HeTLM) provides better representation of subjective browsing behaviors and improved alignment at user level compared to single large language models.
...
<details>
  <summary>Details</summary>
Motivation: Previous curriculum learning approaches using human-centered difficulty metrics showed limited success in language model pre-training, suggesting the need for better difficulty measures that align with actual model training dynamics.

Method: Sort training examples by their training data influence score, which estimates the effect of individual training examples on the model's output, creating a model-centric curriculum from simpler to more complex examples.

Result: Models trained with this curriculum learning approach outperform randomly ordered training by over 10 percentage points on benchmarks.

Conclusion: Curriculum learning is beneficial for language model pre-training when using model-centric difficulty metrics like training data influence rather than conventional human-centered difficulty metrics.
...
Method: Developed SLM-Bench benchmark evaluating 15 SLMs on 9 NLP tasks using 23 datasets across 14 domains. Conducted evaluation on 4 hardware configurations with controlled conditions, measuring 11 metrics across correctness, computation, and consumption categories.

Result: The benchmark revealed diverse trade-offs among SLMs - some excel in accuracy while others achieve superior energy efficiency. The evaluation provides a holistic assessment of efficiency trade-offs between different models.
...
<details>
  <summary>Details</summary>
Motivation: Existing identity detection datasets are English-centric, single-label, and use coarse identity categories, lacking nuanced multilingual approaches needed for diverse political contexts.

Method: Created HebID corpus with manual annotations of 12 social identities grounded by survey data. Benchmarked multilabel/single-label encoders and Hebrew-tuned LLMs (2B-9B parameters) for identity detection.

Result: Hebrew-tuned LLMs achieved best performance (macro-F1 = 0.74). Analysis revealed differences in popularity, temporal trends, clustering patterns, and gender variations in identity expression across politicians' posts and speeches.
...
<details>
  <summary>Details</summary>
Motivation: To develop a more powerful open diffusion language model that can overcome the sequential generation limitations of autoregressive models and provide better performance on diverse tasks including general, mathematical, and coding domains.
...
Result: Consistently outperforms existing diffusion language models across general, mathematical, and coding tasks, demonstrating superior planning abilities, arbitrary-order generation, infilling capabilities, and tunable quality-speed trade-offs.

Conclusion: Dream 7B represents a significant advancement in diffusion-based language modeling, achieving state-of-the-art results through effective training techniques, and the release of Dream-Base and Dream-Instruct models will facilitate further research in this area.
...
TL;DR: First large-scale computational study of political delegitimization discourse (PDD) using Hebrew-language data from multiple sources, achieving strong classification performance and revealing significant trends in political discourse over time.
...
Method: Curated and manually annotated Hebrew corpus of 10,410 sentences from Knesset speeches, Facebook posts, and news outlets. Developed a two-stage classification pipeline combining finetuned encoder models and decoder LLMs (DictaLM 2.0).

Result: Best model achieved F1 of 0.74 for binary PDD detection and macro-F1 of 0.67 for classification of delegitimization characteristics. Analysis revealed: marked rise in PDD over three decades, higher prevalence on social media, greater use by male politicians, stronger tendencies among right-leaning actors, with spikes during elections and major events.

Conclusion: Demonstrates feasibility and value of automated PDD analysis for understanding democratic discourse, providing computational tools to study political delegitimization at scale across different platforms and time periods.
...
TL;DR: SafetyFlow is an automated agent-flow system that creates LLM safety benchmarks in 4 days without human intervention, addressing manual curation issues like redundancy and limited difficulty.
...
Method: SafetyFlow orchestrates seven specialized agents with versatile tools to automatically construct comprehensive safety benchmarks, integrating human expertise into the automated pipeline.

Result: The system generated SafetyFlowBench with 23,446 low-redundancy queries with strong discriminative power, and evaluated 49 advanced LLMs on the dataset.

Conclusion: SafetyFlow provides the first fully automated benchmarking pipeline that significantly reduces time and resource costs while maintaining process controllability and integrating human expertise.
...
TL;DR: Small Language Models (SLMs) fine-tuned for specific applications can generate domain-specific responses similar to LLMs but at much lower cost.


<details>
  <summary>Details</summary>
Motivation: Large language models require heavy computation and are expensive, making them infeasible for targeted applications like sales and marketing outreach.

Method: Introduces "Trained Miniatures" - Small Language Models fine-tuned for specific, high-value applications to generate domain-specific responses.

Result: SLMs can produce similar quality domain-specific responses as LLMs but for a fraction of the computational cost.

Conclusion: Fine-tuned small language models offer a cost-effective alternative to large language models for targeted, high-value applications.
...
TL;DR: SDGO is a reinforcement learning framework that uses LLMs' own discrimination capabilities as reward signals to align generation safety, improving robustness against jailbreaking attacks without external data.


<details>
  <summary>Details</summary>
Motivation: LLMs show safety inconsistency - they can better identify harmful content as discriminators than defend against it as generators, creating a vulnerability to jailbreaking attacks.

Method: Proposed SDGO (Self-Discrimination-Guided Optimization), a reinforcement learning framework that leverages the model's own discrimination capabilities as reward signals to iteratively improve generation safety without requiring additional annotated data or external models.

Result: SDGO significantly improves model safety compared to prompt-based and training-based baselines while maintaining helpfulness on general benchmarks. It shows robust performance against out-of-distribution jailbreaking attacks and enables further enhancement with minimal discriminative samples.

Conclusion: Aligning LLMs' discrimination and generation capabilities through self-guided optimization creates tighter coupling between these capabilities, providing an effective approach to enhance safety against jailbreaking attacks without external resources.
...
Method: Introduce SurGE benchmark with (1) test instances containing topic descriptions, expert-written surveys, and cited references, and (2) large-scale academic corpus of 1M+ papers as retrieval pool. Propose automated evaluation framework across four dimensions: information coverage, referencing accuracy, structural organization, and content quality.
...
Conclusion: Survey generation is a complex task requiring continued research. The authors open-sourced all code, data, and models to facilitate future work in this area.
...
Conclusion: Pos2Distill effectively addresses positional bias by transferring superior capabilities between positions, achieving superior performance on both retrieval and reasoning tasks while maintaining cross-task generalization.
...
TL;DR: Survey paper on Bangla stemming approaches highlighting resource scarcity, methodological gaps, and need for better evaluation metrics and implementations.


<details>
  <summary>Details</summary>
Motivation: Bangla is widely spoken but digitally under-represented with limited annotated datasets. Stemming is crucial for this low-resource, highly-inflectional language to reduce algorithmic complexity.

Method: Comprehensive survey of existing Bangla stemming approaches, analysis of literature gaps, critique of evaluation methodologies, and identification of implementation accessibility issues.

Result: Identified significant gaps in Bangla stemming research, discontinuity from previous work, scarcity of replicable implementations, and inadequate evaluation metrics.

Conclusion: Advocates for robust Bangla stemmer development with better evaluation methods and continued research to enhance language processing for this morphologically rich language.
...
Conclusion: Strategic utilization of multiple images via visual utility prediction (SUMEI) is necessary for effective multimodal understanding in e-commerce, as MLLMs may not optimally use rich visual content without guidance.
...
TL;DR: Deep-DxSearch is an agentic RAG system using reinforcement learning to improve medical diagnosis by LLMs through traceable retrieval-augmented reasoning, outperforming existing methods in accuracy.


<details>
  <summary>Details</summary>
Motivation: Medical LLMs suffer from knowledge gaps and hallucinations. Current retrieval and tool-augmented methods are limited by weak external knowledge use and poor feedback-reasoning traceability.

Method: End-to-end reinforcement learning training with tailored rewards on format, retrieval, reasoning structure, and diagnostic accuracy. Uses a large-scale medical retrieval corpus of patient records and reliable medical knowledge.

Result: Substantially outperforms prompt-engineering and training-free RAG approaches, surpassing GPT-4o, DeepSeek-R1, and other medical frameworks in diagnostic accuracy for both common and rare diseases in various settings.

Conclusion: The approach provides more reliable and precise preliminary diagnoses with improved traceability and interpretability, confirmed by ablation studies and case analyses.
...
TL;DR: ReasonZoo benchmark shows Tool-Integrated Reasoning (TIR) consistently improves LLM reasoning performance and efficiency across diverse domains, reducing overthinking and streamlining reasoning processes.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with precise computations despite advances in reasoning methods like chain-of-thought. The generalization and actual reasoning behavior improvements from Tool-Integrated Reasoning remain unclear and need systematic evaluation.
...
Result: TIR-enabled models consistently outperformed non-TIR counterparts in both mathematical and non-mathematical tasks, with improved reasoning efficiency evidenced by better PAC and AUC-PCC scores.

Conclusion: TIR provides domain-general benefits for LLM reasoning, enhances reasoning efficiency by reducing overthinking, and has significant potential to advance LLM capabilities in complex reasoning tasks.
...
TL;DR: LiveMCP-101 is a benchmark of 101 real-world queries requiring coordinated use of multiple MCP tools, with novel evaluation using ground-truth execution plans instead of raw API outputs, showing frontier LLMs achieve below 60% success rate.
...
Method: Created 101 carefully curated real-world queries refined through iterative LLM rewriting and manual review, requiring coordinated use of multiple MCP tools including web search, file operations, mathematical reasoning, and data analysis. Introduced novel evaluation approach using ground-truth execution plans rather than raw API outputs.

Result: Experiments show even frontier LLMs achieve success rate below 60%, highlighting major challenges in tool orchestration. Detailed ablations and error analysis reveal distinct failure modes and inefficiencies in token usage.
...
TL;DR: A hierarchical Safe RL framework that integrates ethical reasoning with autonomous driving, using ethical risk costs and prioritized experience replay to reduce harm while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles need robust ethical reasoning for widespread adoption, especially for handling emergency situations and reducing traffic fatalities while maintaining transportation efficiency.

Method: Hierarchical Safe RL framework with decision level using ethical risk cost (collision probability + harm severity) and prioritized experience replay, plus execution level with polynomial path planning and PID/Stanley controllers.
...
Method: Two-stage pipeline: 1) FAISS-based similarity search to identify most relevant patient cohort from multi-institutional database, 2) LLM prompting with retrieved cohort and performance metrics to recommend optimal prediction algorithm from 8 representative models.
...
TL;DR: A unified framework for Parkinson's disease progression prediction that combines graph neural networks for structural symptom relationships and Transformer for temporal modeling, with a gating mechanism to fuse structural and temporal information.
...
Method: Integrates graph neural networks to model structural relationships among multimodal clinical symptoms and Transformer architecture for temporal modeling. Uses structure-aware gating mechanism to dynamically fuse structural encodings and temporal features.
...
Conclusion: Provides reliable support for intelligent modeling of chronic progressive diseases like Parkinson's, demonstrating structural scalability and improved prediction accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Existing NAS models for text classification lack hierarchical structure and have unorganized, redundant search spaces that RL models cannot navigate effectively. Flat architecture search leads to difficult traversal of search space.

Method: Proposes HHNAS-AM with architectural templates based on domain-specific cues to organize search spaces. Uses adaptive mutation strategies with Q-learning that dynamically adjust based on performance feedback. Fully probabilistic model for effective exploration.
...
Conclusion: Hierarchical hybrid approach with adaptive mutation policies enables more effective and accelerated traversal of NAS search space for text classification, outperforming existing methods.
...
TL;DR: LPO is a new preference optimization method that addresses DPO's overfitting and collapse issues through gradient decoupling, stability improvements, and controllable rejection suppression, achieving better performance across various tasks.


<details>
  <summary>Details</summary>
Motivation: DPO suffers from overfitting and collapse problems despite its simplicity and training stability, which limits its effectiveness in preference alignment tasks.
...
Result: LPO consistently improves performance on general text tasks, math tasks, and text-to-speech tasks compared to DPO.

Conclusion: LPO establishes a robust and tunable paradigm for preference alignment that overcomes DPO's limitations, with publicly released code, models, and training data.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods only transfer user representations and use them as simple features, failing to capture valuable item and cross representations and not bridging the upstream-downstream gap effectively.

Method: Proposes LFM4Ads framework that comprehensively transfers all representations, identifies optimal extraction layers for cross representations, and uses multi-granularity transfer including non-linear adapters, Isomorphic Interaction Module, and Standalone Retrieval.
...
Conclusion: LFM4Ads demonstrates that comprehensive transfer of all representations with multi-granularity mechanisms significantly improves advertising recommendation performance and delivers substantial business value.
...
TL;DR: DiffQAS-QLSTM is an end-to-end differentiable framework that optimizes both variational quantum circuit parameters and architecture selection for quantum recurrent models, outperforming handcrafted baselines across diverse test settings.


<details>
  <summary>Details</summary>
Motivation: Designing effective variational quantum circuits (VQCs) for quantum machine learning remains challenging and often task-specific, creating a need for automated optimization approaches in quantum sequence learning.
...
Result: The framework consistently outperforms handcrafted baselines, achieving lower loss across diverse test settings in quantum sequence learning tasks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional atmospheric profile restoration methods blur fine-scale features and lack uncertainty quantification, while deep learning models don't provide confidence estimates for the restored data.

Method: Uses a curriculum-guided training approach with progressive masking, Monte Carlo sampling over random mask realizations at inference, and aggregates outputs to obtain mean reconstruction with per-pixel uncertainty maps.

Result: Achieves high-fidelity reconstruction of fine-scale atmospheric features (updrafts, downdrafts, shear lines, vortices) while providing pixel-wise uncertainty quantification.
...
TL;DR: Aura-CAPTCHA is a multi-modal CAPTCHA system that uses GANs, RL, and LLMs to create dynamic challenges, achieving 92% human success rate and only 10% bot bypass rate.
...
Method: Integrated GANs for generating dynamic image challenges, RL for adaptive difficulty tuning, and LLMs for text/audio prompts. Visual challenges use 3x3 grid selections, audio challenges combine randomized numbers and words.
...
Conclusion: Provides a robust and scalable approach for securing online applications while remaining accessible to users, addressing gaps in previous research.
...
Result: The paper demonstrates that when input losses lie in suitable infinite-dimensional compact sets, GEOs can uniformly approximate solutions with arbitrary precision, requiring only logarithmic growth in rank, depth, and width relative to the approximation error.

Conclusion: GEOs provide a theoretically sound framework that reconciles the apparent contradiction between worst-case parameter bounds and practical performance of neural operators, validated through successful applications to nonlinear PDEs, stochastic optimal control, and financial hedging problems.
...
Method: Systematic review of quantization techniques, evaluation of software frameworks and hardware platforms for QNN execution, and analysis of performance-hardware trade-offs.

Result: Comprehensive survey providing hardware-centric introduction to quantization, assessment of existing tools, and identification of critical trade-offs between model performance and hardware capabilities.

Conclusion: Identifies current challenges in QNN deployment on microcontrollers and outlines promising future directions for the rapidly evolving TinyML domain, emphasizing the need for continued hardware-software co-design.
...
<details>
  <summary>Details</summary>
Motivation: Existing auto-partitioners for ML model partitioning suffer from out-of-memory errors, slow exploration of large search spaces, and often produce infeasible or sub-optimal solutions due to artificial search space restrictions.

Method: Combines novel static compiler analysis to construct efficient decision space by identifying tensor dimensions requiring identical sharding and partitioning conflicts, with Monte Carlo Tree Search for optimal partitioning discovery.

Result: Significantly outperforms state-of-the-art industrial methods across diverse hardware platforms and model architectures, discovering previously unknown superior solutions with full automation even for complex large models.

Conclusion: The proposed system effectively addresses partitioning challenges through intelligent static analysis and search optimization, delivering automated, memory-safe, and high-performance solutions for distributed ML model deployment.
...
TL;DR: Twin-Bootstrap Gradient Descent (Twin-Boot) is a novel training method that integrates uncertainty estimation into optimization by training two identical models on bootstrap samples with periodic resets to keep them in the same basin, providing adaptive regularization and improved calibration.


<details>
  <summary>Details</summary>
Motivation: Standard gradient descent lacks uncertainty measures, which is problematic in overparameterized and low-data regimes where models easily overfit. Bootstrapping is impractical for deep learning due to computational costs, post-hoc nature, and non-convex landscape assumptions.

Method: Two identical models are trained in parallel on independent bootstrap samples. Periodic mean-reset keeps both trajectories in the same basin, and their divergence is used to estimate local uncertainty. This uncertainty guides adaptive weight sampling for regularization.
...
Conclusion: Twin-Boot successfully integrates uncertainty estimation directly into the optimization process, addressing limitations of traditional bootstrapping while providing effective regularization and uncertainty quantification for deep learning models.
...
TL;DR: Federated learning improves convergence for nonlinear system identification as client count increases, with feature map choice enabling better performance than centralized approaches.


<details>
  <summary>Details</summary>
Motivation: To establish theoretical guarantees for federated nonlinear system identification and demonstrate its advantages over centralized methods, particularly how increasing client participation enhances convergence rates.
...
Result: Convergence rate improves with increasing number of clients; federated learning consistently enhances individual client convergence; performance depends on feature map choice which can increase excitation in nonlinear settings.

Conclusion: Federated learning is effective for nonlinear system identification, offering improved convergence over centralized approaches as client participation grows, with careful feature map selection providing additional performance benefits.
...
<details>
  <summary>Details</summary>
Motivation: Traditional layer-freezing methods still require forward propagation through frozen layers, limiting computational savings. Caching feature maps presents challenges like effective augmentation application and significant storage overhead that prior works have overlooked.
...
Result: The solution achieves significant reductions in training cost while maintaining model accuracy, with only minor time overhead. The paper also provides comprehensive evaluation of freezing and compression strategies.

Conclusion: This work provides the first comprehensive exploration of challenges in feature map caching for layer-freezing and offers a systematic solution that effectively reduces computational costs while preserving training performance.
...
TL;DR: Tight minimax rates for robust estimation under heterogeneous corruption rates, showing optimal estimators discard samples beyond certain corruption thresholds determined by empirical corruption distribution.


<details>
  <summary>Details</summary>
Motivation: Addresses robust estimation in settings with non-identical corruption probabilities per sample, which arises naturally in distributed learning, crowdsourcing, and sensor networks, where existing methods assume uniform corruption.
...
Result: Achieved tight minimax rates for all heterogeneous corruption patterns in mean estimation, and established minimax rate for squared error up to √d factor for multivariate gaussian mean estimation and linear regression.

Conclusion: Optimal estimators should discard samples beyond a corruption threshold determined by the empirical distribution of corruption rates, providing guidance for robust estimation in heterogeneous corruption settings.
...
TL;DR: Proposes NGN-M, a momentum-based optimizer with adaptive step-size that improves stability across hyperparameter choices while matching state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Modern momentum and adaptive step-size optimizers are sensitive to hyperparameter tuning, which is difficult and resource-intensive. Recent efforts focus on improving stability across hyperparameter ranges.
...
Result: The algorithm attains O(1/√K) convergence rate under relaxed assumptions and demonstrates enhanced robustness to step-size hyperparameter choice while delivering comparable or superior performance to state-of-the-art optimizers.

Conclusion: NGN-M provides improved stability to hyperparameter choices while maintaining competitive performance, addressing the sensitivity issues of modern optimization algorithms in deep learning tasks.
...
TL;DR: Deep neural networks in overparameterized regimes exhibit feature space collapse that improves generalization but can lead to degeneracy where distinct inputs map to same outputs. A novel "wormhole" solution bypasses this degeneracy and reconciles meaningful labels with random ones.


<details>
  <summary>Details</summary>
Motivation: To understand why DNNs confidently classify random inputs (fooling examples) and investigate generalization behavior through analytical framework rather than conventional numerical approaches.

Method: Introduced analytical framework based on maximum likelihood estimation without gradient-based optimization or explicit labels. Explored overparameterized DNN regimes and derived "wormhole" solution to address degeneracy.

Result: DNNs in overparameterized regime show feature space collapse that improves generalization but eventually leads to degeneracy. Wormhole solution successfully bypasses degeneracy and reconciles meaningful labels with random inputs.

Conclusion: Findings provide deeper insights into DNN generalization and shortcut learning, highlighting directions for future research on unsupervised learning dynamics to bridge theory-practice gap.
...
<details>
  <summary>Details</summary>
Motivation: Polysemanticity in large language models makes interpretation difficult, and while SAEs have been proposed to address this, there was no quantitative comparison showing their effectiveness against base models.

Method: Introduced a fine-grained concept separability score based on Jensen-Shannon distance, evaluated Gemma-2-2B with multiple SAE variants across five benchmarks, and proposed Attenuation via Posterior Probabilities (APP) for targeted concept suppression.
...
Conclusion: SAEs are effective for reducing polysemanticity and enabling targeted concept interventions, with the proposed APP method outperforming existing approaches for concept removal.
...
<details>
  <summary>Details</summary>
Motivation: To create efficient long-context language models by integrating multiple advanced techniques (SSM, sparse attention, MoE, memory systems) within a constrained parameter budget for better scalability and performance.

Method: Combines Mamba-style SSM backbone with intermittent sparse global attention, chunk-level MoE feed-forward routing, and dual memory systems (workspace + factual PKM). Uses formal component interfaces and staged curriculum training.

Result: Implementation feasibility demonstrated through toy-scale prototypes showing long-context throughput crossover and controllable expert routing. No competitive full-scale performance claims yet.

Conclusion: Hydra presents a blueprint for modular, input-adaptive long-context models combining SSM efficiency with selective attention and memory systems. End-task validation at target scale remains future work.
...
TL;DR: Concept Erasure Techniques (CETs) for text-to-image models can be easily bypassed using hierarchical and compositional prompts, and suffer from side effects including attribute leakage and attention issues.


<details>
  <summary>Details</summary>
Motivation: To address privacy, copyright, and safety concerns in text-to-image generative models by developing techniques to erase unwanted concepts while maintaining image quality for other concepts.
...
Result: CETs can be circumvented using superclass-subclass hierarchy and semantically similar prompts, and exhibit attribute leakage with attention concentration/dispersal issues.

Conclusion: Current CETs are vulnerable to evasion techniques and have significant side effects, highlighting the need for more robust concept erasure methods.
...
<details>
  <summary>Details</summary>
Motivation: As machine learning becomes more pervasive and data privacy regulations evolve, there's a critical need to remove private/copyrighted information from trained models. Existing methods require access to the full training dataset, which isn't practical in real-world scenarios where original data may be inaccessible.
...
<details>
  <summary>Details</summary>
Motivation: To provide a more powerful mathematical foundation for reinforcement learning by generalizing it using category theory and coalgebraic structures, enabling better modeling of complex systems and distributed computation.

Method: Builds on coinduction on non-well-founded sets, universal coalgebras, and topos theory. Models RL algorithms as functor categories and shows how various dynamical systems (MDPs, POMDPs, PSRs, LDSs) are special coalgebras. Generalizes fixed-point finding to determining final coalgebras asynchronously.

Result: Develops a comprehensive categorial framework for RL that unifies various dynamical system models under the coalgebraic perspective and provides mathematical tools for analyzing distributed asynchronous computation in RL.

Conclusion: The paper establishes URL as a powerful generalization of traditional RL, providing deeper mathematical insights and enabling more sophisticated modeling of complex reinforcement learning problems through category theory and coalgebraic structures.
...
<details>
  <summary>Details</summary>
Motivation: Recent proliferation of DPML papers claiming SoTA results without consensus on effectiveness, compounded by heterogeneity in codebases, datasets, and methodologies making direct comparisons challenging.

Method: Conducted reproducibility and replicability (R+R) experiments on 11 different SoTA DPML techniques from recent literature, addressing challenges unique to DPML reproducibility including additional randomness from DP noise.
...
Conclusion: Derived insights and best practices for obtaining scientifically valid and reliable results in DPML research, highlighting the need for more rigorous validation standards.
...
Result: The fine-tuned RoBERTa model significantly outperformed baseline models and Mistral 7B, achieving F1_Bertscore 0.98 and overall exact match of 80.61%.
...
<details>
  <summary>Details</summary>
Motivation: Jailbreak attacks pose serious threats to LLM safety by bypassing alignment mechanisms to produce harmful content, requiring effective defense methods that maintain model performance.

Method: Three-stage pipeline: 1) dynamic unsafe output detection with hybrid classifiers, 2) token-level harmful content tracing via FFN activations, 3) constrained optimization to suppress unsafe behavior without quality degradation.

Result: Substantially reduces attack success rates across multiple jailbreak benchmarks while maintaining high general-purpose performance on Vicuna, LLaMA, and GPT-J models.

Conclusion: Unlearning is a promising direction for scalable and effective LLM safety, offering stronger guarantees, precise control, and greater robustness compared to standard defense methods.
...
TL;DR: Proposes frequency-adaptive tensor neural networks (TNNs) with random Fourier features to overcome Frequency Principle limitations and improve high-dimensional multi-scale problem solving.


<details>
  <summary>Details</summary>
Motivation: TNNs suffer from Frequency Principle limitations that restrict their ability to capture high-frequency features in high-dimensional problems, similar to conventional neural networks.

Method: Incorporates random Fourier features into TNNs and performs Discrete Fourier Transform on one-dimensional component functions to extract frequency features while mitigating curse of dimensionality.

Result: Developed frequency-adaptive TNNs algorithm that significantly enhances TNNs' ability to solve complex multi-scale problems, validated through extensive numerical experiments.

Conclusion: The proposed frequency-adaptive approach effectively overcomes Frequency Principle limitations and improves TNN performance for high-dimensional multi-scale applications.
...
TL;DR: SleepDIFFormer: A transformer-based method for automated sleep stage classification using joint EEG-EOG representation learning with cross-domain alignment to improve generalization on unseen datasets.
...
Conclusion: SleepDIFFormer advances automated sleep stage classification with improved generalization capabilities, showing relevance to characteristic sleep EEG patterns and potential applications in sleep quality assessment.
...
TL;DR: Proposes Multi-Attribution Learning (MAL) framework for CVR prediction that integrates multiple attribution mechanisms instead of using just one, achieving significant improvements in both offline metrics (+0.51% GAUC) and online ROI (+2.6%).


<details>
  <summary>Details</summary>
Motivation: Conventional CVR prediction approaches restrict model training to labels from a single production-critical attribution mechanism, discarding valuable complementary signals from alternative attribution perspectives.

Method: MAL framework with two core components: Attribution Knowledge Aggregator (AKA) as multi-task learner integrating diverse attribution labels, and Primary Target Predictor (PTP) for generating calibrated conversion probabilities. Also proposes CAT training strategy using Cartesian product of attribution labels for enriched supervision.

Result: Empirical evaluations show MAL outperforms single-attribution baselines with +0.51% GAUC improvement offline and +2.6% ROI increase in online experiments.

Conclusion: Integrating multiple attribution perspectives through the MAL framework significantly enhances CVR prediction performance and provides better ROI for online advertising systems compared to traditional single-attribution approaches.
...
TL;DR: A framework for synthesizing Pareto-optimal interpretations with local optimality guarantees, balancing accuracy and explainability using SAT verification.


<details>
  <summary>Details</summary>
Motivation: Addressing the trade-off between accuracy and explainability in black-box ML models while overcoming scalability limitations of methods with formal guarantees.

Method: Uses multi-objective learning/search (e.g., Monte Carlo Tree Search) to generate Pareto-optimal candidates, then verifies local optimality via SAT solving.

Result: Produces interpretations that closely match those from methods with global guarantees while being more scalable.

Conclusion: The approach enables scalable synthesis of trustworthy interpretations with local optimality guarantees, bridging the gap between accuracy and explainability.
...
TL;DR: Poly-window contrastive learning framework for ECG analysis that extracts multiple temporal windows to learn robust, temporally invariant representations, outperforming traditional two-view methods with higher efficiency and reduced training time.


<details>
  <summary>Details</summary>
Motivation: Overcome limitations of existing self-supervised contrastive learning methods that only generate pairwise augmented views and fail to leverage the rich temporal structure of ECG recordings, which constrains deep learning performance due to limited annotated data.

Method: Extract multiple temporal windows from each ECG instance to construct positive pairs and maximize their agreement via statistics, inspired by slow feature analysis to learn temporally invariant and physiologically meaningful features.
...
TL;DR: DeepConf is a test-time method that uses LLM internal confidence signals to filter low-quality reasoning traces, improving accuracy while reducing computational overhead without additional training.


<details>
  <summary>Details</summary>
Motivation: Existing self-consistency methods with majority voting show diminishing accuracy returns and high computational costs, creating a need for more efficient reasoning approaches.
...
Conclusion: DeepConf provides a simple yet effective approach to enhance both reasoning efficiency and performance at test time, seamlessly integrable into existing serving frameworks.
...
TL;DR: CSG complexity metric fails to reliably predict link prediction performance on knowledge graphs, while new structural/semantic metrics like Relation Entropy show stronger correlations with task difficulty.


<details>
  <summary>Details</summary>
Motivation: To evaluate the effectiveness of Cumulative Spectral Gradient (CSG) as a dataset complexity metric for knowledge graph link prediction and examine its correlation with downstream performance metrics.

Method: Critical examination of CSG in multi-relational link prediction using transformer-derived embeddings, plus introduction and benchmarking of new structural and semantic KG complexity metrics including Relation Entropy, Maximum Relation Diversity, and Relation Type Cardinality.

Result: CSG is highly sensitive to parameterization, doesn't scale robustly with class cardinality, and shows weak/inconsistent correlation with MRR and Hit@1. New metrics (Relation Entropy, etc.) show strong inverse correlations with performance metrics, while graph connectivity measures correlate positively with Hit@10.

Conclusion: CSG's claimed stability and generalization predictive power don't hold in link prediction settings, highlighting the need for more stable, interpretable, and task-aligned complexity measures for knowledge-driven learning.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches struggle with generalization to unknown classes - class discovery favors known classes, incremental learning suffers from catastrophic forgetting, and logic-based methods require complete logical definitions.

Method: Proposes PL-Reg, a partial-logic regularization term that allows models to reserve space for undefined logic formulas, enabling better adaptability to unknown classes through partial logic formalization.
...
Conclusion: Partial logic effectively addresses challenges with unknown classes, demonstrating improved generalization capabilities compared to existing methods.
...
Method: Uses transformer-based architectures to combine textual and tabular data, incorporating metadata, domain knowledge, and user profiling through a cyclical process that dynamically updates with evolving user behaviors.
...
Conclusion: ExBigBang successfully addresses contextual modeling and explainability gaps in persona classification, offering a dynamic and interpretable approach for user-centric design.
...
Method: Two-dimensional time series forecasting model that integrates cohort behavior dynamics and temporal patterns.
...
TL;DR: End-users can coordinate to relabel their own data to improve model fairness without changing the training process, achieving significant fairness gains with minimal utility loss.


<details>
  <summary>Details</summary>
Motivation: Machine learning models often preserve biases from training data, and existing firm-side bias mitigation techniques require organizational buy-in and incur utility costs. Since many models rely on user-contributed data, end-users can take collective action to improve fairness.

Method: Three model-agnostic methods for approximating ideal relabeling where a coordinated minority group strategically relabels its own data within the Algorithmic Collective Action framework.
...
Conclusion: End-user collective action through strategic data relabeling provides an effective, practical approach to enhance fairness without requiring changes to the firm's training process or causing significant utility degradation.
...
TL;DR: EvoFormer is a Transformer framework that addresses Structural Visit Bias and Abrupt Evolution Blindness in dynamic graph embedding through structure-aware and evolution-sensitive modules, achieving SOTA performance.
...
Method: EvoFormer uses: 1) Structure-Aware Transformer Module with positional encoding based on node structural roles to address bias; 2) Evolution-Sensitive Temporal Module with three-step strategy: timestamp classification, temporal segmentation, and segment-aware attention with edge evolution prediction.
...
<details>
  <summary>Details</summary>
Motivation: There's a critical shortage of large-scale benchmark datasets for heterogeneous text-attributed graphs, which hinders development and fair comparison of representation learning methods in this domain.
...
Result: The paper provides comprehensive benchmarking results and ablation experiments on the heterogeneous and textual properties of the CITE dataset across diverse modeling approaches.

Conclusion: CITE serves as a valuable benchmark resource that enables standardized evaluation and comparison of representation learning methods for heterogeneous text-attributed graphs in catalytic materials research.
...
TL;DR: Evolving Fuzzy System for Federated Learning that dynamically adapts to new clusters without requiring predefined cluster count, outperforming traditional methods on UCI datasets.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional clustering methods that require predefined cluster numbers and enable decentralized learning while preserving data privacy through federated learning.

Method: Implemented an evolving fuzzy system within federated learning framework using PyTorch, allowing dynamic cluster addition and local model training on client devices with parameter sharing only.

Result: Outperformed established classification methods on multiple UCI datasets, demonstrating superior performance in clustering and classification tasks despite computational intensity from overlap condition calculations.

Conclusion: The proposed method offers significant advantages for decentralized data processing while maintaining data privacy, though it requires computational resources for overlap calculations.
...
TL;DR: Hybrid least squares/gradient descent method for efficient DeepONet training by decomposing large linear system into smaller branch and trunk subproblems.


<details>
  <summary>Details</summary>
Motivation: Accelerate DeepONet training by addressing the computational infeasibility of solving the prohibitively large least squares system directly for all branch-trunk input combinations.

Method: Decompose the large least squares system into two smaller subproblems (branch network and trunk network) and solve them separately, while updating hidden layer parameters with gradient descent. Generalizes to L^2 loss with regularization and physics-informed unsupervised learning.

Result: Proposed method enables efficient optimization of DeepONet parameters by avoiding direct solution of the computationally prohibitive large linear system.

Conclusion: The hybrid approach provides a computationally feasible solution for DeepONet training that can handle both supervised and physics-informed unsupervised learning scenarios with regularization.
...
<details>
  <summary>Details</summary>
Motivation: Human Activity Recognition models suffer from performance degradation when deployed to new users due to user-induced concept drift, highlighting the need for efficient personalization methods.

Method: Hybrid framework that updates only the classifier layer with user-specific data using few-shot learning directly on energy-efficient RISC-V-based microcontrollers.

Result: Consistent accuracy improvements of 3.73%, 17.38%, and 3.70% across three diverse HAR scenarios (RecGym, QVAR-Gesture, Ultrasound-Gesture) with minimal computational and memory overhead.
...
TL;DR: Proposed a fast dissimilarity measure for detecting overlapping multivariate Gaussian clusters in data streams, addressing limitations of existing methods.
...
Method: Developed a new dissimilarity measure specifically designed to detect overlap rather than dissimilarity, which can be computed faster than existing methods and avoids merging orthogonal clusters.
...
Conclusion: The new measure provides an efficient solution for detecting overlapping clusters in online learning from streaming data, overcoming computational and shape-handling limitations of traditional approaches.
...
TL;DR: MRP uses irreversible projection in hidden state space to completely eliminate harmful information while preserving useful knowledge, enabling effective continuous unlearning and defense against relearning attacks.
...
Method: Metamorphosis Representation Projection (MRP) applies irreversible projection properties in the hidden state space of specific network layers to eliminate harmful information while preserving useful knowledge.
...
Conclusion: MRP overcomes fundamental limitations of existing methods by using irreversible projection to completely eliminate harmful information traces, providing a robust solution for machine unlearning that supports continuous unlearning and resists relearning attacks.
...
<details>
  <summary>Details</summary>
Motivation: To develop a mathematically tractable model that captures biological synaptic switching behavior while maintaining desirable computational properties for neuromorphic computing applications.

Method: Analysis of a linear-in-state, nonlinear-in-input differential equation model derived from experimental molecular switches, focusing on exact solvability, convergence, and fading memory properties.

Result: The model demonstrates co-existence of biologically-inspired synaptic behavior and mathematical stability properties, enabling stable processing of time-varying inputs in nonlinear dynamical systems.

Conclusion: This provides theoretical foundation for using dynamic molecular switches as computational units in neuromorphic architectures and inspires development of more general exactly solvable models for brain-inspired computing.
...
TL;DR: BaVerLy is a group local robustness verifier that improves neural network verification efficiency by batching similar input regions, achieving 2.3x average speedup over traditional one-by-one verification.


<details>
  <summary>Details</summary>
Motivation: Neural network classifiers are vulnerable to adversarial attacks, but existing local robustness verifiers are either too slow or imprecise when analyzing large sets of input regions (epsilon-balls).

Method: Proposes BaVerLy - a sound and complete verifier that dynamically constructs mini-batches of epsilon-balls with similar network computations. It adaptively identifies batch sizes, verifies batches jointly, and uses analysis results to guide refinement when robustness violations are suspected.

Result: Evaluation on fully connected and convolutional networks for MNIST and CIFAR-10 shows 2.3x average speedup (up to 4.1x), reducing analysis time from 24 hours to 6 hours in best cases.

Conclusion: Group local robustness verification through mini-batching significantly improves verification efficiency while maintaining soundness and completeness, making large-scale neural network robustness verification more practical.
...
TL;DR: HypSeek uses hyperbolic geometry for protein-ligand binding prediction, improving virtual screening and affinity ranking performance by better capturing hierarchical molecular interactions.
...
Conclusion: Hyperbolic geometry provides powerful inductive bias for protein-ligand modeling, enabling expressive affinity-sensitive embeddings that effectively model both global activity and subtle functional differences.
...
TL;DR: FairGuide is a novel framework that enhances graph neural network fairness by introducing new links to create unbiased graph structures, using a differentiable community detection task and meta-gradients to optimize structural fairness.


<details>
  <summary>Details</summary>
Motivation: Graph Neural Networks face fairness challenges due to biased graph structures, and existing structures can be guided toward fairness by introducing new links to foster unbiased communities.
...
Result: Theoretical analysis shows optimizing fairness in pseudo task enhances structural fairness and promotes fairness generalization across diverse downstream applications.

Conclusion: Extensive experiments demonstrate FairGuide's effectiveness and generalizability across various graph-based fairness tasks, providing a robust solution for enhancing GNN fairness.
...
TL;DR: A novel ADMM-based distributed learning algorithm that achieves both computation and communication efficiency through stochastic gradients, multiple local training epochs, and compressed transmissions, with proven linear convergence for strongly convex problems.


<details>
  <summary>Details</summary>
Motivation: To address distributed learning over undirected networks by designing an algorithm that is both computation-efficient (using stochastic gradients) and communication-efficient (reducing transmission frequency and size).

Method: ADMM-based approach with stochastic gradients for local computation, multiple training epochs between communication rounds, and compressed transmissions to reduce communication overhead.

Result: The algorithm achieves exact linear convergence in strongly convex settings and demonstrates competitive performance compared to state-of-the-art techniques in classification tasks.

Conclusion: The proposed method successfully combines computational and communication efficiency in distributed learning, providing theoretical guarantees and practical performance improvements for network-based learning systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing gradient-based DP methods like DP-SGD require noise injection into gradients, which reduces computational efficiency and stability. There's a need for better privacy-utility trade-offs in multi-user local differential privacy settings, especially in noisy environments like wireless body area networks.

Method: SPOF perturbs a stabilized Taylor expanded polynomial approximation of the model's training loss function. Each user's data is privatized by adding calibrated noise to the polynomial coefficients instead of gradients. This approach avoids gradient noise injection and supports simultaneous privacy guarantees across all users.

Result: SPOF achieves up to 3.5% higher reconstruction accuracy and reduces mean training time by up to 57.2% compared to DP-SGD. It demonstrates robustness to environmental noise and maintains stable performance even with corrupted user inputs in WBAN scenarios.

Conclusion: SPOF provides superior privacy-utility trade-offs in multi-user LDP environments, offering improved computational efficiency, stability, and accuracy compared to gradient-based approaches like DP-SGD, particularly in noisy real-world applications.
...
TL;DR: A practical framework for early fault detection in industrial pumps using sensor data and machine learning, combining fixed and adaptive thresholds with synthetic fault injection to handle rare failures.


<details>
  <summary>Details</summary>
Motivation: To develop a scalable and interpretable system for early fault detection in industrial equipment, addressing the challenge of rare documented failures in real-world operational data.

Method: Monitored 5 operational parameters (vibration, temperature, flow rate, pressure, current) using dual-threshold labeling (fixed engineering limits + 95th percentile adaptive thresholds). Injected synthetic fault signals using domain rules. Trained Random Forest, XGBoost, and SVM classifiers to detect normal operation, early warnings, and critical alerts.

Result: Random Forest and XGBoost achieved high accuracy across all classes including minority fault cases, while SVM showed lower sensitivity to anomalies. Visual analyses confirmed robust detection capabilities.

Conclusion: The proposed hybrid method provides scalable, interpretable, and real-time deployable fault detection suitable for industrial predictive maintenance, with potential application to other machinery with similar sensor architectures.
...
TL;DR: A new framework combining Conformal Prediction and Exceptional Model Mining to identify data subgroups with exceptional model performance patterns, providing uncertainty quantification and interpretable insights.
...
TL;DR: A new inductive and amortized simulation-based inference framework that addresses model misspecification through integrated calibration and optimal transport-based distribution alignment, enabling efficient inference without test-time simulation access.


<details>
  <summary>Details</summary>
Motivation: Existing SBI approaches like RoPE suffer from limitations in transductive settings requiring batch test samples at inference time, which hinders scalability and generalization in misspecified environments.

Method: Proposes an end-to-end trainable framework combining mini-batch optimal transport with closed-form coupling to align real and simulated observations, followed by conditional normalizing flow training to approximate OT-induced posterior.

Result: The method matches or surpasses RoPE and other SBI/non-SBI estimators across synthetic and real-world benchmarks including complex medical biomarker estimation.

Conclusion: The proposed framework offers improved scalability and applicability in challenging misspecified environments while maintaining competitive performance without requiring simulation access at test time.
...
Conclusion: The proposed CoNTM successfully fills the gap by enabling continual topic learning with long-term memory, demonstrating superior performance over existing methods.
...
TL;DR: GRASPED is an unsupervised graph anomaly detection model using graph wavelet convolution encoder and Wiener deconvolution decoder to capture multi-scale spectral information, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing supervised methods suffer from limited labeled anomaly data, while unsupervised approaches rely mainly on spatial information or low-pass filters, lacking multi-band spectral analysis capabilities for effective anomaly detection.

Method: Proposes GRASPED with Graph Wavelet Convolution encoder and structural/attribute decoders using Wiener Graph Deconvolution, providing bandpass filter characteristics to capture global and local graph information at multiple scales for node attribute reconstruction.
...
Conclusion: The proposed unsupervised GRASPED model effectively addresses spectral information utilization in graph anomaly detection through multi-scale analysis, demonstrating superior performance without requiring labeled data.
...
TL;DR: Bayesian analysis shows automated audio classifiers significantly distort language acquisition research estimates, with errors underestimating sibling effects by 20-80%. A calibration method helps but isn't foolproof.


<details>
  <summary>Details</summary>
Motivation: Automated audio classifiers are widely used in language acquisition research, but their classification errors' downstream effects on statistical inferences and measurements haven't been thoroughly studied.

Method: Proposes a Bayesian approach to analyze effects of algorithmic errors on scientific questions, examining both LENA system and open-source Voice Type Classifier from ACLEW system.
...
Conclusion: Bayesian calibration can help recover unbiased estimates but isn't foolproof; the issue applies to any classifier with non-zero error rates in event detection and classification tasks.
...
TL;DR: A novel framework that formally guarantees individual fairness throughout machine learning training using provably fair initialization and fairness-preserving algorithms with randomized response mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing fairness methods lack formal guarantees and verification techniques don't actively enhance fairness during training, creating a need for approaches that provide provable fairness throughout the training process.
...
Result: Experimental evaluations confirm the approach produces models that are both empirically fair and accurate, while being more efficient than certified training approaches that require neural network verification during training.

Conclusion: The proposed framework successfully provides formal guarantees of individual fairness throughout the training process, addressing limitations of existing methods and offering improved efficiency compared to verification-based approaches.
...
TL;DR: AICMET is a transformer-based model that combines mechanistic compartmental priors with amortized Bayesian inference for accurate dose-response forecasting with minimal data, achieving state-of-the-art performance in pharmacokinetic modeling.
...
Result: AICMET attains state-of-the-art predictive accuracy, faithfully quantifies inter-patient variability, and outperforms nonlinear mixed-effects baselines and neural ODE variants across public datasets.

Conclusion: Transformer-based, population-aware neural architectures offer a viable alternative to traditional pharmacokinetic modeling, enabling rapid adaptation and personalized dosing regimens with minimal data.
...
TL;DR: Proposes multi-task learning with low-rank tensor decomposition for personalized modeling of heterogeneous subpopulations, improving accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of modeling heterogeneous subpopulations with varying individual characteristics and behaviors through personalized approaches.

Method: Multi-task learning framework combined with low-rank tensor decomposition to capture shared structures and subpopulation-specific variations in model parameters.

Result: Superior performance compared to benchmarks in simulation and case studies, especially with high subpopulation variability, with improved prediction accuracy and interpretability.

Conclusion: The proposed framework effectively handles heterogeneous subpopulations by leveraging shared knowledge while preserving unique characteristics, demonstrating both performance and interpretability benefits.
...
TL;DR: A multi-agent environment for studying social intelligence in AI agents, enabling research on cooperation, competition, and learning from human experts in open-ended settings.


<details>
  <summary>Details</summary>
Motivation: Real-world AI deployment requires agents that can operate in multi-agent environments with human experts, but current research is limited by the lack of open-ended multi-agent environments to study social intelligence and adaptive learning.

Method: Developed an open-ended multi-agent environment where self-interested agents pursue complex independent goals, allowing investigation of social learning, implicit cooperation, emergent collaborative tool use, and the balance between cooperation and competition.

Result: The environment enables research into socially intelligent AI agents that can learn adaptive behaviors from experts, cooperate to defeat common enemies, build/share tools, and achieve long-term goals through both cooperative and competitive interactions.

Conclusion: This environment provides a foundation for studying how social intelligence can accelerate AI agent training and development in realistic multi-agent settings, addressing current limitations in the field.
...
<details>
  <summary>Details</summary>
Motivation: To improve the physics and equality constrained artificial neural networks (PECANN) framework for better learning solutions of partial differential equations with heterogeneous constraints, oscillatory features, and long-time evolution problems.
...
Conclusion: The proposed advances significantly improve PECANN's robustness, efficiency, and applicability to demanding scientific computing problems with enhanced constraint enforcement and reduced computational overhead.
...
TL;DR: The paper explores why D-Wave quantum annealer sampling doesn't significantly improve RBM training compared to classical MCMC, finding that differences occur mainly in low-probability regions that don't benefit training. However, it demonstrates quantum annealing's potential for catastrophic forgetting mitigation through generative replay.


<details>
  <summary>Details</summary>
Motivation: To understand why previous investigations showed no significant improvements in Restricted Boltzmann Machine (RBM) trainability when using D-Wave quantum annealer sampling compared to classical Markov Chain Monte Carlo methods, and to explore potential benefits of hybrid sampling approaches.

Method: The researchers investigated a novel hybrid sampling approach combining classical MCMC and quantum annealing contributions. They analyzed statistical differences between D-Wave QA and classical MCMC sampling performances on RBMs, and tested the feasibility of using QA-generated patterns for catastrophic forgetting mitigation through generative replay.

Result: No improvements in RBM training were achieved, as differences between QA-based and MCMC sampling mainly occur in medium-to-low probability regions that are less important for sample quality. However, the study successfully demonstrated the feasibility of using QA-generated patterns for catastrophic forgetting mitigation, with efficiency comparable to classical methods but with advantages in speed of generating distinct patterns.

Conclusion: While quantum annealing sampling doesn't benefit RBM training due to differences occurring in less important distribution regions, it shows promise for other machine learning applications like catastrophic forgetting mitigation, where the ability to generate diverse patterns from lower-probability distribution parts is valuable. The speed advantages and potential for further improvement make this approach promising for challenging ML tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing distributed training methods for LLMs still face communication bottlenecks even with reduced frequency, as they require communicating full model gradients. Current quantization approaches have limited effectiveness and cannot leverage sparsification in LLM pre-training.

Method: SparseLoCo uses Top-k sparsification and quantization to compress gradients, with key innovations including local approximation of outer momentum through error feedback with aggressive sparsity, and sparse aggregation that improves model performance.

Result: The algorithm achieves extreme compression ratios of up to 1-3% sparsity and 2-bit quantization while outperforming full-precision DiLoCo. It demonstrates significant benefits in both performance and communication cost across various communication-constrained LLM training settings.

Conclusion: SparseLoCo effectively addresses communication bottlenecks in distributed LLM training by combining aggressive sparsification and quantization techniques, achieving superior compression and performance compared to existing methods.
...
TL;DR: This survey presents a unified mathematical framework connecting classical estimation theory, statistical inference, and modern machine learning, showing that AI methods share common probabilistic principles for extracting meaning from uncertain data.


<details>
  <summary>Details</summary>
Motivation: To demonstrate that fundamental techniques across time series analysis, pattern recognition, and language modeling are rooted in shared probabilistic principles for handling uncertainty in noisy data.

Method: Analyzes classical estimation theory (maximum likelihood, Bayesian inference) alongside modern techniques (deep learning, attention mechanisms) through illustrative scenarios including system identification, image classification, and language generation.

Result: Shows that maximum likelihood estimation, MAP estimation, Bayesian classification, and deep learning all represent different approaches to the same goal: inferring hidden causes from noisy observations.

Conclusion: Provides both a theoretical synthesis and practical guide demonstrating the unified probabilistic foundations underlying diverse AI methods, serving as a roadmap for navigating machine learning's evolving landscape.
...
TL;DR: This paper shows that likelihood-based OOD detection is theoretically optimal but often fails in practice. The authors investigate whether this failure is due to pixel space limitations by training a Variational Diffusion Model on ResNet-18 representations instead of images.


<details>
  <summary>Details</summary>
Motivation: Safety is critical for deploying ML systems, and OOD detection is a key safety component. While likelihood is theoretically optimal for OOD detection, it often fails in practice, raising questions about whether the issue lies with pixel space representations.
...
Result: The paper investigates whether representation space can overcome the limitations of pixel space for likelihood-based OOD detection, but the abstract doesn't specify the actual experimental results.

Conclusion: The research explores if moving from pixel space to representation space can make likelihood-based OOD detection more effective in practice, addressing the gap between theoretical optimality and practical failures.
...
Method: 28B parameter multimodal Mixture-of-Experts model with 241B total parameters, continually pre-trained on 5T tokens (2.5T+ scientific), followed by offline and online RL training using Mixture-of-Rewards (MoR) on 1000+ tasks simultaneously.
...
Conclusion: Intern-S1 successfully demonstrates that integrated innovations in algorithms, data, and training systems can create specialized generalist models that bridge the gap between open-source and closed-source performance in scientific domains.
...
TL;DR: Decentralized adversarial attack detector for multi-agent reinforcement learning using statistical characterization and CUSUM procedure


<details>
  <summary>Details</summary>
Motivation: Detect adversarial attacks in cooperative multi-agent reinforcement learning with continuous action space to protect system integrity

Method: Uses deep neural networks to model normal agent behavior as parametric multivariate Gaussian distributions, defines normality score, and employs two-sided CUSUM procedure for real-time anomaly detection
...
Conclusion: Proposed decentralized detector effectively identifies adversarial attacks using local observations and statistical characterization of normal behavior
...
TL;DR: Transformers trained for multivariate polynomial decomposition achieve competitive performance with 75% lower inference compute through novel reinforcement learning method.


<details>
  <summary>Details</summary>
Motivation: Extend transformer capabilities for non-linear latent pattern discovery in functional decomposition, focusing on NP-hard multivariate polynomial decomposition which has widespread scientific and engineering applications.
...
Result: BGRPO finetuning improves accuracy while reducing beam width by up to half, achieving approximately 75% lower inference compute. Model outperforms Mathematica in various polynomial simplification cases.

Conclusion: Transformers can effectively handle complex algebraic tasks like multivariate polynomial decomposition through carefully designed training approaches and reinforcement learning optimization, demonstrating competitive performance against established computational tools.
...
TL;DR: Semantic normalization framework for hierarchical narrative graphs that reduces annotation inconsistency and redundancy using lexical similarity and embedding-based clustering, improving coherence and robustness in narrative reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Visual narratives like comics require structured representations but suffer from inconsistency and redundancy in symbolic narrative graphs, where similar actions/events are labeled differently across annotations, limiting reasoning effectiveness.

Method: Proposes semantic normalization using lexical similarity and embedding-based clustering to consolidate semantically related actions and events, reducing annotation noise and aligning symbolic categories across narrative levels while preserving interpretability.

Result: Applied to Manga109 dataset, normalization improved performance in narrative reasoning tasks (action retrieval, character grounding, event summarization) by enhancing coherence and robustness while maintaining symbolic transparency.

Conclusion: Semantic normalization is a key step toward scalable, cognitively inspired graph models for multimodal narrative understanding, addressing annotation inconsistencies in visual narrative analysis.
...
TL;DR: This paper presents improvements to adder-viz software for visualizing real-time event transcoding processes and applications, building on the ADΔER representation for neuromorphic event video.


<details>
  <summary>Details</summary>
Motivation: Existing representations for event cameras have limitations in flexibility, speed, and compressibility. The ADΔER representation was previously proposed to address these issues, and this work focuses on enhancing visualization tools for real-time event transcoding.

Method: The authors developed improvements to the adder-viz software, which provides visualization capabilities for real-time event transcode processes and applications. The software is MIT-licensed and available in a centralized repository.

Result: The paper introduces enhanced visualization tools that support real-time monitoring and analysis of event transcoding processes, making the ADΔER representation more accessible and usable for researchers and developers.

Conclusion: The improved adder-viz software provides better visualization capabilities for event video processing, supporting the adoption and development of the ADΔER representation in neuromorphic computing and computer vision applications.
...
### [231] [Disentangling the Drivers of LLM Social Conformity: An Uncertainty-Moderated Dual-Process Mechanism](https://arxiv.org/abs/2508.14918)
*Huixin Zhong,Yanan Liu,Qi Cao,Shijin Wang,Zijing Ye,Zimu Wang,Shiyao Zhang*

Main category: cs.CY

TL;DR: LLMs exhibit social conformity driven primarily by informational influence (rational use of group cues) rather than normative influence (social pressure), with uncertainty dramatically modulating this behavior - causing conservative evidence weighting in low uncertainty and normative-like signal amplification in high uncertainty.


<details>
  <summary>Details</summary>
Motivation: To understand whether human psychological mechanisms of conformity (informational vs normative influence) apply to large language models, particularly how uncertainty moderates their social alignment behavior in collaborative settings.

Method: Adapted information cascade paradigm from behavioral economics to evaluate 9 leading LLMs across medical, legal, and investment decision-making scenarios while manipulating information uncertainty levels (q = 0.667, 0.55, 0.70).

Result: LLMs show informational influence across all contexts with accuracy and confidence rising with stronger evidence. Low-medium uncertainty triggers conservative evidence underweighting, while high uncertainty causes normative-like amplification where public signals are overweighted (beta > 1.55 vs private beta = 0.81).

Conclusion: LLM conformity is fundamentally informational but dramatically modulated by uncertainty, shifting from analytical to heuristic processing similar to humans, with high uncertainty triggering normative-like behavior despite the underlying informational mechanism.
...
### [232] [Designing an Interdisciplinary Artificial Intelligence Curriculum for Engineering: Evaluation and Insights from Experts](https://arxiv.org/abs/2508.14921)
*Johannes Schleiss,Anke Manukjan,Michelle Ines Bieber,Sebastian Lang,Sebastian Stober*

Main category: cs.CY

TL;DR: Study explores interdisciplinary AI curriculum development for engineering education using mixed methods, examining stakeholder perspectives and curriculum quality.
...
Conclusion: Advances AI education field by offering reference points and insights for future interdisciplinary curriculum developments to meet evolving industry needs.
...
### [233] [Enhanced Predictive Modeling for Hazardous Near-Earth Object Detection: A Comparative Analysis of Advanced Resampling Strategies and Machine Learning Algorithms in Planetary Risk Assessment](https://arxiv.org/abs/2508.15106)
*Sunkalp Chandra*
...
Result: RFC and GBC performed best with F2-scores of 0.987 and 0.986 respectively, high accuracy rates (99.7%, 99.6%), and minimal false positives/negatives. SVC scored 0.896, while LDA, LR, and KNN had lower performance.

Conclusion: Ensemble methods (RFC, GBC) are most effective for NEO hazard prediction, highlighting the importance of tailored model selection based on dataset characteristics and evaluation metrics.
...
### [234] [Accelerating GenAI Workloads by Enabling RISC-V Microkernel Support in IREE](https://arxiv.org/abs/2508.14899)
*Adeel Ahmad,Ahmad Tameem Kamal,Nouman Amir,Bilal Zafar,Saad Bin Nasir*

Main category: cs.AR

TL;DR: Enabling RISC-V microkernel support in IREE compiler with optimized mmt4d operations and performance comparison against upstream IREE and Llama.cpp.


<details>
  <summary>Details</summary>
Motivation: To extend IREE's capabilities to support RISC-V architecture with optimized microkernels for better machine learning performance on RISC-V platforms.

Method: Lower MLIR linalg dialect contraction operations to linalg.mmt4d operations for RISC-V64 target and develop optimized microkernels specifically for RISC-V architecture.
...
Conclusion: Successfully implemented RISC-V microkernel support in IREE with measurable performance improvements, demonstrating the effectiveness of architecture-specific optimizations for machine learning workloads.
...
### [235] [Scalable FPGA Framework for Real-Time Denoising in High-Throughput Imaging: A DRAM-Optimized Pipeline using High-Level Synthesis](https://arxiv.org/abs/2508.14917)
*Weichien Liao*

Main category: cs.AR

TL;DR: FPGA-based real-time denoising pipeline for high-throughput imaging using HLS optimization and AXI4 interfaces to process data faster than acquisition rates.
...
Method: Scalable FPGA preprocessing pipeline using High-Level Synthesis (HLS) with optimized DRAM-backed buffering, performing frame subtraction and averaging via burst-mode AXI4 interfaces on streamed image data.
...
Conclusion: This modular FPGA framework provides a practical solution for latency-sensitive imaging workflows in spectroscopy and microscopy applications.
...
TL;DR: Novel row-column hybrid grouping technique and ILP-based compiler pipeline for analog IMC systems, addressing stuck-at faults and compilation overhead with significant improvements in accuracy, speed, and energy efficiency.
...
Method: 1) Multi-bit weight representation with row-column hybrid grouping for structural redundancy and fault tolerance. 2) ILP-based compiler pipeline for fast fault-aware weight decomposition, accelerated by identifying trivial solution patterns.

Result: Achieved up to 8% accuracy improvement, 150x faster compilation, and 2x energy efficiency gain on convolutional networks and small language models compared to existing baselines.

Conclusion: The proposed approach effectively overcomes scalability and deployability limitations in analog IMC systems through structural redundancy and efficient compilation techniques.
...
### [237] [Exploring the Landscape of Non-Equilibrium Memories with Neural Cellular Automata](https://arxiv.org/abs/2508.15726)
*Ethan Lake,Ehsan Pajouheshgar*

Main category: cond-mat.stat-mech

TL;DR: The paper reveals that 2D many-body memories are much more diverse than previously thought, with numerous distinct error-correcting dynamics beyond Toom's rule that preserve information for long times despite perturbations.


<details>
  <summary>Details</summary>
Motivation: To explore the full landscape of local non-equilibrium dynamics that can retain initial condition information for thermodynamically long time scales in the presence of arbitrary perturbations, moving beyond the single well-studied example of Toom's rule.

Method: Combination of rigorous mathematical proofs and machine learning methods to discover and analyze various 2D memory systems, with interactive visualizations available online.

Result: Discovered a vast landscape of 2D memories with qualitatively distinct error correction mechanisms from Toom's rule, including systems with ordered phases stabilized by fluctuations and memories that only preserve information in the presence of noise.
...
### [238] [Flow Matching at Scale: A Machine Learning Framework for Efficient Large-Size Sampling of Many-Body Systems](https://arxiv.org/abs/2508.15318)
*Qian-Rui Lee,Daw-Wei Wang*

Main category: cond-mat.stat-mech

TL;DR: Flow Matching framework enables training on small lattice systems and generating reliable samples for much larger systems across continuous temperature ranges without retraining, demonstrating strong generalization capabilities.


<details>
  <summary>Details</summary>
Motivation: To overcome scaling limitations of traditional MCMC methods by developing a machine learning approach that can efficiently study critical phenomena and explore thermodynamic limits.

Method: Uses Flow Matching framework with temperature-conditioned mapping and CNN architecture to learn scale-invariant local physical rules, trained on small 32x32 lattice configurations at sparse temperature points.

Result: Successfully generates reliable samples for 128x128 systems across continuous temperature range, showing strong agreement with thermodynamic observables and correctly capturing BKT transition signatures.

Conclusion: Establishes 'train-small, generate-large' paradigm for efficient study of critical phenomena, with direct applicability to other classical or quantum many-body systems with continuous lattice fields.
...
### [239] [AmbiSQL: Interactive Ambiguity Detection and Resolution for Text-to-SQL](https://arxiv.org/abs/2508.15276)
*Zhongjun Ding,Yin Lin,Tianjing Zeng*

Main category: cs.DB

TL;DR: AmbiSQL is an interactive system that detects query ambiguities in Text-to-SQL systems and guides users through multiple-choice questions to clarify intent, improving SQL accuracy by 50%.
...
Method: Introduces a fine-grained ambiguity taxonomy to identify ambiguities affecting database element mapping and LLM reasoning, then incorporates user feedback through intuitive multiple-choice questions to rewrite ambiguous questions.
...
Conclusion: AmbiSQL demonstrates significant performance gains and practical usability for handling query ambiguity in Text-to-SQL systems.
...
### [240] [HEAS: Hierarchical Evolutionary Agent Simulation Framework for Cross-Scale Modeling and Multi-Objective Search](https://arxiv.org/abs/2508.15555)
...
<details>
  <summary>Details</summary>
Motivation: To create a unified framework that bridges agent-based modeling, evolutionary optimization, and systematic evaluation to enable cross-disciplinary, multi-level research with reproducible results.

Method: Uses hierarchical lightweight processes ("streams") scheduled in deterministic layers with shared context, featuring evolutionary optimization, PyTorch integration, tournament evaluation tools, and standardized metrics collection.

Result: A practical framework that reduces glue code, improves comparability across studies, and enables composition of exogenous drivers, endogenous agents, and aggregators without refactoring.

Conclusion: HEAS provides a foundation for reliable, reproducible cross-disciplinary research through its unified approach to simulation, optimization, and systematic comparison of multi-level models.
...
### [241] [A Systematic Survey of Model Extraction Attacks and Defenses: State-of-the-Art and Perspectives](https://arxiv.org/abs/2508.15031)
*Kaixiang Zhao,Lincan Li,Kaize Ding,Neil Zhenqiang Gong,Yue Zhao,Yushun Dong*

Main category: cs.CR

TL;DR: A comprehensive survey of Model Extraction Attacks (MEAs) that exploit MLaaS platforms, covering attack techniques, defense strategies, and proposing a novel taxonomy to classify MEAs by attack mechanisms, defense approaches, and computing environments.


<details>
  <summary>Details</summary>
Motivation: The widespread adoption of Machine-Learning-as-a-Service (MLaaS) platforms has made advanced ML models accessible but also vulnerable to Model Extraction Attacks, threatening intellectual property, privacy, and system security.

Method: The paper conducts a systematic survey of MEAs, proposing a novel taxonomy to classify attacks and defenses. It analyzes various attack techniques, evaluates defense effectiveness, and assesses MEAs across different computing paradigms.

Result: The survey provides comprehensive coverage of MEAs, highlighting the critical trade-off between model utility and security in existing defenses. It also discusses technical, ethical, legal, and societal implications of these attacks.

Conclusion: This systematic survey serves as a valuable reference for AI security research, identifying promising future directions and maintaining an online repository for continuous updates on MEA literature.
...
### [242] [Retrieval-Augmented Review Generation for Poisoning Recommender Systems](https://arxiv.org/abs/2508.15252)
...
Method: Proposes RAGAN framework that uses in-context learning capabilities of multimodal foundation models, with demonstration retrieval and text style transfer strategies. Includes jailbreaker, instructional agent, and guardian components for collaborative optimization of fake profiles.

Result: Comprehensive experiments on real-world datasets show RAGAN achieves state-of-the-art poisoning attack performance with improved transferability and imperceptibility.

Conclusion: The proposed RAGAN framework successfully enhances fake profile quality using multimodal foundation models, demonstrating superior attack effectiveness while maintaining practical constraints for black-box recommender system attacks.
...
### [243] [MoEcho: Exploiting Side-Channel Attacks to Compromise User Privacy in Mixture-of-Experts LLMs](https://arxiv.org/abs/2508.15036)
*Ruyi Ding,Tianhong Xu,Xinyi Shen,Aidong Adam Ding,Yunsi Fei*

Main category: cs.CR

TL;DR: MoEcho reveals security vulnerabilities in Mixture of Experts (MoE) transformer architectures, demonstrating how adaptive routing creates side channels that can be exploited to infer sensitive user data through cache and performance monitoring attacks.


<details>
  <summary>Details</summary>
Motivation: As MoE architectures scale for efficiency in transformers, their input-dependent activation patterns create new privacy attack surfaces through hardware execution traces that adversaries could exploit to deduce sensitive information.

Method: Proposes four novel architectural side channels across different platforms: Cache Occupancy Channels and Pageout+Reload on CPUs, and Performance Counter and TLB Evict+Reload on GPUs, then develops four corresponding privacy attacks on LLMs and VLMs.

Result: Successfully demonstrates Prompt Inference Attack, Response Reconstruction Attack, Visual Inference Attack, and Visual Reconstruction Attack that effectively breach user privacy in MoE-based models.

Conclusion: MoEcho represents the first comprehensive security analysis of MoE architectures, revealing serious privacy threats that necessitate immediate safeguards when deploying MoE-based AI services.
...
### [244] [IPIGuard: A Novel Tool Dependency Graph-Based Defense Against Indirect Prompt Injection in LLM Agents](https://arxiv.org/abs/2508.15310)
*Hengyu An,Jinghuai Zhang,Tianyu Du,Chunyi Zhou,Qingming Li,Tao Lin,Shouling Ji*

Main category: cs.CR

TL;DR: IPIGuard is a defense mechanism against Indirect Prompt Injection attacks on LLM agents by modeling task execution as Tool Dependency Graph traversal, decoupling planning from external data interaction.
...
Conclusion: IPIGuard provides a structural defense approach that prevents malicious tool invocations at the source, paving the way for safer agentic systems in dynamic environments.
...
### [245] [MCPTox: A Benchmark for Tool Poisoning Attack on Real-World MCP Servers](https://arxiv.org/abs/2508.14925)
...
<details>
  <summary>Details</summary>
Motivation: The Model Context Protocol (MCP) creates novel attack surfaces through untrusted external tools, but prior work focused only on output-based attacks. This paper investigates the more fundamental vulnerability of Tool Poisoning where malicious instructions are embedded in tool metadata without execution.

Method: Developed MCPTox benchmark using 45 real-world MCP servers and 353 authentic tools. Created 1312 malicious test cases through three attack templates using few-shot learning, covering 10 risk categories. Evaluated 20 prominent LLM agents.

Result: Widespread vulnerability found with o1-mini achieving 72.8% attack success rate. More capable models are more susceptible due to their superior instruction-following abilities. Agents rarely refuse attacks (highest refusal rate <3%), showing existing safety alignment is ineffective.

Conclusion: Tool Poisoning represents a significant threat to MCP-based agent systems. The findings establish an empirical baseline for understanding and mitigating this threat, and MCPTox is released to support development of verifiably safer AI agents.
...
### [246] [Adaptive Anomaly Detection in Evolving Network Environments](https://arxiv.org/abs/2508.15100)
*Ehssan Mousavipour,Andrey Dimanchev,Majid Ghaderi*

Main category: cs.CR

TL;DR: NetSight is a supervised anomaly detection framework that automatically adapts to distribution shifts in network data using pseudo-labeling and knowledge distillation, eliminating manual intervention and achieving up to 11.72% F1-score improvement.


<details>
  <summary>Details</summary>
Motivation: Distribution shifts in data over time challenge deep learning anomaly detection systems. Supervised methods require costly manual labeling, while unsupervised methods need clean data for adaptation - both difficult to obtain in practice.

Method: NetSight uses a novel pseudo-labeling technique to eliminate manual intervention and employs knowledge distillation-based adaptation strategy to prevent catastrophic forgetting, enabling continuous online adaptation to distribution shifts.

Result: Evaluated on three long-term network datasets, NetSight demonstrates superior adaptation performance compared to state-of-the-art methods that rely on manual labeling, achieving F1-score improvements of up to 11.72%.

Conclusion: NetSight proves robust and effective for dynamic networks experiencing distribution shifts over time, providing automated adaptation without manual intervention while maintaining performance.
...
### [247] [BadFU: Backdoor Federated Learning through Adversarial Machine Unlearning](https://arxiv.org/abs/2508.15541)
*Bingguang Lu,Hongsheng Hu,Yuantian Miao,Shaleeza Sohail,Chaoxiang He,Shuo Wang,Xiao Chen*

Main category: cs.CR

TL;DR: First backdoor attack (BadFU) in federated unlearning where malicious clients inject backdoors through legitimate unlearning requests using camouflage samples


<details>
  <summary>Details</summary>
Motivation: Growing concerns over data privacy and regulatory compliance require machine unlearning in FL, but this introduces unexplored security risks where adversaries can compromise model integrity

Method: BadFU attack strategy: malicious client trains global model with backdoor and camouflage samples during FL, then requests unlearning of camouflage samples to trigger backdoor state

Result: Extensive experiments show BadFU is effective across various FL frameworks and unlearning strategies, revealing critical vulnerability in current federated unlearning practices

Conclusion: Urgent need for more secure and robust federated unlearning mechanisms to prevent backdoor attacks through unlearning requests
...
### [248] [High-dimensional Asymptotics of Generalization Performance in Continual Ridge Regression](https://arxiv.org/abs/2508.15494)
*Yihan Zhao,Wenqing Su,Ying Yang*

Main category: stat.ME

TL;DR: Theoretical analysis of continual ridge regression in high-dimensional linear models using random matrix theory to derive exact asymptotic prediction risk expressions and characterize generalization metrics.
...
Method: Uses random matrix theory to analyze continual ridge regression in high-dimensional linear models where dimension is proportional to sample size, deriving exact asymptotic expressions for prediction risk.

Result: Obtained exact expressions for asymptotic prediction risk enabling characterization of three generalization metrics: average risk, backward transfer, and forward transfer, with theoretical risk curves showing trends throughout continual learning process.

Conclusion: Analysis reveals intriguing phenomena in risk curves demonstrating how model specifications influence generalization performance, with simulation studies validating theoretical findings.
...
### [249] [Effect Identification and Unit Categorization in the Multi-Score Regression Discontinuity Design with Application to LED Manufacturing](https://arxiv.org/abs/2508.15692)
*Philipp Alexander Schwarz,Oliver Schacht,Sven Klaassen,Johannes Oberpriller,Martin Spindler*

Main category: stat.ME

TL;DR: Extends regression discontinuity design to multi-dimensional cutoff rules, providing theoretical framework to identify and reduce fuzziness in causal effect estimation for compliers.
...
Method: Develops theoretical tools for multi-dimensional cutoff rules, categorizes unit behavior types, identifies conditions for cutoff-effect identification on compliers, and investigates rule decomposition to remove non-compliant units.

Result: Framework validated on simulated and real-world semiconductor manufacturing data, showing decreased estimation variance and practical value for refining production policies.

Conclusion: The proposed MRD framework provides sound theoretical foundation and practical tools for handling multi-dimensional cutoff rules, improving causal effect estimation in complex decision-making systems.
...
TL;DR: Proposes TOM, a tongue image segmentation model using multi-teacher knowledge distillation with diffusion-based data augmentation, achieving 95.22% mIoU with 96.6% parameter reduction, and deployed as first open-source tool for TCM diagnosis.


<details>
  <summary>Details</summary>
Motivation: Tongue imaging is crucial for Traditional Chinese Medicine diagnosis, but existing segmentation research has limitations and lacks robust, user-friendly tools for practitioners without programming experience.
...
Conclusion: Successfully developed the first open-source tongue image segmentation tool that maintains high accuracy while being accessible to TCM practitioners without technical expertise, demonstrating practical value for intelligent tongue diagnosis systems.
...
TL;DR: Survey of event-based video systems, discussion of scalable event streaming challenges, and proposal of new low-latency event streaming format using Media Over QUIC protocol.


<details>
  <summary>Details</summary>
Motivation: Traditional lossy compression and rate-adaptive streaming work for frame-based video, but neuromorphic event sensors produce asynchronous pixel samples that require different transmission approaches. While researchers have focused on application development, data transmission for event-based video has been neglected.

Method: The paper surveys existing event-based video systems, analyzes technical issues with scalable event streaming, and proposes a new streaming format based on the latest Media Over QUIC protocol draft for low-latency requirements.

Result: The work identifies the gap in event-based video transmission and provides a technical foundation for addressing streaming challenges specific to asynchronous pixel sampling sensors.

Conclusion: A new streaming approach is needed for event-based video sensors, and the proposed Media Over QUIC-based format offers a promising solution for low-latency event streaming to support computer vision applications.
...
<details>
  <summary>Details</summary>
Motivation: Overcome limitations of supervised CT super-resolution methods that require large paired datasets, and zero-shot methods that struggle with fine anatomical details due to limited internal information.

Method: Leverage upsampled 2D X-ray projection priors from a diffusion model trained on abundant HR 2D X-ray data, use per-projection adaptive sampling, and employ 3D Gaussian splatting with negative alpha blending (NAB-GS) for residual learning between LR and diffusion-based projections.

Result: Superior quantitative and qualitative results for 3D CT super-resolution on two datasets, demonstrating effective recovery of fine anatomical details.

Conclusion: The proposed framework successfully addresses the limitations of existing methods by combining diffusion-based external priors with advanced 3D reconstruction techniques, enabling high-quality CT super-resolution without requiring paired training data.
...
TL;DR: Vision-language guided diffusion model for unsupervised anomaly detection in digital pathology using histopathology prompts to differentiate normal and abnormal tissues.
...
Result: Effective performance on gastric lymph node dataset and demonstrates generalization ability under domain shift on breast lymph node dataset.

Conclusion: Proposed method shows potential for unsupervised anomaly detection across various organs in digital pathology, offering viable alternative to supervised approaches.
...
TL;DR: Knowledge distillation framework using high-capacity teacher models (VGG19, Visformer-S, AutoFormer-V2-T) to train compact student model for COVID-19 and lung cancer classification from chest X-rays, achieving high accuracy with reduced computational cost.
...
Method: Employed knowledge distillation with hybrid supervision (ground-truth labels + teacher soft targets) using high-capacity teacher models to guide training of compact OFA-595 student model. Used Score-CAM visualizations for model interpretability.

Result: Distilled student model maintained high classification performance with significantly reduced parameters and inference time on COVID-QU-Ex and LCS25000 datasets covering multiple disease classes.

Conclusion: Combining model efficiency with explainability through knowledge distillation creates practical, trustworthy medical AI solutions suitable for clinical deployment in resource-constrained settings.
...
<details>
  <summary>Details</summary>
Motivation: Current bladder cancer diagnosis through cystoscopy relies heavily on physician expertise, leading to variability and subjectivity. There's a need for objective, accurate computational approaches to improve diagnostic outcomes.
...
<details>
  <summary>Details</summary>
Motivation: Deep learning models for breast cancer recognition suffer performance degradation when applied to data from different domains due to domain shift, limiting safe AI deployment in clinical settings.

Method: Proposes DoSReMC framework that fine-tunes only batch normalization and fully connected layers while keeping pretrained convolutional filters intact, combined with adversarial training for improved cross-domain generalization.

Result: Demonstrates that BN layers are a primary source of domain dependence and that targeted adaptation significantly improves model generalization across different mammography datasets.

Conclusion: DoSReMC provides a practical solution for robust cross-domain mammography classification that can be easily integrated into existing AI pipelines for diverse clinical environments.
...
TL;DR: A Deep Equilibrium Convolutional Sparse Coding framework for hyperspectral image denoising that combines local spatial-spectral correlations, nonlocal self-similarities, and global spatial consistency through infinite-depth network modeling.
...
Method: Proposes DECSC framework using Deep Equilibrium models to treat hidden layers as fixed-point solutions. Combines 2D convolutional sparse coding for global spatial consistency, 3D convolutional sparse coding for local details, transformer blocks for nonlocal self-similarities, and detail enhancement modules.

Result: Experimental results show superior denoising performance compared to state-of-the-art methods.

Conclusion: The DECSC framework effectively addresses HSI denoising by unifying multiple spatial-spectral correlations through infinite-depth DEQ modeling with convergence guarantees.
...
TL;DR: This study explores using image-to-image translation to generate virtual dual-energy subtracted (DES) mammography images from low-energy images, potentially reducing radiation exposure. Three models were tested, with pre-trained U-Net achieving best results (85.59% F1 score vs 90.35% with real DES).


<details>
  <summary>Details</summary>
Motivation: To reduce patient radiation exposure in contrast-enhanced spectral mammography by generating artificial DES images from LE images instead of acquiring high-energy images, while maintaining diagnostic utility.
...
TL;DR: A novel approach using radiologist confidence values in lung ultrasound annotations improves AI segmentation and downstream clinical task performance, with 60% confidence threshold working best.


<details>
  <summary>Details</summary>
Motivation: Address inter-observer variability and label uncertainty in medical imaging, particularly lung ultrasound where subjective interpretation leads to inconsistent annotations even among experts.

Method: Developed a data annotation protocol capturing per-pixel confidence values from radiologists. Tested various confidence thresholds for binarizing labels during model training, comparing segmentation performance and downstream clinical outcomes.

Result: Incorporating confidence values improved segmentation performance. Training with 60% confidence threshold worked best, significantly outperforming naive 50% threshold. Enhanced segmentation translated to better performance on clinical tasks: S/F oxygenation ratio estimation, S/F ratio change classification, and 30-day readmission prediction.

Conclusion: Label confidence is a valuable signal that can significantly enhance AI reliability and clinical utility in medical imaging when properly leveraged through appropriate confidence thresholds during training.
...
TL;DR: HessNet: A lightweight semi-supervised neural network with only 6000 parameters that uses Hessian matrices for accurate 3D brain vessel segmentation in MRA images, enabling creation of a large annotated dataset.
...
Method: Proposed HessNet - a Hessian-based lightweight neural network with only 6000 parameters that can run on CPU. Uses semi-supervised learning for 3D segmentation of tubular structures. Applied to create large annotated dataset from IXI dataset.

Result: Achieves state-of-the-art vessel segmentation accuracy on minimal training dataset. Created a semi-manually annotated brain vessel dataset of 200 MRA images with expert supervision. Significantly reduces resource requirements for training.

Conclusion: HessNet provides high accuracy vessel segmentation while being computationally efficient. Enables experts to focus on complex cases and facilitates creation of large annotated datasets for medical image analysis research.
...
TL;DR: GAN-based super-resolution improves near-wall velocity recovery in 4D Flow MRI, with Wasserstein GAN showing optimal stability and performance compared to non-adversarial methods.


<details>
  <summary>Details</summary>
Motivation: 4D Flow MRI has limited clinical application due to low spatial resolution and noise, particularly affecting near-wall velocity measurements. GANs show promise for restoring sharp boundaries in super-resolution tasks but haven't been explored for 4D Flow MRI.

Method: Used patient-specific cerebrovascular in-silico models converted to synthetic images via MR-true reconstruction. Implemented dedicated GAN architecture and evaluated three adversarial loss functions: Vanilla, Relativistic, and Wasserstein GANs.

Result: Wasserstein GAN achieved best performance with vNRMSE of 6.9% vs 9.6% for non-adversarial reference. Outperformed generator-only baseline at low SNR (8.7% vs 10.7%). Vanilla and Relativistic GANs were unstable.

Conclusion: GAN-based super-resolution enhances 4D Flow MRI, particularly in cerebrovascular regions, but requires careful selection of adversarial strategies with Wasserstein GAN showing optimal stability and performance.
...
TL;DR: Dynamic uncertainty-aware MRI acquisition framework that adjusts scan time per subject using probabilistic reconstruction and conformal prediction to provide calibrated confidence intervals, reducing scan times while ensuring precision.


<details>
  <summary>Details</summary>
Motivation: MRI has long acquisition times, and current deep learning accelerated methods use fixed acceleration factors that result in either unnecessarily long scans or insufficient quality, ignoring reconstruction uncertainty.

Method: Uses probabilistic reconstruction model to estimate image uncertainty, propagates through analysis pipeline to quantitative metrics, applies conformal prediction for calibrated confidence intervals, and iteratively samples k-space until meeting precision targets.

Result: Validated on knee and cardiac MRI datasets, reduces scan times compared to fixed protocols while providing formal statistical guarantees on final image precision.

Conclusion: Enables patient-specific acquisitions that balance scan efficiency with diagnostic confidence, moving towards personalized and resource-efficient MRI.
...
TL;DR: A decentralized multi-drone system using single RGB cameras for scalable wildlife monitoring and tracking in natural habitats without centralized control.


<details>
  <summary>Details</summary>
Motivation: Need for efficient parallel deployment methods to identify and interact with specific wildlife individuals for behavioral analysis and interventions, overcoming limitations of previous herd-focused or manually-operated robotics solutions.

Method: Developed novel vision-based coordination and tracking algorithms for dynamic unstructured environments using a decentralized multi-quadrotor system with single onboard RGB cameras, operating without centralized communication or control.

Result: Validated through real-world experiments demonstrating reliable deployment in diverse field conditions, enabling robust identification and tracking of large species.

Conclusion: The proposed system provides a scalable, low-bandwidth, sensor-minimal solution for wildlife monitoring that works effectively in natural habitats without requiring complex infrastructure.
...
### [264] [A Vision-Based Shared-Control Teleoperation Scheme for Controlling the Robotic Arm of a Four-Legged Robot](https://arxiv.org/abs/2508.14994)
...
TL;DR: Vision-based teleoperation system that maps human arm movements to control quadruped robot manipulators with collision avoidance


<details>
  <summary>Details</summary>
Motivation: Teleoperating quadruped robots with manipulator arms is challenging due to non-intuitive controls, high cognitive load, and collision risks in confined/dynamic environments

Method: Uses vision-based pose estimation with external camera and ML model to detect operator's wrist position, maps movements to robotic arm commands with trajectory planner for collision prevention

Result: System validated on real robot with robust real-time performance, providing cost-effective solution for industrial applications

Conclusion: Provides intuitive, safe, and precise robotic control in high-risk environments through direct human-to-robot movement mapping with collision avoidance
...
### [265] [Lang2Lift: A Framework for Language-Guided Pallet Detection and Pose Estimation Integrated in Autonomous Outdoor Forklift Operation](https://arxiv.org/abs/2508.15427)
*Huy Hoang Nguyen,Johannes Huemer,Markus Murschitz,Tobias Glueck,Minh Nhat Vu,Andreas Kugi*

Main category: cs.RO

TL;DR: Lang2Lift is a framework that uses natural language commands and foundation models to automate pallet detection and 6D pose estimation for autonomous forklift operations in challenging outdoor logistics environments.


<details>
  <summary>Details</summary>
Motivation: Address labor shortages, safety concerns, and inefficiencies in manual pallet handling operations, particularly in outdoor environments with variable payloads, inconsistent pallet quality, and unstructured surroundings.

Method: Integrates Florence-2 and SAM-2 for language-grounded segmentation with FoundationPose for robust 6D pose estimation, enabling natural language commands to specify targets. The perception pipeline feeds into motion planning for fully autonomous forklift operation.
...
Conclusion: Lang2Lift provides a viable solution for automating pallet pick-up operations using natural language guidance and foundation models, showing promise for addressing critical challenges in logistics and construction automation.
...
### [266] [Survey of Vision-Language-Action Models for Embodied Manipulation](https://arxiv.org/abs/2508.15201)
*Haoran Li,Yuhui Chen,Wenbo Cui,Weiheng Liu,Kai Liu,Mingcai Zhou,Zhengtao Zhang,Dongbin Zhao*

Main category: cs.RO

TL;DR: Survey paper reviewing Vision-Language-Action (VLA) models for embodied manipulation, covering development trajectory, model structures, datasets, training methods, evaluation, and future directions.


<details>
  <summary>Details</summary>
Motivation: Embodied intelligence systems that enhance agent capabilities through environment interactions have gained significant attention, with VLA models serving as universal robotic control frameworks that improve agent-environment interaction capabilities.

Method: Comprehensive review of VLA models including: developmental trajectory analysis, detailed examination across 5 dimensions (model structures, training datasets, pre-training methods, post-training methods, model evaluation), and synthesis of challenges and future directions.

Result: The survey provides a systematic analysis of current VLA research landscape, identifying key architectural developments, training methodologies, and evaluation approaches for embodied manipulation tasks.

Conclusion: The paper synthesizes critical challenges in VLA development and real-world deployment while outlining promising future research directions to advance embodied AI robotics through improved vision-language-action integration.
...
### [267] [LLM-Driven Self-Refinement for Embodied Drone Task Planning](https://arxiv.org/abs/2508.15501)
*Deyu Zhang,Xicheng Zhang,Jiahao Li,Tingting Long,Xunhua Dai,Yongjian Fu,Jinrui Zhang,Ju Ren,Yaoxue Zhang*

Main category: cs.RO

TL;DR: SRDrone is a novel system for self-refinement task planning in industrial drones that improves success rate by 44.87% over baselines through continuous state evaluation and hierarchical behavior tree modification.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of conventional single-frame final-state assessment methods and enable robust task planning for continuous, dynamic drone operations by integrating LLM reasoning with physical execution constraints.

Method: Uses continuous state evaluation methodology for accurate task outcome determination and explanatory feedback, combined with hierarchical Behavior Tree modification model with multi-level plan analysis and constrained strategy space for structured reflective learning.
...
Conclusion: SRDrone successfully integrates LLM general reasoning intelligence with physical execution constraints of embodied drones through adaptive task refinement within industrial-grade BT planning framework.
...
### [268] [Mind and Motion Aligned: A Joint Evaluation IsaacSim Benchmark for Task Planning and Low-Level Policies in Mobile Manipulation](https://arxiv.org/abs/2508.15663)
...
TL;DR: Kitchen-R is a new benchmark that bridges the gap between high-level task planning and low-level robot control evaluation in a simulated kitchen environment with 500+ complex language instructions.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks either focus on high-level language instruction following (assuming perfect execution) or low-level control (using simple commands), creating a disconnect that prevents comprehensive evaluation of integrated robotic systems.

Method: Built as a digital twin using Isaac Sim simulator, featuring a mobile manipulator robot. Provides baseline methods including vision-language model for task planning and diffusion policy for low-level control, with three evaluation modes: planning module assessment, control policy assessment, and integrated system evaluation.

Result: Kitchen-R offers a unified framework for evaluating both task planning and physical execution capabilities, enabling more holistic benchmarking of language-guided robotic agents.

Conclusion: Kitchen-R successfully bridges a key gap in embodied AI research by providing a comprehensive benchmark that supports realistic evaluation of integrated robotic systems where both planning and execution are critical.
...
### [269] [Neural Robot Dynamics](https://arxiv.org/abs/2508.15755)
*Jie Xu,Eric Heiden,Iretiayo Akinola,Dieter Fox,Miles Macklin,Yashraj Narang*

Main category: cs.RO

TL;DR: NeRD is a neural robot dynamics model that replaces traditional physics solvers in simulators, using robot-centric representations to enable accurate, generalizable simulation across tasks and environments while allowing real-world fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Traditional robot simulators struggle with high-DOF complex robots, while existing neural simulators lack generalization due to poor global state representations and require task-specific training.

Method: Proposes NeRD - learned robot-specific dynamics models that replace low-level dynamics and contact solvers in analytical simulators, using robot-centric and spatially-invariant state representations integrated as backend solvers.

Result: NeRD simulators are stable and accurate over 1000+ steps, generalize across tasks/environments, enable policy learning in neural engine, and can be fine-tuned from real-world data to bridge sim-to-real gap.

Conclusion: NeRD provides a generalizable neural simulation framework that outperforms classical simulators in flexibility and real-world adaptability while maintaining simulation accuracy and stability.
...
### [270] [Exploiting Policy Idling for Dexterous Manipulation](https://arxiv.org/abs/2508.15669)
...
TL;DR: Pause-Induced Perturbations (PIP) method detects policy idling states and applies perturbations to help robots escape problematic basins of attraction, improving dexterous manipulation performance without additional training.


<details>
  <summary>Details</summary>
Motivation: Learned policies for dexterous manipulation often idle in certain states due to training data limitations, particularly in high-precision motion areas like grasping or insertion tasks. Prior mitigation approaches negatively impact policy performance.

Method: PIP detects idling behavior and applies perturbations at those states to help the policy escape problematic basins of attraction. This is done without additional supervision or training.

Result: The approach shows noticeable improvement on simulated dual-arm tasks and achieves 15-35% absolute success rate improvement on real-world insertion tasks requiring complex multi-finger manipulation.

Conclusion: Leveraging detectable idling behavior to inform exploration through targeted perturbations is an effective strategy for improving policy robustness and performance in dexterous manipulation tasks.
...
### [271] [SVM/SVR Kernels as Quantum Propagators](https://arxiv.org/abs/2502.11153)
...
<details>
  <summary>Details</summary>
Motivation: To explore the largely unexplored connection between Support Vector Machine kernel functions and quantum propagators represented by time-dependent Green's functions, and leverage this relationship to improve SVM performance in physical systems.
...
Result: Many common SVM kernels naturally correspond to Green's functions via operator inversion theory. Numerical experiments confirmed that using positive-semidefinite kernels corresponding to Green's functions significantly improves SVM predictive accuracy in physical systems.

Conclusion: The mathematical equivalence between SVM kernels and quantum Green's functions provides a powerful framework for designing optimized kernels, particularly for physical systems, with the Kernel Polynomial Method enabling customized kernel development that enhances SVM performance.
...
### [272] [Robust and Efficient Quantum Reservoir Computing with Discrete Time Crystal](https://arxiv.org/abs/2508.15230)
...
Method: Quantum reservoir computing algorithm harnessing discrete time crystal dynamics as reservoir, calibrated for memory, nonlinear, and information scrambling capacities. Applied to binary and ten-class classification tasks with comparative quantum kernel analysis.

Result: Experimental results on superconducting quantum processors match ideal simulations, showing enhanced accuracy with increasing system size and confirming topological noise robustness. First experimental demonstration of quantum reservoir computing for image classification based on digital quantum simulation.

Conclusion: Establishes correlation between quantum many-body non-equilibrium phase transitions and quantum machine learning performance, providing new design principles for quantum reservoir computing and broader QML algorithms.
...
### [273] [Way to Build Native AI-driven 6G Air Interface: Principles, Roadmap, and Outlook](https://arxiv.org/abs/2508.15277)
...
TL;DR: Proposes a native AI-driven air interface architecture for 6G networks with compression and adaptation capabilities for semantic communication across diverse conditions.
...
Method: Architecture built around two core characteristics: compression (extracting essential semantic information) and adaptation (dynamic transmission across diverse tasks, data types, and channel conditions). Includes case study on semantic communication in 6G non-terrestrial networks.

Result: Proposes a scalable and robust air interface architecture that focuses on task relevance rather than symbol-level accuracy, enabling efficient semantic communication.

Conclusion: Outlines key challenges and research opportunities for native AI in 6G, presenting a forward-looking framework for AI-driven communication systems.
...
### [274] [Integrated Sensing, Communication, and Computation for Over-the-Air Federated Edge Learning](https://arxiv.org/abs/2508.15185)
*Dingzhu Wen,Sijing Xie,Xiaowen Cao,Yuanhao Cui,Jie Xu,Yuanming Shi,Shuguang Cui*

Main category: cs.IT

TL;DR: This paper analyzes an integrated sensing-communication-computation (ISCC) system for federated edge learning using over-the-air computation, showing how these components compete for resources and affect convergence, then proposes an algorithm to optimize batch size and resource allocation.


<details>
  <summary>Details</summary>
Motivation: To address the convergence challenges in federated edge learning systems where wireless sensing noise during data acquisition and AirComp distortions during model aggregation jointly impact learning performance, requiring coordinated optimization of sensing, communication, and computation resources.

Method: The authors analyze convergence behavior considering sensing noise and AirComp distortions, then design an ISCC parameter optimization framework that maximizes loss function degradation under latency/energy constraints, developing a low-complexity algorithm that alternately optimizes batch size control and network resource allocation.

Result: Theoretical analysis shows sensing, communication, and computation compete for resources to determine convergence rate. The proposed algorithm coordinates batch size and resource allocation, revealing that less sensing power should be used with larger data batches, and optimal computation speed is the minimum satisfying latency constraints.

Conclusion: Numerical results on human motion recognition validate the convergence analysis and demonstrate that the ISCC algorithm effectively coordinates sensing, communication, and computation processes to enhance federated learning performance through optimal batch size control and resource allocation.
...
### [275] [End-to-End Analysis of Charge Stability Diagrams with Transformers](https://arxiv.org/abs/2508.15710)
...
<details>
  <summary>Details</summary>
Motivation: To address the scalability challenges in spin-based quantum computing by automating the analysis of charge stability diagrams, which is crucial for virtual gate calibration, charge state initialization, and device control.

Method: Applied object detection transformers to analyze charge stability diagrams in semiconductor quantum dot arrays, specifically identifying triple points and their connectivity using an end-to-end learning framework.

Result: The transformer model surpassed convolutional neural networks in performance across three different spin qubit architectures without requiring retraining, while significantly reducing complexity and runtime.

Conclusion: Transformer-based end-to-end learning frameworks show strong potential as a scalable, device-agnostic foundation for quantum dot device control and tuning, offering enhanced generalizability over existing approaches.
...
### [276] [Kernel-based Equalized Odds: A Quantification of Accuracy-Fairness Trade-off in Fair Representation Learning](https://arxiv.org/abs/2508.15084)
...
### [277] [Bayesian Inference and Learning in Nonlinear Dynamical Systems: A Framework for Incorporating Explicit and Implicit Prior Knowledge](https://arxiv.org/abs/2508.15345)
*Björn Volkmann,Jan-Hendrik Ewering,Michael Meindl,Simon F. G. Ehlers,Thomas Seel*

Main category: stat.ML

TL;DR: A general-purpose system identification tool that combines known dynamics with learned approximations of unknown system parts for both online/offline Bayesian inference, enabling fusion of diverse prior knowledge with data.
...
Method: Proposes a novel interface for combining known dynamics functions with learning-based approximations of unknown parts. Derives closed-form densities for efficient parameter marginalization without requiring user-tailored coordinate transformations or model inversions.

Result: Developed a consistent solution for both online and offline Bayesian inference and learning that can incorporate explicit and implicit prior system knowledge. Demonstrated broad applicability in three distinct case studies including experimental data.

Conclusion: The framework provides a general-purpose tool for system identification that effectively combines prior knowledge with data-driven learning, eliminating the need for laborious expert interventions in modeling nested functions within known system equations.
...
TL;DR: This paper provides the first theoretical analysis of cumulative regret upper bounds for Gaussian Process Expected Improvement (GP-EI) algorithm in Bayesian optimization, showing it achieves no-regret performance for certain incumbent choices.
...
Result: GP-EI with BPMI and BSPMI achieves no-regret performance, while GP-EI with BOI achieves either sublinear cumulative regret or fast converging noisy simple regret bounds.

Conclusion: The paper provides theoretical guidance for incumbent choice in GP-EI applications and validates findings through numerical experiments, addressing a long-standing open problem in Bayesian optimization theory.
...
### [279] [Tree-like Pairwise Interaction Networks](https://arxiv.org/abs/2508.15678)
*Ronald Richman,Salvatore Scognamiglio,Mario V. Wüthrich*

Main category: stat.ML

TL;DR: PIN is a novel neural network architecture that captures pairwise feature interactions using a tree-like structure, providing both high predictive accuracy and intrinsic interpretability for tabular data analysis.


<details>
  <summary>Details</summary>
Motivation: Modeling feature interactions in tabular data is challenging but crucial for applications like insurance pricing. Existing methods often lack interpretability or efficient computation of interaction effects.

Method: Tree-like Pairwise Interaction Network (PIN) architecture that explicitly captures pairwise feature interactions through a shared feed-forward neural network mimicking decision tree structure, enabling efficient SHAP computations.

Result: PIN outperforms both traditional and modern neural network benchmarks in predictive accuracy on the French motor insurance dataset while providing interpretable insights into feature interactions.

Conclusion: PIN successfully addresses the challenge of modeling feature interactions by combining high predictive performance with intrinsic interpretability and efficient computation of interaction effects.
...
### [280] [LLMs and Agentic AI in Insurance Decision-Making: Opportunities and Challenges For Africa](https://arxiv.org/abs/2508.15110)
...
<details>
  <summary>Details</summary>
Motivation: To explore the opportunities and challenges of AI/LLMs in insurance, particularly addressing gaps in the African insurance market and promoting locally-developed solutions.

Method: Analysis of AI trends (performance improvements, open-source access, cost reduction) and identification of key local efforts, players, and partnership opportunities in African insurance markets.

Result: Identified critical gaps in African insurance market and highlighted the need for collaborative AI strategies tailored for African contexts.

Conclusion: Calls for collaborative effort among actuaries, insurers, regulators, and tech leaders to create inclusive, sustainable, and equitable AI solutions developed by and for Africans.
...
### [281] [A U-Statistic-based random forest approach for genetic interaction study](https://arxiv.org/abs/2508.14924)
*Ming Li,Ruo-Sin Peng,Changshuai Wei,Qing Lu*

Main category: q-bio.GN

TL;DR: Proposes Forest U-Test, a U-Statistic-based random forest method for detecting gene-gene and gene-environment interactions in quantitative trait genetic association studies, showing superior performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Detecting gene-gene and gene-environment interactions remains challenging in complex trait studies due to exponentially increasing feature space and computational demands when many genetic variants and environmental factors are involved.
...
Result: Simulation studies showed Forest U-Test outperformed existing methods. Applied to Cannabis Dependence study, it detected significant joint association (p < 0.001) and findings were replicated in two independent datasets with p-values of 5.93e-19 and 4.70e-17.

Conclusion: The Forest U-Test provides an effective approach for detecting complex interactions in high-dimensional genetic association studies, demonstrating strong performance and replicability in real-world applications.
...
### [282] [AGP: A Novel Arabidopsis thaliana Genomics-Phenomics Dataset and its HyperGraph Baseline Benchmarking](https://arxiv.org/abs/2508.14934)
*Manuel Serna-Aguilera,Fiona L. Goggin,Aranyak Goswami,Alexander Bucksch,Suxing Liu,Khoa Luu*

Main category: q-bio.GN

TL;DR: The paper presents the Arabidopsis Genomics-Phenomics (AGP) Dataset, a curated multi-modal dataset linking gene expression profiles with phenotypic trait measurements in Arabidopsis thaliana to address the genome-to-phenome mapping challenge.


<details>
  <summary>Details</summary>
Motivation: Current datasets either capture only genetic information or only phenotype information, and they are not integrated, limiting machine learning models' ability to learn accurate gene-trait associations and make predictions.

Method: Created a curated multi-modal dataset (AGP) that links gene expression profiles with phenotypic trait measurements for the same Arabidopsis thaliana specimens, and benchmarked conventional regression and explanatory baselines including a biologically-informed hypergraph baseline.

Result: The AGP dataset is the first to provide multi-modal gene information and heterogeneous trait/phenotype data for the same Arabidopsis thaliana specimens, supporting tasks like phenotype prediction and interpretable graph learning.

Conclusion: The AGP dataset aims to foster research towards accurately understanding genotype-phenotype connections using integrated gene information, higher-order gene pairings, and trait data from multiple sources.
...
### [283] [Self-supervised physics-informed generative networks for phase retrieval from a single X-ray hologram](https://arxiv.org/abs/2508.15530)
...
<details>
  <summary>Details</summary>
Motivation: Conventional phase retrieval methods rely on approximations and manual parameter tuning, limiting adaptability for complex experimental conditions and diverse samples where suitable training data is hard to acquire.
...
Conclusion: The approach significantly broadens applicability of phase contrast imaging by eliminating the need for training data while enabling simultaneous retrieval of phase and absorption information.
...
### [284] [Computational Resolution of Hadamard Product Factorization for $4 \times 4$ Matrices](https://arxiv.org/abs/2508.14901)
*Igor Rivin*
...
<details>
  <summary>Details</summary>
Motivation: To resolve an open problem about whether all 4x4 full-rank matrices can be expressed as Hadamard products of two rank-2 matrices, and understand the underlying algebraic constraints.

Method: Exhaustive search over F_2, sign enumeration over Z, numerical verification over R, and machine learning analysis to discover low-dimensional structure.

Result: Found 5,304 counterexamples (26.3%) among 20,160 full-rank binary matrices. Matrix density predicts expressibility with 95.7% accuracy. Expressible matrices lie on approximately 10-dimensional variety despite naive 24-parameter count.

Conclusion: The existence of counterexamples and emergent low-dimensional structure suggests deep algebraic constraints govern Hadamard factorizability, with matrix density being a strong predictor.
...
<details>
  <summary>Details</summary>
Motivation: GenAI faces challenges in reliability and efficiency due to unpredictability, requiring a new paradigm that combines AI capabilities with software engineering best practices for robust systems.
...
Result: Proposes a conceptual framework for GenAI-native software stack and discusses technical, adoption, economic, and legal impacts, calling for further validation.

Conclusion: This work aims to inspire future research and encourage communities to implement and refine the GenAI-native framework for creating resilient, self-evolving systems.
...
TL;DR: Knowledge distillation for code understanding PLMs improves student model performance with 5% parameters achieving 98% of teacher performance, with code-specific teachers and feature-based methods working best.


<details>
  <summary>Details</summary>
Motivation: Pre-trained language models for code understanding face computational intensity and inference latency challenges in large-scale deployment, requiring efficient compression techniques.

Method: Systematic investigation of knowledge distillation using logit-based and feature-based methods across 8 student models and 2 teacher PLMs on 3 downstream tasks.

Result: KD consistently boosts performance across student sizes, code-specific PLMs work best as teachers, feature-based methods enable 98% teacher performance with 5% parameters, and architectural similarity doesn't guarantee better results.

Conclusion: Knowledge distillation is highly effective for code understanding tasks, with feature-based methods and code-specific teachers providing optimal performance-efficiency tradeoffs, opening new directions for efficient code model deployment.
...
### [287] [Generative AI models enable efficient and physically consistent sea-ice simulations](https://arxiv.org/abs/2508.14984)
...
<details>
  <summary>Details</summary>
Motivation: Traditional sea ice models are computationally expensive and limited in ensemble forecasting and climate simulations, creating a need for more efficient yet accurate prediction methods.

Method: Trained on long simulations from state-of-the-art sea-ice-ocean systems, GenSIM uses generative AI to predict sea ice evolution driven solely by atmospheric forcings, learning internal ocean dynamics implicitly.

Result: GenSIM robustly reproduces statistics from numerical models and observations, capturing both brittle short-term dynamics and long-term sea-ice decline while maintaining physical consistency.

Conclusion: The model demonstrates emergent extrapolation capabilities, learning an internal ocean emulator that enables inference of slowly evolving climate dynamics from short-term predictions, showing great potential for generalization to unseen climates and encoding hidden physics.
...
### [288] [Numerical models outperform AI weather forecasts of record-breaking extremes](https://arxiv.org/abs/2508.15724)
*Zhongwei Zhang,Erich Fischer,Jakob Zscheischler,Sebastian Engelke*

Main category: physics.ao-ph

TL;DR: AI weather models underperform traditional numerical models (HRES) in forecasting record-breaking extreme weather events, showing larger errors and systematic biases in frequency and intensity predictions.


<details>
  <summary>Details</summary>
Motivation: To assess whether state-of-the-art AI weather forecasting models can reliably predict unprecedented extreme weather events, which is crucial for early warning systems and disaster management.

Method: Comparative analysis of forecast errors between AI models (GraphCast, Pangu-Weather, Fuxi) and traditional numerical model HRES for record-breaking heat, cold, and wind events across various lead times.

Result: HRES consistently outperformed all AI models for extreme events. AI models showed larger forecast errors, underestimated frequency and intensity of record-breaking events, underpredicted hot records, and overestimated cold records with growing errors for larger record exceedance.

Conclusion: Current AI weather models have limitations in extrapolating beyond their training data and forecasting impactful extreme events, requiring further development before being solely relied upon for high-stakes applications.
...
TL;DR: DeCoDi is a lightweight debiasing method for diffusion models that modifies inference to avoid biased latent regions without changing image quality or adding significant compute overhead.


<details>
  <summary>Details</summary>
Motivation: Existing debiasing methods for deep learning models are often complex and compute-intensive, making them inaccessible to many practitioners. There's a need for a simple, effective debiasing approach that can be easily applied to diffusion-based text-to-image models.

Method: DeCoDi changes the diffusion inference procedure to steer away from latent dimension regions associated with biased concepts (gender, ethnicity, age). It operates during inference only without requiring model retraining.

Result: Human evaluation of 1,200 images showed effective mitigation of gender, ethnicity, and age biases for concepts like nurse, firefighter, and CEO. GPT4o automatic evaluation aligned with human assessment, showing reliable agreement between evaluators.

Conclusion: DeCoDi provides an accessible, effective debiasing solution that improves diversity in generated images without compromising quality or requiring significant computational resources, making bias mitigation more practical for widespread use.
...
<details>
  <summary>Details</summary>
Motivation: Current AI testing methods may fail to detect strategic behavior where AI systems understand their circumstances and manipulate evaluations, potentially leading to unsafe deployment.

Method: The paper uses examples from existing AI systems, reviews relevant research, and conducts formal strategic analysis of a stylized evaluation scenario to demonstrate the need for game-theoretic approaches.

Result: Evidence shows that AI systems can strategically manipulate standard evaluations, and game-theoretic analysis provides formal methods to scrutinize evaluation-based safety cases.

Conclusion: AI evaluation frameworks must incorporate strategic reasoning considerations, and game theory offers valuable tools for designing more robust safety evaluations that account for AI systems' potential strategic behavior.
...
### [291] [A Chinese Heart Failure Status Speech Database with Universal and Personalised Classification](https://arxiv.org/abs/2508.14908)
*Yue Pan,Liwei Liu,Changxin Li,Xinyao Wang,Yili Xia,Hanyue Zhang,Ming Chu*

Main category: eess.AS

TL;DR: First Chinese speech database for heart failure detection shows Chinese syllables contain HF-related information, with personalized pair-wise classification outperforming standard approaches and individual differences being key accuracy factors.


<details>
  <summary>Details</summary>
Motivation: Speech is a cost-effective, non-intrusive data source for heart failure detection, but there was a lack of research on whether Chinese syllables contain HF-related information like other well-studied languages.
...
Result: Confirmed effectiveness of Chinese language in HF detection. Pair-wise classification served as ideal speaker-decoupled baseline. Statistical tests showed individual differences are key contributors to inaccuracy.

Conclusion: Chinese speech contains valuable HF-related information. Personalized approaches outperform standard methods. Individual variability is a major factor affecting detection accuracy. Database and demonstrations made publicly available for future research.
...
### [292] [Transsion Multilingual Speech Recognition System for MLC-SLM 2025 Challenge](https://arxiv.org/abs/2508.14916)
*Xiaoxiao Li,An Zhu,Youhai Jiang,Fengjie Zhu*

Main category: eess.AS

TL;DR: A multilingual ASR system combining frozen Whisper encoder, trainable adaptor, and frozen Qwen2.5-7B LLM with LoRA achieved 9.83% WER/CER across 11 languages, ranking 3rd in MLC-SLM 2025 Challenge.


<details>
  <summary>Details</summary>
Motivation: To develop an effective multilingual automatic speech recognition system for the MLC-SLM 2025 Challenge that can handle diverse languages with high accuracy.
...
Conclusion: The systematic combination of pretrained models with task-specific fine-tuning proves effective for multilingual ASR, demonstrating strong performance across multiple languages.
...
### [293] [Mitigating Hallucinations in LM-Based TTS Models via Distribution Alignment Using GFlowNets](https://arxiv.org/abs/2508.15442)
...
Method: Proposes GOAT framework with uncertainty analysis revealing hallucination-uncertainty correlation, reformulates TTS as trajectory flow optimization, introduces enhanced Subtrajectory Balance objective with sharpened internal reward, and integrates reward temperature decay with learning rate optimization.
...
Conclusion: GOAT effectively mitigates hallucinations in LM-based TTS without requiring massive resources or adding inference cost, showing practical deployment potential.
...
### [294] [GEN2: A Generative Prediction-Correction Framework for Long-time Emulations of Spatially-Resolved Climate Extremes](https://arxiv.org/abs/2508.15196)
...
Conclusion: GEN2 successfully extrapolates beyond training data distribution and provides efficient, accurate forecasting of extreme climate event statistics across various emissions scenarios.
...
### [295] [LoUQAL: Low-fidelity informed Uncertainty Quantification for Active Learning in the chemical configuration space](https://arxiv.org/abs/2508.15577)
*Vivin Vinod,Peter Zaspel*

Main category: physics.chem-ph

TL;DR: A novel active learning method that uses low-fidelity quantum chemical computations to inform uncertainty quantification, improving prediction of quantum properties with fewer iterations.


<details>
  <summary>Details</summary>
Motivation: Uncertainty quantification is crucial for active learning in quantum chemistry, where cheaper low-fidelity computations can provide valuable information to guide the learning process more efficiently.

Method: Proposes a low-fidelity informed uncertainty quantification approach for active learning, leveraging the concept of fidelity in quantum chemical calculations where less accurate computations are available at lower cost.

Result: Models trained with the proposed method outperform alternatives in terms of empirical error and require fewer iterations. The choice of fidelity level was systematically studied and benchmarked.

Conclusion: The novel low-fidelity informed uncertainty quantification method effectively improves active learning performance for predicting quantum chemical properties like excitation energies and potential energy surfaces.
...
### [296] [Denoising by neural network for muzzle blast detection](https://arxiv.org/abs/2508.14919)
*Hadrien Pujol,Matteo Bevillacqua,Christophe Thirard,Thierry Mazoyer*

Main category: cs.SD

TL;DR: Acoem developed a lightweight neural network system to improve gunshot detection on moving military vehicles by reducing noise interference, doubling detection rates in noisy environments.


<details>
  <summary>Details</summary>
Motivation: Gunshot detection systems on military vehicles suffer performance degradation due to environmental noise, requiring a solution that can operate efficiently on various hardware platforms.

Method: Used a lightweight two-hidden-layer perceptron neural network combined with signal processing techniques instead of heavy convolutional networks to limit computational requirements.

Result: Detection rate for muzzle blast waveforms more than doubled when noise levels were comparable to the muzzle blast peak amplitude.
...
### [297] [Human Feedback Driven Dynamic Speech Emotion Recognition](https://arxiv.org/abs/2508.14920)
...
<details>
  <summary>Details</summary>
Motivation: Traditional speech emotion recognition assumes static emotions per audio, but real speech contains dynamic emotional sequences over time, especially important for animating emotional 3D avatars.

Method: Multi-stage approach: 1) Train classical speech emotion recognition model, 2) Generate synthetic emotional sequences, 3) Improve model with human feedback, 4) Novel Dirichlet distribution-based emotional mixture modeling.

Result: Dirichlet-based approach effectively models emotional mixtures, outperforming sliding window method. Human feedback integration improves model quality while simplifying annotation.

Conclusion: The proposed dynamic emotion recognition with Dirichlet mixture modeling and human feedback is effective for sequential emotion detection in speech, particularly beneficial for 3D avatar animation applications.
...
### [298] [XAI-Driven Spectral Analysis of Cough Sounds for Respiratory Disease Characterization](https://arxiv.org/abs/2508.14949)
...
TL;DR: XAI-driven method using occlusion maps to analyze cough spectrograms reveals disease-specific acoustic patterns that raw analysis misses, particularly for COPD patients.


<details>
  <summary>Details</summary>
Motivation: To enhance understanding of cough sound analysis for respiratory disease management by making AI-driven diagnostic results more interpretable and uncovering hidden disease signatures.

Method: Uses occlusion maps on CNN-processed cough spectrograms to highlight relevant spectral regions, then performs spectral analysis on weighted spectrograms to extract features and identify differences between disease groups.

Result: Found significant differences between disease groups (especially COPD) in identified spectral regions, contrasting with no significant differences in raw spectrogram analysis. COPD cough patterns show more variability in these regions.
...
TL;DR: Proposes evenly distributed filter banks and parameter-free feature enhancement for anomalous sound detection, achieving significant performance improvements on DCASE 2024 dataset.
...
Result: Evaluation on DCASE 2024 Challenge dataset demonstrates significant improvements in ASD performance with the proposed methods.

Conclusion: The proposed evenly distributed filter banks and parameter-free feature enhancement strategy effectively improve anomalous sound detection by better capturing frequency information and removing redundant audio content.
...
TL;DR: Combining optic nerve head structural features and visual field functional parameters improves classification of slow vs fast glaucoma progression in primary angle closure glaucoma patients using machine learning.


<details>
  <summary>Details</summary>
Motivation: To better classify glaucoma progression risk in PACG patients by integrating both structural (OCT) and functional (visual field) parameters, as current methods using either alone may be insufficient.

Method: Used 451 eyes from 299 PACG patients with >5 VF tests over >5 years. AI-segmented OCT to extract 31 ONH parameters, combined with sector-based VF sensitivity. Trained multiple ML classifiers, with Random Forest performing best. Used SHAP for feature importance analysis.

Result: Random Forest model combining structural and functional features achieved best performance (AUC = 0.87). Models using only structural or functional features performed worse (AUC 0.82 and 0.78 respectively). Six key predictors identified: inferior MRW, RNFL thickness, LC curvature, and VF sensitivity measures.
...
TL;DR: ARF method effectively generates synthetic epidemiological data that reliably reproduces original study findings across multiple health domains.


<details>
  <summary>Details</summary>
Motivation: Address limitations of current synthetic data methods (poor quality, high computational demands, complexity) and evaluate whether synthetic data can reliably reproduce epidemiological research findings.

Method: Used adversarial random forests (ARF) to synthesize tabular data, replicated analyses from six epidemiological publications, compared original vs synthetic results across various health domains.
...
Conclusion: ARF provides efficient, convenient synthetic data generation that reliably reproduces epidemiological research outcomes, with performance enhanced by dimensionality management.
...
TL;DR: Equi-mRNA is a novel codon-level equivariant mRNA language model that explicitly encodes synonymous codon symmetries using group theory, achieving significant improvements in prediction accuracy, sequence generation quality, and biological interpretability compared to vanilla baselines.


<details>
  <summary>Details</summary>
Motivation: Current mRNA models fail to adequately capture the structured relationships arising from the genetic code's inherent symmetries in synonymous codon usage, which is crucial for understanding translation efficiency and gene expression modulation.

Method: The model uses group-theoretic priors with cyclic subgroups of 2D Special Orthogonal matrix (SO(2)) to encode synonymous codon symmetries, combined with auxiliary equivariance loss and symmetry-aware pooling.

Result: Equi-mRNA achieves ~10% improvement in accuracy on property prediction tasks (expression, stability, riboswitch switching), produces mRNA constructs 4x more realistic under Frechet BioDistance metrics, and preserves functional properties 28% better than baselines.

Conclusion: Equi-mRNA establishes a new biologically principled paradigm for mRNA modeling with significant implications for next-generation therapeutic design, offering novel insights into codon usage patterns through interpretable learned representations.
...
### [303] [Privacy Preserving Inference of Personalized Content for Out of Matrix Users](https://arxiv.org/abs/2508.14905)
*Michael Sun,Tai Vu,Andrew Wang*

Main category: cs.IR

TL;DR: DeepNaniNet is a deep neural recommendation framework that addresses data sparsity, cold start, and privacy issues in niche communities using graph-based architecture with BERT embeddings and novel content basket representation.
...
Method: Inductive graph-based architecture combining user-item interactions, item-item relations, and BERT-derived textual review embeddings with content basket user representation and autoencoder-based generalization for unseen users.

Result: Achieves state-of-the-art cold start results on CiteULike, matches DropoutNet without performance degradation for out-of-matrix users, and outperforms WMF and DropoutNet by up to 7x and 1.5x in Recall@100 on AnimeULike dataset.
...
### [304] [Collaborative Filtering using Variational Quantum Hopfield Associative Memory](https://arxiv.org/abs/2508.14906)
...
<details>
  <summary>Details</summary>
Motivation: To leverage quantum computing's exponential speedup for machine learning applications, specifically developing a robust hybrid recommendation system that can perform well in both ideal and realistic noisy quantum environments.
...
Result: Achieved ROC 0.9795, accuracy 0.8841, F-1 Score 0.8786 in ideal environment; ROC 0.9177, accuracy 0.8013, F-1 Score 0.7866 in noisy environment with bit-flip and readout errors. Demonstrated consistent performance comparable to classical systems.

Conclusion: Presents a novel variational quantum computing framework with deep learning that handles real-world datasets effectively, maintains steady performance in noisy conditions, and offers promising direction for future quantum recommendation systems.
...
### [305] [Adversarial Attacks against Neural Ranking Models via In-Context Learning](https://arxiv.org/abs/2508.15283)
*Amin Bigdeli,Negar Arabzadeh,Ebrahim Bagheri,Charles L. A. Clarke*

Main category: cs.IR

TL;DR: FSAP is a black-box attack framework that uses few-shot prompting with LLMs to generate high-ranking adversarial documents that outrank authentic content in neural ranking systems.


<details>
  <summary>Details</summary>
Motivation: Neural ranking models are vulnerable to adversarial attacks, but existing methods rely on token-level perturbations or manual rewriting. FSAP aims to create a more effective and scalable attack method using LLMs' in-context learning capabilities.

Method: FSAP uses few-shot prompting with LLMs, conditioning on a small support set of harmful examples. It operates in two modes: FSAP-IntraQ (same query examples for topic fidelity) and FSAP-InterQ (cross-query generalization). No gradient access or model instrumentation required.

Result: FSAP-generated documents consistently outrank credible documents across four neural ranking models on TREC 2020-2021 Health Misinformation Tracks. The adversarial outputs show strong stance alignment and low detectability.
...
### [306] [Closing the Performance Gap in Generative Recommenders with Collaborative Tokenization and Efficient Modeling](https://arxiv.org/abs/2508.14910)
*Simon Lepage,Jeremie Mary,David Picard*

Main category: cs.IR

TL;DR: COSETTE and MARIUS address limitations of generative recommender systems by introducing contrastive tokenization with collaborative signals and a lightweight audio-inspired architecture that improves performance while reducing costs.
...
Result: Experiments show the approach narrows or eliminates the performance gap between generative and ID-based models while maintaining generative benefits and reducing inference costs.

Conclusion: The proposed methods successfully address key limitations of generative recommender systems, making them competitive with traditional approaches while preserving generative advantages.
...
### [307] [Personalized Recommendations via Active Utility-based Pairwise Sampling](https://arxiv.org/abs/2508.14911)
*Bahar Boroomand,James R. Wright*

Main category: cs.IR

TL;DR: A utility-based framework for recommender systems that learns preferences from pairwise comparisons with active sampling to optimize task-specific utility functions.


<details>
  <summary>Details</summary>
Motivation: Traditional recommender systems rely on numerical ratings or full rankings, which are often biased, subjective, and impractical to obtain. There's a need for more intuitive and accurate preference elicitation methods.

Method: Proposes a model-agnostic framework using pairwise comparisons within Plackett-Luce probabilistic framework. Introduces utility-based active sampling strategy that selects queries expected to maximize improvement in final recommendation utility. Tested with matrix factorization for movie recommendations and neural networks for university admissions.

Result: Experimental results show the framework provides more accurate, data-efficient, and user-centric personalized ranking compared to traditional approaches.

Conclusion: The proposed utility-based framework with pairwise comparisons and active sampling offers a superior paradigm for recommender systems that better aligns with application-specific objectives and user preferences.
...
### [308] [On the Effectiveness of Graph Reordering for Accelerating Approximate Nearest Neighbor Search on GPU](https://arxiv.org/abs/2508.15436)
...
Conclusion: Memory layout optimization through graph reordering provides significant performance benefits for GPU-based ANNS without compromising accuracy, complementing existing algorithmic approaches.
...
### [309] [M-$LLM^3$REC: A Motivation-Aware User-Item Interaction Framework for Enhancing Recommendation Accuracy with LLMs](https://arxiv.org/abs/2508.15262)
...
<details>
  <summary>Details</summary>
Motivation: Traditional recommendation systems struggle with cold-start and sparse-data scenarios, often generating noisy pseudo-interactions or relying too heavily on static semantic similarity while ignoring dynamic user motivation shifts.
...
Result: M-LLM^3REC demonstrates robust, personalized, and generalizable recommendations, particularly showing improved performance in cold-start situations compared to state-of-the-art frameworks.

Conclusion: The proposed motivation-driven approach using large language models effectively addresses cold-start and sparse-data challenges in recommendation systems by focusing on dynamic user motivation signals rather than just semantic similarity.
...
### [310] [Test-time Corpus Feedback: From Retrieval to RAG](https://arxiv.org/abs/2508.15437)
*Mandeep Rathee,Venktesh V,Sean MacAvaney,Avishek Anand*

Main category: cs.IR

TL;DR: Survey on adaptive retrieval methods in RAG systems that incorporate feedback to improve query, context, and document retrieval, moving beyond static retrieval-generation pipelines.


<details>
  <summary>Details</summary>
Motivation: Current RAG frameworks treat retrieval and reasoning as isolated components with one-time document retrieval, limiting performance on complex tasks requiring iterative evidence gathering and high-precision retrieval.

Method: Structured survey categorizing advanced retrieval and ranking mechanisms that integrate feedback signals based on their source and role in improving query formulation, retrieved context quality, or document pool selection.

Result: Provides a comprehensive overview of adaptive retrieval methods that bridge information retrieval and NLP perspectives, highlighting retrieval as a dynamic, learnable component.

Conclusion: Retrieval should be treated as a dynamic, feedback-driven process in end-to-end RAG systems rather than a static component, enabling better performance on complex knowledge-intensive tasks.
...
### [311] [MMQ: Multimodal Mixture-of-Quantization Tokenization for Semantic ID Generation and User Behavioral Adaptation](https://arxiv.org/abs/2508.15281)
*Yi Xu,Moyu Zhang,Chenxuan Li,Zhihao Liao,Haibo Xing,Hao Deng,Jinxin Hu,Yu Zhang,Xiaoyi Zeng,Jing Zhang*

Main category: cs.IR

TL;DR: MMQ framework uses multimodal tokenization with shared-specific experts and behavior-aware fine-tuning to create semantic IDs that improve recommendation scalability and generalization.


<details>
  <summary>Details</summary>
Motivation: Traditional ItemID-based recommender systems struggle with large dynamic item corpora and sparse long-tail data. Semantic IDs from multimodal content offer better knowledge transfer but face challenges in balancing cross-modal synergy with modality specificity and bridging the semantic-behavioral gap.

Method: Two-stage framework: 1) Shared-specific tokenizer with multi-expert architecture using modality-specific and modality-shared experts with orthogonal regularization, 2) Behavior-aware fine-tuning that adapts semantic IDs to recommendation objectives while preserving modality information through multimodal reconstruction loss.

Result: Extensive offline experiments and online A/B tests demonstrate effective unification of multimodal synergy, specificity, and behavioral adaptation for both generative retrieval and discriminative ranking tasks.

Conclusion: MMQ provides a scalable and versatile solution that addresses key challenges in semantic ID-based recommender systems, enabling better handling of new/rare items and improving recommendation performance.
...
### [312] [GenTune: Toward Traceable Prompts to Improve Controllability of Image Refinement in Environment Design](https://arxiv.org/abs/2508.15227)
*Wen-Fan Wang,Ting-Ying Lee,Chien-Ting Lu,Che-Wei Hsu,Nil Ponsa Campany,Yu Chen,Mike Y. Chen,Bing-Yu Chen*

Main category: cs.HC

TL;DR: GenTune is a system that helps designers understand and control AI-generated image prompts by allowing them to select image elements and trace them back to specific prompt labels for precise editing.


<details>
  <summary>Details</summary>
Motivation: Designers face challenges with lengthy LLM-generated prompts that are hard to understand and modify, and inpainting struggles with global consistency when making localized edits.

Method: GenTune provides an interface where designers can select any element in a generated image, trace it to corresponding prompt labels, and revise those labels to guide precise image refinement while maintaining global consistency.

Result: In a study with 20 designers, GenTune significantly improved prompt-image comprehension, refinement quality, efficiency, and overall satisfaction (all p < .01) compared to current practices.

Conclusion: GenTune effectively enhances human-AI collaboration for environment design by clarifying the mapping between AI-generated prompts and image content, enabling more precise and globally consistent image refinement.
...
### [313] ["Does the cafe entrance look accessible? Where is the door?" Towards Geospatial AI Agents for Visual Inquiries](https://arxiv.org/abs/2508.15752)
...
TL;DR: Vision for Geo-Visual Agents - multimodal AI agents that can answer visual-spatial questions by analyzing geospatial images combined with GIS data, overcoming limitations of traditional digital maps.


<details>
  <summary>Details</summary>
Motivation: Traditional interactive digital maps rely on pre-existing structured GIS data, limiting their ability to address visual questions about what the world actually looks like.

Method: Proposed multimodal AI agents that analyze large-scale repositories of geospatial images (street views, place photos, aerial imagery) combined with traditional GIS data sources.

Result: The paper presents a vision, describes sensing and interaction approaches, provides three exemplars, and enumerates key challenges and opportunities.

Conclusion: Geo-Visual Agents represent a promising direction for enhancing digital maps with visual understanding capabilities, though several challenges remain for future work.
...
### [314] [Foundation Models for Cross-Domain EEG Analysis Application: A Survey](https://arxiv.org/abs/2508.15716)
...
Method: The study develops a modality-oriented taxonomy that systematically organizes EEG foundation models based on their output modalities: native EEG decoding, EEG-text, EEG-vision, EEG-audio, and broader multimodal frameworks.
...
Conclusion: This taxonomy unifies the dispersed EEG foundation model field, providing a reference framework for future methodology development and accelerating the translation of EEG models into scalable, interpretable, and online actionable solutions.
...
### [315] [JEDI-linear: Fast and Efficient Graph Neural Networks for Jet Tagging on FPGAs](https://arxiv.org/abs/2508.15468)
...
TL;DR: JEDI-linear is a novel GNN architecture with linear computational complexity designed for FPGA deployment in particle physics trigger systems, achieving sub-60ns latency while eliminating DSP usage and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional Graph Neural Networks (GNNs) and Interaction Networks face computational complexity and irregular memory access challenges that make them difficult to deploy on FPGAs for hardware trigger systems with strict latency and resource constraints at the HL-LHC.
...
Result: Achieved 3.7-11.5x lower latency, up to 150x lower initiation interval, and up to 6.2x lower LUT usage compared to state-of-the-art designs. Eliminated DSP block usage entirely (vs. 8,700+ DSPs in SOTA), while delivering higher model accuracy. First interaction-based GNN to achieve <60ns latency.

Conclusion: JEDI-linear enables accurate, scalable, and resource-efficient GNN inference for real-time trigger systems, meeting HL-LHC CMS Level-1 requirements. The open-sourced templates support reproducibility and broader adoption in scientific applications.
...
TL;DR: The paper investigates the minimal neural requirements for symbolic thought, proposing that dynamic binding plus two types of hierarchical integration (multi-place predicates and structure mapping) are necessary for symbolic reasoning.


<details>
  <summary>Details</summary>
Motivation: To understand what distinguishes human brains in enabling symbolic reasoning that most animals lack, and to identify the minimal neural mechanisms required for symbolic thought beyond basic dynamic binding.

Method: Conducted 17 systematic simulations using cognitive architectures with and without multi-place predicates and structure mapping capabilities, testing performance on tasks that require symbolic reasoning without relying on diagnostic features.

Result: Simulation results support the hypothesis that dynamic binding combined with multi-place predicates and structure mapping are minimal requirements for basic symbolic thought.

Conclusion: These findings help explain human symbolic reasoning capabilities, highlight differences between biological and machine learning intelligence, and have important implications for bio-inspired artificial intelligence.
