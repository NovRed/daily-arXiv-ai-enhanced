...
Result: StereoDiff outperforms existing methods on zero-shot, real-world benchmarks, showing superior consistency and accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for combining visual and tactile data often lead to poor feature integration due to simplistic fusion techniques like addition or concatenation.

Method: Proposes ConViTac with a Contrastive Embedding Conditioning (CEC) mechanism, using self-supervised contrastive learning to project inputs into unified latent embeddings and coupling fusion via cross-modal attention.
...
Conclusion: ConViTac effectively enhances visual-tactile feature alignment and fusion, demonstrating significant performance gains in real-world tasks.
...
Method: Uses SAM, SAM 2, MedSAM, SAM-Med-3D, and nnU-Net for zero-shot inference on BraTS 2023 datasets with varied prompt qualities (points, bounding boxes), followed by fine-tuning on pediatric data.

Result: SAM and SAM 2 achieve high Dice scores (up to 0.894, 0.893) with accurate bounding box prompts, surpassing nnU-Net. Fine-tuning improves point prompts but not beyond bounding boxes or nnU-Net.

Conclusion: nnU-Net remains the top choice for medical segmentation due to impracticality of highly accurate prompts, though fine-tuning shows promise for future research.
...
Method: Leverages VLMs (e.g., BLIP-2, GPT-4o) to assess semantic correctness, visual quality, and artifacts, then ensembles top-ranked candidates. Introduces TWS, a hybrid metric combining semantic similarity, structural integrity, and artifact sensitivity.
...
Conclusion: The framework provides a principled, scalable solution for trustworthy SR, aligning outputs with human expectations and semantic correctness.
...
<details>
  <summary>Details</summary>
Motivation: Existing SSDG methods lack explicit regularization for domain-invariant representations, limiting their effectiveness with scarce labels.

Method: FixCLR adapts contrastive learning by using pseudo-labels for class information and a repelling term, enabling domain invariance.
...
Conclusion: FixCLR is a promising SSDG method, addressing domain invariance and improving generalization with limited labeled data.
...
<details>
  <summary>Details</summary>
Motivation: Standard CL's binary optimization leads to over-dispersion, disrupting pixel-wise feature correlations crucial for medical vision.

Method: Proposes COVER, a vector CL framework that models feature distances via displacement vector regression, ensuring pixel-wise feature correlation preservation.

Result: COVER significantly improves pixel-wise SSP across 8 tasks, 2 dimensions, and 4 modalities.
...
<details>
  <summary>Details</summary>
Motivation: Accurate recognition of ambiguous facial expressions in real-world scenarios is challenging but crucial for practical DFER applications.

Method: MIDAS augments training data by convexly combining video frames and their soft emotion labels, extending mixup to soft-labeled video data.

Result: Experiments on DFEW and FERV39k-Plus datasets show MIDAS outperforms state-of-the-art methods.

Conclusion: MIDAS effectively enhances DFER performance by addressing ambiguity through soft-label data augmentation.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods implicitly learn depth from RGB pixels, missing explicit cues humans use. ThirdEye explicitly incorporates these cues for better accuracy.
...
Result: Preliminary results suggest improved depth estimation; detailed quantitative results are pending.

Conclusion: ThirdEye's explicit cue integration and hierarchical fusion offer a promising direction for monocular depth estimation.
...
<details>
  <summary>Details</summary>
Motivation: The challenge of generating images with multiple humans performing complex actions while preserving facial identities lacks a dedicated benchmark, hindering progress in the field.

Method: The benchmark includes 1800 samples with text prompts, 5,550 unique face images, and pose conditioning. It employs four metrics for evaluation and proposes techniques like human segmentation and Hungarian matching for improved ID similarity.

Result: The benchmark and evaluation suite provide standardized tools for assessing generative models, with novel techniques enhancing ID similarity.

Conclusion: MultiHuman-Testbench offers valuable insights and a standardized approach to advance research in multi-human image generation.
...
Method: Revisits the Cyclopean Eye model, introduces novel geometric constraints, and evaluates stereo feature matching and attention mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of cost-efficiently performing complex multi-turn image editing tasks by leveraging the strengths of LLMs and A$^*$ search.

Method: Combines LLM-based high-level subtask planning with A$^*$ search for accurate tool-use. Uses inductive reasoning to extract reusable subroutines, enabling adaptive fast-slow planning.

Result: FaSTA$^*$ is significantly more computationally efficient than recent approaches while maintaining competitive success rates.

Conclusion: FaSTA$^*$ effectively balances speed and accuracy in image editing tasks by reusing subroutines and activating A$^*$ search only for novel subtasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for forgery localization struggle with computational overhead and limited representation power, especially for subtle or complex tampering.
...
Conclusion: PhysRig offers a versatile, physics-based solution for articulated object modeling, surpassing traditional methods.
...
<details>
  <summary>Details</summary>
Motivation: The lack of diverse, large-scale datasets for aviation visibility estimation hinders ML research in this area.

Method: A year-long data collection campaign from the FAA weather camera network was conducted to create a suitable dataset. Three common ML approaches and a baseline were tested on this and other datasets.

Result: Benchmark results show the performance of the tested methods compared to an ASTM standard.

Conclusion: The new dataset fills a critical gap and provides a foundation for future ML research in aviation visibility estimation.
...
Method: Proposes HST-CSLR, a hierarchical sub-action tree to align visual and textual modalities, leveraging gloss knowledge from large language models and contrastive alignment.
...
<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive platform for evaluating models' ability to leverage and understand multiple modalities (visual, auditory, textual).

Method: OmniEval includes 810 audio-visual videos, 2617 question-answer pairs, and introduces a granular video localization task (Grounding).

Result: The benchmark supports diverse tasks (3 major types, 12 sub-tasks) and is tested with omni-modality models like MiniCPM-O 2.6.

Conclusion: OmniEval aims to advance the evaluation of multimodal coherence and understanding, with publicly available codes and data.
...
Method: Developed PathChat+ with over 1M pathology-specific instruction samples and 5.5M Q&A turns, and SlideSeek for hierarchical reasoning on WSIs.
...
Conclusion: PathChat+ and SlideSeek advance computational pathology by addressing key limitations, enabling better diagnostic reasoning and reporting.
...
<details>
  <summary>Details</summary>
Motivation: Existing face aging methods struggle with balancing age accuracy and identity preservation, especially for large age gaps or extreme poses.

Method: A two-pass framework: first pass uses adaptive noise injection (AdaNI) for age accuracy, and the second pass uses identity-aware embeddings (IDEmb) for identity preservation.

Result: Cradle2Cane outperforms existing methods on CelebA-HQ, showing superior age accuracy and identity consistency.

Conclusion: The proposed framework effectively balances age transformation and identity preservation, advancing face aging technology.
...
<details>
  <summary>Details</summary>
Motivation: Photometric distortions in camera imaging degrade image quality, leading to poor 3D scene representations if unaddressed.

Method: Introduces internal and external photometric models, optimizes camera parameters, and uses depth regularization to avoid fitting scene-unrelated data.

Result: Achieves high-quality 3D scene representations despite imaging degradation like vignetting and dirt.

Conclusion: The method effectively separates scene-unrelated information, enhancing 3D scene representation quality under degraded imaging conditions.
...
<details>
  <summary>Details</summary>
Motivation: Dense signals (e.g., depth, DensePose) pose editing difficulties and inconsistencies with text prompts, motivating a revisit to sparse signals for simplicity and shape-agnostic benefits.
...
Result: SP-Ctrl outperforms dense signal-based methods in pose-guided generation and shows promise in diverse and cross-species tasks.

Conclusion: Sparse signals, enhanced by SP-Ctrl, are a viable alternative to dense signals for pose-guided text-to-image generation, offering simplicity and robust performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing CZSL methods inadequately handle semantic subsets and compositional divergence, limiting fine-grained alignment and generalization.
...
Result: Outperforms state-of-the-art CZSL methods on three benchmarks in closed- and open-world settings.

Conclusion: EVA effectively enhances compositional generalization and alignment, demonstrating its superiority in CZSL tasks.
...
Conclusion: The work advances explainable AI in precision oncology, enhancing interpretability and clinical decision-making.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with point-level tasks like segmentation due to lack of direct 3D-text alignment, limiting local 3D feature-text linking.

Method: Proposes TSDASeg, featuring a direct cross-modal alignment module and memory banks for storing and dynamically updating text, visual, and cross-modal features.

Result: Achieves state-of-the-art performance on 3D instruction, reference, and semantic segmentation datasets.

Conclusion: TSDASeg effectively addresses inconsistencies in interactive segmentation by leveraging explicit 3D-text alignment and dynamic memory updates.
...
Method: Sequential generation of audio tracks guided by text prompts and prior audio, using pre-trained models without specialized datasets.

Result: Generates multiple distinct audio tracks per video, achieving higher-quality composite audio than existing methods.

Conclusion: The proposed method effectively synthesizes high-quality audio by leveraging guided generation and accessible training data.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fail in dynamic, blurry scenes due to reliance on static assumptions or high-resolution inputs.

Method: Uses sparse-controlled Gaussian splatting to restore sharpness and reconstruct 3D geometry from blurry videos.

Result: Achieves robust performance and sets a new benchmark for realistic novel view synthesis.

Conclusion: DBMovi-GS effectively addresses dynamic scene synthesis from blurry inputs, outperforming prior methods.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges like lack of annotations, long-tailed data, and inconsistent staining styles in training robust neural networks for cytopathology.
...
Conclusion: SAIC is practical for clinical applications, enhancing detection models effectively. Code will be released.
...
<details>
  <summary>Details</summary>
Motivation: To address misuse risks of STR by detecting processed images and improving STR methods.
...
Result: High accuracy in detecting STR-processed images and localizing removed text regions. Text recovery is challenging but explored.

Conclusion: ISTR effectively detects STR misuse and improves STR by analyzing processed images, though text recovery remains difficult.
...
Method: VisionGuard combines Adaptive Labeling (tracking-based refinement) and Contextual Expander (virtual bounding boxes) to improve classification and recall.

Result: 3.1% improvement in mAP over baseline detectors, proving effectiveness for traffic surveillance.

Conclusion: VisionGuard is a viable solution for real-world deployment, promoting safety and regulatory compliance.
...
<details>
  <summary>Details</summary>
Motivation: Current 2D specimen radiography (SR) lacks accuracy, leading to high re-excision rates. A more precise method is needed.

Method: Combines Segment Anything Model (SAM) with Forward-Forward Contrastive Learning (FFCL) for margin classification and segmentation.

Result: Achieved AUC of 0.8455 for margin classification and 27.4% improvement in Dice similarity, with faster inference time.

Conclusion: FFCL-SAM enhances accuracy and speed of margin assessment, potentially reducing re-excision rates in breast cancer surgery.
...
<details>
  <summary>Details</summary>
Motivation: Prior methods model aging as a single deterministic path, lacking diversity and control over external factors. This work aims to visualize diverse futures and improve realism and condition alignment.
...
<details>
  <summary>Details</summary>
Motivation: Existing AR methods for view synthesis are mentally demanding and restrict scene exploration due to reliance on 3D annotations and small capture areas.

Method: Uses locally reconstructed light fields and visualizes errors to guide users in inserting new views, avoiding 3D annotations.
...
Conclusion: The approach improves user experience and scalability in AR-based view synthesis, supporting broader scene exploration and integration with advanced reconstruction techniques.
...
<details>
  <summary>Details</summary>
Motivation: Traditional VQA methods provide numerical scores but lack detailed quality descriptions. Video LMMs with linguistic output can address this, but existing data generation methods rely on human input and proprietary systems, limiting scalability.

Method: Proposes SIG, an automated pipeline that scores video quality dimensions, maps scores to text levels, and uses hierarchical Chain-of-Thought (CoT) to model quality correlations. Creates the S2I dataset and S2I-Bench for evaluation.

Result: SIG generates 320K instruction-response pairs (S2I) and a benchmark (S2I-Bench). Experiments show improved quality scoring and justification in video LMMs.

Conclusion: SIG automates VQA instruction data generation, enhancing video LMMs' capabilities without human or proprietary dependencies, validated by improved performance on benchmarks.
...
Result: FedSC demonstrates effectiveness in various challenging scenarios, improving performance and convergence.
...
<details>
  <summary>Details</summary>
Motivation: Addressing limitations in skin disease datasets (class imbalance, privacy, bias) and computational inefficiency of classical generative models.

Method: Novel classical-quantum latent space fusion technique to create a hybrid GAN for color medical image generation.

Result: The model surpasses classical and hybrid GANs in quality and classification boost, with 25x fewer parameters and 10x fewer epochs.

Conclusion: Quantum image generation shows promise as quantum hardware improves, demonstrated by robust performance on real quantum machines.
...
<details>
  <summary>Details</summary>
Motivation: Current VLM-based FER methods fail to capture fine-grained textual-visual relationships, crucial for distinguishing subtle facial expressions.
...
Conclusion: MPA-FER enhances FER accuracy and interpretability by integrating fine-grained semantic guidance and preserving CLIP's generalization.
...
Method: LASFNet uses a single feature-level fusion unit and introduces an attention-guided self-modulation feature fusion (ASFF) module, along with a lightweight feature attention transformation module (FATM), to adaptively adjust fusion features.

Result: The approach reduces parameters and computational costs by up to 90% and 85%, respectively, while improving detection accuracy (mAP) by 1%-3%.

Conclusion: LASFNet offers a favorable efficiency-accuracy trade-off, making it a promising solution for multimodal object detection.
...
Result: Achieves competitive performance with 128 tokens for 1024x1024 images, a 32-fold reduction in tokens, and faster training/inference.

Conclusion: The 1D binary latent space offers a scalable, efficient alternative to conventional tokenization, with competitive performance and reduced resource usage.
...
<details>
  <summary>Details</summary>
Motivation: Commercial RGB-D cameras produce noisy, incomplete depth maps for non-Lambertian objects, and traditional methods struggle due to limited training data diversity. Diffusion models offer potential but suffer from biases and feature limitations.
...
Method: Extracts intermediate features from a single-step diffusion process, introduces an object-centered auxiliary branch with box-masked images, and applies consistency loss for alignment. Also aligns standard detectors with diffusion detectors for cross-domain performance.

Result: Achieves competitive results on 3 DA and 5 DG benchmarks, with significant advantages in large domain shifts and low-data scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the trade-off between editability and faithfulness in image editing using diffusion models.
...
Result: FGS achieves superior faithfulness while maintaining editability and is compatible with various editing methods.

Conclusion: FGS enables precise, high-quality image edits across diverse tasks by balancing faithfulness and editability.
...
<details>
  <summary>Details</summary>
Motivation: To explore if self-supervised ViT representations can enhance adversarial transferability, leveraging the synergy between self-supervised learning and Transformers.

Method: Proposes dSVA, a dual self-supervised ViT features attack, combining contrastive learning (CL) and masked image modeling (MIM) in a generative framework.

Result: CL and MIM enable ViTs to focus on distinct features, improving adversarial generalizability and outperforming state-of-the-art methods.

Conclusion: Exploiting self-supervised ViT features significantly boosts adversarial transferability across diverse model architectures.
...
<details>
  <summary>Details</summary>
Motivation: Existing document analysis solutions lack flexibility for user-customized applications, prompting the need for a new task.

Method: The RoI-Matcher framework uses a siamese network and cross-attention layers to match regions in documents.

Result: Experiments confirm the method's effectiveness on the RoI-Matching-Bench, establishing a baseline for future research.

Conclusion: The proposed RoI-Matching task and framework offer a flexible, efficient solution for document analysis, with potential for further development.
...
<details>
  <summary>Details</summary>
Motivation: Challenges in retrieving 3D objects from masked 2D images and language prompts due to limited context, distorted views, and noisy data.

Method: SAMURAI integrates CLIP-based semantic matching, shape-guided re-ranking, and a majority voting strategy, with preprocessing for mask enhancement.
...
<details>
  <summary>Details</summary>
Motivation: Current image-based methods for 3D character modeling suffer from distortion due to self-occlusion and viewpoints, affecting reconstruction quality.
...
<details>
  <summary>Details</summary>
Motivation: Many OBS characters remain undeciphered due to their complexity, requiring innovative methods for interpretation.

Method: OracleFusion combines MLLM with SAR for glyph analysis and OSVF for generating semantically enriched vector fonts.
...
<details>
  <summary>Details</summary>
Motivation: Address the inefficiency of complex deep learning models in change detection by proposing a resource-efficient alternative suitable for on-satellite processing.

Method: Introduces FlickCD with an Enhanced Difference Module (EDM) to highlight critical feature differences and Local-Global Fusion Blocks using Shifted Window Self-Attention (SWSA) and Enhanced Global Self-Attention (EGSA) for multi-scale semantic capture.

Result: Achieves state-of-the-art performance or minor accuracy trade-offs (<1% F1) while reducing computational and storage costs significantly.

Conclusion: FlickCD successfully balances performance and resource efficiency, making it practical for real-world applications like urban monitoring and disaster assessment.
...
<details>
  <summary>Details</summary>
Motivation: To enhance the reasoning capabilities of multimodal large language models by addressing insufficient global context understanding and shortcut problems.

Method: Uses Reinforcement Learning (RL) with context, format, accuracy, and logical rewards judged by a large language model (LLM) to ensure thorough reasoning. Introduces IntentBench for evaluation.

Result: Demonstrates advanced performance on multiple omni-modal benchmarks compared to other open-source models.

Conclusion: The proposed method effectively improves multimodal reasoning by integrating global context and logical rewards, validated by superior benchmark results.
...
<details>
  <summary>Details</summary>
Motivation: Current VideoLLMs struggle with multi-shot scenarios due to lack of annotated datasets and lossy feature encoding.

Method: Developed MultiClip-Bench dataset and IPFormer-VideoLLM model, which uses instance prompts via an attention-based connector.

Result: The dataset and model significantly improve multi-shot video understanding and perform well on various benchmarks.

Conclusion: The proposed solutions address multi-shot challenges effectively, enhancing VideoLLM capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Accurate and efficient updates of 3D scene representations are essential for robotics, mixed reality, and embodied AI, but existing methods often require re-optimizing the entire scene, leading to computational overhead.

Method: CL-Splats uses Gaussian splatting and a change-detection module to segment updated and static scene components, enabling local optimization and avoiding unnecessary re-computation. It also supports storing and recovering previous scene states.

Result: CL-Splats achieves efficient updates with improved reconstruction quality compared to state-of-the-art methods.

Conclusion: CL-Splats provides a robust foundation for real-time adaptation in 3D scene reconstruction, with potential applications in temporal segmentation and scene analysis.
...
<details>
  <summary>Details</summary>
Motivation: The inherent uncertainty and multimodality in trajectory prediction for autonomous driving challenge existing data-driven methods, prompting the need for a more robust approach.
...
Result: The approach achieves state-of-the-art performance on Argoverse and nuScenes benchmarks and shows superior generalization.

Conclusion: GoIRL effectively addresses multimodality and uncertainty in trajectory prediction, offering a competitive alternative to supervised learning methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing learning-based methods lack datasets for extremely dark scenes (0.0001 lux), limiting their exploration.

Method: A paired-to-paired data synthesis pipeline creates the SIED dataset. A diffusion-based framework with an Adaptive Illumination Correction Module (AICM) and color consistency loss is proposed.

Result: The method effectively restores images from extremely low-SNR RAW inputs, validated on SIED and public benchmarks.

Conclusion: The SIED dataset and proposed framework advance low-light RAW image enhancement for extremely dark scenes.
...
TL;DR: YOLO-FDA is a YOLO-based framework for surface defect detection, enhancing fine-grained details and feature fusion with attention mechanisms, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Surface defect detection is challenging due to variability in defects, shapes, and textures. Existing AI methods lack detail sensitivity and robustness under multiscale conditions.

Method: YOLO-FDA integrates fine-grained detail enhancement and attention-guided feature fusion, using a BiFPN-style architecture, a Detail-directional Fusion Module (DDFM), and two attention-based fusion strategies (AC and CAF).

Result: YOLO-FDA outperforms state-of-the-art methods in accuracy and robustness across diverse defect types and scales.

Conclusion: The proposed framework effectively addresses challenges in surface defect detection, offering improved performance and robustness.
...
Method: Two tree-based semantic loss functions are introduced, leveraging hierarchical label organization, and integrated with sparse annotation training.

Result: State-of-the-art performance on a 107-class HSI dataset, with effective OOD pixel detection and maintained ID segmentation performance.

Conclusion: The proposed method enhances segmentation by exploiting label semantics and improves OOD detection without sacrificing ID performance.
...
Conclusion: Establishes a reliable foundation for automated myocardial scar quantification, supporting broader clinical adoption of deep learning in cardiac imaging.
...
Result: The approach achieves higher-fidelity 3D reconstruction and novel view synthesis, outperforming existing methods.
...
TL;DR: HalluSegBench is introduced as the first benchmark for evaluating hallucinations in vision-language segmentation, using counterfactual visual reasoning and a new dataset of 1340 instance pairs.
...
Method: The benchmark includes a novel dataset of counterfactual instance pairs and new metrics to quantify hallucination sensitivity under visually coherent scene edits.
...
<details>
  <summary>Details</summary>
Motivation: The Sim2Real domain gap in 3D point cloud data limits classifier generalization, as current UDA techniques lack robust domain-insensitive descriptors.

Method: TAM uses global spatial topology and self-supervised learning to model topological relations, combined with cross-domain contrastive learning and self-training.

Result: TAM consistently outperforms state-of-the-art methods on three Sim2Real benchmarks.

Conclusion: The TAM framework effectively mitigates the Sim2Real gap, enhancing generalization in 3D point cloud classification.
...
<details>
  <summary>Details</summary>
Motivation: Existing KV compression methods for long-video understanding in MLLMs suffer from significant information loss at high compression ratios, limiting their effectiveness.

Method: Video-X^2L uses bi-level KV compression (L-KVs for fine details, H-KVs for compact representations) and selective KV re-loading during decoding to balance information retention and computational efficiency.
...
Conclusion: Video-X^2L is a simple, effective, and training-free solution for improving long-video understanding in MLLMs by optimizing KV compression and re-loading.
...
TL;DR: The paper introduces Out-of-Distribution Semantic Occupancy Prediction (OccOoD) for autonomous driving, addressing OoD detection in 3D voxel space with synthetic datasets and a novel framework.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail in handling Out-of-Distribution (OoD) objects and long-tail distributions, posing safety risks in autonomous driving.
...
Conclusion: The work advances OoD detection in 3D semantic occupancy prediction, with datasets and code made publicly available.
...
TL;DR: GroundFlow is a plug-in module for temporal reasoning in 3D point cloud sequential grounding, improving accuracy and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current 3DVG methods lack temporal reasoning for sequential instructions with pronouns, limiting their performance in SG3D tasks.

Method: Proposes GroundFlow, a module for temporal reasoning, extracting short-term and long-term step information to enhance context understanding.

Result: Improves 3DVG baseline accuracy by +7.5% and +10.2%, outperforming pre-trained 3D large language models.
...
TL;DR: The paper introduces UNLOCK, a source-free method for panoramic image segmentation, addressing occlusion and distortion challenges without needing labeled source data.
...
Method: UNLOCK uses Omni Pseudo-Labeling Learning and Amodal-Driven Context Learning to adapt models without source data or target labels.

Result: The method achieves state-of-the-art performance (10.9 mAAP, 11.6 mAP) and a +4.3 mAPQ improvement over source-only methods.

Conclusion: UNLOCK offers a practical solution for panoramic segmentation, matching source-dependent methods without their constraints.
...
TL;DR: MedPrompt is a unified framework combining a prompted LLM for task planning and a modular CNN for image processing, enabling scalable, prompt-driven medical imaging.


<details>
  <summary>Details</summary>
Motivation: Address the lack of flexibility and task-specificity in current medical image analysis systems.

Method: Uses a few-shot prompted LLM (Llama-4-17B) for task planning and a modular CNN (DeepFusionLab) for image processing, with dynamic weight routing for task-specific pretrained weights.

Result: Achieves 97% correctness in prompt-driven tasks, 2.5s latency, and competitive accuracy (e.g., Dice 0.9856 on lungs, F1 0.9744 on tuberculosis).
...
Method: BitMark embeds watermarks at the bit level of token streams across multiple resolutions during image generation, preserving visual fidelity and speed.

Result: The watermark is robust against removal techniques and exhibits radioactivity, ensuring traces remain detectable even in models trained on watermarked images.

Conclusion: BitMark offers a reliable method to prevent model collapse by enabling detection of generated outputs, enhancing the sustainability of image generative models.
...
<details>
  <summary>Details</summary>
Motivation: Existing OVS solutions rely on pre-trained models or synthetic data, but their performance is limited by model capabilities or poor reference quality. This work highlights the importance of data quality for training-free OVS.

Method: The proposed framework includes a data pipeline to create a reference set with well-paired segment-text embeddings and a simple similarity-based retrieval method.

Result: Evaluations on ten benchmark datasets show the method surpasses all existing training-free OVS approaches.

Conclusion: Data-centric design is crucial for advancing training-free OVS, as demonstrated by the significant performance improvement achieved by focusing on high-quality reference sets.
...
TL;DR: ESFP is an end-to-end pipeline converting monocular RGB video into executable joint trajectories for a 4-DoF desktop arm, using modules for estimation, smoothing, filtering, and pose-mapping.


<details>
  <summary>Details</summary>
Motivation: To enable low-cost desktop arms to execute joint trajectories derived from monocular RGB video, addressing challenges like noise and anatomical plausibility.
...
Result: Produces executable joint trajectories with enforced anatomical plausibility and reduced noise.

Conclusion: ESFP effectively bridges monocular video to robotic arm control, ensuring smooth and anatomically plausible motion.
...
<details>
  <summary>Details</summary>
Motivation: Spurious correlations in visual data hinder OOD performance, and prior methods focus only on image features.

Method: DiMPLe combines mutual information minimization, spurious feature regularization, and contrastive learning on invariant features.

Result: DiMPLe outperforms CoOp-OOD, with gains of 15.27 in base class accuracy and 44.31 in novel class accuracy.

Conclusion: DiMPLe effectively disentangles features across modalities, enhancing robustness and generalization.
...
TL;DR: The paper introduces Temporal Rate Reduction Clustering (TR²C) for Human Motion Segmentation (HMS), outperforming existing subspace clustering methods by learning structured representations and affinity for video frame sequences.


<details>
  <summary>Details</summary>
Motivation: Existing subspace clustering methods assume high-dimensional temporal data follows a Union-of-Subspaces (UoS) distribution, which may not hold for complex human motions in cluttered backgrounds.
...
Conclusion: TR²C effectively addresses limitations of subspace clustering for HMS by learning better-structured representations.
...
Conclusion: DuET effectively addresses dual incremental learning challenges, offering practical solutions for real-world object detection systems.
...
TL;DR: ViTI proposes a video virtual try-on method using conditional video inpainting with a Diffusion Transformer, ensuring better spatial-temporal consistency and garment detail preservation.


<details>
  <summary>Details</summary>
Motivation: Existing video try-on methods suffer from inconsistency issues when adapting image-based techniques frame by frame or using temporal attention. ViTI aims to address these limitations by treating video try-on as a video inpainting task.

Method: ViTI formulates video try-on as conditional video inpainting, using a Diffusion Transformer with 3D spatial-temporal attention. It employs masking strategies and multi-stage training for garment inpainting, then adds garment conditions for detail accuracy.

Result: ViTI outperforms previous methods in both quantitative and qualitative evaluations, achieving superior spatial-temporal consistency and garment detail preservation.

Conclusion: ViTI effectively addresses inconsistency in video virtual try-on by leveraging video inpainting, demonstrating significant improvements over existing approaches.
...
TL;DR: The paper introduces the Text-Image Alignment (TIA) framework and WordCon, a hybrid PEFT method, to improve word-level typography control in generated images, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Precise word-level typography control in generated images is challenging, necessitating a new approach to enhance text-to-image model training.

Method: The TIA framework uses cross-modal correspondence and grounding models. WordCon, a hybrid PEFT method, reparameterizes key parameters for efficiency and portability. Masked and joint-attention losses enhance controllability.

Result: Qualitative and quantitative results show superiority over state-of-the-art methods.

Conclusion: The proposed method effectively improves word-level typography control, with datasets and code made available for academic use.
...
TL;DR: HieraSurg, a hierarchy-aware surgical video generation framework, uses two diffusion models to ensure consistency with surgical actions and phases, outperforming prior methods in quality and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing surgical video synthesis lacks consistency with surgical actions and phases, requiring fine-grained guidance for factual simulation.

Method: HieraSurg employs two diffusion models: one predicts coarse-grained semantic changes, and the other generates the final video by integrating these with fine-grained visual features.
...
TL;DR: A novel continual self-supervised learning method, CoSMAE, combines data and model mixup to reduce catastrophic forgetting in remote sensing tasks, outperforming existing methods by up to 4.94%.


<details>
  <summary>Details</summary>
Motivation: Existing continual learning methods in remote sensing require many labeled samples, which are costly. CoSMAE addresses this by leveraging self-supervised learning.
...
Result: CoSMAE improves performance by up to 4.94% over state-of-the-art methods, reducing catastrophic forgetting.

Conclusion: CoSMAE is an effective solution for continual learning in remote sensing, requiring fewer labeled samples and enhancing generalization.
...
TL;DR: The paper introduces ‘Drishtikon’, a multi-granular visual grounding framework for document VQA, achieving state-of-the-art accuracy with a focus on interpretability and multilingual support.


<details>
  <summary>Details</summary>
Motivation: Addressing the underexplored challenge of visual grounding in text-rich documents to enhance trust and interpretability in VQA systems.

Method: Integrates multilingual OCR, large language models, and a novel region matching algorithm for precise localization at block, line, word, and point levels.

Result: Achieves state-of-the-art grounding accuracy, with line-level granularity offering the best precision-recall trade-off. Ablations confirm the value of multi-block and multi-line reasoning.

Conclusion: The structured, alignment-based approach outperforms current VLMs in precise localization, paving the way for robust document understanding systems.
...
TL;DR: The paper introduces a method to enhance vision-language models (VLMs) for human-centric tasks by integrating human keypoints with traditional visual features, creating a specialized dataset and benchmark. Fine-tuning LLaVA-1.5-7B with this data improves performance by 33.2%.
...
Method: The method integrates human keypoints with captions and bounding boxes to create a dataset (200,328 samples) for fine-tuning models, focusing on conversation, detailed description, and complex reasoning. An Extended Human Pose and Action Understanding Benchmark (E-HPAUB) is also introduced.

Result: Fine-tuning LLaVA-1.5-7B with the new dataset yields a 33.2% improvement in performance on human-centric tasks.

Conclusion: Keypoint-integrated data significantly enhances VLMs for human-centric visual understanding, as demonstrated by the improved performance of LLaVA-Pose.
...
<details>
  <summary>Details</summary>
Motivation: Long surgical videos challenge current transformer models due to quadratic attention complexity, necessitating a more efficient solution.
...
Result: Achieves significant performance gains (+2.8% to +12.9%) over state-of-the-art methods on benchmark datasets.

Conclusion: The model efficiently processes full-length videos, capturing both local and global dynamics, and sets new performance benchmarks.
...
TL;DR: PanSt3R proposes a unified approach for 3D panoptic segmentation, eliminating test-time optimization by jointly predicting 3D geometry and multi-view segmentation in one forward pass, achieving faster and state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on 2D panoptic segmentation and test-time optimization, which are suboptimal and computationally expensive for 3D multi-view tasks.
...
Conclusion: PanSt3R offers a simple, fast, and scalable solution for 3D panoptic segmentation, outperforming prior approaches.
...
TL;DR: A physics-driven framework for solving electromagnetic inverse scattering problems (EISP) is proposed, improving generalization, accuracy, and robustness over existing methods.
...
Method: The problem is reformulated as a two-stage inverse transmission-scattering process, using induced current as an intermediate representation. A framework with a current estimator and permittivity solver is developed for end-to-end prediction.

Result: The method outperforms state-of-the-art approaches in reconstruction accuracy, generalization, and robustness to transmitter sparsity.

Conclusion: This work provides a new perspective on EISP and advances practical electromagnetic imaging solutions.
...
TL;DR: The paper explores using synthetic data to learn disentangled representations for real images, proposing a new metric and showing transferability is effective.
...
Method: Leverages synthetic data, studies fine-tuning effects, and introduces an interpretable intervention-based metric.

Result: Some disentanglement can transfer from synthetic to real data, proving effective.
...
TL;DR: ShotBench is a benchmark for cinematic language understanding, revealing VLMs' limitations. ShotQA dataset and ShotVL model are introduced to improve performance.
...
Conclusion: ShotBench and ShotQA advance AI-driven cinematic understanding; ShotVL sets new benchmarks, with open-sourced resources for further progress.
...
TL;DR: CoPa-SG is a synthetic dataset addressing the lack of accurate scene graph data, introducing parametric and proto-relations for finer representation and hypothetical reasoning. It enhances scene graph generation models and downstream applications.


<details>
  <summary>Details</summary>
Motivation: Current scene graph datasets lack accuracy and exhaustive annotations, limiting scene understanding and downstream tasks.

Method: Introduces CoPa-SG, a synthetic dataset with precise ground truth and new relation types (parametric and proto-relations). Evaluates scene graph generation models using this dataset.

Result: Demonstrates improved scene graph generation and enhanced planning/reasoning in downstream applications.

Conclusion: CoPa-SG overcomes data limitations and advances scene graph understanding with novel relation types, benefiting applications like planning and reasoning.
...
TL;DR: ToosiCubix is a monocular image-based method for 3D cuboid annotation of vehicles, requiring minimal user input and no specialized equipment, validated on KITTI and Cityscapes3D.
...
Result: Accurately estimates vehicle position, orientation, and dimensions (8 DoF), validated against KITTI and Cityscapes3D.
...
TL;DR: The paper proposes two modules, CAA and GOS, to improve image-to-point cloud registration by addressing feature channel attention differences and redundant correspondences.
...
Method: Introduces Channel Adaptive Adjustment Module (CAA) to enhance intra-modal features and suppress cross-modal sensitivity, and Global Optimal Selection Module (GOS) for global optimization instead of local selection.
...
Conclusion: The proposed CAA and GOS modules effectively address key challenges in cross-modal matching, improving registration accuracy.
...
TL;DR: GenFlow is a modular framework designed to simplify generative art creation by offering an intuitive node-based editor and NLP-powered assistant, making advanced tools accessible to all skill levels.


<details>
  <summary>Details</summary>
Motivation: The technical complexity of generative art limits its accessibility, so GenFlow aims to bridge this gap by reducing barriers and automating workflows.

Method: GenFlow uses a node-based editor for customization and an NLP-powered assistant to simplify workflow creation and deployment.

Result: A user study showed GenFlow optimizes workflows, reduces task times, and improves user understanding, making generative art more accessible.
...
TL;DR: FastRef is a prototype refinement framework for few-shot industrial anomaly detection (FS-IAD) that iteratively transfers query characteristics to prototypes and suppresses anomalies via optimal transport, outperforming existing methods.
...
Method: FastRef uses a two-stage process: (1) characteristic transfer via an optimizable transformation matrix, and (2) anomaly suppression through prototype alignment using optimal transport (OT).

Result: FastRef improves performance when integrated with PatchCore, FastRecon, WinCLIP, and AnomalyDINO, demonstrating effectiveness and efficiency across four benchmark datasets.

Conclusion: FastRef addresses the limitations of existing FS-IAD methods by refining prototypes with query statistics and anomaly suppression, proving robust in few-shot settings.
...
<details>
  <summary>Details</summary>
Motivation: Existing two-stage methods for 3D curve reconstruction suffer from error accumulation due to disconnected stages. The authors aim to develop a direct, efficient one-stage approach.

Method: Proposes CurveGaussian, a bi-directional coupling mechanism between parametric curves and edge-oriented Gaussian components, enabling differentiable rendering and direct optimization. Includes dynamic topology refinement.
...
Conclusion: The one-stage CurveGaussian framework achieves higher efficiency and superior performance, demonstrating the benefits of direct parametric curve optimization.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for fine-grained control in text-to-image generation often compromise editability and coherence, especially for multiple subjects, leading to artifacts or attribute entanglement.

Method: XVerse transforms reference images into offsets for token-specific text-stream modulation, allowing precise control over individual subjects without disrupting image latents or features.
...
Conclusion: XVerse significantly enhances personalized and complex scene generation by addressing the limitations of current approaches.
...
TL;DR: The paper introduces optical flow loss and depth regularization to improve 3D Gaussian Splatting (3DGS) for endoscopic SLAM, addressing challenges like photometric inconsistencies and dynamic motion.


<details>
  <summary>Details</summary>
Motivation: To enhance 3D reconstruction and real-time visualization in endoscopic scenarios, overcoming issues like non-Lambertian surfaces and breathing-induced motion.

Method: Uses optical flow loss as a geometric constraint, depth regularization for photometric inconsistencies, and refines 3DGS by focusing on keyframes with poor rendering quality.
...
Conclusion: The proposed method effectively improves 3DGS-based SLAM for endoscopic applications, with plans to release the source code.
...
TL;DR: HyperSORT is a framework using a hyper-network to predict UNet parameters from latent vectors, addressing biases in medical imaging datasets by learning a distribution of parameters to identify and mitigate noise and biases.


<details>
  <summary>Details</summary>
Motivation: Medical imaging datasets often contain heterogeneous biases (e.g., erroneous labels, inconsistent labeling styles) that degrade deep segmentation networks' performance. Identifying and characterizing these biases is challenging.

Method: HyperSORT employs a hyper-network to predict UNet parameters from latent vectors representing image and annotation variability. It jointly learns hyper-network parameters and latent vectors for each training sample, creating a distribution of UNet parameters.

Result: Validated on two 3D CT datasets (synthetic AMOS and real TotalSegmentator), HyperSORT structures the dataset to identify biases and errors. Latent space clusters yield UNet parameters that segment according to learned biases.

Conclusion: HyperSORT effectively maps dataset biases, enabling identification and mitigation, and improves segmentation robustness by learning parameter distributions.
...
TL;DR: The paper benchmarks deep learning models for classifying atypical mitotic figures (AMFs) in breast cancer, achieving high accuracy with LoRA-based fine-tuning of foundation models.
...
Method: The study compares deep learning approaches, including baseline models, foundation models with linear probing, and LoRA-based fine-tuning, using the AMi-Br dataset and two new hold-out datasets (AtNorM-Br and AtNorM-MD).

Result: Balanced accuracy scores of up to 0.8135 (AMi-Br), 0.7696 (AtNorM-Br), and 0.7705 (AtNorM-MD) were achieved, with LoRA-based adaptation performing particularly well.

Conclusion: Atypical mitosis classification is feasible with transfer learning and fine-tuning techniques, as demonstrated by the study, which also provides open-source code and data.
...
TL;DR: A method for precise object placement in images using visual maps and coarse masks, ensuring high-quality edits while preserving the background.


<details>
  <summary>Details</summary>
Motivation: Overcoming challenges in precise object placement (location and orientation) in image editing, which often requires complex inpainting masks or prompts.
...
Conclusion: The approach combines fine location and appearance control for precise object placement, offering flexibility and quality in image editing.
...
Method: A novel thermal imaging dataset was created by capturing mining activities. State-of-the-art object detection algorithms (YOLOv8, YOLOv10, YOLO11, RT-DETR) were evaluated on this dataset.

Result: The dataset provides a robust foundation for miner detection algorithms, with baseline performance metrics established for the evaluated models.

Conclusion: Thermal imaging is feasible for miner detection, and this work lays the groundwork for future research in emergency safety applications.
...
TL;DR: The paper introduces LF-CFG, a method to reduce oversaturation and artifacts in diffusion models by addressing redundant low-frequency signals.
...
Method: LF-CFG uses adaptive threshold-based measurement to identify redundant low-frequency information and applies a down-weight strategy to mitigate its impact.

Result: LF-CFG effectively reduces oversaturation and artifacts in models like Stable Diffusion-XL and SiT-XL.

Conclusion: LF-CFG provides a practical solution to improve diffusion model performance by targeting low-frequency signal redundancy.
...
TL;DR: The paper proposes dynamic, static, and hybrid methods for traffic signal design using turning movement count (TMC) data. A vision-based system estimates TMC, and simulations show optimal cycle times and method effectiveness based on traffic patterns.


<details>
  <summary>Details</summary>
Motivation: Accurate traffic signal design is essential for efficient traffic flow and congestion management. Existing methods may not adapt well to varying traffic conditions, prompting the need for flexible solutions.

Method: Developed a vision-based tracking system to estimate TMC, synthesized intersection data for simulation, and tested static, dynamic, and hybrid signal methods under diverse traffic patterns.

Result: Optimal cycle times are 90-120 seconds. Dynamic methods suit most intersections, while hybrid methods adapt better to bimodal traffic patterns (peak/off-peak). Zone-based traffic distributions influence method effectiveness.

Conclusion: Hybrid signal methods, adapting to traffic conditions, outperform static methods for uneven traffic distributions, offering improved flow management.
...
TL;DR: The paper introduces Radial Cross-Modal Embeddings (RCME) to model transitive entailment in vision-language models, improving hierarchical data representation.
...
Method: Proposes RCME, a framework optimizing partial order of concepts to enforce transitivity in entailment.

Result: Demonstrates improved performance in hierarchical species classification and retrieval tasks compared to state-of-the-art models.

Conclusion: RCME effectively models hierarchical structures in vision-language models, with open-sourced code and models.
...
TL;DR: The paper introduces TITAN, a method for source-free domain adaptive object detection, addressing pseudo-label noise by partitioning target images into easy and hard subsets and using adversarial modules to reduce domain gaps.


<details>
  <summary>Details</summary>
Motivation: The motivation is to tackle the degradation in performance of student models due to noisy pseudo-labels caused by domain bias and shifts, especially when source data is unavailable.

Method: The proposed TITAN method partitions target images into easy and hard subsets based on detection variance and integrates query-token adversarial modules into a student-teacher framework to reduce domain gaps.

Result: Experiments show TITAN outperforms SOTA methods, with mAP improvements of up to +22.7% on various benchmarks.

Conclusion: TITAN effectively addresses pseudo-label noise and domain gaps, demonstrating superior performance in source-free domain adaptation.
...
TL;DR: The paper addresses miscalibrated confidence scores in object detection models and proposes a statistical framework for better uncertainty quantification, especially in areas without detected objects.
...
Result: The method provides well-defined confidence estimates for drivable regions and demonstrates effectiveness through calibration assessments.

Conclusion: The statistical framework improves uncertainty quantification in object detection, enhancing safety in critical applications.
...
Method: DLC uses CLIP to assess semantic alignment between images and generated text, evaluates candidate tokens' Relative Visual Advantage (RVA), and employs an adaptive weighting mechanism to balance visual guidance and text quality.
...
Conclusion: DLC is an effective and efficient decoding-time solution to enhance LVLM reliability by mitigating hallucinations.
...
TL;DR: GGTalker improves 3D talking head synthesis by combining generalizable priors and identity-specific adaptation, addressing challenges like large head rotations and OOD audio.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in creating high-quality, generalizable speech-driven 3D talking heads, especially with large head rotations and OOD audio, without time-consuming identity-specific training.

Method: GGTalker uses a two-stage Prior-Adaptation strategy: learning Gaussian head priors (Audio-Expression and Expression-Visual) and adapting to individual characteristics. It includes a color MLP for fine textures and a Body Inpainter for blending.
...
Conclusion: GGTalker effectively addresses the limitations of previous methods by leveraging generalizable priors and precise adaptation, producing photorealistic results.
...
<details>
  <summary>Details</summary>
Motivation: Existing scene reconstructions in autonomous driving environments are limited to original observations and cannot support photorealistic synthesis of novel or altered scenarios.

Method: MADrive uses a memory-augmented framework to retrieve and integrate visually similar 3D car assets from a large-scale dataset (MAD-Cars) into target scenes, aligning orientation and relighting.

Result: The framework successfully replaces vehicles in scenes, providing complete multi-view representations for photorealistic synthesis of altered configurations.

Conclusion: MADrive extends the capabilities of current reconstruction methods, enabling more flexible and realistic synthesis of driving scenarios.
...
Method: WAFT replaces cost volumes with high-resolution warping, reducing memory usage and improving speed.

Result: WAFT ranks 1st on Spring and KITTI benchmarks, shows best zero-shot generalization, and is 4.1x faster than comparable methods.

Conclusion: WAFT proves cost volumes aren't essential for high performance, offering a simpler, faster, and more accurate solution for optical flow.
...
TL;DR: The paper addresses challenges in cross-modal image-text retrieval by proposing a set-based approach with Maximal Pair Assignment Similarity and two loss functions to improve representation diversity and distinction.


<details>
  <summary>Details</summary>
Motivation: Traditional single-vector embeddings fail to capture nuanced cross-modal relationships, while set-based approaches face issues like sparse supervision and set collapse.
...
TL;DR: StruMamba3D improves point cloud representation learning by addressing adjacency destruction and long-sequence memory loss in SSM-based methods, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing Mamba-based methods for point cloud learning destroy spatial adjacency and struggle with long-sequence memory, limiting SSM's potential.

Method: Proposes StruMamba3D with spatial states to preserve dependencies, a state-wise update strategy, lightweight convolution, and a length-adaptive approach.

Result: Achieves 95.1% accuracy on ModelNet40 and 92.75% on ScanObjectNN, outperforming existing methods.

Conclusion: StruMamba3D effectively addresses SSM limitations and sets new benchmarks in point cloud representation learning.
...
TL;DR: Proposes an occlusion-aware multi-view generation framework for 3D reconstruction from single occluded images, improving consistency and quality.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of inconsistent views and degraded 3D reconstruction from occluded single images, which existing diffusion-based models fail to handle.

Method: Uses a self-supervised pipeline with the Pix2Gestalt dataset, fine-tuning a view synthesis model for joint completion and multi-view generation.

Result: Enables consistent novel view synthesis from occluded images, facilitating better 3D reconstruction without manual intervention.

Conclusion: Introduces a novel framework and benchmark for occlusion-aware 3D reconstruction, advancing the field with standardized evaluation.
...
Method: Introduces UMPE for feature alignment, MCMA for temporal consistency, and a multi-modal data engine for pseudo-label generation.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the lack of benchmarks for comprehensive 3D ADS, especially in manufacturing where single-instance training and synthetic-to-real generalization are critical.

Method: Introduces a multimodal multiview dataset with high-resolution images and point clouds, alongside CAD models, and adapts singleview methods for baseline performance.
...
Conclusion: SiM3D sets a foundational benchmark for multiview 3D ADS, highlighting challenges in synthetic-to-real generalization and single-instance training.
...
Result: The model learns to predict egocentric video from actions, with hierarchical evaluation tasks assessing its prediction and control abilities.

Conclusion: This work is an initial step toward modeling real-world environments and embodied agent behaviors through egocentric video prediction.
...
TL;DR: The abstract discusses the importance of AI safety and the 2025 Singapore Conference on AI (SCAI), which aimed to prioritize research in AI safety using a defence-in-depth model.


<details>
  <summary>Details</summary>
Motivation: The motivation is to ensure AI is safe, trustworthy, and secure, fostering a trusted ecosystem for innovation without backlash.

Method: The method involves organizing AI safety research into three domains: Development (trustworthy systems), Assessment (risk evaluation), and Control (post-deployment monitoring).

Result: The report synthesizes research priorities in AI safety, building on international efforts like the International AI Safety Report.

Conclusion: A structured approach to AI safety research is essential for fostering trust and innovation in AI development.
...
TL;DR: The paper evaluates LLM-based agents' understanding of contextual privacy and their ability to preserve user privacy in multi-turn conversations. It introduces the MAGPIE benchmark and finds current models like GPT-4o and Claude-2.7-Sonnet often fail to protect private data, even with explicit instructions.


<details>
  <summary>Details</summary>
Motivation: The rise of LLM-based agents in collaborative tasks necessitates robust privacy measures, as these agents often handle sensitive data. Existing benchmarks lack complexity, prompting the need for a more realistic evaluation.

Method: The authors introduce MAGPIE, a benchmark with 158 high-stakes scenarios across 15 domains, designed to test privacy preservation and task completion. They evaluate state-of-the-art LLMs on their ability to classify private data and collaborate without breaches.

Result: Current models misclassify private data as shareable (25.2% for GPT-4o, 43.6% for Claude-2.7-Sonnet) and disclose private information in multi-turn conversations (59.9% and 50.5% of cases). Multi-agent systems fail in 71% of scenarios.

Conclusion: LLM-based agents lack robust contextual privacy understanding and struggle to balance privacy preservation with task completion, highlighting a critical gap in current models.
...
TL;DR: A dynamic context-aware prompt recommendation system for domain-specific AI applications, improving prompt quality through contextual analysis, knowledge grounding, and adaptive ranking.


<details>
  <summary>Details</summary>
Motivation: LLM-powered apps rely heavily on prompt quality, which is challenging for domain-specific tasks. This paper addresses the need for better prompt crafting tools.
...
Result: Achieves high usefulness and relevance in experiments, validated by automated and expert evaluations.

Conclusion: The system effectively enhances prompt quality for domain-specific applications, demonstrating practical utility.
...
TL;DR: A framework for projecting the societal impact of language model advice and a dataset for testing long-term safety awareness, showing improved performance on safety benchmarks.


<details>
  <summary>Details</summary>
Motivation: To ensure language models' beneficial societal impact by understanding and mitigating far-reaching adverse effects of their advice.
...
Conclusion: The approach offers a promising direction for developing safer language model-based agents.
...
<details>
  <summary>Details</summary>
Motivation: To assess and improve LLMs' causal reasoning capabilities, moving beyond shallow reasoning to human-like (level-2) reasoning.
...
Conclusion: G^2-Reasoner offers a promising approach to advance LLMs toward genuine causal reasoning, bridging the gap between level-1 and level-2.
...
Method: WAP introduces four cognitive capabilities (visual appearance, spatial reasoning, functional abstraction, syntactic grounding) and uses curriculum learning with raw visual observations.

Result: Qwen2.5-VL shows a 60.7% absolute improvement in task success, excelling in commonsense reasoning (+60.0) and long-horizon planning (+70.0).

Conclusion: WAP outperforms proprietary systems like GPT-4o and Claude-3.5-Sonnet, demonstrating the effectiveness of environment-aware LVLMs.
...
Method: Developed IXAII, an interactive system with four explainable AI methods, tailored views for five user groups, and user control over explanations.

Result: Evaluated via expert and lay user interviews; IXAII was perceived as helpful for increasing transparency.

Conclusion: IXAII bridges gaps between explainable AI, interactivity, and practical use, offering a new perspective on AI explanations and human-AI interaction.
...
TL;DR: The paper proposes an AI architecture for scientific discovery, emphasizing the need to close abstraction, reasoning, and reality gaps through active inference systems, integrating causal models, Bayesian planning, and real-world interaction.
...
Method: Proposes active inference AI systems with causal self-supervised models, Bayesian planners, knowledge graphs, and closed-loop interaction with simulators and labs.
...
Conclusion: Closing the gaps requires integrating internal models with real-world validation, maintaining human oversight for robust scientific AI.
...
<details>
  <summary>Details</summary>
Motivation: Existing MLLMs struggle with WildStruct conditions (e.g., visual degradation, complex layouts) in table understanding, leading to poor performance and generalization.

Method: Proposes TableMoE with Neuro-Symbolic Routing to predict token roles and route elements to specialized experts (Table-to-HTML, Table-to-JSON, Table-to-Code). Uses the TableMoE-Align dataset (1.2M quadruples) for pretraining.

Result: TableMoE surpasses state-of-the-art models on WildStruct benchmarks (WMMFinQA, WMMTatQA, etc.) and shows robustness and interpretability.

Conclusion: Neuro-symbolic reasoning in TableMoE enhances multimodal table understanding, validated by ablation studies and qualitative analyses.
...
Method: Developed MindCube benchmark, evaluated VLMs, and tested three approaches: unseen intermediate views, natural language reasoning chains, and cognitive maps. The 'map-then-reason' method was most effective.

Result: Initial VLM accuracy was 37.8%; 'map-then-reason' improved it to 60.8%, and reinforcement learning further increased it to 70.7%.
...
TL;DR: The paper introduces AH2AC2, a challenge to improve human-AI coordination in Hanabi using human proxy agents for evaluation, with a limited dataset of 3,079 games.


<details>
  <summary>Details</summary>
Motivation: Seamless human-AI coordination is critical but challenging, especially in games like Hanabi, which involve complex interactions. Current human evaluation methods are costly and hard to reproduce.

Method: The authors develop human proxy agents from a large-scale human dataset to serve as evaluation partners. They also open-source a limited dataset of Hanabi games to promote data-efficient methods.

Result: Baseline results for two- and three-player Hanabi scenarios are provided. Proxy agents are hosted via a controlled system to ensure fair evaluation.

Conclusion: AH2AC2 offers a reproducible and cost-effective framework for advancing human-AI coordination research, with publicly available code and a controlled evaluation system.
...
TL;DR: Mind2Web 2 introduces a benchmark for evaluating agentic search systems, proposing a novel Agent-as-a-Judge framework to assess complex, time-varying answers. The best system achieves 50-70% of human performance.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to evaluate the growing complexity and open-endedness of agentic search systems, necessitating a new approach.

Method: Developed Mind2Web 2 with 130 realistic tasks and proposed an Agent-as-a-Judge framework for automated evaluation.

Result: OpenAI Deep Research achieves 50-70% human performance in half the time, demonstrating potential.

Conclusion: Mind2Web 2 offers a rigorous foundation for future agentic search system development and benchmarking.
...
<details>
  <summary>Details</summary>
Motivation: Address deficiencies in existing AI counseling models, such as dialogue safety, scenario handling, and lightweight deployment.

Method: Two-stage training (hybrid distillation fine-tuning and ORPO optimization), conditional RAG for humor and safety, and quantization for lightweight deployment.

Result: Outperforms baselines in CEval, CPsyCounE (47.6% improvement), and SafeDialBench (2.4% improvement); operates on 5GB memory.

Conclusion: PsyLite offers a viable, efficient solution for AI-driven psychological counseling in resource-limited settings.
...
Conclusion: Hybrid symbolic-neural reasoning enhances probabilistic QA performance.
...
<details>
  <summary>Details</summary>
Motivation: Static benchmarks like Belebele, M-MMLU, and M-GSM inadequately measure practical performance and robustness in multi-lingual settings, prompting the creation of functional benchmarks.
...
Result: Performance drops between static and functional benchmarks vary (e.g., 24% for M-GSM vs. CL-GSM Symbolic in English). Certain languages (Arabic, English) show consistent robustness.

Conclusion: Functional benchmarks provide a more accurate assessment of multi-lingual competence, revealing significant variability in model performance and robustness across languages.
...
TL;DR: LLM-generated research ideas initially seem more novel than human ideas, but their execution leads to worse outcomes, closing the novelty gap and revealing limitations in LLM idea effectiveness.
...
Method: An execution study with 43 expert researchers implementing randomly assigned LLM or human ideas, followed by blind reviews of the resulting short papers.
...
Conclusion: The ideation-execution gap underscores LLMs' current limitations in generating truly effective research ideas and challenges in evaluating ideas without execution.
...
Method: MultiFinRAG uses a quantized multimodal LLM for extraction, embeds outputs with modality-aware retrieval, and employs a tiered fallback strategy for cross-modal reasoning.
...
TL;DR: The study evaluates LLMs' ability to handle morally ambiguous violent content using the VBVQ, revealing biases in their responses across demographics.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' reasoning in real-world violent scenarios and uncover potential biases in their responses.

Method: Used the VBVQ and persona-based prompting to evaluate six LLMs in a zero-shot setting, varying race, age, and geographic identity.

Result: LLMs' text generation often mismatches their internal violent tendencies, and their responses contradict established social science findings.
...
TL;DR: The paper explores the challenges of applying automated fact-checking systems in medicine, highlighting issues like claim-evidence connection, ambiguity, and subjective veracity. It suggests treating fact-checking as an interactive communication problem.
...
Result: Key challenges include connecting claims to clinical trials, ambiguity in claims, and subjective veracity labels.
...
TL;DR: The paper proposes methods to improve language model adaptation, including continued pre-training, parameter-efficient fine-tuning, and better supervised fine-tuning, enhancing robustness and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current fine-tuning methods for language models underutilize unlabelled data, overfit on small datasets, and are computationally expensive, limiting real-world applications.

Method: The study introduces continued pre-training for unlabelled data, parameter-efficient fine-tuning, improved supervised fine-tuning, and new evaluation benchmarks.

Result: The methods improve model robustness, efficiency, and generalization across diverse NLP tasks, including open-ended generation.

Conclusion: These advances enhance LM adaptability, contributing to more robust and efficient models, and progress toward artificial general intelligence.
...
TL;DR: The paper introduces a multilingual pre-training dataset pipeline, FineWeb2, addressing challenges in filtering and deduplication for diverse languages, and demonstrates its effectiveness through evaluations and scaling to 1000+ languages.


<details>
  <summary>Details</summary>
Motivation: Training high-quality multilingual LLMs is challenging due to the lack of tailored filtering and deduplication pipelines for diverse languages.

Method: A new pre-training dataset pipeline (FineWeb) is introduced, adaptable to any language, with ablation studies on nine languages and a novel evaluation task selection process.

Result: The pipeline creates non-English corpora that outperform prior datasets, with an additional performance uplift from a rebalancing approach.

Conclusion: FineWeb2, a 20TB multilingual dataset for 1000+ languages, is released alongside pipeline, training, and evaluation codebases.
...
Method: Uses bidirectional transformers with mean-pooling, multi-stage training (pre-training, fine-tuning, model-soup averaging), and techniques like focal-style reweighting and online hard-negative mixing.
...
Result: Fine-tuning with this method recovers some or all prompted performance, improving tasks like the 'reversal curse' and answering questions after one update.

Conclusion: Gradient descent can be expressive with proper initialization, offering insights for long-context modeling and generalization in gradient-based learning.
...
TL;DR: The paper extends the Machine Personality Inventory (MPI) to the 16PF model for finer personality control in LLMs, introduces a framework (SAC) for dynamic trait intensity, and shows improved consistency and controllability in personality expression.


<details>
  <summary>Details</summary>
Motivation: Existing LLM personality models rely on the coarse Big Five framework and lack mechanisms for controlling trait intensity, limiting nuanced human-like interactions.

Method: The authors extend MPI to the 16PF model and develop the SAC framework, using adjective-based semantic anchoring and five intensity factors for dynamic trait control.

Result: Modelling intensity as a continuous spectrum improves consistency and controllability, and trait changes influence related traits coherently, indicating multi-dimensional personality structures in LLMs.

Conclusion: The work advances nuanced human-machine interactions in fields like healthcare and education, moving closer to human-like social machines.
...
<details>
  <summary>Details</summary>
Motivation: To assess how well LLMs handle domain-specific financial knowledge, especially in the Indian context, where such evaluation is lacking.

Method: Developed CA-Ben, a benchmark using ICAI exam datasets, and tested six LLMs (GPT-4o, LLAMA variants, MISTRAL, Claude, Microsoft Phi) with standardized protocols.
...
TL;DR: Proposes SSUF, a semi-supervised scalable unified framework for e-commerce query classification, addressing insufficient prior information and lack of unified subtask frameworks.


<details>
  <summary>Details</summary>
Motivation: E-commerce queries are short and lack context, leading to insufficient prior information. Existing methods rely on posterior click behavior, creating a Matthew vicious cycle, and lack a unified framework.

Method: SSUF includes knowledge-enhanced, label-enhanced, and structure-enhanced modules to unify tasks, enhance representations, and reduce dependence on posterior labels.

Result: SSUF outperforms state-of-the-art models in offline and online A/B experiments.

Conclusion: SSUF effectively addresses limitations in e-commerce query classification, offering a scalable and unified solution.
...
Method: The authors introduce MT2-CSD, a large annotated dataset, and propose LLM-CRAN, a model using LLMs for better conversational understanding.

Result: LLM-CRAN outperforms baseline models in conversational stance detection on the MT2-CSD dataset.

Conclusion: The study advances conversational stance detection with a robust dataset and an effective LLM-enhanced model.
...
Method: DALR uses a consistency learning module for fine-grained cross-modal alignment and integrates ranking distillation with intra-modal alignment learning.

Result: DALR outperforms state-of-the-art baselines on STS and TR tasks.
...
TL;DR: The paper introduces a chunk-based approach using self-supervised learning models for automatic fluency assessment, improving performance over baselines.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in capturing speech rhythm, pauses, and disfluencies in non-native speakers for better fluency assessment.

Method: Uses SSL models (Wav2Vec2, HuBERT, WavLM) with a CNN-BiLSTM framework, segmented into breath-group chunks using Silero-VAD, and enriched with fluency markers.
...
Conclusion: Chunk-based multi-SSL fusion is effective for fluency evaluation, but future work should explore generalization to dialects with irregular prosody.
...
<details>
  <summary>Details</summary>
Motivation: Effectively leveraging historical interactions and domain knowledge in real-time CQA platforms is challenging, as existing methods underutilize external knowledge or lack suitable memory mechanisms.

Method: ComRAG integrates static knowledge and dynamic historical QA pairs using a centroid-based memory mechanism for retrieval, generation, and efficient storage.

Result: ComRAG achieves up to 25.9% improvement in vector similarity, reduces latency by 8.7% to 23.3%, and lowers chunk growth from 20.23% to 2.06% over iterations.

Conclusion: ComRAG demonstrates superior performance and efficiency in real-time industrial CQA, addressing limitations of existing methods.
...
Result: Achieves ~25% reduction in updated parameters with competitive performance and high adaptability.

Conclusion: Progtuning optimizes resource allocation and works well with parameter-efficient methods, offering a scalable solution.
...
Method: Cosmos learns a compressed latent space via an autoencoder trained for token-level reconstruction and alignment with a pretrained language encoder, enabling efficient diffusion-based generation.

Result: Cosmos compresses text representations by 8×, matches or surpasses baselines in quality, and achieves >2× faster inference across four generative tasks.

Conclusion: Cosmos demonstrates the viability of latent-space diffusion for text generation, combining speed, quality, and flexibility.
...
Conclusion: The engineering practices detailed in the paper provide valuable insights for ensuring reproducibility and usability in evaluation frameworks like MTEB.
...
Method: Extends a transformer-based voice activity projection model by integrating textual prompt embeddings into channel-wise and cross-channel transformers. Synthetic prompts were generated using an LLM.

Result: Improved prediction accuracy and adaptable turn-taking timing behaviors based on textual prompts.

Conclusion: The proposed model successfully integrates textual prompts for dynamic control, enhancing turn-taking prediction in conversational systems.
...
TL;DR: The paper explores using syntactic retrieval for Automatic Term Extraction (ATE) with LLMs, showing improved F1-scores in benchmarks.


<details>
  <summary>Details</summary>
Motivation: ATE is vital for NLP tasks, but LLMs' potential for ATE is understudied. The paper aims to address this gap.
...
Result: Syntactic retrieval improves F1-scores on three ATE benchmarks, highlighting the role of syntactic cues.

Conclusion: Syntactic retrieval is effective for ATE with LLMs, emphasizing the importance of syntactic guidance.
...
Method: Agent-RewardBench evaluates reward models across multiple dimensions (perception, planning, safety) and step-level tasks, with carefully curated difficulty and quality.
...
TL;DR: The paper explores whether increasingly large language models (LLMs) can outpace simple classifiers in generating deceptive text, focusing on classical detective fiction. Findings suggest reliable detection remains feasible despite model growth.


<details>
  <summary>Details</summary>
Motivation: To determine if newer, larger LLMs can consistently outperform simpler classifiers in generating deceptive text, avoiding an endless arms race in detection methods.

Method: Examined the ability of statistical classifiers to detect 'fake text' in classical detective fiction, comparing performance over a 0.5 version increase in models like Gemini and GPT.

Result: Gemini showed improved deceptive text generation, while GPT did not, indicating detection may remain feasible even as models grow.

Conclusion: Reliable detection of fake text may persist despite larger models, though new architectures could enhance deceptiveness.
...
<details>
  <summary>Details</summary>
Motivation: Address the limitation of LLMs in generating informative critiques and refining solutions, aiming to improve their reasoning capabilities.
...
Result: Pass@1 performance on AIME benchmarks improved from 4.4% to 18.2%, showcasing enhanced reasoning.
...
<details>
  <summary>Details</summary>
Motivation: LLMs often generate ungrounded responses when context lacks information, harming factual consistency and trustworthiness.

Method: Fine-tuned RoBERTa and NomicBERT models are used to detect groundedness in queries before LLM answer generation.

Result: These models match state-of-the-art LLMs in accuracy while drastically cutting inference latency.

Conclusion: Task-specific encoders offer a resource-efficient solution for groundedness detection in LLMs.
...
TL;DR: The paper investigates using a text-only autoregressive language model to extract referring expressions from visually grounded dialogue, showing effectiveness despite inherent multimodal limitations.


<details>
  <summary>Details</summary>
Motivation: To determine if linguistic context alone can detect mentions with visual referents in dialogue, without relying on visual data.

Method: Adapts a pretrained LLM for coarse-grained mention span annotation via next-token prediction, using small datasets and parameter-efficient fine-tuning.

Result: A text-only approach is effective, emphasizing linguistic context's importance, but the task remains inherently multimodal.

Conclusion: While text-only methods work, the task's multimodal nature limits unimodal approaches.
...
TL;DR: GLASS, a framework based on Greimas Semiotic Square, enhances LLMs' literary analysis by providing structured criticism and quantitative metrics, outperforming expert critiques.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with deep literary criticism; GLASS aims to bridge this gap by leveraging structured analysis.

Method: Proposes GLASS framework, creates a GSS-based dataset, and uses LLM-as-a-judge for quantitative evaluation.

Result: GLASS achieves high performance compared to expert criticism and produces original analyses for classic works.

Conclusion: GLASS offers a valuable AI tool for literary research and education, addressing cognitive mechanisms in literary engagement.
...
TL;DR: Omni-RAG is a framework enhancing RAG systems for noisy, ambiguous queries by using LLM-assisted query understanding, intent-aware retrieval, and reranking.


<details>
  <summary>Details</summary>
Motivation: Current RAG systems struggle with noisy, multi-intent queries in real-world settings, limiting their effectiveness.

Method: Omni-RAG uses three modules: query understanding/decomposition, intent-aware retrieval, and reranking/generation with LLMs.

Result: The framework improves robustness and effectiveness for complex queries in open-domain applications.
...
TL;DR: A DK-Enhanced LLM framework is proposed to detect deceptive conversations and concept drift, achieving high accuracy by integrating domain knowledge with LLMs.
...
Conclusion: Incorporating domain knowledge and drift awareness significantly improves performance and robustness in high-stakes NLP tasks.
...
Method: Created a multilingual test set by translating English questions to Spanish and Turkish, preserving Cypher queries. Evaluated models using standardized prompts and metrics.
...
Conclusion: Highlights the need for inclusive multilingual evaluation and development, with future work on schema localization and fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: Current preference learning methods are text-focused and unsuitable for real-time speech interactions with dynamic features like interruptions.

Method: A large-scale dataset of 150,000 preference pairs from speech conversations is created, annotated with AI feedback. Offline alignment fine-tunes a full-duplex autoregressive speech-to-speech model.

Result: Feedback improves spoken dialogue models, making interactions more factual, safer, and contextually aligned. Human evaluations confirm broader impact.

Conclusion: Balancing dynamics like interruptions and context is key for natural real-time speech dialogue systems.
...
TL;DR: The paper introduces TopK LMs, a modified transformer architecture with TopK activation, to address shortcomings of sparse autoencoders (SAEs) in interpreting language models.


<details>
  <summary>Details</summary>
Motivation: SAEs have limitations in interpretability and feature stability, making it hard to trace concept learning in LMs. The goal is to integrate interpretability directly into the model.

Method: A TopK activation function is added to transformer layers, making hidden states equivalent to TopK SAE features, eliminating post-hoc training.
...
TL;DR: Reinforcement learning methods for finetuning large language models show strong performance in semi-online and fully online regimes, outperforming offline methods. Similar results were found for verifiable and non-verifiable tasks, with multi-tasking improving performance.


<details>
  <summary>Details</summary>
Motivation: To evaluate the effectiveness of reinforcement learning in transitioning from offline to online regimes for both verifiable (e.g., math) and non-verifiable (e.g., instruction following) tasks.

Method: Compared online and semi-online Direct Preference Optimization and Group Reward Policy Optimization objectives, analyzing training dynamics and hyperparameter strategies.

Result: All online variants performed similarly and outperformed offline methods. Multi-tasking with verifiable and non-verifiable rewards improved performance across tasks.

Conclusion: Online reinforcement learning methods are highly effective for finetuning large language models, with multi-tasking further enhancing performance.
...
<details>
  <summary>Details</summary>
Motivation: Prior methods focus on knowledge or dialogue acts but fail to guarantee user engagement in socially-driven dialogues. The paper aims to directly optimize for engagement by leveraging user reactions as rewards.

Method: The approach involves a user simulator interacting with LLMs via i×MCTS to collect data on engagement levels, followed by aligning LLMs using direct preference optimization (DPO).

Result: Experiments in emotional support and persuasion scenarios show the method effectively improves user engagement in interactive LLMs.

Conclusion: The proposed method successfully enhances user engagement in socially-driven dialogues by directly optimizing for user reactions and leveraging future conversation signals.
...
TL;DR: The paper introduces skLEP, the first benchmark for Slovak NLU models, covering diverse tasks and datasets, and evaluates various pre-trained models.
...
Method: Curated new Slovak datasets and translated English NLU resources, then systematically evaluated Slovak-specific, multilingual, and English pre-trained models.

Result: Provided a thorough assessment of model capabilities and released benchmark data, toolkit, and leaderboard for reproducibility.

Conclusion: skLEP fills a critical gap in Slovak NLU research, enabling future advancements through open resources and evaluations.
...
TL;DR: The paper questions the validity of benchmark evaluations for LLMs, arguing they may only show superficial understanding unless LLMs' misunderstandings mirror humans'. It introduces methods to detect such 'potemkins' and finds them widespread, revealing deeper conceptual incoherence.


<details>
  <summary>Details</summary>
Motivation: To challenge the reliability of benchmark evaluations for LLMs, highlighting the need for tests that align with human-like misunderstandings to ensure genuine understanding.

Method: Introduces a formal framework and two procedures: one using a specially designed benchmark, and another providing a lower-bound estimate of 'potemkins' (superficial understanding).

Result: Potemkins are found to be common across models, tasks, and domains, indicating not just incorrect understanding but deeper incoherence in concept representations.

Conclusion: Benchmark evaluations may mislead unless they account for human-like misunderstandings, as LLMs often exhibit superficial understanding and conceptual incoherence.
...
<details>
  <summary>Details</summary>
Motivation: To understand how users interact with LLMs for healthcare information and identify inherent risks, such as incomplete context and sycophancy, to improve AI healthcare support.
...
Conclusion: Highlights the need for enhancing LLM capabilities in healthcare support to address identified risks and improve user interactions.
...
TL;DR: The paper introduces DELT, a paradigm for optimizing data organization (Data Efficacy) in language model training, featuring components like Data Scoring, Selection, and Ordering. LQS and FO are proposed as novel methods, showing improved performance without extra data or model size.


<details>
  <summary>Details</summary>
Motivation: To explore Data Efficacy, an underexplored area focusing on optimizing training data organization to enhance language model performance, complementing data efficiency efforts.

Method: Proposes DELT, a framework with Data Scoring (LQS), Data Selection, and Data Ordering (FO). LQS scores data by learnability and quality, while FO addresses forgetting and bias.
...
Conclusion: Data efficacy is a promising foundational area for LM training, with DELT demonstrating its potential to enhance performance through optimized data organization.
...
TL;DR: SAFL introduces a size-based adaptive federated learning framework, optimizing training dynamics by dataset size and modality, achieving high accuracy and communication efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing FL approaches overlook dataset size impact. This work addresses how data characteristics should guide FL strategies.

Method: SAFL organizes FL based on dataset size and modality, tested across 13 diverse datasets.

Result: Optimal dataset size is 1000-1500 samples; structured data outperforms unstructured. SAFL achieves 87.68% average accuracy and reduces data transfer to 7.38 GB.

Conclusion: SAFL provides insights and practical guidance for FL deployments, filling gaps in understanding data-driven FL strategies.
...
<details>
  <summary>Details</summary>
Motivation: The need for robust, interpretable tools to analyze complex omics data and detect disease-driving genes.

Method: Integrates SVM, Random Forests, GAEs, and GAANs for anomaly detection in gene expression or methylation networks.

Result: Successfully identifies biologically relevant anomalies in bladder cancer and coeliac disease.

Conclusion: E-ABIN provides a powerful, interpretable solution for anomaly detection in biological networks.
...
TL;DR: The paper introduces the Context-Content Uncertainty Principle (CCUP), a framework for inference under uncertainty, and outlines four hierarchical layers of operational principles derived from entropy asymmetry between context and content.


<details>
  <summary>Details</summary>
Motivation: To provide a unified theoretical foundation for how brains and machines minimize uncertainty through recursive structure-specificity alignment.

Method: Develops a layered computational framework with four hierarchical layers (L1-L4) of operational principles, supported by formal theorems and simulations.

Result: Demonstrates efficiency gains of CCUP-aligned inference and shows how principles like structure-before-specificity and asymmetric learning rates are mutually reducible.

Conclusion: CCUP offers a unified approach to understanding uncertainty minimization in brains and machines, emphasizing cycle-consistent entropy resolution.
...
TL;DR: A tree-based method, Diffusion Tree Sampling (DTS), improves inference-time alignment of diffusion models by reusing past computations, achieving high reward samples with less compute.


<details>
  <summary>Details</summary>
Motivation: Existing methods for adapting pretrained diffusion models suffer from inaccurate value estimation and inefficient compute use.

Method: Proposes DTS and its greedy variant DTS*, which cast alignment as a search problem, reusing past computations and refining value estimates iteratively.

Result: DTS matches baseline FID with 10x less compute; DTS* finds high reward samples with 5x less compute.
...
TL;DR: The paper proves the correctness of the FLIPD LID estimator under realistic assumptions, extending beyond the unrealistic affine submanifold assumption, and explores its applicability with uniform convolutions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the incomplete theoretical foundation of FLIPD, an LID estimator for diffusion models, by validating its correctness under realistic conditions and extending its applicability.

Method: The method involves formal theoretical proofs to validate FLIPD's correctness under realistic assumptions and explores its performance with uniform convolutions instead of Gaussian.

Result: The results confirm FLIPD's correctness under realistic assumptions and demonstrate its analogous validity with uniform convolutions.

Conclusion: The conclusion solidifies FLIPD's theoretical foundation, enhancing its reliability for applications like outlier detection and AI-generated text identification.
...
TL;DR: The paper explores if test-time scaling techniques for LLMs, effective in math reasoning, generalize to advanced physics. A novel symbolic weak-verifier framework outperforms existing methods on TPBench and AIME.


<details>
  <summary>Details</summary>
Motivation: To determine if test-time scaling methods for LLMs, successful in mathematical reasoning, can be applied effectively to advanced theoretical physics.

Method: Evaluates common test-time scaling methods on TPBench and AIME, introducing a symbolic weak-verifier framework to leverage physics problem structure.

Result: The novel framework significantly outperforms existing methods on TPBench and also proves effective on AIME.

Conclusion: Step-wise symbolic verification is powerful for complex scientific problems, bridging math and physics reasoning.
...
Method: The paper provides a comprehensive survey, introducing a task-driven taxonomy for six application areas and reviewing advances in unimodal/multimodal FMs and LLM agents.
...
Conclusion: Recall pathways involve task-specific attention and relation extraction in final layers, revealing redundancy and layer-specific mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: To address the high cost of training large language models by leveraging lower-precision arithmetic formats for efficiency, while understanding and mitigating associated instabilities.

Method: Trains nearly 1,000 language models with varying precision formats, conducts controlled experiments on a proxy model, and proposes stabilization strategies.

Result: Identifies instabilities caused by gradient bias from quantization and demonstrates hybrid precision schemes can recover performance comparable to full-precision training.

Conclusion: Hybrid precision configurations can mitigate instabilities and maintain competitive performance, offering a viable path for efficient large-scale model training.
...
<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of classical closure modeling methods, which are deterministic and local, and to reduce the computational cost of diffusion-based stochastic models.
...
Conclusion: The proposed latent conditional diffusion model offers an efficient and accurate solution for closure modeling in complex dynamical systems.
...
TL;DR: SPD is a scalable, robust alternative to APD for decomposing neural networks, addressing computational cost and hyperparameter sensitivity issues.
...
Method: Introduces Stochastic Parameter Decomposition (SPD), a more scalable and robust method compared to APD.

Result: SPD successfully decomposes larger and more complex models, avoids parameter shrinkage, and better identifies ground truth mechanisms.
...
TL;DR: The paper introduces an LLM-powered 'GPU Kernel Scientist' to automate and optimize GPU kernel refinement, especially for newer or less-documented architectures like AMD MI300.


<details>
  <summary>Details</summary>
Motivation: Optimizing GPU kernels is complex and resource-intensive, particularly for newer architectures with limited documentation. The goal is to automate this process using LLMs.

Method: The methodology uses LLMs in a multi-stage evolutionary process: selecting prior code versions, generating optimization hypotheses, and autonomously implementing experiments with performance feedback.

Result: While quantitative results are embargoed, the paper presents the design, workflow, and qualitative insights, showcasing the potential of LLM-driven optimization.

Conclusion: LLM-driven agents can democratize and accelerate GPU kernel optimization, especially in resource-constrained or rapidly evolving hardware environments.
...
TL;DR: The paper introduces a method to deploy LSTMs on FPGAs using the FINN framework, addressing computational complexity and enabling efficient, quantized LSTM acceleration.


<details>
  <summary>Details</summary>
Motivation: The computational complexity of LSTMs hinders real-time deployment in resource-constrained environments, and existing tools lack generalized support for LSTM acceleration on FPGAs.

Method: The authors leverage the FINN framework and ONNX's Scan operator to model LSTM computations, enabling mixed quantization and functional verification. Custom transformations map quantized ONNX graphs to hardware blocks.

Result: The proposed flow generates a quantized ConvLSTM accelerator that balances performance and resource use, matching or outperforming state-of-the-art models in accuracy.

Conclusion: The method paves the way for resource-efficient RNN accelerators on FPGAs, demonstrating practical viability for tasks like stock prediction.
...
TL;DR: Hellsemble is a novel ensemble framework for binary classification that improves accuracy and efficiency by partitioning data based on difficulty and routing instances to specialized models.
...
Method: Hellsemble partitions data into difficulty levels, trains specialized models on increasingly challenging subsets, and uses a router model to assign instances.

Result: Hellsemble outperforms classical ensemble methods on OpenML-CC18 and Tabzilla benchmarks.
...
TL;DR: A novel method for detecting adversarial examples in DNNs by analyzing layer-wise impact of attacks, using a lightweight regression model for efficient real-time detection.
...
Method: Trains a lightweight regression model to predict deeper-layer features from early-layer features, using prediction error to detect adversarial samples.
...
Conclusion: The proposed detection method offers a practical and efficient solution for identifying adversarial examples in real-time.
...
TL;DR: The paper explores distributed GNN training for link prediction, identifying performance degradation due to graph partitioning and negative sampling. It proposes SpLPG, a method using graph sparsification to reduce communication costs while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Most distributed GNN frameworks focus on node classification, leaving link prediction underexplored. Performance degradation in distributed training for link prediction is a key issue.

Method: The paper investigates the causes of performance degradation and proposes SpLPG, leveraging graph sparsification to reduce communication costs without sacrificing accuracy.

Result: SpLPG reduces communication overhead by up to 80% while mostly preserving link prediction accuracy, as shown in experiments on real-world datasets.

Conclusion: SpLPG effectively addresses performance degradation in distributed GNN training for link prediction, balancing accuracy and communication efficiency.
...
TL;DR: A novel constrained deep reinforcement learning (CDRL) approach optimizes time allocation in dual-functional radar-communication systems for tracking and communication under time constraints.


<details>
  <summary>Details</summary>
Motivation: To enhance communication quality in dynamic environments by efficiently allocating time between tracking multiple targets and data transmission.

Method: Proposes a CDRL framework to optimize resource allocation under time budget constraints.

Result: Demonstrates improved communication quality while adhering to time constraints in dynamic settings.

Conclusion: The CDRL approach effectively balances tracking and communication tasks, maximizing performance in integrated sensing and communication systems.
...
Result: SAC showed better stability and sample efficiency than DDPG; NSGA-II provided an upper bound on the Pareto front.

Conclusion: The approach enhances adaptive cognitive radar systems by efficiently balancing competing objectives.
...
TL;DR: The paper examines memorization in LoRA fine-tuning of LLMs, finding it reduces memorization risks compared to full fine-tuning while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: To understand memorization impacts in LoRA fine-tuning, diverging from prior findings in pre-training and full fine-tuning.
...
Conclusion: LoRA is a safer fine-tuning method for LLMs, mitigating memorization vulnerabilities.
...
<details>
  <summary>Details</summary>
Motivation: To leverage LLMs for predicting GPU kernel performance metrics efficiently, eliminating the need for traditional profiling methods.

Method: Omniwise is a model-agnostic, lightweight pipeline that predicts performance metrics (e.g., memory bandwidth, GFLOPs) directly from kernel code.

Result: Over 90% of predictions are within 10% relative error on AMD MI250 and MI300X GPUs.

Conclusion: Omniwise successfully integrates LLMs into performance profiling, offering a practical tool for developers via an online server and IDE plugin.
...
<details>
  <summary>Details</summary>
Motivation: To address the high cost of full retraining and the failure of existing unlearning methods to mimic retrained models, especially for user deletion rights and bias mitigation.

Method: Proposes RWFT, a probability mass redistribution technique for predictions on the forgotten class, robust to membership inference attacks (MIA-NN). Introduces a TV-based metric for residual leakage.

Result: Matches full retraining results in prior metrics and outperforms state-of-the-art methods by 2.79% in old metrics and 111.45% in the new TV-based metric.

Conclusion: RWFT effectively unlearns classes without retraining, is robust to attacks, and sets a new standard with its TV-based metric.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of computational complexity and poor performance due to irrelevant models in multi-model online conformal prediction.
...
Conclusion: The proposed algorithm improves efficiency and performance, validated by experiments on real and synthetic datasets.
...
Result: They achieve the first fully single-policy sample complexity bound for average-reward offline RL and handle general weakly communicating MDPs.

Conclusion: The work highlights the necessity of coverage assumptions beyond the target policy's stationary distribution and provides nearly matching lower bounds.
...
TL;DR: The paper introduces DL-LIME, a modified LIME method integrating deep learning, to improve explainability in deep reinforcement learning for radar resource management, outperforming conventional LIME.


<details>
  <summary>Details</summary>
Motivation: Address the 'black box' nature of neural networks in deep reinforcement learning for radar resource management by enhancing explainability with a modified LIME approach.

Method: Propose DL-LIME, integrating deep learning into LIME's sampling process to account for feature correlations, applied in deep reinforcement learning for radar resource management.

Result: DL-LIME outperforms conventional LIME in fidelity and task performance, offering insights into decision-making factors for radar resource management.

Conclusion: DL-LIME enhances explainability and performance in deep reinforcement learning for radar resource management, providing valuable insights for decision-making.
...
<details>
  <summary>Details</summary>
Motivation: Traditional optimization methods are impractical when operational constraints are unclear, requiring subjective heuristics. This work aims to automate constraint inference and optimization.

Method: Uses a multi-agent LLM framework (AutoGen with OpenAI's o3 model) for constraint generation, validation, simulation, and optimization, eliminating the need for predefined bounds.

Result: Demonstrated competitive performance in hydrodealkylation, with a 31-fold speedup over grid search and convergence in under 20 minutes.

Conclusion: The framework is effective for poorly characterized constraints, showing promise for emerging processes and retrofits.
...
<details>
  <summary>Details</summary>
Motivation: Traditional rule ensembles rely on axis-parallel decision regions, which may require many complex rules for accuracy, reducing interpretability. The goal is to enhance expressiveness without sacrificing interpretability.

Method: The method extends rule ensembles with propositions of the form $\mathbf{x}^\mathrm{T}\mathbf{w} \geq t$, using learnable sparse weight vectors. It employs sequential greedy optimization based on iteratively reweighted logistic regression.

Result: Experiments show the method achieves comparable test risk to state-of-the-art methods while significantly reducing model complexity across ten benchmark datasets.
...
<details>
  <summary>Details</summary>
Motivation: Large language models often contain problematic data (private, copyrighted, or inaccurate), and retraining is computationally expensive. Unlearning algorithms aim to address this but face challenges in precisely undoing datapoint influence.

Method: The proposed MSA algorithm uses model checkpoints (snapshots of model states during pretraining) to estimate and undo the influence of specific datapoints.

Result: MSA outperforms existing unlearning algorithms across benchmarks, models, and metrics, demonstrating its effectiveness.

Conclusion: MSA offers a promising solution for flexible data erasure in large language models, reducing computational costs compared to retraining.
...
<details>
  <summary>Details</summary>
Motivation: Current computational methods for antibody design struggle with geometric feature capture and generalization to novel antigen interfaces.
...
TL;DR: SharpZO is a hybrid method for fine-tuning VLMs without backpropagation, combining sharpness-aware evolutionary strategies and zeroth-order optimization to improve performance on edge devices.


<details>
  <summary>Details</summary>
Motivation: Current BP-free fine-tuning methods for VLMs often underperform due to reliance on high-variance evolutionary strategies or zeroth-order optimization, limiting their effectiveness.

Method: SharpZO uses a two-stage process: sharpness-aware ES for global exploration and smoothing, followed by fine-grained local search via sparse ZO optimization, all without backpropagation.

Result: SharpZO achieves up to 7% average accuracy gain over state-of-the-art forward-only methods, with improved convergence speed.

Conclusion: SharpZO effectively enhances ZO VLM fine-tuning, making it viable for memory-constrained edge devices while outperforming existing methods.
...
TL;DR: The paper explores knowledge distillation techniques to enhance the performance of smaller student normalizing flows, improving sampling quality and density estimation while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: Explicit density learners like normalizing flows offer advantages such as exact latent-variable inference and density estimation, but face challenges in training and sampling quality. This work aims to address these issues through knowledge distillation.

Method: The study introduces novel knowledge distillation techniques for compositional normalizing flows, leveraging their unique properties for non-traditional knowledge transfer between intermediate layers.

Result: The distillation process enables significant performance gains in smaller student models, with increased throughput due to reduced parameters.
...
<details>
  <summary>Details</summary>
Motivation: Prior works neglect textual and taxonomic data in molecular representation learning, limiting prediction accuracy.
...
Conclusion: Integrating multimodal data (SMILES, text, taxonomy) enhances molecular property prediction.
...
TL;DR: MoRA introduces a fine-grained Mixture-of-Rank Adaptive learning approach for continual learning, addressing interference, redundancy, and ambiguous routing by decomposing rank updates into rank-1 components and enabling sparse rank activation.


<details>
  <summary>Details</summary>
Motivation: Existing LoRA-based MoE approaches for continual learning suffer from interference, redundancy, and ambiguous routing, leading to degraded performance as tasks accumulate.
...
Conclusion: MoRA effectively addresses key challenges in continual learning by enabling fine-grained expert utilization and adaptive rank selection, outperforming existing approaches.
...
TL;DR: The paper analyzes federated learning (FL) under concept drift, proposes an algorithm to mitigate performance degradation, and validates it with experiments.


<details>
  <summary>Details</summary>
Motivation: Real-world data often arrives as streams with shifting distributions, causing concept drift and performance degradation in FL.

Method: The authors model concept drift as a Markov chain, introduce Stationary Generalization Error, and propose an algorithm regularizing empirical risk minimization with KL divergence and mutual information.

Result: The method outperforms existing approaches for periodic, gradual, and random drift patterns, as validated by experiments on an FL testbed with Raspberry Pi4 devices.
...
TL;DR: The paper introduces RL-Selector, a reinforcement learning-based method for efficient data selection in deep learning, outperforming existing methods by leveraging epsilon-sample cover to reduce redundancy.


<details>
  <summary>Details</summary>
Motivation: Training deep models on large datasets is computationally expensive due to redundancies, necessitating more efficient data selection methods.

Method: Proposes RL-Selector, a lightweight RL agent that optimizes data selection using epsilon-sample cover as a reward signal, capturing dataset dynamics.

Result: RL-Selector outperforms state-of-the-art baselines, improving training efficiency and model generalization.

Conclusion: The method effectively reduces redundancy and enhances performance, offering a scalable solution for data-efficient training.
...
TL;DR: SSE is a hierarchical RL framework enforcing strict subgoal reachability, improving exploration and subgoal reliability in long-horizon tasks.
...
Conclusion: SSE effectively tackles long-horizon RL challenges by ensuring subgoal feasibility and enhancing exploration.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for unsupervised skill discovery lack efficiency, especially in high-dimensional settings. The paper aims to address this by introducing a regret-aware approach.

Method: The method frames skill discovery as a min-max game between skill generation and policy learning, using regret to guide exploration and a learnable skill generator to avoid degeneration.
...
Conclusion: The regret-aware approach effectively balances exploration and policy learning, enhancing skill discovery in complex environments.
...
<details>
  <summary>Details</summary>
Motivation: Existing FL methods fail to handle all drift sources (real, virtual, label drift) optimally, leading to catastrophic forgetting. Distinguishing drift types is crucial for effective adaptation.

Method: FedDAA uses three modules: cluster number determination, real drift detection, and concept drift adaptation to dynamically adjust to drift while preserving useful historical data.

Result: FedDAA improves accuracy by 7.84%-8.52% over state-of-the-art methods on Fashion-MNIST, CIFAR-10, and CIFAR-100.

Conclusion: FedDAA effectively addresses multi-source concept drift in FL, outperforming existing methods by dynamically adapting and retaining valuable historical knowledge.
...
Conclusion: Knowledge graphs provide a robust solution for generating high-quality instruction data, improving LLM performance.
...
TL;DR: CHOOSE is a shallow Transformer framework enhanced with Chain of Thought (CoT) for wireless symbol detection, improving performance without increasing model depth.


<details>
  <summary>Details</summary>
Motivation: Deep Transformer models for wireless communication tasks are resource-intensive, making them unsuitable for mobile devices. CHOOSE aims to achieve comparable performance with shallow models.

Method: CHOOSE introduces autoregressive latent reasoning steps within the hidden space to enhance reasoning capacity in shallow (1-2 layer) Transformers.

Result: CHOOSE outperforms conventional shallow Transformers and matches deep models in performance while remaining efficient.
...
TL;DR: The paper introduces FeDa4Fair, a library for benchmarking fairness in Federated Learning (FL) under heterogeneous client biases, addressing limitations in current fairness solutions.


<details>
  <summary>Details</summary>
Motivation: Fairness in FL is challenged by biases in local datasets and heterogeneous data distributions, with existing solutions often limited to single, binary sensitive attributes.
...
Conclusion: FeDa4Fair enables robust and reproducible fairness research in FL by addressing diverse and conflicting fairness needs across clients.
...
TL;DR: H-CMR enhances interpretability in CBMs by providing transparency for both concept and task predictions using a hierarchical graph and logic rules.
...
Method: H-CMR uses a learned directed acyclic graph with logic rules to define concepts hierarchically, employing neural attention for rule selection during inference.
...
Conclusion: H-CMR advances CBMs by offering interpretability at both concept and task levels, enabling practical human-in-the-loop applications.
...
TL;DR: A novel architecture dynamically skips middle Transformer layers using learned gating, but fails to improve efficiency over dense baselines.
...
Method: Learned gating skips symmetric spans of central blocks; gated attention prevents attending to skipped positions. Residual norms and gate sparsity are controlled.

Result: No improvement in trade-off between validation cross-entropy and FLOPs compared to dense baselines.

Conclusion: The proposed method does not outperform simpler dense models at the tested scales.
...
TL;DR: A framework called Unlasting uses dual conditional diffusion models to address unpaired single-cell perturbation data, integrating GRN and masking for better predictions and introducing a biologically grounded evaluation metric.


<details>
  <summary>Details</summary>
Motivation: Single-cell sequencing is destructive, making paired data impossible. Existing methods either force pairings or ignore relationships, limiting accuracy.

Method: Proposes Dual Diffusion Implicit Bridges (DDIB) to map data distributions, integrates GRN for meaningful perturbation signals, and uses masking to predict silent genes. Introduces a new evaluation metric for bimodal distributions.

Result: Unlasting effectively handles unpaired data, improves prediction quality, and captures cell heterogeneity better.
...
TL;DR: An antifragile RL framework using discounted Thompson sampling (DTS) is proposed to enhance UAV navigation resilience against adversarial attacks by dynamically selecting robust policies.


<details>
  <summary>Details</summary>
Motivation: Existing robust RL methods lack generalization to out-of-distribution shifts, limiting their effectiveness against evolving adversarial attacks.
...
Result: The approach outperforms conventional robust RL methods, achieving shorter navigation paths and higher conflict-free trajectory rates in simulations.

Conclusion: The antifragile RL framework effectively adapts to unseen adversarial attacks, demonstrating superior performance in complex navigation scenarios.
...
Method: The framework uses a simulated attacker to incrementally increase perturbations, enforcing adaptation via Wasserstein distance minimization.

Result: The antifragile policy outperforms baselines, achieving higher rewards and fewer conflicts under PGD and GPS spoofing attacks.

Conclusion: The framework proves viable for secure RL in evolving threat environments, enhancing resilience and decision-making.
...
<details>
  <summary>Details</summary>
Motivation: Enhance fine-tuning efficiency for LLMs by reducing data and computational costs while maintaining performance.

Method: Split training data by entropy, apply SFT and distillation selectively, and validate with small models.

Result: Outperforms standard SFT (0.55 vs 0.43 accuracy) and matches distillation with 62% less data.

Conclusion: The method offers a cost-effective alternative to traditional fine-tuning, with potential for broader application.
...
TL;DR: The paper introduces Norm-Aware Linear Attention (NaLaFormer) to address limitations in linear attention mechanisms, improving performance on vision and language tasks.
...
Method: The approach decouples query and key matrices into norm and direction components, uses a query-norm aware kernel for dynamic entropy control, and employs norm-preserving mapping for non-negativity.

Result: NaLaFormer improves performance on vision and language tasks by up to 4.2%, enhancing expressiveness and efficiency.

Conclusion: The proposed method effectively addresses entropy gaps and norm consistency, demonstrating superior performance in linear attention mechanisms.
...
Result: Achieves 357x speedup over vanilla AllReduce for a 107B model on a 1Gbps network with minimal convergence impact.

Conclusion: DiLoCoX is the first successful decentralized framework for 100B+ parameter models, enabling scalable training on slow networks.
...
TL;DR: DBConformer, a dual-branch convolutional Transformer network, improves EEG decoding by integrating temporal and spatial features, outperforming baselines with fewer parameters and offering interpretable results.


<details>
  <summary>Details</summary>
Motivation: Existing CNN-based EEG decoding models struggle with long-range temporal dependencies and global inter-channel relationships, while hybrid models like Conformers lack optimal local-global feature integration and channel-wise modeling.
...
Result: Outperforms 10 baseline models on five MI and two seizure detection datasets, with eight times fewer parameters than EEG Conformer. Features are physiologically interpretable.

Conclusion: DBConformer is reliable for robust and explainable EEG decoding, with superior performance and interpretability.
...
TL;DR: The paper proposes a cGAN-based framework to craft stealthy adversarial attacks on UAV IDS and a CVAE method to detect them, outperforming traditional detectors.


<details>
  <summary>Details</summary>
Motivation: Traditional IDS methods fail to detect novel or stealthy adversarial attacks, necessitating advanced solutions.

Method: A cGAN generates adversarial samples, and a CVAE detects them using negative log-likelihood.

Result: CVAE-based detection outperforms traditional methods in identifying stealthy attacks.

Conclusion: Advanced probabilistic modeling is crucial for robust IDS against generative-model-based cyber threats.
...
Conclusion: pFedDC effectively handles heterogeneity in FL by leveraging dual-prompt learning and cross fusion.
...
TL;DR: The paper introduces Latent Prototype Routing (LPR), a novel routing framework for Mixture-of-Experts (MoE) architectures to address load imbalance, improving expert utilization without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: Current MoE systems face severe load imbalance, where only a few experts are activated, wasting model capacity and computational resources.

Method: The authors propose LPR, a routing framework based on clustering, to generalize existing approaches and promote balanced expert utilization.
...
Conclusion: LPR effectively addresses load imbalance in MoE systems, enhancing resource utilization while maintaining performance.
...
<details>
  <summary>Details</summary>
Motivation: Augment existing neural network compression by leveraging linear behavior in ReLU-like activated neurons.

Method: Uses linearity theory to merge layers in neural networks, evaluated experimentally.

Result: Achieves lossless compression to 1/4 of original size, minimal interference with other methods.

Conclusion: Introduces a new compression type for smaller, more efficient neural networks.
...
<details>
  <summary>Details</summary>
Motivation: Evaluating instances (e.g., human feedback, simulations) is costly, especially in reinforcement learning. Diverse mini-batches can enhance exploration and mitigate mode collapse.

Method: Proposes using determinantal point processes for diverse mini-batch selection in reinforcement learning, tested in drug discovery.

Result: The framework improves diversity and maintains high-quality solutions in drug discovery, as shown in experiments with molecular generation oracles.

Conclusion: Diverse mini-batch selection can accelerate drug discovery by yielding diverse, high-quality solutions, addressing unmet medication needs faster.
...
TL;DR: ScalaBL introduces a scalable Bayesian low-rank adaptation method for LLMs, reducing parameter overhead while maintaining performance.
...
Conclusion: ScalaBL offers an efficient and scalable solution for Bayesian uncertainty quantification in LLMs.
...
TL;DR: Perpetual voting fairness is improved by Artificial Delegates, which compensate for absenteeism and enhance representativeness.


<details>
  <summary>Details</summary>
Motivation: Address fairness in sequential collective decision-making by tackling partial turnout issues in perpetual voting.
...
Result: Artificial Delegates mitigate fairness issues caused by absenteeism and improve robustness.

Conclusion: Artificial Delegates effectively enhance fairness and representativeness in perpetual voting despite partial participation.
...
TL;DR: The paper proposes a zero-shot learning approach using large language models to predict electronic component obsolescence risk, addressing data limitations and demonstrating effectiveness on real-world datasets.


<details>
  <summary>Details</summary>
Motivation: Component obsolescence in industries reliant on electronics leads to increased costs and system disruptions, but accurate risk prediction is challenging due to unreliable data.

Method: The approach uses zero-shot learning with large language models to leverage domain-specific knowledge from tabular datasets for forecasting obsolescence risk.

Result: The method effectively predicts risk on two real-world datasets, and a comparison of four LLMs highlights the importance of model selection for specific tasks.

Conclusion: The proposed ZSL approach with LLMs offers a viable solution for obsolescence risk prediction, emphasizing the need for careful model choice.
...
TL;DR: The paper introduces novel initialization methods for k-means and k-GMM clustering, focusing on seed sampling and selection metrics, and demonstrates consistent improvements over existing techniques.


<details>
  <summary>Details</summary>
Motivation: To improve clustering performance by formalizing and enhancing the seeding process for k-means and k-GMM, addressing limitations of current methods.

Method: Proposes lookahead-based seed selection and multipass strategies to refine seeding, leveraging enhanced coherence with final metrics.

Result: Experiments show constant factor improvements in SSE (k-means) and log-likelihood (k-GMM) over classical methods, with modest overhead.
...
TL;DR: The paper introduces AGTCNet, a novel graph-temporal model for MI-EEG classification, addressing challenges in subject- and session-invariant BCI systems. It outperforms existing methods with reduced model size and faster inference.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of current BCI systems in handling spatiotemporal dependencies in EEG signals and variability across individuals and sessions.

Method: Proposes AGTCNet, integrating graph convolutional attention network (GCAT) to learn spatiotemporal EEG representations using EEG electrode topography as inductive bias.

Result: AGTCNet achieved state-of-the-art performance: 66.82% subject-independent accuracy (improving to 82.88% when fine-tuned) on BCI Competition IV Dataset 2a, and 64.14%/85.22% (4-class/2-class) on EEG Motor Movement/Imagery Dataset.

Conclusion: AGTCNet is effective and practical for BCI deployment, offering superior performance with a compact architecture.
...
TL;DR: DynamicBench is a new benchmark for LLMs focusing on real-time data processing, outperforming GPT4o by 7.0% and 5.8% in document-free and document-assisted scenarios.


<details>
  <summary>Details</summary>
Motivation: Traditional benchmarks lack dynamic evaluation for real-time information processing, necessitating a new benchmark like DynamicBench.
...
Conclusion: DynamicBench effectively measures LLMs' real-time data processing and report generation, with plans to release code and data.
...
<details>
  <summary>Details</summary>
Motivation: Downstream tools like heat-kernel signatures and spectral neural networks rely on persistent Laplacian eigenvalues, but the precise change in eigenvalues when a simplex is added was unknown.

Method: The authors prove a uniform Lipschitz bound, showing that adding one simplex changes eigenvalues by at most twice the Euclidean norm of the simplex's boundary.

Result: The result provides eigenvalue-level robustness guarantees for spectral topological data analysis, ensuring stability under local updates.

Conclusion: This work closes a gap in understanding eigenvalue changes, enabling reliable applications in dynamic data settings.
...
<details>
  <summary>Details</summary>
Motivation: To explore multimodal in-context learning (ICL) in medicine, where clinicians adapt to tasks with limited examples, and assess MLLMs' capabilities in this domain.

Method: Introduced SMMILE, a benchmark with 111 expert-curated problems (517 triplets) across 6 specialties and 13 modalities, and SMMILE++ with 1038 permuted problems. Evaluated 15 MLLMs.

Result: Most MLLMs show poor multimodal ICL ability in medicine. ICL improves performance by only 8-9.4%. Irrelevant examples degrade performance by up to 9.5%, and recency bias affects example ordering.

Conclusion: Current MLLMs have critical limitations and biases in learning multimodal medical tasks from context, highlighting the need for improved models.
...
TL;DR: The paper explores using fine-grained error resilience in DNNs with hardware approximation techniques to boost energy efficiency, achieving up to 54% energy savings with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: To address the energy inefficiency of DNNs while maintaining accuracy by leveraging hardware approximation techniques.
...
Conclusion: Fine-grained hardware approximation effectively balances energy efficiency and accuracy in low-power DNN computing.
...
Method: Propose NANOADAM, which updates only small-magnitude weights during finetuning. This approach is gradient-free, preserves critical pretrained features, and allows larger learning rates.

Result: NANOADAM improves generalization performance in NLP and vision tasks, reducing computational costs and catastrophic forgetting.

Conclusion: NANOADAM offers an efficient and effective finetuning strategy by leveraging the correlation between gradients and weights.
...
TL;DR: Proposes ATGAT, a graph neural network for cryptocurrency fraud detection, improving AUC by 9.2% over XGBoost.


<details>
  <summary>Details</summary>
Motivation: Addresses complex transaction patterns and class imbalance in fraud detection.

Method: Uses temporal embedding, triple attention, and weighted BCE loss.

Result: Achieves 0.9130 AUC, outperforming XGBoost, GCN, and GAT.

Conclusion: ATGAT enhances fraud detection and is generalizable to other temporal graph tasks.
...
Method: Dynamically evaluate stopping in-context learning after each Transformer layer and decode embeddings using a pre-trained layer-wise decoder.
...
TL;DR: The paper introduces D-CHAG, a distributed method for efficient aggregation of multi-channel images in vision transformers, reducing memory usage and improving throughput.


<details>
  <summary>Details</summary>
Motivation: Current distributed methods struggle with the compute-intensive task of tokenizing and aggregating images from diverse sources, limiting the potential of vision-based scientific foundation models.

Method: Proposes Distributed Cross-Channel Hierarchical Aggregation (D-CHAG), compatible with any model-parallel strategy and vision transformer architecture, to enhance computational efficiency.

Result: D-CHAG achieved up to 75% memory reduction and more than doubled throughput on 1,024 AMD GPUs in hyperspectral imaging and weather forecasting tasks.

Conclusion: D-CHAG significantly improves efficiency for vision-based scientific models, enabling scalable and high-performance applications.
...
TL;DR: SSCP is a generative policy for RL that enables efficient one-shot action generation, combining expressiveness with training stability and speed.
...
Result: Achieves strong performance in offline RL benchmarks, offering speed and adaptability over diffusion-based methods.

Conclusion: SSCP is a versatile, efficient framework for RL, suitable for various settings and scalable to goal-conditioned tasks.
...
Method: Compared early and late fusion approaches using audio (speech) and video (Action Units, gaze) data from Swedish native speakers in truth/lie scenarios.
...
Conclusion: The study supports psychological theories on differential control of facial and vocal expressions and provides a foundation for future research in dyadic interactions, including psychotherapy.
...
TL;DR: A training method for ResNets links optimal control theory to neural networks, penalizing hidden states to prune unnecessary layers.


<details>
  <summary>Details</summary>
Motivation: To bridge optimal control theory and neural network training, enabling theory-grounded layer pruning.
...
Conclusion: The approach offers a theoretical basis for layer pruning in ResNets.
...
TL;DR: The paper proposes an electronic system for evaluating subjective answer scripts by analyzing keywords, checking grammar/spelling, and comparing with parsed keywords from open/closed domains. Tested on 100 scripts, it achieved a precision score of 0.91.
...
Method: An integrated system that extracts keywords from answer scripts, compares them with parsed keywords from open/closed domains, and checks for grammatical/spelling errors.

Result: Tested on 100 student answer scripts, the system achieved a precision score of 0.91.

Conclusion: The proposed system is effective for electronic evaluation of subjective answers, offering high precision and potential for broader application.
...
TL;DR: A hybrid Genetic Algorithm and Reinforcement Learning approach optimizes low-storage Extended Stability Runge-Kutta methods, improving computational efficiency by 25% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Balancing accuracy, stability, and efficiency in high-order, low-storage Runge-Kutta methods is challenging, requiring automated heuristic discovery.
...
Result: Achieves 25% runtime reduction in benchmark tests (Brusselator systems, Navier-Stokes) without compromising stability or accuracy.

Conclusion: Demonstrates adaptive heuristic discovery's potential to enhance resource efficiency in simulations, paving the way for further research in Deep RL and AutoML.
...
TL;DR: The paper addresses low cancer screening rates in Telangana by developing ML models to predict susceptibility to breast and cervical cancer, along with a system to locate nearby treatment centers and raise awareness.


<details>
  <summary>Details</summary>
Motivation: Low awareness and screening rates for cervical and breast cancer in Telangana, despite early detection being crucial for reducing mortality.

Method: Developed ML classification models (decision tree for cervical cancer, SVM for breast cancer) and a system to suggest nearby hospitals/treatment centers, integrating health records and awareness campaigns.

Result: A predictive system for cancer susceptibility and a framework to improve access to treatment and awareness.

Conclusion: The solution aims to enhance cancer literacy, reduce mortality, and improve screening rates in Telangana.
...
<details>
  <summary>Details</summary>
Motivation: Manual fault modeling in CPS is complex and error-prone, requiring domain expertise. The paper aims to automate this process for better accuracy and interpretability.

Method: Combines multivariate time-series analysis for anomaly detection, process mining for model extraction, and stochastic simulation for behavioral understanding.

Result: Validated on the Robotic Arm Dataset, the method effectively models and classifies faults, supporting predictive maintenance and digital twins.

Conclusion: The approach enhances fault diagnosis in CPS by automating model creation and improving interpretability, aiding in predictive maintenance and digital twin development.
...
<details>
  <summary>Details</summary>
Motivation: MTS-AD is challenging due to complex dependencies, temporal dynamics, and sparse labels, necessitating a standardized benchmark.

Method: mTSBench evaluates 24 anomaly detection methods, including LLM-based detectors, across 344 labeled time series from 19 datasets.

Result: No single detector excels universally, and state-of-the-art model selection methods are suboptimal.

Conclusion: mTSBench enables reproducible comparisons and advances in adaptive anomaly detection and model selection.
...
<details>
  <summary>Details</summary>
Motivation: To understand the mysterious phenomenon of grokking (delayed generalization) in LLMs and demystify its mechanisms.

Method: Analyzes checkpoints during pretraining of a 7B LLM (OLMoE), evaluates generalization on diverse tasks, and studies pathway evolution using novel metrics.

Result: Grokking occurs in large-scale pretraining; pathways evolve from random to structured, reducing complexity and improving generalization.

Conclusion: Pathway metrics predict generalization, offering practical monitoring tools and theoretical insights into model complexity.
...
### [243] [Uncertainty-Aware Machine-Learning Framework for Predicting Dislocation Plasticity and Stress-Strain Response in FCC Alloys](https://arxiv.org/abs/2506.20839)
*Jing Luo,Yejun Gu,Yanfei Wang,Xiaolong Ma,Jaafar. A El-Awady*

Main category: cond-mat.mtrl-sci

TL;DR: A machine learning approach using a mixed density network (MDN) model predicts dislocation density and stress distributions, improving accuracy and uncertainty quantification in material property predictions.


<details>
  <summary>Details</summary>
Motivation: To enhance the understanding and application of structural materials by integrating existing data and quantifying uncertainties in predictive modeling.

Method: Utilizes a mixed density network (MDN) model trained on experimental data to predict dislocation density and stress distributions, incorporating statistical parameters into a plasticity model.

Result: Accurate stress-strain predictions with explicit uncertainty quantification, improving reliability and aiding alloy design.

Conclusion: The methodology enhances mechanical property predictions and supports the development of new materials in the industry.
...
TL;DR: The paper introduces CovDocker, a benchmark for covalent docking, addressing gaps in existing methods by focusing on covalent bond formation and structural changes. It decomposes the process into three tasks and uses advanced models to establish baselines, proving its effectiveness in predicting interaction sites and molecular transformations.


<details>
  <summary>Details</summary>
Motivation: Existing docking methods and deep learning approaches often overlook covalent bond formation and structural changes, limiting their applicability in covalent drug design.

Method: The authors decompose covalent docking into reactive location prediction, covalent reaction prediction, and covalent docking tasks. They adapt state-of-the-art models like Uni-Mol and Chemformer to establish baseline performances.

Result: The benchmark effectively predicts interaction sites and models molecular transformations, demonstrating its utility in covalent drug design.

Conclusion: CovDocker serves as a rigorous framework for advancing covalent drug design research, highlighting the potential of data-driven approaches to accelerate the discovery of selective covalent inhibitors.
...
### [245] [MegaFold: System-Level Optimizations for Accelerating Protein Structure Prediction Models](https://arxiv.org/abs/2506.20686)
*Hoa La,Ahan Gupta,Alex Morehead,Jianlin Cheng,Minjia Zhang*

Main category: q-bio.BM

TL;DR: MegaFold accelerates AlphaFold3 training by optimizing compute and memory usage, enabling longer sequence training without out-of-memory errors.
...
Method: MegaFold uses ahead-of-time caching, Triton-based kernels for EvoAttention, and deep fusion for small operators.

Result: MegaFold reduces peak memory usage by 1.23×, speeds up training by up to 1.73×, and allows 1.35× longer sequences.
...
### [246] [Hybrid Deep Learning and Signal Processing for Arabic Dialect Recognition in Low-Resource Settings](https://arxiv.org/abs/2506.21386)
...
Result: MFCC+CNN achieved 91.2% accuracy, outperforming DWT+RNN (66.5%). Spectral features with CNNs proved more effective.

Conclusion: The study sets a baseline for Arabic dialect recognition in low-resource scenarios, recommending larger datasets, self-supervised learning, and advanced architectures like Transformers for future work.
...
### [247] [Performance improvement of spatial semantic segmentation with enriched audio features and agent-based error correction for DCASE 2025 Challenge Task 4](https://arxiv.org/abs/2506.21174)
*Jongyeon Park,Joonhee Lee,Do-Hyeon Lim,Hong Kook Kim,Hyeongcheol Geum,Jeong Eun Lim*

Main category: eess.AS

TL;DR: The paper introduces a submission system for DCASE 2025 Challenge Task 4, enhancing audio-tagging models with additional features, label correction, and dataset refinement to improve classification performance.


<details>
  <summary>Details</summary>
Motivation: Mixed audio contains subtle cues not fully captured by mel-spectrograms alone, necessitating additional features and corrections.
...
Result: Achieves up to 14.7% relative improvement in CA-SDRi over the baseline.

Conclusion: The proposed methods effectively enhance audio-tagging performance, demonstrating significant improvements in the DCASE 2025 Challenge.
...
TL;DR: ThinkSound is a novel framework using Chain-of-Thought reasoning for stepwise video-to-audio generation, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Producing high-fidelity audio that authentically captures visual nuances is challenging, requiring sophisticated reasoning about visual dynamics, acoustics, and temporal relationships.

Method: ThinkSound decomposes the process into three stages: foundational foley generation, interactive object-centric refinement, and targeted editing guided by natural language. It uses a multimodal large language model for CoT reasoning and a unified audio foundation model.

Result: ThinkSound achieves state-of-the-art performance in video-to-audio generation, excelling in both audio and CoT metrics, and performs well on the Movie Gen Audio benchmark.

Conclusion: ThinkSound demonstrates the effectiveness of CoT reasoning in video-to-audio generation, offering a structured and interactive approach for high-fidelity sound synthesis.
...
### [249] [IMC-PINN-FE: A Physics-Informed Neural Network for Patient-Specific Left Ventricular Finite Element Modeling with Image Motion Consistency and Biomechanical Parameter Estimation](https://arxiv.org/abs/2506.20696)
...
<details>
  <summary>Details</summary>
Motivation: Traditional FE methods are slow and often inaccurate for cardiac motion analysis, necessitating a more efficient and precise approach.

Method: Combines imaged motion consistency (IMC) with PINN-FE, using MRI or echocardiography data to estimate motion and stiffness, then performs rapid FE modeling.
...
Conclusion: IMC-PINN-FE provides a faster, more accurate, and patient-specific solution for cardiac biomechanical modeling.
...
### [250] [ClusterRCA: Network Failure Diagnosis in HPC Systems Using Multimodal Data](https://arxiv.org/abs/2506.20673)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fail to address data heterogeneity and accuracy in HPC systems, necessitating a new approach.
...
### [251] [Utility-Driven Speculative Decoding for Mixture-of-Experts](https://arxiv.org/abs/2506.20675)
*Anish Saxena,Po-An Tsai,Hritvik Taneja,Aamer Jaleel,Moinuddin Qureshi*

Main category: cs.DC

TL;DR: Speculative decoding, effective in dense LLMs, is inefficient for MoE models due to increased data movement and verification time. Cascade, a utility-driven framework, dynamically adjusts speculation to optimize MoE serving.


<details>
  <summary>Details</summary>
Motivation: GPU memory bandwidth limits LLM inference latency. Speculative decoding improves throughput in dense LLMs but fails in MoE models due to higher overhead.

Method: Cascade introduces a utility-driven approach, dynamically enabling speculation and tuning the number of draft tokens (K) based on a utility metric.

Result: Cascade reduces slowdowns to 5% and boosts throughput by 7-14% compared to static K, making speculative decoding viable for MoEs.

Conclusion: Cascade makes speculative decoding practical for MoE models by dynamically optimizing utility, addressing inefficiencies in traditional approaches.
...
### [252] [Transformer-Based Spatial-Temporal Counterfactual Outcomes Estimation](https://arxiv.org/abs/2506.21154)
...
Result: Simulation experiments show superior estimation capability over baselines; real data experiments reveal causal effects of conflicts on forest loss in Colombia.

Conclusion: The proposed framework effectively addresses limitations of classical methods, validated by simulations and real-world data.
...
### [253] [Automatic Reviewers Assignment to a Research Paper Based on Allied References and Publications Weight](https://arxiv.org/abs/2506.21331)
*Tamim Al Mahmud,B M Mainul Hossain,Dilshad Ara*

Main category: cs.DL

TL;DR: The paper proposes an automated system to select the best reviewers for research papers by analyzing references, extracting keywords, and ranking authors based on metrics like h-index and citations.


<details>
  <summary>Details</summary>
Motivation: The challenge of efficiently selecting expert reviewers for the growing volume of research papers across emerging fields motivates the need for an automated solution.

Method: The method involves analyzing references, extracting topic keywords, ranking authors using metrics (h-index, i10-index, citations), and filtering based on co-authorship.

Result: The system identifies top-ranked authors (professors) as potential referees by eliminating conflicts of interest and prioritizing expertise.

Conclusion: Automated reviewer selection can improve efficiency and accuracy in peer review by leveraging data-driven metrics and eliminating biases.
...
### [254] [Spiking Neural Networks for SAR Interferometric Phase Unwrapping: A Theoretical Framework for Energy-Efficient Processing](https://arxiv.org/abs/2506.20782)
*Marc Bara*

Main category: cs.NE

TL;DR: First theoretical framework for using spiking neural networks (SNNs) in SAR interferometric phase unwrapping, offering energy-efficient processing and comparable accuracy.


<details>
  <summary>Details</summary>
Motivation: Addressing the gap in applying SNNs to phase unwrapping, especially with growing Earth observation data volumes and the need for energy-efficient solutions.

Method: Developed spike encoding for wrapped phase data, proposed SNN architectures leveraging spatial propagation, and analyzed computational complexity and convergence.

Result: SNNs offer 30-100x energy savings over conventional methods while maintaining accuracy, naturally modeling spatial continuity constraints.
...
### [255] [Stochastic Quantum Spiking Neural Networks with Quantum Memory and Local Learning](https://arxiv.org/abs/2506.21324)
*Jiechen Chen,Bipin Rajendran,Osvaldo Simeone*

Main category: cs.NE

TL;DR: The paper proposes a stochastic quantum spiking (SQS) neuron model combining neuromorphic and quantum computing strengths, enabling efficient event-driven probabilistic spiking and scalable training without classical backpropagation.


<details>
  <summary>Details</summary>
Motivation: To address limitations in existing quantum spiking models, such as reliance on classical memory and repeated measurements, by integrating neuromorphic efficiency with quantum state space advantages.

Method: Introduces the SQS neuron using multi-qubit circuits for internal quantum memory and event-driven spiking, and outlines SQSNNs trained via a local learning rule.

Result: The SQS neuron enables single-shot probabilistic spike generation and scalable training, merging neuromorphic time-series efficiency with quantum state space.

Conclusion: The SQSNN model offers a modular, scalable, and quantum-hardware-trainable approach, advancing hybrid neuromorphic-quantum computing.
...
### [256] [On Uniform Weighted Deep Polynomial approximation](https://arxiv.org/abs/2506.21306)
...
### [257] [Poster: Enhancing GNN Robustness for Network Intrusion Detection via Agent-based Analysis](https://arxiv.org/abs/2506.20806)
*Zhonghao Zhan,Huichi Zhou,Hamed Haddadi*

Main category: cs.CR

TL;DR: The paper proposes using Large Language Models (LLMs) as simulated cybersecurity experts to enhance the robustness of Graph Neural Networks (GNNs) in Network Intrusion Detection Systems (NIDS), addressing issues like distribution drift and adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: GNNs in NIDS face performance degradation due to distribution drift and lack robustness against adversarial attacks, with current evaluations relying on unrealistic synthetic perturbations.

Method: The approach integrates LLMs in an agentic pipeline to analyze graph structures from network flow data, identifying and mitigating adversarial elements before GNN processing.

Result: Experiments show LLM integration significantly improves GNN resilience against adversarial attacks, validated using a realistic dataset.

Conclusion: LLMs can serve as a complementary layer in NIDS architectures, enhancing GNN robustness and generalization.
...
### [258] [ZKPROV: A Zero-Knowledge Approach to Dataset Provenance for Large Language Models](https://arxiv.org/abs/2506.20915)
*Mina Namazi,Alexander Nemecek,Erman Ayday*

Main category: cs.CR

TL;DR: ZKPROV is a cryptographic framework for zero-knowledge proofs of LLM provenance, ensuring dataset integrity without revealing sensitive data.
...
Method: Uses zero-knowledge proofs to bind trained models to authorized datasets, avoiding full training verification.
...
### [259] [PhishKey: A Novel Centroid-Based Approach for Enhanced Phishing Detection Using Adaptive HTML Component Extraction](https://arxiv.org/abs/2506.21106)
...
<details>
  <summary>Details</summary>
Motivation: Addressing the rapid evolution of phishing attacks and the need for adaptable, robust, and efficient detection mechanisms.

Method: Uses CNN for URL classification and CAPE for HTML content, integrating predictions via soft-voting ensemble.

Result: Achieves up to 98.70% F1 Score and resists adversarial manipulations effectively.

Conclusion: PhishKey is a highly effective and robust solution for phishing detection.
...
### [260] [Empowering Digital Agriculture: A Privacy-Preserving Framework for Data Sharing and Collaborative Research](https://arxiv.org/abs/2506.20872)
*Osama Zafar,Rosemarie Santa González,Mina Namazi,Alfonso Morales,Erman Ayday*

Main category: cs.CR

TL;DR: A privacy-preserving framework for data-driven agriculture combines dimensionality reduction and differential privacy to enable secure data sharing, collaboration, and personalized ML models while protecting sensitive farmer data.


<details>
  <summary>Details</summary>
Motivation: Privacy concerns deter farmers from sharing agricultural data, limiting the potential of data-driven agriculture. The paper aims to address these concerns to foster collaboration and innovation.

Method: The framework uses PCA for dimensionality reduction and Laplacian noise for differential privacy. It supports federated learning and direct training on aggregated, privacy-protected data.

Result: Validated on real datasets, the framework provides robust privacy protection against adversarial attacks and maintains utility comparable to centralized systems.

Conclusion: The framework facilitates secure data integration, empowering researchers and policymakers to advance data-driven agriculture responsibly.
...
### [261] [Global and Local Contrastive Learning for Joint Representations from Cardiac MRI and ECG](https://arxiv.org/abs/2506.20683)
...
<details>
  <summary>Details</summary>
Motivation: ECGs lack functional cardiac parameters like ventricular volumes, while CMR, though detailed, is expensive and less accessible. PTACL aims to bridge this gap.

Method: PTACL uses global patient-level and local temporal-level contrastive losses to align ECG and CMR data, enriching ECG representations with CMR insights.
...
### [262] [U-R-VEDA: Integrating UNET, Residual Links, Edge and Dual Attention, and Vision Transformer for Accurate Semantic Segmentation of CMRs](https://arxiv.org/abs/2506.20689)
*Racheal Mukisa,Arvind K. Bansal*

Main category: eess.IV

TL;DR: The paper introduces U-R-Veda, a deep learning model combining UNet, vision transformers, and attention mechanisms for accurate cardiac image segmentation, achieving 95.2% accuracy.


<details>
  <summary>Details</summary>
Motivation: Automated and accurate segmentation of cardiac images is crucial for diagnosing and managing cardiac disorders, but existing methods lack precision.

Method: Proposes U-R-Veda, integrating UNet, vision transformers, residual links, channel-spatial attention, and edge-detection skip-connections for CMR image segmentation.

Result: Achieves 95.2% accuracy (DSC metrics), outperforming other models, especially in right-ventricle and left-ventricle-myocardium delineation.

Conclusion: U-R-Veda significantly improves CMR image segmentation, enhancing automated medical image analysis for cardiac disorders.
...
### [263] [A Novel Framework for Integrating 3D Ultrasound into Percutaneous Liver Tumour Ablation](https://arxiv.org/abs/2506.21162)
...
<details>
  <summary>Details</summary>
Motivation: To enhance 3D US adoption in liver tumour ablation by addressing tumour identification challenges in US images.
...
Result: Achieved 2-4 mm landmark error, 0.22s runtime, and 40% alignment error reduction with non-rigid registration.

Conclusion: The framework advances 3D US's therapeutic role in clinical interventions, improving ablation outcomes.
...
### [264] [Development of MR spectral analysis method robust against static magnetic field inhomogeneity](https://arxiv.org/abs/2506.20897)
*Shuki Maruyama,Hidenori Takeshima*

Main category: eess.IV

TL;DR: A deep learning model using modeled spectra improves spectral analysis accuracy under B0 inhomogeneity.


<details>
  <summary>Details</summary>
Motivation: To enhance spectral analysis accuracy in the presence of B0 inhomogeneity.

Method: Proposed a deep learning model trained on modeled spectra derived from B0 maps and metabolite ratios, evaluated against measured spectra.

Result: Modeled spectra closely matched measured spectra; the model reduced MSEs by 49.89% and outperformed LCModel in MAPEs.

Conclusion: The method improves spectral analysis accuracy by leveraging modeled spectra for training.
...
### [265] [Uncover Treasures in DCT: Advancing JPEG Quality Enhancement by Exploiting Latent Correlations](https://arxiv.org/abs/2506.21171)
...
<details>
  <summary>Details</summary>
Motivation: JPEG compression introduces artifacts, and existing enhancement methods in the pixel domain are computationally expensive. Direct DCT-domain enhancement is promising but limited in performance.
...
### [266] [GANet-Seg: Adversarial Learning for Brain Tumor Segmentation with Hybrid Generative Models](https://arxiv.org/abs/2506.21245)
*Qifei Cui,Xinyu Lu*
...
Method: Combines pre-trained GANs and Unet with global anomaly detection, refined mask generation, and adversarial loss constraints. Uses multi-modal MRI and synthetic augmentation.
...
### [267] [Exploring the Design Space of 3D MLLMs for CT Report Generation](https://arxiv.org/abs/2506.21535)
*Mohammed Baharoon,Jun Ma,Congyu Fang,Augustin Toma,Bo Wang*

Main category: eess.IV

TL;DR: The paper explores 3D MLLMs for radiology report generation, focusing on design choices like visual input, projectors, LLMs, and fine-tuning. It introduces knowledge-based methods, achieving 2nd place in a challenge, and shows performance insights on dataset size, volume, and segmentation masks.


<details>
  <summary>Details</summary>
Motivation: To systematically investigate and improve the design of 3D MLLMs for automating radiology report generation, addressing gaps in current methods.

Method: The study examines visual input representation, projectors, LLMs, and fine-tuning techniques for 3D CT reports. It introduces two knowledge-based augmentation methods.

Result: Achieved a 10% improvement in GREEN score, secured 2nd place in MICCAI 2024 challenge, and demonstrated that RRG performance is not strictly tied to LLM size or volume size.

Conclusion: Key findings include the effectiveness of segmentation masks, the limited impact of LLM size, and the importance of matching pre-training volume size. The code is publicly available.
...
### [268] [Lightweight Physics-Informed Zero-Shot Ultrasound Plane Wave Denoising](https://arxiv.org/abs/2506.21499)
*Hojat Asgariandehkordi,Mostafa Sharifzadeh,Hassan Rivaz*

Main category: eess.IV

TL;DR: A zero-shot denoising framework for low-angle CPWC ultrasound enhances contrast without training data, using self-supervised learning to reduce noise while preserving structures.


<details>
  <summary>Details</summary>
Motivation: CPWC improves image contrast but suffers from noise and blurring with limited angles, requiring a solution that avoids training datasets.

Method: Divides transmission angles into subsets to create noisy images, trains a lightweight deep model via self-supervised residual learning to separate noise from tissue signals.

Result: Outperforms classical and deep learning methods in contrast enhancement and structure preservation on simulation, phantom, and in vivo data.

Conclusion: The framework is adaptable, efficient, and effective for CPWC denoising without domain-specific tuning or paired data.
...
### [269] [Parallels Between VLA Model Post-Training and Human Motor Learning: Progress, Challenges, and Trends](https://arxiv.org/abs/2506.20966)
...
<details>
  <summary>Details</summary>
Motivation: VLA models show generalization in manipulation tasks but lack precision without adaptation. Post-training is critical to align models with downstream applications, akin to human motor skill acquisition.

Method: The paper introduces a taxonomy for post-training VLA models, focusing on environments, embodiments, and tasks, structured around human learning mechanisms: perception, awareness, comprehension, and integration.

Result: A structured framework is proposed to guide post-training of VLA models, addressing gaps in precision and accuracy for robotic manipulation.

Conclusion: The work provides a comprehensive overview and practical insights for VLA model development, identifying key challenges and future research directions.
...
### [270] [V2X-REALM: Vision-Language Model-Based Robust End-to-End Cooperative Autonomous Driving with Adaptive Long-Tail Modeling](https://arxiv.org/abs/2506.21041)
*Junwei You,Pei Li,Zhuoyu Jiang,Zilin Huang,Rui Gan,Haotian Shi,Bin Ran*

Main category: cs.RO

TL;DR: V2X-REALM is a vision-language model framework for robust cooperative autonomous driving in long-tail scenarios, featuring adaptive multimodal learning and outperforming baselines in robustness and safety.
...
Conclusion: V2X-REALM advances the scalability of end-to-end cooperative autonomous driving by effectively handling long-tail scenarios.
...
### [271] [WorldVLA: Towards Autoregressive Action World Model](https://arxiv.org/abs/2506.21539)
...
TL;DR: WorldVLA is an autoregressive action world model combining Vision-Language-Action (VLA) and world models to unify action and image understanding/generation, outperforming standalone models. It addresses error propagation in autoregressive action generation with an attention mask strategy.


<details>
  <summary>Details</summary>
Motivation: To unify action and image understanding/generation and improve action generation by learning environmental physics, while enhancing visual understanding through action generation.

Method: Integrates VLA and world models for joint action and image prediction. Uses an attention mask strategy to mitigate error propagation in autoregressive action generation.

Result: WorldVLA outperforms standalone models, showing mutual enhancement between action and world models. The attention mask strategy improves action chunk generation performance.

Conclusion: WorldVLA successfully unifies action and image tasks, with the attention mask strategy addressing autoregressive action generation limitations.
...
### [272] [Model-Based Real-Time Pose and Sag Estimation of Overhead Power Lines Using LiDAR for Drone Inspection](https://arxiv.org/abs/2506.20812)
*Alexandre Girard,Steven A. Parkison,Philippe Hamelin*

Main category: cs.RO

TL;DR: The paper proposes a method for accurately localizing drones relative to overhead power lines using LiDAR, despite challenges like minimal conductor surface and noise.


<details>
  <summary>Details</summary>
Motivation: Inspecting energized power lines with drones is simplified but faces LiDAR challenges like sparse conductor points and noise.
...
Result: The method achieves accurate tracking, converges under 50 ms per frame, and tolerates up to twice as many outliers as valid measurements.

Conclusion: The proposed estimation approach is robust and efficient for drone-based power line inspections.
...
### [273] [ThermalDiffusion: Visual-to-Thermal Image-to-Image Translation for Autonomous Navigation](https://arxiv.org/abs/2506.20969)
*Shruti Bansal,Wenshan Wang,Yifei Liu,Parv Maheshwari*

Main category: cs.RO

TL;DR: The paper proposes using synthetic thermal data to address the lack of thermal imagery in multi-modal datasets for robotics and automation, employing conditional diffusion models to convert RGB images to thermal images.


<details>
  <summary>Details</summary>
Motivation: Thermal cameras are valuable in degraded environments but lack sufficient data for robotics research. Existing datasets are inadequate for tasks like scene segmentation and object detection in thermal imagery.

Method: The paper uses conditional diffusion models with self-attention to learn thermal properties of objects and convert RGB images to synthetic thermal images.

Result: The proposed method enables the augmentation of datasets with synthetic thermal data, facilitating the adaptation of thermal cameras in robotics.
...
### [274] [Control and optimization for Neural Partial Differential Equations in Supervised Learning](https://arxiv.org/abs/2506.20764)
*Alain Bensoussan,Minh-Binh Tran,Bangjie Wang*

Main category: math.OC

TL;DR: The paper introduces a new research direction in control theory, focusing on optimizing and controlling coefficients of parabolic and hyperbolic operators, linking it to neural networks and supervised learning.


<details>
  <summary>Details</summary>
Motivation: The problem of controlling and optimizing coefficients in parabolic and hyperbolic systems is underexplored, despite its relevance to neural networks and supervised learning.

Method: The authors propose interpreting neural networks as PDEs, reformulating the control problem for ODEs into a PDE framework. They introduce a dual system formulation for parabolic PDEs and provide theoretical proofs for minimizers.

Result: Theoretical proofs confirm the existence of minimizers for parabolic PDE control problems and solutions for approximated hyperbolic PDE control problems.

Conclusion: This work lays the foundation for future research on efficient numerical schemes and further exploration of PDE-based control in neural networks.
...
### [275] [Faster Fixed-Point Methods for Multichain MDPs](https://arxiv.org/abs/2506.20910)
*Matthew Zurek,Yudong Chen*

Main category: math.OC

TL;DR: The paper presents improved value-iteration algorithms for multichain MDPs under the average-reward criterion, addressing navigational challenges and achieving faster convergence rates.


<details>
  <summary>Details</summary>
Motivation: The study aims to tackle the theoretical challenges of multichain MDPs, where optimal policies must navigate between connected components while optimizing long-run performance.

Method: The authors develop algorithms that better solve the navigational subproblem, leveraging novel connections between average-reward and discounted problems, and optimal fixed-point methods for discounted VI.

Result: The proposed algorithms achieve improved convergence rates and sharper complexity measures compared to prior work, with key components offering independent theoretical value.

Conclusion: The results advance the theoretical foundations of VI approaches, providing faster convergence for both discounted and average-reward problems in multichain MDPs.
...
### [276] [Structural System Identification via Validation and Adaptation](https://arxiv.org/abs/2506.20799)
*Cristian López,Keegan J. Moore*

Main category: math.DS

TL;DR: A new method for structural system identification and validation uses generative modeling to estimate parameters from data, validated by a discriminator network.
...
Method: A neural network maps noise to parameters, used in known equations to generate fake accelerations, compared to real data via MSE loss. Validation uses a discriminator network.

Result: Accurate parameter estimation and model validation demonstrated for nonlinear structural systems.

Conclusion: The method effectively combines data and theory for system identification and validation.
...
TL;DR: Temporal Fusion Transformers (TFTs) slightly outperform LSTMs in rainfall-runoff modeling, especially for midsections and peaks of hydrographs, and handle longer sequences better. TFT also provides explainable insights but both models struggle with the Caravan dataset.


<details>
  <summary>Details</summary>
Motivation: To compare the effectiveness of TFTs and LSTMs in hydrological predictions, particularly rainfall-runoff modeling, and assess their performance across diverse datasets.
...
Result: TFT slightly outperforms LSTM, excels in midsection and peak simulations, handles longer sequences better, and offers explainability. Both models perform poorly on the Caravan dataset.

Conclusion: TFT shows promise for hydrological modeling due to its performance and explainability, but data quality issues in the Caravan dataset affect both models.
...
### [278] [Exploring the Effects of Chatbot Anthropomorphism and Human Empathy on Human Prosocial Behavior Toward Chatbots](https://arxiv.org/abs/2506.20748)
...
<details>
  <summary>Details</summary>
Motivation: To understand factors motivating people to help chatbots, leveraging the CASA framework to examine anthropomorphism's role.

Method: An online experiment (N=244) where chatbots made mistakes in a collaborative task, measuring participants' prosocial behaviors and intentions.

Result: Human identity and emotional expression in chatbots increased prosocial behavior and intention, mediated by empathy. Qualitative analysis revealed empathy and human-like perception as key motivations.
...
### [279] [A Systematic Review of Human-AI Co-Creativity](https://arxiv.org/abs/2506.21333)
*Saloni Singh,Koen Hndriks,Drik Heylen,Kim Baraka*

Main category: cs.HC

TL;DR: A systematic review of 62 co-creative systems papers identifies key design dimensions and 24 considerations, emphasizing user control and proactive AI for better collaboration and trust.


<details>
  <summary>Details</summary>
Motivation: To build on prior work and guide future co-creative system designs by analyzing existing research.

Method: Conducted a systematic literature review of 62 papers on co-creative systems across various applications.

Result: Identified key design dimensions (e.g., user control, proactive behavior) and 24 considerations. High user control improves satisfaction, while adaptive proactive systems enhance collaboration.

Conclusion: Future systems should prioritize user control, transparency, and social presence, while addressing gaps like early-phase support and user adaptation challenges.
...
### [280] [Multimodal LLMs for Visualization Reconstruction and Understanding](https://arxiv.org/abs/2506.21319)
*Can Liu,Chunlin Da,Xiaoxiao Long,Yuxiao Yang,Yu Zhang,Yong Wang*

Main category: cs.HC

TL;DR: A novel dataset and multimodal visualization LLMs are introduced to improve understanding of visualizations by combining chart images with vectorized representations, encoding schemes, and data features.


<details>
  <summary>Details</summary>
Motivation: Current multimodal models struggle with visualizations due to poor decoding of data-to-visual mapping rules and structured information extraction.

Method: Train multimodal visualization LLMs using chart images paired with vectorized representations, encoding schemes, and data features.

Result: Significant improvements in data extraction accuracy and chart reconstruction quality.

Conclusion: The proposed approach effectively addresses the limitations of current models in visualization understanding.
...
### [281] [From On-chain to Macro: Assessing the Importance of Data Source Diversity in Cryptocurrency Market Forecasting](https://arxiv.org/abs/2506.21246)
*Giorgos Demosthenous,Chryssis Georgiou,Eliada Polydorou*

Main category: q-fin.PM

TL;DR: The study explores how diverse data sources improve cryptocurrency forecasting models, introducing the Crypto100 index and a feature reduction algorithm. Results show enhanced predictive performance, with on-chain metrics being crucial for all timeframes.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of diverse data sources (technical, on-chain, sentiment, traditional markets, macroeconomic) on cryptocurrency forecasting accuracy.

Method: Integration of multiple data categories, creation of the Crypto100 index, and a novel feature reduction algorithm to identify impactful features.

Result: Data diversity boosts model performance; on-chain metrics are key for all forecasts, while traditional and macroeconomic indicators gain importance for long-term predictions.

Conclusion: Diverse data sources improve forecasting accuracy, clarifying cryptocurrency market drivers and aiding future model development.
...
### [282] [Enhancing Homophily-Heterophily Separation: Relation-Aware Learning in Heterogeneous Graphs](https://arxiv.org/abs/2506.20980)
*Ziyu Zheng,Yaming Yang,Ziyu Guan,Wei Zhao,Weigang Lu*

Main category: cs.SI

TL;DR: RASH is a contrastive learning framework for heterogeneous graphs that models high-order semantics and separates homophilic/heterophilic patterns dynamically.


<details>
  <summary>Details</summary>
Motivation: Existing methods lose heterophily in heterogeneous graphs by converting them to homogeneous ones. RASH addresses this gap.

Method: Uses dual heterogeneous hypergraphs, dynamically constructs homophilic/heterophilic graphs, and employs multi-relation contrastive loss.

Result: Effective in resolving heterogeneity and heterophily, demonstrated by experiments on benchmark datasets.

Conclusion: RASH successfully models heterophily in heterogeneous graphs and improves performance in downstream tasks.
...
### [283] [Quantum Reinforcement Learning Trading Agent for Sector Rotation in the Taiwan Stock Market](https://arxiv.org/abs/2506.20930)
...
<details>
  <summary>Details</summary>
Motivation: To explore the application of quantum-enhanced models in financial reinforcement learning, specifically for sector rotation in the Taiwan stock market, and identify challenges in aligning training rewards with real-world investment goals.

Method: Uses Proximal Policy Optimization (PPO) with classical (LSTM, Transformer) and quantum (QNN, QRWKV, QASA) models for policy and value networks, alongside automated feature engineering for financial data.

Result: Quantum models achieve higher training rewards but underperform classical models in real-world metrics like cumulative return and Sharpe ratio, highlighting a reward-performance gap.

Conclusion: Current reward designs may incentivize overfitting to short-term volatility. Future improvements could include reward shaping, regularization, and early stopping, with the work providing a benchmark for quantum reinforcement learning in finance.
...
### [284] [ResQ: A Novel Framework to Implement Residual Neural Networks on Analog Rydberg Atom Quantum Computers](https://arxiv.org/abs/2506.21537)
*Nicholas S. DiBrita,Jason Han,Tirthak Patel*

Main category: quant-ph

TL;DR: ResQ is a new framework using Rydberg atom quantum computers for neural ODE-based ResNets, optimizing quantum dynamics for machine learning classification.


<details>
  <summary>Details</summary>
Motivation: Explore the untapped potential of quantum computing in neural ODE-based ResNets to enhance neural network effectiveness.

Method: Introduce ResQ, a framework optimizing Rydberg atom quantum computer dynamics for analog quantum neural ODEs in classification tasks.

Result: Demonstrates suitability of analog Rydberg atom quantum computers for ResNets.

Conclusion: ResQ effectively bridges quantum computing and neural ODE-based ResNets, offering a promising approach for quantum machine learning.
...
### [285] [Consistent Zero-shot 3D Texture Synthesis Using Geometry-aware Diffusion and Temporal Video Models](https://arxiv.org/abs/2506.20946)
*Donggoo Kang,Jangyeong Kim,Dasol Jeong,Junyoung Choi,Jeonga Wi,Hyunmin Lee,Joonho Gwon,Joonki Paik*

Main category: cs.GR

TL;DR: VideoTex is a framework for seamless 3D texture synthesis using video generation models to address spatial and temporal inconsistencies, outperforming existing methods in fidelity and stability.


<details>
  <summary>Details</summary>
Motivation: Current texture synthesis methods lack global context and geometric understanding, leading to inconsistencies. Video generation models show promise for temporal consistency.
...
Result: VideoTex achieves smoother transitions, high-quality textures, and outperforms existing methods in fidelity, seam blending, and stability.

Conclusion: VideoTex advances dynamic real-time applications by ensuring visual quality and temporal coherence in texture synthesis.
...
### [286] [Generative Blocks World: Moving Things Around in Pictures](https://arxiv.org/abs/2506.20703)
...
Method: Represents scenes as assemblies of convex 3D primitives, uses flow-based image generation conditioned on depth and texture hints, and improves texture consistency.

Result: Outperforms prior works in visual fidelity, editability, and compositional generalization.

Conclusion: The method provides a robust framework for interactive scene editing in generated images.
...
### [287] [3DGH: 3D Head Generation with Composable Hair and Face](https://arxiv.org/abs/2506.20875)
...
TL;DR: 3DGH is a generative model for 3D human heads with composable hair and face components, using template-based 3D Gaussian Splatting and dual generators with cross-attention.


<details>
  <summary>Details</summary>
Motivation: Previous work entangled hair and face modeling, limiting flexibility. 3DGH aims to separate these components for better control and variation.

Method: Uses deformable hair geometry and a 3D GAN-based architecture with dual generators and cross-attention. Trained on synthetic renderings with specialized objectives.

Result: Demonstrates effectiveness in unconditional full-head image synthesis and composable hairstyle editing, outperforming state-of-the-art methods.

Conclusion: 3DGH successfully separates hair and face modeling, enabling flexible and high-quality 3D head generation and editing.
...
### [288] [FairyGen: Storied Cartoon Video from a Single Child-Drawn Character](https://arxiv.org/abs/2506.21272)
*Jiayi Zheng,Xiaodong Cun*

Main category: cs.GR

TL;DR: FairyGen is an automatic system that generates story-driven cartoon videos from a child's drawing, preserving artistic style and enhancing storytelling with cinematic techniques.
...
Method: Uses an MLLM for storyboard generation, style propagation for visual consistency, 3D proxy for motion, and a diffusion model for animation. Includes a two-stage motion customization adapter.

Result: Produces stylistically faithful, narratively structured animations with natural motion.

Conclusion: FairyGen shows potential for personalized story animation, combining style preservation with cinematic quality.
...
### [289] [The final solution of the Hitchhiker's problem #5](https://arxiv.org/abs/2506.20672)
*Matjaž Omladič,Martin Vuk,Aljaž Zalar*

Main category: stat.ML

TL;DR: The paper resolves Open Problem 5 from the 'Hitchhiker's Guide' on quasi-copulas using an analytical approach, disproving a conjecture and extending results to higher dimensions.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the lack of statistical interpretation of quasi-copulas and solve Open Problem 5, a key question in dependence modeling.

Method: An analytical approach is employed to investigate extreme values of mass distribution in multivariate quasi-copulas.

Result: The paper provides a complete solution to Open Problem 5, disproving a recent conjecture and extending findings up to dimension 17.

Conclusion: The analytical approach successfully resolves the open problem, contributing to the understanding of quasi-copulas in dependence modeling.
...
### [290] [Stable Minima of ReLU Neural Networks Suffer from the Curse of Dimensionality: The Neural Shattering Phenomenon](https://arxiv.org/abs/2506.20779)
*Tongtong Liang,Dan Qiao,Yu-Xiang Wang,Rahul Parhi*

Main category: stat.ML

TL;DR: Flatness in overparameterized ReLU networks improves generalization but suffers exponential performance decay in high dimensions, unlike low-norm solutions.


<details>
  <summary>Details</summary>
Motivation: Understanding the implicit bias of flatness and its impact on generalization in high-dimensional settings, motivated by minima stability and edge-of-stability phenomena.

Method: Theoretical analysis of two-layer ReLU networks with multivariate inputs, focusing on flat solutions and MSE in nonparametric function estimation. Upper and lower bounds are derived.

Result: Flatness implies generalization but with exponentially deteriorating convergence rates in high dimensions, contrasting with low-norm solutions.

Conclusion: Flat minima may generalize poorly in high dimensions due to 'neural shattering,' where neurons activate rarely but with high weights, supported by simulations.
...
### [291] [Hyperspherical Variational Autoencoders Using Efficient Spherical Cauchy Distribution](https://arxiv.org/abs/2506.21278)
*Lukas Sablica,Kurt Hornik*

Main category: stat.ML

TL;DR: A novel VAE with a spherical Cauchy (spCauchy) latent distribution is proposed, offering better directional data representation and stability compared to Gaussian or vMF distributions.


<details>
  <summary>Details</summary>
Motivation: Traditional latent distributions (Gaussian, vMF) have limitations like over-regularization and numerical instability. spCauchy addresses these issues for better generative modeling.

Method: The architecture uses spCauchy for latent variables, leveraging its heavy-tailed nature and Möbius transformations for stable training. KL divergence is computed via a power series.

Result: spCauchy provides a more expressive latent space, avoids numerical instabilities, and ensures efficient training, outperforming traditional distributions.

Conclusion: spCauchy is a superior alternative for VAEs, combining theoretical benefits with practical efficiency in high-dimensional generative tasks.
...
### [292] [Active Learning for Manifold Gaussian Process Regression](https://arxiv.org/abs/2506.20928)
*Yuanxing Cheng,Lulu Kang,Yiwei Wang,Chun Liu*

Main category: stat.ML

TL;DR: An active learning framework for manifold Gaussian Process regression improves accuracy in high-dimensional spaces by combining manifold learning and strategic data selection.


<details>
  <summary>Details</summary>
Motivation: To enhance accuracy in high-dimensional regression tasks by integrating manifold learning with active learning.

Method: Joint optimization of a neural network for dimensionality reduction and a Gaussian process regressor in latent space, guided by an active learning criterion.
...
### [293] [Lower Bounds on the Size of Markov Equivalence Classes](https://arxiv.org/abs/2506.20933)
...
<details>
  <summary>Details</summary>
Motivation: To understand the limits of causal discovery under relaxed assumptions, as prior work assumed strict conditions for small equivalence classes.

Method: Proves lower bounds for the expected size of Markov equivalence classes in three settings: sparse random DAGs, acyclic directed mixed graphs, and directed cyclic graphs.

Result: Exponentially large lower bounds for equivalence class sizes are established in all three relaxed settings.

Conclusion: Relaxing key assumptions in causal discovery significantly increases the size of Markov equivalence classes, limiting identifiability from observational data.
...
### [294] [Forecasting Geopolitical Events with a Sparse Temporal Fusion Transformer and Gaussian Process Hybrid: A Case Study in Middle Eastern and U.S. Conflict Dynamics](https://arxiv.org/abs/2506.20935)
*Hsin-Hsiung Huang,Hayden Hampton*

Main category: stat.ML

TL;DR: STFT-VNNGP, a hybrid model combining Temporal Fusion Transformer (TFT) and Variational Nearest Neighbor Gaussian Process (VNNGP), outperforms standalone TFT in forecasting geopolitical conflicts by addressing data sparsity and burstiness.


<details>
  <summary>Details</summary>
Motivation: Forecasting geopolitical conflicts is crucial for national security, but existing models like TFT struggle with sparse, bursty data.

Method: A two-stage process: TFT captures temporal dynamics for multi-quantile forecasts, then VNNGP performs spatiotemporal smoothing and uncertainty quantification.

Result: STFT-VNNGP outperforms TFT in predicting timing and magnitude of conflict events, especially at long-range horizons.

Conclusion: The hybrid model provides a robust framework for reliable conflict forecasting, with publicly available code for reproducibility.
...
### [295] [Homogenization of Multi-agent Learning Dynamics in Finite-state Markov Games](https://arxiv.org/abs/2506.21079)
*Yann Kerzreho*

Main category: stat.ML

TL;DR: The paper proposes a method to approximate RL agents' learning dynamics in Markov games by rescaling the learning process, leading to a deterministic ODE approximation.


<details>
  <summary>Details</summary>
Motivation: To provide a tractable approximation of learning dynamics for multiple RL agents in finite-state Markov games.

Method: Rescale the learning process by reducing the learning rate and increasing update frequency, treating agent parameters as slow-evolving variables.

Result: Convergence of the rescaled process to a deterministic ODE under mild assumptions.

Conclusion: The ODE offers a practical approximation for analyzing RL agents' learning dynamics in Markov games.
...
### [296] [Wild refitting for black box prediction](https://arxiv.org/abs/2506.21460)
*Martin J. Wainwright*

Main category: stat.ML

TL;DR: A computationally efficient refitting procedure, called wild refitting, is proposed to compute high-probability upper bounds on prediction error for penalized nonparametric estimates. It uses Rademacher residual symmetrization and requires only a single dataset and black-box access to the prediction method.


<details>
  <summary>Details</summary>
Motivation: To provide a practical and theoretically sound method for bounding prediction error in nonparametric estimation, addressing noise heterogeneity and ensuring computational efficiency.

Method: The procedure involves three steps: computing residuals, symmetrizing and scaling them with a factor ρ, and solving a modified prediction problem. It leverages Rademacher symmetrization for robustness.

Result: Theoretical guarantees show that wild refitting with a suitable ρ provides high-probability upper bounds on prediction error. The method is validated on problems like non-rigid structure recovery and image restoration.

Conclusion: Wild refitting is a versatile and efficient tool for prediction error estimation, with theoretical backing and practical applicability across diverse problems.
...
### [297] [Gaussian Invariant Markov Chain Monte Carlo](https://arxiv.org/abs/2506.21511)
*Michalis K. Titsias,Angelos Alexopoulos,Siran Liu,Petros Dellaportas*

Main category: stat.ML

TL;DR: The paper introduces Gaussian-invariant versions of RWM, MALA, and second-order MALA, showing improved statistical efficiency and ergodic estimators due to exact analytical solutions for Gaussian targets.


<details>
  <summary>Details</summary>
Motivation: To enhance the efficiency of Markov chain Monte Carlo (MCMC) methods by leveraging Gaussian invariance for better estimators and variance reduction.

Method: Develops Gaussian-invariant RWM, MALA, and second-order MALA, using exact solutions to the Poisson equation for Gaussian targets to construct control variates.

Result: Demonstrates improved efficiency in high-dimensional latent Gaussian models, outperforming advanced methods. Provides theoretical insights on geometric ergodicity and optimal scaling.

Conclusion: Gaussian-invariant sampling methods offer superior performance and theoretical guarantees, making them practical for intractable targets.
...
### [298] [scMamba: A Scalable Foundation Model for Single-Cell Multi-Omics Integration Beyond Highly Variable Feature Selection](https://arxiv.org/abs/2506.20697)
*Zhen Yuan,Shaoqing Jiao,Yihang Xiao,Jiajie Peng*

Main category: q-bio.CB

TL;DR: scMamba is a foundation model for single-cell multi-omics data integration, avoiding feature selection loss and preserving genomic positional information. It outperforms existing methods in biological variation, omics alignment, and downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Current methods for single-cell multi-omics integration discard crucial biological information during preprocessing. scMamba aims to overcome this limitation by integrating data without prior feature selection.

Method: scMamba uses a patch-based cell tokenization strategy, treating genomic regions as tokens and cells as sentences. It employs state space duality and contrastive learning with cosine similarity regularization for superior omics alignment.

Result: scMamba outperforms state-of-the-art methods in preserving biological variation, aligning omics layers, and improving clustering, cell type annotation, and trajectory inference.
...
### [299] [Real-time and personalized product recommendations for large e-commerce platforms](https://arxiv.org/abs/2506.21368)
*Matteo Tolloso,Davide Bacciu,Shahab Mokarizadeh,Marco Varesi*

Main category: cs.IR

TL;DR: A method for real-time, personalized fashion product recommendations using Graph Neural Networks and parsimonious learning, tested on large e-commerce data.
...
### [300] [EraRAG: Efficient and Incremental Retrieval Augmented Generation for Growing Corpora](https://arxiv.org/abs/2506.20963)
*Fangyuan Zhang,Zhengjun Huang,Yingli Zhou,Qintian Guo,Zhixun Li,Wensheng Luo,Di Jiang,Yixiang Fang,Xiaofang Zhou*

Main category: cs.IR

TL;DR: EraRAG introduces a dynamic Graph-RAG framework for efficient updates in evolving corpora, reducing update time and token usage while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing Graph-RAG systems assume static corpora, requiring costly full-graph updates for new documents, limiting scalability in dynamic environments.

Method: Uses hyperplane-based LSH to partition the corpus into hierarchical graphs, enabling localized insertions without disrupting topology or retraining.

Result: Achieves up to 10x reduction in update time and token consumption with superior accuracy on large-scale benchmarks.

Conclusion: EraRAG bridges the gap between retrieval efficiency and adaptability, offering a scalable solution for growing corpora.
...
### [301] [DRAGON: Distributional Rewards Optimize Diffusion Generative Models](https://arxiv.org/abs/2504.15217)
...
<details>
  <summary>Details</summary>
Motivation: To improve media generation models by optimizing reward functions that evaluate individual examples or distributions, enabling compatibility with various reward types.

Method: DRAGON constructs reward functions using encoders and reference examples, contrasts positive and negative sets, and fine-tunes models like text-to-music diffusion.

Result: Achieves an 81.45% average win rate across 20 rewards and a 60.95% human-voted music quality win rate without human preference data.

Conclusion: DRAGON offers a novel, effective approach to optimizing reward functions for better human-perceived quality in media generation.
...
### [302] [Exploring Adapter Design Tradeoffs for Low Resource Music Generation](https://arxiv.org/abs/2506.21298)
...
<details>
  <summary>Details</summary>
Motivation: To address the high computational cost of fine-tuning large music generation models by identifying optimal adapter configurations for specific low-resource music genres.

Method: Study various adapter architectures (convolution-based and transformer-based) and sizes for MusicGen and Mustango on Hindustani Classical and Turkish Makam music, evaluating performance and resource requirements.

Result: Convolution-based adapters capture local musical details better, while transformer-based adapters preserve long-range dependencies. Mid-sized adapters (40M parameters) balance expressivity and quality. Mustango offers diverse outputs but is computationally intensive; MusicGen is faster and more efficient.

Conclusion: Adapter design significantly impacts performance and efficiency, with mid-sized adapters offering a practical balance. Choice between models depends on trade-offs between diversity, stability, and computational cost.
...
### [303] [A Hierarchical Deep Learning Approach for Minority Instrument Detection](https://arxiv.org/abs/2506.21167)
*Dylan Sechet,Francesca Bugiotti,Matthieu Kowalski,Edouard d'Hérouville,Filip Langiewicz*

Main category: cs.SD

TL;DR: The paper explores hierarchical classification for instrument activity detection in music, addressing data scarcity for fine-grained annotations and improving coarse-level recognition.


<details>
  <summary>Details</summary>
Motivation: To improve instrument activity detection in music, especially for instruments with limited data, by leveraging hierarchical classification.

Method: Uses the Hornbostel-Sachs classification and evaluates on the MedleyDB dataset, integrating hierarchical structures into models.

Result: Demonstrates more reliable coarse-level instrument detection by combining detailed and group-level recognition.

Conclusion: Hierarchical classification bridges gaps in instrument recognition, enabling advancements in music information retrieval.
...
### [304] [Integrating Vehicle Acoustic Data for Enhanced Urban Traffic Management: A Study on Speed Classification in Suzhou](https://arxiv.org/abs/2506.21269)
*Pengfei Fan,Yuli Zhang,Xinheng Wang,Ruiyuan Jiang,Hankang Gu,Dongyao Jia,Shangbo Wang*

Main category: cs.SD

TL;DR: The paper introduces the SZUR-Acoustic Dataset and a bimodal-feature-fusion CNN (BMCNN) for modeling vehicular noise and speed, achieving high accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: To address the coupling between vehicular noise and driving speed for smart-city traffic management and noise pollution reduction.

Method: Proposes BMCNN with adaptive denoising, MFCCs, wavelet-packet energy features, and cross-modal attention fusion.

Result: BMCNN achieves 87.56% accuracy on SZUR-Acoustic and 96.28% on IDMT-Traffic datasets, validated by ablation studies.

Conclusion: The method supports real-time noise monitoring and speed estimation for sustainable urban planning.
...
### [305] [SmoothSinger: A Conditional Diffusion Model for Singing Voice Synthesis with Multi-Resolution Architecture](https://arxiv.org/abs/2506.21478)
*Kehan Sui,Jinxu Xiang,Fang Jin*

Main category: cs.SD

TL;DR: SmoothSinger is a diffusion-based model for singing voice synthesis that refines low-quality audio directly, avoiding vocoder distortions and improving naturalness.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models struggle with singing voice synthesis due to complex acoustic characteristics, often causing artifacts. SmoothSinger addresses this by refining audio in a unified framework.

Method: SmoothSinger uses a reference-guided dual-branch architecture and enhances U-Net with a low-frequency upsampling path. It replaces reference audio with degraded ground truth for better alignment.

Result: On the Opencpop dataset, SmoothSinger achieves state-of-the-art results in objective and subjective evaluations, reducing artifacts and improving naturalness.

Conclusion: SmoothSinger effectively synthesizes high-quality, natural singing voices by refining low-quality audio directly and improving alignment during training.
...
TL;DR: A differentiable STFT framework is proposed for gradient-based optimization of parameters, improving TFRs and downstream task performance.
...
Conclusion: The differentiable STFT offers a flexible and efficient alternative to traditional tuning methods, enhancing signal analysis and integration with neural networks.
...
### [307] [Evaluating PDE discovery methods for multiscale modeling of biological signals](https://arxiv.org/abs/2506.20694)
*Andréa Ducos,Audrey Denizot,Thomas Guyet,Hugues Berry*

Main category: q-bio.QM

TL;DR: The paper proposes using PDE discovery to bridge gaps between scales in biological systems, testing methods on calcium diffusion simulations.


<details>
  <summary>Details</summary>
Motivation: Biological systems are complex, non-linear, and multi-scaled, making behavior characterization difficult. PDE discovery can help link micro-scale data to meso-scale dynamics.

Method: Combines particle-based simulations and PDE discovery, evaluating five state-of-the-art methods on calcium diffusion in astrocytes.

Result: Several methods accurately recover the diffusion term, demonstrating PDE discovery's potential for capturing macroscopic dynamics from microscopic data.
...
### [308] [Agile Management for Machine Learning: A Systematic Mapping Study](https://arxiv.org/abs/2506.20759)
*Lucas Romao,Hugo Villamizar,Romeu Oliveira,Silvio Alonso,Marcos Kalinowski*

Main category: cs.SE

TL;DR: The paper reviews agile management for ML-enabled systems, identifying frameworks and challenges like effort estimation.


<details>
  <summary>Details</summary>
Motivation: The dynamic nature of ML development challenges traditional project management, requiring tailored agile approaches.

Method: A systematic mapping study with hybrid search strategy, analyzing 27 papers (2008-2024).

Result: Identified 8 frameworks and categorized practices into 8 themes; main challenge was effort estimation for ML tasks.

Conclusion: Maps state of the art but highlights need for more empirical validation of existing frameworks.
...
### [309] [Generating Reliable Adverse event Profiles for Health through Automated Integrated Data (GRAPH-AID): A Semi-Automated Ontology Building Approach](https://arxiv.org/abs/2506.20851)
*Srikar Reddy Gadusu,Larry Callahan,Samir Lababidi,Arunasri Nishtala,Sophia Healey,Hande McGinty*

Main category: cs.SE

TL;DR: A user-friendly Python-based method using rdflib is proposed to integrate Neo4j databases with OWL for ontology generation, demonstrated with FDA FAERS data.


<details>
  <summary>Details</summary>
Motivation: The rapid expansion of data and the need for efficient ontology generation highlight the challenge of integrating Neo4j with OWL, especially for users unfamiliar with description logics.

Method: The approach uses Python and rdflib to automate class and axiom generation from a Neo4j database (FDA FAERS data), simplifying ontology development.

Result: A practical solution for seamless Neo4j-OWL integration is demonstrated, aiding ontology generation for adverse drug event datasets.

Conclusion: This method improves accessibility and efficiency in ontology generation, supporting drug safety and public health decisions.
...
### [310] [Engineering RAG Systems for Real-World Applications: Design, Development, and Evaluation](https://arxiv.org/abs/2506.20869)
*Md Toufique Hasan,Muhammad Waseem,Kai-Kristian Kemell,Ayman Asad Khan,Mika Saari,Pekka Abrahamsson*

Main category: cs.SE

TL;DR: This paper presents five real-world RAG applications across diverse domains, evaluated by 100 users, and shares 12 key lessons on challenges and best practices.
...
Method: Developed five domain-specific RAG applications with multilingual OCR, semantic retrieval, and domain-adapted LLMs, evaluated via a web-based study with 100 participants.

Result: User feedback highlighted strengths and weaknesses across six dimensions, revealing technical, operational, and ethical challenges.

Conclusion: The study provides actionable insights and lessons learned to improve the reliability and usability of RAG systems in practice.
...
### [311] [Complex Model Transformations by Reinforcement Learning with Uncertain Human Guidance](https://arxiv.org/abs/2506.20883)
...
<details>
  <summary>Details</summary>
Motivation: Manual development of complex MTs is error-prone and infeasible; RL can help but struggles with performance in complex scenarios, where human guidance can be beneficial.

Method: The approach maps user-defined MTs onto RL primitives and executes them as RL programs to find optimal MT sequences, incorporating uncertain human advice.

Result: Human guidance, even uncertain, significantly enhances RL performance and efficiency in developing complex MTs.
...
### [312] [How Good Are Synthetic Requirements ? Evaluating LLM-Generated Datasets for AI4RE](https://arxiv.org/abs/2506.21138)
*Abdelkarim El-Hajjami,Camille Salinesi*

Main category: cs.SE

TL;DR: Synthline v1 enhances synthetic requirements data generation, showing improved quality and diversity through advanced prompting and curation, outperforming human data in some tasks.


<details>
  <summary>Details</summary>
Motivation: The lack of labeled requirements datasets hinders AI for Requirements Engineering (AI4RE). Synthetic data generation offers a solution, but quality control methods are underexplored.

Method: Synthline v1 extends a Product Line approach with advanced generation strategies (multi-sample prompting, PACE for prompt optimization) and curation techniques. Evaluated on four classification tasks.

Result: Multi-sample prompting improves utility and diversity (F1-score gains: 6-44 points). PACE boosts functional classification (+32.5 points) but reduces others. Synthetic data matches or surpasses human data in security (+7.8 points) and defect classification (+15.4 points).

Conclusion: Systematic synthetic generation can mitigate dataset scarcity in AI4RE, with synthetic data performing competitively or better than human-authored data for specific tasks.
...
### [313] [$T^3$: Multi-level Tree-based Automatic Program Repair with Large Language Models](https://arxiv.org/abs/2506.21211)
*Quanming Liu,Xupeng Bu,Zhichao Yan,Ru Li*

Main category: cs.SE

TL;DR: The paper evaluates CoT techniques in APR tasks and introduces $T^3$, a framework combining LLMs and tree search to improve repair solution precision.


<details>
  <summary>Details</summary>
Motivation: To enhance automated defect repair by leveraging LLMs and CoT techniques, addressing the lack of multi-step reasoning in APR.

Method: Systematic evaluation of CoT techniques in APR and development of $T^3$, integrating LLMs with tree search.

Result: $T^3$ improves precision in generating repair solutions and optimizes sample selection and repair strategies.

Conclusion: $T^3$ provides a robust framework for efficient automated debugging in APR.
