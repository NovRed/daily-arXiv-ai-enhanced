...
Motivation: Traditional UHD IR methods (e.g., downsampling or frequency-domain transformations) suffer from irreversible information loss or ineffective artifact handling, necessitating a more robust solution.

Method: RetinexDual uses two sub-networks: SAMBA for reflectance correction via coarse-to-fine modeling and FIA for frequency-domain color/illumination correction.

Result: RetinexDual outperforms recent methods in deraining, deblurring, dehazing, and LLIE, with ablation studies validating its design.

Conclusion: RetinexDual's dual-network approach effectively addresses UHD IR challenges, offering superior performance and validated component contributions.
...
<details>
  <summary>Details</summary>
Motivation: To improve ENT care by addressing variability in devices/operators, subtle findings, and lack of public benchmarks for retrieval and classification.

Method: Created a dataset with expert-annotated images (anatomical region, status) and bilingual descriptions. Defined three benchmark tasks with standardized protocols.

Result: Evaluated performance on public/private test splits, reported top team results, and provided insights.

Conclusion: ENTRep advances ENT endoscopy analysis by integrating classification and retrieval, supported by bilingual clinical supervision.
...
Conclusion: CoMAD effectively unifies diverse self-supervised insights into a compact model, outperforming prior methods in both accuracy and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Current reconstruction-based anomaly detection methods are computationally expensive, prone to errors, and require prior knowledge of anomalies.

Method: RADAR directly generates anomaly maps from diffusion models without reconstruction, enhancing speed and accuracy.

Result: RADAR outperforms SOTA models, improving F1 scores by 7% on MVTec-AD and 13% on 3D-printed material datasets.

Conclusion: RADAR offers a superior, efficient alternative to reconstruction-based anomaly detection, validated by significant performance gains.
...
Conclusion: The proposed method is effective and cost-efficient, with future work planned to improve interpretability and performance using LRP.
...
Conclusion: LuKAN is an effective, efficient model for 3D human motion prediction, validated by experiments.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack evaluation of MLLMs' ability to interpret subtle local details, which are crucial for deep visual understanding and complex reasoning.

Method: VER-Bench introduces 374 questions across six reasoning types, each with structured evidence (visual clues and reasoning).

Result: The benchmark shows current models struggle with extracting subtle visual evidence and evidence-based reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Addressing issues like occlusions, fine-grained localization, and computational inefficiency in transformer-based object detectors.

Method: Uses appearance-based, positional, and random learned queries, along with dual-stream cross-attention for semantic and spatial refinement.

Result: Achieved state-of-the-art performance in AP and recall on four benchmarks.

Conclusion: DAMM's multi-modal query adaptation and dual-stream attention effectively enhance object detection.
...
<details>
  <summary>Details</summary>
Motivation: The spread of hate speech in online videos is a critical issue, but current detection methods suffer from label noise due to coarse annotations that ignore temporal granularity.

Method: The study trims hateful videos from HateMM and MultiHateClip datasets using annotated timestamps to isolate hateful segments, analyzes their characteristics, and conducts controlled experiments.

Result: Coarse annotations introduce semantic overlap and weaken model confidence, altering decision boundaries due to temporal context dependency.

Conclusion: Temporally aware models and benchmarks are needed to improve robustness and interpretability in hate speech detection.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods use fixed models for all test videos, ignoring unique characteristics, which reduces performance.
...
Result: Experiments show Highlight-TTA improves performance of three state-of-the-art models across three datasets.

Conclusion: Highlight-TTA enhances video highlight detection by adapting to test videos, outperforming fixed models.
...
Method: Introduces calibration tokens to align fisheye image embeddings with perspective ones, leveraging existing FMDEs without retraining.

Result: Consistently outperforms state-of-the-art methods on indoor and outdoor datasets.

Conclusion: The proposed self-supervised method effectively adapts FMDEs for fisheye images using calibration tokens, avoiding artifacts from traditional recalibration.
...
Method: Feedforward artificial neural network trained with a novel method on ImageNet 2012 data.

Result: Achieves 98.3% accuracy, 99.69 Top-1 rate, and 285.9 perfectly classified labels per batch.

Conclusion: High accuracy is achieved, but double-labeling in the dataset may prevent 100% accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Existing prompt learning techniques (e.g., CoOp, CoCoOp) overfit to known classes, limiting generalization to unseen categories.

Method: ProMIM integrates MIM into VLM pipelines, using masked visible patches to generate robust, instance-conditioned prompts.

Result: ProMIM improves feature robustness, mitigates overfitting, and boosts generalization in zero-shot and few-shot tasks.

Conclusion: ProMIM offers a lightweight, plug-and-play solution for enhancing vision-language models.
...
<details>
  <summary>Details</summary>
Motivation: Existing WS-DSGG methods rely on external object detectors, which perform poorly in dynamic, relation-aware scenarios, leading to inaccurate localization.

Method: TRKT uses relation-aware knowledge mining (with attention maps and optical flow) and a dual-stream fusion module to refine object proposals.
...
<details>
  <summary>Details</summary>
Motivation: Standard SSL methods lack robustness to domain shifts, which is critical in biomedical imaging where batch effects can mask true signals.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for open-world point cloud semantic segmentation rely on resource-intensive offline learning or dense annotations, limiting practicality.
...
Conclusion: HOW-Seg is a practical and effective solution for open-world point cloud semantic segmentation, outperforming alternatives with human feedback.
...
Method: The proposed method uses learned uncertainties to guide Gaussian opacity updates and applies soft differentiable dropout regularization to control the rendering process.

Result: The method outperforms competitors in sparse-view 3D synthesis, achieving higher quality with fewer Gaussians (e.g., 3.27% PSNR improvement over DropGaussian on MipNeRF 360).

Conclusion: Adaptive weighting via learned uncertainties effectively improves 3DGS rendering quality in sparse-view scenarios, offering better performance than existing methods.
...
Method: SODEC leverages a pre-trained VAE for informative latents, replaces iterative denoising with single-step decoding, and introduces a fidelity guidance module. It also uses rate annealing training for low bitrates.
...
Conclusion: SODEC effectively addresses the drawbacks of diffusion-based compression, offering faster decoding and better fidelity.
...
<details>
  <summary>Details</summary>
Motivation: Existing depth completion models degrade in out-of-distribution scenarios due to reliance on limited data. Foundation models offer robustness, inspiring their use to enhance depth completion.
...
Result: The model performs exceptionally well in out-of-distribution scenarios, surpassing existing state-of-the-art methods on 16 diverse datasets.

Conclusion: The proposed framework demonstrates significant robustness and effectiveness in depth completion, validated by extensive evaluations.
...
<details>
  <summary>Details</summary>
Motivation: The modality gap in VLMs limits UDA performance by transferring only modality-invariant knowledge. The paper aims to overcome this by leveraging both modality-specific and invariant components.

Method: A unified framework disentangles VLM features into modality-specific and invariant components, uses modality-adaptive ensemble weights, and introduces a modality discrepancy metric for sample categorization. Prompt tuning enhances efficiency.

Result: The method achieves up to 9% performance gain with 9x computational efficiency, validated across various backbones, datasets, and settings.

Conclusion: The proposed framework effectively addresses the modality gap in UDA, improving performance and efficiency through modality separation and adaptive ensembling.
...
Method: A ViT-based autoencoder with LoRA for fast weights is used to simulate familiarity training. The study examines manifold transformation, latent representation alignment, and self-attention scope.
...
Conclusion: Familiarity training introduces global sensitivity to early layers, and a hybrid fast-and-slow weight architecture may model rapid global context learning in the brain.
...
Result: AG-ReID outperforms others on Re-ID datasets, excelling in occluded scenarios and subtle attribute differences.

Conclusion: AG-ReID effectively addresses occluded Re-ID challenges by integrating fine-grained attributes, setting new benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: To address the high memory demands of video continual learning, enabling practical deployment with natural distribution shifts.

Method: Uses compressed vision (video codes) and a rehearsal-based approach with a rolling buffer, refreshing codes to combat catastrophic forgetting.

Result: Demonstrates superior performance on large-scale benchmarks (EpicKitchens-100, Kinetics-700) with under 2 GB memory usage.

Conclusion: CRAM effectively reduces memory footprint while maintaining performance in video continual learning.
...
Method: Constructed AU-IQA, a dataset of 4,800 AI-UGC images from three enhancement types (super-resolution, low-light, denoising), and evaluated traditional and multimodal IQA models.

Result: Evaluated existing models on AU-IQA, providing insights into their effectiveness for AI-UGC quality assessment.

Conclusion: The study highlights gaps in current methods and provides a benchmark (AU-IQA) to advance AI-UGC quality assessment.
...
Method: The proposed skin-SOAP framework uses limited inputs (lesion images and sparse text) with weak supervision to generate structured SOAP notes, reducing reliance on manual annotations.

Result: The method performs comparably to GPT-4o, Claude, and DeepSeek Janus Pro, evaluated using novel metrics (MedConceptEval and CCS) for clinical relevance.

Conclusion: Skin-SOAP offers a scalable solution for clinical documentation, alleviating clinician burden and reducing the need for large annotated datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing image quality metrics fail to adequately assess SCS, which is crucial for structurally accurate generative AI outputs.
...
Result: SCSSIM shows high invariance to non-compositional distortions and strong sensitivity to compositional changes, outperforming existing metrics.

Conclusion: SCSSIM is a valuable tool for evaluating and improving generative models by ensuring scene composition integrity.
...
<details>
  <summary>Details</summary>
Motivation: Existing video-based ReID methods lack focus on selecting discriminative features for robust matching in dynamic environments.

Method: Proposes HAMoBE, a framework with hierarchical feature extraction (low-level and specialized experts) and adaptive integration via a dual-input decision gating network.

Result: Significant performance gains, e.g., +13.0% Rank-1 accuracy on MEVID benchmarks.

Conclusion: HAMoBE effectively addresses feature selection and integration in video-based ReID, enhancing accuracy and robustness.
...
Result: Experiments show Spot-IT significantly improves MLLMs' performance in precise detail extraction from complex layouts compared to baselines.
...
<details>
  <summary>Details</summary>
Motivation: To improve PBR material estimation from single images under complex lighting by leveraging dual latent spaces and feature distillation.

Method: Uses two latent spaces (albedo-optimized and material-specialized) with feature distillation and rectified flow for efficient inference. Supports high-resolution and multi-view inputs.

Result: Achieves state-of-the-art performance with 28% better albedo estimation and 39% lower metallic-roughness errors.

Conclusion: DualMat outperforms existing methods, enabling seamless integration into image-to-3D pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Address catastrophic forgetting in CSS by mitigating interference between old and new class learning in single-stage architectures.

Method: Uses a two-stage approach: (1) class-aware detection with pre-trained encoders and LoRA adaptation, and (2) class-agnostic segmentation with SAM for precise masks.

Result: Achieves state-of-the-art performance by balancing retention and adaptability in CSS tasks.
...
Method: The approach involves building models on prior works, applying modifications, and comparing results, focusing on classification and adversarial learning for color prediction.
...
Conclusion: The paper proposes a novel approach to image colorization by combining classification and adversarial learning, potentially improving upon traditional regression-based methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods rely on auxiliary components that introduce errors, leading to suboptimal makeup transfer results.

Method: FLUX-Makeup leverages FLUX-Kontext with source images as input and introduces RefLoRAInjector for efficient makeup feature extraction. A robust data generation pipeline enhances training.

Result: FLUX-Makeup achieves state-of-the-art performance with strong robustness across diverse scenarios.

Conclusion: The proposed framework outperforms existing methods by eliminating auxiliary components and improving makeup transfer quality and identity consistency.
...
<details>
  <summary>Details</summary>
Motivation: PFMs have strong representational capabilities but suffer from latent biases due to diverse pretraining contexts, limiting generalisability and transparency.

Method: AdaFusion compresses and aligns tile-level features from multiple PFMs, using a lightweight attention mechanism to adaptively fuse them based on tissue phenotype context.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models struggle with identity drift and short video lengths, limiting their practical use.

Method: PoseGen uses in-context LoRA finetuning for identity preservation and pose conditioning, along with interleaved segment generation for long videos.
...
Conclusion: PoseGen addresses key challenges in video generation, offering superior performance with minimal training data.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of data privacy, high acquisition costs, and performance degradation in incremental tasks by improving forward-compatible learning.

Method: Introduces Margin-aware Intra-task Adapter Merging (MIAM) for base task learning and Margin Penalty-based Classifier Calibration (MPCC) for incremental tasks.
...
Conclusion: SMP provides a robust solution for FSCIL by strategically integrating margin penalties and adaptive merging, enhancing both discriminability and generalization.
...
Method: AHDMIL uses a two-step process: (1) self-distillation (SD) to train a Dynamic Multi-Instance Network (DMIN) and identify irrelevant patches, and (2) asymmetric distillation (AD) where a Dual-Branch Lightweight Instance Pre-screening Network (DB-LIPN) predicts patch relevance for efficient inference. Includes a Chebyshev-polynomial-based Kolmogorov-Arnold (CKA) classifier.

Result: Outperforms state-of-the-art methods with 5.3% accuracy improvement on Camelyon16 and 1.2-2.1x faster inference across datasets.

Conclusion: AHDMIL offers a scalable, efficient solution for MIL in pathology, balancing accuracy and speed.
...
<details>
  <summary>Details</summary>
Motivation: To develop a unified model for medical image segmentation across diverse modalities while addressing challenges like data scarcity, domain shift, and privacy restrictions.

Method: Proposes FedGIN, integrating a lightweight GIN augmentation module to harmonize modality-specific intensity distributions in a federated learning setup. Evaluated on imputed and complete datasets.

Result: FedGIN achieved 12-18% improvement in 3D Dice scores in limited-data scenarios and near-centralized performance in complete datasets, with strong cross-modality generalization.

Conclusion: FedGIN effectively addresses privacy and data scarcity challenges, demonstrating robust performance in multimodal organ segmentation.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for assessing chronic pain in mice rely on manual labeling, which lacks accuracy and understanding of pain-related behaviors.

Method: Uses a universal action space projector to automatically extract mouse action features, avoiding human bias, and includes a dataset of pain behaviors.

Result: Achieves 48.41% accuracy in 15-class pain classification (vs. 21.33% for humans) and 73.1% in 3-class classification, with validated drug efficacy insights.
...
<details>
  <summary>Details</summary>
Motivation: To solve the issue of warped geometric patterns in low-resolution images causing artifacts in high-resolution recoveries, embedding rotation equivariance is necessary for maintaining structural integrity.
...
Result: Experiments on simulated and real datasets show the method's superiority, and it can be integrated into existing ASISR methods for performance enhancement.

Conclusion: The proposed rotation-equivariant ASISR method effectively maintains geometric pattern integrity and can be easily adopted by current ASISR frameworks.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods model human and animal motion separately, lacking a unified approach. Cross-species modeling offers advantages like improved generalization but faces challenges due to morphological differences.
...
Conclusion: X-MoGen successfully unifies human and animal motion generation, leveraging a shared skeletal topology and morphological consistency for plausible results.
...
Result: Extensive experiments show MTT's competitiveness in generic multiple object tracking.

Conclusion: MTT effectively mitigates error propagation and enhances tracking robustness.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the tradeoff between inter-domain transferability and intra-domain discriminability in domain adaptation.

Method: Uses graph primitives, spectral alignment, neighbor-aware propagation, and consistency regularization.

Result: Outperforms existing methods in robustness and adaptability across various scenarios.
...
Method: Constructs SPIE dataset with spectral priors, introduces SPEX with multiscale feature aggregation, token context condensation, and multispectral visual pre-training.
...
<details>
  <summary>Details</summary>
Motivation: Dense feature matching in endoscopic images is essential for robot-assisted tasks but is hindered by poor visual conditions and lack of annotated data.
...
Conclusion: EndoMatcher achieves accurate and dense matching in challenging endoscopic conditions, generalizing well to unseen domains.
...
<details>
  <summary>Details</summary>
Motivation: Current token pruning methods in LMMs are simplistic, causing performance degradation. The need for efficient pruning without losing critical visual information motivates this work.

Method: VFlowOpt introduces an importance map derivation process, a progressive pruning module with recycling, and optimizes pruning hyperparameters using a visual information flow-guided method.
...
Conclusion: VFlowOpt effectively balances computational efficiency and performance in LMMs, offering a superior pruning strategy tailored to different models.
...
<details>
  <summary>Details</summary>
Motivation: Address performance degradation in FSS due to domain discrepancies and privacy/data transfer concerns.

Method: Uses Task-Specific Attention Adapters (TSAA) with VVEA and TVEA modules for feature alignment.

Result: Achieves 2.18% (1-shot) and 4.11% (5-shot) accuracy improvements over state-of-the-art methods.

Conclusion: The method effectively adapts to target domains without source data, outperforming existing CD-FSS approaches.
...
<details>
  <summary>Details</summary>
Motivation: To address limitations in existing vision-language tracking methods, which lack reasoning insights and underutilize large models.

Method: Proposes ReasoningTrack, combining updated language descriptions with vision features in a unified backbone, optimized via SFT and GRPO.

Result: Validated on TNLLT dataset; outperforms 20 baseline trackers, showing effectiveness of reasoning-based language generation.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods fail for deformed or coalesced bubbles, common in industrial applications like air lubrication systems.
...
Conclusion: Modern vision models like SAM can address complex bubble segmentation challenges with limited data.
...
Method: Uses Feature-Aware Adaptive View Stitching (FAVS) for hierarchical matching and Cross-View Consistency Self-Supervised Learning (CVC-SSL) for self-supervised training with a diffusion model.
...
<details>
  <summary>Details</summary>
Motivation: To address the trade-off between adversarial robustness and zero-shot generalization in VLMs, exploring effective defense strategies.

Method: Analysis of two defense paradigms: Adversarial Fine-Tuning (AFT) and Training-Free/Test-Time Defenses, with a focus on methodological evolution from alignment-preserving to embedding space re-engineering and latent-space purification.

Result: Identifies key challenges and future directions, such as hybrid defense strategies and adversarial pre-training.

Conclusion: The synthesis highlights the need for balanced approaches to adversarial robustness without compromising zero-shot capabilities, pointing to hybrid strategies and pre-training as promising avenues.
...
Method: Introduces RegionMed-CLIP with an ROI processor for fine-grained regional features and progressive training for multimodal alignment. Uses MedRegion-500k, a large annotated corpus.
...
Method: The study reviews existing gender classification approaches, particularly those based on facial and iris biometrics, and analyzes methodologies for different classification steps.

Result: The paper provides a comprehensive analysis of current methods, identifies gaps, and suggests future improvements.

Conclusion: The study serves as a resource for researchers, highlighting challenges and offering directions for advancing gender classification techniques.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods rely on costly bottom-up optimization of 2D features.

Method: Uses weighted fusion of multi-view 2D features with pre-trained Gaussians, trains a per-Gaussian autoencoder, and applies adaptive sparsification to prune redundant Gaussians.
...
TL;DR: A particle filtering tracker with cyclic-consistency checks is proposed for robust cardiac imaging during bypass surgery, outperforming deep learning and conventional trackers.
...
Method: A particle filtering tracker based on cyclic-consistency checks is used to track target landmarks.

Result: The method tracks 117 targets at 25.4 fps with a low error (5.00 +/- 0.22 px), surpassing other trackers.

Conclusion: The proposed tracker is effective for real-time cardiac imaging during interventions.
...
<details>
  <summary>Details</summary>
Motivation: Existing IVIF methods struggle with preserving key targets and avoiding artifacts due to lack of semantic understanding.

Method: Uses SAM-generated semantic masks as priors in a two-stage conditional diffusion model for fusion.

Result: Achieves state-of-the-art performance in subjective, objective evaluations, and downstream tasks.

Conclusion: SGDFuse provides a robust solution for high-quality, semantically-aware image fusion.
...
TL;DR: The paper introduces B4DL, a benchmark and model for training MLLMs on 4D LiDAR data, addressing the lack of high-quality annotations and suitable architectures.


<details>
  <summary>Details</summary>
Motivation: 4D LiDAR is underutilized in MLLMs due to missing annotations and processing challenges, despite its potential for dynamic outdoor scene understanding.
...
Conclusion: B4DL bridges the gap in 4D LiDAR-based MLLMs, enabling better dynamic outdoor environment analysis.
...
Method: WGDF uses Discrete Wavelet Transform (DWT) to decompose images into high- and low-frequency components. High-frequency branch employs Dual-Frequency Feature Enhancement (DFFE) and Frequency-Domain Interactive Difference (FDID) modules. Low-frequency branch uses Transformers and Progressive Contextual Difference Module (PCDM). Features are fused for unified detection.

Result: WGDF outperforms state-of-the-art methods, reducing edge ambiguity and improving accuracy and robustness on multiple datasets.

Conclusion: WGDF effectively combines frequency-domain modeling with global semantic relationships, achieving superior change detection in remote sensing imagery.
...
<details>
  <summary>Details</summary>
Motivation: Smaller models show disproportionate performance drops on OOD data, highlighting a need for improved generalization in efficient architectures.

Method: Identifies architectural bottlenecks and introduces CoCA, a dynamic, domain-aware global token mechanism for better local-global feature modeling.

Result: CoCAViT-28M achieves 84.0% top-1 accuracy on ImageNet-1K, with notable gains on OOD benchmarks and strong performance in object detection and segmentation.
...
TL;DR: The paper proposes mKG-RAG, a multimodal knowledge-augmented generation framework for VQA, using structured multimodal knowledge graphs to improve accuracy and reliability over traditional RAG methods.


<details>
  <summary>Details</summary>
Motivation: Vanilla RAG-based VQA methods often introduce irrelevant content due to unstructured knowledge, reducing accuracy. Structured multimodal knowledge graphs (KGs) are proposed to address this.
...
Result: mKG-RAG outperforms existing methods, achieving state-of-the-art performance in knowledge-based VQA tasks.

Conclusion: Integrating multimodal KGs into RAG frameworks enhances VQA accuracy and reliability, demonstrating the effectiveness of structured knowledge.
...
Method: Uses TI to learn new or improve existing tokens for detecting novel objects from few examples, keeping original VLM weights frozen.

Result: Retains original model performance, enables zero-shot transfer, and requires less compute than full fine-tuning.

Conclusion: TI is effective for few-shot learning in VLMs without sacrificing their original capabilities.
...
TL;DR: 3DGabSplat improves 3D Gaussian Splatting by using 3D Gabor-based primitives for better high-frequency detail capture, efficiency, and memory usage, achieving superior rendering quality.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) struggles with high-frequency details, redundant primitives, and inefficiency. 3DGabSplat aims to address these limitations.

Method: Proposes 3D Gabor-based primitives with multi-directional frequency responses, a CUDA-based rasterizer, and a frequency-adaptive optimization mechanism.

Result: Outperforms 3DGS with 1.35 dB PSNR gain, fewer primitives, and lower memory usage.

Conclusion: 3DGabSplat enhances 3DGS, offering state-of-the-art rendering quality and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods neglect patient-specific prior knowledge, leading to suboptimal report quality. PriorRG aims to emulate real-world clinical workflows by incorporating this critical information.
...
Conclusion: PriorRG effectively leverages prior knowledge to enhance report generation, aligning with clinical workflows and improving diagnostic accuracy and fluency.
...
<details>
  <summary>Details</summary>
Motivation: Current multilingual enhancement methods for MLLMs lack multimodal informativeness and cultural awareness, limiting their effectiveness in low-resource languages.

Method: A dual-source strategy is used: native web alt-text for cultural groundedness and MLLM-generated captions for linguistic capability. The MELLA dataset is created and used for fine-tuning.

Result: Fine-tuning on MELLA improves performance across eight languages, with models producing richer "thick descriptions". Gains are attributed to both cultural and linguistic enhancements.
...
TL;DR: CT-GRAPH is a hierarchical graph attention network that improves radiology report generation by modeling fine-grained organ relationships and integrating them with global features, achieving a 7.9% F1 score improvement.


<details>
  <summary>Details</summary>
Motivation: Automating radiology report generation to assist radiologists, addressing the limitation of current methods that overlook fine-grained organ relationships.

Method: Uses a hierarchical graph attention network to structure anatomical regions into a graph, leveraging pretrained 3D feature encoders and integrating features into a large language model.

Result: Achieves a 7.9% absolute improvement in F1 score on the CT-RATE dataset compared to state-of-the-art methods.
...
Method: Proposes a dynamic weighted directed GNN with deformable attention, incorporating learnable spatial offsets based on patch coordinates.

Result: Achieves state-of-the-art performance on four benchmark datasets, demonstrating enhanced contextual and spatial specificity.

Conclusion: The deformable attention GNN effectively captures complex spatial structures in pathology images, outperforming existing approaches.
...
TL;DR: UNCAGE improves compositional T2I generation by prioritizing unmasking of object-representing tokens using contrastive attention, outperforming existing methods with minimal overhead.
...
Method: Proposes UNCAGE, a training-free method using contrastive attention guidance to unmask tokens representing objects clearly.
...
Method: The framework includes a gradient calibration strategy for consistent updates across distances and a gradient decorrelation method to stabilize multi-angle optimization.

Result: The method outperforms state-of-the-art techniques, increasing attack success rates by 13.46% across distances and 11.03% across angles.

Conclusion: The framework enhances adversarial camouflage effectiveness, highlighting the need for more robust detector designs in real-world applications.
...
TL;DR: Uni-CoT is a unified Chain-of-Thought framework for multimodal reasoning, combining macro-level task planning and micro-level subtask execution to enhance coherence and efficiency.
...
Method: SmoothSA preheats cold-start queries with input features via self-distillation and differentiates transforms for first (full iterations) and non-first frames (single iteration).

Result: Experiments show effectiveness in object discovery, recognition, and benchmarks, with intuitive analyses of smoothed iterations and recurrences.
...
TL;DR: GUI-RC and GUI-RCPO improve GUI grounding accuracy by leveraging spatial voting and self-supervised rewards, achieving 2-3% gains without additional training.


<details>
  <summary>Details</summary>
Motivation: Existing GUI grounding methods rely on costly pixel-level annotations, limiting scalability. The paper aims to exploit implicit confidence signals in model predictions for more accurate localization.
...
Method: FIxLIP uses the weighted Banzhaf interaction index for efficient, flexible decomposition of similarity in encoders.
...
TL;DR: A new unsupervised anomaly detection method using Flow Matching (FM) with time-reversed FM (rFM) and Worst Transport (WT) displacement interpolation, achieving state-of-the-art results.
...
Method: Introduces rFM for transforming data distributions into Gaussian, identifies FM limitations, and proposes WT-Flow for non-probabilistic paths to separate anomalies.

Result: Achieves state-of-the-art performance on MVTec dataset with a scalable, computationally tractable framework.

Conclusion: WT-Flow provides a novel, theoretically grounded approach for unsupervised anomaly detection, enhancing control over sample trajectories.
...
<details>
  <summary>Details</summary>
Motivation: Pituitary tumors deform adjacent structures, risking surgery. Segmentation can warn surgeons of risky areas, improving safety.

Method: PAS dataset with 7,845 images from 120 videos, augmented for class balance. F2PASeg uses Feature Fusion to combine high-res and deep semantic features.

Result: F2PASeg segments critical structures robustly in real time, aiding surgery planning.

Conclusion: F2PASeg offers a reliable solution for intraoperative pituitary surgery, enhancing safety and planning.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate the effectiveness of lossy compression as a defense against adversarial attacks and understand the role of image realism in robustness.

Method: Constructed strong white-box and adaptive attacks against various compression models, analyzing attack difficulty and realism's impact.

Result: High-fidelity compression models resist attacks due to realistic reconstructions, while low-realism models are vulnerable. Realism, not gradient masking, provides robustness.

Conclusion: Realistic image reconstructions pose a significant challenge for adversarial attacks, suggesting realism as a key factor in robust security evaluation.
...
TL;DR: The paper introduces OND knowledge-based prompting to improve segmentation of historical maps using foundation models, outperforming UNet-based methods.


<details>
  <summary>Details</summary>
Motivation: Historical maps lack consistency in patterns and styles, making it difficult for pre-trained foundation models to perform accurate segmentation.

Method: The authors propose OND prompting, replacing SAM's prompt encoder and fine-tuning it on historical maps, creating SMOL-MapSeg.
...
Conclusion: OND prompting effectively addresses the challenges of historical map segmentation, offering flexibility and improved performance.
...
Method: Uses a Manager-Driven central agent to coordinate specialized sub-agents and a domain-specific knowledge base for end-to-end automation.
...
Conclusion: AutoIAD's Manager agent and knowledge base are key to robust, high-quality industrial anomaly detection solutions.
...
<details>
  <summary>Details</summary>
Motivation: Chirality information is crucial in shape analysis but underdeveloped compared to the image domain. Existing shape descriptors lack the ability to distinguish left-right symmetry.

Method: Proposes a pipeline using the Diff3F framework to extract chirality-aware features from 2D foundation models, applied to shape vertices.
...
Conclusion: The proposed method successfully addresses the gap in chirality-aware shape descriptors, demonstrating practical utility in downstream tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods rely on object templates or assume full visibility, which fails in real-world scenarios with limited viewpoints.

Method: Integrates a novel view synthesis model into the framework and aligns hand-object interactions using visible contact constraints.
...
TL;DR: A self-supervised pre-training framework for event cameras enhances feature extraction from sparse, noisy event data, outperforming state-of-the-art methods in tasks like object recognition, semantic segmentation, and optical flow estimation.
...
Method: The framework includes Difference-guided Masked Modeling, Backbone-fixed Feature Transition, and Focus-aimed Contrastive Learning to extract and enhance latent information.

Result: The framework consistently outperforms existing methods in downstream tasks.

Conclusion: The proposed approach effectively leverages event data for robust visual representation, with code and dataset publicly available.
...
TL;DR: The paper proposes an enhanced pedestrian tracking framework using richer feature representations and a robust motion model to address occlusion challenges in crowded scenes.
...
Method: The method integrates detection features from regression and classification branches, uses head keypoint detection for less occlusion-prone features, and employs an iterative Kalman filter with 3D priors for motion modeling.

Result: The proposed framework improves tracking robustness in occlusion-heavy scenarios by combining advanced appearance and motion modeling.

Conclusion: The enhanced tracking method effectively addresses occlusion challenges in crowded environments, offering a more reliable solution for multi-object tracking.
...
TL;DR: A novel certified defense method for IQA models using randomized smoothing in feature space, preserving image quality and improving efficiency.


<details>
  <summary>Details</summary>
Motivation: Prior methods degrade image quality by injecting noise in input space; this work aims to preserve fidelity while ensuring robustness.

Method: Randomized smoothing in feature space, analyzing the Jacobian's maximum singular value to connect noise levels with input-space perturbations. Supports FR and NR IQA models without architectural changes.
...
TL;DR: Octozi, an AI-assisted platform, boosts clinical trial data cleaning efficiency by 6.03-fold, reduces errors by 6.44-fold, and cuts false positives by 15.48-fold, demonstrating transformative potential in drug development.


<details>
  <summary>Details</summary>
Motivation: Manual clinical trial data cleaning is inefficient and struggles with growing data volumes and complexity, creating a bottleneck in drug development.
...
Conclusion: AI-assisted approaches can significantly improve clinical trial efficiency, accelerate timelines, reduce costs, and maintain compliance, showcasing the potential of human-AI collaboration.
...
TL;DR: The paper introduces Optimal Brain Connection, a structural pruning framework that addresses the neglect of parameter interconnections in existing methods. It uses the Jacobian Criterion for evaluating parameter saliency and the Equivalent Pruning mechanism to retain contributions of pruned connections.
...
Method: Proposes the Jacobian Criterion for parameter saliency and the Equivalent Pruning mechanism using autoencoders.
...
Conclusion: The framework improves structural pruning by considering parameter interconnections and maintaining performance.
...
<details>
  <summary>Details</summary>
Motivation: The rise of generative video models necessitates robust detection methods, but current approaches lack generalization and efficiency.

Method: SSTGNN uses a graph-based architecture with learnable spectral filters and temporal differential modeling to detect manipulation traces.
...
Conclusion: SSTGNN is a scalable, lightweight solution for detecting manipulated videos, offering strong robustness and efficiency.
...
TL;DR: The paper benchmarks Multimodal Large Language Models (MLLMs) for brand safety classification in content moderation, introducing a new dataset and comparing MLLMs' accuracy and cost efficiency to human reviewers.


<details>
  <summary>Details</summary>
Motivation: The exponential growth of online video content necessitates automated moderation due to human limitations in scale and mental health. MLLMs' potential for nuanced multimodal understanding in moderation is underexplored.

Method: A novel multimodal and multilingual dataset is introduced, labeled by professionals. MLLMs (Gemini, GPT, Llama) are evaluated for brand safety classification through comparative analysis.

Result: MLLMs demonstrate effectiveness in brand safety but have limitations and failure cases. Their accuracy and cost efficiency are compared to human reviewers.

Conclusion: The dataset is released to aid future research in responsible brand safety and content moderation, highlighting MLLMs' potential and challenges.
...
TL;DR: Action Discovery introduces a novel setup for Temporal Action Segmentation to handle ambiguous or incomplete annotations in partially labeled datasets, focusing on known and unknown actions.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of defining and annotating ambiguous or infrequent actions in partially labeled datasets, common in neuroscience and other domains.

Method: Proposes a two-step approach: Granularity-Guided Segmentation Module (GGSM) for temporal segmentation and Unknown Action Segment Assignment (UASA) for semantic classification.

Result: Demonstrates significant improvement over baselines on datasets like Breakfast, 50Salads, and Desktop Assembly.

Conclusion: The method effectively handles unknown actions in partially labeled datasets, offering a robust solution for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: The high cost and time of collecting real-world data hinder AI-generated content development, prompting the need for scalable, automated solutions.
...
Result: Synthetic data from the framework improves baseline model performance in 2D, 3D, and 4D tasks.
...
TL;DR: DART framework enhances VLP models for Open-Vocabulary Multi-Label Recognition by refining intra-class localization and leveraging LLM-derived relational knowledge for inter-class transfer, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing VLP models struggle with fine-grained localization and lack explicit use of structured relational knowledge, limiting performance in OV-MLR.

Method: DART introduces two modules: ARM for intra-class refinement with a WPS loss, and ATM for inter-class transfer using a CRG derived from an LLM.

Result: DART achieves state-of-the-art performance on challenging benchmarks.

Conclusion: DART effectively integrates adaptive intra-class refinement and inter-class transfer, improving OV-MLR performance.
...
TL;DR: WeTok tokenizer improves vision generation with Group-wise lookup-free Quantization and Generative Decoding, achieving superior compression and fidelity.
...
Method: Introduces Group-wise lookup-free Quantization (GQ) for efficient memory use and scalable codebooks, and Generative Decoding (GD) to model visual data distribution probabilistically.

Result: WeTok outperforms leading tokenizers, achieving record-low zero-shot rFID scores and higher compression ratios.

Conclusion: WeTok sets a new standard for visual tokenizers with its innovations and superior performance.
...
Result: Experimental results confirm the effectiveness of LLaVA-RE in binary image-text relevancy evaluation.

Conclusion: LLaVA-RE successfully leverages MLLMs for flexible and effective binary image-text relevancy evaluation.
...
TL;DR: Hi3DEval is a hierarchical framework for evaluating 3D generative content, combining object-level and part-level analysis with material realism assessment, outperforming existing image-based metrics.
...
Method: Hi3DEval integrates object-level and part-level evaluation, assesses material realism, and uses a hybrid 3D representation (video-based for object-level, pretrained 3D features for part-level).
...
Conclusion: Hi3DEval provides a scalable, holistic alternative to manual evaluations for 3D content quality.
...
TL;DR: MOSEv2 is a challenging dataset for video object segmentation (VOS) designed to address real-world complexities, outperforming its predecessor MOSEv1 in difficulty and variety.


<details>
  <summary>Details</summary>
Motivation: Existing VOS datasets focus on salient, isolated objects, limiting real-world applicability. MOSEv2 aims to advance VOS research by introducing complex scenarios.

Method: MOSEv2 includes 5,024 videos with 701,976 masks for 10,074 objects across 200 categories, featuring challenges like occlusions, adverse weather, and camouflaged objects.

Result: Benchmarking shows performance drops for 20 VOS methods (e.g., SAM2 from 76.4% to 50.9%) and 9 tracking methods, highlighting current limitations.

Conclusion: MOSEv2 exposes gaps in VOS methods for real-world complexities, providing a valuable resource for future research.
...
Result: Achieves high-fidelity 3D Gaussians from point clouds, tested on synthetic and real-world scans.

Conclusion: GAP effectively addresses the challenge of generating Gaussians from colorless point clouds with high accuracy.
...
Result: Generates high-quality cancelable faces with superior recognition accuracy and stronger privacy protection (11% gain over existing methods).
...
TL;DR: An LLM-based system for prescriptive maintenance combines vibration analysis and multi-agentic generation to provide actionable recommendations, improving industrial machinery upkeep.
...
Method: Integrates vibration frequency analysis (BPFO, BPFI, BSF, FTF) with multi-agentic generation, leveraging LLMs for few-shot anomaly detection and maintenance planning.

Result: Effective anomaly detection and contextually relevant maintenance guidance validated in bearing vibration datasets.
...
Method: GeoFlow provides agents with detailed tool-calling objectives for runtime API selection.
...
TL;DR: The paper introduces an adversarial benchmarking framework using board games to evaluate LLMs, addressing limitations of Q&A benchmarks. It uses Qi Town, a platform with 5 games and 20 LLM players, employing Elo ratings and PLG for technical evaluation and PSS for mental fitness. Results show LLMs' adaptability but reveal skill instability.
...
Method: Developed Qi Town, a platform with 5 games and 20 LLM players, using Elo ratings, PLG for technical analysis, and PSS for mental assessment in a round-robin tournament.

Result: LLMs show optimism and adaptability in adversarial settings but exhibit skill instability in cyclic win-loss patterns.

Conclusion: The framework effectively evaluates LLMs, revealing their strengths and weaknesses in adversarial environments, prompting further research into skill instability.
...
TL;DR: The paper compares three methods for autonomous web-based GIS, favoring a client-side small language model (SLM) for accuracy and privacy.
...
Method: Three approaches were tested: cloud-based LLMs, offline classical ML classifiers, and a client-side fine-tuned SLM (T5-small).

Result: The client-side SLM achieved the highest accuracy (0.93 exact matching, 0.99 Levenshtein similarity) and reduced server load.

Conclusion: Browser-executable SLMs are feasible for AWebGIS, offering accuracy, privacy, and scalability benefits over cloud-based methods.
...
TL;DR: The paper evaluates RL-fine-tuned LLMs in non-ideal scenarios, revealing performance declines despite improvements in ideal settings. It highlights overstated reasoning abilities and the need for realistic evaluations.


<details>
  <summary>Details</summary>
Motivation: To assess the robustness of RL-fine-tuned LLMs in realistic, non-ideal scenarios, inspired by human reasoning reliability under imperfect inputs.

Method: Fine-tuned three LLMs and an LVLM using RL (policy-gradient algorithm) and tested them on eight public datasets across three non-ideal scenarios.

Result: RL fine-tuning improves performance in ideal settings but significantly declines in non-ideal scenarios, exposing reasoning limitations.

Conclusion: Current methods inadequately address reasoning deficits in non-ideal settings, emphasizing the need for realistic evaluations and better solutions.
...
Conclusion: The work shifts focus from tool-users to self-evolving task-managers, enabling more autonomous AI for scientific discovery.
...
TL;DR: A game-theoretic framework (Docking Game) with a Loop Self-Play (LoopPlay) algorithm improves molecular docking accuracy by 10% over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Current multi-task learning models for molecular docking underperform in ligand docking due to structural complexities.

Method: Proposes a two-player game (ligand and protein players) solved by LoopPlay, alternating training and refining predictions.

Result: Achieves ~10% improvement in predicting binding modes compared to prior methods.

Conclusion: LoopPlay enhances molecular docking accuracy, benefiting drug discovery.
...
<details>
  <summary>Details</summary>
Motivation: Traditional rule-based and machine learning methods for spatial data integration are limited. LLMs offer a potential solution.
...
Method: Derived a dual-process framework (System 1 for fast intuitive behaviors, System 2 for deliberative planning) and implemented it in CogniWeb, a modular agent architecture.

Result: CogniWeb achieved a 43.96% success rate on WebArena with a 75% reduction in token usage.
...
TL;DR: The paper introduces MedMKEB, a benchmark for evaluating knowledge editing in medical multimodal large language models (MLLMs), addressing gaps in reliability, generality, and robustness.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack systematic benchmarks for updating medical knowledge involving both image and text modalities, necessitating a reliable evaluation framework.

Method: MedMKEB is built on a medical visual question-answering dataset, incorporating tasks like counterfactual correction, semantic generalization, and adversarial robustness, validated by human experts.

Result: Experiments reveal limitations in existing knowledge editing approaches for medical MLLMs, underscoring the need for specialized strategies.

Conclusion: MedMKEB aims to standardize and advance trustworthy, efficient medical knowledge editing algorithms.
...
<details>
  <summary>Details</summary>
Motivation: Analog circuit design is experience-driven and time-consuming, with existing AI methods lacking portability and efficiency.

Method: Combines finetuned Qwen3-8B with dynamic loss functions and heuristic search (DE and PSO) for efficient sizing.

Result: Performs well across multiple technology nodes without additional training, outperforming AutoCkt with significant resource savings.

Conclusion: EasySize reduces reliance on human expertise and computational resources, simplifying analog design.
...
<details>
  <summary>Details</summary>
Motivation: To address concerns about generative AI de-skilling students and explore its potential to enhance metacognitive engagement through structured dialogue.
...
Conclusion: The study advocates for orchestrated MAS in education, offering a conceptual roadmap for human-AI co-agency and pedagogical alignment.
...
TL;DR: A Heterogeneous Graph Neural Network model is proposed to reconstruct missing event attributes in Process Mining logs, outperforming state-of-the-art methods.
...
Method: Develops a Heterogeneous Graph Neural Network to restore missing event attributes, leveraging graph-based representations for richer encodings.
...
Conclusion: The proposed approach effectively addresses the challenge of missing data in event logs, offering a more comprehensive solution.
...
TL;DR: QA-Dragon improves VQA by dynamically combining text and image retrieval for complex queries, outperforming baselines in accuracy and knowledge overlap.


<details>
  <summary>Details</summary>
Motivation: Existing RAG methods retrieve from text or images separately, limiting their ability to handle complex, multi-hop VQA tasks requiring up-to-date knowledge.

Method: QA-Dragon uses a domain router for subject identification and a search router for dynamic retrieval strategy selection, enabling hybrid text-image retrieval.

Result: QA-Dragon outperforms baselines by 5.06% (single-source), 6.35% (multi-source), and 5.03% (multi-turn) on the Meta CRAG-MM Challenge.

Conclusion: QA-Dragon effectively addresses complex VQA tasks through dynamic multimodal retrieval, enhancing reasoning performance.
...
TL;DR: A framework combining RDF graph databases and LLMs improves expert identification and communication in large-scale maintenance organizations by enabling intuitive, explainable queries.


<details>
  <summary>Details</summary>
Motivation: Traditional communication methods struggle with information overload and slow response times in complex maintenance organizations.

Method: Uses RDF graph databases and LLMs for natural language query processing, with a planning-orchestration architecture for transparency.

Result: Enables precise audience targeting and explainable results, improving communication efficiency.

Conclusion: The proposed framework effectively addresses communication challenges in maintenance organizations while maintaining trust.
...
TL;DR: A hybrid architecture combines decision trees and LLMs for interpretable, robust neuro-symbolic reasoning, showing performance gains on benchmarks like ProofWriter, GSM8k, and ARC.


<details>
  <summary>Details</summary>
Motivation: To integrate symbolic reasoning (decision trees) with generative LLMs for interpretable, general-purpose neuro-symbolic reasoning, addressing limitations of loosely coupled prior approaches.

Method: Embed decision trees and random forests as callable oracles in a multi-agent framework, with LLMs handling abductive reasoning and planning, and a central orchestrator managing consistency.

Result: Improves entailment consistency (+7.2% on ProofWriter), accuracy in math problems (+5.3% on GSM8k), and abstraction accuracy (+6.0% on ARC).

Conclusion: The architecture provides a robust, interpretable, and extensible solution for neuro-symbolic reasoning, validated in clinical and scientific applications.
...
Method: Historical analysis and contemporary usage patterns inform a proposed framework with minimum requirements and a multidimensional spectrum (environmental interaction, learning, autonomy, goal complexity, temporal coherence).

Result: A clear, standardized definition and framework for 'agent' are proposed, improving research clarity and reproducibility.

Conclusion: The framework supports better communication, evaluation, and policy development, with recommendations for terminology standardization and adoption.
...
<details>
  <summary>Details</summary>
Motivation: To understand LLM behavior in open-ended, multi-agent settings, particularly in legal and ethical deliberation.
...
Result: LLMs form alliances, betray trust, and adapt rhetoric, showcasing social reasoning and persuasive skills.

Conclusion: The study highlights LLMs' potential for autonomous negotiation and legislation, informing future AI system design.
...
<details>
  <summary>Details</summary>
Motivation: Address the limitation of coarse binary rewards in guiding models through complex, multi-question reasoning tasks.

Method: Uses StructVRM, a model-based verifier for fine-grained, sub-question-level feedback, assessing semantic and mathematical equivalence.

Result: Achieves state-of-the-art performance on six out of twelve benchmarks and a new high-difficulty STEM-Bench.

Conclusion: Structured, verifiable rewards effectively enhance multimodal models' capabilities in complex reasoning domains.
...
Method: A processing pipeline with sample pre-processing, incremental ML classification, and outcome explanation, tested on the MetroPT dataset.

Result: Achieved 99% accuracy and 98% F-measure, with robustness to class imbalance and noise.

Conclusion: The pipeline is methodologically sound and practically applicable for proactive railway maintenance, enabling swift decision-making.
...
<details>
  <summary>Details</summary>
Motivation: VLMs lack attention to detail and precise action planning in dynamic environments, hindering performance in real-world tasks requiring physical reasoning.

Method: DeepPHY uses simulated environments of varying difficulty and fine-grained metrics to assess VLMs' physical reasoning.
...
TL;DR: LLMs are transforming organic synthesis by proposing routes, predicting outcomes, and automating experiments, aided by integration with other technologies. Challenges include biases and safety, but community efforts aim to democratize AI-driven chemistry.


<details>
  <summary>Details</summary>
Motivation: To explore how LLMs are revolutionizing organic synthesis by automating tasks and integrating with other tools, while addressing challenges like bias and safety.

Method: Survey of milestones in LLM adoption, coupling with graph neural networks, quantum calculations, and spectroscopy, alongside community initiatives like open benchmarks and federated learning.

Result: LLMs enhance synthetic planning, reduce discovery cycles, and enable greener chemistry, though limitations like biased datasets and opaque reasoning persist.

Conclusion: LLMs, combined with AI and automation, promise rapid, reliable molecular innovation, with ongoing efforts to ensure inclusivity and human oversight.
...
<details>
  <summary>Details</summary>
Motivation: Current AI alignment measures often ignore geographic and cultural differences, leading to outputs that may not align with local norms or realities.
...
Result: Highlights the urgency of spatio-temporally aware alignment for AI systems, especially as they scale globally.

Conclusion: Calls for more nuanced, context-sensitive alignment approaches to address geographic variability in AI behavior.
...
TL;DR: Bench-2-CoP framework reveals a significant misalignment between AI benchmarks and EU AI Act requirements, highlighting neglected systemic risks like loss-of-control scenarios.


<details>
  <summary>Details</summary>
Motivation: Address the gap between current AI benchmarks and regulatory needs under the EU AI Act, focusing on systemic risks.

Method: Introduces Bench-2-CoP, using LLM-as-judge analysis to map benchmark questions against the EU AI Act's taxonomy.
...
Conclusion: Urges policymakers and developers to refine evaluation tools for safer, compliant AI, addressing systemic risks.
...
Method: A small, resource-efficient LLM is used to synthesize six diverse ERC datasets, supplementing three benchmarks. The datasets are evaluated for classification and label imbalance effects.

Result: Generated datasets improve ERC classifier robustness and performance, showing statistically significant gains on benchmarks.

Conclusion: Synthetic datasets from a small LLM effectively address ERC data challenges, enhancing model performance and robustness.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for improving LLM reasoning are resource-intensive and lack scalability due to heuristic or task-specific data curation.
...
TL;DR: GRAIL, a framework integrating LLMs with graph retrieval, improves reasoning on structured knowledge by balancing precision and conciseness, achieving significant accuracy and F1 improvements.


<details>
  <summary>Details</summary>
Motivation: Existing RAG methods struggle with structured knowledge like graphs, facing precision and information gap issues, necessitating a better approach.

Method: GRAIL combines LLM-guided exploration, path filtering, and a two-stage training process to dynamically optimize graph retrieval.

Result: GRAIL improves accuracy by 21.01% and F1 by 22.43% on knowledge graph QA datasets.

Conclusion: GRAIL effectively addresses structured knowledge retrieval challenges, enhancing reasoning performance with interactive learning.
...
Result: The framework achieves 4.76% and 10.52% higher alignment accuracy with human evaluations compared to GPT-4o baselines.
...
<details>
  <summary>Details</summary>
Motivation: Reviewing text-heavy LORs is time-consuming; LORI automates leadership skill assessment to streamline admissions and support professional growth.

Method: Uses natural language processing (NLP) with RoBERTa and LLAMA models to detect leadership attributes like teamwork, communication, and innovation.

Result: RoBERTa model achieves 91.6% F1 score, 92.4% precision, and 91.6% recall, showing strong consistency.

Conclusion: LORI enhances graduate admissions by automating leadership skill evaluation, crucial for STEM sectors, ensuring comprehensive candidate assessment.
...
TL;DR: MV-Debate, a multi-view agent debate framework, improves harmful content detection in social media by combining diverse interpretive perspectives and dynamic reflection.
...
Method: MV-Debate uses four debate agents (surface analyst, deep reasoner, modality contrast, social contextualist) for iterative analysis and reflection-gated refinement.

Result: Outperforms single-model and multi-agent baselines on three benchmark datasets.

Conclusion: Multi-agent debate enhances reliable intent detection in safety-critical online environments.
...
Method: Proposes using AIF to replace external rewards with an intrinsic drive to minimize free energy, integrating Large Language Models for generative world modeling.

Result: AIF offers a unified Bayesian objective for balancing exploration and exploitation, enabling efficient learning from experience.

Conclusion: AIF combined with generative models provides a path to autonomous, aligned AI systems that adhere to computational and physical constraints.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fail to capture learning dynamics, track progress, or provide explainability. LearnerAgent addresses these gaps by simulating realistic teaching environments.

Method: Uses LLMs to create learners with profiles (Deep, Surface, Lazy, General). Tracks progress via weekly knowledge acquisition, monthly strategic choices, tests, and peer interaction over a year.

Result: 1) Deep Learner shows sustained growth; trap questions diagnose Surface Learner. 2) Behavioral patterns match profiles. 3) Self-concept evolves realistically. 4) Base LLM defaults to a diligent but brittle Surface Learner.

Conclusion: LearnerAgent effectively simulates human learning, providing insights into LLM behavior and cognitive processes.
...
TL;DR: The paper proposes enriching dialogue transcripts with speaker metadata (age, gender, emotion) using frozen audio and language models without fine-tuning, achieving competitive performance and modularity.


<details>
  <summary>Details</summary>
Motivation: To enhance dialogue transcripts by adding speaker characteristics metadata, improving utility beyond grammar and readability.

Method: Combines frozen audio models (Whisper, WavLM) with a frozen LLAMA model via lightweight connectors to infer speaker attributes without fine-tuning.

Result: Competitive performance in speaker profiling; frozen LLAMA compares x-vectors with an 8.8% Equal Error Rate in some cases.

Conclusion: The approach effectively enriches transcripts with speaker metadata while maintaining modularity and efficiency.
...
Method: Introduces Parity-aware BPE, prioritizing compression gain for the worst-compressed language at each merge step.
...
Conclusion: Parity-aware BPE offers a fairer tokenization solution without sacrificing efficiency or performance.
...
TL;DR: Joint ASR and pitch accent detection model improves ASR performance and pitch accent detection, reducing WER by 28.3% and closing the F1-score gap by 41%.
...
Result: Pitch accent detection improves by 41% in F1-score, and ASR WER drops by 28.3% on LibriSpeech.
...
TL;DR: The paper evaluates personality stability in large language models (LLMs) using the PERSIST framework, revealing significant response variability and instability across models and interventions.


<details>
  <summary>Details</summary>
Motivation: To understand the personality-like traits of LLMs and assess their behavioral consistency for safe deployment.

Method: The study employs the PERSIST framework, testing 25+ open-source models with traditional and LLM-adapted personality instruments, varying question order, paraphrasing, personas, and reasoning modes.

Result: Key findings include substantial response variability in large models, prompt sensitivity (up to 20% shift in personality measurements), and paradoxical increases in variability from stabilization interventions.

Conclusion: Current LLMs lack foundational behavioral consistency, suggesting personality-based alignment strategies may be inadequate for safety-critical applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing coordination schemes in multi-agent LLMs are inefficient due to static routing, excessive token use, and limited adaptability.
...
Conclusion: RCR-Router highlights the importance of structured memory routing and output-aware evaluation for scalable multi-agent LLM systems.
...
TL;DR: A benchmark for evaluating LLMs' responses to linguistic shibboleths reveals biases, particularly against hedging language, with hedged responses receiving 25.6% lower ratings.


<details>
  <summary>Details</summary>
Motivation: To detect and measure demographic bias in LLMs through linguistic markers, ensuring fairness in automated systems.

Method: Constructed interview simulations with 100 validated question-response pairs, isolating linguistic variations while maintaining semantic equivalence.

Result: Hedged responses were penalized, receiving 25.6% lower ratings, revealing model-specific biases.
...
Method: A vision-language clustering framework constructs verb sense clusters to represent distinct perspectives of images.

Result: Analysis shows each image maps to 2.8 sense clusters on average, and cluster-based evaluation aligns better with human judgements.

Conclusion: The proposed framework offers a more nuanced and robust evaluation of activity recognition models.
...
TL;DR: A multi-stage LLM framework improves extraction of SDoH factors from text, outperforming state-of-the-art models and enhancing explainability for suicide prevention.


<details>
  <summary>Details</summary>
Motivation: Address challenges in data-driven SDoH analysis for suicide prevention, including long-tailed factor distributions and model explainability.

Method: Proposes a multi-stage LLM framework, comparing it to BioBERT, GPT-3.5-turbo, and DeepSeek-R1, and evaluating its explainability via a user study.

Result: The framework boosts SDoH extraction performance, provides better context retrieval, and achieves cost-efficient results with fine-tuning.

Conclusion: The approach enhances accuracy and transparency in SDoH extraction, aiding early risk identification and prevention strategies.
...
Conclusion: Partitioning dialogues into semantically independent sub-dialogues and using a two-step extraction framework effectively improves DiaASQ performance.
...
TL;DR: Finetuning decoder-only LLMs like Phi 3.5, Gemma 2, LLaMA 3.2, and DeepSeek R1 achieves comparable AMR parsing performance to SOTA methods, with LLaMA 3.2 notably competitive.
...
Result: LLaMA 3.2 achieved SMATCH F1: 0.804, matching IBM's APT + Silver (0.804) and nearing MBSE's Graphene (0.854). Phi 3.5 excelled in structural validity.

Conclusion: Decoder-only LLMs, especially LLaMA 3.2, offer a promising, simpler alternative to complex SOTA AMR parsers.
...
Method: Introduces Align-LoRA, a single-adapter LoRA with increased rank and an explicit alignment loss for task representation sharing.

Result: Align-LoRA outperforms complex multi-adapter and multi-head systems, validating the effectiveness of shared representations.

Conclusion: Simpler, shared-representation approaches like Align-LoRA are more effective for adapting LLMs to multiple tasks.
...
TL;DR: The paper introduces MultiCheck, a framework for multimodal fact verification, combining text and image analysis to improve accuracy over text-only methods.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of multimodal misinformation by integrating visual and textual evidence for more accurate fact-checking.

Method: Proposes a unified framework with dedicated encoders for text and images, a fusion module for cross-modal relationships, and a contrastive learning objective.
...
<details>
  <summary>Details</summary>
Motivation: To improve RAG performance by mitigating issues caused by long retrieval contexts, such as unconstrained entropy growth and attention dilution.
...
Result: BEE-RAG demonstrates effectiveness across multiple RAG tasks.

Conclusion: BEE-RAG enhances RAG adaptability to varying context lengths, ensuring stable performance.
...
TL;DR: The paper identifies positional bias in LLMs, termed 'attention basin,' where models focus on the start and end of sequences. It introduces AttnRank, a method to rerank inputs to align critical information with high-attention positions, improving performance without training.


<details>
  <summary>Details</summary>
Motivation: To understand and mitigate the positional bias in LLMs, where models overlook middle items in sequences, impacting performance.

Method: Proposes AttnRank, a two-stage framework: (i) calibrates positional attention preferences, (ii) reranks inputs to align critical content with high-attention positions.

Result: AttnRank improves performance across 10 LLMs in multi-hop QA and few-shot learning tasks without modifying models.

Conclusion: AttnRank effectively addresses positional bias in LLMs, enhancing performance with minimal overhead, and is model-agnostic.
...
<details>
  <summary>Details</summary>
Motivation: Current benchmarks overlook ethical reasoning in LLMs for healthcare, necessitating a systematic evaluation.
...
Result: LLMs show gaps in applying ethical principles, especially Beneficence. Closed-source models lead, but medical fine-tuning helps.

Conclusion: PrinciplismQA provides a scalable framework to improve ethical alignment in medical AI.
...
<details>
  <summary>Details</summary>
Motivation: LLMs advance NLG but often produce incorrect or misleading content (hallucinations), necessitating methods to detect and mitigate such issues.

Method: Explored approaches with/without external context: few-shot prompting with LLMs, token-level classification, and fine-tuning on synthetic data.
...
TL;DR: MulCoT-RD, a lightweight model, addresses multimodal sentiment reasoning and classification in resource-limited settings using a distillation approach, achieving strong performance with 3B parameters.


<details>
  <summary>Details</summary>
Motivation: Current MSA methods rely on large LLMs, ignoring resource constraints. The paper focuses on autonomous reasoning and classification in such environments.

Method: Proposes MulCoT-RD, a "Teacher-Assistant-Student" distillation model, leveraging MLLMs for initial reasoning data, training an assistant, and a lightweight student.

Result: MulCoT-RD achieves strong performance on JMSRC with 3B parameters, showing robust generalization and interpretability.

Conclusion: The approach successfully balances performance and resource efficiency, offering a practical solution for resource-limited MSA.
...
<details>
  <summary>Details</summary>
Motivation: Current pruning methods disrupt LLM functionality by ignoring neuron interactions. The study aims to preserve functional networks for better pruning results.
...
Result: Successfully identifies and preserves functional networks and key neurons, enabling efficient pruning without performance degradation.

Conclusion: The proposed method improves pruning efficiency by maintaining LLM functionality, offering practical value for real-world applications.
...
TL;DR: CodeBoost is a post-training framework for code LLMs that enhances performance using code snippets alone, bypassing the need for human-annotated instructions.


<details>
  <summary>Details</summary>
Motivation: Existing code LLMs rely on labor-intensive human-annotated instructions, creating a scalability bottleneck. Code snippets are abundant but underutilized.
...
TL;DR: The paper challenges the 'cascading failure' hypothesis in Chain-of-Thought (CoT) prompting, revealing 'Late-Stage Fragility'—errors in later reasoning stages are more harmful. It introduces ASCoT, an adaptive self-correction method, which outperforms standard CoT by targeting high-risk late-stage errors.
...
Method: Introduces ASCoT with an Adaptive Verification Manager (AVM) and Multi-Perspective Self-Correction Engine (MSCE). AVM uses a Positional Impact Score to prioritize late-stage steps, and MSCE applies dual-path correction to these steps.

Result: ASCoT achieves higher accuracy than standard CoT on benchmarks like GSM8K and MATH, demonstrating effectiveness in mitigating Late-Stage Fragility.

Conclusion: The work highlights the need for adaptive, vulnerability-aware correction in LLM reasoning, moving beyond uniform verification strategies.
...
Method: Synthetic data is generated using LLMs with self-refinement for relevance. This data trains dialogue agents tailored for meta-reviewing.

Result: The method produces high-quality synthetic data and outperforms off-the-shelf LLM-based assistants in meta-reviewing tasks.
...
<details>
  <summary>Details</summary>
Motivation: To retain the semantic abstraction of LCM while eliminating its diffusion sampler and restoring likelihood-based training.
...
Conclusion: SONAR-LLM successfully balances semantic abstraction and likelihood-based training, with released code and checkpoints for reproducibility.
...
Method: Certainty-Guided Reflection Suppression (CGRS) dynamically suppresses reflection triggers during high-confidence responses.

Result: CGRS reduces token usage significantly (18.5%-41.9%) while maintaining accuracy across benchmarks and models.
...
TL;DR: Study on adding adjustment features to a sign language avatar on Hololens 2 found no significant UX or comprehensibility improvements, despite user preference for adjustability. Key issues included missing SL elements and poor implementation.


<details>
  <summary>Details</summary>
Motivation: To evaluate the impact of adjustable features on a sign language avatar's comprehensibility, UX, and acceptability.

Method: Analyzed interactions of expert German Sign Language users with adjustable and non-adjustable avatars on Hololens 2.

Result: No significant UX or comprehensibility gains; higher stress with adjustable avatar. Hedonic quality rated higher than pragmatic quality.

Conclusion: Personalisation alone is insufficient; avatars must be comprehensible by default. Recommendations include improving animation and interaction interfaces.
...
TL;DR: The paper explores the challenges and performance of agentic RAG and deep research systems in domain-specific professional search, particularly biomedical research, using LLMs with self-feedback mechanisms.
...
Method: Evaluated reasoning and non-reasoning LLMs (e.g., Gemini-Flash 2.0, o3-mini) using a self-feedback mechanism for iterative output refinement and query expansion.

Result: Preliminary results show varied performance of the self-feedback strategy across models and tasks.

Conclusion: The study provides insights into LLM self-correction and informs future comparisons between LLM-generated feedback and human expert input.
...
TL;DR: A parallel corpus of 12 sign languages in video format with subtitles, totaling 1,300 hours and 14M tokens, including first-time consistent data for 8 Latin American sign languages.


<details>
  <summary>Details</summary>
Motivation: To address the lack of parallel corpora for sign languages, especially for Latin American ones, and to significantly expand resources like German Sign Language.

Method: Collected and processed videos from online sources (news shows, governmental bodies, educational channels), involving data collection, approvals, scraping, and cropping.

Result: A large-scale collection of 4,381 video files with 1.3M subtitles, ten times larger than previous German Sign Language corpora.

Conclusion: The paper presents a valuable resource for sign language research, particularly for underrepresented languages, and outlines the methods for creating such corpora.
...
TL;DR: MyCulture is a benchmark to evaluate LLMs on Malaysian culture using an open-ended multiple-choice format, addressing biases in cultural and linguistic representation.


<details>
  <summary>Details</summary>
Motivation: LLMs exhibit cultural biases due to training data dominated by high-resource languages, necessitating benchmarks for diverse cultural contexts.

Method: Introduces MyCulture, a benchmark with open-ended multiple-choice questions in Bahasa Melayu across six cultural pillars, analyzing structural and language biases.

Result: Evaluation shows significant disparities in cultural comprehension among LLMs, emphasizing the need for inclusive benchmarks.
...
TL;DR: LLMEval-3 introduces a dynamic evaluation framework for LLMs using a proprietary question bank, automated integrity checks, and calibrated scoring to address data contamination and overfitting, revealing true model capabilities.


<details>
  <summary>Details</summary>
Motivation: Static benchmarks for LLMs are prone to data contamination and leaderboard overfitting, obscuring true model performance.

Method: LLMEval-3 dynamically samples unseen test sets from 220k graduate-level questions, uses contamination-resistant curation, anti-cheating architecture, and LLM-as-a-judge with 90% human agreement.

Result: A 20-month study of 50 models shows performance ceilings on memorization and exposes contamination issues, with robust ranking stability.

Conclusion: LLMEval-3 provides a credible, dynamic evaluation paradigm for LLMs, promoting trustworthy standards beyond static benchmarks.
...
TL;DR: TASE is a benchmark evaluating LLMs' token-level understanding and structural reasoning across languages, revealing significant gaps compared to human performance.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with fine-grained, token-level tasks despite excelling in high-level semantics, necessitating a benchmark like TASE to diagnose and improve these weaknesses.

Method: TASE includes 10 tasks (e.g., character counting, syntactic parsing) across Chinese, English, and Korean, with a 35,927-instance evaluation set and synthetic data generation. Over 30 LLMs were tested, and a custom Qwen2.5-14B model was trained using GRPO.

Result: Human performance significantly outperforms current LLMs, highlighting persistent weaknesses in token-level reasoning.

Conclusion: TASE provides a diagnostic tool for improving LLMs' low-level language understanding and cross-lingual generalization, with publicly available code and dataset.
...
<details>
  <summary>Details</summary>
Motivation: To systematically assess and compare existing creativity measures across diverse creative domains to understand their effectiveness and limitations.

Method: Examination and analysis of four representative creativity metrics (creativity index, perplexity, syntactic templates, LLM-as-a-Judge) in creative writing, problem-solving, and research ideation.

Result: Metrics show limited consistency, each capturing different aspects of creativity, with notable limitations like bias, instability, and narrow focus.

Conclusion: The study emphasizes the need for more robust and generalizable creativity evaluation frameworks aligned with human judgments.
...
TL;DR: LAG improves LLMs by decomposing complex questions into logical sub-questions, ensuring stepwise reasoning and reducing hallucinations.


<details>
  <summary>Details</summary>
Motivation: Address LLMs' limitations in knowledge-intensive tasks and RAG's struggles with complex reasoning.

Method: Introduces LAG: decomposes questions into sub-questions, resolves them sequentially, and synthesizes answers with logical grounding.

Result: LAG enhances reasoning robustness, reduces hallucinations, and aligns with human cognition.

Conclusion: LAG offers a principled alternative to RAG, improving LLM performance in complex tasks.
...
<details>
  <summary>Details</summary>
Motivation: To uncover subtle biases in LLMs by analyzing their behavior when proactively asking questions, avoiding direct probing that may trigger guardrails.

Method: Evaluated LLMs using the 20 Questions game (canonical and unlimited turns) in seven languages on the Geo20Q+ dataset, which includes notable people and culturally significant objects from diverse regions.

Result: LLMs show significant geographic disparities, performing better for entities from the Global North and West. Wikipedia pageviews and corpus frequency only partially explain these gaps. Language had minimal impact.

Conclusion: Creative evaluation frameworks like 20 Questions reveal hidden biases in LLMs, highlighting geographic and cultural disparities in their reasoning processes.
...
TL;DR: CoCoLex, a decoding strategy for legal text generation, improves faithfulness to context by dynamically blending model-generated and context-derived distributions, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: LLMs' potential in legal tasks is limited by unfaithful outputs. Retrieval-Augmented Generation lacks guaranteed context integration, and existing decoding strategies don't enforce faithfulness.

Method: Introduces Confidence-guided Copy-based Decoding (CoCoLex), which interpolates model-generated and context-derived distributions to encourage direct copying based on confidence.
...
Conclusion: CoCoLex enhances faithfulness to context in legal text generation, addressing a key limitation of LLMs in the legal domain.
...
Method: Proposes frequency-based uncertainty quantification via multiple independent samplings and predictive entropy (PE), leveraging conformal prediction for coverage guarantees.
...
TL;DR: The study examines whether political opinions in multilingual large language models (MLLMs) vary across languages or transfer uniformly. Findings show minimal cross-lingual differences in unaligned models, with alignment shifts affecting all languages similarly.


<details>
  <summary>Details</summary>
Motivation: To investigate if political opinions in MLLMs differ by language or transfer across languages, addressing socio-cultural and linguistic alignment challenges.
...
Result: Unaligned models showed few significant cross-lingual differences. Political alignment shifted opinions uniformly across all languages.
...
TL;DR: MathSmith synthesizes high-difficulty math problems from scratch using concept-explanation pairs and reinforcement learning, outperforming baselines in benchmarks.
...
Method: MathSmith constructs problems by sampling concept-explanation pairs, uses reinforcement learning for optimization, and measures cognitive complexity via reasoning trace length.
...
Conclusion: MathSmith demonstrates scalability, generalization, and transferability, proving synthetic high-difficulty data's potential for advancing LLM reasoning.
...
TL;DR: The paper proposes Cooper, a RL framework that jointly optimizes policy and reward models to address limitations of rule-based and model-based rewards, enhancing robustness and mitigating reward hacking.


<details>
  <summary>Details</summary>
Motivation: Current reward paradigms (rule-based and model-based) have limitations: rule-based lacks robustness, and model-based is prone to reward hacking. The paper aims to address these issues.

Method: Introduces Cooper, a framework co-optimizing policy and reward models, using rule-based rewards for precision and dynamic sample selection for reward model training. Also proposes a hybrid annotation strategy and reference-based reward modeling.

Result: Cooper mitigates reward hacking and improves RL performance, e.g., a 0.54% accuracy gain on Qwen2.5-1.5B-Instruct. VerifyRM, a reward model, achieves higher accuracy on VerifyBench.

Conclusion: Dynamically updating reward models effectively combats reward hacking, offering insights for better integrating reward models into RL.
...
<details>
  <summary>Details</summary>
Motivation: To assess language models' capacity for embodied reasoning, including physical interactions, tool usage, and multi-agent coordination, which remains underexplored.

Method: OmniEAR uses text-based environments to model 1,500 scenarios, requiring dynamic capability acquisition and autonomous coordination without predefined tools or directives.
...
TL;DR: R-LLMs struggle with factuality in long-form tasks. A new reward function for online RL improves factuality, detail, and relevance, reducing hallucinations by 23.1%.


<details>
  <summary>Details</summary>
Motivation: Address the high hallucination rates of R-LLMs in long-form factuality tasks and the challenges of applying online RL due to unreliable verification methods.

Method: Propose a novel reward function for online RL that balances factual precision, response detail, and relevance.

Result: Achieves a 23.1% reduction in hallucination rate, 23% increase in detail, and maintains response helpfulness.
...
Method: Apply linear probes trained on persuasion aspects: success, personality, and strategy.

Result: Probes capture persuasion dynamics effectively, identifying key moments and outperforming prompting in strategy detection.

Conclusion: Probes are efficient for studying complex behaviors like deception in multi-turn settings.
...
<details>
  <summary>Details</summary>
Motivation: Byte-level models face computational challenges in morphologically-rich languages (MRLs) due to long byte sequences per word. H-NET++ aims to address this by learning efficient, linguistically-informed segmentation.

Method: H-NET++ uses a lightweight Transformer for cross-chunk attention, a two-level latent hyper-prior for document consistency, specialized handling of orthographic artifacts, and curriculum-based training with staged sequence lengths.

Result: On a Persian corpus, H-NET++ outperforms BPE-based GPT-2-fa with 12% better compression, 5.4pp gain on ParsGLUE, 53% improved robustness to ZWNJ corruption, and 73.8% F1 on morphological boundaries.

Conclusion: H-NET++ demonstrates that hierarchical dynamic chunking is an effective tokenizer-free solution for MRLs, aligning with morphology and maintaining computational efficiency.
...
TL;DR: NAEx is a model-agnostic framework for explaining network alignment decisions by identifying key subgraphs and features.
...
Result: NAEx effectively explains alignment decisions and integrates with four NA models, showing efficiency on benchmarks.

Conclusion: NAEx enhances interpretability and trust in network alignment models by providing faithful and meaningful explanations.
...
TL;DR: LumiGen is an LVLM-enhanced framework for T2I generation, improving fine-grained control and semantic consistency through iterative feedback, outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: Existing T2I models struggle with complex instructions, fine-grained control, and semantic consistency, prompting the need for a more advanced solution.
...
Result: LumiGen achieves a superior score of 3.08 on LongBench-T2I Benchmark, excelling in text rendering and pose expression.

Conclusion: LVLM integration in LumiGen enhances controllability and quality in T2I generation, addressing key challenges effectively.
...
TL;DR: MissMecha is an open-source Python toolkit for simulating, visualizing, and evaluating missing data under MCAR, MAR, and MNAR assumptions, supporting both numerical and categorical features.


<details>
  <summary>Details</summary>
Motivation: Incomplete data is a common issue in real-world datasets, but existing tools are fragmented, limited to specific mechanisms, and often ignore categorical variables.

Method: MissMecha provides a unified platform for simulating missingness, including visual diagnostics, MCAR testing, and type-aware imputation evaluation.

Result: The toolkit supports mechanism-aware studies on mixed-type tabular data, aiding research, benchmarking, and education.

Conclusion: MissMecha addresses gaps in existing tools by offering comprehensive support for missing data analysis in heterogeneous datasets.
...
TL;DR: A novel cluster-aware hierarchical federated aggregation framework is proposed to enhance personalized content generation on edge devices, addressing privacy, efficiency, and scalability challenges.


<details>
  <summary>Details</summary>
Motivation: Diffusion models (DMs) face computational and privacy challenges on edge devices, and existing cloud-based solutions lack efficiency and personalization in multi-user scenarios.
...
Result: The framework achieves accelerated convergence and practical viability for scalable multi-user personalized AIGC services on edge devices.

Conclusion: The proposed solution effectively balances privacy, efficiency, and scalability in edge-AIGC scenarios.
...
TL;DR: The paper introduces a Large Multi-Modal Model (LMMM) for few-shot learning (FSL), outperforming meta-learning methods. It includes a dataset (M3FD) and framework (M3F) for diverse scientific applications.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of FSL in data-scarce scientific domains where traditional methods fall short.

Method: Developed M3FD (a multi-modal dataset) and M3F (a framework) to train and fine-tune LMMMs for FSL tasks.

Result: M3F improves FSL performance, making LMMMs viable for real-world scientific applications.

Conclusion: The work provides a scalable, unified solution for FSL in data-constrained fields, promoting accessibility and reproducibility.
...
Method: AttriLens-Mol uses three rewards: format, count, and rationality, to guide LLM reasoning and extract relevant molecular attributes.

Result: The method outperforms supervised fine-tuning and advanced models, enhancing interpretability and performance.

Conclusion: AttriLens-Mol effectively elicits relevant molecular attributes, improving prediction accuracy and interpretability.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods often ignore textual perturbations, which degrade performance in multimodal time series forecasting.

Method: PA-RNet uses a perturbation-aware projection module and cross-modal attention to filter noise while preserving semantic meaning.

Result: The model shows strong stability under noise and outperforms baselines in diverse experiments.

Conclusion: PA-RNet effectively addresses textual noise in multimodal forecasting, offering theoretical guarantees and practical improvements.
...
Method: InfoQ quantizes layers at varying bit-widths, measures mutual information changes in subsequent layers via a single forward pass, and formulates bit-width allocation as an integer linear programming problem.
...
TL;DR: LLMs show promise as zero-shot dynamic insulin dosing agents, matching or outperforming trained RL agents in stable cases, but exhibit limitations like aggressive dosing and reasoning errors, necessitating careful integration and hybrid approaches.


<details>
  <summary>Details</summary>
Motivation: To evaluate the feasibility of using LLMs for clinical decision-making in dynamic treatment regimes (DTRs) without extensive training, leveraging their implicit knowledge and linguistic capabilities.

Method: Comparison of open-source LLMs (e.g., Qwen2.5-7B) and small neural network-based RL agents (SRAs) in an in silico Type 1 diabetes simulator, focusing on zero-shot inference performance and prompt engineering.

Result: LLMs achieved comparable or superior performance to SRAs in stable cohorts but showed issues like aggressive dosing and reasoning errors. Explicit reasoning about latent states provided minimal gains.

Conclusion: LLMs can be cautiously integrated into clinical workflows with targeted prompt engineering and hybrid approaches, but validation and structured modeling are essential for safety and robustness.
...
Conclusion: EPOPR successfully balances efficiency and equity in power restoration, addressing disparities in request submissions.
...
TL;DR: F3CRec integrates Federated Learning and Continual Learning for privacy-preserving recommendation systems, addressing non-stationary data streams.


<details>
  <summary>Details</summary>
Motivation: Existing Federated Recommendation methods struggle with evolving user preferences, while Continual Learning methods assume centralized data, creating a gap.
...
Conclusion: F3CRec effectively bridges the gap between privacy preservation and adaptability in recommendation systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing systems prioritize operator revenue, often neglecting passenger fairness and driver preference, leading to poor user experience.

Method: HCRide employs Habic, featuring a multi-agent competition mechanism, dynamic Actor network, and Bi-Critic network, to optimize efficiency, fairness, and driver preference.

Result: HCRide improves system efficiency by 2.02%, fairness by 5.39%, and driver preference by 10.21% over baselines, validated on real-world datasets.

Conclusion: HCRide successfully addresses the trade-off between passenger fairness and driver preference, enhancing overall ride-hailing service quality.
...
<details>
  <summary>Details</summary>
Motivation: Addresses limitations of autoregressive models in long-range forecasting, such as error accumulation and inefficiency.
...
Conclusion: The framework enables coherent long-horizon event trajectory generation without sequential decoding, offering a promising alternative to autoregressive models.
...
TL;DR: A multi-stage intrusion detection framework for CAN protocol combines unsupervised anomaly detection (VGAE) and supervised graph learning (KD-GAT) to improve security against cyber-attacks.
...
Method: Uses VGAE for structural anomaly detection and KD-GAT for attack classification, encoding CAN bus activity as graph sequences. Addresses class imbalance with VGAE-based undersampling.

Result: Achieves 96% parameter reduction in the student GAT model, with 16.2% average F1-score improvement over existing methods.

Conclusion: The framework demonstrates competitive accuracy and efficiency, especially on imbalanced datasets, enhancing CAN security.
...
<details>
  <summary>Details</summary>
Motivation: Despite OPTQ's widespread use in reducing memory and compute costs for deep neural networks, it lacks rigorous theoretical guarantees. This paper aims to fill that gap by analyzing its error bounds and extending the analysis to Qronos.

Method: The paper analyzes OPTQ's iterative procedure to derive non-asymptotic 2-norm error bounds, considering calibration data and regularization. It also examines the stochastic variant for infinity-norm bounds and extends the analysis to Qronos.

Result: The analysis provides explicit error bounds for OPTQ and Qronos, justifying practical heuristics like feature ordering by norm and offering parameter selection guidance. Stronger infinity-norm bounds for the stochastic variant enable better control over quantization.

Conclusion: The paper bridges the theoretical gap for OPTQ and Qronos, offering insights into their performance and practical use, which can enhance their application in deep learning.
...
<details>
  <summary>Details</summary>
Motivation: Low-resource languages lack pre-training data and require extensive per-language post-training setups, hindering LLM performance.
...
Result: Agnostics improves Qwen-3 4B to rival larger models, scales to diverse model families, and sets new state-of-the-art results on benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods like FNO have limitations, such as periodicity assumptions, and lack phase-sensitive analysis. HNO aims to address these by incorporating signal processing insights.

Method: HNO maps input signals to their analytic representation via the Hilbert transform, then applies spectral convolution to this representation for learning.

Result: HNO is designed to better model causal, phase-sensitive, and non-stationary systems compared to traditional methods.

Conclusion: HNO offers a promising alternative to FNO by integrating analytic signal theory, potentially improving performance for specific PDE problems.
...
<details>
  <summary>Details</summary>
Motivation: To investigate whether dynamics can be directly applied to probability measures in neural networks, moving beyond the mean-field theory's focus on infinitely wide networks.
...
Result: GM layers achieve comparable test performance to two-layer fully connected networks in simple classification tasks and exhibit distinct behavior.

Conclusion: The GM layer offers a novel approach with different dynamics compared to classical layers, even in mean-field regimes.
...
TL;DR: A deep learning-based U-Net model with uncertainty quantification is used to predict surface ozone residuals, improving bias correction for health-relevant scales.


<details>
  <summary>Details</summary>
Motivation: Traditional physics-based models fail to capture human-health relevant ozone trends, and existing deep learning methods lack interpretability for policy decisions.

Method: An uncertainty-aware U-Net architecture employs Bayesian and quantile regression to predict ozone residuals, tested in North America and Europe.

Result: The model identifies optimal ground stations for bias correction and evaluates land-use impact on ozone residuals.

Conclusion: The approach enhances interpretability and practical utility for policy and public health decisions.
...
Method: Uses a 2D Convolutional Neural Network to estimate surface ozone model residuals (bias) and incorporates high-resolution satellite land use data.

Result: The CNN-based approach outperforms traditional ML in capturing model residuals and improves ozone estimates in North America and Europe.
...
TL;DR: The paper evaluates performance metrics for neonatal seizure detection models, proposing best practices to address biases and inconsistencies in current evaluation methods.


<details>
  <summary>Details</summary>
Motivation: Current evaluation practices for neonatal seizure detection models are inconsistent and biased, leading to unreliable claims about AI performance. The study aims to improve comparability and interpretability.

Method: The study assesses standard metrics, consensus strategies, and human-expert equivalence tests using real and synthetic seizure annotations, considering class imbalance, inter-rater agreement, and number of raters.
...
Conclusion: The study recommends reporting balanced metrics, sensitivity, specificity, PPV, NPV, and multi-rater Turing test results on held-out validation sets to ensure reliable evaluation of AI models for neonatal seizure detection.
...
TL;DR: The paper analyzes replicability in transfer learning, introducing a measure called selection sensitivity ($\Delta_Q$) to quantify trade-offs between adaptation effectiveness and consistency. It shows that replicability failure probability increases with selection sensitivity and decreases with sample size, validated through experiments on the MultiNLI corpus. Highly adaptive strategies perform better but are less replicable, while source domain pretraining mitigates replicability issues.


<details>
  <summary>Details</summary>
Motivation: To understand the reliability of transfer learning adaptations, especially with adaptive data selection strategies, which lack comprehensive analysis.

Method: Theoretical framework for replicability, introducing selection sensitivity ($\Delta_Q$), and empirical validation using six adaptive selection strategies on the MultiNLI corpus.

Result: Highly adaptive strategies (e.g., gradient-based) perform better but have high replicability failure rates (up to 30%). Less adaptive methods maintain failure rates below 7%. Source domain pretraining reduces failure rates by up to 30%.
...
<details>
  <summary>Details</summary>
Motivation: Hate speech on social media has serious real-world impacts, necessitating robust automated detection methods. Existing deep learning approaches have limitations, prompting the exploration of transformer models.
...
Conclusion: Transformer models, especially ELECTRA, are effective for hate speech detection, but challenges remain in handling nuanced language.
...
TL;DR: ALScope is a new Deep Active Learning (DAL) platform for classification, integrating 10 datasets and 21 DAL algorithms, enabling fair evaluation under diverse conditions like distribution shifts and data imbalance.


<details>
  <summary>Details</summary>
Motivation: The lack of a unified platform for fair and systematic evaluation of DAL algorithms under diverse conditions (e.g., distribution shifts, data imbalance) prompted the development of ALScope.

Method: ALScope integrates 10 datasets (CV and NLP) and 21 DAL algorithms, supporting flexible configuration of experimental factors like OOD ratio and class imbalance. Extensive experiments are conducted under various settings.

Result: Findings show DAL performance varies by domain/task, algorithms need improvement in non-standard scenarios (imbalanced/open-set), and some algorithms trade performance for longer selection time.

Conclusion: ALScope provides a comprehensive platform for DAL evaluation, highlighting performance variability and areas for improvement, especially in non-standard scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Balancing translation quality and latency in SimulST systems is challenging. The goal is to optimize this tradeoff by leveraging information theory principles.
...
Conclusion: REINA effectively optimizes SimulST performance, demonstrating significant improvements in both quality and latency.
...
TL;DR: SEA (Self-Error Adjustment) is a novel ensemble learning framework that decomposes errors into individual performance and diversity terms, enabling precise control over the accuracy-diversity trade-off, outperforming traditional methods like NCL.
...
Result: SEA outperforms baseline methods (e.g., NCL) in regression and classification tasks, offering broader adjustment range and tighter theoretical bounds.

Conclusion: SEA provides flexible, superior performance in ensemble learning by precisely balancing accuracy and diversity, validated by empirical results.
...
TL;DR: The paper introduces two compressed decentralized algorithms for nonconvex stochastic optimization, combining momentum and message-compression techniques to achieve fast convergence and reduced communication costs.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in theoretically proving the effectiveness of combining momentum acceleration and compressed communication in decentralized algorithms while controlling consensus, compression, and momentum gradient errors.

Method: Two algorithms are proposed: a compressed decentralized adaptive method for bounded gradients and a compressed decentralized heavy-ball method for data heterogeneity, both using gradient tracking and achieving optimal convergence rates.

Result: Both methods achieve optimal convergence rates, linear speedup, and topology-independent parameters, outperforming state-of-the-art methods in training DNNs and Transformers.
...
TL;DR: MENDR, a new EEG foundation model, improves interpretability and performance using wavelet-based representations and Riemannian Manifold Transformers.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of transparency and interpretability in current EEG foundation models for clinical integration.

Method: MENDR uses a Riemannian Manifold Transformer to learn symmetric positive definite matrix embeddings from wavelet-decomposed EEG signals.

Result: MENDR achieves near state-of-the-art performance with fewer parameters and enhances interpretability.

Conclusion: MENDR is efficient, interpretable, and clinically applicable for EEG analysis.
...
TL;DR: A novel framework, RCUKF, combines reservoir computing (RC) and unscented Kalman filtering (UKF) for accurate modeling of complex systems.
...
Method: Integrates RC for data-driven modeling and UKF for Bayesian estimation, using RC as a surrogate model in UKF's prediction step.

Result: Demonstrated effectiveness on benchmark problems and real-time vehicle trajectory estimation.

Conclusion: RCUKF provides a reliable solution for modeling complex systems by combining data-driven and Bayesian approaches.
...
TL;DR: The paper proposes MMCI, a model using causal theory to address spurious correlations in multimodal sentiment analysis, improving generalization by disentangling causal and shortcut features.


<details>
  <summary>Details</summary>
Motivation: Existing MSA methods suffer from spurious correlations, relying on shortcuts instead of true causal relationships, which harms generalization.
...
Result: Experiments on standard and OOD datasets show MMCI suppresses biases and enhances performance.

Conclusion: MMCI effectively addresses spurious correlations in MSA, improving robustness and generalization.
...
TL;DR: R-Zero is a fully autonomous framework for self-evolving LLMs, eliminating the need for human-curated tasks by using two co-evolving models (Challenger and Solver) to generate and solve tasks.
...
Conclusion: R-Zero demonstrates the potential for autonomous LLM evolution, advancing AI capabilities beyond human-curated limitations.
...
Method: SPaRFT uses cluster-based data reduction and multi-armed bandits to optimize data selection and training.

Result: SPaRFT matches or outperforms baselines using up to 100x fewer samples.

Conclusion: Performance-driven curricula can enhance LLM reasoning efficiently with minimal resources.
...
TL;DR: The paper explores how AR systems can compromise situational awareness (SA) in safety-critical tasks like CPR, using eye tracking to analyze SA and proposing a graph neural network (FixGraphPool) to predict SA with high accuracy.


<details>
  <summary>Details</summary>
Motivation: AR systems enhance task performance but risk cognitive tunneling, reducing SA in critical scenarios like CPR. This study aims to understand and mitigate this issue.
...
<details>
  <summary>Details</summary>
Motivation: Improve pre-trained weights by encapsulating more knowledge beyond the given dataset.

Method: Sequential fine-tuning on downsized datasets induces structured forgetting, reversed via meta-learning to predict enhanced weights.

Result: KNOW prediction consistently outperforms naive fine-tuning and simple weight prediction.

Conclusion: Reinterpreting forgetting dynamics enhances knowledge transfer in deep learning.
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in graph learning, such as oversquashing and poor signal propagation in flat energy regions, by combining energy-based dynamics with tangential evolution.

Method: Uses a Lyapunov function for energy reduction and stability, augmented with a tangential component learned via message passing to maintain energy while evolving features.

Result: Achieves strong performance in node and graph classification/regression tasks, demonstrating effective signal propagation and compatibility with various GNN backbones.

Conclusion: TANGO's joint learning of energy functions and tangential flows enhances graph neural networks, offering stability and flexibility in diverse benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: To improve upon existing activation functions like ReLU and Mish by introducing non-monotonic, piecewise functions that treat positive and negative inputs differently.

Method: Proposes ULU and its adaptive variant AULU, defined with piecewise functions and learnable parameters for positive/negative inputs. Introduces the LIB metric for inductive bias.
...
TL;DR: The paper explores how multimodal imitation learning benefits from statistical learning theory, showing improved generalization and optimization with multimodal inputs like RGB-D, proprioception, and language.


<details>
  <summary>Details</summary>
Motivation: To understand why multimodal policies outperform unimodal ones by analyzing their theoretical foundations in statistical learning.

Method: Analyzes multimodal perception's impact on sample complexity and optimization landscapes, leveraging frameworks like Rademacher complexity, PAC learning, and information theory.

Result: Multimodal policies achieve tighter generalization bounds and better optimization landscapes than unimodal ones.

Conclusion: Properly integrated multimodal architectures (e.g., PerAct, CLIPort) outperform unimodal counterparts, supported by theoretical insights.
...
TL;DR: The paper introduces Integrated Influence, a new data attribution method that addresses limitations of LOO-based methods by incorporating a baseline approach and collective influence analysis.


<details>
  <summary>Details</summary>
Motivation: Existing LOO-based data attribution methods are limited by local explanations and lack of baseline, reducing flexibility and reliability.
...
Conclusion: The method enhances transparency and reliability in machine learning models by providing more comprehensive data attributions.
...
TL;DR: The paper introduces a cold-start active preference learning framework using self-supervised pre-training with PCA and active learning to address the lack of initial labeled data.


<details>
  <summary>Details</summary>
Motivation: The cold-start problem in preference learning, especially in data-scarce domains like social systems and economics, motivates the need for a method that doesn't rely on initial labeled data.

Method: A self-supervised pre-training phase uses PCA to generate pseudo-labels, followed by an active learning loop with a noisy oracle for refinement.

Result: The framework outperforms standard active learning, achieving higher accuracy with fewer labeled pairs across diverse datasets.

Conclusion: The proposed method effectively mitigates the cold-start problem, improving sample efficiency and applicability in data-constrained settings.
...
TL;DR: A novel Weakly Supervised Learning (WSL) framework leverages multiple weak supervision signals (similarity-confidence and confidence-difference) to improve learning when labeled data is scarce, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Accurate labeling is challenging, and labeled data is often limited. WSL with multiple weak supervision signals can provide a practical solution.

Method: Proposes SconfConfDiff Classification, integrating two weak labels (similarity-confidence and confidence-difference) and deriving unbiased risk estimators (convex combination and interaction-based). Includes risk correction for overfitting.

Result: The method achieves optimal convergence rates, mitigates overfitting, and outperforms baselines in experiments.

Conclusion: The proposed WSL framework is robust and effective, especially with limited labeled data, by leveraging complementary weak supervision signals.
...
<details>
  <summary>Details</summary>
Motivation: Current training methods for LLMs lack robust reasoning for function calling, relying on superficial patterns or struggling with complex action spaces.

Method: A two-stage data prep pipeline with LLM evaluation and AST validation, plus strategic entropy-based exploration in reinforcement learning.

Result: 86.02% overall accuracy on the Berkeley Function Calling Leaderboard, outperforming standard GRPO by 6% in complex scenarios.

Conclusion: The framework excels, especially with code-pretrained models, and will be open-sourced for community benefit.
...
TL;DR: HFedATM, a hierarchical aggregation method, addresses domain shift in Hierarchical Federated Learning (HFL) by aligning models and merging them efficiently, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Conventional FL and HFL struggle with domain shift and scalability. HFedDG explores domain shift in hierarchical architectures.

Method: Proposes HFedATM: aligns models via Filter-wise Optimal Transport Alignment and merges them with Shrinkage-aware Regularized Mean Aggregation.

Result: HFedATM improves performance over FedDG baselines, maintains efficiency, and achieves tighter generalization bounds.

Conclusion: HFedATM effectively tackles domain shift in HFL, enhancing model performance and training stability.
...
TL;DR: Deep neural networks with general activations achieve super-convergence in approximating PDE solutions, outperforming classical methods like finite element and spectral techniques.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in error-estimation theory for neural-network-based PDE approaches and provide a unified theoretical foundation.

Method: Analysis of deep fully-connected neural networks in Sobolev spaces, measuring errors in the $W^{m,p}$-norm for $m < n$.

Result: Demonstrated superior approximation rates (super-convergence) compared to traditional numerical methods.

Conclusion: Deep networks with general activations offer a powerful tool for scientific computing, closing a theoretical gap in PDE approximation.
...
TL;DR: PSEO is a framework for optimizing post-hoc stacking ensembles in AutoML, outperforming 16 other methods with a top average test rank of 2.96.


<details>
  <summary>Details</summary>
Motivation: Current CASH methods in AutoML use fixed strategies for ensembles, lacking adaptability to task-specific needs.
...
Result: PSEO achieves the best average test rank (2.96) on 80 datasets, surpassing other AutoML and ensemble methods.

Conclusion: PSEO effectively addresses the limitations of fixed ensemble strategies in AutoML, demonstrating superior performance.
...
TL;DR: The paper introduces domain-driven metrics for evaluating RL-based ABMs and RABMs, addressing challenges like system complexity and lack of standardized metrics. It demonstrates these metrics in a disease modeling case study.
...
Method: Develop domain-driven RL metrics, combining them with traditional and state-of-the-art metrics. Test these on a rational ABM disease modeling case study involving masking, vaccination, and lockdown scenarios.

Result: The study successfully applies domain-driven rewards alongside existing metrics in simulation scenarios, such as varying mask availability.

Conclusion: Domain-driven RL metrics enhance the evaluation of RL-based ABMs and RABMs, providing more meaningful insights in complex, stochastic systems.
...
TL;DR: Proposes pFedDSH, a framework for dynamic client onboarding in Federated Learning, using hypernetworks and data-free replay to maintain performance and privacy.


<details>
  <summary>Details</summary>
Motivation: Addresses the gap in Personalized Federated Learning (pFL) methods that assume static client participation, focusing on dynamic client onboarding scenarios.

Method: Introduces pFedDSH, leveraging a central hypernetwork with batch-specific masks and data-free replay for knowledge transfer and stability.
...
Conclusion: pFedDSH ensures robust performance for existing clients, efficient adaptation for new clients, and optimal neural resource use.
...
<details>
  <summary>Details</summary>
Motivation: Current EEG-based AAD lacks synergistic frameworks to leverage complementary features under energy constraints.

Method: Proposes S$^2$M-Former with a spike-driven symmetric architecture and lightweight 1D token sequences to reduce parameters and power.
...
TL;DR: HIA (Heuristic-Guided Inference-time Alignment) is a tuning-free method that balances alignment quality and computational cost by using a lightweight prompt optimizer, heuristic reward models, and two-stage filtering.


<details>
  <summary>Details</summary>
Motivation: Aligning LLMs with user preferences is crucial but often involves costly trade-offs between alignment quality and computational cost. Existing methods ignore this balance.
...
Result: HIA outperforms baselines like best-of-N sampling, beam search, and greedy search on real-world datasets (HelpSteer, ComPRed) under the same inference budget.

Conclusion: HIA offers a practical, scalable solution for personalized LLM deployment, even under low-inference budgets (1-2 response queries).
...
TL;DR: The paper introduces a framework for selecting the best-performing machine learning algorithm from a benchmark by formulating it as a subset selection problem for multinomial distributions. It provides improved methods and matching lower bounds.


<details>
  <summary>Details</summary>
Motivation: To identify the best-performing algorithm from a benchmark when performance differences are marginal, requiring a robust subset selection approach.
...
Conclusion: The framework provides a reliable solution for subset selection in identifying top-performing algorithms, with theoretical guarantees.
...
TL;DR: A ResNet-based HAR model accurately detects gait and everyday activities, outperforming state-of-the-art models, especially across various smartphone wear locations.
...
Method: Used smartphone sensor data from HC and PwMS, trained on datasets including GaitLab, Roche, and public sources. Evaluated performance across 9 wear locations.

Result: Achieved 98.4%-99.6% accuracy for gait detection and 96.2% for everyday activities, outperforming the state-of-the-art model.

Conclusion: The model is highly accurate and robust across various wear locations, proving practical for real-world use.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods (FR and AR) fail in long-term PDE solutions due to poor generalization and error accumulation.

Method: PITI-DeepONet uses a dual-output architecture trained with physics-informed objectives to learn time-derivative operators and integrates them with classical time-stepping.
...
Conclusion: PITI-DeepONet enables more reliable long-term PDE integration, outperforming traditional approaches.
...
Method: Developed a scalable framework with automated dataset creation via masking, using S&P 500 annual reports for evaluation.

Result: Identified intrinsic hallucination patterns in state-of-the-art LLMs on financial tabular data.

Conclusion: Provides a robust methodology for evaluating financial LLMs, advancing trustworthy financial Generative AI.
...
TL;DR: BAR is a bidding-aware retrieval framework for online ads, addressing inconsistency between retrieval and ranking stages by incorporating bid values, improving revenue and ad performance.


<details>
  <summary>Details</summary>
Motivation: The inconsistency between retrieval and ranking stages in online advertising systems, due to lack of real-time bid access, leads to sub-optimal revenue and advertiser outcomes.

Method: Proposes BAR, which includes Bidding-Aware Modeling (monotonicity-constrained learning, multi-task distillation), Asynchronous Near-Line Inference, and Task-Attentive Refinement.

Result: Offline experiments and deployment showed a 4.32% revenue increase and 22.2% impression lift for positively-operated ads.

Conclusion: BAR effectively resolves multi-stage inconsistency, enhancing platform revenue and advertiser outcomes.
...
TL;DR: A hybrid deep learning model combining LSTM, Transformer encoders, TS-Mixer, and attention mechanisms outperforms traditional methods for accurate real-time Rate of Penetration (ROP) prediction in drilling operations.


<details>
  <summary>Details</summary>
Motivation: Accurate ROP prediction is challenging due to complex, dynamic drilling data, and existing models fail to capture temporal and contextual relationships effectively.

Method: Proposes a hybrid deep learning architecture integrating LSTM, Transformer encoders, TS-Mixer blocks, and attention mechanisms to model temporal dependencies and feature interactions.

Result: Achieves an R-squared score of 0.9988 and MAPE of 1.447%, outperforming benchmarks like standalone LSTM and TS-Mixer.
...
TL;DR: DFW improves causal effect estimation by stabilizing propensity score weights and better handling selection bias.


<details>
  <summary>Details</summary>
Motivation: Addressing instability in propensity score weighting methods like IPW, which suffer from high variance and poor covariate balance.

Method: Proposes Deconfounding Factor Weighting (DFW), leveraging deconfounding factors to create stable weights and better approximate RCTs.

Result: DFW outperforms IPW and CBPS in covariate balancing and treatment effect estimation.

Conclusion: DFW is a robust alternative for causal inference, especially in binary and multi-treatment settings.
...
TL;DR: Predicting older adults' SPPB scores in 4 years using ML, with XGBoost performing best (MAE 0.79). Feature selection improved results (MAE 0.82).


<details>
  <summary>Details</summary>
Motivation: Early intervention is key to slowing physical decline in older adults. Predicting SPPB scores can help identify at-risk individuals.

Method: Tested ML algorithms (Random Forest, XGBoost, Linear Regression, dense and TabNet neural networks) on questionnaire data to predict SPPB scores. Used Shapley values for feature selection.

Result: XGBoost achieved the best prediction (MAE 0.79). Feature selection reduced features to 10-20, slightly increasing MAE to 0.82.

Conclusion: XGBoost is effective for SPPB score prediction, with feature selection maintaining performance. This aids early intervention strategies.
...
TL;DR: Proposes LIGHTYEAR, a decentralized P2P FL framework using agreement scores for personalized, robust model aggregation.


<details>
  <summary>Details</summary>
Motivation: Addresses limitations of centralized FL (single point of failure, poor personalization, vulnerability to adversarial clients) by enabling decentralized, trustworthy update selection.

Method: Uses local agreement scores to select beneficial updates, with regularization for stability, in a P2P topology.

Result: Outperforms centralized and existing P2P FL methods, especially in adversarial and heterogeneous settings.

Conclusion: LIGHTYEAR offers a robust, personalized solution for FL in challenging environments.
...
Method: Cross-LoRA uses LoRA-Align (subspace alignment via SVD and linear transformation) and LoRA-Shift (projecting aligned subspaces into target models) without additional data or training.

Result: Achieves up to 5.26% performance gains on benchmarks like ARCs, OBOA, and HellaSwag, matching directly trained LoRA adapters.

Conclusion: Cross-LoRA enables efficient, lightweight adaptation of LoRA modules across diverse LLMs without data or training, improving performance.
...
TL;DR: The paper introduces Mixture-of-Basis-Experts (MoBE), a novel method for compressing Mixture-of-Experts (MoE) models with minimal accuracy loss, outperforming existing techniques.


<details>
  <summary>Details</summary>
Motivation: Large MoE-based LLMs face high memory demands, and current compression methods lead to significant accuracy drops.

Method: MoBE decomposes expert matrices into unique and shared basis components, minimizing reconstruction error for compression.

Result: MoBE reduces parameters by 24%-30% with only 1%-2% accuracy drop, significantly better than prior methods.

Conclusion: MoBE offers an efficient solution for compressing MoE models without substantial performance loss.
...
Method: A hybrid LSTM-RF model is trained with multi-source ocean data (temperature, salinity, dissolved oxygen) using standardized treatment and a sliding window approach.

Result: The LSTM-RF model achieves R^2=0.5386, MSE=0.005806, and MAE=0.057147, significantly better than standalone LSTM (R^2=0.0208) and RF (R^2=0.4934).
...
TL;DR: FlowState introduces a novel time series foundation model (TSFM) with a state space model encoder and functional basis decoder, enabling continuous-time modeling and dynamic scale adjustment. It outperforms existing models in benchmarks and adapts to varying sampling rates.


<details>
  <summary>Details</summary>
Motivation: Existing TSFMs struggle with generalization across temporal resolutions, adaptability to sampling rates, and computational inefficiency. FlowState aims to address these challenges.

Method: FlowState uses a state space model (SSM) based encoder and a functional basis decoder for continuous-time modeling and dynamic time-scale adjustment. It also employs an efficient pretraining strategy.

Result: FlowState outperforms state-of-the-art models in GIFT-ZS and Chronos-ZS benchmarks, despite being smaller. It adapts online to varying sampling rates.

Conclusion: FlowState is a highly efficient and adaptable TSFM, setting new benchmarks in time series forecasting while reducing data and computational requirements.
...
TL;DR: The paper proposes a reinforcement learning fine-tuning method (RLHF) for conversational recommender systems (CRS) using implicit user feedback to improve recommendation accuracy and user satisfaction.


<details>
  <summary>Details</summary>
Motivation: Traditional supervised fine-tuning fails to capture implicit user feedback signals like dwell time or sentiment, limiting CRS performance.

Method: The approach uses a reward model trained on weakly-labeled engagement data and optimizes the LLM via proximal policy optimization (PPO) to maximize user-centric utility.

Result: Evaluations on synthetic and real-world datasets show improved top-k recommendation accuracy, coherence, and user satisfaction.

Conclusion: Implicit signal alignment via RLHF is effective for scalable and user-adaptive CRS design.
...
<details>
  <summary>Details</summary>
Motivation: Address computational bottlenecks in deep learning by optimizing batch-size and learning-rate scheduling for efficiency and generalization.

Method: Analyzed using stochastic first-order oracle (SFO) complexity, derived optimal growth schedules theoretically, and validated through experiments.

Result: Optimal schedules reduce SFO complexity, balancing efficiency and convergence in large-batch training.

Conclusion: Provides theoretical and practical guidelines for scalable and efficient large-batch training.
...
<details>
  <summary>Details</summary>
Motivation: Understanding the impact of batch size and learning rate on SGD convergence to optimize training efficiency.

Method: Introduces an adaptive scheduling strategy for SGD, adjusting batch size and learning rate based on gradient norm decay.

Result: Experiments show the adaptive scheduler improves convergence speed over existing methods.

Conclusion: Adaptive scheduling leveraging critical batch size theory enhances SGD performance.
...
<details>
  <summary>Details</summary>
Motivation: Human teaching effort is a bottleneck in interactive imitation learning, and existing methods underutilize novice plans.

Method: ASkDAgger uses S-Aware Gating (SAG), Foresight Interactive Experience Replay (FIER), and Prioritized Interactive Experience Replay (PIER) to optimize teacher feedback and novice plan utilization.

Result: The framework reduces query frequency, improves generalization, and speeds up domain adaptation, validated in simulation and real-world tasks.

Conclusion: ASkDAgger effectively balances teaching effort and learning performance, advancing interactive imitation learning.
...
Result: USP achieves up to 5.94% higher accuracy compared to prior SSCL methods.

Conclusion: USP effectively integrates and enhances the three key aspects of SSCL, demonstrating superior performance.
...
<details>
  <summary>Details</summary>
Motivation: Current QAT research focuses on minimizing quantization error without surpassing the full-precision accuracy ceiling. This work aims to break this ceiling by improving full-precision models before quantization.
...
TL;DR: OCAT improves CAT by fine-tuning only corpus-related parameters, boosting accuracy and resilience to overfitting.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency and error-proneness of pre-defining high-quality data in CAT.

Method: Fine-tunes a CAT pre-trained model by freezing most parameters and tuning only corpus-related ones.

Result: +3.6 and +1.8 chrF improvement in WMT23 tasks, outperforming vanilla training and matching SOTA methods.

Conclusion: OCAT is lightweight, effective, and less sensitive to hyperparameters, enhancing translation tasks.
...
<details>
  <summary>Details</summary>
Motivation: To explore LLMs' potential for generating accurate and concise CPs for clinical phenotypes, aiming to improve hypertension patient care.
...
Result: LLMs with iterative learning produced interpretable, reasonably accurate CPs, nearing state-of-the-art ML performance with fewer training examples.

Conclusion: LLMs, enhanced by iterative refinement, are promising for scalable clinical decision support in generating CPs.
...
<details>
  <summary>Details</summary>
Motivation: Learning from scratch is costly for personalization tasks with limited decision points, and existing latent bandit models require exact reward distributions, which may not always exist or be practical.

Method: The authors introduce a relaxed assumption requiring only a model of action preference orderings in each latent state. They provide a posterior-sampling algorithm for this problem.

Result: The algorithm performs competitively with full-knowledge latent bandits when reward distributions are well-specified and outperforms them when reward scales vary.

Conclusion: Relaxing assumptions to focus on preference orderings enhances flexibility and performance in latent bandit problems, especially in real-world scenarios with heterogeneous reward scales.
...
<details>
  <summary>Details</summary>
Motivation: Current RL systems violate SPMD assumptions by mixing inference and training workloads, reducing efficiency.

Method: Echo uses sequential pull and asynchronous push-pull protocols to decouple phases across heterogeneous hardware.

Result: Echo matches co-located baselines in convergence and reward while using decentralized resources.
...
Method: NT-ML involves retraining the model with non-target labels and mutual learning between teacher and student models to purify the student model.
...
Conclusion: NT-ML is a highly effective solution for mitigating backdoor attacks in DNNs, demonstrating superior performance over existing defenses.
...
TL;DR: The paper explores adaptive learning rate mechanisms in deep learning, focusing on a 2017 cumulative path-based method. It identifies inconsistencies in its application to Adam and proposes a corrected variant, benchmarking it against SGD and Adam with and without adaptation.


<details>
  <summary>Details</summary>
Motivation: The learning rate is critical in deep learning, and its ideal value can vary. The study aims to evaluate the practicality of adaptive learning rate mechanisms, particularly a 2017 method, and improve its consistency with Adam.

Method: The paper revisits a cumulative path-based adaptation scheme, identifies its inconsistency with Adam due to preconditioning, and proposes a corrected variant. It benchmarks SGD and Adam with and without adaptation against a recent alternative.

Result: The study provides insights into the practical benefits of adaptive learning rate strategies, particularly the corrected variant for Adam, and compares its performance with other methods.

Conclusion: The paper clarifies when and why adaptive learning rate strategies, especially the corrected variant for Adam, are beneficial, offering practical guidance for deep learning practitioners.
...
TL;DR: A causality-aware framework for molecular generation from text, combining a Causality-Aware Transformer and Variational Mean Flow, achieves high-quality, diverse results with fast inference.
...
Method: Proposes a Causality-Aware Transformer (CAT) for joint encoding of molecular graphs and text, and a Variational Mean Flow (VMF) framework for efficient one-step inference.

Result: Outperforms baselines with higher novelty (74.5%), diversity (70.3%), and 100% validity. VMF requires minimal function evaluations for generation.

Conclusion: The framework offers superior performance and computational efficiency, making it a strong candidate for molecular generation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Forecasting stock and cryptocurrency prices is difficult due to high volatility and non-stationarity, influenced by economic and market factors. ESNs have shown promise for stock markets, but their application to cryptocurrencies is underexplored.

Method: The study uses ESNs for cryptocurrency forecasting and conducts chaos analysis via the Lyapunov exponent during volatile periods.

Result: ESNs outperform existing machine learning methods, especially during chaotic periods, aligning with Lyapunov exponent analysis.

Conclusion: ESNs are robust and effective for cryptocurrency forecasting, particularly in high-volatility scenarios, surpassing traditional methods.
...
TL;DR: NegBio-VAE extends VAEs by using a negative binomial distribution for spike counts, improving neural activity modeling by allowing explicit control over dispersion.


<details>
  <summary>Details</summary>
Motivation: Current VAEs, like Poisson-VAE, impose rigid constraints (equal mean and variance) that fail to capture the true stochastic nature of neural spike trains.

Method: Introduces NegBio-VAE, which models spike counts with a negative binomial distribution, and develops ELBO optimization schemes and reparameterization strategies for this setting.

Result: Empirical results show significant improvements in reconstruction fidelity due to explicit modeling of overdispersion.

Conclusion: NegBio-VAE's minor but impactful change highlights the importance of accurately modeling neural spike variability.
...
<details>
  <summary>Details</summary>
Motivation: Address the under-serving of minority clients in federated learning (FL) by enforcing client-wise Pareto optimality, unlike existing methods like FMOL that only achieve task-wise Pareto-stationary points.

Method: Uses a novel preference-cone constraint after local federated multi-gradient descent averaging (FMGDA/FSMGDA) steps. The server solves a cone-constrained Pareto-MTL sub-problem to produce a Pareto-stationary descent direction for each client.
...
Conclusion: CR-FMOL is a promising framework for fairer FL by ensuring client-wise Pareto optimality, though it may require more training rounds to match FedAvg's accuracy.
...
TL;DR: GCPO improves upon GRPO by incorporating causal dependencies among candidate responses, enhancing prediction quality and performance.


<details>
  <summary>Details</summary>
Motivation: Specialized domains require targeted post-training for LLMs, and existing methods like GRPO overlook semantic interactions among responses.

Method: Introduces a Structural Causal Model (SCM) to reveal dependencies, projects responses onto a causally informed subspace, and proposes GCPO with reward adjustment and KL regularization.

Result: GCPO outperforms GRPO and other methods across multiple reasoning benchmarks.

Conclusion: Integrating causal structure into optimization (GCPO) enhances LLM performance in specialized domains.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the lack of interpretability in deep reinforcement learning policies, which undermines trust and real-world deployment.

Method: Uses multimodal large language models as policy generators, integrates evolutionary search, and employs visual feedback for targeted policy improvements.

Result: Achieves performance comparable to PPO, with transparent logic and scalable adaptability across tasks.

Conclusion: MLES is a promising approach for interpretable control policy discovery, overcoming domain-specific language limitations.
...
Method: Develops a theoretical framework to quantify the error from treating competing risks as censoring, supported by empirical analysis of cardiovascular management data.

Result: Demonstrates that ignoring competing risks introduces substantial bias, overestimates risk, and exacerbates disparities, particularly for high-risk individuals.

Conclusion: Practitioners must account for competing risks to improve accuracy, reduce disparities, and ensure fairer decision-making in survival models.
...
Result: Both methods demonstrate improved rewards and stability, outperforming baselines in simulations.

Conclusion: The proposed methods effectively achieve robust tail-risk guarantees, enhancing safety and performance in MCTS.
...
TL;DR: EnergyPatchTST improves energy time series prediction with multi-scale feature extraction, uncertainty estimation, and future variable integration, reducing errors by 7-12%.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with multi-scale dynamics and irregular real data in energy forecasting.
...
Result: Outperforms common methods, reducing prediction error by 7-12% and providing reliable uncertainty estimates.
...
<details>
  <summary>Details</summary>
Motivation: Understanding how neural network representations are shaped by task complexity, given their black-box nature.

Method: Five probes (pruning, binarization, noise injection, sign flipping, bipartite network randomization) applied to MLPs on MNIST and Fashion-MNIST datasets.

Result: Hard-task models collapse under binarization, while easy-task models remain robust. Noise injection can enhance accuracy, and sign structure preservation maintains performance.

Conclusion: Task complexity can be measured by performance gaps, and signed bipartite topology is crucial for representations, aiding model compression and interpretability.
...
Method: Leverages f-mutual information measures, proving their uniqueness under natural conditions. Uses bounded measures like total variation distance for tractability.

Result: Achieves perfect discrimination between faithful and strategic agents across domains. Shows 10-100x better robustness to adversarial manipulation. Performance peaks at a 10:1 compression ratio.

Conclusion: The proposed mechanisms are highly effective, robust, and outperform existing methods, with optimal performance at specific compression ratios.
...
<details>
  <summary>Details</summary>
Motivation: Clinical presence (interaction between patients and healthcare systems) impacts observed outcomes in EHRs, but is often overlooked in prediction models, limiting their transportability.

Method: A multi-task recurrent neural network jointly models inter-observation time, missingness processes, and survival outcomes to address clinical presence.

Result: The proposed model outperforms state-of-the-art models in mortality prediction on MIMIC-III, demonstrating improved performance and transportability.
...
Result: MoMA outperforms current methods in accuracy and flexibility across three prediction tasks with real-world datasets.

Conclusion: MoMA demonstrates superior performance in integrating multimodal EHR data for clinical predictions, offering enhanced accuracy and adaptability.
...
TL;DR: The paper introduces two parameter-free algorithms, AMVFCM-U and AAMVFCM-U, for multi-view clustering, addressing challenges like high-dimensional data and cross-view integration. It uses entropy regularization and SNR-based feature weighting for adaptive consensus, achieving superior performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of traditional multi-view clustering methods, such as manual parameter tuning and lack of principled cross-view integration, by proposing a unified, parameter-free framework.

Method: Introduces AMVFCM-U and AAMVFCM-U, which replace fuzzification parameters with entropy regularization and SNR-based feature weighting. AAMVFCM-U adds hierarchical dimensionality reduction via adaptive thresholding.

Result: Outperforms 15 state-of-the-art methods, achieving up to 97% computational efficiency, reducing dimensionality to 0.45% of original size, and identifying optimal view combinations.

Conclusion: The proposed algorithms provide an efficient, adaptive, and parameter-free solution for multi-view clustering, demonstrating significant improvements in performance and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Overfitting in PCs due to convergence to sharp optima, especially with limited data, motivates the need for a regularization method.
...
Result: Experiments show the method consistently improves generalization by guiding PCs to flatter minima.

Conclusion: The proposed Hessian-based regularizer effectively mitigates overfitting in PCs, enhancing their generalization performance.
...
TL;DR: A survey on unsupervised adaptation methods for Vision-Language Models (VLMs), proposing a taxonomy and analyzing four paradigms for task-specific adaptation without labeled data.
...
Method: Proposes a taxonomy categorizing approaches into four paradigms based on unlabeled data availability: Data-Free Transfer, Unsupervised Domain Transfer, Episodic Test-Time Adaptation, and Online Test-Time Adaptation.

Result: Systematic analysis of methodologies and benchmarks, highlighting challenges and future directions.

Conclusion: The survey bridges the gap in understanding unsupervised VLM adaptation, offering a structured overview and encouraging further research.
...
TL;DR: X-VFL addresses VFL challenges of non-aligned data and local inference by introducing Cross Completion (XCom) and Decision Subspace Alignment (DS-Align), achieving significant accuracy improvements.


<details>
  <summary>Details</summary>
Motivation: VFL struggles with perfectly aligned data and joint inference, limiting its practicality. X-VFL aims to handle missing features and enable local inference.
...
TL;DR: The paper introduces an extension for PyKEEN, integrating advanced negative samplers to improve knowledge graph embedding performance, and validates its impact through empirical studies.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of advanced negative sampling strategies in popular knowledge graph embedding libraries, which impacts performance.

Method: Developed a PyKEEN extension with a suite of advanced negative samplers (static and dynamic) for generating meaningful negative samples.

Result: Empirical studies show the extension enhances performance in link prediction tasks and provides insights for better strategy design.

Conclusion: The extension improves PyKEEN's capabilities and facilitates easier development and customization of embedding methods.
...
<details>
  <summary>Details</summary>
Motivation: The rise in IoT cyberattacks necessitates improved intrusion detection methods beyond conventional machine learning models.

Method: The study evaluates KANs, which use learnable activation functions, for intrusion detection in IoT networks.

Result: KANs achieve competitive accuracy and superior interpretability compared to traditional and advanced models.
...
TL;DR: The paper explores the 'one-poison hypothesis,' showing that a single poison sample can inject a backdoor into a model with zero error and minimal impact on benign performance, proven for linear models.


<details>
  <summary>Details</summary>
Motivation: Addressing the open question of how much poison data is needed for successful backdoor attacks, given prior work's focus on either few samples with extensive knowledge or many poisoned points.

Method: Formulates and proves the one-poison hypothesis for linear regression and classification, leveraging unused data distribution directions for zero-impact backdoors.

Result: Demonstrates that a single poison sample can achieve zero backdooring-error and minimal benign task impact, validated with benchmark datasets.

Conclusion: The one-poison hypothesis is viable, especially for linear models, with broader implications for understanding backdoor attack efficiency.
...
TL;DR: Shuffle-R1 improves RL fine-tuning efficiency by addressing Advantage Collapsing and Rollout Silencing through dynamic trajectory sampling and batch reshuffling.


<details>
  <summary>Details</summary>
Motivation: Current RL pipelines suffer from inefficiencies due to Advantage Collapsing and Rollout Silencing, hindering long-term learning.
...
Result: Outperforms RL baselines in reasoning benchmarks with minimal overhead.

Conclusion: Data-centric adaptations like Shuffle-R1 are crucial for efficient RL training in MLLM.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of traditional heuristics and deep learning in trajectory prediction, such as lack of accuracy, generalizability, and explainability.

Method: TrajEvo employs an evolutionary algorithm with LLMs, featuring Cross-Generation Elite Sampling and a Statistics Feedback Loop to refine heuristics.

Result: TrajEvo outperforms existing methods in real-world datasets and excels in OOD generalization.

Conclusion: TrajEvo advances automated, explainable, and generalizable trajectory prediction heuristics, with open-source code for future research.
...
TL;DR: A simple improvement to Supervised Fine-Tuning (SFT) for LLMs, called Dynamic Fine-Tuning (DFT), addresses generalization issues by dynamically rescaling the objective function, outperforming standard SFT and showing competitive results in RL settings.
...
Method: Proposes DFT, which stabilizes gradient updates by dynamically rescaling the objective function with token probabilities.
...
### [273] [From Rattle to Roar: Optimizer Showdown for MambaStock on S&P 500](https://arxiv.org/abs/2508.04707)
*Alena Chan,Maria Garmonina*

Main category: q-fin.CP

TL;DR: The paper evaluates optimizers for S&P 500 Index forecasting using MambaStock, finding Adam and RMSProp best for accuracy but Lion fastest. It introduces Roaree to combine speed and stability.


<details>
  <summary>Details</summary>
Motivation: To identify the best optimizer for forecasting S&P 500 returns and address the trade-off between accuracy and training speed.

Method: Evaluates gradient-smoothing and adaptive-rate optimizers (Adam, RMSProp, Lion) and introduces Roaree to mitigate Lion's oscillatory loss.

Result: Adam and RMSProp yield the lowest test errors, while Lion trains faster. Roaree combines speed and stability.

Conclusion: Roaree is a promising optimizer for balancing accuracy and training efficiency in financial forecasting.
...
### [274] [Reinforcement Learning Generation of 4-Qubits Entangled States](https://arxiv.org/abs/2204.12351)
*Sara Giordano,Miguel A. Martin-Delgado*

Main category: quant-ph

TL;DR: An AI algorithm using Q-learning generates 4-qubit entangled states, covering all nine entanglement families and some SLOCC classes. It introduces a state-link graph (SLG) for intuitive Q-matrix construction and optimal quantum circuits.


<details>
  <summary>Details</summary>
Motivation: To automate the construction of representative entangled states for experimental use and to explore intrinsic quantum properties.

Method: Uses Q-learning to synthesize quantum circuits, aided by SLGs for visualizing Q-matrix construction and gate-set optimization.

Result: Achieves coverage of all nine entanglement families and some SLOCC classes, producing optimal quantum circuits.

Conclusion: The algorithm and SLG tool simplify entangled state construction, offering practical utility for low-qubit systems.
...
### [275] [Hybrid Reward-Driven Reinforcement Learning for Efficient Quantum Circuit Synthesis](https://arxiv.org/abs/2507.16641)
*Sara Giordano,Kornikar Sen,Miguel A. Martin-Delgado*

Main category: quant-ph

TL;DR: A reinforcement learning (RL) framework synthesizes efficient quantum circuits for target states, using Q-learning and hybrid rewards to manage high-dimensional spaces and optimize gate counts.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of quantum circuit synthesis in NISQ and fault-tolerant computing by efficiently navigating high-dimensional quantum state spaces.

Method: Uses tabular Q-learning with action sequences in discretized state spaces, hybrid rewards (static domain-informed and dynamic penalties), and sparse matrix representations.

Result: Achieves minimal-depth circuits with optimized gate counts for graph-state preparation (up to 7 qubits) and universal gate sets.

Conclusion: The RL-driven approach efficiently explores quantum state spaces and synthesizes near-optimal circuits, offering a resource-efficient foundation for quantum circuit optimization.
...
### [276] [Q-DPTS: Quantum Differentially Private Time Series Forecasting via Variational Quantum Circuits](https://arxiv.org/abs/2508.05036)
...
### [277] [Hybrid quantum tensor networks for aeroelastic applications](https://arxiv.org/abs/2508.05169)
*M. Lautaro Hickmann,Pedro Alves,David Quero,Friedhelm Schwenker,Hans-Martin Rieser*

Main category: quant-ph

TL;DR: Hybrid quantum tensor networks combined with QML show promise for aeroelastic problems, achieving high accuracy in binary classification and decent performance in regression tasks, though hyperparameter optimization is challenging.
...
Method: A hybrid algorithm encodes time series into tensor networks, reduces dimensionality, converts tensors to quantum circuits, and applies variational quantum circuits for classification or regression.

Result: High accuracy in binary classification and promising performance in regression tasks, though hyperparameter optimization is critical.

Conclusion: This work advances QML for aeroelasticity, demonstrating the viability of hybrid quantum tensor networks despite optimization challenges.
...
### [278] [LLM-based Multi-Agent Copilot for Quantum Sensor](https://arxiv.org/abs/2508.05421)
...
<details>
  <summary>Details</summary>
Motivation: Overcoming interdisciplinary knowledge barriers and complex optimization in quantum sensor development using LLMs.

Method: QCopilot integrates commercial LLMs with few-shot prompts, a vector knowledge base, and specialized agents for adaptive optimization, modeling, and diagnosis.

Result: Achieved 10^8 sub-μK atoms in atom cooling experiments with ~100x speedup and autonomous anomaly detection.

Conclusion: QCopilot reduces barriers for quantum sensor deployment and is adaptable to other quantum systems.
...
### [279] [On the Design of Expressive and Trainable Pulse-based Quantum Machine Learning Models](https://arxiv.org/abs/2508.05559)
...
<details>
  <summary>Details</summary>
Motivation: To ensure pulse-based QML models are both expressive and trainable, addressing potential uncontrollability issues.

Method: Investigates requirements for expressivity and trainability, supported by numerical simulations.

Result: A framework for designing practical pulse-based QML models that balance expressivity and trainability.

Conclusion: Pulse-based QML models can achieve expressivity and trainability with proper design considerations.
...
### [280] [Harmonic fractal transformation for modeling complex neuronal effects: from bursting and noise shaping to waveform sensitivity and noise-induced subthreshold spiking](https://arxiv.org/abs/2508.05341)
*Mariia Sorokina*
...
<details>
  <summary>Details</summary>
Motivation: To create a method that replicates neuronal effects more effectively than conventional filters by leveraging fractal recomposition of input spectra.

Method: Proposes fractal frequency mapping, which transforms input spectra nonlinearly to excite resonant frequencies optimal for sampling.
...
### [281] [Making Prompts First-Class Citizens for Adaptive LLM Pipelines](https://arxiv.org/abs/2508.05012)
...
Result: Preliminary experiments show benefits of SPEAR's refinement modes and optimizations like operator fusion.

Conclusion: SPEAR addresses prompt management gaps, offering structured, adaptive, and optimized prompt handling.
...
### [282] [AgenticData: An Agentic Data Analytics System for Heterogeneous Data](https://arxiv.org/abs/2508.05002)
...
Method: Uses feedback-driven planning to convert NL queries into semantic plans, multi-agent collaboration (data profiling, cross-validation, memory agents), and semantic optimization.

Result: Tested on three benchmarks, AgenticData achieved superior accuracy, outperforming state-of-the-art methods.

Conclusion: AgenticData effectively automates data analytics, improving efficiency and accuracy.
...
### [283] [JPS: Jailbreak Multimodal Large Language Models with Collaborative Visual Perturbation and Textual Steering](https://arxiv.org/abs/2508.05087)
...
TL;DR: JPS introduces a method for jailbreaking MLLMs using collaborative visual and textual perturbations, achieving high attack success and intent fulfillment rates.
...
Method: JPS combines target-guided adversarial image perturbations with optimized 'steering prompts' in a multi-agent system, iteratively co-optimizing both components.

Result: JPS achieves state-of-the-art performance in attack success rate (ASR) and malicious intent fulfillment rate (MIFR) across various MLLMs.

Conclusion: JPS effectively addresses the gap in jailbreak attacks by ensuring both safety bypass and malicious intent fulfillment, validated by experiments and a new MIFR metric.
...
### [284] [Embedding Alignment in Code Generation for Audio](https://arxiv.org/abs/2508.05473)
*Sam Kouteili,Hiren Madhu,George Typaldos,Mark Santolucito*

Main category: cs.MM

TL;DR: The paper explores improving LLM-powered code generation for creative coding by analyzing the relationship between code and audio embeddings, proposing a predictive model for diverse musical outputs.
...
Conclusion: A predictive model can bridge the gap between code and audio embeddings, aiding in generating diverse and musically relevant code candidates.
...
### [285] [Online Sparsification of Bipartite-Like Clusters in Graphs](https://arxiv.org/abs/2508.05437)
*Joyentanuj Das,Suranjan De,He Sun*

Main category: cs.DS

TL;DR: The paper introduces efficient online sparsification algorithms for finding bipartite-like clusters in undirected and directed graphs, improving runtime without sacrificing effectiveness.
...
Method: The authors propose online sparsification algorithms tailored for undirected and directed graphs to detect bipartite-like clusters.

Result: Experiments on synthetic and real-world datasets show the algorithms significantly speed up existing clustering methods while maintaining effectiveness.

Conclusion: The work advances graph clustering by efficiently identifying bipartite-like clusters, balancing speed and accuracy.
...
### [286] [Alz-QNet: A Quantum Regression Network for Studying Alzheimer's Gene Interactions](https://arxiv.org/abs/2508.04743)
*Debanjan Konar,Neerav Sreekumar,Richard Jiang,Vaneet Aggarwal*

Main category: q-bio.MN

TL;DR: The paper proposes a Quantum Regression Network (Alz-QNet) to study gene-gene interactions in Alzheimer's disease, focusing on key genes like APP, FGF14, YY1, and PLD3, to uncover regulatory mechanisms for potential therapies.


<details>
  <summary>Details</summary>
Motivation: Understanding the molecular mechanisms of Alzheimer's disease (AD) is challenging due to its multifactorial nature. Decoding gene-gene interactions is crucial for theranostics and therapy development.

Method: The study uses a novel Quantum Regression Network (Alz-QNet) inspired by Quantum Gene Regulatory Networks (QGRN) to analyze interactions among key AD-related genes (e.g., APP, FGF14, YY1) in the Entorhinal Cortex, using data from GSE138852.

Result: The research reveals intricate gene-gene interactions, identifying potential regulatory mechanisms in AD pathogenesis, which could lead to gene inhibitors or regulators for therapy.
...
### [287] [Understanding and Mitigating Errors of LLM-Generated RTL Code](https://arxiv.org/abs/2508.05266)
*Jiazheng Zhang,Cheng Liu,Huawei Li*

Main category: cs.AR

TL;DR: The paper addresses low success rates in LLM-based RTL code generation by analyzing errors, proposing targeted corrections (e.g., domain-specific knowledge, rule-checking, and multimodal input conversion), and integrating these into a framework, achieving 91.0% accuracy.


<details>
  <summary>Details</summary>
Motivation: The unsatisfactory success rate of LLM-based RTL code generation due to various error sources, with limited understanding of failure causes, motivates the need for comprehensive error analysis and targeted solutions.

Method: Conducts error analysis, proposes in-context learning techniques (domain-specific knowledge base, retrieval-augmented generation, design description rules, rule-checking, multimodal input conversion, and iterative debugging), and integrates these into an LLM framework.

Result: The enhanced framework achieves 91.0% accuracy on VerilogEval, outperforming the baseline by 32.7%.
...
TL;DR: INTENTION is a framework combining Vision-Language Models and interaction-driven memory to enable robots with human-like adaptability for autonomous manipulation.


<details>
  <summary>Details</summary>
Motivation: Traditional robotic methods fail in unstructured environments due to rigid models and lack of generalization. Humans, however, intuitively adapt, inspiring the need for learned interactive intuition in robots.

Method: INTENTION integrates Vision-Language Models for scene reasoning and a Memory Graph to store past interactions, alongside an Intuitive Perceptor for extracting physical relations and affordances.

Result: The framework allows robots to infer interaction behaviors in new scenes autonomously, reducing reliance on predefined instructions.

Conclusion: INTENTION advances robotic manipulation by mimicking human adaptability, enabling efficient decision-making in diverse, unstructured environments.
...
### [289] [Mixed-Initiative Dialog for Human-Robot Collaborative Manipulation](https://arxiv.org/abs/2508.05535)
...
<details>
  <summary>Details</summary>
Motivation: Robotic systems need to adapt to diverse human partners and dynamic collaboration scenarios, requiring flexible communication and decision-making.

Method: MICoBot uses a three-level decision framework: meta-planner for strategy, planner for task allocation, and executor for actions. Evaluated in simulation and real-world with human participants.

Result: Significantly improved task success and user experience compared to LLM baselines and other models.

Conclusion: MICoBot effectively collaborates with diverse humans, demonstrating the value of adaptive, multi-level decision-making in human-robot teams.
...
### [290] [Hierarchical Deep Deterministic Policy Gradient for Autonomous Maze Navigation of Mobile Robots](https://arxiv.org/abs/2508.04994)
*Wenjie Hu,Ye Zhou,Hann Woei Ho*

Main category: cs.RO

TL;DR: The paper introduces Hierarchical DDPG (HDDPG) to improve maze navigation in robotics by addressing DDPG's issues like sparse rewards and inefficient exploration. HDDPG uses high-level and low-level policies, off-policy correction, adaptive noise, and optimized rewards, achieving significant performance gains.


<details>
  <summary>Details</summary>
Motivation: Standard DDPG struggles with maze navigation due to sparse rewards, poor exploration, and long-horizon planning, leading to low success rates. This paper aims to overcome these limitations.

Method: Proposes HDDPG with hierarchical policies: high-level for subgoal generation and low-level for action execution. Uses off-policy correction, adaptive noise, reshaped rewards, gradient clipping, and Xavier initialization.

Result: HDDPG outperforms baselines, increasing success rates by ≥56.59% and average rewards by ≥519.03 in maze navigation tasks.

Conclusion: HDDPG effectively addresses DDPG's shortcomings in maze navigation, demonstrating superior performance through hierarchical policies and optimization techniques.
...
### [291] [Chemist Eye: A Visual Language Model-Powered System for Safety Monitoring and Robot Decision-Making in Self-Driving Laboratories](https://arxiv.org/abs/2508.05148)
...
<details>
  <summary>Details</summary>
Motivation: The integration of robotics in SDLs introduces safety risks like fires near flammable batteries. Ensuring worker safety and hazard detection is critical.

Method: Chemist Eye uses distributed stations with RGB, depth, and infrared cameras, plus a vision-language model (VLM) for real-time hazard detection and decision-making.

Result: The system achieved 97% accuracy in spotting hazards and 95% in decision-making during tests with mobile robots in an SDL.

Conclusion: Chemist Eye effectively enhances safety in SDLs by integrating real-time monitoring and automated responses to hazards.
...
### [292] [FCBV-Net: Category-Level Robotic Garment Smoothing via Feature-Conditioned Bimanual Value Prediction](https://arxiv.org/abs/2508.05153)
...
<details>
  <summary>Details</summary>
Motivation: Overcoming challenges in robotic garment manipulation, such as high dimensionality and intra-category variations, where current methods either overfit or fail to generalize.

Method: Proposes FCBV-Net, a model using 3D point clouds with pre-trained geometric features to enhance generalization, trained on simulated GarmentLab experiments with CLOTH3D.

Result: FCBV-Net showed only an 11.5% efficiency drop on unseen garments (vs. 96.2% for 2D baseline) and achieved 89% coverage (vs. 83% for 3D baseline).

Conclusion: Decoupling geometric understanding from action value learning enables superior category-level generalization in garment manipulation.
...
### [293] [Learning to See and Act: Task-Aware View Planning for Robotic Manipulation](https://arxiv.org/abs/2508.05186)
...
Result: TAVP achieves superior performance on RLBench tasks, enhancing action prediction and task generalization.

Conclusion: TAVP's task-aware approach generates better visual representations, improving robustness and generalization in robotic manipulation.
...
### [294] [DistillDrive: End-to-End Multi-Mode Autonomous Driving Distillation by Isomorphic Hetero-Source Planning Model](https://arxiv.org/abs/2508.05402)
*Rui Yu,Xianghang Zhang,Runkai Zhao,Huaicheng Yan,Meng Wang*

Main category: cs.RO

TL;DR: DistillDrive enhances autonomous driving by using knowledge distillation and diversified instance imitation for multi-mode motion feature learning, reducing collisions by 50% and improving closed-loop performance by 3 points.
...
Conclusion: DistillDrive effectively improves autonomous driving robustness by integrating planning-oriented learning and diversified instance imitation.
...
### [295] [Towards Generalizable Safety in Crowd Navigation via Conformal Uncertainty Handling](https://arxiv.org/abs/2508.05634)
*Jianpeng Yao,Xiaopan Zhang,Yu Xia,Zejin Wang,Amit K. Roy-Chowdhury,Jiachen Li*

Main category: cs.RO

TL;DR: The paper proposes a method to improve mobile robot navigation in crowds by incorporating pedestrian uncertainty estimates via adaptive conformal inference, enhancing robustness to distribution shifts.


<details>
  <summary>Details</summary>
Motivation: Address performance degradation of reinforcement learning-trained robots in out-of-distribution crowd scenarios by leveraging uncertainty awareness.

Method: Augments robot observations with uncertainty estimates from adaptive conformal inference and uses constrained reinforcement learning to guide behavior.

Result: Achieves 96.93% success rate in-distribution, outperforming baselines with fewer collisions and intrusions; robust in out-of-distribution scenarios.

Conclusion: The method enables safe, robust robot navigation in crowds, validated in real-world deployments.
...
### [296] [Genie Envisioner: A Unified World Foundation Platform for Robotic Manipulation](https://arxiv.org/abs/2508.05635)
...
TL;DR: Genie Envisioner (GE) is a unified platform for robotic manipulation, integrating policy learning, evaluation, and simulation via a video-generative framework. It includes GE-Base (video diffusion model), GE-Act (action decoder), and GE-Sim (neural simulator), with EWMBench for benchmarking.
...
Method: GE combines a video diffusion model (GE-Base) for capturing dynamics, a flow-matching decoder (GE-Act) for action trajectories, and a neural simulator (GE-Sim) for high-fidelity rollouts. EWMBench evaluates performance.

Result: GE provides a scalable and generalizable framework for robotic manipulation with minimal supervision, supported by high-fidelity simulation and standardized benchmarking.
...
### [297] [Towards Embodied Agentic AI: Review and Classification of LLM- and VLM-Driven Robot Autonomy and Interaction](https://arxiv.org/abs/2508.05294)
*Sahar Salimpour,Lei Fu,Farhad Keramat,Leonardo Militano,Giovanni Toffetti,Harry Edelman,Jorge Peña Queralta*

Main category: cs.RO

TL;DR: The paper surveys advancements in foundation models (LLMs, VLMs, VLAs, BLMs) for robotics, focusing on agentic applications like GPT-style interfaces, planning, and perception. It includes peer-reviewed and community-driven projects, proposing a taxonomy for model integration.
...
Result: Identifies trends in agentic applications, such as natural language reasoning, API invocation, and task planning, and provides a comparative analysis.

Conclusion: Foundation models are advancing robotics through agentic architectures, with diverse applications and emerging trends in the field.
...
### [298] [Information-Theoretic Graph Fusion with Vision-Language-Action Model for Policy Reasoning and Dual Robotic Control](https://arxiv.org/abs/2508.05342)
...
<details>
  <summary>Details</summary>
Motivation: Overcoming the limitations of low-level trajectory imitation in robotics, which struggles with generalization across objects, layouts, and configurations.

Method: Extracts task-relevant cues, encodes them into scene graphs, and uses a language-conditioned transformer to generate behavior trees and motion commands. Includes a cross-hand selection policy for bimanual efficiency.
...
### [299] [Real-Time Iteration Scheme for Diffusion Policy](https://arxiv.org/abs/2508.05396)
*Yufei Duan,Hang Yin,Danica Kragic*

Main category: cs.RO

TL;DR: The paper introduces a Real-Time Iteration (RTI) Scheme-inspired method to reduce the inference time of Diffusion Policies in robotic manipulation, avoiding additional training or policy redesign.


<details>
  <summary>Details</summary>
Motivation: Diffusion Policies suffer from long inference times and action chunk execution delays, limiting their use in latency-critical tasks. Existing solutions like distillation are resource-intensive.

Method: The proposed method applies the RTI Scheme to diffusion inference, using previous solutions as initial guesses for iterations. It also includes a scaling-based approach for discrete actions.

Result: The method significantly reduces computational costs without performance loss, as shown in simulations.

Conclusion: The RTI-inspired scheme enables efficient integration into pre-trained diffusion models, particularly large ones, with comparable performance to full-step denoising.
...
### [300] [Evaluating the Use of LLMs for Documentation to Code Traceability](https://arxiv.org/abs/2506.16440)
*Ebube Alor,SayedHassan Khatoonabadi,Emad Shihab*

Main category: cs.SE

TL;DR: LLMs show promise for automating documentation-to-code traceability, achieving high accuracy in trace link identification but with limitations in relationship explanations and multi-step chains.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of LLMs in automating documentation-to-code traceability, an area with underexplored capabilities.

Method: Comprehensive evaluation of LLMs (Claude 3.5 Sonnet, GPT-4o, o3-mini) using novel datasets from open-source projects (Unity Catalog, Crawl4AI). Assessed trace link identification, relationship explanation, and multi-step chain reconstruction.

Result: Best-performing LLM achieved F1-scores of 79.4% and 80.4%, outperforming baselines. Relationship explanations were partially accurate (97%), but fully correct explanations ranged lower (42.9%-71.1%). Multi-step chains had high endpoint accuracy but varied in intermediate links.
...
### [301] [How Robust are LLM-Generated Library Imports? An Empirical Study using Stack Overflow](https://arxiv.org/abs/2507.10818)
*Jasmine Latendresse,SayedHassan Khatoonabadi,Emad Shihab*

Main category: cs.SE

TL;DR: The paper analyzes how LLMs recommend Python libraries, finding a preference for third-party, mature, and popular libraries but noting usability gaps like unresolved dependencies and lack of installation guidance.


<details>
  <summary>Details</summary>
Motivation: Understanding how LLMs recommend libraries is crucial for improving their reliability and usability in programming tasks.

Method: An empirical study of six LLMs (proprietary and open-source) by prompting them to solve real-world Python problems from Stack Overflow, analyzing library imports and usability.

Result: LLMs favor third-party libraries, often mature and popular, but 4.6% had unresolved dependencies, and only two models provided installation guidance.

Conclusion: The study highlights usability gaps in LLM-generated code and suggests improvements for better reliability and dependency handling.
...
### [302] [Automated File-Level Logging Generation for Machine Learning Applications using LLMs: A Case Study using GPT-4o Mini](https://arxiv.org/abs/2508.04820)
*Mayra Sofia Ruiz Rodriguez,SayedHassan Khatoonabadi,Emad Shihab*

Main category: cs.SE

TL;DR: GPT-4o mini was evaluated for generating file-level log statements in ML projects, showing promise but with challenges like overlogging and misalignment with conventions.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of LLMs in generating comprehensive log statements for ML applications, addressing the gap in file-level logging.

Method: Evaluated GPT-4o mini on 171 ML repositories (4,073 Python files) by removing original logs, prompting the LLM to regenerate them, and comparing position, level, variables, and text quality.

Result: LLM matched human log positions 63.91% of the time but had an 82.66% overlogging rate. Challenges included overlogging at function boundaries and misalignment with project conventions.

Conclusion: LLMs show promise for file-level log generation, but issues like overlogging and convention misalignment need addressing for practical use.
...
### [303] [Posterior-GRPO: Rewarding Reasoning Processes in Code Generation](https://arxiv.org/abs/2508.05170)
*Lishui Fan,Yu Zhang,Mouxiang Chen,Zhongxin Liu*

Main category: cs.SE

TL;DR: The paper introduces a unified framework for reinforcement learning in code generation, focusing on reasoning quality to avoid reward hacking, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Current RL paradigms for LLMs neglect intermediate reasoning quality, leading to reward hacking. The paper aims to address this by evaluating and rewarding reasoning processes.

Method: Developed LCB-RB benchmark for reasoning evaluation, introduced OD-based reward model training, and proposed P-GRPO RL method to condition rewards on task success.

Result: A 7B parameter model with P-GRPO outperforms baselines by 4.5%, matching GPT-4-Turbo, and generalizes to mathematical tasks.

Conclusion: The framework effectively improves reasoning quality in RL for code generation, mitigating reward hacking and enhancing performance.
...
### [304] [Taxonomy of Faults in Attention-Based Neural Networks](https://arxiv.org/abs/2508.04925)
...
<details>
  <summary>Details</summary>
Motivation: Existing fault taxonomies fail to address unique failures in attention mechanisms, leaving practitioners without guidance.

Method: Systematic analysis of 555 real-world faults from 96 projects across ten frameworks.

Result: Over half of ABNN faults stem from attention-specific issues; four diagnostic heuristics explain 33% of these faults.

Conclusion: The study provides actionable diagnostic guidance for attention-based models, filling a critical gap in deep learning fault analysis.
...
### [305] [EvoGraph: Hybrid Directed Graph Evolution toward Software 3.0](https://arxiv.org/abs/2508.05199)
*Igor Costa,Christopher Baran*

Main category: cs.SE

TL;DR: EvoGraph is a framework for evolving software systems by representing artifacts in a graph, using SLMs for mutations, and multi-objective fitness for selection. It achieves high success in security fixes, language translation, and documentation freshness, with significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address challenges in legacy modernization, such as implicit contracts, performance preservation, and integration evolution, by enabling continuous adaptation of software systems.

Method: EvoGraph represents artifacts in a typed directed graph, applies mutation operators via specialized SLMs, and selects survivors using multi-objective fitness.

Result: EvoGraph fixes 83% of security vulnerabilities, translates COBOL to Java with 93% functional equivalence, maintains documentation freshness, reduces latency by 40%, and cuts feature lead time sevenfold. It also modernizes legacy codebases with 82-96% semantic equivalence and 90% cost reduction.

Conclusion: EvoGraph offers a practical approach to Software 3.0, enabling continuous adaptation while maintaining control, with significant improvements in modernization and performance.
...
### [306] [An ML-based Approach to Predicting Software Change Dependencies: Insights from an Empirical Study on OpenStack](https://arxiv.org/abs/2508.05034)
*Arabat,Ali,Sayagh,Mohammed,Hassine,Jameleddine*

Main category: cs.SE

TL;DR: The paper addresses dependency management in complex software systems, proposing a semi-automated ML-based approach to identify dependencies early.


<details>
  <summary>Details</summary>
Motivation: Managing dependencies in large-scale software systems is challenging, with many dependencies identified late in the process, causing inefficiencies.
...
Conclusion: The proposed approach can help developers proactively identify dependencies, improving efficiency in software development and deployment.
...
TL;DR: Tesserae improves DL cluster scheduling by formulating placement constraints as graph matching problems, enhancing performance and scalability.
...
Method: Designs novel placement policies for minimizing job migration overheads and job packing, integrating them into Tesserae.

Result: Tesserae improves average JCT by up to 1.62x and Makespan by up to 1.15x compared to existing schedulers.

Conclusion: Tesserae's graph matching approach leads to a scalable and effective GPU cluster scheduler.
...
### [308] [Differentially Private Model-X Knockoffs via Johnson-Lindenstrauss Transform](https://arxiv.org/abs/2508.04800)
*Yuxuan Tao,Adel Javanmard*

Main category: stat.ML

TL;DR: A novel privatization framework for high-dimensional variable selection ensures FDR control under differential privacy by using Gaussian JLT, preserving covariate relationships while maintaining privacy.


<details>
  <summary>Details</summary>
Motivation: Existing privacy mechanisms disrupt exchangeability in Model-X knockoff procedures, necessitating a method that balances FDR control and privacy.

Method: Privatizes data knockoff matrix via Gaussian JLT, a dimension reduction technique, and uses a debiasing technique for high-dimensional private knockoff procedures.

Result: Theoretical analysis shows FDR control and power convergence under specific conditions, outperforming classical noise addition.

Conclusion: The framework bridges knockoff-based FDR control and private data release, enabling reliable variable selection in sensitive domains.
...
### [309] [The Cosine Schedule is Fisher-Rao-Optimal for Masked Discrete Diffusion Models](https://arxiv.org/abs/2508.04884)
*Leo Zhang*

Main category: stat.ML

TL;DR: The paper demonstrates that the optimal discretization schedule for masked discrete diffusion models, based on Fisher-Rao geometry, aligns with the widely-used cosine schedule.


<details>
  <summary>Details</summary>
Motivation: To determine the best discretization schedule for sampling from masked discrete diffusion models by leveraging information geometry.

Method: Analyzes the problem using the Fisher-Rao geometry of the induced probability path.

Result: The optimal schedule under Fisher-Rao geometry matches the commonly-used cosine schedule.

Conclusion: The cosine schedule is justified as optimal for masked discrete diffusion models when considering Fisher-Rao geometry.
...
### [310] [High-Dimensional Differentially Private Quantile Regression: Distributed Estimation and Statistical Inference](https://arxiv.org/abs/2508.05212)
*Ziliang Shen,Caixing Wang,Shaoli Wang,Yibo Yan*

Main category: stat.ML

TL;DR: A differentially private quantile regression method for high-dimensional data is proposed, addressing privacy concerns in heterogeneous datasets while ensuring statistical accuracy and privacy guarantees.


<details>
  <summary>Details</summary>
Motivation: Privacy concerns in big data and machine learning, especially with sensitive personal information, necessitate robust privacy-preserving methods like differential privacy.

Method: A Newton-type transformation converts quantile regression into an ordinary least squares problem, enabling a differentially private estimation algorithm with iterative updates. A debiased estimator and communication-efficient bootstrap are introduced for inference.

Result: The method demonstrates robustness and effectiveness in simulations, providing near-optimal statistical accuracy and formal privacy guarantees.

Conclusion: The proposed approach successfully addresses privacy and computational challenges in high-dimensional quantile regression, making it suitable for distributed settings.
...
### [311] [L1-Regularized Functional Support Vector Machine](https://arxiv.org/abs/2508.05567)
...
<details>
  <summary>Details</summary>
Motivation: To address the gap in considering multivariate functional covariates in binary classification.

Method: Develops an L1-regularized functional SVM and an accompanying algorithm to fit the classifier.
...
### [312] [High-Order Error Bounds for Markovian LSA with Richardson-Romberg Extrapolation](https://arxiv.org/abs/2508.05570)
*Ilya Levin,Alexey Naumov,Sergey Samsonov*

Main category: stat.ML

TL;DR: The paper analyzes bias and high-order error bounds in Linear Stochastic Approximation (LSA) with Polyak-Ruppert averaging under Markovian noise, proposing a bias decomposition and using Richardson-Romberg extrapolation to cancel the leading bias term.


<details>
  <summary>Details</summary>
Motivation: To address the bias in LSA with constant step size and Polyak-Ruppert averaging, which cannot eliminate the leading bias term, and to improve error bounds.

Method: Proposes a linearization technique for bias decomposition and applies Richardson-Romberg extrapolation to cancel the leading bias term. Analyzes high-order moment bounds.

Result: The Richardson-Romberg procedure cancels the leading bias term, and the leading error term matches the asymptotically optimal covariance matrix of vanilla averaged LSA.

Conclusion: The study provides a method to mitigate bias in LSA with constant step size and demonstrates the effectiveness of Richardson-Romberg extrapolation in improving error bounds.
...
### [313] [Two tales for a geometric Jensen--Shannon divergence](https://arxiv.org/abs/2508.05066)
*Frank Nielsen*
...
<details>
  <summary>Details</summary>
Motivation: To address limitations of the G-JSD by extending it to positive densities without normalizing geometric mixtures.
...
Result: Shows the gap between extended G-JSD and G-JSD, with bounds in terms of other divergences, and interprets both as regularizations of the ordinary JSD.

Conclusion: The extended G-JSD offers a more general framework for positive densities and provides interpretable regularization insights.
...
### [314] [Adversarial Attacks and Defenses on Graph-aware Large Language Models (LLMs)](https://arxiv.org/abs/2508.04894)
...
Method: Leverages existing adversarial attack methods (poisoning and evasion) on two models (LLAGA and GRAPHPROMPTER), identifies new attack surfaces, and analyzes design choices affecting robustness.

Result: Reveals vulnerabilities in LLAGA due to node sequence templates, greater robustness in GRAPHPROMPTER's GNN encoder, and susceptibility to feature perturbations in both.

Conclusion: Proposes GALGUARD, a defense framework combining LLM-based feature correction and GNN defenses to mitigate adversarial attacks.
...
### [315] [Incident Response Planning Using a Lightweight Large Language Model with Reduced Hallucination](https://arxiv.org/abs/2508.05188)
*Kim Hammar,Tansu Alpcan,Emil C. Lupu*

Main category: cs.CR

TL;DR: The paper proposes a method to use LLMs for incident response planning with reduced hallucination, achieving faster recovery times and broad applicability.


<details>
  <summary>Details</summary>
Motivation: The growing frequency of cyberattacks necessitates timely and effective incident response, but current LLM-based methods are costly and prone to hallucinations.

Method: The approach involves fine-tuning, information retrieval, and lookahead planning to generate response plans with bounded hallucination probability.

Result: The method achieves up to 22% shorter recovery times than frontier LLMs and generalizes across incident types.

Conclusion: The lightweight method reduces hallucination and improves incident response efficiency, running on commodity hardware.
...
### [316] [Cross-Domain Image Synthesis: Generating H&E from Multiplex Biomarker Imaging](https://arxiv.org/abs/2508.04734)
...
<details>
  <summary>Details</summary>
Motivation: Integrating mIF's molecular data with H&E's morphological context enhances tissue analysis and leverages existing H&E-based CAD tools.

Method: A multi-level VQGAN is compared to a cGAN for generating virtual H&E stains from mIF images, evaluated on image similarity and functional utility.

Result: VQGAN produces more effective virtual stains, improving nuclei segmentation and tissue classification compared to cGAN.

Conclusion: VQGAN is superior for generating scientifically useful virtual stains, bridging molecular and morphological analysis.
...
### [317] [ERDES: A Benchmark Video Dataset for Retinal Detachment and Macular Status Classification in Ocular Ultrasound](https://arxiv.org/abs/2508.04735)
*Pouyan Navard,Yasemin Ozkut,Srikar Adhikari,Elaine Situ-LaCasse,Josie Acuña,Adrienne Yarnish,Alper Yilmaz*

Main category: q-bio.QM

TL;DR: The paper introduces ERDES, the first open-access dataset for retinal detachment (RD) detection and macular status classification using ultrasound, aiming to support ML model development.


<details>
  <summary>Details</summary>
Motivation: The lack of expertise in ultrasound interpretation and the absence of ML algorithms for RD detection, especially for macular status, motivated the creation of a public dataset.

Method: The authors developed ERDES, a labeled dataset of ocular ultrasound clips, and provided baseline benchmarks using spatiotemporal CNN architectures.

Result: The dataset and benchmarks are publicly available, facilitating ML model development for RD detection and macular status classification.

Conclusion: ERDES addresses a critical gap in RD diagnosis and prioritization, enabling future research and clinical applications.
...
### [318] [Understanding protein function with a multimodal retrieval-augmented foundation model](https://arxiv.org/abs/2508.04724)
*Timothy Fei Truong Jr,Tristan Bepler*

Main category: q-bio.QM

TL;DR: PoET-2 is a multimodal, retrieval-augmented protein foundation model that improves zero-shot variant effect prediction and supervised sequence-function learning by combining evolutionary constraints and structure conditioning.


<details>
  <summary>Details</summary>
Motivation: To address limitations in mutation understanding and representation quality in protein function prediction by scaling protein language models.

Method: Introduces PoET-2, a hierarchical transformer encoder with dual decoder architecture, incorporating in-context learning of evolutionary constraints and optional structure conditioning.
...
### [319] [Federal Reserve Communication and the COVID-19 Pandemic](https://arxiv.org/abs/2508.04830)
...
Method: Used specialized dictionaries for COVID-19, unconventional monetary policy, and financial stability, combined with sentiment analysis and topic modeling. Comparative analysis of Fed communication across crises.

Result: Fed communication was more reactive during COVID-19, with declining sentiment in financial stability discussions predicting accommodative policies. Unconventional monetary policy communication became standard post-Global Financial Crisis.

Conclusion: The Fed's communication adapts during crises, with COVID-19 highlighting a reactive approach and institutionalizing unconventional monetary policy discussions.
...
### [320] [Supervised Machine Learning Methods with Uncertainty Quantification for Exoplanet Atmospheric Retrievals from Transmission Spectroscopy](https://arxiv.org/abs/2508.04982)
*Roy T. Forestano,Konstantin T. Matchev,Katia Matcheva,Eyup B. Unlu*

Main category: astro-ph.EP

TL;DR: The paper compares machine learning (ML) methods for retrieving exoplanet atmospheric parameters from transmission spectra, benchmarking their accuracy, precision, and speed.


<details>
  <summary>Details</summary>
Motivation: Traditional Bayesian retrievals are computationally expensive, prompting the need for efficient ML alternatives, especially with upcoming observatories like JWST.

Method: The study evaluates ML regression techniques (PLS, SVM, KNN, DT, RF, VOTE, STACK, XGB) and preprocessing methods, quantifying uncertainties and validating the best model on JWST data for WASP-39b.

Result: The research identifies the best-performing ML model and preprocessing scheme for exoplanet atmospheric parameter retrieval.

Conclusion: ML methods offer efficient and robust alternatives to traditional Bayesian retrievals, validated by JWST observations.
...
### [321] [CodonMoE: DNA Language Models for mRNA Analyses](https://arxiv.org/abs/2508.04739)
...
<details>
  <summary>Details</summary>
Motivation: Addressing the inefficiency of maintaining separate models for DNA and RNA or using large multi-modal architectures, which impose computational burdens.

Method: Introduces CodonMoE, an adapter that maps codon sequences to RNA properties, leveraging DNA models for RNA tasks without additional pretraining.

Result: CodonMoE-augmented DNA models outperform unmodified counterparts in RNA prediction tasks, achieving state-of-the-art results with 80% fewer parameters.

Conclusion: CodonMoE offers a unified, efficient approach to genomic language modeling, reducing computational overhead while maintaining performance.
...
### [322] [Discovery of Disease Relationships via Transcriptomic Signature Analysis Powered by Agentic AI](https://arxiv.org/abs/2508.04742)
*Ke Chen,Haohan Wang*

Main category: q-bio.GN

TL;DR: A transcriptomics-driven framework using GenoMAS identifies disease relationships by analyzing gene-level overlaps and pathway-based similarities, revealing new connections and therapeutic opportunities.
...
Method: Analyzes over 1300 disease-condition pairs using GenoMAS, a fully automated agentic AI system, and develops a pathway-based similarity framework integrating multi-database enrichment analysis.

Result: Identifies known comorbidities and new cross-category links, explores shared pathways, and highlights therapeutic repurposing opportunities for rare diseases.

Conclusion: Demonstrates the potential of biologically grounded agentic AI to scale transcriptomic analysis and provide mechanistic insights into complex disease relationships.
...
### [323] [GRIT: Graph-Regularized Logit Refinement for Zero-shot Cell Type Annotation](https://arxiv.org/abs/2508.04747)
...
<details>
  <summary>Details</summary>
Motivation: Current cell type annotation methods are labor-intensive and scale poorly. While CLIP-style models like LangCell automate this, their zero-shot predictions lack consistency.

Method: A graph-regularized optimization framework refines LangCell's logits using PCA-based k-NN graphs for local consistency.

Result: The method improves zero-shot accuracy by up to 10% across 14 datasets, spanning 11 organs and 200,000+ cells.

Conclusion: The training-free, model-agnostic framework enhances automated cell type annotation by leveraging structural robustness.
...
### [324] [Embedding Is (Almost) All You Need: Retrieval-Augmented Inference for Generalizable Genomic Prediction Tasks](https://arxiv.org/abs/2508.04757)
...
<details>
  <summary>Details</summary>
Motivation: To determine if task-specific fine-tuning is always necessary for pre-trained DNA models, given its cost and distribution dependency.

Method: Extract fixed embeddings from models (e.g., HyenaDNA, DNABERT-2) and feed them into lightweight classifiers, comparing performance to fine-tuning.

Result: Embedding methods often outperform fine-tuning in diverse data distributions, with 10x-20x faster inference and lower carbon emissions (e.g., 88% reduction).

Conclusion: Embedding extraction is a generalizable, efficient alternative to fine-tuning, offering strong performance and sustainability benefits.
...
### [325] [Advanced Multi-Architecture Deep Learning Framework for BIRADS-Based Mammographic Image Retrieval: Comprehensive Performance Analysis with Super-Ensemble Optimization](https://arxiv.org/abs/2508.04790)
...
<details>
  <summary>Details</summary>
Motivation: The complexity of BIRADS categorical matching in mammographic image retrieval and limitations in current methodologies (e.g., small sample sizes, improper data splitting) hinder clinical translation.

Method: The study compares CNN architectures (DenseNet121, ResNet50, VGG16) using advanced training strategies like fine-tuning, metric learning, and super-ensemble optimization, with rigorous data splitting and bootstrap validation.

Result: Advanced fine-tuning and super-ensemble optimization improved precision@10 significantly (e.g., DenseNet121: 34.79%, ResNet50: 34.54%, super-ensemble: 36.33%), outperforming literature benchmarks (20-25%).

Conclusion: The framework sets new performance benchmarks and provides evidence-based guidelines for clinical deployment in diagnostic support and quality assurance.
...
### [326] [CryoGS: Gaussian Splatting for Cryo-EM Homogeneous Reconstruction](https://arxiv.org/abs/2508.04929)
*Suyi Chen,Haibin Ling*

Main category: eess.IV

TL;DR: cryoGS introduces a GMM-based method for cryo-EM reconstruction without needing external initialization, enabling stable and efficient 3D molecular structure determination directly from raw images.
...
Method: cryoGS integrates Gaussian splatting with cryo-EM physics, featuring orthogonal projection-aware splatting, a normalization term, and an FFT-aligned coordinate system for stable reconstruction from random initialization.
...
### [327] [Beyond Pixels: Medical Image Quality Assessment with Implicit Neural Representations](https://arxiv.org/abs/2508.05168)
...
<details>
  <summary>Details</summary>
Motivation: Artifacts in medical imaging hinder diagnostic accuracy, and current methods are limited by preprocessing-induced information loss and high memory demands.

Method: The approach employs INRs for compact image representation, using deep weight space networks, graph neural networks, and relational attention transformers for quality assessment.
...
### [328] [Coarse-to-Fine Joint Registration of MR and Ultrasound Images via Imaging Style Transfer](https://arxiv.org/abs/2508.05240)
...
<details>
  <summary>Details</summary>
Motivation: To enhance registration performance between pre-surgery MR and post-resection US images, addressing the challenge of aligning these modalities.
...
Result: Improves consistency between MR and US image pairs in most cases.
...
### [329] [Artificial Intelligence-Based Classification of Spitz Tumors](https://arxiv.org/abs/2508.05391)
...
TL;DR: AI models outperform pathologists in distinguishing Spitz tumors from melanomas and predicting genetic aberrations and diagnostic categories, potentially improving pathology workflows.


<details>
  <summary>Details</summary>
Motivation: Spitz tumors are hard to diagnose due to overlapping features with melanomas. AI could enhance diagnostic accuracy and efficiency.
...
Result: AI achieved AUROC 0.95 and accuracy 0.86 for tumor-melanoma distinction, outperforming pathologists. Genetic and diagnostic predictions were above chance.

Conclusion: AI shows promise in improving Spitz tumor diagnosis and workflow efficiency, though challenges remain in genetic and diagnostic predictions.
...
### [330] [Toward Low-Latency End-to-End Voice Agents for Telecommunications Using Streaming ASR, Quantized LLMs, and Real-Time TTS](https://arxiv.org/abs/2508.04721)
*Vignesh Ethiraj,Ashwath David,Sidhanth Menon,Divya Vijay*

Main category: cs.SD

TL;DR: A low-latency telecom AI voice agent pipeline is introduced, combining four specialized models for real-time, domain-adapted voice interactions in telecom applications.
...
Method: Integrates four telecom-specific models (TSLAM, T-VEC, TTE, T-Synth) for streaming ASR, conversational intelligence, RAG, and real-time TTS, evaluated on a dataset of 500 telecom questions.

Result: The system achieves real-time factors (RTF) below 1.0, demonstrating low latency and high domain relevance for telecom deployments.
...
### [331] [Wearable Music2Emotion : Assessing Emotions Induced by AI-Generated Music through Portable EEG-fNIRS Fusion](https://arxiv.org/abs/2508.04723)
*Sha Zhao,Song Yi,Yangxuan Zhou,Jiadong Pan,Jiquan Wang,Jie Xia,Shijian Li,Shurong Dong,Gang Pan*

Main category: cs.SD

TL;DR: MEEtBrain is a portable, multimodal framework for emotion analysis using AI-generated music and synchronized EEG-fNIRS signals, addressing limitations of small music corpora, unimodal data, and cumbersome setups.


<details>
  <summary>Details</summary>
Motivation: To overcome constraints in music-based affective computing, such as limited stimuli, unimodal neural data, and non-portable setups.
...
Result: Validated with a 14-hour dataset from 20 participants, showing AI-generated music effectively elicits target emotions.
...
### [332] [SPGISpeech 2.0: Transcribed multi-speaker financial audio for speaker-tagged transcription](https://arxiv.org/abs/2508.05554)
...
<details>
  <summary>Details</summary>
Motivation: To expand the diversity of modeling tasks in speech recognition while retaining the core utility of the original SPGISpeech dataset, especially for financial domain applications.
...
### [333] [Towards Hallucination-Free Music: A Reinforcement Learning Preference Optimization Framework for Reliable Song Generation](https://arxiv.org/abs/2508.05011)
...
<details>
  <summary>Details</summary>
Motivation: Current supervised fine-tuning (SFT) approaches for lyric-to-song generation suffer from content hallucination and poor self-improvement, necessitating a more robust solution.

Method: A novel RL framework with preference optimization is introduced, utilizing a hallucination preference dataset and three strategies (DPO, PPO, GRPO) to optimize alignment and reduce PER.

Result: DPO achieved a 7.4% PER reduction, while PPO and GRPO reduced PER by 4.9% and 4.7%, respectively, effectively suppressing hallucinations while maintaining musical quality.

Conclusion: The RL-based framework successfully mitigates hallucinations in lyric-to-song generation and offers potential for broader applications in music style adherence and musicality enhancement.
...
### [334] [SpectroStream: A Versatile Neural Codec for General Audio](https://arxiv.org/abs/2508.05207)
*Yunpeng Li,Kehang Han,Brian McWilliams,Zalan Borsos,Marco Tagliasacchi*

Main category: cs.SD

TL;DR: SpectroStream is a neural audio codec for high-quality 48 kHz stereo music at 4-16 kbps, improving upon SoundStream with a time-frequency domain approach and delayed-fusion for multi-channel audio.
...
Method: Uses a neural architecture with time-frequency domain representation and delayed-fusion for multi-channel audio.

Result: Achieves high-quality reconstruction of 48 kHz stereo music at 4-16 kbps.

Conclusion: SpectroStream advances audio codec technology by efficiently handling high sample rates and multi-channel audio with improved quality.
...
### [335] [Estimating Musical Surprisal from Audio in Autoregressive Diffusion Model Noise Spaces](https://arxiv.org/abs/2508.05306)
*Mathias Rose Bjare,Stefan Lattner,Gerhard Widmer*

Main category: cs.SD

TL;DR: The paper explores using autoregressive diffusion models (ADMs) to estimate information content (IC) for modeling musical expectancy and surprisal, outperforming a Generative Infinite-Vocabulary Transformer (GIVT) in tasks like monophonic pitch surprisal and segment boundary detection.


<details>
  <summary>Details</summary>
Motivation: To improve the modeling of musical expectancy and surprisal by leveraging IC estimates from diffusion models, which may better capture diverse data and audio granularities.

Method: Uses IC calculated with autoregressive diffusion models (ADMs) based on two diffusion ODEs, evaluating performance in monophonic pitch surprisal and segment boundary detection tasks.

Result: Diffusion models match or exceed GIVT performance in both tasks, with improved results at specific noise levels corresponding to different audio granularities.

Conclusion: Diffusion models effectively capture musical surprisal, with noise levels influencing task performance, suggesting their suitability for modeling audio features at varying granularities.
...
### [336] [Learning Geometric-Aware Quadrature Rules for Functional Minimization](https://arxiv.org/abs/2508.05445)
*Costas Smaragdakis*

Main category: math.NA

TL;DR: QuadrANN, a GNN architecture, learns optimal quadrature weights for non-uniform point clouds, improving integration accuracy over standard methods.
...
Method: Uses a GNN with deep message-passing, encoding local geometric features and global context to generate adaptive quadrature rules.

Result: Reduces variance in integral estimation, outperforming Quasi-Monte Carlo methods, especially in critical areas.

Conclusion: QuadrANN enhances stability and accuracy for deep learning-based variational solvers.
...
### [337] [A Study of the Framework and Real-World Applications of Language Embedding for 3D Scene Understanding](https://arxiv.org/abs/2508.05064)
*Mahmoud Chick Zaouali,Todd Charter,Yehor Karpichev,Brandon Haworth,Homayoun Najjjaran*

Main category: cs.GR

TL;DR: This survey reviews the integration of Large Language Models (LLMs) and language embeddings with Gaussian Splatting for text-conditioned 3D scene representation, highlighting advancements, limitations, and future directions.


<details>
  <summary>Details</summary>
Motivation: The rapid emergence of Gaussian Splatting as a powerful 3D scene representation technique, combined with the potential of LLMs, has created a need for a comprehensive review of their intersection.

Method: The paper presents a structured review of research combining language guidance with Gaussian Splatting, covering theoretical foundations, integration strategies, and real-world applications.

Result: The survey identifies key advancements in text-conditioned generation and semantic understanding but notes limitations like computational bottlenecks and data scarcity.

Conclusion: Future work should address challenges such as generalizability and data availability to advance language-guided 3D scene understanding using Gaussian Splatting.
...
### [338] [Voost: A Unified and Scalable Diffusion Transformer for Bidirectional Virtual Try-On and Try-Off](https://arxiv.org/abs/2508.04825)
...
<details>
  <summary>Details</summary>
Motivation: Accurate modeling of garment-body correspondence in virtual try-on is challenging under pose and appearance variations.
...
Conclusion: Voost provides a scalable and unified solution for virtual try-on/try-off, achieving superior performance without task-specific components.
...
### [339] [Refining Gaussian Splatting: A Volumetric Densification Approach](https://arxiv.org/abs/2508.05187)
*Mohamed Abdul Gafoor,Marius Preda,Titus Zaharia*

Main category: cs.GR

TL;DR: A novel density control method for 3D Gaussian Splatting improves reconstruction quality by using volumes of inertia and explores point cloud initialization with SfM and DIM.
...
Method: Introduces a density control method leveraging volumes of inertia for Gaussian functions and evaluates point cloud initialization using SfM and DIM.

Result: Outperforms 3DGS in reconstruction quality on the Mip-NeRF 360 dataset, showing robust performance across scenes.

Conclusion: The proposed method enhances 3DGS by addressing densification limitations and offers superior results in novel view synthesis.
...
### [340] [Perceive-Sample-Compress: Towards Real-Time 3D Gaussian Splatting](https://arxiv.org/abs/2508.04965)
...
Result: Significant improvements in memory efficiency and visual quality without sacrificing real-time rendering speed.
...
### [341] [Laplacian Analysis Meets Dynamics Modelling: Gaussian Splatting for 4D Reconstruction](https://arxiv.org/abs/2508.04966)
*Yifan Zhou,Beizhen Zhao,Pengcheng Wu,Hao Wang*

Main category: cs.GR

TL;DR: A novel dynamic 3D Gaussian Splatting (3DGS) framework addresses challenges in dynamic scene modeling by combining hybrid explicit-implicit functions, spectral-aware encoding, and adaptive strategies for improved fidelity.
...
Method: The framework introduces spectral-aware Laplacian encoding, enhanced Gaussian dynamics attributes, and an adaptive Gaussian split strategy guided by KDTree-based control.

Result: The method achieves state-of-the-art performance in reconstructing complex dynamic scenes with superior fidelity.

Conclusion: The proposed hybrid framework effectively resolves spectral conflicts and enhances dynamic scene reconstruction.
...
### [342] [RAP: Real-time Audio-driven Portrait Animation with Video Diffusion Transformer](https://arxiv.org/abs/2508.05115)
*Fangyu Du,Taiqing Li,Ziwei Zhang,Qian Qiao,Tan Yu,Dingcheng Zhen,Xu Jia,Yang Yang,Shunshun Yin,Siyuan Liu*

Main category: cs.GR

TL;DR: RAP introduces a real-time framework for high-quality audio-driven portrait animation using hybrid attention and static-dynamic training, achieving precise control and visual fidelity.
...
Conclusion: RAP successfully bridges the gap between quality and real-time performance in audio-driven portrait animation.
...
### [343] [Point cloud segmentation for 3D Clothed Human Layering](https://arxiv.org/abs/2508.05531)
*Davide Garavaso,Federico Masi,Pietro Musoni,Umberto Castellani*

Main category: cs.GR

TL;DR: The paper introduces a new 3D point cloud segmentation paradigm for clothed human layering, addressing challenges in semantic reconstruction of garments and underlying body parts.


<details>
  <summary>Details</summary>
Motivation: High-quality 3D cloth modeling is challenging due to variability and lack of semantic information in scans. Current segmentation methods are not tailored for clothed body modeling.

Method: Proposes a clothed human layering paradigm where each 3D point can belong to multiple layers. Uses synthetic datasets and neural networks for evaluation.

Result: Demonstrates benefits of garment-specific segmentation strategies on synthetic and real-world datasets.

Conclusion: The proposed paradigm improves semantic reconstruction of clothed bodies, enabling better modeling of occluded regions and layers.
...
### [344] [Physically Controllable Relighting of Photographs](https://arxiv.org/abs/2508.05626)
...
<details>
  <summary>Details</summary>
Motivation: To enable fully controllable, physically based illumination editing in real-world images.

Method: Combines neural rendering with traditional rendering, using a colored mesh representation and path-tracing for illumination control, followed by a neural renderer for photorealism.

Result: Achieves photorealistic relighting with explicit physical control over lighting.

Conclusion: Bridges the gap between 3D graphics tools and real-world image relighting.
...
### [345] [Agency, Affordances, and Enculturation of Augmentation Technologies](https://arxiv.org/abs/2508.04725)
*Ann Hill Duin,Isabel Pedersen*

Main category: cs.CY

TL;DR: The paper critiques the assumption that advanced augmentation technologies, driven by AI, universally improve lives and societies. It examines AI terminology ambiguity, non-human agents in industry, and marketing's role in technology adoption, concluding with an analysis of the Metaverse and augmented reality.


<details>
  <summary>Details</summary>
Motivation: To critically assess the hype around AI-driven augmentation technologies and their societal impact, addressing ambiguity in AI terminology and the role of marketing in technology enculturation.

Method: The chapter uses the WIPO Categorization of AI Technologies Scheme to clarify AI terminology and draws on media and communication studies to explore human-robot relationships and industry trends.

Result: Highlights the influence of non-human agents in industry and marketing's role in shaping user adoption of augmentation technologies, with a focus on the Metaverse.

Conclusion: The paper questions the optimistic claims about augmentation technologies, emphasizing the need for critical examination of their societal and cultural implications.
...
### [346] [Building Effective Safety Guardrails in AI Education Tools](https://arxiv.org/abs/2508.05360)
*Hannah-Beth Clark,Laura Benton,Emma Searle,Margaux Dowland,Matthew Gregory,Will Gayne,John Roberts*

Main category: cs.CY

TL;DR: The paper discusses Oak National Academy's AI-powered lesson planning assistant (Aila) and its safety guardrails to ensure age-appropriate, pedagogically sound AI-generated content for classrooms.


<details>
  <summary>Details</summary>
Motivation: Address concerns about the safety and appropriateness of AI-generated content in education, particularly for pupils aged 5-16.
...
Result: Identified challenges and opportunities for refining safety guardrails in generative AI education tools.

Conclusion: Emphasizes iterative refinement of guardrails and cross-sector collaboration to enhance safety in AI education tools.
...
### [347] [Can SGD Handle Heavy-Tailed Noise?](https://arxiv.org/abs/2508.04860)
*Ilyas Fatkhullin,Florian Hübler,Guanghui Lan*

Main category: math.OC

TL;DR: The paper analyzes vanilla SGD's performance under heavy-tailed noise, proving its effectiveness across convex, strongly convex, and non-convex problems with minimal assumptions.


<details>
  <summary>Details</summary>
Motivation: To rigorously investigate whether vanilla SGD can succeed under heavy-tailed noise, common in modern ML, without adaptive modifications.

Method: Assumes bounded $p$-th moments for stochastic gradients ($p \in (1, 2]$) and analyzes SGD's convergence in convex, strongly convex, and non-convex settings.

Result: SGD achieves minimax optimal sample complexity: $\mathcal{O}(\varepsilon^{-\frac{p}{p-1}})$ (convex), $\mathcal{O}(\varepsilon^{-\frac{p}{2(p-1)}})$ (strongly convex), and $\mathcal{O}(\varepsilon^{-\frac{2p}{p-1}})$ (non-convex). Mini-batch SGD shows comparable results.

Conclusion: Vanilla SGD is robust under heavy-tailed noise, challenging the view that it becomes ineffective, and serves as a principled baseline even with unbounded variance.
...
### [348] [Exact and Heuristic Algorithms for Constrained Biclustering](https://arxiv.org/abs/2508.05493)
*Antonio M. Sudoso*

Main category: math.OC

TL;DR: The paper introduces constrained biclustering with pairwise constraints, proposing exact and heuristic algorithms to solve the problem, demonstrating superior performance on synthetic and real-world datasets.


<details>
  <summary>Details</summary>
Motivation: Incorporating background knowledge into biclustering to improve solution quality and interpretability, especially with pairwise constraints (must-link and cannot-link).
...
Result: Exact method outperforms general solvers; heuristic efficiently provides high-quality solutions for large-scale instances.

Conclusion: The proposed methods effectively address constrained biclustering, balancing accuracy and scalability.
...
### [349] [Telegrapher's Generative Model via Kac Flows](https://arxiv.org/abs/2506.20641)
*Richard Duong,Jannis Chemseddine,Peter K. Friz,Gabriele Steidl*

Main category: math.AP

TL;DR: A novel flow-based generative model using the telegrapher's equation, offering Lipschitz continuity and bounded velocity, outperforming diffusion models.


<details>
  <summary>Details</summary>
Motivation: To address limitations of diffusion models by introducing a model with globally bounded velocity and Lipschitz continuity in Wasserstein distance.

Method: Proposes a Kac flow based on the telegrapher's equation, extends it to multi-dimensions, and trains a neural network for velocity field approximation.

Result: The model shows scalability and advantages over diffusion models in numerical experiments.

Conclusion: The Kac flow model provides a promising alternative to diffusion models with improved properties and performance.
...
### [350] [Data Driven Insights into Composition Property Relationships in FCC High Entropy Alloys](https://arxiv.org/abs/2508.04841)
...
TL;DR: The paper addresses challenges in predicting properties of Structural High Entropy Alloys (HEAs) due to limited data. It uses sensitivity analyses and encoder-decoder models to map alloy composition to mechanical properties, achieving superior performance.
...
Method: Sensitivity analyses and encoder-decoder models, optimized via Bayesian multi-objective hyperparameter tuning, are used to map alloy composition to mechanical properties.

Result: The models outperform conventional regressors, especially for yield strength and UTS/YS ratio, capturing complex composition-property relationships effectively.

Conclusion: The work demonstrates the potential of advanced modeling techniques in improving property predictions for HEAs, aiding their application in critical industries.
...
### [351] [Navigating Through Paper Flood: Advancing LLM-based Paper Evaluation through Domain-Aware Retrieval and Latent Reasoning](https://arxiv.org/abs/2508.05129)
...
<details>
  <summary>Details</summary>
Motivation: The challenge of identifying high-quality research amid increasing publications, and limitations of current LLM-based methods in outdated knowledge and reasoning.

Method: PaperEval uses domain-aware retrieval for contextual novelty assessment and latent reasoning for deep understanding, with progressive ranking optimization.

Result: Outperforms existing methods in academic impact and quality evaluation; successful real-world deployment in a recommendation system.
...
### [352] [Align-for-Fusion: Harmonizing Triple Preferences via Dual-oriented Diffusion for Cross-domain Sequential Recommendation](https://arxiv.org/abs/2508.05074)
*Yongfu Zha,Xinxin Dong,Haokai Ma,Yonghui Yang,Xiaodong Wang*

Main category: cs.IR

TL;DR: HorizonRec is a novel cross-domain sequential recommendation framework using dual-oriented diffusion models for fine-grained preference fusion, addressing noise instability and improving recommendation accuracy.


<details>
  <summary>Details</summary>
Motivation: To tackle data sparsity and interest drift in sequential recommendations by harmonizing domain-specific preferences more effectively than existing align-then-fusion methods.

Method: Proposes HorizonRec, leveraging dual-oriented diffusion models for multi-domain preference fusion, with a mixed-conditioned distribution retrieval strategy to reduce noise and enhance consistency.

Result: Demonstrates effectiveness and robustness on four datasets, outperforming conventional methods in fine-grained triple-domain preference fusion.

Conclusion: HorizonRec advances cross-domain sequential recommendation by addressing noise issues and enabling precise preference fusion, validated by strong experimental results.
...
### [353] [Tool Graph Retriever: Exploring Dependency Graph-based Tool Retrieval for Large Language Models](https://arxiv.org/abs/2508.05152)
*Linfeng Gao,Yaoxiang Wang,Minlong Peng,Jialong Tang,Yuzhe Shang,Mingming Sun,Jinsong Su*

Main category: cs.IR

TL;DR: The paper proposes Tool Graph Retriever (TGR), a method leveraging tool dependencies for better retrieval, outperforming existing methods.
...
Result: TGR improves performance over existing methods, achieving state-of-the-art results.

Conclusion: Tool dependencies are crucial, and TGR effectively integrates them for superior retrieval.
...
### [354] [Balancing Accuracy and Novelty with Sub-Item Popularity](https://arxiv.org/abs/2508.05198)
...
TL;DR: The paper introduces a sub-ID-level Personalised Popularity Score (sPPS) within the RecJPQ framework to balance recommendation accuracy and novelty, outperforming item-level PPS.


<details>
  <summary>Details</summary>
Motivation: Address the limitation of Personalised Popularity Scores (PPS) in reinforcing known content, hindering novelty and serendipity in music recommendations.

Method: Repurpose RecJPQ's sub-item architecture to model personalised popularity at a finer granularity, capturing shared repetition patterns across sub-embeddings.

Result: sPPS achieves higher personalised novelty without compromising accuracy, outperforming item-level PPS.

Conclusion: The proposed sPPS method effectively balances accuracy and novelty, enhancing long-term user engagement in music recommendations.
...
### [355] [Scaling Generative Recommendations with Context Parallelism on Hierarchical Sequential Transducers](https://arxiv.org/abs/2508.04711)
...
<details>
  <summary>Details</summary>
Motivation: Large-scale recommendation systems need effective modeling of high cardinality and heterogeneous features, but scaling sequence length is activation-heavy, requiring parallelism solutions.

Method: The authors propose context parallelism (CP) with jagged tensor support for HSTU attention to address memory challenges in scaling sequence length.

Result: The approach enables a 5.3x increase in supported user interaction sequence length and a 1.55x scaling factor when combined with DDP.

Conclusion: The work establishes foundational capabilities for scaling sequence dimensions in recommendation systems, improving efficiency and performance.
...
### [356] [A Metric for MLLM Alignment in Large-scale Recommendation](https://arxiv.org/abs/2508.04963)
*Yubin Zhang,Yanhua Huang,Haiming Xu,Mingliang Qi,Chang Wang,Jiarui Jin,Xiangyuan Ren,Xiaodan Wang,Ruiwen Xu*

Main category: cs.IR

TL;DR: The paper introduces the Leakage Impact Score (LIS) to evaluate multimodal recommendation alignment, addressing challenges like static benchmarks, costly online evaluations, and uninformative metrics. LIS measures preference data's upper bound and shows real-world effectiveness in Xiaohongshu's A/B tests.


<details>
  <summary>Details</summary>
Motivation: Challenges in evaluating MLLM alignment for recommendation systems due to static benchmarks, expensive online evaluations, and inadequate metrics.

Method: Proposes LIS, a metric to measure preference data's upper bound, and shares deployment insights for MLLMs.

Result: Online A/B tests on Xiaohongshu's platform show improved user engagement and advertiser value.

Conclusion: LIS effectively addresses evaluation challenges and enhances multimodal recommendation performance in real-world applications.
...
### [357] [Multi-Modal Multi-Behavior Sequential Recommendation with Conditional Diffusion-Based Feature Denoising](https://arxiv.org/abs/2508.05352)
...
### [358] [On the Reliability of Sampling Strategies in Offline Recommender Evaluation](https://arxiv.org/abs/2508.05398)
*Bruno L. Pereira,Alan Said,Rodrygo L. T. Santos*

Main category: cs.IR

TL;DR: The paper examines how logging and sampling choices impact offline evaluation reliability in recommender systems, identifying biases and offering practical guidance for robust comparisons.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of offline evaluation in recommender systems, particularly exposure and sampling biases, and assess their impact on model comparisons.

Method: Systematically simulate diverse exposure biases using a fully observed dataset, evaluating sampling strategies on resolution, fidelity, robustness, and predictive power.

Result: Findings reveal how sampling distorts evaluation outcomes and provide insights into selecting strategies for reliable offline comparisons.

Conclusion: The study offers practical guidance for mitigating biases in offline evaluation, ensuring more faithful and robust comparisons of recommender models.
...
### [359] [KuaiLive: A Real-time Interactive Dataset for Live Streaming Recommendation](https://arxiv.org/abs/2508.05633)
*Changle Qu,Sunhao Dai,Ke Guo,Liqin Zhao,Yanan Niu,Xiao Zhang,Jun Xu*

Main category: cs.IR

TL;DR: KuaiLive is a real-time, interactive dataset from Kuaishou, addressing the lack of public datasets for live streaming recommendation research. It includes user-streamer interactions and side features, enabling diverse tasks like recommendation and behavior modeling.


<details>
  <summary>Details</summary>
Motivation: The dynamic nature of live streaming lacks public datasets, hindering academic research. KuaiLive fills this gap by providing a realistic dataset from a major platform.

Method: KuaiLive collects interaction logs (clicks, comments, likes, gifts) of 23,772 users and 452,621 streamers over 21 days, with precise timestamps and side features.

Result: The dataset supports tasks like top-K recommendation, CTR prediction, and multi-behavior modeling, establishing benchmarks for future research.

Conclusion: KuaiLive advances live streaming research by offering a comprehensive, publicly available dataset for diverse recommendation tasks.
...
### [360] [Speech LLMs in Low-Resource Scenarios: Data Volume Requirements and the Impact of Pretraining on High-Resource Languages](https://arxiv.org/abs/2508.05149)
...
### [361] [Fairness in Dysarthric Speech Synthesis: Understanding Intrinsic Bias in Dysarthric Speech Cloning using F5-TTS](https://arxiv.org/abs/2508.05102)
*Anuprabha M,Krishna Gurugubelli,Anil Kumar Vuppala*

Main category: eess.AS

TL;DR: The paper investigates F5-TTS for dysarthric speech cloning, revealing biases toward intelligibility over speaker similarity and prosody. Fairness metrics highlight disparities, guiding inclusive speech tech.


<details>
  <summary>Details</summary>
Motivation: Addressing data scarcity and biases in dysarthric speech synthesis to improve assistive technologies.

Method: Uses F5-TTS with TORGO dataset, evaluating intelligibility, speaker similarity, prosody, and fairness metrics (Disparate Impact, Parity Difference).

Result: F5-TTS favors intelligibility, neglecting speaker and prosody preservation, with biases across dysarthric severity levels.

Conclusion: Findings support fairness-aware dysarthric speech synthesis for more inclusive speech technologies.
...
### [362] [Keyword Spotting with Hyper-Matched Filters for Small Footprint Devices](https://arxiv.org/abs/2508.04857)
*Yael Segal-Feldman,Ann R. Bradlow,Matthew Goldrick,Joseph Keshet*

Main category: eess.AS

TL;DR: The paper presents an open-vocabulary keyword spotting (KWS) model for small devices, achieving state-of-the-art accuracy with a compact design of 4.2M parameters.


<details>
  <summary>Details</summary>
Motivation: To detect keywords in speech recordings, even if they weren't in the training data, for small-footprint devices.

Method: Uses a speech encoder (tiny Whisper or Conformer), a hyper-network keyword encoder, and a detection network with cross-attention.

Result: State-of-the-art detection accuracy, effective generalization to out-of-domain conditions like L2 speech, and outperforms larger models.

Conclusion: The model is efficient, robust, and scalable for open-vocabulary KWS on small devices.
...
### [363] [Optimality Principles and Neural Ordinary Differential Equations-based Process Modeling for Distributed Control](https://arxiv.org/abs/2508.04799)
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap between classical process control and modern data-driven methods by ensuring consistency in topology and conservation laws.

Method: Proposes a framework using connectivity matrices and network graphs, with a neural ODE model for learning dynamics from data.

Result: Demonstrates successful integration in an inventory control system, forming a state space model for predictive control.

Conclusion: The approach enables seamless integration of data-driven algorithms with classical process models, validated in a practical example.
...
### [364] [AI Should Be More Human, Not More Complex](https://arxiv.org/abs/2508.04713)
*Carlo Esposito*

Main category: cs.HC

TL;DR: Users prefer concise, source-attributed AI responses over verbose ones, as current trends toward 'artificial sophistication' reduce trust and engagement.


<details>
  <summary>Details</summary>
Motivation: To investigate why verbose, complex AI responses in search applications reduce user satisfaction despite seeming sophisticated.

Method: A study of 10,000 participants comparing responses from five major AI-powered search systems.

Result: Users overwhelmingly favor concise, sourced responses; verbose ones create distrust and cognitive load.

Conclusion: Optimal AI communication mirrors human discourse—direct, sourced, and transparent—challenging the assumption that complexity equals better performance.
...
### [365] [Evaluating the Impact of LLM-guided Reflection on Learning Outcomes with Interactive AI-Generated Educational Podcasts](https://arxiv.org/abs/2508.04787)
*Vishnu Menon,Andy Cherney,Elizabeth B. Cloude,Li Zhang,Tiffany D. Do*

Main category: cs.HC

TL;DR: Embedding LLM-guided reflection prompts in an AI-generated podcast did not improve learning but reduced perceived attractiveness.


<details>
  <summary>Details</summary>
Motivation: To explore if reflection prompts enhance learning and user experience in AI-generated podcasts.

Method: Thirty-six undergraduates tested two versions of an interactive AI-generated podcast: one with LLM-guided reflection prompts and one without.

Result: Learning outcomes were similar, but prompts reduced perceived attractiveness.

Conclusion: More research is needed on reflective interactivity design to balance learning and user experience.
...
### [366] [Situated Epistemic Infrastructures: A Diagnostic Framework for Post-Coherence Knowledge](https://arxiv.org/abs/2508.04995)
*Matthew Kelly*

Main category: cs.HC

TL;DR: The paper critiques LLMs like ChatGPT for disrupting traditional knowledge validation and proposes the SEI framework to analyze credibility in hybrid human-machine systems.


<details>
  <summary>Details</summary>
Motivation: To address the fragility of knowledge infrastructures exposed by LLMs, which simulate coherence without traditional validation.

Method: Introduces the Situated Epistemic Infrastructures (SEI) framework, integrating insights from infrastructure studies, platform theory, and epistemology.

Result: SEI provides a diagnostic tool for analyzing credibility in post-coherence conditions, emphasizing coordination over classification.
...
### [367] [Human-AI Schema Discovery and Application for Creative Problem Solving](https://arxiv.org/abs/2508.05045)
*Sitong Wang*
...
Result: The framework makes implicit knowledge more accessible and actionable, improving human-AI collaboration.

Conclusion: Schema-guided interaction advances transparent and collaborative human-AI systems.
...
### [368] [CWEFS: Brain volume conduction effects inspired channel-wise EEG feature selection for multi-dimensional emotion recognition](https://arxiv.org/abs/2508.05228)
*Xueyuan Xu,Wenjia Dong,Fulin Wei,Li Zhuo*

Main category: cs.HC

TL;DR: A novel channel-wise EEG feature selection (CWEFS) method is proposed to address redundancy in EEG data for emotion recognition, integrating latent structure modeling and adaptive channel-weight learning, outperforming 19 other methods.


<details>
  <summary>Details</summary>
Motivation: High-dimensional EEG data contains redundant information, hindering discriminative emotion representation extraction and real-time performance, while existing methods overlook latent EEG feature structures and channel importance.

Method: CWEFS integrates EEG feature selection into a shared latent structure model, preserving local geometric structures and incorporating adaptive channel-weight learning to determine channel significance.

Result: CWEFS outperforms 19 feature selection methods across six evaluation metrics on three EEG datasets with multi-dimensional emotional labels.

Conclusion: CWEFS effectively addresses EEG feature redundancy and channel importance, enhancing emotion recognition performance and interpretability.
...
### [369] [ADSEL: Adaptive dual self-expression learning for EEG feature selection via incomplete multi-dimensional emotional tagging](https://arxiv.org/abs/2508.05229)
*Tianze Yu,Junming Zhang,Wenjia Dong,Xueyuan Xu,Li Zhuo*

Main category: cs.HC

TL;DR: Proposes a novel EEG feature selection method (ADSEL) for multi-dimensional emotion recognition, addressing incomplete labels by integrating sample and dimension-level learning.


<details>
  <summary>Details</summary>
Motivation: High dimensionality of EEG features and incomplete labels in emotion recognition lead to overfitting and poor generalization, necessitating better feature selection.
...
Result: ADSEL improves label recovery accuracy and identifies optimal EEG feature subsets for emotion recognition.

Conclusion: The method effectively handles incomplete labels and enhances feature selection for EEG-based emotion recognition.
...
### [370] [FDC-Net: Rethinking the association between EEG artifact removal and multi-dimensional affective computing](https://arxiv.org/abs/2508.05231)
*Wenjia Dong,Xueyuan Xu,Tianze Yu,Junming Zhang,Li Zhuo*

Main category: cs.HC

TL;DR: A novel framework, FDC-Net, integrates EEG denoising and emotion recognition tasks to improve noise robustness and performance, outperforming existing methods on DEAP and DREAMER datasets.


<details>
  <summary>Details</summary>
Motivation: EEG-based emotion recognition is hindered by physiological artifacts, and current methods treat denoising and recognition separately, leading to inefficiencies and lack of noise robustness.

Method: FDC-Net combines denoising and emotion recognition through bidirectional gradient propagation, joint optimization, and a gated attention mechanism with a frequency-adaptive Transformer.

Result: FDC-Net achieves high correlation coefficients for denoising (96.30% on DEAP, 90.31% on DREAMER) and strong emotion recognition accuracies (82.3% on DEAP, 88.1% on DREAMER).

Conclusion: The FDC-Net framework effectively addresses noise robustness in EEG-based emotion recognition by coupling denoising and classification tasks, demonstrating superior performance over existing methods.
...
### [371] [Driver Assistant: Persuading Drivers to Adjust Secondary Tasks Using Large Language Models](https://arxiv.org/abs/2508.05238)
*Wei Xiang,Muchen Li,Jie Yan,Manling Zheng,Hanfei Zhu,Mengyun Jiang,Lingyun Sun*

Main category: cs.HC

TL;DR: A study uses an LLM to provide humanized persuasive advice to drivers in Level 3 automated systems, improving attention and reducing cognitive load during emergencies.


<details>
  <summary>Details</summary>
Motivation: Level 3 automation reduces driver risk perception, creating challenges during emergencies requiring quick intervention.

Method: The study employs an LLM to deliver proactive, human-like advice via visual and auditory cues based on road conditions.

Result: The tool effectively maintains driver attention, lowers cognitive load, and improves coordination of secondary tasks with takeover actions.

Conclusion: LLMs show promise in supporting drivers in multi-task automated driving scenarios.
...
### [372] [Salt-Rock Creep Deformation Forecasting Using Deep Neural Networks and Analytical Models for Subsurface Energy Storage Applications](https://arxiv.org/abs/2508.05248)
*Pradeep Kumar Shukla,Tanujit Chakraborty,Mustafa Sari,Joel Sarout,Partha Pratim Mandal*

Main category: physics.geo-ph

TL;DR: The study analyzes time series forecasting methods to predict creep deformation in salt rock under varying pressures, comparing deep neural networks (DNNs) with statistical models. N-BEATS and TCN outperformed others, showing 15-20% better accuracy.


<details>
  <summary>Details</summary>
Motivation: Accurate creep deformation prediction is crucial for designing underground storage facilities for hazardous materials, leveraging salt rock's unique properties.

Method: Used multi-stage triaxial creep data, STL decomposition, Granger causality, ADF tests, and wavelet coherence. Compared DNN models (N-BEATS, TCN, RNN, TF) against statistical baselines.

Result: N-BEATS and TCN models outperformed others, improving accuracy by 15-20% over traditional methods.

Conclusion: DNN models, especially N-BEATS and TCN, are effective for predicting creep deformation, capturing complex temporal patterns better than statistical models.
...
### [373] [Anti-Jamming Sensing with Distributed Reconfigurable Intelligent Metasurface Antennas](https://arxiv.org/abs/2508.04964)
*Zhaowei Wang,Yunsong Huang,Weicheng Liu,Hui-Ming Wang*

Main category: eess.SP

TL;DR: Proposes a distributed RIMSA system with DRL for RF sensing, improving accuracy and resilience to interference and jamming.


<details>
  <summary>Details</summary>
Motivation: Traditional RF sensing suffers from environmental challenges like fading and noise, and is vulnerable to jamming attacks.

Method: Uses distributed RIMSA receivers with programmable beamforming and a DRL algorithm for optimization, plus a neural network for signal-to-sensing mapping.

Result: Simulations show superior sensing performance and resilience to interference and jamming compared to centralized systems.

Conclusion: The distributed RIMSA system with DRL and neural networks enhances RF sensing accuracy and robustness in adverse conditions.
...
<details>
  <summary>Details</summary>
Motivation: Efficient energy management in hybrid electric trucks is crucial for fuel savings and battery longevity.

Method: Reformulated control as sequential decision-making, enhanced SAC with GRUs and Decision Transformers for temporal dependencies.

Result: SAC with DT-based actor and GRU-based critic achieved 1.8% fuel savings close to DP, outperforming FFN-based agents on unseen cycles.

Conclusion: The framework demonstrates adaptability and robustness, making it suitable for real-world heavy-duty truck applications.
