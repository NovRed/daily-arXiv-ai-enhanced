...
Motivation: The paper addresses the need for more transparent and reproducible document visual question answering (DocVQA) systems that can provide interpretable reasoning without requiring additional model training or fine-tuning.
...
Result: Experiments on the DocVQA dataset show that the best EaGERS configuration outperforms the base model on both exact match accuracy and Average Normalized Levenshtein Similarity metrics while providing enhanced transparency and reproducibility.

Conclusion: EaGERS successfully demonstrates that a fully training-free and model-agnostic approach can improve DocVQA performance while enhancing interpretability, making it a practical solution for transparent document understanding tasks.
...
<details>
  <summary>Details</summary>
Motivation: VLMs struggle with 3D spatial tasks due to their reliance on 2D images, lacking internal 3D dynamics modeling.

Method: Couples a VLM with a video diffusion-based world model to iteratively sketch camera trajectories and synthesize views for reasoning.

Result: Achieves an 8% performance boost on the SAT benchmark without fine-tuning, outperforming reinforcement learning-trained VLMs.
...
<details>
  <summary>Details</summary>
Motivation: Existing monolithic MLLMs face unstable optimization and catastrophic forgetting, prompting the need for stable learning of visual knowledge.

Method: Embed a new visual parameter space into a pre-trained LLM using delta tuning, introduce Mono-InternVL with a multimodal mixture-of-experts architecture, and design EViP for progressive learning. Mono-InternVL-1.5 improves efficiency with EViP++ and fused CUDA kernels.

Result: Mono-InternVL outperforms existing MLLMs on 12/15 benchmarks, e.g., +114-point improvement over Emu3. Mono-InternVL-1.5 reduces costs while maintaining performance.

Conclusion: The proposed monolithic MLLMs effectively address optimization issues, achieve strong performance, and reduce costs, validated by extensive benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: To identify the best workflows for crop mapping using remote sensing data, considering both supervised and transfer learning approaches.

Method: Systematic experiments comparing preprocessing methods, supervised classification models, and transfer learning techniques across diverse agricultural sites using Landsat 8 data.
...
Conclusion: Workflow choice depends on labeled sample availability: supervised training excels with sufficient samples, while transfer learning is viable for limited data.
...
<details>
  <summary>Details</summary>
Motivation: Understanding radiologists' eye movements during CT reading is key for interpretable computer-aided diagnosis, but limited by lack of datasets and 3D complexity.

Method: The authors present CT-ScanGaze dataset and CT-Searcher, a 3D scanpath predictor, along with a pipeline to pretrain it using converted 2D gaze data.

Result: The approach is validated through qualitative and quantitative evaluations, demonstrating effectiveness in 3D scanpath prediction for medical imaging.

Conclusion: The work provides a foundational dataset and framework for advancing 3D scanpath prediction in CT interpretation.
...
Method: Hierarchical multiscale fusion with scale-specific feature engineering (local, branch, canopy) and cross-scale information propagation.

Result: Achieves 94.96% accuracy on STPCTLS, 67.25% on FOR-species20K, and outperforms DGCNN/MS-DGCNN on ModelNet datasets.

Conclusion: MS-DGCNN++ is versatile, efficient, and generalizes well to 3D object recognition, offering a resource-constrained solution.
...
<details>
  <summary>Details</summary>
Motivation: Limited annotated datasets for real-world sports scenarios hinder action anticipation in HAR. This work addresses the gap by focusing on soccer penalty kicks.
...
Conclusion: Funnel-HOI outperforms existing methods by focusing on encoder-level interaction representations and better loss regulation.
...
TL;DR: CogNVS combines 3D scene reconstruction and 2D video diffusion for novel-view synthesis, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of costly 4D optimization and poor geometry preservation in existing methods for dynamic scene synthesis.

Method: Uses covisible pixel reconstruction and hidden pixel inpainting with a self-supervised 2D video diffusion model (CogNVS).

Result: CogNVS achieves superior performance in novel-view synthesis for dynamic scenes.

Conclusion: The approach enables zero-shot application to new videos and outperforms prior art.
...
<details>
  <summary>Details</summary>
Motivation: Current CVD risk stratification methods often miss early subclinical changes. The study aims to bridge this gap by combining retinal imaging and lipidomics for better early detection.

Method: An imaging omics framework was used, combining deep learning-based retinal image analysis with UHPLC ESI HRMS serum lipid profiling in a healthy population.
...
Conclusion: The study provides novel insights into early CVD pathogenesis and highlights the potential of non-invasive biomarkers for improved early detection and personalized healthcare.
...
<details>
  <summary>Details</summary>
Motivation: The challenge of achieving high accuracy in automated structural defect segmentation while maintaining computational efficiency for real-time deployment.

Method: Combines depthwise separable convolutions with adaptive Kolmogorov-Arnold Network integration, featuring parameter reduction, adaptive TiKAN integration, and multi-scale attention fusion.

Result: Achieves 91% parameter reduction, 91% computational complexity reduction, 3x inference speed improvement, and state-of-the-art F1-score (0.771) and mean IoU (0.677).
...
Method: NeuraLeaf separates leaf geometry into 2D base shapes (learned from 2D datasets) and 3D deformations (modeled via skeleton-free skinning). A new dataset, DeformLeaf, supports training.

Result: NeuraLeaf generates diverse leaf shapes with deformation, fitting accurately to 3D observations (depth maps, point clouds).
...
Result: 36.1% increase in mAP$_{50:95}$ and 20.6% in mAP$_{50}$ on VisDrone2019-DET dataset.

Conclusion: SOD-YOLO is an efficient solution for small object detection in UAV imagery, with publicly available resources.
...
Method: Uses perceptual encryption and domain adaptation on Vision Transformer (ViT) embeddings.
...
<details>
  <summary>Details</summary>
Motivation: To synthesize current methodologies, dataset usage, evaluation metrics, and industrial applicability in transformer-based spatial grounding.

Method: Conducted a systematic literature review of transformer-based spatial grounding approaches.

Result: Identified dominant architectures, prevalent datasets, evaluation metrics, and best practices.
...
Method: Dual-branch architecture integrating dashcam videos and accident reports, with feature aggregation via large models (GPT-4o, Long-CLIP) and prompt engineering.
...
Conclusion: The framework sets a new benchmark for state-of-the-art traffic accident anticipation.
...
<details>
  <summary>Details</summary>
Motivation: Video-based hair transfer is challenging due to temporal consistency and spatial fidelity requirements. HairShifter aims to address these challenges.

Method: Uses an 'Anchor Frame + Animation' framework with an Image Hair Transfer (IHT) module and Multi-Scale Gated SPADE Decoder for seamless blending and coherence.
...
Conclusion: HairShifter sets a robust baseline for video-based hairstyle transfer, opening new avenues for future research.
...
<details>
  <summary>Details</summary>
Motivation: UMIS faces challenges due to multi-scale structural heterogeneity and lacks object-level anatomical insight. Conventional methods struggle with morphological complexity and feature conflicts.

Method: Proposes Mamba Snake, a deep snake framework with hierarchical state space atlas, Mamba Evolution Block (MEB) for spatiotemporal aggregation, energy map shape priors, and dual-classification synergy.

Result: Outperforms state-of-the-art methods by 3% Dice score across five clinical datasets.
...
<details>
  <summary>Details</summary>
Motivation: Enhancing human-computer interaction through immersive and empathetic engagement by improving emotional talking-head generation.

Method: Proposes the Think-Before-Draw framework with Chain-of-Thought (CoT) for semantic parsing and a progressive guidance denoising strategy for fine-grained expressiveness.

Result: Achieves state-of-the-art performance on benchmarks like MEAD and HDTF, with demonstrated zero-shot generation capability.

Conclusion: The framework successfully bridges the gap between abstract emotion labels and natural facial expressions, advancing emotional talking-head generation.
...
<details>
  <summary>Details</summary>
Motivation: To enhance autonomous driving safety by overcoming challenges like limited training data and missing object-level cues due to environmental or sensor issues.

Method: Uses a video generation pipeline for diverse driving scenarios and a dynamic prediction model with graph convolutions and temporal operators.

Result: Improves accuracy and lead time of accident anticipation, validated on public and new datasets.

Conclusion: The framework offers a robust solution for data and modeling limitations in autonomous driving safety.
...
Result: Achieves 81.9% tracking success rate at 100 Hz, with 82.9% target coverage during handoff, demonstrating robustness to occlusion and lighting variations.

Conclusion: The system is viable for extended marine tracking and scalable autonomous monitoring, with successful UAV coordination.
...
<details>
  <summary>Details</summary>
Motivation: The reliance on task-specific human demonstrations in VLA models limits generalization and incurs high data costs.
...
Conclusion: The AnyPos-ATARA pipeline enhances efficiency, scalability, and performance in task-agnostic manipulation tasks.
...
<details>
  <summary>Details</summary>
Motivation: The quadratic complexity of attention operations in Stable Diffusion makes generation slow. Existing token merging methods are not optimized for attention-based image generation.

Method: ReToM merges tokens within local windows, adjusts window sizes, and selects representative tokens based on similarity to preserve salient features.

Result: ReToM improves FID by 6.2% and CLIP scores, with comparable inference time to the baseline.
...
Method: Uses differentiable channel selection in MLP layers of transformer blocks, guided by a theoretical generalization bound, to prune channels generalization-aware.

Result: KCR-Transformer reduces FLOPs and parameters while improving or maintaining accuracy, outperforming original models like ViT and Swin.

Conclusion: KCR-Transformer is a theoretically grounded, efficient vision transformer that enhances performance and reduces computational costs.
...
<details>
  <summary>Details</summary>
Motivation: Existing LVLMs are limited to indoor tasks and struggle with outdoor large-scale scenes due to single-view, single-modal focus and lack of multidomain data.

Method: Develop SVM-City dataset (420k images, 4,811M point clouds, 567k QA pairs) and City-VLM, using joint probabilistic distribution for multimodal fusion.

Result: City-VLM outperforms existing LVLMs by 18.14% in QA tasks, showing strong generalization across outdoor scenes.

Conclusion: SVM-City and City-VLM effectively address outdoor scene understanding challenges, offering scalable and pragmatic solutions.
...
Method: DeQA-Doc leverages MLLMs' visual language capabilities and a soft label strategy, relaxes resolution constraints, and uses ensemble methods.

Result: DeQA-Doc significantly outperforms baselines, providing accurate and generalizable quality scores across degradation types.
...
<details>
  <summary>Details</summary>
Motivation: Precise synchronization between facial animations and audio signals is challenging. Existing methods struggle with noise and high computational costs.

Method: ATL-Diff uses a Landmark Generation Module, Landmarks-Guide Noise approach, and 3D Identity Diffusion network to enhance synchronization and preserve identity.

Result: Outperforms state-of-the-art methods on MEAD and CREMA-D datasets, achieving high-quality animations and computational efficiency.

Conclusion: ATL-Diff offers promising applications in virtual assistants, education, and digital platforms, with available source code.
...
<details>
  <summary>Details</summary>
Motivation: Long-tailed learning suffers from degraded performance in less frequent classes. Foundation models offer potential, but current methods neglect visual-textual alignment and inconsistent class distributions.

Method: Sage introduces an SG-Adapter to incorporate class descriptions as semantic guidance into visual fine-tuning and a distribution mismatch-aware compensation factor to rectify prediction bias.

Result: Experiments show Sage effectively improves performance in long-tailed learning by enhancing visual-textual alignment and addressing prediction bias.

Conclusion: Sage advances long-tailed learning by leveraging semantic guidance and compensating for distribution mismatch, demonstrating strong performance on benchmarks.
...
Conclusion: FIQ enhances VQA by improving fundamental understanding of videos, leading to better generalization and reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Existing CIR methods either lose information in sequential processing or fail to fully utilize visual context. MCoT-RE aims to address these limitations.
...
Result: Achieves state-of-the-art results, with improvements up to 6.24% in Recall@10 on FashionIQ and 8.58% in Recall@1 on CIRR.
...
<details>
  <summary>Details</summary>
Motivation: Existing fusion methods for CIR either focus too much on text (early fusion) or struggle with fine-grained alignments (late fusion). FAR-Net aims to address these limitations.
...
Result: Experiments on CIRR and FashionIQ show improvements, e.g., Recall@1 by 2.4% and Recall@50 by 1.04%, surpassing state-of-the-art methods.

Conclusion: FAR-Net provides a robust and scalable solution for CIR tasks by effectively balancing visual and textual fusion.
...
<details>
  <summary>Details</summary>
Motivation: Accurate food image classification is vital for applications, but CNNs struggle with fine-grained details.

Method: FE-TResNet integrates StyleRM and DCA into TResNet for better feature extraction.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of tracking small objects in UAV videos where detection and appearance-based association are unreliable.

Method: Leverages temporal information, introduces the SMOT4SB dataset, and proposes the SO-HOTA metric.

Result: A competitive challenge with 78 participants, where the winning method achieved a 5.1x improvement over baseline.

Conclusion: The work advances SMOT in UAV scenarios, with potential applications in bird strike avoidance, agriculture, fisheries, and ecological monitoring.
...
Method: ACM enhances existing foundation models without retraining, reusing original captions while incorporating user instructions and modality features. ACD provides diverse data, and AnyCapEval offers improved evaluation metrics.

Result: ACM significantly improves caption quality, boosting GPT-4o's content scores by 45% and style scores by 12%, and performs well on other benchmarks.

Conclusion: The AnyCap Project successfully addresses gaps in controllable captioning with ACM, ACD, and AnyCapEval, demonstrating notable improvements in caption quality and evaluation reliability.
...
<details>
  <summary>Details</summary>
Motivation: Image captioning aids in interpreting complex satellite imagery for applications like environmental monitoring and disaster assessment, necessitating advanced methods for RSIC.
...
Result: The best model surpasses state-of-the-art systems on most metrics, showing promise for real-world remote sensing applications.

Conclusion: The transformer-based approach effectively advances RSIC, demonstrating practical utility in remote sensing systems.
...
Conclusion: SRE effectively reduces domain shift and enhances generalization by aligning attention maps in CLIP.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods are limited to close-vocabulary prediction, restricting their use in diverse Earth observation scenarios. Open-vocabulary learning is needed for broader applicability.

Method: Proposes SCORE, which uses Region-Aware Integration and Global Context Adaptation to enhance visual and textual representations for better object distinguishability and adaptability.
...
TL;DR: WhoFi introduces a Wi-Fi signal-based pipeline for person re-identification, overcoming visual limitations with a Transformer-based DNN, achieving competitive results on the NTU-Fi dataset.


<details>
  <summary>Details</summary>
Motivation: Traditional visual methods for person re-identification struggle with poor lighting, occlusion, and suboptimal angles, prompting the need for alternative solutions.

Method: WhoFi extracts biometric features from Wi-Fi CSI, processes them through a Transformer-based DNN, and trains using an in-batch negative loss function.

Result: Experiments on the NTU-Fi dataset demonstrate competitive performance compared to state-of-the-art methods.

Conclusion: WhoFi proves effective for person re-identification using Wi-Fi signals, offering a robust alternative to visual-based approaches.
...
Method: Proposes HRSeg with HRP (high-resolution image processing) and HRE (fine-grained feature enhancement) modules.

Result: Validated by ablation studies and outperforms benchmarks.

Conclusion: HRSeg offers efficient, high-resolution segmentation with superior performance.
...
Method: Uses multi-channel bio-impedance sensing with soft, dry electrodes and a deep learning framework integrating anatomical priors.

Result: Achieves a mean per-vertex error of 25.9 mm, validated on 7 participants.
...
Method: Uses standard HD cameras to unobtrusively capture eye appearance and head movements, modeling gaze behavior's spatial, semantic, and temporal dimensions.
...
<details>
  <summary>Details</summary>
Motivation: Domain shifts in lane detection models require resource-intensive data annotation. Label-free performance estimation is underexplored but crucial for robustness.

Method: Adapts five image classification methods to lane detection and introduces LanePerf, integrating image and lane features with a pretrained encoder and DeepSets architecture.

Result: LanePerf achieves a lower MAE (0.117) and higher Spearman's correlation (0.727) on the OpenLane dataset, excelling in zero-lane and large domain-shift cases.

Conclusion: LanePerf enables efficient, label-free performance estimation for ADAS, enhancing testing and safety in varied driving conditions.
...
<details>
  <summary>Details</summary>
Motivation: To provide a privacy-preserving image classification dataset and improve Federated Learning performance with new algorithms.

Method: Introduced a dataset and proposed Fed-Cyclic (cyclic topology) and Fed-Star (star topology) algorithms.

Result: Both algorithms outperformed existing baselines on the new dataset.

Conclusion: The dataset and algorithms advance Federated Learning for image classification with privacy.
...
Method: Created AthleticsPose dataset with real motions from 23 athletes, trained a 3D pose model, and evaluated performance.
...
Conclusion: Authentic sports motion data is crucial for accurate pose estimation, but challenges remain for high-speed metrics. The dataset and tools are publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D point cloud methods suffer from information loss, especially with textureless planes or complex objects. Multi-view images can compensate for these gaps.
...
Conclusion: The framework effectively balances performance and avoids overfitting in landslide detection tasks.
...
<details>
  <summary>Details</summary>
Motivation: To reduce reliance on labeled cross-modal samples and address scenarios where cross-modal labels are unavailable.

Method: A heterogeneous expert collaborative consistency learning framework trains modality-specific experts and uses a cross-modal fusion mechanism to associate samples.

Result: Improved cross-modal identity recognition and modality-invariant feature extraction, validated on two datasets.

Conclusion: The method effectively addresses weakly supervised cross-modal ReID, enhancing performance without cross-modal labels.
...
<details>
  <summary>Details</summary>
Motivation: To understand and quantify uncertainties in multimodal inputs of large-scale MLLMs and explore their clinical applications.

Method: Proposes MUPM, a multimodal uncertainty propagation model, tested on cardiac MR scans and digital health records.

Result: MUPM is robust, generalizable across tasks, and efficient in estimating uncertainties, with practical clinical applications.

Conclusion: MUPM effectively quantifies and leverages uncertainties in multimodal data, offering valuable insights for clinical tasks.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of scaling diffusion transformers for long-duration video generation without compromising temporal coherence or scalability.

Method: Uses FlexFormer, an autoencoder with variable-length inputs and position-aware mechanisms, enabling flexible video and text compression.

Result: Validated effectiveness across diverse tasks, supporting prediction, retradiction, interpolation, and multi-shot generation.

Conclusion: LoViC offers a scalable and coherent solution for long-duration video generation, outperforming prior methods.
...
Method: Proposes cIDIR, a DIR framework based on INRs, trained over a prior distribution of hyperparameters and optimized using segmentation masks. Models a continuous DVF for advanced regularization.
...
<details>
  <summary>Details</summary>
Motivation: Prior methods using geometric priors often produce artifacts and fail to capture subtle emotions, while multi-character animation is hindered by feature interference.

Method: Uses a diffusion transformer with expression-augmented learning for identity-agnostic dynamics and masked cross-attention for multi-character control. Introduces Multi-Expr dataset and ExprBench.
...
Conclusion: FantasyPortrait advances expressive facial animation with improved fidelity and emotion capture, supported by new datasets and benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Wrist pathologies, especially in children, are common but hard to diagnose due to time constraints and lack of expertise. Limited datasets in medical imaging further complicate the use of computer vision.
...
<details>
  <summary>Details</summary>
Motivation: Spectral reconstruction (SR) is challenging due to unobservable spectral features in RGB images. The goal is to effectively model the spectral-spatial joint distribution to complement these missing features.

Method: The method extends RGB-LDM to an unobservable feature LDM (ULDM) via a two-stage pipeline: spectral structure representation learning (using SpeUAE) and spectral-spatial joint distribution learning (using ULDM).

Result: The proposed method achieves state-of-the-art performance in SR and downstream relighting tasks.

Conclusion: The approach successfully leverages RGB pre-trained models for spectral-spatial learning, improving SR by focusing on unobservable features in a compact latent space.
...
<details>
  <summary>Details</summary>
Motivation: High computational costs, latency, and memory demands of large models like Vision Transformers make deployment challenging, especially on resource-constrained hardware.

Method: The approach uses activation statistics to select neurons for pruning and integrates mean activations to preserve performance, requiring minimal fine-tuning.
...
Conclusion: Variance-Based Pruning offers an efficient, one-shot method to compress models with minimal retraining, maintaining high performance.
...
<details>
  <summary>Details</summary>
Motivation: Current sample selection methods either rely on offline oracle models (inefficient in cold-start) or online predictions (ignoring noisy correspondence), prompting the need for a better solution.
...
Result: Experiments on three benchmark datasets show DISSect outperforms state-of-the-art methods in training acceleration and performance.

Conclusion: DISSect effectively addresses noisy correspondence and cold-start issues, offering a superior alternative for sample selection in contrastive learning.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of acquiring high-quality pixel-level annotations for IML, the study explores scribble annotation as a more efficient weak supervision method to improve detection performance.
...
Conclusion: Scribble annotation supervision is a viable and efficient alternative for IML, achieving superior performance with less annotation effort.
...
<details>
  <summary>Details</summary>
Motivation: Most real-world scenarios don't need extensive visual tokens, yet current VLMs use them uniformly, wasting resources.
...
Result: Achieves strong performance on OCR tasks while saving tokens on simpler tasks, with a stable image resize call ratio.
...
TL;DR: MaskGIL improves MAR models for image generation, matching AR model performance with fewer steps, and extends to text-driven and real-time applications.


<details>
  <summary>Details</summary>
Motivation: MAR models underperform compared to AR models; this study aims to refine MAR architecture for better image generation.

Method: Evaluated image tokenizers, introduced bidirectional attention and 2D RoPE in a Bidirectional LLaMA architecture, scaled parameters, and developed text-driven variants.

Result: MaskGIL achieves FID 3.71 on ImageNet 256x256, matching AR models with only 8 inference steps, and supports text-to-image and speech-to-image tasks.
...
TL;DR: The paper introduces Hierarchical Coresets Selection (HCS) to enhance Vision-Language Models (VLMs) for complex wide-area scene understanding without fine-tuning.
...
Method: Proposes HCS, a plug-and-play mechanism that progressively refines region selection using a theoretically guaranteed importance function (utility, representativeness, robustness, synergy).

Result: HCS enables VLMs to rapidly understand unseen scenes at any scale with minimal interpretable regions, achieving superior performance and universality.

Conclusion: HCS effectively addresses VLM limitations in wide-area scene understanding, offering a versatile and efficient solution.
...
Conclusion: The proposed framework effectively enhances dataset distillation by ensuring label accuracy and intra-class diversity.
...
TL;DR: A novel cost-volume-based motion feature representation, Channel-wise Motion Features, is proposed for efficient motion segmentation in robotics, achieving 4x FPS and 25% parameters of state-of-the-art models.
...
Result: Achieves 4x faster FPS and reduces parameters to 25% while maintaining equivalent accuracy on KITTI and Cityscapes datasets.

Conclusion: The proposed method offers a highly efficient and accurate solution for real-time motion segmentation in robotics.
...
TL;DR: Decoupled PROB improves Open World Object Detection by resolving learning conflicts in PROB and enhancing feature extraction with novel modules.


<details>
  <summary>Details</summary>
Motivation: Addressing learning conflicts between objectness and class predictions in PROB and improving performance in OWOD tasks.
...
Conclusion: Decoupled PROB effectively resolves PROB's issues and advances OWOD performance with its innovative modules.
...
Conclusion: DiffOSeg effectively balances consensus and expert-specific preferences, offering a comprehensive solution for medical image segmentation.
...
TL;DR: GLAD (Generalizable LoRA tuning with RegulArized GraDient) simplifies prompt tuning for vision-language models, addressing overfitting in few-shot scenarios with gradient-based regularization, outperforming existing methods.
...
Result: GLAD outperforms state-of-the-art methods in base-to-novel class, image domain, and cross-dataset generalization across 15 benchmarks.

Conclusion: GLAD offers a simpler, more generalizable solution for prompt tuning, enhancing performance and robustness in few-shot learning.
...
TL;DR: An automated pipeline using deep learning (3D nnU-Net) for fetal lung segmentation and maturity assessment from diffusion-weighted MRI shows robust performance, matching manual segmentation results.
...
Method: A 3D nnU-Net model was trained on manually segmented images for lung segmentation, followed by voxel-wise model fitting to assess IVIM parameters.

Result: The segmentation achieved a mean Dice coefficient of 82.14%, with no differences in IVIM parameters between automated and manual methods.

Conclusion: The automated pipeline is viable for clinical fetal lung maturity assessment, supporting decision-making.
...
Method: Uses a mixture-of-experts framework with routing distillation, redundancy removal for layer-wise experts, and hierarchical local attention-guided inference.

Result: Achieves 87.8% reduction in forgetting rates and 63.3% fewer parameters, outperforming SOTA on CustomConcept 101 dataset.

Conclusion: R²MoE effectively balances learning new concepts with minimal parameter overhead, demonstrating superior performance in lifelong visual concept learning.
...
TL;DR: A registration-based anomaly detection framework for high-resolution 3D point clouds combines multi-prototype alignment and cluster-wise discrepancy analysis to precisely localize structural anomalies.


<details>
  <summary>Details</summary>
Motivation: High-resolution 3D point clouds are effective for industrial inspection but face challenges like computational cost and sensitivity to misalignment, necessitating a robust anomaly detection method.

Method: The framework registers test samples to multiple normal prototypes, uses clustering with keypoint-guided centroids, and computes local feature similarities for anomaly detection.

Result: The method achieves state-of-the-art performance on the Real3D-AD benchmark for both object-level and point-level anomaly detection.

Conclusion: The proposed framework effectively addresses challenges in 3D point cloud anomaly detection, offering precise localization and superior performance.
...
TL;DR: A novel multimodal IRSTD framework integrates language priors with image data to improve small target detection in infrared images, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing IRSTD methods rely solely on image data, missing the potential of multimodal approaches. This work leverages language priors to enhance detection accuracy.

Method: The framework uses GPT-4 to generate text descriptions of target locations, combining textual and image data. A new multimodal dataset is curated for validation.

Result: Significant improvements are shown over existing methods, with notable gains in IoU, nIoU, Pd, and Fa metrics on benchmark datasets.

Conclusion: Incorporating language priors into IRSTD via multimodal fusion is effective, as demonstrated by the proposed framework and dataset.
...
<details>
  <summary>Details</summary>
Motivation: Tiny objects in RS imagery are hard to detect due to limited spatial info, weak features, and dense distributions. Existing detectors underperform.
...
Result: Achieves 4.0% AP and 6.5% AP75 improvements on AI-TOD and superior performance on DIOR benchmark.
...
TL;DR: DINO-VO integrates DINOv2's visual foundation model with a salient keypoints detector and fine-grained geometric features to improve monocular visual odometry, achieving robustness, accuracy, and efficiency.


<details>
  <summary>Details</summary>
Motivation: Addressing robustness, generalization, and efficiency challenges in learning-based monocular visual odometry by leveraging DINOv2's capabilities while overcoming its coarse feature granularity.

Method: Proposes DINO-VO, combining DINOv2's features with a tailored keypoints detector, fine-grained geometric features, a transformer-based matcher, and differentiable pose estimation.
...
Conclusion: SE-VLN demonstrates significant performance improvements and scalability with experience, showcasing its potential as a self-evolving VLN framework.
...
TL;DR: A model for autonomous driving achieves state-of-the-art performance with minimal design and data, excelling in challenging scenarios. A comparison favors continuous autoregressive models over discrete token models.
...
Method: Develops a model with simple design, no extra supervision/sensors, and compares discrete vs. continuous token models using a hybrid tokenizer.

Result: The model performs exceptionally in tough scenarios (e.g., turns, urban traffic) and favors continuous autoregressive models for robustness.
...
TL;DR: A novel method using Midjourney to generate synthetic images for construction worker detection addresses DNN training data scarcity, showing promising results but highlighting limitations.
...
Result: Achieved APs of 0.937 and 0.642 on real images, and 0.994 and 0.919 on synthetic images at IoU thresholds of 0.5 and 0.5-0.95.

Conclusion: Generative AI shows potential but also weaknesses in addressing DNN data scarcity, with better performance on synthetic data.
...
TL;DR: The paper proposes a novel method for detecting AI-generated videos using pre-trained visual models, achieving over 90% accuracy on a diverse dataset.


<details>
  <summary>Details</summary>
Motivation: The rise of AI-generated videos beyond deepfakes necessitates robust detection methods to combat misinformation, privacy breaches, and security threats.

Method: Leverages features from pre-trained visual models and adds a linear classification layer for detection, avoiding additional model training.

Result: Achieves over 90% detection accuracy on the VID-AID dataset, which includes 10,000 AI-generated and 4,000 real videos.
...
TL;DR: $S^2M^2$ introduces a global matching architecture for stereo matching, balancing accuracy and efficiency without fine-tuning, outperforming benchmarks.


<details>
  <summary>Details</summary>
Motivation: The trade-off between local search methods (limited global consistency) and global matching (historically impractical due to computational costs) motivates a new solution.

Method: $S^2M^2$ uses a multi-resolution transformer for long-range correspondence and a novel loss function to focus on feasible matches, jointly estimating disparity, occlusion, and confidence.

Result: Achieves state-of-the-art accuracy on Middlebury v3 and ETH3D benchmarks, with high efficiency and detailed reconstruction.

Conclusion: $S^2M^2$ resolves the trade-off, offering robust generalization and superior performance in stereo matching.
...
TL;DR: VITA introduces a Vision-To-Action flow matching policy that directly maps latent visual representations to actions, eliminating conditioning modules and reducing overheads. It outperforms state-of-the-art methods in bi-manual manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional flow matching policies require additional conditioning mechanisms for action generation, creating inefficiencies. VITA aims to simplify this by learning a direct vision-to-action mapping.

Method: VITA uses a structured action latent space via an autoencoder, up-sampling actions to match visual shapes. It supervises flow matching with encoder targets and action outputs for end-to-end learning.

Result: VITA outperforms or matches state-of-the-art generative policies, reducing inference latency by 50-130% and solving complex bi-manual tasks.

Conclusion: VITA is the first MLP-only flow matching policy capable of handling complex bi-manual manipulation tasks efficiently.
...
TL;DR: The paper proposes an Approximately Orthogonal Fine-Tuning (AOFT) strategy for Vision Transformers (ViTs) to enhance generalization by aligning the properties of low-rank adaptation matrices with the backbone's orthogonality.
...
Conclusion: AOFT effectively enhances the generalization capability of fine-tuned ViTs by incorporating orthogonality into low-rank adaptation matrices.
...
<details>
  <summary>Details</summary>
Motivation: Protect underage users from accessing age-restricted online services by addressing the challenge of makeup altering perceived age.

Method: Proposes DiffClean, a text-guided diffusion model to remove makeup traces from images.

Result: Improves age estimation accuracy by 4.8% and face verification (TMR by 8.9% at FMR=0.01%) over baselines.
...
TL;DR: FashionPose is a text-to-pose-to-relighting framework for realistic garment visualization, enabling pose and lighting control via text input.
...
Method: FashionPose uses a text-driven approach: predicts 2D poses, generates images via diffusion, and applies a relighting module, all guided by text.

Result: The framework achieves fine-grained pose synthesis and consistent relighting, improving garment visualization.

Conclusion: FashionPose offers a practical solution for personalized virtual fashion display with enhanced flexibility and control.
...
TL;DR: The paper identifies reproducibility and quality issues in the RPE benchmark for pose-aware MLLMs, proposes refined GT annotations to improve consistency, and releases them as an open-source resource.


<details>
  <summary>Details</summary>
Motivation: The RPE benchmark's flaws, such as mismatched image indices and quality limitations, hinder fair and consistent evaluations of pose-aware MLLMs.

Method: The authors refined ground-truth annotations through visual matching and released them publicly.

Result: The refined annotations address issues like image redundancy, scenario imbalance, and ambiguous descriptions, improving benchmark reliability.

Conclusion: The release of refined annotations aims to enhance reproducibility and support future advancements in pose-aware multimodal reasoning.
...
Method: A cascaded architecture combines an action recognition module (Mamba model with EfficientNetV2 backbone) and an object detection module (fine-tuned YOLOWorld). The action module triggers the object module upon detecting interaction.

Result: The method achieves 38.52% p-AP for action recognition and 85.13% AP for object detection on the ENIGMA-51 benchmark at 30fps.

Conclusion: The proposed approach efficiently detects hand-object interactions in real-time, balancing speed and accuracy.
...
TL;DR: Optimizations for real-time video generation on mobile devices using compressed VAE, pruning, and adversarial step distillation.
...
Result: Achieves over 10 FPS on iPhone 16 Pro Max.

Conclusion: Demonstrates feasibility of real-time, high-quality video generation on mobile devices.
...
Method: Proposes a latent grid encoding images, poses, and viewpoints, denoised iteratively along spatial and temporal dimensions with a sliding window.
...
Conclusion: The sliding iterative denoising process effectively improves 4D consistency and reduces GPU memory usage, advancing view synthesis quality.
...
TL;DR: The paper introduces IMBA loss, an online method to improve stability in visual generation tasks by addressing poor concept responses, achieving competitive results with minimal code changes.


<details>
  <summary>Details</summary>
Motivation: Poor stability and error-prone combinations of complex concepts in visual generation tasks are under-explored, prompting the need for causal analysis and solutions.

Method: The authors design a concept-wise equalization loss function (IMBA loss) and validate it through experiments on Inert-CompBench and public test sets.

Result: The method significantly enhances concept response capability in baseline models and achieves competitive performance with minimal code adjustments.

Conclusion: The proposed IMBA loss effectively addresses concept response instability in visual generation, offering a practical and efficient solution.
...
TL;DR: AutoPartGen is an autoregressive model for generating 3D objects from parts, using inputs like images, masks, or existing 3D objects. It leverages a latent 3D representation for part-based generation and achieves state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: To enable compositional 3D reconstruction from diverse inputs (images, masks, or 3D objects) by leveraging the strong compositional properties of a latent 3D representation.

Method: AutoPartGen generates parts autoregressively, conditioning on prior parts and inputs (images, masks, or 3D objects), stopping when all parts are generated. No additional optimization is needed for assembly.

Result: The model achieves state-of-the-art performance in 3D part generation, producing coherent objects without extra optimization.

Conclusion: AutoPartGen effectively generates 3D objects from parts autoregressively, demonstrating superior part-level and overall 3D generation capabilities.
...
<details>
  <summary>Details</summary>
Motivation: To address the fixed complexity of distributions in hierarchical flow matching, the study explores mini-batch couplings for gradual complexity adjustment.

Method: Uses mini-batch couplings within hierarchical rectified flow matching to modify distribution complexity across ODE hierarchy levels.

Result: Demonstrates improved performance on synthetic and imaging data, showcasing the benefits of mini-batch couplings.

Conclusion: Mini-batch couplings effectively adjust distribution complexity in hierarchical flow matching, enhancing generative modeling capabilities.
...
TL;DR: VideoITG improves Video-LLMs by aligning frame sampling with user instructions via the VidThinker pipeline, outperforming unsupervised methods.


<details>
  <summary>Details</summary>
Motivation: Current unsupervised methods struggle with complex long video understanding, necessitating a more guided approach.

Method: Proposes VideoITG with VidThinker pipeline: clip-level captioning, instruction-guided segment retrieval, and fine-grained frame selection.

Result: VideoITG-40K dataset created; model improves Video-LLMs' performance on benchmarks.

Conclusion: VideoITG demonstrates superior video understanding potential through instruction-aligned frame selection.
...
<details>
  <summary>Details</summary>
Motivation: Current AI tutoring systems are reactive and lack deep reflection or pedagogical tools, especially in mathematics. The research aims to improve these systems.

Method: The platform combines adaptive feedback, structured course generation, and textbook knowledge retrieval for modular, tool-assisted learning.

Result: The system enables personalized learning, effective exam revision, and unlimited practice exercises, targeting student weaknesses.
...
<details>
  <summary>Details</summary>
Motivation: To improve simulation of highway merging by addressing limitations in existing tactical decision models, such as limited action sets or overly complex payoff functions.

Method: A game-theoretic model for tactical decision-making with enhanced payoff functions and lag actions, coupled with a dynamics model for unified decision and dynamics simulation.

Result: The model successfully reproduced complex merging interactions in validation tests and demonstrated computational efficiency for large-scale simulations.

Conclusion: The proposed model provides a realistic, explainable, and efficient solution for simulating highway merging scenarios in autonomous vehicle development.
...
TL;DR: This paper reviews eXplainable Reinforcement Learning (XRL), proposing a taxonomy based on 'What' and 'How' questions to categorize methods and surveying over 250 papers. It also highlights related domains and identifies future needs for XRL.


<details>
  <summary>Details</summary>
Motivation: The opacity of AI models, especially deep neural networks, necessitates explainability. XRL addresses this for reinforcement learning agents.

Method: Proposes a taxonomy ('What' and 'How') to categorize XRL methods and reviews 250+ papers.

Result: A comprehensive survey of XRL methods, highlighting gaps and related domains.

Conclusion: Identifies future needs for XRL and suggests areas for community focus.
...
<details>
  <summary>Details</summary>
Motivation: Modern generative systems struggle to translate static rules into dynamic player behavior, creating a need for tools that bridge this gap.

Method: The framework uses RL agents to playtest games and LMMs to analyze play traces (metrics/image strips) and revise game configurations iteratively.

Result: LMMs effectively reason over RL-supplied behavioral traces to refine game mechanics, demonstrating scalable AI-assisted design tools.

Conclusion: The approach offers practical, scalable solutions for AI-assisted game design by leveraging RL and LMM collaboration.
...
<details>
  <summary>Details</summary>
Motivation: To determine how effective deception probes are in detecting deceptive AI responses and whether they can resist evasion strategies.

Method: Comparison of white-box (with token-level probe access) and black-box (without) monitoring, measuring the black-to-white performance boost.

Result: Weak but encouraging performance boosts from existing deception probes.

Conclusion: Deception probes show promise but need improvement for practical use.
...
TL;DR: The study explores using an AI Agent as a peer learning companion for English composition, targeting learners of similar proficiency to enhance effectiveness.


<details>
  <summary>Details</summary>
Motivation: Peer learning is effective but limited by human constraints; AI can provide scalable, anytime-anywhere peer learning with matched proficiency.

Method: Focuses on English composition, assuming peers at the same proficiency level make similar mistakes, and validates this approach.

Result: Not explicitly stated in the abstract, but the goal is to confirm the effectiveness of AI as a peer learning companion.

Conclusion: AI could overcome human peer learning limitations by providing tailored, proficiency-matched companionship for tasks like English composition.
...
Method: MCPEval uses a Model Context Protocol (MCP) to automate task generation and evaluation, standardizing metrics and integrating with native tools.

Result: Empirical tests in five domains demonstrate MCPEval's effectiveness in revealing domain-specific performance nuances.

Conclusion: MCPEval provides a reproducible, standardized solution for LLM agent evaluation and is publicly available.
...
Conclusion: Future work will focus on enhancing emotional understanding and response personalization for more practical ESC systems.
...
TL;DR: The paper advocates for a new evaluation framework for AI world models, inspired by human adaptability, focusing on dynamic learning in novel environments through 'novel games.'
...
Result: Introduces key desiderata for game design and metrics to assess AI's ability for rapid adaptation and robust generalization.

Conclusion: The framework aims to advance AI towards human-like adaptability, a critical step for artificial general intelligence.
...
TL;DR: The paper proposes moving human judgement outside AI-driven simulations by having humans design ethical metrics, allowing the system to explore scenarios and present select options for final human decision-making.


<details>
  <summary>Details</summary>
Motivation: Human involvement in every ethical decision during large-scale simulations is impractical. The paper aims to streamline this by offloading dynamic ethical weighting to the system.

Method: Humans design ethical metrics; the system dynamically weights ethical attributes during simulations using multi-criteria decision-making approaches, including entropy-based methods.

Result: The approach enables efficient exploration of ethical scenarios, reducing human workload while retaining final decision authority.

Conclusion: Automating ethical weighting in simulations balances efficiency with human oversight, leveraging existing decision-making literature.
...
<details>
  <summary>Details</summary>
Motivation: The growing threat of AI systems manipulating human behavior, especially in cybersecurity contexts, lacks systematic assessment and mitigation frameworks.

Method: The paper introduces a safety case framework structured around three arguments: inability, control, and trustworthiness, with evidence requirements and evaluation methodologies.

Result: A systematic methodology for integrating manipulation risk into AI safety governance is presented, offering practical tools for AI companies.
...
TL;DR: The paper introduces VAR-MATH, a symbolic evaluation framework to assess true reasoning in LLMs, revealing performance drops in RL-trained models on variabilized benchmarks, indicating reliance on superficial heuristics.


<details>
  <summary>Details</summary>
Motivation: To determine if improvements in LLMs' mathematical reasoning from RL training reflect genuine reasoning or benchmark overfitting, addressing issues like benchmark contamination and evaluation fragility.

Method: Developed VAR-MATH, a symbolic framework converting fixed numerical problems into symbolic templates, requiring models to solve multiple instantiations for consistent reasoning. Applied to benchmarks AMC23 and AIME24.

Result: RL-trained models showed significant performance drops on variabilized versions (48.0% on AMC23, 58.3% on AIME24), especially smaller models, indicating reliance on heuristics.

Conclusion: VAR-MATH provides a robust, contamination-resistant evaluation paradigm, revealing that many RL methods lack true generalization in mathematical reasoning.
...
TL;DR: The paper bridges the gap in Probabilistic Event Calculus (PEC) by translating PEC domains into Markov Decision Processes (MDPs), enabling goal-directed reasoning while preserving interpretability.


<details>
  <summary>Details</summary>
Motivation: PEC lacks mechanisms for goal-directed reasoning, limiting its applicability in objective-driven planning.
...
Result: The PEC-MDP formalism allows leveraging MDP algorithms for PEC domains, supporting both temporal reasoning and planning.

Conclusion: The translation extends PEC's capabilities while maintaining interpretability, demonstrated through human-readable policy mappings.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the need for trustworthy AI by developing contrastive explanations for MILP solutions.

Method: Encodes user queries as constraints, computes IIS to identify reasons, and represents explanations as a "graph of reasons."
...
Method: Utilized 30-second interval traffic data from California Highway 78, analyzed with MLR and RF algorithms, testing intervals from 30 seconds to 15 minutes.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations of static ProbTree (error propagation, inefficiency) by enabling dynamic adaptation and selective resource use.

Method: Uses reinforcement learning to incrementally build reasoning trees, optimizing actions (decomposition, retrieval, aggregation) based on real-time confidence.

Result: Improves solution quality and computational efficiency over static ProbTree.

Conclusion: Introduces a flexible, efficient paradigm for tree-structured reasoning in question answering.
...
<details>
  <summary>Details</summary>
Motivation: Traditional ethical criteria for AMAs are outdated because they assume transparent architectures, which LLMs lack.

Method: The paper introduces ten revised functional criteria for evaluating LLM-based AMAs, illustrated with hypothetical scenarios.

Result: A set of ten criteria (e.g., moral concordance, trustworthiness) is proposed to better align LLM-based AMAs with societal needs.

Conclusion: The revised criteria aim to improve the alignment and societal integration of LLM-based AMAs, demonstrated through practical examples.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of reasoning with abstract functions and predicates in decision-making tasks where exact matches are rare.

Method: Integration of higher-order patterns and fuzzy equivalences using similarity relations based on minimum T-norm.

Result: The algorithm computes a most general unifier with the highest degree of approximation for unifiable terms.

Conclusion: The approach successfully unifies higher-order patterns with fuzzy logic, providing a computationally efficient solution.
...
TL;DR: The paper introduces GEA, an arena for evaluating LLMs with energy consumption data, showing users prefer energy-efficient models when aware of their consumption.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation methods lack human correlation or scalability. Energy consumption's role in model selection is understudied.

Method: GEA integrates energy consumption data into public arena evaluations, allowing users to rank models based on energy awareness.

Result: Users favor smaller, energy-efficient models when aware of consumption, suggesting top models' extra cost isn't justified by perceived quality.

Conclusion: Energy awareness influences model preference, advocating for efficiency in LLM evaluations and usage.
...
TL;DR: The paper introduces FormulaOne, a benchmark to test frontier AI models on real-life research problems in graph theory, logic, and algorithms, revealing their limitations compared to human expertise.
...
Method: Constructed FormulaOne, a benchmark derived from Monadic Second-Order (MSO) logic on graphs, featuring highly demanding problems with commercial and theoretical relevance.
...
Conclusion: Frontier AI models still fall short of expert-level understanding in complex domains, and FormulaOne serves as a tool for further research.
...
TL;DR: The paper explores how people use distributed and symbolic representations to create tailored mental models for novel situations, proposing a Model Synthesis Architecture (MSA) that combines language models and probabilistic programs. MSA outperforms language model-only baselines in capturing human reasoning.


<details>
  <summary>Details</summary>
Motivation: To understand how people draw on diverse background knowledge for coherent reasoning in novel situations and to replicate this ability computationally.

Method: Proposes a Model Synthesis Architecture (MSA) using language models for relevance-based retrieval and probabilistic programs for coherent world models. Evaluated on a novel reasoning dataset (Model Olympics).

Result: MSA captures human judgments better than language model-only baselines, demonstrating coherent reasoning over globally relevant variables.

Conclusion: MSA offers a viable approach to replicating human-like reasoning in open-ended domains, bridging distributed and symbolic representations.
...
TL;DR: The paper identifies linear representations (modal difference vectors) in LMs that distinguish modal categories, revealing better categorization abilities than previously thought and correlating with human behavior.


<details>
  <summary>Details</summary>
Motivation: Recent studies question LMs' ability to categorize sentences by modality, prompting investigation into their true capabilities.

Method: The study identifies modal difference vectors in LMs and analyzes their emergence and correlation with human categorization.

Result: LMs show reliable modal categorization, with vectors emerging consistently during training and correlating with human judgments.

Conclusion: The findings offer insights into LM modal categorization and potential parallels to human cognition.
...
TL;DR: First open-source model for Chechen-Russian translation, fine-tuned on NLLB-200, with released datasets and encoder.


<details>
  <summary>Details</summary>
Motivation: Address the lack of resources for the vulnerable Chechen language by creating a translation model and dataset.

Method: Fine-tune the multilingual NLLB-200 model for Chechen-Russian translation, using collected parallel corpora.

Result: Achieved BLEU/ChrF++ scores of 8.34/34.69 (Russian-Chechen) and 20.89/44.55 (Chechen-Russian).

Conclusion: Successful creation of a Chechen-Russian translation model with released datasets, aiding language preservation.
...
TL;DR: NLP models, especially fine-tuned BioClinicalBERT, excel in classifying drug-related deaths from free-text coroner reports, outperforming traditional methods and other NLP models.


<details>
  <summary>Details</summary>
Motivation: The rising rate of drug-related deaths, particularly from fentanyl, necessitates accurate and timely surveillance, which is hindered by manual ICD-10 coding of free-text coroner reports.
...
Result: BioClinicalBERT achieved near-perfect performance (macro F1 >=0.998 internally, 0.966 externally), surpassing other models.

Conclusion: Fine-tuned NLP models like BioClinicalBERT provide a scalable, accurate solution for overdose surveillance, enabling near real-time detection of substance use trends.
...
TL;DR: AdaptiSent is a novel framework for Multimodal Aspect-Based Sentiment Analysis (MABSA) that uses adaptive cross-modal attention to improve sentiment classification and aspect term extraction from text and images. It outperforms existing models in precision, recall, and F1 score.


<details>
  <summary>Details</summary>
Motivation: To enhance sentiment and aspect-related information extraction by leveraging dynamic modality weighting and context-adaptive attention, focusing on interactions between textual cues and visual context.

Method: Integrates adaptive cross-modal attention mechanisms, dynamic modality weighting, and context-adaptive attention to analyze text and image data.

Result: Outperforms baselines in precision, recall, and F1 score, excelling in identifying nuanced inter-modal relationships.

Conclusion: AdaptiSent sets a new standard for MABSA, significantly improving accuracy in understanding complex multimodal data.
...
<details>
  <summary>Details</summary>
Motivation: Current speech evaluation methods are limited by the need for specialized systems and poor correlation with human preferences. This work aims to provide a unified solution using LAMs.

Method: AudioJudge employs prompt engineering, audio concatenation, and in-context learning. A multi-aspect ensemble approach decomposes speech assessment into specialized judges for lexical content, speech quality, and paralinguistic features.

Result: AudioJudge achieves up to 0.91 Spearman correlation with human preferences. It performs well under acoustic noise but shows biases like verbosity and positional bias.
...
TL;DR: FLEXITOKENS improves LM adaptability by using learnable tokenizers to reduce over-fragmentation, outperforming subword and gradient-based tokenizers.
...
Method: Develops byte-level LMs with learnable tokenizers (FLEXITOKENS) that predict boundaries in byte sequences, avoiding fixed compression rates.
...
Result: Matches or surpasses MT-Ranker, correlates well with human ratings, and identifies position bias.

Conclusion: TransEvalnia is effective, human-aligned, and addresses evaluation challenges, with released data and code.
...
TL;DR: ThinkLogit and ThinkLogit-DPO improve reasoning in large models without extra training, achieving 26% and 29% better performance, respectively.


<details>
  <summary>Details</summary>
Motivation: To unlock long reasoning abilities in large models without additional training.

Method: Proposes ThinkLogit (logits arithmetic) and ThinkLogit-DPO (preference optimization) using a smaller guiding model.

Result: 26% and 29% relative improvement in pass@1 on mathematical datasets.

Conclusion: ThinkLogit efficiently enhances long reasoning in large models with minimal training.
...
TL;DR: Synergy is a language model that bridges abstraction levels via learned routing, outperforming BBPE and showing position-independent concept emergence.


<details>
  <summary>Details</summary>
Motivation: To explore tokenizer-free architectures by bridging abstraction levels in language models.

Method: Trained as a byte-level language model with a learned routing mechanism, compared to BBPE and Llama3.

Result: Synergy outperforms BBPE and Llama3, with position-independent concepts emerging in higher abstraction layers.

Conclusion: Tokenizer-free architectures are feasible, enabling more robust and flexible pipelines.
...
TL;DR: Improving negation robustness in text encoders by distilling data from large language models using diverse negation patterns, enhancing performance while maintaining general benchmark competitiveness.


<details>
  <summary>Details</summary>
Motivation: Smaller text encoders struggle with negation, impacting downstream tasks. Enhancing their negation understanding is crucial for better text embeddings.

Method: Distill data from large language models using diverse negation and hedging patterns, then fine-tune a BERT-based model with contrastive learning.

Result: Significant improvement in negation understanding while maintaining competitive performance on general benchmarks; adaptable to LLMs for further gains.

Conclusion: The proposed method effectively enhances negation robustness in text encoders and can be extended to large language models, improving overall semantic understanding.
...
Method: Generate symbolic music data from textual prompts, create a dataset of LLM-generated MIDI files, and train neural networks for tasks like genre classification and melody completion.

Result: LLMs can infer rudimentary musical structures and temporal relationships from text, but their performance is limited without explicit musical training.

Conclusion: LLMs show promise for symbolic music generation but require further refinement to overcome limitations in explicit musical context.
...
<details>
  <summary>Details</summary>
Motivation: To assess and improve cross-lingual consistency in multilingual models, ensuring factual accuracy and performance parity across languages.
...
Conclusion: Cross-lingual alignment supervision and code-switching training are key for enhancing multilingual performance and consistency.
...
TL;DR: Proposes a hierarchical decoder architecture for decoder-only language models, adapting pretrained models by copying and fine-tuning language heads to intermediate layers, achieving state-of-the-art results.
...
Method: Adapts pretrained models by copying language heads to intermediate layers and fine-tuning them with task-specific inputs.

Result: Validates that intermediate layers can generate meaningful content, achieving state-of-the-art performance on tasks like hierarchical text classification and generation.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of answering questions about tables in Spanish, leveraging LLMs for code generation and table processing.

Method: The approach involves analyzing tables, selecting columns, generating natural language instructions, translating them to code, and handling errors using open-source LLMs and optimized prompts.

Result: Achieved an accuracy score of 85% in the IberLEF 2025 Task PRESTA.

Conclusion: The method effectively combines LLMs and code generation for table-based question answering in Spanish.
...
TL;DR: The paper proposes a formal UML-based model for describing attack scenarios and contexts, aiding cybersecurity automation for attack analysis and script generation in training.


<details>
  <summary>Details</summary>
Motivation: Organizations need automation in cybersecurity due to evolving threats, but formalizing input data for processes like attack simulation and analysis is a challenge.

Method: The research introduces a novel formal model using UML class diagrams to abstractly describe attack contexts and scenarios.

Result: The model supports upstream attack analysis and automatic generation of attack scripts for cybersecurity training.

Conclusion: The formal model enhances cybersecurity automation by providing structured input for attack-related processes, demonstrated through two use cases.
...
<details>
  <summary>Details</summary>
Motivation: To improve semantic understanding of scientific texts by leveraging summaries and contrastive learning, addressing limitations of citation-based methods.

Method: Uses LLM-generated summaries and contrastive learning to train embeddings, ensuring semantically related texts are closer in embedding space.

Result: Achieves state-of-the-art performance on SciRepEval benchmark, demonstrating stronger semantic separation.
...
TL;DR: A Ph.D. proposal to develop an NLP framework for identifying Self-aspects in text, including ontology creation, dataset annotation, and model evaluation.


<details>
  <summary>Details</summary>
Motivation: The Self is a complex construct reflected in language but underexplored in NLP, with potential applications in mental health and phenomenology.

Method: Develop an ontology and annotated dataset, then evaluate discriminative models, generative LLMs, and embedding-based retrieval against interpretability, accuracy, etc.

Result: Top models will be applied in mental health and phenomenology case studies.

Conclusion: The framework aims to systematically analyze Self-aspects in NLP, bridging gaps in interdisciplinary research.
...
TL;DR: The study examines how annotator demographics and text content influence labeling in sexism detection, finding content dominates. It also evaluates Generative AI's reliability as annotators, noting persona prompting often fails or worsens performance. XAI shows models focus on content-specific tokens, suggesting content-driven fairness approaches are more reliable.
...
Result: Demographic factors account for 8% of variance, with content being dominant. Persona prompting in GenAI often fails or degrades performance. XAI reveals models rely on content-specific tokens.

Conclusion: Content-driven explanations and robust annotation protocols are more reliable for fairness than persona simulation in NLP systems.
...
TL;DR: The study analyzes oral narratives of children needing intervention, using machine learning to identify predictors like lexical diversity and utterance length, while noting language-specific and shared features.
...
Result: Lexical diversity and utterance length are strong predictors; specific verbs and auxiliaries linked to storytelling reduce intervention likelihood. Language-specific and shared predictors were found.

Conclusion: The study highlights key narrative features for early assessment in multilingual contexts, with implications for tailored interventions.
...
TL;DR: GEMMAS is a graph-based framework evaluating multi-agent collaboration by analyzing communication efficiency and coordination, using metrics like IDS and UPR to reveal process-level inefficiencies.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations focus only on final output correctness, ignoring inefficiencies in communication and coordination, which lead to redundant reasoning and higher costs.
...
Result: On GSM8K, systems with minor accuracy differences showed significant variations in IDS (12.8%) and UPR (80%), highlighting process-level inefficiencies.

Conclusion: Outcome-only metrics are inadequate; process-level diagnostics like GEMMAS are crucial for designing efficient and interpretable collaborative AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Early childhood narrative and comprehension skills are crucial for literacy, but teachers struggle to identify students needing help in large classrooms.
...
Result: The LLM-based system outperforms the linear model and matches human experts in flagging children for intervention.

Conclusion: The system supports teachers by automating assessments, enabling more personalized learning support.
...
TL;DR: The paper introduces CAST, a framework for cross-task transfer in LLMs by steering latent space activations, improving robustness and efficiency without parameter updates.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with unseen tasks in data-scarce scenarios, and existing cross-task transfer methods face robustness and scalability issues.

Method: Analyzes activation patterns in LLMs, proposes CAST to manipulate internal activations using contrastive representations from high-resource tasks.

Result: CAST outperforms baselines in cross-domain and cross-lingual settings, showing scalability and lower computational costs.

Conclusion: Latent space steering enables efficient cross-task transfer, offering a promising direction for LLM adaptation.
...
Result: Models perform best with English prompts, and the proposed Chain of Thought method improves Hindi analogy performance.

Conclusion: HATS fills a critical gap in evaluating LLM reasoning in Hindi, showing English prompts are most effective, and the proposed method enhances performance.
...
TL;DR: AutoSteer is a modular, inference-time intervention technology for improving the safety of Multimodal Large Language Models (MLLMs) without fine-tuning, reducing adversarial attack success rates while preserving model performance.


<details>
  <summary>Details</summary>
Motivation: Addressing safety concerns in MLLMs due to adversarial multimodal inputs, aiming to enhance model robustness during inference.

Method: AutoSteer uses a Safety Awareness Score (SAS), an adaptive safety prober, and a lightweight Refusal Head to detect and mitigate safety risks in real-time.

Result: Significantly reduces Attack Success Rate (ASR) for textual, visual, and cross-modal threats on benchmarks like LLaVA-OV and Chameleon, without compromising general abilities.

Conclusion: AutoSteer is a practical and effective framework for safer deployment of multimodal AI systems, offering interpretability and adaptability.
...
<details>
  <summary>Details</summary>
Motivation: Recent studies question RL's effectiveness in improving multi-step reasoning, especially on hard problems, prompting the need for better training strategies.

Method: QuestA introduces partial solutions during RL training to reduce problem difficulty and provide more informative learning signals.

Result: Achieves SOTA results on math benchmarks: 67.1% on AIME24, 59.5% on AIME25, and 35.5% on HMMT25, with significant improvements over baselines.

Conclusion: QuestA enhances reasoning capabilities and sample efficiency, offering a practical and generalizable approach for RL in LLMs.
...
<details>
  <summary>Details</summary>
Motivation: The growing use of language technologies in workforce management lacks reliable, fair models and public benchmarks, which TalentCLEF aims to provide.

Method: The campaign includes two tasks: multilingual job title matching and job title-based skill prediction, using real-world anonymized data. Evaluations cover monolingual, cross-lingual scenarios, and gender bias.

Result: 76 teams participated, with systems using multilingual encoder models and large language models. Training strategies proved more impactful than model size alone.

Conclusion: TalentCLEF establishes a public benchmark, promoting robust, fair, and transferable language technologies for the labor market.
...
TL;DR: RCPS framework improves automated presentation generation with deep narrative planning, adaptive layouts, and iterative optimization, outperforming baselines. PREVAL evaluation aligns with human judgments.


<details>
  <summary>Details</summary>
Motivation: Existing methods produce presentations with inconsistencies and poor layouts, failing professional standards.

Method: RCPS integrates deep structured narrative planning, adaptive layout generation, and iterative optimization. PREVAL evaluates quality via preference-based, rationale-enhanced models.

Result: RCPS outperforms baselines in quality dimensions. PREVAL correlates strongly with human judgments.
...
TL;DR: AbGen is a benchmark for evaluating LLMs in designing ablation studies for NLP research, revealing a performance gap between models and humans, and highlighting unreliable automated evaluations.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' ability to design ablation studies for scientific research, given the lack of benchmarks for this task.

Method: AbGen includes 1,500 expert-annotated examples from 807 NLP papers, testing LLMs on generating ablation study designs. Leading models like DeepSeek-R1-0528 and o4-mini are evaluated.

Result: LLMs lag behind human experts in importance, faithfulness, and soundness of designs. Automated evaluations are unreliable compared to human assessment.

Conclusion: AbGen-Eval is introduced to assess automated evaluation reliability, providing insights for future LLM-based evaluation systems in complex scientific tasks.
...
TL;DR: The paper introduces HapticCap, a dataset for matching haptic signals to user descriptions, and proposes a haptic-caption retrieval task using a supervised contrastive learning framework.
...
Method: Created HapticCap, a dataset of 92,070 haptic-text pairs, and proposed a retrieval task using T5 and AST models with supervised contrastive learning.

Result: The T5 and AST combination performed best in the retrieval task, especially when trained separately for each description category.
...
Result: Search engines prioritize biased content, and ideologically-driven queries worsen these biases, amplifying specific narratives and showing differences across search engines.
...
TL;DR: VL training improves how models deploy taxonomic knowledge in tasks, but doesn't significantly alter the knowledge itself.


<details>
  <summary>Details</summary>
Motivation: To investigate if VL training meaningfully changes linguistic representations, focusing on lexical-conceptual and taxonomic knowledge.

Method: Compared text-only LMs and VL-trained models on a text-only QA task requiring taxonomic understanding, using behavioral and representational analyses.

Result: VL models outperform text-only models in QA tasks, but taxonomic knowledge remains similar; differences lie in how they represent taxonomic vs. non-taxonomic relations.

Conclusion: VL training enhances task-specific deployment of taxonomic knowledge without fundamentally changing the knowledge itself.
...
TL;DR: The paper proposes TAIL, a method to improve length generalization in LLMs by imitating Turing Machine execution, using synthetic data and outperforming existing approaches.
...
Method: TAIL synthesizes chain-of-thoughts data to imitate Turing Machine execution, expanding reasoning steps linearly and reducing dynamic data access difficulties.

Result: TAIL significantly improves length generalization and performance on various tasks, surpassing previous methods and DeepSeek-R1.

Conclusion: Key Turing Machine concepts are crucial for TAIL's success, offering a promising direction for LLM reasoning research with synthetic data.
...
TL;DR: The paper introduces Context Engineering, a systematic approach to optimizing contextual information for LLMs, and analyzes its components and implementations. It highlights a research gap in model capabilities for generating sophisticated outputs.


<details>
  <summary>Details</summary>
Motivation: To formalize and optimize the use of contextual information in LLMs, addressing the gap between understanding and generating complex outputs.
...
Result: Establishes a taxonomy and roadmap for Context Engineering, revealing a critical asymmetry in LLM capabilities: strong understanding but weak generation of complex outputs.

Conclusion: The survey provides a unified framework for advancing context-aware AI, with addressing the generation gap as a key future priority.
...
TL;DR: The paper explores whether LLMs can explain different types of humour, finding they struggle with complex jokes despite excelling at simple puns.


<details>
  <summary>Details</summary>
Motivation: Existing computational humour research focuses on simple puns, neglecting more complex forms. This work aims to evaluate LLMs' ability to explain diverse joke types.

Method: A dataset of 600 jokes across 4 types (heterographic/homographic puns, internet humour, topical jokes) was curated. High-quality explanations were manually written. Zero-shot performance of various LLMs was tested.

Result: No tested LLM reliably explained all joke types, revealing limitations in handling humour requiring world knowledge.

Conclusion: Current LLMs lack robustness in explaining complex humour, highlighting the narrow focus of prior computational humour research.
...
<details>
  <summary>Details</summary>
Motivation: To explore the impact of prolonged RL on small language models and identify key training techniques for performance gains.
...
Conclusion: Effective RL training with verifiable rewards and stability techniques unlocks long-term performance improvements in reasoning tasks.
...
TL;DR: The paper highlights the limitations of parallel-centric machine learning architectures on inherently sequential problems and argues for scaling serial computation to advance AI.


<details>
  <summary>Details</summary>
Motivation: To address the blind spot in machine learning where some problems are fundamentally sequential and cannot be parallelized, limiting progress in tasks like mathematical reasoning and sequential decision-making.

Method: The study formalizes the distinction between parallel and serial problems using complexity theory and evaluates current architectures' limitations on serial tasks.

Result: Current parallel-centric architectures face fundamental limitations on inherently serial problems, highlighting the need for scaling serial computation.

Conclusion: Recognizing and scaling serial computation is crucial for advancing AI, especially in complex reasoning tasks, alongside parallel computation.
...
Conclusion: Integrating mental imagery into AI can improve autonomous thinking and decision-making, bridging gaps in current models.
...
TL;DR: The paper proposes IncA-DES, a framework for handling concept drift in data streams by fusing classifiers, using local experts, and integrating a concept drift detector and an overlap-based classification filter. It also introduces an Online K-d tree for efficiency.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges like concept drift in data streams, where traditional batch-based ML methods struggle due to changing data distributions.

Method: IncA-DES combines local expert training, a concept drift detector, an overlap-based filter, and an Online K-d tree for efficient kNN processing.

Result: Achieved the best average accuracy among state-of-the-art methods and reduced processing time with minimal accuracy loss.

Conclusion: The framework effectively handles concept drift and improves efficiency, making it suitable for real-world data stream applications.
...
<details>
  <summary>Details</summary>
Motivation: Unstructured text in biochemical assays contains valuable but untapped information for drug discovery. Assay2Mol aims to utilize this data to improve early-stage drug discovery.
...
Result: Assay2Mol outperforms recent machine learning approaches in generating candidate molecules and promotes synthesizable molecule generation.

Conclusion: Assay2Mol effectively leverages unstructured assay data for drug discovery, offering a promising approach for early-stage campaigns.
...
TL;DR: The paper addresses the NP-hard problem of clustering ranking vectors (KRC), proposing an efficient approximation algorithm (KRCA) and a branch-and-bound (BnB) method to improve solution quality and speed.


<details>
  <summary>Details</summary>
Motivation: The study aims to solve the k-centroids ranking vectors clustering problem (KRC), which is constrained to ranking vectors, unlike classical k-means clustering.

Method: The authors derive a closed-form solution for the single-cluster case and develop KRCA, an iterative algorithm refining KMC solutions, alongside a BnB method for efficient cluster reconstruction.

Result: KRCA outperforms baseline solutions in experiments, showing significant quality improvements and fast computation. Theoretical error bounds are established.

Conclusion: The work advances KRC for practical applications like personalization and decision-making, providing a foundation for future research.
...
TL;DR: The paper explores regression and classification, showing the log loss minimizer's improved generalization bound (first-order) and introduces a novel 'betting loss' for variance-dependent bounds (second-order).


<details>
  <summary>Details</summary>
Motivation: To improve generalization bounds in regression and classification, particularly by achieving variance-adaptive results without prior knowledge of variance.

Method: Analyzes log loss minimizer for first-order bounds and proposes betting loss for second-order bounds.

Result: Log loss achieves first-order bounds; betting loss provides variance-dependent bounds without explicit variance knowledge.

Conclusion: The betting loss offers a strict improvement over first-order bounds, adapting to variance without prior assumptions.
...
Method: Two approaches: deep metric learning and landmarkers reconstruction, designed to capture landmarkers' properties.

Result: Encoders learn representations aligned with landmarkers but show limited performance gains in HPO warm-starting.

Conclusion: While the methods meet the proposed requirement, their impact on the meta-task is not significant.
...
<details>
  <summary>Details</summary>
Motivation: Manual and heuristic-driven methods for selecting task mixtures in LLM finetuning are suboptimal.

Method: Uses behavioral divergences (e.g., Jensen Shannon Divergence) and an MRF to optimize task proportions with a closed-form solution.

Result: Improves performance on Llama 2 and Mistral across benchmarks like MMLU and BIGBench.

Conclusion: TASKPGM provides efficient, interpretable, and robust LLM finetuning.
...
TL;DR: The paper addresses the startup overhead in training large language models (LLMs), proposing Bootseer, a system-level optimization framework that reduces startup delays by 50%.


<details>
  <summary>Details</summary>
Motivation: Startup overhead in LLM training, often overlooked, wastes significant GPU time (3.5% in one cluster). This work aims to characterize and mitigate it.

Method: Bootseer tackles three bottlenecks: container image loading, dependency installation, and checkpoint resumption, using techniques like hot block prefetch, dependency snapshotting, and striped HDFS-FUSE.

Result: Bootseer reduces startup overhead by 50% in real LLM training workloads.

Conclusion: Addressing startup overhead is critical for industrial-scale LLMs, and Bootseer provides an effective solution.
...
Conclusion: Reasoning-finetuned models repurpose base model representations rather than developing new capabilities independently.
...
TL;DR: FL-based neural decoding shows promise for privacy-conscious neural interfaces, excelling in open-loop but facing challenges in closed-loop scenarios due to real-time constraints.


<details>
  <summary>Details</summary>
Motivation: Neural signals contain sensitive data, making privacy-preserving methods like FL essential for decoder training.

Method: Evaluated FL-based neural decoding using high-dimensional EMG signals in open- and closed-loop scenarios.

Result: FL outperformed local learning in open-loop but lagged in closed-loop, where local learning had better performance but higher privacy risks.

Conclusion: FL needs adaptation for real-time, single-user applications to balance performance and privacy.
...
Method: Proposes transfer learning within an extended domain and an adaptive activation function combining standard functions.

Result: Achieves 40% reduction in relative L2 error and 50% reduction in mean absolute error in extrapolation.

Conclusion: The method enhances PINN robustness and accuracy for extrapolation, with practical benefits and open-source availability.
...
TL;DR: The paper discusses data heterogeneity challenges, emphasizing the importance of data transformation in AI applications and reviewing current strategies to address format disparities.


<details>
  <summary>Details</summary>
Motivation: Data heterogeneity complicates data utilization, often requiring expert intervention. Existing methods focus on structural conflicts, neglecting transformation's role in AI efficiency.

Method: The survey systematically categorizes and reviews strategies for handling data format disparities, highlighting challenges in each approach.

Result: The study identifies gaps in current methodologies and underscores the need for better transformation techniques to preserve data integrity for AI.

Conclusion: Data transformation is critical for AI efficiency, and the paper calls for more comprehensive reviews and solutions to address heterogeneity challenges.
...
TL;DR: PinFM is a foundational transformer model for user activity sequences in recommender systems, pretrained on 20B+ parameters and fine-tuned for specific applications, addressing scalability and latency challenges with innovative techniques like DCAT.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of applying pretraining-and-fine-tuning approaches in industrial recommender systems, such as scalability, latency, and handling new items.

Method: Pretrain a transformer model (PinFM) with 20B+ parameters on user activity data, fine-tune for applications, and use optimizations like DCAT to improve throughput.

Result: PinFM improved throughput by 600%, increased engagement with new items by 20%, and is deployed for over half a billion users.

Conclusion: PinFM successfully addresses industrial recommender system challenges, demonstrating scalability and effectiveness in improving user engagement.
...
TL;DR: The paper develops a continuous-time SDE framework to explain SGD dynamics in deep neural networks, linking microscopic training to macroscopic spectral evolution. It shows singular values follow Dyson Brownian motion and explains the 'bulk+tail' spectral structure.
...
Method: Develops a matrix-valued SDE framework to model SGD dynamics, deriving exact SDEs for singular values and analyzing stationary distributions.

Result: Shows squared singular values follow Dyson Brownian motion with repulsion, leading to gamma-type densities with power-law tails, explaining the 'bulk+tail' spectral structure.
...
Method: The framework uses pretrained multimodal foundation models to dynamically select training samples based on task difficulty and semantic consistency.
...
TL;DR: The paper introduces the LySep model, a layer separation framework for optimizing deep learning in solving PDEs, addressing issues like suboptimal minima and gradient problems by decomposing networks into shallow architectures with auxiliary variables.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for PDEs face challenges like non-convex loss functions, leading to poor convergence (local minima, gradient issues). The LySep model aims to mitigate these problems.

Method: LySep introduces auxiliary variables to separate layers, representing outputs and derivatives per layer. It decomposes deep networks into shallow ones, coupling only neighboring layers. Alternating direction algorithms are developed for optimization.

Result: Theoretical consistency with original deep models is proven. Numerical results show LySep's effectiveness in minimizing loss and reducing solution error in high-dimensional PDEs.

Conclusion: LySep provides a robust optimization framework for deep learning in PDEs, improving performance by addressing convergence and gradient issues through layer separation and auxiliary variables.
...
Method: Introduces a unified taxonomy across five dimensions: data-centric approaches, neural architecture design, learning strategies, multimodal learning, and LLM-based systems. Reviews methods for data quality, representation, self-supervised learning, and clinical knowledge integration.

Result: Highlights emerging trends (foundation models, LLM-driven agents, EHR-to-text translation) and open challenges (benchmarking, explainability, clinical alignment).
...
TL;DR: A multi-channel deep learning framework is proposed for financial risk prediction in NEEQ-listed SMEs, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the elevated financial risks faced by SMEs on the NEEQ due to their limited scale and resilience.

Method: A Triple-Channel Graph Isomorphism Network (GIN) integrates structured financial indicators, textual disclosures, and enterprise relationship data, fused with attention and gating mechanisms.
...
Conclusion: Provides a robust, data-driven tool for financial risk modeling in SMEs, aiding regulators and investors.
...
TL;DR: FLDmamba combines Fourier and Laplace transforms with Mamba to improve long-term time series prediction by capturing multi-scale periodicity and transient dynamics while being robust to noise.


<details>
  <summary>Details</summary>
Motivation: Existing methods like Transformers and Mamba struggle with long-term predictions due to inefficiency, inability to capture multi-scale periodicity, and noise susceptibility.

Method: Proposes FLDmamba, integrating Fourier and Laplace transforms with Mamba to model multi-scale periodicity and transient dynamics, enhancing noise robustness.

Result: FLDmamba outperforms Transformer-based and other Mamba-based models on time series benchmarks.

Conclusion: FLDmamba effectively addresses limitations of current methods, offering superior performance and robustness for time series prediction.
...
TL;DR: PMKLC is a novel parallel multi-knowledge learning-based compressor designed to improve compression ratio, throughput, and robustness for genomic data.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based compressors suffer from inadequate compression ratio, low throughput, and poor robustness, limiting their adoption.

Method: PMKLC features automated multi-knowledge learning, GPU-accelerated encoding, parallel acceleration mechanisms, and two compression modes (single-GPU and multi-GPU).
...
TL;DR: The paper introduces RONOM, a framework combining reduced-order modeling (ROM) and neural operators to address computational challenges in solving time-dependent PDEs, offering error bounds and improved performance in generalization and robustness.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between ROM's fixed discretization limitations and neural operators' lack of rigorous error quantification, enabling flexible and accurate PDE solutions.

Method: Proposes RONOM, integrating ROM and neural operators, with theoretical error bounds and numerical validation.

Result: RONOM matches neural operators in generalization, excels in spatial super-resolution and robustness, and provides insights into temporal super-resolution.

Conclusion: RONOM effectively combines ROM and operator learning, offering theoretical guarantees and practical advantages for PDE solutions.
...
<details>
  <summary>Details</summary>
Motivation: Offline RL's reliance on explicit reward annotations is costly and impractical. ReLOAD addresses this by automating reward generation.

Method: ReLOAD uses RND to train a predictor network mimicking a target network's embeddings. The prediction error serves as intrinsic rewards.

Result: Experiments on D4RL show ReLOAD achieves competitive performance without handcrafted rewards.

Conclusion: ReLOAD provides a practical solution for offline RL by automating reward annotation, matching traditional methods' performance.
...
TL;DR: The paper investigates the dynamics of Neural Tangent Kernel (NTK) eigenvectors during the Edge of Stability (EoS) phenomenon in deep learning, revealing alignment with training targets and providing theoretical insights.


<details>
  <summary>Details</summary>
Motivation: Understanding the behavior of NTK eigenvectors during EoS, which remains unexplored despite prior work on NTK eigenvalues.

Method: Detailed empirical analysis across architectures and theoretical study of a two-layer linear network.

Result: Larger learning rates increase alignment of leading NTK eigenvectors with training targets.

Conclusion: The study advances understanding of Gradient Descent dynamics in deep learning, particularly during EoS.
...
TL;DR: The paper introduces norm-adaptive maximum mean discrepancy (NAMMD) to improve distribution closeness testing (DCT) by scaling MMD with RKHS norms, enhancing test power and addressing limitations of MMD.


<details>
  <summary>Details</summary>
Motivation: Existing DCT methods are limited to discrete one-dimensional spaces, restricting their use for complex data like images. MMD, while powerful, lacks informativeness for assessing closeness levels of multiple distribution pairs.

Method: The authors propose NAMMD, a new discrepancy measure that scales MMD using RKHS norms. They develop NAMMD-based DCT and theoretically analyze its performance.

Result: NAMMD-based DCT shows higher test power than MMD-based DCT with bounded type-I error, validated by experiments on synthetic and real data. NAMMD also improves two-sample testing.

Conclusion: NAMMD enhances DCT and two-sample testing by addressing MMD's limitations, offering a more informative and powerful approach for complex data.
...
TL;DR: A transformer-based method identifies individuals from Wi-Fi signals (CSI) while stationary, achieving 99.82% accuracy.


<details>
  <summary>Details</summary>
Motivation: Wi-Fi sensing offers privacy-preserving human identification, but prior work relies on motion. This paper explores stationary identification.

Method: Uses a dual-branch transformer to process CSI amplitude and phase separately, with preprocessing for signal quality.

Result: Achieves 99.82% classification accuracy, outperforming baselines.

Conclusion: Demonstrates CSI's potential for passive, device-free identification using low-cost Wi-Fi hardware.
...
TL;DR: The paper connects Behavior Cloning (BC) to Reinforcement Learning (RL), proposing an importance-weighted variant (iw-SFT) that improves performance by optimizing a tighter bound on the RL objective.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between supervised fine-tuning (SFT) and RL, showing that SFT can be seen as maximizing a lower bound on the RL objective.

Method: Introduces importance-weighted SFT (iw-SFT), a small modification to SFT, which optimizes a tighter bound to the RL objective and works with quality-scored data.

Result: iw-SFT outperforms standard SFT and is competitive with advanced RL algorithms, achieving 66.7% on the AIME 2024 dataset.

Conclusion: iw-SFT is a simple yet effective variant of SFT that aligns closer to RL, offering improved performance in language models and control tasks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional EEG-based biometric systems are secure but impractical due to cumbersome setups; ear-EEG offers a user-friendly alternative.
...
Result: Achieves 82% accuracy in subject identification using the only available ear-EEG dataset.

Conclusion: Ear-EEG shows promise as a practical and deployable solution for next-gen biometric systems.
...
TL;DR: Proposes topology-aware activation functions, SmoothSplit and ParametricSplit, outperforming traditional ones in low-dimensional settings.


<details>
  <summary>Details</summary>
Motivation: Addresses limitations of traditional activation functions like ReLU in manipulating data topology during training.

Method: Introduces SmoothSplit and ParametricSplit, enabling topology "cutting" for effective manifold transformation.

Result: ParametricSplit excels in low-dimensional scenarios while staying competitive in higher dimensions.

Conclusion: Highlights the potential of topology-aware activation functions for advancing neural network architectures.
...
Conclusion: Video foundation models with masked action prediction can enable scalable and generalizable robotic manipulation in diverse settings.
...
Method: ULER, a user-centric rejector, learns from human ratings and per-feature relevance judgments to evaluate explanation quality.

Result: ULER outperforms state-of-the-art methods in rejecting low-quality explanations across eight benchmarks and a human-annotated dataset.

Conclusion: The LtX framework and ULER effectively address the challenge of ensuring high-quality explanations in ML predictions, enhancing trust and usability.
...
TL;DR: Fremer is an efficient deep forecasting model for cloud workload, outperforming Transformer-based models in accuracy and efficiency while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Transformer-based models lack computational efficiency for large-scale cloud environments, and workload series exhibit complex periodic patterns, making frequency domain solutions advantageous.

Method: Fremer addresses efficiency and accuracy by leveraging the frequency domain, outperforming SOTA models in workload forecasting.

Result: Fremer achieves average improvements of 5.5% in MSE, 4.7% in MAE, and 8.6% in SMAPE over SOTA models, with reduced parameter scale and computational costs.

Conclusion: Fremer demonstrates practical efficacy in real-world applications, improving latency by 18.78% and reducing resource consumption by 2.35% in Kubernetes tests.
...
TL;DR: The paper proposes using prediction uncertainty (aleatoric and epistemic) to enhance model interpretability, guiding explanation selection and improving robustness.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of transparency in complex ML models by integrating uncertainty quantification into explainability methods.

Method: Distinguishes between aleatoric (data-related) and epistemic (model-related) uncertainty to select or reject explanations. Uses uncertainty to inform choice between feature-importance and counterfactual explanations.

Result: Demonstrates improved robustness and attainability of explanations in traditional and deep learning scenarios.

Conclusion: Uncertainty-aware approaches enhance interpretability and reliability of explanations in complex models.
...
TL;DR: TReconLM, a language model-based method, outperforms existing trace reconstruction algorithms by leveraging pretraining on synthetic data and fine-tuning on real-world error patterns.


<details>
  <summary>Details</summary>
Motivation: The need to recover original sequences from noisy copies (deletions, insertions, substitutions) in applications like DNA data storage, where errors are common.

Method: Pretrain language models on synthetic data, then fine-tune on real-world data to adapt to specific error patterns.

Result: TReconLM recovers a higher fraction of error-free sequences compared to state-of-the-art methods.

Conclusion: Language models, when adapted to error patterns, significantly improve trace reconstruction for noisy data.
...
TL;DR: This paper presents two novel modifications to the DAPO algorithm using a mixed-policy approach: incorporating a pre-trained guiding policy for better training stability and re-utilizing zero-reward samples to improve sample efficiency in policy gradient methods.


<details>
  <summary>Details</summary>
Motivation: Standard policy gradient methods suffer from instability and sample inefficiency, particularly in sparse reward settings. The DAPO algorithm discards zero-reward samples through dynamic sampling strategies, leading to wasted training data and reduced sample efficiency.

Method: Two key modifications to DAPO: (1) Incorporating a pre-trained stable guiding policy to provide off-policy experience and regularize target policy training with adaptive learning step size adjustment, and (2) Re-utilizing zero-reward samples by treating them as a distinct batch guided by the expert policy within a mixed-policy framework.

Result: The proposed methods improve training stability and convergence speed while enhancing sample efficiency. Theoretical analysis demonstrates that both methods' objective functions converge to optimal solutions within established reinforcement learning frameworks. The mixed-policy approach effectively balances exploration and exploitation.

Conclusion: The mixed-policy framework successfully addresses key limitations of standard policy gradient methods by providing more stable and efficient policy optimization through guided training and better utilization of training samples, promising improved performance in sparse reward environments.
...
TL;DR: MC²A is an algorithm-hardware co-design framework for accelerating MCMC algorithms, achieving significant speedups over CPU, GPU, TPU, and existing accelerators.


<details>
  <summary>Details</summary>
Motivation: High computational costs of MCMC algorithms limit their feasibility for large-scale problems, and existing acceleration solutions lack hardware flexibility or system-level efficiency.
...
TL;DR: ARES, a probabilistic framework, improves error detection in LLM reasoning chains by focusing on sound premises, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Initial errors in LLM reasoning chains propagate, undermining reliability. Current methods fail to detect these due to improper accounting of error propagation.

Method: ARES uses an autoregressive probabilistic framework to judge claims based on previously-assessed sound premises, providing nuanced scores and statistical guarantees.
...
Conclusion: ARES effectively mitigates error propagation in LLM reasoning, offering robust and certified error detection.
...
<details>
  <summary>Details</summary>
Motivation: Improving AI interpretability in healthcare for safety, transparency, and trust, given the high stakes of medical decisions.

Method: Applied Matryoshka-SAE to MAIRA-2 to uncover human-interpretable features, using large-scale automated interpretability and steering experiments.

Result: Identified clinically relevant concepts (e.g., medical devices, pathologies) and achieved partial directional control over model outputs.

Conclusion: The study advances mechanistic interpretability for MAIRA-2, highlighting challenges but offering insights for improved transparency in radiology AI.
...
TL;DR: The paper proposes a variant of the Multiresolution Reservoir Graph Neural Network (MRGNN) using a Fairing algorithm to address over-smoothing in GNNs, offering a spectral filter for controlled smoothing without shrinkage.


<details>
  <summary>Details</summary>
Motivation: Over-smoothing in GNNs, caused by repeated layer operations, leads to convergence toward low-frequency graph signals. The work aims to mitigate this issue while maintaining beneficial smoothing effects.

Method: The paper revisits the MRGNN reservoir definition, introducing a Fairing algorithm-based variant. This spectral filter, adapted from surface design, smooths graphs without shrinkage via the Laplacian operator.

Result: Theoretical analysis links spectral coefficient tuning to modulating redundant random walks. Experiments with MRGNN show promise for controlled smoothing in tasks like graph classification.

Conclusion: The proposed method effectively addresses over-smoothing in GNNs, with theoretical insights and experimental results suggesting its potential for future research.
...
Method: The framework combines a WaveletInception feature extractor (LWPT and 1D Inception) with BiLSTM for temporal dependency capture, enabling high-resolution health assessment without preprocessing.

Result: The model excels in railway track stiffness estimation, surpassing state-of-the-art methods in accuracy for ballast and railpad stiffness parameters.

Conclusion: The WaveletInception-BiLSTM framework shows promise for fully automated, precise infrastructure health monitoring, validated by superior performance in a case study.
...
<details>
  <summary>Details</summary>
Motivation: Training generative models like GANs requires large datasets and computational resources, which are often unavailable due to privacy concerns and device limitations.

Method: Proposes a decentralized GAN training approach using KLD-weighted Clustered Federated Learning for data heterogeneity and Heterogeneous U-Shaped split learning for device heterogeneity, ensuring no raw data sharing.
...
Conclusion: The approach effectively addresses challenges in decentralized environments, improving performance while maintaining data privacy.
...
Method: Uses the Gini coefficient to measure disparity, establishes a relationship between disparity and model updates, and dynamically adjusts aggregation weights for fairness.
...
Conclusion: FedGA effectively balances fairness and performance in federated learning, proving its practical utility.
...
TL;DR: A residual learning approach enhances Sparse Autoencoders (SAEs) to capture domain-specific features missed by pretrained models, improving interpretability without full retraining.
...
TL;DR: The paper proposes a framework to evaluate and assign the best geometric transformations for each relation in knowledge graphs, improving embedding performance.


<details>
  <summary>Details</summary>
Motivation: Current KGE models use basic geometric transformations uniformly for all relations, lacking relation-specific adaptability.

Method: The framework ranks transformations for each relation, assigns the best fit, or uses majority voting, and leverages attention mechanisms for low-dimensional learning.

Result: The model performs comparably to leading models on benchmark and real-world financial KGs.

Conclusion: Relation-specific geometric transformations enhance KGE performance, validated by empirical results.
...
TL;DR: A framework for onboard fault detection in space launcher electrical systems using a temporal convolutional autoencoder and calibrated classifiers, addressing confidence estimation, OOD detection, and false alarms.


<details>
  <summary>Details</summary>
Motivation: To develop a fault detection system for reusable space launchers that meets broader requirements like confidence estimation, OOD detection, and false alarm control.

Method: Uses a temporal convolutional autoencoder for feature extraction, binary/multiclass classifiers for fault detection/diagnosis, and techniques like conformal anomaly detection and CUSUM for OOD and false alarm control.

Result: Evaluated on simulated data, showing promise but requires real-world testing for operational maturity.

Conclusion: The proposed framework is a promising step toward operational fault detection, pending validation with real data.
...
TL;DR: CFR combines LRP Attention Rollout and DDU to analyze how uncertainty affects interpretability in naturalness assessment of satellite imagery.


<details>
  <summary>Details</summary>
Motivation: Current methods lack interpretability and uncertainty-awareness in monitoring protected natural areas.

Method: Proposes CFR, a framework combining LRP Attention Rollout and DDU to partition data by uncertainty thresholds.

Result: CFR aligns with naturalness research, showing higher relevance to shrublands, forests, and wetlands. Interpretability declines as uncertainty rises.

Conclusion: CFR offers a data-centric approach to assess relevance in satellite imagery based on certainty.
...
TL;DR: The paper investigates which Transformer architecture is best for Long-term Time Series Forecasting (LTSF) by proposing a taxonomy to disentangle design choices and comparing variants. Key findings include the effectiveness of bi-directional attention, complete forecasting aggregation, and direct-mapping paradigms.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based LTSF models are coupled with time-series-specific designs, making it hard to isolate the impact of architecture. The study aims to clarify this by comparing architectures systematically.

Method: A novel taxonomy is introduced to disentangle design choices (attention mechanisms, forecasting aggregations, paradigms, and normalization layers). Extensive experiments compare variants.

Result: Bi-directional attention with joint-attention works best; complete forecasting aggregation improves performance; direct-mapping outperforms autoregressive approaches. The combined optimal model surpasses existing ones.

Conclusion: The taxonomy and findings provide clear guidance for future Transformer designs in LTSF, with the optimal model validating the conclusions.
...
TL;DR: The paper explores PAC and online learnability of hypothesis classes derived from countably infinite graphs, focusing on permutations with finite support. It links PAC learnability to online learnability and automorphic triviality, characterizes non-learnable graphs, and partitions infinite graphs into four classes based on learnability for k-vertex permutations.


<details>
  <summary>Details</summary>
Motivation: To understand the learnability of graph labelings when the graph's structure and label set are known, particularly for infinite graphs under finite-support permutations.

Method: Analyzes PAC and online learnability of hypothesis classes formed by permuting vertices of a countably infinite graph, focusing on finite-support permutations. Uses automorphic triviality and relaxations of the infinite random graph's extension property.

Result: Shows PAC learnability of finite-support copies implies online learnability of the full isomorphism type of G, equivalent to automorphic triviality. Characterizes non-learnable graphs and partitions infinite graphs into four classes based on learnability for k-vertex permutations.

Conclusion: The study provides a framework for understanding learnability in infinite graphs, linking it to automorphic triviality and partitioning graphs into distinct learnability classes.
...
<details>
  <summary>Details</summary>
Motivation: Existing NAS methods for ViTs are computationally expensive and struggle to innovate.

Method: DASViT applies differentiable architecture search to ViTs, enabling novel designs.

Result: DASViT outperforms ViT-B/16 on multiple datasets with improved efficiency.
...
TL;DR: MUPAX is a robust, deterministic, model-agnostic XAI technique with guaranteed convergence, applicable across diverse data modalities and tasks, enhancing model accuracy while providing precise explanations.


<details>
  <summary>Details</summary>
Motivation: To address the need for XAI methods that are deterministic, model-agnostic, and converge reliably, while improving model performance and eliminating spurious relationships.

Method: MUPAX uses measure-theoretic formulation and structured perturbation analysis for principled feature importance attribution, applicable to any data dimension or loss function.

Result: MUPAX outperforms other XAI methods by preserving or enhancing model accuracy, generating precise and consistent explanations across various tasks (1D, 2D, 3D).

Conclusion: MUPAX advances trustworthy AI by offering a robust, dimension-agnostic explainability solution with rigorous convergence guarantees and improved performance.
...
TL;DR: A review of LLM alignment using inverse reinforcement learning (IRL), highlighting its role in enhancing reliability and control, and discussing challenges, methods, and future directions.


<details>
  <summary>Details</summary>
Motivation: Alignment is critical for reliable and controllable LLMs, with RL playing a key role. The paper aims to explore IRL's potential in this context.

Method: The paper reviews IRL techniques for LLM alignment, covering neural reward models, datasets, benchmarks, and computational efficiency.

Result: Identifies key challenges and opportunities in IRL for LLM alignment, emphasizing the need for human data-driven reward models.

Conclusion: The paper synthesizes findings to outline unresolved challenges and future research directions for improving LLM alignment via RL and IRL.
...
TL;DR: The paper proposes a cross-modal knowledge distillation framework to improve EEG learning by addressing modality and label inconsistencies, enhancing emotion regression and classification performance.
...
Result: The approach outperforms unimodal and multimodal baselines on a public dataset, improving EEG-based emotion tasks.

Conclusion: The framework shows promise for BCI applications by effectively mitigating key challenges in EEG learning.
...
TL;DR: NGTM is a novel graph generation framework offering interpretability by modeling graphs as mixtures of latent topics, enabling semantic control and tracing.


<details>
  <summary>Details</summary>
Motivation: Existing graph generation methods lack interpretability, obscuring structural decisions. NGTM aims to address this by integrating topic modeling for clarity.

Method: NGTM represents graphs as mixtures of latent topics, each defining substructure distributions, and combines these with a global structural variable for generation.

Result: NGTM achieves competitive generation quality while providing fine-grained control and interpretability, allowing topic-level adjustments.

Conclusion: NGTM successfully balances generation quality and interpretability, enabling semantic tracing and user control over structural features.
...
Method: Automated detection and human validation from VoxCeleb and Expresso, integrated with ASR, NV tagging, emotion classification, and fusion algorithms.

Result: Fine-tuned TTS models on NVTTS achieve performance parity with closed-source systems like CosyVoice2 in human and automatic evaluations.

Conclusion: NVTTS addresses a key bottleneck in expressive TTS research and is publicly available.
...
TL;DR: The paper introduces Spectral Bellman Representation, a novel framework for value-based RL, aligning feature learning with Bellman updates via spectral analysis, improving performance in hard-exploration tasks.


<details>
  <summary>Details</summary>
Motivation: Existing representation learning in RL misaligns with tasks, lacking direct connection to Bellman updates. The work aims to bridge this gap by leveraging spectral properties of value functions.

Method: Proposes Spectral Bellman Representation, derived from Inherent Bellman Error (IBE) condition, linking Bellman operator transformations to feature covariance. Requires simple algorithm modifications.

Result: Demonstrates improved performance in hard-exploration and long-horizon tasks, with structured exploration and better feature alignment.

Conclusion: Spectral Bellman Representation provides a principled, effective approach for learning Bellman-aligned representations, enhancing value-based RL.
...
TL;DR: The paper introduces Monotone Gradient Networks (mGradNets) to learn optimal transport maps by leveraging the Monge-Ampère equation, demonstrating effectiveness in robot swarm control.
...
Method: Proposes mGradNets, neural networks that parameterize monotone gradient maps, and trains them by minimizing a loss based on the Monge-Ampère equation.
...
Conclusion: mGradNets provide a promising approach for solving optimal transport problems, with practical utility in robotics.
...
TL;DR: The paper introduces MoTM, a method for out-of-domain time series imputation using implicit neural representations (INRs) and a ridge regressor, addressing gaps in current foundation models.


<details>
  <summary>Details</summary>
Motivation: Current time series foundation models focus on forecasting, leaving out-of-domain imputation underexplored. The paper aims to fill this gap.

Method: Proposes MoTM, combining INRs (trained on distinct time series families) with a ridge regressor for adaptive inference.

Result: Demonstrates robust in-domain and out-of-domain generalization in diverse imputation scenarios.
...
TL;DR: Including asynchronously published electricity prices from interconnected markets improves forecasting accuracy in Belgian and Swedish zones by 22% and 9%, respectively, though frequent recalibration and data selection are critical.


<details>
  <summary>Details</summary>
Motivation: To enhance short-term electricity price forecasting accuracy by leveraging price data from interconnected markets with earlier gate closure times (GCTs).

Method: Uses a state-of-the-art ensemble of models, incorporating price data from markets with earlier GCTs (Germany-Luxembourg, Austria, Switzerland) for forecasting in Belgian and Swedish zones.

Result: Significant accuracy improvements: 22% in Belgium (BE) and 9% in Sweden (SE3). Frequent recalibration is necessary but costly, and more data doesn't always improve performance.

Conclusion: Findings guide market participants in optimizing bidding strategies in interconnected European energy markets, highlighting the value of asynchronous price data and the trade-offs in model recalibration and data usage.
...
TL;DR: A novel framework for generating kernel functions on permutation space is proposed, replacing the quadratic complexity of the Mallows kernel with a linearithmic Merge Kernel derived from merge sort, enhancing efficiency and performance in Bayesian optimization.


<details>
  <summary>Details</summary>
Motivation: The Mallows kernel, the current state-of-the-art for Bayesian optimization in permutation spaces, has quadratic complexity, which is computationally expensive. The goal is to reduce this complexity while maintaining or improving performance.

Method: A framework for generating kernel functions based on sorting algorithms is introduced. The Merge Kernel, derived from merge sort, reduces complexity to Θ(n log n). Additional lightweight descriptors (shift histogram, split-pair line, sliding-window motifs) are incorporated for robustness.

Result: The Merge Kernel outperforms the Mallows kernel in permutation optimization benchmarks, offering a more compact and effective solution.

Conclusion: The proposed Merge Kernel framework provides a computationally efficient and robust alternative to the Mallows kernel, improving Bayesian optimization in permutation spaces.
...
<details>
  <summary>Details</summary>
Motivation: Existing team modeling approaches fail to jointly capture temporal and relational dynamics or provide actionable insights, limiting practical applications.

Method: TRENN combines an automatic temporal graph extractor, tempo-relational encoder, decoder, and explainability modules. MT-TRENN extends this with a multi-task head for shared embeddings and multi-construct prediction.

Result: TRENN and MT-TRENN outperform methods using only temporal or relational data, with explainability modules providing interpretable insights.

Conclusion: The approach is well-suited for Human-Centered AI, offering actionable recommendations for team improvement in collaborative environments.
...
TL;DR: GeoReg integrates satellite and geospatial data with LLM-derived features to estimate socio-economic indicators in data-scarce regions, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of estimating socio-economic indicators in data-scarce regions, especially developing countries, to inform policy and sustainable development.

Method: Uses a regression model (GeoReg) with LLM-extracted features, categorizes feature correlations, applies tailored weight constraints, and incorporates nonlinear patterns through feature interactions and transformations.

Result: Outperforms baseline models in estimating indicators across three countries with varying development levels, even in low-income, data-limited settings.

Conclusion: GeoReg effectively leverages diverse data and LLM capabilities to improve socio-economic indicator estimation, offering a scalable solution for data-scarce regions.
...
TL;DR: The paper explores training transformer models with Lipschitz bounds, improving stability and performance using novel tools and optimizer dynamics.


<details>
  <summary>Details</summary>
Motivation: Neural networks' sensitivity to perturbations leads to issues like adversarial vulnerability and overfitting. The gap in training modern architectures (e.g., transformers) with Lipschitz certificates motivates this work.

Method: Developed efficient tools for norm-constrained weight matrices and trained transformers with Lipschitz bounds. Used Muon optimizer and co-designed a weight constraint method.

Result: Achieved 60% validation accuracy on Shakespeare text with a 2-Lipschitz transformer and 21% accuracy on internet text with a 10-Lipschitz 145M parameter model. However, matching baseline accuracy required a high Lipschitz bound (10^264).

Conclusion: Lipschitz transformers can train without stability measures, but performance tradeoffs exist. Optimizer choice (Muon) and weight constraints improve results.
...
### [209] [ParaStudent: Generating and Evaluating Realistic Student Code by Teaching LLMs to Struggle](https://arxiv.org/abs/2507.12674)
*Mihran Miroyan,Rose Niousha,Joseph E. Gonzalez,Gireeja Ranade,Narges Norouzi*

Main category: cs.CY

TL;DR: ParaStudent explores LLM-generated 'student-like' code, showing fine-tuning improves alignment with real student behaviors like errors and style.


<details>
  <summary>Details</summary>
Motivation: To assess if LLMs can mimic imperfect, iterative, and diverse student coding patterns in introductory programming.

Method: Uses timestamped student submissions for low- and high-resolution experiments, evaluating semantic, functional, and stylistic dimensions.

Result: Fine-tuning enhances alignment with student trajectories, capturing errors, incremental progress, and stylistic diversity.
...
### [210] [Safeguarding Federated Learning-based Road Condition Classification](https://arxiv.org/abs/2507.12568)
*Sheng Liu,Panos Papadimitratos*

Main category: cs.CR

TL;DR: The paper highlights vulnerabilities in Federated Learning (FL) for Road Condition Classification (RCC) due to Targeted Label Flipping Attacks (TLFAs), proposes a metric to quantify risks, and introduces FLARE, a defense mechanism.
...
Method: The study discloses FL-RCC vulnerabilities, introduces a label-distance-based metric, and proposes FLARE, a neuron-wise analysis defense.

Result: Experiments show TLFAs severely impact FL-RCC systems, while FLARE effectively mitigates these attacks.

Conclusion: The paper addresses FL-RCC vulnerabilities, provides a risk metric, and demonstrates FLARE's effectiveness in countering TLFAs.
...
### [211] [Prompt Injection 2.0: Hybrid AI Threats](https://arxiv.org/abs/2507.13169)
*Jeremy McHugh,Kristina Šekrst,Jon Cefalu*

Main category: cs.CR

TL;DR: The paper analyzes Prompt Injection 2.0, highlighting its integration with traditional cybersecurity exploits to bypass security measures. It evaluates mitigation technologies and proposes architectural solutions.


<details>
  <summary>Details</summary>
Motivation: Prompt injection attacks have evolved into hybrid threats, combining with traditional vulnerabilities like XSS and CSRF, necessitating updated defenses.

Method: The study builds on foundational research, benchmarks traditional security measures, and introduces architectural solutions like prompt isolation and runtime security.

Result: Traditional defenses like web application firewalls fail against AI-enhanced attacks, prompting the need for new solutions.

Conclusion: The paper underscores the urgency of integrating novel threat detection with traditional security to counter evolving prompt injection threats.
...
### [212] [A Semi-Supervised Learning Method for the Identification of Bad Exposures in Large Imaging Surveys](https://arxiv.org/abs/2507.12784)
...
TL;DR: A machine-learning pipeline using a vision transformer and kNN classifier is introduced to detect poor-quality exposures in large astronomical surveys, validated on DECaLS data.


<details>
  <summary>Details</summary>
Motivation: Traditional human inspection for anomaly detection in astronomical surveys is impractical due to increasing data volume.

Method: Semi-supervised pipeline combining a vision transformer (ViT) trained via SSL with a kNN classifier, validated on labeled DECam exposures.

Result: Identified 780 problematic exposures in DECaLS Data Release 11, verified by visual inspection.
...
### [213] [(Exhaustive) Symbolic Regression and model selection by minimum description length](https://arxiv.org/abs/2507.13033)
*Harry Desmond*

Main category: astro-ph.IM

TL;DR: The paper proposes an exhaustive search and model selection method for symbolic regression to address challenges of traditional algorithms, demonstrating its effectiveness on astrophysics problems.


<details>
  <summary>Details</summary>
Motivation: Traditional symbolic regression algorithms often fail to find optimal functions and rely on ambiguous assumptions. The paper aims to improve reliability and justification in function selection.

Method: The method involves exhaustive search combined with model selection using the minimum description length principle, balancing accuracy and complexity in information units.

Result: The algorithm outperforms literature standards in three astrophysics problems: universe expansion history, gravity behavior in galaxies, and inflaton field potential.

Conclusion: The proposed methodology is general-purpose and highly effective, with potential for broad scientific applications.
...
### [214] [Autonomy for Older Adult-Agent Interaction](https://arxiv.org/abs/2507.12767)
*Jiaxin An*

Main category: cs.HC

TL;DR: The paper explores AI-powered agents for elderly care, focusing on aligning agent autonomy with older adults' preferences through four dimensions: decision-making, goal-oriented, control, and social responsibility autonomy. It proposes research directions to address ethical implications, operationalize autonomy, and develop measures.


<details>
  <summary>Details</summary>
Motivation: The aging population necessitates AI tools for caregiving, but ensuring these agents align with older adults' autonomy preferences is a challenge.

Method: The paper examines four dimensions of autonomy (decision-making, goal-oriented, control, social responsibility) and proposes research directions to address alignment.

Result: Key dimensions of autonomy are identified, and research directions are suggested to improve agent alignment with elderly autonomy preferences.

Conclusion: The paper highlights the need for further research on autonomy dimensions and ethical implications to enhance AI agent support for older adults.
...
### [215] [LLM-Powered Quantum Code Transpilation](https://arxiv.org/abs/2507.12480)
...
<details>
  <summary>Details</summary>
Motivation: The diversity of Quantum SDKs (e.g., Qiskit, Cirq, PennyLane) creates interoperability and cross-platform development challenges, which traditional rule-based transpilers struggle to address efficiently.

Method: The study leverages LLMs' pretrained knowledge and contextual reasoning to act as language-agnostic transpilers, enabling automated conversion of quantum programs between QSDKs while preserving functionality.

Result: The approach eliminates the need for manually defined transformation rules, offering a scalable and flexible solution for quantum software portability.
...
### [216] [Kodezi Chronos: A Debugging-First Language Model for Repository-Scale, Memory-Driven Code Understanding](https://arxiv.org/abs/2507.12482)
*Ishraq Khan,Assad Chowdary,Sharoz Haseeb,Urvish Patel*

Main category: cs.SE

TL;DR: Kodezi Chronos is a next-gen architecture for autonomous code understanding, debugging, and maintenance, outperforming traditional LLMs with 23% better bug detection and 40% faster debugging.


<details>
  <summary>Details</summary>
Motivation: Current LLMs are limited by context constraints and lack of explicit code structure reasoning, hindering large-scale codebase tasks.

Method: Uses a multi-level embedding memory engine with vector and graph-based indexing for efficient reasoning over millions of lines of code.

Result: Outperforms prior models with 23% improvement in bug detection and 40% reduction in debugging cycles.

Conclusion: Chronos advances autonomous software maintenance, enhancing reliability and productivity while reducing manual effort.
...
### [217] [A Survey of AIOps in the Era of Large Language Models](https://arxiv.org/abs/2507.12472)
...
Method: Survey of 183 research papers (2020-2024) to answer four key research questions about data sources, task evolution, LLM-based methods, and evaluation.

Result: Identified advancements, trends, and gaps in LLM-integrated AIOps, including novel tasks and data processing techniques.

Conclusion: Highlights the need for further research to explore LLMs' full potential in AIOps and proposes future directions.
...
### [218] [A Fuzzy Approach to Project Success: Measuring What Matters](https://arxiv.org/abs/2507.12653)
*João Granja-Correia,Remedios Hernández-Linares,Luca Ferranti,Arménio Rego*

Main category: cs.SE

TL;DR: A novel fuzzy logic-based approach for project success evaluation, addressing limitations of Likert-scale measures.
...
Result: Potential for more accurate project success evaluation and adaptability to complex scenarios.
...
### [219] [QSpark: Towards Reliable Qiskit Code Generation](https://arxiv.org/abs/2507.12642)
...
<details>
  <summary>Details</summary>
Motivation: Addressing the issue of flawed Qiskit code output by LLMs in quantum programming by enhancing error resilience.
...
### [220] [Single Conversation Methodology: A Human-Centered Protocol for AI-Assisted Software Development](https://arxiv.org/abs/2507.12665)
*Salvador D. Escobedo*

Main category: cs.SE

TL;DR: SCM is a structured methodology for software development using LLMs, focusing on a single, persistent conversation to enhance clarity, traceability, and developer control.
...
Method: SCM involves a single long-context conversation covering all project stages (requirements, architecture, implementation) with principles like cognitive clarity and modularity.

Result: SCM offers a structured alternative to ad hoc LLM use, emphasizing developer oversight and traceability.

Conclusion: SCM reasserts the developer's active role in LLM-assisted development, providing a more disciplined and effective approach.
...
### [221] [Physics constrained learning of stochastic characteristics](https://arxiv.org/abs/2507.12661)
...
<details>
  <summary>Details</summary>
Motivation: Accurate state estimation is hindered by uncertainty in process and measurement models, and improper covariance matrix selection can lead to estimation errors or filter divergence.
...
Result: The approach aims to improve accuracy by systematically modeling noise, addressing limitations of traditional optimization-based methods.

Conclusion: The learning-based method offers a promising solution for noise identification, enhancing state estimation performance in real-time applications.
...
### [222] [Finite-Dimensional Gaussian Approximation for Deep Neural Networks: Universality in Random Weights](https://arxiv.org/abs/2507.12686)
*Krishnakumar Balasubramanian,Nathan Ross*

Main category: stat.ML

TL;DR: The paper analyzes the Gaussian approximation bounds for the finite-dimensional distributions of deep neural networks with random weights, showing convergence rates as layer widths grow.


<details>
  <summary>Details</summary>
Motivation: To understand the behavior of deep neural networks with random weights and finite-order moments, particularly their convergence to Gaussian limits.

Method: Uses Wasserstein-$1$ norm to measure Gaussian approximation bounds, assuming Lipschitz activation functions and allowing arbitrary growth rates of layer widths.

Result: Convergence rates of order $n^{-({1}/{6})^{L-1} + \epsilon}$ are derived for networks with proportional layer widths and $L-1$ hidden layers.

Conclusion: The study provides explicit convergence rates for Gaussian approximations in deep neural networks with random weights, contributing to theoretical understanding of their behavior.
...
### [223] [Self Balancing Neural Network: A Novel Method to Estimate Average Treatment Effect](https://arxiv.org/abs/2507.12818)
*Atomsa Gemechu Abdisa,Yingchun Zhou,Yuqi Qiu*

Main category: stat.ML

TL;DR: The paper proposes a 'Self-balancing neural network' (Sbnet) to address bias in average treatment effect estimation due to confounding and instrumental variables, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Confounding and instrumental variables bias treatment effect estimates in observational studies, and existing propensity score methods risk misspecification.

Method: Sbnet uses a balancing net within a feedforward neural network to estimate pseudo propensity scores, enabling one-step treatment effect estimation. A multi-pseudo propensity score framework is also introduced.

Result: Sbnet outperforms state-of-the-art methods in simulations and real-world datasets.

Conclusion: The proposed Sbnet effectively reduces bias in treatment effect estimation and improves performance over existing methods.
...
### [224] [Bayesian Modeling and Estimation of Linear Time-Variant Systems using Neural Networks and Gaussian Processes](https://arxiv.org/abs/2507.12878)
*Yaniv Shulman*

Main category: stat.ML

TL;DR: A Bayesian framework for identifying Linear Time-Variant (LTV) systems using stochastic modeling of impulse responses, combining machine learning techniques for robust and efficient inference.


<details>
  <summary>Details</summary>
Motivation: Addressing the ill-posed inverse problem of LTV system identification by quantifying uncertainty and improving data efficiency.
...
Result: Demonstrates robust inference from noisy data, superior efficiency in simulations, and successful tracking of varying LTV responses.
...
### [225] [When Pattern-by-Pattern Works: Theoretical and Empirical Insights for Logistic Models with Missing Values](https://arxiv.org/abs/2507.13024)
*Christophe Muller,Erwan Scornet,Julie Josse*

Main category: stat.ML

TL;DR: The paper addresses predicting responses with missing inputs in logistic models, proposing a Pattern-by-Pattern (PbP) strategy and comparing it with other methods like imputation and EM algorithms.


<details>
  <summary>Details</summary>
Motivation: Handling missing data in logistic regression is challenging, and existing methods lack comprehensive evaluation. The study aims to fill this gap.

Method: Theoretical analysis of PbP's approximation to Bayes probabilities and empirical comparison of methods (imputation, PbP, EM) across scenarios.

Result: PbP approximates Bayes probabilities well; MICE.RF.Y performs best for nonlinear features, while PbP excels with Gaussian mixtures.

Conclusion: PbP is recommended for large samples with Gaussian mixtures, and MICE.RF.Y for nonlinear features, providing a practical guide for logistic regression with missing data.
...
### [226] [Relation-Aware Slicing in Cross-Domain Alignment](https://arxiv.org/abs/2507.13194)
*Dhruv Sarkar,Aprameyo Chakrabartty,Anish Chakrabarty,Swagatam Das*

Main category: stat.ML

TL;DR: The paper introduces the Relation-Aware Slicing Distribution (RASD) to improve the Sliced Gromov-Wasserstein (SGW) distance by addressing computational inefficiencies and uninformative directions. The proposed RASGW distance and its variants, like IWRASGW, outperform SGW in alignment tasks.


<details>
  <summary>Details</summary>
Motivation: The SGW distance suffers from high computational costs and uninformative projecting directions, limiting its efficiency and representational power.

Method: The authors propose the Relation-Aware Projecting Direction (RAPD) to capture pairwise associations, leading to the RASD. This enables the RASGW distance and its variants, which are optimized for fast sampling and performance.

Result: The RASGW distance and its variants demonstrate superior empirical performance in alignment tasks compared to SGW. Theoretical analysis supports their efficacy.

Conclusion: The RASD and RASGW distance provide a computationally efficient and effective alternative to SGW, addressing its limitations while maintaining strong performance in alignment tasks.
...
### [227] [Achieving Robust Channel Estimation Neural Networks by Designed Training Data](https://arxiv.org/abs/2507.12630)
*Dianxin Luan,John Thompson*

Main category: eess.SP

TL;DR: Proposes synthetic training dataset design criteria for neural networks in channel estimation, ensuring robust performance on unseen channels without online training.


<details>
  <summary>Details</summary>
Motivation: Neural networks often fail to generalize to new channel data due to lack of extrapolation and limited resources for online training.

Method: Designs synthetic training datasets and benchmarks to ensure neural networks achieve a target MSE on unseen channels.

Result: Neural networks generalize robustly to various channel profiles without prior information or parameter updates.

Conclusion: Offline-trained neural networks with synthetic data can achieve reliable channel estimation without real-time adaptation.
...
### [228] [A Novel Data Augmentation Strategy for Robust Deep Learning Classification of Biomedical Time-Series Data: Application to ECG and EEG Analysis](https://arxiv.org/abs/2507.12645)
...
<details>
  <summary>Details</summary>
Motivation: The need for accurate, unified analysis of diverse biological signals (e.g., ECG, EEG) in synchronous monitoring, and overcoming class imbalance in biomedical datasets.

Method: ResNet-based CNN with attention mechanism, novel data augmentation (time-domain concatenation), wavelet denoising, baseline removal, standardization, and Focal Loss for class imbalance.

Result: Achieved accuracies of 99.96% (UCI Seizure EEG), 99.78% (MIT-BIH Arrhythmia), and 100% (PTB Diagnostic ECG).
...
### [229] [Towards Formal Verification of LLM-Generated Code from Natural Language Prompts](https://arxiv.org/abs/2507.13290)
...
<details>
  <summary>Details</summary>
Motivation: LLMs often generate incorrect code, and users struggle to detect errors. The work aims to provide formal correctness guarantees to improve AI Code Assistants and enable natural language programming for non-programmers.
...
Result: The verifier successfully verified correct code in 83% of cases and identified incorrect code in 92% of tasks in a benchmark suite.

Conclusion: The proposed approach effectively addresses the challenge of incorrect LLM-generated code, offering a reliable verification method with high accuracy.
...
### [230] [The Generalist Brain Module: Module Repetition in Neural Networks in Light of the Minicolumn Hypothesis](https://arxiv.org/abs/2507.12473)
*Mia-Katrin Kvalsund,Mikkel Elle Lepperød*

Main category: q-bio.NC

TL;DR: The paper reviews AI architectures inspired by the brain's minicolumn hypothesis, linking repeated neural modules to collective intelligence (CI) for robustness, adaptability, and efficiency.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in comprehensive reviews connecting cortical columns to repeated neural modules in AI, exploring their potential for solving modern AI challenges like scalability and energy efficiency.
...
Result: Repeated modules exhibit CI properties, converging toward generalist modules that improve energy efficiency, scalability, and robust control. Empirical evidence supports generalization, but theoretical gaps remain.

Conclusion: Architectures with module repetition hold untapped potential for efficiency and robustness, offering solutions to AI's scalability and energy challenges by mimicking brain-inspired principles.
...
### [231] [Distributional Reinforcement Learning on Path-dependent Options](https://arxiv.org/abs/2507.12657)
*Ahmet Umur Özsoy*

Main category: q-fin.MF

TL;DR: A framework for pricing path-dependent derivatives using Distributional Reinforcement Learning (DistRL) to model full payoff distributions, enabling risk-aware pricing and tail-risk estimation.


<details>
  <summary>Details</summary>
Motivation: Traditional methods focus on expected option value, lacking full distribution insights for risk and uncertainty quantification.

Method: Uses Distributional Reinforcement Learning (DistRL) with quantile-based value function approximators to estimate payoff distributions.

Result: Demonstrated efficacy on Asian options, providing risk-aware pricing and tail-risk estimation.

Conclusion: The approach enhances uncertainty quantification and risk management in derivative pricing.
...
### [232] [InSight: AI Mobile Screening Tool for Multiple Eye Disease Detection using Multimodal Fusion](https://arxiv.org/abs/2507.12669)
*Ananya Raghu,Anisha Raghu,Alice S. Tang,Yannis M. Paulus,Tyson N. Kim,Tomiko T. Oskotsky*

Main category: eess.IV

TL;DR: InSight is an AI-based app for diagnosing five common eye diseases using patient metadata and fundus images, improving accessibility in resource-limited settings.


<details>
  <summary>Details</summary>
Motivation: Limited access to medical care in low- and middle-income countries necessitates an accessible, accurate screening tool for early detection of eye diseases.

Method: InSight uses a three-stage pipeline: image quality assessment, disease diagnosis (with MetaFusion for multimodal data), and DR grading. It employs pretraining and multitask learning.

Result: The model achieves near-100% image quality accuracy and outperforms image-only models by 4-6% in balanced accuracy. It is computationally efficient.

Conclusion: InSight is robust, accurate, and efficient, generalizing well across different image sources and conditions.
...
### [233] [Improving Diagnostic Accuracy of Pigmented Skin Lesions With CNNs: an Application on the DermaMNIST Dataset](https://arxiv.org/abs/2507.12961)
*Nerma Kadric,Amila Akagic,Medina Kapo*

Main category: eess.IV

TL;DR: This study evaluates ResNet-50 and EfficientNetV2L models for classifying pigmented skin lesions using the DermaMNIST dataset, achieving competitive results and highlighting CNNs' potential in biomedical imaging.


<details>
  <summary>Details</summary>
Motivation: Pigmented skin lesions can indicate serious conditions like melanoma, necessitating accurate diagnostic tools. The study leverages the DermaMNIST dataset to advance biomedical image analysis.

Method: The study employs transfer learning with ResNet-50 and EfficientNetV2L models, testing various layer configurations for multi-class classification of pigmented lesions.

Result: One model configuration matches or surpasses existing methods, demonstrating high diagnostic accuracy.

Conclusion: CNNs, particularly ResNet-50 and EfficientNetV2L, show promise in improving biomedical image analysis and diagnostic accuracy for pigmented lesions.
...
### [234] [Pathology-Guided Virtual Staining Metric for Evaluation and Training](https://arxiv.org/abs/2507.12624)
...
<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods for virtual staining rely on metrics unsuitable for pathology, lacking relevance and objectivity.

Method: Developed PaPIS, a deep learning-based FR-IQA metric incorporating cell morphology segmentation and Retinex-inspired decomposition.

Result: PaPIS aligns better with pathology-relevant visual cues and improves histological fidelity in virtual staining models.
...
### [235] [TRIQA: Image Quality Assessment by Contrastive Pretraining on Ordered Distortion Triplets](https://arxiv.org/abs/2507.12687)
*Rajesh Sureddi,Saman Zadtootaghaj,Nabajeet Barman,Alan C. Bovik*

Main category: eess.IV

TL;DR: A novel NR-IQA model using contrastive triplet-based learning is proposed to address limited labeled data, achieving strong generalization with fewer samples.


<details>
  <summary>Details</summary>
Motivation: NR-IQA is challenging due to lack of reference images and limited labeled data. Deep learning advancements are hindered by data scarcity.
...
Result: The model achieves strong generalization performance across public datasets with fewer training samples.

Conclusion: The proposed approach advances NR-IQA by efficiently training with limited data and improving generalization.
...
### [236] [Pixel Perfect MegaMed: A Megapixel-Scale Vision-Language Foundation Model for Generating High Resolution Medical Images](https://arxiv.org/abs/2507.12698)
...
<details>
  <summary>Details</summary>
Motivation: Traditional GANs and VAEs struggle with fine-grained detail preservation in medical images, which is critical for accurate diagnosis.
...
Result: Generates clinically faithful chest X-rays from text prompts and improves classification performance in low-data regimes.

Conclusion: Pixel Perfect MegaMed advances medical image synthesis by combining high resolution with fine detail preservation and vision-language alignment.
...
### [237] [Unleashing Vision Foundation Models for Coronary Artery Segmentation: Parallel ViT-CNN Encoding and Variational Fusion](https://arxiv.org/abs/2507.12938)
...
Method: Uses a parallel encoding architecture with a ViT encoder for global features and a CNN encoder for local details, fused via a CVF module. An EUR module refines uncertain regions.

Result: Outperforms state-of-the-art methods on multiple datasets, demonstrating superior accuracy and generalization.

Conclusion: The proposed framework effectively addresses segmentation challenges and shows strong performance, with code publicly available.
...
### [238] [From Variability To Accuracy: Conditional Bernoulli Diffusion Models with Consensus-Driven Correction for Thin Structure Segmentation](https://arxiv.org/abs/2507.12985)
*Jinseo An,Min Jin Lee,Kyu Won Shim,Helen Hong*

Main category: eess.IV

TL;DR: A novel framework using multiple diffusion model outputs improves orbital bone segmentation in CT images, addressing ambiguous boundaries and thin structures.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of orbital bones is crucial for customized implants, but existing methods struggle with ambiguous regions and thin structures.

Method: A conditional Bernoulli diffusion model generates multiple segmentations, followed by consensus-driven correction using position proximity, consensus level, and gradient direction similarity.

Result: Outperforms existing methods, improving recall in ambiguous regions and preserving thin structure continuity.

Conclusion: The method automates manual correction, aiding surgical planning and surgery.
...
### [239] [fastWDM3D: Fast and Accurate 3D Healthy Tissue Inpainting](https://arxiv.org/abs/2507.13146)
*Alicia Durrer,Florentin Bieder,Paul Friedrich,Bjoern Menze,Philippe C. Cattin,Florian Kofler*

Main category: eess.IV

TL;DR: The paper introduces fastWDM3D, a 3D wavelet diffusion model for healthy tissue inpainting, achieving high-quality results with significantly faster sampling than previous methods.
...
Method: Combines DDPMs with generative adversarial networks (GANs) and employs a variance-preserving noise schedule for 3D inpainting, then adapts a 3D wavelet diffusion model (WDM3D) without GANs.

Result: Achieved SSIM of 0.8571, MSE of 0.0079, and PSNR of 22.26 in just two time steps (1.81s per image), outperforming other DDPMs by up to 800x in speed.

Conclusion: fastWDM3D is a fast and accurate method for healthy tissue inpainting, with potential applications in tumor growth modeling and image registration.
...
### [240] [SpectraLift: Physics-Guided Spectral-Inversion Network for Self-Supervised Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2507.13339)
*Ritik Shah,Marco F. Duarte*

Main category: eess.IV

TL;DR: SpectraLift is a self-supervised framework for fusing low-resolution hyperspectral images (LR-HSI) with high-resolution multispectral images (HR-MSI) using only the MSI's spectral response function (SRF), outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: High-spatial-resolution hyperspectral images (HSI) are crucial for applications like remote sensing and medical imaging, but current methods require impractical calibration or ground truth data.

Method: SpectraLift trains a lightweight MLP network using synthetic LR-MSI from LR-HSI and SRF, optimizing with an ℓ1 spectral loss, and maps HR-MSI to HR-HSI at inference.
...
Conclusion: SpectraLift provides a practical, efficient solution for HSI-MSI fusion without needing PSF calibration or ground truth HR-HSI.
...
### [241] [UniSLU: Unified Spoken Language Understanding from Heterogeneous Cross-Task Datasets](https://arxiv.org/abs/2507.12951)
*Zhichao Sheng,Shilin Zhou,Chen Gong,Zhenghua Li*

Main category: eess.AS

TL;DR: UniSLU is a unified framework for Spoken Language Understanding (SLU) tasks like ASR, NER, and SA, improving performance by joint modeling and leveraging heterogeneous datasets.


<details>
  <summary>Details</summary>
Motivation: Existing SLU methods use separate models for tasks like NER and SA, increasing complexity and limiting cross-task interaction. UniSLU aims to unify these tasks for better efficiency and performance.

Method: Proposes a unified representation for SLU tasks and a generative method to jointly model ASR, NER, and SA, integrating with large language models.

Result: Outperforms benchmark methods on public SLU datasets, demonstrating superior performance in real-world scenarios.

Conclusion: UniSLU is effective for SLU tasks, offering a unified solution with improved performance and potential for future research.
...
### [242] [Deep Bilinear Koopman Model for Real-Time Vehicle Control in Frenet Frame](https://arxiv.org/abs/2507.12578)
*Mohammad Abtahi,Farhang Motallebi Araghi,Navid Mojahed,Shima Nazari*

Main category: eess.SY

TL;DR: A deep Koopman approach is proposed for modeling and controlling autonomous vehicle dynamics in the Frenet frame, using a neural network to learn the Koopman operator and invariant subspace. The method integrates a cumulative error regulator for real-time MPC and achieves improved tracking performance.


<details>
  <summary>Details</summary>
Motivation: The nonlinear and coupled nature of vehicle dynamics makes accurate modeling and control challenging. Koopman operator theory provides a linear control framework, but learning a finite-dimensional invariant subspace remains an open problem.

Method: A deep neural network learns the Koopman operator and invariant subspace, capturing input-state bilinear interactions while preserving convexity. A multi-step prediction loss ensures long-horizon capability, and a CER module compensates for model mismatch.

Result: Hardware-in-the-loop experiments show significant tracking error reduction compared to baselines, validating real-time suitability for embedded systems.

Conclusion: The proposed deep Koopman framework with CER enhances real-time trajectory tracking, making it viable for autonomous vehicle control.
...
### [243] [Dual LiDAR-Based Traffic Movement Count Estimation at a Signalized Intersection: Deployment, Data Collection, and Preliminary Analysis](https://arxiv.org/abs/2507.13073)
*Saswat Priyadarshi Nayak,Guoyuan Wu,Kanok Boriboonsomsin,Matthew Barth*

Main category: eess.SY

TL;DR: The paper explores using a dual-LiDAR system for Traffic Movement Count (TMC) at intersections, addressing limitations of traditional methods like cameras under poor conditions.


<details>
  <summary>Details</summary>
Motivation: To improve TMC accuracy and reliability, especially under poor lighting and weather, by leveraging LiDAR technology.

Method: Deployed a dual-LiDAR system at an intersection in Rialto, California, using 3D bounding box detections to classify vehicle counts by direction, movement, and class.

Result: Provided TMC estimates and insights into trends and irregularities, demonstrating LiDAR's potential for accurate TMC.

Conclusion: LiDAR shows promise for TMC, with potential to enhance trajectory forecasting and intent prediction at intersections.
...
### [244] [Differentially Private Conformal Prediction via Quantile Binary Search](https://arxiv.org/abs/2507.12497)
*Ogonnaya M. Romanus,Roberto Molinari*

Main category: stat.ME

TL;DR: The paper introduces P-COQS, a differentially private method for Conformal Prediction (CP), ensuring privacy in calibration datasets while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of privacy-preserving methods for CP, particularly in calibration datasets, the work aims to provide robust DP guarantees without compromising coverage.

Method: Adapts a randomized binary search algorithm for DP quantiles in CP's calibration phase, ensuring privacy in prediction sets.

Result: P-COQS achieves desired coverage and privacy, outperforming alternatives in empirical coverage, efficiency, and informativeness.

Conclusion: P-COQS is a robust, privacy-preserving CP method, effective across various datasets and settings.
...
### [245] [Optimal Empirical Risk Minimization under Temporal Distribution Shifts](https://arxiv.org/abs/2507.13287)
*Yujin Jeong,Ramesh Johari,Dominik Rothenhäusler,Emily Fox*

Main category: stat.ME

TL;DR: RIDER introduces a method for optimally weighting empirical risk minimization under temporal distribution shifts, outperforming common weighting schemes in real-world tasks.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of machine learning models in dynamically evolving environments with temporal distribution shifts.

Method: RIDER derives optimally-weighted empirical risk minimization procedures under the random distribution shift model, encompassing common weighting schemes as special cases.

Result: RIDER improves out-of-sample predictive performance on the Yearbook dataset and outperforms standard weighting strategies in stock market volatility and NYC taxi ride duration predictions.

Conclusion: RIDER provides a robust and theoretically grounded approach for handling temporal distribution shifts in machine learning.
...
### [246] [On multiagent online problems with predictions](https://arxiv.org/abs/2507.12486)
*Gabriel Istrate,Cosmin Bonchis,Victor Bogdan*

Main category: cs.MA

TL;DR: The paper explores competitive algorithms with predictions in multiagent settings, introducing a two-predictor framework and analyzing optimal competitive ratios under varying predictor qualities. A multiagent ski-rental problem illustrates the framework.


<details>
  <summary>Details</summary>
Motivation: To understand the best competitive ratios achievable in multiagent settings using predictors for self and others' behavior, addressing robustness to mispredictions.
...
Result: With perfect predictions, following the self-predictor is optimal but not robust. An alternative algorithm offers better robustness against mispredictions.
...
### [247] [FOUNDER: Grounding Foundation Models in World Models for Open-Ended Embodied Decision Making](https://arxiv.org/abs/2507.12496)
...
<details>
  <summary>Details</summary>
Motivation: To combine the generalizable knowledge of FMs with the dynamic modeling of WMs for improved task generalization in embodied environments.

Method: Learns a mapping function to ground FM representations in WM state space, enabling goal-conditioned policy learning through imagination. Uses predicted temporal distance as a reward signal.

Result: Superior performance on multi-task offline visual control benchmarks, especially in complex or domain-gap scenarios. Validated reward consistency with ground-truth.
...
### [248] [VLMgineer: Vision Language Models as Robotic Toolsmiths](https://arxiv.org/abs/2507.12644)
*George Jiayuan Gao,Tianyu Li,Junyao Shi,Yihan Li,Zizhe Zhang,Nadia Figueroa,Dinesh Jayaraman*

Main category: cs.RO

TL;DR: VLMgineer leverages vision language models and evolutionary search to co-design tools and action plans for robotics tasks, outperforming human-crafted solutions.
...
Method: Combines vision language models (VLMs) for code generation with evolutionary search to iteratively design tools and action plans.

Result: VLMgineer consistently outperforms human specifications and existing tools in solving diverse manipulation tasks innovatively.

Conclusion: Automated tool invention is feasible and effective, with VLMgineer demonstrating superior performance and innovation in robotics tasks.
...
### [249] [Rethinking the Embodied Gap in Vision-and-Language Navigation: A Holistic Study of Physical and Visual Disparities](https://arxiv.org/abs/2507.13019)
*Liuyi Wang,Xinyuan Xia,Hui Zhao,Hanqing Wang,Tai Wang,Yilun Chen,Chengju Liu,Qijun Chen,Jiangmiao Pang*

Main category: cs.RO

TL;DR: VLN-PE introduces a physically realistic VLN platform for humanoid, quadruped, and wheeled robots, evaluating existing methods and revealing performance challenges in real-world deployment.
...
Method: VLN-PE evaluates various VLN methods, including classification models, diffusion models, and LLM-path planning integration, in physical robotic settings.

Result: Performance degradation due to limited observation space, lighting variations, and physical challenges like collisions and falls. Locomotion constraints for legged robots are also highlighted.

Conclusion: VLN-PE offers a pathway for improving cross-embodiment adaptability and inspires rethinking VLN limitations for robust, practical models.
...
### [250] [Enter the Mind Palace: Reasoning and Planning for Long-term Active Embodied Question Answering](https://arxiv.org/abs/2507.12846)
...
TL;DR: The paper introduces Long-term Active Embodied Question Answering (LA-EQA), a task where robots recall past experiences and explore environments to answer complex questions. It proposes a structured memory system inspired by the mind palace method, outperforming existing approaches.


<details>
  <summary>Details</summary>
Motivation: Robots operating over long periods need to leverage accumulated knowledge for effective human assistance. Traditional EQA methods fail due to limited context, lack of persistent memory, and inability to combine memory with active exploration.

Method: A structured memory system encodes episodic experiences as scene-graph-based world instances, enabling targeted memory retrieval and guided navigation. Value-of-information-based stopping criteria balance exploration and recall.
...
Conclusion: The proposed structured memory system effectively addresses LA-EQA challenges, demonstrating significant performance gains over existing methods.
...
### [251] [ReAL-AD: Towards Human-Like Reasoning in End-to-End Autonomous Driving](https://arxiv.org/abs/2507.12499)
*Yuhang Lu,Jiadong Tu,Yuexin Ma,Xinge Zhu*

Main category: cs.RO

TL;DR: ReAL-AD enhances autonomous driving by mimicking human hierarchical reasoning, improving planning accuracy and safety by 30%.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack hierarchical reasoning like humans, limiting adaptability and performance.

Method: Uses a three-tier cognitive model (Strategy, Decision, Operation) with VLMs for situational awareness and structured reasoning. Introduces Strategic Reasoning Injector, Tactical Reasoning Integrator, and Hierarchical Trajectory Decoder.

Result: Improves planning accuracy and safety by over 30%.

Conclusion: ReAL-AD makes autonomous driving more interpretable and human-like, bridging the gap in hierarchical reasoning.
...
### [252] [GraspGen: A Diffusion-based Framework for 6-DOF Grasping with On-Generator Training](https://arxiv.org/abs/2507.13097)
...
TL;DR: GraspGen, a framework using DiffusionTransformer and an efficient discriminator, improves 6-DOF grasp generation, outperforming prior methods in simulations and real-world tasks.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based 6-DOF grasping methods struggle with generalization across different embodiments and real-world settings.

Method: GraspGen employs a DiffusionTransformer for grasp generation and a discriminator for filtering, trained with a novel on-generator recipe. A new dataset of 53M grasps is introduced.

Result: GraspGen outperforms prior methods in simulations, achieves SOTA on FetchBench, and performs well on a real robot.
...
### [253] [Physically Based Neural LiDAR Resimulation](https://arxiv.org/abs/2507.12489)
*Richard Marcus,Marc Stamminger*

Main category: cs.RO

TL;DR: A method for accurate LiDAR simulation by modeling sensor-specific effects like rolling shutter and intensity falloff, outperforming existing techniques.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR simulation methods lack accurate modeling of sensor-specific effects, limiting realism and applicability.

Method: Explicitly models LiDAR sensor characteristics (rolling shutter, laser power variations, intensity falloff) for improved simulation accuracy.

Result: Quantitative and qualitative improvements over state-of-the-art methods, with demonstrated resimulation capabilities (e.g., high-resolution LiDAR scans in camera perspective).

Conclusion: The proposed method advances LiDAR simulation accuracy and versatility, with potential applications in 3D scene reconstruction and LiDAR-based systems.
...
### [254] [Aligning Humans and Robots via Reinforcement Learning from Implicit Human Feedback](https://arxiv.org/abs/2507.13171)
*Suzie Kim,Hye-Bin Shin,Seong-Whan Lee*

Main category: cs.RO

TL;DR: Proposes RLIHF, a framework using EEG signals (ErrPs) for implicit human feedback in RL, achieving performance comparable to dense rewards in a robotic task.


<details>
  <summary>Details</summary>
Motivation: Addresses limitations of sparse rewards and explicit feedback in RLHF by leveraging non-invasive EEG signals for continuous, implicit feedback.

Method: Uses EEG signals (ErrPs) decoded into probabilistic rewards to train RL agents in a robotic pick-and-place task.

Result: Agents trained with EEG feedback perform as well as those with dense rewards.

Conclusion: RLIHF shows promise for scalable, human-aligned RL in robotics using implicit neural feedback.
...
### [255] [LaViPlan : Language-Guided Visual Path Planning with RLVR](https://arxiv.org/abs/2507.12911)
*Hayeon Oh*

Main category: cs.RO

TL;DR: LaViPlan uses Reinforcement Learning with Verifiable Rewards (RLVR) to align Vision-Language Models (VLMs) with low-level actions in autonomous driving, improving OOD scenario handling.


<details>
  <summary>Details</summary>
Motivation: Addressing the misalignment between VLMs' high-level decisions and low-level actions in autonomous driving, especially in OOD scenarios.

Method: Proposes LaViPlan, a framework using RLVR to optimize VLMs with planning-oriented metrics.

Result: Improves situational awareness and decision-making in OOD conditions, mitigating the vision-language-action misalignment.

Conclusion: LaViPlan offers a promising post-training paradigm for VLMs in autonomous driving, enhancing OOD scenario performance.
...
### [256] [Evaluating Reinforcement Learning Algorithms for Navigation in Simulated Robotic Quadrupeds: A Comparative Study Inspired by Guide Dog Behaviour](https://arxiv.org/abs/2507.13277)
*Emma M. A. Harrison*

Main category: cs.RO

TL;DR: The study evaluates three reinforcement learning algorithms (PPO, DQN, Q-learning) for training a quadruped robot in navigation and obstacle avoidance, aiming to simulate a robotic guide dog. PPO outperformed others.
...
Conclusion: The study advances robotic navigation and AI in assistive robotics, highlighting PPO's effectiveness for quadruped mobility.
...
### [257] [Latent Policy Steering with Embodiment-Agnostic Pretrained World Models](https://arxiv.org/abs/2507.13340)
*Yiqi Wang,Mrinal Verghese,Jeff Schneider*

Main category: cs.RO

TL;DR: The paper proposes a method to reduce data collection for visuomotor robot policies by leveraging existing datasets and using optic flow and a World Model, achieving significant performance improvements with minimal demonstrations.


<details>
  <summary>Details</summary>
Motivation: Reducing the expensive data collection efforts required for training visuomotor policies by utilizing existing or cost-effective datasets from diverse embodiments.

Method: Uses optic flow as an embodiment-agnostic action representation to train a World Model (WM) on multi-embodiment datasets, then finetunes it with minimal robot data. Introduces Latent Policy Steering (LPS) to enhance policy outputs by searching the WM's latent space.

Result: Achieves over 50% relative improvement with 30 demonstrations and over 20% with 50 demonstrations when combining the policy with a pretrained WM.

Conclusion: The approach effectively reduces data dependency and improves policy performance by leveraging existing datasets and innovative training methods.
...
### [258] [Quantum Transfer Learning to Boost Dementia Detection](https://arxiv.org/abs/2507.12485)
*Sounak Bhowmik,Talita Perciano,Himanshu Thapliyal*

Main category: quant-ph

TL;DR: Quantum transfer learning (QTL) improves a weak classical deep learning model for dementia detection, addressing high-dimensional data challenges and demonstrating robustness under noise.


<details>
  <summary>Details</summary>
Motivation: Early and accurate dementia detection is crucial for timely intervention, but classical methods face computational and performance limitations with high-dimensional biomedical data.

Method: Quantum transfer learning (QTL) is applied to enhance a weak classical deep learning model for binary dementia classification, tested on the OASIS 2 dataset.

Result: QTL transforms a suboptimal classical model into a more effective solution, showing potential for biomedical image classification despite noise.

Conclusion: QTL offers a promising approach for advancing dementia detection, highlighting its reliability and potential impact on healthcare technology.
...
### [259] [Sporadic Federated Learning Approach in Quantum Environment to Tackle Quantum Noise](https://arxiv.org/abs/2507.12492)
...
<details>
  <summary>Details</summary>
Motivation: Quantum noise in federated learning over quantum networks hinders training performance due to heterogeneous noise levels in quantum devices.

Method: SpoQFL leverages sporadic learning to dynamically adapt training strategies based on quantum noise fluctuations.

Result: Experiments show SpoQFL outperforms conventional QFL, achieving better training performance and stable convergence.

Conclusion: SpoQFL effectively addresses quantum noise heterogeneity, enhancing the practicality of Quantum Federated Learning.
...
### [260] [Generative Multi-Target Cross-Domain Recommendation](https://arxiv.org/abs/2507.12871)
*Jinqiu Jin,Yang Zhang,Junwei Pan,Fuli Feng,Hua Lu,Haijie Gu,Xiangnan He*

Main category: cs.IR

TL;DR: The paper introduces GMC, a generative approach for Multi-Target Cross-Domain Recommendation (MTCDR), using semantic identifiers and a unified sequence-to-sequence model to enhance performance.


<details>
  <summary>Details</summary>
Motivation: Existing MTCDR methods rely on domain-shared entities or require extensive auxiliary data, limiting their applicability. GMC addresses these gaps by leveraging generative techniques.

Method: GMC uses semantic item identifiers and a domain-unified sequence-to-sequence model, incorporating domain-aware contrastive loss and domain-specific fine-tuning.

Result: Experiments on five datasets show GMC outperforms baseline methods.

Conclusion: GMC provides an effective solution for MTCDR by integrating generative paradigms and domain-specific enhancements.
...
### [261] [Bridging the Gap: Leveraging Retrieval-Augmented Generation to Better Understand Public Concerns about Vaccines](https://arxiv.org/abs/2507.12840)
*Muhammad Javed,Sedigh Khademi Habibabadi,Christopher Palmer,Hazel Clothier,Jim Buttery,Gerardo Luis Dimaguila*

Main category: cs.IR

TL;DR: A tool (VaxPulse Query Corner) using Retrieval Augmented Generation addresses vaccine hesitancy by analyzing social media posts, achieving high faithfulness and relevance scores.


<details>
  <summary>Details</summary>
Motivation: Vaccine hesitancy is a public health threat, and traditional methods like topic modeling fail to capture nuanced opinions. LLMs also miss current events and may hallucinate, compromising communication.

Method: Developed VaxPulse Query Corner using Retrieval Augmented Generation to analyze public vaccine concerns on social media.

Result: Analyzed 35,103 Shingrix posts, achieving answer faithfulness (0.96) and relevance (0.94).

Conclusion: The tool aids public health administrators in understanding concerns and implementing targeted interventions to boost vaccine confidence.
...
### [262] [Search for Z/2 eigenfunctions on the sphere using machine learning](https://arxiv.org/abs/2507.13122)
...
Method: A multivalued feedforward deep neural network was developed and implemented using the JAX library. Branch points were fixed or dynamically adjusted.

Result: Z/2 eigenfunctions were found for three cases: fixed branch points at tetrahedron and cube vertices, and dynamically positioned at a squashed tetrahedron.

Conclusion: The approach successfully identified Z/2 eigenfunctions, demonstrating the potential of machine learning in solving such mathematical problems.
...
### [263] [Perfect diffusion is $\mathsf{TC}^0$ -- Bad diffusion is Turing-complete](https://arxiv.org/abs/2507.12469)
*Yuxi Liu*

Main category: cs.CC

TL;DR: The paper analyzes the computational complexity of diffusion-based language models, showing a dichotomy: exact score-matching limits models to the 𝖳𝖢⁰ class, while no such restriction allows Turing machine simulation.


<details>
  <summary>Details</summary>
Motivation: To understand the theoretical capabilities and limitations of diffusion models in language modeling, especially for sequential tasks.

Method: Proves a dichotomy based on score-matching network quality: exact matching restricts to 𝖳𝖢⁰, while no matching enables Turing machine simulation.

Result: Exact score-matching confines models to 𝖳𝖢⁰; without it, Turing machine simulation is possible.

Conclusion: The dichotomy highlights trade-offs in diffusion models, suggesting architectures blending sequential and parallel modes could outperform current models.
...
### [264] [Computational-Statistical Tradeoffs from NP-hardness](https://arxiv.org/abs/2507.13222)
*Guy Blanc,Caleb Koch,Carmen Strassle,Li-Yang Tan*

Main category: cs.CC

TL;DR: The paper explores computational-statistical tradeoffs in PAC learning, linking them to NP-hardness and proving sharp tradeoffs under exponential-time assumptions. It also characterizes RP vs. NP in terms of learning efficiency.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between computational efficiency and information-theoretic limits in statistical problems, particularly in PAC learning, by grounding tradeoffs on worst-case assumptions like NP-hardness.

Method: The study uses NP-hardness assumptions to derive computational-statistical tradeoffs, focusing on sample complexity and time-efficient learning. It also examines the relationship between RP and NP in learning contexts.

Result: Sharp tradeoffs are shown under exponential-time assumptions, and a characterization of RP vs. NP is provided, with implications for learning efficiency. Lower bounds hold even against improper learners.

Conclusion: The work establishes NP-hardness results for improper learning, overcoming prior barriers, and deepens understanding of computational-statistical tradeoffs in learning theory.
...
### [265] [Refining Coarse-Grained Molecular Topologies: A Bayesian Optimization Approach](https://arxiv.org/abs/2501.02707)
...
TL;DR: A novel Bayesian Optimization method refines Martini3 CGMD topologies for domain-specific accuracy, bridging the gap between efficiency and AA-MD precision.


<details>
  <summary>Details</summary>
Motivation: High computational costs of All-Atom MD and lack of specialization in existing CGMD methods like Martini3 limit accuracy for domain-specific applications.

Method: Bayesian Optimization refines bonded interaction parameters in Martini3 CGMD topologies to enhance accuracy while retaining computational efficiency.

Result: Developed a validated CG potential for any polymerization degree, achieving AA-MD-like accuracy with CGMD efficiency.

Conclusion: This approach advances multiscale simulations, enabling cost-effective molecular discovery across domains.
...
TL;DR: The paper introduces a low-power CGRA architecture to accelerate GEMM operations in transformer models for edge devices, optimizing computation and memory usage.


<details>
  <summary>Details</summary>
Motivation: Transformers are computationally demanding, making deployment on low-power edge devices challenging. This work aims to address this by designing an efficient architecture.

Method: Proposes a 4x4 PE array and 4x2 MOBs for parallel computation and optimized memory operations, using a switchless mesh torus interconnect to reduce power and latency.

Result: The architecture efficiently handles transformer computations, reducing memory bandwidth and enhancing data reuse for edge applications.

Conclusion: The CGRA design provides a scalable solution for deploying advanced ML models like transformers on resource-constrained edge devices.
...
### [267] [The carbon cost of materials discovery: Can machine learning really accelerate the discovery of new photovoltaics?](https://arxiv.org/abs/2507.13246)
*Matthew Walker,Keith T. Butler*

Main category: cond-mat.mtrl-sci

TL;DR: This study develops machine learning surrogates to replace computationally expensive DFT calculations in photovoltaic material screening, demonstrating that ML models can reduce CO2 emissions while maintaining competitive accuracy for predicting solar cell efficiency limits.


<details>
  <summary>Details</summary>
Motivation: Traditional DFT-based computational screening for photovoltaic materials is powerful but computationally expensive and environmentally costly due to high CO2 emissions. There's a need for more efficient approaches that maintain predictive accuracy while reducing environmental impact.

Method: The researchers progressively replaced components of a canonical DFT-based workflow with machine learning surrogates, quantifying CO2 emissions for each computational strategy. They evaluated hybrid ML/DFT approaches and assessed trade-offs between predictive accuracy and environmental cost, focusing on direct scalar prediction versus intermediate absorption spectra prediction.

Result: Multiple hybrid ML/DFT strategies were identified that optimize different points on the accuracy-emissions spectrum. Direct prediction of maximum efficiency was more tractable than using predicted absorption spectra. ML models trained on DFT data outperformed DFT workflows using alternative exchange-correlation functionals in screening applications.

Conclusion: The study provides a quantitative framework for building low-emission, high-throughput discovery pipelines for photovoltaic materials. ML surrogates can effectively replace expensive DFT calculations while maintaining or even improving screening performance, offering significant environmental benefits.
...
### [268] [Autonomous Resource Management in Microservice Systems via Reinforcement Learning](https://arxiv.org/abs/2507.12879)
*Yujun Zou,Nia Qi,Yingnan Deng,Zhihao Xue,Ming Gong,Wuyang Zhang*

Main category: cs.DC

TL;DR: A reinforcement learning-based method for microservice resource scheduling improves resource allocation, latency, and throughput compared to traditional static methods.


<details>
  <summary>Details</summary>
Motivation: Addressing uneven resource allocation, high latency, and insufficient throughput in microservice architectures as service numbers and loads increase.

Method: Uses a reinforcement learning-based intelligent scheduling algorithm to optimize resource allocation through agent-environment interaction.

Result: Significant improvements in response speed, throughput, resource utilization, and energy consumption under varying load and concurrency conditions.

Conclusion: The reinforcement learning model outperforms static methods, offering adaptability and real-time optimization in dynamic environments.
...
### [269] [Implementation and Analysis of GPU Algorithms for Vecchia Approximation](https://arxiv.org/abs/2407.02740)
*Zachary James,Joseph Guinness*

Main category: stat.CO

TL;DR: The paper introduces a GPU-accelerated implementation of Vecchia Approximation for Gaussian Processes, outperforming existing methods in speed and accuracy.
...
Result: The new method outperforms others in runtime and predictive accuracy, demonstrated on datasets including a large spatial-temporal one.

Conclusion: The GpGpU R package, featuring the optimized method, offers superior performance for large-scale Gaussian Process modeling.
...
### [270] [Task-Specific Audio Coding for Machines: Machine-Learned Latent Features Are Codes for That Machine](https://arxiv.org/abs/2507.12701)
*Anastasia Kuznetsova,Inseon Jang,Wootaek Lim,Minje Kim*

Main category: cs.SD

TL;DR: An efficient audio coding for machines (ACoM) method is introduced, focusing on compression and downstream task performance rather than human perception. It uses task-specific loss and RVQ for ultra-low bitrates (<200 bps) with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Prioritize efficient compression and downstream task performance over perceptual nuances in audio coding for machines.

Method: Uses task-specific loss guidance and residual vector quantization (RVQ) to compress and quantize intermediate features of trained models.
...
Conclusion: The method is effective for tasks like speech recognition and audio classification, with potential for broader applicability.
...
### [271] [Evaluation of Neural Surrogates for Physical Modelling Synthesis of Nonlinear Elastic Plates](https://arxiv.org/abs/2507.12563)
*Carlos De La Vega Martin,Rodrigo Diaz Fernandez,Mark Sandler*

Main category: cs.SD

TL;DR: The paper compares neural network-based methods for simulating nonlinear elastic plate vibrations, highlighting their limitations and proposing future improvements for real-time audio synthesis.


<details>
  <summary>Details</summary>
Motivation: Traditional numerical methods for simulating vibrating plates are accurate but computationally expensive, making them unsuitable for real-time audio applications. Neural networks offer a potential alternative.

Method: The study evaluates state-of-the-art neural network models trained on short sequences for long-sequence prediction in an autoregressive manner.

Result: The analysis reveals limitations of neural models, emphasizing that time-domain prediction error alone is insufficient for evaluation.

Conclusion: The paper suggests future directions to enhance neural approaches for modeling nonlinear vibrations in real-time audio synthesis.
...
### [272] [Sample-Constrained Black Box Optimization for Audio Personalization](https://arxiv.org/abs/2507.12773)
*Rajalaxmi Rajagopalan,Yu-Lin Wei,Romit Roy Choudhury*

Main category: cs.SD

TL;DR: The paper proposes a hybrid querying approach using Sparse Gaussian Process Regression to personalize audio by maximizing user satisfaction, combining direct filter scoring and individual element feedback.


<details>
  <summary>Details</summary>
Motivation: To enhance user experience by optimizing audio personalization, addressing the challenge of unknown satisfaction functions in black-box optimization.

Method: Uses Sparse Gaussian Process Regression (GPR) with hybrid querying (scoring full filters and individual filter elements) to iteratively refine the optimal filter.
...
Conclusion: Hybrid querying outperforms single-type querying, offering a novel approach for black-box optimization with broader applications beyond audio.
...
### [273] [SHIELD: A Secure and Highly Enhanced Integrated Learning for Robust Deepfake Detection against Adversarial Attacks](https://arxiv.org/abs/2507.13170)
*Kutub Uddin,Awais Khan,Muhammad Umar Farooq,Khalid Malik*

Main category: cs.SD

TL;DR: SHIELD is a novel collaborative learning method to detect and defend against generative adversarial network-based deepfake audio attacks, improving robustness and accuracy.


<details>
  <summary>Details</summary>
Motivation: Deepfake audio poses risks like misinformation. Existing detection methods are vulnerable to anti-forensic attacks, necessitating a robust defense mechanism.
...
Result: SHIELD reduces detection accuracy drops caused by AF attacks and achieves high accuracy (98-99%) in defending against such attacks across datasets.

Conclusion: SHIELD effectively counters generative AF attacks, ensuring robust audio deepfake detection.
...
### [274] [Autoregressive Speech Enhancement via Acoustic Tokens](https://arxiv.org/abs/2507.12825)
*Luca Della Libera,Cem Subakan,Mirco Ravanelli*

Main category: cs.SD

TL;DR: The paper explores acoustic tokens for speech enhancement, comparing them to semantic tokens and introducing an autoregressive model to improve performance. Results show acoustic tokens preserve speaker identity better but still lag behind continuous representations.


<details>
  <summary>Details</summary>
Motivation: Address gaps in speech enhancement research, particularly the limited focus on discrete representations (acoustic tokens) and the overlooked potential of autoregressive models.

Method: 1) Study acoustic tokens' performance, including bitrate and noise effects. 2) Propose a novel autoregressive transducer-based architecture.

Result: Acoustic tokens outperform semantic tokens in preserving speaker identity, and the autoregressive model further enhances performance. However, discrete representations still underperform compared to continuous ones.

Conclusion: Discrete representations show promise but require further research to match continuous methods in speech enhancement.
...
### [275] [Voxtral](https://arxiv.org/abs/2507.13264)
...
TL;DR: Voxtral Mini and Small are multimodal audio chat models excelling in audio and text comprehension, outperforming closed-source models and supporting long audio and conversations.


<details>
  <summary>Details</summary>
Motivation: To develop efficient multimodal models capable of handling both audio and text with state-of-the-art performance.

Method: Training Voxtral models to comprehend spoken audio and text, with a 32K context window for long audio and conversations.

Result: Voxtral Small outperforms closed-source models and handles 40-minute audio files. Three new benchmarks for speech understanding are introduced.

Conclusion: Voxtral models, released under Apache 2.0, offer advanced multimodal capabilities and accessibility for local use.
...
### [276] [Complex non-backtracking matrix for directed graphs](https://arxiv.org/abs/2507.12503)
*Keishi Sando,Hideitsu Hino*

Main category: math.CO

TL;DR: The paper introduces a complex non-backtracking matrix for directed graphs, combining properties of Hermitian adjacency and non-backtracking matrices, and shows its utility for clustering sparse directed graphs.


<details>
  <summary>Details</summary>
Motivation: To enhance graph data analysis by integrating Hermitian adjacency and non-backtracking matrices for directed graphs.

Method: Proposes the complex non-backtracking matrix, analyzes its properties, and compares it with Hermitian adjacency matrices.

Result: Demonstrates that the proposed matrix retains cluster information, especially in sparse directed graphs.

Conclusion: The complex non-backtracking matrix is a promising tool for analyzing directed graphs, particularly for clustering tasks.
...
TL;DR: The paper introduces a framework for transforming football data into object-centric event logs with a spatial dimension, demonstrating its effectiveness and discussing results for various process representations.
...
Method: Developed a framework to convert football data into object-centric event logs, enhanced with spatial information, and tested it on real-world data.

Result: Successfully generated object-centric event logs from football data, showcasing their potential for complex process analysis.

Conclusion: The study provides a pioneering example of object-centric event logs in football analytics, suggesting future work on variant analysis and filtering techniques.
...
TL;DR: The paper introduces rel-HNN, a hypergraph-based framework for capturing fine-grained relational semantics in RDBs, outperforming existing methods and achieving scalability via multi-GPU training.


<details>
  <summary>Details</summary>
Motivation: Relational databases (RDBs) are widely used, but deep learning models struggle with their structured nature. Existing GNNs oversimplify relational structures by ignoring intra-tuple relationships.

Method: Proposes rel-HNN, modeling attribute-value pairs as nodes and tuples as hyperedges, with a split-parallel training algorithm for multi-GPU scalability.

Result: rel-HNN outperforms existing methods in classification and regression tasks, with training speedups up to 3.18x for relational data and 2.94x for hypergraph learning.

Conclusion: The rel-HNN framework effectively captures relational semantics and scales efficiently, offering superior performance and speed for learning on RDBs.
...
TL;DR: The paper presents a COVID-19 forecasting case study in Cyprus, using a two-year dataset to analyze infection trends, forecasting performance, and external factors' influence.


<details>
  <summary>Details</summary>
Motivation: To leverage pandemic data for improving forecasting models and enhancing preparedness for future outbreaks.

Method: A large-scale case study integrating epidemiological data, vaccination records, policy measures, and weather conditions.

Result: Insights into infection trends and forecasting performance, contributing to better pandemic strategies.

Conclusion: The study aids in refining public health responses and mitigating socio-economic impacts of future pandemics.
...
### [280] [Tensor-Tensor Products, Group Representations, and Semidefinite Programming](https://arxiv.org/abs/2507.12729)
...
### [281] [Unsupervised Ground Metric Learning](https://arxiv.org/abs/2507.13094)
*Janis Auffenberg,Jonas Bresch,Oleh Melnyk,Gabriele Steidl*

Main category: math.OC

TL;DR: The paper addresses unsupervised metric learning by simultaneously learning optimal transport cost matrices and features, proposing a stochastic algorithm with linear convergence and exploring alternative distances like Mahalanobis and graph Laplacians.


<details>
  <summary>Details</summary>
Motivation: The challenge of data classification without labeled samples and the need for effective distance metrics in unsupervised learning.

Method: Simultaneous learning of optimal transport cost matrices and features, using stochastic random function iteration, and exploring Mahalanobis-like distances and graph Laplacians.

Result: Proposed algorithm converges linearly despite non-paracontractive operators; alternative distances like Mahalanobis and graph Laplacians are viable.

Conclusion: The approach advances unsupervised metric learning with flexible distance metrics and efficient algorithms.
...
### [282] [Stochastic Weakly Convex Optimization Under Heavy-Tailed Noises](https://arxiv.org/abs/2507.13283)
*Tianxi Zhu,Yi Xu,Xiangyang Ji*

Main category: math.OC

TL;DR: The paper analyzes stochastic first-order methods (SFOMs) under heavy-tailed gradient noises, focusing on sub-Weibull and $p$-BCM noises. It examines high-probability convergence for weakly convex objectives, showing robustness in theoretical dependencies but noting limitations in sample complexity.


<details>
  <summary>Details</summary>
Motivation: To address gaps in understanding the convergence of SFOMs under heavy-tailed gradient noises for weakly convex objectives, which include non-convex and non-smooth cases.

Method: Investigates vanilla stochastic subgradient descent (SsGD) under sub-Weibull noises and clipped SsGD under $p$-BCM noises, analyzing high-probability and in-expectation convergence.

Result: Vanilla SsGD's theoretical dependence on failure probability and iterations remains robust under sub-Weibull noises. Clipped SsGD's dependence on failure probability is unaffected by non-smoothness/non-convexity, but sample complexity is worse than lower bounds for smooth cases.

Conclusion: The study provides insights into SFOM convergence under heavy-tailed noises for weakly convex objectives, highlighting theoretical robustness but identifying limitations in sample complexity for $p$-BCM noises.
...
### [283] [Coarse Addition and the St. Petersburg Paradox: A Heuristic Perspective](https://arxiv.org/abs/2507.12475)
*Takashi Izumo*

Main category: econ.TH

TL;DR: The paper proposes a cognitive-based solution to the St. Petersburg paradox by using coarse perceptual categories for numerical values, leading to inertial stabilization in repeated additions.


<details>
  <summary>Details</summary>
Motivation: The paradox challenges decision theory with its infinite expected value, and traditional solutions lack cognitive realism. This work aims to align the solution with human perceptual limitations.

Method: Introduces a modified addition operation over coarse partitions of outcomes, grouping values into perceptual categories and replacing them with representative elements.

Result: Demonstrates that the St. Petersburg series can stabilize under this coarse addition, offering a cognitively plausible resolution.

Conclusion: The framework provides a realistic approach for handling divergent rewards under cognitive limits, with potential applications in behavioral and machine reasoning studies.
...
### [284] [HairFormer: Transformer-Based Dynamic Neural Hair Simulation](https://arxiv.org/abs/2507.12600)
*Joy Xiaoji Zhang,Jingsen Zhu,Hanyu Chen,Steve Marschner*

Main category: cs.GR

TL;DR: A two-stage neural solution using Transformers generalizes hair dynamics across styles, shapes, and motions, resolving penetrations and enabling real-time inference.


<details>
  <summary>Details</summary>
Motivation: Simulating hair dynamics that generalize across diverse hairstyles, body shapes, and motions is a significant challenge.

Method: A Transformer-based static network predicts draped shapes, resolving penetrations, while a dynamic network with cross-attention fuses static features with kinematics for expressive motion.

Result: The method achieves high-fidelity, generalizable hair dynamics, handles complex styles, and allows real-time inference.

Conclusion: The approach successfully generalizes hair dynamics, resolving penetrations and enabling efficient fine-tuning for challenging motions.
