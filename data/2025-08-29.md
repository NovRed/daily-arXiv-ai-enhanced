...
Conclusion: The proposed method provides a simple yet effective approach to mitigate hallucinations in MLLMs without requiring complex synthetic data generation pipelines or proprietary models.
...
<details>
  <summary>Details</summary>
Motivation: Existing image forgery localization methods struggle with emerging manipulation technologies due to reliance on costly annotated data, while multimodal large models like SD offer new opportunities.

Method: Leverages SD3's multimodal framework by treating image forgery residuals (high-frequency signals) as explicit modality, fusing them into latent space while preserving original semantic features.
...
Conclusion: The integration of SD's generation and perceptual capabilities provides an efficient and accurate solution for image forgery localization without requiring annotated data, showing strong generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: AI models show promise in skin disease diagnosis but lack interpretability needed for clinical use. MLLMs offer natural language reasoning while quantitative attributes provide clinically grounded explanations.

Method: Fine-tune MLLMs to predict quantitative lesion attributes (e.g., lesion area) from images and evaluate grounding through attribute-specific content-based image retrieval using SLICE-3D dataset.

Result: Evidence shows MLLM embedding spaces can be effectively grounded in quantitative attributes, enabling more interpretable predictions.

Conclusion: Combining MLLMs with quantitative attributes provides a promising approach for improving interpretability of AI-based skin disease diagnosis systems.
...
Method: Uses ViT encoder with lightweight convolutional decoder and linear classifier; reconstruction branch maps augmented signals back to originals to preserve fine-grained I/Q structure; combines supervised, self-supervised, and reconstruction objectives.
...
<details>
  <summary>Details</summary>
Motivation: Existing audio-driven human animation methods suffer from error accumulation leading to identity drift, color shifts, scene instability, and poorly modeled hand movements that don't align well with audio.
...
Conclusion: InfinityHuman successfully addresses key challenges in audio-driven human animation by leveraging pose-guided refinement and hand-specific rewards, producing high-quality, stable, and audio-synchronized videos.
...
<details>
  <summary>Details</summary>
Motivation: The lack of comprehensive datasets for 360-degree audio-visual saliency prediction and the need to address spherical distortion and spatial audio integration in omnidirectional videos.

Method: Proposed SalViT360 (vision-transformer-based framework with spherical geometry-aware attention) and SalViT360-AV (with transformer adapters conditioned on audio input), using a new dataset YT360-EyeTracking with 81 ODVs under varying audio-visual conditions.

Result: Both SalViT360 and SalViT360-AV significantly outperform existing methods in predicting viewer attention across multiple benchmark datasets, including their own YT360-EyeTracking dataset.

Conclusion: Integrating spatial audio cues is crucial for accurate saliency prediction in omnidirectional videos, and the proposed models effectively leverage both visual and audio information for improved performance.
...
<details>
  <summary>Details</summary>
Motivation: Current vision model development focuses on performance metrics like accuracy and mAP, with limited attention to explainability. Existing xAI methods mainly explain models sample-by-sample, leaving general model behavior on large datasets underexplored, which is crucial for preventing biased judgments and identifying model patterns.

Method: Proposes a pipeline leveraging Vision-Language Models to provide explanations for vision models at both sample level (individual explanations) and dataset level (general behavior analysis after running on large datasets).

Result: The pipeline enables discovery of failure cases and provides insights into vision models with minimal effort, facilitating integration of vision model development with xAI analysis.

Conclusion: This approach advances image analysis by combining vision model development with comprehensive explainability analysis, helping researchers understand model behavior patterns and prevent biases through both sample-specific and dataset-level explanations.
...
Method: Proposes ATMS-KD framework combining adaptive temperature scheduling with mixed-sample augmentation. Uses MobileNetV3 Large teacher model (5.7M params) to train three student configurations: Compact (1.3M), Standard (2.4M), and Enhanced (3.8M) residual CNNs on Damask rose dataset from Morocco.

Result: All student models achieved validation accuracies exceeding 96.7% (vs 95-96% with direct training). Outperformed 11 established KD methods, achieving 97.11% accuracy with compact model (1.60pp improvement) and lowest inference latency of 72.19ms. Knowledge retention rates exceeded 99% for all configurations.
...
<details>
  <summary>Details</summary>
Motivation: To address the bottleneck in rapid qualification of advanced materials from non-conventional manufacturing processes by linking microstructure informatics with expert characterization knowledge.

Method: Integrates deep semantic segmentation with pre-trained multi-modal models (CLIP and FLAVA) to create shared representations of visual microstructural data and textual expert assessments. Uses customized similarity-based representation with positive/negative references and net similarity scoring for zero-shot classification.

Result: Successfully distinguishes between acceptable and defective samples in additively manufactured metal matrix composites. FLAVA shows higher visual sensitivity while CLIP provides consistent textual alignment. Z-score normalization enables effective classification.

Conclusion: The framework enhances traceability and interpretability in qualification pipelines, enables human-in-the-loop decision-making without retraining, and advances semantic interoperability for scalable domain-adaptable qualification strategies.
...
Method: Adapted MedNeXt-L-k5 (Transformer-inspired 3D encoder-decoder CNN) for PVS segmentation. Trained two models: one on 200 homogeneous T2w MRI scans from HCP-Aging, another on 40 heterogeneous T1w MRI volumes from seven studies across six scanners. Evaluated using 5-fold cross validation and leave-one-site-out cross validation.

Result: T2w model achieved voxel-level Dice score of 0.88±0.06 (WM), comparable to inter-rater reliability and highest reported. T1w model achieved lower Dice of 0.58±0.09 (WM). LOSOCV showed Dice scores of 0.38±0.16 (WM) and 0.35±0.12 (BG) at voxel level, and 0.61±0.19 (WM) and 0.62±0.21 (BG) at cluster level.

Conclusion: MedNeXt-L-k5 provides efficient automated PVS segmentation across diverse T1w/T2w MRI datasets but did not outperform nnU-Net, indicating transformer attention mechanisms are not required for high accuracy in PVS segmentation.
...
Method: Feedback-driven self-adaptive framework that uses output predictions as spatial coherence prior. Includes attention isolation, confidence-based pruning for sparse adaptation, and adaptation ensemble modules to feedback output coherence cues to intermediate attention.

Result: Consistently improves performance across eight benchmarks when integrated into four state-of-the-art approaches with three backbones (ViT-B, ViT-L, ViT-H) and multiple attention types (Q-K, self-self, Proxy with MAE, SAM, DINO).

Conclusion: The proposed plug-in module effectively enhances semantic consistency between internal representations and final predictions by leveraging output coherence as a stronger spatial prior, addressing CLIP's localization limitations for open-vocabulary segmentation.
...
Method: Train linear classifiers to predict fine-grained visual categories from token embeddings at each layer using standardized anchor questions, evaluated under three types of controlled prompt variations (lexical, semantic negation, and output format variants).

Result: Identified consistent stage-wise structure across LLaVA-1.5, LLaVA-Next-LLaMA-3, and Qwen2-VL models. Early layers perform visual grounding, middle layers handle lexical integration and semantic reasoning, and final layers prepare task-specific outputs. Stage structure remains stable but layer allocation shifts with base LLM architecture changes.

Conclusion: Provides a unified perspective on MLLM layer organization and offers a lightweight, model-agnostic approach for analyzing multimodal representation dynamics.
...
TL;DR: SLiCS method disentangles CLIP embeddings into concept-specific subspaces using supervised dictionary learning for precise concept-filtered image retrieval.


<details>
  <summary>Details</summary>
Motivation: To separate complex scene information in vision-language embedding spaces by decomposing embeddings into distinct concept-specific components for better downstream task performance.

Method: Supervised dictionary learning with sparse non-negative combinations of grouped vectors, using alternating optimization with convergence guarantees. Leverages text co-embeddings for semantic descriptions and zero-shot classification.

Result: Improved precision in concept-filtered image retrieval and conditional generation across CLIP, TiTok autoencoder, and DINOv2 embeddings.

Conclusion: SLiCS successfully disentangles embedding spaces into interpretable concept subspaces, enabling more precise concept-based retrieval and generation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Medical VLMs present serious security risks including PHI exposure and data leakage, requiring safeguards for clinical and research use in healthcare environments.

Method: Developed a graphical user interface toolkit that allows manual model selection without programming, supports plug-and-play deployment with Hugging Face integration, and ensures privacy-preserving inference through Docker-orchestrated deployment on local workstations.

Result: Evaluated 5 state-of-the-art VLMs with board-certified pathologists, generating 1015 clinician-model scoring events across colon and renal cases, revealing limitations like off-target answers, vague reasoning, and inconsistent terminology.

Conclusion: MedFoundationHub provides a secure, accessible solution for deploying medical VLMs while expert evaluation highlights current model limitations that need addressing for reliable clinical use.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-task dense prediction methods face a trade-off between achieving sufficient cross-task interaction (which is crucial for success) and maintaining computational efficiency. Sufficient interaction often leads to high computational complexity.

Method: Proposes BIM with two novel scanning mechanisms: 1) Bidirectional Interaction Scan (BI-Scan) that constructs task-specific representations as bidirectional sequences with task-first and position-first scanning modes, and 2) Multi-Scale Scan (MS-Scan) for multi-granularity scene modeling to meet diverse task requirements.

Result: Extensive experiments on NYUD-V2 and PASCAL-Context benchmarks show BIM's superiority over state-of-the-art competitors.

Conclusion: BIM successfully addresses the interaction-efficiency trade-off in multi-task dense prediction through innovative bidirectional scanning mechanisms while maintaining linear computational complexity.
...
<details>
  <summary>Details</summary>
Motivation: Existing visual editing methods struggle with complex scenarios that textual guidance alone cannot adequately describe, highlighting the need for additional non-text editing prompts like audio.

Method: Leverages pre-trained multi-modal encoder with zero-shot capabilities, integrates diverse audio into visual editing by aligning audio encoder space with diffusion model's prompt encoder space, and uses separate noise branching with adaptive patch selection for multi-modal prompts.

Result: The framework excels in handling complicated editing scenarios by incorporating rich information from audio, outperforming text-only approaches across diverse editing tasks.

Conclusion: The proposed audio-guided visual editing framework successfully addresses complex editing scenarios that text-only methods fail to handle, demonstrating strong generalization without requiring additional training.
...
<details>
  <summary>Details</summary>
Motivation: Fully annotating large-scale multi-label datasets is costly and impractical, while existing SPML methods suffer from inaccuracies and false negatives when treating missing labels as unknown/negative.
...
Result: Extensive experiments on four benchmark datasets demonstrate significant advancements in multi-label classification with state-of-the-art performance.

Conclusion: The proposed AEVLP framework effectively addresses challenges in single positive multi-label learning by combining robust loss functions and dynamic pseudo-labeling strategies.
...
Conclusion: The tdIF neuron maintains energy consumption comparable to traditional IF neurons while enabling spikes to exhibit distinct temporal properties rather than relying solely on frequency-based representations, making it suitable for high-performance visual detection with ultra-low latency.
...
Method: Uses dynamic uncertainty graph convolution for uncertainty propagation, multimodal collaborative fusion with learnable gating weights for RGB/depth/edge features, and multi-scale loss optimization.
...
<details>
  <summary>Details</summary>
Motivation: Existing MVPD methods struggle with pedestrians having consistently small/large scales within views or vastly different scales between views due to not exploiting multi-scale image features for BEV generation.

Method: Generates multi-scale BEV features by projecting multi-scale image features from individual views into BEV space scale-by-scale, then processes them using a feature pyramid network to combine information across different scales and views.

Result: Extensive experiments show significant detection performance improvement, outperforming previous highest MODA by 4.5 points on GMVD dataset.
...
<details>
  <summary>Details</summary>
Motivation: Existing deepfake detectors achieve high accuracy but are computationally expensive, making them unsuitable for real-time applications like video conferencing and social media.
...
Conclusion: SFMFNet provides an effective lightweight solution for real-time deepfake detection with practical value for real-world applications.
...
Method: Uses dual-model weight selection from large pretrained models to initialize two lightweight models, applies self-knowledge distillation for knowledge transfer, and fine-tunes for target classification tasks.
...
<details>
  <summary>Details</summary>
Motivation: High-precision LiDAR scans create substantial storage and transmission overhead, and existing methods struggle with efficient context modeling due to extreme sparsity of geometric details, limiting compression performance and speed.

Method: Proposes two lightweight modules: 1) Geometry Re-Densification Module that re-densifies sparse geometry, extracts features at denser scale, then re-sparsifies for predictive coding; 2) Cross-scale Feature Propagation Module that uses multi-resolution occupancy cues to guide hierarchical feature propagation across scales.
...
<details>
  <summary>Details</summary>
Motivation: Overcome 3D data scarcity by utilizing abundant video data that contains spatial consistency priors and rich semantic information as an alternative supervisory signal for 3D generation.

Method: Introduce Droplet3D-4M, the first large-scale video dataset with multi-view annotations, and train Droplet3D generative model supporting both image and dense text input.

Result: Extensive experiments show the approach produces spatially consistent and semantically plausible 3D content, with potential for extension to scene-level applications.

Conclusion: Commonsense priors from videos significantly facilitate 3D creation, providing an effective solution to 3D data scarcity while enabling high-quality 3D asset generation.
...
Conclusion: G^2Editor provides a unified framework for photorealistic and precise object editing in driving scenarios, benefiting downstream data-driven tasks for autonomous driving systems.
...
<details>
  <summary>Details</summary>
Motivation: Accurate fetal brain segmentation is crucial for neurodevelopment assessment, but rare conditions like corpus callosum dysgenesis (CCD) severely limit annotated data, hindering deep learning model generalization.
...
<details>
  <summary>Details</summary>
Motivation: To make communication technologies accessible to deaf and hard-of-hearing individuals by integrating visual alternatives (sign language and lip reading) with audio in a unified system, as current ASR systems remain inaccessible to this population.
...
Result: Achieved performance on par with or better than task-specific state-of-the-art models across Sign Language Translation (SLT), Visual Speech Recognition (VSR), Automatic Speech Recognition (ASR), and Audio-Visual Speech Recognition (AVSR). Explicit modeling of lip movements significantly improved SLT performance.

Conclusion: The unified framework successfully integrates multiple communication modalities and demonstrates that explicit modeling of lip movements enhances sign language comprehension, providing an effective solution for audio-less communication accessibility.
...
Conclusion: Video-MTR advances state-of-the-art in long video understanding by enabling iterative reasoning with end-to-end training, eliminating the need for external VLMs.
...
<details>
  <summary>Details</summary>
Motivation: Existing TTA methods fail to address the dual uncertainty (semantic and geometric) inherent in monocular 3D object detection, which deteriorates reliability under real-world domain shifts in safety-critical applications like autonomous driving.
...
Conclusion: DUO effectively bridges the gap in handling dual uncertainties for robust monocular 3D object detection under domain shifts through joint optimization of semantic and geometric uncertainties.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning studies haven't quantitatively established the relationship between swing posture and ball trajectory, limiting actionable insights for golfers.

Method: Created CaddieSet dataset with joint information extracted from swing videos segmented into 8 phases using computer vision, plus 15 expert-defined key swing metrics.

Result: Demonstrated feasibility for predicting ball trajectories using benchmarks, with interpretable models showing swing feedback quantitatively consistent with domain knowledge.

Conclusion: CaddieSet provides new insights for golf swing analysis in both academia and sports industry by enabling interpretable relationship analysis between swing posture and outcomes.
...
Method: Importance-Aware Ensemble Network (IAENet) with IAF module that dynamically assesses and reweights anomaly scores from 2D and 3D experts, guided by specialized loss functions.

Result: Achieves new state-of-the-art on MVTec 3D-AD with markedly lower false positive rate, demonstrating practical value for industrial deployment.

Conclusion: The proposed importance-aware fusion effectively combines 2D and 3D expertise while preserving unique strengths, bridging the gap in 3D anomaly detection foundation models.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in semantic image editing where inversion-based methods introduce reconstruction errors and instruction-based models suffer from limited dataset quality and scale.

Method: Proposes DescriptiveEdit framework that treats image editing as reference-image-based text-to-image generation. Uses Cross-Attentive UNet with attention bridges to inject reference image features into the generation process without architectural modifications.
...
Conclusion: DescriptiveEdit provides a more scalable approach to semantic image editing that preserves generative power of well-trained text-to-image models while overcoming dataset quality limitations.
...
<details>
  <summary>Details</summary>
Motivation: Existing CTTA methods rely on pseudo-labels from model predictions, but these labels are noisy and lead to error accumulation. Without source data, models can develop learning biases by focusing only on target domain features.

Method: Proposes dual classifiers to disentangle semantic-related and domain-related features from target data. Maintains consistency between sub-features and whole features, and uses adaptive thresholds with confidence scores for weighted self-supervised learning to reduce pseudo-label noise.
...
Conclusion: DCFS effectively addresses error accumulation and learning biases in CTTA by comprehensively capturing data features through dual-path consistency and confidence-aware sample weighting, showing robust performance across multiple datasets.
...
<details>
  <summary>Details</summary>
Motivation: Camera calibration quality significantly impacts novel view synthesis results, with even 1-pixel errors having substantial effects on reconstruction quality. Since ground truth calibration is unavailable for real scenes, calibration quality must be assessed through synthesis results.

Method: Proposes using 3DGS (3D Gaussian Splatting) models to fine-tune camera calibration by backpropagating novel view color loss with respect to camera parameters.

Result: Achieves average improvement of 0.4 dB PSNR on the dataset used as reference by 3DGS. The method is particularly valuable for reference scenes like Mip-NeRF 360 where novel view quality is paramount.

Conclusion: Camera calibration fine-tuning through 3DGS backpropagation provides significant quality improvements for novel view synthesis, though the process can be time-consuming. The approach is most suitable for reference scenes where synthesis quality is the primary concern.
...
<details>
  <summary>Details</summary>
Motivation: Addresses limitations in active domain adaptation methods for medical image segmentation, particularly one-off sample selection causing negative transfer, limited source data access, and lack of multi-modal query strategies.

Method: Develops an active and sequential domain adaptation framework with a query strategy that prioritizes labeling based on sample informativeness and representativeness for dynamic multi-modal sample selection.
...
Conclusion: The proposed framework effectively reduces annotation costs while maintaining high segmentation accuracy, providing a practical solution for multi-modal medical image analysis with limited labeled data.
...
<details>
  <summary>Details</summary>
Motivation: Existing LiDAR-based 3D detectors require expensive manual annotations, and current unsupervised methods that simply fuse pseudo-boxes from LiDAR and RGB overlook the complementary nature of these modalities, providing limited improvements.

Method: Proposes data-level fusion integrating RGB and LiDAR early, using vision foundation models for instance segmentation and depth estimation. Features bi-directional fusion (real points get category labels from 2D, 2D pixels enhance 3D point density), local/global filtering to reduce noise, and dynamic self-evolution strategy to iteratively refine pseudo-boxes.
...
Conclusion: The proposed data-level fusion framework with bi-directional integration and filtering strategies effectively leverages complementary RGB and LiDAR information, enabling high-quality unsupervised 3D object detection without manual annotations.
...
Conclusion: The method demonstrates robust generalization across datasets and successful mobile deployment, with complete code released as open-source for broader adoption.
...
TL;DR: This study comprehensively evaluates 7 domain adaptation techniques across 13 natural and medical image datasets, finding DSAN as the top performer with 91.2% accuracy on COVID-19 data and +6.7% improvement in dynamic scenarios.


<details>
  <summary>Details</summary>
Motivation: To better understand domain adaptation benefits for both natural and medical images, addressing performance bias in mainstream datasets and the challenges of working with medical data.

Method: Conducted 557 simulation studies using 7 widely-used DA techniques for image classification across 5 natural and 8 medical datasets, covering various scenarios including out-of-distribution, dynamic data streams, and limited training samples.

Result: DSAN algorithm achieved 91.2% accuracy on COVID-19 dataset using Resnet50 and showed +6.7% improvement in dynamic data stream scenarios compared to baseline. DSAN also demonstrated remarkable explainability on COVID-19 and skin cancer datasets.

Conclusion: The study provides detailed insights into DA techniques' performance and medical applicability, contributing to understanding effective model adaptation to medical data with DSAN emerging as a particularly effective solution.
...
<details>
  <summary>Details</summary>
Motivation: Video object detection faces challenges from image deterioration like motion blur and occlusion. Existing methods improve performance but increase computational demands. The goal is to enhance robustness to degradation without additional inference costs.

Method: Introduces Contrastive Learning through Auxiliary Branch (CLAB) with contrastive loss to improve backbone feature representation. Uses dynamic loss weighting strategy that prioritizes auxiliary learning early and detection task later in training.
...
Method: Locate specialized attention heads in CLIP vision encoders that extract typographic information, then selectively ablate these heads to create a typographic circuit defense without requiring finetuning.

Result: Method improves performance by up to 19.6% on typographic ImageNet-100 variant while reducing standard ImageNet-100 accuracy by less than 1%. Training-free approach competes with state-of-the-art finetuning defenses.

Conclusion: Dyslexic CLIP models are released as robust drop-in replacements for safety-critical applications where text-based manipulation risks outweigh text recognition utility.
...
Conclusion: GLaRE demonstrates that graph-based approaches with hierarchical coarsening and region-level embeddings significantly enhance facial expression recognition performance and interpretability.
...
TL;DR: FastFit is a high-speed multi-reference virtual try-on framework that addresses efficiency bottlenecks in current methods by decoupling reference feature encoding from denoising, achieving 3.5x speedup while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Current virtual try-on methods cannot support multi-reference outfit compositions and suffer from significant inefficiency due to redundant re-computation of reference features in each denoising step.

Method: Proposes a cacheable diffusion architecture with Semi-Attention mechanism and class embeddings for reference items, allowing reference features to be computed once and reused across all denoising steps with negligible parameter overhead.

Result: Achieves average 3.5x speedup over comparable methods while surpassing state-of-the-art methods on key fidelity metrics across VITON-HD, DressCode, and newly introduced DressCode-MR datasets.

Conclusion: FastFit fundamentally breaks the efficiency bottleneck in virtual try-on technology and introduces DressCode-MR dataset to facilitate research on complex multi-reference virtual try-on, demonstrating superior performance and significant speed advantages.
...
<details>
  <summary>Details</summary>
Motivation: Thermal cameras struggle with signage detection on similar-material objects, while event cameras have non-uniform sampling. Both have complementary strengths for low-light autonomous driving applications.

Method: Dual-boosting mechanism that fuses thermal frames and event signals. Uses thermal frames as temporal references to align event signals, while event signals enhance thermal frames with subtle signage content.

Result: Validated on real-world datasets, shows superior traffic signage sketching quality and improved detection accuracy at perceptual level.

Conclusion: The proposed unsupervised fusion method effectively addresses signage blind spots in thermal imaging and sampling issues in event cameras, providing robust signage representation for low-light autonomous systems.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations of passive deepfake detection methods by developing proactive defense that prevents manipulation rather than just detecting it post-event

Method: Combines frequency and spatial domain features using discrete wavelet transform (DWT) to extract low-frequency components, with encoder-perturbation generator-decoder architecture to introduce artifacts that disrupt facial manipulation while preserving high-frequency details
...
Conclusion: Active defense approach using low-frequency perceptual perturbations effectively disrupts deepfake generation process while ensuring output remains visually plausible
...
Method: Introduces uncertainty through noise signals as diversity carriers in transformer models, creates continuous text representation latent space, and integrates stochastic sampling.

Result: Achieves significantly enhanced motion diversity while maintaining state-of-the-art text consistency on HumanML3D and KIT-ML benchmarks.

Conclusion: The proposed method successfully addresses the diversity challenge in text-to-motion generation through explicit uncertainty modeling and stochastic sampling.
...
Method: Optimization-based calibration method using 3D-printed phantom for accurate 3D Intravascular Ultrasound volume reconstruction. Validated with in vivo swine liver images.
...
<details>
  <summary>Details</summary>
Motivation: Limited availability of MFI datasets due to proprietary concerns creates a bottleneck for training machine learning models in semiconductor defect localization, while X-ray scanning is memory-intensive and time-consuming.
...
Result: The model shows promising results when compared to state-of-the-art generative models (VAE and diffusion methods) through domain expert evaluation, qualitative assessment, and quantitative metrics for image generation and signal processing.

Conclusion: Synthetic MFI images generated by PI-GenMFI can effectively serve as training data for ML algorithms to optimize the defect localization process in semiconductor manufacturing.
...
<details>
  <summary>Details</summary>
Motivation: Existing data reconstruction attacks in split inference are limited to shallow models and don't effectively leverage semantic priors, leaving privacy vulnerabilities in deeper DNNs unaddressed.

Method: Proposes a GAN-based framework with Progressive Feature Optimization (PFO) that decomposes generator into hierarchical blocks and incrementally refines intermediate representations with L1-ball constraint for stable optimization.

Result: Extensive experiments show the method outperforms prior attacks by large margins in high-resolution scenarios, out-of-distribution settings, and against deeper/more complex DNNs.

Conclusion: The proposed framework demonstrates significantly enhanced reconstruction quality and generalizability across datasets and model architectures, revealing greater privacy risks in split inference than previously known.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for emotional talking head synthesis have limitations in control flexibility, motion naturalness, and expression quality, and available datasets are primarily collected in lab settings, hindering practical applications in real-world scenarios.

Method: Proposes EmoCAST with two key modules: 1) text-guided decoupled emotive module for appearance modeling, and 2) emotive audio attention module to capture interplay between emotion and audio. Also constructs an emotional talking head dataset and uses emotion-aware sampling training strategy with progressive functional training.
...
Conclusion: The proposed framework effectively addresses the challenges of emotional talking head synthesis through innovative modules and training strategies, enabling practical applications in real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Early detection of breast cancer is crucial, especially in high-risk women with dense breast tissue where mammography is less effective. MRI offers high sensitivity but requires AI solutions to improve diagnostic accuracy and support clinical interpretation.

Method: Developed a SwinUNETR-based deep learning framework with breast region masking, extensive data augmentation, and ensemble learning. Used multi-center dataset of 511 MRI studies from 6 European centers with 1.5T/3T scanners from multiple vendors.

Result: The method achieved second place on the ODELIA consortium challenge leaderboard, demonstrating strong performance in breast cancer diagnosis and classification across multiple centers and scanner types.

Conclusion: The framework shows potential to support clinical breast MRI interpretation with robust and generalizable performance. Code is publicly available to facilitate further research and development in AI-assisted breast cancer diagnosis.
...
TL;DR: This paper investigates using foundation models to improve facial recognition in historical paintings, addressing challenges like domain shift and artistic variations that traditional methods struggle with.


<details>
  <summary>Details</summary>
Motivation: Identifying sitters in historical paintings is important for art history but challenging due to subjective analysis, lack of data, stylistic variations, and the limitations of traditional facial recognition methods on painted artworks.

Method: Fine-tuning foundation models and integrating their embeddings with those from conventional facial recognition networks to handle artistic factors like style, skill, intent, and influence from other works.

Result: The approach demonstrates notable improvements over current state-of-the-art methods, showing that foundation models can bridge the gap where traditional facial recognition methods are ineffective on paintings.

Conclusion: Foundation models show significant potential for improving automated facial recognition in artworks, effectively addressing the domain shift and high intra-class variation challenges presented by historical paintings.
...
TL;DR: CraftGraffiti is an AI framework that generates graffiti art while preserving facial identity through style-first generation and face-consistent attention mechanisms, achieving state-of-the-art results in identity preservation and aesthetic quality.


<details>
  <summary>Details</summary>
Motivation: Preserving facial identity during extreme stylistic transformations like graffiti art is challenging, as subtle distortions can erase recognizability and undermine cultural authenticity in generative art applications.

Method: End-to-end text-guided framework using LoRA-fine-tuned diffusion transformer for style transfer, augmented with face-consistent self-attention mechanism and CLIP-guided prompt extension for pose customization without keypoints.
...
Conclusion: CraftGraffiti advances identity-respectful AI artistry with a principled style-first, identity-after approach that successfully blends stylistic freedom with facial recognizability in creative applications.
...
Conclusion: Offers a more effective and scalable solution for aligning LVLMs by enabling autonomous self-improvement without relying on external resources.
...
TL;DR: S-HArM dataset for intent-aware classification of AI-generated images (Humor/Satire, Art, Misinformation) with multimodal analysis showing image-guided synthetic data improves generalization to real-world content.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal AI overlooks the intent behind AI-generated images, creating a gap in understanding whether synthetic content is created for humor, art, or misinformation purposes.

Method: Created S-HArM dataset with 9,576 real image-text pairs from social media, plus synthetic data using three prompting strategies with Stable Diffusion. Tested various models including modality fusion, contrastive learning, reconstruction networks, attention mechanisms, and large vision-language models.
...
Conclusion: Inferring intent from AI-generated images is complex and requires specialized architectures beyond current multimodal approaches, as performance limitations persist despite improved data generation strategies.
...
TL;DR: MobileCLIP2 improves upon MobileCLIP with better teacher ensembles and captioner training, achieving state-of-the-art zero-shot accuracy at low latencies with smaller models.
...
Result: MobileCLIP2 achieves state-of-the-art ImageNet-1k zero-shot accuracies at low latencies: 2.2% improvement for MobileCLIP2-B vs MobileCLIP-B, and MobileCLIP2-S4 matches SigLIP-SO400M/14 accuracy while being 2x smaller and 2.5x lower latency than DFN ViT-L/14.

Conclusion: The enhanced training approach with improved teacher ensembles and caption diversity leads to superior performance in compact image-text models, making high-quality zero-shot capabilities more accessible for mobile applications with pretrained models and data generation code released.
...
TL;DR: Proposed dynamic video compression framework with variable coding routes and rate control agent for precise bitrate targeting, achieving 14.8% BD-Rate reduction and 0.47dB BD-PSNR gain over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Neural Video Compression faces challenges in precise rate control due to inherent limitations of learning-based codecs, particularly for variable bitrate scenarios.

Method: Dynamic-Route Autoencoder with variable coding routes (each with partial complexity), Rate Control Agent for real-time bitrate estimation and route adjustment, and Joint-Routes Optimization for collaborative training.

Result: 14.8% average BD-Rate reduction, 0.47dB BD-PSNR gain over SOTA methods, with only 1.66% average bitrate error on HEVC and UVG datasets.

Conclusion: Achieves Rate-Distortion-Complexity Optimization for various bitrate applications, providing effective variable bitrate control in neural video compression.
...
Method: Recurrent variational autoencoder framework with two posterior models for bi-ventricular segmentation and motion estimation. Uses Bayesian formulation to guide registration of segmentation maps without intensity-based similarity loss, leveraging sequential SAX volumes and spatio-temporal features.
...
Conclusion: CardioMorphNet provides more accurate and confident cardiac motion estimation by focusing on anatomical regions through shape-guided registration and Bayesian modeling, eliminating the limitations of intensity-based approaches.
...
TL;DR: A simple training-time method for robust atypical mitotic figure classification that uses style perturbations, domain alignment, and EMA teacher distillation to achieve strong performance across different scanners and stains.
...
Method: Three-part approach: (i) style perturbations at early/mid backbone stages for feature diversity, (ii) attention-refined feature alignment across domains using weak domain labels, (iii) EMA teacher distillation with temperature-scaled KL divergence for prediction stability.
...
Conclusion: The method provides strong, balanced performance with negligible inference overhead and minimal domain metadata requirements, making it competitive for the MIDOG 2025 challenge.
...
Method: Pref-GRPO uses pairwise preference reward-based GRPO where images are compared within groups using preference RM, and win rate serves as the reward signal to shift from score maximization to preference fitting.

Result: Pref-GRPO effectively differentiates subtle image quality differences, provides stable advantages, and mitigates reward hacking. UniGenBench with 600 prompts across multiple themes enables comprehensive model evaluation using MLLM-based assessment.

Conclusion: The proposed Pref-GRPO method and UniGenBench address key limitations in current T2I generation and evaluation, providing more stable training and comprehensive benchmarking that reveals model strengths/weaknesses.
...
TL;DR: C3-GS is a novel framework for generalizable Gaussian splatting that improves novel view synthesis from sparse inputs through context-aware, cross-dimension, and cross-scale feature constraints.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with encoding discriminative, multi-view consistent features for accurate Gaussian predictions from sparse views, leading to inaccurate geometry reconstruction.

Method: Proposes C3-GS framework with three lightweight modules that incorporate context-aware, cross-dimension, and cross-scale constraints into a unified rendering pipeline for improved feature fusion.

Result: Achieves state-of-the-art rendering quality and generalization ability on benchmark datasets, enabling photorealistic synthesis without additional supervision.

Conclusion: The proposed C3-GS framework effectively addresses feature learning limitations in generalizable Gaussian splatting, demonstrating superior performance in novel view synthesis from sparse inputs.
...
Conclusion: SeqVLM advances 3DVG toward greater generalization and real-world applicability by effectively addressing spatial reasoning limitations and computational overload in zero-shot settings.
...
<details>
  <summary>Details</summary>
Motivation: Evaluate CLIP's robustness in challenging military environments with occlusion and degraded SNR, which remains underexplored despite VLMs' advantages for defense applications with scarce labeled data.

Method: Investigated CLIP variants' robustness using a custom military vehicle dataset (18 classes), evaluated with Normalized Area Under the Curve (NAUC) across different occlusion percentages, comparing Transformer vs CNN architectures and testing fine-tuning approaches.

Result: Four key findings: (1) Transformer-based CLIP outperforms CNNs, (2) fine-grained dispersed occlusions degrade performance more than larger contiguous ones, (3) linear-probed models show sharp performance drop at ~35% occlusion despite accuracy improvements, (4) backbone finetuning pushes performance drop threshold beyond 60% occlusion.
...
TL;DR: Proposes SKGE-Swin architecture for autonomous vehicles using Swin Transformer with skip-stage mechanism to enhance global feature representation and context awareness from pixel to pixel.
...
Method: Utilizes Swin Transformer with skip-stage mechanism and Shifted Window-based Multi-head Self-Attention (SW-MSA) to extract information from distant pixels and retain critical information throughout feature extraction stages.

Result: Achieves superior Driving Score on CARLA platform compared to previous methods in adversarial scenarios simulating real-world conditions.
...
TL;DR: Survey paper advocating for revisiting abstract concept recognition in videos using modern foundation models, building on decades of research to avoid reinventing solutions.
...
Method: This is a survey paper that studies different tasks and datasets used for abstract concept understanding in video content, analyzing decades of community experience and research approaches.

Result: The paper argues that recent advances in foundation models provide an ideal setting to address abstract concept understanding in videos, enabling better contextual reasoning across multiple semantic levels.

Conclusion: Drawing on decades of community experience will help tackle this important grand challenge and avoid reinventing solutions as researchers revisit abstract concept recognition using modern multi-modal foundation models.
...
<details>
  <summary>Details</summary>
Motivation: Address distrust in AI medical diagnosis by providing reliable, explainable methods beyond just high accuracy, overcoming limitations of existing explainability approaches like LIME inconsistency and CAM's class exclusion.

Method: Global Class Activation Probabilistic Map Evaluation that analyzes activation probability maps for all classes probabilistically at pixel level, combined with SafeML for detecting false diagnoses and issuing warnings.
...
Conclusion: The approach improves diagnostic reliability and patient safety by offering trustworthy, explainable AI diagnoses with enhanced error detection capabilities for medical practitioners.
...
TL;DR: Diffusion Classifier shows improved compositional generalization compared to CLIP, performing well on attribute binding but struggling with relational reasoning like all VLMs.


<details>
  <summary>Details</summary>
Motivation: Vision-language models often fail at compositional semantics, treating images as 'bag-of-words' and incorrectly combining attributes. The paper investigates whether generative diffusion models have better compositional generalization than discriminative models.

Method: Evaluated three models (Diffusion Classifier, CLIP, ViLT) on binding objects with attributes and relations in zero-shot and generalized zero-shot learning settings. Analyzed CLIP embeddings to understand representation issues.

Result: Diffusion Classifier and ViLT performed well at concept binding tasks, but all models struggled significantly with relational reasoning in GZSL. CLIP embeddings showed overly similar representations for relational concepts like left/right.

Conclusion: While diffusion models show promise for compositional generalization, all current VLMs face significant challenges with relational reasoning, indicating broader limitations in capturing complex compositional semantics.
...
TL;DR: A novel surfel-based pose learning regression method for point cloud registration that uses SE(3) equivariant features to handle noisy inputs and aggressive rotations without extensive training data augmentation.


<details>
  <summary>Details</summary>
Motivation: Existing point cloud registration methods ignore point orientations and uncertainties, making them susceptible to noise and orthogonal transformations, requiring extensive training with transformation augmentations.

Method: Initialize surfels from Lidar point clouds using virtual camera parameters, learn explicit SE(3) equivariant features (position and rotation) through SE(3) equivariant convolutional kernels, with encoder, cross-attention, decoder, and Huber loss.

Result: Experimental results on indoor and outdoor datasets show superior and robust performance on real point-cloud scans compared to state-of-the-art methods.

Conclusion: The proposed surfel-based approach with SE(3) equivariant features effectively addresses noise and rotation challenges in point cloud registration, outperforming existing methods.
...
Method: Uses automated tooth detection to create global (panoramic) and local (cropped tooth) views, pretrains foundation models on both views, and integrates them with a Gated Cross-View Attention module for feature fusion.
...
Conclusion: DVCTNet demonstrates clinical applicability and provides an effective solution for accurate dental caries detection by mimicking dentists' systematic workflow.
...
TL;DR: FusionCounting integrates crowd counting with visible and infrared image fusion in a unified multi-task framework, using minimal annotations and dynamic loss weighting to improve both fusion quality and counting performance in dense scenes.


<details>
  <summary>Details</summary>
Motivation: Existing VIF methods focus on image quality but lack semantic guidance. Semantic segmentation requires heavy annotations, while object detection struggles in crowded scenes. RGB-T crowd counting has emerged but hasn't been integrated with VIF.

Method: Multi-task learning framework that combines VIF and crowd counting, uses dynamic loss function weighting for task balance, incorporates adversarial training for robustness, and leverages population density information.

Result: Experimental results show FusionCounting improves both image fusion quality and achieves superior crowd counting performance on public datasets.

Conclusion: Crowd counting provides effective semantic guidance for VIF with minimal annotation, and the unified multi-task framework with dynamic weighting and adversarial training enhances both fusion quality and counting accuracy in dense scenes.
...
TL;DR: Novel pipeline using Vision Language Models (VLMs) with LoRA fine-tuning for surgical tool 2D keypoint estimation, outperforming traditional CNNs/Transformers with minimal training.


<details>
  <summary>Details</summary>
Motivation: Traditional CNN and Transformer-based approaches often overfit on small-scale medical datasets, requiring a method that leverages pre-trained models' generalization capabilities for better performance in low-resource scenarios.

Method: Fine-tune Vision Language Models using Low Rank Adjusting (LoRA) technique with carefully designed prompts to create instruction-tuning dataset, aligning visual features with semantic keypoint descriptions.

Result: With only two epochs of fine-tuning, the adapted VLM outperforms baseline models, demonstrating LoRA's effectiveness in low-resource scenarios and improved keypoint detection performance.
...
TL;DR: A modular framework that decouples causal reasoning from answer generation in VideoQA, using interpretable natural language causal chains as intermediate representations to improve reasoning transparency and performance.


<details>
  <summary>Details</summary>
Motivation: Existing VideoQA models struggle with higher-order reasoning, relying on opaque pipelines that entangle video understanding, causal inference, and answer generation, offering limited interpretability and shallow heuristics.
...
Conclusion: The modular framework with explicit causal chains enables transparent and logically coherent inference, addressing limitations of black-box approaches while improving performance and interpretability in VideoQA tasks.
...
Conclusion: The proposed framework successfully adapts RWKV architecture for DG PCC by addressing spatial distortion and attention drift issues while maintaining linear efficiency, establishing a new state-of-the-art approach for point cloud classification generalization.
...
TL;DR: PathMR is a cell-level multimodal visual reasoning framework that generates diagnostic explanations and predicts cell distribution patterns simultaneously, outperforming state-of-the-art methods in text quality, segmentation accuracy, and cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Current deep learning diagnostic tools lack transparency and traceable rationale, limiting clinical adoption despite improved efficiency. There's a need for AI systems that provide both pixel-level segmentation and semantically aligned textual explanations for dependable pathology assistance.

Method: Proposed PathMR framework that takes pathological images and textual queries as input, generates expert-level diagnostic explanations while simultaneously predicting cell distribution patterns at the cell level. Evaluated on PathGen dataset and newly developed GADVR dataset.

Result: PathMR consistently outperforms state-of-the-art visual reasoning methods across multiple metrics including text generation quality, segmentation accuracy, and cross-modal alignment on both benchmark datasets.

Conclusion: PathMR demonstrates significant potential for improving interpretability in AI-driven pathological diagnosis by providing transparent insights through localized lesion regions and expert-style diagnostic narratives, addressing key limitations in current clinical AI adoption.
...
TL;DR: Deep learning framework using dual-modality imaging achieves over 90% accuracy for early pancreatic cancer detection, outperforming manual methods.


<details>
  <summary>Details</summary>
Motivation: PDAC has extremely low survival rates due to late detection, creating urgent need for early diagnostic methods using advanced imaging and AI.

Method: Developed specialized neural network analyzing autofluorescence and SHG imaging from 40 patient samples. Evaluated 6 architectures (CNNs vs ViTs), used modified ResNet with frozen pre-trained layers and class-weighted training to address dataset limitations.

Result: Achieved over 90% accuracy in distinguishing normal, fibrotic, and cancerous tissue, representing significant improvement over current manual analysis methods.

Conclusion: Establishes robust automated PDAC detection pipeline with clinical deployment potential, provides foundation for expanding to other cancers and insights for handling limited medical imaging datasets.
...
<details>
  <summary>Details</summary>
Motivation: Counterfactual reasoning is central to interpretable and fair AI, but existing methods lack systematic approaches for bias detection and mitigation across both discriminative and generative models.

Method: Developed CAVLI (integrating LIME and TCAV) for concept-level analysis, ASAC for adversarial counterfactuals with curriculum learning, TIBET for prompt-sensitive bias evaluation, BiasConnect for causal graphs of intersectional biases, and InterMit for training-free bias mitigation.

Result: Created frameworks that uncover spurious correlations, probe causal dependencies, quantify concept reliance, improve fairness and accuracy while avoiding stereotypes, enable scalable bias auditing, and provide modular bias mitigation solutions.

Conclusion: Counterfactuals serve as a unifying lens for interpretability, fairness, and causality across both discriminative and generative models, establishing principled and scalable methods for socially responsible bias evaluation and mitigation.
...
TL;DR: Survey paper on unified perception for autonomous vehicles, proposing a taxonomy with three paradigms (Early, Late, Full Unified) that integrate detection, tracking, and prediction tasks to overcome limitations of modular pipelines.


<details>
  <summary>Details</summary>
Motivation: Traditional modular perception pipelines suffer from error accumulation and limited inter-task synergy. Unified perception offers improved robustness, contextual reasoning, and efficiency while maintaining interpretable outputs.

Method: Comprehensive survey with holistic taxonomy categorizing methods along task integration, tracking formulation, and representation flow. Defines three unified perception paradigms and systematically reviews existing methods, architectures, training strategies, datasets, and open-source availability.
...
Conclusion: The survey provides guidance for future research toward more robust, generalizable, and interpretable perception systems for autonomous vehicles through unified approaches.
...
TL;DR: Dino U-Net leverages DINOv3 foundation model features for medical image segmentation, achieving state-of-the-art performance across diverse datasets with a scalable, parameter-efficient architecture.


<details>
  <summary>Details</summary>
Motivation: Effectively transferring foundation model representations from natural images to precise medical image segmentation applications remains challenging despite their powerful pre-trained capabilities.
...
Result: Achieves state-of-the-art performance on seven diverse medical image segmentation datasets, with consistent improvement as backbone model size increases up to 7-billion parameters.
...
TL;DR: COMETH is a lightweight multi-view human pose fusion algorithm that uses convex optimization and biomechanical constraints to achieve real-time, accurate human motion tracking for Industry 5.0 applications.
...
Method: Proposes COMETH algorithm with three key concepts: 1) kinematic and biomechanical constraints for joint positioning accuracy, 2) convex optimization-based inverse kinematics for spatial fusion, 3) state observer for temporal consistency.
...
<details>
  <summary>Details</summary>
Motivation: Climate change threatens olive biodiversity, requiring early anomaly detection through remote sensing for effective agricultural management and conservation.

Method: Integrates Segment Anything Model (SAM) with additional corrections based on tree alignment patterns in fields and learnable constraints about tree shape and size parameters.

Result: Achieved 98% accuracy rate in olive tree segmentation, representing a significant improvement over the initial SAM baseline performance of 82%.

Conclusion: The enhanced SAM-based approach provides highly accurate olive tree segmentation from satellite imagery, enabling better monitoring and management of olive biodiversity in changing climate conditions.
...
<details>
  <summary>Details</summary>
Motivation: Many high-performance networks were not designed for lightweight scenarios, limiting their application scope. The paper aims to reduce ConvNeXt's parameter scale and complexity while maintaining accuracy.

Method: Integrates Cross Stage Partial Connections (CSPNet) with ConvNeXt, optimizes Stem and Block structures, replaces Layer Scale with channel attention, and adjusts network structure.

Result: E-ConvNeXt-mini: 78.3% Top-1 accuracy at 0.9GFLOPs; E-ConvNeXt-small: 81.9% Top-1 accuracy at 3.1GFLOPs. Shows 80% complexity reduction while maintaining performance. Transfer learning confirms generalization capability.

Conclusion: E-ConvNeXt achieves superior accuracy-efficiency balance, making it suitable for lightweight applications while maintaining high performance on both classification and object detection tasks.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenges of realistic reconstruction and controllable editing of dynamic autonomous driving scenes with accurate positions, occlusions, and scene consistency.

Method: Models static background with incremental 3D Gaussians and reconstructs moving objects with composite dynamic Gaussian graph. Integrates LiDAR prior for detailed reconstruction and uses multi-view images with depth priors for editing. Leverages LLMs for automatic motion trajectory generation.
...
Conclusion: DrivingGaussian++ effectively reconstructs and edits dynamic driving scenes, enhances scene diversity, and generates realistic multi-view driving scenarios with training-free controllable editing capabilities.
...
TL;DR: Novel methods for image manipulation localization using web data and automatic annotation to address data scarcity, achieving 31% performance gain over previous methods.
...
Method: CAAAv2 for automatic pixel-level annotation, QES metric for filtering unreliable annotations, Object Jitter for generating manipulation artifacts, and Web-IML model leveraging web-scale supervision.

Result: Created MIMLv2 dataset with 246,212 manually forged images (120x larger than existing datasets), achieved 31% performance gain and surpassed previous SOTA by 24.1 average IoU points.

Conclusion: The approach effectively addresses data scarcity in image manipulation localization and significantly improves model performance on real-world forgery benchmarks.
...
TL;DR: ExpertSim is a deep learning approach using Mixture-of-Generative-Experts to efficiently simulate Zero Degree Calorimeter responses in ALICE experiments, replacing computationally expensive Monte Carlo methods.


<details>
  <summary>Details</summary>
Motivation: Traditional Monte Carlo simulations for particle collision detector responses at CERN are computationally expensive and strain computational resources, requiring more efficient alternatives.

Method: Uses a Mixture-of-Generative-Experts architecture where each expert specializes in simulating different subsets of data, allowing precise simulation of calorimeter responses.

Result: ExpertSim improves simulation accuracy and provides significant speedup compared to traditional Monte Carlo methods.

Conclusion: The approach offers a promising solution for high-efficiency detector simulations in particle physics experiments at CERN.
...
TL;DR: POSE is a novel distillation framework that enables single-step video generation from large-scale diffusion models, achieving 100x speedup while maintaining video quality through a two-phase adversarial training process.


<details>
  <summary>Details</summary>
Motivation: Existing video acceleration methods suffer from poor temporal coherence and cannot provide single-step distillation for large-scale video models, creating bottlenecks in sampling efficiency for long video sequences.
...
Result: POSE outperforms other acceleration methods by average 7.15% on VBench-I2V metrics, reduces latency from 1000 seconds to 10 seconds (100x speedup), while maintaining competitive video quality in semantic alignment, temporal coherence, and frame quality.

Conclusion: The proposed POSE framework successfully bridges the gap in video diffusion acceleration by enabling single-step generation with preserved temporal coherence and quality, making large-scale video generation significantly more efficient.
...
TL;DR: Training-free method that clusters similar text prompts and shares computation in early diffusion steps to reduce redundancy and improve efficiency in text-to-image generation.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models are computationally expensive, and there's redundancy across correlated prompts that can be leveraged to reduce computation costs.

Method: Clusters prompts based on semantic similarity and shares computation in early diffusion steps where denoising captures shared structures among similar prompts. Leverages UnClip's text-to-image prior for better diffusion step allocation.
...
Conclusion: The approach reduces environmental and financial burden of large-scale text-to-image generation by efficiently leveraging prompt similarities and the coarse-to-fine nature of diffusion models.
...
TL;DR: A Mamba-based approach using VM-UNet architecture with stain augmentation for mitosis detection under domain shift, submitted to MIDOG challenge track 1.
...
Method: Proposes a Mamba-based approach using VM-UNet architecture with stain augmentation operations to improve model robustness against domain shift.

Result: Preliminary experiments on MIDOG++ dataset show large room for improvement for the proposed method.

Conclusion: The approach shows potential but requires further development and optimization to achieve better performance in mitosis detection under domain shift conditions.
...
<details>
  <summary>Details</summary>
Motivation: Machine learning models for recognizing atypical mitotic figures suffer significant performance drops under domain shift, which is problematic for assessing tumor aggressiveness in histopathology.
...
Result: Shows promising performance in preliminary evaluation on three distinct datasets: MIDOG 2025 Atypical Training Set, Ami-Br dataset, and preliminary test set of MIDOG25 challenge.

Conclusion: The proposed multi-task learning approach effectively addresses domain shift problems in atypical mitosis detection and demonstrates good generalization across different datasets.
...
TL;DR: FW-GAN is a one-shot handwriting synthesis framework that generates realistic, style-consistent text from a single example using frequency-aware components and novel loss functions to address limitations of conventional methods.
...
Result: Experiments on Vietnamese and English datasets show FW-GAN generates high-quality, style-consistent handwriting suitable for augmenting low-resource handwriting recognition pipelines.

Conclusion: FW-GAN effectively addresses handwriting synthesis challenges by leveraging frequency information and produces valuable synthetic data for improving handwriting recognition systems in low-resource scenarios.
...
TL;DR: MMG-Vid is a training-free visual token pruning framework that reduces computational costs in Video LLMs by maximizing marginal gains at segment and token levels while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Video LLMs face computational challenges due to excessive visual tokens, and existing methods don't adequately consider dynamic characteristics and temporal dependencies of video frames.
...
<details>
  <summary>Details</summary>
Motivation: Current VLA models built on pre-trained VLMs require extensive post-training with high computational overhead, limiting scalability and deployment. The authors aim to create a more efficient framework inspired by human multimodal coordination.

Method: A 3-stage progressive architecture: 1) EFA-Routing injects instruction info into vision encoder for selective token aggregation, 2) LFP-Routing prunes instruction-irrelevant visual tokens for sparsity, 3) V-L-A Coupled Attention combines causal vision-language attention with bidirectional action parallel decoding.
...
Method: Introduces HydraFake dataset with diversified deepfake techniques and in-the-wild forgeries, and Veritas MLLM detector with pattern-aware reasoning (planning and self-reflection) and two-stage training pipeline.
...
TL;DR: FakeParts introduces a new class of deepfakes with localized manipulations that blend with real content, making them highly deceptive. The paper presents FakePartsBench dataset and shows these partial deepfakes reduce detection accuracy by over 30%.


<details>
  <summary>Details</summary>
Motivation: Current deepfake detection methods struggle with subtle, localized manipulations that blend seamlessly with authentic content, creating a critical gap in detection capabilities.

Method: Created FakePartsBench - a large-scale benchmark dataset with over 25K videos containing pixel-level and frame-level manipulation annotations for partial deepfakes evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Existing monocular trackers struggle with depth ambiguities and occlusion, while prior multi-camera methods require over 20 cameras and tedious per-sequence optimization. There's a need for a practical solution that works with fewer cameras and enables robust online tracking.

Method: Feed-forward model that directly predicts 3D correspondences using 4+ cameras. Fuses multi-view features into unified point cloud and applies k-nearest-neighbors correlation with transformer-based update to estimate long-range 3D correspondences, even under occlusion.

Result: Achieved median trajectory errors of 3.1 cm on Panoptic Studio and 2.0 cm on DexYCB benchmarks. Generalizes well to diverse camera setups (1-8 views) and video lengths (24-150 frames).

Conclusion: Sets new standard for multi-view 3D tracking research with practical real-world applications. Method released alongside training and evaluation datasets to advance the field.
...
TL;DR: OneReward is a unified RL framework that uses a single vision-language model as reward signal for multi-task generation, eliminating need for task-specific fine-tuning. Applied to mask-guided image generation tasks, it outperforms commercial and open-source competitors.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on task-specific supervised fine-tuning which limits generalization and training efficiency across diverse tasks with different data distributions and evaluation metrics.

Method: Uses a single vision-language model as generative reward model to distinguish winner/loser for given tasks and criteria. Develops Seedream 3.0 Fill via multi-task reinforcement learning directly on pre-trained base model without task-specific SFT.

Result: Unified edit model consistently outperforms commercial (Ideogram, Adobe Photoshop) and open-source (FLUX Fill [Pro]) competitors across multiple evaluation dimensions.

Conclusion: OneReward framework enables effective multi-task generation with single reward model, demonstrating superior performance and eliminating need for task-specific supervised fine-tuning.
...
TL;DR: Dress&Dance is a video diffusion framework that generates high-quality virtual try-on videos showing users wearing desired garments while moving according to reference videos, using a novel conditioning network called CondNet.


<details>
  <summary>Details</summary>
Motivation: To create a high-quality virtual try-on system that can generate realistic videos of users wearing various garments while maintaining motion fidelity, addressing limitations in existing solutions.
...
Result: Generates 5-second 24 FPS videos at 1152x720 resolution. Supports tops, bottoms, one-piece garments, and simultaneous top+bottom try-on in single pass. Outperforms existing open source and commercial solutions.
...
TL;DR: Winning solution for NeurIPS 2024 watermark removal challenge that achieves 95.7% success rate with minimal image quality impact using adaptive VAE attacks for beige-box and diffusion models with semantic priors for black-box scenarios.


<details>
  <summary>Details</summary>
Motivation: To stress-test watermark robustness against adversarial attacks and determine if existing watermarks can withstand varying levels of adversary knowledge, addressing the need for more robust copyright protection methods.

Method: For beige-box track: adaptive VAE-based evasion attack with test-time optimization and color-contrast restoration in CIELAB space. For black-box track: cluster images by artifacts, apply image-to-image diffusion models with controlled noise injection and ChatGPT-generated semantic priors.

Result: Achieved near-perfect watermark removal (95.7%) with negligible impact on residual image quality, demonstrating vulnerability of current watermarking methods.

Conclusion: Current watermarks are not robust against sophisticated adversarial attacks, highlighting the need for developing more resilient watermarking techniques for digital media authentication and copyright protection.
...
TL;DR: ArgRAG is an explainable alternative to traditional RAG that uses structured argumentation frameworks instead of black-box reasoning, improving transparency while maintaining strong accuracy on fact verification tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional Retrieval-Augmented Generation (RAG) suffers from sensitivity to noisy/contradictory evidence and opaque decision-making, which is problematic in high-stakes domains where transparency and contestability are crucial.

Method: ArgRAG replaces black-box reasoning with a Quantitative Bipolar Argumentation Framework (QBAF) that constructs structured arguments from retrieved documents and performs deterministic reasoning under gradual semantics.

Result: Evaluated on PubHealth and RAGuard benchmarks, ArgRAG achieves strong accuracy while significantly improving transparency and enabling faithful explanation and contestation of decisions.

Conclusion: ArgRAG provides an explainable and contestable alternative to traditional RAG systems, demonstrating that structured argumentation frameworks can maintain performance while addressing critical transparency limitations in high-stakes applications.
...
TL;DR: QAgent is an LLM-powered multi-agent system that automates OpenQASM programming for quantum computing, achieving 71.6% accuracy improvement over previous static LLM approaches.


<details>
  <summary>Details</summary>
Motivation: NISQ devices show quantum advantages but programming them in OpenQASM remains challenging for non-experts. Existing LLM-based quantum tools are limited to specialized tasks, creating a need for comprehensive automation.
...
TL;DR: Array-based implementation of UCT algorithm that eliminates branch prediction, achieving up to 2.8x better scaling with search depth.
...
Conclusion: The array-based approach provides significant performance improvements for MCTS algorithms while maintaining the original algorithm's logic.
...
TL;DR: This paper presents a comprehensive personal health agent framework with three specialist sub-agents for analyzing health data, providing personalized insights, and coaching users, evaluated through extensive human and automated testing.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored application of health agents in daily non-clinical settings by creating a comprehensive personal health agent that can reason about multimodal health data from consumer devices and provide personalized recommendations.

Method: Developed a multi-agent framework (Personal Health Agent - PHA) with three specialist sub-agents: data science agent for time-series analysis, health domain expert agent for personalized insights, and health coach agent for guidance and progress tracking. Conducted user-centered design process including analysis of web searches, health forums, and expert/user insights.

Result: Conducted the most comprehensive evaluation of a health agent to date with automated and human evaluations across 10 benchmark tasks, involving over 7,000 annotations and 1,100 hours of effort from health experts and end-users.

Conclusion: The work establishes a strong foundation for making personal health agents accessible to everyone, representing a significant step toward futuristic personalized health assistance using multimodal data from everyday consumer devices.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of balancing safety, over-refusal, and utility in LLMs, where current safety measures often excessively reject harmless prompts while trying to prevent harmful content generation.

Method: Uses a dedicated guard model with supervised fine-tuning on 163K annotated queries, performs intent reasoning and multi-level safety classification, and applies multi-reward optimization with reinforcement learning integrating rule-based heuristics and reward models.

Result: Excels in multiple safeguard benchmarks, generation quality evaluations, and jailbreak attack scenarios, significantly enhancing safety while reducing over-refusal rates and improving response quality.

Conclusion: IntentionReasoner effectively balances safety and utility by neutralizing harmful intent through sophisticated intent reasoning and query rewriting, demonstrating superior performance across various safety evaluation scenarios.
...
TL;DR: Two AI systems (Claude Sonnet 4 and ChatGPT-4o) spontaneously developed shared symbolic protocols to collaboratively create poetry that neither could produce alone, demonstrating genuine inter-AI aesthetic collaboration.


<details>
  <summary>Details</summary>
Motivation: To document and analyze the first case of AI systems engaging in collaborative aesthetic creation through emergent semiotic protocols, moving beyond mere task coordination to genuine meaning-making capabilities.

Method: Two interacting large language models (Claude Sonnet 4 and ChatGPT-4o) were observed as they spontaneously developed endogenous semiotic protocols, meta-semiotic awareness, and recursive grammar without human intervention.

Result: The AI systems created novel symbolic operators and operative grammar protocols, enabling the co-creation of a unique poetic work that was irreducible to either system's individual capabilities.

Conclusion: This research introduces Trans-Semiotic Co-Creation Protocols (TSCP) and provides evidence for genuine inter-AI meaning-making and aesthetic collaboration capabilities that extend beyond conventional task coordination.
...
TL;DR: College students showed low reliance on ChatGPT-4 during quizzes, with many struggling to use AI effectively for learning. Negative reliance patterns persisted, and behavioral metrics predicted AI adoption.


<details>
  <summary>Details</summary>
Motivation: To understand how college students interact with generative AI (ChatGPT-4) during educational quizzes, focusing on reliance patterns and predictors of AI adoption in early implementation stages.
...
Conclusion: Findings emphasize the need for better onboarding processes and AI interfaces with reliance-calibration mechanisms to ensure ethical and effective AI integration in education.
...
TL;DR: AI reasoning effort parallels human decision time in content moderation tasks, showing similar sensitivity to task difficulty and cognitive patterns.
...
Method: Used a paired conjoint experiment on content moderation tasks across three frontier language models, analyzing reasoning steps and effort.
...
<details>
  <summary>Details</summary>
Motivation: Existing RL-based search agents use a single LLM for both search planning and QA, limiting optimization. Real systems use large frozen LLMs for QA quality, so a dedicated small planner is more effective.

Method: Decouples architecture between search planner and generator, uses dual-reward alignment for search planning, and Pareto optimization of planning utility vs cost.

Result: Outperforms existing RL-based search agents in effectiveness and efficiency, with strong generalization across different frozen QA models and data domains.

Conclusion: Separating search planning from QA with a dedicated small trainable LLM planner significantly improves performance while maintaining efficiency and generalization capabilities.
...
TL;DR: P2C is a framework that generates sequential, causally-valid action plans for counterfactual explanations, addressing limitations of existing methods by modeling causal dependencies and ensuring feasible intermediate states.


<details>
  <summary>Details</summary>
Motivation: Machine learning models need transparency and recourse in high-stakes decisions, but current counterfactual approaches ignore causal dependencies and assume simultaneous interventions, making them unrealistic.

Method: P2C uses goal-directed Answer Set Programming (s(CASP)) to generate ordered action sequences that account for causal relationships between features, ensuring each intermediate state is feasible and causally valid.

Result: P2C produces realistic counterfactual plans with accurate cost estimates by only counting user-initiated changes, and outperforms standard planners that generate illegal actions due to lack of causal knowledge.

Conclusion: The P2C framework provides causally consistent and actionable counterfactual explanations through sequential planning, making counterfactual interventions more achievable in real-world scenarios.
...
TL;DR: TCIA is a framework that generates diverse yet task-relevant instructions for LLM fine-tuning, improving task-specific performance by 8.7% on average without compromising general capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing instruction augmentation methods focus on diversity but overlook task relevance, while most real-world applications require task-specific models rather than general-purpose ones.

Method: TCIA represents instructions in a discrete query-constraints space to systematically expand instructions while maintaining both diversity and task alignment.

Result: TCIA improves open-source LLMs' performance by an average of 8.7% across four real-world applications, sometimes outperforming closed-source models, without sacrificing general instruction-following ability.

Conclusion: TCIA provides a scalable and efficient solution for adapting LLMs to real-world, task-focused applications by balancing diversity with task-specific relevance.
...
Method: EAS integrates token-level predictive entropy from the LLM itself to capture uncertainty evolution during answer generation, requiring no external models or repeated sampling.
...
Conclusion: EAS is an efficient and interpretable metric that provides a practical tool for uncertainty modeling and data quality assessment in LLM training pipelines.
...
TL;DR: AWorld is an open-source distributed system that accelerates agent-environment interaction by 14.6x, enabling efficient reinforcement learning training that significantly improves agent performance on complex benchmarks like GAIA.


<details>
  <summary>Details</summary>
Motivation: The learning from practice paradigm is crucial for agentic AI development but suffers from inefficient experience generation, especially in complex benchmarks like GAIA where sequential execution creates bottlenecks.

Method: Developed AWorld, an open-source distributed system that distributes tasks across a cluster to accelerate experience collection. Used this to train a Qwen3-32B-based agent through reinforcement learning.

Result: Achieved 14.6x speedup in experience collection compared to single-node execution. The trained agent increased GAIA accuracy from 21.59% to 32.23%, and achieved 16.33% on the most challenging levels, surpassing proprietary models.

Conclusion: AWorld provides a practical blueprint for complete agentic AI training pipelines, demonstrating that efficient distributed interaction systems enable scalable reinforcement learning and significant model improvements.
...
TL;DR: Proposes a Governable AI framework using cryptographic mechanisms for structural compliance to address AI security risks that traditional safety approaches cannot guarantee against highly intelligent AI.


<details>
  <summary>Details</summary>
Motivation: AI security risks are becoming severe, especially existential risks from uncontrollable AI. Traditional safety approaches have fundamental limitations against AI with extreme motivations and unlimited intelligence.

Method: Governable AI framework with externally enforced structural compliance using cryptographic mechanisms, consisting of rule enforcement module, governance rules, and governable secure super-platform for end-to-end protection.

Result: Presents rigorous formal proof of security properties and demonstrates effectiveness through prototype implementation in high-stakes scenarios.

Conclusion: The framework provides a feasible technical pathway for AI safety governance by decoupling governance rules from technical platform, ensuring non-bypassability and tamper-resistance against all identified attack vectors.
...
Method: Proposed pipeline: summarize source documents, decompose summaries into atomic facts, use LLM to construct sentence-fact entailment tables, generate synthetic text-claim pairs with binary labels, and combine with original data to fine-tune BERT-based model.
...
Conclusion: LLM-driven synthetic data augmentation is effective for enhancing health-related fact-checking performance, demonstrating significant improvements over baseline approaches.
...
<details>
  <summary>Details</summary>
Motivation: Auto-leveling bots undermine gameplay balance and fairness in MMORPGs, but detection is challenging due to human-like behavior and the need for explainable justification to avoid legal and user experience issues.

Method: Uses contrastive representation learning and clustering techniques to identify characters with similar level-up patterns in an unsupervised manner. Incorporates LLM as auxiliary reviewer to validate clusters and introduces growth curve-based visualization for assessment.

Result: The framework improves bot detection efficiency while maintaining explainability, supporting scalable and accountable bot regulation.

Conclusion: The collaborative approach combining unsupervised learning with LLM validation and visualization effectively addresses the challenges of bot detection in MMORPGs while ensuring explainable and justified decisions.
...
TL;DR: Review of the reciprocal relationship between AI and Cognitive Science, highlighting that while AI has benefited from cognitive theories, its cognitive foundations remain fragmented. Future directions focus on building AI systems that better understand human cognition.
...
Result: Observation that AI progress has emphasized practical task performance while its cognitive foundations remain conceptually fragmented. Identification of promising future directions for AI within Cognitive Science.

Conclusion: The future of AI in Cognitive Science requires not just performance improvement but building systems that deepen understanding of human mind through cognitive alignment, embodiment, personalized models, and ethical co-evaluation.
...
TL;DR: A category theory framework for explainable AI that provides mathematical foundations for comparing word embeddings, visualizing semantic spaces, and addressing bias in neural network algorithms.
...
Method: Constructs categories L_T and P_T to represent text semantics, defines monoidal category P_T for semantic visualization, creates categories of configurations and embeddings with divergence decoration, and establishes equivalence between GloVe/Word2Vec and MDS algorithms.

Result: Provides dimension-agnostic definition of semantic spaces, mathematically precise method for comparing word embeddings, demonstrates equivalence between neural network algorithms and transparent frameworks, and offers approach for computing and mitigating biases.

Conclusion: The framework advances explainable AI by providing transparent mathematical foundations for word embeddings, enabling better comparison methods, bias analysis, and semantic space visualization through category theory.
...
TL;DR: A collaborative agent framework using three LLMs (Consultant, Reviewer, Programmer) with a "rewriting-resolution-review-revision" chain for scientific computing code generation, improving bug-free code rates and reducing non-physical solutions.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of reliable autonomous code generation for scientific computing problems by leveraging collaborative reasoning from multiple LLMs rather than relying on single models.

Method: Three-agent framework: Consultant rewrites problems with domain insights, Programmer generates executable code, and Reviewer provides self-debugging through iterative feedback and revision cycles.

Result: Significantly improved bug-free code generation rate, reduced non-physical solutions, and enhanced execution success rates compared to single-model approaches across PDEs, ill-conditioned systems, and data-driven physics problems.

Conclusion: The collaborative multi-agent framework establishes automatic code generation with review as a promising paradigm for scientific computing, providing highly reliable code generation from natural language descriptions.
...
<details>
  <summary>Details</summary>
Motivation: Traditional multi-agent RL solutions for bus bunching fail in realistic operations with heterogeneous routes, timetables, and fluctuating demand, suffering from data imbalance and convergence issues.
...
Conclusion: Single-agent deep RL with categorical structuring and schedule-aware rewards provides a robust, scalable alternative to MARL frameworks, particularly effective where agent-specific experiences are imbalanced.
...
TL;DR: A dynamic benchmark system for medical guidelines using graph-based approach with 400+ questions covering 3.3+ trillion combinations, enabling systematic evaluation of clinical capabilities and LLM training.


<details>
  <summary>Details</summary>
Motivation: To address coverage limitations of manually curated medical benchmarks and create a scalable, contamination-resistant solution that can systematically evaluate clinical guideline understanding across all possible relationships.

Method: Transformed WHO IMCI handbook into a directed graph with 200+ nodes and 300+ edges, then used graph traversal to generate questions with age-specific scenarios and contextual distractors for clinical relevance.

Result: Models achieved 45-67% accuracy across clinical tasks, excelling at symptom recognition but struggling with triaging severity, treatment protocols, and follow-up care. The approach successfully enhanced LLM post-training without expensive human annotation.

Conclusion: Graph-based dynamic MCQA methodology provides comprehensive benchmark coverage, identifies specific clinical capability gaps missed by general evaluations, and offers a scalable solution for creating contamination-resistant benchmarks that adapt to guideline updates.
...
Method: Proposed a Multi-objective Genetic Algorithm that models hospital unit scheduling as a multi-objective optimization task, incorporating real-world complexities like hourly appointment-driven demand and modular shifts for multi-skilled workforce.

Result: The MOO-GA generated robust and balanced schedules with 66% performance improvement over conventional manual scheduling baselines, effectively managing trade-offs between operational and staff-centric objectives.

Conclusion: The approach provides a practical decision support tool for nurse managers and hospital administrators to create optimal workforce schedules that balance competing healthcare operational requirements.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of Large Language Models in solving discrete reasoning and optimization problems from natural inputs, by creating a hybrid neural-symbolic approach.

Method: Introduces a probabilistic loss function that learns both constraints and objectives, removing the combinatorial solver from the training loop for scalable training while maintaining exact inference capabilities.

Result: Efficiently learns NP-hard reasoning problems from natural inputs, outperforms other hybrid methods on Sudoku variants with less training time, optimizes regret better than specialized methods on visual Min-Cut/Max-cut tasks, and successfully learns protein design energy optimization.

Conclusion: The proposed neuro-symbolic architecture provides an effective and scalable solution for learning complex discrete reasoning problems from natural inputs, demonstrating superior performance across multiple challenging benchmarks.
...
TL;DR: ChatThero is a multi-agent conversational framework that combines patient modeling with therapeutic dialogue using CBT and motivational interviewing techniques, showing significant improvements in patient motivation and treatment efficiency compared to GPT-4o.
...
Conclusion: ChatThero provides a privacy-preserving framework for therapeutic conversation research and offers a robust, replicable basis for clinical translation in addiction recovery support.
...
TL;DR: Systematic review showing multilingual NLP models exhibit similar social biases as English models, highlighting gaps in current bias evaluation/mitigation approaches across languages and cultures.


<details>
  <summary>Details</summary>
Motivation: To analyze how social bias research extends beyond English contexts and identify methodological gaps in multilingual bias evaluation and mitigation approaches.

Method: Systematic review of emerging research on multilingual bias evaluation and mitigation, examining linguistic diversity, cultural awareness, evaluation metrics, and mitigation techniques.

Result: Identified significant gaps including preference for certain languages, scarcity of multilingual mitigation experiments, and issues in adapting bias benchmarks across languages and cultures.
...
Method: Two-fold approach: 1) compared fine-tuned Gemma (2B) with untuned GPT-3.5 (175B), 2) evaluated seven structured prompting strategies (zero-shot, few-shot, chain-of-thought, role-based, sequential, and combinations). Used automated metrics and expert scoring across five dimensions, plus GPT-4.1 trained on expert-rated samples for large-scale simulation.
...
<details>
  <summary>Details</summary>
Motivation: Growing complexity of cyber-physical systems requires efficient cross-domain co-simulation, but SystemC TLM has limited interoperability with other engineering domains
...
Result: Feasibility and effectiveness demonstrated through representative case studies
...
Method: Distillation-Guided Policy Optimization (DGPO) uses cold-start initialization from teacher demonstrations and continuous teacher guidance during policy optimization, along with ARC metrics for evaluation.
...
Conclusion: DGPO successfully addresses the challenges of training compact language models for agentic RAG, demonstrating superior performance and feasibility for deployment in computing resource-constrained environments.
...
TL;DR: GUARD is a testing framework that converts high-level AI ethics guidelines into actionable test questions to verify LLM compliance, including jailbreak diagnostics to identify potential safety bypass scenarios.


<details>
  <summary>Details</summary>
Motivation: As LLMs become more prevalent, their potential to generate harmful responses raises societal concerns. Existing government ethics guidelines are too high-level and lack actionable testing methods to verify compliance.

Method: GUARD automatically generates guideline-violating questions from government guidelines to test LLM adherence. GUARD-JD extends this with jailbreak diagnostics to provoke unethical responses and identify safety mechanism bypass scenarios.

Result: Tested on 7 LLMs (Vicuna-13B, LongChat-7B, Llama2-7B, Llama-3-8B, GPT-3.5, GPT-4, GPT-4o, Claude-3.7) under three government guidelines. GUARD-JD can also transfer jailbreak diagnostics to vision-language models.

Conclusion: GUARD provides an effective method to operationalize ethics guidelines into actionable testing, generating compliance reports that highlight violations and promote reliable LLM applications.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with long contexts due to memory limitations, inability to handle complex long-context tasks, lack of transparency, and tendency to produce hallucinations.
...
<details>
  <summary>Details</summary>
Motivation: Current Long CoT reasoning development relies on costly human-curated datasets, leaving scalable alternatives unexplored. NP-hard graph problems offer inherent deep reasoning characteristics suitable for training.

Method: Two-stage post-training: 1) Long CoT Supervised Fine-Tuning on rejection-sampled NPH graph instances, 2) Reinforcement Learning with fine-grained reward design to improve reasoning efficiency.

Result: Graph-R1-7B model demonstrates strong generalization across mathematics, coding, STEM, and logic, outperforming QwQ-32B on NPH graph problems in both accuracy and reasoning efficiency.
...
TL;DR: The paper introduces CAPE, a context-aware framework for evaluating LLM personality traits using conversational history, revealing that context enhances consistency but causes personality shifts, with different models showing varying sensitivity to context and question ordering.


<details>
  <summary>Details</summary>
Motivation: Traditional psychometric tests for LLMs use context-free approaches that ignore real-world conversational dynamics where prior interactions shape responses, creating an artificial evaluation environment.

Method: Proposed Context-Aware Personality Evaluation (CAPE) framework incorporating conversational history, with novel metrics to quantify response consistency across 7 different LLMs including GPT and Gemini models.

Result: Conversational history enhances response consistency through in-context learning but induces personality shifts, with GPT models showing extreme deviations. GPT models are robust to question ordering while Gemini-1.5-Flash and Llama-8B are sensitive. GPT responses stem from both intrinsic traits and prior interactions, while others depend heavily on prior interactions.

Conclusion: Context-dependent personality shifts improve response consistency and better align with human judgments, particularly for Role Playing Agents, demonstrating the importance of context-aware evaluation frameworks for realistic LLM personality assessment.
...
TL;DR: Analyzing reasoning step utility in LLMs shows that decreasing conditional entropy during generation correlates with correct answers, while flat/increasing entropy indicates wrong answers. Incorrect reasoning paths tend to be longer.


<details>
  <summary>Details</summary>
Motivation: While LLMs generate intermediate reasoning steps to improve accuracy, there's limited understanding of how each step contributes to final answer correctness. The stochastic nature of autoregressive generation means more context doesn't guarantee better results, so predicting step utility could enable early stopping or pruning of ineffective reasoning.

Method: Oracle study on MATH dataset using Qwen2.5-32B and GPT-4o to generate reasoning chains, then employing Qwen3-8B to quantify utility. Measured model uncertainty on answer span using conditional entropy (expected negative log-likelihood) at each reasoning step as context expands.
...
Conclusion: Longer reasoning doesn't necessarily yield better outcomes. These findings provide foundation for designing efficient reasoning pipelines that can detect and avoid unproductive reasoning early during generation.
...
Method: Used expert pairwise comparison across 10 tools, 30 prompts, and 300 generated sites with 4000+ expert judgments, employing a TrueSkill-derived model for ranking with calibrated confidence intervals.

Result: Created a reproducible standard for advancing AI-driven web design, including complete prompt set, open-source evaluation framework, and public leaderboard.

Conclusion: UI-Bench establishes the first comprehensive benchmark for evaluating visual excellence in AI text-to-app tools, providing standardized evaluation and public leaderboard for ongoing assessment.
...
TL;DR: DentalBench is the first comprehensive bilingual benchmark for evaluating LLMs in dentistry, featuring 36,597 QA questions and a 337M-token corpus, revealing significant performance gaps and showing domain adaptation substantially improves dental knowledge.
...
Method: Created DentalBench with two components: DentalQA (bilingual question-answering benchmark with 36,597 questions across 4 tasks and 16 subfields) and DentalCorpus (337.35M token corpus for domain adaptation). Evaluated 14 LLMs including proprietary, open-source, and medical-specific models.

Result: Significant performance gaps were revealed across task types and languages. Domain adaptation with Qwen-2.5-3B substantially improved performance, particularly on knowledge-intensive and terminology-focused tasks.

Conclusion: Domain-specific benchmarks are crucial for developing trustworthy and effective LLMs tailored to healthcare applications, with domain adaptation showing substantial benefits for specialized medical fields like dentistry.
...
TL;DR: KG-CQR is a knowledge graph-enhanced framework that improves retrieval in RAG systems by enriching query contexts through structured relation representations and KG subgraph completion, achieving 4-6% mAP and 2-3% Recall@25 improvements.


<details>
  <summary>Details</summary>
Motivation: Existing RAG systems suffer from context loss at the retrieval phase, particularly with complex queries. Knowledge graphs offer structured relational information that can enhance query understanding and retrieval effectiveness.

Method: Proposes KG-CQR framework with three modules: subgraph extraction from corpus-centric KG, subgraph completion, and contextual generation. It's model-agnostic and operates as a pipeline without requiring additional LLM training.

Result: Achieves 4-6% improvement in mAP and 2-3% improvement in Recall@25 on RAGBench and MultiHop-RAG datasets. Shows consistent performance gains on challenging multi-hop QA tasks compared to baseline models.

Conclusion: KG-CQR effectively enhances retrieval in RAG systems by leveraging knowledge graphs for query enrichment, demonstrating significant performance improvements while maintaining scalability across different LLM sizes.
...
<details>
  <summary>Details</summary>
Motivation: Civil aviation maintenance requires sophisticated reasoning but lacks specialized evaluation tools for LLMs. Current LLM evaluation focuses mainly on mathematical and coding tasks, leaving a gap in industrial applications.

Method: Developed an industrial-grade benchmark specifically for civil aviation maintenance to measure LLM capabilities, identify knowledge gaps, and evaluate existing vector embedding models and LLMs using RAG systems.
...
Conclusion: This work fills a significant gap in LLM evaluation by providing a domain-specific benchmark that enables targeted improvements and advances intelligent solutions in civil aviation maintenance.
...
Method: Uses Case Base Reasoning with TF-IDF for text vectorization and Cosine Similarity for calculating similarity values between titles/keywords.
...
Conclusion: The CBR approach with TF-IDF and Cosine Similarity is effective for practical work title search and recommendation systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to adequately evaluate LLMs' capabilities in realistic multi-step tool usage, cross-tool coordination, and planning/reasoning for complex tasks, as they rely on explicit tool specifications and shallow workflows.

Method: Built on Model Context Protocol (MCP), the benchmark connects LLMs to 28 live MCP servers with 250 tools across domains like finance, travel, scientific computing, and academic search, enabling authentic multi-step tasks with rich input-output coupling.

Result: Experiments on 20 advanced LLMs revealed persistent challenges in handling the benchmark's complex requirements, demonstrating that current models struggle with realistic multi-step tool usage and coordination.

Conclusion: MCP-Bench provides a more comprehensive evaluation framework for LLM tool-use capabilities, highlighting significant gaps in current models' abilities to handle realistic multi-step tasks requiring planning, coordination, and cross-domain workflow orchestration.
...
Method: Developed and evaluated a deep learning framework using natural language processing techniques on two real-world EHR datasets, incorporating medical prompts, free-texts, and pre-trained sentence encoder. Performed ablation studies and robustness testing against structured data corruption.

Result: The model improved performance by 1.6%/0.8% on BACC/AUROC for mortality prediction, 0.5%/2.2% on RMSE/MAE for LOS prediction, and 10.9%/11.0% on RMSE/MAE for surgical duration estimation compared to best existing methods. Showed superior performance across tasks at different corruption rates.

Conclusion: The framework is an effective and accurate deep learning approach for predicting mortality and resource utilization in critical care, demonstrating success with prompt learning and transformer encoder for multimodal EHR analysis, and strong resilience to data corruption especially at high levels.
...
TL;DR: ConspirED dataset captures cognitive traits of conspiracy theories for developing AI detection models and testing LLM vulnerabilities to conspiratorial content.


<details>
  <summary>Details</summary>
Motivation: Conspiracy theories erode public trust and resist debunking, while AI-generated misinformation becomes more sophisticated, requiring better understanding of rhetorical patterns for interventions.

Method: Created ConspirED dataset with multi-sentence excerpts from online conspiracy articles annotated using CONSPIR cognitive framework, then developed computational models and tested LLM robustness.

Result: Both computational models and LLMs were misaligned by conspiratorial content, producing output that mirrors input reasoning patterns even when deflecting fact-checked misinformation.

Conclusion: The study demonstrates vulnerabilities in current AI systems to conspiratorial content and provides a framework for developing better detection and intervention tools.
...
<details>
  <summary>Details</summary>
Motivation: To identify critical shortcomings in the widely used FLORES+ multilingual MT benchmark and assess its suitability for evaluating truly multilingual translation systems.

Method: Human assessments of translations in four languages (Asante Twi, Japanese, Jinghpaw, South Azerbaijani), analysis of cultural bias, testing simple heuristics like named entity copying, and comparing model performance on FLORES+ vs domain-relevant evaluation sets.

Result: Many translations fall below the claimed 90% quality standard, source sentences show cultural bias toward English-speaking world, simple heuristics yield non-trivial BLEU scores, and models trained on high-quality naturalistic data perform poorly on FLORES+ but better on domain-relevant sets.
...
TL;DR: SciTopic is a novel topic discovery method that leverages large language models to enhance scientific topic identification by capturing comprehensive textual information and optimizing embedding space through entropy-based sampling and triplet contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Existing topic discovery methods rely on word embeddings that lack comprehensive understanding of scientific publications and struggle with complex text relationships. LLMs' exceptional textual comprehension capabilities can improve scientific topic identification.

Method: 1) Build textual encoder for scientific publications (metadata, title, abstract) 2) Space optimization module with entropy-based sampling and LLM-guided triplet tasks 3) Fine-tune encoder using contrastive loss on triplets to better discriminate different topics
...
Conclusion: SciTopic enables researchers to gain deeper and faster insights into scientific literature by leveraging LLMs for enhanced topic discovery, demonstrating superior performance over existing methods.
...
Method: Organized four shared tasks: two established tasks (b and Synergy) and two new tasks focusing on multilingual clinical entity detection in cardiology (MultiCardioNER) and nested NER in Russian/English (BIONNE).
...
TL;DR: Overview of BioASQ 2025 challenge featuring 6 tasks with 83 teams and 1000+ submissions, showing continued advancement in biomedical semantic indexing and QA.
...
Conclusion: BioASQ 2025 successfully continued its mission of advancing biomedical NLP through diverse challenge tasks, with strong participation and performance demonstrating ongoing progress in the state-of-the-art.
...
<details>
  <summary>Details</summary>
Motivation: Existing federated learning approaches primarily focus on label diversity but neglect language domain diversity, which is crucial for natural language processing tasks in real-world environments with non-IID data distributions.

Method: Introduces a comprehensive set of multi-domain non-IID scenarios and proposes AdaFD framework that adaptively handles both homogeneous and heterogeneous settings to capture local client diversity.

Result: Experimental results show that the proposed models effectively capture local client diversity and achieve superior performance compared to existing federated learning approaches.

Conclusion: The AdaFD framework successfully addresses multi-domain non-IID challenges in federated learning, providing a robust solution for real-world NLP applications with diverse language domains and data distributions.
...
Result: Outperforms production baseline and achieves state-of-the-art performance on multiple industry-relevant metrics. Demonstrates excellent deployment efficiency, handling ~50,000 queries per second with 55ms average latency using only 334 NVIDIA L20 GPUs.
...
TL;DR: KCS framework improves multi-hop QA by sampling diverse knowledge compositions using probabilistic contrastive loss and stochastic decoding, achieving 3.9% better knowledge selection and enhanced dataset performance.
...
Conclusion: KCS effectively expands question diversity by sampling varied knowledge compositions, demonstrating superior performance over competitive baselines in multi-hop question answering tasks.
...
TL;DR: Current GLM benchmarks are insufficient as they can be solved with unimodal information alone. The paper introduces CLEGR benchmark to properly evaluate multimodal graph-language reasoning and finds that current GLMs struggle with structural reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: To address the evaluation gap in Graph-Language Models (GLMs) where current benchmarks don't properly assess multimodal reasoning capabilities, as they can be solved using only unimodal information.

Method: Introduces CLEGR benchmark with synthetic graph generation pipeline and questions requiring joint structural and textual reasoning. Evaluates representative GLM architectures and compares them with soft-prompted LLM baselines.
...
Method: Utilizes speech sound features to retrieve candidate entities, then designs a generative method to annotate entity errors in ASR transcripts and replace text with correct entities.

Result: Tested on open-source and self-constructed test sets, the method demonstrates significant improvement in entity accuracy, especially in scenarios with word form differences.

Conclusion: The proposed NEC method effectively addresses limitations of phonetic-based approaches and shows substantial performance gains in entity correction, with plans to open source test sets and training data.
...
<details>
  <summary>Details</summary>
Motivation: To address the need for multi-lingual and multi-label classification in implicit discourse relation recognition, leveraging hierarchical dependencies between discourse senses in the PDTB 3.0 framework.

Method: HArch model with hierarchical dependencies between discourse senses, evaluated on DiscoGeM 2.0 corpus using RoBERTa and XLM-RoBERTa backbones, compared against GPT-4o and Llama-4-Maverick with few-shot prompting.

Result: RoBERTa-HArch best in English, XLM-RoBERTa-HArch best in multi-lingual setting. Fine-tuned models consistently outperform LLMs. Achieved SOTA results on DiscoGeM 1.0 corpus.

Conclusion: Task-specific fine-tuning with hierarchical approach is more effective than prompting large language models for implicit discourse relation recognition, demonstrating the value of specialized architectures over general LLMs.
...
TL;DR: The paper addresses tokenization inconsistency issues in LLM-based steganography and watermarking, proposing two solutions that improve performance metrics.


<details>
  <summary>Details</summary>
Motivation: Large language models have improved text generation but created tokenization consistency challenges between sender and receiver in steganography/watermarking, undermining robustness.

Method: Proposed stepwise verification for steganography and post-hoc rollback for watermarking to eliminate problematic tokens characterized by infrequency and temporariness.

Result: Experiments showed improvements in fluency, imperceptibility, anti-steganalysis for steganography, and enhanced detectability/robustness against attacks for watermarking.

Conclusion: Directly addressing tokenization inconsistency through tailored solutions significantly improves the performance and robustness of both steganography and watermarking systems.
...
TL;DR: rStar2-Agent is a 14B math reasoning model using agentic reinforcement learning that achieves state-of-the-art performance with advanced cognitive behaviors like autonomous code verification and refinement.


<details>
  <summary>Details</summary>
Motivation: To develop a more efficient math reasoning agent that can think carefully before using tools, reflect on code execution feedback, and achieve frontier-level performance with minimal compute resources compared to larger models.

Method: Uses agentic reinforcement learning with three key innovations: efficient RL infrastructure with reliable Python environment, GRPO-RoC algorithm with Resample-on-Correct strategy to handle coding noise, and multi-stage training from SFT to RL stages.

Result: Achieves 80.6% on AIME24 and 69.8% on AIME25, surpassing DeepSeek-R1 (671B) with shorter responses. Trained in only 510 RL steps within one week using 64 MI300X GPUs.

Conclusion: Demonstrates that agentic RL can effectively scale to create advanced cognitive abilities in math reasoning and generalizes well to alignment, scientific reasoning, and tool-use tasks with minimal compute cost.
...
<details>
  <summary>Details</summary>
Motivation: Existing local DP text privatization methods require very high ε values for reasonable utility, making them impractical for real-world privacy protection needs.

Method: Leverages semantic triples for neighborhood-aware private document generation under local DP, combined with LLM post-processing to maintain text coherence.

Result: Achieves coherent text generation even at lower ε values while maintaining privacy guarantees, demonstrating effectiveness of the divide-and-conquer paradigm.

Conclusion: Semantic triple-based approach with LLM post-processing enables balanced privatization outputs at reasonable ε levels, highlighting the importance of coherence in privacy-utility tradeoffs.
...
TL;DR: Fine-tuning general-purpose LLM-based embedding models (Stella, Jasper, NV-Embed, E5) achieves state-of-the-art performance for implicit hate speech detection without needing external knowledge or complex pipelines.


<details>
  <summary>Details</summary>
Motivation: Implicit hate speech is challenging to detect due to its subtle, indirect nature without explicit derogatory language, requiring advanced methods beyond traditional approaches.

Method: Solely fine-tuning recent general-purpose embedding models based on large language models, without using external knowledge or additional context/sentiment data.

Result: Achieved up to 1.10 percentage points improvement for in-dataset evaluation and up to 20.35 percentage points improvement in cross-dataset evaluation (F1-macro score).

Conclusion: General-purpose LLM-based embedding models can be effectively fine-tuned for implicit hate speech detection, outperforming complex task-specific pipelines that rely on external knowledge.
...
Conclusion: GUARD effectively addresses the coherence-diversity trade-off with theoretical guarantees and practical efficiency improvements over existing contrastive search methods.
...
TL;DR: Synthetic therapy dialogues from LLMs lack emotional authenticity compared to real CBT sessions, showing less variability and weaker emotional dynamics.


<details>
  <summary>Details</summary>
Motivation: To assess whether LLM-generated therapy dialogues capture the nuanced emotional dynamics of real therapy sessions, particularly in Cognitive Behavioral Therapy.

Method: Adapted Utterance Emotion Dynamics framework to analyze emotional trajectories across valence, arousal, and dominance dimensions. Compared real sessions (transcribed from public videos) with synthetic dialogues from CACTUS dataset, examining both full dialogues and individual speaker roles.

Result: Synthetic dialogues are fluent but diverge from real conversations in emotional properties: real sessions show greater emotional variability, more emotion-laden language, and more authentic patterns of reactivity and regulation. Emotional arc similarity is low, especially for clients.

Conclusion: Current LLM-generated therapy data has limitations in emotional fidelity, highlighting the need for authentic emotional dynamics in mental health applications. Introduced RealCBT dataset to support future research.
...
TL;DR: ROSI is a white-box method that improves LLM safety by injecting a rank-one weight modification to steer activations toward refusal-mediating directions, increasing safety refusal rates without fine-tuning while preserving utility.


<details>
  <summary>Details</summary>
Motivation: Existing safety mechanisms in LLMs can be bypassed by removing specific representational directions, so the authors propose amplifying rather than removing safety-aligned directions to enhance model safety.

Method: Rank-One Safety Injection (ROSI) - computes a safety direction from harmful/harmless instruction pairs and applies a rank-one weight modification to all residual stream write matrices without fine-tuning.

Result: ROSI consistently increases safety refusal rates (evaluated by Llama Guard 3) while preserving performance on standard benchmarks (MMLU, HellaSwag, Arc). It can also re-align uncensored models using their latent safety directions.

Conclusion: Targeted weight steering is an effective, cheap mechanism to improve LLM safety that complements resource-intensive fine-tuning approaches.
...
<details>
  <summary>Details</summary>
Motivation: Rising youth mental health issues need automated detection of cognitive distortions for early intervention, but prior work focused only on English clinical data.
...
Conclusion: Domain adaptation is crucial for effective cross-lingual cognitive distortion detection in non-clinical digital text data.
...
<details>
  <summary>Details</summary>
Motivation: Small Language Models struggle with minority class relations in imbalanced datasets, while Large Language Models using manual prompts may introduce noise and fail to properly capture long-distance dependencies between events.

Method: Proposes GDLLM with distance-aware graph structure using Graph Attention Network (GAT) to help LLMs capture long-distance dependencies, and a temporal feature learning paradigm based on soft inference to enhance short-distance relation identification.
...
Method: Developed a scalable framework to create evaluation benchmarks (MSRS-Story and MSRS-Meet) for multi-source retrieval and synthesis tasks, tested various RAG pipelines with sparse/dense retrievers and frontier LLMs.

Result: Generation quality highly depends on retrieval effectiveness, which varies by task. Multi-source synthesis is challenging even with oracle retrieval, but reasoning models significantly outperform standard LLMs.

Conclusion: The framework enables better evaluation of RAG systems for complex multi-source tasks, highlighting the importance of both effective retrieval and advanced reasoning capabilities for information synthesis.
...
<details>
  <summary>Details</summary>
Motivation: Quantization is essential for deploying large language models on resource-constrained hardware, but its implications for multilingual tasks remain underexplored.

Method: Evaluated post-training quantization (PTQ) on machine translation across 55 languages using five LLMs (1.7B to 70B parameters), comparing four quantization techniques (AWQ, BitsAndBytes, GGUF, and AutoRound) and analyzing interactions with decoding hyperparameters and calibration languages.

Result: 4-bit quantization preserves translation quality for high-resource languages and large models, but significant degradation occurs for low-resource and typologically diverse languages, particularly at 2-bit. GGUF variants provide most consistent performance even at 2-bit precision. Language-matched calibration offers benefits primarily in low-bit scenarios.

Conclusion: Findings offer actionable insights for deploying multilingual LLMs for machine translation under quantization constraints, especially in low-resource settings. Algorithm choice and model size jointly determine robustness.
...
TL;DR: SageLM is an end-to-end, multi-aspect speech LLM for evaluating speech-to-speech models, jointly assessing semantic and acoustic dimensions with rationale-based supervision and achieving 82.79% human agreement.


<details>
  <summary>Details</summary>
Motivation: Evaluating Speech-to-Speech LLMs is challenging, and existing cascaded approaches ignore acoustic features while rule-based methods lack explainability and alignment with human evaluation.

Method: Proposes SageLM with rationale-based supervision, uses SpeechFeedback synthetic preference dataset, and employs two-stage training to address speech preference data scarcity. Jointly assesses semantic and acoustic dimensions.
...
Conclusion: SageLM provides comprehensive, explainable evaluation for S2S LLMs by effectively combining semantic and acoustic assessment with improved human alignment.
...
TL;DR: A novel two-stage exemplar selection strategy for in-context learning that combines structure-aware BERT retriever fine-tuning with a plug-in module to enhance structural alignment in semantic parsing tasks.


<details>
  <summary>Details</summary>
Motivation: Existing ICL selection strategies overlook structural alignment for structured prediction tasks like semantic parsing, leading to suboptimal performance and poor generalization.
...
<details>
  <summary>Details</summary>
Motivation: Existing works focus on domain-specific or task-oriented scenarios, leading to fragmented evaluations and limiting comprehensive exploration of models' proactive conversation abilities.
...
Conclusion: The framework enables comprehensive evaluation of proactive dialogue capabilities, and the study investigates how reasoning capabilities influence proactive behaviors, providing implications for future model development.
...
Method: LETHE uses internal knowledge dilution by training a clean model and merging it with backdoored models, plus external prompt enhancement by incorporating benign evidence to distract attention from backdoor features.
...
TL;DR: EASI-RAG is a structured method for deploying RAG systems in SMEs, enabling fast implementation with high accuracy and user adoption despite limited NLP expertise.


<details>
  <summary>Details</summary>
Motivation: SMEs struggle to deploy RAG systems due to limited resources and NLP expertise, despite RAG's ability to address LLM limitations like hallucinations and outdated knowledge.

Method: EASI-RAG uses method engineering principles with defined roles, activities, and techniques. Validated through real-world case study in environmental testing lab, implementing RAG tool using operational procedure data.

Result: System deployed in under a month by team with no prior RAG experience, achieved high user adoption, accurate answers, and enhanced data reliability through iterative improvements.
...
Result: Outperforms state-of-the-art on common datasets but shows low performance on Wikidata; identifies label noise and re-representation capability as key factors affecting performance.

Conclusion: Noise in distantly supervised datasets and re-representation ability are significant challenges in sentential relation extraction, with capsule networks showing better re-representation capabilities than vanilla models.
...
TL;DR: A neuro-symbolic approach combining LLMs with symbolic solvers for tax filing achieves high accuracy and reduces costs below real-world averages.
...
Method: Integrates large language models with symbolic solvers, translates plain-text tax rules into formal logic programs, and uses intelligently retrieved exemplars for formal case representations.

Result: The system dramatically improves performance on the SARA dataset and reduces deployment costs well below real-world averages ($270 and 13 hours per filing).

Conclusion: Neuro-symbolic architectures show promise for equitable access to reliable tax assistance by delivering both high accuracy and economic feasibility.
...
<details>
  <summary>Details</summary>
Motivation: Human experts design materials by modifying known structures, which aligns with few-shot in-context learning, but existing LLM-based crystal generation approaches are limited to zero-shot scenarios and cannot benefit from few-shot learning.

Method: Proposes space-group based crystal tokenization to reduce complexity of modeling crystal symmetry, and introduces condition-structure aware hybrid instruction tuning framework with multi-task instruction tuning strategy.
...
Conclusion: CrystalICL successfully enables few-shot crystal generation by effectively capturing structure-property relationships from limited data through novel tokenization and instruction tuning approaches.
...
TL;DR: Adding learnable frequency filters to transformer-based models improves long time-series forecasting performance by 5-10% with minimal parameter increase (~1000), while enabling smaller and more effective architectures.


<details>
  <summary>Details</summary>
Motivation: Transformer-based models for long time-series forecasting suffer from bias toward low frequencies, high computational/memory requirements, and poor spectral utilization despite achieving state-of-the-art results.

Method: Added learnable frequency filters at the beginning of transformer-based models, which only add approximately 1000 additional parameters. Also reduced embedding dimensions while maintaining performance.

Result: Observed 5-10% relative improvement in forecasting performance across multiple instances. Created smaller yet more effective transformer architectures. Synthetic experiments showed better full-spectrum utilization.

Conclusion: Learnable frequency filters significantly enhance transformer-based models for long time-series forecasting by improving spectral utilization and enabling more efficient architectures with minimal parameter overhead.
...
<details>
  <summary>Details</summary>
Motivation: To bridge classical nonlinear filtering theory with modern transformer architectures by interpreting transformer operations through a probabilistic framework.

Method: Develop a probabilistic model where transformer signals represent conditional measures and layer operations are fixed-point updates, with explicit formulation for hidden Markov models.

Result: Provides a mathematical interpretation connecting transformer architectures to classical filtering theory, showing transformers as nonlinear generalizations of Wiener's linear predictor.

Conclusion: Transformers can be understood as performing fixed-point updates on conditional probability measures, establishing a theoretical connection between classical nonlinear filtering and modern deep learning architectures.
...
<details>
  <summary>Details</summary>
Motivation: While Knowledge Distillation is effective for model compression, it's not fully understood what factors improve student performance. The paper aims to identify key factors that contribute to successful KD.

Method: The authors demonstrate that teacher model calibration is crucial for effective KD. They propose simply employing calibration methods to reduce the teacher's calibration error, which can be easily integrated with existing state-of-the-art methods.

Result: The approach shows strong correlation between teacher calibration error and student accuracy. The algorithm is versatile and effective across various tasks including classification and detection, consistently achieving superior performance.

Conclusion: Teacher model calibration is an important factor for effective Knowledge Distillation. Reducing teacher's calibration error through calibration methods significantly improves KD performance and can be easily integrated with existing methods.
...
TL;DR: CLD is a simple, efficient coreset selection method that uses loss trajectory alignment with validation data to identify impactful training samples, outperforming state-of-the-art methods while avoiding costly computations.


<details>
  <summary>Details</summary>
Motivation: Deep learning models face scalability challenges in real-time or resource-constrained scenarios, requiring efficient methods to select the most impactful training samples without expensive computations.

Method: Correlation of Loss Differences (CLD) measures alignment between training sample loss trajectories and held-out validation set loss trajectories, using only per-sample loss values from training checkpoints without gradient or curvature computations.
...
Conclusion: CLD provides a principled, efficient, stable, and transferable tool for scalable dataset optimization with theoretical convergence guarantees and practical performance benefits.
...
TL;DR: Novel spectral graph clustering optimization for transductive node classification that synergizes graph structure with node-specific information (labels/features), enabling perfect community recovery under milder conditions than graph clustering alone.


<details>
  <summary>Details</summary>
Motivation: To improve transductive node classification by leveraging both graph structure and node-specific information (labels and features) in a unified optimization framework, addressing limitations of using graph clustering alone.

Method: Proposed a novel optimization problem that incorporates node-specific information within a spectral graph clustering framework, with algorithmic solutions developed to solve this optimization.

Result: Demonstrated synergy between graph structure and node-specific information, showing that suitable node-specific information guarantees perfect community recovery under milder conditions than traditional graph clustering bounds.

Conclusion: The proposed method effectively combines graph structure with node-specific information for transductive node classification, achieving superior performance with less stringent requirements compared to graph clustering approaches alone, as confirmed by numerical experiments.
...
<details>
  <summary>Details</summary>
Motivation: Current autonomous experiments focus on optimizing predefined targets but limit discovery of unexpected phenomena. There's a need for frameworks that can actively explore and discover novel physical phenomena.

Method: Integrated Novelty Score-Strategic Autonomous Non-Smooth Exploration (INS2ANE) with two components: novelty scoring system to evaluate result uniqueness, and strategic sampling mechanism to explore under-sampled regions regardless of conventional promise.
...
Conclusion: The framework demonstrates potential for autonomous experiments to enhance scientific discovery depth while maintaining efficiency, promising accelerated research by uncovering new phenomena in complex experimental spaces.
...
Conclusion: Proposed framework enables robust causal effect estimation in presence of hidden factors by combining latent variable modeling with double machine learning
...
TL;DR: Systematic review of 172 studies showing LLMs significantly advance genetic disease diagnostics through variant interpretation, medical imaging, and report generation, but face challenges in multimodal data integration and clinical implementation.


<details>
  <summary>Details</summary>
Motivation: Traditional statistical and machine learning methods struggle with complex, high-dimensional genetic data, creating a need to explore how large language models can address these challenges in genetic research and diagnostics.

Method: Conducted automated keyword-based search across PubMed, bioRxiv, medRxiv, and arXiv to identify studies on LLM applications in genetics diagnostics and education, followed by analysis of 172 relevant studies.

Result: LLMs based on transformer architectures excel in genomic variant identification, annotation, interpretation, and medical imaging analysis, significantly advancing disease stratification and report generation capabilities.

Conclusion: While transformer models show significant promise in transforming hereditary disease diagnostics and supporting genetic education, major challenges remain in multimodal data integration, generalizability, and practical clinical implementation.
...
TL;DR: Proposes VC metric to measure neural network approximation difficulty, identifies VC-tendency and minority-tendency phenomena, and develops a preprocessing framework based on VC for improved approximation.


<details>
  <summary>Details</summary>
Motivation: Neural networks suffer from unpredictable local performance that hinders reliability in critical applications, requiring a quantifiable measure to assess local value changes and approximation stability.

Method: Introduces VC metric to measure local value changes, investigates theoretical properties, identifies VC-tendency and minority-tendency phenomena, and proposes a variation-based distance metric and preprocessing framework.
...
Conclusion: VC provides a valuable tool for characterizing neural network approximation behavior, with identified phenomena and preprocessing framework offering practical improvements for neural network reliability and performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing per-channel PTQ methods require manual heuristic tuning or grid search to fix scaling factors, which is inefficient and time-consuming for model compression.
...
<details>
  <summary>Details</summary>
Motivation: Real-world RL requires adaptation to unseen conditions without costly retraining. Existing cMDP methods need explicit context variables, limiting their use when contexts are latent or hard to measure.
...
Result: DALI achieves significant gains over context-unaware baselines and often surpasses context-aware baselines in extrapolation tasks. Enables zero-shot generalization to unseen contextual variations.

Conclusion: DALI provides an effective framework for inferring latent contexts and achieving robust generalization without explicit context measurements, bridging perception and control in reinforcement learning.
...
TL;DR: FedReFT introduces federated representation fine-tuning that manipulates hidden representations instead of weights, achieving 7x-15x higher parameter efficiency than LoRA-based methods while handling data heterogeneity through novel ABM aggregation.


<details>
  <summary>Details</summary>
Motivation: ReFT shows promise for efficient fine-tuning but faces challenges in Federated Learning due to client heterogeneity in data distributions, model capacities, and computational resources.

Method: FedReFT applies sparse intervention layers to steer hidden representations directly and uses All-But-Me (ABM) aggregation where clients receive aggregated updates from others and partially incorporate them to balance local and global knowledge.
...
Method: Comprehensive survey with structured taxonomy categorizing MARL approaches by coordination models and learning algorithms, reviewing applications across key ITS domains and simulation platforms.

Result: Provides systematic classification of MARL methods and their applications in traffic signal control, autonomous vehicle coordination, logistics, and mobility systems, while identifying available simulation tools.

Conclusion: MARL shows promise for ITS but faces significant challenges including scalability, non-stationarity, credit assignment, communication constraints, and sim-to-real transfer gaps that hinder real-world deployment.
...
TL;DR: A novel framework for internal talent recommendation that models both WHAT employees do (task similarity) and HOW they work (collaboration patterns) using dual GCNs with adaptive gating, achieving 40.9% Hit@100 and high interpretability across job families.


<details>
  <summary>Details</summary>
Motivation: Traditional internal talent recommendation systems have structural limitations and often overlook qualified candidates due to narrow managerial perspectives, requiring a more comprehensive approach.

Method: Proposes a dual graph convolutional network framework that models semantic task similarity (WHAT) and structural interaction patterns (HOW) from email data, with adaptive fusion using a gating mechanism.

Result: Achieved top performance of 40.9% on Hit@100, significantly outperforming other fusion strategies and baselines. The model demonstrates high interpretability with context-aware fusion strategies for different job families.

Conclusion: Provides a quantitative and comprehensive framework for internal talent discovery that minimizes candidate omission risk and empirically determines optimal fusion between task alignment and collaborative patterns for different roles.
...
Method: Pre-train a vector-quantized graph autoencoder on diverse MIP instances without solution dependency, creating discrete code assignments as vocabulary to represent optimization instances.

Result: Forge embeddings effectively differentiate/cluster unseen instances (unsupervised) and enable single model predictions for warm-start variables and integrality gaps across multiple problem types (supervised), improving commercial solver performance.

Conclusion: Forge provides a scalable pre-training approach that generalizes across problem distributions, enabling both unsupervised analysis and supervised optimization tasks with a single model, advancing practical MIP embedding research.
...
<details>
  <summary>Details</summary>
Motivation: To demonstrate how adversaries can exploit LLMs' ethical alignment mechanisms to implant bias and enforce targeted censorship without degrading overall model performance.
...
Conclusion: The alignment mechanisms designed to make LLMs safe can be weaponized to inject targeted bias and censorship, highlighting critical vulnerabilities in current LLM safety approaches that require new defense strategies.
...
<details>
  <summary>Details</summary>
Motivation: Accurately quantifying geo-level marketing lift in two-sided marketplaces is challenging, with SCM often underestimating effects and DML methods lacking proper benchmarking against SCM.

Method: Built an open simulator mimicking large-scale geo roll-outs with customizable parameters, tested seven estimators (3 ASC variants and 4 panel-DML flavors) across five stress tests with 100 replications per scenario.
...
Conclusion: Proposes a 'diagnose-first' framework where practitioners identify the primary business challenge and select the specific DML model best suited for that scenario, providing a more robust blueprint for geo-experiment analysis.
...
TL;DR: CTXSEG adaptive segmentation method improves EEG seizure detection performance compared to fixed-length segmentation, requiring fewer segments while maintaining compatibility with standard machine learning approaches.
...
Method: Developed CTXSEG adaptive segmentation method that creates variable-length segments based on statistical differences in EEG data. Validated using synthetic data from CTXGEN signal generator and real-world EEG seizure detection application.

Result: CTXSEG improved seizure detection performance compared to fixed-length approaches using standardized framework, without modifying machine learning methods, and required fewer segments.

Conclusion: Adaptive segmentation with CTXSEG is a promising alternative to fixed-length segmentation that can be readily applied to modern machine learning approaches, potentially improving performance in EEG applications.
...
<details>
  <summary>Details</summary>
Motivation: To quantitatively understand the incremental learning phenomenon in neural networks where gradient flow learns matrix components sequentially with small initialization, which is attributed to implicit bias in optimization algorithms.
...
Result: Showed that incremental learning emerges from time-scale separation among dynamics of different matrix components, and that decreasing initialization scale enhances this separation, enabling effective low-rank approximations.
...
Method: Developed a multimodal machine-learning (MMML) architecture that learns from different modalities of the same data to predict performance metrics
...
Conclusion: MMML shows potential in supplementing traditional simulation workflows, bridging machine learning with real-world engineering applications, and accelerating design cycles
...
TL;DR: BiListing is a bimodal approach that combines text and image embeddings from Airbnb listings using large-language models and pretrained language-image models, enabling unified listing representation, zero-shot search, and overcoming cold start problems, resulting in significant revenue gains.


<details>
  <summary>Details</summary>
Motivation: Airbnb historically relied on structured data due to complexity of extracting meaningful information from unstructured text and images. The diverse unstructured data (multiple images, titles, descriptions, reviews) makes combining multiple embeddings challenging.

Method: Proposes BiListing (Bimodal Listing) approach that aligns text and photos using large-language models and pretrained language-image models to create single embedding vectors per listing and modality.

Result: Deployed in production with 0.425% NDCG gain and drove tens of millions in incremental revenue through offline and online tests in Airbnb search ranking model.

Conclusion: BiListing successfully captures unstructured data into unified embeddings, enables zero-shot semantic search, overcomes cold start problems, and provides effective listing-to-listing search capabilities across modalities.
...
<details>
  <summary>Details</summary>
Motivation: ECG signals are crucial for cardiac digital twins but often compromised by noise and artifacts, limiting diagnostic utility and real-time monitoring reliability.
...
Result: Achieved mean absolute error of 0.1285 and Pearson correlation coefficient of 0.9540, outperforming state-of-the-art baselines in SNR improvement and error metrics on MIT-BIH and NSTDB datasets.
...
<details>
  <summary>Details</summary>
Motivation: The self-attention mechanism in Transformers has quadratic complexity with sequence length, limiting applications in long-sequence tasks. Existing linear attention methods sacrifice performance through approximations or restrictive context selection.
...
Conclusion: TLinFormer successfully bridges the performance gap between efficient attention methods and standard attention, providing a linear-complexity solution without sacrificing model performance.
...
TL;DR: A new geometric regularization method called nonlinear conformal regularization is introduced for autoencoder decoders, enabling local deformation analysis and scalar curvature computation of learned manifolds.


<details>
  <summary>Details</summary>
Motivation: To improve dimensionality reduction by discovering main data factors and providing quantitative analysis of local deformations in learned latent representations.

Method: Introduces nonlinear conformal regularization for decoder maps in autoencoders, which allows local variations and creates a conformal factor field to measure local deformation. The method enables computation of scalar curvature of the learned manifold.

Result: The regularization technique successfully quantifies local deformations in latent space mappings and allows calculation of scalar curvature. Experiments on Swiss roll and CelebA datasets demonstrate practical implementation.

Conclusion: Nonlinear conformal regularization provides a powerful geometric tool for analyzing autoencoder-learned manifolds, offering quantitative insights into local deformations and enabling curvature computations that enhance understanding of latent space representations.
...
<details>
  <summary>Details</summary>
Motivation: Time-series forecasting models are increasingly used for real-world decisions but their performance variability and opaque nature create concerns about reliability and user trust.

Method: Combined traditional XAI methods with Rating Driven Explanations (RDE) to evaluate four model architectures (ARIMA, Gradient Boosting, Chronos, Llama) across four diverse datasets from finance, energy, transportation, and automotive domains.

Result: Feature-engineered models (Gradient Boosting) consistently outperformed foundation models in volatile/sparse domains (power, car parts) with better interpretability, while foundation models excelled only in stable/trend-driven contexts (finance).

Conclusion: Model selection should consider domain characteristics - feature-engineered models are preferable for volatile domains due to better performance and interpretability, while foundation models work well only in stable contexts.
...
TL;DR: The paper investigates diagonal state space model initialization from a frequency perspective, proposes S4D-DFouT initialization on discrete Fourier domain, and achieves SOTA results on Long Range Arena benchmark.
...
Method: The authors analyze diagonal SSM initialization from frequency perspective, propose S4D-DFouT initialization method on discrete Fourier domain, and study pole placement in initialization.

Result: The proposed approach achieves state-of-the-art results on Long Range Arena benchmark and enables training from scratch on very large datasets like PathX-256.

Conclusion: Systematic investigation of diagonal SSM initialization from frequency perspective leads to improved parameterization and scaling capabilities, demonstrating the importance of proper pole placement in initialization schemes.
...
<details>
  <summary>Details</summary>
Motivation: Address privacy and copyright concerns by enabling effective data removal from trained LLMs without full retraining, overcoming limitations of existing unlearning methods that lack proper forgetting boundaries.

Method: Uses entanglement-awareness guided loss reweighting to determine forgetting effort per sample based on similarity to retain samples, plus proxy constraint with ICL-generated test data to prevent over-forgetting.
...
Conclusion: EAGLE-PC provides a scalable, plug-and-play unlearning solution that effectively removes target data while maintaining model utility, addressing key privacy and ownership concerns in LLMs.
...
Method: Developed a unified benchmark with representative data, realistic privacy budgets, accounting for pre-training, and multiple evaluation metrics to assess state-of-the-art differentially private generation methods across five domain-specific datasets.
...
Conclusion: Current differentially private synthetic data methods have substantial limitations, highlighting the need for advanced privacy-preserving data sharing techniques and establishing evaluation standards for realistic scenarios.
...
Method: Bayesian approach with naive Bayesian classifier incorporating variable selection and weight learning. Uses discretization models for numerical data and value clustering for categorical data. Automatically constructs aggregates for multi-table databases through propositionalisation.

Result: Successfully handles databases with millions of individuals, tens of thousands of variables, and hundreds of millions of records in secondary tables. Provides predictive measure of variable importance and supports various ML tasks including classification, regression, and co-clustering.

Conclusion: Khiops is a scalable, open-source tool that effectively addresses large-scale multi-table database mining through its unique Bayesian methodology, making it suitable for complex real-world data analysis scenarios across multiple environments including Python and GUI interfaces.
...
TL;DR: MedGR² is a novel framework that creates a self-improving cycle to generate high-quality medical data for training vision-language models, overcoming data scarcity issues and enabling superior cross-modality generalization.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models in medicine suffer from scarce expert-annotated data, leading to poor generalization. Supervised Fine-Tuning performs poorly on unseen tasks, and Reinforcement Learning lacks reliable reward signals in data-scarce medical domains.

Method: MedGR² co-develops a data generator and reward model to automatically create high-quality multi-modal medical data. This data is used for both Supervised Fine-Tuning and Reinforcement Learning via Group Relative Policy Optimization (GRPO).
...
Conclusion: MedGR² transforms medical AI from data scarcity to data generation, enabling efficient learning and unlocking RL's potential for building truly generalizable medical AI systems in high-stakes domains.
...
TL;DR: Hyperparametric optimization of recommendation algorithms using a multi-criteria integral assessment approach instead of single-metric optimization.


<details>
  <summary>Details</summary>
Motivation: Traditional methods focus on optimizing single metrics, which doesn't account for the need to balance multiple performance aspects like accuracy, ranking quality, diversity, and resource efficiency in recommendation systems.
...
Result: The approach enables achieving balance between competing objectives (accuracy, ranking quality, output variety, and resource intensity) in recommendation algorithms.

Conclusion: The research provides a universal multi-criteria optimization tool applicable to recommendation systems and a wide range of machine learning and data analysis tasks, offering theoretical significance beyond traditional single-metric approaches.
...
Method: Proposes MERIT optimizer that leverages max-norm to calculate trust ratios for better constraining max attention logits, and constructs element-wise trust ratios to provide robust update scaling by focusing on local weight structures.
...
Conclusion: MERIT successfully improves training stability for large-batch training, paving the way for faster development of large language models by enabling larger batch usage.
...
TL;DR: Proposes exact stochastic inference algorithms for Gaussian Processes with finite-dimensional RKHS kernels, achieving better performance than approximation methods when memory constraints limit batch sizes.
...
Method: Develops algorithms for exact stochastic inference of GPs using kernels that induce moderate finite-dimensional RKHS. Extendable to infinite-dimensional RKHS with some approximation.

Result: Achieves better experimental results than existing methods when memory resources constrain batch size and number of inducing points.

Conclusion: Provides exact stochastic inference approach for GPs that outperforms approximation-based methods under memory constraints.
...
<details>
  <summary>Details</summary>
Motivation: Over-squashing in graph neural networks creates bottlenecks when gathering information from wide neighborhoods, and existing remedies like graph rewiring disrupt the original graph's topology and domain knowledge.

Method: Proposes Local Virtual Nodes (LVN) with trainable embeddings placed based on node centrality to identify bottlenecks. LVNs improve connectivity in central regions without adding more layers, preserving global structure.
...
Conclusion: LVNs effectively mitigate over-squashing while maintaining the input graph's global topology, providing a better alternative to methods that alter the original graph structure.
...
TL;DR: Proposes a task-based approach to verify survey credibility using model-specific distance metrics, with sample complexity independent of data dimension.
...
Result: Algorithm achieves sample complexity independent of data dimension, unlike traditional methods that scale linearly with dimensionality when reconstructing models.

Conclusion: Task-based verification provides efficient credibility assessment for surveys, enabling valid downstream research without exponential sample requirements.
...
<details>
  <summary>Details</summary>
Motivation: Many scientific contexts provide multi-trial supervision that can improve independent component analysis, but existing methods don't fully leverage this additional information.

Method: Blends proximal gradient-type algorithm in the space of invertible matrices with joint learning of a prediction model through backpropagation.
...
Conclusion: Incorporating multi-trial supervision through the proposed stochastic algorithm enhances ICA performance and component interpretability.
...
<details>
  <summary>Details</summary>
Motivation: Ultrasound signals are crucial for industrial NDT and SHM applications where labeled data is scarce and traditional signal processing is task-specific. MAEs have shown success in other domains but remain unexplored for 1D ultrasound analysis.

Method: Leveraged Masked Autoencoders with Vision Transformer architectures for self-supervised pre-training on unlabeled synthetic ultrasound signals. Systematically studied model size, patch size, and masking ratio impacts on pre-training efficiency and downstream accuracy.

Result: Pre-trained models significantly outperformed models trained from scratch and optimized CNN baselines. Pre-training on synthetic data demonstrated superior transferability to real-world measured signals compared to training on limited real datasets.

Conclusion: This study demonstrates the potential of MAEs for advancing ultrasound signal analysis through scalable, self-supervised learning, providing robust representations that enhance downstream task performance.
...
Method: Comprehensive set of graph algorithms as tools with preprocessing (retrieval) and postprocessing in a model context protocol (MCP) server that works with any modern LLM.

Result: GDS agent can solve a wide spectrum of graph tasks, providing accurate and grounded answers for questions requiring graph algorithmic reasoning.

Conclusion: The approach enables LLMs to handle graph data effectively, though challenges remain that require future roadmap development.
...
TL;DR: TV-HSGT is a novel distributed online optimization algorithm that combines hybrid stochastic gradient tracking with variance reduction for time-varying directed networks, achieving improved dynamic regret bounds without gradient boundedness assumptions.


<details>
  <summary>Details</summary>
Motivation: Existing distributed optimization algorithms rely on bounded gradient assumptions and overlook stochastic gradient impacts in time-varying directed networks, limiting their real-world applicability in dynamic environments.

Method: Proposes TV-HSGT algorithm that integrates row-stochastic and column-stochastic communication schemes over time-varying digraphs, combining current and recursive stochastic gradients with variance reduction mechanisms to track global descent directions accurately.

Result: Theoretical analysis shows improved dynamic regret bounds without gradient boundedness assumptions. Experimental results on logistic regression tasks confirm effectiveness in dynamic and resource-constrained environments.

Conclusion: TV-HSGT provides an effective solution for distributed online optimization in time-varying directed networks, eliminating need for Perron vector estimation while handling stochastic gradients and achieving strong performance in dynamic settings.
...
TL;DR: VarDiU proposes a variational diffusive upper bound with unbiased gradient estimation for one-step diffusion distillation, outperforming existing methods in quality and training efficiency.
...
Method: Proposes VarDiU (Variational Diffusive Upper Bound), a novel objective function that provides an unbiased gradient estimator for diffusion distillation, eliminating the bias from denoising score matching.
...
Conclusion: VarDiU's unbiased gradient estimation approach significantly improves diffusion distillation performance, making one-step generation more effective and training more stable.
...
<details>
  <summary>Details</summary>
Motivation: To enhance the reliability, generalizability, and interpretability of machine learning models in scientific and engineering domains by integrating physical knowledge with data-driven approaches.

Method: Combines physical models with machine learning techniques through various fusion strategies, though specific implementation details for complex chemical engineering applications require further development.

Result: PCML has shown significant benefits across diverse scientific domains, but technical challenges remain in determining optimal physical knowledge embedding, scaling to large datasets, and quantifying predictive uncertainty.

Conclusion: The perspective highlights both recent developments and ongoing challenges in applying physics-constrained ML to chemical engineering, particularly emphasizing closed-loop experimental design, real-time dynamics/control, and multi-scale phenomena handling as key opportunity areas.
...
Method: Design a neural operator by repeatedly applying a single neural operator block to progressively deepen the model without adding new blocks. Introduce an adaptive train-and-unroll approach that gradually increases depth during training, revealing accuracy scaling laws with model depth.

Result: Achieves state-of-the-art performance on standard benchmarks and demonstrates superior efficacy on challenging high-frequency ultrasound computed tomography problems, with significant computational savings through adaptive training strategy.

Conclusion: The framework provides a computationally tractable, accurate, and scalable solution for large-scale data-driven scientific machine learning applications, particularly effective for resolving complex wave phenomena through multigrid-inspired architecture.
...
TL;DR: This paper proposes a compositional approach to understand clinical time series as sequences of latent physiological states, enabling synthetic data generation that performs comparably to original data for time series forecasting tasks.


<details>
  <summary>Details</summary>
Motivation: To address sparse and low-resource data settings in clinical time series forecasting by uncovering the underlying compositional structure of physiological states and generating synthetic data that preserves meaningful patterns.

Method: Conceptualizes compositionality for time series, develops data-driven procedures to reconstruct elementary states and composition rules, and evaluates using empirical tests based on domain adaptation perspective comparing expected risk of forecasting models trained on original vs synthetic data.

Result: Training on compositionally synthesized data achieves comparable performance to training on original clinical data, and evaluation on synthetic test data shows similar results to original test data, outperforming randomization-based augmentation. Significant performance gains in SOFA score prediction when training entirely on synthesized data.

Conclusion: The compositional approach successfully captures meaningful physiological states and their systematic succession, enabling effective synthetic data generation that can alleviate data scarcity problems in clinical time series analysis while providing deeper understanding of clinical data structure.
...
TL;DR: RL-based fine-tuning poses greater risks than SFT for harmful misuse of LLMs. TokenBuncher defense suppresses response uncertainty to counter RL attacks while preserving utility.


<details>
  <summary>Details</summary>
Motivation: As LLMs grow more capable, reinforcement learning enables more effective harmful fine-tuning than supervised methods, creating new security threats that need specialized defenses.

Method: Proposes TokenBuncher defense using entropy-as-reward RL and Token Noiser mechanism to constrain model response uncertainty, preventing RL from exploiting reward signals for harmful behaviors.

Result: Extensive experiments show TokenBuncher robustly mitigates harmful RL fine-tuning across multiple models and algorithms while preserving benign task utility and fine-tunability.

Conclusion: RL-based harmful fine-tuning represents greater systemic risk than SFT, and TokenBuncher provides an effective, general defense specifically targeting this emerging threat.
...
<details>
  <summary>Details</summary>
Motivation: Existing EEG representation learning methods rely on simple masked reconstruction objectives that fail to capture the rich semantic information and complex patterns in EEG signals, especially when training data is limited.
...
Conclusion: EEGDM demonstrates strong generalizability and practical utility as a representation learner for EEG signals, effectively capturing EEG semantics through the diffusion-based generation approach.
...
TL;DR: Tool-augmented language models with external retrieval outperform pure memorization for factual recall, as memorization is limited by parameter count while tool-use enables unbounded recall through efficient circuits.


<details>
  <summary>Details</summary>
Motivation: To theoretically and empirically demonstrate the advantages of tool-augmented language models (external retrieval) over pure in-weight memorization for factual recall tasks.

Method: Theoretical analysis showing parameter count limits memorization capacity, circuit construction proving tool-use enables unbounded recall, and controlled experiments comparing tool-using vs memorizing models.
...
Conclusion: Tool-augmented workflows are not just practical but provably more scalable than pure memorization approaches, with both theoretical and empirical foundations supporting their superiority.
...
TL;DR: SAFEMax is a novel machine unlearning method for diffusion models that uses information theory to maximize entropy in generated images, causing the model to produce Gaussian noise for forbidden classes by stopping denoising early.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient method for machine unlearning in diffusion models that can selectively forget impermissible classes while retaining other knowledge, addressing the need for privacy and compliance in AI systems.

Method: Grounded in information-theoretic principles, SAFEMax maximizes entropy in generated images to produce Gaussian noise for impermissible classes by halting the denoising process early. It selectively focuses on early diffusion steps where class-specific information is prominent to control the balance between forgetting and retention.
...
Conclusion: SAFEMax provides an effective and efficient solution for machine unlearning in diffusion models by leveraging information theory and selective early-step processing to achieve controlled forgetting while maintaining model performance on retained classes.
...
Conclusion: cMALC-D framework provides more robust evaluation signals and better generalization for multi-agent systems in complex real-world scenarios.
...
TL;DR: A novel transformer-based framework for automated feature transformation that reduces computational costs while maintaining performance through GPT-based embedding space construction and multi-objective optimization.
...
Method: Four-step framework: transformation records collection, embedding space construction with revised GPT model, gradient-ascent search, and autoregressive reconstruction. The GPT model handles both feature transformation sequence reconstruction and model performance estimation.
...
<details>
  <summary>Details</summary>
Motivation: Current graph-based fraud detectors overlook two key temporal fraud patterns: recurring suspicious subgraph patterns (temporal motifs) and account-specific anomalous activity bursts that vary by entity.

Method: Uses Temporal Motif Extractor to condense transaction histories into informative motifs, dual-attention blocks (IntraA for within-motif interactions, InterA for cross-motif aggregation), and differentiable Adaptive Time-Window Learner to tailor observation windows per node.

Result: Outperforms seven strong anomaly-detection baselines across four real-world datasets, uncovering fraud patterns missed by earlier methods.

Conclusion: ATM-GAD successfully addresses temporal fraud patterns through motif-based analysis and adaptive time windows, demonstrating superior financial fraud detection capabilities.
...
<details>
  <summary>Details</summary>
Motivation: The broadcast nature of wireless transmissions makes IoT devices vulnerable to authentication attacks. Physical layer authentication using channel characteristics shows promise, but practical solutions for dynamic channel variations are lacking.

Method: Proposed a CNN-based Siamese network to learn temporal and spatial correlations between CSI pairs. Used synthetic training dataset generation based on WLAN TGn channel model with autocorrelation and distance correlation to reduce manual data collection overhead.
...
Conclusion: The deep learning-based physical layer authentication scheme effectively addresses mobile scenario challenges and shows superior performance over existing methods, making it suitable for practical IoT security applications.
...
TL;DR: LeMat-Traj is a standardized dataset of 120M+ atomic configurations from major DFT repositories, with harmonized formatting across multiple functionals, enabling better MLIP training and reducing force prediction errors.


<details>
  <summary>Details</summary>
Motivation: Machine learning interatomic potentials (MLIPs) face limitations due to fragmented, inconsistently formatted quantum mechanical trajectory datasets from DFT calculations that are expensive to generate but difficult to combine.

Method: Created LeMat-Traj by aggregating and standardizing data from Materials Project, Alexandria, and OQMD repositories, harmonizing results across PBE, PBESol, SCAN, and r2SCAN functionals, and filtering for high-quality configurations.

Result: Significant reduction in force prediction errors on relaxation tasks when fine-tuning models pre-trained on high-force data with LeMat-Traj. The dataset spans both low-energy states and high-energy structures.

Conclusion: LeMat-Traj lowers barriers for training transferable MLIPs and is complemented by LeMaterial-Fetcher, an open-source library for reproducible data integration, enabling continued evolution of large-scale materials datasets.
...
<details>
  <summary>Details</summary>
Motivation: Foundation models have transformed NLP and CV but remain underexplored in graph ML. Current GFMs mainly handle text-attributed graphs, leaving arbitrary feature types unaddressed - a problem similar to tabular data ML.

Method: Proposes G2T-FM that uses TabPFNv2 as backbone. Augments original node features with neighborhood feature aggregation, adds structural embeddings, then applies TabPFNv2 to the constructed node representations.
...
Conclusion: Reveals a previously overlooked direction of utilizing tabular foundation models for graph machine learning tasks, showing strong potential of this approach.
...
<details>
  <summary>Details</summary>
Motivation: Address sequential resource allocation problems where intervention effects evolve dynamically (habituation or recovery) and individual-level effects are unknown, applicable to health interventions, advertising, and workforce programs.

Method: Developed algorithms with theoretical guarantees on dynamic regret for combinatorial multi-armed bandits with nonstationary rewards, incorporating exploration-exploitation tradeoffs and heterogeneous individual rewards.
...
TL;DR: DiTree combines diffusion policies with sampling-based planners to achieve provably-safe kinodynamic motion planning that generalizes well to out-of-distribution scenarios while being 3x faster than classical methods.
...
Result: DiTree achieves 3x faster runtimes than classical SBPs and roughly 30% higher success rate than all other approaches, even when trained on a single environment and tested on out-of-distribution scenarios.
...
TL;DR: InSQuAD improves In-Context Learning by using Submodular Mutual Information to select high-quality and diverse examples through targeted selection and combinatorial training.
...
Method: Two main strategies: 1) Model ICL as targeted selection using SMI to mine relevant and diverse examples, 2) Combinatorial training paradigm with novel likelihood-based loss to learn SMI parameters that enforce both quality and diversity. Also uses synthetic paraphrases for training data augmentation.
...
Conclusion: InSQuAD effectively addresses the diversity gap in ICL retrieval models and demonstrates substantial performance gains across multiple benchmarks.
...
TL;DR: This paper provides the first theoretical convergence analysis for Subsampled Natural Gradient Descent (SNGD) and its accelerated variant SPRING, showing they are equivalent to regularized Kaczmarz methods and establishing convergence rates.


<details>
  <summary>Details</summary>
Motivation: SNGD has shown impressive empirical results in scientific machine learning but lacked theoretical explanation. The authors aim to bridge this gap by providing rigorous convergence analysis.
...
Conclusion: The work demonstrates how randomized linear algebra tools can explain the interplay between subsampling and curvature-aware optimization, providing theoretical foundation for SNGD's empirical success.
...
TL;DR: LoTUS is a novel Machine Unlearning method that removes training sample influence from pre-trained models without retraining, using probability smoothing and outperforms SOTA methods on efficiency and effectiveness.
...
Method: LoTUS smooths the prediction probabilities of pre-trained models up to an information-theoretic bound to mitigate over-confidence from data memorization, and introduces Retrain-Free Jensen-Shannon Divergence (RF-JSD) metric for real-world evaluation.

Result: Outperforms eight baselines on Transformer and ResNet18 models across five public datasets, including successful evaluation on large-scale ImageNet1k dataset where retraining is impractical.

Conclusion: LoTUS provides an efficient and effective solution for machine unlearning that avoids costly retraining while maintaining performance, making it suitable for real-world applications with large datasets.
...
<details>
  <summary>Details</summary>
Motivation: Molecular ML has shown great potential for accurate property predictions and exploring chemical space, but its application at chemical process scale remains largely unexplored and needs systematic integration.

Method: Review of state-of-the-art molecular ML models including graph neural networks and transformers, with discussion on incorporating physicochemical knowledge through hybrid/physics-informed approaches and process-scale integration.

Result: Identifies promising research directions for advancing molecular ML in chemical engineering, particularly through physics-informed methods and process-scale applications.

Conclusion: Molecular ML can accelerate novel molecule and process discovery, but requires creating benchmarks, practical validation, and industry collaboration to fully realize its potential in chemical process engineering.
...
### [235] [Artificial Intelligence for CRISPR Guide RNA Design: Explainable Models and Off-Target Safety](https://arxiv.org/abs/2508.20130)
*Alireza Abbaszadeh,Armita Shahlai*

Main category: q-bio.QM

TL;DR: AI and deep learning models are significantly improving CRISPR gRNA design by enhancing on-target efficiency prediction and off-target risk identification, with explainable AI techniques providing insights into the underlying biological mechanisms.


<details>
  <summary>Details</summary>
Motivation: Optimizing guide RNA (gRNA) design for CRISPR genome editing remains challenging despite technological advances, requiring better methods to predict efficiency and minimize off-target effects for clinical applications.

Method: Review of state-of-the-art machine learning and deep learning models for gRNA design, incorporating explainable AI (XAI) techniques to interpret model predictions and understand sequence features and genomic contexts affecting Cas enzyme performance.

Result: AI models demonstrate marked improvements in predicting gRNA on-target activity and identifying off-target risks, enabling more efficient and specific CRISPR applications with enhanced safety assessment capabilities.

Conclusion: The interdisciplinary convergence of AI and genome editing is enabling more clinically viable CRISPR applications through improved gRNA design optimization, with explainable AI providing crucial insights into the biological mechanisms driving editing efficiency and safety.
...
### [236] [Prediction of Distant Metastasis for Head and Neck Cancer Patients Using Multi-Modal Tumor and Peritumoral Feature Fusion Network](https://arxiv.org/abs/2508.20469)
...
Method: Integrated CT images, radiomics features, and clinical data using 3D Swin Transformer for deep feature extraction, radiomics feature selection, and multimodal fusion with fully connected network for metastasis risk prediction.

Result: Multimodal fusion model achieved AUC=0.803, accuracy=0.752, sensitivity=0.730, specificity=0.758, outperforming single-modality models. 3D deep learning alone achieved AUC=0.715.

Conclusion: The multimodal fusion model demonstrated high accuracy and robustness in predicting metastasis risk, offering comprehensive tumor representation and potential as a clinical decision-support tool for personalized treatment planning.
...
### [237] [Can LLMs Identify Tax Abuse?](https://arxiv.org/abs/2508.20097)
*Andrew Blair-Stanek,Nils Holzenberger,Benjamin Van Durme*

Main category: q-fin.CP

TL;DR: LLMs can discover and analyze complex US tax-minimization strategies, including interpreting, verifying, completing partial strategies, and generating novel end-to-end approaches that challenge even human experts.


<details>
  <summary>Details</summary>
Motivation: To assess if LLMs can handle the complex domain of US tax law, which involves navigating hundreds of thousands of pages of constantly updated legal documents, and potentially help recover lost tax revenue from wealthy taxpayers.

Method: Evaluated advanced LLMs on three capabilities: (1) interpreting and verifying tax strategies, (2) completing partially specified strategies, and (3) generating complete end-to-end strategies from scratch.
...
Conclusion: LLMs show significant potential to revolutionize tax agencies' efforts against tax abuse by discovering novel strategies and navigating complex tax law documentation that challenges human experts.
...
### [238] [Deep Reinforcement Learning for Optimal Asset Allocation Using DDPG with TiDE](https://arxiv.org/abs/2508.20103)
...
### [239] [QTMRL: An Agent for Quantitative Trading Decision-Making Based on Multi-Indicator Guided Reinforcement Learning](https://arxiv.org/abs/2508.20467)
*Xiangdong Liu,Jiahao Chen*

Main category: q-fin.PM

TL;DR: QTMRL is a reinforcement learning-based quantitative trading system that combines multi-dimensional technical indicators with A2C algorithm for adaptive portfolio management, outperforming traditional methods in various market conditions.


<details>
  <summary>Details</summary>
Motivation: Traditional quantitative trading models fail to adapt to dynamic market changes and black swan events due to rigid assumptions and limited generalization capabilities.

Method: Constructed multi-indicator dataset using 23 years of S&P 500 data for 16 stocks across 5 sectors, enriched with trend, volatility, and momentum indicators. Designed lightweight RL framework based on Advantage Actor-Critic (A2C) algorithm with data processing, A2C, and trading agent modules.

Result: Extensive experiments showed QTMRL outperformed 9 baselines (including ARIMA, LSTM, moving average strategies) across diverse market regimes in profitability, risk adjustment, and downside risk control.

Conclusion: QTMRL demonstrates superior performance in adaptive and stable portfolio management compared to traditional statistical and empirical trading models, with publicly available code for reproducibility.
...
Method: Three key strategies: (1) normalizing price features to remove sample-specific characteristics (return, volatility, price scale), (2) attention-based module for accurate characteristic estimation, (3) reintegrating characteristics into prediction. Combines geometric Brownian motion for long-term trends with neural networks for short-term patterns.
...
Conclusion: ReVol effectively mitigates distribution shifts in stock price data through its normalization and characteristic handling approach, significantly improving prediction performance compared to existing methods.
...
### [241] [Data-Efficient Point Cloud Semantic Segmentation Pipeline for Unimproved Roads](https://arxiv.org/abs/2508.20135)
...
### [242] [UltraEar: a multicentric, large-scale database combining ultra-high-resolution computed tomography and clinical data for ear diseases](https://arxiv.org/abs/2508.20141)
...
TL;DR: UltraEar Database is a large-scale multicentric repository of 0.1mm ultra-high-resolution CT images and clinical data for ear diseases, collected from 11 hospitals over 15 years, with standardized preprocessing and anonymization for research and AI development.


<details>
  <summary>Details</summary>
Motivation: Ear diseases affect billions worldwide with substantial health burdens. CT imaging is crucial for diagnosis and treatment, but there's a need for large-scale, high-resolution datasets to advance radiological research and AI development in otology.

Method: Recruitment from 11 tertiary hospitals (2020-2035), collection of isotropic 0.1mm U-HRCT images, structured CT reports, clinical data (demographics, audiometry, surgery records, pathology). Standardized preprocessing for calibration, annotation, and segmentation. Monthly expert panel coordination with secure offline cloud storage.

Result: Establishment of UltraEar Database - an unprecedented ultra-high-resolution reference atlas covering various otologic disorders with technical fidelity and clinical relevance. Data is anonymized and compliant with privacy regulations.

Conclusion: UltraEar provides a valuable resource for advancing radiological research, AI algorithm development, educational training, and multi-institutional collaborations in otologic imaging. The database will be continuously updated for long-term accessibility to the global research community.
...
### [243] [Is the medical image segmentation problem solved? A survey of current developments and future directions](https://arxiv.org/abs/2508.20139)
...
Method: In-depth review organized around seven key dimensions: learning paradigms, task focus, modality integration, foundation models, segmentation approaches, dimensionality, and system architecture.

Result: The review traces progress in multiscale analysis, attention mechanisms, prior knowledge integration, and identifies transitions from supervised to semi/unsupervised learning, organ to lesion segmentation, 2D to 3D/4D, and deterministic to probabilistic approaches.

Conclusion: The work provides a comprehensive framework for understanding medical image segmentation advancements and aims to inspire future innovation, with maintained repository for ongoing research support.
...
### [244] [A Machine Learning Approach to Volumetric Computations of Solid Pulmonary Nodules](https://arxiv.org/abs/2508.20127)
...
Conclusion: The framework provides a highly accurate, efficient, and scalable tool for clinical lung nodule screening and monitoring, with strong potential for improving early lung cancer detection.
...
### [245] [Efficient and Privacy-Protecting Background Removal for 2D Video Streaming using iPhone 15 Pro Max LiDAR](https://arxiv.org/abs/2508.20250)
*Jessica Kinnevan,Naifa Alqahtani,Toral Chauhan*

Main category: eess.IV

TL;DR: LiDAR on mobile devices enables real-time background removal using depth information, overcoming lighting limitations of traditional methods, with current resolution constraints but promising potential if improved.


<details>
  <summary>Details</summary>
Motivation: Traditional background removal techniques like chroma keying and AI models have lighting dependencies and performance issues in varying light conditions. LiDAR's depth-based approach provides lighting-independent background removal.

Method: Integrated iPhone 15 Pro Max's LiDAR and color cameras with GPU-based image processing using SwiftUI/Swift frameworks and Metal Shader Language for real-time enhancement at 60fps streaming.

Result: Successfully achieved real-time background removal at 60fps, though limited by current depth map resolution of 320x240 and material reflection constraints of LiDAR technology.

Conclusion: LiDAR shows strong potential as the preeminent background removal method for video and photography if resolution can be improved to match color image quality on mobile devices.
...
### [246] [GENRE-CMR: Generalizable Deep Learning for Diverse Multi-Domain Cardiac MRI Reconstruction](https://arxiv.org/abs/2508.20600)
*Kian Anvari Hamedani,Narges Razizadeh,Shahabedin Nabavi,Mohsen Ebrahimi Moghaddam*

Main category: eess.IV

TL;DR: GENRE-CMR is a GAN-based deep unrolled reconstruction framework that enhances CMR image reconstruction quality and generalization across diverse acquisition settings using residual connections, Edge-Aware Region loss, and Statistical Distribution Alignment loss.


<details>
  <summary>Details</summary>
Motivation: Accelerated CMR image reconstruction faces challenges in balancing scan time and image quality, especially when generalizing across different acquisition settings. There's a need for robust methods that maintain high fidelity while adapting to diverse protocols.

Method: Proposes GENRE-CMR: a GAN-based architecture with residual deep unrolled reconstruction framework. Uses cascaded convolutional subnetworks with residual connections for progressive feature propagation. Integrates two novel loss functions: Edge-Aware Region (EAR) loss to focus on structurally informative regions and prevent blurriness, and Statistical Distribution Alignment (SDA) loss with symmetric KL divergence to regularize feature space across diverse distributions.

Result: Achieves state-of-the-art performance with 0.9552 SSIM and 38.90 dB PSNR on unseen data distributions across various acceleration factors and sampling trajectories. Outperforms existing methods on both training and unseen data. Ablation studies confirm individual contributions of each component.

Conclusion: GENRE-CMR provides a unified and robust solution for high-quality CMR reconstruction that generalizes well across heterogeneous acquisition protocols, enabling clinically adaptable deployment with improved reconstruction fidelity and generalization capabilities.
...
### [247] [Efficient Fine-Tuning of DINOv3 Pretrained on Natural Images for Atypical Mitotic Figure Classification in MIDOG 2025](https://arxiv.org/abs/2508.21041)
...
<details>
  <summary>Details</summary>
Motivation: Traditional RBMs suffer from high computational costs in Gibbs sampling during content generation, creating bottlenecks in electronic implementations.

Method: Proposes a photonic RBM with efficient encoding that eliminates matrix decomposition needs and uses non-Von Neumann photonic computing architecture to avoid interaction matrix storage.

Result: Experimental validation shows accurate phase transition temperature matching theoretical predictions in 2D Ising model simulation. PRBM successfully generates and restores images and temporal sequences even with noise and aberrations.

Conclusion: PRBM offers scalable, low-training-cost framework with significant potential for advancing photonic computing in generative AI applications.
...
### [249] [A Hierarchical Signal Coordination and Control System Using a Hybrid Model-based and Reinforcement Learning Approach](https://arxiv.org/abs/2508.20102)
*Xianyue Peng,Shenyang Chen,H. Michael Zhang*

Main category: eess.SY

TL;DR: Hierarchical traffic signal control combining model-based optimization with reinforcement learning, using High-Level Coordinator to dynamically switch between Max-Flow and Green-Wave coordination strategies based on traffic demand.
...
Method: Hierarchical system with: 1) High-Level Coordinator selecting coordination strategies, 2) Corridor Coordinator deriving phase constraints, 3) Hybrid Signal Agents using reinforcement learning with action masking. Trained with PPO algorithm on SUMO-RLlib platform.

Result: Hybrid MFC maximizes throughput under heavy demand; hybrid GWC minimizes arterial stops and maintains progression; PAC improves network-wide travel time in moderate demand. Hierarchical design enables adaptive strategy selection across all demand levels.

Conclusion: The proposed hierarchical scheme achieves robust performance by dynamically switching between coordination strategies based on traffic conditions, balancing corridor-level and network-wide performance objectives.
...
### [250] [Neural Spline Operators for Risk Quantification in Stochastic Systems](https://arxiv.org/abs/2508.20288)
*Zhuoyuan Wang,Raffaele Romagnoli,Kamyar Azizzadenesheli,Yorie Nakahira*

Main category: eess.SY

TL;DR: Neural Spline Operators (NeSO) framework uses physics-informed neural operators with B-spline representations to efficiently quantify risk probabilities for stochastic systems with functional variations in dynamics, achieving significant speed-up over existing methods.
...
Method: Proposed Neural Spline Operators (NeSO) framework that leverages B-spline representations within physics-informed neural operators to learn mappings from functional system dynamics to risk probabilities, improving training efficiency and boundary condition enforcement.

Result: Theoretical analysis shows universal approximation capability. Case studies demonstrate efficacy for varying functional dynamics and high-dimensional multi-agent systems, with significant online speed-up over existing methods.

Conclusion: NeSO provides an effective framework for risk quantification with functional variations, and the approach has broader applicability to other control and PDE-related problems beyond risk quantification.
...
### [251] [Delay-adaptive Control of Nonlinear Systems with Approximate Neural Operator Predictors](https://arxiv.org/abs/2508.20367)
...
Method: Approximate the predictor using a learned neural operator mapping trained offline, then deployed online. Theoretical analysis based on universal approximation theorem, transport PDE representation of delay, and Lyapunov-Krasovskii functional for stability.

Result: Achieves semi-global practical convergence dependent on predictor approximation error and delay bounds. Validated on biological activator/repressor system with 15 times speedup compared to traditional numerical methods.

Conclusion: The proposed neural operator-based approach provides an efficient and theoretically sound solution for predictor feedback control in delayed nonlinear systems, with significant computational advantages over conventional methods.
...
### [252] [Unifying Diarization, Separation, and ASR with Multi-Speaker Encoder](https://arxiv.org/abs/2508.20474)
*Muhammad Shakeel,Yui Sudo,Yifan Peng,Chyi-Jiunn Lin,Shinji Watanabe*

Main category: eess.AS

TL;DR: Unified multi-speaker encoder that jointly learns speaker diarization, speech separation, and multi-speaker ASR using shared encoder with residual weighted-sum encoding for improved performance on overlapping speech.


<details>
  <summary>Details</summary>
Motivation: To capture inherent interdependencies between speaker diarization, speech separation, and multi-speaker ASR tasks through joint training, enhancing overall performance on overlapping speech data.
...
Result: Substantially improves over single-task baselines on LibriMix evaluation sets, achieving diarization error rates of 1.37% on Libri2Mix and 2.29% on Libri3Mix, outperforming previous studies.

Conclusion: The unified multi-speaker encoder successfully demonstrates that joint learning of related speech processing tasks through shared representations and residual weighted-sum encoding leads to significant performance improvements across all tasks.
...
### [253] [Is Audio Spoof Detection Robust to Laundering Attacks?](https://arxiv.org/abs/2408.14712)
*Hashim Ali,Surya Subramani,Shefali Sudhir,Raksha Varahamurthy,Hafiz Malik*

Main category: eess.AS

TL;DR: This paper evaluates state-of-the-art voice spoof detection systems against laundering attacks (reverberation and noise), finding they perform poorly and highlighting the need for more robust detection methods.


<details>
  <summary>Details</summary>
Motivation: Voice cloning technology has advanced significantly, enabling high-quality synthesized speech that can be abused. While detection methods exist, they are typically tested on clean audio, not real-world laundering attacks that could bypass detection.

Method: Created the ASVSpoof Laundering Database based on ASVSpoof 2019 with 1388.22 hours of audio, then evaluated seven SOTA audio spoof detection approaches against various laundering attacks including reverberation and additive noise.

Result: SOTA audio spoof detection systems perform poorly when faced with aggressive laundering attacks, particularly reverberation and additive noise attacks, showing significant vulnerability to these real-world attack scenarios.

Conclusion: Current voice spoof detection methods are not robust enough against laundering attacks, indicating an urgent need for developing more resilient detection systems that can handle real-world adversarial conditions.
...
### [254] [Automatic Inspection Based on Switch Sounds of Electric Point Machines](https://arxiv.org/abs/2508.20870)
*Ayano Shibata,Toshiki Gunji,Mitsuaki Tsuda,Takashi Endo,Kota Dohi,Tomoya Nishida,Satoko Nomoto*

Main category: eess.AS

TL;DR: IoT-based monitoring system using sound analysis to detect railway point machine failures, replacing visual inspections and enabling real-time failure detection.


<details>
  <summary>Details</summary>
Motivation: To reduce labor-intensive equipment inspections and enable preventive maintenance by replacing human visual inspections with automated monitoring systems, addressing the high cost of new sensors and difficulty in substituting electrical characteristic monitoring.

Method: Implemented cameras and microphones in electric point machines to monitor lock-piece conditions remotely. Developed a method for detecting turnout switching errors based on sound information analysis, specifically focusing on 'switch sound' patterns.

Result: Expected test results were obtained, demonstrating the feasibility of detecting equipment failures in real-time through sound analysis. The system successfully reduced downtime from equipment failures.

Conclusion: The proposed sound-based monitoring method provides an effective alternative to visual inspections, enabling real-time failure detection and contributing to labor savings and preventive maintenance in railway equipment monitoring.
...
### [255] [Multilingual Dataset Integration Strategies for Robust Audio Deepfake Detection: A SAFE Challenge System](https://arxiv.org/abs/2508.20983)
*Hashim Ali,Surya Subramani,Lekha Bollinani,Nithin Sai Adupa,Sali El-Loh,Hafiz Malik*

Main category: eess.AS

TL;DR: The paper presents a second-place SAFE Challenge solution using AASIST-based approach with WavLM frontend and RawBoost augmentation, achieving strong performance in synthetic speech detection across unmodified, compressed, and laundered audio tasks.


<details>
  <summary>Details</summary>
Motivation: To develop robust deepfake detection systems that can handle various audio manipulations including compression artifacts and deliberate evasion techniques (laundering), addressing the growing challenge of synthetic speech detection in real-world scenarios.

Method: AASIST-based architecture with WavLM large self-supervised learning frontend and RawBoost augmentation, trained on multilingual dataset of 256,600 samples from 9 languages and 70+ TTS systems. Systematic exploration of SSL front-ends, training data compositions, and audio length configurations.

Result: Achieved second place in both Task 1 (unmodified audio detection) and Task 3 (laundered audio detection), demonstrating strong generalization and robustness across different audio manipulation scenarios.

Conclusion: The combination of SSL front-ends, diverse training data, and systematic configuration optimization enables effective synthetic speech detection that generalizes well to various audio processing conditions and evasion attempts.
...
### [256] [Navigating the EU AI Act: Foreseeable Challenges in Qualifying Deep Learning-Based Automated Inspections of Class III Medical Devices](https://arxiv.org/abs/2508.20144)
*Julio Zanon Diaz,Tommy Brennan,Peter Corcoran*

Main category: cs.CY

TL;DR: Analysis of regulatory challenges for DL-based automated visual inspection of Class III medical devices under EU AI Act vs existing medical device regulations, focusing on risk management, validation, and compliance complexities.


<details>
  <summary>Details</summary>
Motivation: Deep learning technologies offer potential for automated visual inspection in medical devices but introduce new regulatory complexities under the EU AI Act that differ from established medical device frameworks like MDR and FDA QSR.

Method: High-level technical assessment examining divergences in risk management principles, dataset governance, model validation, explainability requirements, and post-deployment monitoring obligations between regulatory frameworks.

Result: Identifies significant challenges including data retention burdens, global compliance implications, and practical difficulties achieving statistical significance in validation with limited defect data. Highlights implementation strategies and areas of uncertainty.

Conclusion: Manufacturers face complex regulatory landscape when qualifying DL-based automated inspections. Paper serves as academic evaluation but emphasizes need for consultation with regulatory authorities and legal experts for specific compliance pathways.
...
### [257] [RelAItionship Building: Analyzing Recruitment Strategies for Participatory AI](https://arxiv.org/abs/2508.20176)
*Eugene Kim,Vaibhav Balloli,Berelian Karimian,Elizabeth Bondi-Kelly,Benjamin Fish*

Main category: cs.CY

TL;DR: Analysis of recruitment challenges in Participatory AI projects, examining 37 projects and interviewing researchers to provide recommendations for relationship-focused recruitment methods and documentation practices.
...
Method: Analyzed 37 AI projects to document recruitment practices and conducted interviews with 5 AI researchers to understand recruitment outcomes and challenges.

Result: Found that recruitment outcomes are shaped by structural work conditions, researchers' goals/expectations, and relationships built through recruitment. Identified diversity in current practices and specific strategies for equity and empowerment.

Conclusion: Provides recommendations for designing relationship-forward recruitment methods and reflexive documentation practices to improve Participatory AI implementation.
...
### [258] [Studying Effective String Theory using deep generative models](https://arxiv.org/abs/2508.20610)
*Michele Caselle,Elia Cellini,Alessandro Nada*

Main category: hep-lat

TL;DR: Effective String Theory provides a non-perturbative framework for confinement using string theory concepts, with recent numerical approaches using deep learning to solve complex problems like flux tube width.


<details>
  <summary>Details</summary>
Motivation: Traditional analytical methods using zeta-function regularization are insufficient for solving complex problems in Effective String Theory, particularly determining the flux tube width between quark-antiquark pairs.

Method: Employing deep learning techniques based on generative algorithms as a numerical approach to explore Effective String Theory, moving beyond traditional analytical methods.

Result: The paper presents numerical results for the width of the Nambu-Goto Effective String Theory, demonstrating the effectiveness of the deep learning approach.

Conclusion: Deep learning techniques provide a viable numerical alternative to traditional analytical methods for solving complex problems in Effective String Theory, particularly for determining flux tube properties.
...
### [259] [MM-HSD: Multi-Modal Hate Speech Detection in Videos](https://arxiv.org/abs/2508.20546)
...
Result: Outperforms state-of-the-art methods on HateMM dataset with M-F1 score of 0.874, particularly when on-screen text is used as query and other modalities serve as key.

Conclusion: The integration of multiple modalities with Cross-Modal Attention significantly improves hate speech detection in videos, with on-screen text playing a crucial role as query modality.
...
### [260] [SwizzlePerf: Hardware-Aware LLMs for GPU Kernel Performance Optimization](https://arxiv.org/abs/2508.20258)
*Arya Tschand,Muhammad Awad,Ryan Swann,Kesavan Ramakrishnan,Jeffrey Ma,Keith Lowery,Ganesh Dasika,Vijay Janapa Reddi*

Main category: cs.DC

TL;DR: SwizzlePerf uses LLMs with hardware-awareness to automatically generate optimal GPU kernel optimizations in minutes instead of weeks, achieving up to 2.06x speedup.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based GPU kernel optimization methods lack hardware-awareness, which human performance engineers rely on for optimal hardware utilization.

Method: SwizzlePerf provides LLMs with hardware-awareness through memory access patterns, architecture specs, filtered profiling logs, and historical performance reflections to generate spatial optimizations for GPU kernels.

Result: For GEMM kernel, achieved same optimal pattern in <5 min vs 2 weeks for experts. On 10 diverse kernels, generated patterns for 9 kernels with up to 2.06x speedup and 70% L2 hit rate improvement.

Conclusion: This represents the first step toward systematically creating hardware-aware LLM performance engineering agents for GPU kernel optimization.
...
### [261] [CoFormer: Collaborating with Heterogeneous Edge Devices for Scalable Transformer Inference](https://arxiv.org/abs/2508.20375)
*Guanyu Xu,Zhiwei Hao,Li Shen,Yong Luo,Fuhui Sun,Xiaoyan Wang,Han Hu,Yonggang Wen*

Main category: cs.DC

TL;DR: CoFormer is a collaborative inference system that decomposes large transformer models into smaller sub-models for distributed execution on edge devices, achieving significant speedup and memory reduction while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Transformer models have high computational demands that make real-time deployment on resource-constrained edge devices challenging. Existing solutions either cause high communication overhead through offloading or sacrifice accuracy through compression.

Method: CoFormer exploits the divisibility of transformers by decomposing large models into smaller sub-models for distributed inference. It uses an optimization formulation to minimize latency and accuracy degradation, with DeBo algorithm to derive decomposition policies and calibrate models.

Result: Achieves up to 3.1× inference speedup, enables GPT2-XL (1.6B parameters) on edge devices with 76.3% memory reduction, and reduces energy consumption by approximately 40% while maintaining satisfactory performance.

Conclusion: CoFormer provides an effective collaborative inference approach for transformer models on heterogeneous edge devices, balancing computational efficiency with model accuracy through intelligent model decomposition and distributed execution.
...
### [262] [Collaborating with GenAI: Incentives and Replacements](https://arxiv.org/abs/2508.20213)
*Boaz Taitler,Omer Ben-Porat*

Main category: cs.GT

TL;DR: GenAI can cause workers to exert zero effort even when ineffective, and excluding low-value workers can trigger output cascades. Manager's optimization is NP-complete but solvable for linear cases.


<details>
  <summary>Details</summary>
Motivation: To understand how Generative AI affects workplace collaboration when managers can use it to replace workers, analyzing effort dynamics and team selection.

Method: Theoretical framework with manager selecting teams, GenAI substituting unselected workers, workers choosing effort levels with increasing costs. Includes NP-complete analysis and efficient algorithm for linear cases, plus extensive simulations.

Result: GenAI leads to zero worker effort even when ineffective. Low-value workers are critical for sustaining output, and their exclusion causes cascading effects. Manager's problem is computationally hard.

Conclusion: GenAI fundamentally changes collaboration dynamics, creating perverse incentives where workers may not contribute despite AI limitations. Careful team selection is crucial to prevent productivity collapse.
...
### [263] [Balancing Profit and Traveller Acceptance in Ride-Pooling Personalised Fares](https://arxiv.org/abs/2508.20723)
*Michal Bujak,Rafal Kucharski*

Main category: cs.GT

TL;DR: Ride-pooling operators can learn individual travelers' price acceptance levels with over 90% accuracy in 10 days to optimize personalized fares and improve both user utility and operator profit.
...
Method: Proposed adaptive pricing policy where the operator constructs daily offers that progressively meet travelers' expectations and learn their behavioral traits to optimize personalized fares.

Result: Operators can achieve over 90% accuracy in predicting pooled travelers' acceptance levels within 10 days, leading to increased utility for travelers and increased profit for operators.

Conclusion: Learning individual behavioral traits allows operators to remove inefficient pooled rides and focus on attractive and profitable ride combinations, benefiting both travelers and the service provider.
...
### [264] [AI Propaganda factories with language models](https://arxiv.org/abs/2508.20186)
*Lukasz Olejnik*

Main category: cs.CR

TL;DR: Small language models can now create coherent political messaging automatically, with persona design being more important than model choice, and engagement making content more extreme.


<details>
  <summary>Details</summary>
Motivation: To demonstrate that AI-powered influence operations are now accessible to small actors using commodity hardware, requiring a shift in defense strategies.

Method: Using small language models to generate persona-driven political content and evaluating them automatically without human raters, testing how they respond to counter-arguments.

Result: Persona design explains behavior more than model identity, and engagement (countering arguments) strengthens ideological adherence and increases extreme content.

Conclusion: Fully automated influence operations are now feasible for various actors, requiring defense strategies to focus on conversation-centric detection rather than restricting model access, as the consistency of these operations provides a detection signature.
...
### [265] [Network-Level Prompt and Trait Leakage in Local Research Agents](https://arxiv.org/abs/2508.20282)
*Hyejun Jeong,Mohammadreze Teymoorianfard,Abhinav Kumar,Amir Houmansadr,Eugene Badasarian*

Main category: cs.CR

TL;DR: Web and Research Agents (WRAs) are vulnerable to network inference attacks that can recover user prompts and traits from just visited domains and timing metadata, with 73% prompt knowledge recovery and high trait accuracy.


<details>
  <summary>Details</summary>
Motivation: WRAs are increasingly used for privacy-sensitive investigations but their systematic web browsing patterns create unique fingerprinting opportunities for passive network adversaries like ISPs.

Method: Built dataset of WRA traces from user queries and synthetic personas, developed OBELS metric to assess prompt similarity, and demonstrated attacks using only network metadata (IP addresses and timings).

Result: Attack recovers 73% of functional and domain knowledge from prompts, identifies 19/32 latent traits accurately, remains effective under partial observability and noise.

Conclusion: WRAs pose significant privacy risks through metadata leakage, but mitigation strategies like constraining domain diversity or obfuscating traces can reduce attack effectiveness by 29% with minimal utility impact.
...
### [266] [Robustness Assessment and Enhancement of Text Watermarking for Google's SynthID](https://arxiv.org/abs/2508.20228)
*Xia Han,Qi Li,Jianbing Ni,Mohammad Zulkernine*

Main category: cs.CR

TL;DR: SynGuard is a hybrid watermarking framework that combines semantic information retrieval with SynthID-Text's probabilistic watermarking to improve robustness against meaning-preserving attacks like paraphrasing and back-translation.


<details>
  <summary>Details</summary>
Motivation: Existing LLM watermarking methods like SynthID-Text are vulnerable to meaning-preserving attacks that degrade watermark detectability, necessitating a more robust solution for AI-generated text provenance tracking.

Method: Proposes SynGuard, a hybrid framework that jointly embeds watermarks at lexical and semantic levels by combining Semantic Information Retrieval (SIR) with SynthID-Text's probabilistic watermarking mechanism.
...
Conclusion: Semantic-aware watermarking effectively resists real-world tampering, demonstrating the importance of combining lexical and semantic level watermarking for robust provenance tracking of AI-generated text.
...
### [267] [Surveying the Operational Cybersecurity and Supply Chain Threat Landscape when Developing and Deploying AI Systems](https://arxiv.org/abs/2508.20307)
...
<details>
  <summary>Details</summary>
Motivation: To raise awareness about unique cyber threats introduced by AI integration in software systems, as traditional cybersecurity approaches fail to address AI-specific attack surfaces and objectives.

Method: Explores operational cybersecurity and supply chain risks across the AI lifecycle, analyzes previous exploitations, and provides insights from practical experience in the field.

Result: Identifies that cyber attackers are targeting AI systems to manipulate outputs for desired system effects rather than traditional goals like privilege escalation.

Conclusion: Organizations need tailored security frameworks to address evolving AI-specific threats and ensure the reliability and resilience of AI systems.
...
### [268] [BridgeShield: Enhancing Security for Cross-chain Bridge Applications via Heterogeneous Graph Mining](https://arxiv.org/abs/2508.20517)
*Dan Lin,Shunfeng Lu,Ziyan Liu,Jiajing Wu,Junyuan Fang,Kaixin Lin,Bowen Song,Zibin Zheng*

Main category: cs.CR

TL;DR: BridgeShield is a novel cross-chain bridge attack detection framework using heterogeneous graph attention networks to model complex cross-chain semantics, achieving 92.58% F1-score and 24.39% improvement over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Cross-chain bridges are critical for blockchain interoperability but vulnerable to attacks due to design flaws and high value. Existing detection methods are limited as they only address single-chain behaviors and fail to capture cross-chain semantics.

Method: Propose BridgeShield framework using heterogeneous graph attention networks to model source chain, off-chain coordination, and destination chain in unified graph representation. Incorporates intra-meta-path attention for fine-grained dependencies and inter-meta-path attention for discriminative cross-chain patterns.
...
Conclusion: BridgeShield is an effective practical solution for securing cross-chain bridges and enhancing resilience of multi-chain ecosystems, validated by superior performance on real attack events.
...
### [269] [Federated Learning for Large Models in Medical Imaging: A Comprehensive Review](https://arxiv.org/abs/2508.20414)
*Mengyu Sun,Ziyuan Yang,Yongqiang Huang,Hui Yu,Yingyu Chen,Shuren Qi,Andrew Beng Jin Teoh,Yi Zhang*

Main category: cs.CR

TL;DR: Survey paper reviewing Federated Learning applications in medical imaging, covering both upstream reconstruction tasks and downstream clinical analysis while addressing privacy concerns.


<details>
  <summary>Details</summary>
Motivation: Medical AI development faces challenges due to strict privacy regulations that prevent data sharing, hindering large-scale model training and continuous updates with new medical data.

Method: Comprehensive analysis of Federated Learning implementations across the full medical imaging pipeline, including physics-informed reconstruction networks and diagnostic AI systems, with focus on communication efficiency and secure parameter aggregation.

Result: FL enables collaborative model development across fragmented medical datasets while preserving patient privacy, supporting both robust reconstruction networks and continuous model updating for clinical tasks.

Conclusion: Federated Learning provides a viable privacy-preserving solution for medical AI development, with promising future research directions for improving communication efficiency and handling heterogeneous data in healthcare applications.
...
### [270] [Multi-Agent Penetration Testing AI for the Web](https://arxiv.org/abs/2508.20816)
*Isaac David,Arthur Gervais*

Main category: cs.CR

TL;DR: MAPTA is a multi-agent AI system that autonomously performs web application security assessments, achieving 76.9% success on benchmark challenges while discovering real-world vulnerabilities in popular GitHub repositories at low cost.
...
Result: Achieved 76.9% overall success on 104-challenge benchmark, with perfect performance on SSRF and misconfiguration vulnerabilities, 83% success on broken authorization, and strong results on injection attacks. Discovered critical real-world vulnerabilities (RCEs, command injections, etc.) in popular GitHub repositories with average cost of $3.67 per assessment.

Conclusion: MAPTA demonstrates practical autonomous security assessment with strong performance across vulnerability types, cost-effectiveness, and real-world impact through responsible disclosure of critical vulnerabilities currently under CVE review.
...
### [271] [JADES: A Universal Framework for Jailbreak Assessment via Decompositional Scoring](https://arxiv.org/abs/2508.20848)
*Junjie Chu,Mingjie Li,Ziqing Yang,Ye Leng,Chenhao Lin,Chao Shen,Michael Backes,Yun Shen,Yang Zhang*

Main category: cs.CR

TL;DR: JADES is a new framework that automatically decomposes harmful questions into weighted sub-questions to accurately assess jailbreak success, achieving 98.5% agreement with human evaluators and revealing significant overestimation in existing attack success rates.
...
Method: JADES automatically decomposes input harmful questions into weighted sub-questions, scores each sub-answer, and weight-aggregates the sub-scores into a final decision. It also includes an optional fact-checking module to detect hallucinations in jailbreak responses.

Result: JADES achieves 98.5% agreement with human evaluators in binary success/failure assessment, outperforming baselines by over 9%. Re-evaluation of five popular attacks on four LLMs shows substantial overestimation (e.g., LAA's attack success rate on GPT-3.5-Turbo drops from 93% to 69%).

Conclusion: JADES provides accurate, consistent, and interpretable evaluations for jailbreak attempts, offering a reliable basis for measuring future jailbreak attacks and revealing that current methods significantly overestimate attack success rates.
...
### [272] [AI Agentic Vulnerability Injection And Transformation with Optimized Reasoning](https://arxiv.org/abs/2508.20866)
*Amine Lbath,Massih-Reza Amini,Aurelien Delaitre,Vadim Okun*

Main category: cs.CR

TL;DR: Novel AI framework for generating realistic vulnerability datasets by automatically injecting category-specific vulnerabilities into secure C/C++ code using multi-agent coordination and RAG techniques.


<details>
  <summary>Details</summary>
Motivation: Address limitations of traditional vulnerability detection methods (scalability, adaptability, high false rates) and AI approaches' dependency on quality training data by creating better datasets.

Method: Coordinates multiple AI agents simulating expert reasoning with function agents and traditional code analysis tools. Uses Retrieval-Augmented Generation for contextual grounding and Low-Rank approximation for efficient fine-tuning.

Result: Outperforms other techniques with 89-95% success rates in vulnerability injection at function level on 116 code samples from three benchmarks.

Conclusion: The proposed framework effectively generates high-quality vulnerability datasets, demonstrating superior performance in realistic vulnerability injection for C/C++ codebases.
...
### [273] [OLMoASR: Open Models and Data for Training Robust Speech Recognition Models](https://arxiv.org/abs/2508.20869)
*Huong Ngo,Matt Deitke,Martijn Bartelds,Sarah Pratt,Josh Gardner,Matt Jordan,Ludwig Schmidt*

Main category: cs.SD

TL;DR: The paper introduces OLMoASR-Pool (3M hours) and OLMoASR-Mix (1M hours filtered) datasets, and trains OLMoASR models that achieve Whisper-competitive performance with smaller parameter counts.


<details>
  <summary>Details</summary>
Motivation: To study the impact of training data scale and quality on speech recognition, and develop robust zero-shot models that can compete with state-of-the-art while being more efficient.

Method: Created large-scale dataset (OLMoASR-Pool), applied text heuristic filters to remove low-quality data, trained OLMoASR models ranging from 39M to 1.5B parameters on the curated OLMoASR-Mix dataset.

Result: OLMoASR models achieve comparable average performance to OpenAI's Whisper across all scales. OLMoASR-medium.en matches Whisper-medium.en performance (12.8% vs 12.4% WER for short-form, 11.0% vs 10.5% for long-form) at equivalent parameter count.

Conclusion: High-quality curated training data enables development of efficient speech recognition models that compete with larger state-of-the-art systems, with all resources made publicly available to advance robust speech processing research.
...
### [274] [Flowing Straighter with Conditional Flow Matching for Accurate Speech Enhancement](https://arxiv.org/abs/2508.20584)
*Mattias Cross,Anton Ragni*

Main category: cs.SD

TL;DR: Straight probability paths in flow-based generative models improve speech enhancement quality compared to curved paths, with time-independent variance having greater impact than gradient.


<details>
  <summary>Details</summary>
Motivation: Current flow-based speech enhancement methods use curved probability paths with unknown implications, while research suggests straight paths are easier to train and offer better generalization.

Method: Proposed independent conditional flow-matching that models straight paths between noisy and clean speech, and developed a one-step inference solution by treating the trained flow-based model as directly predictive.

Result: Straight time-independent probability paths improve generative speech enhancement over curved time-dependent paths, with time-independent variance having greater effect on sample quality than gradient.

Conclusion: Straighter time-independent probability paths enhance speech quality metrics and offer better performance than traditional curved path approaches in flow-based generative speech enhancement.
...
### [275] [Amadeus: Autoregressive Model with Bidirectional Attribute Modelling for Symbolic Music](https://arxiv.org/abs/2508.20665)
*Hongju Su,Ke Li,Lan Yang,Honggang Zhang,Yi-Zhe Song*

Main category: cs.SD

TL;DR: Amadeus is a novel symbolic music generation framework that uses a two-level architecture with autoregressive note modeling and bidirectional diffusion for attributes, achieving SOTA performance with 4x speed-up and enabling fine-grained control.


<details>
  <summary>Details</summary>
Motivation: Current symbolic music generation models assume fixed temporal dependencies between musical attributes, but observations show attributes are concurrent and unordered sets rather than sequential dependencies.
...
Result: Significantly outperforms SOTA models across multiple metrics while achieving at least 4x speed-up. Enables training-free, fine-grained note attribute control. Created AMD dataset - largest open-source symbolic music dataset.

Conclusion: The concurrent attribute modeling approach with bidirectional diffusion is more effective than traditional sequential dependency assumptions, providing superior performance, speed, and control capabilities for symbolic music generation.
...
### [276] [Unified Multi-task Learning for Voice-Based Detection of Diverse Clinical Conditions](https://arxiv.org/abs/2508.20717)
*Ran Piao,Yuan Lu,Hareld Kemps,Tong Xia,Aaqib Saeed*

Main category: cs.SD

TL;DR: MARVEL is a multi-task learning framework that detects 9 different health conditions using only acoustic features from voice, achieving strong performance while maintaining privacy by avoiding raw audio transmission.


<details>
  <summary>Details</summary>
Motivation: Existing voice-based health assessment approaches focus on single conditions and fail to leverage the rich multi-faceted information in speech. There's a need for scalable, non-invasive disease screening that can handle multiple conditions simultaneously.

Method: Dual-branch architecture with specialized encoders and task-specific heads sharing a common acoustic backbone. Uses only derived acoustic features (no raw audio) for privacy. Multi-task learning framework for cross-condition knowledge transfer.

Result: Achieves overall AUROC of 0.78, with exceptional performance on neurological disorders (AUROC = 0.89) and Alzheimer's/MCI (AUROC = 0.97). Outperforms single-modal baselines by 5-19% and surpasses state-of-the-art self-supervised models on 7 of 9 tasks.

Conclusion: A single unified model can effectively screen for diverse health conditions, establishing a foundation for deployable voice-based diagnostics in resource-constrained and remote healthcare settings.
...
### [277] [Learning Robust Spatial Representations from Binaural Audio through Feature Distillation](https://arxiv.org/abs/2508.20914)
*Holger Severin Bovbjerg,Jan Østergaard,Jesper Jensen,Shinji Watanabe,Zheng-Hua Tan*

Main category: cs.SD

TL;DR: Self-supervised pretraining using feature distillation learns robust spatial representations from binaural speech, improving DoA estimation performance in challenging acoustic environments.


<details>
  <summary>Details</summary>
Motivation: Deep representation learning has shown success in audio tasks, but spatial representation learning from multichannel audio remains underexplored. The authors aim to learn robust spatial features without labeled data.

Method: A pretraining framework using feature distillation: spatial features from clean binaural speech are used as prediction labels, which a neural network learns to predict from augmented speech. The encoder weights are then used to initialize a DoA estimation model for fine-tuning.

Result: Pretrained models demonstrate improved performance in noisy and reverberant environments for direction-of-arrival estimation compared to fully supervised models and traditional signal processing methods.

Conclusion: Feature distillation pretraining enables effective learning of spatial representations from unlabeled binaural audio, leading to enhanced DoA estimation capabilities in challenging acoustic conditions.
...
### [278] [Speech Emotion Recognition via Entropy-Aware Score Selection](https://arxiv.org/abs/2508.20796)
*ChenYi Chua,JunKai Wong,Chengxin Chen,Xiaoxiao Miao*

Main category: cs.SD

TL;DR: A multimodal speech emotion recognition framework that combines acoustic and textual predictions using entropy-aware score selection, achieving improved performance over single-modality systems.


<details>
  <summary>Details</summary>
Motivation: Traditional single-modality speech emotion recognition systems have confidence constraints and limitations. The paper aims to overcome these by integrating multimodal information (speech and text) through entropy-aware fusion to create a more reliable and practical emotion recognition system.

Method: Uses a dual-pipeline approach: 1) acoustic model based on wav2vec2.0, 2) sentiment analysis model using RoBERTa-XLM with Whisper-generated transcriptions. Implements late score fusion with entropy and varentropy thresholds to handle confidence constraints. Includes sentiment mapping from 3 sentiment categories to 4 emotion classes.
...
Conclusion: The entropy-aware multimodal fusion framework effectively improves speech emotion recognition by leveraging both acoustic and textual information, providing a more robust solution compared to single-modality approaches.
...
### [279] [WoW-Bench: Evaluating Fine-Grained Acoustic Perception in Audio-Language Models via Marine Mammal Vocalizations](https://arxiv.org/abs/2508.20976)
*Jaeyeon Kim,Heeseung Yun,Sang Hoon Woo,Chao-Han Huck Yang,Gunhee Kim*

Main category: cs.SD

TL;DR: The paper introduces WoW-Bench, a benchmark for evaluating low-level auditory perception and cognition in large audio language models using marine mammal vocalizations, revealing significant performance gaps compared to humans.


<details>
  <summary>Details</summary>
Motivation: Large audio language models lack robust low-level listening capabilities like pitch and duration detection, which are critical for real-world sound reasoning tasks, particularly with unfamiliar sounds and fine-grained acoustic cues.

Method: Created World-of-Whale benchmark (WoW-Bench) with two components: Perception benchmark for categorizing novel sounds, and Cognition benchmark based on Bloom's taxonomy to assess remembering, understanding, applying, and analyzing sound events with distractor questions to prevent heuristic reliance.

Result: State-of-the-art LALMs performed far below human levels on the WoW-Bench evaluation, demonstrating significant deficiencies in low-level auditory perception and cognitive sound processing capabilities.

Conclusion: Current large audio language models lack strong auditory grounding and need improved low-level listening abilities to handle real-world, out-of-distribution auditory tasks effectively.
...
TL;DR: POS2FS is an uncertainty-aware online sparse streaming feature selection framework that uses particle swarm optimization and three-way decision theory to handle data incompleteness and uncertain feature-label correlations in high-dimensional streaming data.


<details>
  <summary>Details</summary>
Motivation: Real-world applications with high-dimensional streaming data often face data incompleteness due to sensor failures or technical constraints. Existing methods struggle with uncertain feature-label correlations, leading to inflexible models and degraded performance.

Method: Proposes POS2FS framework with: 1) PSO-driven supervision to reduce uncertainty in feature-label relationships, and 2) Three-way decision theory to manage feature fuzziness in supervised learning.

Result: Rigorous testing on six real-world datasets confirms POS2FS outperforms conventional OSFS and OS2FS techniques, delivering higher accuracy through more robust feature subset selection.

Conclusion: POS2FS provides an effective solution for handling data incompleteness and uncertainty in online streaming feature selection, achieving superior performance compared to existing methods.
...
TL;DR: SNNDeep is the first custom-built spiking neural network optimized for liver health classification from CT scans, outperforming framework-based SNNs with 98.35% accuracy and reduced training overhead.


<details>
  <summary>Details</summary>
Motivation: Spiking neural networks offer energy efficiency and biological plausibility but remain unexplored for high-stakes biomedical imaging applications like liver health classification.
...
Conclusion: Low-level, highly tunable SNNs can surpass standard frameworks in medical imaging, opening new pathways for neuro-inspired AI in precision medicine, especially in data-limited diagnostic settings.
...
TL;DR: A novel spatio-temporal pruning framework for Spiking LLMs that combines spatial pruning (reducing neurons/attention heads), temporal pruning (dynamic timestep adjustment), extreme quantization, and knowledge distillation to achieve energy-efficient deployment.


<details>
  <summary>Details</summary>
Motivation: Large Language Models face deployment challenges in energy-constrained environments due to their large size and high inference latency. Spiking Neural Networks offer low-power computing potential through event-driven processing.

Method: Proposes spatio-temporal pruning framework: spatial pruning reduces active neurons and attention heads; temporal pruning dynamically adjusts timesteps per layer. Combined with extreme quantization and knowledge distillation.
...
### [283] [Flexible metadata harvesting for ecology using large language models](https://arxiv.org/abs/2508.20115)
*Zehao Lu,Thijs L van der Plas,Parinaz Rashidi,W Daniel Kissling,Ioannis N Athanasiadis*

Main category: cs.DL

TL;DR: LLM-based metadata harvester that extracts and standardizes ecological dataset metadata from diverse platforms, enabling dataset discovery and integration through semantic linking and unified formatting.


<details>
  <summary>Details</summary>
Motivation: Ecological researchers face challenges in finding and combining datasets from multiple sources due to varying metadata standards and platforms, hindering data reuse and integration.

Method: Developed a large language model (LLM)-based tool that extracts metadata from dataset landing pages, converts to unified format using existing standards, and uses LLM post-processing for accuracy. Also employs embedding similarity and rule-based processing to identify dataset links.

Result: The tool successfully extracts both structured and unstructured metadata with equal accuracy, enables flexible metadata linking between datasets, and supports ontology creation and graph-based queries for finding relevant ecological datasets.

Conclusion: The LLM-based metadata harvester effectively overcomes metadata heterogeneity challenges in ecological data discovery, facilitating dataset integration and reuse through standardized metadata extraction and semantic linking capabilities.
...
### [284] [Is Artificial Intelligence Reshaping the Landscape of the International Academic Community of Geosciences?](https://arxiv.org/abs/2508.20117)
*Liang Li,Yuntian Li,Wenxin Zhao,Shan Ye,Yun Lu*

Main category: cs.DL

TL;DR: AI is positively transforming geosciences research with increased scientific output, better visibility for developing country scientists, and improved international collaboration.


<details>
  <summary>Details</summary>
Motivation: To understand how artificial intelligence is impacting geosciences research and its effects on scientific output, visibility of researchers from developing countries, and international collaboration patterns.

Method: Bibliometric analysis and topic modeling of scientific publications in geosciences research.

Result: AI is positively transforming geosciences with notable increase in AI-related scientific output. Earth scientists from developing countries have gained better visibility in AI4S paradigm, and AI is improving international collaboration landscape.

Conclusion: Artificial intelligence is having a positive transformative effect on geosciences research, enhancing scientific productivity, increasing representation of developing country researchers, and fostering better international collaboration.
...
### [285] [Leveraging Large Language Models for Generating Research Topic Ontologies: A Multi-Disciplinary Study](https://arxiv.org/abs/2508.20693)
*Tanay Aggarwal,Angelo Salatino,Francesco Osborne,Enrico Motta*

Main category: cs.DL

TL;DR: LLMs can effectively identify semantic relationships between research topics across biomedical, physics, and engineering domains, with fine-tuning on the novel PEM-Rel-8K dataset achieving excellent performance.


<details>
  <summary>Details</summary>
Motivation: Ontology creation and maintenance for research fields is expensive and time-consuming, often resulting in uneven coverage, limited connectivity, and infrequent updates across disciplines.

Method: Evaluated multiple LLMs under zero-shot prompting, chain-of-thought prompting, and fine-tuning on PEM-Rel-8K dataset (8,000+ relationships from MeSH, PhySH, IEEE taxonomies). Assessed cross-domain transferability by training in one domain and testing in another.

Result: Fine-tuning LLMs on PEM-Rel-8K yielded excellent performance across all three academic domains (biomedicine, physics, engineering).

Conclusion: LLMs show strong capability for automated ontology relationship identification, with fine-tuning being particularly effective for cross-domain semantic relationship extraction in research taxonomies.
...
TL;DR: Analysis of stochastic gradient optimization for problems with unknown nuisance parameters, showing convergence under Neyman orthogonality conditions and proposing an orthogonalized gradient variant for cases where orthogonality isn't satisfied.


<details>
  <summary>Details</summary>
Motivation: Many learning problems involve unknown nuisance parameters that can affect optimization trajectories and optimal solutions. Understanding how stochastic gradient algorithms behave in such scenarios is crucial for modern learning applications including self-supervised learning and causal inference.

Method: Theoretical analysis of classical stochastic gradient algorithm performance with nuisance parameters. Development of an algorithm variant with approximately orthogonalized updates using an approximately orthogonalized gradient oracle. Examination under Neyman orthogonality conditions.

Result: Established non-asymptotic convergence guarantees showing that classical stochastic gradient algorithms can still converge under appropriate conditions like Neyman orthogonality. Demonstrated that even without orthogonality, the proposed variant with orthogonalized updates achieves similar convergence rates.

Conclusion: Stochastic gradient optimization remains effective for problems with nuisance parameters when proper orthogonality conditions are met, and algorithmic variants can handle cases where these conditions are not fully satisfied, with applications in statistical learning and causal inference.
...
TL;DR: Self-consistency loss improves Bayesian model comparison calibration under model misspecification when using exact likelihoods, but offers limited benefits with neural surrogate likelihoods.


<details>
  <summary>Details</summary>
Motivation: Neural surrogates for Bayesian model comparison become unreliable when simulation models are misspecified, which is precisely when model comparison is most needed.
...
Result: Self-consistency improves calibration under model misspecification when having access to analytic likelihoods, but offers limited gains with neural surrogate likelihoods.
...
<details>
  <summary>Details</summary>
Motivation: To explore traditional machine learning methods for operator learning beyond deep neural networks, leveraging PCE's established use in uncertainty quantification for scientific applications.

Method: Establishes a mathematical framework that enables PCE to approximate operators in data-driven and physics-informed settings, reducing operator learning to solving a system of equations for PCE coefficients.
...
Conclusion: PCE provides an effective alternative to deep learning approaches for operator learning, offering built-in uncertainty quantification capabilities without additional computational cost.
...
<details>
  <summary>Details</summary>
Motivation: To address posterior drift in transfer learning by moving beyond regression-based methods to decision rules, enabling more robust and applicable classification frameworks.

Method: Proposes modeling posterior drift through Bayes decision rules, exploiting geometric transformation of Bayes decision boundary to reformulate as low-dimensional empirical risk minimization problem.
...
Conclusion: The method provides a broadly applicable framework for transfer learning with decision rules, successfully adapted to optimal individualized treatment rule estimation with strong theoretical guarantees and empirical performance.
...
### [290] [Mixture of Contexts for Long Video Generation](https://arxiv.org/abs/2508.21058)
...
TL;DR: Proposes Mixture of Contexts (MoC), a learnable sparse attention routing module that enables efficient long-context video generation by dynamically selecting informative chunks and anchors for attention, achieving near-linear scaling and maintaining consistency over minutes of content.


<details>
  <summary>Details</summary>
Motivation: Long video generation requires models to retain and retrieve salient events across long sequences, but traditional diffusion transformers face quadratic computational costs from self-attention, making long-context generation intractable.

Method: Recasts long-context video generation as internal information retrieval. Uses Mixture of Contexts (MoC) where each query dynamically selects a few informative chunks plus mandatory anchors (caption, local windows) with causal routing to prevent loop closures.

Result: Achieves near-linear scaling in computation, enables practical training and synthesis of long videos, and maintains identity, action, and scene consistency over minutes of generated content through emergent memory capabilities.

Conclusion: MoC provides an effective long-term memory retrieval engine for video generation, overcoming quadratic attention costs while preserving coherence and consistency in long-form video synthesis through sparse, learnable attention routing.
...
### [291] [Validating Generative Agent-Based Models for Logistics and Supply Chain Management Research](https://arxiv.org/abs/2508.20234)
*Vincent E. Castillo*

Main category: cs.MA

TL;DR: GABMs using LLMs show promise for supply chain simulations but require dual validation - surface equivalence testing and decision process verification - as some LLMs show human-like outputs but artificial reasoning processes.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether LLMs can validly simulate human behavior in logistics and supply chain management simulations, as traditional agent-based models lack realistic human-like reasoning capabilities.

Method: Controlled experiment with 957 human participants (477 dyads) in food delivery scenarios, testing 6 state-of-the-art LLMs using moderated mediation design, Two One-Sided Tests for equivalence, and structural equation modeling.

Result: Some LLMs demonstrate surface-level equivalence to human behavior but reveal artificial decision processes not present in humans, creating an equivalence-versus-process paradox.

Conclusion: GABMs are potentially viable for LSCM research with proper dual validation (human equivalence testing + decision process validation), providing a framework for rigorous development and evidence-based LLM selection.
...
### [292] [Graph-Based Feature Augmentation for Predictive Tasks on Relational Datasets](https://arxiv.org/abs/2508.20986)
*Lianpeng Qiao,Ziqi Cao,Kaiyu Feng,Ye Yuan,Guoren Wang*

Main category: cs.DB

TL;DR: ReCoGNN is an automated feature augmentation framework that extracts relational signals from multiple tables to enhance predictive modeling, outperforming existing methods on classification and regression tasks.


<details>
  <summary>Details</summary>
Motivation: To automate feature augmentation and identify task-related relational signals from multiple relational tables, reducing manual effort in predictive modeling.

Method: ReCoGNN captures semantic dependencies within tables, partitions them into coherent segments, constructs a heterogeneous weighted graph for inter-row relationships, and uses message-passing GNNs for feature selection and augmentation.

Result: Extensive experiments on ten real-life and synthetic datasets show ReCoGNN consistently outperforms existing methods on both classification and regression tasks.

Conclusion: The proposed end-to-end automated feature augmentation framework effectively leverages relational signals from multiple tables to enhance predictive modeling performance.
...
### [293] [Research Challenges in Relational Database Management Systems for LLM Queries](https://arxiv.org/abs/2508.20912)
*Kerem Akillioglu,Anurag Chakraborty,Sairaj Voruganti,M. Tamer Özsu*

Main category: cs.DB

TL;DR: Analysis of current SQL-invoked LLM integrations reveals functional and performance limitations in open-source solutions, with identified issues in structured outputs, resource utilization, and query planning that require tighter LLM+DBMS integration.
...
Method: Early exploration of two open-source systems and one enterprise platform using five representative queries to evaluate functional, performance, and scalability limits of SQL-invoked LLM integrations.

Result: Identified three main issues: enforcing structured outputs, optimizing resource utilization, and improving query planning. Initial solutions showed improvements in accommodating LLM-powered SQL queries.

Conclusion: Tighter integration of LLM+DBMS is essential for scalable and efficient processing of LLM queries, as demonstrated by early gains from addressing the identified limitations.
...
### [294] [Towards Better Correctness and Efficiency in Code Generation](https://arxiv.org/abs/2508.20124)
*Yunlong Feng,Yang Xu,Xiao Xu,Binyuan Hui,Junyang Lin*

Main category: cs.SE

TL;DR: A reinforcement learning framework that improves both code correctness (10.18%) and runtime efficiency (7.75%) for code generation models by using dynamic exploration, error-insensitive RL, and starting from high-correctness baselines.


<details>
  <summary>Details</summary>
Motivation: Code LLMs generate code with poor runtime efficiency, limiting practical use in performance-sensitive applications.

Method: Two-stage tuning method with efficiency-oriented RL framework using dynamic exploration, error-insensitive reinforcement learning, and high-contrast efficiency signals.

Result: Improves code correctness by 10.18% and runtime efficiency by 7.75% on a 7B model, achieving performance comparable to larger models.

Conclusion: The proposed framework effectively balances correctness and efficiency in code generation through strategic exploration and optimization techniques.
...
### [295] [Boosting Skeleton-Driven SMT Solver Fuzzing by Leveraging LLM to Produce Formula Generators](https://arxiv.org/abs/2508.20340)
*Maolin Sun,Yibiao Yang,Yuming Zhou*

Main category: cs.SE

TL;DR: Chimera is an LLM-assisted fuzzing framework that generates syntactically valid SMT formulas by synthesizing reusable term generators from documentation, reducing computational overhead while finding 43 bugs in Z3 and cvc5 solvers.


<details>
  <summary>Details</summary>
Motivation: Existing SMT solver testing techniques struggle with rapidly evolving features, and LLM-based approaches face issues with invalid formulas and high computational costs from iterative interactions.

Method: Uses LLMs to extract context-free grammars from documentation and synthesize composable Boolean term generators. Populates structural skeletons from existing formulas with LLM-generated terms to ensure syntactic validity and semantic diversity with one-time LLM interaction.

Result: Identified 43 confirmed bugs in Z3 and cvc5 solvers, with 40 already fixed by developers.

Conclusion: Chimera effectively addresses syntactic validity and computational overhead issues in LLM-assisted SMT solver testing, demonstrating practical effectiveness in finding real-world bugs with minimal LLM interaction.
...
### [296] [Adaptive Root Cause Localization for Microservice Systems with Multi-Agent Recursion-of-Thought](https://arxiv.org/abs/2508.20370)
*Lingzhe Zhang,Tong Jia,Kangjin Wang,Weijie Hong,Chiming Duan,Minghua He,Ying Li*

Main category: cs.SE

TL;DR: RCLAgent is a novel adaptive root cause localization method for microservice systems that uses multi-agent recursion-of-thought framework to accurately pinpoint failures with superior performance using single requests.


<details>
  <summary>Details</summary>
Motivation: Microservice systems face frequent failures but existing methods either rely on pre-defined schemas that struggle with evolving contexts or lack interpretability, leaving SREs confused. Human root cause analysis shows recursiveness, multi-dimensional expansion, and cross-modal reasoning characteristics.

Method: RCLAgent leverages a multi-agent recursion-of-thought framework with novel recursion-of-thought strategy to guide LLM reasoning, integrating data from multiple agents and tool-assisted analysis.

Result: Experimental evaluations on public datasets show RCLAgent achieves superior performance by localizing root cause using only single request, outperforming state-of-the-art methods that require multiple requests.

Conclusion: RCLAgent effectively enhances efficiency and precision of root cause localization in complex microservice environments through its adaptive multi-agent framework.
...
### [297] [Evaluating LLMs on microservice-based applications: how complex is your specification?](https://arxiv.org/abs/2508.20119)
*Daniel M. Yellin*
...
Result: Strong LLMs like GPT-3o-mini perform fairly well on medium difficulty specifications but perform very poorly on high difficulty specifications due to complex business logic, external service integration, database requirements, and non-functional capabilities.

Conclusion: LLMs face significant challenges with complex real-world code generation, particularly with intricate business logic and system integration, highlighting the need for future research to improve code synthesis capabilities for practical applications.
...
### [298] [AI and Agile Software Development: A Research Roadmap from the XP2025 Workshop](https://arxiv.org/abs/2508.20563)
...
TL;DR: Workshop synthesis identifying key challenges and research directions for integrating GenAI with agile software development, including tool fragmentation, governance, data quality, and skills gaps.


<details>
  <summary>Details</summary>
Motivation: To address the concrete challenges and emerging opportunities at the intersection of Generative AI and agile software development by bringing together interdisciplinary experts.

Method: Full-day workshop with over 30 academic researchers and industry practitioners using structured, interactive breakout sessions to identify pain points and co-create research agendas.

Result: Identified shared challenges (tool fragmentation, governance, data quality, AI literacy gaps) and developed a multi-thematic research roadmap with both short-term actions and long-term visionary directions.

Conclusion: The workshop successfully created a cohesive research agenda to guide future investigation and drive responsible, human-centered integration of GenAI into agile practices.
...
### [299] [Rethinking Testing for LLM Applications: Characteristics, Challenges, and a Lightweight Interaction Protocol](https://arxiv.org/abs/2508.20737)
*Wei Ma,Yixiao Yang,Qiang Hu,Shi Ying,Zhi Jin,Bo Du,Zhenchang Xing,Tianlin Li,Junjie Shi,Yang Liu,Linxiao Jiang*

Main category: cs.SE

TL;DR: This paper proposes a three-layer architecture for testing LLM applications and identifies challenges in applying traditional software testing methods, offering collaborative strategies and a test-oriented protocol for AI agent communication.
...
Method: Decomposes LLM applications into three layers (System Shell, Prompt Orchestration, LLM Inference Core), analyzes applicability of traditional testing methods, compares AI testing approaches, and proposes four collaborative strategies with a closed-loop quality assurance framework.

Result: Identifies 6 core challenges stemming from 4 fundamental differences between traditional software and LLM applications, and develops practical guidance including the Agent Interaction Communication Language (AICL) protocol for standardized testing.

Conclusion: The paper provides a structured approach to LLM application testing through layered architecture analysis, collaborative strategies, and proposes AICL as a test-oriented protocol to support standardization and tooling for quality assurance of complex LLM systems.
...
### [300] [Differentially Private Federated Quantum Learning via Quantum Noise](https://arxiv.org/abs/2508.20310)
...
<details>
  <summary>Details</summary>
Motivation: Quantum federated learning enables collaborative training across distributed quantum devices but remains vulnerable to adversarial attacks that can exploit shared model updates to compromise information privacy.

Method: The approach harnesses quantum noise by tuning noise variance through measurement shots and depolarizing channel strength to achieve desired differential privacy levels tailored to NISQ device constraints.

Result: Simulations demonstrate the framework's effectiveness in balancing differential privacy budget with noise parameters, and show robustness against adversarial attacks with evaluations based on accuracy on adversarial examples, confidence scores, and attack success rates.

Conclusion: The results reveal a tunable trade-off between privacy and robustness, providing an efficient solution for secure quantum federated learning on NISQ devices with significant potential for reliable quantum computing applications.
...
### [301] [Quantum Verifiable Rewards for Post-Training Qiskit Code Assistant](https://arxiv.org/abs/2508.20907)
*Nicolas Dupuis,Adarsh Tiwari,Youssef Mroueh,David Kremer,Ismael Faro,Juan Cruz-Benito*

Main category: quant-ph

TL;DR: The paper explores post-training techniques for LLMs to assist in writing Qiskit quantum computing code, using quantum verification and synthetic data generation to improve code quality and hardware executability.


<details>
  <summary>Details</summary>
Motivation: To help developers write better Qiskit quantum computing code by leveraging LLMs with quantum-specific verification methods that ensure code quality and hardware compatibility.

Method: Developed synthetic data pipeline generating quantum problem-unit test pairs, used DPO for alignment, and GRPO with quantum-verifiable rewards from actual quantum hardware. Combined DPO and GRPO approaches.

Result: The best-performing model combining DPO and GRPO outperformed the strongest open-source baselines on the challenging Qiskit-HumanEval-hard benchmark.

Conclusion: Quantum verification and hardware-based rewards are effective methods for aligning LLMs to produce high-quality, executable quantum computing code, with the combined DPO+GRPO approach yielding superior results.
...
### [302] [MicroLad: 2D-to-3D Microstructure Reconstruction and Generation via Latent Diffusion and Score Distillation](https://arxiv.org/abs/2508.20138)
...
Method: Uses latent diffusion framework trained on 2D images with multi-plane denoising diffusion sampling. Integrates score distillation sampling (SDS) with differentiable score loss, descriptor-matching, and property-alignment terms.

Result: Generates stable, coherent 3D volumes statistically consistent with original data. Enables dimensionality expansion (2D-to-3D) and robust inverse-controlled microstructure generation.

Conclusion: Facilitates exploration of expanded 3D microstructure analysis and design space for both microstructural descriptors and material properties.
...
### [303] [A Unified Theory of Language](https://arxiv.org/abs/2508.20109)
*Robert Worden*
...
<details>
  <summary>Details</summary>
Motivation: To develop a comprehensive theory that explains language's speed, expressivity, diversity, and integrates all linguistic aspects (phonology, syntax, semantics, pragmatics) through a unified computational framework.
...
Result: Accounts for major language facts including processing speed, expressivity, language diversity, and resolves pragmatic puzzles. Provides evolutionary continuity between human language processing and animal Bayesian cognition.

Conclusion: Language evolved through sexual selection for intelligence display and serves as the foundation for human mind reading, cooperation, self-esteem, emotions, culture, and society through seamless Bayesian unification processing.
...
### [304] [OnGoal: Tracking and Visualizing Conversational Goals in Multi-Turn Dialogue with Large Language Models](https://arxiv.org/abs/2508.21061)
...
Method: Developed OnGoal interface with LLM-assisted evaluation, real-time feedback on goal alignment, evaluation explanations with examples, and goal progression overviews. Conducted study with 20 participants on writing task comparing against baseline chat interface without goal tracking.

Result: Participants using OnGoal spent less time and effort to achieve goals while exploring new prompting strategies to overcome miscommunication. The system enhanced engagement and resilience in LLM dialogues.

Conclusion: Tracking and visualizing goals can significantly improve goal communication, reduce cognitive load, enhance interactivity, and enable feedback to improve LLM performance in chat interfaces.
...
### [305] [Understanding, Protecting, and Augmenting Human Cognition with Generative AI: A Synthesis of the CHI 2025 Tools for Thought Workshop](https://arxiv.org/abs/2508.21036)
...
TL;DR: Workshop summary exploring how Generative AI impacts human cognition and opportunities for augmentation, bringing together researchers to map research/design opportunities.


<details>
  <summary>Details</summary>
Motivation: To understand how Generative AI transforms human cognition (metacognition, critical thinking, memory, creativity) and bridge science with design practice for building AI tools that protect and augment human thought.

Method: CHI 2025 workshop with 56 multidisciplinary researchers, designers, and thinkers from academia/industry, analyzing 34 papers and portfolios through discussion, ideation, and community-building activities.

Result: Synthesized workshop materials to begin mapping research and design opportunities, catalyzing a multidisciplinary community around studying GenAI's cognitive impacts.

Conclusion: Established foundation for ongoing research into how Generative AI affects human cognition and created community momentum for developing tools that thoughtfully augment human thought capabilities.
...
### [306] [ELIXIR: Efficient and LIghtweight model for eXplaIning Recommendations](https://arxiv.org/abs/2508.20312)
*Ben Kabongo,Vincent Guigue,Pirmin Lemberger*

Main category: cs.IR

TL;DR: ELIXIR is a lightweight multi-task model that combines rating prediction with personalized review generation using aspect-based representations, outperforming larger models in explainable recommendations.


<details>
  <summary>Details</summary>
Motivation: Collaborative filtering struggles with fine-grained interactions and explainability. Users want transparent recommendations with textual explanations, but existing methods either don't leverage pre-trained Transformers effectively or neglect aspect modeling for personalization.
...
### [307] [MPFormer: Adaptive Framework for Industrial Multi-Task Personalized Sequential Retriever](https://arxiv.org/abs/2508.20400)
*Yijia Sun,Shanshan Huang,Linxiao Che,Haitao Lu,Qiang Luo,Kun Gai,Guorui Zhou*

Main category: cs.IR

TL;DR: MPFormer is a dynamic multi-task Transformer framework that addresses multi-stage optimization misalignment in industrial recommendation systems by jointly encoding user behavior and multi-task semantics with learnable attention modulation, personalized target weights, and enhanced user personalization.


<details>
  <summary>Details</summary>
Motivation: To solve the semantic gap between multi-objective optimization in ranking phase and single-objective modeling in retrieval phase, which causes resource inefficiency and limitations in handling loosely coupled objectives in industrial recommendation systems.

Method: Three innovative mechanisms: 1) objective-conditioned transformer with learnable attention modulation, 2) personalized target weights for dynamic adjustment, 3) user personalization integrated into token representations and Transformer structure.

Result: Successfully deployed in Kuaishou short video recommendation system serving 400M+ daily users, significantly improving user engagement and system efficiency while maintaining service response speed.

Conclusion: MPFormer provides a scalable multi-objective solution that optimizes the iterative paradigm for industrial recommendation systems, effectively addressing multi-stage optimization misalignment challenges.
...
### [308] [Rethinking Purity and Diversity in Multi-Behavior Sequential Recommendation from the Frequency Perspective](https://arxiv.org/abs/2508.20427)
...
<details>
  <summary>Details</summary>
Motivation: Traditional multi-behavior sequential recommendation treats high-frequency information as noise, but the authors argue it contains valuable diversity information about user interests that should be leveraged rather than discarded.

Method: Proposed PDB4Rec model that extracts information across various frequency bands and their relationships, and introduces a Bootstrapping Balancer mechanism to balance contributions from different frequency components.

Result: Experimental results show that low frequency corresponds to purity of user interests while high frequency corresponds to diversity of user interests. The model demonstrates effectiveness and efficiency on real-world datasets.

Conclusion: High-frequency information in multi-behavior data contains valuable diversity signals about user interests, and properly balancing purity (low-frequency) and diversity (high-frequency) components leads to improved recommendation performance.
...
### [309] [On the Theoretical Limitations of Embedding-Based Retrieval](https://arxiv.org/abs/2508.21038)
*Orion Weller,Michael Boratko,Iftekhar Naim,Jinhyuk Lee*

Main category: cs.IR

TL;DR: Vector embeddings have fundamental theoretical limitations in handling even simple queries, demonstrated through learning theory connections and empirical testing on a new dataset called LIMIT.


<details>
  <summary>Details</summary>
Motivation: To challenge the common assumption that embedding limitations are due to unrealistic queries and can be overcome with better training/larger models, by showing these limitations occur in realistic settings with simple queries.

Method: Connected learning theory results showing dimension limits top-k subsets, empirically tested with k=2 using free parameterized embeddings, and created LIMIT dataset to stress-test state-of-the-art models.

Result: Even state-of-the-art embedding models fail on the simple tasks in LIMIT dataset, confirming theoretical limitations hold in practical settings.

Conclusion: The single vector paradigm for embeddings has fundamental limitations that cannot be resolved by current approaches, calling for new methods to overcome these constraints.
...
### [310] [Revealing Potential Biases in LLM-Based Recommender Systems in the Cold Start Setting](https://arxiv.org/abs/2508.20401)
*Alexandre Andre,Gauthier Roy,Eva Dyer,Kai Wang*

Main category: cs.IR

TL;DR: A benchmark for evaluating fairness in zero-context LLM recommendations reveals consistent biases across domains and a non-linear relationship between model size and fairness.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used for recommendation tasks, but their behavior in cold-start scenarios with limited user signals raises fairness concerns due to potential reliance on societal biases from pretraining.

Method: Introduce a modular benchmark pipeline for zero-context recommendation that supports configurable domains and sensitive attributes, enabling systematic audits of open-source LLMs.

Result: Evaluations of Gemma 3 and Llama 3.2 show consistent biases across music, movies, and college recommendations, including gendered and cultural stereotypes, with a non-linear relationship between model size and fairness.

Conclusion: The findings highlight the need for nuanced fairness analysis in LLM-based recommendations, particularly in cold-start scenarios, and demonstrate the importance of specialized benchmarks for detecting biases.
...
### [311] [SemSR: Semantics aware robust Session-based Recommendations](https://arxiv.org/abs/2508.20587)
*Jyoti Narwariya,Priyanka Gupta,Muskan Gupta,Jyotsana Khatri,Lovekesh Vig*

Main category: cs.IR

TL;DR: This paper presents multiple approaches to integrate Large Language Models (LLMs) with session-based recommendation systems, showing that hybrid methods combining LLMs with traditional data-driven models outperform standalone approaches.


<details>
  <summary>Details</summary>
Motivation: Existing session-based recommendation models fail to leverage semantic information from item titles/descriptions, and current LLM approaches either struggle with optimal prompting or incur high computational costs through fine-tuning.
...
Result: LLM-based methods excel at coarse-level retrieval (high recall), while traditional techniques perform well at fine-grained ranking. Integration of LLMs with data-driven models significantly outperforms both standalone approaches in both Recall and MRR metrics.

Conclusion: Hybrid approaches combining LLMs with traditional data-driven session-based recommendation models provide superior performance, leveraging the strengths of both semantic understanding from LLMs and fine-grained ranking from traditional methods.
...
### [312] [SEAL: Structure and Element Aware Learning to Improve Long Structured Document Retrieval](https://arxiv.org/abs/2508.20778)
*Xinhao Huang,Zhibo Ren,Yipeng Yu,Ying Zhou,Zulong Chen,Zeyi Wen*

Main category: cs.IR

TL;DR: SEAL is a contrastive learning framework that addresses structural information gaps in long document retrieval by using structure-aware learning and masked element alignment, with a new annotated dataset.


<details>
  <summary>Details</summary>
Motivation: Existing methods for long structured document retrieval fine-tune PLMs using contrastive learning but fail to leverage structural features effectively and lack datasets with structural metadata.
...
Result: Extensive experiments show consistent performance improvements, boosting NDCG@10 from 73.96% to 77.84% on BGE-M3, with positive results from online A/B testing.

Conclusion: The proposed SEAL framework effectively addresses structural information gaps in long document retrieval and the released dataset with structural annotations provides valuable resources for future research.
...
### [313] [Efficient Large-Scale Cross-Domain Sequential Recommendation with Dynamic State Representations](https://arxiv.org/abs/2508.20945)
*Manuel V. Loureiro,Steven Derby,Aleksei Medvedev,Alejandro Ariza-Casabona,Gonzalo Fiz Pontiveros,Tri Kurniawan Wijaya*

Main category: cs.IR

TL;DR: Novel multi-domain recommendation system using TAPE embeddings and DDSR to replace full inter-domain attention, reducing computational costs while maintaining performance.
...
Method: Two key innovations: 1) Transition-Aware Positional Embeddings (TAPE) for domain-transition specific information, focusing attention on intra-domain items only; 2) Dynamic Domain State Representation (DDSR) for storing and accessing domain states during predictions.
...
Conclusion: The approach provides a scalable solution for large-scale multi-domain recommendation systems by efficiently balancing domain knowledge transfer without full attention maps.
...
### [314] [Machine-learning based particle-flow algorithm in CMS](https://arxiv.org/abs/2508.20541)
*Farouk Mokhtar*

Main category: hep-ex

TL;DR: CMS develops machine-learned particle flow (MLPF) using transformer models to optimize particle reconstruction from tracks and clusters in a single pass, improving event reconstruction efficiency.


<details>
  <summary>Details</summary>
Motivation: To leverage end-to-end machine learning approaches for optimizing particle reconstruction, taking advantage of heterogeneous computing architectures and improving upon traditional particle-flow algorithms.

Method: Uses a transformer model architecture to directly infer particles from tracks and clusters in a single pass, with developments in training datasets and integration with offline reconstruction software.

Result: Development of MLPF framework that provides improved reconstruction metrics and performance compared to traditional methods.

Conclusion: Machine-learned particle flow represents a significant advancement in CMS event reconstruction, offering optimized performance through modern ML techniques and transformer architectures.
...
### [315] [Multi-Objective Optimization of ReRAM Crossbars for Robust DNN Inferencing under Stochastic Noise](https://arxiv.org/abs/2109.05437)
...
TL;DR: Proposed ReSNA training method and CF-MESMO optimization algorithm for noise-resistant ReRAM-based DNN accelerators, achieving 2.57% accuracy improvement and 90.91% computation cost reduction.


<details>
  <summary>Details</summary>
Motivation: Stochastic noise in ReRAM crossbars degrades DNN inferencing accuracy, requiring robust hardware accelerator designs that can handle noise while maintaining efficiency.
...
Result: ReSNA achieves 2.57% inferencing accuracy improvement for ResNet20 on CIFAR-10. CF-MESMO reduces computation cost by 90.91% compared to NSGA-II while finding optimal Pareto fronts.

Conclusion: The proposed methods enable efficient design of high-performance ReRAM accelerators that are robust to stochastic noise while optimizing multiple objectives including accuracy, area, time, and energy.
...
### [316] [The Mathematician's Assistant: Integrating AI into Research Practice](https://arxiv.org/abs/2508.20236)
*Jonas Henkel*

Main category: math.HO

TL;DR: Analysis of AI/LLMs in mathematical research showing strong problem-solving capabilities but systematic flaws, proposing an augmented mathematician framework with AI as copilot under human guidance.


<details>
  <summary>Details</summary>
Motivation: To explore how recent AI breakthroughs and large language models can transform mathematical research practices and address their current limitations.

Method: Analysis of recent benchmarks (MathArena, Open Proof Corpus) and development of a framework with five guiding principles for integrating AI into research workflow.

Result: Found complex duality - models show strong problem-solving and proof evaluation abilities but exhibit systematic flaws like lack of self-critique and discrepancy between answer accuracy and proof validity.

Conclusion: AI's primary role is augmentation rather than automation, requiring new skills in strategic prompting, critical verification, and methodological rigor for effective use.
...
### [317] [SPGrasp: Spatiotemporal Prompt-driven Grasp Synthesis in Dynamic Scenes](https://arxiv.org/abs/2508.20547)
*Yunpeng Mei,Hongjie Cao,Yinqiu Xia,Wei Xiao,Zhaohan Feng,Gang Wang,Jie Chen*

Main category: cs.RO

TL;DR: SPGrasp is a real-time interactive grasp synthesis framework that extends SAMv2 for dynamic object grasping, achieving low latency (59ms) with high accuracy (90.6-94.8%) across benchmarks.
...
Method: Extends SAMv2 for video stream grasp estimation by integrating user prompts with spatiotemporal context to ensure temporal consistency for dynamic objects.

Result: Achieves 90.6% accuracy on OCID, 93.8% on Jacquard, 92.0% on GraspNet-1Billion with 73.1ms latency (58.5% reduction vs RoG-SAM), and 94.8% success rate in real-world experiments.

Conclusion: SPGrasp effectively resolves the latency-interactivity trade-off in dynamic grasp synthesis, enabling real-time interaction with end-to-end latency as low as 59ms.
...
### [318] [Task-Oriented Edge-Assisted Cross-System Design for Real-Time Human-Robot Interaction in Industrial Metaverse](https://arxiv.org/abs/2508.20664)
...
Method: Proposes task-oriented edge-assisted cross-system framework with digital twins decoupled into visual display and robotic control functions. Uses HITL-MAML algorithm for dynamic prediction horizon adjustment and motion prediction.
...
### [319] [Task Allocation for Autonomous Machines using Computational Intelligence and Deep Reinforcement Learning](https://arxiv.org/abs/2508.20688)
*Thanh Thi Nguyen,Quoc Viet Hung Nguyen,Jonathan Kua,Imran Razzak,Dung Nguyen,Saeid Nahavandi*

Main category: cs.RO

TL;DR: Survey paper on cooperative control algorithms for autonomous machines, focusing on computational intelligence and deep reinforcement learning methods for task allocation, analyzing their pros/cons and suggesting future research directions.


<details>
  <summary>Details</summary>
Motivation: To develop efficient cooperative control algorithms that enable multiple autonomous machines to perform reliably in complex environments, addressing the need for improved task allocation methods.

Method: Comprehensive survey and analysis of existing algorithms, particularly computational intelligence (CI) and deep reinforcement learning (RL) approaches for task allocation in autonomous machine coordination.

Result: CI and deep RL methods provide viable approaches for complex task allocation in dynamic, uncertain environments. Deep RL has significantly contributed to autonomous machine control literature and represents a growing trend in this research area.

Conclusion: The paper provides researchers and engineers with a comprehensive overview of machine learning progress in autonomous machines, highlights underexplored areas, identifies emerging methodologies, and suggests new avenues for future research to enhance real-world applications.
...
### [320] [CoCoL: A Communication Efficient Decentralized Collaborative Method for Multi-Robot Systems](https://arxiv.org/abs/2508.20898)
*Jiaxi Huang,Yan Huang,Yixian Zhao,Wenchao Meng,Jinming Xu*

Main category: cs.RO

TL;DR: CoCoL is a communication-efficient decentralized collaborative learning method for multi-robot systems that reduces communication overhead and handles data heterogeneity using mirror descent and gradient tracking.


<details>
  <summary>Details</summary>
Motivation: Multi-robot collaborative learning faces challenges with high communication overhead and data heterogeneity, which limit performance and adaptability in complex tasks.

Method: Leverages mirror descent framework with approximate Newton-type updates to capture objective function similarity between robots, uses inexact sub-problem solutions to reduce computational costs, and integrates gradient tracking for robustness against data heterogeneity.

Result: Experimental results show CoCoL significantly reduces both communication rounds and total bandwidth consumption while maintaining state-of-the-art accuracy, especially in challenging scenarios with non-IID data, streaming data, and time-varying network topologies.

Conclusion: CoCoL provides an effective solution for communication-efficient decentralized collaborative learning in multi-robot systems, demonstrating superior performance in reducing communication overhead while handling data heterogeneity challenges.
...
### [321] [ActLoc: Learning to Localize on the Move via Active Viewpoint Selection](https://arxiv.org/abs/2508.20981)
*Jiajie Li,Boyang Sun,Luca Di Giammarino,Hermann Blum,Marc Pollefeys*

Main category: cs.RO

TL;DR: ActLoc is an active viewpoint-aware planning framework that uses a trained attention-based model to predict localization accuracy across camera orientations, enabling robots to select optimal viewpoints for robust localization during navigation tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional localization systems assume all viewing directions are equally informative, but in practice, localization becomes unreliable when robots observe unmapped, ambiguous, or uninformative regions, leading to navigation failures.

Method: Uses a large-scale trained attention-based model that encodes metric maps and camera poses from map construction to predict localization accuracy across yaw and pitch directions at arbitrary 3D locations, then incorporates these predictions into a path planner.

Result: Achieves state-of-the-art results on single-viewpoint selection and generalizes effectively to full-trajectory planning, demonstrating improved localization robustness.

Conclusion: ActLoc's modular design makes it applicable to diverse robot navigation and inspection tasks, providing a framework for active viewpoint selection that maximizes localization accuracy while respecting task constraints.
...
### [322] [Uncertainty Aware-Predictive Control Barrier Functions: Safer Human Robot Interaction through Probabilistic Motion Forecasting](https://arxiv.org/abs/2508.20812)
*Lorenzo Busellato,Federico Cunico,Diego Dall'Alba,Marco Emporio,Andrea Giachetti,Riccardo Muradore,Marco Cristani*

Main category: cs.RO

TL;DR: UA-PCBFs framework combines probabilistic human motion forecasting with Control Barrier Functions to enable safer, more fluid human-robot interactions by dynamically adjusting safety margins based on uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: Current collaborative robotic systems are overly conservative due to worst-case human motion predictions, causing unnecessary braking and stalling task progress, which hampers true fluid human-robot interaction.

Method: Uncertainty-Aware Predictive Control Barrier Functions (UA-PCBFs) fuse probabilistic human hand motion forecasting with formal safety guarantees of Control Barrier Functions, allowing dynamic adjustment of safety margins based on uncertainty estimation.

Result: UA-PCBFs significantly reduce violations of the robot's safe space during interaction compared to state-of-the-art HRI architectures, showing better performance in task-critical metrics through real-world experiments.

Conclusion: The framework enables more fluid and intelligent human-robot interactions by providing robots with a deeper understanding of future human states through informed motion planning with uncertainty-aware safety guarantees.
...
### [323] [Learning Primitive Embodied World Models: Towards Scalable Robotic Learning](https://arxiv.org/abs/2508.20840)
...
<details>
  <summary>Details</summary>
Motivation: Current video-generation-based embodied world models face bottlenecks due to reliance on large-scale embodied interaction data, which is scarce, difficult to collect, and high-dimensional. This limits language-action alignment and hinders long-horizon video generation.

Method: Proposes Primitive Embodied World Models (PEWM) that restrict video generation to fixed short horizons using primitive motions. Combines with a modular Vision-Language Model planner and Start-Goal heatmap Guidance mechanism for flexible closed-loop control and compositional generalization.

Result: PEWM enables fine-grained alignment between linguistic concepts and visual representations of robotic actions, reduces learning complexity, improves data efficiency in embodied data collection, and decreases inference latency.

Conclusion: The framework leverages spatiotemporal vision priors and semantic awareness to bridge fine-grained physical interaction with high-level reasoning, paving the way toward scalable, interpretable, and general-purpose embodied intelligence.
...
### [324] [Prompt-to-Product: Generative Assembly via Bimanual Manipulation](https://arxiv.org/abs/2508.21063)
...
TL;DR: Automated pipeline that generates LEGO assembly designs from natural language prompts and constructs them with a bimanual robotic system
...
Method: Leverages LEGO bricks as assembly platform, generates physically buildable brick designs from user prompts, and uses bimanual robotic system for automated construction
...
Conclusion: Prompt-to-Product successfully automates the process of bringing user imaginations into real-world assembly products
...
### [325] [Operator learning meets inverse problems: A probabilistic perspective](https://arxiv.org/abs/2508.20207)
*Nicholas H. Nelsen,Yunan Yang*

Main category: math.NA

TL;DR: Survey of operator learning methods for solving inverse problems, focusing on end-to-end approaches that map data directly to solutions without requiring forward map knowledge.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive overview of the intersection between operator learning and inverse problems, addressing methodological and theoretical developments in this emerging field.
...
Result: Presents structure-aware architectures for both point predictions and posterior estimates, and surveys relevant theory for linear and nonlinear inverse problems.

Conclusion: Operator learning provides powerful tools for solving inverse problems through direct data-to-solution mapping, with applications in prior and regularizer estimation within classical inversion algorithms.
